...
static bool pretty_print_str(IO_CACHE *cache, const char *str, size_t len, bool identifier)
{
  ...
  while (str < end)
  {
    ...
    switch ((c = *str++))
    {
      case '\n':
        ...
        break;
      case '\r':
        error = my_b_write(cache, (uchar *)"\\r", 2);
        break;
      case '\\':
        error = my_b_write(cache, (uchar *)"\\\\", 2);
        break;
      case '\b':
        error = my_b_write(cache, (uchar *)"\\b", 2);
        break;
      case '\t':
        error = my_b_write(cache, (uchar *)"\\t", 2);
        break;
      case '\'':
        ...
        break;
      case 0:
        error = my_b_write(cache, (uchar *)"\\0", 2);
        break;
      default:
...
*/
static inline bool pretty_print_str(IO_CACHE *cache, const char *str, size_t len)
{
...
/**
  ...
  @param[in] len  length of the string.
 */
static inline bool pretty_print_identifier(IO_CACHE *cache, const char *str, size_t len)
{
...
static bool print_set_option(IO_CACHE *file, uint32 bits_changed, uint32 option, uint32 flags, const char *name,
                             bool *need_comma)
{
...
static bool hexdump_minimal_header_to_io_cache(IO_CACHE *file, my_off_t offset, uchar *ptr)
{
  ...
  */
  ...
  if (my_b_printf(file,
                  ...
                  "|Flags\n"))
    goto err;
  emit_buf_written = my_snprintf(emit_buf, sizeof(emit_buf),
                                 "# %8llx  "                         /* Position */
  ...
  if (my_b_write(file, reinterpret_cast<uchar *>(emit_buf), emit_buf_written) || my_b_write(file, (uchar *)"#\n", 2))
    goto err;
...
{
  memset(emit_buff + 1, ' ', 1 + 8 + 2 + (HEXDUMP_BYTES_PER_LINE * 3 + 1) + 2 + HEXDUMP_BYTES_PER_LINE);
  emit_buff[0] = '#';
  emit_buff[2 + 8 + 2 + (HEXDUMP_BYTES_PER_LINE * 3 + 1) + 1] = '|';
  emit_buff[2 + 8 + 2 + (HEXDUMP_BYTES_PER_LINE * 3 + 1) + 2 + HEXDUMP_BYTES_PER_LINE] = '|';
  emit_buff[2 + 8 + 2 + (HEXDUMP_BYTES_PER_LINE * 3 + 1) + 2 + HEXDUMP_BYTES_PER_LINE + 1] = '\n';
  emit_buff[2 + 8 + 2 + (HEXDUMP_BYTES_PER_LINE * 3 + 1) + 2 + HEXDUMP_BYTES_PER_LINE + 2] = '\0';
}

static bool hexdump_data_to_io_cache(IO_CACHE *file, my_off_t offset, uchar *ptr, my_off_t size)
{
  /*
    2 = '# '
    8 = address
    ...
    1 = '\0'
  */
  char emit_buffer[2 + 8 + 2 + (HEXDUMP_BYTES_PER_LINE * 3 + 1) + 2 + HEXDUMP_BYTES_PER_LINE + 2 + 1];
  char *h, *c;
  my_off_t i;
  ...
  */
  ...
  for (i = 0, c = emit_buffer + 2 + 8 + 2 + (HEXDUMP_BYTES_PER_LINE * 3 + 1) + 2, h = emit_buffer + 2 + 8 + 2; i < size;
       i++, ptr++)
  {
    ...
    *c++ = my_isprint(&my_charset_bin, *ptr) ? *ptr : '.';
    ...
    if ((i % HEXDUMP_BYTES_PER_LINE) == (HEXDUMP_BYTES_PER_LINE - 1))
    {
      ...
      *h = ' ';
      ...
      /* print address */
      size_t const emit_buf_written = my_snprintf(emit_buffer + 2, 9, "%8llx", (ulonglong)starting_offset);
      /* remove \0 left after printing address */
      emit_buffer[2 + emit_buf_written] = ' ';
      if (my_b_write(file, reinterpret_cast<uchar *>(emit_buffer), sizeof(emit_buffer) - 1))
        goto err;
    ...
    else if ((i % (HEXDUMP_BYTES_PER_LINE / 2)) == ((HEXDUMP_BYTES_PER_LINE / 2) - 1))
    {
      /*
  ...
  */
  if (h != emit_buffer + 2 + 8 + 2)
  {
    ...
    *c++ = '\n';
    ...
    emit_buffer[2 + emit_buf_written] = ' ';
    /* pad unprinted area */
    memset(h, ' ', (HEXDUMP_BYTES_PER_LINE * 3 + 1) - (h - (emit_buffer + 2 + 8 + 2)));
    if (my_b_write(file, reinterpret_cast<uchar *>(emit_buffer), c - emit_buffer))
      goto err;
  }
...
static inline bool is_enum_or_set_type(uint type) { return type == MYSQL_TYPE_ENUM || type == MYSQL_TYPE_SET; }

/*
  Log_event::print_header()
*/

bool Log_event::print_header(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info, bool is_more __attribute__((unused)))
{
  char llbuff[22];
  my_off_t hexdump_from = print_event_info->hexdump_from;
  DBUG_ENTER("Log_event::print_header");

  if (my_b_write_byte(file, '#') || print_timestamp(file) ||
      my_b_printf(file, " server id %lu  end_log_pos %s ", (ulong)server_id, llstr(log_pos, llbuff)))
    goto err;
  ...
  if (checksum_alg != BINLOG_CHECKSUM_ALG_OFF && checksum_alg != BINLOG_CHECKSUM_ALG_UNDEF)
  {
    ...
    size_t const bytes_written = my_snprintf(checksum_buf, sizeof(checksum_buf), "0x%08x ", crc);
    if (my_b_printf(file, "%s ", get_type(&binlog_checksum_typelib, checksum_alg)) ||
...
*/
static bool my_b_write_sint32_and_uint32(IO_CACHE *file, int32 si, uint32 ui)
{
...
static size_t log_event_print_value(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info, const uchar *ptr, uint type,
                                    uint meta, char *typestr, size_t typestr_length)
{
  uint32 length = 0;
  ...
  switch (type)
  {
    ...
    case MYSQL_TYPE_DATETIME:
    {
      ...
      my_b_printf(file, "'%04d-%02d-%02d %02d:%02d:%02d'", (int)(d / 10000), (int)(d % 10000) / 100, (int)(d % 100),
    ...
    case MYSQL_TYPE_TIME:
    {
      ...
      my_b_printf(file, "'%s%02d:%02d:%02d'", sign, i32 / 10000, (i32 % 10000) / 100, i32 % 100, i32);
      return 3;
    }
    ...
    case MYSQL_TYPE_DATE:
    {
      ...
      my_b_printf(file, "'%04d:%02d:%02d'", (int)(i32 / (16L * 32L)), (int)(i32 / 32L % 16L), (int)(i32 % 32L));
      return 3;
    }
...
size_t Rows_log_event::print_verbose_one_row(IO_CACHE *file, table_def *td, PRINT_EVENT_INFO *print_event_info,
                                             ...
                                             const my_bool no_fill_output)
{
  ...
  {
    size_t size;
    int is_null = (null_bits[null_bit_index / 8] >> (null_bit_index % 8)) & 0x01;
    ...
    if (!no_fill_output)
    {
      size = log_event_print_value(file, print_event_info, is_null ? NULL : value, td->type(i), td->field_metadata(i),
      ...
      if (need_flashback_review)
      {
        ...
        size = log_event_print_value(&tmp_cache, print_event_info, is_null ? NULL : value, td->type(i),
        ...
        switch (td->type(i))  // Converting a string to HEX format
        {
          ...
          default:
            tmp_str.free();
            if (tmp_str.append(review_str.str, review_str.length) || my_b_printf(review_sql, ", %s", tmp_str.ptr()))
              goto err;
            break;
        }
    ...
    {
      if (my_b_write(file, (uchar *)" /* ", 4) || my_b_printf(file, "%s ", typestr) ||
          my_b_printf(file, "meta=%d nullable=%d is_null=%d ", td->field_metadata(i), td->maybe_null(i), is_null) ||
          my_b_write(file, (uchar *)"*/", 2))
        goto err;
    }
...
void Rows_log_event::change_to_flashback_event(PRINT_EVENT_INFO *print_event_info, uchar *rows_buff,
                                               Log_event_type ev_type)
{
  ...
  if (!(map = print_event_info->m_table_map.get_table(m_table_id)) || !(td = map->create_table_def()))
    return;
  ...
  for (uchar *value = m_rows_buf; value < m_rows_end;)
  {
    ...
    size_t length1 = 0;
    if (!(length1 = print_verbose_one_row(NULL, td, print_event_info, &m_cols, value, (const uchar *)"", TRUE)))
    {
      fprintf(stderr, "\nError row length: %zu\n", length1);
      exit(1);
    }
    ...
    {
      if (!(length2 = print_verbose_one_row(NULL, td, print_event_info, &m_cols, value, (const uchar *)"", TRUE)))
      {
        fprintf(stderr, "\nError row length: %zu\n", length2);
        exit(1);
      }
...
size_t Rows_log_event::calc_row_event_length(table_def *td, PRINT_EVENT_INFO *print_event_info, MY_BITMAP *cols_bitmap,
                                             const uchar *value)
{
  const uchar *value0 = value;
  const uchar *null_bits = value;
  ...
  for (uint i = 0; i < (uint)td->size(); i++)
  {
    ...
    if (!is_null)
    {
      ...
      if (!(size = calc_field_event_length(value, td->type(i), td->field_metadata(i))))
        return 0;
      value += size;
    }
    null_bit_index++;
...
void Rows_log_event::count_row_events(PRINT_EVENT_INFO *print_event_info)
{
  ...
  for (const uchar *value = m_rows_buf; value < m_rows_end;)
  {
    ...
    if (!(length = calc_row_event_length(td, print_event_info, &m_cols, value)))
      break;
    value += length;
    ...
    if (row_events == 2)
    {
      if (!(length = calc_row_event_length(td, print_event_info, &m_cols_ai, value)))
        break;
      value += length;
      DBUG_ASSERT(value <= m_rows_end);
    }
...
bool Rows_log_event::print_verbose(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info)
{
  Table_map_log_event *map;
  table_def *td = 0;
  ...
  if (m_extra_row_data)
  {
    ...
    if (my_b_printf(file, "### Extra row data format: %u, len: %u :", m_extra_row_data[EXTRA_ROW_INFO_FORMAT_OFFSET],
                    extra_payload_len))
      goto err;
    if (extra_payload_len)
    {
      ...
      */
      ...
      char buff[buff_len];
      str_to_hex(buff, (const char *)&m_extra_row_data[EXTRA_ROW_INFO_HDR_BYTES], extra_payload_len);
  ...
  if (!(map = print_event_info->m_table_map.get_table(m_table_id)) || !(td = map->create_table_def()))
  {
    return (my_b_printf(file, "### Row event for unknown table #%lu", (ulong)m_table_id));
  }
  ...
  for (const uchar *value = m_rows_buf; value < m_rows_end;)
  {
...
#ifdef WHEN_FLASHBACK_REVIEW_READY
    if (need_flashback_review)
      if (my_b_printf(review_sql, "\nINSERT INTO `%s`.`%s` VALUES ('%s'", map->get_review_dbname(),
                      map->get_review_tablename(), sql_command_short))
        goto err;
#endif

    /* Print the first image */
    if (!(length = print_verbose_one_row(file, td, print_event_info, &m_cols, value, (const uchar *)sql_clause1)))
      goto err;
    value += length;
    ...
    if (sql_clause2)
    {
      if (!(length = print_verbose_one_row(file, td, print_event_info, &m_cols_ai, value, (const uchar *)sql_clause2)))
        goto err;
      value += length;
    }
#ifdef WHEN_FLASHBACK_REVIEW_READY
...
/**
  ...
  @param do_print_encoded   whether to store base64-encoded event
                            into @file.
*/
bool Log_event::print_base64(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info, bool do_print_encoded)
{
  uchar *ptr = temp_buf;
  ...
  if (is_flashback)
  {
    ...
    if (checksum_alg != BINLOG_CHECKSUM_ALG_UNDEF && checksum_alg != BINLOG_CHECKSUM_ALG_OFF)
      tmp_size -= BINLOG_CHECKSUM_LEN;  // checksum is displayed through the header
    switch (ev_type)
    {
      case WRITE_ROWS_EVENT:
        ptr[EVENT_TYPE_OFFSET] = DELETE_ROWS_EVENT;
        ev = new Delete_rows_log_event(ptr, tmp_size, glob_description_event);
      ...
      case DELETE_ROWS_EVENT:
        ...
        ev = new Write_rows_log_event(ptr, tmp_size, glob_description_event);
        ev->change_to_flashback_event(print_event_info, ptr, ev_type);
        break;
      ...
      case UPDATE_ROWS_EVENT_V1:
        ev = new Update_rows_log_event(ptr, tmp_size, glob_description_event);
        ev->change_to_flashback_event(print_event_info, ptr, ev_type);
        break;
      default:
        break;
...
#ifdef WHEN_FLASHBACK_REVIEW_READY
  if (print_event_info->verbose || print_event_info->print_row_count || need_flashback_review)
#else
  // Flashback need the table_map to parse the event
  if (print_event_info->verbose || print_event_info->print_row_count || is_flashback)
#endif
  {
    Rows_log_event *ev = NULL;
    ...
    switch (et)
    {
      ...
      {
        Table_map_log_event *map;
        map = new Table_map_log_event(ptr, size, glob_description_event);
#ifdef WHEN_FLASHBACK_REVIEW_READY
        if (need_flashback_review)
        {
      ...
      case WRITE_ROWS_EVENT_V1:
      {
        ev = new Write_rows_log_event(ptr, size, glob_description_event);
        break;
      }
      case DELETE_ROWS_EVENT:
      case DELETE_ROWS_EVENT_V1:
      {
        ev = new Delete_rows_log_event(ptr, size, glob_description_event);
        break;
      }
      case UPDATE_ROWS_EVENT:
      case UPDATE_ROWS_EVENT_V1:
      {
        ev = new Update_rows_log_event(ptr, size, glob_description_event);
        break;
      }
      case WRITE_ROWS_COMPRESSED_EVENT:
      case WRITE_ROWS_COMPRESSED_EVENT_V1:
      {
        ev = new Write_rows_compressed_log_event(ptr, size, glob_description_event);
        break;
      }
      case UPDATE_ROWS_COMPRESSED_EVENT:
      case UPDATE_ROWS_COMPRESSED_EVENT_V1:
      {
        ev = new Update_rows_compressed_log_event(ptr, size, glob_description_event);
        break;
      }
      case DELETE_ROWS_COMPRESSED_EVENT:
      case DELETE_ROWS_COMPRESSED_EVENT_V1:
      {
        ev = new Delete_rows_compressed_log_event(ptr, size, glob_description_event);
        break;
      }
      default:
        break;
    ...
    if (ev)
    {
      ...
      else
      {
        ...
        if (open_cached_file(&tmp_cache, NULL, NULL, 0, MYF(MY_WME | MY_NABP)))
        {
          delete ev;
          goto err;
        }
...
bool Log_event::print_timestamp(IO_CACHE *file, time_t *ts)
{
  ...
  DBUG_RETURN(my_b_printf(file, "%02d%02d%02d %2d:%02d:%02d", res->tm_year % 100, res->tm_mon + 1, res->tm_mday,
...
/**
  ...
  @todo
    print the catalog ??
*/
bool Query_log_event::print_query_header(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  if (!print_event_info->short_form)
  {
    if (print_header(file, print_event_info, FALSE) ||
        my_b_printf(file,
                    ...
                    "\txid=%lu\n",
                    get_type_str(), (ulong)thread_id, (ulong)exec_time, error_code, (ulong)xid))
      goto err;
  }
  ...
  *end++ = '\n';
  ...
  if ((!print_event_info->thread_id_printed ||
       ((flags & LOG_EVENT_THREAD_SPECIFIC_F) && thread_id != print_event_info->thread_id)))
  {
  ...
  if (likely(flags2_inited)) /* likely as this will mainly read 5.0 logs */
  {
    ...
    if (unlikely(tmp)) /* some bits have changed */
    {
      ...
      if (my_b_write_string(file, "SET ") ||
          print_set_option(file, tmp, OPTION_NO_FOREIGN_KEY_CHECKS, ~flags2, "@@session.foreign_key_checks",
                           &need_comma) ||
          ...
          print_set_option(file, tmp, OPTION_NOT_AUTOCOMMIT, ~flags2, "@@session.autocommit", &need_comma) ||
          print_set_option(file, tmp, OPTION_NO_CHECK_CONSTRAINT_CHECKS, ~flags2, "@@session.check_constraint_checks",
                           &need_comma) ||
          print_set_option(file, tmp, mask & OPTION_IF_EXISTS, flags2, "@@session.sql_if_exists", &need_comma) ||
          ...
          print_set_option(file, tmp, mask & OPTION_INSERT_HISTORY, flags2,
                           "@@session.system_versioning_insert_history", &need_comma) ||
          my_b_printf(file, "%s\n", print_event_info->delimiter))
        goto err;
  ...
  */
  ...
  if (likely(sql_mode_inited) &&
      (unlikely(print_event_info->sql_mode != sql_mode || !print_event_info->sql_mode_inited)))
  {
    char llbuff[22];
      ...
      print_event_info->auto_increment_offset != auto_increment_offset)
  {
    if (my_b_printf(file, "SET @@session.auto_increment_increment=%lu, @@session.auto_increment_offset=%lu%s\n",
                    auto_increment_increment, auto_increment_offset, print_event_info->delimiter))
      goto err;
  ...
  if (likely(charset_inited) &&
      (unlikely(!print_event_info->charset_inited || memcmp(print_event_info->charset, charset, 6))))
  {
    ...
    if (my_b_printf(file,
                    ...
                    "%s\n",
                    uint2korr(charset), uint2korr(charset + 2), uint2korr(charset + 4), print_event_info->delimiter))
      goto err;
...
{
  if (my_b_printf(cache, "### ") || my_b_write(cache, (uchar *)query, q_len) || my_b_printf(cache, "\n"))
  {
    goto err;
  }
  return 0;
...
bool Query_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
   ...
   */
  DBUG_EXECUTE_IF("simulate_file_write_error", { (&cache)->write_pos = (&cache)->write_end - 500; });
  if (print_query_header(&cache, print_event_info))
    goto err;
  if (!is_flashback)
  {
    if (gtid_flags_extra &
        (Gtid_log_event::FL_START_ALTER_E1 | Gtid_log_event::FL_COMMIT_ALTER_E1 | Gtid_log_event::FL_ROLLBACK_ALTER_E1))
    {
      bool do_print_encoded = print_event_info->base64_output_mode != BASE64_OUTPUT_NEVER &&
                              print_event_info->base64_output_mode != BASE64_OUTPUT_DECODE_ROWS &&
                              !print_event_info->short_form;
      bool comment_mode = do_print_encoded &&
                          gtid_flags_extra & (Gtid_log_event::FL_START_ALTER_E1 | Gtid_log_event::FL_ROLLBACK_ALTER_E1);
    ...
    {
      if (my_b_write(&cache, (uchar *)query, q_len) || my_b_printf(&cache, "\n%s\n", print_event_info->delimiter))
        goto err;
    }
  }
  else  // is_flashback == 1
  {
    ...
    {
      if (my_b_write(&cache, (uchar *)"COMMIT", 6) || my_b_printf(&cache, "\n%s\n", print_event_info->delimiter))
        goto err;
    }
...
bool Start_log_event_v3::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  Write_on_release_cache cache(&print_event_info->head_cache, file, Write_on_release_cache::FLUSH_F);

  if (!print_event_info->short_form)
  {
    if (print_header(&cache, print_event_info, FALSE) ||
        my_b_printf(&cache, "\tStart: binlog v %d, server v %s created ", binlog_version, server_version) ||
  ...
  if (temp_buf && print_event_info->base64_output_mode != BASE64_OUTPUT_NEVER && !print_event_info->short_form)
  {
    /* BINLOG is matched with the delimiter below on the same level */
    bool do_print_encoded = print_event_info->base64_output_mode != BASE64_OUTPUT_DECODE_ROWS;
    if (do_print_encoded)
      my_b_printf(&cache, "BINLOG '\n");
...
bool Start_encryption_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
...
bool Load_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info) { return print(file, print_event_info, 0); }

bool Load_log_event::print(FILE *file_arg, PRINT_EVENT_INFO *print_event_info, bool commented)
{
  ...
  if (db)
  {
    /*
      ...
      update the last_db.
    */
    if ((different_db = memcmp(print_event_info->db, db, db_len + 1)) && !commented)
      memcpy(print_event_info->db, db, db_len + 1);
  }

  if (db && db[0] && different_db)
    if (my_b_printf(&cache, "%suse %`s%s\n", commented ? "# " : "", db, print_event_info->delimiter))
      goto err;
  ...
  if (flags & LOG_EVENT_THREAD_SPECIFIC_F)
    if (my_b_printf(&cache, "%sSET @@session.pseudo_thread_id=%lu%s\n", commented ? "# " : "", (ulong)thread_id,
                    print_event_info->delimiter))
      goto err;
  if (my_b_printf(&cache, "%sLOAD DATA ", commented ? "# " : ""))
    goto err;
  ...
  if (my_b_printf(&cache, "INTO TABLE `%s`", table_name) || my_b_write_string(&cache, " FIELDS TERMINATED BY ") ||
  ...
  if (sql_ex.opt_flags & OPT_ENCLOSED_FLAG)
    if (my_b_write_string(&cache, " OPTIONALLY "))
      goto err;
  if (my_b_write_string(&cache, " ENCLOSED BY ") || pretty_print_str(&cache, sql_ex.enclosed, sql_ex.enclosed_len) ||
      my_b_write_string(&cache, " ESCAPED BY ") || pretty_print_str(&cache, sql_ex.escaped, sql_ex.escaped_len) ||
...
bool Rotate_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  char buf[22];
  Write_on_release_cache cache(&print_event_info->head_cache, file, Write_on_release_cache::FLUSH_F);
  if (print_header(&cache, print_event_info, FALSE) || my_b_write_string(&cache, "\tRotate to "))
    goto err;
  if (new_log_ident)
...
bool Binlog_checkpoint_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return 0;
  ...
  if (print_header(&cache, print_event_info, FALSE) || my_b_write_string(&cache, "\tBinlog checkpoint ") ||
      my_b_write(&cache, (uchar *)binlog_file_name, binlog_file_len) || my_b_write_byte(&cache, '\n'))
    return 1;
  return cache.flush_data();
}

bool Gtid_list_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return 0;
  ...
  if (print_header(&cache, print_event_info, FALSE) || my_b_printf(&cache, "\tGtid list ["))
    goto err;

  for (i = 0; i < count; ++i)
  {
    longlong10_to_str(list[i].seq_no, buf, 10);
    if (my_b_printf(&cache, "%u-%u-%s", list[i].domain_id, list[i].server_id, buf))
      goto err;
    if (i < count - 1)
      if (my_b_printf(&cache, ",\n# "))
        goto err;
...
{
  Write_on_release_cache cache(&print_event_info->head_cache, file, Write_on_release_cache::FLUSH_F, this);
  ...
  if (!print_event_info->short_form)
  {
    ...
    if (print_header(&cache, print_event_info, FALSE) || my_b_printf(&cache, "\tXid = %s\n", buf))
      goto err;
  }
...
bool User_var_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  if (my_b_write_string(&cache, "SET @") || my_b_write_backtick_quote(&cache, name, name_len))
    goto err;
  ...
  else
  {
    switch (type)
    {
      ...
      case INT_RESULT:
        char int_buf[22];
        longlong10_to_str(uint8korr(val), int_buf, ((flags & User_var_log_event::UNSIGNED_F) ? 10 : -10));
...
bool Create_file_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info, bool enable_local)
{
  if (print_event_info->short_form)
  {
  ...
  if (enable_local)
  {
    if (Load_log_event::print(file, print_event_info, !check_fname_outside_temp_buf()))
      goto err;
...
bool Create_file_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
...
bool Append_block_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return 0;
...
bool Delete_file_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return 0;
...
bool Execute_load_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return 0;
...
bool Execute_load_query_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  return print(file, print_event_info, 0);
}
...
*/
bool Execute_load_query_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info, const char *local_fname)
{
  Write_on_release_cache cache(&print_event_info->head_cache, file);
  ...
  if (local_fname)
  {
    if (my_b_write(&cache, (uchar *)query, fn_pos_start) || my_b_write_string(&cache, " LOCAL INFILE ") ||
    ...
    if (my_b_write_string(&cache, " INTO") || my_b_write(&cache, (uchar *)query + fn_pos_end, q_len - fn_pos_end) ||
        my_b_printf(&cache, "\n%s\n", print_event_info->delimiter))
      goto err;
  }
  else
...
*/
bool copy_cache_to_file_wrapped(IO_CACHE *body, FILE *file, bool do_wrap, const char *delimiter,
                                bool is_verbose /*TODO: remove */)
{
  ...
  else if (4 + sizeof(str_binlog) + cache_size + sizeof(fmt_delim) > opt_binlog_rows_event_max_encoded_size)
  {
    /*
...
/**
  ...
  The function signals on any error through setting @c body->error to -1.
*/
bool copy_cache_to_string_wrapped(IO_CACHE *cache, LEX_STRING *to, bool do_wrap, const char *delimiter, bool is_verbose)
{
  const my_off_t cache_size = my_b_tell(cache);
  // contribution to total size estimate of formating
  const size_t fmt_size = sizeof(str_binlog) + 2 * (sizeof(fmt_frag) + 2 /* %d */) + sizeof(fmt_delim) +
                          sizeof(fmt_n_delim) + sizeof(fmt_binlog2) + 3 * PRINT_EVENT_INFO::max_delimiter_size;

  if (reinit_io_cache(cache, READ_CACHE, 0L, FALSE, FALSE))
    goto err;

  if (!(to->str = (char *)my_malloc(PSI_NOT_INSTRUMENTED, (size_t)cache->end_of_file + fmt_size, MYF(0))))
  {
    perror(
  ...
  if (!do_wrap)
  {
    if (my_b_read(cache, (uchar *)to->str, (to->length = (size_t)cache->end_of_file)))
      goto err;
  }
  else if (4 + sizeof(str_binlog) + cache_size + sizeof(fmt_delim) > opt_binlog_rows_event_max_encoded_size)
  {
...
*/
bool Rows_log_event::print_helper(FILE *file, PRINT_EVENT_INFO *print_event_info, char const *const name)
{
  ...
  if (last_stmt_event)
  {
    if (!is_flashback)
    {
      if (copy_event_cache_to_file_and_reinit(head, file) ||
          copy_cache_to_file_wrapped(body, file, do_print_encoded, print_event_info->delimiter,
    ...
    else
    {
      ...
      if (copy_cache_to_string_wrapped(body, &tmp_str, do_print_encoded, print_event_info->delimiter,
                                       print_event_info->verbose))
        return 1;
...
bool Annotate_rows_log_event::print(FILE *file, PRINT_EVENT_INFO *pinfo)
{
  ...
  if (!pinfo->short_form)
  {
    if (print_header(&pinfo->head_cache, pinfo, TRUE) || my_b_printf(&pinfo->head_cache, "\tAnnotate_rows:\n"))
      goto err;
  }
  ...
  for (pbeg = m_query_txt;; pbeg = pend)
  {
    ...
    for (pend = pbeg + 1; ++cnt <= m_query_len && *pend != '\r' && *pend != '\n'; pend++)
      ;
...
int Table_map_log_event::rewrite_db(const char *new_db, size_t new_len, const Format_description_log_event *desc)
{
  DBUG_ENTER("Table_map_log_event::rewrite_db");
  DBUG_ASSERT(temp_buf);

  uint header_len = MY_MIN(desc->common_header_len, LOG_EVENT_MINIMAL_HEADER_LEN) + TABLE_MAP_HEADER_LEN;
  int len_diff;
  ...
  uchar *new_temp_buf = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, event_new_len, MYF(MY_WME));
  ...
  *ptr++ = (char)new_len;
  ...
  m_memory = (uchar *)my_multi_malloc(PSI_NOT_INSTRUMENTED, MYF(MY_WME), &m_dbnam, (uint)m_dblen + 1, &m_tblnam,
                                      (uint)m_tbllen + 1, &m_coltype, (uint)m_colcnt, NullS);
...
bool Table_map_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (!print_event_info->short_form)
  {
    ...
    if (my_b_printf(&print_event_info->head_cache, "\tTable_map: %`s.%`s mapped to number %s%s\n", m_dbnam, m_tblnam,
                    ullstr(m_table_id, llbuff), ((m_flags & TM_BIT_HAS_TRIGGERS_F) ? " (has triggers)" : "")))
      goto err;
  }
  if (!print_event_info->short_form || print_event_info->print_row_count)
  {
    ...
    {
      Optional_metadata_fields fields(m_optional_metadata, m_optional_metadata_len);
    ...
    if (print_base64(&print_event_info->body_cache, print_event_info, do_print_encoded) ||
...
{
 public:
  typedef Table_map_log_event::Optional_metadata_fields::Default_charset Default_charset;
  ...
  */
  static std::unique_ptr<Charset_iterator> create_charset_iterator(const Default_charset &default_charset,
...
class Table_map_log_event::Default_charset_iterator : public Charset_iterator
{
 public:
  Default_charset_iterator(const Default_charset &default_charset)
      : m_iterator(default_charset.charset_pairs.begin()),
        ...
        m_column_index(0),
        m_default_charset_info(get_charset(default_charset.default_charset, 0))
  {
  }
 ...
 private:
  std::vector<Optional_metadata_fields::uint_pair>::const_iterator m_iterator, m_end;
  uint m_column_index;
...
std::unique_ptr<Table_map_log_event::Charset_iterator> Table_map_log_event::Charset_iterator::create_charset_iterator(
    const Default_charset &default_charset, const std::vector<uint> &column_charset)
{
  if (!default_charset.empty())
    return std::unique_ptr<Charset_iterator>(new Default_charset_iterator(default_charset));
  else
    return std::unique_ptr<Charset_iterator>(new Column_charset_iterator(column_charset));
}
/**
 ...
 */
static void get_type_name(uint type, unsigned char **meta_ptr, const CHARSET_INFO *cs, char *typestr,
                          uint typestr_length, unsigned int geometry_type)
{
  switch (type)
  {
    ...
    case MYSQL_TYPE_LONGLONG:
      ...
      break;
    case MYSQL_TYPE_NEWDECIMAL:
      my_snprintf(typestr, typestr_length, "DECIMAL(%d,%d)", (*meta_ptr)[0], (*meta_ptr)[1]);
      (*meta_ptr) += 2;
      break;
    case MYSQL_TYPE_FLOAT:
    ...
    case MYSQL_TYPE_DOUBLE:
      ...
      break;
    case MYSQL_TYPE_BIT:
      my_snprintf(typestr, typestr_length, "BIT(%d)", (((*meta_ptr)[0])) + (*meta_ptr)[1] * 8);
      (*meta_ptr) += 2;
      break;
    case MYSQL_TYPE_TIMESTAMP2:
      if (**meta_ptr != 0)
    ...
    case MYSQL_TYPE_VAR_STRING:
      if (cs && cs->number != my_charset_bin.number)
        my_snprintf(typestr, typestr_length, "VARCHAR(%d)", uint2korr(*meta_ptr) / cs->mbmaxlen);
      else
        my_snprintf(typestr, typestr_length, "VARBINARY(%d)", uint2korr(*meta_ptr));
    ...
    case MYSQL_TYPE_GEOMETRY:
    {
      const char *names[8] = {"GEOMETRY",   "POINT",           "LINESTRING",   "POLYGON",
                              "MULTIPOINT", "MULTILINESTRING", "MULTIPOLYGON", "GEOMETRYCOLLECTION"};
      if (geometry_type < 8)
        my_snprintf(typestr, typestr_length, names[geometry_type]);
      else
        my_snprintf(typestr, typestr_length, "INVALID_GEOMETRY_TYPE(%u)", geometry_type);
      (*meta_ptr)++;
    }
    break;
    default:
...
void Table_map_log_event::print_columns(IO_CACHE *file, const Optional_metadata_fields &fields)
{
  ...
  std::unique_ptr<Charset_iterator> charset_it =
      Charset_iterator::create_charset_iterator(fields.m_default_charset, fields.m_column_charset);
  std::unique_ptr<Charset_iterator> enum_and_set_charset_it = Charset_iterator::create_charset_iterator(
      fields.m_enum_and_set_default_charset, fields.m_enum_and_set_column_charset);
  std::vector<std::string>::const_iterator col_names_it = fields.m_column_name.begin();
  std::vector<Optional_metadata_fields::str_vector>::const_iterator set_str_values_it = fields.m_set_str_value.begin();
  std::vector<Optional_metadata_fields::str_vector>::const_iterator enum_str_values_it =
      fields.m_enum_str_value.begin();
  std::vector<unsigned int>::const_iterator geometry_type_it = fields.m_geometry_type.begin();
  ...
  for (unsigned long i = 0; i < m_colcnt; i++)
  {
    ...
    if (real_type == MYSQL_TYPE_STRING &&
        (*field_metadata_ptr == MYSQL_TYPE_ENUM || *field_metadata_ptr == MYSQL_TYPE_SET))
    ...
    {
      geometry_type = (geometry_type_it != fields.m_geometry_type.end()) ? *geometry_type_it++ : 0;
    }

    // print column type
    const uint TYPE_NAME_LEN = 100;
    char type_name[TYPE_NAME_LEN];
    get_type_name(real_type, &field_metadata_ptr, cs, type_name, TYPE_NAME_LEN, geometry_type);
    ...
    if (is_numeric_type(real_type) && signedness_it != fields.m_signedness.end())
    {
    ...
    if (real_type == MYSQL_TYPE_ENUM && enum_str_values_it != fields.m_enum_str_value.end())
    {
      str_values = &(*enum_str_values_it);
      enum_str_values_it++;
    }
    else if (real_type == MYSQL_TYPE_SET && set_str_values_it != fields.m_set_str_value.end())
    {
    ...
    if (str_values != NULL)
    {
      ...
      for (Optional_metadata_fields::str_vector::const_iterator it = str_values->begin(); it != str_values->end(); it++)
      {
    ...
    if (cs != NULL && (is_enum_or_set_type(real_type) || cs->number != my_charset_bin.number))
      my_b_printf(file, " CHARSET %s COLLATE %s", cs->cs_name.str, cs->coll_name.str);
    if (i != m_colcnt - 1)
      my_b_printf(file, ",\n#         ");
  }
  my_b_printf(file, ")");
...
void Table_map_log_event::print_primary_key(IO_CACHE *file, const Optional_metadata_fields &fields)
{
  if (!fields.m_primary_key.empty())
  {
    my_b_printf(file, "# Primary Key(");

    std::vector<Optional_metadata_fields::uint_pair>::const_iterator it = fields.m_primary_key.begin();
...
{
  DBUG_EXECUTE_IF("simulate_cache_read_error", { DBUG_SET("+d,simulate_my_b_fill_error"); });
  return Rows_log_event::print_helper(file, print_event_info, is_flashback ? "Delete_rows" : "Write_rows");
}

bool Write_rows_compressed_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  uchar *new_buf;
  ulong len;
  bool is_malloc = false;
  if (!row_log_event_uncompress(glob_description_event, checksum_alg == BINLOG_CHECKSUM_ALG_CRC32, temp_buf, UINT_MAX32,
                                NULL, 0, &is_malloc, &new_buf, &len))
  {
    free_temp_buf();
    register_temp_buf(new_buf, true);
    if (Rows_log_event::print_helper(file, print_event_info, "Write_compressed_rows"))
      goto err;
  }
  else
  {
...
bool Delete_rows_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  return Rows_log_event::print_helper(file, print_event_info, is_flashback ? "Write_rows" : "Delete_rows");
}

bool Delete_rows_compressed_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  uchar *new_buf;
  ulong len;
  bool is_malloc = false;
                                ...
                                NULL, 0, &is_malloc, &new_buf, &len))
  {
    free_temp_buf();
    register_temp_buf(new_buf, true);
    if (Rows_log_event::print_helper(file, print_event_info, "Delete_compressed_rows"))
      goto err;
  }
  else
  {
...
bool Update_rows_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  return Rows_log_event::print_helper(file, print_event_info, "Update_rows");
}

bool Update_rows_compressed_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  uchar *new_buf;
  ulong len;
  bool is_malloc = false;
                                ...
                                NULL, 0, &is_malloc, &new_buf, &len))
  {
    free_temp_buf();
    register_temp_buf(new_buf, true);
    if (Rows_log_event::print_helper(file, print_event_info, "Update_compressed_rows"))
      goto err;
  }
  else
  {
...
bool Incident_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return 0;
...
bool Ignorable_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return 0;
...
bool Gtid_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  Write_on_release_cache cache(&print_event_info->head_cache, file, Write_on_release_cache::FLUSH_F, this);
  char buf[21];
  char buf2[21];
  ...
  if (!print_event_info->short_form && !is_flashback)
  {
    ...
    if (!print_event_info->allow_parallel_printed || print_event_info->allow_parallel != !!(flags2 & FL_ALLOW_PARALLEL))
    {
      if (my_b_printf(&cache, "/*!100101 SET @@session.skip_parallel_replication=%u*/%s\n",
    ...
    if (!print_event_info->domain_id_printed || print_event_info->domain_id != domain_id)
    {
      if (my_b_printf(&cache, "/*!100001 SET @@session.gtid_domain_id=%u*/%s\n", domain_id,
                      print_event_info->delimiter))
        goto err;
    ...
    if (!print_event_info->server_id_printed || print_event_info->server_id != server_id)
    {
