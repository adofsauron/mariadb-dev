...
PSI_mutex_key key_LOCK_wsrep_replaying, key_LOCK_wsrep_ready, key_LOCK_wsrep_sst, key_LOCK_wsrep_sst_thread,
    key_LOCK_wsrep_sst_init, key_LOCK_wsrep_slave_threads, key_LOCK_wsrep_gtid_wait_upto, key_LOCK_wsrep_desync,
    key_LOCK_wsrep_config_state, key_LOCK_wsrep_cluster_config, key_LOCK_wsrep_group_commit, key_LOCK_wsrep_SR_pool,
    key_LOCK_wsrep_SR_store, key_LOCK_wsrep_thd_queue, key_LOCK_wsrep_joiner_monitor, key_LOCK_wsrep_donor_monitor;

PSI_cond_key key_COND_wsrep_thd, key_COND_wsrep_replaying, key_COND_wsrep_ready, key_COND_wsrep_sst,
    key_COND_wsrep_sst_init, key_COND_wsrep_sst_thread, key_COND_wsrep_thd_queue, key_COND_wsrep_slave_threads,
    key_COND_wsrep_gtid_wait_upto, key_COND_wsrep_joiner_monitor, key_COND_wsrep_donor_monitor;

PSI_file_key key_file_wsrep_gra_log;

static PSI_mutex_info wsrep_mutexes[] = {{&key_LOCK_wsrep_ready, "LOCK_wsrep_ready", PSI_FLAG_GLOBAL},
...
static PSI_file_info wsrep_files[] = {{&key_file_wsrep_gra_log, "wsrep_gra_log", 0}};

PSI_thread_key key_wsrep_sst_joiner, key_wsrep_sst_donor, key_wsrep_rollbacker, key_wsrep_applier,
    key_wsrep_sst_joiner_monitor, key_wsrep_sst_donor_monitor;

static PSI_thread_info wsrep_threads[] = {{&key_wsrep_sst_joiner, "wsrep_sst_joiner_thread", PSI_FLAG_GLOBAL},
...
void wsrep_init_gtid()
{
  ...
  if (stored_gtid.server_id == 0)
  {
    ...
    if (mysql_bin_log.is_open() && mysql_bin_log.lookup_domain_in_binlog_state(stored_gtid.domain_id, &wsrep_last_gtid))
    {
...
bool wsrep_get_binlog_gtid_seqno(wsrep_server_gtid_t &gtid)
{
  ...
  int ret = 0;
  if (mysql_bin_log.is_open() && mysql_bin_log.find_in_binlog_state(gtid.domain_id, gtid.server_id, &binlog_gtid))
  {
...
bool wsrep_check_gtid_seqno(const uint32 &domain, const uint32 &server, uint64 &seqno)
{
  if (domain == wsrep_gtid_server.domain_id && server == wsrep_gtid_server.server_id)
  {
    if (wsrep_gtid_server.seqno_committed() < seqno)
      return 1;
    return 0;
  }
  return 0;
...
void wsrep_recover_sr_from_storage(THD *orig_thd)
{
  switch (wsrep_SR_store_type)
  {
    case WSREP_SR_STORE_TABLE:
      if (!wsrep_schema)
      {
        WSREP_ERROR(
            ...
            "streaming transactions: wsrep_on %d",
            WSREP_ON);
    ...
    default:
      /* */
      WSREP_ERROR("Unsupported wsrep SR store type: %lu wsrep_on: %d", wsrep_SR_store_type, WSREP_ON);
      trans_commit(orig_thd);
      break;
  }
}
...
void wsrep_verify_SE_checkpoint(const wsrep_uuid_t &uuid, wsrep_seqno_t const seqno) {}
...
{
  if (mysql_mutex_lock(&LOCK_wsrep_ready))
    abort();
...
static std::string wsrep_server_incoming_address()
{
  ...
  */
  if ((!wsrep_node_incoming_address || !strcmp(wsrep_node_incoming_address, WSREP_NODE_INCOMING_AUTO)))
  {
    bool is_ipv6 = false;
    unsigned int my_bind_ip = INADDR_ANY;  // default if not set

    if (my_bind_addr_str && strlen(my_bind_addr_str) && strcmp(my_bind_addr_str, "*") != 0)
    {
    ...
    else /* mysqld binds to 0.0.0.0, try taking IP from wsrep_node_address. */
    {
      if (node_addr.size())
      {
        ...
        if (ip_len_mdb + 7 /* :55555\0 */ < inc_addr_max)
        {
          ...
          snprintf(inc_addr + ip_len_mdb, inc_addr_max - ip_len_mdb, ":%u", (int)mysqld_port);
        }
        else
        {
          WSREP_WARN(
  ...
  else
  {
    ...
    if (!addr.is_valid())
    {
      WSREP_WARN("Could not parse wsrep_node_incoming_address : %s", wsrep_node_incoming_address);
      goto done;
    }
...
static void wsrep_init_provider_status_variables()
{
  ...
  strncpy(provider_version, provider.version().c_str(), sizeof(provider_version) - 1);
  strncpy(provider_vendor, provider.vendor().c_str(), sizeof(provider_vendor) - 1);
}
...
int wsrep_init_server()
{
  ...
  try
  {
    ...
    Wsrep_server_state::init_once(server_name, incoming_address, node_address, working_dir, initial_position,
...
int wsrep_init()
{
  ...
  if (strlen(wsrep_provider) == 0 || !strcmp(wsrep_provider, WSREP_NONE))
  {
    ...
    int err = Wsrep_server_state::instance().load_provider(wsrep_provider,
  ...
  if (!wsrep_provider_is_SR_capable() && global_system_variables.wsrep_trx_fragment_size > 0)
  {
    WSREP_ERROR(
  ...
  wsrep_init_provider_status_variables();
  wsrep_capabilities_export(Wsrep_server_state::instance().provider().capabilities(), &wsrep_provider_capabilities);

  WSREP_DEBUG("SR storage init for: %s", (wsrep_SR_store_type == WSREP_SR_STORE_TABLE) ? "table" : "void");
...
void wsrep_thr_init()
{
  ...
  mysql_mutex_init(key_LOCK_wsrep_SR_pool, &LOCK_wsrep_SR_pool, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_LOCK_wsrep_SR_store, &LOCK_wsrep_SR_store, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_LOCK_wsrep_joiner_monitor, &LOCK_wsrep_joiner_monitor, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_LOCK_wsrep_donor_monitor, &LOCK_wsrep_donor_monitor, MY_MUTEX_INIT_FAST);
...
void wsrep_recover()
{
  ...
  if (wsrep_uuid_compare(&local_uuid, &WSREP_UUID_UNDEFINED) == 0 && local_seqno == -2)
  {
    wsrep_uuid_print(&local_uuid, uuid_str, sizeof(uuid_str));
    WSREP_INFO("Position %s:%lld given at startup, skipping position recovery", uuid_str, (long long)local_seqno);
    return;
  }
  ...
  if (wsrep_gtid_mode)
  {
    ...
    WSREP_INFO("Recovered position: %s,%d-%d-%llu", oss.str().c_str(), server_gtid.domain_id, server_gtid.server_id,
               server_gtid.seqno);
  }
  else
  {
...
void wsrep_stop_replication(THD *thd)
{
  ...
  if (Wsrep_server_state::instance().state() != Wsrep_server_state::s_disconnected)
  {
...
bool wsrep_start_replication(const char *wsrep_cluster_address)
{
  ...
  */
  ...
  if (!wsrep_new_cluster && (strlen(wsrep_cluster_address) == 8) && !strncmp(wsrep_cluster_address, "gcomm://", 8))
  {
  ...
  if ((rcode = Wsrep_server_state::instance().connect(wsrep_cluster_name, wsrep_cluster_address, wsrep_sst_donor,
                                                      bootstrap)))
  {
    DBUG_PRINT("wsrep", ("wsrep_ptr->connect(%s) failed: %d", wsrep_cluster_address, rcode));
    WSREP_ERROR("wsrep::connect(%s) failed: %d", wsrep_cluster_address, rcode);
    return false;
  }
  else
  {
...
bool wsrep_check_mode(enum_wsrep_mode mask) { return wsrep_mode & mask; }
...
static const char *wsrep_warning_name(const enum wsrep_warning_type type)
{
  switch (type)
  {
    case WSREP_REQUIRE_PRIMARY_KEY:
      return "WSREP_REQUIRE_PRIMARY_KEY";
      break;
    case WSREP_REQUIRE_INNODB:
      return "WSREP_REQUIRE_INNODB";
      break;
    default:
      assert(0);
      return " ";
...
static bool wsrep_protect_against_warning_flood(enum wsrep_warning_type warning_type)
{
  ulonglong count;
      ...
      */
      if (diff_time <= WSREP_WARNING_ACTIVATION_TIMEOUT)
      {
        ...
        WSREP_INFO("Suppressing warnings of type '%s' for up to %d seconds because of flooding",
                   wsrep_warning_name(warning_type), WSREP_WARNING_ACTIVATION_TIMEOUT);
      }
      else
      {
        /*
...
*/
static void wsrep_push_warning(THD *thd, enum wsrep_warning_type type, const handlerton *hton, const TABLE_LIST *tables)
{
  switch (type)
  {
    case WSREP_REQUIRE_PRIMARY_KEY:
      ...
      if (global_system_variables.log_warnings > 1 && !wsrep_protect_against_warning_flood(type))
        WSREP_WARN(
            "wsrep_mode = REQUIRED_PRIMARY_KEY enabled. "
            "Table '%s'.'%s' should have PRIMARY KEY defined",
            tables->db.str, tables->table_name.str);
      break;
    case WSREP_REQUIRE_INNODB:
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_OPTION_PREVENTS_STATEMENT,
                          ...
                          ha_resolve_storage_engine_name(hton), tables->db.str, tables->table_name.str);
      if (global_system_variables.log_warnings > 1 && !wsrep_protect_against_warning_flood(type))
        WSREP_WARN(
            "wsrep_mode = STRICT_REPLICATION enabled. "
            "Storage engine %s for table '%s'.'%s' is "
            "not supported in Galera",
            ha_resolve_storage_engine_name(hton), tables->db.str, tables->table_name.str);
      break;
...
bool wsrep_check_mode_after_open_table(THD *thd, const handlerton *hton, TABLE_LIST *tables)
{
  ...
  if (replicate)
  {
    ...
    {
      if (tbl->s->primary_key == MAX_KEY && wsrep_check_mode(WSREP_MODE_REQUIRED_PRIMARY_KEY))
      {
  ...
  else if (db_type != DB_TYPE_UNKNOWN && db_type != DB_TYPE_PERFORMANCE_SCHEMA)
  {
    bool is_system_db =
        (tbl && ((strcmp(tbl->s->db.str, "mysql") == 0) || (strcmp(tbl->s->db.str, "information_schema") == 0)));

    if (!is_system_db && !is_temporary_table(tables))
    {
      if (db_type != DB_TYPE_INNODB && wsrep_check_mode(WSREP_MODE_STRICT_REPLICATION))
      {
        /* Table is not an InnoDB table and strict replication is requested*/
        wsrep_push_warning(thd, WSREP_REQUIRE_INNODB, hton, tables);
      }

      if (tbl->s->primary_key == MAX_KEY && db_type == DB_TYPE_INNODB &&
      ...
      if (db_type != DB_TYPE_INNODB && thd->variables.sql_log_bin == 1 &&
...
{
  bool ret = true;
  if (wsrep_check_mode(WSREP_MODE_BINLOG_ROW_FORMAT_ONLY) && !thd->is_current_stmt_binlog_format_row() &&
  ...
  if (wsrep_check_mode(WSREP_MODE_REQUIRED_PRIMARY_KEY) && thd->lex->sql_command == SQLCOM_CREATE_TABLE)
  {
    Key *key;
...
bool wsrep_must_sync_wait(THD *thd, uint mask)
{
  ...
  if (thd->variables.wsrep_on)
  {
    ...
    ret = (thd->variables.wsrep_sync_wait & mask) && thd->wsrep_client_thread && WSREP_ON &&
          !(thd->variables.wsrep_dirty_reads && !is_update_query(thd->lex->sql_command)) &&
          !thd->in_active_multi_stmt_transaction() && thd->wsrep_trx().state() != wsrep::transaction::s_replaying &&
...
enum wsrep::provider::status wsrep_sync_wait_upto(THD *thd, wsrep_gtid_t *upto, int timeout)
{
  DBUG_ASSERT(upto);
  enum wsrep::provider::status ret;
  if (upto)
  {
    wsrep::gtid upto_gtid(wsrep::id(upto->uuid.data, sizeof(upto->uuid.data)), wsrep::seqno(upto->seqno));
...
static enum enum_wsrep_sync_wait wsrep_sync_wait_mask_for_command(enum enum_sql_command command)
{
  switch (command)
  {
    case SQLCOM_SELECT:
...
*/
bool wsrep_append_fk_parent_table(THD *thd, TABLE_LIST *tables, wsrep::key_array *keys)
{
  bool fail = false;
  TABLE_LIST *table;
  ...
  for (table = tables; table; table = table->next_local)
  {
    ...
    {
      WSREP_DEBUG("Temporary table %s.%s already opened query=%s", table->db.str, table->table_name.str,
                  wsrep_thd_query(thd));
      return false;
    }
  }
  ...
  for (table = tables; table; table = table->next_local)
  {
    if (!is_temporary_table(table) && table->table)
    {
      ...
      while ((f_key_info = it++))
      {
        ...
        keys->push_back(wsrep_prepare_key_for_toi(f_key_info->referenced_db->str, f_key_info->referenced_table->str,
                                                  wsrep::key::shared));
      }
    }
  }
...
static bool wsrep_prepare_key_for_isolation(const char *db, const char *table, wsrep_buf_t *key, size_t *key_len)
{
  if (*key_len < 2)
    return false;
...
static bool wsrep_prepare_key_for_isolation(const char *db, const char *table, wsrep_key_arr_t *ka)
{
  wsrep_key_t *tmp;
  tmp = (wsrep_key_t *)my_realloc(PSI_INSTRUMENT_ME, ka->keys, (ka->keys_len + 1) * sizeof(wsrep_key_t),
  ...
  ka->keys = tmp;
  if (!(ka->keys[ka->keys_len].key_parts =
            (wsrep_buf_t *)my_malloc(PSI_INSTRUMENT_ME, sizeof(wsrep_buf_t) * 2, MYF(0))))
  {
  ...
  ++ka->keys_len;
  if (!wsrep_prepare_key_for_isolation(db, table, (wsrep_buf_t *)ka->keys[ka->keys_len - 1].key_parts,
...
static bool wsrep_prepare_keys_for_alter_add_fk(const char *child_table_db, const Alter_info *alter_info,
                                                wsrep_key_arr_t *ka)
{
  Key *key;
...
static bool wsrep_prepare_keys_for_isolation(THD *thd, const char *db, const char *table, const TABLE_LIST *table_list,
                                             Alter_info *alter_info, wsrep_key_arr_t *ka)
{
  ka->keys = 0;
  ka->keys_len = 0;
...
bool wsrep_prepare_keys_for_isolation(THD *thd, const char *db, const char *table, const TABLE_LIST *table_list,
                                      wsrep_key_arr_t *ka)
{
  return wsrep_prepare_keys_for_isolation(thd, db, table, table_list, NULL, ka);
}

bool wsrep_prepare_key(const uchar *cache_key, size_t cache_key_len, const uchar *row_id, size_t row_id_len,
...
bool wsrep_prepare_key_for_innodb(THD *thd, const uchar *cache_key, size_t cache_key_len, const uchar *row_id,
                                  size_t row_id_len, wsrep_buf_t *key, size_t *key_len)
{
...
wsrep::key wsrep_prepare_key_for_toi(const char *db, const char *table, enum wsrep::key::type type)
{
  ...
  if (table)
    ret.append_key_part(table, strlen(table));
  return ret;
}

wsrep::key_array wsrep_prepare_keys_for_alter_add_fk(const char *child_table_db, const Alter_info *alter_info)

{
  ...
  while ((key = key_iterator++))
  {
    if (key->type == Key::FOREIGN_KEY)
    {
      ...
      ret.push_back(wsrep_prepare_key_for_toi(db_name, table_name, wsrep::key::exclusive));
    }
  }
  return ret;
}

wsrep::key_array wsrep_prepare_keys_for_toi(const char *db, const char *table, const TABLE_LIST *table_list,
                                            const Alter_info *alter_info, const wsrep::key_array *fk_tables)
{
  wsrep::key_array ret;
  if (db || table)
  {
  ...
  for (const TABLE_LIST *table = table_list; table; table = table->next_global)
  {
    ret.push_back(wsrep_prepare_key_for_toi(table->db.str, table->table_name.str, wsrep::key::exclusive));
  }
  if (alter_info)
  {
 ...
 */
int wsrep_to_buf_helper(THD *thd, const char *query, uint query_len, uchar **buf, size_t *buf_len)
{
  IO_CACHE tmp_io_cache;
  Log_event_writer writer(&tmp_io_cache, 0);
  if (open_cached_file(&tmp_io_cache, mysql_tmpdir, TEMP_PREFIX, 65536, MYF(MY_WME)))
    return 1;
  int ret(0);
  enum enum_binlog_checksum_alg current_binlog_check_alg = (enum_binlog_checksum_alg)binlog_checksum_options;
   ...
   */
  if (thd->slave_thread || wsrep_thd_is_applying(thd) || thd->variables.wsrep_gtid_seq_no)
  {
    ...
    Gtid_log_event gtid_event(thd, seqno, domain_id, true, LOG_EVENT_SUPPRESS_USE_F, true, 0);
    gtid_event.server_id = server_id;
    if (!gtid_event.is_valid())
      ret = 0;
  ...
  {
    Query_log_event ev(thd, thd->wsrep_TOI_pre_query, thd->wsrep_TOI_pre_query_len, FALSE, FALSE, FALSE, 0);
...
static int wsrep_alter_query_string(THD *thd, String *buf)
{
  ...
  if (buf->append(thd->lex->stmt_definition_begin, thd->lex->stmt_definition_end - thd->lex->stmt_definition_begin))
    return 1;
...
static int wsrep_alter_event_query(THD *thd, uchar **buf, size_t *buf_len)
{
  ...
  {
    WSREP_WARN("events alter string failed: schema: %s, query: %s", thd->get_db(), thd->query());
    return 1;
  }
...
static int create_view_query(THD *thd, uchar **buf, size_t *buf_len)
{
  ...
  const LEX_CSTRING command[3] = {
      {STRING_WITH_LEN("CREATE ")}, {STRING_WITH_LEN("ALTER ")}, {STRING_WITH_LEN("CREATE OR REPLACE ")}};
  ...
  if (views->db.str && views->db.str[0] && (thd->db.str == NULL || cmp(&views->db, &thd->db)))
  {
  ...
  buff.append(thd->lex->create_view->select.str, thd->lex->create_view->select.length);
...
static int wsrep_drop_table_query(THD *thd, uchar **buf, size_t *buf_len)
{
  ...
  {
    return wsrep_to_buf_helper(thd, thd->query(), thd->query_length(), buf, buf_len);
  }
}
...
bool wsrep_should_replicate_ddl_iterate(THD *thd, const TABLE_LIST *table_list)
{
  if (WSREP(thd))
  {
    ...
    {
      if (it->table && !wsrep_should_replicate_ddl(thd, it->table->s->db_type()))
        return false;
    }
  }
  return true;
...
bool wsrep_should_replicate_ddl(THD *thd, const handlerton *hton)
{
  ...
  my_error(ER_GALERA_REPLICATION_NOT_SUPPORTED, MYF(0));
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_ILLEGAL_HA,
                      "WSREP: wsrep_mode = STRICT_REPLICATION enabled. "
 ...
 */
bool wsrep_can_run_in_toi(THD *thd, const char *db, const char *table, const TABLE_LIST *table_list,
...
static int wsrep_create_sp(THD *thd, uchar **buf, size_t *buf_len)
{
  ...
  if (sp->m_handler->show_create_sp(thd, &log_query, sp->m_explicit_name ? sp->m_db : null_clex_str, sp->m_name,
                                    sp->m_params, returns, sp->m_body, sp->chistics(), thd->lex->definer[0],
                                    thd->lex->create_info, saved_mode))
  {
    WSREP_WARN("SP create string failed: schema: %s, query: %s", thd->get_db(), thd->query());
    return 1;
  }
...
static int wsrep_TOI_event_buf(THD *thd, uchar **buf, size_t *buf_len)
{
  ...
  switch (thd->lex->sql_command)
  {
    ...
    case SQLCOM_CREATE_ROLE:
      ...
      /* fallthrough */
    default:
      err = wsrep_to_buf_helper(thd, thd->query(), thd->query_length(), buf, buf_len);
      break;
  }
  ...
  if (wsrep_thd_trx_seqno(thd) > 0)
  {
    ...
    if (ret)
    {
      WSREP_ERROR(
          ...
          (long long)thd->real_id, thd->db.str, thd->query(), ret, err.c_str());
      goto fail;
    }
  }
  return;
 ...
 */
static int wsrep_TOI_begin(THD *thd, const char *db, const char *table, const TABLE_LIST *table_list,
                           const Alter_info *alter_info, const wsrep::key_array *fk_tables,
                           const HA_CREATE_INFO *create_info)
{
  ...
  wsrep::key_array key_array = wsrep_prepare_keys_for_toi(db, table, table_list, alter_info, fk_tables);

  if (thd->has_read_only_protection())
  {
    /* non replicated DDL, affecting temporary tables only */
    WSREP_DEBUG(
        "TO isolation skipped, sql: %s."
  ...
  int ret = cs.enter_toi_local(key_array, wsrep::const_buffer(buff.ptr, buff.len));

  if (ret)
  {
    DBUG_ASSERT(cs.current_error());
    WSREP_DEBUG("to_execute_start() failed for %llu: %s, seqno: %lld", thd->thread_id, wsrep_thd_query(thd),
...
static void wsrep_TOI_end(THD *thd)
{
  ...
  WSREP_DEBUG("TO END: %lld: %s", client_state.toi_meta().seqno().get(), wsrep_thd_query(thd));
  ...
  if (wsrep_thd_is_local_toi(thd))
  {
    ...
    {
      WSREP_WARN("TO isolation end failed for: %d, schema: %s, sql: %s", ret, (thd->db.str ? thd->db.str : "(null)"),
                 wsrep_thd_query(thd));
    }
  }
}
...
{
  WSREP_DEBUG("RSU BEGIN: %lld, : %s", wsrep_thd_trx_seqno(thd), wsrep_thd_query(thd));

  if (thd->variables.wsrep_OSU_method == WSREP_OSU_RSU && thd->variables.sql_log_bin == 1 &&
      wsrep_check_mode(WSREP_MODE_DISALLOW_LOCAL_GTID))
  {
...
{
  WSREP_DEBUG("RSU END: %lld : %s", wsrep_thd_trx_seqno(thd), wsrep_thd_query(thd));
...
int wsrep_to_isolation_begin(THD *thd, const char *db_, const char *table_, const TABLE_LIST *table_list,
                             const Alter_info *alter_info, const wsrep::key_array *fk_tables,
                             const HA_CREATE_INFO *create_info)
{
  /*
  ...
  if (!wsrep_thd_is_local(thd))
  {
    if (wsrep_OSU_method_get(thd) == WSREP_OSU_TOI)
      WSREP_DEBUG("%s TOI Begin: %s", is_replaying_connection(thd) ? "Replay" : "Apply", wsrep_thd_query(thd));
  ...
  if (Wsrep_server_state::instance().desynced_on_pause())
  {
    ...
    WSREP_DEBUG("Aborting TOI: Replication paused on node for FTWRL/BACKUP STAGE.: %s %llu", wsrep_thd_query(thd),
                thd->thread_id);
    return -1;
  }
  ...
  {
    WSREP_DEBUG("thread holds MDL locks at TI begin: %s %llu", wsrep_thd_query(thd), thd->thread_id);
  }
  ...
  if (thd->variables.wsrep_on && wsrep_thd_is_local(thd))
  {
    ...
    {
      case WSREP_OSU_TOI:
        ret = wsrep_TOI_begin(thd, db_, table_, table_list, alter_info, fk_tables, create_info);
        break;
      case WSREP_OSU_RSU:
        ret = wsrep_RSU_begin(thd, db_, table_);
        break;
      default:
        WSREP_ERROR("Unsupported OSU method: %lu", wsrep_OSU_method_get(thd));
        ret = -1;
        break;
    }
...
void wsrep_to_isolation_end(THD *thd)
{
  ...
  else
  {
    ...
    {
      wsrep::client_state &client_state(thd->wsrep_cs());
      WSREP_DEBUG("%s TO END: %lld: %s", is_replaying_connection(thd) ? "Replay" : "Apply",
...
#define WSREP_MDL_LOG(severity, msg, schema, schema_len, req, gra)                                                  \
  WSREP_##severity(                                                                                                 \
      ...
      msg, schema_len, schema, req->thread_id, (long long)wsrep_thd_trx_seqno(req), wsrep_thd_client_mode_str(req), \
      wsrep_thd_client_state_str(req), wsrep_thd_transaction_state_str(req), req->get_command(),                    \
      req->lex->sql_command, req->query(), gra->thread_id, (long long)wsrep_thd_trx_seqno(gra),                     \
...
void wsrep_handle_mdl_conflict(MDL_context *requestor_ctx, const MDL_ticket *ticket, const MDL_key *key)
{
  THD *request_thd = requestor_ctx->get_thd();
  THD *granted_thd = ticket->get_ctx()->get_thd();

  /* Fallback to the non-wsrep behaviour */
  if (!WSREP(request_thd))
    return;
  ...
  {
    WSREP_DEBUG("wsrep_handle_mdl_conflict request TOI/APPLY for %s", wsrep_thd_query(request_thd));
    THD_STAGE_INFO(request_thd, stage_waiting_isolation);
    mysql_mutex_unlock(&request_thd->LOCK_thd_data);
    WSREP_MDL_LOG(DEBUG, "MDL conflict ", schema, schema_len, request_thd, granted_thd);
    ...
    if (wsrep_thd_is_toi(granted_thd) || wsrep_thd_is_applying(granted_thd))
    {
      ...
      {
        WSREP_DEBUG("BF thread waiting for SR in aborting state for %s", wsrep_thd_query(request_thd));
        THD_STAGE_INFO(request_thd, stage_waiting_isolation);
        ticket->wsrep_report(wsrep_debug);
      ...
      {
        WSREP_MDL_LOG(INFO, "MDL conflict, DDL vs SR", schema, schema_len, request_thd, granted_thd);
        mysql_mutex_unlock(&granted_thd->LOCK_thd_data);
        WSREP_DEBUG("wsrep_handle_mdl_conflict DDL vs SR for %s", wsrep_thd_query(request_thd));
        THD_STAGE_INFO(request_thd, stage_waiting_isolation);
        wsrep_abort_thd(request_thd, granted_thd, 1);
      }
      else
      {
        WSREP_MDL_LOG(INFO, "MDL BF-BF conflict", schema, schema_len, request_thd, granted_thd);
    ...
    {
      WSREP_DEBUG("BF thread waiting for FLUSH for %s", wsrep_thd_query(request_thd));
    ...
    {
      WSREP_DEBUG("DROP caused BF abort, conf %s for %s", wsrep_thd_transaction_state_str(granted_thd),
    ...
    {
      WSREP_MDL_LOG(DEBUG, "MDL conflict-> BF abort", schema, schema_len, request_thd, granted_thd);
      WSREP_DEBUG("wsrep_handle_mdl_conflict -> BF abort for %s", wsrep_thd_query(request_thd));
      THD_STAGE_INFO(request_thd, stage_waiting_isolation);
      ticket->wsrep_report(wsrep_debug);
      ...
      else
      {
        ...
        */
        ...
        {
          WSREP_MDL_LOG(INFO, "MDL unknown BF-BF conflict", schema, schema_len, request_thd, granted_thd);
          ticket->wsrep_report(true);
          unireg_abort(1);
        }
      }
...
/**/
static inline bool is_client_connection(THD *thd) { return (thd->wsrep_client_thread && thd->variables.wsrep_on); }
...
{
  DBUG_PRINT("quit", ("Informing thread %lld that it's time to die", (longlong)thd->thread_id));
...
static my_bool kill_remaining_threads(THD *thd, THD *caller_thd)
{
  ...
  if (is_client_connection(thd) && !abort_replicated(thd) && !is_replaying_connection(thd) &&
      thd_is_connection_alive(thd) && thd != caller_thd)
  {
...
void wsrep_close_threads(THD *thd) { server_threads.iterate(wsrep_close_threads_callback, thd); }
...
void wsrep_wait_appliers_close(THD *thd)
{
  ...
  */
  ...
  DBUG_PRINT("quit", ("applier threads have died (count=%u)", uint32_t(wsrep_running_threads)));
...
int wsrep_must_ignore_error(THD *thd)
{
  ...
  if ((flags & CF_WSREP_MAY_IGNORE_ERRORS) && (wsrep_ignore_apply_errors & WSREP_IGNORE_ERRORS_ON_RECONCILING_DDL))
  {
    switch (error)
    {
...
ignore_error:
  WSREP_WARN(
      ...
      thd->get_stmt_da()->message(), print_slave_db_safe(thd->db.str), thd->query(), error);
  return 1;
}
...
int wsrep_ignored_error_code(Log_event *ev, int error)
{
  ...
  if ((wsrep_ignore_apply_errors & WSREP_IGNORE_ERRORS_ON_RECONCILING_DML))
  {
    ...
    if ((ev_type == DELETE_ROWS_EVENT || ev_type == DELETE_ROWS_EVENT_V1) && error == ER_KEY_NOT_FOUND)
      goto ignore_error;
  }
...
ignore_error:
  WSREP_WARN("Ignoring error '%s' on %s event. Error_code: %d", thd->get_stmt_da()->message(), ev->get_type_str(),
             error);
  return 1;
}
...
extern bool wsrep_thd_ignore_table(THD *thd) { return thd->wsrep_ignore_table; }

bool wsrep_create_like_table(THD *thd, TABLE_LIST *table, TABLE_LIST *src_table, HA_CREATE_INFO *create_info)
{
  ...
  else
  {
    ...
    int result __attribute__((unused)) = show_create_table(thd, src_table, &query, NULL, WITH_DB_NAME);
...
int wsrep_create_trigger_query(THD *thd, uchar **buf, size_t *buf_len)
{
  ...
  const LEX_CSTRING command[2] = {{C_STRING_WITH_LEN("CREATE ")}, {C_STRING_WITH_LEN("CREATE OR REPLACE ")}};
  ...
  stmt_definition.length = thd->lex->stmt_definition_end - thd->lex->stmt_definition_begin;
  trim_whitespace(thd->charset(), &stmt_definition);

  stmt_query.append(stmt_definition.str, stmt_definition.length);

  return wsrep_to_buf_helper(thd, stmt_query.c_ptr(), stmt_query.length(), buf, buf_len);
}
...
void *start_wsrep_THD(void *arg)
{
  ...
  /*
    ...
    stack overruns.
  */
  DBUG_PRINT("wsrep", ("handle_one_connection called by thread %lld", (long long)thd->thread_id));
  ...
  WSREP_DEBUG("wsrep system thread %llu, %p starting", thd->thread_id, thd);
  thd_args->fun()(thd, static_cast<void *>(thd_args));

  WSREP_DEBUG("wsrep system thread: %llu, %p closing", thd->thread_id, thd);
...
my_bool get_wsrep_recovery() { return wsrep_recovery; }
...
void wsrep_commit_empty(THD *thd, bool all)
{
  ...
  WSREP_DEBUG(
      "wsrep_commit_empty for %llu client_state %s client_mode"
      " %s trans_state %s sql %s",
      thd_get_thread_id(thd), wsrep::to_c_string(thd->wsrep_cs().state()), wsrep::to_c_string(thd->wsrep_cs().mode()),
      wsrep::to_c_string(thd->wsrep_cs().transaction().state()), wsrep_thd_query(thd));

  if (wsrep_is_real(thd, all) && wsrep_thd_is_local(thd) && thd->wsrep_trx().active() && !thd->internal_transaction() &&
      thd->wsrep_trx().state() != wsrep::transaction::s_committed)
  {
    ...
    int ret = wsrep_before_rollback(thd, all) || wsrep_after_rollback(thd, all) || wsrep_after_statement(thd);
