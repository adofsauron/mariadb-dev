...
partition_info *partition_info::get_clone(THD *thd, bool empty_data_and_index_file)
{
  ...
  while ((part = (part_it++)))
  {
    ...
    part_elem_value *new_val_arr =
        (part_elem_value *)alloc_root(mem_root, sizeof(part_elem_value) * part->list_val_list.elements);
    if (!new_val_arr)
      DBUG_RETURN(NULL);

    p_column_list_val *new_colval_arr = (p_column_list_val *)alloc_root(
        mem_root, sizeof(p_column_list_val) * num_columns * part->list_val_list.elements);
    if (!new_colval_arr)
      DBUG_RETURN(NULL);
    ...
    while ((val = list_val_it++))
    {
      ...
      if (!val->null_value)
      {
        ...
        memcpy(new_colval, val->col_val_array, sizeof(p_column_list_val) * num_columns);
...
bool partition_info::add_named_partition(const char *part_name, size_t length)
{
  ...
  part_def = (PART_NAME_DEF *)my_hash_search(part_name_hash, (const uchar *)part_name, length);
  if (!part_def)
  {
  ...
  DBUG_PRINT("info", ("Found partition %u is_subpart %d for name %.*s", part_def->part_id, part_def->is_subpart, length,
                      part_name));
  DBUG_RETURN(false);
}
...
{
  List<String> *partition_names = table_list ? NULL : table_list->partition_names;
...
char *partition_info::create_default_partition_names(THD *thd, uint part_no, uint num_parts_arg, uint start_no)
{
...
char *partition_info::create_default_subpartition_name(THD *thd, uint subpart_no, const char *part_name)
{
...
bool partition_info::set_up_default_partitions(THD *thd, handler *file, HA_CREATE_INFO *info, uint start_no)
{
  uint i;
  char *default_name;
  bool result = TRUE;
  ...
  if ((num_parts == 0) && ((num_parts = file->get_default_no_partitions(info)) == 0))
  {
  ...
  if (unlikely((!(default_name = create_default_partition_names(thd, 0, num_parts, start_no)))))
    goto end;
  i = 0;
  do
  {
...
bool partition_info::set_up_default_subpartitions(THD *thd, handler *file, HA_CREATE_INFO *info)
{
  uint i, j;
  bool result = TRUE;
  ...
  do
  {
    ...
    do
    {
      ...
      if (likely(subpart_elem != 0 && (!part_elem->subpartitions.push_back(subpart_elem))))
      {
        char *ptr = create_default_subpartition_name(thd, j, part_elem->partition_name);
        if (!ptr)
          goto end;
...
bool partition_info::set_up_defaults_for_partitioning(THD *thd, handler *file, HA_CREATE_INFO *info, uint start_no)
{
  ...
  if (!default_partitions_setup)
  {
    ...
    if (use_default_partitions && set_up_default_partitions(thd, file, info, start_no))
      DBUG_RETURN(TRUE);
...
const char *partition_info::find_duplicate_field()
{
  ...
  for (i = 0; i < num_fields; i++)
  {
    ...
    for (j = 0; j < num_fields; j++)
    {
      ...
      if (i >= j)
        continue;
      if (!(my_strcasecmp(system_charset_info, field_name_outer, field_name_inner)))
      {
...
*/
partition_element *partition_info::get_part_elem(const char *partition_name, char *file_name, size_t file_name_size,
                                                 uint32 *part_id)
{
  List_iterator<partition_element> part_it(partitions);
  uint i = 0;
  ...
  do
  {
    ...
    if (is_sub_partitioned())
    {
      ...
      do
      {
        ...
        if (!my_strcasecmp(system_charset_info, sub_part_elem->partition_name, partition_name))
        {
          if (file_name)
            if (create_subpartition_name(file_name, file_name_size, "", part_elem->partition_name, partition_name,
                                         NORMAL_PART_NAME))
              DBUG_RETURN(NULL);
      ...
      if (!my_strcasecmp(system_charset_info, part_elem->partition_name, partition_name))
        DBUG_RETURN(part_elem);
    }
    else if (!my_strcasecmp(system_charset_info, part_elem->partition_name, partition_name))
    {
      if (file_name)
        if (create_partition_name(file_name, file_name_size, "", partition_name, NORMAL_PART_NAME, TRUE))
          DBUG_RETURN(NULL);
      *part_id = i;
      DBUG_RETURN(part_elem);
    }
...
static const char *get_part_name_from_elem(const char *name, size_t *length, my_bool not_used __attribute__((unused)))
{
  *length = strlen(name);
  return name;
}
...
bool partition_info::has_unique_name(partition_element *element)
{
  ...
  while ((el = (parts_it++)))
  {
    if (!(my_strcasecmp(system_charset_info, el->partition_name, name_to_check)) && el != element)
      DBUG_RETURN(FALSE);
    ...
    if (!el->subpartitions.is_empty())
    {
      ...
      {
        if (!(my_strcasecmp(system_charset_info, sub_el->partition_name, name_to_check)) && sub_el != element)
          DBUG_RETURN(FALSE);
      }
    }
  }
...
{
  DBUG_ASSERT(!thd->lex->last_table() || !thd->lex->last_table()->vers_conditions.delete_history);
  ...
  else if (vers_info->interval.is_set() && vers_info->hist_part->range_value <= thd->query_start())
  {
    partition_element *next = NULL;
    bool error = true;
    List_iterator<partition_element> it(partitions);
    while (next != vers_info->hist_part) next = it++;
    ...
    if (error)
    {
      if (auto_hist)
      {
        *create_count = 0;
        ...
        while (h <= q)
        {
          if (date_add_interval(thd, &h0, vers_info->interval.type, vers_info->interval.step))
            return true;
          h = pack_time(&h0);
          ++*create_count;
          if (*create_count == MAX_PARTITIONS - 2)
          {
            ...
            my_error(ER_VERS_HIST_PART_FAILED, MYF(0), table->s->db.str, table->s->table_name.str);
            return true;
          }
        }
      }
      ...
      {
        my_error(WARN_VERS_PART_FULL, MYF(ME_WARNING | ME_ERROR_LOG), table->s->db.str, table->s->table_name.str,
...
bool vers_create_partitions(THD *thd, TABLE_LIST *tl, uint num_parts)
{
...

  {
    ...
    MDL_REQUEST_INIT(&tl->mdl_request, MDL_key::TABLE, tl->db.str, tl->table_name.str, MDL_SHARED_NO_WRITE,
                     MDL_TRANSACTION);
    if (thd->mdl_context.acquire_lock(&tl->mdl_request, thd->variables.lock_wait_timeout))
      goto exit;
    ...
    if (part_info->set_up_defaults_for_partitioning(thd, table->file, NULL, table->part_info->next_part_no(num_parts)))
    {
      my_error(ER_VERS_HIST_PART_FAILED, MYF(ME_WARNING), tl->db.str, tl->table_name.str);
      goto exit;
    }
    bool partition_changed = false;
    bool fast_alter_partition = false;
    if (prep_alter_part_table(thd, table, &alter_info, &create_info, &partition_changed, &fast_alter_partition))
    {
      my_error(ER_VERS_HIST_PART_FAILED, MYF(ME_WARNING), tl->db.str, tl->table_name.str);
      goto exit;
    }
    ...
    if (mysql_prepare_alter_table(thd, table, &create_info, &alter_info, &alter_ctx))
    {
      my_error(ER_VERS_HIST_PART_FAILED, MYF(ME_WARNING), tl->db.str, tl->table_name.str);
      goto exit;
    }

    if (fast_alter_partition_table(thd, table, &alter_info, &alter_ctx, &create_info, tl))
    {
      my_error(ER_VERS_HIST_PART_FAILED, MYF(ME_WARNING), tl->db.str, tl->table_name.str);
      goto exit;
    }
...
{
  if (vers_info->auto_hist || !vers_info->limit || vers_info->hist_part->id + 1 < vers_info->now_part->id)
    return;
  ...
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, WARN_VERS_PART_FULL, ER_THD(thd, WARN_VERS_PART_FULL),
                        table->s->db.str, table->s->table_name.str, vers_info->hist_part->partition_name, "LIMIT");

    sql_print_warning(ER_THD(thd, WARN_VERS_PART_FULL), table->s->db.str, table->s->table_name.str,
...
static bool check_engine_condition(partition_element *p_elem, bool table_engine_set, handlerton **engine_type,
                                   bool *first)
{
  DBUG_ENTER("check_engine_condition");

  DBUG_PRINT("enter",
             ("p_eng %s t_eng %s t_eng_set %u first %u state %u", ha_resolve_storage_engine_name(p_elem->engine_type),
              ha_resolve_storage_engine_name(*engine_type), table_engine_set, *first, p_elem->part_state));
  if (*first && !table_engine_set)
  {
    *engine_type = p_elem->engine_type;
    DBUG_PRINT("info", ("setting table_engine = %s", ha_resolve_storage_engine_name(*engine_type)));
  }
  *first = FALSE;
  if ((table_engine_set && (p_elem->engine_type != (*engine_type) && p_elem->engine_type)) ||
...
bool partition_info::check_engine_mix(handlerton *engine_type, bool table_engine_set)
{
  ...
  DBUG_PRINT("info", ("in: engine_type = %s, table_engine_set = %u", ha_resolve_storage_engine_name(engine_type),
                      table_engine_set));
  if (n_parts)
  {
    List_iterator<partition_element> part_it(partitions);
    ...
    {
      partition_element *part_elem = part_it++;
      DBUG_PRINT("info", ("part = %d engine = %s table_engine_set %u", i,
                          ha_resolve_storage_engine_name(part_elem->engine_type), table_engine_set));
      if (is_sub_partitioned() && part_elem->subpartitions.elements)
      {
        ...
        do
        {
          ...
          DBUG_PRINT("info", ("sub = %d engine = %s table_engie_set %u", j,
                              ha_resolve_storage_engine_name(sub_elem->engine_type), table_engine_set));
          if (check_engine_condition(sub_elem, table_engine_set, &engine_type, &first))
            goto error;
        } while (++j < n_subparts);
        /* ensure that the partition also has correct engine */
        if (check_engine_condition(part_elem, table_engine_set, &engine_type, &first))
          goto error;
      }
      else if (check_engine_condition(part_elem, table_engine_set, &engine_type, &first))
        goto error;
    } while (++i < n_parts);
  }
  ...
  DBUG_PRINT("info", ("out: engine_type = %s", ha_resolve_storage_engine_name(engine_type)));
...
bool partition_info::check_partition_info(THD *thd, handlerton **eng_type, handler *file, HA_CREATE_INFO *info,
                                          partition_info *add_or_reorg_part)
{
  ...
  DBUG_PRINT("info", ("default table_engine = %s", ha_resolve_storage_engine_name(table_engine)));
  if (!add_or_reorg_part)
  {
    ...
    if (!err && is_sub_partitioned() && !list_of_subpart_fields)
    {
      ...
      err = subpart_expr->walk(&Item::check_partition_func_processor, 0, NULL);
    }
  ...
  if (unlikely(!is_sub_partitioned() && !(use_default_subpartitions && use_default_num_subpartitions)))
  {
    my_error(ER_SUBPARTITION_ERROR, MYF(0));
    goto end;
  }
  if (unlikely(is_sub_partitioned() &&
               (!(part_type == RANGE_PARTITION || part_type == LIST_PARTITION || part_type == VERSIONING_PARTITION))))
  {
  ...
  */
  if (info && info->used_fields & HA_CREATE_USED_ENGINE)
  {
    ...
    DBUG_PRINT("info", ("Using table_engine = %s", ha_resolve_storage_engine_name(table_engine)));
  }
  else
  {
    ...
    {
      table_engine_set = TRUE;
      DBUG_PRINT("info", ("No create, table_engine = %s", ha_resolve_storage_engine_name(table_engine)));
  ...
  i = 0;
  {
    ...
    do
    {
      ...
      if (!is_sub_partitioned())
      {
        ...
        if (check_table_name(part_elem->partition_name, strlen(part_elem->partition_name), FALSE))
        {
          my_error(ER_WRONG_PARTITION_NAME, MYF(0));
          goto end;
        }
        DBUG_PRINT("info", ("part = %d engine = %s", i, ha_resolve_storage_engine_name(part_elem->engine_type)));
      }
      else
      {
        uint j = 0;
        ...
        do
        {
          ...
          if (check_table_name(sub_elem->partition_name, strlen(sub_elem->partition_name), FALSE))
          {
            my_error(ER_WRONG_PARTITION_NAME, MYF(0));
            goto end;
          }
          ...
          DBUG_PRINT("info",
                     ("part = %d sub = %d engine = %s", i, j, ha_resolve_storage_engine_name(sub_elem->engine_type)));
        } while (++j < num_subparts);
        ...
        if (!table_engine_set && prev_num_subparts_not_set != num_subparts_not_set)
        {
          DBUG_PRINT("info", ("num_subparts_not_set = %u num_subparts = %u", num_subparts_not_set, num_subparts));
    ...
    if (!table_engine_set && num_parts_not_set != 0 && num_parts_not_set != num_parts)
    {
      DBUG_PRINT("info", ("num_parts_not_set = %u num_parts = %u", num_parts_not_set, num_subparts));
      my_error(ER_MIX_HANDLER_ERROR, MYF(0));
      goto end;
    }
  }
  ...
  if (hist_parts > 1)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, WARN_VERS_PARAMETERS, ER_THD(thd, WARN_VERS_PARAMETERS),
  ...
  DBUG_ASSERT(table_engine != partition_hton && default_engine_type == table_engine);
  if (eng_type)
...
void partition_info::print_no_partition_found(TABLE *table_arg, myf errflag)
{
  ...
  {
    my_message(ER_NO_PARTITION_FOR_GIVEN_VALUE, ER_THD(thd, ER_NO_PARTITION_FOR_GIVEN_VALUE_SILENT), errflag);
  }
  else
  {
    if (column_list)
...
bool partition_info::set_up_charset_field_preps(THD *thd)
{
  ...
  if (!(part_type == HASH_PARTITION && list_of_part_fields) && check_part_func_fields(part_field_array, FALSE))
  {
  ...
  if (is_sub_partitioned() && !list_of_subpart_fields && check_part_func_fields(subpart_field_array, FALSE))
  {
    /* Set up arrays and buffers for those fields */
...
bool check_partition_dirs(partition_info *part_info)
{
  ...
  while ((part_elem = part_it++))
  {
    if (part_elem->subpartitions.elements)
    {
      ...
      {
        if (unlikely(error_if_data_home_dir(subpart_elem->data_file_name, "DATA DIRECTORY")) ||
            unlikely(error_if_data_home_dir(subpart_elem->index_file_name, "INDEX DIRECTORY")))
          return 1;
      }
    }
    else
    {
      if (unlikely(error_if_data_home_dir(part_elem->data_file_name, "DATA DIRECTORY")) ||
          unlikely(error_if_data_home_dir(part_elem->index_file_name, "INDEX DIRECTORY")))
        return 1;
    }
  }
  return 0;
...
void partition_info::report_part_expr_error(bool use_subpart_expr)
{
  ...
  if (expr->type() == Item::FIELD_ITEM)
  {
    ...
    */
    ...
    if (!column_list && item_field->field && item_field->field->result_type() != INT_RESULT &&
        !(type == HASH_PARTITION && list_of_fields))
    {
      my_error(ER_FIELD_TYPE_NOT_ALLOWED_AS_PARTITION_FIELD, MYF(0), item_field->name.str);
      DBUG_VOID_RETURN;
    }
  }
  if (use_subpart_expr)
...
int partition_info::add_max_value(THD *thd)
{
  ...
  */
  uint max_val = (num_columns && part_type == LIST_PARTITION) ? num_columns : 1;
...
part_column_list_val *partition_info::add_column_value(THD *thd)
{
  ...
  DBUG_PRINT("enter", ("num_columns = %u, curr_list_object %u, max_val = %u", num_columns, curr_list_object, max_val));
...
bool partition_info::add_column_list_value(THD *thd, Item *item)
{
  ...
  if (part_type == LIST_PARTITION && num_columns == 1U)
  {
    if (init_column_part(thd))
    {
      DBUG_RETURN(TRUE);
  ...
  if (item->fix_fields(thd, (Item **)0) || ((context->table_list = save_list), FALSE) || (!item->const_item()))
  {
...
bool partition_info::init_column_part(THD *thd)
{
  ...
  if (!(list_val = (part_elem_value *)thd->calloc(sizeof(part_elem_value))) ||
  ...
  if (!(col_val_array = (part_column_list_val *)thd->calloc(loc_num_columns * sizeof(part_column_list_val))))
    DBUG_RETURN(TRUE);
...
/*
  ...
  RETURN VALUES
    ...
    FALSE                    Success
*/
int partition_info::fix_partition_values(THD *thd, part_elem_value *val, partition_element *part_elem)
{
...
{
  if (field->result_type() == STRING_RESULT && item->collation.collation != field->charset())
  {
...
bool partition_info::fix_column_value_functions(THD *thd, part_elem_value *val, uint part_id)
{
...
bool partition_info::fix_parser_data(THD *thd)
{
  ...
  if (!(part_type == RANGE_PARTITION || part_type == LIST_PARTITION))
  {
    if (part_type == HASH_PARTITION && list_of_part_fields)
    {
      ...
      if ((thd_sql_command(thd) == SQLCOM_CREATE_TABLE || thd_sql_command(thd) == SQLCOM_ALTER_TABLE) &&
  ...
  do
  {
    ...
    if (unlikely(!num_elements && error_if_requires_values()))
      DBUG_RETURN(true);
    DBUG_ASSERT(part_type == RANGE_PARTITION ? num_elements == 1U : TRUE);
    ...
    for (j = 0; j < num_elements; j++)
    {
      ...
      */
      if (val->added_items && val->col_val_array[0].max_value && (!column_list || part_type == LIST_PARTITION))
      {
        DBUG_ASSERT(part_type == RANGE_PARTITION || part_type == LIST_PARTITION);
        if (defined_max_value)
        {
          my_error((part_type == RANGE_PARTITION) ? ER_PARTITION_MAXVALUE_ERROR : ER_PARTITION_DEFAULT_ERROR, MYF(0));
          DBUG_RETURN(TRUE);
        }

        /* For RANGE PARTITION MAX_VALUE must be last */
        if (i != (num_parts - 1) && part_type != LIST_PARTITION)
        {
...
bool partition_info::has_same_partitioning(partition_info *new_part_info)
{
  ...
  */
  ...
  if (!new_part_info || part_type != new_part_info->part_type || num_parts != new_part_info->num_parts ||
  ...
  if (part_type != HASH_PARTITION)
  {
    ...
    */
    if (!is_sub_partitioned() || !new_part_info->is_sub_partitioned() || column_list || new_part_info->column_list ||
        !list_of_subpart_fields || !new_part_info->list_of_subpart_fields ||
        new_part_info->num_subparts != num_subparts ||
        new_part_info->subpart_field_list.elements != subpart_field_list.elements ||
  ...
  if (!use_default_partitions)
  {
    ...
    */
    ...
    do
    {
      ...
      */
      if (!part_elem || !new_part_elem || strcmp(part_elem->partition_name, new_part_elem->partition_name) ||
          part_elem->part_state != PART_NORMAL || new_part_elem->part_state != PART_NORMAL ||
          part_elem->max_value != new_part_elem->max_value || part_elem->signed_flag != new_part_elem->signed_flag ||
          part_elem->has_null_value != new_part_elem->has_null_value)
        DBUG_RETURN(false);

      /* new_part_elem may not have engine_type set! */
      if (new_part_elem->engine_type && part_elem->engine_type != new_part_elem->engine_type)
        DBUG_RETURN(false);
      ...
      if (is_sub_partitioned())
      {
        ...
        */
        if (part_type == LIST_PARTITION)
        {
          ...
          List_iterator<part_elem_value> new_list_vals(new_part_elem->list_val_list);
          part_elem_value *val;
          ...
          while ((val = list_vals++))
          {
            ...
            if (!new_val)
              DBUG_RETURN(false);
            if ((!val->null_value && !new_val->null_value) && val->value != new_val->value)
              DBUG_RETURN(false);
          }
          if (new_list_vals++)
            DBUG_RETURN(false);
        ...
        {
          List_iterator<partition_element> sub_part_it(part_elem->subpartitions);
          List_iterator<partition_element> new_sub_part_it(new_part_elem->subpartitions);
          uint j = 0;
          do
          {
            partition_element *sub_part_elem = sub_part_it++;
            ...
            /* new_part_elem may not have engine_type set! */
            if (new_sub_part_elem->engine_type && sub_part_elem->engine_type != new_sub_part_elem->engine_type)
              DBUG_RETURN(false);

            if (strcmp(sub_part_elem->partition_name, new_sub_part_elem->partition_name) ||
                sub_part_elem->part_state != PART_NORMAL || new_sub_part_elem->part_state != PART_NORMAL ||
                sub_part_elem->part_min_rows != new_sub_part_elem->part_min_rows ||
                sub_part_elem->part_max_rows != new_sub_part_elem->part_max_rows ||
                sub_part_elem->nodegroup_id != new_sub_part_elem->nodegroup_id)
              DBUG_RETURN(false);

            if (strcmp_null(sub_part_elem->data_file_name, new_sub_part_elem->data_file_name) ||
                strcmp_null(sub_part_elem->index_file_name, new_sub_part_elem->index_file_name))
              DBUG_RETURN(false);
      ...
      else
      {
        ...
        if (strcmp_null(part_elem->data_file_name, new_part_elem->data_file_name) ||
            strcmp_null(part_elem->index_file_name, new_part_elem->index_file_name))
          DBUG_RETURN(false);
      }
    } while (++i < num_parts);
  }
...
*/
part_column_list_val *partition_info::add_column_value(THD *thd) { return NULL; }
...
int partition_info::reorganize_into_single_field_col_val(THD *thd) { return 0; }

bool partition_info::init_column_part(THD *thd) { return FALSE; }

bool partition_info::add_column_list_value(THD *thd, Item *item) { return FALSE; }
int partition_info::add_max_value(THD *thd) { return 0; }

void partition_info::print_debug(const char *str, uint *value) {}

bool check_partition_dirs(partition_info *part_info) { return 0; }
...
bool partition_info::vers_set_interval(THD *thd, Item *interval, interval_type int_type, Item *starts, bool auto_hist,
                                       const char *table_name)
{
  ...
  if (interval->fix_fields_if_needed_for_scalar(thd, &interval))
    return true;
  bool error = get_interval_value(thd, interval, int_type, &vers_info->interval.step) || vers_info->interval.step.neg ||
               vers_info->interval.step.second_part ||
               !(vers_info->interval.step.year || vers_info->interval.step.month || vers_info->interval.step.day ||
                 vers_info->interval.step.hour || vers_info->interval.step.minute || vers_info->interval.step.second);
  if (error)
  {
  ...
  if (starts)
  {
    ...
    if (!table)
    {
      ...
      {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_PART_STARTS_BEYOND_INTERVAL,
                            ER_THD(thd, ER_PART_STARTS_BEYOND_INTERVAL), table_name);
      }
    }
  }
...
bool partition_info::vers_set_limit(ulonglong limit, bool auto_hist, const char *table_name)
{
  DBUG_ASSERT(part_type == VERSIONING_PARTITION);
