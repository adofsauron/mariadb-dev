...
inline Item_result item_cmp_type(const Item *a, const Item *b) { return item_cmp_type(a->cmp_type(), b->cmp_type()); }
inline Item_result item_cmp_type(Item_result a, const Item *b) { return item_cmp_type(a, b->cmp_type()); }
...
class Arg_comparator : public Sql_alloc
{
  ...
  int set_cmp_func(THD *thd, Item_func_or_sum *owner_arg, Item **a1, Item **a2);
 ...
 public:
  ...
  inline int set_cmp_func(THD *thd, Item_func_or_sum *owner_arg, Item **a1, Item **a2, bool set_null_arg)
  {
  ...
  void min_max_update_field_native(THD *thd, Field *field, Item *item, int cmp_sign);

  Item **cache_converted_constant(THD *thd, Item **value, Item **cache, const Type_handler *type);
  inline bool is_owner_equal_func()
  {
    return (owner->type() == Item::FUNC_ITEM && ((Item_func *)owner)->functype() == Item_func::EQUAL_FUNC);
  }
...
class Item_bool_func : public Item_int_func, public Type_cmp_attributes
{
 protected:
  /*
  ...
  */
  virtual SEL_TREE *get_func_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *value)
  {
  ...
  */
  SEL_TREE *get_full_func_mm_tree(RANGE_OPT_PARAM *param, Item_field *field_item, Item *value);
  /**
  ...
  */
  SEL_TREE *get_full_func_mm_tree_for_args(RANGE_OPT_PARAM *param, Item *item, Item *value)
  {
    DBUG_ENTER("Item_bool_func::get_full_func_mm_tree_for_args");
    Item *field = item->real_item();
    if (field->type() == Item::FIELD_ITEM && !field->const_item() && (!value || !value->is_expensive()))
      DBUG_RETURN(get_full_func_mm_tree(param, (Item_field *)field, value));
    DBUG_RETURN(NULL);
  }
  SEL_TREE *get_mm_parts(RANGE_OPT_PARAM *param, Field *field, Item_func::Functype type, Item *value);
  SEL_TREE *get_ne_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *lt_value, Item *gt_value);
  virtual SEL_ARG *get_mm_leaf(RANGE_OPT_PARAM *param, Field *field, KEY_PART *key_part, Item_func::Functype type,
                               Item *value);
...
class Item_func_istrue : public Item_func_truth
{
 public:
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_istrue>(thd, this); }
};
...
{
 public:
  Item_func_isnottrue(THD *thd, Item *a) : Item_func_truth(thd, a, true, false) {}
  ~Item_func_isnottrue() {}
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_isnottrue>(thd, this); }
...
class Item_func_isfalse : public Item_func_truth
{
 public:
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_isfalse>(thd, this); }
};
...
{
 public:
  Item_func_isnotfalse(THD *thd, Item *a) : Item_func_truth(thd, a, false, false) {}
  ...
  bool find_not_null_fields(table_map allowed) override { return false; }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_isnotfalse>(thd, this); }
  bool eval_not_null_tables(void *) override
  {
    not_null_tables_cache = 0;
    return false;
...
class Item_in_optimizer : public Item_bool_func
{
 ...
 public:
  Item_in_optimizer(THD *thd, Item *a, Item *b)
      : Item_bool_func(thd, a, b), cache(0), expr_cache(0), save_cache(0), result_for_null_param(UNKNOWN)
  {
  ...
  void set_join_tab_idx(uint8 join_tab_idx_arg) override { args[1]->set_join_tab_idx(join_tab_idx_arg); }
  ...
  Item *get_wrapped_in_subselect_item() { return args[1]; }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_in_optimizer>(thd, this); }
...
{ /* Bool with 2 string args */
 protected:
  void add_key_fields_optimize_op(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                  SARGABLE_PARAM **sargables, bool equal_func);

 public:
  Item_bool_func2(THD *thd, Item *a, Item *b) : Item_bool_func(thd, a, b) {}

  bool is_null() { return MY_TEST(args[0]->is_null() || args[1]->is_null()); }
  COND *remove_eq_conds(THD *thd, Item::cond_result *cond_value, bool top_level);
...
{
 protected:
  SEL_TREE *get_func_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *value)
  {
    DBUG_ENTER("Item_bool_func2_with_rev::get_func_mm_tree");
    Item_func::Functype func_type = (value != arguments()[0]) ? functype() : rev_functype();
    DBUG_RETURN(get_mm_parts(param, field, func_type, value));
  }

 public:
  Item_bool_func2_with_rev(THD *thd, Item *a, Item *b) : Item_bool_func2(thd, a, b) {}
...
class Item_bool_rowready_func2 : public Item_bool_func2_with_rev
{
 protected:
  Arg_comparator cmp;
  bool check_arguments() const override { return check_argument_types_like_args0(); }

 public:
  Item_bool_rowready_func2(THD *thd, Item *a, Item *b) : Item_bool_func2_with_rev(thd, a, b), cmp(tmp_arg, tmp_arg + 1)
  {
  }
  Sql_mode_dependency value_depends_on_sql_mode() const override;
  void print(String *str, enum_query_type query_type) override { Item_func::print_op(str, query_type); }
  enum precedence precedence() const override { return CMP_PRECEDENCE; }
  Item *neg_transformer(THD *thd) override;
  virtual Item *negated_item(THD *thd);
  Item *propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *cond) override
  {
    Item_args::propagate_equal_fields(thd, Context(ANY_SUBST, cmp.compare_type_handler(), compare_collation()), cond);
    return this;
  }
  bool fix_length_and_dec(THD *thd) override;
  int set_cmp_func(THD *thd) { return cmp.set_cmp_func(thd, this, tmp_arg, tmp_arg + 1, true); }
  CHARSET_INFO *compare_collation() const override { return cmp.compare_collation(); }
  const Type_handler *compare_type_handler() const override { return cmp.compare_type_handler(); }
  ...
  void add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                      SARGABLE_PARAM **sargables) override
  {
    return add_key_fields_optimize_op(join, key_fields, and_level, usable_tables, sargables, false);
  }
  Item *build_clone(THD *thd) override
  {
    Item_bool_rowready_func2 *clone = (Item_bool_rowready_func2 *)Item_func::build_clone(thd);
    if (clone)
    {
...
class Item_func_xor : public Item_bool_func
{
 public:
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_xor>(thd, this); }
};
...
class Item_func_not : public Item_bool_func
{
 public:
  ...
  bool fix_fields(THD *, Item **) override;
  void print(String *str, enum_query_type query_type) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_not>(thd, this); }
};
...
class Item_func_trig_cond : public Item_bool_func
{
 ...
 public:
  ...
  void add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                      SARGABLE_PARAM **sargables) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_trig_cond>(thd, this); }
};
...
class Item_func_not_all : public Item_func_not
{
 ...
 public:
  ...
  Item_func_not_all(THD *thd, Item *a) : Item_func_not(thd, a), test_sum_item(0), test_sub_item(0), show(0) {}
  table_map not_null_tables() const override { return 0; }
  longlong val_int() override;
  ...
  bool fix_fields(THD *thd, Item **ref) override { return Item_func::fix_fields(thd, ref); }
  void print(String *str, enum_query_type query_type) override;
  void set_sum_test(Item_sum_min_max *item)
  {
    test_sum_item = item;
    test_sub_item = 0;
  };
...
class Item_func_nop_all : public Item_func_not_all
{
 public:
  ...
  Item *neg_transformer(THD *thd) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_nop_all>(thd, this); }
};

class Item_func_eq : public Item_bool_rowready_func2
{
 public:
  Item_func_eq(THD *thd, Item *a, Item *b) : Item_bool_rowready_func2(thd, a, b), in_equality_no(UINT_MAX) {}
  longlong val_int() override;
  ...
  void add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                      SARGABLE_PARAM **sargables) override
  {
    return add_key_fields_optimize_op(join, key_fields, and_level, usable_tables, sargables, true);
  }
  ...
  */
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_eq>(thd, this); }
};

class Item_func_equal final : public Item_bool_rowready_func2
{
 public:
  Item_func_equal(THD *thd, Item *a, Item *b) : Item_bool_rowready_func2(thd, a, b) {}
  longlong val_int() override;
  bool fix_length_and_dec(THD *thd) override;
  table_map not_null_tables() const override { return 0; }
  bool find_not_null_fields(table_map allowed) override { return false; }
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_equal>(thd, this); }
};

class Item_func_ge : public Item_bool_rowready_func2
{
 public:
  Item_func_ge(THD *thd, Item *a, Item *b) : Item_bool_rowready_func2(thd, a, b){};
  longlong val_int() override;
  ...
  Item *negated_item(THD *thd) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_ge>(thd, this); }
};

class Item_func_gt : public Item_bool_rowready_func2
{
 public:
  Item_func_gt(THD *thd, Item *a, Item *b) : Item_bool_rowready_func2(thd, a, b){};
  longlong val_int() override;
  ...
  Item *negated_item(THD *thd) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_gt>(thd, this); }
};

class Item_func_le : public Item_bool_rowready_func2
{
 public:
  Item_func_le(THD *thd, Item *a, Item *b) : Item_bool_rowready_func2(thd, a, b){};
  longlong val_int() override;
  ...
  Item *negated_item(THD *thd) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_le>(thd, this); }
};

class Item_func_lt : public Item_bool_rowready_func2
{
 public:
  Item_func_lt(THD *thd, Item *a, Item *b) : Item_bool_rowready_func2(thd, a, b) {}
  longlong val_int() override;
  ...
  Item *negated_item(THD *thd) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_lt>(thd, this); }
};

class Item_func_ne : public Item_bool_rowready_func2
{
 protected:
  SEL_TREE *get_func_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *value) override;

 public:
  Item_func_ne(THD *thd, Item *a, Item *b) : Item_bool_rowready_func2(thd, a, b) {}
  longlong val_int() override;
  ...
  void add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                      SARGABLE_PARAM **sargables) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_ne>(thd, this); }
};
...
class Item_func_opt_neg : public Item_bool_func
{
 ...
 public:
  Item_func_opt_neg(THD *thd, Item *a, Item *b, Item *c) : Item_bool_func(thd, a, b, c), negated(0) {}
  Item_func_opt_neg(THD *thd, List<Item> &list) : Item_bool_func(thd, list), negated(0) {}
 ...
 public:
  ...
  Item *propagate_equal_fields(THD *, const Context &, COND_EQUAL *) override = 0;
};
...
class Item_func_between : public Item_func_opt_neg
{
 ...
 public:
  ...
  Item_func_between(THD *thd, Item *a, Item *b, Item *c) : Item_func_opt_neg(thd, a, b, c) {}
  longlong val_int() override
  {
    DBUG_ASSERT(fixed());
  ...
  bool fix_length_and_dec(THD *thd) override;
  bool fix_length_and_dec_string(THD *) { return agg_arg_charsets_for_comparison(cmp_collation, args, 3); }
  ...
  Item *propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *cond) override
  {
    Item_args::propagate_equal_fields(thd, Context(ANY_SUBST, m_comparator.type_handler(), compare_collation()), cond);
    return this;
  }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_between>(thd, this); }
...
class Item_func_strcmp : public Item_long_func
{
 ...
 public:
  Item_func_strcmp(THD *thd, Item *a, Item *b) : Item_long_func(thd, a, b) {}
  longlong val_int() override;
  decimal_digits_t decimal_precision() const override { return 1; }
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_strcmp>(thd, this); }
};
...
class Item_func_interval : public Item_long_func
{
 ...
 public:
  Item_func_interval(THD *thd, Item_row *a) : Item_long_func(thd, a), row(a), intervals(0) {}
  bool fix_fields(THD *, Item **) override;
  longlong val_int() override;
  bool fix_length_and_dec(THD *thd) override;
  LEX_CSTRING func_name_cstring() const override
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_interval>(thd, this); }
};

class Item_func_coalesce : public Item_func_case_expression
{
 public:
  Item_func_coalesce(THD *thd, Item *a, Item *b) : Item_func_case_expression(thd, a, b) {}
  Item_func_coalesce(THD *thd, List<Item> &list) : Item_func_case_expression(thd, list) {}
  ...
  table_map not_null_tables() const override { return 0; }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_coalesce>(thd, this); }
};
...
class Item_func_case_abbreviation2 : public Item_func_case_expression
{
 ...
 public:
  Item_func_case_abbreviation2(THD *thd, Item *a, Item *b) : Item_func_case_expression(thd, a, b) {}
  Item_func_case_abbreviation2(THD *thd, Item *a, Item *b, Item *c) : Item_func_case_expression(thd, a, b, c) {}
};

class Item_func_ifnull : public Item_func_case_abbreviation2
{
 public:
  Item_func_ifnull(THD *thd, Item *a, Item *b) : Item_func_case_abbreviation2(thd, a, b) {}
  double real_op() override;
  longlong int_op() override;
  ...
  table_map not_null_tables() const override { return 0; }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_ifnull>(thd, this); }
};
...
class Item_func_case_abbreviation2_switch : public Item_func_case_abbreviation2
{
 ...
 public:
  Item_func_case_abbreviation2_switch(THD *thd, Item *a, Item *b, Item *c) : Item_func_case_abbreviation2(thd, a, b, c)
  {
  }
  ...
  bool time_op(THD *thd, MYSQL_TIME *ltime) { return (null_value = Time(find_item()).copy_to_mysql_time(ltime)); }
  longlong int_op() { return val_int_from_item(find_item()); }
  double real_op() { return val_real_from_item(find_item()); }
  my_decimal *decimal_op(my_decimal *decimal_value) { return val_decimal_from_item(find_item(), decimal_value); }
  String *str_op(String *str) { return val_str_from_item(find_item(), str); }
  bool native_op(THD *thd, Native *to)
  {
    return val_native_with_conversion_from_item(thd, find_item(), to, type_handler());
  }
};
...
class Item_func_if : public Item_func_case_abbreviation2_switch
{
 ...
 public:
  Item_func_if(THD *thd, Item *a, Item *b, Item *c) : Item_func_case_abbreviation2_switch(thd, a, b, c) {}
  bool fix_fields(THD *, Item **) override;
  bool fix_length_and_dec(THD *thd) override { return fix_length_and_dec2_eliminate_null(args + 1); }
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  bool eval_not_null_tables(void *opt_arg) override;
  void fix_after_pullout(st_select_lex *new_parent, Item **ref, bool merge) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_if>(thd, this); }
...
class Item_func_nvl2 : public Item_func_case_abbreviation2_switch
{
 ...
 public:
  Item_func_nvl2(THD *thd, Item *a, Item *b, Item *c) : Item_func_case_abbreviation2_switch(thd, a, b, c) {}
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  bool fix_length_and_dec(THD *thd) override { return fix_length_and_dec2_eliminate_null(args + 1); }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_nvl2>(thd, this); }
};
...
class Item_func_nullif : public Item_func_case_expression
{
  ...
  */
  Item_func_nullif(THD *thd, Item *a, Item *b) : Item_func_case_expression(thd, a, b, a), m_cache(NULL), m_arg0(NULL)
  {
    arg_count--;
  }
  void cleanup() override
  ...
  Item *propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *cond) override
  {
    Context cmpctx(ANY_SUBST, cmp.compare_type_handler(), cmp.compare_collation());
    const Item *old0 = args[0];
    args[0]->propagate_equal_fields_and_change_item_tree(thd, cmpctx, cond, &args[0]);
    args[1]->propagate_equal_fields_and_change_item_tree(thd, cmpctx, cond, &args[1]);
    /*
    ...
    */
    if (old0 != args[0])
      args[2]->propagate_equal_fields_and_change_item_tree(thd, Context_identity(), cond, &args[2]);
    return this;
  }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_nullif>(thd, this); }
  Item *derived_field_transformer_for_having(THD *thd, uchar *arg) override
  {
    reset_first_arg_if_needed();
    return this;
  }
...
class in_vector : public Sql_alloc
{
 public:
  ...
  uint used_count;
  in_vector() {}
  in_vector(THD *thd, uint elements, uint element_length, qsort2_cmp cmp_func, CHARSET_INFO *cmp_coll)
  ...
  void sort() { my_qsort2(base, used_count, size, compare, (void *)collation); }
  bool find(Item *item);
...
class in_string : public in_vector
{
  ...
  {
   public:
    Item_string_for_in_vector(THD *thd, CHARSET_INFO *cs) : Item_string(thd, cs) {}
...
class in_longlong : public in_vector
{
 ...
 public:
  ...
  void value_to_item(uint pos, Item *item) override
  {
    ...
    ((Item_int *)item)->unsigned_flag = (bool)((packed_longlong *)base)[pos].unsigned_flag;
  }
...
class in_temporal : public in_longlong
{
 public:
  ...
  in_temporal(THD *thd, uint elements) : in_longlong(thd, elements){};
...
{
 public:
  in_datetime(THD *thd, uint elements) : in_temporal(thd, elements) {}
  void set(uint pos, Item *item) override;
  uchar *get_value(Item *item) override;
...
{
 public:
  in_time(THD *thd, uint elements) : in_temporal(thd, elements) {}
  void set(uint pos, Item *item) override;
  uchar *get_value(Item *item) override;
...
class in_double : public in_vector
{
 ...
 public:
  ...
  uchar *get_value(Item *item) override;
  Item *create_item(THD *thd) override;
  void value_to_item(uint pos, Item *item) override { ((Item_float *)item)->value = ((double *)base)[pos]; }
...
class cmp_item : public Sql_alloc
{
  ...
  */
  ...
  virtual void store_value_by_template(THD *thd, cmp_item *tmpl, Item *item) { store_value(item); }
};
...
class cmp_item_sort_string : public cmp_item_string
{
 ...
 public:
  cmp_item_sort_string() : cmp_item_string() {}
...
{
 public:
  cmp_item_datetime() : cmp_item_temporal() {}
  void store_value(Item *item)
  {
...
{
 public:
  cmp_item_time() : cmp_item_temporal() {}
  void store_value(Item *item)
  {
...
class cmp_item_sort_string_in_static : public cmp_item_string
{
 ...
 public:
  ...
  cmp_item *make_same(THD *thd) { return new cmp_item_sort_string_in_static(cmp_charset); }
};
...
class Predicant_to_list_comparator
{
  ...
  */
  void detect_unique_handlers(Type_handler_hybrid_field_type *compatible, uint *unique_cnt, uint *found_types);
  /**
  ...
  */
  ...
  {
    Predicant_to_value_comparator *cmp = &m_comparators[m_comparators[i].m_handler_index];
    ...
    */
    if (args->arguments()[m_predicant_index]->null_value && m_comparators[i].m_handler != &type_handler_row)
      return UNKNOWN;
 ...
 public:
  Predicant_to_list_comparator(THD *thd, uint nvalues) : m_comparator_count(0), m_predicant_index(0)
  {
    ...
    for (uint i = 0; i < m_comparator_count; i++)
    {
      DBUG_EXECUTE_IF("Predicant_to_list_comparator",
                      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                                          "DBUG: [%d] arg=%d handler=%d (%s)", i, m_comparators[i].m_arg_index,
                                          m_comparators[i].m_handler_index,
                                          m_comparators[m_comparators[i].m_handler_index].m_handler->name().ptr()););
    }
  }
#endif
  ...
  */
  bool add_value(const LEX_CSTRING &funcname, Item_args *args, uint value_index);
  ...
  */
  bool add_value_skip_null(const LEX_CSTRING &funcname, Item_args *args, uint value_index, bool *nulls_found);
  ...
  /**
    ...
    @param [OUT] found_type - The bit mask for all found cmp_type()'s.
  */
  void all_values_added(Type_handler_hybrid_field_type *compatible, uint *unique_cnt, uint *found_types)
  {
  ...
  */
  ...
  void cleanup()
  {
    ...
    for (uint i = 0; i < m_comparator_count; i++) m_comparators[i].cleanup();
...
class Item_func_case : public Item_func_case_expression
{
 protected:
  ...
  inline void print_when_then_arguments(String *str, enum_query_type query_type, Item **items, uint count);
  inline void print_else_argument(String *str, enum_query_type query_type, Item *item);
  void reorder_args(uint start);

 public:
  Item_func_case(THD *thd, List<Item> &list) : Item_func_case_expression(thd, list) {}
  double real_op() override;
  longlong int_op() override;
  String *str_op(String *) override;
  my_decimal *decimal_op(my_decimal *) override;
...
class Item_func_case_searched : public Item_func_case
{
 ...
 public:
  Item_func_case_searched(THD *thd, List<Item> &list) : Item_func_case(thd, list)
  {
    DBUG_ASSERT(arg_count >= 2);
    reorder_args(0);
  }
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_case_searched>(thd, this); }
};
...
*/
class Item_func_case_simple : public Item_func_case, public Predicant_to_list_comparator
{
 protected:
  uint m_found_types;
  ...
  bool prepare_predicant_and_values(THD *thd, uint *found_types, bool nulls_equal);

 public:
  Item_func_case_simple(THD *thd, List<Item> &list)
      : Item_func_case(thd, list), Predicant_to_list_comparator(thd, arg_count), m_found_types(0)
  {
  ...
  Item *build_clone(THD *thd) override
  {
    Item_func_case_simple *clone = (Item_func_case_simple *)Item_func_case::build_clone(thd);
    uint ncases = when_count();
    if (clone && clone->Predicant_to_list_comparator::init_clone(thd, ncases))
      return NULL;
    return clone;
  }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_case_simple>(thd, this); }
};

class Item_func_decode_oracle : public Item_func_case_simple
{
 public:
  Item_func_decode_oracle(THD *thd, List<Item> &list) : Item_func_case_simple(thd, list) {}
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  bool fix_length_and_dec(THD *thd) override;
  Item *find_item() override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_decode_oracle>(thd, this); }
};
...
*/
class Item_func_in : public Item_func_opt_neg, public Predicant_to_list_comparator
{
  /**
  ...
  */
  ...
  bool agg_all_arg_charsets_for_comparison() { return agg_arg_charsets_for_comparison(cmp_collation, args, arg_count); }
  void fix_in_vector();
  ...
  Item *propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *cond) override
  {
    ...
    */
    ...
    {
      Context cmpctx(ANY_SUBST, m_comparator.type_handler(), Item_func_in::compare_collation());
      args[0]->propagate_equal_fields_and_change_item_tree(thd, cmpctx, cond, &args[0]);
    }
    for (uint i = 0; i < comparator_count(); i++)
    {
      Context cmpctx(ANY_SUBST, get_comparator_type_handler(i), Item_func_in::compare_collation());
      uint idx = get_comparator_arg_index(i);
      args[idx]->propagate_equal_fields_and_change_item_tree(thd, cmpctx, cond, &args[idx]);
    }
    return this;
  }
  void print(String *str, enum_query_type query_type) override;
  ...
  void fix_after_pullout(st_select_lex *new_parent, Item **ref, bool merge) override;
  bool count_sargable_conds(void *arg) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_in>(thd, this); }
  Item *build_clone(THD *thd) override
  {
...
class cmp_item_row : public cmp_item
{
  ...
  bool aggregate_row_elements_for_comparison(THD *thd, Type_handler_hybrid_field_type *cmp, Item_args *tmp,
 ...
 public:
  ...
  ~cmp_item_row();
  void store_value(Item *item);
  bool prepare_comparators(THD *, const LEX_CSTRING &funcname, const Item_args *args, uint level);
  int cmp(Item *arg);
  int cmp_not_null(const Value *val)
  {
    DBUG_ASSERT(false);
...
class Item_func_null_predicate : public Item_bool_func
{
 protected:
  ...
  SEL_ARG *get_mm_leaf(RANGE_OPT_PARAM *param, Field *field, KEY_PART *key_part, Item_func::Functype type,
                       Item *value) override;
 ...
 public:
  ...
  CHARSET_INFO *compare_collation() const override { return args[0]->collation.collation; }
  bool fix_length_and_dec(THD *thd) override
  {
    decimals = 0;
    max_length = 1;
...
class Item_func_isnull : public Item_func_null_predicate
{
 public:
  ...
  COND *remove_eq_conds(THD *thd, Item::cond_result *cond_value, bool top_level) override;
  table_map not_null_tables() const override { return 0; }
  bool find_not_null_fields(table_map allowed) override;
  Item *neg_transformer(THD *thd) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_isnull>(thd, this); }
};
...
class Item_is_not_null_test : public Item_func_isnull
{
 ...
 public:
  Item_is_not_null_test(THD *thd, Item_in_subselect *ow, Item *a) : Item_func_isnull(thd, a), owner(ow) {}
...
{
 public:
  Item_func_isnotnull(THD *thd, Item *a) : Item_func_null_predicate(thd, a) {}
  longlong val_int() override;
  ...
  enum precedence precedence() const override { return CMP_PRECEDENCE; }
  table_map not_null_tables() const override { return is_top_level_item() ? not_null_tables_cache : 0; }
  Item *neg_transformer(THD *thd) override;
  void print(String *str, enum_query_type query_type) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_isnotnull>(thd, this); }
};
...
class Item_func_like : public Item_bool_func2
{
 ...
 public:
  ...
  Item *propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *cond) override
  {
    ...
    */
    ...
    if ((flags & MY_CS_NOPAD) && !(flags & MY_CS_NON1TO1))
      Item_args::propagate_equal_fields(thd, Context(ANY_SUBST, &type_handler_long_blob, compare_collation()), cond);
    return this;
  }
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  bool walk(Item_processor processor, bool walk_subquery, void *arg) override
  {
    return (walk_args(processor, walk_subquery, arg) || escape_item->walk(processor, walk_subquery, arg) ||
            (this->*processor)(arg));
  }

  bool find_selective_predicates_list_processor(void *arg) override;

  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_like>(thd, this); }
};
...
class Regexp_processor_pcre
{
  ...
  int pcre_exec_with_warn(const pcre2_code *code, pcre2_match_data *data, const char *subject, int length,
 ...
 public:
  ...
  bool recompile(Item *item) { return !m_is_const && compile(item, false); }
  ...
  size_t subpattern_start(int n) const { return m_pcre_exec_rc <= 0 ? 0 : m_SubStrVec[n * 2]; }
  size_t subpattern_end(int n) const { return m_pcre_exec_rc <= 0 ? 0 : m_SubStrVec[n * 2 + 1]; }
  size_t subpattern_length(int n) const { return subpattern_end(n) - subpattern_start(n); }
  void reset()
  {
    m_pcre = NULL;
    m_pcre_match_data = NULL;
...
class Item_func_regexp_instr : public Item_long_func
{
 ...
 public:
  Item_func_regexp_instr(THD *thd, Item *a, Item *b) : Item_long_func(thd, a, b) {}
  void cleanup() override
  {
...
class Item_cond : public Item_bool_func
{
 ...
 public:
  ...
  Item_cond(THD *thd, List<Item> &nlist) : Item_bool_func(thd), list(nlist) {}
...
template <template <class> class LI, class T>
...
class Item_equal : public Item_bool_func
{
 ...
 public:
  ...
  Item_equal(THD *thd, const Type_handler *handler, Item *f1, Item *f2, bool with_const_item);
  ...
  void merge_into_list(THD *thd, List<Item_equal> *list, bool save_merged, bool only_intersected);
  ...
  */
  bool excl_dep_on_table(table_map tab_map) override { return used_tables() & tab_map; }
  bool excl_dep_on_in_subq_left_part(Item_in_subselect *subq_pred) override;
  bool excl_dep_on_grouping_fields(st_select_lex *sel) override;
  bool create_pushable_equalities(THD *thd, List<Item> *equalities, Pushdown_checker checker, uchar *arg,
                                  bool clone_const);
  ...
  friend Item *eliminate_item_equal(THD *thd, COND *cond, COND_EQUAL *upper_levels, Item_equal *item_equal);
...
class COND_EQUAL : public Sql_alloc
{
 public:
  ...
  COND_EQUAL(Item_equal *item, MEM_ROOT *mem_root) : upper_levels(0) { current_level.push_back(item, mem_root); }
  ...
  bool is_empty() { return (current_level.elements == 0); }
};
...
template <template <class> class LI, typename T>
class Item_equal_iterator : public LI<T>
{
 protected:
 ...
 public:
  Item_equal_iterator(Item_equal &item_eq) : LI<T>(item_eq.equal_items)
  {
    curr_item = NULL;
    item_equal = &item_eq;
...
class Item_equal_fields_iterator : public Item_equal_iterator_fast
{
 public:
  Item_equal_fields_iterator(Item_equal &item_eq) : Item_equal_iterator_fast(item_eq) {}
  Item **ref() { return List_iterator_fast<Item>::ref(); }
};

typedef Item_equal_iterator<List_iterator, Item> Item_equal_iterator_iterator_slow;

class Item_equal_fields_iterator_slow : public Item_equal_iterator_iterator_slow
{
 public:
  Item_equal_fields_iterator_slow(Item_equal &item_eq) : Item_equal_iterator_iterator_slow(item_eq) {}
  void remove() { List_iterator<Item>::remove(); }
};
...
class Item_cond_and final : public Item_cond
{
 public:
  ...
  table_map not_null_tables() const override { return is_top_level_item() ? not_null_tables_cache : and_tables_cache; }
  ...
  void add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                      SARGABLE_PARAM **sargables) override;
  SEL_TREE *get_mm_tree(RANGE_OPT_PARAM *param, Item **cond_ptr) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_cond_and>(thd, this); }
};
...
class Item_cond_or final : public Item_cond
{
 public:
  ...
  Item *copy_andor_structure(THD *thd) override;
  Item *neg_transformer(THD *thd) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_cond_or>(thd, this); }
};
...
class Item_func_dyncol_check : public Item_bool_func
{
 public:
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_dyncol_check>(thd, this); }
};

class Item_func_dyncol_exists : public Item_bool_func
{
 public:
  Item_func_dyncol_exists(THD *thd, Item *str, Item *num) : Item_bool_func(thd, str, num) {}
  longlong val_int() override;
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  bool need_parentheses_in_default() override { return false; }
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_dyncol_exists>(thd, this); }
};
...
class Item_func_cursor_bool_attr : public Item_bool_func, public Cursor_ref
{
 public:
  ...
  void print(String *str, enum_query_type query_type) { Cursor_ref::print_func(str, func_name_cstring()); }
};
...
class Item_func_cursor_isopen : public Item_func_cursor_bool_attr
{
 public:
  ...
  longlong val_int() override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_cursor_isopen>(thd, this); }
};

class Item_func_cursor_found : public Item_func_cursor_bool_attr
{
 public:
  Item_func_cursor_found(THD *thd, const LEX_CSTRING *name, uint offset) : Item_func_cursor_bool_attr(thd, name, offset)
  {
    set_maybe_null();
  }
  LEX_CSTRING func_name_cstring() const override
  ...
  longlong val_int() override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_cursor_found>(thd, this); }
};
...
class Item_func_cursor_notfound : public Item_func_cursor_bool_attr
{
 public:
  ...
  longlong val_int() override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_cursor_notfound>(thd, this); }
};
...
class Comp_creator
{
  ...
  */
  virtual Item_bool_rowready_func2 *create(THD *thd, Item *a, Item *b) const = 0;
  /**
    Create operation with given arguments in swap order.
  */
  virtual Item_bool_rowready_func2 *create_swap(THD *thd, Item *a, Item *b) const = 0;
