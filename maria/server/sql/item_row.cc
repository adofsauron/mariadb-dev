...
bool Item_row::eval_not_null_tables(void *opt_arg)
{
  Item **arg, **arg_end;
...
bool Item_row::find_not_null_fields(table_map allowed)
{
...
void Item_row::split_sum_func(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &fields, uint flags)
{
  Item **arg, **arg_end;
  for (arg = args, arg_end = args + arg_count; arg != arg_end; arg++)
...
void Item_row::fix_after_pullout(st_select_lex *new_parent, Item **ref, bool merge)
{
  used_tables_and_const_cache_init();
  not_null_tables_cache = 0;
...
{
  for (uint i = 0; i < arg_count; i++) args[i]->bring_value();
}

Item *Item_row::build_clone(THD *thd)
{
  Item **copy_args = static_cast<Item **>(alloc_root(thd->mem_root, sizeof(Item *) * arg_count));
