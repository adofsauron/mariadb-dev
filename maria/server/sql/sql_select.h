...
typedef struct st_table_ref
{
  ...
  */
  ...
  bool tmp_table_index_lookup_init(THD *thd, KEY *tmp_key, Item_iterator &it, bool value, uint skip = 0);
...
typedef struct st_join_table
{
  ...
  */
  ...
  inline bool is_using_loose_index_scan()
  {
    ...
    return (sel && sel->quick && (sel->quick->get_type() == QUICK_SELECT_I::QS_TYPE_GROUP_MIN_MAX));
  }
  bool is_using_agg_loose_index_scan()
  {
    return (is_using_loose_index_scan() && ((QUICK_GROUP_MIN_MAX_SELECT *)select->quick)->is_agg_distinct());
  }
  bool is_inner_table_of_semi_join_with_first_match() { return first_sj_inner_tab != NULL; }
  bool is_inner_table_of_semijoin() { return emb_sj_nest != NULL; }
  bool is_inner_table_of_outer_join() { return first_inner != NULL; }
  bool is_single_inner_of_semi_join_with_first_match()
  {
    return first_sj_inner_tab == this && last_sj_inner_tab == this;
  }
  bool is_single_inner_of_outer_join() { return first_inner == this && first_inner->last_inner == this; }
  bool is_first_inner_for_outer_join() { return first_inner == this; }
  bool use_match_flag() { return is_first_inner_for_outer_join() || first_sj_inner_tab == this; }
  bool check_only_first_match()
  {
    return is_inner_table_of_semi_join_with_first_match() ||
           (is_inner_table_of_outer_join() && table->reginfo.not_exists_optimize);
  }
  bool is_last_inner_table() { return (first_inner && first_inner->last_inner == this) || last_sj_inner_tab == this; }
  /*
  ...
  */
  ...
  {
    if (first_inner && (first_inner != first_inner->last_inner || first_inner->first_upper))
      return TRUE;
  ...
  {
    DBUG_PRINT("info", ("select_cond changes %p -> %p at line %u tab %p", select_cond, to, line, this));
    select_cond = to;
  }
  ...
  */
  int get_non_merged_semijoin_select() const
  {
    ...
    if (table->pos_in_table_list && (subq = table->pos_in_table_list->jtbm_subselect))
    {
  ...
  bool access_from_tables_is_allowed(table_map used_tables, table_map sjm_lookup_tables)
  {
    table_map used_sjm_lookup_tables = used_tables & sjm_lookup_tables;
    return !used_sjm_lookup_tables || (emb_sj_nest && !(used_sjm_lookup_tables & ~emb_sj_nest->sj_inner_tables));
  }
  ...
  bool save_explain_data(Explain_table_access *eta, table_map prefix_tables, bool distinct,
                         struct st_join_table *first_top_tab);
  ...
  SplM_plan_info *choose_best_splitting(double record_count, table_map remaining_tables);
  bool fix_splitting(SplM_plan_info *spl_plan, table_map remaining_tables, bool is_const_table);
} JOIN_TAB;
...
class Semi_join_strategy_picker
{
  ...
  */
  ...
  virtual bool check_qep(JOIN *join, uint idx, table_map remaining_tables, const JOIN_TAB *new_join_tab,
                         double *record_count, double *read_time, table_map *handled_fanout, sj_strategy_enum *strategy,
...
class Duplicate_weedout_picker : public Semi_join_strategy_picker
{
 ...
 public:
  ...
  bool check_qep(JOIN *join, uint idx, table_map remaining_tables, const JOIN_TAB *new_join_tab, double *record_count,
                 double *read_time, table_map *handled_fanout, sj_strategy_enum *stratey, POSITION *loose_scan_pos);
...
class Firstmatch_picker : public Semi_join_strategy_picker
{
 ...
 public:
  ...
  void set_from_prev(POSITION *prev);
  bool check_qep(JOIN *join, uint idx, table_map remaining_tables, const JOIN_TAB *new_join_tab, double *record_count,
                 double *read_time, table_map *handled_fanout, sj_strategy_enum *strategy, POSITION *loose_scan_pos);
...
class LooseScan_picker : public Semi_join_strategy_picker
{
  ...
  */
  ...
  friend void best_access_path(JOIN *join, JOIN_TAB *s, table_map remaining_tables, const POSITION *join_positions,
                               uint idx, bool disable_jbuf, double record_count, POSITION *pos,
...
class JOIN : public Sql_alloc
{
  ...
  */
  class Join_plan_state
  {
   ...
   public:
    Join_plan_state(uint tables) : error(0)
    {
      ...
      error = my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &best_positions, sizeof(*best_positions) * (tables + 1),
                              &join_tab_keyuse, sizeof(*join_tab_keyuse) * tables, &join_tab_checked_keys,
                              sizeof(*join_tab_checked_keys) * tables, &sj_mat_info, sizeof(sj_mat_info) * tables,
                              NullS) == 0;
    }
  ...
  enum_reopt_result reoptimize(Item *added_where, table_map join_tables, Join_plan_state *save_to);
  ...
  */
  uint total_join_tab_cnt() { return exec_join_tab_cnt() + aggr_tables - 1; }

  int prepare(TABLE_LIST *tables, COND *conds, uint og_num, ORDER *order, bool skip_order_by, ORDER *group,
              Item *having, ORDER *proc_param, SELECT_LEX *select, SELECT_LEX_UNIT *unit);
  bool prepare_stage2();
  int optimize();
  int optimize_inner();
  int optimize_stage2();
  ...
  Ref_ptr_array ref_ptr_array_slice(size_t slice_num)
  {
    ...
    return Ref_ptr_array(&select_lex->ref_pointer_array[slice_num * slice_sz], slice_sz);
  }
  ...
  {
    return (do_send_rows && implicit_grouping && !group_optimized_away && having_value != Item::COND_FALSE);
  }
  ...
  inline table_map all_tables_map() { return (table_map(1) << table_count) - 1; }
  ...
  */
  ...
  {
    return (need_tmp || !sort_by_table || skip_sort_order || ((group || tmp_table_param.sum_func_count) && !group_list))
               ? NULL
               : join_tab + const_tables;
  }
  bool setup_subquery_caches();
  bool shrink_join_buffers(JOIN_TAB *jt, ulonglong curr_space, ulonglong needed_space);
  ...
  */
  ...
  {
    return (
        (const_tables != table_count && ((select_distinct || !simple_order || !simple_group) || (group_list && order) ||
                                         MY_TEST(select_options & OPTION_BUFFER_RESULT))) ||
        (rollup.state != ROLLUP::STATE_NONE && select_distinct) || select_lex->have_window_funcs());
  }
  bool choose_subquery_plan(table_map join_tables);
  void get_partial_cost_and_fanout(int end_tab_idx, table_map filter_map, double *read_time_arg,
  ...
  bool is_in_subquery() { return (unit->item && unit->item->is_in_predicate()); }
  bool save_explain_data(Explain_query *output, bool can_overwrite, bool need_tmp_table, bool need_order,
                         bool distinct);
  int save_explain_data_intern(Explain_query *output, bool need_tmp_table, bool need_order, bool distinct,
                               const char *message);
  ...
  */
  bool create_postjoin_aggr_table(JOIN_TAB *tab, List<Item> *tmp_table_fields, ORDER *tmp_table_group,
                                  bool save_sum_fields, bool distinct, bool keep_row_ordermake);
  /**
...
enum enum_with_bush_roots
{
  WITH_BUSH_ROOTS,
  WITHOUT_BUSH_ROOTS
};
enum enum_with_const_tables
{
  WITH_CONST_TABLES,
...
bool setup_copy_fields(THD *thd, TMP_TABLE_PARAM *param, Ref_ptr_array ref_pointer_array, List<Item> &new_list1,
...
int opt_sum_query(THD *thd, List<TABLE_LIST> &tables, List<Item> &all_fields, COND *conds);
...
class store_key : public Sql_alloc
{
 public:
  ...
  enum Type
  {
    FIELD_STORE_KEY,
    ITEM_STORE_KEY,
    CONST_ITEM_STORE_KEY
  };
  store_key(THD *thd, Field *field_arg, uchar *ptr, uchar *null, uint length) : null_key(0), null_ptr(null), err(0)
  {
    to_field = field_arg->new_key_field(thd->mem_root, field_arg->table, ptr, length, null, 1);
  }
  store_key(store_key &arg)
      : Sql_alloc(), null_key(arg.null_key), to_field(arg.to_field), null_ptr(arg.null_ptr), err(arg.err)
...
class store_key_field : public store_key
{
 ...
 public:
  store_key_field(THD *thd, Field *to_field_arg, uchar *ptr, uchar *null_ptr_arg, uint length, Field *from_field,
                  const char *name_arg)
...
class store_key_item : public store_key
{
 ...
 public:
  store_key_item(THD *thd, Field *to_field_arg, uchar *ptr, uchar *null_ptr_arg, uint length, Item *item_arg, bool val)
      : store_key(thd, to_field_arg, ptr,
                  null_ptr_arg             ? null_ptr_arg
  ...
  store_key_item(store_key &arg, Item *new_item, bool val) : store_key(arg), item(new_item), use_value(val) {}
 ...
 protected:
  enum store_key_result copy_inner() override
  {
    ...
    */
    ...
    return ((err != 0 || res < 0 || res > 2) ? STORE_KEY_FATAL : (store_key_result)res);
  }
};
 ...
 public:
  store_key_const_item(THD *thd, Field *to_field_arg, uchar *ptr, uchar *null_ptr_arg, uint length, Item *item_arg)
      : store_key_item(thd, to_field_arg, ptr,
                       ...
                       : item_arg->maybe_null() ? &err
                                                : (uchar *)0,
                       length, item_arg, FALSE),
        inited(0)
  {
  }
  store_key_const_item(store_key &arg, Item *new_item) : store_key_item(arg, new_item, FALSE), inited(0) {}
...
int setup_order(THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables, List<Item> &fields,
                List<Item> &all_fields, ORDER *order, bool from_window_spec = false);
int setup_group(THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables, List<Item> &fields,
                List<Item> &all_fields, ORDER *order, bool *hidden_group_fields, bool from_window_spec = false);
bool fix_inner_refs(THD *thd, List<Item> &all_fields, SELECT_LEX *select, Ref_ptr_array ref_pointer_array);
int join_read_key2(THD *thd, struct st_join_table *tab, TABLE *table, struct st_table_ref *table_ref);

bool handle_select(THD *thd, LEX *lex, select_result *result, ulonglong setup_tables_done_option);
bool mysql_select(THD *thd, TABLE_LIST *tables, List<Item> &list, COND *conds, uint og_num, ORDER *order, ORDER *group,
                  Item *having, ORDER *proc_param, ulonglong select_type, select_result *result, SELECT_LEX_UNIT *unit,
                  SELECT_LEX *select_lex);
...
class Virtual_tmp_table : public TABLE
{
  ...
  */
  ...
  /**
    ...
    @return      false      - on success (idx[0] was set to the field index)
  */
  bool sp_find_field_by_name_or_error(uint *idx, const LEX_CSTRING &var_name, const LEX_CSTRING &field_name) const;
};
...
inline Virtual_tmp_table *create_virtual_tmp_table(THD *thd, List<Spvar_definition> &field_list)
{
  ...
  */
  DBUG_EXECUTE_IF("simulate_create_virtual_tmp_table_out_of_memory", DBUG_SET("+d,simulate_out_of_memory"););

  if (table->init(field_list.elements) || table->add(field_list) || table->open())
  {
    delete table;
    return NULL;
  }
...
/**
  ...
  @return !NULL - A pointer to the created table that is ready
                  for read and write.
*/
inline TABLE *create_virtual_tmp_table(THD *thd, Field *field)
{
  Virtual_tmp_table *table;
  DBUG_ASSERT(field);
  if (!(table = new (thd) Virtual_tmp_table(thd)))
    return NULL;
  if (table->init(1) || table->add(field) || table->open())
  {
    delete table;
    return NULL;
  }
...
extern bool test_if_ref(Item *, Item_field *left_item, Item *right_item);

inline bool optimizer_flag(THD *thd, ulonglong flag) { return (thd->variables.optimizer_switch & flag); }
...
bool const_expression_in_where(COND *cond, Item *comp_item, Field *comp_field = NULL, Item **const_item = NULL);
...
TABLE *create_tmp_table(THD *thd, TMP_TABLE_PARAM *param, List<Item> &fields, ORDER *group, bool distinct,
                        bool save_sum_fields, ulonglong select_options, ha_rows rows_limit, const LEX_CSTRING *alias,
                        bool do_not_open = FALSE, bool keep_row_order = FALSE);
TABLE *create_tmp_table_for_schema(THD *thd, TMP_TABLE_PARAM *param, const ST_SCHEMA_TABLE &schema_table,
                                   longlong select_options, const LEX_CSTRING &alias, bool do_not_open,
                                   bool keep_row_order);
...
class Pushdown_query : public Sql_alloc
{
 public:
  ...
  Pushdown_query(SELECT_LEX *select_lex_arg, group_by_handler *handler_arg)
      : select_lex(select_lex_arg), store_data_in_temp_table(0), handler(handler_arg), having(0)
  {
  }
