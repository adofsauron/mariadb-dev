...
Func_handler_date_add_interval_datetime_arg0_time func_handler_date_add_interval_datetime_arg0_time;
...
static bool extract_date_time(THD *thd, DATE_TIME_FORMAT *format, const char *val, uint length, MYSQL_TIME *l_time,
                              timestamp_type cached_timestamp_type, const char **sub_pattern_end,
                              const char *date_time_type, date_conv_mode_t fuzzydate)
{
    ...
    if (*ptr == '%' && ptr + 1 != end)
    {
      ...
      switch (*++ptr)
      {
        ...
        case 'c':
          ...
          break;
        case 'M':
          if ((l_time->month = check_word(my_locale_en_US.month_names, val, val_end, &val)) <= 0)
            goto err;
          break;
        case 'b':
          if ((l_time->month = check_word(my_locale_en_US.ab_month_names, val, val_end, &val)) <= 0)
            goto err;
          break;
          /* Day */
        case 'd':
        ...
        case 'w':
          tmp = (char *)val + 1;
          if (unlikely((weekday = (int)my_strtoll10(val, &tmp, &error)) < 0 || weekday >= 7))
            goto err;
        ...
        case 'u':
          ...
          if (unlikely((week_number = (int)my_strtoll10(val, &tmp, &error)) < 0 ||
        ...
        case 'r':
          ...
          */
          if (extract_date_time(thd, &time_ampm_format, val, (uint)(val_end - val), l_time, cached_timestamp_type, &val,
                                "time", fuzzydate))
            DBUG_RETURN(1);
          break;
        ...
        case 'T':
          if (extract_date_time(thd, &time_24hrs_format, val, (uint)(val_end - val), l_time, cached_timestamp_type,
                                &val, "time", fuzzydate))
            DBUG_RETURN(1);
          break;
  ...
  if (val != val_end)
  {
    do
    {
      if (!my_isspace(&my_charset_latin1, *val))
      {
        ...
        make_truncated_value_warning(thd, Sql_condition::WARN_LEVEL_WARN, &err, cached_timestamp_type, nullptr, nullptr,
                                     nullptr);
        break;
      }
    } while (++val != val_end);
...
err:
{
  ...
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_VALUE_FOR_TYPE,
                      ER_THD(thd, ER_WRONG_VALUE_FOR_TYPE), date_time_type, buff, "str_to_date");
}
  DBUG_RETURN(1);
}
...
static bool make_date_time(const String *format, const MYSQL_TIME *l_time, timestamp_type type, const MY_LOCALE *locale,
                           String *str)
{
  char intbuff[15];
  uint hours_i;
      ...
      switch (*++ptr)
      {
        case 'M':
          ...
          str->append(locale->month_names->type_names[l_time->month - 1],
                      (uint)strlen(locale->month_names->type_names[l_time->month - 1]), system_charset_info);
          break;
        case 'b':
          if (type == MYSQL_TIMESTAMP_TIME || !l_time->month)
            return 1;
          str->append(locale->ab_month_names->type_names[l_time->month - 1],
                      (uint)strlen(locale->ab_month_names->type_names[l_time->month - 1]), system_charset_info);
          break;
        case 'W':
          ...
          str->append(locale->day_names->type_names[weekday], (uint)strlen(locale->day_names->type_names[weekday]),
                      system_charset_info);
          break;
        case 'a':
          if (type == MYSQL_TIMESTAMP_TIME || !(l_time->month || l_time->year))
          ...
          str->append(locale->ab_day_names->type_names[weekday],
                      (uint)strlen(locale->ab_day_names->type_names[weekday]), system_charset_info);
          break;
        case 'D':
        ...
        case 'j':
          if (type == MYSQL_TIMESTAMP_TIME || !l_time->month || !l_time->year)
            return 1;
          length = (uint)(int10_to_str(
                              calc_daynr(l_time->year, l_time->month, l_time->day) - calc_daynr(l_time->year, 1, 1) + 1,
                              intbuff, 10) -
                          intbuff);
        ...
        case 'p':
          ...
          break;
        case 'r':
          length = sprintf(intbuff, ((l_time->hour % 24) < 12) ? "%02d:%02d:%02d AM" : "%02d:%02d:%02d PM",
        ...
        case 's':
          ...
          break;
        case 'T':
          length = sprintf(intbuff, "%02d:%02d:%02d", l_time->hour, l_time->minute, l_time->second);
          str->append(intbuff, length);
          break;
        case 'U':
        case 'u':
        {
          ...
          if (type == MYSQL_TIMESTAMP_TIME)
            return 1;
          length = (uint)(int10_to_str(calc_week(l_time, (*ptr) == 'U' ? WEEK_FIRST_WEEKDAY : WEEK_MONDAY_FIRST, &year),
                                       intbuff, 10) -
                          intbuff);
          str->append_with_prefill(intbuff, length, 2, '0');
        }
                              ...
                              bool transform_msec)
{
  ...
  for (i = 0; i < count; i++)
  {
    ...
    */
    ...
    for (value = 0; str != local_end && my_isdigit(cs, *str); str++) value = value * 10 + *str - '0';
    ...
    if (str == end && i != count - 1)
    {
      ...
      bmove_upp((uchar *)(values + count), (uchar *)(values + i), sizeof(*values) * i);
...
longlong Item_func_to_seconds::val_int_endpoint(bool left_endp, bool *incl_endp)
{
  DBUG_ASSERT(fixed());
...
enum_monotonicity_info Item_func_to_seconds::get_monotonicity_info() const
{
  if (args[0]->type() == Item::FIELD_ITEM)
  {
    if (args[0]->field_type() == MYSQL_TYPE_DATE || args[0]->field_type() == MYSQL_TYPE_DATETIME)
      return MONOTONIC_STRICT_INCREASING_NOT_NULL;
  }
  return NON_MONOTONIC;
}
...
longlong Item_func_to_days::val_int_endpoint(bool left_endp, bool *incl_endp)
{
  ...
  */
  ...
  if ((!left_endp && dt.hhmmssff_is_zero()) ||
      (left_endp && ltime.hour == 23 && ltime.minute == 59 && ltime.second == 59))
    /* do nothing */
    ;
  else
    *incl_endp = TRUE;
...
String *Item_func_monthname::val_str(String *str)
{
  ...
  str->copy(month_name, (uint)strlen(month_name), &my_charset_utf8mb3_bin, collation.collation, &err);
  return str;
}
...
longlong Item_func_yearweek::val_int()
{
  ...
  THD *thd = current_thd;
  Datetime d(thd, args[0], Datetime::Options(TIME_NO_ZEROS, thd));
  return (null_value = !d.is_valid_datetime()) ? 0 : d.yearweek((week_mode((uint)args[1]->val_int()) | WEEK_YEAR));
}
...
String *Item_func_dayname::val_str(String *str)
{
  ...
  str->copy(day_name, (uint)strlen(day_name), &my_charset_utf8mb3_bin, collation.collation, &err);
  return str;
}
...
{
  if (args[0]->type() == Item::FIELD_ITEM &&
      (args[0]->field_type() == MYSQL_TYPE_DATE || args[0]->field_type() == MYSQL_TYPE_DATETIME))
...
longlong Item_func_year::val_int_endpoint(bool left_endp, bool *incl_endp)
{
  ...
  */
  const MYSQL_TIME &ltime = dt.get_mysql_time()[0];
  if (!left_endp && ltime.day == 1 && ltime.month == 1 && dt.hhmmssff_is_zero())
    ; /* do nothing */
  else
    *incl_endp = TRUE;
  return ltime.year;
...
bool Item_func_unix_timestamp::get_timestamp_value(my_time_t *seconds, ulong *second_part)
{
  DBUG_ASSERT(fixed());
...
my_decimal *Item_func_unix_timestamp::decimal_op(my_decimal *buf)
{
  ...
  return seconds2my_decimal(seconds < 0, seconds < 0 ? -seconds : seconds, second_part, buf);
}

enum_monotonicity_info Item_func_unix_timestamp::get_monotonicity_info() const
{
  if (args[0]->type() == Item::FIELD_ITEM && (args[0]->field_type() == MYSQL_TYPE_TIMESTAMP))
    return MONOTONIC_INCREASING;
  return NON_MONOTONIC;
}
...
{
  DBUG_ASSERT(fixed());
  DBUG_ASSERT(arg_count == 1 && args[0]->type() == Item::FIELD_ITEM && args[0]->field_type() == MYSQL_TYPE_TIMESTAMP);
  Field *field = ((Item_field *)args[0])->field;
...
bool Item_func_from_days::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  if ((null_value = (args[0]->null_value || ((fuzzydate & TIME_NO_ZERO_DATE) && value == 0))))
    return true;
  bzero(ltime, sizeof(MYSQL_TIME));
  if (get_date_from_daynr((long)value, &ltime->year, &ltime->month, &ltime->day))
    return 0;
...
bool Item_func_curdate::get_date(THD *thd, MYSQL_TIME *res, date_mode_t fuzzydate __attribute__((unused)))
{
...
bool Item_func_curtime::fix_fields(THD *thd, Item **items)
{
  ...
  {
    my_error(ER_TOO_BIG_PRECISION, MYF(0), func_name(), TIME_SECOND_PART_DIGITS);
    return 1;
  }
  return Item_timefunc::fix_fields(thd, items);
}

bool Item_func_curtime::get_date(THD *thd, MYSQL_TIME *res, date_mode_t fuzzydate __attribute__((unused)))
{
  query_id_t query_id = thd->query_id;
  /* Cache value for this query */
  if (last_query_id != query_id)
...
{
  DBUG_ASSERT(item->decimals == AUTO_SEC_PART_DIGITS || item->decimals <= TIME_SECOND_PART_DIGITS);
  if (item->decimals)
  {
...
bool Item_func_now::get_date(THD *thd, MYSQL_TIME *res, date_mode_t fuzzydate __attribute__((unused)))
{
  query_id_t query_id = thd->query_id;
  /* Cache value for this query */
  if (last_query_id != query_id)
...
bool Item_func_sysdate_local::get_date(THD *thd, MYSQL_TIME *res, date_mode_t fuzzydate __attribute__((unused)))
{
...
bool Item_func_date_format::fix_length_and_dec(THD *thd)
{
  ...
  */
  ...
  {
    fixed_length = 0;
    max_length = MY_MIN(arg1->max_length, MAX_BLOB_WIDTH) * 10 * collation.collation->mbmaxlen;
...
String *Item_func_date_format::val_str(String *str)
{
  ...
  if ((null_value = args[0]->get_date(thd, &l_time, Temporal::Options(mode, thd))))
    return 0;
  ...
  if (!make_date_time(format, &l_time, is_time_format ? MYSQL_TIMESTAMP_TIME : MYSQL_TIMESTAMP_DATE, lc, str))
    return str;
...
static uint parse_special(char cfmt, const char *ptr, const char *end, uint16 *array)
{
  int offset = 0;
  char tmp1;
...
bool Item_func_tochar::parse_format_string(const String *format, uint *fmt_len)
{
  ...
  if (format->length() > MAX_DATETIME_FORMAT_MODEL_LEN)
  {
    warning_message.append(
        STRING_WITH_LEN("datetime format string is too "
                        "long"));
    return 1;
  }
  ...
  for (; ptr < end; ptr++, tmp_fmt++)
  {
    ...
    switch (cfmt)
    {
      ...
      case 'B':  // BC and B.C
        ...
        else if (next_char == '.' && ptr + 3 < end && my_toupper(system_charset_info, *(ptr + 2)) == 'C' &&
                 *(ptr + 3) == '.')
        {
          *tmp_fmt = FMT_BC_DOT;
          ptr += 3;
      ...
      case 'P':  // PM or P.M.
        ...
        else if (next_char == '.' && my_toupper(system_charset_info, *(ptr + 2)) == 'M' &&
      ...
      case 'Y':  // Y, YY, YYY o YYYYY
        ...
        if (ptr + 2 == end || my_toupper(system_charset_info, *(ptr + 2)) != 'Y') /* YY */
        {
          *tmp_fmt = FMT_YY;
          ulen = 2;
        }
      ...
      case 'M':
      {
        ...
        else if (tmp1 == 'O')
        {
          ...
          if (ptr + 4 >= end || my_toupper(system_charset_info, *(ptr + 3)) != 'T' ||
          ...
          {
            *tmp_fmt = FMT_MONTH;
            tmp_len += (locale->max_month_name_length * my_charset_utf8mb3_bin.mbmaxlen);
            ptr += 4;
          }
        }
        else
...
static bool make_date_time_oracle(const uint16 *fmt_array, const MYSQL_TIME *l_time, const MY_LOCALE *locale,
                                  String *str)
{
  bool quotation_flag = false;
    ...
    switch (*ptr)
    {
      ...
      case FMT_MONTH:
      {
        ...
        else
        {
          ...
          month_char_len = my_numchars_mb(&my_charset_utf8mb3_general_ci, month_name, month_name + month_byte_len);
          if (str->fill(str->length() + locale->max_month_name_length - month_char_len, ' '))
            goto err_exit;
        }
      }
      break;
      ...
      case FMT_DAY:
      {
        ...
        else
        {
          ...
          day_char_len = my_numchars_mb(&my_charset_utf8mb3_general_ci, day_name, day_name + day_byte_len);
          if (str->fill(str->length() + locale->max_day_name_length - day_char_len, ' '))
            goto err_exit;
        }
      }
      break;
...
bool Item_func_tochar::fix_length_and_dec(THD *thd)
{
  ...
  switch (arg0_mysql_type)
  {
    ...
    default:
    {
      my_printf_error(ER_STD_INVALID_ARGUMENT, ER(ER_STD_INVALID_ARGUMENT), MYF(0),
  ...
  if (args[1]->basic_const_item() && (str = args[1]->val_str(&buffer)))
  {
    ...
    {
      my_printf_error(ER_STD_INVALID_ARGUMENT, ER(ER_STD_INVALID_ARGUMENT), MYF(0), warning_message.c_ptr(),
                      func_name());
      return TRUE;
    }
  ...
  {
    fixed_length = 0;
    max_length = (uint32)MY_MIN(arg1->max_length * 10 * collation.collation->mbmaxlen, MAX_BLOB_WIDTH);
  }
  set_maybe_null();
  return FALSE;
}
...
String *Item_func_tochar::val_str(String *str)
{
  ...
  {
    uint ulen;
    if (!(format = args[1]->val_str(&format_buffer)) || !format->length() || parse_format_string(format, &ulen))
      goto null_date;
...
null_date:
  ...
  if (warning_message.length())
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_STD_INVALID_ARGUMENT,
                        ER_THD(thd, ER_STD_INVALID_ARGUMENT), warning_message.c_ptr(), func_name());
    if (!fixed_length)
...
bool Item_func_from_unixtime::fix_length_and_dec(THD *thd)
{
  ...
  Type_std_attributes::set(Type_temporal_attributes_not_fixed_dec(MAX_DATETIME_WIDTH, args[0]->decimals, false),
                           DTCollation_numeric());
  set_maybe_null();
  return FALSE;
}

bool Item_func_from_unixtime::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate __attribute__((unused)))
{
...
bool Item_func_convert_tz::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate __attribute__((unused)))
{
...
bool Item_date_add_interval::fix_length_and_dec(THD *thd)
{
  ...
  {
    my_error(ER_ILLEGAL_PARAMETER_DATA_TYPES2_FOR_OPERATION, MYF(0), args[0]->type_handler()->name().ptr(), "interval",
             func_name());
    return TRUE;
  }
  /*
...
bool Func_handler_date_add_interval_datetime_arg0_time::get_date(THD *thd, Item_handled_func *item, MYSQL_TIME *to,
                                                                 date_mode_t fuzzy) const
{
  // time_expr + INTERVAL {YEAR|QUARTER|MONTH|WEEK|YEAR_MONTH}
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_DATETIME_FUNCTION_OVERFLOW,
...
bool Item_date_add_interval::eq(const Item *item, bool binary_cmp) const
{
  ...
  return ((int_type == other->int_type) && (date_sub_interval == other->date_sub_interval));
}
...
bool Item_extract::check_arguments() const
{
  if (!args[0]->type_handler()->can_return_extract_source(int_type))
  {
    ...
    my_error(ER_ILLEGAL_PARAMETER_DATA_TYPE_FOR_OPERATION, MYF(0), args[0]->type_handler()->name().ptr(), tmp);
    return true;
  }
  return false;
}
...
{
  set_maybe_null();  // If wrong date
  uint32 daylen = args[0]->cmp_type() == TIME_RESULT ? 2 : TIME_MAX_INTERVAL_DAY_CHAR_LENGTH;
  switch (int_type)
  {
    case INTERVAL_YEAR:
      set_date_length(4);
    ...
    case INTERVAL_WEEK:
      ...
      break;  // 0..52
    case INTERVAL_DAY:
      set_day_length(daylen);
      break;  // DD
    case INTERVAL_DAY_HOUR:
      set_day_length(daylen + 2);
      break;  // DDhh
    case INTERVAL_DAY_MINUTE:
      set_day_length(daylen + 4);
      break;  // DDhhmm
    case INTERVAL_DAY_SECOND:
      set_day_length(daylen + 6);
      break;  // DDhhmmss
    case INTERVAL_HOUR:
      set_time_length(2);
      break;  // hh
    ...
    case INTERVAL_MICROSECOND:
      ...
      break;  // ffffff
    case INTERVAL_DAY_MICROSECOND:
      set_time_length(daylen + 12);
      break;  // DDhhmmssffffff
    case INTERVAL_HOUR_MICROSECOND:
      set_time_length(12);
      break;  // hhmmssffffff
    case INTERVAL_MINUTE_MICROSECOND:
...
longlong Item_extract::val_int()
{
  ...
  switch (int_type)
  {
    ...
    case INTERVAL_DAY_HOUR:
      return dt.day_hour();
    case INTERVAL_DAY_MINUTE:
      return dt.day_minute();
    case INTERVAL_DAY_SECOND:
      return dt.day_second();
    case INTERVAL_HOUR:
      return dt.hour();
    case INTERVAL_HOUR_MINUTE:
      return dt.hour_minute();
    case INTERVAL_HOUR_SECOND:
      return dt.hour_second();
    case INTERVAL_MINUTE:
      return dt.minute();
    case INTERVAL_MINUTE_SECOND:
      return dt.minute_second();
    case INTERVAL_SECOND:
      return dt.second();
    case INTERVAL_MICROSECOND:
      return dt.microsecond();
    case INTERVAL_DAY_MICROSECOND:
      return dt.day_microsecond();
    case INTERVAL_HOUR_MICROSECOND:
      return dt.hour_microsecond();
    case INTERVAL_MINUTE_MICROSECOND:
      return dt.minute_microsecond();
    case INTERVAL_SECOND_MICROSECOND:
      return dt.second_microsecond();
    case INTERVAL_LAST:
      DBUG_ASSERT(0);
      break; /* purecov: deadcode */
  }
...
void Item_char_typecast::check_truncation_with_warn(String *src, size_t dstlen)
{
  if (dstlen < src->length())
  {
    ...
    my_snprintf(char_type, sizeof(char_type), "%s(%lu)", cast_cs == &my_charset_bin ? "BINARY" : "CHAR",
                (ulong)cast_length);
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_TRUNCATED_WRONG_VALUE,
...
String *Item_char_typecast::copy(String *str, CHARSET_INFO *strcs)
{
  ...
  if (copier.copy_with_warn(cast_cs, &tmp_value, strcs, str->ptr(), str->length(), cast_length))
  {
    null_value = 1;  // EOM
    return 0;
  }
...
uint Item_char_typecast::adjusted_length_with_warn(uint length)
{
  ...
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                      ...
                      cast_cs == &my_charset_bin ? "cast_as_binary" : func_name(), thd->variables.max_allowed_packet);
...
String *Item_char_typecast::val_str_generic(String *str)
{
  ...
  if (cast_cs == &my_charset_bin && has_explicit_length() && cast_length > res->length())
  {
  ...
  return ((null_value = (res->length() > adjusted_length_with_warn(res->length())))) ? 0 : res;
}
...
String *Item_char_typecast::val_str_binary_from_native(String *str)
{
  ...
  return ((null_value = (str->length() > adjusted_length_with_warn(str->length())))) ? 0 : str;
}
...
class Item_char_typecast_func_handler : public Item_handled_func::Handler_str
{
 public:
  ...
  bool fix_length_and_dec(Item_handled_func *item) const { return false; }
...
{
  fix_length_and_dec_internal(from_cs = cast_cs->mbminlen == 1 ? cast_cs : &my_charset_latin1);
  set_func_handler(&item_char_typecast_func_handler);
}

void Item_char_typecast::fix_length_and_dec_generic()
{
  fix_length_and_dec_internal(from_cs = args[0]->dynamic_result() ? 0 : args[0]->collation.collation);
  set_func_handler(&item_char_typecast_func_handler);
}
...
void Item_char_typecast::fix_length_and_dec_native_to_binary(uint32 octet_length)
{
...
void Item_char_typecast::fix_length_and_dec_internal(CHARSET_INFO *from_cs)
{
  ...
  */
  ...
  char_length = ((cast_length != ~0U)
                     ? cast_length
                     : args[0]->max_length / (cast_cs == &my_charset_bin ? 1 : args[0]->collation.collation->mbmaxlen));
...
{
  Time *tm = new (to) Time(thd, args[0], Time::Options_for_cast(mode, thd), MY_MIN(decimals, TIME_SECOND_PART_DIGITS));
...
Sql_mode_dependency Item_time_typecast::value_depends_on_sql_mode() const
{
  return Item_timefunc::value_depends_on_sql_mode() |
         Sql_mode_dependency(decimals < args[0]->decimals ? MODE_TIME_ROUND_FRACTIONAL : 0, 0);
}
...
bool Item_datetime_typecast::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  Datetime *dt = new (ltime) Datetime(thd, args[0], opt, MY_MIN(decimals, TIME_SECOND_PART_DIGITS));
...
bool Item_func_add_time::fix_length_and_dec(THD *thd)
{
  ...
  if (!args[0]->type_handler()->is_traditional_scalar_type() || !args[1]->type_handler()->is_traditional_scalar_type())
  {
  ...
  */
  ...
  {
    set_func_handler(sign > 0 ? &func_handler_add_time_datetime_add : &func_handler_add_time_datetime_sub);
  }
  else if (arg0_field_type == MYSQL_TYPE_TIME)
  {
    set_func_handler(sign > 0 ? &func_handler_add_time_time_add : &func_handler_add_time_time_sub);
  }
  else
  {
    set_func_handler(sign > 0 ? &func_handler_add_time_string_add : &func_handler_add_time_string_sub);
  }
...
bool Item_func_timediff::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  if (args[0]->get_time(thd, &l_time1) || args[1]->get_time(thd, &l_time2) || l_time1.time_type != l_time2.time_type)
    return (null_value = 1);
...
bool Item_func_maketime::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  if (args[0]->null_value || args[1]->null_value || sec.is_null() || minute < 0 || minute > 59 || sec.neg() ||
      sec.truncated())
    return (null_value = 1);

  int warn;
  new (ltime)
      Time(&warn, hour.neg(), hour.abs(), (uint)minute, sec.to_const_sec9(), thd->temporal_round_mode(), decimals);
  if (warn)
  {
    ...
    int len = (int)(ptr - buf) + sprintf(ptr, ":%02u:%02u", (uint)minute, (uint)sec.sec());
...
longlong Item_func_microsecond::val_int()
{
  ...
  THD *thd = current_thd;
  Time tm(thd, args[0], Time::Options_for_cast(thd));
  return ((null_value = !tm.is_valid_time())) ? 0 : tm.get_mysql_time()->second_part;
}
...
String *Item_func_get_format::val_str_ascii(String *str)
{
  ...
  for (format = &known_date_time_formats[0]; (format_name = format->format_name); format++)
  {
    ...
    if (val_len == format_name_len && !my_charset_latin1.strnncoll(val->ptr(), val_len, format_name, val_len))
    {
...
static const Item_handled_func::Handler *get_date_time_result_type(const char *format, uint length)
{
...
bool Item_func_str_to_date::fix_length_and_dec(THD *thd)
{
  ...
  if ((const_item = args[1]->const_item()))
  {
    ...
    String *format = args[1]->val_str(&format_str, &format_converter, internal_charset);
...
bool Item_func_str_to_date::get_date_common(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate, timestamp_type tstype)
{
  ...
  if (extract_date_time(thd, &date_time_format, val->ptr(), val->length(), ltime, tstype, 0, "datetime",
                        date_conv_mode_t(fuzzydate) | sql_mode_for_dates(thd)))
    return (null_value = 1);
  return (null_value = 0);
}
...
{
  Datetime::Options opt(date_conv_mode_t(fuzzydate & ~TIME_TIME_ONLY), time_round_mode_t(fuzzydate));
