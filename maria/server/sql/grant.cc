...
bool Grant_privilege::add_column_privilege(THD *thd, const Lex_ident_sys &name, privilege_t which_grant)
{
  String *new_str = new (thd->mem_root) String((const char *)name.str, name.length, system_charset_info);
  ...
  while ((point = iter++))
  {
    if (!my_strcasecmp(system_charset_info, point->column.c_ptr(), new_str->c_ptr()))
      break;
  }
...
bool Grant_privilege::add_column_list_privilege(THD *thd, List<Lex_ident_sys> &list, privilege_t privilege)
{
  Lex_ident_sys *col;
...
privilege_t Grant_object_name::all_privileges_by_type() const
{
  ...
  {
    case STAR:
      return DB_ACLS & ~GRANT_ACL;
    case IDENT_STAR:
      return DB_ACLS & ~GRANT_ACL;
    case STAR_STAR:
      return GLOBAL_ACLS & ~GRANT_ACL;
    case TABLE_IDENT:
      return TABLE_ACLS & ~GRANT_ACL;
  }
  return NO_ACL;
}

bool Grant_privilege::set_object_name(THD *thd, const Grant_object_name &ident, SELECT_LEX *sel,
                                      privilege_t with_grant_option)
{
  ...
  switch (ident.m_type)
  {
    ...
    case Lex_grant_object_name::TABLE_IDENT:
      ...
      return !sel->add_table_to_list(thd, ident.m_table_ident, NULL, TL_OPTION_UPDATING);
  }
