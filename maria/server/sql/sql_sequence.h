  ...
  inline void print_dbug()
  {
    DBUG_PRINT(
        ...
        ("reserved: %lld  start: %lld  increment: %lld  min_value: %lld  max_value: %lld  cache: %lld  round: %lld",
         reserved_until, start, increment, min_value, max_value, cache, round));
  }
...
class SEQUENCE : public sequence_definition
{
 public:
  ...
  int set_value(TABLE *table, longlong next_value, ulonglong round_arg, bool is_used);
  longlong increment_value(longlong value)
  {
    if (real_increment > 0)
    {
      if (value > max_value - real_increment || value + real_increment > max_value)
        value = max_value + 1;
      else
    ...
    {
      if (value + real_increment < min_value || value < min_value - real_increment)
        value = min_value - 1;
      else
        value += real_increment;
    }
...
{
 public:
  SEQUENCE_LAST_VALUE(uchar *key_arg, uint length_arg) : key(key_arg), length(length_arg) {}
  ~SEQUENCE_LAST_VALUE() { my_free((void *)key); }
