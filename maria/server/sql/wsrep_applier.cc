...
*/
static Log_event *wsrep_read_log_event(char **arg_buf, size_t *arg_buf_len,
...
void wsrep_store_error(const THD *const thd, wsrep::mutable_buffer &dst)
{
  ...
  for (cond = it++; cond && slider < buf_end; cond = it++)
  {
    ...
    slider += my_snprintf(slider, buf_end - slider, " %s, Error_code: %d;", err_str, err_code);
  }
  ...
  WSREP_DEBUG("Error buffer for thd %llu seqno %lld, %zu bytes: '%s'", thd->thread_id,
              (long long)wsrep_thd_trx_seqno(thd), dst.size(), dst.size() ? dst.data() : "(null)");
}

int wsrep_apply_events(THD *thd, Relay_log_info *rli, const void *events_buf, size_t buf_len)
{
  ...
  if (!buf_len)
    WSREP_DEBUG("empty rbr buffer to apply: %lld", (long long)wsrep_thd_trx_seqno(thd));
  ...
  {
    int exec_res;
    Log_event *ev = wsrep_read_log_event(&buf, &buf_len, wsrep_get_apply_format(thd));
    if (!ev)
    {
      WSREP_ERROR("applier could not read binlog event, seqno: %lld, len: %zu", (long long)wsrep_thd_trx_seqno(thd),
                  buf_len);
    ...
    switch (typ)
    {
      ...
      case GTID_EVENT:
      {
        ...
        if ((gtid_ev->server_id == wsrep_gtid_server.server_id) && (gtid_ev->domain_id == wsrep_gtid_server.domain_id))
        {
    ...
    if (!thd->variables.gtid_seq_no && wsrep_thd_is_toi(thd) && (ev->get_type_code() == QUERY_EVENT))
    {
    ...
    if (exec_res)
    {
      WSREP_WARN("Event %d %s apply failed: %d, seqno %lld", event, ev->get_type_str(), exec_res,
