...
static uint add_table_data_fields_to_join_cache(JOIN_TAB *tab, MY_BITMAP *field_set, uint *field_cnt,
                                                CACHE_FIELD **descr, uint *field_ptr_cnt, CACHE_FIELD ***descr_ptr)
{
  Field **fld_ptr;
  uint len = 0;
...
void JOIN_CACHE::collect_info_on_key_args()
{
  ...
  do
  {
    for (tab = cache->start_tab; tab != cache->join_tab; tab = next_linear_tab(join, tab, WITHOUT_BUSH_ROOTS))
    {
      uint key_args;
...
int JOIN_CACHE::alloc_fields()
{
  ...
  field_descr = (CACHE_FIELD *)join->thd->alloc(fields_size + sizeof(CACHE_FIELD *) * ptr_cnt);
...
void JOIN_CACHE::create_flag_fields()
{
  ...
  if (with_match_flag)
    length += add_flag_field_to_join_cache((uchar *)&join_tab->found, sizeof(join_tab->found), &copy);

  /* Create fields for all null bitmaps and null row flags that are needed */
  for (tab = start_tab; tab != join_tab; tab = next_linear_tab(join, tab, WITHOUT_BUSH_ROOTS))
  {
    TABLE *table = tab->table;

    /* Create a field for the null bitmap from table if needed */
    if (tab->used_null_fields || tab->used_uneven_bit_fields)
      length += add_flag_field_to_join_cache(table->null_flags, table->s->null_bytes, &copy);

    /* Create table for the null row flag if needed */
    if (table->maybe_null)
      length += add_flag_field_to_join_cache((uchar *)&table->null_row, sizeof(table->null_row), &copy);
  }
...
void JOIN_CACHE::create_key_arg_fields()
{
  ...
  */
  ...
  while (ext_key_arg_cnt)
  {
    ...
    for (tab = cache->start_tab; tab != cache->join_tab; tab = next_linear_tab(join, tab, WITHOUT_BUSH_ROOTS))
    {
      ...
      for (copy = cache->field_descr + cache->flag_fields; copy < copy_end; copy++)
      {
        ...
        */
        if (copy->field &&  // (1)
            copy->field->table == tab->table && bitmap_is_set(key_read_set, copy->field->field_index))
        {
          *copy_ptr++ = copy;
          ext_key_arg_cnt--;
  ...
  for (tab = start_tab; tab != join_tab; tab = next_linear_tab(join, tab, WITHOUT_BUSH_ROOTS))
  {
    length += add_table_data_fields_to_join_cache(tab, &tab->table->tmp_set, &data_field_count, &copy,
...
void JOIN_CACHE::create_remaining_fields()
{
  ...
  for (tab = start_tab; tab != join_tab; tab = next_linear_tab(join, tab, WITHOUT_BUSH_ROOTS))
  {
    ...
    length += add_table_data_fields_to_join_cache(tab, rem_field_set, &data_field_count, &copy, &data_field_ptr_count,
                                                  &copy_ptr);
...
void JOIN_CACHE::set_constants()
{
  ...
  */
  with_length = is_key_access() || join_tab->is_inner_table_of_semi_join_with_first_match() ||
  ...
  */
  uint len = length + fields * sizeof(uint) + blobs * sizeof(uchar *) +
             (prev_cache ? prev_cache->get_size_of_rec_offset() : 0) + sizeof(ulong);
  /*
  ...
  */
  ...
  base_prefix_length = (with_length ? size_of_rec_len : 0) + (prev_cache ? prev_cache->get_size_of_rec_offset() : 0);
  pack_length = (with_length ? size_of_rec_len : 0) + (prev_cache ? prev_cache->get_size_of_rec_offset() : 0) + length +
                fields * sizeof(uint);
  pack_length_with_blob_ptrs = pack_length + blobs * sizeof(uchar *);
  min_buff_size = 0;
  min_records = 1;
  buff_size = (size_t)MY_MAX(join->thd->variables.join_buff_size, get_min_join_buffer_size());
...
{
  uint len = get_prefix_length() + MY_TEST(with_match_flag) + size_of_fld_ofs * data_field_count;
  return len;
}
...
size_t JOIN_CACHE::get_min_join_buffer_size()
{
  if (!min_buff_size)
  {
    size_t len = 0;
    size_t len_last = 0;
    for (JOIN_TAB *tab = start_tab; tab != join_tab; tab = next_linear_tab(join, tab, WITHOUT_BUSH_ROOTS))
    {
      len += tab->get_max_used_fieldlength();
      len_last += tab->get_used_fieldlength();
    }
    size_t len_addon = get_record_max_affix_length() + get_max_key_addon_space_per_record();
    len += len_addon;
    len_last += len_addon;
    size_t min_sz = len * (min_records - 1) + len_last;
    min_sz += pack_length_with_blob_ptrs;
    size_t add_sz = 0;
    for (uint i = 0; i < min_records; i++) add_sz += join_tab_scan->aux_buffer_incr(i + 1);
...
int JOIN_CACHE::alloc_buffer()
{
  ...
  ulonglong join_buff_space_limit = join->thd->variables.join_buff_space_limit;
  bool optimize_buff_size = optimizer_flag(join->thd, OPTIMIZER_SWITCH_OPTIMIZE_JOIN_BUFFER_SIZE);
  double partial_join_cardinality = (join_tab - 1)->get_partial_join_cardinality();
  buff = NULL;
  min_buff_size = 0;
  max_buff_size = 0;
  min_records = 1;
  max_records = (size_t)(partial_join_cardinality <= join_buff_space_limit ? (ulonglong)partial_join_cardinality
  ...
  if (curr_min_buff_space_sz > join_buff_space_limit ||
      (curr_buff_space_sz > join_buff_space_limit &&
       (!optimize_buff_size || join->shrink_join_buffers(join_tab, curr_buff_space_sz, join_buff_space_limit))))
    goto fail;
  ...
  for (size_t buff_size_decr = (buff_size - min_buff_size) / 4 + 1;;)
  {
    ...
    if ((buff = (uchar *)my_malloc(key_memory_JOIN_CACHE, buff_size, MYF(MY_THREAD_SPECIFIC))))
      break;

    next_buff_size = buff_size > buff_size_decr ? buff_size - buff_size_decr : 0;
    if (next_buff_size < min_buff_size ||
        join->shrink_join_buffers(join_tab, curr_buff_space_sz, curr_buff_space_sz - buff_size_decr))
      goto fail;
...
{
  free();
  buff = (uchar *)my_malloc(key_memory_JOIN_CACHE, buff_size, MYF(MY_THREAD_SPECIFIC));
  reset(TRUE);
  return buff == NULL;
}
...
bool JOIN_CACHE::check_emb_key_usage()
{
  ...
  */
  ...
  for (; copy < copy_end; copy++)
  {
    ...
    */
    if (copy->field->type() == MYSQL_TYPE_BIT && ((Field_bit *)(copy->field))->bit_len)
      return FALSE;
    len += copy->length;
  }
  ...
  */
    ...
    switch (copy->type)
    {
      case CACHE_BLOB:
      {
        ...
        if (last_record)
        {
          ...
          blob_field->get_image(cp, copy->length + sizeof(char *), blob_field->charset());
  ...
  if (referenced_fields)
  {
    ...
    for (copy = field_descr + flag_fields; copy < copy_end; copy++)
    {
      if (copy->referenced_field_no)
      {
        store_fld_offset(cp + size_of_fld_ofs * (copy->referenced_field_no - 1), copy->offset);
        cnt++;
      }
    }
...
enum JOIN_CACHE::Match_flag JOIN_CACHE::get_match_flag_by_pos_from_join_buffer(uchar *rec_ptr, JOIN_TAB *tab)
{
...
uint JOIN_CACHE::aux_buffer_incr(size_t recno) { return join_tab_scan->aux_buffer_incr(recno); }
  ...
  switch (copy->type)
  {
    case CACHE_BLOB:
    {
      ...
      */
      if (blob_in_rec_buff)
      {
        blob_field->set_image(pos, copy->length + sizeof(char *), blob_field->charset());
...
bool JOIN_CACHE::read_referenced_field(CACHE_FIELD *copy, uchar *rec_ptr, uint *len)
{
  ...
  offset = get_fld_offset(ptr + *len - size_of_fld_ofs * (referenced_fields + 1 - copy->referenced_field_no));
  bool is_null = FALSE;
  Field *field = copy->field;
...
bool JOIN_CACHE::skip_if_matched()
{
  ...
  if (get_match_flag_by_pos_from_join_buffer(pos + offset, join_tab->first_unmatched) == MATCH_FOUND)
  {
...
bool JOIN_CACHE::skip_if_not_needed_match()
{
  ...
  skip = join_tab->first_sj_inner_tab ? match_fl == MATCH_FOUND :  // the case of semi-join
...
finish:
  if (outer_join_first_inner && join_tab->first_inner == join_tab->first_unmatched)
  {
    /*
    ...
    */
    for (tab = join_tab->first_inner; tab <= join_tab->last_inner; tab++) tab->first_unmatched = 0;
  }
  restore_last_record();
  reset(TRUE);
...
enum_nested_loop_state JOIN_CACHE::join_matching_records(bool skip_last)
{
  ...
  bool check_only_first_match =
      join_tab->check_only_first_match() && (!join_tab->first_inner ||                             // semi-join case
  ...
  */
  ...
  while (!(error = join_tab_scan->next()))
  {
    ...
    while ((rec_ptr = get_next_candidate_for_match()))
    {
      ...
      */
      if ((!check_only_first_match && !outer_join_first_inner) || !skip_next_candidate_for_match(rec_ptr))
      {
...
enum_nested_loop_state JOIN_CACHE::generate_full_extensions(uchar *rec_ptr)
{
  ...
  */
  if (check_match(rec_ptr))
  {
    ...
    if (!join_tab->check_weed_out_table || !(res = join_tab->check_weed_out_table->sj_weedout_check_row(join->thd)))
    {
...
inline bool JOIN_CACHE::check_match(uchar *rec_ptr)
{
  ...
  */
  ...
  do
  {
    ...
    if (first_inner->check_only_first_match() && !join_tab->first_inner)
      DBUG_RETURN(TRUE);
    /*
  ...
  } while ((first_inner = first_inner->first_upper) && first_inner->last_inner == join_tab);
  DBUG_RETURN(TRUE);
}
...
THD *JOIN_CACHE::thd() { return join->thd; }

static bool add_mrr_explain_info(String *str, uint mrr_mode, handler *file)
{
  char mrr_str_buf[128] = {0};
  int len;
  len = file->multi_range_read_explain_info(mrr_mode, mrr_str_buf, sizeof(mrr_str_buf));
  if (len > 0)
  {
    if (str->length())
    {
...
int JOIN_CACHE_HASHED::init(bool for_explain)
{
  ...
  rec_fields_offset =
      get_size_of_rec_offset() + get_size_of_rec_length() + (prev_cache ? prev_cache->get_size_of_rec_offset() : 0);
...
int JOIN_CACHE_HASHED::init_hash_table()
{
  ...
  for (size_of_key_ofs = 2; size_of_key_ofs <= max_size_of_key_ofs; size_of_key_ofs += 2)
  {
    ...
    size_t space_per_rec = avg_record_length + avg_aux_buffer_incr + key_entry_length + size_of_key_ofs;
    ...
    */
    size_t max_n = buff_size / (pack_length - length + key_entry_length + size_of_key_ofs);

    hash_entries = (uint)(n / 0.7);
    set_if_bigger(hash_entries, 1);

    if (offset_size((uint)(max_n * key_entry_length)) <= size_of_key_ofs)
      break;
  }
...
uint JOIN_CACHE_HASHED::get_max_key_addon_space_per_record()
{
  ...
  */
  len = (use_emb_key ? get_size_of_rec_offset() : ref->key_length) + size_of_rec_ofs +  // size of the key chain header
...
bool JOIN_CACHE_HASHED::key_search(uchar *key, uint key_len, uchar **key_ref_ptr)
{
  bool is_found = FALSE;
  ...
  while (!is_null_key_ref(ref_ptr))
  {
    ...
    next_key = use_emb_key ? get_emb_key(ref_ptr - get_size_of_rec_offset()) : ref_ptr - key_length;
...
inline uint JOIN_CACHE_HASHED::get_hash_idx_simple(uchar *key, uint key_len)
{
  ulong nr = 1;
  ulong nr2 = 4;
  uchar *pos = key;
...
inline uint JOIN_CACHE_HASHED::get_hash_idx_complex(uchar *key, uint key_len)
{
  return (uint)(key_hashnr(ref_key_info, ref_used_key_parts, key) % hash_entries);
}
...
inline bool JOIN_CACHE_HASHED::equal_keys_simple(uchar *key1, uchar *key2, uint key_len)
{
...
inline bool JOIN_CACHE_HASHED::equal_keys_complex(uchar *key1, uchar *key2, uint key_len)
{
...
static void save_or_restore_used_tabs(JOIN_TAB *join_tab, bool save)
{
  JOIN_TAB *first = join_tab->bush_root_tab ? join_tab->bush_root_tab->bush_children->start
  ...
  for (JOIN_TAB *tab = join_tab - 1; tab != first && !tab->cache; tab--)
  {
    if (tab->bush_children)
    {
      for (JOIN_TAB *child = tab->bush_children->start; child != tab->bush_children->end; child++)
      {
        if (save)
...
void JOIN_TAB_SCAN::close() { save_or_restore_used_tabs(join_tab, TRUE); }
...
uchar *JOIN_CACHE_BNLH::get_next_candidate_for_match()
{
  ...
  next_matching_rec_ref_ptr =
      get_next_rec_ref(next_matching_rec_ref_ptr ? next_matching_rec_ref_ptr : last_matching_rec_ref_ptr);
...
{
  return join_tab->check_only_first_match() && (get_match_flag_by_pos(rec_ptr) == MATCH_FOUND);
}
...
void JOIN_CACHE_BNLH::read_next_candidate_for_match(uchar *rec_ptr) { get_record_by_pos(rec_ptr); }
...
uint JOIN_TAB_SCAN_MRR::aux_buffer_incr(size_t recno)
{
  ...
  ha_rows rec_per_key = (ha_rows)tab->key_info[ref->key].actual_rec_per_key(ref->key_parts - 1);
...
int JOIN_TAB_SCAN_MRR::open()
{
  ...
  */
  ...
  return file->multi_range_read_init(&range_seq_funcs, (void *)cache, ranges, mrr_mode, &mrr_buff);
}
...
static void bka_range_seq_key_info(void *init_params, uint *length, key_part_map *map)
{
...
static bool bka_range_seq_next(range_seq_t rseq, KEY_MULTI_RANGE *range)
{
...
static bool bka_range_seq_skip_record(range_seq_t rseq, range_id_t range_info, uchar *rowid)
{
  DBUG_ENTER("bka_range_seq_skip_record");
  JOIN_CACHE_BKA *cache = (JOIN_CACHE_BKA *)rseq;
  bool res = cache->get_match_flag_by_pos((uchar *)range_info) == JOIN_CACHE::MATCH_FOUND;
  DBUG_RETURN(res);
}
...
static bool bka_skip_index_tuple(range_seq_t rseq, range_id_t range_info)
{
...
void JOIN_CACHE_BKA::read_next_candidate_for_match(uchar *rec_ptr) { get_record_by_pos(rec_ptr); }
...
int JOIN_CACHE_BKA::init(bool for_explain)
{
  ...
  RANGE_SEQ_IF rs_funcs = {bka_range_seq_key_info, bka_range_seq_init, bka_range_seq_next,
                           check_only_first_match ? bka_range_seq_skip_record : 0, bka_skip_index_tuple};

  DBUG_ENTER("JOIN_CACHE_BKA::init");

  JOIN_TAB_SCAN_MRR *jsm;
  if (!(join_tab_scan = jsm = new JOIN_TAB_SCAN_MRR(join, join_tab, mrr_mode, rs_funcs)))
    DBUG_RETURN(1);
...
start:
  ...
  if (with_match_flag && (Match_flag)curr_rec_pos[0] == MATCH_IMPOSSIBLE)
  {
  ...
  else
  {
    ...
    if (external_key_arg_fields)
    {
      ...
      for (cache = prev_cache; key_arg_count; cache = cache->prev_cache)
      {
        ...
        while (key_arg_count && cache->read_referenced_field(*copy_ptr, rec_ptr, &len))
        {
          copy_ptr++;
          --key_arg_count;
        }
...
static range_seq_t bkah_range_seq_init(void *init_param, uint n_ranges, uint flags)
{
...
static bool bkah_range_seq_next(range_seq_t rseq, KEY_MULTI_RANGE *range)
{
...
static bool bkah_range_seq_skip_record(range_seq_t rseq, range_id_t range_info, uchar *rowid)
{
...
static bool bkah_skip_index_tuple(range_seq_t rseq, range_id_t range_info)
{
...
{
  last_matching_rec_ref_ptr = next_matching_rec_ref_ptr = 0;
  if (no_association && !(curr_matching_chain = get_matching_chain_by_join_key()))  // psergey: added '!'
    return 1;
  last_matching_rec_ref_ptr = get_next_rec_ref(curr_matching_chain);
  return 0;
}
...
int JOIN_CACHE_BKAH::init(bool for_explain)
{
  ...
  RANGE_SEQ_IF rs_funcs = {bka_range_seq_key_info, bkah_range_seq_init, bkah_range_seq_next,
                           check_only_first_match && !no_association ? bkah_range_seq_skip_record : 0,
                           bkah_skip_index_tuple};

  DBUG_ENTER("JOIN_CACHE_BKAH::init");

  if (!(join_tab_scan = new JOIN_TAB_SCAN_MRR(join, join_tab, mrr_mode, rs_funcs)))
    DBUG_RETURN(1);
