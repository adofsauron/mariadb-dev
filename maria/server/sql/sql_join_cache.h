...
class JOIN_CACHE : public Sql_alloc
{
 ...
 protected:
  ...
  uint offset_size(size_t len) { return (len < 256 ? 1 : len < 256 * 256 ? 2 : 4); }
  ...
  */
  ...
  uchar *get_rec_ref(uchar *ptr) { return buff + get_offset(size_of_rec_ofs, ptr - size_of_rec_ofs); }
  ulong get_rec_length(uchar *ptr) { return (ulong)get_offset(size_of_rec_len, ptr); }
  ulong get_fld_offset(uchar *ptr) { return (ulong)get_offset(size_of_fld_ofs, ptr); }

  void store_rec_ref(uchar *ptr, uchar *ref)
  {
    store_offset(size_of_rec_ofs, ptr - size_of_rec_ofs, (ulong)(ref - buff));
  }
  void store_rec_length(uchar *ptr, ulong len) { store_offset(size_of_rec_len, ptr, len); }
  void store_fld_offset(uchar *ptr, ulong ofs) { store_offset(size_of_fld_ofs, ptr, ofs); }
  ...
  */
  ...
  virtual size_t rem_space() { return MY_MAX(buff_size - (end_pos - buff) - aux_buff_size, 0); }
 ...
 */
  ...
  */
  ...
  /*
    ...
    record buffers
  */
  bool blob_data_is_in_rec_buff(uchar *rec_ptr) { return rec_ptr == last_rec_pos && last_rec_blob_data_is_in_rec_buff; }
  ...
  */
  virtual enum Match_flag get_match_flag_by_pos_from_join_buffer(uchar *rec_ptr, JOIN_TAB *tab);
  ...
  virtual uchar *get_curr_rec_link() { return (curr_rec_link ? curr_rec_link : get_curr_rec()); }
...
class JOIN_CACHE_HASHED : public JOIN_CACHE
{
  ...
  typedef bool (JOIN_CACHE_HASHED::*Hash_cmp_func)(uchar *key1, uchar *key2, uint key_len);
  ...
  */
  uchar *get_next_key_ref(uchar *key_ref_ptr) { return hash_table - get_offset(size_of_key_ofs, key_ref_ptr); }
  ...
  */
  ...
  uchar *get_next_rec_ref(uchar *ref_ptr) { return buff + get_offset(get_size_of_rec_offset(), ref_ptr); }
  ...
  */
  uchar *get_curr_emb_key() { return get_curr_rec() + data_fields_offset; }
  ...
  */
  uchar *get_emb_key(uchar *ref_ptr) { return buff + get_offset(get_size_of_rec_offset(), ref_ptr); }
  ...
  */
  ...
  uint get_prefix_length() { return base_prefix_length + get_size_of_rec_offset(); }
  ...
  */
  size_t rem_space() { return MY_MAX(last_key_entry - end_pos - aux_buff_size, 0); }
  ...
  */
  ...
  /*
    ...
    cache object to which this cache is linked.
  */
  JOIN_CACHE_HASHED(JOIN *j, JOIN_TAB *tab, JOIN_CACHE *prev) : JOIN_CACHE(j, tab, prev) {}
  ...
  */
  ...
  uchar *get_curr_key_chain() { return get_next_rec_ref(curr_key_entry + key_entry_length - get_size_of_rec_offset()); }
};
...
class JOIN_CACHE_BNL : public JOIN_CACHE
{
  ...
  */
  ...
  /*
    ...
    cache object to which this cache is linked.
  */
  JOIN_CACHE_BNL(JOIN *j, JOIN_TAB *tab, JOIN_CACHE *prev) : JOIN_CACHE(j, tab, prev) {}
...
class JOIN_CACHE_BNLH : public JOIN_CACHE_HASHED
{
  ...
  */
  ...
  /*
    ...
    cache object to which this cache is linked.
  */
  JOIN_CACHE_BNLH(JOIN *j, JOIN_TAB *tab, JOIN_CACHE *prev) : JOIN_CACHE_HASHED(j, tab, prev) {}
...
class JOIN_TAB_SCAN_MRR : public JOIN_TAB_SCAN
{
  ...
  virtual void init_mrr_buff() { cache->setup_aux_buffer(mrr_buff); }
...
class JOIN_CACHE_BKA : public JOIN_CACHE
{
  ...
  */
  JOIN_CACHE_BKA(JOIN *j, JOIN_TAB *tab, uint flags) : JOIN_CACHE(j, tab), mrr_mode(flags) {}
  /*
    ...
    The MRR mode initially is set to 'flags'.
  */
  JOIN_CACHE_BKA(JOIN *j, JOIN_TAB *tab, uint flags, JOIN_CACHE *prev) : JOIN_CACHE(j, tab, prev), mrr_mode(flags) {}

  JOIN_CACHE_BKA(JOIN_CACHE_BKA *bka) : JOIN_CACHE(bka->join, bka->join_tab, bka->prev_cache), mrr_mode(bka->mrr_mode)
  {
  }
...
class JOIN_CACHE_BKAH : public JOIN_CACHE_BNLH
{
 ...
 public:
  /*
    ...
    The MRR mode initially is set to 'flags'.
  */
  JOIN_CACHE_BKAH(JOIN *j, JOIN_TAB *tab, uint flags) : JOIN_CACHE_BNLH(j, tab), mrr_mode(flags) {}
  ...
  /*
    ...
    The MRR mode initially is set to 'flags'.
  */
  JOIN_CACHE_BKAH(JOIN *j, JOIN_TAB *tab, uint flags, JOIN_CACHE *prev) : JOIN_CACHE_BNLH(j, tab, prev), mrr_mode(flags)
  {
  }

  JOIN_CACHE_BKAH(JOIN_CACHE_BKAH *bkah)
      : JOIN_CACHE_BNLH(bkah->join, bkah->join_tab, bkah->prev_cache), mrr_mode(bkah->mrr_mode)
  {
  }
