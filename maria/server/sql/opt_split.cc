...
bool JOIN::check_for_splittable_materialized()
{
  ...
  else if (select_lex->have_window_funcs() && select_lex->window_specs.elements == 1)  // (7.2)
  {
    partition_list = select_lex->window_specs.head()->partition_list->first;
  }
  if (!partition_list)
    return false;
  ...
  */
  ...
  for (JOIN_TAB *tab = join_tab; tab < join_tab + top_join_tab_count; tab++)
  {
    ...
    for (i = 0; i < table->s->keys; i++)
    {
      ...
      for (; usable_kp_cnt < key_parts; usable_kp_cnt++)
      {
        ...
        {
          if (cand->underlying_field->table == table && cand->underlying_field->field_index + 1 == fldnr)
          {
  ...
  */
  ...
  SplM_field_info *spl_field = (SplM_field_info *)(thd->calloc(sizeof(SplM_field_info) * spl_field_cnt));
  ...
  for (cand = cand_start; cand < cand_end; cand++)
  {
    ...
    spl_opt_info->tables_usable_for_splitting |= cand->underlying_field->table->map;
    spl_field++;
  }
  ...
  */
  if (derived && derived->is_materialized_derived() && derived->embedding && derived->embedding->sj_subq_pred)
...
void TABLE::add_splitting_info_for_key_field(KEY_FIELD *key_field)
{
  ...
  */
  ...
  {
    right_item->walk(&Item::set_fields_as_dependent_processor, false, join->select_lex);
    right_item->update_used_tables();
    eq_item = new (thd->mem_root) Item_func_eq(thd, left_item, right_item);
  }
  if (!eq_item)
    return;
  KEY_FIELD *added_key_field = (KEY_FIELD *)thd->alloc(sizeof(KEY_FIELD));
  if (!added_key_field || spl_opt_info->added_key_fields.push_back(added_key_field, thd->mem_root))
    return;
  ...
  if ((real->type() == Item::FIELD_ITEM) && ((Item_field *)real)->field->maybe_null())
...
static bool add_ext_keyuse_for_splitting(Dynamic_array<KEYUSE_EXT> *ext_keyuses, KEY_FIELD *added_key_field, uint key,
                                         uint part)
{
...
static int sort_ext_keyuse(KEYUSE_EXT *a, KEYUSE_EXT *b)
{
...
static void sort_ext_keyuses(Dynamic_array<KEYUSE_EXT> *keyuses)
{
  KEYUSE_EXT *first_keyuse = &keyuses->at(0);
  my_qsort(first_keyuse, keyuses->elements(), sizeof(KEYUSE_EXT), (qsort_cmp)sort_ext_keyuse);
}
...
static bool add_ext_keyuses_for_splitting_field(Dynamic_array<KEYUSE_EXT> *ext_keyuses, KEY_FIELD *added_key_field)
{
  Field *field = added_key_field->field;
...
static double spl_postjoin_oper_cost(THD *thd, double join_record_count, uint rec_len)
{
  double cost;
  cost = get_tmp_table_write_cost(thd, join_record_count, rec_len) * join_record_count;  // cost to fill tmp table
  ...
  cost += get_tmp_table_lookup_cost(thd, join_record_count, rec_len) +
          (join_record_count == 0 ? 0 : join_record_count * log2(join_record_count)) *
...
void JOIN::add_keyuses_for_splitting()
{
  ...
  {
    (void)add_ext_keyuses_for_splitting_field(ext_keyuses_for_splitting, added_key_field);
  }
  ...
  spl_opt_info->unsplit_cost = best_positions[table_count - 1].read_time + oper_cost;
  ...
  if (!keyuse.buffer &&
      my_init_dynamic_array(PSI_INSTRUMENT_ME, &keyuse, sizeof(KEYUSE), 20, 64, MYF(MY_THREAD_SPECIFIC)))
    goto err;
  ...
  if (keyuse.elements)
    memcpy(keyuse.buffer, save_qep->keyuse.buffer, (size_t)keyuse.elements * keyuse.size_of_element);
...
SplM_plan_info *SplM_opt_info::find_plan(TABLE *table, uint key, uint parts)
{
  ...
  while ((spl_plan = li++))
  {
    if (spl_plan->table == table && spl_plan->key == key && spl_plan->parts == parts)
      break;
  }
  return spl_plan;
}
...
static void reset_validity_vars_for_keyuses(KEYUSE_EXT *key_keyuse_ext_start, TABLE *table, uint key,
                                            table_map remaining_tables, bool validity_val)
{
...
SplM_plan_info *JOIN_TAB::choose_best_splitting(double record_count, table_map remaining_tables)
{
  SplM_opt_info *spl_opt_info = table->spl_opt_info;
  DBUG_ASSERT(spl_opt_info != NULL);
  JOIN *join = spl_opt_info->join;
  THD *thd = join->thd;
  table_map tables_usable_for_splitting = spl_opt_info->tables_usable_for_splitting;
  ...
  for (uint tablenr = 0; tablenr < join->table_count; tablenr++)
  {
    ...
    do
    {
      ...
      do
      {
        ...
        {
          if ((!found_parts && !keyuse_ext->keypart) || (found_parts && ((keyuse_ext->keypart_map >> 1) & found_parts)))
        ...
        KEY *key_info = table->key_info + key;
        double rec_per_key = key_info->actual_rec_per_key(keyuse_ext->keypart);
    ...
    */
    ...
    if (!spl_plan)
    {
      ...
      */
      ...
      reset_validity_vars_for_keyuses(best_key_keyuse_ext_start, best_table, best_key, remaining_tables, true);
      ...
      */
      ...
      if (!(first_non_const_pos->key && spl_keys.is_set(first_non_const_pos->key->key)) ||
          !(spl_plan = (SplM_plan_info *)thd->alloc(sizeof(SplM_plan_info))) ||
          !(spl_plan->best_positions = (POSITION *)thd->alloc(sizeof(POSITION) * join->table_count)) ||
          spl_opt_info->plan_cache.push_back(spl_plan))
      {
        reset_validity_vars_for_keyuses(best_key_keyuse_ext_start, best_table, best_key, remaining_tables, false);
        return 0;
      }
      ...
      spl_plan->split_sel = best_rec_per_key / (spl_opt_info->unsplit_card ? spl_opt_info->unsplit_card : 1);

      uint rec_len = table->s->rec_buff_length;

      double split_card = spl_opt_info->unsplit_card * spl_plan->split_sel;
      double oper_cost = split_card * spl_postjoin_oper_cost(thd, split_card, rec_len);
      spl_plan->cost = join->best_positions[join->table_count - 1].read_time + +oper_cost;
      ...
      memcpy((char *)spl_plan->best_positions, (char *)join->best_positions, sizeof(POSITION) * join->table_count);
      reset_validity_vars_for_keyuses(best_key_keyuse_ext_start, best_table, best_key, remaining_tables, false);
    }
    if (spl_plan)
    {
...
bool JOIN_TAB::fix_splitting(SplM_plan_info *spl_plan, table_map remaining_tables, bool is_const_table)
{
  SplM_opt_info *spl_opt_info = table->spl_opt_info;
  ...
  {
    memcpy((char *)md_join->best_positions, (char *)spl_plan->best_positions, sizeof(POSITION) * md_join->table_count);
    /*
      This is called for a proper work of JOIN::get_best_combination()
      called for the join that materializes T
    */
    reset_validity_vars_for_keyuses(spl_plan->keyuse_ext_start, spl_plan->table, spl_plan->key, remaining_tables, true);
  }
...
bool JOIN::fix_all_splittings_in_plan()
{
  ...
  for (uint tablenr = 0; tablenr < table_count; tablenr++)
  {
    ...
    if (tab->table->is_splittable())
    {
      ...
      if (tab->fix_splitting(spl_plan, all_tables & ~prev_tables, tablenr < const_tables))
        return true;
    }
...
bool JOIN::inject_splitting_cond_for_all_tables_with_split_opt()
{
  ...
  for (uint tablenr = 0; tablenr < table_count; tablenr++)
  {
    ...
    */
    if (join->inject_best_splitting_cond((all_tables & ~prev_tables) | sjm_lookup_tables))
      return true;
  }
  return false;
}
