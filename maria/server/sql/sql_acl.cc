...
static LEX_CSTRING native_password_plugin_name = {STRING_WITH_LEN("mysql_native_password")};

static LEX_CSTRING old_password_plugin_name = {STRING_WITH_LEN("mysql_old_password")};
...
class ACL_ACCESS
{
 public:
  ulonglong sort;
  privilege_t access;
  ACL_ACCESS() : sort(0), access(NO_ACL) {}
};
...
{
 public:
  ACL_USER_BASE() : flags(0), user(null_clex_str) { bzero(&role_grants, sizeof(role_grants)); }
...
{
 public:
  ACL_USER_PARAM() { bzero(this, sizeof(*this)); }
  acl_host_and_ip host;
  ...
  struct AUTH
  {
    LEX_CSTRING plugin, auth_string, salt;
  } * auth;
  uint nauth;
  bool account_locked;
  ...
  bool alloc_auth(MEM_ROOT *root, uint n) { return !(auth = (AUTH *)alloc_root(root, (nauth = n) * sizeof(AUTH))); }
};
...
class ACL_USER : public ACL_USER_BASE, public ACL_USER_PARAM
{
 public:
  ACL_USER() {}
  ACL_USER(THD *thd, const LEX_USER &combo, const Account_options &options, const privilege_t privileges);

  ACL_USER *copy(MEM_ROOT *root)
  {
    ACL_USER *dst;
    AUTH *dauth;
    if (!multi_alloc_root(root, &dst, sizeof(ACL_USER), &dauth, sizeof(AUTH) * nauth, NULL))
      return 0;
    *dst = *this;
    ...
    {
      if (auth[i].plugin.str == native_password_plugin_name.str || auth[i].plugin.str == old_password_plugin_name.str)
...
ulong role_global_merges = 0, role_db_merges = 0, role_table_merges = 0, role_column_merges = 0,
      role_routine_merges = 0;
#endif

#ifndef NO_EMBEDDED_ACCESS_CHECKS
static void update_hostname(acl_host_and_ip *host, const char *hostname);
static bool show_proxy_grants(THD *, const char *, const char *, char *, size_t);
static bool show_role_grants(THD *, const char *, ACL_USER_BASE *, char *, size_t);
static bool show_default_role(THD *, ACL_USER *, char *, size_t);
static bool show_global_privileges(THD *, ACL_USER_BASE *, bool, char *, size_t);
static bool show_database_privileges(THD *, const char *, const char *, char *, size_t);
static bool show_table_and_column_privileges(THD *, const char *, const char *, char *, size_t);
static int show_routine_grants(THD *, const char *, const char *, const Sp_handler *sph, char *, int);
...
class ACL_PROXY_USER : public ACL_ACCESS
{
 ...
 public:
  ...
  void init(const char *host_arg, const char *user_arg, const char *proxied_host_arg, const char *proxied_user_arg,
            bool with_grant_arg)
  {
    user = user_arg;
    update_hostname(&host, (host_arg && *host_arg) ? host_arg : NULL);
    proxied_user = proxied_user_arg;
    update_hostname(&proxied_host, (proxied_host_arg && *proxied_host_arg) ? proxied_host_arg : NULL);
    with_grant = with_grant_arg;
    sort = get_magic_sort("huhu", host.hostname, user, proxied_host.hostname, proxied_user);
  }

  void init(MEM_ROOT *mem, const char *host_arg, const char *user_arg, const char *proxied_host_arg,
            const char *proxied_user_arg, bool with_grant_arg)
  {
    init((host_arg && *host_arg) ? strdup_root(mem, host_arg) : NULL, strdup_root(mem, user_arg),
         (proxied_host_arg && *proxied_host_arg) ? strdup_root(mem, proxied_host_arg) : NULL,
         strdup_root(mem, proxied_user_arg), with_grant_arg);
  }
  ...
  void set_user(MEM_ROOT *mem, const char *user_arg) { user = *user_arg ? strdup_root(mem, user_arg) : ""; }
  void set_host(MEM_ROOT *mem, const char *host_arg) { update_hostname(&host, safe_strdup_root(mem, host_arg)); }

  bool check_validity(bool check_no_resolve)
  {
    if (check_no_resolve &&
        (hostname_requires_resolving(host.hostname) || hostname_requires_resolving(proxied_host.hostname)))
    {
      sql_print_warning(
          "'proxies_priv' entry '%s@%s %s@%s' "
          "ignored in --skip-name-resolve mode.",
          proxied_user, safe_str(proxied_host.hostname), user, safe_str(host.hostname));
      return TRUE;
    }
    return FALSE;
  }
  ...
  bool matches(const char *host_arg, const char *user_arg, const char *ip_arg, const char *proxied_user_arg)
  {
    ...
    DBUG_PRINT("info", ("compare_hostname(%s,%s,%s) &&"
                        ...
                        "wild_compare (%s,%s) &&"
                        "wild_compare (%s,%s)",
                        host.hostname, host_arg, ip_arg, proxied_host.hostname, host_arg, ip_arg, user_arg, user,
                        proxied_user_arg, proxied_user));
    DBUG_RETURN(compare_hostname(&host, host_arg, ip_arg) && compare_hostname(&proxied_host, host_arg, ip_arg) &&
                (!*user || !strcmp(user_arg, user)) && (!*proxied_user || !strcmp(proxied_user_arg, proxied_user)));
  }
  ...
  bool pk_equals(ACL_PROXY_USER *grant)
  {
    ...
    DBUG_PRINT("info", ("strcmp(%s,%s) &&"
                        ...
                        "wild_compare (%s,%s) &&"
                        "wild_compare (%s,%s)",
                        user, grant->user, proxied_user, grant->proxied_user, host.hostname, grant->host.hostname,
                        proxied_host.hostname, grant->proxied_host.hostname));

    bool res = auth_element_equals(user, grant->user) && auth_element_equals(proxied_user, grant->proxied_user) &&
               auth_element_equals(host.hostname, grant->host.hostname) &&
               auth_element_equals(proxied_host.hostname, grant->proxied_host.hostname);
    DBUG_RETURN(res);
  }

  bool granted_on(const char *host_arg, const char *user_arg)
  {
    return (!strcmp(user, user_arg) && ((!host.hostname && (!host_arg || !host_arg[0])) ||
  ...
  static int store_pk(TABLE *table, const LEX_CSTRING *host, const LEX_CSTRING *user, const LEX_CSTRING *proxied_host,
                      const LEX_CSTRING *proxied_user)
  {
    DBUG_ENTER("ACL_PROXY_USER::store_pk");
    DBUG_PRINT("info", ("host=%s, user=%s, proxied_host=%s, proxied_user=%s", host->str, user->str, proxied_host->str,
                        proxied_user->str));
    if (table->field[MYSQL_PROXIES_PRIV_HOST]->store(host->str, host->length, system_charset_info))
      DBUG_RETURN(TRUE);
    if (table->field[MYSQL_PROXIES_PRIV_USER]->store(user->str, user->length, system_charset_info))
      DBUG_RETURN(TRUE);
    if (table->field[MYSQL_PROXIES_PRIV_PROXIED_HOST]->store(proxied_host->str, proxied_host->length,
                                                             system_charset_info))
      DBUG_RETURN(TRUE);
    if (table->field[MYSQL_PROXIES_PRIV_PROXIED_USER]->store(proxied_user->str, proxied_user->length,
                                                             system_charset_info))
      DBUG_RETURN(TRUE);
  ...
  static int store_data_record(TABLE *table, const LEX_CSTRING *host, const LEX_CSTRING *user,
                               const LEX_CSTRING *proxied_host, const LEX_CSTRING *proxied_user, bool with_grant,
                               const char *grantor)
  {
    DBUG_ENTER("ACL_PROXY_USER::store_pk");
    ...
    if (table->field[MYSQL_PROXIES_PRIV_WITH_GRANT]->store(with_grant ? 1 : 0, TRUE))
      DBUG_RETURN(TRUE);
    if (table->field[MYSQL_PROXIES_PRIV_GRANTOR]->store(grantor, strlen(grantor), system_charset_info))
      DBUG_RETURN(TRUE);
...
static uchar *acl_role_get_key(ACL_ROLE *entry, size_t *length, my_bool not_used __attribute__((unused)))
{
...
struct ROLE_GRANT_PAIR : public Sql_alloc
{
  ...
  bool init(MEM_ROOT *mem, const char *username, const char *hostname, const char *rolename, bool with_admin_option);
};

static uchar *acl_role_map_get_key(ROLE_GRANT_PAIR *entry, size_t *length, my_bool not_used __attribute__((unused)))
{
  *length = (uint)entry->hashkey.length;
  return (uchar *)entry->hashkey.str;
}

bool ROLE_GRANT_PAIR::init(MEM_ROOT *mem, const char *username, const char *hostname, const char *rolename,
...
static int traverse_role_graph_up(ACL_ROLE *, void *, int (*)(ACL_ROLE *, void *),
                                  int (*)(ACL_ROLE *, ACL_ROLE *, void *));

static int traverse_role_graph_down(ACL_USER_BASE *, void *, int (*)(ACL_USER_BASE *, void *),
                                    int (*)(ACL_USER_BASE *, ACL_ROLE *, void *));

HASH *Sp_handler_procedure::get_priv_hash() const { return &proc_priv_hash; }

HASH *Sp_handler_function::get_priv_hash() const { return &func_priv_hash; }

HASH *Sp_handler_package_spec::get_priv_hash() const { return &package_spec_priv_hash; }

HASH *Sp_handler_package_body::get_priv_hash() const { return &package_body_priv_hash; }
...
static LEX_CSTRING MYSQL_TABLE_NAME[USER_TABLE + 1] = {{STRING_WITH_LEN("db")},
...
{
  if (thd->variables.old_passwords == 1 || password_len == SCRAMBLED_PASSWORD_CHAR_LENGTH_323)
  ...
  bool init_read_record(READ_RECORD *info) const
  {
    ...
    if (num_fields() < min_columns)
    {
      my_printf_error(ER_UNKNOWN_ERROR,
                      "Fatal error: mysql.%s table is "
                      "damaged or in unsupported 3.20 format",
                      MYF(ME_ERROR_LOG), m_table->s->table_name.str);
      return 1;
    }

    bool result = ::init_read_record(info, m_table->in_use, m_table, NULL, NULL, 1, true, false);
    if (!result)
  ...
  */
  void set_table(TABLE *table)
  {
    ...
    for (end_priv_columns = 0; end_priv_columns < num_fields(); end_priv_columns++)
    {
      ...
      if (field->real_type() == MYSQL_TYPE_ENUM && static_cast<Field_enum *>(field)->typelib->count == 2)
      {
...
{
 public:
  bool init_read_record(READ_RECORD *info) const { return Grant_table_base::init_read_record(info) || setup_sysvars(); }
  ...
  char *get_host(MEM_ROOT *root) const { return ::get_field(root, m_table->field[0]); }
  int set_host(const char *s, size_t l) const { return m_table->field[0]->store(s, l, system_charset_info); };
  char *get_user(MEM_ROOT *root) const { return ::get_field(root, m_table->field[1]); }
  int set_user(const char *s, size_t l) const { return m_table->field[1]->store(s, l, system_charset_info); };
...
class User_table_tabular : public User_table
{
 public:
    ...
    if (plugin() && authstr())
    {
      ...
      if (tmpstr)
      {
        ...
        if (*pw)
        {
          ...
          {
            sql_print_warning(
                "'user' entry '%s@%s' has both a password and an "
  ...
  bool set_auth(const ACL_USER &u) const
  {
    ...
    {
      if (u.auth->plugin.str != native_password_plugin_name.str && u.auth->plugin.str != old_password_plugin_name.str)
        return 1;
  ...
  privilege_t get_access() const
  {
    ...
    if (num_fields() <= 18)
    {
      ...
      if (access & FILE_ACL)
        access |= BINLOG_MONITOR_ACL | REPL_SLAVE_ACL | BINLOG_ADMIN_ACL | BINLOG_REPLAY_ACL;
  ...
  */
  ...
  {
    username_char_length = MY_MIN(m_table->field[1]->char_length(), USERNAME_CHAR_LENGTH);
    using_global_priv_table = false;

    if (have_password())  // Password column might be missing. (MySQL 5.7.6+)
    {
      int password_length = password()->field_length / password()->charset()->mbmaxlen;
...
class User_table_json : public User_table
{
  ...
  int get_auth(THD *thd, MEM_ROOT *root, ACL_USER *u) const
  {
    ...
    if (json_get_array_item(array, array + array_len, (int)array_len, &v, &vl) != JSV_NOTHING)
      return 1;
    ...
    for (uint i = 0; i < u->nauth; i++)
    {
      ...
      {
        case JSV_STRING:
          u->auth[i].plugin.str = strmake_root(root, p, pl);
          u->auth[i].plugin.length = pl;
          break;
        case JSV_NOTHING:
          if (get_auth1(thd, root, u, i))
            return 1;
          else
            continue;
        default:
      ...
      {
        case JSV_NOTHING:
          u->auth[i].auth_string = empty_clex_str;
          break;
        case JSV_STRING:
          u->auth[i].auth_string.str = strmake_root(root, a, al);
  ...
  bool append_str_value(String *to, const LEX_CSTRING &str) const
  {
    ...
    int len = json_escape(system_charset_info, (uchar *)str.str, (uchar *)str.str + str.length, to->charset(),
  ...
  bool set_auth(const ACL_USER &u) const
  {
    ...
    for (uint i = 0; i < u.nauth; i++)
    {
      ...
      if (i)
        json.append(',');
      json.append('{');
      if (!top_done && (auth->plugin.str == native_password_plugin_name.str ||
                        auth->plugin.str == old_password_plugin_name.str || i == u.nauth - 1))
      {
        if (set_auth1(u, i))
          return 1;
        top_done = true;
  ...
  {
    return set_str_value("plugin", u.auth[i].plugin.str, u.auth[i].plugin.length) ||
           set_str_value("authentication_string", u.auth[i].auth_string.str, u.auth[i].auth_string.length);
  }
  ...
  void print_warning_bad_version_id(ulonglong version_id) const
  {
    sql_print_warning("'user' entry '%s@%s' has a wrong 'version_id' value %lld",
                      safe_str(get_user(current_thd->mem_root)), safe_str(get_host(current_thd->mem_root)), version_id);
  }

  void print_warning_bad_access(ulonglong version_id, privilege_t mask, ulonglong access) const
  {
    sql_print_warning(
        "'user' entry '%s@%s' "
        "has a wrong 'access' value 0x%llx "
        "(allowed mask is 0x%llx, version_id=%lld)",
        safe_str(get_user(current_thd->mem_root)), safe_str(get_host(current_thd->mem_root)), access, mask, version_id);
  }
  ...
  privilege_t get_access() const
  {
    ...
    */
    if ((longlong)version_id < 0 || version_id > 999999 || (version_id > 0 && version_id < 100400))
    {
  ...
  const char *unsafe_str(const char *s) const { return s[0] ? s : NULL; }

  SSL_type get_ssl_type() const { return (SSL_type)get_int_value("ssl_type"); }
  int set_ssl_type(SSL_type x) const { return set_int_value("ssl_type", x); }
  const char *get_ssl_cipher(MEM_ROOT *root) const { return unsafe_str(get_str_value(root, "ssl_cipher")); }
  int set_ssl_cipher(const char *s, size_t l) const { return set_str_value("ssl_cipher", s, l); }
  const char *get_x509_issuer(MEM_ROOT *root) const { return unsafe_str(get_str_value(root, "x509_issuer")); }
  int set_x509_issuer(const char *s, size_t l) const { return set_str_value("x509_issuer", s, l); }
  const char *get_x509_subject(MEM_ROOT *root) const { return unsafe_str(get_str_value(root, "x509_subject")); }
  int set_x509_subject(const char *s, size_t l) const { return set_str_value("x509_subject", s, l); }
  longlong get_max_questions() const { return get_int_value("max_questions"); }
  int set_max_questions(longlong x) const { return set_int_value("max_questions", x); }
  longlong get_max_updates() const { return get_int_value("max_updates"); }
  int set_max_updates(longlong x) const { return set_int_value("max_updates", x); }
  longlong get_max_connections() const { return get_int_value("max_connections"); }
  int set_max_connections(longlong x) const { return set_int_value("max_connections", x); }
  longlong get_max_user_connections() const { return get_int_value("max_user_connections"); }
  int set_max_user_connections(longlong x) const { return set_int_value("max_user_connections", x); }
  double get_max_statement_time() const { return get_double_value("max_statement_time"); }
  int set_max_statement_time(double x) const { return set_double_value("max_statement_time", x); }
  bool get_is_role() const { return get_bool_value("is_role"); }
  int set_is_role(bool x) const { return set_bool_value("is_role", x); }
  const char *get_default_role(MEM_ROOT *root) const { return get_str_value(root, "default_role"); }
  int set_default_role(const char *s, size_t l) const { return set_str_value("default_role", s, l); }
  bool get_account_locked() const { return get_bool_value("account_locked"); }
  int set_account_locked(bool x) const { return set_bool_value("account_locked", x); }
  my_time_t get_password_last_changed() const { return static_cast<my_time_t>(get_int_value("password_last_changed")); }
  int set_password_last_changed(my_time_t x) const
  {
    return set_int_value("password_last_changed", static_cast<longlong>(x));
  }
  int set_password_lifetime(longlong x) const { return set_int_value("password_lifetime", x); }
  longlong get_password_lifetime() const { return get_int_value("password_lifetime", -1); }
  /*
     ...
     in mysql.user
  */
  bool get_password_expired() const { return get_int_value("password_last_changed", -1) == 0; }
  int set_password_expired(bool x) const { return x ? set_password_last_changed(0) : 0; }
 ...
 private:
  ...
  bool get_value(const char *key, enum json_types vt, const char **v, size_t *vl) const
  {
    enum json_types value_type;
    int int_vl;
    String str, *res = m_table->field[2]->val_str(&str);
    if (!res || (value_type = json_get_object_key(res->ptr(), res->end(), key, v, &int_vl)) == JSV_BAD_JSON)
      return 1;  // invalid
    *vl = int_vl;
  ...
  const char *get_str_value(MEM_ROOT *root, const char *key) const
  {
    ...
    int len =
        json_unescape(m_table->field[2]->charset(), (const uchar *)value_start, (const uchar *)value_start + value_len,
                      system_charset_info, (uchar *)ptr, (uchar *)ptr + value_len);
    if (len < 0)
      return NULL;
  ...
  enum json_types set_value(const char *key, const char *val, size_t vlen, bool string) const
  {
    int value_len;
    const char *value_start;
    enum json_types value_type;
    ...
    value_type = json_get_object_key(res->ptr(), res->end(), key, &value_start, &value_len);
  ...
  {
    char buf[JSON_SIZE];
    int blen = json_escape(system_charset_info, (const uchar *)val, (const uchar *)val + vlen,
                           m_table->field[2]->charset(), (uchar *)buf, (uchar *)buf + sizeof(buf));
    if (blen < 0)
      return 1;
...
class Grant_tables
{
 public:
   ...
   */
  int build_table_list(THD *thd, TABLE_LIST **ptr_first, int which_tables, enum thr_lock_type lock_type,
                       TABLE_LIST *tables)
  {
    ...
    for (int i = USER_TABLE; i >= 0; i--)
    {
      ...
      if (which_tables & (1 << i))
      {
        tl->init_one_table(&MYSQL_SCHEMA_NAME, &MYSQL_TABLE_NAME[i], NULL, lock_type);
  ...
  int open_and_lock(THD *thd, int which_tables, enum thr_lock_type lock_type)
  {
    ...
    if (!res && (which_tables & Table_user) && !tables[USER_TABLE].table)
    {
      ...
      tl->init_one_table(&MYSQL_SCHEMA_NAME, &MYSQL_TABLE_NAME_USER, NULL, lock_type);
      tl->open_type = OT_BASE_ONLY;
      tl->i_s_requested_object = OPEN_TABLE_ONLY;
      tl->updating = lock_type >= TL_FIRST_WRITE;
    ...
    if (lock_tables(thd, first, counter, MYSQL_LOCK_IGNORE_TIMEOUT | MYSQL_OPEN_IGNORE_LOGGING_FORMAT))
      DBUG_RETURN(-1);
  ...
  inline const User_table &user_table() const { return *p_user_table; }

  inline const Db_table &db_table() const { return m_db_table; }

  inline const Tables_priv_table &tables_priv_table() const { return m_tables_priv_table; }

  inline const Columns_priv_table &columns_priv_table() const { return m_columns_priv_table; }

  inline const Host_table &host_table() const { return m_host_table; }

  inline const Procs_priv_table &procs_priv_table() const { return m_procs_priv_table; }

  inline const Proxies_priv_table &proxies_priv_table() const { return m_proxies_priv_table; }

  inline const Roles_mapping_table &roles_mapping_table() const { return m_roles_mapping_table; }
  ...
  */
  int rpl_ignore_tables(THD *thd, TABLE_LIST *tables, int which_tables = 0, enum thr_lock_type lock_type = TL_IGNORE)
  {
    ...
    if (!tables)
    {
      int rc __attribute__((unused)) = build_table_list(thd, &tables, which_tables, lock_type, all_tables);
...
void ACL_PROXY_USER::init(const Proxies_priv_table &proxies_priv_table, MEM_ROOT *mem)
{
  init(get_field(mem, proxies_priv_table.host()), safe_str(get_field(mem, proxies_priv_table.user())),
       get_field(mem, proxies_priv_table.proxied_host()), safe_str(get_field(mem, proxies_priv_table.proxied_user())),
...
static void free_acl_user(ACL_USER *user) { delete_dynamic(&(user->role_grants)); }
...
static my_bool check_if_exists(THD *, plugin_ref, void *) { return TRUE; }

static bool has_validation_plugins()
{
  return plugin_foreach(NULL, check_if_exists, MariaDB_PASSWORD_VALIDATION_PLUGIN, NULL);
}

struct validation_data
{
  const LEX_CSTRING *user, *password, *host;
};

static my_bool do_validate(THD *, plugin_ref plugin, void *arg)
{
  struct validation_data *data = (struct validation_data *)arg;
  struct st_mariadb_password_validation *handler = (st_mariadb_password_validation *)plugin_decl(plugin)->info;
...
static bool validate_password(THD *thd, const LEX_CSTRING &user, const LEX_CSTRING &host, const LEX_CSTRING &pwtext,
                              bool has_hash)
{
  if (pwtext.length || !has_hash)
  {
    struct validation_data data = {&user, pwtext.str ? &pwtext : &empty_clex_str, &host};
    if (plugin_foreach(NULL, do_validate, MariaDB_PASSWORD_VALIDATION_PLUGIN, &data))
    {
      return true;
    }
  }
  ...
  {
    if (!thd->slave_thread && strict_password_validation && has_validation_plugins()
#ifdef WITH_WSREP
        && !thd->wsrep_applier
#endif
    )
...
static int set_user_salt(ACL_USER::AUTH *auth, plugin_ref plugin)
{
  ...
  if (info->interface_version >= 0x0202 && info->preprocess_hash && auth->auth_string.length)
  {
    uchar buf[MAX_SCRAMBLE_LENGTH];
    size_t len = sizeof(buf);
    if (info->preprocess_hash(auth->auth_string.str, auth->auth_string.length, buf, &len))
      return 1;
...
*/
static int set_user_auth(THD *thd, const LEX_CSTRING &user, const LEX_CSTRING &host, ACL_USER::AUTH *auth,
                         const LEX_CSTRING &pwtext)
{
  ...
  if (!plugin)
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_PLUGIN_IS_NOT_LOADED,
  ...
  if (info->hash_password && validate_password(thd, user, host, pwtext, auth->auth_string.length))
  {
...
*/
static bool set_user_salt_if_needed(ACL_USER *user_copy, int curr_auth, plugin_ref plugin)
{
...
bool acl_init(bool dont_read_acl_tables)
{
  ...
  acl_cache = new Hash_filo<acl_entry>(key_memory_acl_cache, ACL_CACHE_SIZE, 0, 0, (my_hash_get_key)acl_entry_get_key,
                                       (my_hash_free_key)my_free, &my_charset_utf8mb3_bin);

  /*
    cache built-in native authentication plugins,
    to avoid hash searches and a global mutex lock on every connect
  */
  native_password_plugin = my_plugin_lock_by_name(0, &native_password_plugin_name, MYSQL_AUTHENTICATION_PLUGIN);
  old_password_plugin = my_plugin_lock_by_name(0, &old_password_plugin_name, MYSQL_AUTHENTICATION_PLUGIN);
...
static bool acl_load(THD *thd, const Grant_tables &tables)
{
  ...
  if (host_table.table_exists())  // "host" table may not exist (e.g. in MySQL 5.6.7+)
  {
    ...
    while (!(read_record_info.read_record()))
    {
      ...
      if (check_no_resolve && hostname_requires_resolving(host.host.hostname))
      {
        sql_print_warning(
            "'host' entry '%s|%s' "
            "ignored in --skip-name-resolve mode.",
            safe_str(host.host.hostname), safe_str(host.db));
        continue;
      }
    ...
    my_qsort((uchar *)dynamic_element(&acl_hosts, 0, ACL_HOST *), acl_hosts.elements, sizeof(ACL_HOST),
  ...
  while (!(read_record_info.read_record()))
  {
    ...
    my_init_dynamic_array(key_memory_acl_mem, &user.role_grants, sizeof(ACL_ROLE *), 0, 8, MYF(0));
    ...
    if (is_role)
    {
      ...
      my_init_dynamic_array(key_memory_acl_mem, &entry->parent_grantee, sizeof(ACL_USER_BASE *), 0, 8, MYF(0));
  ...
  if (proxies_priv_table.table_exists())
  {
    ...
    my_qsort((uchar *)dynamic_element(&acl_proxy_users, 0, ACL_PROXY_USER *), acl_proxy_users.elements,
  ...
  if (roles_mapping_table.table_exists())
  {
    ...
    while (!(read_record_info.read_record()))
    {
      ...
      {
        sql_print_error("Invalid roles_mapping table entry user:'%s@%s', rolename:'%s'", username, hostname, rolename);
        continue;
      }
...
bool acl_reload(THD *thd)
{
  ...
  */
  const uint tables_to_open = Table_host | Table_user | Table_db | Table_proxies_priv | Table_roles_mapping;
  ...
  my_hash_init2(key_memory_acl_mem, &acl_roles, 50, &my_charset_utf8mb3_bin, 0, 0, 0, (my_hash_get_key)acl_role_get_key,
                0, (void (*)(void *))free_acl_role, 0);
  my_hash_init2(key_memory_acl_mem, &acl_roles_mappings, 50, &my_charset_utf8mb3_bin, 0, 0, 0,
...
static void rebuild_acl_users()
{
  my_qsort((uchar *)dynamic_element(&acl_users, 0, ACL_USER *), acl_users.elements, sizeof(ACL_USER),
           (qsort_cmp)acl_user_compare);
}

static void rebuild_acl_dbs() { acl_dbs.sort(acl_db_compare); }
...
*/
template <typename T>
size_t find_first_user(T *arr, size_t len, const char *user)
{
  size_t low = 0;
  size_t high = len;
  size_t mid;
...
#ifndef DBUG_OFF
  for (uint i = 0; i < len - 1; i++) DBUG_ASSERT(strcmp(arr[i].get_username(), arr[i + 1].get_username()) <= 0);
#endif
  while (low < high)
  {
    mid = low + (high - low) / 2;
...
*/
template <typename T>
T *find_by_username_or_anon(T *arr, size_t len, const char *user, const char *host, const char *ip, const char *db,
                            my_bool db_is_pattern, bool (*match_db_func)(T *, const char *, my_bool))
{
  size_t i;
  T *ret = NULL;
...
{
  return find_by_username_or_anon(acl_dbs.front(), acl_dbs.elements(), user, host, ip, db, db_is_pattern, match_db);
}
...
bool acl_getroot(Security_context *sctx, const char *user, const char *host, const char *ip, const char *db)
{
  int res = 1;
  ACL_USER *acl_user = 0;
  DBUG_ENTER("acl_getroot");

  DBUG_PRINT("enter", ("Host: '%s', Ip: '%s', User: '%s', db: '%s'", host, ip, user, db));
  sctx->init();
...
static int check_role_is_granted_callback(ACL_USER_BASE *grantee, void *data)
{
  ...
  if (rolename->length == grantee->user.length && !strcmp(rolename->str, grantee->user.str))
...
{
  return find_by_username_or_anon<ACL_USER>(reinterpret_cast<ACL_USER *>(acl_users.buffer), acl_users.elements, user,
                                            host, ip, NULL, FALSE, NULL);
}

static int check_user_can_set_role(THD *thd, const char *user, const char *host, const char *ip, const char *rolename,
                                   privilege_t *access)
{
  ACL_ROLE *role;
  ...
  switch (result)
  {
    ...
    case 1:
      ...
      mysql_mutex_lock(&acl_cache->lock);
      ACL_USER *cur_user =
          find_user_or_anon(thd->security_ctx->priv_host, thd->security_ctx->priv_user, thd->security_ctx->ip);
      ...
      */
      if (cur_user &&
          (read_access || traverse_role_graph_down(cur_user, &role_lex, check_role_is_granted_callback, NULL) == -1))
      {
        /* Role is not granted but current user can see the role */
        my_printf_error(ER_INVALID_ROLE, "User %`s@%`s has not been granted role %`s", MYF(0),
                        thd->security_ctx->priv_user, thd->security_ctx->priv_host, rolename);
      }
      else
      {
...
int acl_check_setrole(THD *thd, const char *rolename, privilege_t *access)
{
  ...
  return check_user_can_set_role(thd, thd->security_ctx->priv_user, thd->security_ctx->host, thd->security_ctx->ip,
                                 rolename, access);
}
...
ACL_USER::ACL_USER(THD *thd, const LEX_USER &combo, const Account_options &options, const privilege_t privileges)
{
...
static int acl_user_update(THD *thd, ACL_USER *acl_user, uint nauth, const LEX_USER &combo,
                           const Account_options &options, const privilege_t privileges)
{
  ACL_USER_PARAM::AUTH *work_copy = NULL;
  if (nauth)
  {
    if (!(work_copy = (ACL_USER_PARAM::AUTH *)alloc_root(thd->mem_root, nauth * sizeof(ACL_USER_PARAM::AUTH))))
      return 1;
    ...
    for (uint i = 0; i < nauth; i++, auth = auth->next)
    {
      ...
      work_copy[i].auth_string = safe_lexcstrdup_root(&acl_memroot, auth->auth_str);
      if (fix_user_plugin_ptr(work_copy + i))
        work_copy[i].plugin = safe_lexcstrdup_root(&acl_memroot, auth->plugin);
      if (set_user_auth(thd, acl_user->user, {acl_user->host.hostname, acl_user->hostname_length}, work_copy + i,
                        auth->pwtext))
        return 1;
    }
  }
  ...
  if (options.ssl_type != SSL_TYPE_NOT_SPECIFIED)
  {
    ...
    acl_user->x509_issuer = safe_strdup_root(&acl_memroot, safe_str(options.x509_issuer.str));
    acl_user->x509_subject = safe_strdup_root(&acl_memroot, safe_str(options.x509_subject.str));
  }
...
static void acl_insert_role(const char *rolename, privilege_t privileges)
{
  ...
  my_init_dynamic_array(key_memory_acl_mem, &entry->parent_grantee, sizeof(ACL_USER_BASE *), 0, 8, MYF(0));
  my_init_dynamic_array(key_memory_acl_mem, &entry->role_grants, sizeof(ACL_ROLE *), 0, 8, MYF(0));

  my_hash_insert(&acl_roles, (uchar *)entry);
}

static bool acl_update_db(const char *user, const char *host, const char *db, privilege_t privileges)
{
  mysql_mutex_assert_owner(&acl_cache->lock);
...
static void acl_insert_db(const char *user, const char *host, const char *db, const privilege_t privileges)
{
  ACL_DB acl_db;
  mysql_mutex_assert_owner(&acl_cache->lock);
...
privilege_t acl_get(const char *host, const char *ip, const char *user, const char *db, my_bool db_is_pattern)
{
...
exit:
  ...
  if (!db_is_pattern &&
      (entry = (acl_entry *)my_malloc(key_memory_acl_cache, sizeof(acl_entry) + key_length, MYF(MY_WME))))
  {
...
static void init_check_host(void)
{
  ...
  (void)my_init_dynamic_array(key_memory_acl_mem, &acl_wild_hosts, sizeof(struct acl_host_and_ip), acl_users.elements,
                              1, MYF(0));
  (void)my_hash_init(key_memory_acl_mem, &acl_check_hosts, system_charset_info, acl_users.elements, 0, 0,
  ...
  if (!allow_all_hosts)
  {
    for (size_t i = 0; i < acl_users.elements; i++)
    {
      ...
      else if (!my_hash_search(&acl_check_hosts, (uchar *)acl_user->host.hostname, strlen(acl_user->host.hostname)))
      {
...
*/
static my_bool acl_role_reset_role_arrays(void *ptr, void *not_used __attribute__((unused)))
{
...
{
  return push_dynamic(&grantee->role_grants, (uchar *)&role) || push_dynamic(&role->parent_grantee, (uchar *)&grantee);
}
...
static void remove_role_user_mapping(ACL_USER_BASE *grantee, ACL_ROLE *role, int grantee_idx = -1, int role_idx = -1)
{
...
static int check_alter_user(THD *thd, const char *host, const char *user)
{
  ...
  if (IF_WSREP((!WSREP(thd) || !thd->wsrep_applier), 1) && !thd->slave_thread && !thd->security_ctx->priv_user[0] &&
      !thd->bootstrap)
  {
    my_message(ER_PASSWORD_ANONYMOUS_USER, ER_THD(thd, ER_PASSWORD_ANONYMOUS_USER), MYF(0));
    goto end;
  }
  if (!host)  // Role
  {
  ...
  if (!thd->slave_thread && IF_WSREP((!WSREP(thd) || !thd->wsrep_applier), 1) &&
...
bool change_password(THD *thd, LEX_USER *user)
{
  ...
  DBUG_PRINT("enter",
             ("host: '%s'  user: '%s'  new_password: '%s'", user->host.str, user->user.str, user->auth->auth_str.str));
  ...
  */
  ...
  if (acl_user->nauth == 1 && (acl_user->auth[0].plugin.str == native_password_plugin_name.str ||
  ...
  for (uint i = 0; i < acl_user->nauth; i++)
  {
    ...
    int r = set_user_auth(thd, user->user, user->host, &auth, user->auth->pwtext);
  ...
  {
    query_length = sprintf(buff, "SET PASSWORD FOR '%-.120s'@'%-.120s'='%-.120s'", user->user.str,
                           safe_str(user->host.str), auth.auth_string.str);
    DBUG_ASSERT(query_length);
    thd->clear_error();
    result = thd->binlog_query(THD::STMT_QUERY_TYPE, buff, query_length, FALSE, FALSE, FALSE, 0) > 0;
  }
end:
...
int acl_check_set_default_role(THD *thd, const char *host, const char *user, const char *role)
{
  ...
  */
  ...
  if (tables.rpl_ignore_tables(thd, first, Table_roles_mapping, TL_WRITE))
    DBUG_RETURN(0);
#endif
  DBUG_RETURN(check_alter_user(thd, host, user) || check_user_can_set_role(thd, user, host, NULL, role, NULL));
}

int acl_set_default_role(THD *thd, const char *host, const char *user, const char *rolename)
{
  Grant_tables tables;
  ...
  DBUG_PRINT("enter", ("host: '%s'  user: '%s'  rolename: '%s'", user, safe_str(host), safe_str(rolename)));

  if (!strcasecmp(rolename, "NONE"))
    clear_role = TRUE;

  if (mysql_bin_log.is_open() || (WSREP(thd) && !IF_WSREP(thd->wsrep_applier, 0)))
  {
    query_length =
        sprintf(buff, "SET DEFAULT ROLE '%-.120s' FOR '%-.120s'@'%-.120s'", safe_str(rolename), user, safe_str(host));
  }
  ...
  */
  {
    ...
    if (table->file->ha_index_read_idx_map(table->record[0], 0, (uchar *)user_key, HA_WHOLE_KEY, HA_READ_KEY_EXACT))
    {
      mysql_mutex_unlock(&acl_cache->lock);
      my_message(ER_PASSWORD_NO_MATCH, ER_THD(thd, ER_PASSWORD_NO_MATCH), MYF(0));
      goto end;
    ...
    user_table.set_default_role(acl_user->default_rolename.str, acl_user->default_rolename.length);
    if (unlikely(error = table->file->ha_update_row(table->record[1], table->record[0])) &&
...
static void update_hostname(acl_host_and_ip *host, const char *hostname)
{
  ...
  if (!(hostname = calc_ip(hostname, &host->ip, '/')) || !(hostname = calc_ip(hostname + 1, &host->ip_mask, '\0')))
  {
...
bool hostname_requires_resolving(const char *hostname)
{
  ...
  if (hostname == my_localhost || (hostname_len == localhost_len &&
                                   !system_charset_info->strnncoll((const uchar *)hostname, hostname_len,
...
static bool update_user_table_password(THD *thd, const User_table &user_table, const ACL_USER &user)
{
  char user_key[MAX_KEY_LENGTH];
  int error;
  ...
  {
    my_error(ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE, MYF(0), user_table.name().str, 3, user_table.num_fields(),
...
static bool test_if_create_new_users(THD *thd)
{
  ...
  bool create_new_users = MY_TEST(sctx->master_access & INSERT_ACL) ||
                          (!opt_safe_user_create && MY_TEST(sctx->master_access & CREATE_USER_ACL));
  if (!create_new_users)
  {
    TABLE_LIST tl;
    privilege_t db_access(NO_ACL);
    tl.init_one_table(&MYSQL_SCHEMA_NAME, &MYSQL_TABLE_NAME[USER_TABLE], NULL, TL_WRITE);
    create_new_users = 1;

    db_access = acl_get(sctx->host, sctx->ip, sctx->priv_user, tl.db.str, 0);
    if (sctx->priv_role[0])
...
static int replace_user_table(THD *thd, const User_table &user_table, LEX_USER *const combo, privilege_t rights,
                              const bool revoke_grant, const bool can_create_user, const bool no_auto_create)
{
  int error = -1;
  uint nauth = 0;
  ...
  if (table->file->ha_index_read_idx_map(table->record[0], 0, user_key, HA_WHOLE_KEY, HA_READ_KEY_EXACT))
  {
    if (revoke_grant)
    {
  ...
  if (handle_as_role)
  {
    ...
    {
      my_error(ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE, MYF(0), user_table.name().str, ROLE_ASSIGN_COLUMN_IDX + 1,
               user_table.num_fields(), static_cast<int>(table->s->mysql_version), MYSQL_VERSION_ID);
      goto end;
    }
  }
  else
  {
    ...
    new_acl_user = old_row_exists ? *old_acl_user : ACL_USER(thd, *combo, lex->account_options, rights);
    if (acl_user_update(thd, &new_acl_user, nauth, *combo, lex->account_options, rights))
      goto end;
    ...
    switch (lex->account_options.ssl_type)
    {
      ...
      case SSL_TYPE_SPECIFIED:
        ...
        if (lex->account_options.ssl_cipher.str)
          user_table.set_ssl_cipher(lex->account_options.ssl_cipher.str, lex->account_options.ssl_cipher.length);
        else
          user_table.set_ssl_cipher("", 0);
        if (lex->account_options.x509_issuer.str)
          user_table.set_x509_issuer(lex->account_options.x509_issuer.str, lex->account_options.x509_issuer.length);
        else
          user_table.set_x509_issuer("", 0);
        if (lex->account_options.x509_subject.str)
          user_table.set_x509_subject(lex->account_options.x509_subject.str, lex->account_options.x509_subject.length);
        else
          user_table.set_x509_subject("", 0);
        break;
    }
...
static int replace_db_table(TABLE *table, const char *db, const LEX_USER &combo, privilege_t rights,
                            const bool revoke_grant)
{
  ...
  if (!find_user_wild(combo.host.str, combo.user.str))
  {
    ...
    {
      my_message(ER_PASSWORD_NO_MATCH, ER_THD(table->in_use, ER_PASSWORD_NO_MATCH), MYF(0));
      DBUG_RETURN(-1);
    }
  }
  ...
  else if (rights && (unlikely(error = table->file->ha_write_row(table->record[0]))))
  {
  ...
  else if (rights)
  {
    ...
    */
    if (!combo.is_role() || !acl_update_db(combo.user.str, combo.host.str, db, rights))
    {
...
*/
static int replace_roles_mapping_table(TABLE *table, LEX_CSTRING *user, LEX_CSTRING *host, LEX_CSTRING *role,
                                       bool with_admin, ROLE_GRANT_PAIR *existing, bool revoke_grant)
{
  ...
  {
    key_copy(row_key, table->record[0], table->key_info, table->key_info->key_length);
    if (table->file->ha_index_read_idx_map(table->record[1], 0, row_key, HA_WHOLE_KEY, HA_READ_KEY_EXACT))
    {
      /* No match */
      DBUG_RETURN(1);
    }
    if (revoke_grant && !with_admin)
    {
      if (unlikely((error = table->file->ha_delete_row(table->record[1]))))
      {
        DBUG_PRINT("info", ("error deleting row '%s' '%s' '%s'", host->str, user->str, role->str));
        goto table_error;
      }
    }
    else if (with_admin)
    {
      ...
      if (unlikely((error = table->file->ha_update_row(table->record[1], table->record[0]))))
      {
        DBUG_PRINT("info", ("error updating row '%s' '%s' '%s'", host->str, user->str, role->str));
        goto table_error;
      }
    }
    DBUG_RETURN(0);
  ...
  {
    DBUG_PRINT("info", ("error inserting row '%s' '%s' '%s'", host->str, user->str, role->str));
    goto table_error;
  }
...
/**
  ...
  @param revoke_grant   true for REVOKE, false for GRANT
*/
static int update_role_mapping(LEX_CSTRING *user, LEX_CSTRING *host, LEX_CSTRING *role, bool with_admin,
...
static void acl_update_proxy_user(ACL_PROXY_USER *new_value, bool is_revoke)
{
  ...
  {
    ACL_PROXY_USER *acl_user = dynamic_element(&acl_proxy_users, i, ACL_PROXY_USER *);
...
static int replace_proxies_priv_table(THD *thd, TABLE *table, const LEX_USER *user, const LEX_USER *proxied_user,
                                      bool with_grant_arg, bool revoke_grant)
{
  ...
  if (!table)
  {
    my_error(ER_NO_SUCH_TABLE, MYF(0), MYSQL_SCHEMA_NAME.str, MYSQL_TABLE_NAME[PROXIES_PRIV_TABLE].str);
    DBUG_RETURN(-1);
  }
  ...
  table->use_all_columns();
  ACL_PROXY_USER::store_pk(table, &user->host, &user->user, &proxied_user->host, &proxied_user->user);
  ...
  if (table->file->ha_index_read_map(table->record[0], user_key, HA_WHOLE_KEY, HA_READ_KEY_EXACT))
  {
    ...
    old_row_exists = 0;
    restore_record(table, s->default_values);
    ACL_PROXY_USER::store_data_record(table, &user->host, &user->user, &proxied_user->host, &proxied_user->user,
  ...
  if (old_row_exists)
  {
    new_grant.init(user->host.str, user->user.str, proxied_user->host.str, proxied_user->user.str, with_grant_arg);
    acl_update_proxy_user(&new_grant, revoke_grant);
  }
  else
  {
    new_grant.init(&acl_memroot, user->host.str, user->user.str, proxied_user->host.str, proxied_user->user.str,
                   with_grant_arg);
...
class GRANT_COLUMN : public Sql_alloc
{
 public:
  ...
  GRANT_COLUMN(GRANT_COLUMN *source)
      : column(source->column), rights(source->rights), init_rights(NO_ACL), key_length(source->key_length)
  {
  }
};
...
class GRANT_NAME : public Sql_alloc
{
 public:
  ...
  ulonglong sort;
  size_t key_length;
  GRANT_NAME(const char *h, const char *d, const char *u, const char *t, privilege_t p, bool is_routine);
...
class GRANT_TABLE : public GRANT_NAME
{
 public:
  ...
  GRANT_TABLE(const char *h, const char *d, const char *u, const char *t, privilege_t p, privilege_t c);
  GRANT_TABLE(TABLE *form, TABLE *col_privs);
  ~GRANT_TABLE();
  bool ok() { return privs != NO_ACL || cols != NO_ACL; }
  void init_hash()
  {
    my_hash_init2(key_memory_acl_memex, &hash_columns, 4, system_charset_info, 0, 0, 0, (my_hash_get_key)get_key_column,
                  0, 0, 0);
  }
};

void GRANT_NAME::set_user_details(const char *h, const char *d, const char *u, const char *t, bool is_routine)
{
  /* Host given by user */
...
GRANT_NAME::GRANT_NAME(const char *h, const char *d, const char *u, const char *t, privilege_t p, bool is_routine)
    : db(0), tname(0), privs(p), init_privs(p)
{
  set_user_details(h, d, u, t, is_routine);
}

GRANT_TABLE::GRANT_TABLE(const char *h, const char *d, const char *u, const char *t, privilege_t p, privilege_t c)
...
/*
  ...
  to 0
*/
GRANT_NAME::GRANT_NAME(TABLE *form, bool is_routine) : privs(NO_ACL), init_privs(NO_ACL)
{
...
GRANT_TABLE::GRANT_TABLE(TABLE *form, TABLE *col_privs) : GRANT_NAME(form, FALSE), cols(NO_ACL), init_cols(NO_ACL)
{
  ...
  */
  ...
  if (cols)
  {
    ...
    col_privs->field[0]->store(host.hostname, (uint)safe_strlen(host.hostname), system_charset_info);
    ...
    if (col_privs->file->ha_index_read_map(col_privs->record[0], (uchar *)key, (key_part_map)15, HA_READ_KEY_EXACT))
    {
...
GRANT_TABLE::~GRANT_TABLE() { my_hash_free(&hash_columns); }
...
static void free_grant_table(GRANT_TABLE *grant_table) { grant_table->~GRANT_TABLE(); }
                                    ...
                                    const char *tname, bool exact, bool name_tolower)
{
  ...
  for (grant_name = (GRANT_NAME *)my_hash_first(name_hash, (uchar *)helping, len, &state); grant_name;
       grant_name = (GRANT_NAME *)my_hash_next(name_hash, (uchar *)helping, len, &state))
  {
    if (exact)
    {
      if (!grant_name->host.hostname ||
          (host && !my_strcasecmp(system_charset_info, host, grant_name->host.hostname)) ||
...
static GRANT_NAME *routine_hash_search(const char *host, const char *ip, const char *db, const char *user,
                                       const char *tname, const Sp_handler *sph, bool exact)
{
  return (GRANT_NAME *)name_hash_search(sph->get_priv_hash(), host, ip, db, user, tname, exact, TRUE);
}

static GRANT_TABLE *table_hash_search(const char *host, const char *ip, const char *db, const char *user,
                                      const char *tname, bool exact)
{
  return (GRANT_TABLE *)name_hash_search(&column_priv_hash, host, ip, db, user, tname, exact, FALSE);
}

static bool column_priv_insert(GRANT_TABLE *grant) { return my_hash_insert(&column_priv_hash, (uchar *)grant); }

static GRANT_COLUMN *column_hash_search(GRANT_TABLE *t, const char *cname, size_t length)
{
...
static int replace_column_table(GRANT_TABLE *g_t, TABLE *table, const LEX_USER &combo, List<LEX_COLUMN> &columns,
                                const char *db, const char *table_name, privilege_t rights, bool revoke_grant)
{
  int result = 0;
  uchar key[MAX_KEY_LENGTH];
  ...
  */
  ...
  if (revoke_grant)
  {
    ...
    if (table->file->ha_index_read_map(table->record[0], user_key, (key_part_map)15, HA_READ_KEY_EXACT))
      goto end;
    ...
    do
    {
      ...
      if (privileges & rights)  // is in this record the priv to be revoked ??
      {
        ...
        {
          int tmp_error;
          if (unlikely(tmp_error = table->file->ha_update_row(table->record[1], table->record[0])) &&
        ...
        {
          int tmp_error;
          if (unlikely((tmp_error = table->file->ha_delete_row(table->record[1]))))
          {                                              /* purecov: deadcode */
            table->file->print_error(tmp_error, MYF(0)); /* purecov: deadcode */
            result = -1;                                 /* purecov: deadcode */
            goto end;                                    /* purecov: deadcode */
    ...
    } while (!table->file->ha_index_next(table->record[0]) && !key_cmp_if_same(table, key, 0, key_prefix_length));
  }
...
static int replace_table_table(THD *thd, GRANT_TABLE *grant_table, TABLE *table, const LEX_USER &combo, const char *db,
                               const char *table_name, privilege_t rights, privilege_t col_rights, bool revoke_grant)
{
  char grantor[USER_HOST_BUFF_SIZE];
  ...
  */
  if (!find_user_wild(combo.host.str, combo.user.str))
  {
    if (!combo.host.length && !find_acl_role(combo.user.str))
    {
      my_message(ER_PASSWORD_NO_MATCH, ER_THD(thd, ER_PASSWORD_NO_MATCH), MYF(0)); /* purecov: deadcode */
...
static int replace_routine_table(THD *thd, GRANT_NAME *grant_name, TABLE *table, const LEX_USER &combo, const char *db,
                                 const char *routine_name, const Sp_handler *sph, privilege_t rights, bool revoke_grant)
{
  char grantor[USER_HOST_BUFF_SIZE];
  int old_row_exists = 1;
  int error = 0;
  ...
  if (!table)
  {
    my_error(ER_NO_SUCH_TABLE, MYF(0), MYSQL_SCHEMA_NAME.str, MYSQL_TABLE_NAME[PROCS_PRIV_TABLE].str);
    DBUG_RETURN(-1);
  }
  ...
  */
  ...
  if (table->file->ha_index_read_idx_map(table->record[0], 0, (uchar *)table->field[0]->ptr, HA_WHOLE_KEY,
                                         HA_READ_KEY_EXACT))
  {
    /*
      The following should never happen as we first check the in memory
...
/**
  ...
  as necessary.
*/
static void propagate_role_grants(ACL_ROLE *role, enum PRIVS_TO_MERGE::what what, const char *db = 0,
                                  const char *name = 0)
{
  if (!role)
    return;
...
/**
  ...
  @retval <0                traversal was aborted, because a callback returned
                            this error code
*/
static int traverse_role_graph_impl(ACL_USER_BASE *user, void *context, off_t offset,
...
static int traverse_role_graph_up(ACL_ROLE *role, void *context, int (*on_node)(ACL_ROLE *role, void *context),
                                  int (*on_edge)(ACL_ROLE *current, ACL_ROLE *neighbour, void *context))
{
  return traverse_role_graph_impl(role, context, my_offsetof(ACL_ROLE, parent_grantee),
...
static int traverse_role_graph_down(ACL_USER_BASE *user, void *context,
                                    ...
                                    int (*on_edge)(ACL_USER_BASE *current, ACL_ROLE *neighbour, void *context))
{
  return traverse_role_graph_impl(user, context, my_offsetof(ACL_USER_BASE, role_grants), on_node, on_edge);
}
...
static int db_name_sort(const int *db1, const int *db2) { return strcmp(acl_dbs.at(*db1).db, acl_dbs.at(*db2).db); }
...
*/
static int update_role_db(int merged, int first, privilege_t access, const char *role)
{
  if (first < 0)
    return 0;
...
*/
static bool merge_role_db_privileges(ACL_ROLE *grantee, const char *dbname, role_hash_t *rhash)
{
...
*/
static int update_role_columns(GRANT_TABLE *merged, GRANT_TABLE **cur, GRANT_TABLE **last)
{
  ...
  for (; cur < last; cur++)
  {
    ...
    for (uint i = 0; i < ch->records; i++)
    {
      ...
      GRANT_COLUMN *mcol = (GRANT_COLUMN *)my_hash_search(mh, (uchar *)ccol->column, ccol->key_length);
      if (mcol)
...
*/
static int update_role_table_columns(GRANT_TABLE *merged, GRANT_TABLE **first, GRANT_TABLE **last, privilege_t privs,
                                     privilege_t cols, const char *role)
{
  if (!first)
    return 0;
  ...
  if (merged == NULL)
  {
    ...
    */
    ...
    merged = new (&grant_memroot) GRANT_TABLE("", first[0]->db, role, first[0]->tname, privs, cols);
  ...
  else if ((privs | cols) == NO_ACL)
  {
    ...
    */
    DBUG_EXECUTE_IF("role_merge_stats", role_column_merges += MY_TEST(merged->cols););
...
*/
static bool merge_role_table_and_column_privileges(ACL_ROLE *grantee, const char *db, const char *tname,
                                                   role_hash_t *rhash)
{
  ...
  */
  ...
  {
    if (!first || (!tname && (strcmp(cur[0]->db, cur[-1]->db) || strcmp(cur[0]->tname, cur[-1]->tname))))
    {  // new db.tname series
      update_flags |= update_role_table_columns(merged, first, cur, privs, cols, grantee->user.str);
      merged = NULL;
      privs = cols = NO_ACL;
      first = cur;
    }
...
*/
static int update_role_routines(GRANT_NAME *merged, GRANT_NAME **first, privilege_t privs, const char *role, HASH *hash)
{
  if (!first)
    return 0;
  ...
  if (merged == NULL)
  {
    ...
    */
    DBUG_ASSERT(privs);
    merged = new (&grant_memroot) GRANT_NAME("", first[0]->db, role, first[0]->tname, privs, true);
...
*/
static bool merge_role_routine_grant_privileges(ACL_ROLE *grantee, const char *db, const char *tname,
                                                role_hash_t *rhash, HASH *hash)
{
  ulong update_flags = 0;
  ...
  for (GRANT_NAME **cur = grants.front(); cur <= grants.back(); cur++)
  {
    if (!first || (!tname && (strcmp(cur[0]->db, cur[-1]->db) || strcmp(cur[0]->tname, cur[-1]->tname))))
    {  // new db.tname series
      update_flags |= update_role_routines(merged, first, privs, grantee->user.str, hash);
      merged = NULL;
      privs = NO_ACL;
      first = cur;
    }
...
*/
static int merge_role_privileges(ACL_ROLE *role __attribute__((unused)), ACL_ROLE *grantee, void *context)
{
  ...
  if (all || data->what == PRIVS_TO_MERGE::TABLE_COLUMN)
    changed |= merge_role_table_and_column_privileges(grantee, data->db, data->name, &role_hash);
  if (all || data->what == PRIVS_TO_MERGE::PROC)
    changed |= merge_role_routine_grant_privileges(grantee, data->db, data->name, &role_hash, &proc_priv_hash);
  if (all || data->what == PRIVS_TO_MERGE::FUNC)
    changed |= merge_role_routine_grant_privileges(grantee, data->db, data->name, &role_hash, &func_priv_hash);
  if (all || data->what == PRIVS_TO_MERGE::PACKAGE_SPEC)
    changed |= merge_role_routine_grant_privileges(grantee, data->db, data->name, &role_hash, &package_spec_priv_hash);
  if (all || data->what == PRIVS_TO_MERGE::PACKAGE_BODY)
    changed |= merge_role_routine_grant_privileges(grantee, data->db, data->name, &role_hash, &package_body_priv_hash);
...
{
  return user->has_auth() || lex->account_options.ssl_type != SSL_TYPE_NOT_SPECIFIED ||
         lex->account_options.ssl_cipher.str || lex->account_options.x509_issuer.str ||
         lex->account_options.x509_subject.str || lex->account_options.specified_limits;
}
...
int mysql_table_grant(THD *thd, TABLE_LIST *table_list, List<LEX_USER> &user_list, List<LEX_COLUMN> &columns,
                      privilege_t rights, bool revoke_grant)
{
  ...
  {
    my_message(ER_ILLEGAL_GRANT_FOR_TABLE, ER_THD(thd, ER_ILLEGAL_GRANT_FOR_TABLE), MYF(0));
    DBUG_RETURN(TRUE);
  }
  ...
  if (!revoke_grant)
  {
    if (columns.elements)
    {
      ...
      while ((column = column_iter++))
      {
        ...
        TABLE_LIST *dummy;
        Field *f = find_field_in_table_ref(thd, table_list, column->column.ptr(), column->column.length(),
                                           column->column.ptr(), NULL, NULL, ignored_tables_list_t(NULL), NULL, TRUE,
                                           FALSE, &unused_field_idx, FALSE, &dummy);
        if (unlikely(f == (Field *)0))
        {
          my_error(ER_BAD_FIELD_ERROR, MYF(0), column->column.c_ptr(), table_list->alias.str);
          DBUG_RETURN(TRUE);
        }
    ...
    else
    {
      ...
      if (table_list->grant.want_privilege)
      {
        ...
        my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), command, thd->security_ctx->priv_user,
                 thd->security_ctx->host_or_ip, table_list->db.str, table_list->alias.str);
        DBUG_RETURN(-1);
      }
    }
  }
  ...
  */
  ...
  while ((tmp_Str = str_list++))
  {
    ...
    error = copy_and_check_auth(Str, tmp_Str, thd) ||
            replace_user_table(thd, tables.user_table(), Str, NO_ACL, revoke_grant, create_new_users,
                               MY_TEST(thd->variables.sql_mode & MODE_NO_AUTO_CREATE_USER));
    if (unlikely(error))
    {
    ...
    if (!grant_table)
    {
      if (revoke_grant)
      {
        my_error(ER_NONEXISTING_TABLE_GRANT, MYF(0), Str->user.str, Str->host.str, table_list->table_name.str);
        result = TRUE;
        continue;
      }
      grant_table =
          new (&grant_memroot) GRANT_TABLE(Str->host.str, db_name, Str->user.str, table_name, rights, column_priv);
      if (!grant_table || column_priv_insert(grant_table))
      {
    ...
    if (revoke_grant)
    {
      ...
      {
        grant_column = (GRANT_COLUMN *)my_hash_element(&grant_table->hash_columns, idx);
    ...
    if (tables.columns_priv_table().table_exists())
    {
      /* TODO(cvicentiu) refactor replace_column_table to use Columns_priv_table
         instead of TABLE directly. */
      if (replace_column_table(grant_table, tables.columns_priv_table().table(), *Str, columns, db_name, table_name,
                               rights, revoke_grant))
        result = TRUE;
    }
    if ((res = replace_table_table(thd, grant_table, tables.tables_priv_table().table(), *Str, db_name, table_name,
    ...
    if (Str->is_role())
      propagate_role_grants(find_acl_role(Str->user.str), PRIVS_TO_MERGE::TABLE_COLUMN, db_name, table_name);
  }
...
bool mysql_routine_grant(THD *thd, TABLE_LIST *table_list, const Sp_handler *sph, List<LEX_USER> &user_list,
                         privilege_t rights, bool revoke_grant, bool write_to_binlog)
{
  List_iterator<LEX_USER> str_list(user_list);
  LEX_USER *Str, *tmp_Str;
  bool create_new_users = 0;
  ...
  while ((tmp_Str = str_list++))
  {
    ...
    grant_name = routine_hash_search(Str->host.str, NullS, db_name, Str->user.str, table_name, sph, 1);
    ...
    if (!grant_name)
    {
      ...
      if (!grant_name || my_hash_insert(sph->get_priv_hash(), (uchar *)grant_name))
      {
        result = TRUE;
        continue;
      }
    ...
    if (replace_routine_table(thd, grant_name, tables.procs_priv_table().table(), *Str, db_name, table_name, sph,
                              rights, revoke_grant) != 0)
    {
      result = TRUE;
      continue;
    }
    if (Str->is_role())
      propagate_role_grants(find_acl_role(Str->user.str), sp_privs_to_merge(sph->type()), db_name, table_name);
  }
  thd->mem_root = old_root;
  mysql_mutex_unlock(&acl_cache->lock);
...
*/
static void append_user(THD *thd, String *str, const LEX_CSTRING *u, const LEX_CSTRING *h)
{
  if (str->length())
    str->append(',');
  append_query_string(system_charset_info, str, u->str, u->length, thd->variables.sql_mode & MODE_NO_BACKSLASH_ESCAPES);
...
static void append_user(THD *thd, String *str, LEX_USER *user) { append_user(thd, str, &user->user, &user->host); }
...
static int can_grant_role_callback(ACL_USER_BASE *grantee, ACL_ROLE *role, void *data)
{
...
static bool can_grant_role(THD *thd, ACL_ROLE *role)
{
  ...
  return traverse_role_graph_down(grantee, role, NULL, can_grant_role_callback) == -1;
}
...
bool mysql_grant_role(THD *thd, List<LEX_USER> &list, bool revoke)
{
  ...
  no_auto_create_user = MY_TEST(thd->variables.sql_mode & MODE_NO_AUTO_CREATE_USER);
  ...
  if (!can_grant_role(thd, role))
  {
    mysql_mutex_unlock(&acl_cache->lock);
    mysql_rwlock_unlock(&LOCK_grant);
    my_error(ER_ACCESS_DENIED_NO_PASSWORD_ERROR, MYF(0), thd->security_ctx->priv_user, thd->security_ctx->priv_host);
    DBUG_RETURN(TRUE);
  }
  ...
  while ((user = user_list++))
  {
    ...
    if (user->user.str == current_role.str)
    {
      ...
      {
        LEX_CSTRING ls = {thd->security_ctx->priv_role, strlen(thd->security_ctx->priv_role)};
    ...
    else
    {
      ...
      if (user->host.str)
        hostname = user->host;
      else if ((role_as_user = find_acl_role(user->user.str)))
        hostname = empty_clex_str;
      else
      {
    ...
    ROLE_GRANT_PAIR *hash_entry = find_role_grant_pair(&username, &hostname, &rolename);
    ...
    if (!grantee && !revoke)
    {
      ...
      if (copy_and_check_auth(&user_combo, &user_combo, thd) ||
          replace_user_table(thd, tables.user_table(), &user_combo, NO_ACL, false, create_new_user,
                             no_auto_create_user))
      {
    ...
    if (!revoke)
    {
      ...
      else
      {
        ...
        */
        if (role_as_user && traverse_role_graph_down(role, 0, 0, 0) == ROLE_CYCLE_FOUND)
        {
          append_user(thd, &wrong_users, &username, &empty_clex_str);
          result = 1;
    ...
    if (replace_roles_mapping_table(tables.roles_mapping_table().table(), &username, &hostname, &rolename,
    ...
    update_role_mapping(&username, &hostname, &rolename, thd->lex->with_admin_option, hash_entry, revoke);
  ...
  if (result)
    my_error(revoke ? ER_CANNOT_REVOKE_ROLE : ER_CANNOT_GRANT_ROLE, MYF(0), rolename.str, wrong_users.c_ptr_safe());
  else
    result = write_bin_log(thd, TRUE, thd->query(), thd->query_length());
...
bool mysql_grant(THD *thd, const char *db, List<LEX_USER> &list, privilege_t rights, bool revoke_grant, bool is_proxy)
{
  ...
  while ((tmp_Str = str_list++))
  {
    ...
    if (copy_and_check_auth(Str, tmp_Str, thd) ||
        replace_user_table(thd, tables.user_table(), Str, (!db ? rights : NO_ACL), revoke_grant, create_new_users,
                           MY_TEST(thd->variables.sql_mode & MODE_NO_AUTO_CREATE_USER)))
      result = true;
    else if (db)
    {
      ...
      if (db_rights == rights)
      {
        if (replace_db_table(tables.db_table().table(), db, *Str, db_rights, revoke_grant))
          result = true;
      }
      else
      {
    ...
    {
      if (replace_proxies_priv_table(thd, tables.proxies_priv_table().table(), Str, proxied_user,
                                     rights & GRANT_ACL ? TRUE : FALSE, revoke_grant))
        result = true;
    }
    if (Str->is_role())
      propagate_role_grants(find_acl_role(Str->user.str), db ? PRIVS_TO_MERGE::DB : PRIVS_TO_MERGE::GLOBAL, db);
  }
  mysql_mutex_unlock(&acl_cache->lock);
...
static bool grant_load(THD *thd, const Tables_priv_table &tables_priv, const Columns_priv_table &columns_priv,
                       const Procs_priv_table &procs_priv)
{
  ...
  (void)my_hash_init(key_memory_acl_memex, &column_priv_hash, &my_charset_utf8mb3_bin, 0, 0, 0,
                     (my_hash_get_key)get_grant_table, (my_hash_free_key)free_grant_table, 0);
  (void)my_hash_init(key_memory_acl_memex, &proc_priv_hash, &my_charset_utf8mb3_bin, 0, 0, 0,
                     (my_hash_get_key)get_grant_table, 0, 0);
  (void)my_hash_init(key_memory_acl_memex, &func_priv_hash, &my_charset_utf8mb3_bin, 0, 0, 0,
                     (my_hash_get_key)get_grant_table, 0, 0);
  (void)my_hash_init(key_memory_acl_memex, &package_spec_priv_hash, &my_charset_utf8mb3_bin, 0, 0, 0,
                     (my_hash_get_key)get_grant_table, 0, 0);
  (void)my_hash_init(key_memory_acl_memex, &package_body_priv_hash, &my_charset_utf8mb3_bin, 0, 0, 0,
                     (my_hash_get_key)get_grant_table, 0, 0);
      ...
      if (check_no_resolve)
      {
        if (hostname_requires_resolving(mem_check->host.hostname))
        {
          sql_print_warning(
              ...
              "ignored in --skip-name-resolve mode.",
              mem_check->tname, mem_check->user, safe_str(mem_check->host.hostname));
          continue;
        }
      }
...
static my_bool propagate_role_grants_action(void *role_ptr, void *ptr __attribute__((unused)))
{
...
bool grant_reload(THD *thd)
{
  ...
  */
  ...
  if ((result = grant_load(thd, tables.tables_priv_table(), tables.columns_priv_table(), tables.procs_priv_table())))
...
bool check_grant(THD *thd, privilege_t want_access, TABLE_LIST *tables, bool any_combination_will_do, uint number,
                 bool no_errors)
{
  TABLE_LIST *tl;
  ...
  */
  for (i = 0, tl = tables; i < number && tl != first_not_own_table; tl = tl->next_global, i++)
  {
    /*
  ...
  {
    TABLE_LIST *const t_ref = tl->correspondent_table ? tl->correspondent_table : tl;
    ...
    /*
      ...
      sequence is used.
    */
    if (t_ref->sequence && !(want_access & ~(SELECT_ACL | INSERT_ACL | UPDATE_ACL | DELETE_ACL)))
    {
      /*
        We want to have either SELECT or INSERT rights to sequences depending
        on how they are accessed
      */
      orig_want_access = ((t_ref->lock_type >= TL_FIRST_WRITE) ? INSERT_ACL : SELECT_ACL);
    }

    if (tl->with || !tl->db.str || (tl->select_lex && (tl->with = tl->select_lex->find_table_def_in_with_clauses(tl))))
      continue;

    const ACL_internal_table_access *access =
        get_cached_table_access(&t_ref->grant.m_internal, t_ref->get_db_name(), t_ref->get_table_name());
    ...
    if (!(~t_ref->grant.privilege & want_access) || t_ref->is_anonymous_derived_table() || t_ref->schema_table ||
        t_ref->table_function)
    {
      ...
      */
      if (t_ref->is_anonymous_derived_table() || t_ref->schema_table || t_ref->table_function)
      {
        /*
    ...
    grant_table =
        table_hash_search(sctx->host, sctx->ip, t_ref->get_db_name(), sctx->priv_user, t_ref->get_table_name(), FALSE);
    if (sctx->priv_role[0])
      grant_table_role =
          table_hash_search("", NULL, t_ref->get_db_name(), sctx->priv_role, t_ref->get_table_name(), TRUE);
    ...
    */
    ...
    if ((want_access &= ~((grant_table ? grant_table->cols : NO_ACL) |
                          (grant_table_role ? grant_table_role->cols : NO_ACL) | t_ref->grant.privilege)))
    {
      goto err;  // impossible
    }
  }
...
err:
  ...
  if (!no_errors)  // Not a silent skip of table
  {
    ...
    my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), command, sctx->priv_user, sctx->host_or_ip,
             tl ? tl->db.str : "unknown", tl ? tl->get_table_name() : "unknown");
  }
  DBUG_RETURN(TRUE);
}

static void check_grant_column_int(GRANT_TABLE *grant_table, const char *name, uint length, privilege_t *want_access)
{
  if (grant_table)
  {
                        ...
                        size_t length, Security_context *sctx)
{
  ...
  DBUG_PRINT("enter", ("table: %s  want_access: %llx", table_name, (longlong)want_access));
  ...
  if (grant->version != grant_version)
  {
    ...
    grant->grant_table_role =
        sctx->priv_role[0] ? table_hash_search("", NULL, db_name, sctx->priv_role, table_name, TRUE) : NULL;
    grant->version = grant_version; /* purecov: inspected */
  }

  check_grant_column_int(grant->grant_table_user, name, (uint)length, &want_access);
  check_grant_column_int(grant->grant_table_role, name, (uint)length, &want_access);
...
bool check_column_grant_in_table_ref(THD *thd, TABLE_LIST *table_ref, const char *name, size_t length, Field *fld)
{
  GRANT_INFO *grant;
  const char *db_name;
  const char *table_name;
  Security_context *sctx = table_ref->security_ctx ? table_ref->security_ctx : thd->security_ctx;
  if (fld && fld != not_found_field && fld != view_ref_found && fld->invisible >= INVISIBLE_SYSTEM)
    return false;
  ...
  if (grant->want_privilege)
    return check_grant_column(thd, grant, db_name, table_name, name, length, sctx);
  else
    return FALSE;
}
...
*/
bool check_grant_all_columns(THD *thd, privilege_t want_access_arg, Field_iterator_table_ref *fields)
{
  Security_context *sctx = thd->security_ctx;
  ...
  */
  ...
  {
    if (fields->field() && fields->field()->invisible >= INVISIBLE_SYSTEM)
      continue;
    ...
    if (want_access)
    {
      ...
      if (grant_table)
      {
        GRANT_COLUMN *grant_column = column_hash_search(grant_table, field_name->str, field_name->length);
        if (grant_column)
          have_access = grant_column->rights;
      }
      if (grant_table_role)
      {
        GRANT_COLUMN *grant_column = column_hash_search(grant_table_role, field_name->str, field_name->length);
        if (grant_column)
          have_access |= grant_column->rights;
      }
...
err:
  ...
  */
  if (using_column_privileges)
    my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), command, sctx->priv_user, sctx->host_or_ip, db_name, table_name);
  else
    my_error(ER_COLUMNACCESS_DENIED_ERROR, MYF(0), command, sctx->priv_user, sctx->host_or_ip, fields->name()->str,
             table_name);
  return 1;
}
...
static bool check_grant_db_routine(THD *thd, const char *db, HASH *hash)
{
  ...
  for (uint idx = 0; idx < hash->records; ++idx)
  {
    ...
    if (strcmp(item->user, sctx->priv_user) == 0 && strcmp(item->db, db) == 0 &&
        compare_hostname(&item->host, sctx->host, sctx->ip))
    {
      return FALSE;
    }
    if (sctx->priv_role[0] && strcmp(item->user, sctx->priv_role) == 0 && strcmp(item->db, db) == 0 &&
...
bool check_grant_db(THD *thd, const char *db)
{
  ...
  */
  ...
  {
    GRANT_TABLE *grant_table = (GRANT_TABLE *)my_hash_element(&column_priv_hash, idx);
    ...
    if (sctx->priv_role[0] && len2 < grant_table->key_length && !memcmp(grant_table->hash_key, helping2, len2) &&
  ...
  if (error)
    error = check_grant_db_routine(thd, db, &proc_priv_hash) && check_grant_db_routine(thd, db, &func_priv_hash) &&
...
bool check_grant_routine(THD *thd, privilege_t want_access, TABLE_LIST *procs, const Sp_handler *sph, bool no_errors)
{
  TABLE_LIST *table;
  Security_context *sctx = thd->security_ctx;
  ...
  for (table = procs; table; table = table->next_global)
  {
    ...
    if ((grant_proc = routine_hash_search(host, sctx->ip, table->db.str, user, table->table_name.str, sph, 0)))
      table->grant.privilege |= grant_proc->privs;
    if (role[0]) /* current role set check */
    {
      if ((grant_proc = routine_hash_search("", NULL, table->db.str, role, table->table_name.str, sph, 0)))
        table->grant.privilege |= grant_proc->privs;
    }
...
bool check_routine_level_acl(THD *thd, const char *db, const char *name, const Sp_handler *sph)
{
  bool no_routine_acl = 1;
  GRANT_NAME *grant_proc;
  Security_context *sctx = thd->security_ctx;
  mysql_rwlock_rdlock(&LOCK_grant);
  if ((grant_proc = routine_hash_search(sctx->priv_host, sctx->ip, db, sctx->priv_user, name, sph, 0)))
    no_routine_acl = !(grant_proc->privs & SHOW_PROC_ACLS);

  if (no_routine_acl && sctx->priv_role[0]) /* current set role check */
  {
    if ((grant_proc = routine_hash_search("", NULL, db, sctx->priv_role, name, sph, 0)))
      no_routine_acl = !(grant_proc->privs & SHOW_PROC_ACLS);
  }
  mysql_rwlock_unlock(&LOCK_grant);
...
#else
  grant_table = table_hash_search(sctx->host, sctx->ip, db, sctx->priv_user, table->table_name.str, 0);
  if (sctx->priv_role[0])
    grant_table_role = table_hash_search("", "", db, sctx->priv_role, table->table_name.str, 0);
#endif
...
privilege_t get_column_grant(THD *thd, GRANT_INFO *grant, const char *db_name, const char *table_name,
                             const char *field_name)
{
  ...
  if (grant->version != grant_version)
  {
    ...
    grant->grant_table_user =
        table_hash_search(sctx->host, sctx->ip, db_name, sctx->priv_user, table_name, 0); /* purecov: inspected */
    grant->grant_table_role =
        sctx->priv_role[0] ? table_hash_search("", "", db_name, sctx->priv_role, table_name, TRUE) : NULL;
    grant->version = grant_version; /* purecov: inspected */
  }
  ...
  else
  {
    ...
    if (grant_table_role)
    {
      grant_column = column_hash_search(grant_table_role, field_name, (uint)strlen(field_name));
      if (!grant_column)
...
static void add_user_option(String *grant, long value, const char *name, bool is_signed)
{
  if (value)
  {
...
static void add_user_parameters(THD *thd, String *result, ACL_USER *acl_user, bool with_grant)
{
  result->append('@');
  append_identifier(thd, result, acl_user->host.hostname, acl_user->hostname_length);

  if (acl_user->nauth == 1 && (acl_user->auth->plugin.str == native_password_plugin_name.str ||
  ...
  else if (acl_user->ssl_type == SSL_TYPE_SPECIFIED)
  {
    ...
    if (acl_user->x509_subject[0])
    {
      ...
      result->append(acl_user->x509_subject, strlen(acl_user->x509_subject), system_charset_info);
      result->append('\'');
    }
    if (acl_user->ssl_cipher)
    {
      ...
      result->append(acl_user->ssl_cipher, strlen(acl_user->ssl_cipher), system_charset_info);
      result->append('\'');
    }
  }
  if (with_grant ||
      (acl_user->user_resource.questions || acl_user->user_resource.updates || acl_user->user_resource.conn_per_hour ||
       acl_user->user_resource.user_conn || acl_user->user_resource.max_statement_time != 0.0))
  {
    result->append(STRING_WITH_LEN(" WITH"));
    if (with_grant)
      result->append(STRING_WITH_LEN(" GRANT OPTION"));
    add_user_option(result, acl_user->user_resource.questions, "MAX_QUERIES_PER_HOUR", false);
    add_user_option(result, acl_user->user_resource.updates, "MAX_UPDATES_PER_HOUR", false);
    add_user_option(result, acl_user->user_resource.conn_per_hour, "MAX_CONNECTIONS_PER_HOUR", false);
    add_user_option(result, acl_user->user_resource.user_conn, "MAX_USER_CONNECTIONS", true);
    add_user_option(result, acl_user->user_resource.max_statement_time, "MAX_STATEMENT_TIME");
  }
}
...
static uint command_lengths[] = {6,  6,  6, 6,  6,  4,  6, 8, 7,  4,  5, 10, 5,  5,  14, 5,  23, 11, 7, 17,
                                 14, 11, 9, 14, 13, 11, 5, 7, 17, 14, 8, 15, 16, 15, 23, 24, 12, 13, 13};
...
static bool print_grants_for_role(THD *thd, ACL_ROLE *role)
{
  ...
  if (show_routine_grants(thd, role->user.str, "", &sp_handler_procedure, buff, sizeof(buff)))
    return TRUE;

  if (show_routine_grants(thd, role->user.str, "", &sp_handler_function, buff, sizeof(buff)))
    return TRUE;

  if (show_routine_grants(thd, role->user.str, "", &sp_handler_package_spec, buff, sizeof(buff)))
    return TRUE;

  if (show_routine_grants(thd, role->user.str, "", &sp_handler_package_body, buff, sizeof(buff)))
    return TRUE;
...
bool mysql_show_create_user(THD *thd, LEX_USER *lex_user)
{
  ...
  head_length = (uint)(strxmov(buff, "CREATE USER for ", username, "@", hostname, NullS) - buff);
...
void mysql_show_grants_get_fields(THD *thd, List<Item> *fields, const char *name, size_t length)
{
  Item_string *field = new (thd->mem_root) Item_string_ascii(thd, "", 0);
...
/** checks privileges for SHOW GRANTS and SHOW CREATE USER
  ...
  be assigned to
*/
bool get_show_user(THD *thd, LEX_USER *lex_user, const char **username, const char **hostname, const char **rolename)
{
  ...
  else
  {
    ...
    *hostname = lex_user->host.str;
    do_check_access = strcmp(*username, sctx->priv_user) || strcmp(*hostname, sctx->priv_host);
  }
...
bool mysql_show_grants(THD *thd, LEX_USER *lex_user)
{
  ...
  if (username)
  {
    ...
    if (!acl_user)
    {
      ...
      my_error(ER_NONEXISTING_GRANT, MYF(0), username, hostname);
      DBUG_RETURN(TRUE);
    }
    ...
    if (show_routine_grants(thd, username, hostname, &sp_handler_procedure, buff, sizeof(buff)))
      goto end;

    if (show_routine_grants(thd, username, hostname, &sp_handler_function, buff, sizeof(buff)))
      goto end;

    if (show_routine_grants(thd, username, hostname, &sp_handler_package_spec, buff, sizeof(buff)))
      goto end;

    if (show_routine_grants(thd, username, hostname, &sp_handler_package_body, buff, sizeof(buff)))
      goto end;
  ...
  if (rolename)
  {
    ...
    else
    {
      if (lex_user->user.str == current_role.str)
      {
        mysql_mutex_unlock(&acl_cache->lock);
        mysql_rwlock_unlock(&LOCK_grant);
        my_error(ER_NONEXISTING_GRANT, MYF(0), thd->security_ctx->priv_user, thd->security_ctx->priv_host);
        DBUG_RETURN(TRUE);
      }
    }
  }
...
static ROLE_GRANT_PAIR *find_role_grant_pair(const LEX_CSTRING *u, const LEX_CSTRING *h, const LEX_CSTRING *r)
{
  ...
  strmov(strmov(strmov(const_cast<char *>(pair_key.ptr()), safe_str(u->str)) + 1, h->str) + 1, r->str);

  return (ROLE_GRANT_PAIR *)my_hash_search(&acl_roles_mappings, (uchar *)pair_key.ptr(), key_length);
}

static bool show_default_role(THD *thd, ACL_USER *acl_entry, char *buff, size_t buffsize)
{
  Protocol *protocol = thd->protocol;
  ...
  if (def_rolename.length)
  {
    ...
    def_str.append('@');
    append_identifier(thd, &def_str, acl_entry->host.hostname, acl_entry->hostname_length);
    protocol->prepare_for_resend();
...
static bool show_role_grants(THD *thd, const char *hostname, ACL_USER_BASE *acl_entry, char *buff, size_t buffsize)
{
  size_t counter;
  Protocol *protocol = thd->protocol;
  ...
  for (counter = 0; counter < acl_entry->role_grants.elements; counter++)
  {
    ...
    ACL_ROLE *acl_role = *(dynamic_element(&acl_entry->role_grants, counter, ACL_ROLE **));
    ...
    ROLE_GRANT_PAIR *pair = find_role_grant_pair(&acl_entry->user, &host, &acl_role->user);
    DBUG_ASSERT(pair);
...
static bool show_global_privileges(THD *thd, ACL_USER_BASE *acl_entry, bool handle_as_role, char *buff, size_t buffsize)
{
  uint counter;
  privilege_t want_access(NO_ACL);
  Protocol *protocol = thd->protocol;
  ...
  if (!handle_as_role)
    add_user_parameters(thd, &global, (ACL_USER *)acl_entry, (want_access & GRANT_ACL));
...
static void add_to_user(THD *thd, String *result, const char *user, bool is_user, const char *host)
{
...
static bool show_database_privileges(THD *thd, const char *username, const char *hostname, char *buff, size_t buffsize)
{
  privilege_t want_access(NO_ACL);
  Protocol *protocol = thd->protocol;
  ...
  for (size_t i = 0; i < acl_dbs.elements(); i++)
  {
    ...
    */
    ...
    if (!strcmp(username, user) && !my_strcasecmp(system_charset_info, hostname, host))
    {
      /*
...
static bool show_table_and_column_privileges(THD *thd, const char *username, const char *hostname, char *buff,
                                             size_t buffsize)
{
  uint counter, index;
  Protocol *protocol = thd->protocol;
  ...
  {
    const char *user, *host;
    GRANT_TABLE *grant_table = (GRANT_TABLE *)my_hash_element(&column_priv_hash, index);
          ...
          for (counter = 0, j = SELECT_ACL; j <= TABLE_ACLS; counter++, j <<= 1)
          {
            if (test_access & j)
            {
              ...
              if (grant_table->cols)
              {
                ...
                for (uint col_index = 0; col_index < hash_columns->records; col_index++)
                {
                  GRANT_COLUMN *grant_column = (GRANT_COLUMN *)my_hash_element(hash_columns, col_index);
...
static int show_routine_grants(THD *thd, const char *username, const char *hostname, const Sp_handler *sph, char *buff,
                               int buffsize)
{
  uint counter, index;
  int error = 0;
...
*/
static int handle_roles_mappings_table(TABLE *table, bool drop, LEX_USER *user_from, LEX_USER *user_to)
{
  /*
  ...
  */
  ...
  DBUG_PRINT("info", ("Rewriting entry in roles_mapping table: %s@%s", user_from->user.str, user_from->host.str));
  table->use_all_columns();
  ...
  {
    while ((error = table->file->ha_rnd_next(table->record[0])) != HA_ERR_END_OF_FILE)
    {
      if (error)
      {
      ...
      else
      {
        ...
        {
          store_record(table, record[1]);
          role_field->store(user_to->user.str, user_to->user.length, system_charset_info);
          if (unlikely(error = table->file->ha_update_row(table->record[1], table->record[0])) &&
              error != HA_ERR_RECORD_IS_THE_SAME)
            table->file->print_error(error, MYF(0));
        }
...
static int handle_grant_table(THD *thd, const Grant_table_base &grant_table, enum enum_acl_tables which_table,
                              bool drop, LEX_USER *user_from, LEX_USER *user_to)
{
  ...
  Field *host_field = table->field[0];
  Field *user_field = table->field[which_table == USER_TABLE || which_table == PROXIES_PRIV_TABLE ? 1 : 2];
  ...
  if (which_table == USER_TABLE)  // mysql.user table
  {
    ...
    */
    ...
    error = table->file->ha_index_read_idx_map(table->record[0], 0, user_key, (key_part_map)3, HA_READ_KEY_EXACT);
    if (!unlikely(error) && !*host_str)
    {
      // verify that we got a role or a user, as needed
      if (static_cast<const User_table &>(grant_table).get_is_role() != user_from->is_role())
  ...
  else
  {
    ...
    */
    ...
    {
#ifdef EXTRA_DEBUG
      DBUG_PRINT("info", ("scan table: '%s'  search: '%s'@'%s'", table->s->table_name.str, user_str, host_str));
#endif
      while ((error = table->file->ha_rnd_next(table->record[0])) != HA_ERR_END_OF_FILE)
      {
        if (error)
...
static int handle_grant_struct(enum enum_acl_lists struct_no, bool drop, LEX_USER *user_from, LEX_USER *user_to)
{
  ...
  if (struct_no == ROLE_ACL)  // no need to scan the structures in this case
  {
    ...
    if (drop)
    {
      ...
      {
        ACL_ROLE *grant = *dynamic_element(&acl_role->role_grants, i, ACL_ROLE **);
        remove_ptr_from_dynarray(&grant->parent_grantee, acl_role);
      }

      for (size_t i = 0; i < acl_role->parent_grantee.elements; i++)
      {
        ACL_USER_BASE *grantee = *dynamic_element(&acl_role->parent_grantee, i, ACL_USER_BASE **);
    ...
    my_hash_update(&acl_roles, (uchar *)acl_role, (uchar *)old_key, old_key_length);
    DBUG_RETURN(1);
  }
      ...
      if (struct_no == ROLES_MAPPINGS_HASH)
      {
        ...
        if (user_from->is_role())
        {
          /* When searching for roles within the ROLES_MAPPINGS_HASH, we have
             ...
             string.
          */
          if ((*host || strcmp(user_from->user.str, user)) && strcmp(user_from->user.str, role))
            continue;
        }
        else
        {
        ...
        switch (struct_no)
        {
          ...
          case ROLES_MAPPINGS_HASH:
          {
            ...
            */
            ...
            if (user_to->is_role())
              oom = role_grant_pair->init(&acl_memroot, role_grant_pair->u_uname, role_grant_pair->u_hname,
                                          user_to->user.str, false);
            else
              oom = role_grant_pair->init(&acl_memroot, user_to->user.str, user_to->host.str, role_grant_pair->r_uname,
                                          false);
            if (oom)
              DBUG_RETURN(-1);

            my_hash_update(roles_mappings_hash, (uchar *)role_grant_pair, (uchar *)old_key, old_key_length);
            restart = true;
            break;
          }
...
static int handle_grant_data(THD *thd, Grant_tables &tables, bool drop, LEX_USER *user_from, LEX_USER *user_to)
{
  int result = 0;
  int found;
  ...
  /* Handle db table. */
  if ((found = handle_grant_table(thd, tables.db_table(), DB_TABLE, drop, user_from, user_to)) < 0)
  {
  ...
  {
    /* Handle db array. */
    if ((handle_grant_struct(DB_ACL, drop, user_from, user_to) || found) && !result)
    {
  ...
  if ((found = handle_grant_table(thd, tables.procs_priv_table(), PROCS_PRIV_TABLE, drop, user_from, user_to)) < 0)
  {
  ...
  {
    /* Handle procs array. */
    if ((handle_grant_struct(PROC_PRIVILEGES_HASH, drop, user_from, user_to) || found) && !result)
    {
      result = 1; /* At least one record/element found. */
      /* If search is requested, we do not need to search further. */
      if (search_only)
    ...
    /* Handle funcs array. */
    if ((handle_grant_struct(FUNC_PRIVILEGES_HASH, drop, user_from, user_to) || found) && !result)
    {
      result = 1; /* At least one record/element found. */
      /* If search is requested, we do not need to search further. */
      if (search_only)
    ...
    if ((handle_grant_struct(PACKAGE_SPEC_PRIVILEGES_HASH, drop, user_from, user_to) || found) && !result)
    {
      result = 1; /* At least one record/element found. */
      /* If search is requested, we do not need to search further. */
      if (search_only)
    ...
    if ((handle_grant_struct(PACKAGE_BODY_PRIVILEGES_HASH, drop, user_from, user_to) || found) && !result)
    {
      result = 1; /* At least one record/element found. */
      /* If search is requested, we do not need to search further. */
      if (search_only)
  ...
  /* Handle tables table. */
  if ((found = handle_grant_table(thd, tables.tables_priv_table(), TABLES_PRIV_TABLE, drop, user_from, user_to)) < 0)
  {
  ...
  else
  {
    ...
    if ((found = handle_grant_table(thd, tables.columns_priv_table(), COLUMNS_PRIV_TABLE, drop, user_from, user_to)) <
        0)
    {
      /* Handle of table failed, don't touch the in-memory array. */
      result = -1;
    ...
    {
      /* Handle columns hash. */
      if ((handle_grant_struct(COLUMN_PRIVILEGES_HASH, drop, user_from, user_to) || found) && !result)
        result = 1; /* At least one record/element found. */
      if (search_only)
        goto end;
    }
  ...
  {
    if ((found = handle_grant_table(thd, tables.proxies_priv_table(), PROXIES_PRIV_TABLE, drop, user_from, user_to)) <
        0)
    {
      /* Handle of table failed, don't touch the in-memory array. */
      result = -1;
    ...
    else
    {
      ...
      if ((handle_grant_struct(PROXY_USERS_ACL, drop, user_from, user_to) || found) && !result)
        result = 1; /* At least one record/element found. */
      if (search_only)
        goto end;
    }
  ...
  if (tables.roles_mapping_table().table_exists() &&
      (found = handle_grant_table(thd, tables.roles_mapping_table(), ROLES_MAPPING_TABLE, drop, user_from, user_to)) <
          0)
  {
    /* Handle of table failed, don't touch the in-memory array. */
    result = -1;
  ...
  else
  {
    ...
    if ((handle_grant_struct(ROLES_MAPPINGS_HASH, drop, user_from, user_to) || found) && !result)
      result = 1; /* At least one record/element found */
    if (search_only)
      goto end;
  }
  ...
  /* Handle user table. */
  if ((found = handle_grant_table(thd, tables.user_table(), USER_TABLE, drop, user_from, user_to)) < 0)
  {
    /* Handle of table failed, don't touch the in-memory array. */
    result = -1;
  }
...
bool mysql_create_user(THD *thd, List<LEX_USER> &list, bool handle_as_role)
{
  ...
  Grant_tables tables;
  const uint tables_to_open = Table_user | Table_db | Table_tables_priv | Table_columns_priv | Table_procs_priv |
    ...
    */
    if (handle_grant_data(thd, tables, 0, user_name, NULL))
    {
      ...
      else if (thd->lex->create_info.if_not_exists())
      {
        binlog = true;
        if (handle_as_role)
          push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_ROLE_CREATE_EXISTS,
                              ER_THD(thd, ER_ROLE_CREATE_EXISTS), user_name->user.str);
        else
          push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_USER_CREATE_EXISTS,
                              ER_THD(thd, ER_USER_CREATE_EXISTS), user_name->user.str, user_name->host.str);
        continue;
      }
      else
      {
    ...
    if (replace_user_table(thd, tables.user_table(), user_name, NO_ACL, 0, 1, 0))
    {
      append_user(thd, &wrong_users, user_name);
      result = TRUE;
      continue;
    ...
    if (handle_as_role)
    {
      ACL_USER_BASE *grantee = find_acl_user_base(thd->lex->definer->user.str, thd->lex->definer->host.str);
      ...
      */
      ...
      /* TODO(cvicentiu) refactor replace_roles_mapping_table to use
         Roles_mapping_table instead of TABLE directly. */
      if (replace_roles_mapping_table(tables.roles_mapping_table().table(), &thd->lex->definer->user,
                                      &thd->lex->definer->host, &user_name->user, true, NULL, false))
      {
        append_user(thd, &wrong_users, user_name);
        if (grantee)
          undo_add_role_user_mapping(grantee, role);
        ...
        update_role_mapping(&thd->lex->definer->user, &thd->lex->definer->host, &user_name->user, true, NULL, false);
    }
  }
  ...
  if (result)
  {
    my_error(ER_CANNOT_USER, MYF(0), (handle_as_role) ? "CREATE ROLE" : "CREATE USER", wrong_users.c_ptr_safe());
  }
...
bool mysql_drop_user(THD *thd, List<LEX_USER> &list, bool handle_as_role)
{
  ...
  while ((tmp_user_name = user_list++))
  {
    ...
    if (rc == 0 && thd->lex->if_exists())
    {
      ...
      if (handle_as_role)
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_ROLE_DROP_EXISTS, ER_THD(thd, ER_ROLE_DROP_EXISTS),
                            user_name->user.str);
      else
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_USER_DROP_EXISTS, ER_THD(thd, ER_USER_DROP_EXISTS),
  ...
  if (result)
    my_error(ER_CANNOT_USER, MYF(0), (handle_as_role) ? "DROP ROLE" : "DROP USER", wrong_users.c_ptr_safe());
...
bool mysql_rename_user(THD *thd, List<LEX_USER> &list)
{
  ...
  while ((tmp_user_from = user_list++))
  {
    ...
    */
    if (handle_grant_data(thd, tables, 0, user_to, NULL) || handle_grant_data(thd, tables, 0, user_from, user_to) <= 0)
    {
...
int mysql_alter_user(THD *thd, List<LEX_USER> &users_list)
{
  ...
  while ((tmp_lex_user = users_list_iterator++))
  {
    ...
    if (!lex_user || replace_user_table(thd, tables.user_table(), lex_user, NO_ACL, false, false, true))
    {
      thd->clear_error();
  ...
  if (result)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_CANNOT_USER, ER_THD(thd, ER_CANNOT_USER),
...
static bool mysql_revoke_sp_privs(THD *thd, Grant_tables *tables, const Sp_handler *sph, const LEX_USER *lex_user)
{
  bool rc = false;
  ...
  do
  {
    ...
    for (counter = 0, revoked = 0; counter < hash->records;)
    {
      ...
      {
        if (replace_routine_table(thd, grant_proc, tables->procs_priv_table().table(), *lex_user, grant_proc->db,
                                  grant_proc->tname, sph, ALL_KNOWN_ACL, 1) == 0)
        {
          revoked = 1;
          continue;
        }
...
bool mysql_revoke_all(THD *thd, List<LEX_USER> &list)
{
  ...
  while ((tmp_lex_user = user_list++))
  {
    ...
    if (!lex_user->is_role() && !find_user_exact(lex_user->host.str, lex_user->user.str))
    {
      result = -1;
      continue;
    }

    if (replace_user_table(thd, tables.user_table(), lex_user, ALL_KNOWN_ACL, 1, 0, 0))
    {
      result = -1;
      continue;
    }
    ...
    do
    {
      for (counter = 0, revoked = 0; counter < acl_dbs.elements();)
      {
        ...
        if (!strcmp(lex_user->user.str, user) && !strcmp(lex_user->host.str, host))
        {
          ...
          if (!replace_db_table(tables.db_table().table(), acl_db->db, *lex_user, ALL_KNOWN_ACL, 1))
          {
            /*
    ...
    do
    {
      ...
      {
        const char *user, *host;
        GRANT_TABLE *grant_table = ((GRANT_TABLE *)my_hash_element(&column_priv_hash, counter));
        ...
        if (!strcmp(lex_user->user.str, user) && !strcmp(lex_user->host.str, host))
        {
          ...
          /* TODO(cvicentiu) refactor replace_db_table to use
             Db_table instead of TABLE directly. */
          if (replace_column_table(grant_table, tables.columns_priv_table().table(), *lex_user, columns,
                                   grant_table->db, grant_table->tname, ALL_KNOWN_ACL, 1))
            result = -1;

          /* TODO(cvicentiu) refactor replace_db_table to use
             Db_table instead of TABLE directly. */
          if ((res = replace_table_table(thd, grant_table, tables.tables_priv_table().table(), *lex_user,
                                         grant_table->db, grant_table->tname, ALL_KNOWN_ACL, NO_ACL, 1)))
          {
            if (res > 0)
              result = -1;
            else
    ...
    */
    ...
    {
      ACL_ROLE *role_grant = *dynamic_element(&user_or_role->role_grants, counter, ACL_ROLE **);
      ROLE_GRANT_PAIR *pair = find_role_grant_pair(&lex_user->user, &lex_user->host, &role_grant->user);
      /* TODO(cvicentiu) refactor replace_roles_mapping_table to use
         Roles_mapping_table instead of TABLE directly. */
      if (replace_roles_mapping_table(tables.roles_mapping_table().table(), &lex_user->user, &lex_user->host,
                                      &role_grant->user, false, pair, true))
      {
        result = -1;  // Something went wrong
      }
      update_role_mapping(&lex_user->user, &lex_user->host, &role_grant->user, false, pair, true);
      /*
...
class Silence_routine_definer_errors : public Internal_error_handler
{
 public:
  ...
  virtual bool handle_condition(THD *thd, uint sql_errno, const char *sqlstate,
                                Sql_condition::enum_warning_level *level, const char *msg, Sql_condition **cond_hdl);
...
bool Silence_routine_definer_errors::handle_condition(THD *thd, uint sql_errno, const char *,
  ...
  if (*level == Sql_condition::WARN_LEVEL_ERROR)
  {
    switch (sql_errno)
    {
      case ER_NONEXISTING_PROC_GRANT:
        ...
        push_warning(thd, Sql_condition::WARN_LEVEL_WARN, sql_errno, msg);
        return TRUE;
      default:
        is_grave = TRUE;
    }
...
bool sp_revoke_privileges(THD *thd, const char *sp_db, const char *sp_name, const Sp_handler *sph)
{
  uint counter, revoked;
  int result;
  HASH *hash = sph->get_priv_hash();
...
bool sp_grant_privileges(THD *thd, const char *sp_db, const char *sp_name, const Sp_handler *sph)
{
  Security_context *sctx = thd->security_ctx;
  LEX_USER *combo;
  TABLE_LIST tables[1];
  ...
  mysql_mutex_lock(&acl_cache->lock);
  if ((au = find_user_exact(combo->host.str = (char *)sctx->priv_host, combo->user.str)))
    goto found_acl;
...
found_acl:
  ...
  */
  thd->push_internal_handler(&error_handler);
  result = mysql_routine_grant(thd, tables, sph, user_list, DEFAULT_CREATE_PROC_ACLS, FALSE, FALSE);
  thd->pop_internal_handler();
  DBUG_RETURN(result);
}
...
static ACL_PROXY_USER *acl_find_proxy_user(const char *user, const char *host, const char *ip,
                                           const char *authenticated_as, bool *proxy_used)
{
  ...
  *proxy_used = TRUE;
  ...
  {
    ACL_PROXY_USER *proxy = dynamic_element(&acl_proxy_users, i, ACL_PROXY_USER *);
...
bool acl_check_proxy_grant_access(THD *thd, const char *host, const char *user, bool with_grant)
{
  ...
  */
  ...
  {
    DBUG_PRINT("info", ("strcmp (%s, %s) my_casestrcmp (%s, %s) equal", thd->security_ctx->priv_user, user, host,
  ...
  {
    ACL_PROXY_USER *proxy = dynamic_element(&acl_proxy_users, i, ACL_PROXY_USER *);
    if (proxy->matches(thd->security_ctx->host, thd->security_ctx->user, thd->security_ctx->ip, user) &&
  ...
  mysql_mutex_unlock(&acl_cache->lock);
  my_error(ER_ACCESS_DENIED_NO_PASSWORD_ERROR, MYF(0), thd->security_ctx->user, thd->security_ctx->host_or_ip);
  DBUG_RETURN(TRUE);
}

static bool show_proxy_grants(THD *thd, const char *username, const char *hostname, char *buff, size_t buffsize)
{
  Protocol *protocol = thd->protocol;
  int error = 0;
...
static int enabled_roles_insert(ACL_USER_BASE *role, void *context_data)
{
  ...
  table->field[0]->store(role->user.str, role->user.length, system_charset_info);
...
static int applicable_roles_insert(ACL_USER_BASE *grantee, ACL_ROLE *role, void *ptr)
{
  APPLICABLE_ROLES_DATA *data = (APPLICABLE_ROLES_DATA *)ptr;
  CHARSET_INFO *cs = system_charset_info;
  TABLE *table = data->table;
  bool is_role = grantee != data->user;
  const LEX_CSTRING *user_and_host = is_role ? &grantee->user : &data->user_and_host;
  ...
  ROLE_GRANT_PAIR *pair = find_role_grant_pair(&grantee->user, host, &role->user);
  DBUG_ASSERT(pair);
  ...
  if (!is_role)
  {
    if (data->user->default_rolename.length && lex_string_eq(&data->user->default_rolename, &role->user))
...
*/
static my_bool count_column_grants(void *grant_table, void *current_count)
{
...
*/
static int show_column_grants(THD *thd, SHOW_VAR *var, char *buff, enum enum_var_type scope)
{
  var->type = SHOW_ULONG;
  var->value = buff;
  *(ulong *)buff = 0;
...
static int show_database_grants(THD *thd, SHOW_VAR *var, char *buff, enum enum_var_type scope)
{
  var->type = SHOW_UINT;
  var->value = buff;
...
#else
static bool set_user_salt_if_needed(ACL_USER *, int, plugin_ref) { return 0; }
bool check_grant(THD *, privilege_t, TABLE_LIST *, bool, uint, bool) { return 0; }
...
void Sql_cmd_grant::warn_hostname_requires_resolving(THD *thd, List<LEX_USER> &users)
{
  LEX_USER *user;
  ...
  while ((user = it++))
  {
    if (specialflag & SPECIAL_NO_RESOLVE && hostname_requires_resolving(user->host.str))
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_HOSTNAME_WONT_WORK,
...
bool Sql_cmd_grant_proxy::check_access_proxy(THD *thd, List<LEX_USER> &users)
{
  ...
  if ((user = it++))
  {
    ...
    if (!(user = get_current_user(thd, user)) || !user->host.str)
      return true;
    if (acl_check_proxy_grant_access(thd, user->host.str, user->user.str, m_grant_option & GRANT_ACL))
      return true;
  }
  return false;
}
...
bool Sql_cmd_grant_proxy::execute(THD *thd)
{
  ...
  if (thd->security_ctx->user /* If not replication */ && check_access_proxy(thd, lex->users_list))
    return true;

  WSREP_TO_ISOLATION_BEGIN(WSREP_MYSQL_DB, NULL, NULL);
  /* Conditionally writes to binlog */
  if (mysql_grant(thd, NULL /*db*/, lex->users_list, m_grant_option, is_revoke(), true /*proxy*/))
    return true;
...
bool Sql_cmd_grant_object::grant_stage0_exact_object(THD *thd, TABLE_LIST *table)
{
  privilege_t priv = m_object_privilege | m_column_privilege_total | GRANT_ACL;
  if (check_access(thd, priv, table->db.str, &table->grant.privilege, &table->grant.m_internal, 0, 0))
    return true;
  grant_stage0(thd);
  return false;
}
...
bool Sql_cmd_grant_table::execute_exact_table(THD *thd, TABLE_LIST *table)
{
  ...
  if (grant_stage0_exact_object(thd, table) ||
      check_grant(thd, m_object_privilege | m_column_privilege_total | GRANT_ACL, lex->query_tables, FALSE, UINT_MAX,
                  FALSE))
    return true;
  /* Conditionally writes to binlog */
  WSREP_TO_ISOLATION_BEGIN(WSREP_MYSQL_DB, NULL, NULL);
  return mysql_table_grant(thd, lex->query_tables, lex->users_list, m_columns, m_object_privilege, is_revoke());
#ifdef WITH_WSREP
wsrep_error_label:
  return true;
#endif  // WITH_WSREP
...
bool Sql_cmd_grant_sp::execute(THD *thd)
{
  ...
  privilege_t grants =
      m_all_privileges ? (PROC_ACLS & ~GRANT_ACL) | (m_object_privilege & GRANT_ACL) : m_object_privilege;
  ...
  /* Conditionally writes to binlog */
  WSREP_TO_ISOLATION_BEGIN(WSREP_MYSQL_DB, NULL, NULL);
  if (mysql_routine_grant(thd, lex->query_tables, &m_sph, lex->users_list, grants, is_revoke(), true))
    return true;
  my_ok(thd);
  return false;
#ifdef WITH_WSREP
...
bool Sql_cmd_grant_table::execute_table_mask(THD *thd)
{
  ...
  if (check_access(thd, m_object_privilege | m_column_privilege_total | GRANT_ACL, m_db.str, NULL, NULL, 1, 0))
    return true;
  ...
  WSREP_TO_ISOLATION_BEGIN(WSREP_MYSQL_DB, NULL, NULL);
  /* Conditionally writes to binlog */
  if (mysql_grant(thd, m_db.str, lex->users_list, m_object_privilege, is_revoke(), false /*not proxy*/))
    return true;
...
bool Sql_cmd_grant_table::execute(THD *thd)
{
  ...
  return table ? execute_exact_table(thd, table) : execute_table_mask(thd);
}
...
/* Check if a role is granted to a user/role. We traverse the role graph
   ...
   otherwise a user.
*/
bool check_role_is_granted(const char *username, const char *hostname, const char *rolename)
{
  ...
  if (root && /* No grantee, nothing to search. */
      traverse_role_graph_down(root, &role_lex, check_role_is_granted_callback, NULL) == -1)
  {
...
int fill_schema_applicable_roles(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  if (initialized)
  {
    ...
    if (user)
    {
      ...
      APPLICABLE_ROLES_DATA data = {
          table, {user->host.hostname, user->hostname_length}, {buff, (size_t)(end - buff)}, user};
...
#ifndef NO_EMBEDDED_ACCESS_CHECKS
static bool update_schema_privilege(THD *thd, TABLE *table, const char *buff, const char *db, const char *t_name,
                                    const char *column, uint col_length, const char *priv, uint priv_length,
...
int fill_schema_user_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  ACL_USER *acl_user;
  TABLE *table = tables->table;
  bool no_global_access = check_access(thd, SELECT_ACL, "mysql", NULL, NULL, 1, 1);
  ...
  for (counter = 0; counter < acl_users.elements; counter++)
  {
    ...
    if (no_global_access && !thd->security_ctx->is_priv_user(user, host))
      continue;
    ...
    if (!(want_access & ~GRANT_ACL))
    {
      if (update_schema_privilege(thd, table, grantee, 0, 0, 0, 0, STRING_WITH_LEN("USAGE"), is_grantable))
      {
        error = 1;
        goto err;
      }
    ...
    else
    {
      ...
      for (priv_id = 0, j = SELECT_ACL; j <= GLOBAL_ACLS; priv_id++, j <<= 1)
      {
        if (test_access & j)
        {
          if (update_schema_privilege(thd, table, grantee, 0, 0, 0, 0, command_array[priv_id], command_lengths[priv_id],
                                      is_grantable))
          {
            error = 1;
            goto err;
...
int fill_schema_schema_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (counter = 0; counter < acl_dbs.elements(); counter++)
  {
    ...
    if (want_access)
    {
      ...
      if (!(want_access & ~GRANT_ACL))
      {
        if (update_schema_privilege(thd, table, grantee, acl_db->db, 0, 0, 0, STRING_WITH_LEN("USAGE"), is_grantable))
        {
          error = 1;
          goto err;
        }
      ...
      else
      {
        ...
        for (cnt = 0, j = SELECT_ACL; j <= DB_ACLS; cnt++, j <<= 1)
          if (test_access & j)
          {
            if (update_schema_privilege(thd, table, grantee, acl_db->db, 0, 0, 0, command_array[cnt],
  ...
  for (index = 0; index < column_priv_hash.records; index++)
  {
    ...
    if (table_access)
    {
      ...
      */
      ...
      if (!test_access)
      {
        if (update_schema_privilege(thd, table, grantee, grant_table->db, grant_table->tname, 0, 0,
      ...
      else
      {
        ...
        for (cnt = 0, j = SELECT_ACL; j <= TABLE_ACLS; cnt++, j <<= 1)
        {
          if (test_access & j)
          {
            ...
            {
              GRANT_COLUMN *grant_column = (GRANT_COLUMN *)my_hash_element(&grant_table->hash_columns, col_index);
              if ((grant_column->rights & j) && (table_access & j))
              {
                if (update_schema_privilege(thd, table, grantee, grant_table->db, grant_table->tname,
                                            grant_column->column, grant_column->key_length, command_array[cnt],
                                            command_lengths[cnt], is_grantable))
                {
                  error = 1;
                  goto err;
...
void fill_effective_table_privileges(THD *thd, GRANT_INFO *grant, const char *db, const char *table)
{
  ...
  DBUG_PRINT("enter", ("Host: '%s', Ip: '%s', User: '%s', table: `%s`.`%s`", sctx->priv_host, sctx->ip, sctx->priv_user,
                       db, table));
  /* --skip-grants */
  if (!initialized)
  {
  ...
  if (grant->version != grant_version)
  {
    ...
    grant->grant_table_role = sctx->priv_role[0] ? table_hash_search("", "", db, sctx->priv_role, table, TRUE) : NULL;
    grant->version = grant_version; /* purecov: inspected */
  }
...
bool check_routine_level_acl(THD *thd, const char *db, const char *name, const Sp_handler *sph) { return FALSE; }
...
*/
void ACL_internal_schema_registry::register_schema(const LEX_CSTRING *name, const ACL_internal_schema_access *access)
{
...
const ACL_internal_schema_access *ACL_internal_schema_registry::lookup(const char *name)
{
  ...
  {
    if (my_strcasecmp(system_charset_info, registry_array[i].m_name->str, name) == 0)
                                                           ...
                                                           const char *schema_name)
{
  if (grant_internal_info)
  {
    ...
    {
      grant_internal_info->m_schema_access = ACL_internal_schema_registry::lookup(schema_name);
...
{
  my_error(access_denied_error_code(thd->password), MYF(0), thd->main_security_ctx.user,
           thd->main_security_ctx.host_or_ip, thd->password ? ER_THD(thd, ER_YES) : ER_THD(thd, ER_NO));
  general_log_print(thd, COM_CONNECT, ER_THD(thd, access_denied_error_code(thd->password)), thd->main_security_ctx.user,
                    thd->main_security_ctx.host_or_ip, thd->password ? ER_THD(thd, ER_YES) : ER_THD(thd, ER_NO));
  status_var_increment(thd->status_var.access_denied_errors);
  /*
  ...
  */
  ...
  {
    sql_print_warning(ER_THD(thd, access_denied_error_code(thd->password)), thd->main_security_ctx.user,
                      thd->main_security_ctx.host_or_ip, thd->password ? ER_THD(thd, ER_YES) : ER_THD(thd, ER_NO));
  }
}
...
static bool send_server_handshake_packet(MPVIO_EXT *mpvio, const char *data, uint data_len)
{
  ...
  *end++ = 0;
  ...
  /* write scramble tail */
  end = (char *)memcpy(end, data + SCRAMBLE_LENGTH_323, data_len - SCRAMBLE_LENGTH_323);
  end += data_len - SCRAMBLE_LENGTH_323;
  end = strmake(end, plugin_name(mpvio->plugin)->str, plugin_name(mpvio->plugin)->length);

  int res = my_net_write(&mpvio->auth_info.thd->net, (uchar *)buff, (size_t)(end - buff + 1)) ||
...
static bool secure_auth(THD *thd)
{
  ...
  */
  ...
  {
    my_error(ER_SERVER_IS_IN_SECURE_AUTH_MODE, MYF(0), thd->security_ctx->user, thd->security_ctx->host_or_ip);
    general_log_print(thd, COM_CONNECT, ER_THD(thd, ER_SERVER_IS_IN_SECURE_AUTH_MODE), thd->security_ctx->user,
                      thd->security_ctx->host_or_ip);
  }
  else
  {
    my_error(ER_NOT_SUPPORTED_AUTH_MODE, MYF(0));
    general_log_print(thd, COM_CONNECT, ER_THD(thd, ER_NOT_SUPPORTED_AUTH_MODE));
  }
  return 1;
}
...
static bool send_plugin_request_packet(MPVIO_EXT *mpvio, const uchar *data, uint data_len)
{
  ...
  const char *client_auth_plugin = ((st_mysql_auth *)(plugin_decl(mpvio->plugin)->info))->client_auth_plugin;
  ...
  */
  bool switch_from_long_to_short_scramble =
      ...
      my_strcasecmp(system_charset_info, mpvio->cached_client_reply.plugin, native_password_plugin_name.str) == 0;

  if (switch_from_long_to_short_scramble)
    DBUG_RETURN(secure_auth(mpvio->auth_info.thd) || my_net_write(net, switch_plugin_request_buf, 1) || net_flush(net));
  ...
  */
  bool switch_from_short_to_long_scramble =
      ...
      my_strcasecmp(system_charset_info, mpvio->cached_client_reply.plugin, old_password_plugin_name.str) == 0;

  if (switch_from_short_to_long_scramble)
  {
    my_error(ER_NOT_SUPPORTED_AUTH_MODE, MYF(0));
    general_log_print(mpvio->auth_info.thd, COM_CONNECT, ER_THD(mpvio->auth_info.thd, ER_NOT_SUPPORTED_AUTH_MODE));
    DBUG_RETURN(1);
  }

  DBUG_PRINT("info", ("requesting client to use the %s plugin", client_auth_plugin));
  DBUG_RETURN(net_write_command(net, switch_plugin_request_buf[0], (uchar *)client_auth_plugin,
                                strlen(client_auth_plugin) + 1, (uchar *)data, data_len));
}
...
static bool find_mpvio_user(MPVIO_EXT *mpvio)
{
  ...
  if (mpvio->acl_user->password_errors >= max_password_errors && !ignore_max_password_errors(mpvio->acl_user))
  {
    my_error(ER_USER_IS_BLOCKED, MYF(0));
    general_log_print(mpvio->auth_info.thd, COM_CONNECT, ER_THD(mpvio->auth_info.thd, ER_USER_IS_BLOCKED));
    DBUG_RETURN(1);
  }
  ...
  {
    DBUG_ASSERT(my_strcasecmp(system_charset_info, mpvio->acl_user->auth->plugin.str, native_password_plugin_name.str));
    DBUG_ASSERT(my_strcasecmp(system_charset_info, mpvio->acl_user->auth->plugin.str, old_password_plugin_name.str));
    my_error(ER_NOT_SUPPORTED_AUTH_MODE, MYF(0));
    general_log_print(mpvio->auth_info.thd, COM_CONNECT, ER_THD(mpvio->auth_info.thd, ER_NOT_SUPPORTED_AUTH_MODE));
    DBUG_RETURN(1);
  }
...
static bool read_client_connect_attrs(char **ptr, char *end, CHARSET_INFO *from_cs)
{
  ulonglong length;
  char *ptr_save = *ptr;
  ...
  if (PSI_CALL_set_thread_connect_attrs(*ptr, (uint)length, from_cs) && current_thd->variables.log_warnings)
    sql_print_warning("Connection attributes of length %llu were truncated", length);
  return false;
}
...
static bool parse_com_change_user_packet(MPVIO_EXT *mpvio, uint packet_length)
{
  ...
  */
  uint passwd_len = (thd->client_capabilities & CLIENT_SECURE_CONNECTION ? (uchar)(*passwd++) : (uint)strlen(passwd));
  ...
  */
  ...
  db_len =
      copy_and_convert(db_buff, sizeof(db_buff) - 1, system_charset_info, db, db_len, thd->charset(), &dummy_errors);

  user_len = copy_and_convert(user_buff, sizeof(user_buff) - 1, system_charset_info, user, user_len, thd->charset(),
                              &dummy_errors);

  if (!(sctx->user = my_strndup(key_memory_MPVIO_EXT_auth_info, user_buff, user_len, MYF(MY_WME))))
    DBUG_RETURN(1);
  ...
  */
  ...
  else
  {
    ...
    else
    {
      ...
      */
      client_plugin = passwd_len ? old_password_plugin_name.str : native_password_plugin_name.str;
    }
  }

  if ((thd->client_capabilities & CLIENT_CONNECT_ATTRS) && read_client_connect_attrs(&next_field, end, thd->charset()))
  {
    my_message(ER_UNKNOWN_COM_ERROR, ER_THD(thd, ER_UNKNOWN_COM_ERROR), MYF(0));
    DBUG_RETURN(1);
  }
...
static ulong parse_client_handshake_packet(MPVIO_EXT *mpvio, uchar **buff, ulong pkt_len)
{
  ...
  */
  ...
  if (client_capabilities & CLIENT_PROTOCOL_41)
  {
    ...
    if (!(client_capabilities & CLIENT_MYSQL))
    {
      ...
      ulonglong ext_client_capabilities = (((ulonglong)uint4korr(net->read_pos + 28)) << 32);
  ...
  */
  ...
  {
    len = safe_net_field_length_ll((uchar **)&passwd, net->read_pos + pkt_len - (uchar *)passwd);
    if (len > pkt_len)
      return packet_error;
  }
  ...
  if (passwd == NULL || passwd + passwd_len + MY_TEST(db) > (char *)net->read_pos + pkt_len)
    return packet_error;
  ...
  */
  if (unlikely(thd->copy_with_error(system_charset_info, (LEX_STRING *)&mpvio->db, thd->charset(), db, db_len)))
    return packet_error;
  ...
  /*
    ...
    old 5.1 did)
  */
  user_len = Well_formed_prefix(system_charset_info, user, user_len, username_char_length).length();
  user[user_len] = '\0';
  ...
  */
  ...
  if ((thd->client_capabilities & CLIENT_PLUGIN_AUTH) && (client_plugin < (char *)net->read_pos + pkt_len))
  {
    next_field += strlen(next_field) + 1;
  }
  else
  ...
  if ((thd->client_capabilities & CLIENT_CONNECT_ATTRS) &&
      read_client_connect_attrs(&next_field, ((char *)net->read_pos) + pkt_len, mpvio->auth_info.thd->charset()))
    return packet_error;
  ...
  */
  ...
  if (client_auth_plugin && my_strcasecmp(system_charset_info, client_plugin, client_auth_plugin))
  {
    mpvio->cached_client_reply.plugin = client_plugin;
    if (send_plugin_request_packet(mpvio, (uchar *)mpvio->cached_server_packet.pkt,
...
static int server_mpvio_write_packet(MYSQL_PLUGIN_VIO *param, const uchar *packet, int packet_len)
{
  ...
  else if (packet_len > 0 && (*packet == 1 || *packet == 255 || *packet == 254))
  {
    ...
    */
    res = net_write_command(&mpvio->auth_info.thd->net, 1, (uchar *)"", 0, packet, packet_len);
  }
  else
  {
    res = my_net_write(&mpvio->auth_info.thd->net, packet, packet_len) || net_flush(&mpvio->auth_info.thd->net);
  }
...
static int server_mpvio_read_packet(MYSQL_PLUGIN_VIO *param, uchar **buf)
{
  ...
  if (mpvio->status == MPVIO_EXT::RESTART)
  {
    ...
    if (mpvio->cached_client_reply.pkt)
    {
      ...
      */
      if (my_strcasecmp(system_charset_info, mpvio->cached_client_reply.plugin, client_auth_plugin) == 0)
      {
        mpvio->status = MPVIO_EXT::FAILURE;
  ...
  */
  ...
  if (set_user_salt_if_needed(mpvio->acl_user, mpvio->curr_auth, mpvio->plugin))
  {
    ...
    my_error(ER_ACCESS_DENIED_ERROR, MYF(0), ai->thd->security_ctx->user, ai->thd->security_ctx->host_or_ip,
             ER_THD(ai->thd, ER_YES));
    goto err;
  }
  ...
  switch (acl_user->ssl_type)
  {
    ...
    case SSL_TYPE_SPECIFIED: /* Client should have specified attrib */
      ...
      if (acl_user->ssl_cipher)
      {
        ...
        DBUG_PRINT("info", ("comparing ciphers: '%s' and '%s'", acl_user->ssl_cipher, ssl_cipher));
      ...
      if (acl_user->x509_subject[0])
      {
        ...
        {
          if (global_system_variables.log_warnings)
            sql_print_information("X509 subject mismatch: should be '%s' but is '%s'", acl_user->x509_subject, ptr);
          free(ptr);
          X509_free(cert);
          return 1;
        }
...
bool acl_authenticate(THD *thd, uint com_change_user_pkt_len)
{
  int res = CR_OK;
  MPVIO_EXT mpvio;
  enum enum_server_command command = com_change_user_pkt_len ? COM_CHANGE_USER : COM_CONNECT;
  ...
  mpvio.auth_info.host_or_ip_length = (unsigned int)strlen(thd->security_ctx->host_or_ip);
  ...
  if (command == COM_CHANGE_USER)
  {
    ...
    DBUG_ASSERT(mpvio.status == MPVIO_EXT::RESTART || mpvio.status == MPVIO_EXT::SUCCESS);
  }
  else
  {
  ...
  if (acl_user)
  {
    ...
    */
    for (mpvio.curr_auth = mpvio.status != MPVIO_EXT::RESTART; res != CR_OK && mpvio.curr_auth < acl_user->nauth;
         mpvio.curr_auth++)
    {
      thd->clear_error();
      mpvio.status = MPVIO_EXT::RESTART;
  ...
  */
  if (sctx->user)
  {
    general_log_print(thd, command, (char *)"%s@%s on %s using %s", sctx->user, sctx->host_or_ip,
  ...
  if (initialized)  // if not --skip-grant-tables
  {
    ...
    */
    ...
    bool client_can_handle_exp_pass = thd->client_capabilities & CLIENT_CAN_HANDLE_EXPIRED_PASSWORDS;
    bool password_expired = thd->password != PASSWORD_USED_NO_MENTION &&
                            (acl_user->password_expired || check_password_lifetime(thd, *acl_user));

    if (!client_can_handle_exp_pass && disconnect_on_expired_password && password_expired)
    {
      status_var_increment(denied_connections);
    ...
    if (!password_expired)
    {
      ...
      proxy_user =
          acl_find_proxy_user(auth_user, sctx->host, sctx->ip, mpvio.auth_info.authenticated_as, &is_proxy_user);
      if (is_proxy_user)
      {
        ...
        my_snprintf(sctx->proxy_user, sizeof(sctx->proxy_user) - 1, "'%s'@'%s'", auth_user,
                    safe_str(acl_user->host.hostname));

        /* we're proxying : find the proxy user definition */
        mysql_mutex_lock(&acl_cache->lock);
        acl_proxy_user = find_user_exact(safe_str(proxy_user->get_proxied_host()), mpvio.auth_info.authenticated_as);
        if (!acl_proxy_user)
        {
          mysql_mutex_unlock(&acl_cache->lock);
    ...
    */
    if ((acl_user->user_resource.questions || acl_user->user_resource.updates ||
         ...
         acl_user->user_resource.max_statement_time != 0.0 || max_user_connections_checking) &&
        get_or_create_user_conn(thd, (opt_old_style_user_limits ? sctx->user : sctx->priv_user),
    ...
    {
      thd->variables.max_statement_time_double = acl_user->user_resource.max_statement_time;
      thd->variables.max_statement_time = (ulonglong)(thd->variables.max_statement_time_double * 1e6 + 0.1);
    }
  }
  else
    sctx->skip_grants();
  ...
  DBUG_PRINT("info", ("Capabilities: %llu  packet_length: %ld  Host: '%s'  "
                      "Login user: '%s' Priv_user: '%s'  Using password: %s "
                      "Access: %llx  db: '%s'",
                      thd->client_capabilities, thd->max_client_packet_length, sctx->host_or_ip, sctx->user,
                      sctx->priv_user, thd->password ? "yes" : "no", (longlong)sctx->master_access, mpvio.db.str));

  if (command == COM_CONNECT && !(thd->main_security_ctx.master_access & PRIV_IGNORE_MAX_CONNECTIONS))
  {
  ...
  */
    ...
    if (err)
    {
      if (err == ER_DBACCESS_DENIED_ERROR)
      {
        ...
        */
        ...
        {
          Security_context *sctx = thd->security_ctx;
          sql_print_warning(ER_THD(thd, err), sctx->priv_user, sctx->priv_host, mpvio.db.str);
        }
      }
      DBUG_RETURN(1);
    }
  ...
  if (mpvio.auth_info.external_user[0])
    sctx->external_user = my_strdup(key_memory_MPVIO_EXT_auth_info, mpvio.auth_info.external_user, MYF(0));
  ...
  PSI_CALL_set_thread_account(thd->main_security_ctx.user, static_cast<uint>(strlen(thd->main_security_ctx.user)),
                              thd->main_security_ctx.host_or_ip,
                              static_cast<uint>(strlen(thd->main_security_ctx.host_or_ip)));
...
static int native_password_make_scramble(const char *password, size_t password_length, char *hash, size_t *hash_length)
{
...
static int native_password_get_salt(const char *hash, size_t hash_length, unsigned char *out, size_t *out_length)
{
...
static int old_password_authenticate(MYSQL_PLUGIN_VIO *vio, MYSQL_SERVER_AUTH_INFO *info)
{
  uchar *pkt;
  int pkt_len;
  MPVIO_EXT *mpvio = (MPVIO_EXT *)vio;
  ...
  */
  ...
  if (pkt_len == SCRAMBLE_LENGTH_323)
  {
    ...
    return check_scramble_323(pkt, thd->scramble, (ulong *)info->auth_string) ? CR_AUTH_USER_CREDENTIALS : CR_OK;
  }
...
static int old_password_make_scramble(const char *password, size_t password_length, char *hash, size_t *hash_length)
{
...
static int old_password_get_salt(const char *hash, size_t hash_length, unsigned char *out, size_t *out_length)
{
...
static struct st_mysql_auth native_password_handler = {MYSQL_AUTHENTICATION_INTERFACE_VERSION,
                                                       native_password_plugin_name.str, native_password_authenticate,
                                                       native_password_make_scramble, native_password_get_salt};

static struct st_mysql_auth old_password_handler = {MYSQL_AUTHENTICATION_INTERFACE_VERSION,
                                                    old_password_plugin_name.str, old_password_authenticate,
                                                    old_password_make_scramble, old_password_get_salt};
...
*/
extern "C" int maria_compare_hostname(const char *wild_host, long wild_ip, long ip_mask, const char *host,
                                      const char *ip)
{
#ifndef NO_EMBEDDED_ACCESS_CHECKS
  acl_host_and_ip h;
...
extern "C" void maria_update_hostname(const char **wild_host, long *wild_ip, long *ip_mask, const char *host)
{
#ifndef NO_EMBEDDED_ACCESS_CHECKS
  acl_host_and_ip h;
