...
#ifdef WITH_PARTITION_STORAGE_ENGINE
static int get_part_iter_for_interval_via_mapping(partition_info *, bool, uint32 *, uchar *, uchar *, uint, uint, uint,
                                                  PARTITION_ITERATOR *);
static int get_part_iter_for_interval_cols_via_map(partition_info *, bool, uint32 *, uchar *, uchar *, uint, uint, uint,
                                                   PARTITION_ITERATOR *);
static int get_part_iter_for_interval_via_walking(partition_info *, bool, uint32 *, uchar *, uchar *, uint, uint, uint,
                                                  PARTITION_ITERATOR *);
static int cmp_rec_and_tuple(part_column_list_val *val, uint32 nvals_in_rec);
static int cmp_rec_and_tuple_prune(part_column_list_val *val, uint32 n_vals_in_rec, bool is_left_endpoint,
...
bool partition_default_handling(THD *thd, TABLE *table, partition_info *part_info, bool is_create_table_ind,
                                const char *normalized_path)
{
  ...
  if (!is_create_table_ind)
  {
    ...
    else if (part_info->is_sub_partitioned() && part_info->use_default_num_subpartitions)
    {
      uint num_parts;
  ...
  part_info->set_up_defaults_for_partitioning(thd, table->file, NULL, 0U);
  DBUG_RETURN(FALSE);
}
...
int get_part_for_buf(const uchar *buf, const uchar *rec0, partition_info *part_info, uint32 *part_id)
{
  int error;
  longlong func_value;
...
static bool set_up_field_array(THD *thd, TABLE *table, bool is_sub_part)
{
  ...
  while ((field = *(ptr++)))
  {
    ...
    {
      if (table->versioned(VERS_TRX_ID) && unlikely(field->flags & VERS_SYSTEM_FIELD))
      {
    ...
    if (field->flags & GET_FIXED_FIELDS_FLAG)
    {
      ...
      if (likely(!result))
      {
        if (!is_sub_part && part_info->column_list)
        {
          ...
          {
            field_name = it++;
            if (!my_strcasecmp(system_charset_info, field_name, field->field_name.str))
              break;
...
static bool create_full_part_field_array(THD *thd, TABLE *table, partition_info *part_info)
{
  ...
  */
  if (!(bitmap_buf = (my_bitmap_map *)thd->alloc(bitmap_buffer_size(table->s->fields))))
  {
    result = TRUE;
    goto end;
  }
  if (unlikely(my_bitmap_init(&part_info->full_part_field_set, bitmap_buf, table->s->fields)))
  {
    result = TRUE;
    goto end;
  }
  ...
  */
  ...
  if ((ptr = part_info->full_part_field_array))
    for (; *ptr; ptr++) table->mark_column_with_deps(*ptr);
...
static bool handle_list_of_fields(THD *thd, List_iterator<const char> it, TABLE *table, partition_info *part_info,
                                  bool is_sub_part)
{
  Field *field;
  bool result;
  ...
  if (is_list_empty && part_info->part_type == HASH_PARTITION)
  {
    ...
    {
      if (table->s->db_type()->partition_flags && (table->s->db_type()->partition_flags() & HA_USE_AUTO_PARTITION) &&
...
int check_signed_flag(partition_info *part_info)
{
  int error = 0;
  uint i = 0;
  if (part_info->part_type != HASH_PARTITION && part_info->part_expr->unsigned_flag)
  {
...
static bool fix_fields_part_func(THD *thd, Item *func_expr, TABLE *table, bool is_sub_part, bool is_create_table_ind)
{
  ...
  func_expr->walk(&Item::change_context_processor, 0, &lex.first_select_lex()->context);
  ...
  */
  if (func_expr->walk(&Item::check_valid_arguments_processor, 0, NULL))
  {
    if (is_create_table_ind)
    {
      ...
      push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_EXPR_IN_PARTITION_FUNC_ERROR,
...
static bool check_primary_key(TABLE *table)
{
  ...
  if (primary_key < MAX_KEY)
  {
    ...
    check_fields_in_PF(table->part_info->full_part_field_array, &all_fields, &some_fields);
...
{
  uint32 *bitmap_buf;
  uint bitmap_bits = part_info->num_subparts ? (part_info->num_subparts * part_info->num_parts) : part_info->num_parts;
  ...
  if (unlikely(!(bitmap_buf = (uint32 *)alloc_root(&part_info->table->mem_root, bitmap_bytes * 2))))
    DBUG_RETURN(TRUE);
  ...
  my_bitmap_init(&part_info->lock_partitions, bitmap_buf + (bitmap_bytes / 4), bitmap_bits);
...
static void set_up_partition_key_maps(TABLE *table, partition_info *part_info)
{
  ...
  {
    set_indicator_in_key_fields(table->key_info + i);
    check_fields_in_PF(part_info->full_part_field_array, &all_fields, &some_fields);
    if (all_fields)
    ...
    if (part_info->is_sub_partitioned())
    {
      check_fields_in_PF(part_info->part_field_array, &all_fields, &some_fields);
      if (all_fields)
        part_info->all_fields_in_PPF.set_bit(i);
      check_fields_in_PF(part_info->subpart_field_array, &all_fields, &some_fields);
      if (all_fields)
...
extern "C" int partition_info_list_part_cmp(const void *a, const void *b)
{
...
extern "C" int partition_info_compare_column_values(const void *first_arg, const void *second_arg)
{
  ...
  for (field = part_info->part_field_array; *field; field++, first++, second++)
  {
    ...
    int res = (*field)->cmp((const uchar *)first->column_value, (const uchar *)second->column_value);
    if (res)
      return res;
  }
  return 0;
...
static bool check_range_constants(THD *thd, partition_info *part_info)
{
  ...
  DBUG_PRINT("enter", ("RANGE with %d parts, column_list = %u", part_info->num_parts, part_info->column_list));
  ...
  if (part_info->column_list)
  {
    ...
    part_info->range_col_array = (part_column_list_val *)thd->calloc(part_info->num_parts * size_entries);
    ...
    do
    {
      part_def = it++;
      {
        ...
        if (!first)
        {
          if (partition_info_compare_column_values(current_largest_col_val, col_val) >= 0)
  ...
  else
  {
    ...
    part_info->range_int_array = (longlong *)thd->alloc(part_info->num_parts * sizeof(longlong));
      ...
      if (!first)
      {
        if (current_largest > part_range_value ||
            (current_largest == part_range_value &&
             (part_range_value < LONGLONG_MAX || i != part_info->num_parts - 1 || !part_info->defined_max_value)))
          goto range_not_increasing_error;
      }
...
static bool check_list_constants(THD *thd, partition_info *part_info)
{
  ...
  */
  ...
  size_entries = part_info->column_list ? (num_column_values * sizeof(part_column_list_val)) : sizeof(LIST_PART_ENTRY);
  ...
  else
  {
    ...
    /*
      ...
      partition functions.
    */
    type_add = (longlong)(part_info->part_expr->unsigned_flag ? 0x8000000000000000ULL : 0ULL);
  ...
  if (part_info->num_list_values)
  {
    ...
    */
    my_qsort(part_info->list_array, part_info->num_list_values, size_entries, compare_func);

    i = 0;
    do
    {
      DBUG_ASSERT(i < part_info->num_list_values);
      curr_value = part_info->column_list ? (void *)&part_info->list_col_array[num_column_values * i]
...
static bool check_vers_constants(THD *thd, partition_info *part_info)
{
  ...
  {
    if (date_add_interval(thd, &ltime, vers_info->interval.type, vers_info->interval.step))
      goto err;
    uint error = 0;
    part_info->range_int_array[el->id] = el->range_value = my_tz_OFFSET0->TIME_to_gmt_sec(&ltime, &error);
    if (error)
      goto err;
...
bool fix_partition_func(THD *thd, TABLE *table, bool is_create_table_ind)
{
  ...
  {
    if (partition_default_handling(thd, table, part_info, is_create_table_ind, table->s->normalized_path.str))
    {
      DBUG_RETURN(TRUE);
    }
  }
  if (part_info->is_sub_partitioned())
  {
    ...
    */
    ...
    {
      if (unlikely(fix_fields_part_func(thd, part_info->subpart_expr, table, TRUE, is_create_table_ind)))
        goto end;
  ...
  */
  ...
  if (part_info->part_type == HASH_PARTITION)
  {
    ...
    {
      if (unlikely(fix_fields_part_func(thd, part_info->part_expr, table, FALSE, is_create_table_ind)))
        goto end;
  ...
  else
  {
    ...
    {
      if (part_info->part_type == VERSIONING_PARTITION && part_info->vers_fix_field_list(thd))
        goto end;
      if (unlikely(fix_fields_part_func(thd, part_info->part_expr, table, FALSE, is_create_table_ind)))
        goto end;
    }
    ...
    {
      const char *error_str = part_info->part_type == LIST_PARTITION ? "LIST" : "RANGE";
      my_error(ER_PARTITIONS_MUST_BE_DEFINED_ERROR, MYF(0), error_str);
      goto end;
    }
    if (unlikely(!part_info->column_list && part_info->part_expr->result_type() != INT_RESULT &&
  ...
  if (((part_info->part_type != HASH_PARTITION || part_info->list_of_part_fields == FALSE) && !part_info->column_list &&
       check_part_func_fields(part_info->part_field_array, TRUE)) ||
      (part_info->list_of_subpart_fields == FALSE && part_info->is_sub_partitioned() &&
...
static int add_keyword_string(String *str, const char *keyword, bool quoted, const char *keystr)
{
  int err = str->append(' ');
...
void truncate_partition_filename(char *path)
{
  if (path)
  {
    ...
    if (last_slash)
    {
      ...
      for (char *pound = strchr(last_slash, '#'); pound; pound = strchr(pound + 1, '#'))
      {
...
*/
static int add_keyword_path(String *str, const char *keyword, const char *path)
{
...
static int add_server_part_options(String *str, partition_element *p_elem)
{
  ...
  if (p_elem->connect_string.length)
    err += add_keyword_string(str, "CONNECTION", true, p_elem->connect_string.str);
  err += add_keyword_string(str, "ENGINE", false, ha_resolve_storage_engine_name(p_elem->engine_type));
  return err;
}
...
static int add_engine_part_options(String *str, partition_element *p_elem)
{
  ...
  for (; opt; opt = opt->next)
  {
    if (!opt->value.str)
      continue;
    if ((add_keyword_string(str, opt->name.str, opt->quoted_value, opt->value.str)))
      return 1;
  }
  return 0;
}
...
static Create_field *get_sql_field(const char *field_name, Alter_info *alter_info)
{
  ...
  while ((sql_field = it++))
  {
    if (!(my_strcasecmp(system_charset_info, sql_field->field_name.str, field_name)))
    {
...
static int add_column_list_values(String *str, partition_info *part_info, part_elem_value *list_value,
                                  HA_CREATE_INFO *create_info, Alter_info *alter_info)
{
  ...
  bool use_parenthesis = (part_info->part_type == LIST_PARTITION && part_info->num_columns > 1U);
    ...
    else
    {
      ...
      else
      {
        ...
        */
        if (create_info)
        {
          const Column_derived_attributes derived_attr(create_info->default_table_charset);
          Create_field *sql_field;

          if (!(sql_field = get_sql_field(field_name, alter_info)))
          {
            my_error(ER_FIELD_NOT_FOUND_PART_ERROR, MYF(0));
            return 1;
          }
        ...
        if (th->partition_field_append_value(
                str, item_expr, field_cs,
                alter_info == NULL ? PARTITION_VALUE_PRINT_MODE_SHOW : PARTITION_VALUE_PRINT_MODE_FRM))
          return 1;
      }
    }
    if (i != (num_elements - 1))
...
static int add_partition_values(String *str, partition_info *part_info, partition_element *p_elem,
                                HA_CREATE_INFO *create_info, Alter_info *alter_info)
{
  int err = 0;
  ...
  if (part_info->part_type == RANGE_PARTITION)
  {
    ...
    if (part_info->column_list)
    {
      ...
      err += str->append('(');
      err += add_column_list_values(str, part_info, list_value, create_info, alter_info);
      err += str->append(')');
    }
    else
    {
  ...
  else if (part_info->part_type == LIST_PARTITION)
  {
    ...
    if (p_elem->max_value)
    {
      DBUG_ASSERT(part_info->defined_max_value || current_thd->lex->sql_command == SQLCOM_ALTER_TABLE);
...
char *generate_partition_syntax_for_frm(THD *thd, partition_info *part_info, uint *buf_length,
                                        HA_CREATE_INFO *create_info, Alter_info *alter_info)
{
  Sql_mode_instant_remove sms(thd, MODE_ANSI_QUOTES);
  char *res = generate_partition_syntax(thd, part_info, buf_length, true, create_info, alter_info);
  DBUG_EXECUTE_IF("generate_partition_syntax_for_frm",
                  push_warning(thd, Sql_condition::WARN_LEVEL_NOTE, ER_YES,
                               ErrConvString(res, (uint32)*buf_length, system_charset_info).ptr()););
  return res;
}
...
char *generate_partition_syntax(THD *thd, partition_info *part_info, uint *buf_length, bool show_partition_options,
                                HA_CREATE_INFO *create_info, Alter_info *alter_info)
{
  ...
  if (part_info->part_type == VERSIONING_PARTITION)
  {
    ...
    if (vers_info->interval.is_set())
    {
      ...
      err += append_interval(&str, vers_info->interval.type, vers_info->interval.step);
    ...
    {
      DBUG_ASSERT(vers_info->interval.is_set() || vers_info->limit);
  ...
  if ((!part_info->use_default_num_partitions) && part_info->use_default_partitions)
  {
  ...
  if (part_info->is_sub_partitioned())
  {
    ...
    if ((!part_info->use_default_num_subpartitions) && part_info->use_default_subpartitions)
    {
      err += str.append(STRING_WITH_LEN("\nSUBPARTITIONS "));
  ...
  if (!part_info->use_default_partitions)
  {
    ...
    {
      part_elem = part_it++;
      if (part_elem->part_state != PART_TO_BE_DROPPED && part_elem->part_state != PART_REORGED_DROPPED)
      {
        if (!first)
          err += str.append(STRING_WITH_LEN(",\n "));
        first = FALSE;
        err += str.append(STRING_WITH_LEN("PARTITION "));
        err += append_identifier(thd, &str, part_elem->partition_name, strlen(part_elem->partition_name));
        err += add_partition_values(&str, part_info, part_elem, create_info, alter_info);
        if (!part_info->is_sub_partitioned() || part_info->use_default_subpartitions)
        {
...
bool partition_key_modified(TABLE *table, const MY_BITMAP *fields)
{
  ...
  if (!part_info)
    DBUG_RETURN(FALSE);
  if (table->s->db_type()->partition_flags && (table->s->db_type()->partition_flags() & HA_CAN_UPDATE_PARTITION_KEY))
    DBUG_RETURN(FALSE);
...
static inline int part_val_int(Item *item_expr, longlong *result)
{
  switch (item_expr->cmp_type())
  {
    case DECIMAL_RESULT:
    {
      ...
      if (val && my_decimal2int(E_DEC_FATAL_ERROR, val, item_expr->unsigned_flag, result, FLOOR) != E_DEC_OK)
        return true;
      break;
    }
    case INT_RESULT:
...
inline static uint32 get_part_id_for_sub(uint32 loc_part_id, uint32 sub_part_id, uint num_subparts)
{
...
static int get_part_id_linear_hash(partition_info *part_info, uint num_parts, Item *part_expr, uint32 *part_id,
                                   longlong *func_value)
{
  ...
  *part_id = get_part_id_from_linear_hash(*func_value, part_info->linear_hash_mask, num_parts);
  DBUG_RETURN(FALSE);
}
...
inline static uint32 get_part_id_key(handler *file, Field **field_array, uint num_parts, longlong *func_value)
{
...
inline static uint32 get_part_id_linear_key(partition_info *part_info, Field **field_array, uint num_parts,
                                            longlong *func_value)
{
  DBUG_ENTER("get_part_id_linear_key");

  *func_value = ha_partition::calculate_key_hash_value(field_array);
  DBUG_RETURN(get_part_id_from_linear_hash(*func_value, part_info->linear_hash_mask, num_parts));
}
    ...
    *restore_ptr = field->ptr;
    ...
    if (!field->maybe_null() || !field->is_null())
    {
      ...
      if (field->type() == MYSQL_TYPE_VARCHAR)
      {
        ...
        cs->strnxfrm(field_buf + len_bytes, max_len, field->ptr + len_bytes, data_len);
        if (len_bytes == 1)
      ...
      {
        cs->strnxfrm(field_buf, max_len, field->ptr, max_len);
      }
      field->ptr = field_buf;
    }
    field_bufs++;
...
static int get_part_id_charset_func_part(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  ...
  copy_to_part_field_buffers(part_info->part_charset_field_array, part_info->part_field_buffers,
                             part_info->restore_part_field_ptrs);
  res = part_info->get_part_partition_id_charset(part_info, part_id, func_value);
  restore_part_field_pointers(part_info->part_charset_field_array, part_info->restore_part_field_ptrs);
  DBUG_RETURN(res);
}
...
static int get_part_id_charset_func_subpart(partition_info *part_info, uint32 *part_id)
{
  ...
  copy_to_part_field_buffers(part_info->subpart_charset_field_array, part_info->subpart_field_buffers,
                             part_info->restore_subpart_field_ptrs);
  res = part_info->get_subpartition_id_charset(part_info, part_id);
  restore_part_field_pointers(part_info->subpart_charset_field_array, part_info->restore_subpart_field_ptrs);
  DBUG_RETURN(res);
}
...
int get_partition_id_list_col(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  ...
  while (max_list_index >= min_list_index)
  {
    ...
    cmp = cmp_rec_and_tuple(list_col_array + list_index * num_columns, num_columns);
    if (cmp > 0)
                                               ...
                                               uint32 nparts)
{
  ...
  do
  {
    ...
    list_index = (max_list_index + min_list_index) >> 1;
    cmp = cmp_rec_and_tuple_prune(list_col_array + list_index * num_columns, nparts, left_endpoint, include_endpoint);
    if (cmp > 0)
    {
      min_list_index = list_index + 1;
    }
  ...
  DBUG_ASSERT(list_index == part_info->num_list_values ||
              (0 >= cmp_rec_and_tuple_prune(list_col_array + list_index * num_columns, nparts, left_endpoint,
                                            include_endpoint)));
  /* Given value must be GREATER THAN the previous partition. */
  DBUG_ASSERT(list_index == 0 || (0 < cmp_rec_and_tuple_prune(list_col_array + (list_index - 1) * num_columns, nparts,
                                                              left_endpoint, include_endpoint)));

  /* Include the right endpoint if not already passed end of array. */
  if (!left_endpoint && include_endpoint && cmp == 0 && list_index < part_info->num_list_values)
    list_index++;
...
uint32 get_list_array_idx_for_endpoint_charset(partition_info *part_info, bool left_endpoint, bool include_endpoint)
{
  uint32 res;
  copy_to_part_field_buffers(part_info->part_field_array, part_info->part_field_buffers,
                             part_info->restore_part_field_ptrs);
  res = get_list_array_idx_for_endpoint(part_info, left_endpoint, include_endpoint);
  restore_part_field_pointers(part_info->part_field_array, part_info->restore_part_field_ptrs);
  return res;
}
...
uint32 get_list_array_idx_for_endpoint(partition_info *part_info, bool left_endpoint, bool include_endpoint)
{
  ...
  longlong part_func_value = part_info->part_expr->val_int_endpoint(left_endpoint, &include_endpoint);
  bool unsigned_flag = part_info->part_expr->unsigned_flag;
  ...
  if (part_info->part_expr->null_value)
  {
    ...
    */
    ...
    if (monotonic != MONOTONIC_INCREASING_NOT_NULL && monotonic != MONOTONIC_STRICT_INCREASING_NOT_NULL)
    {
...
int get_partition_id_range_col(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  ...
  while (max_part_id > min_part_id)
  {
    ...
    if (cmp_rec_and_tuple(range_col_array + loc_part_id * num_columns, num_columns) >= 0)
  ...
  *part_id = (uint32)loc_part_id;
  if (loc_part_id == max_partition &&
      (cmp_rec_and_tuple(range_col_array + loc_part_id * num_columns, num_columns) >= 0))
    DBUG_RETURN(HA_ERR_NO_PARTITION_FOUND);
...
int get_partition_id_range(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  ...
  *func_value = part_func_value;
  ...
  loc_part_id = max_part_id;
  *part_id = (uint32)loc_part_id;
  if (loc_part_id == max_partition && part_func_value >= range_array[loc_part_id] && !part_info->defined_max_value)
    DBUG_RETURN(HA_ERR_NO_PARTITION_FOUND);
...
static uint32 get_partition_id_range_for_endpoint_charset(partition_info *part_info, bool left_endpoint,
                                                          bool include_endpoint)
{
  uint32 res;
  copy_to_part_field_buffers(part_info->part_field_array, part_info->part_field_buffers,
                             part_info->restore_part_field_ptrs);
  res = get_partition_id_range_for_endpoint(part_info, left_endpoint, include_endpoint);
  restore_part_field_pointers(part_info->part_field_array, part_info->restore_part_field_ptrs);
  return res;
}
...
uint32 get_partition_id_range_for_endpoint(partition_info *part_info, bool left_endpoint, bool include_endpoint)
{
  ...
  */
  ...
  if (left_endpoint)
  {
    DBUG_ASSERT(part_func_value > part_end_val ? (loc_part_id == max_partition && !part_info->defined_max_value) : 1);
    /*
    ...
    */
    if (part_func_value >= part_end_val && (loc_part_id < max_partition || !part_info->defined_max_value))
      loc_part_id++;
    if (part_info->part_type == VERSIONING_PARTITION && part_func_value < INT_MAX32 &&
  ...
  else
  {
    ...
    if (include_endpoint && loc_part_id < max_partition && part_func_value == part_end_val)
      loc_part_id++;
...
{
  return get_part_id_hash(part_info->num_parts, part_info->part_expr, part_id, func_value);
}

int get_partition_id_linear_hash_nosub(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  return get_part_id_linear_hash(part_info, part_info->num_parts, part_info->part_expr, part_id, func_value);
}

int get_partition_id_key_nosub(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  *part_id = get_part_id_key(part_info->table->file, part_info->part_field_array, part_info->num_parts, func_value);
  return 0;
}

int get_partition_id_linear_key_nosub(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  *part_id = get_part_id_linear_key(part_info, part_info->part_field_array, part_info->num_parts, func_value);
  return 0;
}
...
int get_partition_id_with_sub(partition_info *part_info, uint32 *part_id, longlong *func_value)
{
  ...
  if (unlikely((error = part_info->get_part_partition_id(part_info, &loc_part_id, func_value))))
  {
    DBUG_RETURN(error);
  }
  num_subparts = part_info->num_subparts;
...
int get_partition_id_hash_sub(partition_info *part_info, uint32 *part_id)
{
  longlong func_value;
  return get_part_id_hash(part_info->num_subparts, part_info->subpart_expr, part_id, &func_value);
}

int get_partition_id_linear_hash_sub(partition_info *part_info, uint32 *part_id)
{
  longlong func_value;
  return get_part_id_linear_hash(part_info, part_info->num_subparts, part_info->subpart_expr, part_id, &func_value);
}

int get_partition_id_key_sub(partition_info *part_info, uint32 *part_id)
{
  longlong func_value;
  *part_id =
      get_part_id_key(part_info->table->file, part_info->subpart_field_array, part_info->num_subparts, &func_value);
  return FALSE;
}

int get_partition_id_linear_key_sub(partition_info *part_info, uint32 *part_id)
{
  longlong func_value;
  *part_id = get_part_id_linear_key(part_info, part_info->subpart_field_array, part_info->num_subparts, &func_value);
  return FALSE;
}
...
static int get_sub_part_id_from_key(const TABLE *table, uchar *buf, KEY *key_info, const key_range *key_spec,
                                    uint32 *part_id)
{
...
bool get_part_id_from_key(const TABLE *table, uchar *buf, KEY *key_info, const key_range *key_spec, uint32 *part_id)
{
  bool result;
  uchar *rec0 = table->record[0];
  partition_info *part_info = table->part_info;
...
void get_full_part_id_from_key(const TABLE *table, uchar *buf, KEY *key_info, const key_range *key_spec,
                               part_id_range *part_spec)
{
  ...
  if (likely(rec0 == buf))
  {
    result = part_info->get_partition_id(part_info, &part_spec->start_part, &func_value);
  }
  else
  {
    Field **part_field_array = part_info->full_part_field_array;
...
bool verify_data_with_partition(TABLE *table, TABLE *part_table, uint32 part_id)
{
  ...
  DBUG_ASSERT(table && table->file && part_table && part_table->part_info && part_table->file);
  ...
  */
  ...
  do
  {
    ...
    if (unlikely((error = part_info->get_partition_id(part_info, &found_part_id, &func_value))))
    {
...
err:
  part_info->table->move_fields(part_info->full_part_field_array, old_rec, table->record[0]);
...
/*
  ...
  RETURN VALUE
    part_spec
*/
void get_partition_set(const TABLE *table, uchar *buf, const uint index, const key_range *key_spec,
                       part_id_range *part_spec)
{
  partition_info *part_info = table->part_info;
  ...
  if ((index < MAX_KEY) && key_spec && key_spec->flag == (uint)HA_READ_KEY_EXACT &&
      part_info->some_fields_in_PF.is_set(index))
  {
    ...
    */
    ...
    else
    {
      ...
      */
      if ((found_part_field = set_PF_fields_in_key(key_info, key_spec->length)))
      {
  ...
  if (!(part_part == num_parts && sub_part == num_parts))
  {
    ...
    */
    ...
    else
    {
      ...
      part_spec->end_part = sub_part + (part_info->num_subparts * (part_info->num_parts - 1));
      for (i = 0, part_id = sub_part; i < part_info->num_parts; i++, part_id += part_info->num_subparts)
...
bool mysql_unpack_partition(THD *thd, char *part_buf, uint part_info_len, TABLE *table, bool is_create_table_ind,
                            handlerton *default_db_type, bool *work_part_info_used)
{
  ...
  *work_part_info_used = FALSE;
  ...
  if (unlikely(parse_sql(thd, &parser_state, NULL)) || unlikely(part_info->fix_parser_data(thd)))
  {
    thd->free_items();
...
static int fast_end_partition(THD *thd, ulonglong copied, ulonglong deleted, TABLE_LIST *table_list)
{
  char tmp_name[80];
  ...
  my_snprintf(tmp_name, sizeof(tmp_name), ER_THD(thd, ER_INSERT_INFO), (ulong)(copied + deleted), (ulong)deleted,
              (ulong)0);
...
static bool check_native_partitioned(HA_CREATE_INFO *create_info, bool *ret_val, partition_info *part_info, THD *thd)
{
  ...
  DBUG_PRINT("info", ("engine_type = %s, table_engine_set = %u", ha_resolve_storage_engine_name(engine_type),
                      table_engine_set));
  ...
  */
  ...
  DBUG_PRINT("info", ("engine_type = %s", ha_resolve_storage_engine_name(engine_type)));
  if (engine_type->partition_flags && (engine_type->partition_flags() & HA_CAN_PARTITION))
  {
...
bool set_part_state(Alter_info *alter_info, partition_info *tab_part_info, enum partition_state part_state)
{
  uint part_count = 0;
  ...
  do
  {
    ...
    if ((alter_info->partition_flags & ALTER_PARTITION_ALL) ||
        (is_name_in_list(part_elem->partition_name, alter_info->partition_names)))
    {
      /*
        Mark the partition.
      ...
      */
      ...
      DBUG_PRINT("info", ("Setting part_state to %u for partition %s", part_state, part_elem->partition_name));
    }
    else
      part_elem->part_state = PART_NORMAL;
  } while (++part_count < tab_part_info->num_parts);

  if (num_parts_found != alter_info->partition_names.elements && !(alter_info->partition_flags & ALTER_PARTITION_ALL))
  {
 ...
 */
static void warn_if_datadir_altered(THD *thd, const partition_element *part_elem)
{
  DBUG_ASSERT(part_elem);

  if (part_elem->engine_type && part_elem->engine_type->db_type != DB_TYPE_INNODB)
    return;

  if (part_elem->data_file_name)
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, WARN_INNODB_PARTITION_OPTION_IGNORED,
                        ER(WARN_INNODB_PARTITION_OPTION_IGNORED), "DATA DIRECTORY");
  }
  if (part_elem->index_file_name)
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, WARN_INNODB_PARTITION_OPTION_IGNORED,
                        ER(WARN_INNODB_PARTITION_OPTION_IGNORED), "INDEX DIRECTORY");
  }
}
 ...
 */
static void check_datadir_altered_for_innodb(THD *thd, partition_info *tab_part_info, partition_info *alt_part_info)
{
  if (tab_part_info->default_engine_type->db_type != DB_TYPE_INNODB)
    return;

  for (List_iterator_fast<partition_element> it(alt_part_info->partitions); partition_element *part_elem = it++;)
  {
...
uint prep_alter_part_table(THD *thd, TABLE *table, Alter_info *alter_info, HA_CREATE_INFO *create_info,
                           bool *partition_changed, bool *fast_alter_table)
{
  DBUG_ENTER("prep_alter_part_table");

  /* Foreign keys on partitioned tables are not supported, waits for WL#148 */
  if (table->part_info && (alter_info->flags & (ALTER_ADD_FOREIGN_KEY | ALTER_DROP_FOREIGN_KEY)))
  {
  ...
  if (!table->part_info && (alter_info->partition_flags & ALTER_PARTITION_REMOVE))
  {
  ...
  */
  bool only_default_value_added =
      (alt_part_info && alt_part_info->current_partition &&
       alt_part_info->current_partition->list_val_list.elements == 1 &&
       alt_part_info->current_partition->list_val_list.head()->added_items >= 1 &&
       alt_part_info->current_partition->list_val_list.head()->col_val_array[0].max_value) &&
      alt_part_info->part_type == LIST_PARTITION && (alter_info->partition_flags & ALTER_PARTITION_ADD);
  if (only_default_value_added && !thd->lex->part_info->num_columns)
  ...
  */
  if (thd->work_part_info && !(thd->work_part_info = thd->work_part_info->get_clone(thd)))
    DBUG_RETURN(TRUE);
  ...
  if (alter_info->partition_flags &
      (ALTER_PARTITION_ADD | ALTER_PARTITION_DROP | ALTER_PARTITION_CONVERT_OUT | ALTER_PARTITION_COALESCE |
       ALTER_PARTITION_REORGANIZE | ALTER_PARTITION_TABLE_REORG | ALTER_PARTITION_REBUILD | ALTER_PARTITION_CONVERT_IN))
  {
    /*
    ...
    */
    DBUG_ASSERT(thd->mdl_context.is_lock_owner(MDL_key::TABLE, table->s->db.str, table->s->table_name.str,
      ...
      if (thd->work_part_info->part_type != tab_part_info->part_type)
      {
        ...
        else
        {
          ...
          */
          ...
          {
            DBUG_ASSERT(tab_part_info->part_type == RANGE_PARTITION || tab_part_info->part_type == LIST_PARTITION);
            (void)tab_part_info->error_if_requires_values();
          }
          goto err;
        }
      }
      if ((tab_part_info->column_list && alt_part_info->num_columns != tab_part_info->num_columns &&
           !only_default_value_added) ||
          (!tab_part_info->column_list &&
           (tab_part_info->part_type == RANGE_PARTITION || tab_part_info->part_type == LIST_PARTITION) &&
           alt_part_info->num_columns != 1U && !only_default_value_added) ||
          (!tab_part_info->column_list && tab_part_info->part_type == HASH_PARTITION &&
    ...
    if (alter_info->partition_flags & ALTER_PARTITION_ADD)
    {
      ...
      */
      if (thd->lex->no_write_to_binlog && tab_part_info->part_type != HASH_PARTITION &&
          tab_part_info->part_type != VERSIONING_PARTITION)
      {
      ...
      if (tab_part_info->defined_max_value &&
          (tab_part_info->part_type == RANGE_PARTITION || alt_part_info->defined_max_value))
      {
        my_error(
            (tab_part_info->part_type == RANGE_PARTITION ? ER_PARTITION_MAXVALUE_ERROR : ER_PARTITION_DEFAULT_ERROR),
            MYF(0));
        goto err;
      }
      ...
      if (tab_part_info->is_sub_partitioned())
      {
        ...
        check_total_partitions = new_total_partitions * alt_part_info->num_subparts;
      }
      ...
      */
      {
        ...
        if (tab_part_info->part_type == VERSIONING_PARTITION)
        {
          ...
          if (*fast_alter_table && !(alter_info->partition_flags & ALTER_PARTITION_AUTO_HIST) &&
        ...
        do
        {
          ...
          if (unlikely(tab_part_info->partitions.push_back(part_elem, thd->mem_root)))
            goto err;
        ...
        if (tab_part_info->part_type == VERSIONING_PARTITION)
        {
          ...
          if (unlikely(tab_part_info->partitions.push_back(now_part, thd->mem_root)))
            goto err;
        }
      }
      /*
    ...
    {
      const char *const cmd = (alter_info->partition_flags & ALTER_PARTITION_CONVERT_OUT) ? "CONVERT" : "DROP";
      /*
      ...
      */
      ...
      {
        if (!(tab_part_info->part_type == RANGE_PARTITION || tab_part_info->part_type == LIST_PARTITION))
        {
      ...
      {
        partition_element *part_elem = part_it++;
        if (is_name_in_list(part_elem->partition_name, alter_info->partition_names))
        {
          if (tab_part_info->part_type == VERSIONING_PARTITION)
          {
            ...
            if (tab_part_info->vers_info->interval.is_set())
            {
              ...
              tab_part_info->vers_info->interval.start = (my_time_t)part_elem->range_value;
            }
          }
          /*
      ...
      DBUG_ASSERT(!(alter_info->partition_flags & ALTER_PARTITION_CONVERT_OUT) || num_parts_dropped == 1);
      /* NOTE: num_parts is used in generate_partition_syntax() */
      tab_part_info->num_parts -= num_parts_dropped;
      if ((alter_info->partition_flags & ALTER_PARTITION_CONVERT_OUT) && tab_part_info->is_sub_partitioned())
      {
    ...
    {
      set_engine_all_partitions(tab_part_info, tab_part_info->default_engine_type);
      ...
      */
      {
        ...
        uint end_part = 0, end_sec_part = 0;
        bool all_parts = TRUE;
        if (*fast_alter_table && tab_part_info->linear_hash_ind)
        {
          uint upper_2n = tab_part_info->linear_hash_mask + 1;
          uint lower_2n = upper_2n >> 1;
          all_parts = FALSE;
        ...
        do
        {
          ...
          if (*fast_alter_table && (all_parts || (part_count >= start_part && part_count <= end_part) ||
    ...
    else if (alter_info->partition_flags & ALTER_PARTITION_REORGANIZE)
    {
      ...
      */
      ...
      tab_part_info->use_default_partitions = FALSE;
      if (alt_part_info->set_up_defaults_for_partitioning(thd, table->file, 0, 0))
      {
        goto err;
      }
      ...
      */
      {
        ...
        do
        {
          ...
          if (is_name_in_list(part_elem->partition_name, alter_info->partition_names))
          {
            ...
            if (*fast_alter_table && unlikely(tab_part_info->temp_partitions.push_back(part_elem, thd->mem_root)))
              goto err;
    ...
    *partition_changed = TRUE;
    thd->work_part_info = tab_part_info;
    if (alter_info->partition_flags & (ALTER_PARTITION_ADD | ALTER_PARTITION_REORGANIZE))
    {
      if (tab_part_info->use_default_subpartitions && !alt_part_info->use_default_subpartitions)
      {
        tab_part_info->use_default_subpartitions = FALSE;
        tab_part_info->use_default_num_subpartitions = FALSE;
      }

      if (tab_part_info->check_partition_info(thd, (handlerton **)NULL, table->file, 0, alt_part_info))
      {
        goto err;
      }
      /*
        ...
        to reorganize into
      */
      if (alter_info->partition_flags == ALTER_PARTITION_REORGANIZE && tab_part_info->part_type == RANGE_PARTITION &&
          ((is_last_partition_reorged &&
            (tab_part_info->column_list ? (partition_info_compare_column_values(alt_max_elem_val->col_val_array,
    ...
    */
    ...
    if (tab_part_info)
    {
      if (alter_info->partition_flags & ALTER_PARTITION_REMOVE)
      {
        ...
        DBUG_PRINT("info", ("New engine type: %s", ha_resolve_storage_engine_name(create_info->db_type)));
        thd->work_part_info = NULL;
        *partition_changed = TRUE;
      }
      ...
      */
      if (!*partition_changed && tab_part_info->part_field_array && !tab_part_info->part_field_list.elements &&
          table->s->primary_key != MAX_KEY)
      {
        ...
        else
        {
          ...
          while ((drop = drop_it++))
          {
            if (drop->type == Alter_drop::KEY && 0 == my_strcasecmp(system_charset_info, primary_name, drop->name))
              break;
          }
          if (drop)
            *partition_changed = TRUE;
    ...
    if (thd->work_part_info)
    {
      ...
      if (tab_part_info && tab_part_info->part_type == VERSIONING_PARTITION && tab_part_info != part_info &&
          part_info->part_type == VERSIONING_PARTITION && part_info->num_parts == 0)
      {
        if (part_info->vers_info->interval.is_set() &&
            (!tab_part_info->vers_info->interval.is_set() ||
             /* TODO: equivalent intervals like 1 hour and 60 mins should be considered equal */
             memcmp(&part_info->vers_info->interval, &tab_part_info->vers_info->interval,
      ...
      */
      else if (part_info != tab_part_info)
      {
        ...
        */
        if (alter_info->partition_flags != ALTER_PARTITION_INFO || !table->part_info ||
            alter_info->algorithm(thd) != Alter_info::ALTER_TABLE_ALGORITHM_INPLACE ||
      ...
      */
      ...
      DBUG_ASSERT(part_info->default_engine_type && part_info->default_engine_type != partition_hton);
      if (check_native_partitioned(create_info, &is_native_partitioned, part_info, thd))
      {
        goto err;
      }
...
static bool mysql_change_partitions(ALTER_PARTITION_PARAM_TYPE *lpt, bool copy_data)
{
  ...
  if (unlikely((error = file->ha_change_partitions(lpt->create_info, path, &lpt->copied, &lpt->deleted,
                                                   lpt->pack_frm_data, lpt->pack_frm_len))))
  {
...
static bool mysql_drop_partitions(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  DBUG_ASSERT(lpt->thd->mdl_context.is_lock_owner(MDL_key::TABLE, lpt->table->s->db.str, lpt->table->s->table_name.str,
                                                  MDL_EXCLUSIVE));
...
static bool alter_partition_convert_out(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  build_table_filename(to_name, sizeof(to_name) - 1, lpt->alter_ctx->new_db.str, lpt->alter_ctx->new_name.str, "", 0);
  ...
  for (const partition_element &e : part_info->partitions)
  {
    ...
    if (unlikely((error = create_partition_name(from_name, sizeof(from_name), path, e.partition_name, NORMAL_PART_NAME,
                                                FALSE))))
    {
      DBUG_ASSERT(thd->is_error());
      return true;
    }
    if (DBUG_IF("error_convert_partition_00") || unlikely(error = file->ha_rename_table(from_name, to_name)))
    {
...
bool write_log_replace_frm(ALTER_PARTITION_PARAM_TYPE *lpt, uint next_entry, const char *from_path, const char *to_path)
{
...
static bool write_log_changed_partitions(ALTER_PARTITION_PARAM_TYPE *lpt, uint *next_entry, const char *path)
{
  DDL_LOG_ENTRY ddl_log_entry;
  ...
  {
    partition_element *part_elem = part_it++;
    if (part_elem->part_state == PART_IS_CHANGED || (part_elem->part_state == PART_IS_ADDED && temp_partitions))
    {
      bzero(&ddl_log_entry, sizeof(ddl_log_entry));
      if (part_info->is_sub_partitioned())
      {
        ...
        do
        {
          partition_element *sub_elem = sub_it++;
          ddl_log_entry.next_entry = *next_entry;
          lex_string_set(&ddl_log_entry.handler_name, ha_resolve_storage_engine_name(sub_elem->engine_type));
          if (create_subpartition_name(tmp_path, sizeof(tmp_path), path, part_elem->partition_name,
                                       sub_elem->partition_name, TEMP_PART_NAME) ||
              create_subpartition_name(normal_path, sizeof(normal_path), path, part_elem->partition_name,
      ...
      {
        ddl_log_entry.next_entry = *next_entry;
        lex_string_set(&ddl_log_entry.handler_name, ha_resolve_storage_engine_name(part_elem->engine_type));
        if (create_partition_name(tmp_path, sizeof(tmp_path), path, part_elem->partition_name, TEMP_PART_NAME, TRUE) ||
            create_partition_name(normal_path, sizeof(normal_path), path, part_elem->partition_name, NORMAL_PART_NAME,
                                  TRUE))
          DBUG_RETURN(TRUE);
        lex_string_set(&ddl_log_entry.name, normal_path);
        lex_string_set(&ddl_log_entry.from_name, tmp_path);
...
static bool log_drop_or_convert_action(ALTER_PARTITION_PARAM_TYPE *lpt, uint *next_entry, const char *path,
                                       const char *from_name, bool temp_list, const log_action_enum convert_action)
{
  DDL_LOG_ENTRY ddl_log_entry;
  ...
  ddl_log_entry.action_type = convert_action ? DDL_LOG_RENAME_ACTION : DDL_LOG_DELETE_ACTION;
  if (temp_list)
  ...
  while (num_elements--)
  {
    ...
    if (temp_list)
      ...
      part_elem = part_it++;
    if (part_elem->part_state == PART_TO_BE_DROPPED || part_elem->part_state == PART_TO_BE_ADDED ||
        part_elem->part_state == PART_CHANGED)
    {
      uint name_variant;
      if (part_elem->part_state == PART_CHANGED || (part_elem->part_state == PART_TO_BE_ADDED && num_temp_partitions))
        name_variant = TEMP_PART_NAME;
      else
        name_variant = NORMAL_PART_NAME;
      DBUG_ASSERT(convert_action != ACT_CONVERT_IN || part_elem->part_state == PART_TO_BE_ADDED);
      DBUG_ASSERT(convert_action != ACT_CONVERT_OUT || part_elem->part_state == PART_TO_BE_DROPPED);
      if (part_info->is_sub_partitioned())
      {
      ...
      else
      {
        ddl_log_entry.next_entry = *next_entry;
        lex_string_set(&ddl_log_entry.handler_name, ha_resolve_storage_engine_name(part_elem->engine_type));
        if (create_partition_name(tmp_path, sizeof(tmp_path), path, part_elem->partition_name, name_variant, TRUE))
          DBUG_RETURN(TRUE);
...
inline static bool write_log_dropped_partitions(ALTER_PARTITION_PARAM_TYPE *lpt, uint *next_entry, const char *path,
                                                bool temp_list)
{
  return log_drop_or_convert_action(lpt, next_entry, path, NULL, temp_list, ACT_DROP);
}

inline static bool write_log_convert_partition(ALTER_PARTITION_PARAM_TYPE *lpt, uint *next_entry, const char *path)
{
  ...
  const log_action_enum convert_action = (f & ALTER_PARTITION_CONVERT_IN) ? ACT_CONVERT_IN : ACT_CONVERT_OUT;
  build_table_filename(other_table, sizeof(other_table) - 1, lpt->alter_ctx->new_db.str, lpt->alter_ctx->new_name.str,
                       "", 0);
  DDL_LOG_MEMORY_ENTRY *main_entry = lpt->part_info->main_entry;
  bool res = log_drop_or_convert_action(lpt, next_entry, path, other_table, false, convert_action);
  /*
...
static bool write_log_drop_frm(ALTER_PARTITION_PARAM_TYPE *lpt, DDL_LOG_STATE *drop_chain)
{
  char path[FN_REFLEN + 1];
  ...
  if (drop_backup && (lpt->alter_info->partition_flags & ALTER_PARTITION_CONVERT_IN))
  {
    ...
    build_table_filename(path, sizeof(path) - 1, table_from->db.str, table_from->table_name.str, "", 0);

    if (ddl_log_delete_frm(drop_chain, (const char *)path))
      goto error;
  }

  if (ddl_log_write_execute_entry(drop_chain->list->entry_pos, drop_backup ? main_chain->execute_entry->entry_pos : 0,
...
static inline bool write_log_drop_shadow_frm(ALTER_PARTITION_PARAM_TYPE *lpt)
{
...
static bool write_log_rename_frm(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  if (ddl_log_write_execute_entry(log_entry->entry_pos, &exec_log_entry))
    goto error;
...
static bool write_log_drop_partition(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  mysql_mutex_lock(&LOCK_gdl);
  if (write_log_dropped_partitions(lpt, &next_entry, (const char *)path, FALSE))
    goto error;
  if (write_log_replace_frm(lpt, next_entry, (const char *)tmp_path, (const char *)path))
    goto error;
  log_entry = part_info->list;
  part_info->main_entry = log_entry;
  if (ddl_log_write_execute_entry(log_entry->entry_pos, &exec_log_entry))
...
static bool write_log_convert_partition(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  if (ddl_log_write_execute_entry(part_info->list->entry_pos, &part_info->execute_entry))
    goto error;
  mysql_mutex_unlock(&LOCK_gdl);
  return false;
...
static bool write_log_add_change_partition(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  if (ddl_log_write_execute_entry(log_entry->entry_pos, &part_info->execute_entry))
    goto error;
  mysql_mutex_unlock(&LOCK_gdl);
  DBUG_RETURN(FALSE);
...
/*
  TODO: Partitioning atomic DDL refactoring: this should be replaced with
        ddl_log_complete().
*/
static void write_log_completed(ALTER_PARTITION_PARAM_TYPE *lpt, bool dont_crash)
{
  partition_info *part_info = lpt->part_info;
...
*/
static void handle_alter_part_error(ALTER_PARTITION_PARAM_TYPE *lpt, bool action_completed, bool drop_partition,
                                    bool frm_install, bool reopen)
{
  THD *thd = lpt->thd;
  ...
  */
  if (!thd->mdl_context.is_lock_owner(MDL_key::TABLE, lpt->db.str, lpt->table_name.str, MDL_EXCLUSIVE) &&
      wait_while_table_is_used(thd, table, HA_EXTRA_FORCE_REOPEN))
  {
    /*
      ...
      reopened.
    */
    thd->locked_tables_list.unlink_from_list(thd, table->pos_in_locked_tables, false);
    /*
  ...
  if (part_info->list && ddl_log_execute_entry(thd, part_info->list->entry_pos))
  {
    /*
...
static void downgrade_mdl_if_lock_tables_mode(THD *thd, MDL_ticket *ticket, enum_mdl_type type)
{
  if (thd->locked_tables_mode)
...
static bool check_table_data(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  */
  ...
  DBUG_ASSERT(
      thd->mdl_context.is_lock_owner(MDL_key::TABLE, table_to->s->db.str, table_to->s->table_name.str, MDL_EXCLUSIVE));

  DBUG_ASSERT(thd->mdl_context.is_lock_owner(MDL_key::TABLE, table_from->s->db.str, table_from->s->table_name.str,
                                             MDL_EXCLUSIVE));

  uint32 new_part_id;
  partition_element *part_elem;
  const char *partition_name = thd->lex->part_info->curr_part_elem->partition_name;
  part_elem = table_to->part_info->get_part_elem(partition_name, nullptr, 0, &new_part_id);
...
uint fast_alter_partition_table(THD *thd, TABLE *table, Alter_info *alter_info, Alter_table_ctx *alter_ctx,
                                HA_CREATE_INFO *create_info, TABLE_LIST *table_list)
{
  ...
  */
  ...
  if (table->file->alter_table_flags(alter_info->flags) & HA_PARTITION_ONE_PHASE &&
      !(alter_info->partition_flags & ALTER_PARTITION_AUTO_HIST))
  {
    ...
    */
    if (mysql_write_frm(lpt, WFRM_WRITE_SHADOW) || mysql_change_partitions(lpt, true))
    {
      goto err;
    }
  }
  else if (alter_info->partition_flags & ALTER_PARTITION_DROP)
  {
    ...
    */
    if (write_log_drop_shadow_frm(lpt) || ERROR_INJECT("drop_partition_1") || mysql_write_frm(lpt, WFRM_WRITE_SHADOW) ||
        ERROR_INJECT("drop_partition_2") || wait_while_table_is_used(thd, table, HA_EXTRA_NOT_USED) ||
        ERROR_INJECT("drop_partition_3") || write_log_drop_partition(lpt) || (action_completed = TRUE, FALSE) ||
        ERROR_INJECT("drop_partition_4") || alter_close_table(lpt) || ERROR_INJECT("drop_partition_5") ||
        ERROR_INJECT("drop_partition_6") || (frm_install = TRUE, FALSE) || mysql_write_frm(lpt, WFRM_INSTALL_SHADOW) ||
        log_partition_alter_to_ddl_log(lpt) || (frm_install = FALSE, FALSE) || ERROR_INJECT("drop_partition_7") ||
        mysql_drop_partitions(lpt) || ERROR_INJECT("drop_partition_8") || (write_log_completed(lpt, FALSE), FALSE) ||
  ...
  else if (alter_info->partition_flags & ALTER_PARTITION_CONVERT_OUT)
  {
    ...
    if (mysql_write_frm(lpt, WFRM_WRITE_CONVERTED_TO) || ERROR_INJECT("convert_partition_1") ||
        write_log_drop_shadow_frm(lpt) || ERROR_INJECT("convert_partition_2") ||
        mysql_write_frm(lpt, WFRM_WRITE_SHADOW) || ERROR_INJECT("convert_partition_3") ||
        wait_while_table_is_used(thd, table, HA_EXTRA_NOT_USED) || ERROR_INJECT("convert_partition_4") ||
        write_log_convert_partition(lpt) || ERROR_INJECT("convert_partition_5") || alter_close_table(lpt) ||
        ERROR_INJECT("convert_partition_6") || alter_partition_convert_out(lpt) ||
        ERROR_INJECT("convert_partition_7") || write_log_drop_frm(lpt, &chain_drop_backup) ||
        mysql_write_frm(lpt, WFRM_INSTALL_SHADOW | WFRM_BACKUP_ORIGINAL) || log_partition_alter_to_ddl_log(lpt) ||
        ERROR_INJECT("convert_partition_8") ||
        ((!thd->lex->no_write_to_binlog) &&
         ((thd->binlog_xid = thd->query_id), ddl_log_update_xid(lpt->part_info, thd->binlog_xid),
          write_bin_log(thd, false, thd->query(), thd->query_length()), (thd->binlog_xid = 0))) ||
    ...
    if (alter_partition_lock_handling(lpt) || ERROR_INJECT("convert_partition_11"))
      goto err;
  }
  else if ((alter_info->partition_flags & ALTER_PARTITION_CONVERT_IN))
  {
    ...
    if (wait_while_table_is_used(thd, table, HA_EXTRA_NOT_USED) ||
        wait_while_table_is_used(thd, table_from, HA_EXTRA_PREPARE_FOR_RENAME) || ERROR_INJECT("convert_partition_1") ||
        compare_table_with_partition(thd, table_from, table, NULL, 0) || ERROR_INJECT("convert_partition_2") ||
        check_table_data(lpt))
      goto err;

    if (write_log_drop_shadow_frm(lpt) || ERROR_INJECT("convert_partition_3") ||
        mysql_write_frm(lpt, WFRM_WRITE_SHADOW) || ERROR_INJECT("convert_partition_4") || alter_close_table(lpt) ||
        ERROR_INJECT("convert_partition_5") || write_log_convert_partition(lpt) ||
        ERROR_INJECT("convert_partition_6") || alter_partition_convert_in(lpt) || ERROR_INJECT("convert_partition_7") ||
        (frm_install = true, false) || write_log_drop_frm(lpt, &chain_drop_backup) ||
        mysql_write_frm(lpt, WFRM_INSTALL_SHADOW | WFRM_BACKUP_ORIGINAL) || log_partition_alter_to_ddl_log(lpt) ||
        (frm_install = false, false) || ERROR_INJECT("convert_partition_8") ||
        ((!thd->lex->no_write_to_binlog) &&
         ((thd->binlog_xid = thd->query_id), ddl_log_update_xid(lpt->part_info, thd->binlog_xid),
          write_bin_log(thd, false, thd->query(), thd->query_length()), (thd->binlog_xid = 0))) ||
        ERROR_INJECT("convert_partition_9"))
  ...
  */
           ...
           (part_info->part_type == RANGE_PARTITION || part_info->part_type == LIST_PARTITION ||
            alter_info->partition_flags & ALTER_PARTITION_AUTO_HIST))
  {
    ...
    /*
      ...
      12)Complete query
    */
    if (write_log_drop_shadow_frm(lpt) || ERROR_INJECT("add_partition_1") || mysql_write_frm(lpt, WFRM_WRITE_SHADOW) ||
        ERROR_INJECT("add_partition_2") || wait_while_table_is_used(thd, table, HA_EXTRA_NOT_USED) ||
        ERROR_INJECT("add_partition_3") || write_log_add_change_partition(lpt) || ERROR_INJECT("add_partition_4") ||
        mysql_change_partitions(lpt, false) || ERROR_INJECT("add_partition_5") || alter_close_table(lpt) ||
        ERROR_INJECT("add_partition_6") || ERROR_INJECT("add_partition_7") || write_log_rename_frm(lpt) ||
        (action_completed = TRUE, FALSE) || ERROR_INJECT("add_partition_8") || (frm_install = TRUE, FALSE) ||
        mysql_write_frm(lpt, WFRM_INSTALL_SHADOW) || log_partition_alter_to_ddl_log(lpt) ||
        (frm_install = FALSE, FALSE) || ERROR_INJECT("add_partition_9") || (write_log_completed(lpt, FALSE), FALSE) ||
        ((!thd->lex->no_write_to_binlog) && (write_bin_log(thd, FALSE, thd->query(), thd->query_length()), FALSE)) ||
  ...
  else
  {
    ...
    */
    if (write_log_drop_shadow_frm(lpt) || ERROR_INJECT("change_partition_1") ||
        mysql_write_frm(lpt, WFRM_WRITE_SHADOW) || ERROR_INJECT("change_partition_2") ||
        write_log_add_change_partition(lpt) || ERROR_INJECT("change_partition_3") ||
        mysql_change_partitions(lpt, true) || ERROR_INJECT("change_partition_4") ||
        wait_while_table_is_used(thd, table, HA_EXTRA_NOT_USED) || ERROR_INJECT("change_partition_5") ||
        alter_close_table(lpt) || ERROR_INJECT("change_partition_6") || write_log_final_change_partition(lpt) ||
        (action_completed = TRUE, FALSE) || ERROR_INJECT("change_partition_7") || ERROR_INJECT("change_partition_8") ||
        ((frm_install = TRUE), FALSE) || mysql_write_frm(lpt, WFRM_INSTALL_SHADOW) ||
        log_partition_alter_to_ddl_log(lpt) || (frm_install = FALSE, FALSE) || ERROR_INJECT("change_partition_9") ||
        mysql_drop_partitions(lpt) || ERROR_INJECT("change_partition_10") || mysql_rename_partitions(lpt) ||
        ERROR_INJECT("change_partition_11") || (write_log_completed(lpt, FALSE), FALSE) ||
        ((!thd->lex->no_write_to_binlog) && (write_bin_log(thd, FALSE, thd->query(), thd->query_length()), FALSE)) ||
...
void set_key_field_ptr(KEY *key_info, const uchar *new_buf, const uchar *old_buf)
{
...
void append_row_to_str(String &str, const uchar *row, TABLE *table)
{
  ...
  fields = (Field **)my_malloc(PSI_INSTRUMENT_ME, sizeof(void *) * (num_fields + 1), MYF(0));
  if (!fields)
    return;
...
void make_used_partitions_str(MEM_ROOT *alloc, partition_info *part_info, String *parts_str,
    ...
    while ((head_pe = it++))
    {
      ...
      while ((pe = it2++))
      {
        if (bitmap_is_set(&part_info->read_partitions, partition_id))
        {
          ...
          parts_str->append(head_pe->partition_name, strlen(head_pe->partition_name), system_charset_info);
          parts_str->append('_');
          parts_str->append(pe->partition_name, strlen(pe->partition_name), system_charset_info);
  ...
  switch (part_info->part_type)
  {
    ...
    case LIST_PARTITION:
      if (!part_info->column_list)
      {
        ...
        {
          part_info->get_part_iter_for_interval = get_part_iter_for_interval_via_mapping;
          goto setup_subparts;
        }
      }
      else
      {
        part_info->get_part_iter_for_interval = get_part_iter_for_interval_cols_via_map;
        goto setup_subparts;
      }
    default:;
  }
  ...
  if (part_info->num_part_fields == 1)
  {
    ...
    switch (field->type())
    {
      ...
      case MYSQL_TYPE_LONGLONG:
        part_info->get_part_iter_for_interval = get_part_iter_for_interval_via_walking;
        break;
      default:;
    }
  }
  ...
  */
  if (part_info->num_subpart_fields == 1)
  {
    ...
    switch (field->type())
    {
      ...
      case MYSQL_TYPE_LONG:
      case MYSQL_TYPE_LONGLONG:
        part_info->get_subpart_iter_for_interval = get_part_iter_for_interval_via_walking;
        break;
      default:;
    }
  }
...
*/
uint32 store_tuple_to_record(Field **pfield, uint32 *store_length_array, uchar *value, uchar *value_end)
{
...
uint32 get_partition_id_cols_range_for_endpoint(partition_info *part_info, bool is_left_endpoint, bool include_endpoint,
                                                uint32 nparts)
{
  ...
  do
  {
    ...
    if (0 <= cmp_rec_and_tuple_prune(range_col_array + loc_part_id * num_columns, nparts, is_left_endpoint,
                                     include_endpoint))
      min_part_id = loc_part_id + 1;
    else
      max_part_id = loc_part_id;
  ...
  DBUG_ASSERT(loc_part_id == part_info->num_parts ||
              (0 > cmp_rec_and_tuple_prune(range_col_array + loc_part_id * num_columns, nparts, is_left_endpoint,
                                           include_endpoint)));
  /* Given value must be GREATER THAN or EQUAL to the previous partition. */
  DBUG_ASSERT(loc_part_id == 0 || (0 <= cmp_rec_and_tuple_prune(range_col_array + (loc_part_id - 1) * num_columns,
...
static int get_part_iter_for_interval_cols_via_map(partition_info *part_info, bool is_subpart,
                                                   uint32 *store_length_array, uchar *min_value, uchar *max_value,
                                                   ...
                                                   PARTITION_ITERATOR *part_iter)
{
  ...
  {
    if (part_info->has_default_partititon() && part_info->num_parts == 1)
  ...
  for (uint32 i = 0; i < part_info->num_columns; i++) full_length += store_length_array[i];

  can_match_multiple_values = ((flags & (NO_MIN_RANGE | NO_MAX_RANGE | NEAR_MIN | NEAR_MAX)) || (min_len != max_len) ||
                               (min_len != full_length) || memcmp(min_value, max_value, min_len));
  ...
  else
  {
    ...
    nparts = store_tuple_to_record(part_info->part_field_array, store_length_array, min_value, min_value + min_len);
  ...
  else
  {
    ...
    nparts = store_tuple_to_record(part_info->part_field_array, store_length_array, max_value, max_value + max_len);
    part_iter->part_nums.end = get_col_endpoint(part_info, FALSE, !(flags & NEAR_MAX), nparts);
  }
...
static int get_part_iter_for_interval_via_mapping(partition_info *part_info, bool is_subpart,
                                                  ...
                                                  uint flags, PARTITION_ITERATOR *part_iter)
{
  ...
  can_match_multiple_values =
      ((flags & (NO_MIN_RANGE | NO_MAX_RANGE | NEAR_MIN | NEAR_MAX)) || memcmp(min_value, max_value, field_len));
  DBUG_ASSERT(can_match_multiple_values || (flags & EQ_RANGE) || flags == 0);
  if (can_match_multiple_values && part_info->has_default_partititon())
    part_iter->ret_default_part = part_iter->ret_default_part_orig = TRUE;
  if (can_match_multiple_values && (part_info->part_type == RANGE_PARTITION || part_info->has_null_value))
  {
  ...
  /*
    Find minimum: Do special handling if the interval has left bound in form
     " NULL <= X ":
  */
  if (field->real_maybe_null() && part_info->has_null_value && !(flags & (NO_MIN_RANGE | NEAR_MIN)) && *min_value)
  {
    part_iter->ret_null_part = part_iter->ret_null_part_orig = TRUE;
    part_iter->part_nums.start = part_iter->part_nums.cur = 0;
    ...
    else
    {
      ...
      */
      ...
      if (!can_match_multiple_values && part_info->part_expr->null_value)
      {
        ...
        */
        if (part_info->part_type == RANGE_PARTITION || part_info->has_null_value)
        {
          part_iter->ret_null_part = part_iter->ret_null_part_orig = TRUE;
          DBUG_RETURN(1);
        }
      ...
      {
        if (!(flags & NO_MAX_RANGE) && (field->type() == MYSQL_TYPE_DATE || field->type() == MYSQL_TYPE_DATETIME))
        {
          ...
          DBUG_PRINT("info", ("zero start %u %04d-%02d-%02d", zero_in_start_date, start_date.year, start_date.month,
                              start_date.day));
        }
      }
  ...
  else
  {
    ...
    if (check_zero_dates && !zero_in_start_date && !part_info->part_expr->null_value)
    {
      MYSQL_TIME end_date;
      DBUG_ASSERT(field->cmp_type() == TIME_RESULT);  // No rounding/truncation
      ...
      /*
        ...
        month/day.
      */
      DBUG_PRINT("info", ("zero end %u %04d-%02d-%02d", zero_in_end_date, end_date.year, end_date.month, end_date.day));
      DBUG_ASSERT(!memcmp(((Item_func *)part_info->part_expr)->func_name(), "to_days", 7) ||
                  !memcmp(((Item_func *)part_info->part_expr)->func_name(), "to_seconds", 10));
      if (!zero_in_end_date && start_date.month == end_date.month && start_date.year == end_date.year)
        part_iter->ret_null_part = part_iter->ret_null_part_orig = false;
    }
    if (part_iter->part_nums.start >= part_iter->part_nums.end && !part_iter->ret_null_part)
      goto not_found;
  }
...
static int get_part_iter_for_interval_via_walking(partition_info *part_info, bool is_subpart,
                                                  uint32 *store_length_array, /* ignored */
                                                  uchar *min_value, uchar *max_value, uint min_len,
                                                  uint max_len, /* ignored */
                                                  uint flags, PARTITION_ITERATOR *part_iter)
{
  ...
  if (field->real_maybe_null() && !(flags & (NO_MIN_RANGE | NO_MAX_RANGE)) && *min_value && *max_value)
  {
    /*
    ...
    */
    ...
    {
      longlong dummy;
      int res = part_info->is_sub_partitioned() ? part_info->get_part_partition_id(part_info, &part_id, &dummy)
  ...
  if ((field->real_maybe_null() && ((!(flags & NO_MIN_RANGE) && *min_value) ||    // NULL <? X
...
static uint32 get_next_partition_via_walking(PARTITION_ITERATOR *part_iter)
{
  ...
  while (part_iter->field_vals.cur != part_iter->field_vals.end)
  {
    ...
    if ((part_iter->part_info->is_sub_partitioned() &&
         !part_iter->part_info->get_part_partition_id(part_iter->part_info, &part_id, &dummy)) ||
        !part_iter->part_info->get_partition_id(part_iter->part_info, &part_id, &dummy))
      return part_id;
  }
...
static const char *longest_str(const char *s1, const char *s2, const char *s3 = 0)
{
  if (strlen(s2) > strlen(s1))
    s1 = s2;
  if (s3 && strlen(s3) > strlen(s1))
    s1 = s3;
  return s1;
}
...
int create_partition_name(char *out, size_t outlen, const char *in1, const char *in2, uint name_variant, bool translate)
{
...
int create_subpartition_name(char *out, size_t outlen, const char *in1, const char *in2, const char *in3,
                             uint name_variant)
{
  ...
  if (name_variant == NORMAL_PART_NAME)
    end = strxnmov(out, outlen - 1, in1, "#P#", transl_part_name, "#SP#", transl_subpart_name, NullS);
  else if (name_variant == TEMP_PART_NAME)
    end = strxnmov(out, outlen - 1, in1, "#P#", transl_part_name, "#SP#", transl_subpart_name, "#TMP#", NullS);
  else
  {
    DBUG_ASSERT(name_variant == RENAMED_PART_NAME);
    end = strxnmov(out, outlen - 1, in1, "#P#", transl_part_name, "#SP#", transl_subpart_name, "#REN#", NullS);
  }
  if (end - out == static_cast<ptrdiff_t>(outlen - 1))
  {
    my_error(ER_PATH_LENGTH, MYF(0), longest_str(in1, transl_part_name, transl_subpart_name));
    return HA_WRONG_CREATE_OPTION;
  }
  return 0;
}
