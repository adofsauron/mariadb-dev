...
{
 public:
  CHARSET_INFO *get_db_cl() { return m_db_cl; }
 ...
 protected:
  ...
  Stored_program_creation_ctx(CHARSET_INFO *client_cs, CHARSET_INFO *connection_cl, CHARSET_INFO *db_cl)
...
class sp_name : public Sql_alloc, public Database_qualified_name
{
 public:
  bool m_explicit_name; /**< Prepend the db name? */

  sp_name(const LEX_CSTRING *db, const LEX_CSTRING *name, bool use_explicit_name)
...
class sp_head : private Query_arena, public Database_qualified_name, public Sql_alloc
{
  ...
  */
  ...
  inline void set_chistics_agg_type(enum enum_sp_aggregate_type type) { m_chistics.agg_type = type; }
  ...
  */
  ...
  void set_sp_cache_version(ulong version_arg) const { m_sp_cache_version = version_arg; }

  sp_rcontext *rcontext_create(THD *thd, Field *retval, List<Item> *args);
  sp_rcontext *rcontext_create(THD *thd, Field *retval, Item **args, uint arg_count);
  sp_rcontext *rcontext_create(THD *thd, Field *retval, Row_definition_list *list, bool switch_security_ctx);
 ...
 public:
  inline Stored_program_creation_ctx *get_creation_ctx() { return m_creation_ctx; }
 ...
 protected:
  sp_head(MEM_ROOT *mem_root, sp_package *parent, const Sp_handler *handler, enum_sp_aggregate_type agg_type);
  virtual ~sp_head();

 public:
  static void destroy(sp_head *sp);
  static sp_head *create(sp_package *parent, const Sp_handler *handler, enum_sp_aggregate_type agg_type);
  ...
  void set_stmt_end(THD *thd);

  bool execute_trigger(THD *thd, const LEX_CSTRING *db_name, const LEX_CSTRING *table_name, GRANT_INFO *grant_info);

  bool execute_function(THD *thd, Item **args, uint argcount, Field *return_fld, sp_rcontext **nctx,
                        Query_arena *call_arena);

  bool execute_procedure(THD *thd, List<Item> *args);

  static void show_create_routine_get_fields(THD *thd, const Sp_handler *sph, List<Item> *fields);

  bool show_create_routine(THD *thd, const Sp_handler *sph);

  MEM_ROOT *get_main_mem_root() { return &main_mem_root; }

  int add_instr(sp_instr *instr);

  bool add_instr_jump(THD *thd, sp_pcontext *spcont);

  bool add_instr_jump(THD *thd, sp_pcontext *spcont, uint dest);

  bool add_instr_jump_forward_with_backpatch(THD *thd, sp_pcontext *spcont, sp_label *lab);
  bool add_instr_jump_forward_with_backpatch(THD *thd, sp_pcontext *spcont)
  {
    return add_instr_jump_forward_with_backpatch(thd, spcont, spcont->last_label());
  }

  bool add_instr_freturn(THD *thd, sp_pcontext *spcont, Item *item, LEX *lex);

  bool add_instr_preturn(THD *thd, sp_pcontext *spcont);
  ...
  */
  bool set_local_variable(THD *thd, sp_pcontext *spcont, const Sp_rcontext_handler *rh, sp_variable *spv, Item *val,
                          LEX *lex, bool responsible_to_free_lex);
  bool set_local_variable_row_field(THD *thd, sp_pcontext *spcont, const Sp_rcontext_handler *rh, sp_variable *spv,
                                    uint field_idx, Item *val, LEX *lex);
  bool set_local_variable_row_field_by_name(THD *thd, sp_pcontext *spcont, const Sp_rcontext_handler *rh,
  ...
  */
  bool add_set_cursor_param_variable(THD *thd, sp_pcontext *param_spcont, uint idx, sp_assignment_lex *prm)
  {
    ...
    */
    ...
    if (set_local_variable(thd, param_spcont, &sp_rcontext_handler_local, spvar, prm->get_item(), prm, true))
      return true;
    /*
      Safety:
  ...
  */
  bool add_set_cursor_param_variables(THD *thd, sp_pcontext *param_spcont, List<sp_assignment_lex> *parameters)
  {
  ...
  /**
    ...
    @param
  */
  bool add_set_for_loop_cursor_param_variables(THD *thd, sp_pcontext *param_spcont, sp_assignment_lex *param_lex,
                                               Item_args *parameters);

  bool bind_input_param(THD *thd, Item *arg_item, uint arg_no, sp_rcontext *nctx, bool is_function);

  bool bind_output_param(THD *thd, Item *arg_item, uint arg_no, sp_rcontext *octx, sp_rcontext *nctx);
  ...
  */
  bool add_open_cursor(THD *thd, sp_pcontext *spcont, uint offset, sp_pcontext *param_spcont,
  ...
  /**
    ...
    @retval false     - on success
  */
  bool add_for_loop_open_cursor(THD *thd, sp_pcontext *spcont, sp_variable *index, const sp_pcursor *pcursor,
  ...
  */
  ...
  inline uint instructions() { return (uint)m_instr.elements; }
  ...
  */
  ...
  bool reset_lex(THD *thd, sp_lex_local *sublex);
  ...
  */
  bool merge_lex(THD *thd, LEX *oldlex, LEX *sublex);
  ...
  */
  ...
  int push_backpatch(THD *thd, sp_instr *, sp_label *);
  int push_backpatch_goto(THD *thd, sp_pcontext *ctx, sp_label *lab);

  /// Update all instruction with this label in the backpatch list to
  /// the current position.
  void backpatch(sp_label *);
  void backpatch_goto(THD *thd, sp_label *, sp_label *);

  /// Check for unresolved goto label
  bool check_unresolved_goto();

  /// Start a new cont. backpatch level. If 'i' is NULL, the level is just incr.
  int new_cont_backpatch(sp_instr_opt_meta *i);

  /// Add an instruction to the current level
  int add_cont_backpatch(sp_instr_opt_meta *i);
  ...
  Field *create_result_field(uint field_max_length, const LEX_CSTRING *field_name, TABLE *table) const;
  ...
  */
  bool fill_field_definition(THD *thd, Column_definition *field_def)
  {
    ...
    return h->Column_definition_fix_attributes(field_def) || field_def->sp_prepare_create_field(thd, mem_root);
  }
  ...
  */
  ...
  bool fill_spvar_definition(THD *thd, Column_definition *def, LEX_CSTRING *name)
  {
    def->field_name = *name;
  ...
  */
  void fill_spvar_using_type_reference(sp_variable *spvar, Qualified_column_ident *ref)
  {
  ...
  void fill_spvar_using_table_rowtype_reference(THD *thd, sp_variable *spvar, Table_ident *ref)
  {
 ...
 public:
  ...
  bool spvar_fill_type_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &table, const LEX_CSTRING &column);
  bool spvar_fill_type_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &db, const LEX_CSTRING &table,
                                 const LEX_CSTRING &column);
  bool spvar_fill_table_rowtype_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &table);
  bool spvar_fill_table_rowtype_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &db,
                                          const LEX_CSTRING &table);

  void set_c_chistics(const st_sp_chistics &chistics);
  void set_info(longlong created, longlong modified, const st_sp_chistics &chistics, sql_mode_t sql_mode);
  ...
  */
  void propagate_attributes(Query_tables_list *prelocking_ctx)
  {
    ...
    */
    ...
    DBUG_PRINT("info", ("sp_head(%p=%s)->unsafe_flags: 0x%x", this, name()->str, unsafe_flags));
  ...
  */
  ...
  int push_backpatch(THD *thd, sp_instr *, sp_label *, List<bp_t> *list, backpatch_instr_type itype);
...
class sp_package : public sp_head
{
 ...
 public:
  class LexList : public List<LEX>
  {
   public:
    ...
    bool check_dup_qualified(const sp_head *sp) { return check_dup_qualified(sp->m_name, sp->m_handler); }
    void cleanup();
  };
  /*
 ...
 private:
  sp_package(MEM_ROOT *mem_root, LEX *top_level_lex, const sp_name *name, const Sp_handler *sph);
  ~sp_package();

 public:
  static sp_package *create(LEX *top_level_lex, const sp_name *name, const Sp_handler *sph);
...
class sp_lex_cursor : public sp_lex_local, public Query_arena
{
 public:
  sp_lex_cursor(THD *thd, const LEX *oldlex, MEM_ROOT *mem_root_arg)
      : sp_lex_local(thd, oldlex), Query_arena(mem_root_arg, STMT_INITIALIZED_FOR_SP)
  {
  }
  sp_lex_cursor(THD *thd, const LEX *oldlex)
      : sp_lex_local(thd, oldlex), Query_arena(thd->lex->sphead->get_main_mem_root(), STMT_INITIALIZED_FOR_SP)
  {
  }
  ~sp_lex_cursor() { free_items(); }
  virtual bool cleanup_stmt(bool /*restore_set_statement_vars*/) override { return false; }
...
class sp_instr : public Query_arena, public Sql_alloc
{
 ...
 public:
  ...
  sp_instr(uint ip, sp_pcontext *ctx) : Query_arena(0, STMT_INITIALIZED_FOR_SP), marked(0), m_ip(ip), m_ctx(ctx) {}
...
class sp_lex_keeper
{
  ...
  */
  ...
  int cursor_reset_lex_and_exec_core(THD *thd, uint *nextp, bool open_tables, sp_instr *instr);
...
class sp_instr_set : public sp_instr
{
 ...
 public:
  sp_instr_set(uint ip, sp_pcontext *ctx, const Sp_rcontext_handler *rh, uint offset, Item *val, LEX *lex,
               bool lex_resp)
      : sp_instr(ip, ctx), m_rcontext_handler(rh), m_offset(offset), m_value(val), m_lex_keeper(lex, lex_resp)
  {
  }
...
class sp_instr_set_row_field : public sp_instr_set
{
 ...
 public:
  sp_instr_set_row_field(uint ip, sp_pcontext *ctx, const Sp_rcontext_handler *rh, uint offset, uint field_offset,
                         Item *val, LEX *lex, bool lex_resp)
      : sp_instr_set(ip, ctx, rh, offset, val, lex, lex_resp), m_field_offset(field_offset)
  {
  }
...
class sp_instr_set_row_field_by_name : public sp_instr_set
{
 ...
 public:
  sp_instr_set_row_field_by_name(uint ip, sp_pcontext *ctx, const Sp_rcontext_handler *rh, uint offset,
                                 const LEX_CSTRING &field_name, Item *val, LEX *lex, bool lex_resp)
      : sp_instr_set(ip, ctx, rh, offset, val, lex, lex_resp), m_field_name(field_name)
  {
  }
...
class sp_instr_opt_meta : public sp_instr
{
 public:
  ...
  sp_instr_opt_meta(uint ip, sp_pcontext *ctx)
      : sp_instr(ip, ctx), m_dest(0), m_cont_dest(0), m_optdest(0), m_cont_optdest(0)
  {
  }

  sp_instr_opt_meta(uint ip, sp_pcontext *ctx, uint dest)
      : sp_instr(ip, ctx), m_dest(dest), m_cont_dest(0), m_optdest(0), m_cont_optdest(0)
  {
  }
...
class sp_instr_jump : public sp_instr_opt_meta
{
 ...
 public:
  sp_instr_jump(uint ip, sp_pcontext *ctx) : sp_instr_opt_meta(ip, ctx) {}

  sp_instr_jump(uint ip, sp_pcontext *ctx, uint dest) : sp_instr_opt_meta(ip, ctx, dest) {}
...
class sp_instr_preturn : public sp_instr
{
 ...
 public:
  sp_instr_preturn(uint ip, sp_pcontext *ctx) : sp_instr(ip, ctx) {}
...
class sp_instr_freturn : public sp_instr
{
 ...
 public:
  sp_instr_freturn(uint ip, sp_pcontext *ctx, Item *val, const Type_handler *handler, LEX *lex)
      : sp_instr(ip, ctx), m_value(val), m_type_handler(handler), m_lex_keeper(lex, TRUE)
  {
  }
...
class sp_instr_hpush_jump : public sp_instr_jump
{
 ...
 public:
  sp_instr_hpush_jump(uint ip, sp_pcontext *ctx, sp_handler *handler)
      : sp_instr_jump(ip, ctx), m_handler(handler), m_opt_hpop(0), m_frame(ctx->current_var_count())
  {
  ...
  void add_condition(sp_condition_value *condition_value) { m_handler->condition_values.push_back(condition_value); }
...
class sp_instr_hpop : public sp_instr
{
 ...
 public:
  ...
  void update_count(uint count) { m_count = count; }
...
class sp_instr_hreturn : public sp_instr_jump
{
 ...
 public:
  sp_instr_hreturn(uint ip, sp_pcontext *ctx) : sp_instr_jump(ip, ctx), m_frame(ctx->current_var_count()) {}
...
class sp_instr_cursor_copy_struct : public sp_instr
{
 ...
 public:
  sp_instr_cursor_copy_struct(uint ip, sp_pcontext *ctx, uint coffs, sp_lex_cursor *lex, uint voffs)
      : sp_instr(ip, ctx), m_lex_keeper(lex, FALSE), m_cursor(coffs), m_var(voffs)
  {
  }
...
class sp_instr_agg_cfetch : public sp_instr
{
 ...
 public:
  sp_instr_agg_cfetch(uint ip, sp_pcontext *ctx) : sp_instr(ip, ctx) {}
...
class sp_instr_set_case_expr : public sp_instr_opt_meta
{
 public:
  sp_instr_set_case_expr(uint ip, sp_pcontext *ctx, uint case_expr_id, Item *case_expr, LEX *lex)
      : sp_instr_opt_meta(ip, ctx), m_case_expr_id(case_expr_id), m_case_expr(case_expr), m_lex_keeper(lex, TRUE)
  {
  }
