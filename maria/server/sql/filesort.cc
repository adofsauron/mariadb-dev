...
static uchar *read_buffpek_from_file(IO_CACHE *buffer_file, uint count, uchar *buf);
static ha_rows find_all_keys(THD *thd, Sort_param *param, SQL_SELECT *select, SORT_INFO *fs_info, IO_CACHE *buffer_file,
                             IO_CACHE *tempfile, Bounded_queue<uchar, uchar> *pq, ha_rows *found_rows);
static bool write_keys(Sort_param *param, SORT_INFO *fs_info, uint count, IO_CACHE *buffer_file, IO_CACHE *tempfile);
static uint make_sortkey(Sort_param *param, uchar *to, uchar *ref_pos, bool using_packed_sortkeys = false);
...
static uint sortlength(THD *thd, Sort_keys *sortorder, bool *allow_packing_for_sortkeys);
static Addon_fields *get_addon_fields(TABLE *table, uint sortlength, uint *addon_length, uint *m_packable_length);

static bool check_if_pq_applicable(Sort_param *param, SORT_INFO *info, TABLE *table, ha_rows records,
...
void Sort_param::init_for_filesort(uint sortlen, TABLE *table, ha_rows maxrows, Filesort *filesort)
{
  ...
  if (!(table->file->ha_table_flags() & HA_FAST_KEY_READ) && !table->fulltext_searched && !filesort->sort_positions)
  {
    /*
      Get the descriptors of all fields whose values are appended
      to sorted fields and get its total length in addon_buf.length
    */
    addon_fields = get_addon_fields(table, sort_length, &addon_length, &m_packable_length);
  }
...
SORT_INFO *filesort(THD *thd, TABLE *table, Filesort *filesort, Filesort_tracker *tracker, JOIN *join,
                    table_map first_table_bit)
{
  ...
  DBUG_ASSERT(sort->sorted_result_in_fsbuf == FALSE || sort->record_pointers == NULL);
  ...
  if (check_if_pq_applicable(&param, sort, table, num_rows, memory_available))
  {
    ...
    */
    ...
    if (pq.init(param.max_rows,
                ...
                compare_length, &make_sortkey, &param, sort->get_sort_keys()))
    {
      /*
  ...
  else
  {
    ...
    while (memory_available >= min_sort_memory)
    {
      ...
      param.max_keys_per_buffer = (uint)MY_MAX(MERGEBUFF2, MY_MIN(num_rows, keys));
      ...
      if (memory_available < min_sort_memory && old_memory_available > min_sort_memory)
  ...
  param.local_sortorder = Bounds_checked_array<SORT_FIELD>(filesort->sortorder, s_length);

  num_rows = find_all_keys(thd, &param, select, sort, &buffpek_pointers, &tempfile, pq.is_initialized() ? &pq : NULL,
                           &sort->found_rows);
  ...
  else
  {
    ...
    if (!(sort->buffpek.str = (char *)read_buffpek_from_file(&buffpek_pointers, maxbuffer, (uchar *)sort->buffpek.str)))
      goto err;
    ...
    */
    ...
    param.max_keys_per_buffer = static_cast<uint>(sort->sort_buffer_size()) / param.rec_length;
    set_if_bigger(param.max_keys_per_buffer, 1);
    maxbuffer--;  // Offset from 0

    if (merge_many_buff(&param, sort->get_raw_buf(), buffpek, &maxbuffer, &tempfile))
      goto err;
    if (flush_io_cache(&tempfile) || reinit_io_cache(&tempfile, READ_CACHE, 0L, 0, 0))
      goto err;
    if (merge_index(&param, sort->get_raw_buf(), buffpek, maxbuffer, &tempfile, outfile))
      goto err;
  }
...
err:
  ...
  if (unlikely(error))
  {
    ...
    my_printf_error(ER_FILSORT_ABORT, "%s: %s", MYF(0), ER_THD(thd, ER_FILSORT_ABORT),
                    kill_errno                   ? ER_THD(thd, kill_errno)
                    : thd->killed == ABORT_QUERY ? ""
                                                 : thd->get_stmt_da()->message());

    if ((thd->killed == ABORT_QUERY || kill_errno) && global_system_variables.log_warnings > 1)
    {
      sql_print_warning("%s, host: %s, user: %s, thread: %lu, query: %-.4096s", ER_THD(thd, ER_FILSORT_ABORT),
                        thd->security_ctx->host_or_ip, &thd->security_ctx->priv_user[0], (ulong)thd->thread_id,
                        thd->query());
    }
  }
  else
  ...
  DBUG_PRINT("exit", ("num_rows: %lld examined_rows: %lld found_rows: %lld", (longlong)sort->return_rows,
                      (longlong)sort->examined_rows, (longlong)sort->found_rows));
...
Sort_keys *Filesort::make_sortorder(THD *thd, JOIN *join, table_map first_table_bit)
{
  uint count;
  SORT_FIELD *sort, *pos;
  ORDER *ord;
  ...
  for (ord = order; ord; ord = ord->next, pos++)
  {
    ...
    */
    ...
    if (join && (item_map & ~join->const_table_map) && !(item_map & first_table_bit) && join->cond_equal &&
    ...
    else if (item->type() == Item::SUM_FUNC_ITEM && !item->const_item())
    {
      ...
      DBUG_ASSERT(
          first->type() == Item::SUM_FUNC_ITEM ||
          (first->type() == Item::REF_ITEM && static_cast<Item_ref *>(first)->ref_type() == Item_ref::AGGREGATE_REF));
...
static uchar *read_buffpek_from_file(IO_CACHE *buffpek_pointers, uint count, uchar *buf)
{
  ...
  if (tmp)
  {
    if (reinit_io_cache(buffpek_pointers, READ_CACHE, 0L, 0, 0) || my_b_read(buffpek_pointers, (uchar *)tmp, length))
    {
      my_free(tmp);
      tmp = 0;
    }
...
{
  Field **pfield;
  String tmp(dbug_print_row_buff_tmp, sizeof(dbug_print_row_buff_tmp), &my_charset_bin);

  String output(dbug_print_row_buff, sizeof(dbug_print_row_buff), &my_charset_bin);
...
static void dbug_print_record(TABLE *table, bool print_rowid)
{
  ...
  for (pfield = table->field; *pfield; pfield++)
    fprintf(DBUG_FILE, "%s%s", (*pfield)->field_name.str, (pfield[1]) ? ", " : "");
  fprintf(DBUG_FILE, ") = ");
...
static ha_rows find_all_keys(THD *thd, Sort_param *param, SQL_SELECT *select, SORT_INFO *fs_info,
                             IO_CACHE *buffpek_pointers, IO_CACHE *tempfile, Bounded_queue<uchar, uchar> *pq,
                             ha_rows *found_rows)
{
  ...
  *found_rows = 0;
  ...
  sort_cond = (!select ? 0 : (!select->pre_idx_push_select_cond ? select->cond : select->pre_idx_push_select_cond));
  if (sort_cond)
  ...
  for (;;)
  {
    ...
    if (write_record)
    {
      ...
      else
      {
        ...
        const uint rec_sz = make_sortkey(param, start_of_rec, ref_pos, using_packed_sortkeys);
  ...
  if (indexpos && idx && write_keys(param, fs_info, idx, buffpek_pointers, tempfile))
...
static bool write_keys(Sort_param *param, SORT_INFO *fs_info, uint count, IO_CACHE *buffpek_pointers,
                       IO_CACHE *tempfile)
{
  Merge_chunk buffpek;
...
void Type_handler_string_result::make_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                    Sort_param *param) const
{
  ...
  if (use_strnxfrm(cs))
  {
    ...
    size_t tmp_length =
#endif
        cs->strnxfrm(to, sort_field->length, item->max_char_length() * cs->strxfrm_multiply, (uchar *)res->ptr(),
                     res->length(), MY_STRXFRM_PAD_WITH_SPACE | MY_STRXFRM_PAD_TO_MAXLEN);
...
void Type_handler_int_result::make_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                 Sort_param *param) const
{
  longlong value = item->val_int_result();
  make_sort_key_longlong(to, item->maybe_null(), item->null_value, item->unsigned_flag, value);
}

void Type_handler_temporal_result::make_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                      Sort_param *param) const
{
  MYSQL_TIME buf;
  ...
  if (item->get_date_result(current_thd, &buf, opt))
  {
    ...
    make_sort_key_longlong(to, item->maybe_null(), true, item->unsigned_flag, 0);
  }
  else
    make_sort_key_longlong(to, item->maybe_null(), false, item->unsigned_flag, pack_time(&buf));
}

void Type_handler_timestamp_common::make_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                       Sort_param *param) const
{
  THD *thd = current_thd;
...
void Type_handler::store_sort_key_longlong(uchar *to, bool unsigned_flag, longlong value) const
{
  to[7] = (uchar)value;
  to[6] = (uchar)(value >> 8);
  to[5] = (uchar)(value >> 16);
...
void Type_handler::make_sort_key_longlong(uchar *to, bool maybe_null, bool null_value, bool unsigned_flag,
...
uint Type_handler::make_packed_sort_key_longlong(uchar *to, bool maybe_null, bool null_value, bool unsigned_flag,
                                                 longlong value, const SORT_FIELD_ATTR *sort_field) const
{
  if (maybe_null)
  {
    if (null_value)
...
void Type_handler_decimal_result::make_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                     Sort_param *param) const
{
  ...
  dec_val->to_binary(to, item->max_length - (item->decimals ? 1 : 0), item->decimals);
}

void Type_handler_real_result::make_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                  Sort_param *param) const
{
...
static uint make_sortkey(Sort_param *param, uchar *to, uchar *ref_pos, bool using_packed_sortkeys)
{
  uchar *orig_to = to;

  to += using_packed_sortkeys ? make_packed_sortkey(param, to) : make_sortkey(param, to);
...
static void register_used_fields(Sort_param *param)
{
  ...
  for (sort_field = param->local_sortorder.begin(); sort_field != param->local_sortorder.end(); sort_field++)
  {
    Field *field;
...
static bool save_index(Sort_param *param, uint count, SORT_INFO *table_sort)
{
  ...
  if (!(to = table_sort->record_pointers = (uchar *)my_malloc(key_memory_Filesort_info_record_pointers,
  ...
  for (uint ix = 0; ix < count; ++ix)
  {
    ...
    length = using_packed_sortkeys ? Sort_keys::read_sortkey_length(record) : offset;
...
static bool check_if_pq_applicable(Sort_param *param, SORT_INFO *filesort_info, TABLE *table, ha_rows num_rows,
                                   size_t memory_available)
{
  ...
  */
  ...
  size_t num_available_keys = memory_available / (param->rec_length + sizeof(char *));
  ...
  if (num_rows < num_available_keys)
  {
    ...
    {
      filesort_info->alloc_sort_buffer(param->max_keys_per_buffer, param->rec_length);
  ...
  {
    const size_t row_length = param->sort_length + param->ref_length + sizeof(char *);
    num_available_keys = memory_available / row_length;

    // Can we fit all the keys in memory?
    if (param->max_keys_per_buffer < num_available_keys)
    {
      const double sort_merge_cost = get_merge_many_buffs_cost_fast(num_rows, num_available_keys, (uint)row_length);
      /*
        PQ has cost:
      ...
      */
      const double pq_cpu_cost = (PQ_slowness * num_rows + param->max_keys_per_buffer) *
                                 log((double)param->max_keys_per_buffer) / TIME_FOR_COMPARE_ROWID;
      const double pq_io_cost = param->max_rows * table->file->scan_time() / 2.0;
      const double pq_cost = pq_cpu_cost + pq_io_cost;

      if (sort_merge_cost < pq_cost)
        DBUG_RETURN(false);

      filesort_info->alloc_sort_buffer(param->max_keys_per_buffer, param->sort_length + param->ref_length);
...
int merge_many_buff(Sort_param *param, Sort_buffer sort_buffer, Merge_chunk *buffpek, uint *maxbuffer, IO_CACHE *t_file)
{
  uint i;
...
ulong read_to_buffer(IO_CACHE *fromfile, Merge_chunk *buffpek, Sort_param *param, bool packed_format)
{
  ...
  if ((count = MY_MIN(buffpek->max_keys(), buffpek->rowcount())))
  {
    ...
    if (packed_format)
    {
      ...
      bytes_to_read =
          MY_MIN(buffpek->buffer_size(), static_cast<size_t>(fromfile->end_of_file - buffpek->file_position()));
    }
    else
      bytes_to_read = rec_length * static_cast<size_t>(count);

    if (unlikely(my_b_pread(fromfile, buffpek->buffer_start(), bytes_to_read, buffpek->file_position())))
      return ((ulong)-1);
    ...
    if (packed_format)
    {
      ...
      */
      ...
      uint ix = 0;
      uint size_of_addon_length = param->using_packed_addons() ? Addon_fields::size_of_length_field : 0;

      uint size_of_sort_length = param->using_packed_sortkeys() ? Sort_keys::size_of_length_field : 0;

      for (; ix < count; ++ix)
      {
        if (record + size_of_sort_length > buffpek->buffer_end())
          break;
        uint sort_length = param->using_packed_sortkeys() ? Sort_keys::read_sortkey_length(record) : param->sort_length;

        DBUG_ASSERT(sort_length <= param->sort_length);

        if (record + sort_length + size_of_addon_length > buffpek->buffer_end())
      ...
      DBUG_PRINT("info", ("read %llu bytes of complete records", static_cast<ulonglong>(bytes_to_read)));
    }
    else
...
{
  for (uint i = queue_first_element(queue); i <= queue_last_element(queue); i++)
  {
                   ...
                   Merge_chunk *lastbuff, Merge_chunk *Fb, Merge_chunk *Tb, int flag)
{
  ...
  offset = (rec_length - (flag && min_dupl_count ? sizeof(dupl_count) : 0) - res_length);
  ...
  if (unlikely(init_queue(&queue, (uint)(Tb - Fb) + 1, offsetof(Merge_chunk, m_current_key), 0, (queue_compare)cmp,
                          first_cmp_arg, 0, 0)))
    DBUG_RETURN(1); /* purecov: inspected */
  ...
  if (unique_buff)
  {
    ...
    */
    ...
    {
      if (unlikely(!(bytes_read = read_to_buffer(from_file, buffpek, param, packed_format))))
      {
  ...
  while (queue.elements > 1)
  {
    ...
    for (;;)
    {
      ...
      {
        param->get_rec_and_res_len(buffpek->current_key(), &rec_length, &res_length);
        ...
        */
        if (!check_dupl_count || dupl_count >= min_dupl_count)
        {
          if (my_b_write(to_file,
                         src + (offset_for_packing ? rec_length - res_length :  // sort length
                                    wr_offset),
                         bytes_to_write))
            goto err; /* purecov: inspected */
        }
  ...
  buffpek = (Merge_chunk *)queue_top(&queue);
  buffpek->set_buffer(sort_buffer.array(), sort_buffer.array() + sort_buffer.size());
  ...
  */
  ...
  do
  {
    ...
    for (uint ix = 0; ix < buffpek->mem_count(); ++ix)
    {
      ...
      if (check_dupl_count)
      {
        memcpy((uchar *)&dupl_count, buffpek->current_key() + offset + dupl_count_ofs, sizeof(dupl_count));
  ...
  } while (likely(!(error = (bytes_read = read_to_buffer(from_file, buffpek, param, packed_format)) == (ulong)-1)) &&
           bytes_read != 0);
...
void Type_handler_string_result::sort_length(THD *thd, const Type_std_attributes *item,
...
void Type_handler_temporal_result::sort_length(THD *thd, const Type_std_attributes *item,
                                               SORT_FIELD_ATTR *sortorder) const
{
  sortorder->original_length = sortorder->length = 8;  // Sizof intern longlong
}

void Type_handler_timestamp_common::sort_length(THD *thd, const Type_std_attributes *item,
                                                SORT_FIELD_ATTR *sortorder) const
{
...
void Type_handler_int_result::sort_length(THD *thd, const Type_std_attributes *item, SORT_FIELD_ATTR *sortorder) const
{
  sortorder->original_length = sortorder->length = 8;  // Sizof intern longlong
}

void Type_handler_real_result::sort_length(THD *thd, const Type_std_attributes *item, SORT_FIELD_ATTR *sortorder) const
{
  sortorder->original_length = sortorder->length = sizeof(double);
}

void Type_handler_decimal_result::sort_length(THD *thd, const Type_std_attributes *item,
                                              SORT_FIELD_ATTR *sortorder) const
{
  sortorder->length = my_decimal_get_binary_size(item->max_length - (item->decimals ? 1 : 0), item->decimals);
  sortorder->original_length = sortorder->length;
}
...
static uint sortlength(THD *thd, Sort_keys *sort_keys, bool *allow_packing_for_sortkeys)
{
  uint length;
  *allow_packing_for_sortkeys = true;
  ...
  for (SORT_FIELD *sortorder = sort_keys->begin(); sortorder != sort_keys->end(); sortorder++)
  {
    ...
    if (sortorder->field)
    {
      ...
      sortorder->type = field->is_packable() ? SORT_FIELD_ATTR::VARIABLE_SIZE : SORT_FIELD_ATTR::FIXED_SIZE;
      ...
      if (sortorder->is_variable_sized() && allow_packing_for_keys)
      {
        ...
        sortorder->length_bytes =
            number_storage_requirement(MY_MIN(sortorder->original_length, thd->variables.max_sort_length));
      }
    ...
    {
      sortorder->type =
          sortorder->item->type_handler()->is_packable() ? SORT_FIELD_ATTR::VARIABLE_SIZE : SORT_FIELD_ATTR::FIXED_SIZE;
      sortorder->item->type_handler()->sort_length(thd, sortorder->item, sortorder);
...
bool filesort_use_addons(TABLE *table, uint sortlength, uint *length, uint *fields, uint *null_fields,
                         uint *packable_length)
{
  ...
  */
  return *length + sortlength < table->in_use->variables.max_length_for_sort_data;
}
...
static Addon_fields *get_addon_fields(TABLE *table, uint sortlength, uint *addon_length, uint *m_packable_length)
{
  ...
  */
  ...
  if (!filesort_use_addons(table, sortlength, &length, &fields, &null_fields, &packable_length) ||
      !(my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME | MY_THREAD_SPECIFIC), &raw_mem, sizeof(Addon_fields),
                        &raw_mem_addon_field, sizeof(SORT_ADDON_FIELD) * fields, NullS)))
    DBUG_RETURN(0);

  Addon_fields_array addon_array(static_cast<SORT_ADDON_FIELD *>(raw_mem_addon_field), fields);
...
bool SORT_INFO::using_packed_addons() { return addon_fields != NULL && addon_fields->using_packed_addons(); }
...
*/
bool SORT_INFO::using_packed_sortkeys() { return sort_keys != NULL && sort_keys->using_packed_sortkeys(); }
...
void Sort_param::try_to_pack_sortkeys()
{
  ...
  */
  ...
  sort_length = sort_len + sz + size_of_packable_fields + (using_addon_fields() ? 0 : res_length);
...
uint Type_handler_string_result::make_packed_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                           Sort_param *param) const
{
  CHARSET_INFO *cs = item->collation.collation;
  bool maybe_null = item->maybe_null();
...
uint Type_handler_int_result::make_packed_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                        Sort_param *param) const
{
  longlong value = item->val_int_result();
  return make_packed_sort_key_longlong(to, item->maybe_null(), item->null_value, item->unsigned_flag, value,
                                       sort_field);
}

uint Type_handler_decimal_result::make_packed_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                            Sort_param *param) const
{
  my_decimal dec_buf, *dec_val = item->val_decimal_result(&dec_buf);
  if (item->maybe_null())
...
uint Type_handler_real_result::make_packed_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                         Sort_param *param) const
{
  double value = item->val_result();
  if (item->maybe_null())
...
uint Type_handler_temporal_result::make_packed_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                             Sort_param *param) const
{
  MYSQL_TIME buf;
  // This is a temporal type. No nanoseconds. Rounding mode is not important.
  ...
  if (item->get_date_result(current_thd, &buf, opt))
  {
    DBUG_ASSERT(item->maybe_null());
    DBUG_ASSERT(item->null_value);
    return make_packed_sort_key_longlong(to, item->maybe_null(), true, item->unsigned_flag, 0, sort_field);
  }
  return make_packed_sort_key_longlong(to, item->maybe_null(), false, item->unsigned_flag, pack_time(&buf), sort_field);
}

uint Type_handler_timestamp_common::make_packed_sort_key_part(uchar *to, Item *item, const SORT_FIELD_ATTR *sort_field,
                                                              Sort_param *param) const
{
  THD *thd = current_thd;
  uint binlen = my_timestamp_binary_length(item->decimals);
...
bool SORT_FIELD_ATTR::check_if_packing_possible(THD *thd) const
{
  /*
    ...
    such a case
  */
  if (original_length > thd->variables.max_sort_length && cs->state & MY_CS_NON1TO1)
    return false;
  return true;
}
...
qsort2_cmp get_packed_keys_compare_ptr() { return (qsort2_cmp)compare_packed_sort_keys; }
...
int SORT_FIELD_ATTR::compare_packed_varstrings(uchar *a, size_t *a_len, uchar *b, size_t *b_len)
{
  int retval;
  ...
  *b_len += length_bytes + b_length;
  ...
  retval = cs->strnncollsp(a + length_bytes, a_length - suffix_length, b + length_bytes, b_length - suffix_length);
...
int SORT_FIELD_ATTR::compare_packed_fixed_size_vals(uchar *a, size_t *a_len, uchar *b, size_t *b_len)
{
  if (maybe_null)
  {
    *a_len = 1;
...
int compare_packed_sort_keys(void *sort_param, unsigned char **a_ptr, unsigned char **b_ptr)
{
  int retval = 0;
  size_t a_len, b_len;
  ...
  for (SORT_FIELD *sort_field = sort_keys->begin(); sort_field != sort_keys->end(); sort_field++)
  {
    retval = sort_field->is_variable_sized() ? sort_field->compare_packed_varstrings(a, &a_len, b, &b_len)
...
uint SORT_FIELD_ATTR::pack_sort_string(uchar *to, const Binary_string *str, CHARSET_INFO *cs) const
{
  uchar *orig_to = to;
...
static uint make_sortkey(Sort_param *param, uchar *to)
{
  ...
  for (sort_field = param->local_sortorder.begin(); sort_field != param->local_sortorder.end(); sort_field++)
  {
    ...
    {  // Item
      sort_field->item->type_handler()->make_sort_key_part(to, sort_field->item, sort_field, param);
...
static uint make_packed_sortkey(Sort_param *param, uchar *to)
{
  ...
  for (sort_field = param->local_sortorder.begin(); sort_field != param->local_sortorder.end(); sort_field++)
  {
    ...
    {  // Item
      ...
      length = item->type_handler()->make_packed_sort_key_part(to, item, sort_field, param);
      if ((maybe_null = sort_field->item->maybe_null()))
        to++;
    }
    to += length;
