...
static my_bool write_parameter(IO_CACHE *file, const uchar *base, File_option *parameter)
{
  ...
  switch (parameter->type)
  {
    ...
    case FILE_OPTIONS_TIMESTAMP:
    {
      ...
      val_s->length = snprintf(val_s->str, MICROSECOND_TIMESTAMP_BUFFER_SIZE, "%019lld", tm.val);
      DBUG_ASSERT(val_s->length == MICROSECOND_TIMESTAMP_BUFFER_SIZE - 1);
      if (my_b_write(file, (const uchar *)val_s->str, PARSE_FILE_TIMESTAMPLENGTH))
        DBUG_RETURN(TRUE);
      break;
    }
    case FILE_OPTIONS_STRLIST:
    {
      ...
      while ((str = it++))
      {
        ...
        if ((!first && my_b_write(file, (const uchar *)STRING_WITH_LEN(" "))) ||
            my_b_write(file, (const uchar *)STRING_WITH_LEN("\'")) || write_escaped_string(file, str) ||
...
my_bool sql_create_definition_file(const LEX_CSTRING *dir, const LEX_CSTRING *file_name, const LEX_CSTRING *type,
                                   uchar *base, File_option *parameters)
{
  ...
  DBUG_PRINT("enter", ("Dir: %s, file: %s, base %p", dir ? dir->str : "", file_name->str, base));
  ...
  path[path_end] = '~';
  path[path_end + 1] = '\0';
  if ((handler = mysql_file_create(key_file_fileparser, path, CREATE_MODE, O_RDWR | O_TRUNC, MYF(MY_WME))) < 0)
  {
    DBUG_RETURN(TRUE);
  }
  ...
  {
    if (my_b_write(&file, (const uchar *)param->name.str, param->name.length) ||
        my_b_write(&file, (const uchar *)STRING_WITH_LEN("=")) || write_parameter(&file, base, param) ||
        my_b_write(&file, (const uchar *)STRING_WITH_LEN("\n")))
      goto err_w_cache;
  }
...
int sql_backup_definition_file(const LEX_CSTRING *org_name, LEX_CSTRING *new_name)
{
...
int sql_restore_definition_file(const LEX_CSTRING *name)
{
  ...
  return mysql_file_rename(key_file_fileparser, name->str, new_name, MYF(MY_WME));
}
                              ...
                              const char *new_name)
{
  ...
  build_table_filename(old_path, sizeof(old_path) - 1, schema, old_name, reg_ext, 0);
  build_table_filename(new_path, sizeof(new_path) - 1, new_db, new_name, reg_ext, 0);
...
File_parser *sql_parse_prepare(const LEX_CSTRING *file_name, MEM_ROOT *mem_root, bool bad_format_errors)
{
  MY_STAT stat_info;
  size_t len;
  char *buff, *end, *sign;
  ...
  if (!mysql_file_stat(key_file_fileparser, file_name->str, &stat_info, MYF(MY_WME)))
  {
    DBUG_RETURN(0);
  }
  ...
  if ((file = mysql_file_open(key_file_fileparser, file_name->str, O_RDONLY | O_SHARE, MYF(MY_WME))) < 0)
  {
    DBUG_RETURN(0);
  }

  if ((len = mysql_file_read(file, (uchar *)buff, (size_t)stat_info.st_size, MYF(MY_WME))) == MY_FILE_ERROR)
  {
  ...
  *end = '\0';  // barrier for more simple parsing
  ...
  if (len < 7 || buff[0] != 'T' || buff[1] != 'Y' || buff[2] != 'P' || buff[3] != 'E' || buff[4] != '=')
    goto frm_error;
...
const char *parse_escaped_string(const char *ptr, const char *end, MEM_ROOT *mem_root, LEX_CSTRING *str)
{
  const char *eol = strchr(ptr, '\n');

  if (eol == 0 || eol >= end || !(str->str = (char *)alloc_root(mem_root, (eol - ptr) + 1)) ||
...
static const char *parse_quoted_escaped_string(const char *ptr, const char *end, MEM_ROOT *mem_root, LEX_STRING *str)
{
  ...
  // process string
  if (eol >= end || !(str->str = (char *)alloc_root(mem_root, result_len + 1)) || read_escaped_string(ptr, eol, str))
    return 0;
...
my_bool File_parser::parse(uchar *base, MEM_ROOT *mem_root, struct File_option *parameters, uint required,
      ...
      if (parameter < parameters_end)
      {
        ...
        */
        ...
        switch (parameter->type)
        {
          ...
          case FILE_OPTIONS_ESTRING:
          {
            if (!(ptr = parse_escaped_string(ptr, end, mem_root, (LEX_CSTRING *)(base + parameter->offset))))
            {
              my_error(ER_FPARSER_ERROR_IN_PARAMETER, MYF(0), parameter->name.str, line);
              DBUG_RETURN(TRUE);
            }
          ...
          case FILE_OPTIONS_ULLLIST:
            if (get_file_options_ulllist(ptr, end, line, base, parameter, mem_root))
              DBUG_RETURN(TRUE);
            break;
          default:
            DBUG_ASSERT(0);  // never should happened
...
bool File_parser_dummy_hook::process_unknown_string(const char *&unknown_key, uchar *base, MEM_ROOT *mem_root,
                                                    const char *end)
{
