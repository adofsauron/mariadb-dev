...
static void raise_ER_CONFLICTING_DECLARATIONS(const char *clause1, const char *name1, const char *clause2,
                                              const char *name2, bool reverse_order)
{
  if (!reverse_order)
    my_error(ER_CONFLICTING_DECLARATIONS, MYF(0), clause1, name1, clause2, name2);
  else
    my_error(ER_CONFLICTING_DECLARATIONS, MYF(0), clause2, name2, clause1, name1);
}

static void raise_ER_CONFLICTING_DECLARATIONS(const char *clause1, const char *name1, const char *name1_part2,
                                              const char *clause2, const char *name2, bool reverse_order)
{
  char def[MY_CS_CHARACTER_SET_NAME_SIZE * 2];
  my_snprintf(def, sizeof(def), "%s (%s)", name1, name1_part2);
  raise_ER_CONFLICTING_DECLARATIONS(clause1, def, clause2, name2, reverse_order);
}
...
bool Lex_exact_charset::raise_if_not_equal(const Lex_exact_charset &rhs) const
{
  if (m_ci == rhs.m_ci)
    return false;
  my_error(ER_CONFLICTING_DECLARATIONS, MYF(0), "CHARACTER SET ", m_ci->cs_name.str, "CHARACTER SET ",
           rhs.m_ci->cs_name.str);
  return true;
}

bool Lex_exact_charset::raise_if_not_applicable(const Lex_exact_collation &cl) const
{
  return Lex_exact_charset_opt_extended_collate(m_ci, false).raise_if_not_applicable(cl);
}

bool Lex_exact_charset_opt_extended_collate::raise_if_charsets_differ(const Lex_exact_charset &cs) const
{
  if (!my_charset_same(m_ci, cs.charset_info()))
  {
    my_error(ER_CONFLICTING_DECLARATIONS, MYF(0), "CHARACTER SET ", m_ci->cs_name.str, "CHARACTER SET ",
             cs.charset_info()->cs_name.str);
    return true;
  }
  return false;
}

bool Lex_exact_charset_opt_extended_collate::raise_if_not_applicable(const Lex_exact_collation &cl) const
{
  if (!my_charset_same(m_ci, cl.charset_info()))
  {
    my_error(ER_COLLATION_CHARSET_MISMATCH, MYF(0), cl.charset_info()->coll_name.str, m_ci->cs_name.str);
    return true;
  }
  return false;
}

bool Lex_exact_collation::raise_if_not_equal(const Lex_exact_collation &cl) const
{
  if (m_ci != cl.m_ci)
  {
    my_error(ER_CONFLICTING_DECLARATIONS, MYF(0), "COLLATE ", m_ci->coll_name.str, "COLLATE ", cl.m_ci->coll_name.str);
    return true;
  }
  return false;
}
...
*/
bool Lex_exact_collation::raise_if_conflicts_with_context_collation(const Lex_context_collation &cl,
                                                                    bool reverse_order) const
{
  if (cl.is_contextually_typed_collate_default())
  {
    ...
    {
      raise_ER_CONFLICTING_DECLARATIONS("COLLATE ", m_ci->coll_name.str, "COLLATE ", "DEFAULT", reverse_order);
      return true;
    }
    return false;
  }
  ...
  if (cl.is_contextually_typed_binary_style())
  {
    ...
    {
      raise_ER_CONFLICTING_DECLARATIONS("COLLATE ", m_ci->coll_name.str, "", "BINARY", reverse_order);
      return true;
    }
    return false;
  }

  DBUG_ASSERT(!strncmp(cl.charset_info()->coll_name.str, STRING_WITH_LEN("utf8mb4_uca1400_")));

  Charset_loader_server loader;
  CHARSET_INFO *ci = loader.get_exact_collation_by_context_name(m_ci, cl.collation_name_context_suffix().str, MYF(0));
  if (m_ci != ci)
  {
    raise_ER_CONFLICTING_DECLARATIONS("COLLATE ", m_ci->coll_name.str, "COLLATE ", cl.collation_name_for_show().str,
                                      reverse_order);
    return true;
  }
  return false;
...
bool Lex_context_collation::raise_if_not_equal(const Lex_context_collation &cl) const
{
  /*
  ...
  */
  if (m_ci != cl.m_ci)
  {
    my_error(ER_CONFLICTING_DECLARATIONS, MYF(0), is_contextually_typed_binary_style() ? "" : "COLLATE ",
             collation_name_for_show().str, cl.is_contextually_typed_binary_style() ? "" : "COLLATE ",
...
*/
bool Lex_exact_charset_opt_extended_collate::merge_context_collation_override(const Lex_context_collation &cl)
{
  DBUG_ASSERT(m_ci);
  ...
  CHARSET_INFO *ci = Charset_loader_server().get_exact_collation_by_context_name_or_error(
      m_ci, cl.charset_info()->coll_name.str + 8, MYF(0));
  if (!ci)
    return true;
  m_ci = ci;
  m_with_collate = true;
...
bool Lex_extended_collation_st::merge_exact_collation(const Lex_exact_collation &rhs)
{
  switch (m_type)
  {
    case TYPE_EXACT:
    ...
    case TYPE_CONTEXTUALLY_TYPED:
    {
      ...
      */
      if (rhs.raise_if_conflicts_with_context_collation(Lex_context_collation(m_ci), true))
        return true;
...
bool Lex_extended_collation_st::raise_if_conflicts_with_context_collation(const Lex_context_collation &rhs) const
{
  switch (m_type)
  {
    case TYPE_EXACT:
      ...
      */
      return Lex_exact_collation(m_ci).raise_if_conflicts_with_context_collation(rhs, false);
...
bool Lex_extended_collation_st::merge(const Lex_extended_collation_st &rhs)
{
  switch (rhs.type())
  {
    ...
    case TYPE_CONTEXTUALLY_TYPED:
      ...
      */
      return raise_if_conflicts_with_context_collation(Lex_context_collation(rhs.m_ci));
  }
  DBUG_ASSERT(0);
  return false;
}
...
CHARSET_INFO *Lex_exact_charset_opt_extended_collate::find_default_collation() const
{
  ...
  */
  ...
  /*
    ...
    See comments in find_bin_collation.
  */
  CHARSET_INFO *cs = get_charset_by_csname(m_ci->cs_name.str, MY_CS_PRIMARY, MYF(MY_WME));
  /*
...
*/
CHARSET_INFO *Lex_exact_charset_extended_collation_attrs_st::resolved_to_character_set(CHARSET_INFO *def) const
{
  DBUG_ASSERT(def);
...
bool Lex_exact_charset_extended_collation_attrs_st::merge_exact_collation(const Lex_exact_collation &cl)
{
  switch (m_type)
  {
    case TYPE_EMPTY:
    ...
    {
      // COLLATE DEFAULT .. COLLATE latin1_swedish_ci
      if (cl.raise_if_conflicts_with_context_collation(Lex_context_collation(m_ci), true))
        return true;
      *this = Lex_exact_charset_extended_collation_attrs(cl);
      return false;
    }
...
bool Lex_exact_charset_extended_collation_attrs_st::merge_context_collation(const Lex_context_collation &cl)
{
  switch (m_type)
  {
    case TYPE_EMPTY:
    ...
    case TYPE_COLLATE_EXACT:
      ...
      return Lex_exact_collation(m_ci).raise_if_conflicts_with_context_collation(cl, false);
    case TYPE_COLLATE_CONTEXTUALLY_TYPED:
      // COLLATE DEFAULT .. COLLATE DEFAULT
...
bool Lex_exact_charset_opt_extended_collate::merge_exact_collation(const Lex_exact_collation &cl)
{
  // CHARACTER SET latin1 [COLLATE latin1_bin] .. COLLATE latin1_bin
  if (m_with_collate)
    return Lex_exact_collation(m_ci).raise_if_not_equal(cl);
...
bool Lex_exact_charset_opt_extended_collate::merge_exact_collation_override(const Lex_exact_collation &cl)
{
  // CHARACTER SET latin1 [COLLATE latin1_bin] .. COLLATE latin1_bin
...
bool Lex_exact_charset_opt_extended_collate::merge_context_collation(const Lex_context_collation &cl)
{
...
bool Lex_exact_charset_extended_collation_attrs_st::merge_collation(const Lex_extended_collation_st &cl)
{
  switch (cl.type())
  {
...
*/
bool Lex_exact_charset_extended_collation_attrs_st::merge_exact_charset(const Lex_exact_charset &cs)
{
  ...
  switch (m_type)
  {
    ...
    case TYPE_CHARACTER_SET_COLLATE_EXACT:
      ...
      return Lex_exact_charset_opt_extended_collate(m_ci, true).raise_if_charsets_differ(cs);
...
bool Lex_extended_charset_extended_collation_attrs_st::merge_exact_charset(const Lex_exact_charset &cs)
{
  if (m_charset_order == CHARSET_TYPE_EMPTY)
...
bool Lex_extended_charset_extended_collation_attrs_st::raise_if_charset_conflicts_with_default(
    const Lex_exact_charset_opt_extended_collate &def) const
{
  ...
  {
    raise_ER_CONFLICTING_DECLARATIONS("CHARACTER SET ", "DEFAULT", def.collation().charset_info()->cs_name.str,
                                      "CHARACTER SET ", m_ci->cs_name.str, m_charset_order == CHARSET_TYPE_EXACT);
    return true;
  }
  return false;
}

CHARSET_INFO *Lex_extended_charset_extended_collation_attrs_st::resolved_to_context(
    const Charset_collation_context &ctx) const
{
  if (Lex_opt_context_charset_st::is_empty())
  {
    // Without CHARACTER SET DEFAULT
    return Lex_exact_charset_extended_collation_attrs_st::resolved_to_character_set(
  ...
  switch (type())
  {
    ...
    case TYPE_COLLATE_EXACT:
    {
      /*
        CREATE DATABASE db1
          ...
          CHARACTER SET DEFAULT;
      */
      if (m_type == TYPE_CHARACTER_SET_COLLATE_EXACT && raise_if_charset_conflicts_with_default(ctx.charset_default()))
      {
        /*
          A possible scenario:
            SET character_set_server=utf8mb4;
      ...
      /*
        ...
        CHARACTER SET DEFAULT.
      */
      if (ctx.charset_default().raise_if_not_applicable(Lex_exact_collation(m_ci)))
      {
        /*
          A possible scenario:
            SET character_set_server=utf8mb4;
    ...
    case TYPE_COLLATE_CONTEXTUALLY_TYPED:
      ...
      */
      return Lex_exact_charset_extended_collation_attrs_st::resolved_to_character_set(
          ctx.charset_default().collation().charset_info());
  }
  DBUG_ASSERT(0);
  return NULL;
}
