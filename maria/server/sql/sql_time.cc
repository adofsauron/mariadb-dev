...
LEX_CSTRING interval_type_to_name[INTERVAL_LAST] = {{STRING_WITH_LEN("YEAR")},
...
int append_interval(String *str, interval_type int_type, const INTERVAL &interval)
{
  ...
  switch (int_type)
  {
    ...
    case INTERVAL_DAY_SECOND:
      len = my_snprintf(buf, sizeof(buf), "%u %u:%02u:%02u", interval.day, interval.hour, interval.minute,
                        interval.second);
      break;
    ...
    case INTERVAL_DAY_MICROSECOND:
      len = my_snprintf(buf, sizeof(buf), "%u %u:%02u:%02u.%06u", interval.day, interval.hour, interval.minute,
                        interval.second, interval.second_part);
      break;
    case INTERVAL_HOUR_MICROSECOND:
      len = my_snprintf(buf, sizeof(buf), "%u:%02u:%02u.%06u", interval.hour, interval.minute, interval.second,
  ...
  return str->append(buf, len) || str->append(' ') || str->append(interval_type_to_name + int_type);
}
...
bool get_date_from_daynr(long daynr, uint *ret_year, uint *ret_month, uint *ret_day)
{
...
bool check_date_with_warn(THD *thd, const MYSQL_TIME *ltime, date_conv_mode_t fuzzydate, timestamp_type ts_type)
{
  int unused;
  if (check_date(ltime, fuzzydate, &unused))
  {
    ErrConvTime str(ltime);
    make_truncated_value_warning(thd, Sql_condition::WARN_LEVEL_WARN, &str, ts_type, nullptr, nullptr, nullptr);
    return true;
  }
  return false;
}
...
/*
  ...
  respected scripts.
*/
static uint to_ascii(CHARSET_INFO *cs, const char *src, size_t src_length, char *dst, size_t dst_length)

{
  ...
  while (dst < dstend && (cnvres = cs->mb_wc(&wc, (const uchar *)src, (const uchar *)srcend)) > 0 && wc < 128)
  {
    src += cnvres;
...
bool Temporal::str_to_datetime_or_date_or_time(THD *thd, MYSQL_TIME_STATUS *st, const char *str, size_t length,
...
bool Temporal::str_to_datetime_or_date(THD *thd, MYSQL_TIME_STATUS *status, const char *str, size_t length,
                                       CHARSET_INFO *cs, date_mode_t flags)
{
  TemporalAsciiBuffer tmp(str, length, cs);
...
bool Temporal::str_to_temporal(THD *thd, MYSQL_TIME_STATUS *status, const char *str, size_t length, CHARSET_INFO *cs,
                               date_mode_t flags)
{
  TemporalAsciiBuffer tmp(str, length, cs);
...
bool Interval_DDhhmmssff::str_to_DDhhmmssff(MYSQL_TIME_STATUS *status, const char *str, size_t length, CHARSET_INFO *cs,
                                            ulong max_hour)
{
  TemporalAsciiBuffer tmp(str, length, cs);
...
bool str_to_datetime_with_warn(THD *thd, CHARSET_INFO *cs, const char *str, size_t length, MYSQL_TIME *to,
                               date_mode_t mode)
{
...
bool double_to_datetime_with_warn(THD *thd, double value, MYSQL_TIME *ltime, date_mode_t fuzzydate,
                                  const TABLE_SHARE *s, const char *field_name)
{
  Temporal::Warn_push warn(thd, s ? s->db.str : nullptr, s ? s->table_name.str : nullptr, field_name, ltime, fuzzydate);
  Temporal_hybrid *t = new (ltime) Temporal_hybrid(thd, &warn, value, fuzzydate);
  return !t->is_valid_temporal();
}

bool decimal_to_datetime_with_warn(THD *thd, const my_decimal *value, MYSQL_TIME *ltime, date_mode_t fuzzydate,
                                   const TABLE_SHARE *s, const char *field_name)
{
  Temporal::Warn_push warn(thd, s ? s->db.str : nullptr, s ? s->table_name.str : nullptr, field_name, ltime, fuzzydate);
  Temporal_hybrid *t = new (ltime) Temporal_hybrid(thd, &warn, value, fuzzydate);
...
bool int_to_datetime_with_warn(THD *thd, const Longlong_hybrid &nr, MYSQL_TIME *ltime, date_mode_t fuzzydate,
                               const TABLE_SHARE *s, const char *field_name)
{
  /*
  ...
  */
  ...
  switch (format_type)
  {
    ...
    case MYSQL_TIMESTAMP_TIME:
      ...
      */
      if (format_length == 6 && !need_p && !my_charset_bin.strnncoll(format, 6, format_str, 6))
        return 0;
    ...
    case MYSQL_TIMESTAMP_DATETIME:
      ...
      */
      if ((format_length == 12 && !need_p &&
           !my_charset_bin.strnncoll(format, 12, known_date_time_formats[INTERNAL_FORMAT].datetime_format, 12)) ||
...
DATE_TIME_FORMAT *date_time_format_copy(THD *thd, DATE_TIME_FORMAT *format)
{
  ...
  if (thd)
    ...
    new_format = (DATE_TIME_FORMAT *)my_malloc(key_memory_DATE_TIME_FORMAT, length, MYF(MY_WME));
  if (new_format)
  {
...
void make_truncated_value_warning(THD *thd, Sql_condition::enum_warning_level level, const ErrConv *sval,
                                  timestamp_type time_type, const char *db_name, const char *table_name,
                                  const char *field_name)
{
  const char *type_str = Temporal::type_name_by_timestamp_type(time_type);
  return thd->push_warning_wrong_or_truncated_value(level, time_type <= MYSQL_TIMESTAMP_ERROR, type_str, sval->ptr(),
                                                    db_name, table_name, field_name);
}

/* Daynumber from year 0 to 9999-12-31 */
#define COMBINE(X) \
  (((((X)->day * 24LL + (X)->hour) * 60LL + (X)->minute) * 60LL + (X)->second) * 1000000LL + (X)->second_part)
#define GET_PART(X, N) \
  X % N##LL;           \
  X /= N##LL

bool date_add_interval(THD *thd, MYSQL_TIME *ltime, interval_type int_type, const INTERVAL &interval, bool push_warn)
{
  long period, sign;
  ...
  switch (int_type)
  {
    ...
    case INTERVAL_DAY:
    {
      ...
      if (((ulonglong)interval.day + (ulonglong)interval.hour / 24 + (ulonglong)interval.minute / 24 / 60 +
      ...
      if (get_date_from_daynr((long)daynr, &ltime->year, &ltime->month, &ltime->day))
        goto invalid_date;
      break;
    }
    case INTERVAL_WEEK:
      ...
      /* Daynumber from year 0 to 9999-12-31 */
      if (get_date_from_daynr((long)period, &ltime->year, &ltime->month, &ltime->day))
        goto invalid_date;
      break;
    case INTERVAL_YEAR:
...
invalid_date:
  if (push_warn)
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_DATETIME_FUNCTION_OVERFLOW,
                        ...
                        ltime->time_type == MYSQL_TIMESTAMP_TIME ? "time" : "datetime");
  }
null_date:
  return 1;
}
...
bool calc_time_diff(const MYSQL_TIME *l_time1, const MYSQL_TIME *l_time2, int l_sign, ulonglong *seconds_out,
                    ulong *microseconds_out)
{
  long days;
  bool neg;
  ...
  */
  ...
  microseconds =
      ((longlong)days * SECONDS_IN_24H + (longlong)(l_time1->hour * 3600LL + l_time1->minute * 60L + l_time1->second) -
       l_sign * (longlong)(l_time2->hour * 3600LL + l_time2->minute * 60L + l_time2->second)) *
          1000000LL +
...
bool calc_time_diff(const MYSQL_TIME *l_time1, const MYSQL_TIME *l_time2, int l_sign, MYSQL_TIME *l_time3,
                    date_mode_t fuzzydate)
{
  ...
  */
  ...
  return ((fuzzydate & TIME_NO_ZERO_DATE) && (seconds == 0) && (microseconds == 0));
}
...
*/
static void mix_date_and_time_simple(MYSQL_TIME *ldate, const MYSQL_TIME *ltime)
{
  DBUG_ASSERT(ldate->time_type == MYSQL_TIMESTAMP_DATE || ldate->time_type == MYSQL_TIMESTAMP_DATETIME);
...
*/
static void mix_date_and_time_complex(MYSQL_TIME *ldate, const MYSQL_TIME *ltime)
{
  DBUG_ASSERT(ldate->time_type == MYSQL_TIMESTAMP_DATE || ldate->time_type == MYSQL_TIMESTAMP_DATETIME);
  ulonglong seconds;
  ulong days, useconds;
...
*/
static void mix_date_and_time(MYSQL_TIME *to, const MYSQL_TIME *from)
{
...
*/
static bool time_to_datetime_old(THD *thd, const MYSQL_TIME *from, MYSQL_TIME *to)
{
...
*/
bool time_to_datetime(THD *thd, const MYSQL_TIME *from, MYSQL_TIME *to)
{
...
bool time_to_datetime_with_warn(THD *thd, const MYSQL_TIME *from, MYSQL_TIME *to, date_conv_mode_t fuzzydate)
{
  int warn = 0;
  DBUG_ASSERT(from->time_type == MYSQL_TIMESTAMP_TIME);
  /*
  ...
  */
  if (time_to_datetime(thd, from, to) ||
      ((thd->variables.old_behavior & OLD_MODE_ZERO_DATE_TIME_CAST) && check_date(to, fuzzydate, &warn)))
  {
...
{
  return ((((((my_time->year * 13ULL + my_time->month) * 32ULL + my_time->day) * 24ULL + my_time->hour) * 60ULL +
            my_time->minute) *
               60ULL +
           my_time->second) *
              1000000ULL +
...
void unpack_time(longlong packed, MYSQL_TIME *my_time, enum_mysql_timestamp_type ts_type)
{
