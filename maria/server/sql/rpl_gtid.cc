...
const LEX_CSTRING rpl_gtid_slave_state_table_name = {STRING_WITH_LEN("gtid_slave_pos")};

void rpl_slave_state::update_state_hash(uint64 sub_id, rpl_gtid *gtid, void *hton, rpl_group_info *rgi)
{
  int err;
  /*
...
int rpl_slave_state::record_and_update_gtid(THD *thd, rpl_group_info *rgi)
{
...
*/
int rpl_slave_state::check_duplicate_gtid(rpl_gtid *gtid, rpl_group_info *rgi)
{
  ...
  */
  ...
  for (;;)
  {
    ...
    */
    if (!did_enter_cond)
    {
      thd->ENTER_COND(&elem->COND_gtid_ignore_duplicates, &LOCK_slave_state, &stage_gtid_wait_other_connection,
                      &old_stage);
      did_enter_cond = true;
    }
    mysql_cond_wait(&elem->COND_gtid_ignore_duplicates, &LOCK_slave_state);
  }
...
void rpl_slave_state::release_domain_owner(rpl_group_info *rgi)
{
  element *elem = NULL;
...
static void rpl_slave_state_free_element(void *arg)
{
...
rpl_slave_state::rpl_slave_state() : pending_gtid_count(0), last_sub_id(0), gtid_pos_tables(0), loaded(false)
{
  mysql_mutex_init(key_LOCK_slave_state, &LOCK_slave_state, MY_MUTEX_INIT_SLOW);
  my_hash_init(PSI_INSTRUMENT_ME, &hash, &my_charset_bin, 32, offsetof(element, domain_id), sizeof(element::domain_id),
...
void rpl_slave_state::truncate_hash()
{
  uint32 i;
...
int rpl_slave_state::update(uint32 domain_id, uint32 server_id, uint64 sub_id, uint64 seq_no, void *hton,
                            rpl_group_info *rgi)
{
  element *elem = NULL;
  ...
  if (!(list_elem = (list_element *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*list_elem), MYF(MY_WME))))
    return 1;
...
struct rpl_slave_state::element *rpl_slave_state::get_element(uint32 domain_id)
{
  ...
  elem = (element *)my_hash_search(&hash, (const uchar *)&domain_id, sizeof(domain_id));
  if (elem)
    return elem;
  ...
  mysql_cond_init(key_COND_gtid_ignore_duplicates, &elem->COND_gtid_ignore_duplicates, 0);
...
int rpl_slave_state::put_back_list(list_element *list)
{
  element *e = NULL;
  int err = 0;
  ...
  while (list)
  {
    ...
    if ((!e || e->domain_id != list->domain_id) &&
        !(e = (element *)my_hash_search(&hash, (const uchar *)&list->domain_id, sizeof(list->domain_id))))
    {
      err = 1;
      goto end;
    }
...
int rpl_slave_state::truncate_state_table(THD *thd)
{
  TABLE_LIST tlist;
  int err = 0;
  ...
  if (!(err = open_and_lock_tables(thd, &tlist, FALSE, MYSQL_OPEN_IGNORE_LOGGING_FORMAT)))
  {
...
static const TABLE_FIELD_TYPE mysql_rpl_slave_state_coltypes[4] = {
    {{STRING_WITH_LEN("domain_id")}, {STRING_WITH_LEN("int(10) unsigned")}, {NULL, 0}},
    {{STRING_WITH_LEN("sub_id")}, {STRING_WITH_LEN("bigint(20) unsigned")}, {NULL, 0}},
    {{STRING_WITH_LEN("server_id")}, {STRING_WITH_LEN("int(10) unsigned")}, {NULL, 0}},
    {{STRING_WITH_LEN("seq_no")}, {STRING_WITH_LEN("bigint(20) unsigned")}, {NULL, 0}},
};

static const uint mysql_rpl_slave_state_pk_parts[] = {0, 1};

static const TABLE_FIELD_DEF mysql_gtid_slave_pos_tabledef = {
    array_elements(mysql_rpl_slave_state_coltypes), mysql_rpl_slave_state_coltypes,
    array_elements(mysql_rpl_slave_state_pk_parts), mysql_rpl_slave_state_pk_parts};
...
*/
int gtid_check_rpl_slave_state_table(TABLE *table)
{
  int err;

  if ((err = gtid_table_intact.check(table, &mysql_gtid_slave_pos_tabledef)))
    my_error(ER_GTID_OPEN_TABLE_FAILED, MYF(0), "mysql", rpl_gtid_slave_state_table_name.str);
  return err;
}
...
*/
void rpl_slave_state::select_gtid_pos_table(THD *thd, LEX_CSTRING *out_tablename)
{
  /*
  ...
  */
  *out_tablename = default_gtid_pos_table.load(std::memory_order_acquire)->table_name;
...
*/
int rpl_slave_state::record_gtid(THD *thd, const rpl_gtid *gtid, uint64 sub_id, bool in_transaction, bool in_statement,
                                 void **out_hton)
{
  TABLE_LIST tlist;
  ...
  */
  ...
  if (not_sql_thread)
  {
    LEX_CSTRING *tmp = thd->make_clex_string(gtid_pos_table_name.str, gtid_pos_table_name.length);
    mysql_mutex_unlock(&LOCK_slave_state);
    if (!tmp)
      DBUG_RETURN(1);
  ...
  */
  ...
  if (!in_transaction)
  {
    ...
    thd->variables.option_bits &=
        ~(ulonglong)(OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN | OPTION_BIN_LOG | OPTION_GTID_BEGIN);
  }
  else
  ...
  *out_hton = hton;
  ...
  if (opt_bin_log && (err = mysql_bin_log.bump_seq_no_counter_if_needed(gtid->domain_id, gtid->seq_no)))
  {
    my_error(ER_OUT_OF_RESOURCES, MYF(0));
    goto end;
  }
...
*/
rpl_slave_state::list_element *rpl_slave_state::gtid_grab_pending_delete_list()
{
  uint32 i;
...
LEX_CSTRING *rpl_slave_state::select_gtid_pos_table(void *hton)
{
  /*
    See comments on rpl_slave_state::gtid_pos_tables for rules around proper
    access to the list.
...
void rpl_slave_state::gtid_delete_pending(THD *thd, rpl_slave_state::list_element **list_ptr)
{
  int err = 0;
  ...
  */
  ...
  thd->variables.option_bits &= ~(ulonglong)(OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN | OPTION_BIN_LOG | OPTION_GTID_BEGIN);
  ...
  while (*list_ptr)
  {
    ...
    */
    ...
    gtid_pos_table_name = thd->make_clex_string(tmp_table_name->str, tmp_table_name->length);
    mysql_mutex_unlock(&LOCK_slave_state);
    ...
    do
    {
      ...
      if (cur->hton == hton)
      {
        ...
        else
        {
          ...
          res = table->file->ha_index_read_map(table->record[0], key_buffer, HA_WHOLE_KEY, HA_READ_KEY_EXACT);
        }
        ...
        {
          sql_print_error("Error deleting old GTID row: %s", thd->get_stmt_da()->message());
          /*
...
uint64 rpl_slave_state::next_sub_id(uint32 domain_id)
{
  uint64 sub_id = 0;
...
bool rpl_slave_state_tostring_helper(String *dest, const rpl_gtid *gtid, bool *first)
{
  if (*first)
    *first = false;
  else if (dest->append(','))
    return true;
  return dest->append_ulonglong(gtid->domain_id) || dest->append('-') || dest->append_ulonglong(gtid->server_id) ||
         dest->append('-') || dest->append_ulonglong(gtid->seq_no);
}
...
/*
  ...
  string.
*/
static bool rpl_slave_state_tostring_helper(DYNAMIC_ARRAY *gtid_dynarr, String *str)
{
...
static bool rpl_slave_state_tostring_helper(DYNAMIC_ARRAY *gtid_dynarr, int (*cb)(rpl_gtid *, void *), void *data)
{
  rpl_gtid *gtid;
  bool res = true;
...
int rpl_slave_state::iterate(int (*cb)(rpl_gtid *, void *), void *data, rpl_gtid *extra_gtids, uint32 num_extra,
                             bool sort)
{
  uint32 i;
  HASH gtid_hash;
  ...
  my_hash_init(PSI_INSTRUMENT_ME, &gtid_hash, &my_charset_bin, 32, offsetof(rpl_gtid, domain_id),
               sizeof(rpl_gtid::domain_id), NULL, NULL, HASH_UNIQUE);
  ...
  for (i = 0; i < hash.records; ++i)
  {
    ...
    rec = my_hash_search(&gtid_hash, (const uchar *)&best_gtid.domain_id, sizeof(best_gtid.domain_id));
    if (rec)
    {
      gtid = (rpl_gtid *)rec;
    ...
    if ((res = sort ? insert_dynamic(&gtid_sort_array, (const void *)&best_gtid) : (*cb)(&best_gtid, data)))
    {
      goto err;
    }
  }
  ...
  for (i = 0; i < gtid_hash.records; ++i)
  {
    ...
    if ((res = sort ? insert_dynamic(&gtid_sort_array, (const void *)gtid) : (*cb)(gtid, data)))
    {
      goto err;
    }
  }
...
static int rpl_slave_state_tostring_cb(rpl_gtid *gtid, void *data)
{
...
*/
int rpl_slave_state::tostring(String *dest, rpl_gtid *extra_gtids, uint32 num_extra)
{
  struct rpl_slave_state_tostring_data data;
  data.first = true;
  data.dest = dest;

  return iterate(rpl_slave_state_tostring_cb, &data, extra_gtids, num_extra, true);
}
...
/*
  ...
  GTID.
*/
bool rpl_slave_state::domain_to_gtid(uint32 domain_id, rpl_gtid *out_gtid)
{
  element *elem;
  list_element *list;
  uint64 best_sub_id;
...
*/
static int gtid_parser_helper(const char **ptr, const char *end, rpl_gtid *out_gtid)
{
  char *q;
  const char *p = *ptr;
  uint64 v1, v2, v3;
...
rpl_gtid *gtid_parse_string_to_list(const char *str, size_t str_len, uint32 *out_len)
{
  ...
  for (;;)
  {
    ...
    if ((!list || len >= alloc_len) &&
        !(list = (rpl_gtid *)my_realloc(PSI_INSTRUMENT_ME, list, (alloc_len = alloc_len * 2) * sizeof(rpl_gtid),
...
*/
int rpl_slave_state::load(THD *thd, const char *state_from_master, size_t len, bool reset, bool in_statement)
{
  ...
  for (;;)
  {
    ...
    if (gtid_parser_helper(&state_from_master, end, &gtid) || !(sub_id = next_sub_id(gtid.domain_id)) ||
...
void rpl_slave_state::free_gtid_pos_tables(struct rpl_slave_state::gtid_pos_table *list)
{
...
*/
void rpl_slave_state::set_gtid_pos_tables_list(rpl_slave_state::gtid_pos_table *new_list,
...
void rpl_slave_state::add_gtid_pos_table(rpl_slave_state::gtid_pos_table *entry)
{
  mysql_mutex_assert_owner(&LOCK_slave_state);
...
struct rpl_slave_state::gtid_pos_table *rpl_slave_state::alloc_gtid_pos_table(
    LEX_CSTRING *table_name, void *hton, rpl_slave_state::gtid_pos_table_state state)
{
  struct gtid_pos_table *p;
  char *allocated_str;

  if (!my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &p, sizeof(*p), &allocated_str, table_name->length + 1, NULL))
  {
...
void rpl_binlog_state::init()
{
  ...
  my_init_dynamic_array(PSI_INSTRUMENT_ME, &gtid_sort_array, sizeof(rpl_gtid), 8, 8, MYF(0));
  mysql_mutex_init(key_LOCK_binlog_state, &LOCK_binlog_state, MY_MUTEX_INIT_SLOW);
  initialized = 1;
}

void rpl_binlog_state::reset_nolock()
{
  uint32 i;

  for (i = 0; i < hash.records; ++i) my_hash_free(&((element *)my_hash_element(&hash, i))->hash);
  my_hash_reset(&hash);
}
...
bool rpl_binlog_state::load(struct rpl_gtid *list, uint32 count)
{
  uint32 i;
  bool res = false;
...
bool rpl_binlog_state::load(rpl_slave_state *slave_pos)
{
  bool res = false;
...
rpl_binlog_state::~rpl_binlog_state() { free(); }
...
*/
int rpl_binlog_state::update_nolock(const struct rpl_gtid *gtid, bool strict)
{
  element *elem;

  if ((elem = (element *)my_hash_search(&hash, (const uchar *)(&gtid->domain_id), sizeof(gtid->domain_id))))
  {
    if (strict && elem->last_gtid && elem->last_gtid->seq_no >= gtid->seq_no)
    {
      my_error(ER_GTID_STRICT_OUT_OF_ORDER, MYF(0), gtid->domain_id, gtid->server_id, gtid->seq_no,
               elem->last_gtid->domain_id, elem->last_gtid->server_id, elem->last_gtid->seq_no);
      return 1;
    }
...
int rpl_binlog_state::update(const struct rpl_gtid *gtid, bool strict)
{
  int res;
  mysql_mutex_lock(&LOCK_binlog_state);
...
/*
  ...
  accordingly.
*/
int rpl_binlog_state::update_with_next_gtid(uint32 domain_id, uint32 server_id, rpl_gtid *gtid)
{
  element *elem;
  int res = 0;
  ...
  mysql_mutex_lock(&LOCK_binlog_state);
  if ((elem = (element *)my_hash_search(&hash, (const uchar *)(&domain_id), sizeof(domain_id))))
  {
...
int rpl_binlog_state::element::update_element(const rpl_gtid *gtid)
{
  ...
  */
  ...
  lookup_gtid = (rpl_gtid *)my_hash_search(&hash, (const uchar *)&gtid->server_id, sizeof(gtid->server_id));
  if (lookup_gtid)
  {
  ...
  lookup_gtid = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*lookup_gtid), MYF(MY_WME));
  if (!lookup_gtid)
    return 1;
...
int rpl_binlog_state::alloc_element_nolock(const rpl_gtid *gtid)
{
  element *elem;
  rpl_gtid *lookup_gtid;
  ...
  if (elem && lookup_gtid)
  {
    ...
    my_hash_init(PSI_INSTRUMENT_ME, &elem->hash, &my_charset_bin, 32, offsetof(rpl_gtid, server_id),
...
*/
bool rpl_binlog_state::check_strict_sequence(uint32 domain_id, uint32 server_id, uint64 seq_no, bool no_error)
{
  element *elem;
  bool res = 0;

  mysql_mutex_lock(&LOCK_binlog_state);
  if ((elem = (element *)my_hash_search(&hash, (const uchar *)(&domain_id), sizeof(domain_id))) && elem->last_gtid &&
      elem->last_gtid->seq_no >= seq_no)
  {
    if (!no_error)
      my_error(ER_GTID_STRICT_OUT_OF_ORDER, MYF(0), domain_id, server_id, seq_no, elem->last_gtid->domain_id,
...
*/
int rpl_binlog_state::bump_seq_no_if_needed(uint32 domain_id, uint64 seq_no)
{
  element *elem;
  int res;
  ...
  elem->domain_id = domain_id;
  my_hash_init(PSI_INSTRUMENT_ME, &elem->hash, &my_charset_bin, 32, offsetof(rpl_gtid, server_id),
               sizeof(rpl_gtid::server_id), NULL, my_free, HASH_UNIQUE);
  elem->last_gtid = NULL;
  elem->seq_no_counter = seq_no;
  if (0 == my_hash_insert(&hash, (const uchar *)elem))
  {
...
/*
  ...
  within this domain.
*/
int rpl_binlog_state::write_to_iocache(IO_CACHE *dest)
{
  ulong i, j;
  char buf[21];
  int res = 0;
...
int rpl_binlog_state::read_from_iocache(IO_CACHE *src)
{
  ...
  for (;;)
  {
    ...
    if (gtid_parser_helper(&p, end, &gtid) || update_nolock(&gtid, false))
    {
      res = 1;
      break;
    }
...
rpl_gtid *rpl_binlog_state::find_nolock(uint32 domain_id, uint32 server_id)
{
  element *elem;
  if (!(elem = (element *)my_hash_search(&hash, (const uchar *)&domain_id, sizeof(domain_id))))
    return NULL;
  return (rpl_gtid *)my_hash_search(&elem->hash, (const uchar *)&server_id, sizeof(server_id));
}

rpl_gtid *rpl_binlog_state::find(uint32 domain_id, uint32 server_id)
{
  rpl_gtid *p;
  mysql_mutex_lock(&LOCK_binlog_state);
...
rpl_gtid *rpl_binlog_state::find_most_recent(uint32 domain_id)
{
  element *elem;
  rpl_gtid *gtid = NULL;
...
uint32 rpl_binlog_state::count()
{
  ...
  mysql_mutex_lock(&LOCK_binlog_state);
  for (i = 0; i < hash.records; ++i) c += ((element *)my_hash_element(&hash, i))->hash.records;
  mysql_mutex_unlock(&LOCK_binlog_state);

  return c;
}

int rpl_binlog_state::get_gtid_list(rpl_gtid *gtid_list, uint32 list_size)
{
  uint32 i, j, pos;
  int res = 0;
...
*/
int rpl_binlog_state::get_most_recent_gtid_list(rpl_gtid **list, uint32 *size)
{
  uint32 i;
  ...
  if (!(*list = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, alloc_size * sizeof(rpl_gtid), MYF(MY_WME))))
  {
    res = 1;
    goto end;
  }
...
bool rpl_binlog_state::append_pos(String *str)
{
  uint32 i;
  ...
  {
    element *e = (element *)my_hash_element(&hash, i);
    if (e->last_gtid && insert_dynamic(&gtid_sort_array, (const void *)e->last_gtid))
    {
      mysql_mutex_unlock(&LOCK_binlog_state);
      return true;
    }
...
bool rpl_binlog_state::append_state(String *str)
{
  uint32 i, j;
  bool res = false;
...
*/
const char *rpl_binlog_state::drop_domain(DYNAMIC_ARRAY *ids, Gtid_list_log_event *glev, char *errbuf)
{
  ...
  my_init_dynamic_array2(PSI_INSTRUMENT_ME, &domain_unique, sizeof(element *), domain_unique_buffer,
  ...
  */
  ...
  {
    rpl_gtid *rb_state_gtid = find_nolock(glev->list[l].domain_id, glev->list[l].server_id);
    if (!rb_state_gtid)
      sprintf(errbuf,
              ...
              glev->list[l].domain_id, glev->list[l].server_id, glev->list[l].domain_id);
      ...
      sprintf(errbuf,
              ...
              rb_state_gtid->domain_id, rb_state_gtid->server_id, rb_state_gtid->seq_no, glev->list[l].domain_id,
              glev->list[l].server_id, glev->list[l].seq_no);
    if (strlen(errbuf))  // use strlen() as cheap flag
      push_warning_printf(current_thd, Sql_condition::WARN_LEVEL_WARN, ER_BINLOG_CANT_DELETE_GTID_DOMAIN,
  ...
  */
  for (ulong i = 0; i < ids->elements; i++)
  {
    ...
    elem = (rpl_binlog_state::element *)my_hash_search(&hash, (const uchar *)ptr_domain_id, sizeof(ptr_domain_id[0]));
    if (!elem)
    {
      push_warning_printf(current_thd, Sql_condition::WARN_LEVEL_WARN, ER_BINLOG_CANT_DELETE_GTID_DOMAIN,
                          "The gtid domain being deleted ('%lu') is not in "
                          "the current binlog state",
                          (unsigned long)*ptr_domain_id);
      continue;
    }
    ...
    for (not_match = true, k = 0; k < elem->hash.records; k++)
    {
      ...
      for (ulong l = 0; l < glev->count && not_match; l++) not_match = !(*d_gtid == glev->list[l]);
    }
    ...
    {
      if ((rpl_binlog_state::element *)dynamic_array_ptr(&domain_unique, k) == elem)
  ...
  for (k = 0; k < domain_unique.elements; k++)
  {
    rpl_binlog_state::element *elem = *(rpl_binlog_state::element **)dynamic_array_ptr(&domain_unique, k);
    my_hash_free(&elem->hash);
...
{
  my_hash_init(PSI_INSTRUMENT_ME, &hash, &my_charset_bin, 32, offsetof(entry, gtid) + offsetof(rpl_gtid, domain_id),
               sizeof(rpl_gtid::domain_id), NULL, my_free, HASH_UNIQUE);
  my_init_dynamic_array(PSI_INSTRUMENT_ME, &gtid_sort_array, sizeof(rpl_gtid), 8, 8, MYF(0));
}
...
int slave_connection_state::load(const char *slave_request, size_t len)
{
  const char *p, *end;
  uchar *rec;
  rpl_gtid *gtid;
  ...
  for (;;)
  {
    ...
    if ((e = (const entry *)my_hash_search(&hash, (const uchar *)(&gtid->domain_id), sizeof(gtid->domain_id))))
    {
      my_error(ER_DUPLICATE_GTID_DOMAIN, MYF(0), gtid->domain_id, gtid->server_id, (ulonglong)gtid->seq_no,
               e->gtid.domain_id, e->gtid.server_id, (ulonglong)e->gtid.seq_no, gtid->domain_id);
      my_free(rec);
      return 1;
    }
    ((entry *)rec)->flags = 0;
...
static int slave_connection_state_load_cb(rpl_gtid *gtid, void *data)
{
...
/*
  ...
  instead.
*/
int slave_connection_state::load(rpl_slave_state *state, rpl_gtid *extra_gtids, uint32 num_extra)
{
  reset();
  return state->iterate(slave_connection_state_load_cb, this, extra_gtids, num_extra, false);
}

slave_connection_state::entry *slave_connection_state::find_entry(uint32 domain_id)
{
  return (entry *)my_hash_search(&hash, (const uchar *)(&domain_id), sizeof(domain_id));
}

rpl_gtid *slave_connection_state::find(uint32 domain_id)
{
...
{
  entry *e;
  uchar *rec = my_hash_search(&hash, (const uchar *)(&in_gtid->domain_id), sizeof(in_gtid->domain_id));
  if (rec)
  {
    e = (entry *)rec;
    e->gtid = *in_gtid;
...
int slave_connection_state::to_string(String *out_str)
{
  out_str->length(0);
  return append_to_string(out_str);
}

int slave_connection_state::append_to_string(String *out_str)
{
  uint32 i;
  bool first;
...
int slave_connection_state::get_gtid_list(rpl_gtid *gtid_list, uint32 list_size)
{
  uint32 i, pos;
...
/*
  ...
  --log-slave-updates=0).
*/
bool slave_connection_state::is_pos_reached()
{
  uint32 i;
...
*/
int gtid_waiting::wait_for_pos(THD *thd, String *gtid_str, longlong timeout_us)
{
  int err;
  rpl_gtid *wait_pos;
  uint32 count, i;
  ...
  if (!(wait_pos = gtid_parse_string_to_list(gtid_str->ptr(), gtid_str->length(), &count)))
  {
    my_error(ER_INCORRECT_GTID_STATE, MYF(0));
    return 1;
  }
  ...
  switch (err)
  {
    case -1:
      ...
      /* fall through */
    case 0:
      status_var_add(thd->status_var.master_gtid_wait_time, static_cast<ulong>(microsecond_interval_timer() - before));
  }
  my_free(wait_pos);
  return err;
}

void gtid_waiting::promote_new_waiter(gtid_waiting::hash_element *he)
{
  queue_element *qe;
...
void gtid_waiting::process_wait_hash(uint64 wakeup_seq_no, gtid_waiting::hash_element *he)
{
  mysql_mutex_assert_owner(&LOCK_gtid_waiting);
...
*/
int gtid_waiting::wait_for_gtid(THD *thd, rpl_gtid *wait_gtid, struct timespec *wait_until)
{
  ...
  */
  elem.do_small_wait = (queue_empty(&he->queue) || ((queue_element *)queue_top(&he->queue))->wait_seq_no > seq_no);
  ...
  */
  for (;;)
  {
    ...
    if (elem.do_small_wait)
    {
      ...
      */
      if (!slave_state_elem && !(slave_state_elem = rpl_global_gtid_slave_state->get_element(domain_id)))
      {
      ...
      else if ((cur_waiter = slave_state_elem->gtid_waiter) && slave_state_elem->min_wait_seq_no <= seq_no)
      {
        /*
      ...
      else
      {
        ...
        */
        ...
        if (did_enter_cond)
        {
          ...
          mysql_mutex_unlock(&LOCK_gtid_waiting);
        thd->ENTER_COND(&slave_state_elem->COND_wait_gtid, &rpl_global_gtid_slave_state->LOCK_slave_state,
        ...
        do
        {
          ...
          {
            int err = mysql_cond_timedwait(&slave_state_elem->COND_wait_gtid,
                                           &rpl_global_gtid_slave_state->LOCK_slave_state, wait_until);
            ...
            mysql_cond_wait(&slave_state_elem->COND_wait_gtid, &rpl_global_gtid_slave_state->LOCK_slave_state);
    ...
    else
    {
      ...
      if (!did_enter_cond)
      {
        thd->ENTER_COND(&thd->COND_wakeup_ready, &LOCK_gtid_waiting, &stage_master_gtid_wait, &old_stage);
        did_enter_cond = true;
      }
      while (!elem.done && likely(!thd->check_killed(1)))
      {
        ...
        if (wait_until)
        {
          int err = mysql_cond_timedwait(&thd->COND_wakeup_ready, &LOCK_gtid_waiting, wait_until);
          if (err == ETIMEDOUT || err == ETIME)
            timed_out = true;
        }
        else
...
static void free_hash_element(void *p)
{
...
void gtid_waiting::init()
{
  my_hash_init(PSI_INSTRUMENT_ME, &hash, &my_charset_bin, 32, offsetof(hash_element, domain_id),
               sizeof(hash_element::domain_id), NULL, free_hash_element, HASH_UNIQUE);
  mysql_mutex_init(key_LOCK_gtid_waiting, &LOCK_gtid_waiting, 0);
}

void gtid_waiting::destroy()
{
  mysql_mutex_destroy(&LOCK_gtid_waiting);
  my_hash_free(&hash);
}

static int cmp_queue_elem(void *, uchar *a, uchar *b)
{
...
gtid_waiting::hash_element *gtid_waiting::get_entry(uint32 domain_id)
{
  hash_element *e;

  if ((e = (hash_element *)my_hash_search(&hash, (const uchar *)&domain_id, sizeof(domain_id))))
    return e;

  if (!(e = (hash_element *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*e), MYF(MY_WME))))
    return NULL;

  if (init_queue(&e->queue, 8, offsetof(queue_element, wait_seq_no), 0, cmp_queue_elem, NULL,
                 1 + offsetof(queue_element, queue_idx), 1))
  {
    my_error(ER_OUT_OF_RESOURCES, MYF(0));
    my_free(e);
    return NULL;
...
int gtid_waiting::register_in_wait_queue(THD *thd, rpl_gtid *wait_gtid, gtid_waiting::hash_element *he,
...
void gtid_waiting::remove_from_wait_queue(gtid_waiting::hash_element *he, gtid_waiting::queue_element *elem)
{
  mysql_mutex_assert_owner(&LOCK_gtid_waiting);
...
{
  struct Binlog_gtid_state_validator::audit_elem *audit_elem = (struct Binlog_gtid_state_validator::audit_elem *)p;
...
Binlog_gtid_state_validator::Binlog_gtid_state_validator()
{
  my_hash_init(PSI_INSTRUMENT_ME, &m_audit_elem_domain_lookup, &my_charset_bin, 32,
               offsetof(struct audit_elem, domain_id), sizeof(uint32), NULL, free_domain_lookup_element, HASH_UNIQUE);
}

Binlog_gtid_state_validator::~Binlog_gtid_state_validator() { my_hash_free(&m_audit_elem_domain_lookup); }

void Binlog_gtid_state_validator::initialize_start_gtids(rpl_gtid *start_gtids, size_t n_gtids)
{
  size_t i;
  for (i = 0; i < n_gtids; i++)
  {
    ...
    */
    struct audit_elem *audit_elem = (struct audit_elem *)my_hash_search(
        &m_audit_elem_domain_lookup, (const uchar *)&(domain_state_gtid->domain_id), 0);
    if (audit_elem)
    {
      /*
    ...
    audit_elem = (struct audit_elem *)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(struct audit_elem), MYF(MY_WME));
    if (!audit_elem)
    {
      my_error(ER_OUT_OF_RESOURCES, MYF(0));
      return;
    ...
    my_init_dynamic_array(PSI_INSTRUMENT_ME, &audit_elem->late_gtids_real, sizeof(rpl_gtid), 8, 8, MYF(0));
    my_init_dynamic_array(PSI_INSTRUMENT_ME, &audit_elem->late_gtids_previous, sizeof(rpl_gtid), 8, 8, MYF(0));
...
my_bool Binlog_gtid_state_validator::initialize_gtid_state(FILE *out, rpl_gtid *gtids, size_t n_gtids)
{
  size_t i;
  my_bool err = FALSE;
  ...
  */
  ...
  for (i = 0; i < n_gtids; i++)
  {
    ...
    if (!audit_elem)
    {
      Binlog_gtid_state_validator::error(out,
...
my_bool Binlog_gtid_state_validator::verify_stop_state(FILE *out, rpl_gtid *stop_gtids, size_t n_stop_gtids)
{
  size_t i;
  for (i = 0; i < n_stop_gtids; i++)
  {
    ...
    struct audit_elem *audit_elem =
        (struct audit_elem *)my_hash_search(&m_audit_elem_domain_lookup, (const uchar *)&(stop_gtid->domain_id), 0);
...
my_bool Binlog_gtid_state_validator::verify_gtid_state(FILE *out, rpl_gtid *domain_state_gtid)
{
  struct audit_elem *audit_elem = (struct audit_elem *)my_hash_search(
      &m_audit_elem_domain_lookup, (const uchar *)&(domain_state_gtid->domain_id), 0);
  ...
  if (!audit_elem)
  {
    Binlog_gtid_state_validator::warn(out,
...
{
  struct audit_elem *audit_elem =
      (struct audit_elem *)my_hash_search(&m_audit_elem_domain_lookup, (const uchar *)&(gtid->domain_id), 0);
  ...
  {
    /* Out of order check */
    if (gtid->seq_no <= audit_elem->last_gtid.seq_no && gtid->seq_no >= audit_elem->start_gtid.seq_no)
    {
      /* GTID is out of order */
      insert_dynamic(&audit_elem->late_gtids_real, (const void *)gtid);
      insert_dynamic(&audit_elem->late_gtids_previous, (const void *)&(audit_elem->last_gtid));
...
{
  struct Binlog_gtid_state_validator::audit_elem *audit_el = (struct Binlog_gtid_state_validator::audit_elem *)entry;

  struct gtid_report_ctx *report_ctx = (struct gtid_report_ctx *)report_ctx_arg;
  ...
  if (audit_el)
  {
    ...
    {
      report_f(out, "Binary logs never reached expected GTID state of %u-%u-%llu", PARAM_GTID(audit_el->start_gtid));
    ...
    {
      rpl_gtid *real_gtid = (rpl_gtid *)dynamic_array_ptr(&(audit_el->late_gtids_real), i);
      rpl_gtid *last_gtid = (rpl_gtid *)dynamic_array_ptr(&(audit_el->late_gtids_previous), i);
      DBUG_ASSERT(real_gtid && last_gtid);

      report_f(out, "Found out of order GTID. Got %u-%u-%llu after %u-%u-%llu", PARAM_GTID((*real_gtid)),
               PARAM_GTID((*last_gtid)));
      report_ctx->contains_err = TRUE;
    }
  }
...
Window_gtid_event_filter::Window_gtid_event_filter()
    : m_has_start(FALSE), m_has_stop(FALSE), m_is_active(FALSE), m_has_passed(FALSE)
{
...
static inline my_bool is_gtid_at_or_after(rpl_gtid *boundary, rpl_gtid *test_gtid)
{
  return test_gtid->domain_id == boundary->domain_id && test_gtid->seq_no >= boundary->seq_no;
}

static inline my_bool is_gtid_at_or_before(rpl_gtid *boundary, rpl_gtid *test_gtid)
{
  return test_gtid->domain_id == boundary->domain_id && test_gtid->seq_no <= boundary->seq_no;
}
    ...
    */
             ...
             (!m_has_stop || is_gtid_at_or_before(&m_stop, gtid)))
    {
      ...
      DBUG_PRINT("gtid-event-filter",
                 ("Window: Begin (%d-%d-%llu, %d-%d-%llu]", PARAM_GTID(m_start), PARAM_GTID(m_stop)));
      ...
      */
      ...
      if (m_has_stop && gtid->seq_no == m_stop.seq_no)
      {
        m_has_passed = TRUE;
        DBUG_PRINT("gtid-event-filter",
                   ("Window: End (%d-%d-%llu, %d-%d-%llu]", PARAM_GTID(m_start), PARAM_GTID(m_stop)));
      }
    }
  } /* if (!m_is_active && !m_has_passed) */
...
my_bool Window_gtid_event_filter::has_finished() { return m_has_stop ? m_has_passed : FALSE; }
...
Id_delegating_gtid_event_filter<T>::Id_delegating_gtid_event_filter()
    : m_num_stateful_filters(0), m_num_completed_filters(0), m_id_restriction_mode(id_restriction_mode::MODE_NOT_SET)
{
  void (*free_func)(void *);
  ...
  my_hash_init(PSI_INSTRUMENT_ME, &m_filters_by_id_hash, &my_charset_bin, 32,
               offsetof(gtid_filter_element<T>, identifier), sizeof(T), NULL, free_func, HASH_UNIQUE);
...
template <typename T>
void Id_delegating_gtid_event_filter<T>::set_default_filter(Gtid_event_filter *filter)
{
  if (m_default_filter)
    delete m_default_filter;
...
template <typename T>
gtid_filter_element<T> *Id_delegating_gtid_event_filter<T>::find_or_create_filter_element_for_id(T filter_id)
{
  gtid_filter_element<T> *fe =
      (gtid_filter_element<T> *)my_hash_search(&m_filters_by_id_hash, (const uchar *)&filter_id, 0);

  if (!fe)
  {
    gtid_filter_element<T> *new_fe =
        (gtid_filter_element<T> *)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(gtid_filter_element<T>), MYF(MY_WME));
    new_fe->filter = NULL;
...
my_bool Id_delegating_gtid_event_filter<T>::has_finished()
{
  /*
    ...
    deactivated
  */
  return m_num_stateful_filters && m_num_completed_filters == m_num_stateful_filters;
}
...
my_bool Id_delegating_gtid_event_filter<T>::exclude(rpl_gtid *gtid)
{
  ...
  gtid_filter_element<T> *filter_element =
      (gtid_filter_element<T> *)my_hash_search(&m_filters_by_id_hash, (const uchar *)&filter_id, 0);
  Gtid_event_filter *filter = (filter_element ? filter_element->filter : m_default_filter);
  my_bool ret = TRUE;
...
template <typename F>
Gtid_event_filter *create_event_filter()
{
  return new F();
}

template <typename T>
int Id_delegating_gtid_event_filter<T>::set_id_restrictions(T *id_list, size_t n_ids, id_restriction_mode mode)
{
  ...
  */
  if (mode == Gtid_event_filter::id_restriction_mode::WHITELIST_MODE)
  {
    ...
    construct_filter = create_event_filter<Accept_all_gtid_filter>;
    construct_default_filter = create_event_filter<Reject_all_gtid_filter>;
  }
  else
  {
    DBUG_ASSERT(mode == Gtid_event_filter::id_restriction_mode::BLACKLIST_MODE);
    filter_name = BLACKLIST_NAME;
    opposite_filter_name = WHITELIST_NAME;
    construct_filter = create_event_filter<Reject_all_gtid_filter>;
    construct_default_filter = create_event_filter<Accept_all_gtid_filter>;
  }

  if (m_id_restriction_mode != Gtid_event_filter::id_restriction_mode::MODE_NOT_SET)
  {
    if (mode != m_id_restriction_mode)
    {
      ...
      */
      sql_print_error(
          "Cannot create %s filtering rule for %s id because "
          "%s rule already exists",
          filter_name, get_id_type_name(), opposite_filter_name);
      err = 1;
      goto err;
    }
  ...
  for (id_ctr = 0; id_ctr < n_ids; id_ctr++)
  {
    ...
    gtid_filter_element<T> *map_element = find_or_create_filter_element_for_id(filter_id);
    ...
    else
    {
      DBUG_ASSERT(map_element->filter->get_filter_type() ==
                  (mode == Gtid_event_filter::id_restriction_mode::WHITELIST_MODE
...
Window_gtid_event_filter *Domain_gtid_event_filter::find_or_create_window_filter_for_id(
    decltype(rpl_gtid::domain_id) domain_id)
{
  gtid_filter_element<decltype(rpl_gtid::domain_id)> *filter_element = find_or_create_filter_element_for_id(domain_id);
  ...
  else
  {
    ...
    */
    sql_print_error(
        "cannot subset domain id %d by position, another rule "
...
{
  gtid_filter_element<decltype(rpl_gtid::domain_id)> *fe = (gtid_filter_element<decltype(rpl_gtid::domain_id)> *)entry;
...
{
  int err = 0;
  Window_gtid_event_filter *filter_to_update = find_or_create_window_filter_for_id(gtid->domain_id);
  ...
  else if (!(err = filter_to_update->set_start_gtid(gtid)))
  {
    gtid_filter_element<decltype(rpl_gtid::domain_id)> *fe =
        (gtid_filter_element<decltype(rpl_gtid::domain_id)> *)my_hash_search(&m_filters_by_id_hash,
...
rpl_gtid *Domain_gtid_event_filter::get_start_gtids()
{
  ...
  gtid_list = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, n_start_gtids * sizeof(rpl_gtid), MYF(MY_WME));

  for (i = 0; i < n_start_gtids; i++)
  {
    gtid_filter_element<decltype(rpl_gtid::domain_id)> *fe =
        *(gtid_filter_element<decltype(rpl_gtid::domain_id)> **)dynamic_array_ptr(&m_start_filters, i);
    DBUG_ASSERT(fe->filter && fe->filter->get_filter_type() == WINDOW_GTID_FILTER_TYPE);
...
rpl_gtid *Domain_gtid_event_filter::get_stop_gtids()
{
  ...
  gtid_list = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, n_stop_gtids * sizeof(rpl_gtid), MYF(MY_WME));

  for (i = 0; i < n_stop_gtids; i++)
  {
    gtid_filter_element<decltype(rpl_gtid::domain_id)> *fe =
        *(gtid_filter_element<decltype(rpl_gtid::domain_id)> **)dynamic_array_ptr(&m_stop_filters, i);
    DBUG_ASSERT(fe->filter && fe->filter->get_filter_type() == WINDOW_GTID_FILTER_TYPE);
    Window_gtid_event_filter *wgef = (Window_gtid_event_filter *)fe->filter;
...
my_bool Domain_gtid_event_filter::exclude(rpl_gtid *gtid)
{
  ...
  */
  ...
  return include_domain ? Id_delegating_gtid_event_filter::exclude(gtid) : TRUE;
}

Intersecting_gtid_event_filter::Intersecting_gtid_event_filter(Gtid_event_filter *filter1, Gtid_event_filter *filter2)
{
  my_init_dynamic_array(PSI_INSTRUMENT_ME, &m_filters, sizeof(Gtid_event_filter *), 3, 3, MYF(0));
