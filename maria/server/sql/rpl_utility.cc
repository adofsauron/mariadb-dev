...
uint32 table_def::calc_field_size(uint col, uchar *master_data) const
{
  ...
  switch (type(col))
  {
    case MYSQL_TYPE_NEWDECIMAL:
      length = my_decimal_get_binary_size(m_field_metadata[col] >> 8, m_field_metadata[col] & 0xff);
      break;
...
table_def::table_def(unsigned char *types, ulong size, uchar *field_metadata, int metadata_size, uchar *null_bitmap,
                     uint16 flags)
    : m_size(size),
      m_type(0),
      ...
      m_memory(NULL)
{
  m_memory = (uchar *)my_multi_malloc(key_memory_table_def_memory, MYF(MY_WME), &m_type, size, &m_field_metadata,
                                      size * sizeof(uint16), &m_null_bits, (size + 7) / 8, NULL);
...
bool event_checksum_test(uchar *event_buf, ulong event_len, enum enum_binlog_checksum_alg alg)
{
  ...
  if (alg != BINLOG_CHECKSUM_ALG_OFF && alg != BINLOG_CHECKSUM_ALG_UNDEF)
  {
    ...
    if (event_buf[EVENT_TYPE_OFFSET] == FORMAT_DESCRIPTION_EVENT)
    {
      ...
      int8 fd_alg = event_buf[event_len - BINLOG_CHECKSUM_LEN - BINLOG_CHECKSUM_ALG_DESC_LEN];
#endif
      /*
