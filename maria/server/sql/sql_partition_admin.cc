...
{
  if (thd->slave_thread && write_bin_log_with_if_exists(thd, true, false, true))
    return (true);
  my_ok(thd);
  return (false);
}
...
bool Sql_cmd_alter_table_exchange_partition::execute(THD *thd)
{
  ...
  */
  ...
  if (unlikely(check_access(thd, priv_needed, first_table->db.str, &first_table->grant.privilege,
                            &first_table->grant.m_internal, 0, 0)) ||
      unlikely(check_access(thd, priv_needed, first_table->next_local->db.str,
                            &first_table->next_local->grant.privilege, &first_table->next_local->grant.m_internal, 0,
                            0)))
    DBUG_RETURN(TRUE);

  if (unlikely(check_grant(thd, priv_needed, first_table, FALSE, UINT_MAX, FALSE)))
    DBUG_RETURN(TRUE);
...
*/
bool compare_table_with_partition(THD *thd, TABLE *table, TABLE *part_table, partition_element *part_elem, uint part_id)
{
                                       ...
                                       handlerton *ht)
{
  ...
  lex_string_set(&exchange_entry.handler_name, ha_resolve_storage_engine_name(ht));
  ...
  */
  ...
  if (unlikely(ddl_log_write_execute_entry(log_entry->entry_pos, &exec_log_entry)))
  ...
  */
  ...
  DBUG_EXECUTE_IF("exchange_partition_fail_3", my_error(ER_ERROR_ON_RENAME, MYF(0), name, tmp_name, 0);
  ...
  DBUG_EXECUTE_IF("exchange_partition_fail_5", my_error(ER_ERROR_ON_RENAME, MYF(0), from_name, name, 0);
                  error_set = TRUE; goto err_rename;);
  ...
  DBUG_EXECUTE_IF("exchange_partition_fail_7", my_error(ER_ERROR_ON_RENAME, MYF(0), tmp_name, from_name, 0);
                  error_set = TRUE; goto err_rename;);
                                                                ...
                                                                Alter_info *alter_info)
{
  ...
  */
  ...
  if (unlikely(open_tables(thd, &table_list, &table_counter, 0, &alter_prelocking_strategy)))
  {
    if (thd->lex->if_exists() && thd->get_stmt_da()->sql_errno() == ER_NO_SUCH_TABLE)
    {
      /*
      ...
      */
      thd->clear_error();
      if (thd->slave_thread && write_bin_log(thd, true, thd->query(), thd->query_length()))
        DBUG_RETURN(true);
      my_ok(thd);
      DBUG_RETURN(false);
    }
  ...
  {
    my_error(ER_WRONG_OBJECT, MYF(0), table_list->db.str, swap_table_list->table_name.str, "BASE TABLE");
    DBUG_RETURN(TRUE);
  }
  ...
  if (part_table->file->partition_ht()->flags & HTON_TABLE_MAY_NOT_EXIST_ON_SLAVE)
    force_if_exists = 1;
  ...
  memcpy(part_tabledef_version, part_table->s->tabledef_version.str, MY_UUID_SIZE);
  ddl_log.org_table_id.str = part_tabledef_version;
  ddl_log.org_table_id.length = MY_UUID_SIZE;
  memcpy(swap_tabledef_version, swap_table->s->tabledef_version.str, MY_UUID_SIZE);
  ...
  if (unlikely(table_list->table->part_info->set_named_partition_bitmap(partition_name, strlen(partition_name))))
    DBUG_RETURN(true);
  ...
  part_file_name_len = build_table_filename(part_file_name, sizeof(part_file_name), table_list->db.str,
                                            table_list->table_name.str, "", 0);
  build_table_filename(swap_file_name, sizeof(swap_file_name), swap_table_list->db.str, swap_table_list->table_name.str,
                       "", 0);
  /* create a unique temp name */
  my_snprintf(temp_name, sizeof(temp_name), "%s-exchange-%lx-%llx", tmp_file_prefix, current_pid, thd->thread_id);
  if (lower_case_table_names)
    my_casedn_str(files_charset_info, temp_name);
  build_table_filename(temp_file_name, sizeof(temp_file_name), table_list->next_local->db.str, temp_name, "",
                       FN_IS_TMP);

  if (unlikely(!(part_elem =
                     part_table->part_info->get_part_elem(partition_name, part_file_name + part_file_name_len,
                                                          sizeof(part_file_name) - part_file_name_len, &swap_part_id))))
  {
    DBUG_RETURN(TRUE);
  }
  ...
  if (unlikely(compare_table_with_partition(thd, swap_table, part_table, part_elem, swap_part_id)))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  if (unlikely(exchange_name_with_ddl_log(thd, swap_file_name, part_file_name, temp_file_name, table_hton)))
    goto err;
  ...
  */
  ...
  if (unlikely((error = write_bin_log(thd, TRUE, thd->query(), thd->query_length()))))
  {
    /*
...
bool Sql_cmd_alter_table_truncate_partition::execute(THD *thd)
{
  ...
  /*
    ...
    by ha_partition.
  */
  thd->lex->alter_info.partition_flags |= (ALTER_PARTITION_ADMIN | ALTER_PARTITION_TRUNCATE);
...
#ifdef WITH_WSREP
  if (WSREP(thd) && (!thd->is_current_stmt_binlog_format_row() || !thd->find_temporary_table(first_table)) &&
      wsrep_to_isolation_begin(thd, first_table->db.str, first_table->table_name.str, NULL))
  {
  ...
  */
  ...
  for (i = 0; i < num_names; i++)
  {
    ...
    String *str_partition_name =
        new (thd->mem_root) String(partition_name, strlen(partition_name), system_charset_info);
    if (!str_partition_name)
      DBUG_RETURN(true);
    partition_names_list.push_back(str_partition_name, thd->mem_root);
  }
  if (first_table->table->part_info->set_partition_bitmaps(&partition_names_list))
    DBUG_RETURN(true);
  ...
  */
  ...
  if (unlikely(error = partition->truncate_partition(alter_info, &binlog_stmt)))
...
bool alter_partition_convert_in(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  if (create_partition_name(part_file_name, sizeof(part_file_name), path, partition_name, NORMAL_PART_NAME, false))
    return true;

  char from_file_name[FN_REFLEN + 1];

  build_table_filename(from_file_name, sizeof(from_file_name), table_from->db.str, table_from->table_name.str, "", 0);

  handler *file = get_new_handler(nullptr, thd->mem_root, table_from->table->file->ht);
  if (unlikely(!file))
    return true;

  close_all_tables_for_name(thd, table_from->table->s, HA_EXTRA_PREPARE_FOR_RENAME, nullptr);

  bool res = file->ha_rename_table(from_file_name, part_file_name);

  if (res)
    my_error(ER_ERROR_ON_RENAME, MYF(0), from_file_name, part_file_name, my_errno);
