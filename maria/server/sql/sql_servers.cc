...
/* update functions */
static void prepare_server_struct_for_update(LEX_SERVER_OPTIONS *server_options, FOREIGN_SERVER *existing,
                                             FOREIGN_SERVER *altered);
static int update_server(THD *thd, FOREIGN_SERVER *existing, FOREIGN_SERVER *altered);
static int update_server_record(TABLE *table, FOREIGN_SERVER *server);
static int update_server_record_in_cache(FOREIGN_SERVER *existing, FOREIGN_SERVER *altered);
/* utility functions */
static void merge_server_struct(FOREIGN_SERVER *from, FOREIGN_SERVER *to);

static uchar *servers_cache_get_key(FOREIGN_SERVER *server, size_t *length, my_bool not_used __attribute__((unused)))
{
  DBUG_ENTER("servers_cache_get_key");
  DBUG_PRINT("info", ("server_name_length %zd server_name %s", server->server_name_length, server->server_name));
...
static PSI_memory_info all_servers_cache_memory[] = {{&key_memory_servers, "servers_cache", PSI_FLAG_GLOBAL}};
...
static my_bool close_cached_connection_tables_callback(TDC_element *element, close_cached_connection_tables_arg *arg)
{
  TABLE_LIST *tmp;

  mysql_mutex_lock(&element->LOCK_table_share);
  /* Ignore if table is not open or does not have a connect_string */
  if (!element->share || !element->share->connect_string.length || !element->ref_count)
    goto end;
      ...
      (arg->connection->length > element->share->connect_string.length ||
       (arg->connection->length < element->share->connect_string.length &&
        (element->share->connect_string.str[arg->connection->length] != '/' &&
         element->share->connect_string.str[arg->connection->length] != '\\')) ||
       strncasecmp(arg->connection->str, element->share->connect_string.str, arg->connection->length)))
    goto end;
  ...
  if (!(tmp = (TABLE_LIST *)alloc_root(arg->thd->mem_root, sizeof(TABLE_LIST))) ||
      ...
      !(arg->thd->make_lex_string(&tmp->table_name, element->share->table_name.str, element->share->table_name.length)))
  {
  ...
  MDL_REQUEST_INIT(&tmp->mdl_request, MDL_key::TABLE, tmp->db.str, tmp->table_name.str, MDL_EXCLUSIVE, MDL_TRANSACTION);
  arg->tables = tmp;
...
static bool close_cached_connection_tables(THD *thd, LEX_CSTRING *connection)
{
  ...
  if (tdc_iterate(thd, (my_hash_walk_action)close_cached_connection_tables_callback, &argument))
    DBUG_RETURN(true);

  DBUG_RETURN(argument.tables ? close_cached_tables(thd, argument.tables, true, thd->variables.lock_wait_timeout)
                              : false);
}
...
bool servers_init(bool dont_read_servers_table)
{
  ...
  init_sql_alloc(key_memory_servers, &mem, ACL_ALLOC_BLOCK_SIZE, 0, MYF(MY_THREAD_SPECIFIC));
...
static bool servers_load(THD *thd, TABLE_LIST *tables)
{
  ...
  if (init_read_record(&read_record_info, thd, table = tables[0].table, NULL, NULL, 1, 0, FALSE))
    DBUG_RETURN(1);
  ...
  if (unlikely(open_and_lock_tables(thd, tables, FALSE, MYSQL_LOCK_IGNORE_TIMEOUT)))
  {
    ...
    */
    if (thd->get_stmt_da()->is_error())
      sql_print_error("Can't open and lock privilege tables: %s", thd->get_stmt_da()->message());
    return_val = FALSE;
    goto end;
  }
...
static bool get_server_from_table_to_cache(TABLE *table)
{
  ...
  FOREIGN_SERVER *server = (FOREIGN_SERVER *)alloc_root(&mem, sizeof(FOREIGN_SERVER));
...
static int insert_server(THD *thd, FOREIGN_SERVER *server)
{
  int error = -1;
  TABLE_LIST tables;
  TABLE *table;
...
static int insert_server_record_into_cache(FOREIGN_SERVER *server)
{
  ...
  */
  DBUG_PRINT("info",
             ("inserting server %s at %p, length %zd", server->server_name, server, server->server_name_length));
  if (my_hash_insert(&servers_cache, (uchar *)server))
  {
    DBUG_PRINT("info", ("had a problem inserting server %s at %p", server->server_name, server));
    // error handling needed here
...
static void store_server_fields(TABLE *table, FOREIGN_SERVER *server)
{
  ...
  */
  if (server->host)
    table->field[1]->store(server->host, (uint)strlen(server->host), system_charset_info);
  if (server->db)
    table->field[2]->store(server->db, (uint)strlen(server->db), system_charset_info);
  if (server->username)
    table->field[3]->store(server->username, (uint)strlen(server->username), system_charset_info);
  if (server->password)
    table->field[4]->store(server->password, (uint)strlen(server->password), system_charset_info);
  if (server->port > -1)
    table->field[5]->store(server->port);

  if (server->socket)
    table->field[6]->store(server->socket, (uint)strlen(server->socket), system_charset_info);
  if (server->scheme)
    table->field[7]->store(server->scheme, (uint)strlen(server->scheme), system_charset_info);
  if (server->owner)
    table->field[8]->store(server->owner, (uint)strlen(server->owner), system_charset_info);
}
...
static int insert_server_record(TABLE *table, FOREIGN_SERVER *server)
{
  ...
  table->field[0]->store(server->server_name, server->server_name_length, system_charset_info);

  /* read index until record is that specified in server_name */
  if (unlikely((error = table->file->ha_index_read_idx_map(table->record[0], 0, (uchar *)table->field[0]->ptr,
                                                           HA_WHOLE_KEY, HA_READ_KEY_EXACT))))
  {
    ...
    DBUG_PRINT("info", ("record for server '%s' not found!", server->server_name));
...
static int drop_server_internal(THD *thd, LEX_SERVER_OPTIONS *server_options)
{
  ...
  DBUG_PRINT("info", ("server name server->server_name %s", server_options->server_name.str));
  ...
  if (unlikely(!(table = open_ltable(thd, &tables, TL_WRITE, MYSQL_LOCK_IGNORE_TIMEOUT))))
  {
    error = my_errno;
    goto end;
  }
  ...
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_UNKNOWN_ERROR, "Server connection in use");
  }
...
static int delete_server_record_in_cache(LEX_SERVER_OPTIONS *server_options)
{
  ...
  DBUG_PRINT("info", ("trying to obtain server name %s length %zu", server_options->server_name.str,
                      server_options->server_name.length));

  if (!(server = (FOREIGN_SERVER *)my_hash_search(&servers_cache, (uchar *)server_options->server_name.str,
                                                  server_options->server_name.length)))
  {
    DBUG_PRINT("info", ("server_name %s length %zu not found!", server_options->server_name.str,
                        server_options->server_name.length));
    goto end;
  }
  /*
  ...
  */
  DBUG_PRINT("info", ("deleting server %s length %zd", server->server_name, server->server_name_length));
...
int update_server_record_in_cache(FOREIGN_SERVER *existing, FOREIGN_SERVER *altered)
{
  int error = 0;
  ...
  */
  ...
  {
    DBUG_PRINT("info", ("had a problem inserting server %s at %p", altered->server_name, altered));
...
int create_server(THD *thd, LEX_SERVER_OPTIONS *server_options)
{
  ...
  DBUG_PRINT("info", ("server_options->server_name %s", server_options->server_name.str));

  mysql_rwlock_wrlock(&THR_LOCK_servers);

  /* hit the memory first */
  if (my_hash_search(&servers_cache, (uchar *)server_options->server_name.str, server_options->server_name.length))
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_FOREIGN_SERVER_EXISTS,
                          ER_THD(thd, ER_FOREIGN_SERVER_EXISTS), server_options->server_name.str);
      error = 0;
      goto end;
    }
    else
  ...
  if (unlikely(error))
  {
    DBUG_PRINT("info", ("problem creating server <%s>", server_options->server_name.str));
...
int alter_server(THD *thd, LEX_SERVER_OPTIONS *server_options)
{
  ...
  if (!(existing = (FOREIGN_SERVER *)my_hash_search(&servers_cache, (uchar *)server_options->server_name.str,
                                                    server_options->server_name.length)))
    goto end;
...
#define SET_SERVER_OR_RETURN(X, DEFAULT)                                                                         \
  ...
  {                                                                                                              \
    if (!(server->X =                                                                                            \
              server_options->X.str ? strmake_root(&mem, server_options->X.str, server_options->X.length) : "")) \
      DBUG_RETURN(NULL);                                                                                         \
  } while (0)
  ...
  if (!strcasecmp(server->scheme, "mysql"))
  {
    ...
    {
      my_error(ER_CANT_CREATE_FEDERATED_TABLE, MYF(0), "either HOST or SOCKET must be set");
      DBUG_RETURN(NULL);
    }
  }
  ...
  server->port = server_options->port > -1 ? server_options->port : default_port;
                                             ...
                                             FOREIGN_SERVER *altered)
{
  ...
  DBUG_PRINT("info", ("existing name %s altered name %s", existing->server_name, altered->server_name));
...
#define SET_ALTERED(X)                                                                     \
  do                                                                                       \
  {                                                                                        \
    altered->X = (server_options->X.str && strcmp(server_options->X.str, existing->X))     \
  ...
  */
  altered->port = (server_options->port > -1 && server_options->port != existing->port) ? server_options->port : -1;
...
static FOREIGN_SERVER *clone_server(MEM_ROOT *mem, const FOREIGN_SERVER *server, FOREIGN_SERVER *buffer)
{
  ...
  buffer->server_name = strmake_root(mem, server->server_name, server->server_name_length);
...
FOREIGN_SERVER *get_server_by_name(MEM_ROOT *mem, const char *server_name, FOREIGN_SERVER *buff)
{
  ...
  {
    DBUG_PRINT("info", ("server_name %s length %u not found!", server_name, (unsigned)server_name_length));
