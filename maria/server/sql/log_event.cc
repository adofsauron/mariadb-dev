...
unsigned int binlog_checksum_type_length[] = {sizeof("NONE") - 1, sizeof("CRC32") - 1, 0};

TYPELIB binlog_checksum_typelib = {array_elements(binlog_checksum_type_names) - 1, "", binlog_checksum_type_names,
...
class Write_on_release_cache
{
 public:
   ...
   */
  IO_CACHE *operator&() { return m_cache; }
...
#define DBUG_DUMP_EVENT_BUF(B, L)                                                                                  \
  ...
  {                                                                                                                \
    ...
    {                                                                                                              \
      DBUG_PRINT("data",                                                                                           \
                 ("header: timestamp:%u type:%u server_id:%u len:%u log_pos:%u flags:%u", uint4korr(_buf),         \
                  _buf[EVENT_TYPE_OFFSET], uint4korr(_buf + SERVER_ID_OFFSET), uint4korr(_buf + EVENT_LEN_OFFSET), \
                  uint4korr(_buf + LOG_POS_OFFSET), uint4korr(_buf + FLAGS_OFFSET)));                              \
      DBUG_DUMP("data", _buf + LOG_EVENT_MINIMAL_HEADER_LEN, _len - LOG_EVENT_MINIMAL_HEADER_LEN);                 \
    }                                                                                                              \
    else                                                                                                           \
...
static inline bool read_str(const uchar **buf, const uchar *buf_end, const char **str, uint8 *len)
{
...
uint32 binlog_get_compress_len(uint32 len)
{
  ...
  return ALIGN_SIZE((BINLOG_COMPRESSED_HEADER_LEN + BINLOG_COMPRESSED_ORIGINAL_LENGTH_MAX_BYTES) + compressBound(len) +
                    1);
}
                           ...
                           ulong *newlen)
{
  ...
  *is_malloc = false;
  ...
  int32 comp_len = (int32)(len - (tmp - src) - (contain_checksum ? BINLOG_CHECKSUM_LEN : 0));
...
/**
   ...
   return zero if successful, others otherwise.
*/
int binlog_buf_uncompress(const uchar *src, uchar *dst, uint32 len, uint32 *newlen)
{
  if ((src[0] & 0x80) == 0)
    return 1;
  ...
  switch (alg)
  {
    case 0:
      // zlib
      if (uncompress((Bytef *)dst, &buflen, (const Bytef *)src + 1 + lenlen, len - 1 - lenlen) != Z_OK)
        return 1;
      break;
    default:
      // TODO
...
const char *Log_event::get_type_str(Log_event_type type)
{
  switch (type)
  {
    ...
    case IGNORABLE_LOG_EVENT:
      return "Ignorable log event";
    case ROWS_QUERY_LOG_EVENT:
      return "MySQL Rows_query";
    case GTID_LOG_EVENT:
      return "MySQL Gtid";
    case ANONYMOUS_GTID_LOG_EVENT:
      return "MySQL Anonymous_Gtid";
    case PREVIOUS_GTIDS_LOG_EVENT:
      return "MySQL Previous_gtids";
    ...
    case WRITE_ROWS_COMPRESSED_EVENT_V1:
      return "Write_rows_compressed_v1";
    case UPDATE_ROWS_COMPRESSED_EVENT_V1:
      return "Update_rows_compressed_v1";
    case DELETE_ROWS_COMPRESSED_EVENT_V1:
      return "Delete_rows_compressed_v1";
...
Log_event::Log_event(const uchar *buf, const Format_description_log_event *description_event)
    : temp_buf(0), exec_time(0), cache_type(Log_event::EVENT_INVALID_CACHE), checksum_alg(BINLOG_CHECKSUM_ALG_UNDEF)
{
#ifndef MYSQL_CLIENT
  thd = 0;
#endif
  ...
  */
  if (description_event->binlog_version == 3 && (uchar)buf[EVENT_TYPE_OFFSET] < FORMAT_DESCRIPTION_EVENT && log_pos)
  {
    /*
...
int Log_event::read_log_event(IO_CACHE *file, String *packet, const Format_description_log_event *fdle,
                              enum enum_binlog_checksum_alg checksum_alg_arg)
{
  ...
  ulong max_allowed_packet =
      thd ? thd->slave_thread ? slave_max_allowed_packet : thd->variables.max_allowed_packet : ~(uint)0;
#endif
  ...
  if (my_b_read(file, (uchar *)buf, sizeof(buf)))
  {
    ...
    */
    ...
    DBUG_RETURN(file->error == 0 ? LOG_READ_EOF : file->error > 0 ? LOG_READ_TRUNC : LOG_READ_IO);
  }
    ...
    if (packet->append(file, data_len - LOG_EVENT_MINIMAL_HEADER_LEN))
    {
      /*
        ...
        3. Failed to allocate memory for packet, my_errno
           ...
           be uninitialized)
      */
      DBUG_RETURN(my_errno == ENOMEM ? LOG_READ_MEM : (file->error >= 0 ? LOG_READ_TRUNC : LOG_READ_IO));
    }
  }
  ...
  if (fdle->crypto_data.scheme)
  {
    ...
    */
    ...
    if (encryption_crypt(src + 4, data_len - 4, dst + 4, &dstlen, fdle->crypto_data.key, fdle->crypto_data.key_length,
                         iv, sizeof(iv), ENCRYPTION_FLAG_DECRYPT | ENCRYPTION_FLAG_NOPAD, ENCRYPTION_KEY_SYSTEM_DATA,
                         fdle->crypto_data.key_version))
    {
      my_free(newpkt);
    ...
    packet->reset(newpkt, data_len + ev_offset, data_len + ev_offset + 1, &my_charset_bin);
  }
  ...
  */
  if (data_len > LOG_EVENT_MINIMAL_HEADER_LEN)
  {
    ...
    DBUG_EXECUTE_IF(
        "corrupt_read_log_event2", uchar *debug_event_buf_c = (uchar *)packet->ptr() + ev_offset;
    ...
    if (event_checksum_test((uchar *)packet->ptr() + ev_offset, data_len, checksum_alg_arg))
      DBUG_RETURN(LOG_READ_CHECKSUM_FAILURE);
  }
  DBUG_RETURN(0);
}

Log_event *Log_event::read_log_event(IO_CACHE *file, const Format_description_log_event *fdle, my_bool crc_check)
{
  ...
  if ((res = read_log_event((uchar *)event.ptr(), event.length(), &error, fdle, crc_check)))
  ...
  if (unlikely(error))
  {
    ...
    if (event.length() >= OLD_HEADER_LEN)
      sql_print_error(
          ...
          " data_len: %lu, event_type: %u",
          error, (ulong)uint4korr(&event[EVENT_LEN_OFFSET]), (uint)(uchar)event[EVENT_TYPE_OFFSET]);
    else
...
Log_event *Log_event::read_log_event(const uchar *buf, uint event_len, const char **error,
 ...
 */
  ...
  */
  alg = (event_type != FORMAT_DESCRIPTION_EVENT) ? fdle->checksum_alg : get_checksum_alg(buf, event_len);
  // Emulate the corruption during reading an event
  DBUG_EXECUTE_IF(
      "corrupt_read_log_event_char", if (event_type != FORMAT_DESCRIPTION_EVENT) {
  ...
  if (event_type > fdle->number_of_event_types && event_type != FORMAT_DESCRIPTION_EVENT)
  {
    /*
    ...
    */
    DBUG_PRINT("error", ("event type %d found, but the current "
                         ...
                         "types",
                         event_type, fdle->number_of_event_types));
    ev = NULL;
  }
  else
  {
    ...
    */
    if (fdle->event_type_permutation)
    {
      ...
      DBUG_PRINT("info", ("converting event type %d to %d (%s)", event_type, new_event_type,
    ...
    */
    ...
    {
      ev = new Ignorable_log_event(buf, fdle, get_type_str((Log_event_type)event_type));
      goto exit;
    }
    switch (event_type)
    {
      ...
      case START_ENCRYPTION_EVENT:
        ...
        break;
      default:
        DBUG_PRINT("error", ("Unknown event code: %d", (uchar)buf[EVENT_TYPE_OFFSET]));
        ev = NULL;
        break;
    }
  }
exit:
  ...
  DBUG_PRINT("read_event", ("%s(type_code: %u; event_len: %u)", ev ? ev->get_type_str() : "<unknown>",
                            (uchar)buf[EVENT_TYPE_OFFSET], event_len));
  /*
...
*/
static int get_str_len_and_pointer(const Log_event::Byte **src, const char **dst, uint *len, const Log_event::Byte *end)
{
  if (*src >= end)
...
static void copy_str_and_move(const char **src, Log_event::Byte **dst, size_t len)
{
  memcpy(*dst, *src, len);
  *src = (const char *)*dst;
  (*dst) += len;
...
Query_log_event::Query_log_event(const uchar *buf, uint event_len,
                                 const Format_description_log_event *description_event, Log_event_type event_type)
      ...
      sa_seq_no(0)
{
  ...
  DBUG_PRINT("info", ("event_len: %u  common_header_len: %d  post_header_len: %d", event_len, common_header_len,
                      post_header_len));
  ...
  */
  ...
  if (tmp)
  {
    ...
    */
    ...
    {
      DBUG_PRINT("info", ("status_vars_len (%u) > data_len (%lu); query= 0", status_vars_len, data_len));
      query = 0;
      DBUG_VOID_RETURN;
    }
  ...
  */
  ...
  for (const Log_event::Byte *pos = start; pos < end;)
  {
    switch (*pos++)
    {
      ...
      case Q_CATALOG_NZ_CODE:
        DBUG_PRINT("info", ("case Q_CATALOG_NZ_CODE; pos:%p; end:%p", pos, end));
 ...
 code: %u), skipping the rest of them",
...
#if !defined(MYSQL_CLIENT)
  if (description_event->server_version_split.kind == Format_description_log_event::master_version_split::KIND_MYSQL)
  {
  ...
  if (!(start = data_buf =
            (Log_event::Byte *)my_malloc(PSI_INSTRUMENT_ME,
                                         catalog_len + 1 + time_zone_len + 1 + user.length + 1 + host.length + 1 +
                                             data_len + 1 + sizeof(size_t)  // for db_len
                                             + db_len + 1 + QUERY_CACHE_DB_LENGTH_SIZE + QUERY_CACHE_FLAGS_SIZE,
                                         MYF(MY_WME))))
#else
  if (!(start = data_buf = (Log_event::Byte *)my_malloc(
            PSI_INSTRUMENT_ME, catalog_len + 1 + time_zone_len + 1 + user.length + 1 + host.length + 1 + data_len + 1,
            MYF(MY_WME))))
#endif
    DBUG_VOID_RETURN;
  ...
  */
  ...
  if (data_len && (data_len < db_len || data_len < q_len || data_len != (db_len + q_len + 1)))
  {
    q_len = 0;
    query = NULL;
    DBUG_VOID_RETURN;
  ...
  uint32 max_length = uint32(event_len - ((end + db_len + 1) - (buf - common_header_len)));
  if (q_len != max_length || (event_len < uint((end + db_len + 1) - (buf - common_header_len))))
  {
    q_len = 0;
    query = NULL;
    DBUG_VOID_RETURN;
...
Query_compressed_log_event::Query_compressed_log_event(const uchar *buf, uint event_len,
    ...
    : Query_log_event(buf, event_len, description_event, event_type), query_buf(NULL)
{
  if (query)
  {
    ...
    query_buf = (Log_event::Byte *)my_malloc(PSI_INSTRUMENT_ME, ALIGN_SIZE(un_len + 1), MYF(MY_WME));
    if (query_buf && !binlog_buf_uncompress((uchar *)query, (uchar *)query_buf, q_len, &un_len))
    {
      query_buf[un_len] = 0;
...
*/
int Query_log_event::dummy_event(String *packet, ulong ev_offset, enum enum_binlog_checksum_alg checksum_alg)
{
  uchar *p = (uchar *)packet->ptr() + ev_offset;
  size_t data_len = packet->length() - ev_offset;
  uint16 flags;
  static const size_t min_user_var_event_len = LOG_EVENT_HEADER_LEN + UV_NAME_LEN_SIZE + 1 + UV_VAL_IS_NULL;  // 25
  static const size_t min_query_event_len = LOG_EVENT_HEADER_LEN + QUERY_HEADER_LEN + 1 + 1;                  // 34
  ...
  else
  {
    ...
    */
    static const char message[] = "# Dummy event replacing event type %u that slave cannot handle.";
...
/*
  ...
  with an old slave.
*/
int Query_log_event::begin_event(String *packet, ulong ev_offset, enum enum_binlog_checksum_alg checksum_alg)
{
  uchar *p = (uchar *)packet->ptr() + ev_offset;
  uchar *q = p + LOG_EVENT_HEADER_LEN;
  size_t data_len = packet->length() - ev_offset;
  ...
  */
  ...
  if (data_len != LOG_EVENT_HEADER_LEN + GTID_HEADER_LEN && data_len != LOG_EVENT_HEADER_LEN + GTID_HEADER_LEN + 2)
    return 1;
    ...
    : Start_log_event_v3(), event_type_permutation(0)
{
  ...
  switch (binlog_ver)
  {
    case 4: /* MySQL 5.0 */
      ...
      DBUG_EXECUTE_IF("pretend_version_50034_in_binlog", strmov(server_version, "5.0.34"););
      common_header_len = LOG_EVENT_HEADER_LEN;
      number_of_event_types = LOG_EVENT_TYPES;
      /* we'll catch my_malloc() error in is_valid() */
      post_header_len = (uint8 *)my_malloc(
          PSI_INSTRUMENT_ME, number_of_event_types * sizeof(uint8) + BINLOG_CHECKSUM_ALG_DESC_LEN, MYF(0));
      /*
      ...
      if (post_header_len)
      {
        ...
        */
        DBUG_EXECUTE_IF("old_row_based_repl_4_byte_map_id_master",
                        post_header_len[TABLE_MAP_EVENT - 1] = post_header_len[WRITE_ROWS_EVENT_V1 - 1] =
                            post_header_len[UPDATE_ROWS_EVENT_V1 - 1] = post_header_len[DELETE_ROWS_EVENT_V1 - 1] = 6;);
        ...
        memset(post_header_len + MYSQL_EVENTS_END - 1, 0, (MARIA_EVENTS_BEGIN - MYSQL_EVENTS_END) * sizeof(uint8));
        ...
        int i;
        for (i = 0; i < number_of_event_types; i++) DBUG_ASSERT(post_header_len[i] != 255);
      }
      break;
    ...
    case 3: /* 4.0.x x>=2 */
      ...
      */
      ...
      post_header_len = (uint8 *)my_malloc(PSI_INSTRUMENT_ME, number_of_event_types * sizeof(uint8), MYF(0));
      if (post_header_len)
      {
        post_header_len[START_EVENT_V3 - 1] = START_V3_HEADER_LEN;
...
Format_description_log_event::Format_description_log_event(const uchar *buf, uint event_len,
                                                           const Format_description_log_event *description_event)
      ...
      event_type_permutation(0)
{
  ...
  post_header_len = (uint8 *)my_memdup(PSI_INSTRUMENT_ME, buf + ST_COMMON_HEADER_LEN_OFFSET + 1,
...
Format_description_log_event::master_version_split::master_version_split(const char *version)
{
  const char *p;
...
void Format_description_log_event::calc_server_version_split()
{
  ...
  DBUG_PRINT("info", ("Format_description_log_event::server_version_split:"
                      " '%s' %d %d %d",
                      server_version, server_version_split[0], server_version_split[1], server_version_split[2]));
}

void Format_description_log_event::deduct_options_written_to_bin_log()
{
  options_written_to_bin_log = OPTION_AUTO_IS_NULL | OPTION_NOT_AUTOCOMMIT | OPTION_NO_FOREIGN_KEY_CHECKS |
                               OPTION_RELAXED_UNIQUE_CHECKS | OPTION_INSERT_HISTORY;
  if (!server_version_split.version_is_valid() || server_version_split.kind == master_version_split::KIND_MYSQL ||
  ...
  if (server_version_split[0] == 10)
  {
    ...
    if (server_version_split[1] < 10 && server_version_split[2] < v[server_version_split[1]])
      return;
  }
...
*/
bool Format_description_log_event::is_version_before_checksum(const master_version_split *version_split)
{
  return *version_split < (version_split->kind == master_version_split::KIND_MARIADB ? checksum_version_split_mariadb
                                                                                     : checksum_version_split_mysql);
}
...
enum enum_binlog_checksum_alg get_checksum_alg(const uchar *buf, ulong len)
{
  ...
  memcpy(version, buf + LOG_EVENT_MINIMAL_HEADER_LEN + ST_SERVER_VER_OFFSET, ST_SERVER_VER_LEN);
  ...
  DBUG_ASSERT(ret == BINLOG_CHECKSUM_ALG_OFF || ret == BINLOG_CHECKSUM_ALG_UNDEF || ret == BINLOG_CHECKSUM_ALG_CRC32);
  DBUG_RETURN(ret);
}

Start_encryption_log_event::Start_encryption_log_event(const uchar *buf, uint event_len,
                                                       const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
  if ((int)event_len == LOG_EVENT_MINIMAL_HEADER_LEN + Start_encryption_log_event::get_data_size())
  {
    buf += LOG_EVENT_MINIMAL_HEADER_LEN;
    crypto_scheme = *buf;
    key_version = uint4korr(buf + BINLOG_CRYPTO_SCHEME_LENGTH);
    memcpy(nonce, buf + BINLOG_CRYPTO_SCHEME_LENGTH + BINLOG_KEY_VERSION_LENGTH, BINLOG_NONCE_LENGTH);
  }
  else
...
Load_log_event::Load_log_event(const uchar *buf, uint event_len, const Format_description_log_event *description_event)
    : Log_event(buf, description_event),
      num_fields(0),
      fields(0),
      field_lens(0),
      ...
      is_concurrent(FALSE)
{
  ...
  */
  if (event_len)
    copy_log_event(
        buf, event_len,
        (((uchar)buf[EVENT_TYPE_OFFSET] == LOAD_EVENT) ? LOAD_HEADER_LEN + description_event->common_header_len
...
int Load_log_event::copy_log_event(const uchar *buf, ulong event_len, int body_offset,
                                   const Format_description_log_event *description_event)
{
  ...
  */
  if (!(field_lens = (uchar *)sql_ex.init(buf + body_offset, buf_end, buf[EVENT_TYPE_OFFSET] != LOAD_EVENT)))
    DBUG_RETURN(1);
    ...
    : Log_event(buf, description_event), new_log_ident(0), flags(DUP_NAME)
{
  ...
  new_log_ident = my_strndup(PSI_INSTRUMENT_ME, (char *)buf + ident_offset, (uint)ident_len, MYF(MY_WME));
...
Binlog_checkpoint_log_event::Binlog_checkpoint_log_event(const uchar *buf, uint event_len,
                                                         const Format_description_log_event *description_event)
    : Log_event(buf, description_event), binlog_file_name(0)
{
  uint8 header_size = description_event->common_header_len;
  uint8 post_header_len = description_event->post_header_len[BINLOG_CHECKPOINT_EVENT - 1];
  if (event_len < (uint)header_size + (uint)post_header_len || post_header_len < BINLOG_CHECKPOINT_HEADER_LEN)
    return;
  buf += header_size;
  ...
  if (event_len - (header_size + post_header_len) < binlog_file_len)
    return;
  binlog_file_name = my_strndup(PSI_INSTRUMENT_ME, (char *)buf + post_header_len, binlog_file_len, MYF(MY_WME));
  return;
}
...
Gtid_log_event::Gtid_log_event(const uchar *buf, uint event_len, const Format_description_log_event *description_event)
    : Log_event(buf, description_event), seq_no(0), commit_id(0), flags_extra(0), extra_engines(0)
{
  uint8 header_size = description_event->common_header_len;
  uint8 post_header_len = description_event->post_header_len[GTID_EVENT - 1];
  const uchar *buf_0 = buf;
  if (event_len < (uint)header_size + (uint)post_header_len || post_header_len < GTID_HEADER_LEN)
    return;
...
#endif
#endif
    DBUG_ASSERT(static_cast<uint>(buf - buf_0) == event_len || buf_0[event_len - 1] == 0);
}
    ...
    : Log_event(buf, description_event), count(0), list(0), sub_id_list(0)
{
  ...
  if (event_len < (uint)header_size + (uint)post_header_len || post_header_len < GTID_LIST_HEADER_LEN)
    return;
...
*/
bool Gtid_list_log_event::peek(const char *event_start, size_t event_len, enum enum_binlog_checksum_alg checksum_alg,
                               rpl_gtid **out_gtid_list, uint32 *out_list_len, const Format_description_log_event *fdev)
{
  const char *p;
  ...
  if (event_len < (uint32)fdev->common_header_len + GTID_LIST_HEADER_LEN + element_size * count)
    return true;
  if (!(gtid_list = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, sizeof(rpl_gtid) * count + (count == 0), MYF(MY_WME))))
    return true;
...
Intvar_log_event::Intvar_log_event(const uchar *buf, const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
  /* The Post-Header is empty. The Variable Data part begins immediately. */
  buf += description_event->common_header_len + description_event->post_header_len[INTVAR_EVENT - 1];
...
Rand_log_event::Rand_log_event(const uchar *buf, const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
  /* The Post-Header is empty. The Variable Data part begins immediately. */
  buf += description_event->common_header_len + description_event->post_header_len[RAND_EVENT - 1];
...
Xid_log_event::Xid_log_event(const uchar *buf, const Format_description_log_event *description_event)
    : Xid_apply_log_event(buf, description_event)
{
  /* The Post-Header is empty. The Variable Data part begins immediately. */
  buf += description_event->common_header_len + description_event->post_header_len[XID_EVENT - 1];
...
**************************************************************************/
XA_prepare_log_event::XA_prepare_log_event(const uchar *buf, const Format_description_log_event *description_event)
    : Xid_apply_log_event(buf, description_event)
{
  buf += description_event->common_header_len + description_event->post_header_len[XA_PREPARE_LOG_EVENT - 1];
  one_phase = *(bool *)buf;
  buf += 1;
...
User_var_log_event::User_var_log_event(const uchar *buf, uint event_len,
                                       const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
#ifndef MYSQL_CLIENT
      ,
...
#endif
{
  ...
  /* The Post-Header is empty. The Variable Data part begins immediately. */
  buf += description_event->common_header_len + description_event->post_header_len[USER_VAR_EVENT - 1];
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    {
      flags = (uint) * (buf + UV_VAL_IS_NULL + UV_VAL_TYPE_SIZE + UV_CHARSET_NUMBER_SIZE + UV_VAL_LEN_SIZE + val_len);
    }
  }
    ...
    : Load_log_event(buf, 0, description_event), fake_base(0), block(0), inited_from_old(0)
{
  ...
  if (!(event_buf = (uchar *)my_memdup(PSI_INSTRUMENT_ME, buf, len, MYF(MY_WME))) ||
...
Begin_load_query_log_event::Begin_load_query_log_event(const uchar *buf, uint len,
...
Execute_load_query_log_event::Execute_load_query_log_event(const uchar *buf, uint event_len,
                                                           const Format_description_log_event *desc_event)
...
const uchar *sql_ex_info::init(const uchar *buf, const uchar *buf_end, bool use_new_format)
{
...
Rows_log_event::Rows_log_event(const uchar *buf, uint event_len, const Format_description_log_event *description_event)
    : Log_event(buf, description_event),
      m_row_count(0),
#ifndef MYSQL_CLIENT
      m_table(NULL),
...
#endif
{
  ...
  if (post_header_len == ROWS_HEADER_LEN_V2)
  {
    ...
    */
    ...
    if (var_header_len < 2 || event_len < static_cast<unsigned int>(var_header_len + (post_start - buf)))
    {
      m_cols.bitmap = 0;
      DBUG_VOID_RETURN;
    }
    ...
    for (const uchar *pos = start; pos < end;)
    {
      switch (*pos++)
      {
        case RW_V_EXTRAINFO_TAG:
        {
          ...
          {
            m_extra_row_data = (uchar *)my_malloc(PSI_INSTRUMENT_ME, infoLen, MYF(MY_WME));
  ...
  uchar const *const var_start = (const uchar *)buf + common_header_len + post_header_len + var_header_len;
  ...
  DBUG_PRINT("info", ("m_table_id: %llu  m_flags: %d  m_width: %lu  data_size: %lu", m_table_id, m_flags, m_width,
                      (ulong)data_size));
...
void Rows_log_event::uncompress_buf()
{
  ...
  if (new_buf)
  {
    if (!binlog_buf_uncompress(m_rows_buf, new_buf, (uint32)(m_rows_cur - m_rows_buf), &un_len))
    {
      my_free(m_rows_buf);
      m_rows_buf = new_buf;
#if !defined(MYSQL_CLIENT) && defined(HAVE_REPLICATION)
...
int Rows_log_event::get_data_size()
{
  ...
  DBUG_EXECUTE_IF("old_row_based_repl_4_byte_map_id_master",
                  return (int)(6 + no_bytes_in_map(&m_cols) + (end - buf) +
                               (general_type_code == UPDATE_ROWS_EVENT ? no_bytes_in_map(&m_cols_ai) : 0) + m_rows_cur -
                               m_rows_buf););
  int data_size = 0;
  ...
  {
    data_size =
        ROWS_HEADER_LEN_V2 + (m_extra_row_data ? RW_V_TAG_LEN + m_extra_row_data[EXTRA_ROW_INFO_LEN_OFFSET] : 0);
  }
  else
  {
...
Annotate_rows_log_event::Annotate_rows_log_event(const uchar *buf, uint event_len,
...
int Annotate_rows_log_event::get_data_size() { return m_query_len; }

Log_event_type Annotate_rows_log_event::get_type_code() { return ANNOTATE_ROWS_EVENT; }

bool Annotate_rows_log_event::is_valid() const { return (m_query_txt != NULL && m_query_len != 0); }
      ...
      m_optional_metadata(NULL)
{
  ...
  DBUG_PRINT("info", ("m_dblen: %lu  off: %ld  m_tbllen: %lu  off: %ld  m_colcnt: %lu  off: %ld", (ulong)m_dblen,
                      (long)(ptr_dblen - vpart), (ulong)m_tbllen, (long)(ptr_tbllen - vpart), m_colcnt,
                      (long)(ptr_colcnt - vpart)));

  /* Allocate mem for all fields in one go. If fails, caught in is_valid() */
  m_memory = (uchar *)my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &m_dbnam, (uint)m_dblen + 1, &m_tblnam,
  ...
  if (m_memory)
  {
    ...
    if (m_field_metadata_size <= (m_colcnt * 2))
    {
      ...
      m_meta_memory = (uchar *)my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &m_null_bits, num_null_bytes,
                                               &m_field_metadata, m_field_metadata_size, NULL);
 ...
 */
static void parse_signedness(std::vector<bool> &vec, unsigned char *field, unsigned int length)
{
...
/**
   ...
   @param[in]  length  length of the field
 */
static void parse_default_charset(Table_map_log_event::Optional_metadata_fields::Default_charset &default_charset,
                                  unsigned char *field, unsigned int length)
{
  ...
  while (p < field + length)
  {
    ...
    default_charset.charset_pairs.push_back(std::make_pair(col_index, col_charset));
  }
}
...
/**
   ...
   @param[in]  length  length of the field
 */
static void parse_column_charset(std::vector<unsigned int> &vec, unsigned char *field, unsigned int length)
{
  unsigned char *p = field;

  while (p < field + length) vec.push_back(net_field_length(&p));
}
...
/**
   ...
   @param[in]  length  length of the field
 */
static void parse_column_name(std::vector<std::string> &vec, unsigned char *field, unsigned int length)
{
  unsigned char *p = field;
...
/**
   ...
   @param[in]  length  length of the field
 */
static void parse_set_str_value(std::vector<Table_map_log_event::Optional_metadata_fields::str_vector> &vec,
                                unsigned char *field, unsigned int length)
{
  unsigned char *p = field;
...
/**
   ...
   @param[in]  length  length of the field
 */
static void parse_geometry_type(std::vector<unsigned int> &vec, unsigned char *field, unsigned int length)
{
  unsigned char *p = field;
...
/**
   ...
   @param[in]  length  length of the field
 */
static void parse_simple_pk(std::vector<Table_map_log_event::Optional_metadata_fields::uint_pair> &vec,
                            unsigned char *field, unsigned int length)
{
  unsigned char *p = field;

  while (p < field + length) vec.push_back(std::make_pair(net_field_length(&p), 0));
}
...
static void parse_pk_with_prefix(std::vector<Table_map_log_event::Optional_metadata_fields::uint_pair> &vec,
                                 unsigned char *field, unsigned int length)
{
  unsigned char *p = field;
...
Table_map_log_event::Optional_metadata_fields::Optional_metadata_fields(unsigned char *optional_metadata,
                                                                        unsigned int optional_metadata_len)
{
  ...
  {
    unsigned int len;
    Optional_metadata_field_type type = static_cast<Optional_metadata_field_type>(field[0]);
...
Write_rows_compressed_log_event::Write_rows_compressed_log_event(const uchar *buf, uint event_len,
                                                                 const Format_description_log_event *description_event)
...
Delete_rows_compressed_log_event::Delete_rows_compressed_log_event(
    const uchar *buf, uint event_len, const Format_description_log_event *description_event)
    ...
    : Log_event(buf, descr_event)
{
  ...
  uint8 const post_header_len = descr_event->post_header_len[INCIDENT_EVENT - 1];
...
Ignorable_log_event::Ignorable_log_event(const uchar *buf, const Format_description_log_event *descr_event,
                                         const char *event_name)
    : Log_event(buf, descr_event), number((int)(uchar)buf[EVENT_TYPE_OFFSET]), description(event_name)
{
...
{
  return (my_b_copy_all_to_file(cache, file) || reinit_io_cache(cache, WRITE_CACHE, 0, FALSE, TRUE));
}
