...
*/
const char *global_plugin_typelib_names[] = {"OFF", "ON", "FORCE", "FORCE_PLUS_PERMANENT", NULL};
static TYPELIB global_plugin_typelib = {array_elements(global_plugin_typelib_names) - 1, "",
...
*/
uint plugin_maturity_map[] = {0, 1, 2, 3, 4, 5, 6};
...
const LEX_CSTRING plugin_type_names[MYSQL_MAX_PLUGIN_TYPE_NUM] = {
    {STRING_WITH_LEN("UDF")},         {STRING_WITH_LEN("STORAGE ENGINE")},     {STRING_WITH_LEN("FTPARSER")},
    {STRING_WITH_LEN("DAEMON")},      {STRING_WITH_LEN("INFORMATION SCHEMA")}, {STRING_WITH_LEN("AUDIT")},
    {STRING_WITH_LEN("REPLICATION")}, {STRING_WITH_LEN("AUTHENTICATION")},     {STRING_WITH_LEN("PASSWORD VALIDATION")},
    {STRING_WITH_LEN("ENCRYPTION")},  {STRING_WITH_LEN("DATA TYPE")},          {STRING_WITH_LEN("FUNCTION")}};
...
/*
  ...
  defined.
*/
plugin_type_init plugin_type_initialize[MYSQL_MAX_PLUGIN_TYPE_NUM] = {0,
...
plugin_type_init plugin_type_deinitialize[MYSQL_MAX_PLUGIN_TYPE_NUM] = {
    0, ha_finalize_handlerton,     0, 0, finalize_schema_table, finalize_audit_plugin, 0, 0,
    0, finalize_encryption_plugin, 0,
    0  // FUNCTION
};
...
static int plugin_type_initialization_order[MYSQL_MAX_PLUGIN_TYPE_NUM] = {
    MYSQL_DAEMON_PLUGIN,     MariaDB_ENCRYPTION_PLUGIN,   MariaDB_DATA_TYPE_PLUGIN,
    MariaDB_FUNCTION_PLUGIN, MYSQL_STORAGE_ENGINE_PLUGIN, MYSQL_INFORMATION_SCHEMA_PLUGIN,
    MYSQL_FTPARSER_PLUGIN,   MYSQL_AUTHENTICATION_PLUGIN, MariaDB_PASSWORD_VALIDATION_PLUGIN,
    MYSQL_AUDIT_PLUGIN,      MYSQL_REPLICATION_PLUGIN,    MYSQL_UDF_PLUGIN};
...
static const char *maria_plugin_interface_version_sym = "_maria_plugin_interface_version_";
static const char *maria_sizeof_st_plugin_sym = "_maria_sizeof_struct_st_plugin_";
static const char *maria_plugin_declarations_sym = "_maria_plugin_declarations_";
static int min_maria_plugin_interface_version = MARIA_PLUGIN_INTERFACE_VERSION & ~0xFF;
#endif
...
class sys_var_pluginvar : public sys_var, public Sql_alloc
{
 public:
  ...
  sys_var_pluginvar(sys_var_chain *chain, const char *name_arg, st_plugin_int *p, st_mysql_sys_var *plugin_var_arg,
  ...
  const uchar *global_value_ptr(THD *thd, const LEX_CSTRING *base) const { return do_value_ptr(thd, OPT_GLOBAL, base); }
  const uchar *default_value_ptr(THD *thd) const { return do_value_ptr(thd, OPT_DEFAULT, 0); }
...
static int test_plugin_options(MEM_ROOT *, struct st_plugin_int *, int *, char **);
static bool register_builtin(struct st_maria_plugin *, struct st_plugin_int *, struct st_plugin_int **);
...
{
  return p->load_option == PLUGIN_FORCE || p->load_option == PLUGIN_FORCE_PLUS_PERMANENT;
}
...
static void fix_dl_name(MEM_ROOT *root, LEX_CSTRING *dl)
{
  ...
  if (dl->length < so_ext_len || my_strcasecmp(&my_charset_latin1, dl->str + dl->length - so_ext_len, SO_EXT))
  {
...
static const char *item_val_str(struct st_mysql_value *value, char *buffer, int *length)
{
...
static struct st_plugin_dl *plugin_dl_find(const LEX_CSTRING *dl)
{
  ...
  for (i = 0; i < plugin_dl_array.elements; i++)
  {
    ...
    if (tmp->ref_count && !files_charset_info->strnncoll(dl->str, dl->length, tmp->dl.str, tmp->dl.length))
      DBUG_RETURN(tmp);
  }
  DBUG_RETURN(0);
}
...
static st_plugin_dl *plugin_dl_insert_or_reuse(struct st_plugin_dl *plugin_dl)
{
  ...
  if (insert_dynamic(&plugin_dl_array, (uchar *)&plugin_dl))
    DBUG_RETURN(0);
  tmp = *dynamic_element(&plugin_dl_array, plugin_dl_array.elements - 1, struct st_plugin_dl **) =
      (struct st_plugin_dl *)memdup_root(&plugin_mem_root, (uchar *)plugin_dl, sizeof(struct st_plugin_dl));
  DBUG_RETURN(tmp);
}
#else
static struct st_plugin_dl *plugin_dl_find(const LEX_STRING *) { return 0; }
#endif /* HAVE_DLOPEN */
...
#ifdef HAVE_DLOPEN
static my_bool read_mysql_plugin_info(struct st_plugin_dl *plugin_dl, void *sym, char *dlpath, myf MyFlags)
{
  ...
  {
    my_error(ER_CANT_OPEN_LIBRARY, MyFlags, dlpath, ENOEXEC, "plugin interface version mismatch");
    DBUG_RETURN(TRUE);
  }
  ...
  /* convert mysql declaration to maria one */
  {
    ...
    for (i = 0; ((struct st_mysql_plugin *)(ptr + i * sizeof_st_plugin))->info; i++) /* no op */;

    cur = (struct st_maria_plugin *)my_malloc(key_memory_mysql_plugin, (i + 1) * sizeof(struct st_maria_plugin),
                                              MYF(MY_ZEROFILL | MY_WME));
    if (!cur)
    {
      my_error(ER_OUTOFMEMORY, MyFlags, static_cast<int>(plugin_dl->dl.length));
      DBUG_RETURN(TRUE);
    }
    /*
...
static my_bool read_maria_plugin_info(struct st_plugin_dl *plugin_dl, void *sym, char *dlpath, myf MyFlags)
{
  DBUG_ENTER("read_maria_plugin_info");
  ...
  if (!(sym))
  {
    ...
    */
    my_error(ER_CANT_FIND_DL_ENTRY, MyFlags, maria_plugin_interface_version_sym);
    DBUG_RETURN(TRUE);
  }
  ...
  if (plugin_dl->mariaversion != MARIA_PLUGIN_INTERFACE_VERSION)
  {
    ...
    {
      int i;
      for (i = 0; ((struct st_maria_plugin *)(ptr + i * sizeof_st_plugin))->info; i++) /* no op */;
      ...
      /*
        ...
        have less values than the struct definition.
      */
      for (i = 0; (old = (struct st_maria_plugin *)(ptr + i * sizeof_st_plugin))->info; i++)
...
static st_plugin_dl *plugin_dl_add(const LEX_CSTRING *dl, myf MyFlags)
{
  ...
  DBUG_PRINT("enter", ("dl->str: '%s', dl->length: %d", dl->str, (int)dl->length));
  ...
  */
  if (check_string_char_length((LEX_CSTRING *)dl, 0, NAME_CHAR_LEN, system_charset_info, 1) ||
      check_valid_path(dl->str, dl->length) || plugin_dir_len + dl->length + 1 >= FN_REFLEN)
  {
  ...
  {
    if (read_mysql_plugin_info(&plugin_dl, dlsym(plugin_dl.handle, plugin_interface_version_sym), dlpath, MyFlags))
      goto ret;
  }
  else
  {
  ...
  for (i = 0; i < array_elements(list_of_services); i++)
  {
    if ((sym = dlsym(plugin_dl.handle, list_of_services[i].name)))
    {
      ...
      if (ver > list_of_services[i].version || (ver >> 8) < (list_of_services[i].version >> 8))
      {
        char buf[MYSQL_ERRMSG_SIZE];
        my_snprintf(buf, sizeof(buf), "service '%s' interface version mismatch", list_of_services[i].name);
  ...
  if (plugin_dl.nbackups)
  {
    ...
    plugin_dl.ptr_backup = (st_ptr_backup *)my_malloc(key_memory_mysql_plugin_dl, bytes, MYF(0));
  ...
  if (!(plugin_dl.dl.str = (char *)my_malloc(key_memory_mysql_plugin_dl, plugin_dl.dl.length, MYF(0))))
  {
    my_error(ER_OUTOFMEMORY, MyFlags, static_cast<int>(plugin_dl.dl.length));
    goto ret;
  }
  plugin_dl.dl.length = copy_and_convert((char *)plugin_dl.dl.str, plugin_dl.dl.length, files_charset_info, dl->str,
                                         dl->length, system_charset_info, &dummy_errors);
  ((char *)plugin_dl.dl.str)[plugin_dl.dl.length] = 0;
  /* Add this dll to array */
  if (!(tmp = plugin_dl_insert_or_reuse(&plugin_dl)))
  {
    my_error(ER_OUTOFMEMORY, MyFlags, static_cast<int>(sizeof(struct st_plugin_dl)));
    goto ret;
  }
...
static struct st_plugin_int *plugin_find_internal(const LEX_CSTRING *name, int type)
{
  uint i;
  ...
  if (type == MYSQL_ANY_PLUGIN)
  {
    ...
    {
      struct st_plugin_int *plugin =
          (st_plugin_int *)my_hash_search(&plugin_hash[i], (const uchar *)name->str, name->length);
      if (plugin)
        DBUG_RETURN(plugin);
    }
  }
  else
    DBUG_RETURN((st_plugin_int *)my_hash_search(&plugin_hash[type], (const uchar *)name->str, name->length));
  DBUG_RETURN(0);
}
...
static plugin_ref intern_plugin_lock(LEX *lex, plugin_ref rc,
                                     uint state_mask = PLUGIN_IS_READY | PLUGIN_IS_UNINITIALIZED | PLUGIN_IS_DELETED)
{
  ...
  if (pi->state & state_mask)
  {
    ...
    */
    if (!(plugin = (plugin_ref)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(pi), MYF(MY_WME))))
      DBUG_RETURN(NULL);

    *plugin = pi;
#endif
    pi->ref_count++;
    DBUG_PRINT("lock", ("thd: %p  plugin: \"%s\" LOCK ref_count: %d", current_thd, pi->name.str, pi->ref_count));
...
static st_plugin_int *plugin_insert_or_reuse(struct st_plugin_int *plugin)
{
  ...
  tmp = *dynamic_element(&plugin_array, plugin_array.elements - 1, struct st_plugin_int **) =
      (struct st_plugin_int *)memdup_root(&plugin_mem_root, (uchar *)plugin, sizeof(struct st_plugin_int));
  DBUG_RETURN(tmp);
}
...
*/
static enum install_status plugin_add(MEM_ROOT *tmp_root, bool if_not_exists, const LEX_CSTRING *name, LEX_CSTRING *dl,
                                      myf MyFlags)
{
  ...
  for (plugin = tmp.plugin_dl->plugins; plugin->info; plugin++)
  {
    ...
    if (plugin->type == MYSQL_UDF_PLUGIN ||
        (plugin->type == MariaDB_PASSWORD_VALIDATION_PLUGIN && tmp.plugin_dl->mariaversion == 0))
      continue;  // unsupported plugin type

    if (name->str && system_charset_info->strnncoll(name->str, name->length, tmp.name.str, tmp.name.length))
    ...
    if (*(int *)plugin->info < min_plugin_info_interface_version[plugin->type] ||
        ((*(int *)plugin->info) >> 8) > (cur_plugin_info_interface_version[plugin->type] >> 8))
    {
      char buf[256];
      strxnmov(buf, sizeof(buf) - 1, "API version for ", plugin_type_names[plugin->type].str, " plugin ", tmp.name.str,
    ...
    {
      char buf[256];
      strxnmov(buf, sizeof(buf) - 1, "Loading of ", plugin_maturity_names[plugin->maturity], " plugin ", tmp.name.str,
               " is prohibited by --plugin-maturity=", plugin_maturity_names[plugin_maturity], NullS);
      my_error(ER_CANT_OPEN_LIBRARY, MyFlags, dl->str, EPERM, buf);
      goto err;
    }
    else if (plugin_maturity_map[plugin->maturity] < SERVER_MATURITY_LEVEL)
    {
      sql_print_warning("Plugin '%s' is of maturity level %s while the server is %s", tmp.name.str,
                        plugin_maturity_names[plugin->maturity], plugin_maturity_names[SERVER_MATURITY_LEVEL]);
    }
    ...
    init_alloc_root(key_memory_plugin_int_mem_root, &tmp_plugin_ptr->mem_root, 4096, 4096, MYF(0));
...
{
  for (sys_var *var = plugin->system_vars; var; var = var->next) (*var->test_load) = FALSE;
  ...
  */
  ...
  if (plugin->plugin->status_vars)
  {
    ...
    */
    ...
    SHOW_VAR tmp_array[2] = {{plugin->plugin->name, (char *)plugin->plugin->status_vars, SHOW_ARRAY},
                             {0, 0, SHOW_UNDEF}};
  ...
  if (deinit && deinit(plugin))
  {
    if (THD *thd = current_thd)
      push_warning(thd, Sql_condition::WARN_LEVEL_WARN, WARN_PLUGIN_BUSY, ER_THD(thd, WARN_PLUGIN_BUSY));
  }
  else
    plugin->state = PLUGIN_IS_UNINITIALIZED;  // free to unload

  if (ref_check && plugin->ref_count)
    sql_print_error("Plugin '%s' has ref_count=%d after deinitialization.", plugin->name.str, plugin->ref_count);
...
static void reap_plugins(void)
{
  ...
  *(reap++) = NULL;
  ...
  while ((plugin = *(--reap))) plugin_del(plugin, 0);
...
static void intern_plugin_unlock(LEX *lex, plugin_ref plugin)
{
  ...
  DBUG_PRINT("lock", ("thd: %p  plugin: \"%s\" UNLOCK ref_count: %d", current_thd, pi->name.str, pi->ref_count));
...
static int plugin_initialize(MEM_ROOT *tmp_root, struct st_plugin_int *plugin, int *argc, char **argv,
                             bool options_only)
{
  int ret = 1;
  ...
  if (plugin_type_initialize[plugin->plugin->type])
  {
    ...
    {
      sql_print_error("Plugin '%s' registration as a %s failed.", plugin->name.str,
...
uchar *get_plugin_hash_key(const uchar *buff, size_t *length, my_bool not_used __attribute__((unused)))
{
...
uchar *get_bookmark_hash_key(const uchar *buff, size_t *length, my_bool not_used __attribute__((unused)))
{
...
static PSI_mutex_info all_plugin_mutexes[] = {{&key_LOCK_plugin, "LOCK_plugin", PSI_FLAG_GLOBAL}};

static PSI_memory_info all_plugin_memory[] = {{&key_memory_plugin_mem_root, "plugin_mem_root", PSI_FLAG_GLOBAL},
...
int plugin_init(int *argc, char **argv, int flags)
{
  ...
  if (my_hash_init(key_memory_plugin_bookmark, &bookmark_hash, &my_charset_bin, 32, 0, 0, get_bookmark_hash_key, NULL,
                   HASH_UNIQUE))
    goto err;
  ...
  */
  if (my_init_dynamic_array(key_memory_mysql_plugin_dl, &plugin_dl_array, sizeof(struct st_plugin_dl *), 16, 16,
                            MYF(0)) ||
      my_init_dynamic_array(key_memory_mysql_plugin, &plugin_array, sizeof(struct st_plugin_int *), 80, 32, MYF(0)))
    goto err;

  for (i = 0; i < MYSQL_MAX_PLUGIN_TYPE_NUM; i++)
  {
    if (my_hash_init(key_memory_plugin_mem_root, &plugin_hash[i], system_charset_info, 32, 0, 0, get_plugin_hash_key,
                     NULL, HASH_UNIQUE))
      goto err;
  }
  ...
  */
  ...
  for (builtins = mysql_mandatory_plugins; *builtins || mandatory; builtins++)
  {
    ...
    {
      if (opt_ignore_builtin_innodb && !my_charset_latin1.strnncoll(plugin->name, 6, "InnoDB", 6))
        continue;
      ...
      {
        if (!my_strcasecmp(&my_charset_latin1, plugin->name, override_plugin_load_policy[i].plugin_name))
        {
  ...
  */
  ...
  if (plugin_ptr)
  {
    ...
    */
    global_system_variables.table_plugin = intern_plugin_lock(NULL, plugin_int_to_ref(plugin_ptr));
  ...
  while (i_string *item = opt_plugin_load_list_iter++) plugin_load_list(&tmp_root, item->ptr);

  if (!(flags & PLUGIN_INIT_SKIP_PLUGIN_TABLE))
  {
    char path[FN_REFLEN + 1];
    build_table_filename(path, sizeof(path) - 1, "mysql", "plugin", reg_ext, 0);
    Table_type ttype = dd_frm_type(0, path, &plugin_table_engine_name, NULL, NULL);
  ...
  for (;;)
  {
    for (i = 0; i < MYSQL_MAX_PLUGIN_TYPE_NUM; i++)
    {
      ...
      for (uint idx = 0; idx < hash->records; idx++)
      {
        ...
        {
          bool plugin_table_engine = lex_string_eq(&plugin_table_engine_name, &plugin_ptr->name);
...
static bool register_builtin(struct st_maria_plugin *plugin, struct st_plugin_int *tmp, struct st_plugin_int **ptr)
{
  ...
  *ptr = *dynamic_element(&plugin_array, plugin_array.elements - 1, struct st_plugin_int **) =
      (struct st_plugin_int *)memdup_root(&plugin_mem_root, (uchar *)tmp, sizeof(struct st_plugin_int));
...
static void plugin_load(MEM_ROOT *tmp_root)
{
  ...
  bool result;
  unsigned long event_class_mask[MYSQL_AUDIT_CLASS_MASK_SIZE] = {MYSQL_AUDIT_GENERAL_CLASSMASK};
  ...
  if (result)
  {
    ...
    if (!opt_help)
      sql_print_error(
          "Could not open mysql.plugin table: \"%s\". "
  ...
  if (init_read_record(&read_record_info, new_thd, table, NULL, NULL, 1, 0, FALSE))
  {
    sql_print_error(
        "Could not initialize init_read_record; Plugins not "
        "loaded");
    goto end;
  }
  ...
  if (unlikely(error > 0))
    sql_print_error(ER_THD(new_thd, ER_GET_ERRNO), my_errno, table->file->table_type());
    ...
    switch ((*(p++) = *(list++)))
    {
      ...
      case ':': /* can't use this as delimiter as it may be drive letter */
        ...
        if (str == &name)  // load all plugins in named module
        {
          ...
          if (plugin_add(tmp_root, false, &name, &dl, MYF(ME_ERROR_LOG)) != INSTALL_GOOD)
            goto error;
        }
        else
        {
...
error:
  mysql_mutex_unlock(&LOCK_plugin);
  if (name.str)
    sql_print_error("Couldn't load plugin '%s' from '%s'.", name.str, dl.str);
  else
...
void plugin_shutdown(void)
{
    ...
    */
    for (i = 0; i < count; i++)
    {
      if (plugins[i]->ref_count)
        sql_print_error("Plugin '%s' has ref_count=%d after shutdown.", plugins[i]->name.str, plugins[i]->ref_count);
  ...
  for (i = 0; i < MYSQL_MAX_PLUGIN_TYPE_NUM; i++) my_hash_free(&plugin_hash[i]);
  delete_dynamic(&plugin_array);

  count = plugin_dl_array.elements;
  dl = (struct st_plugin_dl **)my_alloca(sizeof(void *) * count);
  for (i = 0; i < count; i++) dl[i] = *dynamic_element(&plugin_dl_array, i, struct st_plugin_dl **);
  for (i = 0; i < plugin_dl_array.elements; i++) free_plugin_mem(dl[i]);
  my_afree(dl);
...
*/
static bool finalize_install(THD *thd, TABLE *table, const LEX_CSTRING *name, int *argc, char **argv)
{
  ...
  else
  {
    ...
    {
      my_error(ER_CANT_INITIALIZE_UDF, MYF(0), name->str, "Plugin initialization function failed.");
  ...
  if (tmp->state == PLUGIN_IS_DISABLED)
  {
    if (global_system_variables.log_warnings)
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_CANT_INITIALIZE_UDF,
                          ER_THD(thd, ER_CANT_INITIALIZE_UDF), name->str, "Plugin is disabled");
  }
  ...
  */
  ...
  table->field[1]->store(tmp->plugin_dl->dl.str, tmp->plugin_dl->dl.length, files_charset_info);
...
bool mysql_install_plugin(THD *thd, const LEX_CSTRING *name, const LEX_CSTRING *dl_arg)
{
  TABLE_LIST tables;
  TABLE *table;
  ...
  */
  ...
  if (name->str)
    error = finalize_install(thd, table, name, &argc, argv) ? INSTALL_FAIL_NOT_OK : INSTALL_GOOD;
  else
  {
...
bool mysql_uninstall_plugin(THD *thd, const LEX_CSTRING *name, const LEX_CSTRING *dl_arg)
{
  TABLE *table;
  TABLE_LIST tables;
  LEX_CSTRING dl = *dl_arg;
  ...
  if (!table->key_info)
  {
    my_printf_error(ER_UNKNOWN_ERROR,
                    ...
                    "create the primary key accordingly.",
                    MYF(0), table->s->db.str, table->s->table_name.str);
    DBUG_RETURN(TRUE);
  }
  ...
  */
  ...
  else
  {
    ...
    if (plugin_dl)
    {
      for (struct st_maria_plugin *plugin = plugin_dl->plugins; plugin->info; plugin++)
      {
        LEX_CSTRING str = {plugin->name, strlen(plugin->name)};
  ...
  */
  if (type == MYSQL_ANY_PLUGIN)
  {
    ...
    for (idx = 0; idx < plugin_array.elements; idx++)
    {
      ...
      if ((plugins[total] = intern_plugin_lock(0, plugin_int_to_ref(plugin), state_mask)))
        total++;
    }
  }
  else
...
static bool plugin_dl_foreach_internal(THD *thd, st_plugin_dl *plugin_dl, st_maria_plugin *plug,
...
bool plugin_dl_foreach(THD *thd, const LEX_CSTRING *dl, plugin_foreach_func *func, void *arg)
{
  bool err = 0;
  ...
  if (dl)
  {
    ...
    err = plugin_dl_foreach_internal(thd, plugin_dl, plugin_dl->plugins, func, arg);
...
{
  return (flags & PLUGIN_VAR_TYPEMASK) | (flags & PLUGIN_VAR_MEMALLOC ? BOOKMARK_MEMALLOC : 0);
}
...
static int check_func_bool(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  ...
  {
    length = sizeof(buff);
    if (!(str = value->val_str(value, buff, &length)) || (result = find_type(&bool_typelib, str, length, 1) - 1) < 0)
      goto err;
  }
  else
  {
...
static int check_func_int(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  ...
  if (var->flags & PLUGIN_VAR_UNSIGNED)
  {
    if ((fixed1 = (!value->is_unsigned(value) && val < 0)))
      val = 0;
    *(uint *)save = (uint)getopt_ull_limit_value((ulonglong)val, &options, &fixed2);
  }
  else
  {
    if ((fixed1 = (value->is_unsigned(value) && val < 0)))
  ...
  return throw_bounds_warning(thd, var->name, fixed1 || fixed2, value->is_unsigned(value), (longlong)orig);
}

static int check_func_long(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  my_bool fixed1, fixed2;
  long long orig, val;
  struct my_option options;
  ...
  if (var->flags & PLUGIN_VAR_UNSIGNED)
  {
    if ((fixed1 = (!value->is_unsigned(value) && val < 0)))
      val = 0;
    *(ulong *)save = (ulong)getopt_ull_limit_value((ulonglong)val, &options, &fixed2);
  }
  else
  {
    if ((fixed1 = (value->is_unsigned(value) && val < 0)))
...
static int check_func_longlong(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  my_bool fixed1, fixed2;
  long long orig, val;
  struct my_option options;
  ...
  if (var->flags & PLUGIN_VAR_UNSIGNED)
  {
    if ((fixed1 = (!value->is_unsigned(value) && val < 0)))
      val = 0;
    *(ulonglong *)save = getopt_ull_limit_value((ulonglong)val, &options, &fixed2);
  }
  else
  {
    if ((fixed1 = (value->is_unsigned(value) && val < 0)))
...
static int check_func_str(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  char buff[STRING_BUFFER_USUAL_SIZE];
  const char *str;
  int length;
...
static int check_func_enum(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  char buff[STRING_BUFFER_USUAL_SIZE];
  const char *str;
  TYPELIB *typelib;
...
static int check_func_set(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  ...
  if (value->value_type(value) == MYSQL_VALUE_TYPE_STRING)
  {
    ...
    if (!(str = value->val_str(value, buff, &length)))
      goto err;
    result = find_set(typelib, str, length, NULL, &error, &error_len, &not_used);
  ...
  else
  {
    if (value->val_int(value, (long long *)&result))
      goto err;
    if (unlikely((result >= (1ULL << typelib->count)) && (typelib->count < sizeof(long) * 8)))
      goto err;
  }
...
static int check_func_double(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  double v;
  my_bool fixed;
...
static void update_func_bool(THD *thd, struct st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(my_bool *)tgt = *(my_bool *)save ? 1 : 0;
}

static void update_func_int(THD *thd, struct st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(int *)tgt = *(int *)save;
}

static void update_func_long(THD *thd, struct st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(long *)tgt = *(long *)save;
}

static void update_func_longlong(THD *thd, struct st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(longlong *)tgt = *(ulonglong *)save;
}

static void update_func_str(THD *thd, struct st_mysql_sys_var *var, void *tgt, const void *save)
{
  ...
  if (var->flags & PLUGIN_VAR_MEMALLOC)
  {
    char *old = *(char **)tgt;
    if (value)
      *(char **)tgt = my_strdup(key_memory_global_system_variables, value, MYF(0));
    else
      *(char **)tgt = 0;
    my_free(old);
  }
...
static void update_func_double(THD *thd, struct st_mysql_sys_var *var, void *tgt, const void *save)
{
...
sys_var *find_sys_var(THD *thd, const char *str, size_t length, bool throw_error)
{
  sys_var *var;
  ...
  if ((var = intern_find_sys_var(str, length)) && (pi = var->cast_pluginvar()))
  {
    mysql_mutex_lock(&LOCK_plugin);
    if (!intern_plugin_lock(thd ? thd->lex : 0, plugin_int_to_ref(pi->plugin), PLUGIN_IS_READY))
  ...
  if (unlikely(!throw_error && !var))
    my_error(ER_UNKNOWN_SYSTEM_VARIABLE, MYF(0), (int)(length ? length : strlen(str)), (char *)str);
  DBUG_RETURN(var);
}
...
static st_bookmark *find_bookmark(const char *plugin, const char *name, int flags)
{
  ...
  result = (st_bookmark *)my_hash_search(&bookmark_hash, (const uchar *)varname, length - 1);
...
static st_bookmark *register_var(const char *plugin, const char *name, int flags)
{
  ...
  {
    result = (st_bookmark *)alloc_root(&plugin_vars_mem_root, sizeof(struct st_bookmark) + length - 1);
    varname[0] = plugin_var_bookmark_key(flags);
    ...
    if (new_size > global_variables_dynamic_size)
    {
      global_system_variables.dynamic_variables_ptr =
          (char *)my_realloc(key_memory_global_system_variables, global_system_variables.dynamic_variables_ptr,
                             new_size, MYF(MY_WME | MY_FAE | MY_ALLOW_ZERO_PTR));
      max_system_variables.dynamic_variables_ptr =
          (char *)my_realloc(key_memory_global_system_variables, max_system_variables.dynamic_variables_ptr, new_size,
      ...
      /*
        ...
        string.
      */
      bzero(global_system_variables.dynamic_variables_ptr + global_variables_dynamic_size,
            new_size - global_variables_dynamic_size);
      bzero(max_system_variables.dynamic_variables_ptr + global_variables_dynamic_size,
            new_size - global_variables_dynamic_size);
...
void sync_dynamic_session_variables(THD *thd, bool global_lock)
{
  ...
  thd->variables.dynamic_variables_ptr =
      (char *)my_realloc(key_memory_THD_variables, thd->variables.dynamic_variables_ptr, global_variables_dynamic_size,
                         MYF(MY_WME | MY_FAE | MY_ALLOW_ZERO_PTR));
  ...
  memcpy(thd->variables.dynamic_variables_ptr + thd->variables.dynamic_variables_size,
         global_system_variables.dynamic_variables_ptr + thd->variables.dynamic_variables_size,
         global_system_variables.dynamic_variables_size - thd->variables.dynamic_variables_size);
  ...
  */
  for (idx = 0; idx < bookmark_hash.records; idx++)
  {
    ...
    if ((v->key[0] & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_STR && v->key[0] & BOOKMARK_MEMALLOC)
    {
  ...
  thd->variables.dynamic_variables_version = global_system_variables.dynamic_variables_version;
  thd->variables.dynamic_variables_head = global_system_variables.dynamic_variables_head;
  thd->variables.dynamic_variables_size = global_system_variables.dynamic_variables_size;
}
...
static uchar *intern_sys_var_ptr(THD *thd, int offset, bool global_lock)
{
  ...
  */
  if (!thd->variables.dynamic_variables_ptr || (uint)offset > thd->variables.dynamic_variables_head)
  {
    mysql_prlock_rdlock(&LOCK_system_variables_hash);
...
static int *mysql_sys_var_int(THD *thd, int offset) { return (int *)intern_sys_var_ptr(thd, offset, true); }

static long *mysql_sys_var_long(THD *thd, int offset) { return (long *)intern_sys_var_ptr(thd, offset, true); }
...
void plugin_thdvar_init(THD *thd)
{
  ...
  mysql_mutex_lock(&LOCK_plugin);
  thd->variables.table_plugin = intern_plugin_lock(NULL, global_system_variables.table_plugin);
  if (global_system_variables.tmp_table_plugin)
    thd->variables.tmp_table_plugin = intern_plugin_lock(NULL, global_system_variables.tmp_table_plugin);
  if (global_system_variables.enforced_table_plugin)
    thd->variables.enforced_table_plugin = intern_plugin_lock(NULL, global_system_variables.enforced_table_plugin);
...
void plugin_thdvar_cleanup(THD *thd)
{
  ...
  if ((idx = thd->lex->plugins.elements))
  {
    ...
    while ((uchar *)list >= thd->lex->plugins.buffer) intern_plugin_unlock(NULL, *list--);
  }
...
static void plugin_vars_free_values(sys_var *vars)
{
  ...
  for (sys_var *var = vars; var; var = var->next)
  {
    ...
    if (piv && ((piv->plugin_var->flags & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_STR) &&
        (piv->plugin_var->flags & PLUGIN_VAR_MEMALLOC))
    {
      /* Free the string from global_system_variables. */
      char **valptr = (char **)piv->real_value_ptr(NULL, OPT_GLOBAL);
      DBUG_PRINT("plugin", ("freeing value for: '%s'  addr: %p", var->name.str, valptr));
      my_free(*valptr);
      *valptr = NULL;
    }
  }
...
sys_var_pluginvar::sys_var_pluginvar(sys_var_chain *chain, const char *name_arg, st_plugin_int *p, st_mysql_sys_var *pv,
                                     const char *substitute)
    : sys_var(chain, name_arg, pv->comment, pluginvar_sysvar_flags(pv), 0, pv->flags & PLUGIN_VAR_NOCMDOPT ? -1 : 0,
              NO_ARG, pluginvar_show_type(pv), 0, NULL, VARIABLE_NOT_IN_BINLOG, NULL, NULL, substitute),
      plugin(p),
      plugin_var(pv)
{
...
{
  uchar *value = plugin_var->flags & PLUGIN_VAR_THDLOCAL ? intern_sys_var_ptr(thd, *(int *)(plugin_var + 1), true)
                                                         : *(uchar **)(plugin_var + 1);
...
const uchar *sys_var_pluginvar::do_value_ptr(THD *thd, enum_var_type type, const LEX_CSTRING *base) const
{
  ...
  if ((plugin_var->flags & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_ENUM)
    ...
    result = (uchar *)set_to_string(thd, 0, *(ulonglong *)result, plugin_var_typelib()->type_names);
  return result;
}
...
bool sys_var_pluginvar::session_update(THD *thd, set_var *var)
{
  ...
  const void *src = var->value ? (void *)&var->save_result : (void *)real_value_ptr(thd, OPT_GLOBAL);
  mysql_mutex_unlock(&LOCK_global_system_variables);
...
void plugin_opt_set_limits(struct my_option *options, const struct st_mysql_sys_var *opt)
{
  options->sub_size = 0;

  switch (opt->flags & (PLUGIN_VAR_TYPEMASK | PLUGIN_VAR_UNSIGNED | PLUGIN_VAR_THDLOCAL))
  {
    ...
    case PLUGIN_VAR_BOOL:
      ...
      break;
    case PLUGIN_VAR_STR:
      options->var_type = ((opt->flags & PLUGIN_VAR_MEMALLOC) ? GET_STR_ALLOC : GET_STR);
...
static int construct_options(MEM_ROOT *mem_root, struct st_plugin_int *tmp, my_option *options)
{
  ...
  plugin_name_with_prefix_ptr = (char *)alloc_root(mem_root, plugin_name_len + plugin_dash.length + 1);
  ...
  if (!plugin_is_forced(tmp))
  {
    ...
    */
    options[0].value = options[1].value = (uchar **)alloc_root(mem_root, sizeof(ulong));
  ...
  */
  ...
  for (plugin_option = tmp->plugin->system_vars; plugin_option && *plugin_option; plugin_option++, index++)
  {
    opt = *plugin_option;

    if (!opt->name)
    {
      sql_print_error("Missing variable name in plugin '%s'.", plugin_name);
      DBUG_RETURN(-1);
    }
    ...
    switch (opt->flags & PLUGIN_VAR_TYPEMASK)
    {
      ...
      case PLUGIN_VAR_DOUBLE:
        ...
        break;
      default:
        sql_print_error("Unknown variable type code 0x%x in plugin '%s'.", opt->flags, plugin_name);
        DBUG_RETURN(-1);
    };
  }
  ...
  for (plugin_option = tmp->plugin->system_vars; plugin_option && *plugin_option; plugin_option++, index++)
  {
    ...
    if ((opt->flags & (PLUGIN_VAR_NOCMDOPT | PLUGIN_VAR_THDLOCAL)) == PLUGIN_VAR_NOCMDOPT)
      continue;
    ...
    else
    {
      ...
      *(int *)(opt + 1) = offset = v->offset;
      ...
      if (opt->flags & PLUGIN_VAR_NOCMDOPT)
      {
        ...
        if (((opt->flags & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_STR) && (opt->flags & PLUGIN_VAR_MEMALLOC))
        {
    ...
    if (opt->flags & PLUGIN_VAR_THDLOCAL)
      options->value = options->u_max_value = (uchar **)(global_system_variables.dynamic_variables_ptr + offset);
    else
      options->value = options->u_max_value = *(uchar ***)(opt + 1);

    char *option_name_ptr;
    options[1] = options[0];
    options[1].name = option_name_ptr = (char *)alloc_root(mem_root, plugin_dash.length + optnamelen + 1);
...
static my_option *construct_help_options(MEM_ROOT *mem_root, struct st_plugin_int *p)
{
...
extern "C" my_bool mark_changed(const struct my_option *, const char *, const char *);
my_bool mark_changed(const struct my_option *opt, const char *, const char *filename)
{
  if (opt->app_type)
  {
...
static int test_plugin_options(MEM_ROOT *tmp_root, struct st_plugin_int *tmp, int *argc, char **argv)
{
  struct sys_var_chain chain = {NULL, NULL};
  bool disable_plugin;
  enum_plugin_load_option plugin_load_option = tmp->load_option;

  MEM_ROOT *mem_root = alloc_root_inited(&tmp->mem_root) ? &tmp->mem_root : &plugin_vars_mem_root;
  st_mysql_sys_var **opt;
  my_option *opts = NULL;
  int error = 1;
  ...
  if (tmp->plugin->system_vars || (*argc > 1))
  {
    ...
    */
    ...
    if (unlikely(error))
    {
      sql_print_error("Parsing options for plugin '%s' failed.", tmp->name.str);
      goto err;
    }
    /*
  ...
  */
  if (disable_plugin)
  {
    if (global_system_variables.log_warnings && !opt_help)
      sql_print_information("Plugin '%s' is disabled.", tmp->name.str);
    goto err;
  }
  ...
  if (tmp->plugin->system_vars)
  {
    if (mysqld_server_started)
    {
      ...
      */
      for (opt = tmp->plugin->system_vars; *opt; opt++)
      {
        if ((((*opt)->flags & (PLUGIN_VAR_TYPEMASK | PLUGIN_VAR_NOCMDOPT | PLUGIN_VAR_MEMALLOC)) == PLUGIN_VAR_STR))
        {
    ...
    if (chain.first)
    {
      ...
      {
        sql_print_error("Plugin '%s' has conflicting system variables", tmp->name.str);
        goto err;
      }
...
{
  int flags = PLUGIN_VAR_THDLOCAL | PLUGIN_VAR_STR | PLUGIN_VAR_NOSYSVAR | PLUGIN_VAR_NOCMDOPT;
  char namebuf[256];
...
void wsrep_plugins_pre_init()
{
  ...
  global_system_variables.table_plugin = plugin_int_to_ref(wsrep_dummy_plugin_ptr);
}
