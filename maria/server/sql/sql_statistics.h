...
typedef enum enum_use_stat_tables_mode
{
  NEVER,
  COMPLEMENTARY,
  PREFERABLY,
...
typedef enum enum_histogram_type
{
  SINGLE_PREC_HB,
  DOUBLE_PREC_HB,
  JSON_HB,
...
inline Use_stat_tables_mode get_use_stat_tables_mode(THD *thd)
{
  return (Use_stat_tables_mode)(thd->variables.use_stat_tables);
}
inline bool check_eits_collection_allowed(THD *thd)
{
  return (get_use_stat_tables_mode(thd) == COMPLEMENTARY || get_use_stat_tables_mode(thd) == PREFERABLY);
}

inline bool check_eits_preferred(THD *thd)
{
  return (get_use_stat_tables_mode(thd) == PREFERABLY || get_use_stat_tables_mode(thd) == PREFERABLY_FOR_QUERIES);
}
...
{
 public:
  virtual bool parse(MEM_ROOT *mem_root, const char *db_name, const char *table_name, Field *field,
                     Histogram_type type_arg, const char *hist_data, size_t hist_data_len) = 0;
  ...
  */
  ...
  virtual Histogram_builder *create_builder(Field *col, uint col_len, ha_rows rows) = 0;

  /*
    This function checks that histograms should be usable only when
      1) the level of optimizer_use_condition_selectivity > 3
  */
  bool is_usable(THD *thd) { return thd->variables.optimizer_use_condition_selectivity > 3; }

  virtual double point_selectivity(Field *field, key_range *endpoint, double avg_sel) = 0;
  virtual double range_selectivity(Field *field, key_range *min_endp, key_range *max_endp, double avg_sel) = 0;
...
class Histogram_binary : public Histogram_base
{
 ...
 private:
  ...
  uint find_bucket(double pos, bool first)
  {
    ...
    if (val == get_value(i))
    {
      if (first)
      {
        while (i && val == get_value(i - 1)) i--;
      }
      else
      {
        while (i + 1 < get_width() && val == get_value(i + 1)) i++;
      }
    }
    return i;
  }
 ...
 public:
  ...
  bool parse(MEM_ROOT *mem_root, const char *, const char *, Field *, Histogram_type type_arg, const char *hist_data,
...
class Basic_stats_collector
{
 ...
 public:
  ...
  ulonglong get_count_single_occurence() const { return count_distinct_single_occurence; }
...
class Histogram_builder
{
 protected:
  ...
  Histogram_builder(Field *col, uint col_len, ha_rows rows) : column(col), col_length(col_len), records(rows) {}
...
class Column_statistics
{
 ...
 public:
  ...
  {
    return ((1 << (COLUMN_STAT_HISTOGRAM - COLUMN_STAT_COLUMN_NAME)) - 1) << (COLUMN_STAT_COLUMN_NAME + 1);
  }

  void set_all_nulls() { column_stat_nulls = no_values_provided_bitmap(); }

  void set_not_null(uint stat_field_no) { column_stat_nulls &= ~(1 << stat_field_no); }

  bool is_null(uint stat_field_no) { return MY_TEST(column_stat_nulls & (1 << stat_field_no)); }

  double get_nulls_ratio() { return (double)nulls_ratio / Scale_factor_nulls_ratio; }

  double get_avg_length() { return (double)avg_length / Scale_factor_avg_length; }

  double get_avg_frequency() { return (double)avg_frequency / Scale_factor_avg_frequency; }

  void set_nulls_ratio(double val) { nulls_ratio = (ulong)(val * Scale_factor_nulls_ratio); }

  void set_avg_length(double val) { avg_length = (ulonglong)(val * Scale_factor_avg_length); }

  void set_avg_frequency(double val) { avg_frequency = (ulonglong)(val * Scale_factor_avg_frequency); }

  bool min_max_values_are_provided() { return !is_null(COLUMN_STAT_MIN_VALUE) && !is_null(COLUMN_STAT_MAX_VALUE); }
  /*
...
class Index_statistics
{
 ...
 public:
  ...
  double get_avg_frequency(uint i) { return (double)avg_frequency[i] / Scale_factor_avg_frequency; }

  void set_avg_frequency(uint i, double val) { avg_frequency[i] = (ulonglong)(val * Scale_factor_avg_frequency); }
};
