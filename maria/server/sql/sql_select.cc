...
static bool make_join_statistics(JOIN *join, List<TABLE_LIST> &leaves, DYNAMIC_ARRAY *keyuse);
static bool update_ref_and_keys(THD *thd, DYNAMIC_ARRAY *keyuse, JOIN_TAB *join_tab, uint tables, COND *conds,
                                table_map table_map, SELECT_LEX *select_lex, SARGABLE_PARAM **sargables);
static int sort_keyuse(KEYUSE *a, KEYUSE *b);
static bool are_tables_local(JOIN_TAB *jtab, table_map used_tables);
static bool create_ref_for_key(JOIN *join, JOIN_TAB *j, KEYUSE *org_keyuse, bool allow_full_scan,
...
static bool greedy_search(JOIN *join, table_map remaining_tables, uint depth, uint use_cond_selectivity);
...
static enum_best_search best_extension_by_limited_search(JOIN *join, table_map remaining_tables, uint idx,
                                                         double record_count, double read_time, uint depth,
                                                         uint use_cond_selectivity, table_map *processed_eq_ref_tables);
...
static Item *make_cond_after_sjm(THD *thd, Item *root_cond, Item *cond, table_map tables, table_map sjm_tables,
                                 bool inside_or_clause);
...
static int return_zero_rows(JOIN *join, select_result *res, List<TABLE_LIST> &tables, List<Item> &fields, bool send_row,
                            ulonglong select_options, const char *info, Item *having, List<Item> &all_fields);
static COND *build_equal_items(JOIN *join, COND *cond, COND_EQUAL *inherited, List<TABLE_LIST> *join_list,
                               bool ignore_on_conds, COND_EQUAL **cond_equal_ref, bool link_equal_fields = FALSE);
static COND *substitute_for_best_equal_field(THD *thd, JOIN_TAB *context_tab, COND *cond, COND_EQUAL *cond_equal,
                                             void *table_join_idx, bool do_substitution);
static COND *simplify_joins(JOIN *join, List<TABLE_LIST> *join_list, COND *conds, bool top, bool in_sj);
...
static COND *optimize_cond(JOIN *join, COND *conds, List<TABLE_LIST> *join_list, bool ignore_on_conds,
                           Item::cond_result *cond_value, COND_EQUAL **cond_equal, int flags = 0);
...
static COND *make_cond_for_table(THD *thd, Item *cond, table_map table, table_map used_table, int join_tab_idx_arg,
                                 bool exclude_expensive_cond, bool retain_ref_cond);
static COND *make_cond_for_table_from_pred(THD *thd, Item *root_cond, Item *cond, table_map tables,
                                           table_map used_table, int join_tab_idx_arg, bool exclude_expensive_cond,
                                           bool retain_ref_cond, bool is_top_and_level);

static Item *part_of_refkey(TABLE *form, Field *field);
uint find_shortest_key(TABLE *table, const key_map *usable_keys);
static bool test_if_cheaper_ordering(const JOIN_TAB *tab, ORDER *order, TABLE *table, key_map usable_keys, int key,
                                     ha_rows select_limit, int *new_key, int *new_key_direction,
                                     ha_rows *new_select_limit, uint *new_used_key_parts = NULL,
...
static int remove_dup_with_compare(THD *thd, TABLE *entry, Field **field, Item *having);
...
static ORDER *create_distinct_group(THD *thd, Ref_ptr_array ref_pointer_array, ORDER *order, List<Item> &fields,
                                    List<Item> &all_fields, bool *all_order_by_fields_used);
static bool test_if_subpart(ORDER *group_by, ORDER *order_by);
static TABLE *get_sort_by_table(ORDER *a, ORDER *b, List<TABLE_LIST> &tables, table_map const_tables);
static void calc_group_buffer(JOIN *join, ORDER *group);
static bool make_group_fields(JOIN *main_join, JOIN *curr_join);
static bool alloc_group_fields(JOIN *join, ORDER *group);
static bool alloc_order_fields(JOIN *join, ORDER *group, uint max_number_of_elements);
// Create list for using with tempory table
static bool change_to_use_tmp_fields(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &new_list1,
                                     List<Item> &new_list2, uint elements, List<Item> &items);
// Create list for using with tempory table
static bool change_refs_to_tmp_fields(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &new_list1,
                                      List<Item> &new_list2, uint elements, List<Item> &items);
...
static bool prepare_sum_aggregators(THD *thd, Item_sum **func_ptr, bool need_distinct);
...
static JOIN_TAB *next_breadth_first_tab(JOIN_TAB *first_top_tab, uint n_top_tabs_count, JOIN_TAB *tab);
static bool find_order_in_list(THD *, Ref_ptr_array, TABLE_LIST *, ORDER *, List<Item> &, List<Item> &, bool, bool,
                               bool);

static double table_cond_selectivity(JOIN *join, uint idx, JOIN_TAB *s, table_map rem_tables);
...
static bool build_notnull_conds_for_range_scans(JOIN *join, COND *cond, table_map allowed);
static void build_notnull_conds_for_inner_nest_of_outer_join(JOIN *join, TABLE_LIST *nest_tbl);
...
static bool process_direct_rownum_comparison(THD *thd, SELECT_LEX_UNIT *unit, Item *cond);
...
void JOIN::init(THD *thd_arg, List<Item> &fields_arg, ulonglong select_options_arg, select_result *result_arg)
{
  join_tab = 0;
  table = 0;
  table_count = 0;
...
static void trace_table_dependencies(THD *thd, JOIN_TAB *join_tabs, uint table_count)
{
  ...
  for (uint i = 0; i < table_count; i++)
  {
    ...
    trace_one_table.add("row_may_be_null", (bool)table_ref->table->maybe_null);
    ...
    uint dep_bit;
    while ((dep_bit = it++) != Table_map_iterator::BITMAP_END) depends_on.add(static_cast<longlong>(dep_bit));
  }
}
...
bool handle_select(THD *thd, LEX *lex, select_result *result, ulonglong setup_tables_done_option)
{
  bool res;
  ...
  if (select_lex->master_unit()->is_unit_op() || select_lex->master_unit()->fake_select_lex)
  ...
  else
  {
    ...
    */
    res = mysql_select(thd, select_lex->table_list.first, select_lex->item_list, select_lex->where,
                       select_lex->order_list.elements + select_lex->group_list.elements, select_lex->order_list.first,
                       select_lex->group_list.first, select_lex->having, lex->proc_list.first,
                       select_lex->options | thd->variables.option_bits | setup_tables_done_option, result, unit,
                       select_lex);
  }
  DBUG_PRINT("info", ("res: %d  is_error(): %d", res, thd->is_error()));
  res |= thd->is_error();
  if (unlikely(res))
  ...
  if (unlikely(thd->killed == ABORT_QUERY && !thd->no_errors))
  {
    ...
    */
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_QUERY_EXCEEDED_ROWS_EXAMINED_LIMIT,
                        ER_THD(thd, ER_QUERY_EXCEEDED_ROWS_EXAMINED_LIMIT), thd->accessed_rows_and_keys,
...
bool fix_inner_refs(THD *thd, List<Item> &all_fields, SELECT_LEX *select, Ref_ptr_array ref_pointer_array)
{
  Item_outer_ref *ref;
  ...
  while ((ref = ref_it++))
  {
    ...
    */
    ...
    if (ref->in_sum_func)
    {
      ...
      {
        for (sum_func = ref->in_sum_func; sum_func && sum_func->aggr_level >= select->nest_level;
    ...
    new_ref = direct_ref
                  ? new (thd->mem_root) Item_direct_ref(thd, ref->context, item_ref, ref->table_name, ref->field_name,
                                                        ref->alias_name_used)
                  : new (thd->mem_root)
                        Item_ref(thd, ref->context, item_ref, ref->table_name, ref->field_name, ref->alias_name_used);
    if (!new_ref)
      return TRUE;
...
static void remove_redundant_subquery_clauses(st_select_lex *subq_select_lex)
{
  ...
  DBUG_ASSERT(subq_predicate->substype() == Item_subselect::EXISTS_SUBS || subq_predicate->is_in_predicate());
  ...
  /*
    ...
    clause
  */
  if (subq_select_lex->group_list.elements && !subq_select_lex->with_sum_func && !subq_select_lex->join->having)
  {
...
*/
static inline int setup_without_group(THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables,
                                      List<TABLE_LIST> &leaves, List<Item> &fields, List<Item> &all_fields,
                                      COND **conds, ORDER *order, ORDER *group, List<Window_spec> &win_specs,
                                      List<Item_window_func> &win_funcs, bool *hidden_group_fields, uint *reserved)
{
  int res;
  ...
  */
  ...
  thd->lex->allow_sum_func.set_bit(select->nest_level);
  res = res || setup_windows(thd, ref_pointer_array, tables, fields, all_fields, win_specs, win_funcs);
...
bool vers_select_conds_t::init_from_sysvar(THD *thd)
{
  ...
  if (type != SYSTEM_TIME_UNSPECIFIED && type != SYSTEM_TIME_ALL)
  {
    ...
    start.item = new (thd->mem_root) Item_datetime_literal(thd, &dt, TIME_SECOND_PART_DIGITS);
    if (!start.item)
      return true;
  }
  else
...
static Item *period_get_condition(THD *thd, TABLE_LIST *table, SELECT_LEX *select, vers_select_conds_t *conds,
                                  bool timestamp)
{
  DBUG_ASSERT(table);
...
#define newx new (thd->mem_root)
  ...
  conds->field_start =
      newx Item_field(thd, &select->context, table->db, table->alias, thd->strmake_lex_cstring(fstart));
  conds->field_end = newx Item_field(thd, &select->context, table->db, table->alias, thd->strmake_lex_cstring(fend));
  ...
  else
  {
    ...
    if (conds->start.item && conds->start.unit == VERS_TIMESTAMP)
    {
      ...
      trx_id0 = newx Item_func_trt_id(thd, conds->start.item, TR_table::FLD_TRX_ID, backwards);
    }
    if (conds->end.item && conds->end.unit == VERS_TIMESTAMP)
    {
      trx_id1 = newx Item_func_trt_id(thd, conds->end.item, TR_table::FLD_TRX_ID, false);
    }
...
static bool skip_setup_conds(THD *thd)
{
  return (!thd->stmt_arena->is_conventional() && !thd->stmt_arena->is_stmt_prepare_or_first_sp_execute()) ||
...
int SELECT_LEX::period_setup_conds(THD *thd, TABLE_LIST *tables)
{
  ...
  for (TABLE_LIST *table = tables; table; table = table->next_local)
  {
    ...
    if (update_conds)
    {
      ...
      result = and_items(thd, result, period_get_condition(thd, table, this, &conds, true));
    }
  }
  if (update_conds)
...
int SELECT_LEX::vers_setup_conds(THD *thd, TABLE_LIST *tables)
{
  ...
  if (!versioned_tables)
  {
    for (TABLE_LIST *table = tables; table; table = table->next_local)
    {
      if (table->table && table->table->versioned())
        versioned_tables++;
      else if (table->vers_conditions.is_set() && (table->is_non_derived() || !table->vers_conditions.used))
      {
  ...
  for (TABLE_LIST *table = tables; table; table = table->next_local)
  {
    ...
    */
    ...
    {
      if (vers_conditions.was_set() && table->lock_type >= TL_FIRST_WRITE && !vers_conditions.delete_history)
      {
    ...
    if (vers_conditions.is_set() && vers_conditions.type != SYSTEM_TIME_HISTORY)
    {
      ...
      if (vers_conditions.check_units(thd))
        DBUG_RETURN(-1);
      if (timestamps_only && (vers_conditions.start.unit == VERS_TRX_ID || vers_conditions.end.unit == VERS_TRX_ID))
      {
    ...
    if (update_conds)
    {
      ...
      Item *cond = period_get_condition(thd, table, this, &vers_conditions, timestamps_only);
      if (is_select)
...
/**
  ...
  @retval
    0   on success
*/
int JOIN::prepare(TABLE_LIST *tables_init, COND *conds_init, uint og_num, ORDER *order_init, bool skip_order_by,
                  ORDER *group_init, Item *having_init, ORDER *proc_param_init, SELECT_LEX *select_lex_arg,
                  SELECT_LEX_UNIT *unit_arg)
{
  ...
  DBUG_PRINT("info", ("select %p (%u) = JOIN %p", select_lex, select_lex->select_number, this));
  ...
  */
  ...
  if (!(select_options & OPTION_SETUP_TABLES_DONE) &&
      setup_tables_and_check_access(thd, &select_lex->context, join_list, tables_list, select_lex->leaf_tables, FALSE,
                                    SELECT_ACL, SELECT_ACL, FALSE))
    DBUG_RETURN(-1);
  ...
  */
  ...
  if ((~thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY) && select_lex->with_sum_func && !group_list)
  {
  ...
  enum_parsing_place save_place = thd->lex->current_select->context_analysis_place;
...

  {
    ...
    while ((tbl = it++))
    {
      if (tbl->table_function && tbl->table_function->setup(thd, tbl, select_lex_arg))
        DBUG_RETURN(-1);
    }
  }

  if (setup_fields(thd, ref_ptrs, fields_list, MARK_COLUMNS_READ, &all_fields, &select_lex->pre_fix, 1))
    DBUG_RETURN(-1);
  thd->lex->current_select->context_analysis_place = save_place;
  rand_table_in_field_list = select_lex->select_list_tables & RAND_TABLE_BIT;

  if (setup_without_group(thd, ref_ptrs, tables_list, select_lex->leaf_tables, fields_list, all_fields, &conds, order,
                          group_list, select_lex->window_specs, select_lex->window_funcs, &hidden_group_fields,
  ...
  */
  ...
  if (skip_order_by && select_lex != select_lex->master_unit()->global_parameters())
  {
    ...
    for (ORDER *order = select_lex->order_list.first; order; order = order->next)
    {
      ...
      if (find_order_in_list(thd, ref_ptrs, tables_list, order, fields_list, all_fields, false, false, false))
        DBUG_RETURN(-1);
    }
    thd->lex->allow_sum_func = save_allow_sum_func;
  ...
  if (having)
  {
    ...
    */
    if (having->type() == Item::REF_ITEM && ((Item_ref *)having)->ref_type() == Item_ref::REF)
  ...
  */
  ...
  if (with_elem && select_lex->check_unrestricted_recursive(thd->variables.only_standard_compliant_cte))
    DBUG_RETURN(-1);
    ...
    for (ORDER *ord = order; ord; ord = ord->next)
    {
      ...
      */
      if (!requires_sorting &&
          ...
          ((item->type() != Item::FIELD_ITEM || ((Item_field *)item)->field->maybe_null() ||
            ((Item_field *)item)->field->sort_length()) &&
           /* AND not a zero length NOT NULL string function. */
           (item->type() != Item::FUNC_ITEM || item->maybe_null() || item->result_type() != STRING_RESULT ||
            item->max_length)))
        requires_sorting = TRUE;

      if ((item->with_sum_func() && item->type() != Item::SUM_FUNC_ITEM) || item->with_window_func())
  ...
  if (having && (having->with_sum_func() || having->with_rownum_func()))
    having->split_sum_func2(thd, ref_ptrs, all_fields, &having, SPLIT_SUM_SKIP_REGISTERED);
  if (select_lex->inner_sum_func_list)
  {
    ...
    do
    {
      ...
      item_sum->split_sum_func2(thd, ref_ptrs, all_fields, item_sum->ref_by, 0);
    } while (item_sum != end);
  }

  if (select_lex->inner_refs_list.elements && fix_inner_refs(thd, all_fields, select_lex, ref_ptrs))
    DBUG_RETURN(-1);
  ...
  if (group_list)
  {
    ...
    */
    ...
    {
      if ((*ord->item)->type() == Item::FIELD_ITEM && (*ord->item)->field_type() == MYSQL_TYPE_BIT)
      {
  ...
  */
  if (thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY && !group_list &&
      !(select_lex->master_unit()->item && select_lex->master_unit()->item->is_in_predicate() &&
        select_lex->master_unit()->item->get_IN_subquery()->test_set_strategy(SUBS_MAXMIN_INJECTED)) &&
      select_lex->non_agg_field_used() && select_lex->agg_func_used())
  {
    my_message(ER_MIX_OF_GROUP_FUNC_AND_FIELDS, ER_THD(thd, ER_MIX_OF_GROUP_FUNC_AND_FIELDS), MYF(0));
    DBUG_RETURN(-1);
  }
  {
  ...
  if (procedure)
  {
    ...
    if (procedure->group)
    {
      ...
      {                                                                            /* purecov: inspected */
        my_message(ER_DIFF_GROUPS_PROC, ER_THD(thd, ER_DIFF_GROUPS_PROC), MYF(0)); /* purecov: inspected */
        goto err;                                                                  /* purecov: inspected */
      }
    }
    if (order && (procedure->flags & PROC_NO_SORT))
    {                                                                          /* purecov: inspected */
      my_message(ER_ORDER_WITH_PROC, ER_THD(thd, ER_ORDER_WITH_PROC), MYF(0)); /* purecov: inspected */
      goto err;                                                                /* purecov: inspected */
    }
    if (thd->lex->derived_tables)
    {
      ...
      */
      my_error(ER_WRONG_USAGE, MYF(0), "PROCEDURE", thd->lex->derived_tables & DERIVED_VIEW ? "view" : "subquery");
      goto err;
    }
...
bool JOIN::prepare_stage2()
{
  ...
  if (select_lex->olap == ROLLUP_TYPE && rollup_init())
    goto err;
  if (alloc_func_list() || make_sum_func_list(all_fields, fields_list, false))
    goto err;
...
bool JOIN::build_explain()
{
  ...
  */
  ...
  bool res = save_explain_data(thd->lex->explain, false /* can overwrite */, need_tmp,
                               !skip_sort_order && !no_order && (order || group_list), select_distinct);
  ...
  for (uint i = 0; i < aggr_tables; i++, curr_tab++)
  {
    if (select_nr == FAKE_SELECT_LEX_ID)
    {
      ...
      curr_tab->tracker = thd->lex->explain->get_union(select_nr)->get_tmptable_read_tracker();
    }
...
int JOIN::optimize()
{
  ...
  if (!with_two_phase_optimization || init_state == JOIN::OPTIMIZATION_PHASE_1_DONE)
  {
...
bool JOIN::make_range_rowid_filters()
{
  ...
  */
       ...
       tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS))
  {
    ...
    DBUG_ASSERT(!(tab->ref.key >= 0 && tab->ref.key == (int)tab->range_rowid_filter_info->key_no));
    DBUG_ASSERT(!(tab->ref.key == -1 && tab->quick && tab->quick->index == tab->range_rowid_filter_info->key_no));

    int err;
    SQL_SELECT *sel = NULL;
    Rowid_filter_container *filter_container = NULL;
    Item **sargable_cond = get_sargable_cond(this, tab->table);
    sel = make_select(tab->table, const_table_map, const_table_map, *sargable_cond, (SORT_INFO *)0, 1, &err);
    if (!sel)
      continue;
    ...
    int rc = sel->test_quick_select(thd, filter_map, (table_map)0, (ha_rows)HA_POS_ERROR, true, false, true, true);
    ...
    */
    ...
    filter_container = tab->range_rowid_filter_info->create_container();
    if (filter_container)
    {
      tab->rowid_filter =
          new (thd->mem_root) Range_rowid_filter(tab->table, tab->range_rowid_filter_info, filter_container, sel);
      if (tab->rowid_filter)
        continue;
    }
  no_filter:
...
bool JOIN::init_range_rowid_filters()
{
...
int JOIN::optimize_inner()
{
  ...
  if (unlikely(thd->lex->with_rownum && select_lex->first_cond_optimization && select_lex->master_unit()->derived))
  ...
  */
  ...
  if (select_lex->first_cond_optimization && transform_in_predicates_into_in_subq(thd))
    DBUG_RETURN(1);
  ...
  */
  if (select_lex->first_cond_optimization && conds && conds->walk(&Item::exists2in_processor, 0, thd))
    DBUG_RETURN(1);
  /*
    TODO
    make view to decide if it is possible to write to WHERE directly or make Semi-Joins able to process ON condition if
  it is possible for (TABLE_LIST *tbl= tables_list; tbl; tbl= tbl->next_local)
  {
    if (tbl->on_expr &&
  ...
  */
  ...
  if (select_lex->options & OPTION_SCHEMA_TABLE && optimize_schema_tables_memory_usage(select_lex->leaf_tables))
    DBUG_RETURN(1);

  if (setup_ftfuncs(select_lex)) /* should be after having->fix_fields */
    DBUG_RETURN(-1);

  row_limit = ((select_distinct || order || group_list) ? HA_POS_ERROR : unit->lim.get_select_limit());
  ...
  if (conds && conds->with_subquery())
    (void)conds->walk(&Item::cleanup_is_expensive_cache_processor, 0, (void *)0);
  if (having && having->with_subquery())
    (void)having->walk(&Item::cleanup_is_expensive_cache_processor, 0, (void *)0);
  ...
  if (eq_list.elements != 0)
  {
    ...
    if (new_cond && ((new_cond->fix_fields(thd, &new_cond) || !(conds = and_items(thd, conds, new_cond)) ||
                      conds->fix_fields(thd, &conds))))
      DBUG_RETURN(TRUE);
  }
  eq_list.empty();
  ...
  */
  ...
  conds = optimize_cond(this, conds, join_list, ignore_on_expr, &cond_value, &cond_equal, OPT_LINK_EQUAL_FIELDS);
  ...
  if (select_lex->with_rownum && !order && !group_list && !select_distinct && conds &&
      select_lex == unit->global_parameters())
    optimize_rownum(thd, unit, conds);

  having = optimize_cond(this, having, join_list, TRUE, &having_value, &having_equal);
  ...
  if (thd->lex->sql_command == SQLCOM_SELECT && optimizer_flag(thd, OPTIMIZER_SWITCH_COND_PUSHDOWN_FROM_HAVING) &&
      cond_value != Item::COND_FALSE)
  {
    having = select_lex->pushdown_from_having_into_where(thd, having);
    if (select_lex->attach_to_conds.elements != 0)
    {
      conds = and_new_conditions_to_optimized_cond(thd, conds, &cond_equal, select_lex->attach_to_conds, &cond_value);
  ...
  if (eq_list.elements != 0)
  {
    conds = and_new_conditions_to_optimized_cond(thd, conds, &cond_equal, eq_list, &cond_value);

    if (!conds && cond_value != Item::COND_FALSE && cond_value != Item::COND_TRUE)
      DBUG_RETURN(TRUE);
  }
    ...
    while ((tbl = li++))
    {
      ...
      */
      if (tbl->is_materialized_derived())
      {
        ...
        if (join && join->optimization_state == JOIN::OPTIMIZATION_PHASE_1_DONE && join->with_two_phase_optimization)
          continue;
        /*
          Do not push conditions from where into materialized inner tables
          of outer joins: this is not valid.
  ...
  }
  {
    ...
    if (cond_value == Item::COND_FALSE || having_value == Item::COND_FALSE ||
        (!unit->lim.get_select_limit() && !(select_options & OPTION_FOUND_ROWS)))
    { /* Impossible cond */
      if (unit->lim.get_select_limit())
      {
        DBUG_PRINT("info", (having_value == Item::COND_FALSE ? "Impossible HAVING" : "Impossible WHERE"));
        zero_result_cause = having_value == Item::COND_FALSE ? "Impossible HAVING" : "Impossible WHERE";
      }
      else
      {
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  {
    ...
    while ((tbl = li++))
    {
      ...
      tbl->table->all_partitions_pruned_away = prune_partitions(thd, tbl->table, *prune_cond);
    }
  }
#endif
    ...
    */
    ...
    if (res)
    {
      ...
      */
      ...
      {
        COND *table_independent_conds = make_cond_for_table(thd, conds, PSEUDO_TABLE_BITS, 0, -1, FALSE, FALSE);
        DBUG_EXECUTE("where", print_where(table_independent_conds, "where after opt_sum_query()", QT_ORDINARY););
  ...
  */
  ...
  {
    group_list = remove_const(this, group_list, conds, rollup.state == ROLLUP::STATE_NONE, &simple_group);
  ...
  if (unlikely(make_join_statistics(this, select_lex->leaf_tables, &keyuse)) || unlikely(thd->is_fatal_error))
  {
...
int JOIN::optimize_stage2()
{
  ...
  if (!(thd->variables.option_bits & OPTION_BIG_SELECTS) && best_read > (double)thd->variables.max_join_size &&
  ...
  if (const_tables && !thd->locked_tables_mode && !(select_options & SELECT_NO_UNLOCK))
  {
    /*
    ...
    */
    (void)mysql_unlock_some_tables(thd, table, const_tables, GET_LOCK_SKIP_SEQUENCES);
  }
  if (!conds && outer_join)
  {
  ...
  select = make_select(*table, const_table_map, const_table_map, conds, (SORT_INFO *)0, 1, &error);
  if (unlikely(error))
  {             /* purecov: inspected */
  ...
  */
  if (conds)
  {
    conds = substitute_for_best_equal_field(thd, NO_PARTICULAR_TAB, conds, cond_equal, map2table, true);
    if (unlikely(thd->is_error()))
    {
      error = 1;
    ...
    DBUG_EXECUTE("where", print_where(conds, "after substitute_best_equal", QT_ORDINARY););
  }
  if (having)
  {
    having = substitute_for_best_equal_field(thd, NO_PARTICULAR_TAB, having, having_equal, map2table, false);
    if (thd->is_error())
    {
      error = 1;
      DBUG_PRINT("error", ("Error from substitute_for_best_equal"));
    ...
    DBUG_EXECUTE("having", print_where(having, "after substitute_best_equal", QT_ORDINARY););
  }
  ...
  */
       ...
       tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS))
  {
    ...
    {
      *tab->on_expr_ref =
          substitute_for_best_equal_field(thd, NO_PARTICULAR_TAB, *tab->on_expr_ref, tab->cond_equal, map2table, true);
      if (unlikely(thd->is_error()))
      {
        error = 1;
        DBUG_PRINT("error", ("Error from substitute_for_best_equal"));
      ...
      if (unlikely(thd->trace_started()))
      {
        trace_condition(thd, "ON expr", "substitute_best_equal", (*tab->on_expr_ref), tab->table->alias.c_ptr());
      }
    }
  }
  ...
  */
       ...
       tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS))
  {
    ...
    for (uint i = 0; i < tab->ref.key_parts; i++)
    {
      ...
      while (equals)
      {
        ref_item = substitute_for_best_equal_field(thd, tab, ref_item, equals, map2table, true);
        ...
        *ref_item_ptr = ref_item;
        ...
        if (key_copy->type() == store_key::FIELD_STORE_KEY)
        {
          if (item->basic_const_item())
          {
            ...
            tab->ref.key_copy[key_copy_index] = new store_key_const_item(*tab->ref.key_copy[key_copy_index], item);
          }
          else if (item->const_item())
          {
            tab->ref.key_copy[key_copy_index] = new store_key_item(*tab->ref.key_copy[key_copy_index], item, TRUE);
          }
          else
          {
  ...
  if (conds && const_table_map != found_const_table_map && (select_options & SELECT_DESCRIBE))
  ...
  */
  if (unit->lim.is_with_ties())
  {
    ...
    for (ORDER *it = order; it; it = it->next) with_ties_order_count += 1;
  }
  ...
  */
  if (table_count - const_tables == 1 && (group || select_distinct) && !tmp_table_param.sum_func_count &&
      (!join_tab[const_tables].select || !join_tab[const_tables].select->quick ||
       join_tab[const_tables].select->quick->get_type() != QUICK_SELECT_I::QS_TYPE_GROUP_MIN_MAX) &&
      !select_lex->have_window_funcs())
  {
    if (group && rollup.state == ROLLUP::STATE_NONE &&
        list_contains_unique_index(join_tab[const_tables].table, find_field_in_order_list, (void *)group_list))
    {
      /*
      ...
      {
        if (skip_sort_order || (select_lex->master_unit()->item && select_limit == HA_POS_ERROR))  // This is a subquery
          order = NULL;
        else
          order = group_list;
      }
      ...
      */
      join_tab->table->keys_in_use_for_order_by = join_tab->table->keys_in_use_for_group_by;
      group_list = 0;
      group = 0;
    }
    if (select_distinct &&
        list_contains_unique_index(join_tab[const_tables].table, find_field_in_item_list, (void *)&fields_list))
    {
      select_distinct = 0;
    }
  }
  ...
  {
    if (!hidden_group_fields && rollup.state == ROLLUP::STATE_NONE && !select_lex->have_window_funcs())
      select_distinct = 0;
  }
  else if (select_distinct && table_count - const_tables == 1 && rollup.state == ROLLUP::STATE_NONE &&
           !select_lex->have_window_funcs())
  {
    /*
    ...
    */
    ...
    if (order)
    {
      skip_sort_order = test_if_skip_sort_order(tab, order, select_limit,
                                                true,  // no_changes
                                                &tab->table->keys_in_use_for_order_by);
    }
    if ((group_list = create_distinct_group(thd, select_lex->ref_pointer_array, order, fields_list, all_fields,
                                            &all_order_fields_used)))
    {
      const bool skip_group = skip_sort_order && test_if_skip_sort_order(tab, group_list, select_limit,
                                                                         true,  // no_changes
  ...
  */
  if (order && (test_if_subpart(group_list, order) || (!group_list && tmp_table_param.sum_func_count)))
    order = 0;

  // Can't use sort on head table if using join buffering
  if (full_join || hash_join)
  {
    TABLE *stable = (sort_by_table == (TABLE *)1 ? join_tab[const_tables].table : sort_by_table);
    /*
      FORCE INDEX FOR ORDER BY can be used to prevent join buffering when
      sorting on the first table.
    */
    if (!stable || (!stable->force_index_order && !map2table[stable->tablenr]->keep_current_rowid))
    {
      if (group_list)
        simple_group = 0;
      if (order)
  ...
  */
  ...
  select_opts_for_readinfo = (select_options & (SELECT_DESCRIBE | SELECT_NO_JOIN_CACHE)) |
                             (select_lex->ftfunc_list->elements ? SELECT_NO_JOIN_CACHE : 0);

  if (select_lex->options & OPTION_SCHEMA_TABLE && optimize_schema_tables_reads(this))
    DBUG_RETURN(1);
  ...
  */
  ...
  if (!only_const_tables())
  {
    ...
    */
    if ((order || group_list) && tab->type != JT_ALL && tab->type != JT_FT && tab->type != JT_REF_OR_NULL &&
    ...
    */
      ...
      */
           ...
           tab->select->quick->get_type() == QUICK_SELECT_I::QS_TYPE_GROUP_MIN_MAX))
      {
            ...
            !select_distinct)  // .. if not preceded by a DISTINCT
        {
          ...
          if (test_if_skip_sort_order(tab, group_list, limit, false, &tab->table->keys_in_use_for_group_by))
          {
        ...
        if ((ordered_index_usage != ordered_index_group_by) &&
            ((tmp_table_param.quick_group && !procedure) ||
             (tab->emb_sj_nest && best_positions[const_tables].sj_strategy == SJ_OPT_LOOSE_SCAN)))
        {
          need_tmp = 1;
    ...
    {
      if (test_if_skip_sort_order(tab, order, select_limit, false, &tab->table->keys_in_use_for_order_by))
      {
...
bool JOIN::add_having_as_table_cond(JOIN_TAB *tab)
{
  ...
  Item *sort_table_cond = make_cond_for_table(thd, tmp_having, used_tables, (table_map)0, 0, false, false);
  if (sort_table_cond)
  {
    if (!tab->select)
    {
    ...
    {
      if (!(tab->select->cond = new (thd->mem_root) Item_cond_and(thd, tab->select->cond, sort_table_cond)))
        DBUG_RETURN(true);
    }
    if (tab->pre_idx_push_select_cond)
    {
      ...
      if (!(tab->pre_idx_push_select_cond =
                new (thd->mem_root) Item_cond_and(thd, tab->pre_idx_push_select_cond, sort_table_cond)))
        DBUG_RETURN(true);
    }
    if (tab->select->cond)
    ...
    DBUG_EXECUTE("where", print_where(tab->select->cond, "select and having", QT_ORDINARY););

    having = make_cond_for_table(thd, tmp_having, ~(table_map)0, ~used_tables, 0, false, false);
    DBUG_EXECUTE("where", print_where(having, "having after sort", QT_ORDINARY););
  }
...
bool JOIN::make_aggr_tables_info()
{
  ...
  bool implicit_grouping_with_window_funcs = implicit_grouping && select_lex->have_window_funcs();
  bool implicit_grouping_without_tables = implicit_grouping && !tables_list;
    ...
    if (ht && ht->create_group_by)
    {
      ...
      */
      Query query = {&all_fields, select_distinct || group_optimized_away,
                     tables_list, conds,
                     group_list,  order ? order : group_list,
                     having,      &select_lex->master_unit()->lim};
      ...
      if (gbh)
      {
        ...
        */
        ...
        if (!(curr_tab->tmp_table_param = new TMP_TABLE_PARAM(tmp_table_param)))
          DBUG_RETURN(1);
        TABLE *table = create_tmp_table(thd, curr_tab->tmp_table_param, all_fields, NULL, distinct, TRUE,
                                        select_options, HA_POS_ERROR, &empty_clex_str, !need_tmp, keep_row_order);
        if (!table)
          DBUG_RETURN(1);
  ...
  */
  if (join_tab && top_join_tab_count && tables_list && join_tab->is_using_loose_index_scan())
    tmp_table_param.precomputed_group_by = !join_tab->is_using_agg_loose_index_scan();
  ...
  if (need_tmp)
  {
    ...
    */
    ...
    distinct = select_distinct && !group_list && !select_lex->have_window_funcs();
    keep_row_order = thd->lex->with_rownum && (group_list || order);
    bool save_sum_fields = (group_list && simple_group) || implicit_grouping_with_window_funcs;
    if (create_postjoin_aggr_table(curr_tab, &all_fields, tmp_group, save_sum_fields, distinct, keep_row_order))
      DBUG_RETURN(true);
    ...
    items1 = ref_ptr_array_slice(2);
    if ((sort_and_group || curr_tab->table->group || tmp_table_param.precomputed_group_by) &&
    ...
    /*
      ...
      to the client.
    */
    if (tmp_having && (sort_and_group || (exec_tmp_table->distinct && !group_list) || select_lex->have_window_funcs()))
    {
      /*
      ...
      */
      if (!select_distinct && !select_lex->have_window_funcs() && add_having_as_table_cond(curr_tab))
        DBUG_RETURN(true);
    ...
    */
    ...
    { /* Must copy to another table */
      ...
      calc_group_buffer(this, group_list);
      count_field_types(select_lex, &tmp_table_param, tmp_all_fields1, select_distinct && !group_list);
      tmp_table_param.hidden_field_count = tmp_all_fields1.elements - tmp_fields_list1.elements;
      ...
      */
      ...
      tmp_table_param.hidden_field_count = curr_all_fields->elements - curr_fields_list->elements;
      ORDER *dummy = NULL;  // TODO can use table->group here also

      if (create_postjoin_aggr_table(curr_tab, curr_all_fields, dummy, true, distinct, keep_row_order))
        DBUG_RETURN(true);
      ...
      if (group_list || tmp_table_param.sum_func_count)
      {
        if (make_sum_func_list(*curr_all_fields, *curr_fields_list, true))
          DBUG_RETURN(true);
        if (prepare_sum_aggregators(thd, sum_funcs, !join_tab->is_using_agg_loose_index_scan()))
          DBUG_RETURN(true);
        group_list = NULL;
    ...
    tmp_table_param.field_count = tmp_table_param.sum_func_count = tmp_table_param.func_count = 0;
  ...
  if (group || (implicit_grouping && !implicit_grouping_with_window_funcs) || tmp_table_param.sum_func_count)
  {
    if (make_group_fields(this, this))
      DBUG_RETURN(true);
    ...
    setup_copy_fields(thd, &tmp_table_param, items3, tmp_fields_list3, tmp_all_fields3, curr_fields_list->elements,
                      *curr_all_fields);
    ...
    if (make_sum_func_list(*curr_all_fields, *curr_fields_list, true))
      DBUG_RETURN(true);
    if (prepare_sum_aggregators(thd, sum_funcs, !join_tab || !join_tab->is_using_agg_loose_index_scan()))
      DBUG_RETURN(true);
  ...
  if (group_list || order)
  {
    ...
    if (tmp_having && !is_having_added_as_table_cond && !group_list && !sort_and_group)
    {
    ...
    */
    ...
    if (top_join_tab_count + aggr_tables > const_tables &&
        ordered_index_usage != (group_list ? ordered_index_group_by : ordered_index_order_by) &&
        curr_tab->type != JT_CONST && curr_tab->type != JT_EQ_REF)  // Don't sort 1 row
    {
      ...
      */
      sort_tab->filesort->limit = (has_group_by || (join_tab + top_join_tab_count > curr_tab + 1))
                                      ? select_limit
    ...
    if (!only_const_tables() && !join_tab[const_tables].filesort && !(select_options & SELECT_DESCRIBE))
    {
      /*
  ...
  */
  ...
  if (select_lex->window_funcs.elements)
  {
    if (!(curr_tab->window_funcs_step = new Window_funcs_computation))
      DBUG_RETURN(true);
    if (curr_tab->window_funcs_step->setup(thd, &select_lex->window_funcs, curr_tab))
      DBUG_RETURN(true);
  ...
  */
  ...
  if (join_tab)
    join_tab[exec_join_tab_cnt() + aggr_tables - 1].next_select = setup_end_select_func(this, NULL);
  group = has_group_by;

  DBUG_RETURN(false);
}

bool JOIN::create_postjoin_aggr_table(JOIN_TAB *tab, List<Item> *table_fields, ORDER *table_group, bool save_sum_fields,
                                      bool distinct, bool keep_row_order)
{
  ...
  /*
    ...
    WITH TIES.
  */
  ha_rows table_rows_limit =
      ((order == NULL || skip_sort_order) && !table_group && !select_lex->with_sum_func && !unit->lim.is_with_ties())
          ? select_limit
          : HA_POS_ERROR;
  ...
  TABLE *table = create_tmp_table(thd, tab->tmp_table_param, *table_fields, table_group, distinct, save_sum_fields,
                                  select_options, table_rows_limit, &empty_clex_str, true, keep_row_order);
  if (!table)
    DBUG_RETURN(true);
  tmp_table_param.using_outer_summary_function = tab->tmp_table_param->using_outer_summary_function;
  tab->join = this;
  ...
  if ((group_list && simple_group) || (implicit_grouping && select_lex->have_window_funcs()))
  {
    DBUG_PRINT("info", ("Sorting for group"));
    THD_STAGE_INFO(thd, stage_sorting_for_group);

    if (ordered_index_usage != ordered_index_group_by && !only_const_tables() &&
        (join_tab + const_tables)->type != JT_CONST &&  // Don't sort 1 row
        !implicit_grouping && add_sorting_to_table(join_tab + const_tables, group_list))
      goto err;
    ...
    if (make_sum_func_list(all_fields, fields_list, true))
      goto err;
    if (prepare_sum_aggregators(thd, sum_funcs, !(tables_list && join_tab->is_using_agg_loose_index_scan())))
      goto err;
    if (setup_sum_funcs(thd, sum_funcs))
      goto err;
    group_list = NULL;
  ...
  else
  {
    ...
    if (!group_list && !table->distinct && order && simple_order && tab == join_tab + const_tables)
    {
      DBUG_PRINT("info", ("Sorting for order"));
      THD_STAGE_INFO(thd, stage_sorting_for_order);

      if (ordered_index_usage != ordered_index_order_by && !only_const_tables() &&
...
void JOIN::optimize_distinct()
{
  ...
  {
    if (select_lex->select_list_tables & last_join_tab->table->map || last_join_tab->use_join_cache)
      break;
...
{
  tab->filesort = new (thd->mem_root) Filesort(order, HA_POS_ERROR, tab->keep_current_rowid, tab->select);
  if (!tab->filesort)
    return true;

  TABLE *table = tab->table;
  if ((tab == join_tab + const_tables) && table->pos_in_table_list && table->pos_in_table_list->is_sjm_scan_table())
  {
...
bool JOIN::setup_subquery_caches()
{
  ...
  */
  if (select_lex->expr_cache_may_be_used[IN_WHERE] || select_lex->expr_cache_may_be_used[IN_HAVING] ||
      select_lex->expr_cache_may_be_used[IN_ON] || select_lex->expr_cache_may_be_used[NO_MATTER])
  {
    JOIN_TAB *tab;
    if (conds && !(conds = conds->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
      DBUG_RETURN(TRUE);
    for (tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
         tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS))
    {
      if (tab->select_cond &&
          !(tab->select_cond = tab->select_cond->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
        DBUG_RETURN(TRUE);
      if (tab->cache_select && tab->cache_select->cond)
        if (!(tab->cache_select->cond =
                  tab->cache_select->cond->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
          DBUG_RETURN(TRUE);
    }

    if (having && !(having = having->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
      DBUG_RETURN(TRUE);

    if (tmp_having)
    {
      DBUG_ASSERT(having == NULL);
      if (!(tmp_having = tmp_having->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
        DBUG_RETURN(TRUE);
    }
  }
  if (select_lex->expr_cache_may_be_used[SELECT_LIST] || select_lex->expr_cache_may_be_used[IN_GROUP_BY] ||
      select_lex->expr_cache_may_be_used[NO_MATTER])
  {
    ...
    {
      Item *new_item;
      if (!(new_item = item->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
        DBUG_RETURN(TRUE);
      if (new_item != item)
      {
    ...
    {
      if (!(*tmp_group->item = (*tmp_group->item)->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
        DBUG_RETURN(TRUE);
    }
  }
  if (select_lex->expr_cache_may_be_used[NO_MATTER])
  {
    for (ORDER *ord = order; ord; ord = ord->next)
    {
      if (!(*ord->item = (*ord->item)->transform(thd, &Item::expr_cache_insert_transformer, NULL)))
        DBUG_RETURN(TRUE);
    }
  }
  DBUG_RETURN(FALSE);
...
bool JOIN::shrink_join_buffers(JOIN_TAB *jt, ulonglong curr_space, ulonglong needed_space)
{
  JOIN_TAB *tab;
  JOIN_CACHE *cache;
  for (tab = first_linear_tab(this, WITHOUT_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab != jt;
...
int JOIN::reinit()
{
  ...
  if (no_rows_in_result_called)
  {
    ...
    Item *item;
    no_rows_in_result_called = 0;
    while ((item = it++)) item->restore_to_before_no_rows_in_result();
  }
...
bool JOIN::prepare_result(List<Item> **columns_list)
{
  ...
  if (!zero_result_cause && select_lex->handle_derived(thd->lex, DT_CREATE))
    goto err;

  if (result->prepare2(this))
    goto err;

  if ((select_lex->options & OPTION_SCHEMA_TABLE) && get_schema_tables_result(this, PROCESSED_BY_JOIN_EXEC))
    goto err;
...
bool JOIN::save_explain_data(Explain_query *output, bool can_overwrite, bool need_tmp_table, bool need_order,
                             bool distinct)
{
  DBUG_ENTER("JOIN::save_explain_data");
  DBUG_PRINT("enter",
             ("Save explain Select_lex: %u (%p)  parent lex: %p  stmt_lex: %p  present select: %u (%p)",
              select_lex->select_number, select_lex, select_lex->parent_lex, thd->lex->stmt_lex,
              (output->get_select(select_lex->select_number) ? select_lex->select_number : 0),
              (output->get_select(select_lex->select_number) ? output->get_select(select_lex->select_number)->select_lex
                                                             : NULL)));
  /*
  ...
  */
  DBUG_ASSERT(select_lex->select_number == FAKE_SELECT_LEX_ID || !output ||
              ...
              output->get_select(select_lex->select_number)->select_lex == select_lex);

  if (select_lex->select_number != FAKE_SELECT_LEX_ID && have_query_plan != JOIN::QEP_NOT_PRESENT_YET &&
  ...
  if (select_lex == select_lex->master_unit()->fake_select_lex && join_tab)
  {
    ...
    */
    ...
    for (uint i = 0; i < exec_join_tab_cnt() + aggr_tables; i++)
    {
      ...
      {
        if (!(join_tab[i].filesort->tracker = new Filesort_tracker(thd->lex->analyze_stmt)))
          DBUG_RETURN(1);
      }
    }
  }
...
void JOIN::exec()
{
  DBUG_EXECUTE_IF("show_explain_probe_join_exec_start",
                  if (dbug_user_var_equals_int(thd, "show_explain_probe_select_id", select_lex->select_number))
                      dbug_serve_apcs(thd, 1););
...
void JOIN::exec_inner()
{
  ...
  */
  ...
  if (procedure)
  {
    ...
    if (procedure->change_columns(thd, procedure_fields_list) || result->prepare(procedure_fields_list, unit))
    {
  ...
  if (!tables_list && (table_count || !select_lex->with_sum_func) && !select_lex->have_window_funcs())
  {  // Only test of functions
    if (select_options & SELECT_DESCRIBE)
      select_describe(this, FALSE, FALSE, FALSE, (zero_result_cause ? zero_result_cause : "No tables used"));
    else
    {
      if (result->send_result_set_metadata(*columns_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
      {
        DBUG_VOID_RETURN;
      }
      ...
      */
      DBUG_ASSERT(error == 0);
      if (cond_value != Item::COND_FALSE && having_value != Item::COND_FALSE && (!conds || conds->val_int()) &&
          (!having || having->val_int()))
      {
        if (do_send_rows && (procedure ? (procedure->send_row(procedure_fields_list) || procedure->end_of_records())
                                       : result->send_data_with_check(fields_list, unit, 0) > 0))
          error = 1;
        else
          send_records = ((select_options & OPTION_FOUND_ROWS) ? 1 : thd->get_sent_row_count());
      }
      else
        send_records = 0;
      if (likely(!error))
  ...
  */
  if (!zero_result_cause && exec_const_cond && !(select_options & SELECT_DESCRIBE) && !exec_const_cond->val_int())
    zero_result_cause = "Impossible WHERE noticed after reading const tables";
  ...
  */
  ...
  if (zero_result_cause)
  {
    ...
    {
      (void)return_zero_rows(this, result, select_lex->leaf_tables, *columns_list, send_row_on_empty_set(),
                             select_options, zero_result_cause, having ? having : tmp_having, all_fields);
      DBUG_VOID_RETURN;
    }
  }
  ...
  /*
    ...
    not the case.
  */
  if (exec_const_order_group_cond.elements && !(select_options & SELECT_DESCRIBE) && !select_lex->pushdown_select)
  {
  ...
  */
  ...
  result->send_result_set_metadata(procedure ? procedure_fields_list : *fields,
  ...
  DBUG_PRINT("counts", ("thd->examined_row_count: %lu", (ulong)thd->get_examined_row_count()));
...
int JOIN::destroy()
{
  ...
  if (join_tab)
  {
    for (JOIN_TAB *tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITH_CONST_TABLES); tab;
         tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS))
    {
      if (tab->aggr)
      {
...
bool mysql_select(THD *thd, TABLE_LIST *tables, List<Item> &fields, COND *conds, uint og_num, ORDER *order,
                  ORDER *group, Item *having, ORDER *proc_param, ulonglong select_options, select_result *result,
                  SELECT_LEX_UNIT *unit, SELECT_LEX *select_lex)
{
  ...
  if (select_lex->join != 0)
  {
    ...
    /*
      ...
      creation
    */
    if (select_lex->get_linkage() != DERIVED_TABLE_TYPE || (select_options & SELECT_DESCRIBE))
    {
      ...
      {
        if ((err = join->prepare(tables, conds, og_num, order, false, group, having, proc_param, select_lex, unit)))
        {
          goto err;
        }
      }
...
static ha_rows get_quick_record_count(THD *thd, SQL_SELECT *select, TABLE *table, const key_map *keys, ha_rows limit)
{
  ...
  if (select)
  {
    ...
    if (likely((error = select->test_quick_select(thd, *(key_map *)keys, (table_map)0, limit, 0, FALSE,
...
void mark_join_nest_as_const(JOIN *join, TABLE_LIST *join_nest, table_map *found_const_table_map, uint *const_count)
{
...
static Item **get_sargable_cond(JOIN *join, TABLE *table)
{
  ...
  else if (table->pos_in_table_list->embedding && !table->pos_in_table_list->embedding->sj_on_expr)
  {
    /*
...
static bool make_join_statistics(JOIN *join, List<TABLE_LIST> &tables_list, DYNAMIC_ARRAY *keyuse_array)
{
  int error = 0;
  ...
  */
  ...
  if (!multi_alloc_root(join->thd->mem_root, &stat, sizeof(JOIN_TAB) * (table_count), &stat_ref,
                        sizeof(JOIN_TAB *) * MAX_TABLES, &stat_vector, sizeof(JOIN_TAB *) * (table_count + 1),
                        &table_vector, sizeof(TABLE *) * (table_count * 2), &join->positions,
                        sizeof(POSITION) * (table_count + 1), &join->sort_positions, sizeof(POSITION) * (sort_space),
                        &join->best_positions, sizeof(POSITION) * (table_count + 1), NullS))
    DBUG_RETURN(1);
  ...
  for (i = 0; i <= table_count; i++) (void)new ((char *)(join->positions + i)) POSITION;
  for (i = 0; i <= sort_space; i++) (void)new ((char *)(join->sort_positions + i)) POSITION;
  ...
  for (s = stat, i = 0; (tables = ti++); s++, i++)
  {
    ...
    DBUG_EXECUTE_IF("bug11747970_raise_error", { join->thd->set_killed(KILL_QUERY_HARD); });
    if (unlikely(error))
    {
    ...
    if (*s->on_expr_ref)
    {
      ...
      if (!table->is_filled_at_execution() &&
          ((!table->file->stats.records && (table->file->ha_table_flags() & HA_STATS_RECORDS_IS_EXACT)) ||
      ...
      s->embedding_map = 0;
      for (; embedding; embedding = embedding->embedding) s->embedding_map |= embedding->nested_join->nj_map;
      continue;
    }
    if (embedding)
    {
    ...
    if (!table->is_filled_at_execution() &&
        (table->s->system ||
         (table->file->stats.records <= 1 && (table->file->ha_table_flags() & HA_STATS_RECORDS_IS_EXACT)) ||
         all_partitions_pruned_away) &&
        !s->dependent && !table->fulltext_searched && !join->no_const_tables)
    {
      set_position(join, const_count++, s, (KEYUSE *)0);
      no_rows_const_tables |= table->map;
    }
    ...
    if (table->pos_in_table_list->jtbm_subselect && table->pos_in_table_list->jtbm_subselect->is_jtbm_const_tab)
    {
      set_position(join, const_count++, s, (KEYUSE *)0);
      no_rows_const_tables |= table->map;
    }
    ...
    */
    ...
    for (i = 0, s = stat; i < table_count; i++, s++)
    {
      if (s->dependent & s->table->map)
      {
        ...
        my_message(ER_WRONG_OUTER_JOIN, ER_THD(join->thd, ER_WRONG_OUTER_JOIN), MYF(0));
        goto error;
      }
  ...
  {
    if (update_ref_and_keys(thd, keyuse_array, stat, join->table_count, join->conds, ~outer_join, join->select_lex,
                            &sargables))
      goto error;
    /*
    ...
    */
    bool skip_unprefixed_keyparts =
        !(join->is_in_subquery() && join->unit->item->get_IN_subquery()->test_strategy(SUBS_IN_TO_EXISTS));

    if (keyuse_array->elements && sort_and_filter_keyuse(join, keyuse_array, skip_unprefixed_keyparts))
      goto error;
  ...
  more_const_tables_found:
    ...
    */
    ...
    for (JOIN_TAB **pos = stat_vector + const_count; (s = *pos); pos++)
    {
      ...
      */
      if ((keyuse = s->keyuse) && *s->on_expr_ref && !s->embedding_map && !(table->map & join->eliminated_tables))
      {
        /*
        ...
        */
        ...
        {
          if (!keyuse->is_for_hash_join() && !(keyuse->val->used_tables() & ~join->const_table_map) &&
      ...
      if (s->dependent)  // If dependent on some table
      {
        ...
        if (s->dependent & ~(found_const_table_map))
          continue;
        if (table->file->stats.records <= 1L && (table->file->ha_table_flags() & HA_STATS_RECORDS_IS_EXACT) &&
      ...
      if ((keyuse = s->keyuse))
      {
        ...
        while (keyuse->table == table)
        {
          ...
          do
          {
            if (keyuse->val->type() != Item::NULL_ITEM && !keyuse->optimize && keyuse->keypart != FT_KEYPART)
            {
          ...
          */
          ...
          if (eq_part.is_prefix(key_parts) && !table->fulltext_searched &&
              (!embedding || (embedding->sj_on_expr && !embedding->embedding)))
          {
            ...
            {
              if (base_const_ref == base_eq_part && !has_expensive_keyparts &&
              ...
              {  // Found everything for ref.
                ...
                if (create_ref_for_key(join, s, start_keyuse, FALSE, found_const_table_map))
                  goto error;
                if ((tmp = join_read_const_table(join->thd, s, join->positions + const_count - 1)))
                {
                  if (tmp > 0)
  ...
  join->sort_by_table =
      get_sort_by_table(join->order, join->group_list, join->select_lex->leaf_tables, join->const_table_map);
  /*
  ...
  */
  ...
  if (join->conds && const_count)
  {
    ...
    if (conds && conds->type() == Item::COND_ITEM && ((Item_cond *)conds)->functype() == Item_func::COND_AND_FUNC)
    ...
    if (conds)
    {
      if (conds->type() == Item::COND_ITEM && ((Item_cond *)conds)->functype() == Item_func::COND_AND_FUNC)
        join->cond_equal = (&((Item_cond_and *)conds)->m_cond_equal);
      else if (conds->type() == Item::FUNC_ITEM && ((Item_func *)conds)->functype() == Item_func::MULT_EQUAL_FUNC)
      {
        if (!join->cond_equal)
          join->cond_equal = new COND_EQUAL;
        join->cond_equal->current_level.empty();
        join->cond_equal->current_level.push_back((Item_equal *)conds, join->thd->mem_root);
      }
    }
    ...
    if (orig_cond_equal != join->cond_equal)
    {
      ...
      */
      for (JOIN_TAB **pos = stat_vector + const_count; (s = *pos); pos++)
      {
        if (*s->on_expr_ref && s->cond_equal && s->cond_equal->upper_levels == orig_cond_equal)
  ...
  */
    ...
    for (s = stat; s < stat_end; s++)
    {
      ...
      if (s->type == JT_SYSTEM || s->type == JT_CONST)
      {
        ...
        table_records.add_table_name(s)
            ...
            .add("table_type", s->type == JT_CONST ? "const" : "system");
        continue;
      }
      /* Approximate found rows and time to read them */
      if (s->table->is_filled_at_execution())
      {
        get_delayed_table_estimates(s->table, &s->records, &s->read_time, &s->startup_cost);
      ...
      /*
        ...
        table scan.
      */
      s->worst_seeks = MY_MIN((double)s->found_records / 10, (double)s->read_time * 3);
      ...
      */
      if ((!s->const_keys.is_clear_all() || !bitmap_is_clear_all(&s->table->cond_set)) &&  // (1)
            ...
            s->table->pos_in_table_list->is_materialized_derived()))                       // (3)
      {
        ...
        if (!s->const_keys.is_clear_all())
        {
          ...
          select = make_select(s->table, found_const_table_map, found_const_table_map, *sargable_cond, (SORT_INFO *)0,
                               1, &error);
          if (!select)
            goto error;
          ...
          *sargable_cond = select->cond;
          ...
          if (is_sargable_cond_of_where && join->conds && join->conds->type() == Item::COND_ITEM &&
              ((Item_cond *)(join->conds))->functype() == Item_func::COND_AND_FUNC)
        ...
        if (!impossible_range)
        {
          ...
          if (join->thd->variables.optimizer_use_condition_selectivity > 1)
            calculate_cond_selectivity_for_table(join->thd, s->table, sargable_cond);
        ...
        if (impossible_range)
        {
          ...
          */
          ...
          if (emb && !emb->sj_on_expr)
          {
            ...
            mark_join_nest_as_const(join, emb, &found_const_table_map, &const_count);
          }
          else
          {
            join->const_table_map |= s->table->map;
  ...
  DBUG_ASSERT(!join->conds || !join->cond_equal || !join->cond_equal->current_level.elements ||
              (join->conds->type() == Item::COND_ITEM &&
               ((Item_cond *)(join->conds))->functype() == Item_func::COND_AND_FUNC &&
               join->cond_equal == &((Item_cond_and *)(join->conds))->m_cond_equal) ||
              (join->conds->type() == Item::FUNC_ITEM &&
               ((Item_func *)(join->conds))->functype() == Item_func::MULT_EQUAL_FUNC &&
               join->cond_equal->current_level.elements == 1 && join->cond_equal->current_level.head() == join->conds));
...

  {
    ...
    if (!(join->select_options & SELECT_DESCRIBE) && unit->derived && unit->derived->is_materialized_derived())
    {
      /*
  ...
  */
  {
    ...
    while ((tmp_table = ti2++)) tmp_table->table->reginfo.join_tab = NULL;
  }
  DBUG_RETURN(1);
}
          ...
          */
          if (old->val->eq(new_fields->val, old->field->binary()))
          {
            ...
            old->null_rejecting = (old->null_rejecting && new_fields->null_rejecting);
                 ...
                 old->val->eq_by_collation(new_fields->val, old->field->binary(), old->field->charset()))

        {
          ...
          old->null_rejecting = (old->null_rejecting && new_fields->null_rejecting);
                 ...
                 ((old->val->can_eval_in_optimize() && old->val->is_null()) ||
                  (!new_fields->val->is_expensive() && new_fields->val->is_null())))
        {
          ...
          /*
            ...
            on other tables.
          */
          if (!old->val->used_tables() && !old->val->is_expensive() && old->val->is_null())
...
static uint get_semi_join_select_list_index(Field *field)
{
  ...
  if ((emb_sj_nest = field->table->pos_in_table_list->embedding) && emb_sj_nest->sj_on_expr)
  {
    ...
    if (ncols == 1)
    {
      ...
      if (sel_item->type() == Item::FIELD_ITEM && ((Item_field *)sel_item)->field->eq(field))
      {
        res = 0;
      }
    }
...
static void add_key_field(JOIN *join, KEY_FIELD **key_fields, uint and_level, Item_bool_func *cond, Field *field,
                          bool eq_func, Item **value, uint num_values, table_map usable_tables,
                          SARGABLE_PARAM **sargables, uint row_col_no = 0)
{
  uint optimize = 0;
  if (eq_func && ((join->is_allowed_hash_join_access() && field->hash_join_is_possible() &&
                   !(field->table->pos_in_table_list->is_materialized_derived() && field->table->is_created())) ||
                  (field->table->pos_in_table_list->is_materialized_derived() && !field->table->is_created() &&
...
static void add_key_equal_fields(JOIN *join, KEY_FIELD **key_fields, uint and_level, Item_bool_func *cond,
                                 Item *field_item, bool eq_func, Item **val, uint num_values, table_map usable_tables,
                                 SARGABLE_PARAM **sargables, uint row_col_no = 0)
{
  Field *field = ((Item_field *)(field_item->real_item()))->field;
  add_key_field(join, key_fields, and_level, cond, field, eq_func, val, num_values, usable_tables, sargables,
                row_col_no);
  ...
  if (item_equal)
  {
    ...
    */
    ...
    while (it++)
    {
      ...
      {
        add_key_field(join, key_fields, and_level, cond, equal_field, eq_func, val, num_values, usable_tables,
...
{
  return field->real_item()->type() == Item::FIELD_ITEM && !(field->used_tables() & OUTER_REF_TABLE_BIT) &&
...
void Item_cond_and::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                   SARGABLE_PARAM **sargables)
{
  ...
  Item *item;
  while ((item = li++)) item->add_key_fields(join, key_fields, and_level, usable_tables, sargables);
  for (; org_key_fields != *key_fields; org_key_fields++) org_key_fields->level = *and_level;
}

void Item_cond::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                               SARGABLE_PARAM **sargables)
{
  List_iterator_fast<Item> li(*argument_list());
  KEY_FIELD *org_key_fields = *key_fields;
  ...
  (*and_level)++;
  (li++)->add_key_fields(join, key_fields, and_level, usable_tables, sargables);
  Item *item;
  while ((item = li++))
  {
    KEY_FIELD *start_key_fields = *key_fields;
    (*and_level)++;
    item->add_key_fields(join, key_fields, and_level, usable_tables, sargables);
    *key_fields = merge_key_fields(org_key_fields, start_key_fields, *key_fields, ++(*and_level));
  }
}

void Item_func_trig_cond::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                         SARGABLE_PARAM **sargables)
{
  /*
  ...
  */
  if (!join->group_list && !join->order && join->unit->item &&
      join->unit->item->substype() == Item_subselect::IN_SUBS && !join->unit->is_unit_op())
  {
    KEY_FIELD *save = *key_fields;
    args[0]->add_key_fields(join, key_fields, and_level, usable_tables, sargables);
    // Indicate that this ref access candidate is for subquery lookup:
    for (; save != *key_fields; save++) save->cond_guard = get_trig_var();
  }
}

void Item_func_between::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                       SARGABLE_PARAM **sargables)
{
  /*
  ...
  */
  ...
  bool binary_cmp =
      (args[0]->real_item()->type() == Item::FIELD_ITEM) ? ((Item_field *)args[0]->real_item())->field->binary() : true;
  /*
  ...
  */
  if (is_local_field(args[0]))
  {
    ...
    add_key_equal_fields(join, key_fields, *and_level, this, field_item, equal_func, &args[1], num_values,
  ...
  */
  for (uint i = 1; i <= num_values; i++)
  {
    if (is_local_field(args[i]))
    {
      ...
      add_key_equal_fields(join, key_fields, *and_level, this, field_item, equal_func, args, 1, usable_tables,
                           sargables);
    }
  }
}

void Item_func_in::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                  SARGABLE_PARAM **sargables)
{
  if (is_local_field(args[0]) && !(used_tables() & OUTER_REF_TABLE_BIT))
  {
    DBUG_ASSERT(arg_count != 2);
    add_key_equal_fields(join, key_fields, *and_level, this, (Item_field *)(args[0]->real_item()), false, args + 1,
                         arg_count - 1, usable_tables, sargables);
  }
  else if (key_item()->type() == Item::ROW_ITEM && !(used_tables() & OUTER_REF_TABLE_BIT))
  {
    ...
    for (uint i = 0; i < row_cols; i++, key_col++)
    {
      if (is_local_field(*key_col))
      {
        ...
        add_key_equal_fields(join, key_fields, *and_level, this, field_item, false, args + 1, arg_count - 1,
...
void Item_func_ne::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                  SARGABLE_PARAM **sargables)
{
  if (!(used_tables() & OUTER_REF_TABLE_BIT))
  {
    ...
    */
    if (is_local_field(args[0]) && !is_local_field(args[1]))
      add_key_equal_fields(join, key_fields, *and_level, this, (Item_field *)(args[0]->real_item()), false, &args[1], 1,
                           usable_tables, sargables);
    /*
      QQ: perhaps test for !is_local_field(args[0]) is not really needed here.
    */
    if (is_local_field(args[1]) && !is_local_field(args[0]))
      add_key_equal_fields(join, key_fields, *and_level, this, (Item_field *)(args[1]->real_item()), false, &args[0], 1,
                           usable_tables, sargables);
  }
}

void Item_func_like::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                    SARGABLE_PARAM **sargables)
{
...
void Item_bool_func2::add_key_fields_optimize_op(JOIN *join, KEY_FIELD **key_fields, uint *and_level,
                                                 table_map usable_tables, SARGABLE_PARAM **sargables, bool equal_func)
{
  /* If item is of type 'field op field/constant' add it to key_fields */
  if (is_local_field(args[0]))
  {
    add_key_equal_fields(join, key_fields, *and_level, this, (Item_field *)args[0]->real_item(), equal_func, args + 1,
                         1, usable_tables, sargables);
  }
  if (is_local_field(args[1]))
  {
    add_key_equal_fields(join, key_fields, *and_level, this, (Item_field *)args[1]->real_item(), equal_func, args, 1,
                         usable_tables, sargables);
  }
}

void Item_func_null_predicate::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level,
                                              table_map usable_tables, SARGABLE_PARAM **sargables)
{
  ...
  if (is_local_field(args[0]) && !(used_tables() & OUTER_REF_TABLE_BIT))
  {
    ...
    if (unlikely(!tmp))  // Should never be true
      return;
    add_key_equal_fields(join, key_fields, *and_level, this, (Item_field *)args[0]->real_item(),
                         functype() == Item_func::ISNULL_FUNC, &tmp, 1, usable_tables, sargables);
  }
}

void Item_equal::add_key_fields(JOIN *join, KEY_FIELD **key_fields, uint *and_level, table_map usable_tables,
                                SARGABLE_PARAM **sargables)
{
  ...
  if (const_item2)
  {
    ...
    */
    ...
    {
      Field *equal_field = it.get_curr_field();
      add_key_field(join, key_fields, *and_level, this, equal_field, TRUE, &const_item2, 1, usable_tables, sargables);
    }
  }
  else
  {
    ...
    */
    ...
    while (fi++)
    {
      ...
      while ((item = it++))
      {
        ...
        if (!field->eq(equal_field))
        {
          add_key_field(join, key_fields, *and_level, this, field, TRUE, &item, 1, usable_tables, sargables);
        }
      }
      it.rewind();
    }
...
static bool add_keyuse(DYNAMIC_ARRAY *keyuse_array, KEY_FIELD *key_field, uint key, uint part)
{
  KEYUSE keyuse;
...
static bool add_key_part(DYNAMIC_ARRAY *keyuse_array, KEY_FIELD *key_field)
{
  ...
  if (key_field->eq_func && !(key_field->optimize & KEY_OPTIMIZE_EXISTS))
  {
    ...
    if (field->hash_join_is_possible() && (key_field->optimize & KEY_OPTIMIZE_EQ) && key_field->val->used_tables())
    {
...
static bool add_ft_keys(DYNAMIC_ARRAY *keyuse_array, JOIN_TAB *stat, COND *cond, table_map usable_tables)
{
  ...
  if (cond->type() == Item::FUNC_ITEM)
  {
    ...
    {
      Item *arg0 = (Item *)(func->arguments()[0]), *arg1 = (Item *)(func->arguments()[1]);
      if (arg1->const_item() && arg1->cols() == 1 && arg0->type() == Item::FUNC_ITEM &&
          ((Item_func *)arg0)->functype() == Item_func::FT_FUNC &&
          ((functype == Item_func::GE_FUNC && arg1->val_real() > 0) ||
           (functype == Item_func::GT_FUNC && arg1->val_real() >= 0)))
        cond_func = (Item_func_match *)arg0;
      else if (arg0->const_item() && arg0->cols() == 1 && arg1->type() == Item::FUNC_ITEM &&
...
static int sort_keyuse(KEYUSE *a, KEYUSE *b)
{
  ...
  if (a->key == MAX_KEY && b->key == MAX_KEY && a->used_tables != b->used_tables)
    return (int)((ulong)a->used_tables - (ulong)b->used_tables);
  if (a->keypart != b->keypart)
    return (int)(a->keypart - b->keypart);
  // Place const values before other ones
  if ((res = MY_TEST((a->used_tables & ~OUTER_REF_TABLE_BIT)) - MY_TEST((b->used_tables & ~OUTER_REF_TABLE_BIT))))
    return res;
...
static void add_key_fields_for_nj(JOIN *join, TABLE_LIST *nested_join_table, KEY_FIELD **end, uint *and_level,
                                  SARGABLE_PARAM **sargables)
{
  ...
  while ((table = li++) || (have_another && (li = li2, have_another = FALSE, (table = li++))))
  {
  ...
  if (nested_join_table->on_expr)
    nested_join_table->on_expr->add_key_fields(join, end, and_level, tables, sargables);
}
...
static bool update_ref_and_keys(THD *thd, DYNAMIC_ARRAY *keyuse, JOIN_TAB *join_tab, uint tables, COND *cond,
                                table_map normal_tables, SELECT_LEX *select_lex, SARGABLE_PARAM **sargables)
{
  uint and_level, i;
  ...
  /*
    ...
    substitutions.
  */
  sz = MY_MAX(sizeof(KEY_FIELD), sizeof(SARGABLE_PARAM)) * ((sel->cond_count * 2 + sel->between_count) * m + 1);
  if (!(key_fields = (KEY_FIELD *)thd->alloc(sz)))
    DBUG_RETURN(TRUE); /* purecov: inspected */
  and_level = 0;
  field = end = key_fields;
  *sargables = (SARGABLE_PARAM *)key_fields + (sz - sizeof((*sargables)[0].field)) / sizeof(SARGABLE_PARAM);
  /* set a barrier for the array of SARGABLE_PARAM */
  (*sargables)[0].field = 0;

  if (my_init_dynamic_array2(thd->mem_root->psi_key, keyuse, sizeof(KEYUSE), thd->alloc(sizeof(KEYUSE) * 20), 20, 64,
  ...
  if (cond)
  {
    ...
    cond->add_key_fields(join_tab->join, &end, &and_level, normal_tables, sargables);
    for (; field != end; field++)
    {
  ...
  for (i = 0; i < tables; i++)
  {
    ...
    */
    if (*join_tab[i].on_expr_ref)
      (*join_tab[i].on_expr_ref)->add_key_fields(join_tab->join, &end, &and_level, join_tab[i].table->map, sargables);
  }
  ...
  /* Process ON conditions for the nested joins */
  {
    ...
    {
      if (table->nested_join)
        add_key_fields_for_nj(join_tab->join, table, &end, &and_level, sargables);
    }
  }
...
bool sort_and_filter_keyuse(JOIN *join, DYNAMIC_ARRAY *keyuse, bool skip_unprefixed_keyparts)
{
  THD *thd = join->thd;
  ...
  my_qsort(keyuse->buffer, keyuse->elements, sizeof(KEYUSE), (qsort_cmp)sort_keyuse);
  ...
  for (i = 0; i < keyuse->elements - 1; i++, use++)
  {
    ...
    {
      if (!(use->used_tables & ~OUTER_REF_TABLE_BIT) && use->optimize != KEY_OPTIMIZE_REF_OR_NULL)
...
static void add_group_and_distinct_keys(JOIN *join, JOIN_TAB *join_tab)
{
  ...
  { /* Collect all query fields referenced in the GROUP clause. */
    for (cur_group = join->group_list; cur_group; cur_group = cur_group->next)
      (*cur_group->item)->walk(&Item::collect_item_field_processor, 0, &indexed_fields);
  }
  ...
  { /* Collect all query fields referenced in the SELECT clause. */
    ...
    Item *item;
    while ((item = select_items_it++)) item->walk(&Item::collect_item_field_processor, 0, &indexed_fields);
  }
  else if (!join->tmp_table_param.sum_func_count || !is_indexed_agg_distinct(join, &indexed_fields))
  {
    /*
...
inline double matching_candidates_in_table(JOIN_TAB *s, bool with_found_constraint, uint use_cond_selectivity)
{
  ha_rows records;
  double dbl_records;
...
double cost_for_index_read(const THD *thd, const TABLE *table, uint key, ha_rows records, ha_rows worst_seeks)
{
  ...
  if (file->is_clustering_key(key))
    ...
    cost = ((file->keyread_time(key, 0, records) + file->read_time(key, 1, MY_MIN(records, worst_seeks))));
...
{
  double cost = (quick_cost - MULTI_RANGE_READ_SETUP_COST - rows2double(records) / TIME_FOR_COMPARE);
...
void best_access_path(JOIN *join, JOIN_TAB *s, table_map remaining_tables, const POSITION *join_positions, uint idx,
                      bool disable_jbuf, double record_count, POSITION *pos, POSITION *loose_scan_pos)
{
  THD *thd = join->thd;
  ...
  */
  ...
  { /* Use key if possible */
    ...
    for (keyuse = s->keyuse; keyuse->table == table;)
    {
      ...
      if (is_hash_join_key_no(key))
      {
        ...
        */
        if (!(remaining_tables & keyuse->used_tables) && idx > join->const_tables)
        {
          if (!hj_start_key)
            hj_start_key = keyuse;
      ...
      DBUG_PRINT("info", ("Considering ref access on key %s", keyuse->table->key_info[keyuse->key].name.str));
      ...
      do /* For each keypart */
      {
        ...
        do /* For each way to access the keypart */
        {
          ...
          */
          ...
          if (!(remaining_tables & keyuse->used_tables) && (!keyuse->validity_ref || *keyuse->validity_ref) &&
              s->access_from_tables_is_allowed(keyuse->used_tables, join->sjm_lookup_tables) &&
              !(ref_or_null_part && (keyuse->optimize & KEY_OPTIMIZE_REF_OR_NULL)))
          {
            ...
            double tmp2 = prev_record_reads(join_positions, idx, (found_ref | keyuse->used_tables));
      ...
      */
      if (ft_key)
      {
        ...
        */
        ...
        records = 1.0;
        type = JT_FT;
        trace_access_idx.add("access_type", join_type_str[type]).add("full-text index", keyinfo->name);
      }
      else
      {
        ...
        { /* use eq key */
          ...
          */
               ...
               all_key_parts == notnull_part))              //  (3)
          {
            ...
            type = JT_EQ_REF;
            trace_access_idx.add("access_type", join_type_str[type]).add("index", keyinfo->name);
          ...
          else
          {
            ...
            { /* We found a const key */
              ...
              */
              if (table->opt_range_keys.is_set(key))
              {
                ...
                tmp = adjust_quick_cost(table->opt_range[key].cost, table->opt_range[key].rows);
                goto got_cost;
              }
              else
              {
                ...
                trace_access_idx.add("used_range_estimates", false).add("reason", "not available");
              }
            }
            else
            {
              ...
              */
              if (table->opt_range_keys.is_set(key) &&
                  (const_part & (((key_part_map)1 << table->opt_range[key].key_parts) - 1)) ==
                      (((key_part_map)1 << table->opt_range[key].key_parts) - 1) &&
                  table->opt_range[key].ranges == 1 && records > (double)table->opt_range[key].rows)
              {
                records = (double)table->opt_range[key].rows;
            ...
            tmp = cost_for_index_read(thd, table, key, (ha_rows)records, (ha_rows)s->worst_seeks);
          got_cost:
            ...
            */
            ...
            else
            {
              ...
              else
              {
                ...
                */
                double rec_per_key;
                if (!(rec_per_key = (double)keyinfo->rec_per_key[keyinfo->user_defined_key_parts - 1]))
                ...
                else
                {
                  ...
                  if (keyinfo->user_defined_key_parts > 1)
                    records = (max_key_part * (rec_per_key - a) + a * keyinfo->user_defined_key_parts - rec_per_key) /
              ...
              */
              if (table->opt_range_keys.is_set(key) && table->opt_range[key].key_parts <= max_key_part &&
                  const_part & ((key_part_map)1 << table->opt_range[key].key_parts) &&
                  table->opt_range[key].ranges == (1 + MY_TEST(ref_or_null_part & const_part)) &&
                  records > (double)table->opt_range[key].rows)
              {
                trace_access_idx.add("used_range_estimates", true);
                records = (double)table->opt_range[key].rows;
            ...
            tmp = cost_for_index_read(thd, table, key, (ha_rows)tmp, (ha_rows)s->worst_seeks);
          got_cost2:
            tmp = COST_MULT(tmp, record_count);
          }
          else
        ...
        loose_scan_opt.check_ref_access_part2(key, start_key, records, tmp, found_ref);
      } /* not ft_key */

      if (records < DBL_MAX && (found_part & 1))  // start_key->key can be used for index access
      {
        double rows = record_count * records;
        double access_cost_factor = MY_MIN(tmp / rows, 1.0);
        filter = table->best_range_rowid_filter_for_partial_join(start_key->key, rows, access_cost_factor);
        if (filter)
        {
          filter->get_cmp_gain(rows);
          tmp -= filter->get_adjusted_gain(rows) - filter->get_cmp_gain(rows);
          DBUG_ASSERT(tmp >= 0);
          trace_access_idx.add("rowid_filter_key", table->key_info[filter->key_no].name);
        }
      }
      ...
      {
        trace_access_idx.add("chosen", false).add("cause", cause ? cause : "cost");
      }
      cause = nullptr;
    } /* for each key */
  ...
  DBUG_ASSERT((key_dependent & (s->key_dependent | s->embedded_dependent)) == key_dependent);
  ...
  */
  if (idx > join->const_tables && best_key == 0 && (join->allowed_join_cache_types & JOIN_CACHE_HASHED_BIT) &&
      join->max_allowed_join_cache_level > 2 && !bitmap_is_clear_all(eq_join_set) && !disable_jbuf &&
      (!s->emb_sj_nest || join->allowed_semijoin_with_cache) &&                      // (1)
      (!(s->table->map & join->outer_join) || join->allowed_outer_join_with_cache))  // (2)
  {
    double join_sel = 0.1;
    /* Estimate the cost of  the hash join access to the table */
    double rnd_records = matching_candidates_in_table(s, found_constraint, use_cond_selectivity);
    ...
    double refills =
        (1.0 + floor((double)cache_record_length(join, idx) * record_count / (double)thd->variables.join_buff_size));
    tmp = COST_MULT(tmp, refills);
    best_time = COST_ADD(tmp, COST_MULT((record_count * join_sel) / TIME_FOR_COMPARE, rnd_records));
    best = tmp;
    records = rnd_records;
  ...
  */
  ...
  if ((records >= s->found_records || best > s->read_time) &&                         // (1)
      ...
      !(s->quick && s->quick->get_type() != QUICK_SELECT_I::QS_TYPE_GROUP_MIN_MAX &&  // (2)
  ...
  {                                                                                   // Check full join
    ...
    */
    ...
    if (s->quick)
    {
      ...
      */
      ...
      tmp = COST_MULT(record_count, COST_ADD(s->quick->read_time, cmp_time));
      ...
      if (s->quick->get_type() == QUICK_SELECT_I::QS_TYPE_RANGE)
      {
        ...
        filter = s->table->best_range_rowid_filter_for_partial_join(key_no, rows, access_cost_factor);
        if (filter)
        {
    ...
    trace_access_scan.add("access_type", type == JT_ALL ? "scan" : join_type_str[type]);
    ...
    */
    ...
    const double best_filter_cmp_gain = best_filter ? best_filter->get_cmp_gain(record_count * records) : 0;
    trace_access_scan.add("resulting_rows", rnd_records);
    trace_access_scan.add("cost", tmp);

    if (best == DBL_MAX || COST_ADD(tmp, record_count / TIME_FOR_COMPARE * rnd_records) <
                               (best_key->is_for_hash_join()
                                    ? best_time
                                    : COST_ADD(best - best_filter_cmp_gain, record_count / TIME_FOR_COMPARE * records)))
    {
      /*
      ...
      */
      ...
      best_ref_depends_map = 0;
      best_uses_jbuf = MY_TEST(!disable_jbuf && !((s->table->map & join->outer_join)));
      spl_plan = 0;
      best_type = type;
    }
  ...
  pos->key_dependent = (best_type == JT_EQ_REF ? (table_map)0 : key_dependent & remaining_tables);
    ...
    if (get_emb_subq(*subq_tab) != cur_subq_nest)
    {
      ...
      */
      ...
      for (last_tab_for_subq = subq_tab;
           last_tab_for_subq < subq_tabs_end && get_emb_subq(*last_tab_for_subq) == cur_subq_nest; last_tab_for_subq++)
      {
      }
      ...
      */
      ...
      for (JOIN_TAB **top_level_tab = join->best_ref + join->const_tables; top_level_tab < last_top_level_tab;
           ...
           top_level_tab++)
      {
        ...
        if (!need_tables)
        {
          ...
          memmove(top_level_tab + 1 + n_subquery_tables, top_level_tab + 1, sizeof(JOIN_TAB *) * top_tail_len);
...
bool choose_plan(JOIN *join, table_map join_tables)
{
  ...
  uint use_cond_selectivity = join->thd->variables.optimizer_use_condition_selectivity;
  ...
  */
  my_qsort2(join->best_ref + join->const_tables, join->table_count - join->const_tables, sizeof(JOIN_TAB *),
  ...
  */
  ...
  else
  {
    ...
    if (join->prune_level >= 1 && search_depth >= thd->variables.optimizer_extra_pruning_depth)
    {
...
static int compare_embedding_subqueries(JOIN_TAB *jt1, JOIN_TAB *jt2)
{
  ...
  {
    tbl1_select_no = tbl1->jtbm_subselect->unit->first_select()->select_number;
  }
  else if (tbl1->embedding && tbl1->embedding->sj_subq_pred)
  {
    tbl1_select_no = tbl1->embedding->sj_subq_pred->unit->first_select()->select_number;
  }
  else
  ...
  {
    tbl2_select_no = tbl2->jtbm_subselect->unit->first_select()->select_number;
  }
  else if (tbl2->embedding && tbl2->embedding->sj_subq_pred)
  {
    tbl2_select_no = tbl2->embedding->sj_subq_pred->unit->first_select()->select_number;
  }
  else
...
static void optimize_straight_join(JOIN *join, table_map remaining_tables)
{
  JOIN_TAB *s;
  ...
  for (JOIN_TAB **pos = join->best_ref + idx; (s = *pos); pos++)
  {
    ...
    best_access_path(join, s, remaining_tables, join->positions, idx, disable_jbuf, record_count, position,
                     &loose_scan_pos);

    /* Compute the cost of the new plan extended with 's' */
    record_count = COST_MULT(record_count, position->records_read);
    const double filter_cmp_gain =
        position->range_rowid_filter_info ? position->range_rowid_filter_info->get_cmp_gain(record_count) : 0;
    read_time = COST_ADD(read_time, COST_ADD(position->read_time - filter_cmp_gain, record_count / TIME_FOR_COMPARE));
    optimize_semi_joins(join, remaining_tables, idx, &record_count, &read_time, &loose_scan_pos);

    remaining_tables &= ~(s->table->map);
    double pushdown_cond_selectivity = 1.0;
    if (use_cond_selectivity > 1)
      pushdown_cond_selectivity = table_cond_selectivity(join, idx, s, remaining_tables);
  ...
  memcpy((uchar *)join->best_positions, (uchar *)join->positions, sizeof(POSITION) * idx);
...
static bool greedy_search(JOIN *join, table_map remaining_tables, uint search_depth, uint use_cond_selectivity)
{
  double record_count = 1.0;
  double read_time = 0.0;
  ...
  usable_tables =
      (join->emb_sjm_nest ? (join->emb_sjm_nest->sj_inner_tables & ~join->const_table_map & remaining_tables)
                          : remaining_tables);
  ...
  do
  {
    ...
    */
    ...
    if ((int)best_extension_by_limited_search(join, remaining_tables, idx, record_count, read_time, search_depth,
                                              use_cond_selectivity, &eq_ref_tables) < (int)SEARCH_OK)
      DBUG_RETURN(TRUE);
    /*
    ...
    */
    ...
    /*
      ...
      on exit.
    */
    bool is_interleave_error __attribute__((unused)) = check_interleaving_with_nj(best_table);
    ...
    */
    ...
    read_time = COST_ADD(read_time, COST_ADD(join->positions[idx].read_time, record_count / TIME_FOR_COMPARE));

    remaining_tables &= ~(best_table->table->map);
    --size_remain;
    ++idx;

    DBUG_EXECUTE("opt", print_plan(join, idx, record_count, read_time, read_time, "extended"););
  } while (TRUE);
}
...
void JOIN::get_partial_cost_and_fanout(int end_tab_idx, table_map filter_map, double *read_time_arg,
                                       double *record_count_arg)
{
  ...
  */
  ...
  for (tab = first_depth_first_tab(this), i = const_tables; tab; tab = next_depth_first_tab(this, tab), i++)
  {
    end_tab = tab;
    if (i == end_tab_idx)
      break;
  ...
  for (tab = first_depth_first_tab(this), i = const_tables;; tab = next_depth_first_tab(this, tab), i++)
  {
    ...
    */
    ...
    if (tab->records_read && (cur_table_map & filter_map))
    {
      ...
      read_time = COST_ADD(read_time, COST_ADD(tab->read_time, record_count / TIME_FOR_COMPARE));
      if (tab->emb_sj_nest)
...
void JOIN::get_prefix_cost_and_fanout(uint n_tables, double *read_time_arg, double *record_count_arg)
{
  double record_count = 1;
  double read_time = 0.0;
...
double JOIN::get_examined_rows()
{
  ...
  while ((tab = next_breadth_first_tab(first_breadth_first_tab(), top_join_tab_count, tab)))
  {
    prev_fanout = COST_MULT(prev_fanout, prev_tab->records_read);
    records = COST_ADD(records, COST_MULT((double)(tab->get_examined_rows()), prev_fanout));
    prev_tab = tab;
  }
  examined_rows = (double)(records > (double)HA_ROWS_MAX ? HA_ROWS_MAX : (ha_rows)records);
...
static double table_multi_eq_cond_selectivity(JOIN *join, uint idx, JOIN_TAB *s, table_map rem_tables, uint keyparts,
      ...
      */
      ...
      while ((fi++))
      {
        ...
        if (!(fld->table->map & ~(table_bit | rem_tables)))
          continue;
        curr_eq_fld_sel = get_column_avg_frequency(fld) / fld->table->stat_records();
...
static double table_cond_selectivity(JOIN *join, uint idx, JOIN_TAB *s, table_map rem_tables)
{
  ...
  if (pos->key != 0)
  {
    ...
    */
    ...
    {
      key_part_map quick_key_map = (key_part_map(1) << table->opt_range[key].key_parts) - 1;
      if (table->opt_range[key].rows && !(quick_key_map & ~table->const_key_parts[key]))
      {
        /*
        ...
        */
        ...
        {
          while (keyuse->table == table && keyuse->key == key && keyuse->keypart == keyparts)
          {
            keyuse++;
          }
          keyparts++;
    ...
    while (keyuse->table == table && keyuse->key == key)
    {
      if (!(keyuse->used_tables & (rem_tables | table->map)))
      {
        if (are_tables_local(s, keyuse->val->used_tables()))
        {
          ...
          {
            if (keyparts == keyuse->keypart && !((keyuse->val->used_tables()) & ~pos->ref_depend_map) &&
            ...
            if (keyparts > 1)
            {
              ...
              */
              if (keyparts - 2 >= ref_keyuse_size)
              {
                ...
                void *new_buf;
                if (!(new_buf = my_malloc(PSI_INSTRUMENT_ME, sizeof(*ref_keyuse_steps) * new_size, MYF(0))))
                {
                  sel = 1.0;  // As if no selectivity was computed
                  goto exit;
                }
                memcpy(new_buf, ref_keyuse_steps, sizeof(*ref_keyuse_steps) * ref_keyuse_size);
  ...
  */
  if (pos->key != NULL)
  {
    ...
    {
      if (!bitmap_is_set(read_set, field->field_index) || !field->next_equal_field)
        continue;
      for (Field *next_field = field->next_equal_field; next_field != field; next_field = next_field->next_equal_field)
      {
  ...
  sel *= table_multi_eq_cond_selectivity(join, idx, s, rem_tables, keyparts, ref_keyuse_steps);
exit:
  if (ref_keyuse_steps != ref_keyuse_steps_buf)
    my_free(ref_keyuse_steps);
  return sel;
...
static inline enum_best_search check_if_edge_table(POSITION *pos, double pushdown_cond_selectivity)
{
  if ((pos->type == JT_EQ_REF || (pos->type == JT_REF && pos->records_read == 1 && !pos->range_rowid_filter_info)) &&
...
static int sort_positions(SORT_POSITION *a, SORT_POSITION *b)
{
  int cmp;
...
static bool get_costs_for_tables(JOIN *join, table_map remaining_tables, uint idx, double record_count,
                                 Json_writer_object *trace_one_table, JOIN_TAB **pos, SORT_POSITION **store_position,
                                 table_map *allowed_tables, bool stop_on_eq_ref)
{
  THD *thd = join->thd;
  ...
  do
  {
    ...
    if ((*allowed_tables & real_table_bit) && !(remaining_tables & s->dependent))
    {
      ...
      Json_writer_object wrapper(thd);
      /* Find the best access method from 's' to the current partial plan */
      best_access_path(join, s, remaining_tables, join->positions, idx, disable_jbuf, record_count, sort_position,
                       sort_position + 1);
    ...
    else
    {
      ...
      DBUG_ASSERT((remaining_tables & s->dependent) || !(remaining_tables & real_table_bit) ||
                  !(*allowed_tables & real_table_bit) || check_interleaving_with_nj(s));
    }
  } while ((s = *++pos));
...
static enum_best_search best_extension_by_limited_search(JOIN *join, table_map remaining_tables, uint idx,
                                                         double record_count, double read_time, uint search_depth,
                                                         uint use_cond_selectivity, table_map *processed_eq_ref_tables)
{
  THD *thd = join->thd;
  /*
  ...
  */
  ...
  DBUG_EXECUTE_IF("show_explain_probe_best_ext_lim_search",
                  if (dbug_user_var_equals_int(thd, "show_explain_probe_select_id", join->select_lex->select_number))
                      dbug_serve_apcs(thd, 1););

  if (unlikely(thd->check_killed()))  // Abort
    DBUG_RETURN(SEARCH_ABORT);

  DBUG_EXECUTE("opt", print_plan(join, idx, record_count, read_time, read_time, "part_plan"););
  ...
  sort_end = sort;
  {
    ...
    if (idx > join->const_tables && join->prune_level >= 2 && join->positions[idx - 1].type == JT_EQ_REF &&
        (join->eq_ref_tables & allowed_current_tables))
    {
      ...
      */
      ...
      if (get_costs_for_tables(join, remaining_tables, idx, record_count, &trace_one_table, best_ref, &sort_end,
                               &table_map, 1))
      ...
      else
      {
        ...
        if ((table_map = allowed_current_tables & ~table_map))
          get_costs_for_tables(join, remaining_tables, idx, record_count, &trace_one_table, best_ref, &sort_end,
                               &table_map, 0);
      }
    }
    else
    {
      ...
      get_costs_for_tables(join, remaining_tables, idx, record_count, &trace_one_table, best_ref, &sort_end, &table_map,
                           0);
    }
    ...
    */
    if (found_tables > 1)
      my_qsort(sort, found_tables, sizeof(SORT_POSITION), (qsort_cmp)sort_positions);
  }
  DBUG_ASSERT(join->next_sort_position <= join->sort_positions + join->sort_space);
  ...
  {
    s = *pos->join_tab;
    if (!(found_eq_ref_tables & s->table->map) && !check_interleaving_with_nj(s))
    {
      table_map real_table_bit = s->table->map;
      ...
      *position = *pos->position;  // Get stored result
      ...
      const double filter_cmp_gain =
          position->range_rowid_filter_info ? position->range_rowid_filter_info->get_cmp_gain(current_record_count) : 0;
      current_read_time =
          COST_ADD(read_time, COST_ADD(position->read_time - filter_cmp_gain, current_record_count / TIME_FOR_COMPARE));
      ...
      optimize_semi_joins(join, remaining_tables, idx, &current_record_count, &current_read_time, loose_scan_pos);

      /* Expand only partial plans with lower cost than the best QEP so far */
      if (current_read_time >= join->best_read)
      {
        DBUG_EXECUTE("opt",
                     print_plan(join, idx + 1, current_record_count, read_time, current_read_time, "prune_by_cost"););
      ...
      */
      if (join->prune_level >= 1)
      {
        ...
        if (join->extra_heuristic_pruning &&
            (!(position->key_dependent & allowed_tables) || position->records_read < 2.0))
        {
        ...
        else
        {
          ...
          */
          DBUG_EXECUTE("opt", print_plan(join, idx + 1, current_record_count, read_time, current_read_time,
                                         "pruned_by_heuristic"););
        ...
        if (!min_rec_hit && current_record_count >= min_rec_count && current_read_time >= min_rec_count_read_time)
          prune_reason = "min_record_count";

        if (!min_cost_hit && current_record_count >= min_cost_record_count && current_read_time >= min_cost)
      ...
      double pushdown_cond_selectivity = 1.0;
      if (use_cond_selectivity > 1)
        pushdown_cond_selectivity = table_cond_selectivity(join, idx, s, remaining_tables & ~real_table_bit);
      join->positions[idx].cond_selectivity = pushdown_cond_selectivity;

      partial_join_cardinality = (current_record_count * pushdown_cond_selectivity);
      ...
      if (unlikely(thd->trace_started()))
      {
        if (pushdown_cond_selectivity < 1.0)
        {
          ...
          trace_one_table.add("estimated_join_cardinality", partial_join_cardinality);
        }
      }
      ...
      if ((search_depth > 1) && ((remaining_tables & ~real_table_bit) & allowed_tables))
      {
        ...
        best_res = best_extension_by_limited_search(join, remaining_tables & ~real_table_bit, idx + 1,
                                                    partial_join_cardinality, current_read_time, search_depth - 1,
                                                    use_cond_selectivity, &found_eq_ref_tables);
        swap_variables(JOIN_TAB *, join->best_ref[idx], *pos->join_tab);

        if ((int)best_res < (int)SEARCH_OK)
          goto end;  // Return best_res
        if (best_res == SEARCH_FOUND_EDGE &&
            check_if_edge_table(join->positions + idx, pushdown_cond_selectivity) != SEARCH_FOUND_EDGE)
          best_res = SEARCH_OK;
      }
      else
      {
        ...
        */
        ...
        if (current_read_time < join->best_read)
        {
          memcpy((uchar *)join->best_positions, (uchar *)join->positions, sizeof(POSITION) * (idx + 1));
          join->join_record_count = partial_join_cardinality;
          join->best_read = current_read_time - COST_EPS;
        }
        DBUG_EXECUTE("opt",
                     print_plan(join, idx + 1, current_record_count, read_time, current_read_time, "full_plan"););
        best_res = check_if_edge_table(join->positions + idx, pushdown_cond_selectivity);
      }
      restore_prev_nj_state(s);
      restore_prev_sj_state(remaining_tables, s, idx);
      if (best_res == SEARCH_FOUND_EDGE)
...
int JOIN_TAB::make_scan_filter()
{
  ...
  Item *cond = is_inner_table_of_outer_join() ? *get_first_inner_table()->on_expr_ref : join->conds;

  if (cond &&
      (tmp = make_cond_for_table(join->thd, cond, join->const_table_map | table->map, table->map, -1, FALSE, TRUE)))
  {
    DBUG_EXECUTE("where", print_where(tmp, "cache", QT_ORDINARY););
    if (!(cache_select = (SQL_SELECT *)join->thd->memdup((uchar *)select, sizeof(SQL_SELECT))))
      DBUG_RETURN(1);
...
bool JOIN_TAB::keyuse_is_valid_for_access_in_chosen_plan(JOIN *join, KEYUSE *keyuse)
{
  if (!access_from_tables_is_allowed(keyuse->used_tables, join->sjm_lookup_tables))
    return false;
  if (join->sjm_scan_tables & table->map)
    return true;
  table_map keyuse_sjm_scan_tables = keyuse->used_tables & join->sjm_scan_tables;
  if (!keyuse_sjm_scan_tables)
    return true;
  uint sjm_tab_nr = 0;
  while (!(keyuse_sjm_scan_tables & table_map(1) << sjm_tab_nr)) sjm_tab_nr++;
  JOIN_TAB *sjm_tab = join->map2table[sjm_tab_nr];
  TABLE_LIST *emb_sj_nest = sjm_tab->emb_sj_nest;
  if (!(emb_sj_nest->sj_mat_info && emb_sj_nest->sj_mat_info->is_used && emb_sj_nest->sj_mat_info->is_sj_scan))
    return true;
...
{
  tab = next_breadth_first_tab(join->first_breadth_first_tab(), join->top_join_tab_count, tab);
  if (tab && tab->bush_root_tab)
    tab = NULL;
  return tab;
}

JOIN_TAB *first_linear_tab(JOIN *join, enum enum_with_bush_roots include_bush_roots,
...
JOIN_TAB *next_linear_tab(JOIN *join, JOIN_TAB *tab, enum enum_with_bush_roots include_bush_roots)
{
 ...
 */
  uint aggr_tables = (group_list ? 1 : 0) +
                     (select_distinct ? (tmp_table_param.using_outer_summary_function ? 2 : 1) : 0) + (order ? 1 : 0) +
  ...
  */
  ...
  if (!(join_tab = (JOIN_TAB *)thd->alloc(sizeof(JOIN_TAB) * (top_join_tab_count + aggr_tables))))
    DBUG_RETURN(TRUE);
  ...
  for (j = join_tab, tablenr = 0; tablenr < table_count; tablenr++, j++)
  {
    ...
    if (cur_pos->sj_strategy == SJ_OPT_MATERIALIZE || cur_pos->sj_strategy == SJ_OPT_MATERIALIZE_SCAN)
    {
      /*
      ...
      */
      ...
      if (!(jt = (JOIN_TAB *)thd->alloc(sizeof(JOIN_TAB) * sjm->tables)) || !(jt_range = new JOIN_TAB_RANGE))
        DBUG_RETURN(TRUE);
      jt_range->start = jt;
    ...
    *j = *best_positions[tablenr].table;
    ...
    {
      j->type = JT_ALL;
      if (best_positions[tablenr].use_join_buffer && tablenr != const_tables)
        full_join = 1;
    }
    ...
    if ((j->type == JT_REF || j->type == JT_EQ_REF) && is_hash_join_key_no(j->ref.key))
      hash_join = TRUE;
  ...
  for (j = join_tab, tablenr = 0; tablenr < table_count; tablenr++, j++)
  {
    ...
    {
      if ((keyuse = best_positions[tablenr].key) && create_ref_for_key(this, j, keyuse, TRUE, used_tables))
  ...
  top_join_tab_count = (uint)(join_tab_ranges.head()->end - join_tab_ranges.head()->start);
...
static bool create_hj_key_for_table(JOIN *join, JOIN_TAB *join_tab, KEYUSE *org_keyuse, table_map used_tables)
{
  KEY *keyinfo;
  ...
  do
  {
    if (!(~used_tables & keyuse->used_tables) && join_tab->keyuse_is_valid_for_access_in_chosen_plan(join, keyuse) &&
        are_tables_local(join_tab, keyuse->used_tables))
    {
      ...
      else
      {
        ...
        {
          if (curr->keypart == keyuse->keypart && !(~used_tables & curr->used_tables) &&
              join_tab->keyuse_is_valid_for_access_in_chosen_plan(join, curr) &&
  ...
  if (!(keyinfo = (KEY *)thd->alloc(sizeof(KEY))) ||
      !(key_part_info = (KEY_PART_INFO *)thd->alloc(sizeof(KEY_PART_INFO) * key_parts)))
    DBUG_RETURN(TRUE);
...
static bool are_tables_local(JOIN_TAB *jtab, table_map used_tables)
{
  if (jtab->bush_root_tab)
  {
    ...
    */
    table_map local_tables =
        jtab->emb_sj_nest->nested_join->used_tables | jtab->join->const_table_map | OUTER_REF_TABLE_BIT;
...
static bool create_ref_for_key(JOIN *join, JOIN_TAB *j, KEYUSE *org_keyuse, bool allow_full_scan, table_map used_tables)
{
  ...
  else
  {
    ...
    */
    do
    {
      if (!(~used_tables & keyuse->used_tables) && (!keyuse->validity_ref || *keyuse->validity_ref) &&
  ...
  else
  {
    ...
    {
      while (((~used_tables) & keyuse->used_tables) || (keyuse->validity_ref && !(*keyuse->validity_ref)) ||
             !j->keyuse_is_valid_for_access_in_chosen_plan(join, keyuse) || keyuse->keypart == NO_KEYPART ||
             (keyuse->keypart != (is_hash_join_key_no(key) ? keyinfo->key_part[i].field->field_index : i)) ||
      ...
      */
      ...
      if (keyuse->null_rejecting && (real->type() == Item::FIELD_ITEM) && ((Item_field *)real)->field->maybe_null())
      ...
      */
      ...
      {  // Compare against constant
        store_key_item tmp(thd, keyinfo->key_part[i].field, key_buff + maybe_null, maybe_null ? key_buff : 0,
                           keyinfo->key_part[i].length, keyuse->val, FALSE);
        if (unlikely(thd->is_fatal_error))
          DBUG_RETURN(TRUE);
        tmp.copy(thd);
      ...
      {
        *ref_key++ = get_store_key(thd, keyuse, join->const_table_map, &keyinfo->key_part[i], key_buff, maybe_null);
  ...
  *ref_key = 0;  // end_marker
  ...
  if (j->type == JT_CONST)
    j->table->const_table = 1;
             ...
             (keyparts > keyinfo->user_defined_key_parts && MY_TEST(key_flags & HA_EXT_NOSAME) &&
                                ...
                                uchar *key_buff, uint maybe_null)
{
  if (!((~used_tables) & keyuse->used_tables))  // if const item
  {
    ...
    return new store_key_field(thd, key_part->field, key_buff + maybe_null, maybe_null ? key_buff : 0, key_part->length,
                               ((Item_field *)keyuse->val->real_item())->field, keyuse->val->real_item()->full_name());
...
static void add_not_null_conds(JOIN *join)
{
  ...
  {
    if (tab->type == JT_REF || tab->type == JT_EQ_REF || tab->type == JT_REF_OR_NULL)
    {
      for (uint keypart = 0; keypart < tab->ref.key_parts; keypart++)
      {
        if (tab->ref.null_rejecting & ((key_part_map)1 << keypart))
        {
          ...
          */
          if (!(notnull = new (join->thd->mem_root) Item_func_isnotnull(join->thd, item)))
            DBUG_VOID_RETURN;
          /*
          ...
          */
          ...
          DBUG_EXECUTE("where",
                       print_where(notnull, (referred_tab ? referred_tab->table->alias.c_ptr() : "outer_ref_cond"),
                                   QT_ORDINARY););
          if (!tab->first_inner)
          {
            COND *new_cond =
                (referred_tab && referred_tab->join == join) ? referred_tab->select_cond : join->outer_ref_cond;
...
static COND *add_found_match_trig_cond(THD *thd, JOIN_TAB *tab, COND *cond, JOIN_TAB *root_tab)
{
  COND *tmp;
  DBUG_ASSERT(cond != 0);
  if (tab == root_tab)
...
bool TABLE_LIST::is_active_sjm() { return sj_mat_info && sj_mat_info->is_used; }
...
bool build_tmp_join_prefix_cond(JOIN *join, JOIN_TAB *last_tab, Item **ret)
{
  ...
  else if (last_tab->table->pos_in_table_list && last_tab->table->pos_in_table_list->embedding &&
           !last_tab->table->pos_in_table_list->embedding->sj_on_expr)
  {
    res = last_tab->table->pos_in_table_list->embedding->on_expr;
  }

  for (JOIN_TAB *tab = first_depth_first_tab(join); tab; tab = next_depth_first_tab(join, tab))
  {
    if (tab->select_cond)
    {
      if (!res)
      ...
      else
      {
        if (!all_conds)
        {
          if (!(all_conds = new (thd->mem_root) Item_cond_and(thd, res, tab->select_cond)))
            return true;
          res = all_conds;
        }
        else
...
static bool make_join_select(JOIN *join, SQL_SELECT *select, COND *cond)
{
  ...
  if (select)
  {
    ...
    {         /* there may be a select without a cond. */
      ...
      */
      {  // Check const tables
        ...
        const_cond = make_cond_for_table(thd, cond, join->const_table_map, (table_map)0, -1, FALSE, FALSE);
        /* Add conditions added by add_not_null_conds(). */
        for (uint i = 0; i < join->const_tables; i++) add_cond_and_fix(thd, &const_cond, join->join_tab[i].select_cond);
        ...
        if (const_cond)
        {
          ...
          {
            trace_const_cond.add("evaluated", "false").add("cause", "expensive cond");
          }
          else
          {
            ...
            if (!const_cond_result)
            {
              ...
              trace_const_cond.add("evaluated", "true").add("found", "impossible where");
        ...
        {
          COND *outer_ref_cond = make_cond_for_table(thd, cond, join->const_table_map | OUTER_REF_TABLE_BIT,
                                                     OUTER_REF_TABLE_BIT, -1, FALSE, FALSE);
          if (outer_ref_cond)
          {
        ...
        {
          COND *pseudo_bits_cond = make_cond_for_table(thd, cond, join->const_table_map | PSEUDO_TABLE_BITS,
                                                       PSEUDO_TABLE_BITS, -1, FALSE, FALSE);
          if (pseudo_bits_cond)
          {
            add_cond_and_fix(thd, &pseudo_bits_cond, join->pseudo_bits_cond);
    ...
    */
    ...
    i = join->const_tables;
    for (tab = first_depth_first_tab(join); tab; tab = next_depth_first_tab(join, tab))
    {
      bool is_hj;
      ...
      */
      if (tab->emb_sj_nest && tab->emb_sj_nest->sj_mat_info && tab->emb_sj_nest->sj_mat_info->is_used &&
      ...
      if (tab->type == JT_REF && tab->quick &&
          (((uint)tab->ref.key == tab->quick->index && tab->ref.key_length < tab->quick->max_used_key_length) ||
           (!is_hash_join_key_no(tab->ref.key) && tab->table->intersect_keys.is_set(tab->ref.key))))
      {
      ...
      if (cond)
      {
        if (tab->bush_children)
        {
          ...
          tmp = make_cond_after_sjm(thd, cond, cond, save_used_tables, used_tables, /*inside_or_clause=*/FALSE);
          used_tables = save_used_tables | used_tables;
          save_used_tables = 0;
        }
        else
        {
          tmp = make_cond_for_table(thd, cond, used_tables, current_map, i, FALSE, FALSE);
          if (tab == join->join_tab + last_top_base_tab_idx)
          {
            ...
            */
            ...
            COND *rand_cond = make_cond_for_table(thd, cond, used_tables, rand_table_bit, -1, FALSE, FALSE);
      ...
      is_hj = (tab->type == JT_REF || tab->type == JT_EQ_REF) &&
              ...
              ((join->max_allowed_join_cache_level + 1) / 2 == 2 ||
               ((join->max_allowed_join_cache_level + 1) / 2 > 2 && is_hash_join_key_no(tab->ref.key))) &&
              (!tab->emb_sj_nest || join->allowed_semijoin_with_cache) &&
              (!(tab->table->map & join->outer_join) || join->allowed_outer_join_with_cache);
      ...
      if (tmp || !cond || tab->type == JT_REF || tab->type == JT_REF_OR_NULL || tab->type == JT_EQ_REF ||
          first_inner_tab)
      {
        DBUG_EXECUTE("where", print_where(tmp, tab->table ? tab->table->alias.c_ptr() : "sjm-nest", QT_ORDINARY););
        SQL_SELECT *sel = tab->select = ((SQL_SELECT *)thd->memdup((uchar *)select, sizeof(*select)));
        if (!sel)
        ...
        */
        if (cond && tmp)
        {
          ...
          */
          COND *tmp_cond;
          if (!(tmp_cond = add_found_match_trig_cond(thd, first_inner_tab, tmp, 0)))
            DBUG_RETURN(1);
          sel->cond = tmp_cond;
          ...
          if (tab->table)
          {
            ...
            if ((tab->table->file->ha_table_flags() & HA_CAN_TABLE_CONDITION_PUSHDOWN) && !first_inner_tab)
            {
              Json_writer_object wrap(thd);
              Json_writer_object trace_cp(thd, "table_condition_pushdown");
              trace_cp.add_table_name(tab->table);

              COND *push_cond = make_cond_for_table(thd, tmp_cond, current_map, current_map, -1, FALSE, FALSE);
              if (push_cond)
              {
        ...
        if (tab->quick)
        {
          ...
          if ((tab->needed_reg.is_clear_all() && tab->type != JT_EQ_REF && tab->type != JT_FT &&
               ((tab->type != JT_CONST && tab->type != JT_REF) || (uint)tab->ref.key == tab->quick->index)) ||
              is_hj)
          {
        ...
        else if (tab->type == JT_ALL && !use_quick_range)
        {
          ...
          */
          ...
          if (!tab->table->is_filled_at_execution() && !tab->loosescan_match_tab &&  // (1)
              ((cond && (!tab->keys.is_subset(tab->const_keys) && i > 0)) ||
               (!tab->const_keys.is_clear_all() && i == join->const_tables &&
                join->unit->lim.get_select_limit() < join->best_positions[i].records_read &&
                !(join->select_options & OPTION_FOUND_ROWS))))
          {
            ...
            */
            ...
            if (sel->test_quick_select(
                    thd, tab->keys, ((used_tables & ~current_map) | OUTER_REF_TABLE_BIT),
                    (join->select_options & OPTION_FOUND_ROWS ? HA_POS_ERROR : join->unit->lim.get_select_limit()), 0,
                    FALSE, FALSE, FALSE) < 0)
            {
              /*
          ...
          if (i != join->const_tables && tab->use_quick != 2 && !tab->first_inner)
          { /* Read with cache */
            /*
      ...
      */
      JOIN_TAB *start_from =
          tab->bush_root_tab ? tab->bush_root_tab->bush_children->start : join->join_tab + join->const_tables;
      JOIN_TAB *end_with =
          tab->bush_root_tab ? tab->bush_root_tab->bush_children->end : join->join_tab + join->top_join_tab_count;
      for (JOIN_TAB *join_tab = start_from; join_tab != end_with; join_tab++)
      {
        if (*join_tab->on_expr_ref)
        {
          JOIN_TAB *cond_tab = join_tab->first_inner;
          COND *tmp_cond =
              make_cond_for_table(thd, *join_tab->on_expr_ref, join->const_table_map, (table_map)0, -1, FALSE, FALSE);
          if (!tmp_cond)
            continue;
          tmp_cond = new (thd->mem_root) Item_func_trig_cond(thd, tmp_cond, &cond_tab->not_null_compl);
          if (!tmp_cond)
            DBUG_RETURN(1);
          tmp_cond->quick_fix_field();
          cond_tab->select_cond = !cond_tab->select_cond ? tmp_cond
                                                         : new (thd->mem_root)
                                                               Item_cond_and(thd, cond_tab->select_cond, tmp_cond);
      ...
      */
      while (first_inner_tab && first_inner_tab->last_inner == last_tab)
      {
        ...
        */
        ...
        table_map used_tables2 = (join->const_table_map | OUTER_REF_TABLE_BIT | RAND_TABLE_BIT);

        start_from =
            tab->bush_root_tab ? tab->bush_root_tab->bush_children->start : join->join_tab + join->const_tables;
        for (JOIN_TAB *inner_tab = start_from; inner_tab <= last_tab; inner_tab++)
        {
          ...
          */
          COND *tmp_cond = make_cond_for_table(thd, on_expr, used_tables2, current_map,
                                               /*(inner_tab - first_tab)*/ -1, FALSE, FALSE);
          if (tab == last_tab)
          {
            /*
            ...
            */
            table_map rand_table_bit = (table_map)RAND_TABLE_BIT;
            COND *rand_cond = make_cond_for_table(thd, on_expr, used_tables2, rand_table_bit, -1, FALSE, FALSE);
          ...
          if (inner_tab->bush_children)
          {
            ...
            */
            SJ_MATERIALIZATION_INFO *sjm = inner_tab->bush_children->start->emb_sj_nest->sj_mat_info;
            if (sjm->is_used && !sjm->is_sj_scan)
              is_sjm_lookup_tab = TRUE;
          }

          if (inner_tab == first_inner_tab && inner_tab->on_precond && !is_sjm_lookup_tab)
            add_cond_and_fix(thd, &tmp_cond, inner_tab->on_precond);
          if (tmp_cond && !is_sjm_lookup_tab)
          {
            JOIN_TAB *cond_tab = (inner_tab < first_inner_tab ? first_inner_tab : inner_tab);
            Item **sel_cond_ref =
                (inner_tab < first_inner_tab ? &first_inner_tab->on_precond : &inner_tab->select_cond);
            /*
              First add the guards for match variables of
              all embedding outer join operations.
            */
            if (!(tmp_cond = add_found_match_trig_cond(thd, cond_tab->first_inner, tmp_cond, first_inner_tab)))
              DBUG_RETURN(1);
            /*
            ...
            */
            ...
            tmp_cond = new (thd->mem_root) Item_func_trig_cond(thd, tmp_cond, &first_inner_tab->not_null_compl);
            DBUG_PRINT("info", ("Item_func_trig_cond %p", tmp_cond));
            if (tmp_cond)
              tmp_cond->quick_fix_field();
            /* Add the predicate to other pushed down predicates */
            DBUG_PRINT("info", ("Item_cond_and"));
            *sel_cond_ref =
                !(*sel_cond_ref) ? tmp_cond : new (thd->mem_root) Item_cond_and(thd, *sel_cond_ref, tmp_cond);
            DBUG_PRINT("info", ("Item_cond_and %p", (*sel_cond_ref)));
            if (!(*sel_cond_ref))
              DBUG_RETURN(1);
...
static uint get_next_field_for_derived_key(uchar *arg)
{
  ...
  uint keypart = keyuse->keypart_map == (key_part_map)1 ? 0 : (keyuse - 1)->keypart + 1;
  for (; keyuse->table == table && keyuse->key == key && keyuse->keypart == fldno; keyuse++) keyuse->keypart = keypart;
  if (keyuse->key != key)
    keyuse = 0;
  *((KEYUSE **)arg) = keyuse;
  return fldno;
...
static uint get_next_field_for_derived_key_simple(uchar *arg)
{
  KEYUSE *keyuse = *(KEYUSE **)arg;
  if (!keyuse)
    return (uint)(-1);
  ...
  uint key = keyuse->key;
  uint fldno = keyuse->keypart;
  for (; keyuse->table == table && keyuse->key == key && keyuse->keypart == fldno; keyuse++)
    ;
  if (keyuse->key != key)
    keyuse = 0;
  *((KEYUSE **)arg) = keyuse;
...
static bool generate_derived_keys_for_table(KEYUSE *keyuse, uint count, uint keys)
{
  TABLE *table = keyuse->table;
  ...
  for (; i < count && key_count < keys;)
  {
    do
    {
      ...
      keyuse++;
      i++;
    } while (i < count && keyuse->used_tables == first_keyuse->used_tables && keyuse->keypart == prev_part);
    parts++;
    ...
    else
    {
      ...
      if (table->check_tmp_key(table->s->keys, parts, get_next_field_for_derived_key_simple, (uchar *)&first_keyuse))

      {
        JOIN_TAB *tab;
        first_keyuse = save_first_keyuse;
        if (table->add_tmp_key(table->s->keys, parts, get_next_field_for_derived_key, (uchar *)&first_keyuse, FALSE))
          return TRUE;
        table->reginfo.join_tab->keys.set_bit(table->s->keys);
        tab = table->reginfo.join_tab;
        for (uint i = 0; i < parts; i++) tab->key_dependent |= save_first_keyuse[i].used_tables;
      }
      else
      {
...
static void set_join_cache_denial(JOIN_TAB *join_tab)
{
  if (join_tab->cache)
  {
    /*
...
static void revise_cache_usage(JOIN_TAB *join_tab)
{
  ...
  if (join_tab->first_inner)
  {
    ...
    for (first_inner = join_tab->first_inner; first_inner; first_inner = first_inner->first_upper)
    {
      for (tab = end_tab; tab >= first_inner; tab--) set_join_cache_denial(tab);
      end_tab = first_inner;
    }
  }
...
enum_nested_loop_state end_sj_materialize(JOIN *join, JOIN_TAB *join_tab, bool end_of_records)
{
  int error;
  THD *thd = join->thd;
  ...
  if (!end_of_records)
  {
    ...
    if (unlikely((error = table->file->ha_write_tmp_row(table->record[0]))))
    {
      ...
      if (table->file->is_fatal_error(error, HA_CHECK_DUP) &&
          create_internal_tmp_table_from_heap(thd, table, sjm->sjm_table_param.start_recinfo,
...
static uint check_join_cache_usage(JOIN_TAB *tab, ulonglong options, uint no_jbuf_after, uint table_index,
                                   JOIN_TAB *prev_tab)
{
  Cost_estimate cost;
  uint flags = 0;
  ...
  bool force_unlinked_cache = !(join->allowed_join_cache_types & JOIN_CACHE_INCREMENTAL_BIT);
  bool no_hashed_cache = !(join->allowed_join_cache_types & JOIN_CACHE_HASHED_BIT);
  bool no_bka_cache = !(join->allowed_join_cache_types & JOIN_CACHE_BKA_BIT);
  ...
  */
  ...
  if (tab->is_inner_table_of_semijoin() && !join->allowed_semijoin_with_cache)
    goto no_join_cache;
  if (tab->is_inner_table_of_outer_join() && !join->allowed_outer_join_with_cache)
    goto no_join_cache;
  ...
  */
  ...
  for (JOIN_TAB *first_inner = tab->first_inner; first_inner; first_inner = first_inner->first_upper)
  {
    if (first_inner != tab && (!first_inner->use_join_cache || !(tab - 1)->use_join_cache))
      goto no_join_cache;
  }
  ...
  if (!prev_tab->use_join_cache)
  {
    ...
    */
    ...
    {
      for (JOIN_TAB *first_inner = tab[-1].first_inner; first_inner; first_inner = first_inner->first_upper)
      {
  ...
  switch (tab->type)
  {
    ...
    case JT_EQ_REF:
      ...
      if ((cache_level <= 4 && !no_hashed_cache) || no_bka_cache || tab->is_ref_for_hash_join() ||
          ((flags & HA_MRR_NO_ASSOCIATION) && cache_level <= 6))
      {
        if (!tab->hash_join_is_possible() || tab->make_scan_filter())
          goto no_join_cache;
        if (cache_level == 3)
          prev_cache = 0;
      ...
      if ((flags & HA_MRR_NO_ASSOCIATION) && (cache_level <= 6 || no_hashed_cache))
        goto no_join_cache;
...
void check_join_cache_usage_for_tables(JOIN *join, ulonglong options, uint no_jbuf_after)
{
  JOIN_TAB *tab;
  JOIN_TAB *prev_tab;
  ...
  restart:
    ...
    */
    ...
    switch (tab->type)
    {
      ...
      case JT_REF_OR_NULL:
      case JT_ALL:
        tab->used_join_cache_level = check_join_cache_usage(tab, options, no_jbuf_after, idx, prev_tab);
...
void JOIN_TAB::remove_redundant_bnl_scan_conds()
{
  if (!(select_cond && cache_select && cache &&
        (cache->get_join_alg() == JOIN_CACHE::BNL_JOIN_ALG || cache->get_join_alg() == JOIN_CACHE::BNLH_JOIN_ALG)))
    return;
  ...
  */
  ...
  if (is_cond_and(select_cond))
  {
    ...
    Item *pushed_item;
    Item_cond_and *reduced_select_cond = new (join->thd->mem_root) Item_cond_and(join->thd);
...
static bool make_join_readinfo(JOIN *join, ulonglong options, uint no_jbuf_after)
{
  ...
  if (!join->select_lex->sj_nests.is_empty() && setup_semijoin_dups_elimination(join, options, no_jbuf_after))
    DBUG_RETURN(TRUE); /* purecov: inspected */
       ...
       prev_tab = tab, tab = next_linear_tab(join, tab, WITH_BUSH_ROOTS))
  {
    ...
    */
    ...
    tab->partial_join_cardinality =
        join->best_positions[i].records_read * (prev_tab ? prev_tab->partial_join_cardinality : 1);
    if (!tab->bush_children)
      i++;
  }
  ...
  JOIN_TAB *first_tab;
  for (tab = first_tab = first_linear_tab(join, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
       tab = next_linear_tab(join, tab, WITH_BUSH_ROOTS))
  {
    if (tab->bush_children)
    {
    ...
    /*
      ...
      end_sj_materialize.
    */
    if (!(tab->bush_root_tab && tab->bush_root_tab->bush_children->end == tab + 1))
    {
    ...
    {
      if (!(tab->loosescan_buf = (uchar *)join->thd->alloc(tab->loosescan_key_len)))
    ...
    switch (tab->type)
    {
      ...
      case JT_CONST:   // Only happens with left join
        ...
        tab->read_first_record = tab->type == JT_SYSTEM ? join_read_system : join_read_const;
        tab->read_record.unlock_row = join_const_unlock_row;
        if (!(table->covering_keys.is_set(tab->ref.key) && !table->no_keyread) && (!jcl || jcl > 4) &&
        ...
        else
        {
          ...
          if (!table->no_keyread)
          {
            if (!(tab->select && tab->select->quick && tab->select->quick->index != MAX_KEY &&  // not index_merge
                ...
                (!table->covering_keys.is_clear_all() && !(tab->select && tab->select->quick)))
          ...
          if (tab->select && tab->select->quick && tab->select->quick->index != MAX_KEY &&
    ...
    DBUG_EXECUTE("where", char buff[256]; String str(buff, sizeof(buff), system_charset_info); str.length(0);
                 if (tab->table) str.append(tab->table->alias); else str.append(STRING_WITH_LEN("<no_table_name>"));
                 str.append(STRING_WITH_LEN(" final_pushdown_cond"));
                 print_where(tab->select_cond, str.c_ptr_safe(), QT_ORDINARY););
  }
  uint n_top_tables = (uint)(join->join_tab_ranges.head()->end - join->join_tab_ranges.head()->start);
  ...
  */
  for (tab = join->join_tab + join->const_tables; tab != join->join_tab + n_top_tables; tab++)
  {
    if (tab->use_join_cache)
    {
      JOIN_TAB *sort_by_tab = join->group && join->simple_group && join->group_list
      ...
      */
      ...
      if (sort_by_tab)
      {
        ...
        if (sort_by_tab->type == JT_NEXT && !sort_by_tab->table->covering_keys.is_set(sort_by_tab->index))
        {
          sort_by_tab->type = JT_ALL;
          sort_by_tab->read_first_record = join_init_read_record;
        }
        else if (sort_by_tab->type == JT_HASH_NEXT && !sort_by_tab->table->covering_keys.is_set(sort_by_tab->index))
        {
...
{
  for (JOIN_TAB *tab = first_top_level_tab(join, WITH_CONST_TABLES); tab; tab = next_top_level_tab(join, tab))
  {
    if (tab->type == JT_ALL && (!tab->select || !tab->select->quick))
    {
      my_message(ER_UPDATE_WITHOUT_KEY_IN_SAFE_MODE, ER_THD(join->thd, ER_UPDATE_WITHOUT_KEY_IN_SAFE_MODE), MYF(0));
      return (1);
    }
  }
  return (0);
...
void JOIN_TAB::cleanup()
{
  ...
  DBUG_PRINT("enter", ("tab: %p  table %s.%s", this, (table ? table->s->db.str : "?"),
  ...
  if (table && (table->s->tmp_table != INTERNAL_TMP_TABLE || table->is_created()))
  {
...
double JOIN_TAB::scan_time()
{
  ...
  if (table->is_created())
  {
    if (table->is_filled_at_execution())
    {
      get_delayed_table_estimates(table, &records, &read_time, &startup_cost);
...
ha_rows JOIN_TAB::get_examined_rows()
{
  ...
  else if (type == JT_NEXT || type == JT_ALL || type == JT_HASH || type == JT_HASH_NEXT)
  {
    if (limit)
    {
      /*
...
bool JOIN_TAB::preread_init()
{
  ...
  if ((!derived->get_unit()->executed || derived->is_recursive_with_table() || derived->get_unit()->uncacheable) &&
      mysql_handle_single_derived(join->thd->lex, derived, DT_CREATE | DT_FILL))
    DBUG_RETURN(TRUE);

  if (!(derived->get_unit()->uncacheable & UNCACHEABLE_DEPENDENT) || derived->is_nonrecursive_derived_with_rec_ref())
    preread_init_done = TRUE;
  ...
  DBUG_EXECUTE_IF("show_explain_probe_join_tab_preread",
                  if (dbug_user_var_equals_int(join->thd, "show_explain_probe_select_id",
                                               join->select_lex->select_number)) dbug_serve_apcs(join->thd, 1););
...
bool TABLE_REF::tmp_table_index_lookup_init(THD *thd, KEY *tmp_key, Item_iterator &it, bool value, uint skip)
{
  ...
  if (!(key_buff = (uchar *)thd->calloc(ALIGN_SIZE(tmp_key->key_length) * 2)) ||
      !(key_copy = (store_key **)thd->alloc((sizeof(store_key *) * (tmp_key_parts + 1)))) ||
      !(items = (Item **)thd->alloc(sizeof(Item *) * tmp_key_parts)))
    DBUG_RETURN(TRUE);
  ...
  for (i = 0; i < tmp_key_parts; i++, cur_key_part++, ref_key++)
  {
    ...
    *ref_key = new store_key_item(thd, cur_key_part->field,
                                  ...
                                  */
                                  cur_ref_buff + null_count, null_count ? cur_ref_buff : 0, cur_key_part->length,
                                  items[i], value);
...
void JOIN::join_free()
{
  ...
  */
  ...
  for (tmp_unit = select_lex->first_inner_unit(); tmp_unit; tmp_unit = tmp_unit->next_unit())
  {
  ...
  */
  if (can_unlock && lock && thd->lock && !thd->locked_tables_mode && !(select_options & SELECT_NO_UNLOCK) &&
      !select_lex->subquery_in_having &&
      (select_lex == (thd->lex->unit.fake_select_lex ? thd->lex->unit.fake_select_lex : thd->lex->first_select_lex())))
  {
    /*
...
void JOIN::cleanup(bool full)
{
  ...
  DBUG_PRINT("enter", ("select: %d (%p)  join: %p  full: %u", select_lex->select_number, select_lex, this, (uint)full));
      ...
      */
      if (top_join_tab_count && tables_list)
      {
        for (tab = first_breadth_first_tab(); tab;
             tab = next_breadth_first_tab(first_breadth_first_tab(), top_join_tab_count, tab))
        {
          tab->cleanup();
  ...
  if (full)
  {
    ...
    */
    ...
    if (!join_tab)
    {
      ...
      while ((table_ref = li++))
      {
        if (table_ref->table && table_ref->jtbm_subselect && table_ref->jtbm_subselect->is_jtbm_const_tab)
        {
...
{
  JOIN_TAB *join_tab;
  for (join_tab = first_linear_tab(join, WITH_BUSH_ROOTS, WITH_CONST_TABLES); join_tab;
...
static ORDER *remove_const(JOIN *join, ORDER *first_order, COND *cond, bool change_list, bool *simple_order)
{
  ...
  */
  if (join->join_tab)
  {
    ...
    */
    for (JOIN_TAB *tab = join->join_tab + join->const_tables; tab < join->join_tab + join->top_join_tab_count; tab++)
  ...
  for (order = first_order; order; order = order->next)
  {
    ...
    if (order->item[0]->with_sum_func() || order->item[0]->with_window_func() ||
        /*
          ...
          outer join.
         */
        (join->table_count > 1 && join->rollup.state == ROLLUP::STATE_INITED && join->outer_join))
    ...
    else if (!(order_tables & not_const_tables))
    {
      if (order->item[0]->with_subquery())
      {
        ...
        */
        join->exec_const_order_group_cond.push_back(order->item[0], join->thd->mem_root);
      }
      ...
      else
      {
        ...
        if (first_is_base_table && (ref = order_tables & (not_const_tables ^ first_table)))
        {
          ...
          */
          ...
          if (optimizer_flag(join->thd, OPTIMIZER_SWITCH_ORDERBY_EQ_PROP) && first_is_base_table &&
              order->item[0]->real_item()->type() == Item::FIELD_ITEM && join->cond_equal)
          {
            table_map first_table_bit = join->join_tab[join->const_tables].table->map;
            ...
            /*
              ...
              accordingly.
            */
            Item *res = item->propagate_equal_fields(join->thd, Value_source::Context_identity(), join->cond_equal);
            Item_equal *item_eq;
            if ((item_eq = res->get_item_equal()))
            {
              Item *first = item_eq->get_first(NO_PARTICULAR_TAB, NULL);
              if (first->const_item() || first->used_tables() == first_table_bit)
              {
    ...
    for (tmp_order = first_order; tmp_order != order; tmp_order = tmp_order->next)
    {
...
static int return_zero_rows(JOIN *join, select_result *result, List<TABLE_LIST> &tables, List<Item> &fields,
                            bool send_row, ulonglong select_options, const char *info, Item *having,
                            List<Item> &all_fields)
{
  ...
  if (!(result->send_result_set_metadata(fields, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF)))
  {
...
bool check_simple_equality(THD *thd, const Item::Context &ctx, Item *left_item, Item *right_item,
                           COND_EQUAL *cond_equal)
{
  ...
  if (left_item->type() == Item::REF_ITEM)
  {
    ...
    if (left_ref == Item_ref::VIEW_REF || left_ref == Item_ref::REF)
    {
      if (((Item_ref *)left_item)->get_depended_from())
        return FALSE;
      if (left_ref == Item_ref::VIEW_REF &&
          ((Item_direct_view_ref *)left_item)->get_null_ref_table() != NO_NULL_TABLE &&
  ...
  if (right_item->type() == Item::REF_ITEM)
  {
    ...
    if (right_ref == Item_ref::VIEW_REF || (right_ref == Item_ref::REF))
    {
      if (((Item_ref *)right_item)->get_depended_from())
        return FALSE;
      if (right_ref == Item_ref::VIEW_REF &&
          ((Item_direct_view_ref *)right_item)->get_null_ref_table() != NO_NULL_TABLE &&
  ...
  if (left_item->type() == Item::FIELD_ITEM && right_item->type() == Item::FIELD_ITEM &&
      !((Item_field *)left_item)->get_depended_from() && !((Item_field *)right_item)->get_depended_from())
  {
    ...
    else
    {
      ...
      else
      {
        ...
        Type_handler_hybrid_field_type tmp(orig_left_item->type_handler_for_comparison());
        if (tmp.aggregate_for_comparison(orig_right_item->type_handler_for_comparison()))
          return false;
        Item_equal *item_equal =
            new (thd->mem_root) Item_equal(thd, tmp.type_handler(), orig_left_item, orig_right_item, false);
...

  {
    ...
    if (left_item->type() == Item::FIELD_ITEM && !((Item_field *)left_item)->get_depended_from() &&
    ...
    else if (right_item->type() == Item::FIELD_ITEM && !((Item_field *)right_item)->get_depended_from() &&
    ...
    if (const_item && field_item->field->test_if_equality_guarantees_uniqueness(const_item))
    {
      /*
      ...
      */
      ...
      Item *const_item2 = field_item->field->get_equal_const_item(thd, ctx, const_item);
      if (!const_item2)
        return false;
      ...
      else
      {
        ...
        if (tmp.aggregate_for_comparison(orig_right_item->type_handler_for_comparison()))
          return false;
        item_equal = new (thd->mem_root) Item_equal(thd, tmp.type_handler(), const_item2, orig_field_item, true);
        item_equal->set_context_field(field_item);
        cond_equal->current_level.push_back(item_equal, thd->mem_root);
      }
      return TRUE;
...
static bool check_row_equality(THD *thd, const Arg_comparator *comparators, Item *left_row, Item_row *right_row,
                               COND_EQUAL *cond_equal, List<Item> *eq_list)
{
  ...
  for (uint i = 0; i < n; i++)
  {
    ...
    if (left_item->type() == Item::ROW_ITEM && right_item->type() == Item::ROW_ITEM)
    {
      /*
      ...
      */
      ...
      is_converted = check_row_equality(thd, comparators[i].subcomparators(), (Item_row *)left_item,
                                        (Item_row *)right_item, cond_equal, eq_list);
    }
    else
    {
      const Arg_comparator *tmp = &comparators[i];
      is_converted = check_simple_equality(
          thd, Item::Context(Item::ANY_SUBST, tmp->compare_type_handler(), tmp->compare_collation()), left_item,
          right_item, cond_equal);
    }

    if (!is_converted)
    {
      Item_func_eq *eq_item;
      if (!(eq_item = new (thd->mem_root) Item_func_eq(thd, left_item, right_item)) || eq_item->set_cmp_func(thd))
        return FALSE;
...
bool Item_func_eq::check_equality(THD *thd, COND_EQUAL *cond_equal, List<Item> *eq_list)
{
  ...
  if (left_item->type() == Item::ROW_ITEM && right_item->type() == Item::ROW_ITEM)
  {
    ...
    */
    if (left_item->get_item_splocal() || right_item->get_item_splocal())
      return false;
    return check_row_equality(thd, cmp.subcomparators(), (Item_row *)left_item, (Item_row *)right_item, cond_equal,
                              eq_list);
  }
  return check_simple_equality(thd, Context(ANY_SUBST, compare_type_handler(), compare_collation()), left_item,
                               right_item, cond_equal);
}
...
COND *Item_cond_and::build_equal_items(THD *thd, COND_EQUAL *inherited, bool link_item_fields,
 ...
 */
  ...
  /*
    ...
    (a=a AND b=b AND a=a).
  */
  if (!cond_args->elements && !cond_equal.current_level.elements && !eq_list.elements)
  ...
  while ((item_equal = it++))
  {
    ...
    set_if_bigger(thd->lex->current_select->max_equal_elems, item_equal->n_field_items());
  }
  ...
  */
  ...
  {
    Item *new_item;
    if ((new_item = item->build_equal_items(thd, inherited, false, NULL)) != item)
    {
...
COND *Item_cond::build_equal_items(THD *thd, COND_EQUAL *inherited, bool link_item_fields, COND_EQUAL **cond_equal_ref)
{
  List<Item> *cond_args = argument_list();
...
COND *Item_func_eq::build_equal_items(THD *thd, COND_EQUAL *inherited, bool link_item_fields,
                                      COND_EQUAL **cond_equal_ref)
{
  COND_EQUAL cond_equal;
  cond_equal.upper_levels = inherited;
  ...
  if (Item_func_eq::check_equality(thd, &cond_equal, &eq_list))
  {
    ...
    else if (n == 1)
    {
      if ((item_equal = cond_equal.current_level.pop()))
      {
        ...
        if (cond_equal_ref)
          *cond_equal_ref = new (thd->mem_root) COND_EQUAL(item_equal, thd->mem_root);
        return item_equal;
      }
  ...
  return Item_func::build_equal_items(thd, inherited, link_item_fields, cond_equal_ref);
}

COND *Item_func::build_equal_items(THD *thd, COND_EQUAL *inherited, bool link_item_fields, COND_EQUAL **cond_equal_ref)
{
  /*
...
COND *Item_equal::build_equal_items(THD *thd, COND_EQUAL *inherited, bool link_item_fields, COND_EQUAL **cond_equal_ref)
{
  COND *cond = Item_func::build_equal_items(thd, inherited, link_item_fields, cond_equal_ref);
  if (cond_equal_ref)
...
static COND *build_equal_items(JOIN *join, COND *cond, COND_EQUAL *inherited, List<TABLE_LIST> *join_list,
                               bool ignore_on_conds, COND_EQUAL **cond_equal_ref, bool link_equal_fields)
{
  THD *thd = join->thd;
  ...
  *cond_equal_ref = NULL;
  ...
  if (cond)
  {
    cond = cond->build_equal_items(thd, inherited, link_equal_fields, cond_equal_ref);
    if (*cond_equal_ref)
    {
  ...
  if (join_list && !ignore_on_conds)
  {
    ...
    while ((table = li++))
    {
      if (table->on_expr)
      {
        ...
        */
        table->on_expr =
            build_equal_items(join, table->on_expr, inherited, nested_join_list, ignore_on_conds, &table->cond_equal);
        if (unlikely(join->thd->trace_started()))
        {
          ...
          trace_condition(join->thd, "ON expr", "build_equal_items", table->on_expr, table_name);
        }
      }
    }
  }
...
static int compare_fields_by_table_order(Item *field1, Item *field2, void *table_join_idx)
{
  int cmp = 0;
  bool outer_ref = 0;
...
Item *eliminate_item_equal(THD *thd, COND *cond, COND_EQUAL *upper_levels, Item_equal *item_equal)
{
  List<Item> eq_list;
  ...
  DBUG_ASSERT(!cond || cond->is_bool_literal() ||
              (cond->type() == Item::FUNC_ITEM && ((Item_func *)cond)->functype() == Item_func::EQ_FUNC) ||
              (cond->type() == Item::COND_ITEM && ((Item_func *)cond)->functype() == Item_func::COND_AND_FUNC));
    ...
    */
    if (upper)
    {
      ...
      else
      {
        ...
        {
          if (embedding_sjm(item) == field_sjm && item->find_item_equal(upper_levels) == upper)
            break;
        }
      }
    ...
    if (produce_equality)
    {
      ...
      */
      Item *head_item = (!item_const && current_sjm && current_sjm_head != field_item) ? current_sjm_head : head;
      eq_item = new (thd->mem_root)
          Item_func_eq(thd, field_item->remove_item_direct_ref(), head_item->remove_item_direct_ref());
                                             ...
                                             void *table_join_idx, bool do_substitution)
{
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    ...
    {
      Item *new_item =
          substitute_for_best_equal_field(thd, context_tab, item, cond_equal, table_join_idx, do_substitution);
      /*
    ...
    if (and_level)
    {
      ...
      while ((item_equal = it++))
      {
        if (item_equal->get_extraction_flag() == MARKER_DELETION)
          continue;
        all_deleted = false;
        eq_cond = eliminate_item_equal(thd, eq_cond, cond_equal->upper_levels, item_equal);
        if (!eq_cond)
        {
          eq_cond = 0;
          break;
      ...
      if (eq_cond && !false_eq_cond)
      {
        ...
        if (eq_cond->type() == Item::COND_ITEM)
          ((Item_cond *)cond)->add_at_head(((Item_cond *)eq_cond)->argument_list());
        else
        {
          ...
          else
          {
            ...
            if (!eq_cond->is_bool_literal() && cond_list->push_front(eq_cond, thd->mem_root))
              eq_cond = 0;
          }
        }
      }
  ...
  else if (cond->type() == Item::FUNC_ITEM && ((Item_func *)cond)->functype() == Item_func::MULT_EQUAL_FUNC)
  {
  ...
  else if (do_substitution)
  {
    while (cond_equal)
    {
      ...
      while ((item_equal = it++))
      {
        ...
        if (!(cond = cond->transform(thd, &Item::replace_equal_field, (uchar *)&arg)))
          return 0;
      }
      cond_equal = cond_equal->upper_levels;
    }
...
static void update_const_equal_items(THD *thd, COND *cond, JOIN_TAB *tab, bool const_key)
{
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    ...
    while ((item = li++))
      update_const_equal_items(
          thd, item, tab, cond->is_top_level_item() && ((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC);
  }
  else if (cond->type() == Item::FUNC_ITEM && ((Item_func *)cond)->functype() == Item_func::MULT_EQUAL_FUNC)
  {
        ...
        */
        if (!possible_keys.is_clear_all())
        {
          ...
          for (use = stat->keyuse; use && use->table == field_tab; use++)
            if (const_key && !use->is_for_hash_join() && possible_keys.is_set(use->key) &&
                field_tab->key_info[use->key].key_part[use->keypart].field == field)
...
*/
static bool can_change_cond_ref_to_const(Item_bool_func2 *target, Item *target_expr, Item *target_value,
                                         Item_bool_func2 *source, Item *source_expr, Item *source_const)
{
  return target_expr->eq(source_expr, 0) && target_value != source_const &&
         target->compare_type_handler()->can_change_cond_ref_to_const(target, target_expr, target_value, source,
                                                                      source_expr, source_const);
}
...
static void change_cond_ref_to_const(THD *thd, I_List<COND_CMP> *save_list, Item *and_father, Item *cond,
                                     Item_bool_func2 *field_value_owner, Item *field, Item *value)
{
  if (cond->type() == Item::COND_ITEM)
  {
    bool and_level = ((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC;
    ...
    Item *item;
    while ((item = li++))
      change_cond_ref_to_const(thd, save_list, and_level ? cond : item, item, field_value_owner, field, value);
    return;
  }
  ...
  if (can_change_cond_ref_to_const(func, right_item, left_item, field_value_owner, field, value))
  {
  ...
  else if (can_change_cond_ref_to_const(func, left_item, right_item, field_value_owner, field, value))
  {
    Item *tmp = value->clone_item(thd);
    if (tmp)
    {
...
static void propagate_cond_constants(THD *thd, I_List<COND_CMP> *save_list, COND *and_father, COND *cond)
{
    ...
    {  // Handle other found items
      ...
      while ((cond_cmp = cond_itr++))
      {
        ...
        if (!args[0]->const_item())
          change_cond_ref_to_const(thd, &save, cond_cmp->and_level, cond_cmp->and_level, cond_cmp->cmp_func, args[0],
                                   args[1]);
      }
    }
  }
  else if (and_father != cond && cond->marker == MARKER_UNUSED)  // In a AND group
  {
                                            ...
                                            ((Item_func *)cond)->functype() == Item_func::EQUAL_FUNC))
    {
      ...
      if (!(left_const && right_const) && args[0]->cmp_type() == args[1]->cmp_type())
      {
        if (right_const)
        {
          resolve_const_item(thd, &args[1], args[0]);
          func->update_used_tables();
          change_cond_ref_to_const(thd, save_list, and_father, and_father, func, args[0], args[1]);
        }
        else if (left_const)
        {
          resolve_const_item(thd, &args[0], args[1]);
          func->update_used_tables();
          change_cond_ref_to_const(thd, save_list, and_father, and_father, func, args[1], args[0]);
        }
      }
    }
  }
    ...
    if ((nested_join = table->nested_join))
    {
      ...
      */
      if (table->on_expr)
      {
        ...
        */
        expr = simplify_joins(join, &nested_join->join_list, expr, FALSE, in_sj || table->sj_on_expr);
    ...
    if (prev_table)
    {
      ...
      if (prev_table->on_expr)
      {
        ...
        table_map prev_used_tables =
            prev_table->nested_join ? prev_table->nested_join->used_tables : prev_table->get_map();
        /*
        ...
        */
        if (!((prev_table->on_expr->used_tables() & ~(OUTER_REF_TABLE_BIT | RAND_TABLE_BIT)) & ~prev_used_tables))
          prev_table->dep_tables |= used_tables;
      }
    }
    prev_table = table;
    ...
    if ((nested_join = table->nested_join))
    {
      ...
      */
      if (nested_join->n_tables != 1)
      {
        ...
        first_unused = build_bitmap_for_nested_joins(&nested_join->join_list, first_unused);
      }
    }
  }
...
static uint reset_nj_counters(JOIN *join, List<TABLE_LIST> *join_list)
{
  ...
  while ((table = li++))
  {
    ...
    const table_map removed_tables = join->eliminated_tables | join->const_table_map;

    if ((table->nested_join && !is_eliminated_nest) || (!table->nested_join && (table->table->map & ~removed_tables)))
      n++;
  }
  DBUG_RETURN(n);
}
  ...
  */
  for (; next_emb && next_emb != join->emb_sjm_nest; next_emb = next_emb->embedding)
  {
    if (!next_emb->sj_on_expr)
    {
      ...
      if (next_emb->nested_join->n_tables != next_emb->nested_join->counter)
        break;
...
static void restore_prev_nj_state(JOIN_TAB *last)
{
  ...
  JOIN *join = last->join;
  for (; last_emb != NULL && last_emb != join->emb_sjm_nest; last_emb = last_emb->embedding)
  {
...
{
  TABLE_LIST *last_emb;
  if (idx > const_tables && (last_emb = positions[idx - 1].table->table->pos_in_table_list->embedding))
  {
    for (; last_emb && last_emb != emb_sjm_nest; last_emb = last_emb->embedding)
    {
      if (!last_emb->sj_on_expr)
      {
        NESTED_JOIN *nest = last_emb->nested_join;
...
void optimize_wo_join_buffering(JOIN *join, uint first_tab, uint last_tab, table_map last_remaining_tables,
                                bool first_alt, uint no_jbuf_before, double *outer_rec_count, double *reopt_cost)
{
  ...
  *outer_rec_count = rec_count;
  for (i = first_tab; i <= last_tab; i++) reopt_remaining_tables |= join->positions[i].table->table->map;
  ...
  */
  ...
  for (i = first_tab; i <= last_tab; i++)
  {
    ...
    if ((i == first_tab && first_alt) || join->positions[i].use_join_buffer)
    {
      ...
      best_access_path(join, rs, reopt_remaining_tables, join->positions, i, TRUE, rec_count, &pos, &loose_scan_pos);
    }
    else
    ...
    if (join->thd->variables.optimizer_use_condition_selectivity > 1)
    {
      pushdown_cond_selectivity = table_cond_selectivity(join, i, rs, reopt_remaining_tables & ~real_table_bit);
    }
...
static COND *optimize_cond(JOIN *join, COND *conds, List<TABLE_LIST> *join_list, bool ignore_on_conds,
                           Item::cond_result *cond_value, COND_EQUAL **cond_equal, int flags)
{
  THD *thd = join->thd;
  ...
  if (!conds)
  {
    *cond_value = Item::COND_TRUE;
    if (!ignore_on_conds)
      build_equal_items(join, NULL, NULL, join_list, ignore_on_conds, cond_equal);
  }
  else
  {
    /*
    ...
    */
    ...
    trace_cond.add("condition", join->conds == conds ? "WHERE" : "HAVING").add("original_condition", conds);

    Json_writer_array trace_steps(thd, "steps");
    DBUG_EXECUTE("where", print_where(conds, "original", QT_ORDINARY););
    conds = build_equal_items(join, conds, NULL, join_list, ignore_on_conds, cond_equal,
                              MY_TEST(flags & OPT_LINK_EQUAL_FIELDS));
    DBUG_EXECUTE("where", print_where(conds, "after equal_items", QT_ORDINARY););
    {
      Json_writer_object equal_prop_wrapper(thd);
      equal_prop_wrapper.add("transformation", "equality_propagation").add("resulting_condition", conds);
    }
    ...
    */
    DBUG_EXECUTE("where", print_where(conds, "after const change", QT_ORDINARY););
    {
      ...
      const_prop_wrapper.add("transformation", "constant_propagation").add("resulting_condition", conds);
    }
...

    {
      ...
      cond_removal_wrapper.add("transformation", "trivial_condition_removal").add("resulting_condition", conds);
    }
...
void propagate_new_equalities(THD *thd, Item *cond, List<Item_equal> *new_equalities, COND_EQUAL *inherited,
                              bool *is_simplifiable_cond)
{
  if (cond->type() == Item::COND_ITEM)
  {
    ...
    while ((item = li++))
    {
      COND_EQUAL *new_inherited =
          and_level && item->type() == Item::COND_ITEM ? &((Item_cond_and *)cond)->m_cond_equal : inherited;
      propagate_new_equalities(thd, item, new_equalities, new_inherited, is_simplifiable_cond);
    }
  }
  else if (cond->type() == Item::FUNC_ITEM && ((Item_func *)cond)->functype() == Item_func::MULT_EQUAL_FUNC)
  {
  ...
  {
    cond = cond->propagate_equal_fields(thd, Item::Context_boolean(), inherited);
    cond->update_used_tables();
  }
}
...
COND *Item_cond::remove_eq_conds(THD *thd, Item::cond_result *cond_value, bool top_level_arg)
{
  ...
  if (and_level)
  {
    ...
    */
    List<Item_equal> *cond_equalities = &((Item_cond_and *)this)->m_cond_equal.current_level;
  ...
  *cond_value = cond_arg_list->elements ? Item::COND_UNDEF : Item::COND_TRUE;
  ...
  while ((item = li++))
  {
    ...
    else if (item != new_item)
    {
      ...
      */
      if (new_item->type() == Item::COND_ITEM && item->type() == Item::COND_ITEM)
      {
        DBUG_ASSERT(functype() == ((Item_cond *)new_item)->functype());
        List<Item> *new_item_arg_list = ((Item_cond *)new_item)->argument_list();
        if (and_level)
        {
          /*
            ...
            of cond_arg_list.
          */
          List<Item_equal> *new_item_equalities = &((Item_cond_and *)new_item)->m_cond_equal.current_level;
      ...
      else if (and_level && new_item->type() == Item::FUNC_ITEM &&
               ((Item_func *)new_item)->functype() == Item_func::MULT_EQUAL_FUNC)
      {
        li.remove();
      ...
      {
        if (new_item->type() == Item::COND_ITEM && ((Item_cond *)new_item)->functype() == functype())
        {
          List<Item> *new_item_arg_list = ((Item_cond *)new_item)->argument_list();
          uint cnt = new_item_arg_list->elements;
          li.replace(*new_item_arg_list);
  ...
  if (!new_equalities.is_empty())
  {
    ...
    */
    ...
    propagate_new_equalities(thd, this, cond_equalities, cond_equal->upper_levels, &is_simplifiable_cond);
    /*
...
COND *Item::remove_eq_conds(THD *thd, Item::cond_result *cond_value, bool top_level_arg)
{
...
COND *Item_bool_func2::remove_eq_conds(THD *thd, Item::cond_result *cond_value, bool top_level_arg)
{
  if (can_eval_in_optimize())
  {
    *cond_value = eval_const_cond() ? Item::COND_TRUE : Item::COND_FALSE;
  ...
  if ((*cond_value = eq_cmp_result()) != Item::COND_OK)
  {
    ...
    {
      if (*cond_value == Item::COND_FALSE || !args[0]->maybe_null() || functype() == Item_func::EQUAL_FUNC)
...
COND *Item_func_isnull::remove_eq_conds(THD *thd, Item::cond_result *cond_value, bool top_level_arg)
{
  ...
  if (real_item->type() == Item::FIELD_ITEM)
  {
    ...
    if ((field->flags & NOT_NULL_FLAG) && field->type_handler()->cond_notnull_field_isnull_to_field_eq_zero())
    {
    ...
    */
    ...
    if (top_level_arg)  // "auto_increment_column IS NULL" is the only condition
    {
      if (field->flags & AUTO_INCREMENT_FLAG && !field->table->maybe_null &&
          ...
          (thd->first_successful_insert_id_in_prev_stmt > 0 && thd->substitute_null_with_insert_id))
      {
        ...
        if ((new_cond = new (thd->mem_root)
                 Item_func_eq(thd, args[0],
                              new (thd->mem_root)
                                  Item_int(thd, "last_insert_id()", thd->read_first_successful_insert_id_in_prev_stmt(),
...
{
  return (r->const_item() || !(r->used_tables() & ~OUTER_REF_TABLE_BIT)) && item_cmp_type(l, r) == l->cmp_type() &&
         (l->cmp_type() != STRING_RESULT || l->collation.collation == r->collation.collation);
}
...
*/
bool const_expression_in_where(COND *cond, Item *comp_item, Field *comp_field, Item **const_item)
{
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    ...
    while ((item = li++))
    {
      bool res = const_expression_in_where(item, comp_item, comp_field, const_item);
...
Field *Item::create_tmp_field_int(MEM_ROOT *root, TABLE *table, uint convert_int_length)
{
  ...
  return h->make_and_init_table_field(root, &name, Record_addr(maybe_null()), *this, table);
}

Field *Item::tmp_table_field_from_field_type_maybe_null(MEM_ROOT *root, TABLE *table, Tmp_field_src *src,
                                                        const Tmp_field_param *param, bool is_explicit_null)
{
  /*
...
Field *Item_sum::create_tmp_field(MEM_ROOT *root, bool group, TABLE *table)
{
  ...
  switch (cmp_type())
  {
    case REAL_RESULT:
    {
      new_field = new (root) Field_double(max_char_length(), maybe_null(), &name, decimals, TRUE);
      break;
    }
    case INT_RESULT:
    case TIME_RESULT:
...
Field *Item_field::create_tmp_field_from_item_field(MEM_ROOT *root, TABLE *new_table, Item_ref *orig_item,
                                                    const Tmp_field_param *param)
{
  DBUG_ASSERT(!is_result_field());
  Field *result;
  LEX_CSTRING *new_name = (orig_item ? &orig_item->name : !param->modify_item() ? &name : &field->field_name);

  /*
    If item have to be able to store NULLs but underlaid field can't do it,
    create_tmp_field_from_field() can't be used for tmp field creation.
  */
  if (((maybe_null() && in_rollup()) || (new_table->in_use->create_tmp_table_for_derived && /* for mat. view/dt */
      ...
      !field->maybe_null())
  {
    ...
    */
    ...
    const Type_handler *handler = type_handler()->type_handler_for_tmp_table(this);
    result = handler->make_and_init_table_field(root, new_name, rec, *this, new_table);
  }
  else if (param->table_cant_handle_bit_fields() && field->type() == MYSQL_TYPE_BIT)
  {
    const Type_handler *handler = Type_handler::type_handler_long_or_longlong(max_char_length(), true);
    result = handler->make_and_init_table_field(root, new_name, Record_addr(maybe_null()), *this, new_table);
  }
  else
  {
    bool tmp_maybe_null = param->modify_item() ? maybe_null() : field->maybe_null();
...
Field *Item_field::create_tmp_field_ex(MEM_ROOT *root, TABLE *table, Tmp_field_src *src, const Tmp_field_param *param)
{
  DBUG_ASSERT(!is_result_field());
  Field *result;
  src->set_field(field);
...
Field *Item_default_value::create_tmp_field_ex(MEM_ROOT *root, TABLE *table, Tmp_field_src *src,
                                               const Tmp_field_param *param)
{
...
Field *Item_ref::create_tmp_field_ex(MEM_ROOT *root, TABLE *table, Tmp_field_src *src, const Tmp_field_param *param)
{
  Item *item = real_item();
  DBUG_ASSERT(is_result_field());
  if (item->type() == Item::FIELD_ITEM)
  {
    ...
    if (!(result = field->create_tmp_field_from_item_field(root, table, this, &prm2)))
      return NULL;
...
void Item_result_field::get_tmp_field_src(Tmp_field_src *src, const Tmp_field_param *param)
{
...
Field *Item_result_field::create_tmp_field_ex_from_handler(MEM_ROOT *root, TABLE *table, Tmp_field_src *src,
                                                           const Tmp_field_param *param, const Type_handler *h)
{
  /*
    Possible Item types:
  ...
  */
  ...
  get_tmp_field_src(src, param);
  Field *result;
  if ((result = h->make_and_init_table_field(root, &name, Record_addr(maybe_null()), *this, table)) &&
      param->modify_item())
    result_field = result;
  return result;
}

Field *Item_func_sp::create_tmp_field_ex(MEM_ROOT *root, TABLE *table, Tmp_field_src *src, const Tmp_field_param *param)
{
  Field *result;
  get_tmp_field_src(src, param);
...
/**
  ...
  either direct or referenced by an Item_ref.
*/
Field *create_tmp_field(TABLE *table, Item *item, Item ***copy_func, Field **from_field, Field **default_field,
                        bool group, bool modify_item, bool table_cant_handle_bit_fields, bool make_copy_field)
{
  Tmp_field_src src;
  Tmp_field_param prm(group, modify_item, table_cant_handle_bit_fields, make_copy_field);
  Field *result = item->create_tmp_field_ex(table->in_use->mem_root, table, &src, &prm);
...
void setup_tmp_table_column_bitmaps(TABLE *table, uchar *bitmaps, uint field_count)
{
  ...
  bitmaps += bitmap_size;
  my_bitmap_init(&table->tmp_set, (my_bitmap_map *)bitmaps, field_count);
  bitmaps += bitmap_size;
  my_bitmap_init(&table->eq_join_set, (my_bitmap_map *)bitmaps, field_count);
  bitmaps += bitmap_size;
  my_bitmap_init(&table->cond_set, (my_bitmap_map *)bitmaps, field_count);
  bitmaps += bitmap_size;
...
Create_tmp_table::Create_tmp_table(ORDER *group, bool distinct, bool save_sum_fields, ulonglong select_options,
                                   ha_rows rows_limit)
...
TABLE *Create_tmp_table::start(THD *thd, TMP_TABLE_PARAM *param, const LEX_CSTRING *table_alias)
{
  ...
  DBUG_PRINT("enter",
             ("table_alias: '%s'  distinct: %d  save_sum_fields: %d  "
              "rows_limit: %lu  group: %d",
              table_alias->str, (int)m_distinct, (int)m_save_sum_fields, (ulong)m_rows_limit, MY_TEST(m_group)));

  if (use_temp_pool && !(test_flags & TEST_KEEP_TMP_TABLES))
    m_temp_pool_slot = temp_pool_set_next();

  if (m_temp_pool_slot != MY_BIT_NONE)  // we got a slot
    sprintf(path, "%s-%s-%lx-%i", tmp_file_prefix, param->tmp_name, current_pid, m_temp_pool_slot);
  else
  {
    /* if we run out of slots or we are not using tempool */
    sprintf(path, "%s-%s-%lx-%llx-%x", tmp_file_prefix, param->tmp_name, current_pid, thd->thread_id, thd->tmp_table++);
  }
  ...
  */
  ...
  if (m_group)
  {
    ...
    if (!param->quick_group)
      ...
      for (ORDER *tmp = m_group; tmp; tmp = tmp->next)
      {
  ...
  */
  ...
  init_sql_alloc(key_memory_TABLE, &own_root, TABLE_ALLOC_BLOCK_SIZE, 0, MYF(MY_THREAD_SPECIFIC));

  if (!multi_alloc_root(&own_root, &table, sizeof(*table), &share, sizeof(*share), &reg_field,
                        sizeof(Field *) * (field_count + 1), &m_default_field, sizeof(Field *) * (field_count),
                        &blob_field, sizeof(uint) * (field_count + 1), &m_from_field, sizeof(Field *) * field_count,
                        &param->items_to_copy, sizeof(param->items_to_copy[0]) * (copy_func_count + 1), &param->keyinfo,
                        sizeof(*param->keyinfo), &m_key_part_info, sizeof(*m_key_part_info) * (param->group_parts + 1),
                        &param->start_recinfo, sizeof(*param->recinfo) * (field_count * 2 + 4), &tmpname,
                        (uint)strlen(path) + 1, &m_group_buff,
                        (m_group && !m_using_unique_constraint ? param->group_length : 0), &m_bitmaps,
                        bitmap_buffer_size(field_count) * 6, &const_key_parts, sizeof(*const_key_parts), NullS))
  {
...
bool Create_tmp_table::add_fields(THD *thd, TABLE *table, TMP_TABLE_PARAM *param, List<Item> &fields)
{
  ...
  {
    uint uneven_delta;
    current_counter = (((param->hidden_field_count < (fieldnr + 1)) && distinct_record_structure &&
    ...
    if (not_all_columns)
    {
      if (item->with_sum_func() && type != Item::SUM_FUNC_ITEM)
      {
        if (item->used_tables() & OUTER_REF_TABLE_BIT)
          item->update_used_tables();
        if ((item->real_type() == Item::SUBSELECT_ITEM) || (item->used_tables() & ~OUTER_REF_TABLE_BIT))
        {
          /*
      ...
      if (item->const_item() && param->hidden_field_count < (fieldnr + 1))
    ...
    { /* Can't calc group yet */
      ...
      for (uint i = 0; i < sum_item->get_arg_count(); i++)
      {
        ...
        {
          Item *tmp_item;
          Field *new_field = create_tmp_field(table, arg, &copy_func, tmp_from_field, &m_default_field[fieldnr],
                                              m_group != 0, not_all_columns, distinct_record_structure, false);
          if (!new_field)
    ...
    else
    {
      ...
      */
      ...
      Field *new_field =
          create_tmp_field(table, item, &copy_func, tmp_from_field, &m_default_field[fieldnr], m_group != 0,
                           !param->force_copy_fields && (not_all_columns || m_group != 0),
                           /*
                           ...
                           */
                           item->marker == MARKER_NULL_KEY || param->bit_fields_as_long, param->force_copy_fields);
...
bool Create_tmp_table::choose_engine(THD *thd, TABLE *table, TMP_TABLE_PARAM *param)
{
  TABLE_SHARE *share = table->s;
  ...
  */
  ...
  if (share->blob_fields || m_using_unique_constraint ||
      ...
      (m_select_options & TMP_TABLE_FORCE_MYISAM) || thd->variables.tmp_memory_table_size == 0)
  {
...
bool Create_tmp_table::finalize(THD *thd, TABLE *table, TMP_TABLE_PARAM *param, bool do_not_open, bool keep_row_order)
{
  ...
  null_pack_length[other] = (m_null_count[other] + 7 + m_uneven_bit[other]) / 8;
  null_pack_base[other] = 0;
  null_pack_length[distinct] = (m_null_count[distinct] + 7 + m_uneven_bit[distinct]) / 8;
  null_pack_base[distinct] = null_pack_length[other];
  whole_null_pack_length = null_pack_length[other] + null_pack_length[distinct];
  share->reclength += whole_null_pack_length;
  if (!share->reclength)
    share->reclength = 1;  // Dummy select
  share->stored_rec_length = share->reclength;
  /* Use packed rows if there is blobs or a lot of space to gain */
  if (share->blob_fields || (string_total_length() >= STRING_TOTAL_LENGTH_TO_PACK_ROWS &&
...

  {
    ...
    if (!(table->record[0] = (uchar *)alloc_root(&table->mem_root, alloc_length * 3)))
      goto err;
  ...
  for (uint i = 0; i < share->fields; i++, recinfo++)
  {
    ...
    current_counter = ((field->flags & FIELD_PART_OF_TMP_UNIQUE) ? distinct : other);

    if (!(field->flags & NOT_NULL_FLAG))
    {
      recinfo->null_bit = (uint8)1 << (null_counter[current_counter] & 7);
      recinfo->null_pos = (null_pack_base[current_counter] + null_counter[current_counter] / 8);
    ...
    if (field->type() == MYSQL_TYPE_BIT)
    {
      ...
      ((Field_bit *)field)
          ->set_bit_ptr(null_flags + null_pack_base[current_counter] + null_counter[current_counter] / 8,
    ...
    if (m_default_field[i] && m_default_field[i]->ptr)
    {
      ...
      */
      ...
      else
      {
        ...
        */
        my_ptrdiff_t ptr_diff = (orig_field->table->s->default_values - orig_field->table->record[0]);
        field->set_notnull();
  ...
  if (thd->variables.tmp_memory_table_size == ~(ulonglong)0)  // No limit
    ...
    share->max_rows = (ha_rows)(((share->db_type() == heap_hton)
                                     ? MY_MIN(thd->variables.tmp_memory_table_size, thd->variables.max_heap_table_size)
  ...
  */
  ...
  if (m_group)
  {
    ...
    for (; cur_group; cur_group = cur_group->next, m_key_part_info++)
    {
      ...
      m_key_part_info->key_type = ((ha_base_keytype)m_key_part_info->type == HA_KEYTYPE_TEXT ||
      ...
      if (!m_using_unique_constraint)
      {
        ...
        if (!(cur_group->field = field->new_key_field(thd->mem_root, table, m_group_buff + MY_TEST(maybe_null),
                                                      m_key_part_info->length, field->null_ptr, field->null_bit)))
          goto err; /* purecov: inspected */
        ...
        if (maybe_null)
        {
          ...
          */
          ...
          m_key_part_info->null_offset = (uint)(field->null_ptr - (uchar *)table->record[0]);
    ...
    */
    DBUG_ASSERT(m_using_unique_constraint || m_group_buff <= param->group_buff + param->group_length);
  }

  if (m_distinct && (share->fields != param->hidden_field_count || m_with_cycle))
  {
    uint i;
    Field **reg_field;
    /*
    ...
    */
    ...
    keyinfo->user_defined_key_parts =
        m_field_count[distinct] + (share->uniques ? MY_TEST(null_pack_length[distinct]) : 0);
    keyinfo->ext_key_parts = keyinfo->user_defined_key_parts;
    keyinfo->usable_key_parts = keyinfo->user_defined_key_parts;
    table->distinct = 1;
    share->keys = 1;
    if (!(m_key_part_info =
              (KEY_PART_INFO *)alloc_root(&table->mem_root, keyinfo->user_defined_key_parts * sizeof(KEY_PART_INFO))))
      goto err;
    ...
    */
    ...
    if (!(keyinfo->rec_per_key = (ulong *)alloc_root(&table->mem_root, rpk_size)))
      goto err;
    ...
    */
    if (null_pack_length[distinct] && share->uniques)
    {
      ...
      m_key_part_info->field = new Field_string(table->record[0], (uint32)m_key_part_info->length, (uchar *)0, (uint)0,
                                                Field::NONE, &null_clex_str, &my_charset_bin);
    ...
    for (i = param->hidden_field_count, reg_field = table->field + i; i < share->fields; i++, reg_field++)
    {
      ...
      m_key_part_info->null_offset = (uint)((*reg_field)->null_ptr - (uchar *)table->record[0]);
  ...
  table->used_for_duplicate_elimination = (param->sum_func_count == 0 && (table->group || table->distinct));
  table->keep_row_order = keep_row_order;

  if (!do_not_open)
  {
    if (instantiate_tmp_table(table, param->keyinfo, param->start_recinfo, &param->recinfo, m_select_options))
      goto err;
  }
...
bool Create_tmp_table::add_schema_fields(THD *thd, TABLE *table, TMP_TABLE_PARAM *param,
...
TABLE *create_tmp_table(THD *thd, TMP_TABLE_PARAM *param, List<Item> &fields, ORDER *group, bool distinct,
                        ...
                        const LEX_CSTRING *table_alias, bool do_not_open, bool keep_row_order)
{
  TABLE *table;
  Create_tmp_table maker(group, distinct, save_sum_fields, select_options, rows_limit);
  if (!(table = maker.start(thd, param, table_alias)) || maker.add_fields(thd, table, param, fields) ||
...
TABLE *create_tmp_table_for_schema(THD *thd, TMP_TABLE_PARAM *param, const ST_SCHEMA_TABLE &schema_table,
                                   longlong select_options, const LEX_CSTRING &table_alias, bool do_not_open,
                                   bool keep_row_order)
{
  TABLE *table;
  Create_tmp_table maker((ORDER *)NULL, false, false, select_options, HA_POS_ERROR);
  if (!(table = maker.start(thd, param, &table_alias)) || maker.add_schema_fields(thd, table, param, schema_table) ||
      maker.finalize(thd, table, param, do_not_open, keep_row_order))
  {
    maker.cleanup_on_failure(thd, table);
    return NULL;
...
bool Virtual_tmp_table::init(uint field_count)
{
  ...
  if (!multi_alloc_root(in_use->mem_root, &s, sizeof(*s), &field, (field_count + 1) * sizeof(Field *), &blob_field,
                        (field_count + 1) * sizeof(uint), &bitmaps, bitmap_buffer_size(field_count) * 6, NullS))
    DBUG_RETURN(true);
  s->reset();
...
void Virtual_tmp_table::setup_field_pointers()
{
  ...
  for (Field **cur_ptr = field; *cur_ptr; ++cur_ptr)
  {
    ...
    if (cur_field->type() == MYSQL_TYPE_BIT && cur_field->key_type() == HA_KEYTYPE_BIT)
    {
...
bool Virtual_tmp_table::sp_find_field_by_name(uint *idx, const LEX_CSTRING &name) const
{
  Field *f;
  for (uint i = 0; (f = field[i]); i++)
  {
    // Use the same comparison style with sp_context::find_variable()
    if (!system_charset_info->strnncoll(f->field_name.str, f->field_name.length, name.str, name.length))
    {
      *idx = i;
      return false;
    }
...
bool Virtual_tmp_table::sp_find_field_by_name_or_error(uint *idx, const LEX_CSTRING &var_name,
                                                       const LEX_CSTRING &field_name) const
{
  if (sp_find_field_by_name(idx, field_name))
  {
    my_error(ER_ROW_VARIABLE_DOES_NOT_HAVE_FIELD, MYF(0), var_name.str, field_name.str);
    return true;
  }
  return false;
}

bool Virtual_tmp_table::sp_set_all_fields_from_item_list(THD *thd, List<Item> &items)
{
...
bool open_tmp_table(TABLE *table)
{
  int error;
  if (unlikely((
          error = table->file->ha_open(table, table->s->path.str, O_RDWR, HA_OPEN_TMP_TABLE | HA_OPEN_INTERNAL_TABLE))))
  {
...
bool create_internal_tmp_table(TABLE *table, KEY *keyinfo, TMP_ENGINE_COLUMNDEF *start_recinfo,
                               TMP_ENGINE_COLUMNDEF **recinfo, ulonglong options)
{
  int error;
  MARIA_KEYDEF keydef;
  ...
  {  // Get keys for ni_create
    ...
    HA_KEYSEG *seg = (HA_KEYSEG *)alloc_root(&table->mem_root, sizeof(*seg) * keyinfo->user_defined_key_parts);
    if (!seg)
      goto err;
    ...
    */
    if (keyinfo->key_length > table->file->max_key_length() ||
        keyinfo->user_defined_key_parts > table->file->max_key_parts() || share->uniques)
    {
      if (!share->uniques && !(keyinfo->flags & HA_NOSAME))
      {
        my_error(ER_INTERNAL_ERROR, MYF(0), "Using too big key for internal temp tables");
        DBUG_RETURN(1);
      }
    ...
    for (uint i = 0; i < keyinfo->user_defined_key_parts; i++, seg++)
    {
      ...
      else
      {
        ...
        if (field->real_type() == MYSQL_TYPE_STRING && keyinfo->key_part[i].length > 32)
  ...
  */
  {
    enum data_file_type file_type = table->no_rows ? NO_RECORD
                                                   : (share->reclength < 64 && !share->blob_fields ? STATIC_RECORD
                                                      : table->used_for_duplicate_elimination      ? DYNAMIC_RECORD
                                                                                                   : BLOCK_RECORD);
    uint create_flags =
        HA_CREATE_TMP_TABLE | HA_CREATE_INTERNAL_TABLE | (table->keep_row_order ? HA_PRESERVE_INSERT_ORDER : 0);
    ...
    if (unlikely(
            (error = maria_create(share->path.str, file_type, share->keys, &keydef, (uint)(*recinfo - start_recinfo),
                                  start_recinfo, share->uniques, &uniquedef, &create_info, create_flags))))
    {
      table->file->print_error(error, MYF(0)); /* purecov: inspected */
      table->db_stat = 0;
      goto err;
                               ...
                               TMP_ENGINE_COLUMNDEF **recinfo, ulonglong options)
{
  ...
  if (unlikely((error = mi_create(share->path.str, share->keys, &keydef, (uint)(*recinfo - start_recinfo),
                                  start_recinfo, share->uniques, &uniquedef, &create_info,
                                  HA_CREATE_TMP_TABLE | HA_CREATE_INTERNAL_TABLE |
                                      ((share->db_create_options & HA_OPTION_PACK_RECORD) ? HA_PACK_RECORD : 0)))))
  {
    table->file->print_error(error, MYF(0)); /* purecov: inspected */
    table->db_stat = 0;
    goto err;
...
bool create_internal_tmp_table_from_heap(THD *thd, TABLE *table, TMP_ENGINE_COLUMNDEF *start_recinfo,
                                         TMP_ENGINE_COLUMNDEF **recinfo, int error, bool ignore_last_dupp_key_error,
                                         bool *is_duplicate)
{
  TABLE new_table;
  TABLE_SHARE share;
  ...
  if (unlikely(!(new_table.file = get_new_handler(&share, &new_table.mem_root, TMP_ENGINE_HTON))))
    DBUG_RETURN(1);  // End of memory
  ...
  if (create_internal_tmp_table(&new_table, table->key_info, start_recinfo, recinfo,
                                thd->lex->first_select_lex()->options | thd->variables.option_bits))
    goto err2;
  ...
  */
  ...
  {
    if (new_table.file->is_fatal_error(write_err, HA_CHECK_DUP) || !ignore_last_dupp_key_error)
      goto err;
    if (is_duplicate)
      *is_duplicate = TRUE;
  }
...
void free_tmp_table(THD *thd, TABLE *entry)
{
  ...
  DBUG_PRINT("enter", ("table: %s  alias: %s", entry->s->table_name.str, entry->alias.c_ptr()));
  ...
  if (entry->file && entry->is_created())
  {
    if (entry->db_stat)
    {
      ...
      thd->tmp_tables_size += (entry->file->stats.data_file_length + entry->file->stats.index_file_length);
    }
...
void set_postjoin_aggr_write_func(JOIN_TAB *tab)
{
  ...
  if (table->group && tmp_tbl->sum_func_count && !tmp_tbl->precomputed_group_by)
  {
    /*
  ...
  else if (join->sort_and_group && !tmp_tbl->precomputed_group_by && !join->sort_and_group_aggr_tab &&
           join->tables_list && join->top_join_tab_count)
  {
  ...
  else
  {
    ...
    if (tmp_tbl->precomputed_group_by)
    {
      ...
      */
      memcpy(tmp_tbl->items_to_copy + tmp_tbl->func_count, join->sum_funcs, sizeof(Item *) * tmp_tbl->sum_func_count);
...
static int do_select(JOIN *join, Procedure *procedure)
{
  ...
  if (join->only_const_tables() && !join->need_tmp)
  {
    ...
    */
    ...
    else if (join->send_row_on_empty_set())
    {
      ...
      {
        List<Item> *columns_list = (procedure ? &join->procedure_fields_list : join->fields);
        rc = join->result->send_data_with_check(*columns_list, join->unit, 0) > 0;
      }
      /*
  ...
  else
  {
    ...
    JOIN_TAB *join_tab = join->join_tab + (join->tables_list ? join->const_tables : 0);
  ...
  if (error == NESTED_LOOP_NO_MORE_ROWS || unlikely(join->thd->killed == ABORT_QUERY))
  ...
  */
  if (join->order)
  {
    ...
    if (sort_tab->filesort && join->select_options & OPTION_FOUND_ROWS && sort_tab->filesort->sortorder &&
        sort_tab->filesort->limit != HA_POS_ERROR)
    {
...
bool instantiate_tmp_table(TABLE *table, KEY *keyinfo, TMP_ENGINE_COLUMNDEF *start_recinfo,
                           TMP_ENGINE_COLUMNDEF **recinfo, ulonglong options)
{
  if (table->s->db_type() == TMP_ENGINE_HTON)
  {
    ...
    */
    MEM_CHECK_DEFINED(table->record[0], table->s->reclength);
    if (create_internal_tmp_table(table, keyinfo, start_recinfo, recinfo, options))
      return TRUE;
...
enum_nested_loop_state sub_select_postjoin_aggr(JOIN *join, JOIN_TAB *join_tab, bool end_of_records)
{
...
enum_nested_loop_state sub_select_cache(JOIN *join, JOIN_TAB *join_tab, bool end_of_records)
{
  ...
  */
  ...
  if (end_of_records)
  {
    ...
    if (rc == NESTED_LOOP_OK || rc == NESTED_LOOP_NO_MORE_ROWS || rc == NESTED_LOOP_QUERY_LIMIT)
      rc = sub_select(join, join_tab, end_of_records);
    DBUG_RETURN(rc);
  }
...
enum_nested_loop_state sub_select(JOIN *join, JOIN_TAB *join_tab, bool end_of_records)
{
  ...
  if (join_tab->last_inner)
  {
    ...
    for (JOIN_TAB *jt = join_tab; jt <= last_inner_tab; jt++) jt->table->null_row = 0;
  }
  else
  ...
  if (end_of_records)
  {
    enum_nested_loop_state nls = (*join_tab->next_select)(join, join_tab + 1, end_of_records);
    DBUG_RETURN(nls);
  }
  ...
  for (SJ_TMP_TABLE *flush_dups_table = join_tab->flush_weedout_table; flush_dups_table;
  ...
  if (rc != NESTED_LOOP_NO_MORE_ROWS && (rc = join_tab_execution_startup(join_tab)) < 0)
    DBUG_RETURN(rc);
  ...
  */
  ...
  {
    if (join_tab->loosescan_match_tab && join_tab->loosescan_match_tab->found_match)
    {
      KEY *key = join_tab->table->key_info + join_tab->loosescan_key;
      key_copy(join_tab->loosescan_buf, join_tab->table->record[0], key, join_tab->loosescan_key_len);
      skip_over = TRUE;
    }
    ...
    {
      if (!key_cmp(join_tab->table->key_info[join_tab->loosescan_key].key_part, join_tab->loosescan_buf,
...
static enum_nested_loop_state evaluate_join_record(JOIN *join, JOIN_TAB *join_tab, int error)
{
  ...
  DBUG_PRINT("enter", ("evaluate_join_record join: %p  join_tab: %p  "
                       "cond: %p  abort: %d  alias %s",
                       join, join_tab, select_cond, error, join_tab->table->alias.ptr()));
  ...
  if (!select_cond || select_cond_result)
  {
    ...
    while (join_tab->first_unmatched && found)
    {
      ...
      */
      ...
      for (JOIN_TAB *tab = first_unmatched; tab <= join_tab; tab++)
      {
        ...
        */
        bool not_exists_opt_is_applicable = tab->table->reginfo.not_exists_optimize;
        for (JOIN_TAB *first_upper = first_unmatched->first_upper; not_exists_opt_is_applicable && first_upper;
    ...
    */
    join->join_examined_rows++;
    DBUG_PRINT("counts", ("join->examined_rows++: %lu  found: %d", (ulong)join->join_examined_rows, (int)found));
...
int report_error(TABLE *table, int error)
{
  ...
  */
  if (error != HA_ERR_LOCK_DEADLOCK && error != HA_ERR_LOCK_WAIT_TIMEOUT && error != HA_ERR_TABLE_DEF_CHANGED &&
      !table->in_use->killed)
    sql_print_error("Got error %d when reading table '%s'", error, table->s->path.str);
  table->file->print_error(error, MYF(0));
  return 1;
}
...
int safe_index_read(JOIN_TAB *tab)
{
  int error;
  TABLE *table = tab->table;
  if (unlikely((error = table->file->ha_index_read_map(table->record[0], tab->ref.key_buff,
...
static int join_read_const_table(THD *thd, JOIN_TAB *tab, POSITION *pos)
{
  ...
  if (tab->table->pos_in_table_list->is_materialized_derived() && !tab->table->pos_in_table_list->fill_me)
  {
...
static int join_read_system(JOIN_TAB *tab)
{
  ...
  {
    if (unlikely((error = table->file->ha_read_first_row(table->record[0], table->s->primary_key))))
    {
...
static int join_read_const(JOIN_TAB *tab)
{
  ...
  if (table->status & STATUS_GARBAGE)  // If first read
  {
    ...
    {
      error = table->file->ha_index_read_idx_map(table->record[0], tab->ref.key, (uchar *)tab->ref.key_buff,
                                                 make_prev_keypart_map(tab->ref.key_parts), HA_READ_KEY_EXACT);
    }
    if (unlikely(error))
    {
...
static int join_read_key(JOIN_TAB *tab) { return join_read_key2(tab->join->thd, tab, tab->table, &tab->ref); }
  ...
  */
      ...
      (table->status & (STATUS_GARBAGE | STATUS_NO_PARENT | STATUS_NULL_ROW)))
  {
    ...
    */
    ...
    error = table->file->ha_index_read_map(table->record[0], table_ref->key_buff,
...
void join_const_unlock_row(JOIN_TAB *tab) { DBUG_ASSERT(tab->type == JT_CONST); }
...
static int join_read_always_key(JOIN_TAB *tab)
{
  ...
  if (!table->file->inited)
  {
    if (unlikely((error = table->file->ha_index_init(tab->ref.key, tab->sorted))))
    {
      (void)report_error(table, error);
      return 1;
    }
  ...
  if (unlikely(cp_buffer_from_ref(tab->join->thd, table, &tab->ref)))
    return -1;
  if (unlikely((error = table->file->prepare_index_key_scan_map(tab->ref.key_buff,
                                                                make_prev_keypart_map(tab->ref.key_parts)))))
  {
    report_error(table, error);
    return -1;
  }
  if ((error = table->file->ha_index_read_map(table->record[0], tab->ref.key_buff,
                                              make_prev_keypart_map(tab->ref.key_parts), HA_READ_KEY_EXACT)))
  {
    if (error != HA_ERR_KEY_NOT_FOUND && error != HA_ERR_END_OF_FILE)
      return report_error(table, error);
...
static int join_read_last_key(JOIN_TAB *tab)
{
  ...
  if (!table->file->inited && unlikely((error = table->file->ha_index_init(tab->ref.key, tab->sorted))))
  {
    (void)report_error(table, error);
    return 1;
  }
  ...
  if (unlikely(
          (error = table->file->ha_index_read_map(table->record[0], tab->ref.key_buff,
                                                  make_prev_keypart_map(tab->ref.key_parts), HA_READ_PREFIX_LAST))))
  {
    if (error != HA_ERR_KEY_NOT_FOUND && error != HA_ERR_END_OF_FILE)
      return report_error(table, error);
    return -1; /* purecov: inspected */
...
static int join_read_next_same(READ_RECORD *info)
{
  ...
  if (unlikely((error = table->file->ha_index_next_same(table->record[0], tab->ref.key_buff, tab->ref.key_length))))
  {
    if (error != HA_ERR_END_OF_FILE)
      return report_error(table, error);
    table->status = STATUS_GARBAGE;
...
static int test_if_quick_select(JOIN_TAB *tab)
{
  ...
  int res = tab->select->test_quick_select(tab->join->thd, tab->keys, (table_map)0, HA_POS_ERROR, 0, FALSE,
                                           /*remove where parts*/ FALSE, FALSE);
...
int join_init_read_record(JOIN_TAB *tab)
{
  ...
  */
  ...
  DBUG_EXECUTE_IF("kill_join_init_read_record", tab->join->thd->set_killed(KILL_QUERY););
  ...
  if (tab->select && tab->select->quick && tab->select->quick->reset())
  {
    ...
    report_error(tab->table, tab->join->thd->killed ? HA_ERR_QUERY_INTERRUPTED : HA_ERR_OUT_OF_MEM);
    return 1;
  }
  /* make sure we won't get ER_QUERY_INTERRUPTED from any code below */
  DBUG_EXECUTE_IF("kill_join_init_read_record", tab->join->thd->reset_killed(););
  ...
  */
  ...
  if (init_read_record(&tab->read_record, tab->join->thd, tab->table, tab->select, tab->filesort_result, 1, 1, FALSE))
    return 1;
  ...
  if (need_unpacking)
  {
    tab->read_record.read_record_func_and_unpack_calls = tab->read_record.read_record_func;
...
bool JOIN_TAB::sort_table()
{
  int rc;
  DBUG_PRINT("info", ("Sorting for index"));
  THD_STAGE_INFO(join->thd, stage_creating_sort_index);
  DBUG_ASSERT(join->ordered_index_usage !=
              (filesort->order == join->order ? JOIN::ordered_index_order_by : JOIN::ordered_index_group_by));
...
static int join_read_first(JOIN_TAB *tab)
{
  ...
  DBUG_ASSERT(table->no_keyread || !table->covering_keys.is_set(tab->index) || table->file->keyread == tab->index);
  tab->table->status = 0;
  ...
  if (unlikely(error) || unlikely(error = tab->table->file->ha_index_first(tab->table->record[0])))
  {
    if (error != HA_ERR_KEY_NOT_FOUND && error != HA_ERR_END_OF_FILE)
      report_error(table, error);
    DBUG_RETURN(-1);
...
static int join_read_last(JOIN_TAB *tab)
{
  ...
  if (likely(!error))
    error = table->file->prepare_index_scan();
  if (unlikely(error) || unlikely(error = tab->table->file->ha_index_last(tab->table->record[0])))
...
static int join_ft_read_first(JOIN_TAB *tab)
{
  ...
  if (!table->file->inited && (error = table->file->ha_index_init(tab->ref.key, 1)))
  {
    (void)report_error(table, error);
    return 1;
  }
...
static enum_nested_loop_state end_send(JOIN *join, JOIN_TAB *join_tab, bool end_of_records)
{
  ...
  */
  ...
  if (join->send_records >= join->unit->lim.get_select_limit() && join->unit->lim.is_with_ties())
  {
    /*
  ...
  if (join->do_send_rows)
  {
    ...
    if (unlikely((error = join->result->send_data_with_check(*fields, join->unit, join->send_records))))
    {
      if (error > 0)
        DBUG_RETURN(NESTED_LOOP_ERROR);
  ...
  if (join->send_records >= join->unit->lim.get_select_limit())
  {
    if (!join->do_send_rows)
    {
      /*
        ...
        join_tab.
      */
      if (join->order && join->select_options & OPTION_FOUND_ROWS && join_tab > join->join_tab &&
    ...
    if (join->select_options & OPTION_FOUND_ROWS)
    {
      ...
      if ((join->table_count == 1) && !join->sort_and_group && !join->send_group_parts && !join->having &&
          !jt->select_cond && !(jt->select && jt->select->quick) &&
          (jt->table->file->ha_table_flags() & HA_STATS_RECORDS_IS_EXACT) && (jt->ref.key < 0))
      {
...
enum_nested_loop_state end_send_group(JOIN *join, JOIN_TAB *join_tab, bool end_of_records)
{
  ...
  {
    if (!join->group_sent && (join->first_record || (end_of_records && !join->group && !join->group_optimized_away)))
    {
      if (join->procedure)
      ...
      if (idx < (int)join->send_group_parts)
      {
        ...
        else
        {
          ...
          else
          {
            if (join->do_send_rows)
            {
              error = join->result->send_data_with_check(*fields, join->unit, join->send_records);
              if (unlikely(error < 0))
              {
        ...
        if (end_of_records)
          DBUG_RETURN(NESTED_LOOP_OK);
        if (join->send_records >= join->unit->lim.get_select_limit() && join->do_send_rows)
        {
          ...
          if (!join->unit->lim.is_with_ties() || idx < (int)join->with_ties_order_count)
          {
            if (!(join->select_options & OPTION_FOUND_ROWS))
              DBUG_RETURN(NESTED_LOOP_QUERY_LIMIT);  // Abort nicely
            join->do_send_rows = 0;
  ...
  if (!end_of_records)
  {
    ...
    if (likely(!join_tab->having || join_tab->having->val_int()))
    {
      ...
      if ((error = table->file->ha_write_tmp_row(table->record[0])))
      {
        ...
        bool is_duplicate;
        if (create_internal_tmp_table_from_heap(join->thd, table, join_tab->tmp_table_param->start_recinfo,
                                                &join_tab->tmp_table_param->recinfo, error, 1, &is_duplicate))
          DBUG_RETURN(NESTED_LOOP_ERROR);  // Not a table_is_full error
        if (is_duplicate)
          goto end;
        table->s->uniques = 0;  // To ensure rows are the same
      }
      if (++join_tab->send_records >= join_tab->tmp_table_param->end_write_records && join->do_send_rows)
      {
        if (!(join->select_options & OPTION_FOUND_ROWS))
          DBUG_RETURN(NESTED_LOOP_QUERY_LIMIT);
        join->do_send_rows = 0;
...
static enum_nested_loop_state end_update(JOIN *join, JOIN_TAB *join_tab __attribute__((unused)), bool end_of_records)
{
  ...
  for (group = table->group; group; group = group->next)
  {
    ...
    {
      DBUG_PRINT("info", ("new setup %p -> %p", group->fast_field_copier_setup, group->field));
      group->fast_field_copier_setup = group->field;
      group->fast_field_copier_func = item->setup_fast_field_copier(group->field);
    }
  ...
  if (!table->file->ha_index_read_map(table->record[1], join_tab->tmp_table_param->group_buff, HA_WHOLE_KEY,
                                      HA_READ_KEY_EXACT))
  { /* Update old record */
    restore_record(table, record[1]);
    update_tmptable_sum_func(join->sum_funcs, table);
    if (unlikely((error = table->file->ha_update_tmp_row(table->record[1], table->record[0]))))
    {
      table->file->print_error(error, MYF(0)); /* purecov: inspected */
      DBUG_RETURN(NESTED_LOOP_ERROR);          /* purecov: inspected */
    }
  ...
  {
    if (create_internal_tmp_table_from_heap(join->thd, table, join_tab->tmp_table_param->start_recinfo,
                                            &join_tab->tmp_table_param->recinfo, error, 0, NULL))
      DBUG_RETURN(NESTED_LOOP_ERROR);  // Not a table_is_full error
                                                ...
                                                bool end_of_records)
{
  ...
  else
  {
    ...
    if (!rnd_inited && ((error = table->file->ha_index_end()) || (error = table->file->ha_rnd_init(0))))
    {
      table->file->print_error(error, MYF(0));
      DBUG_RETURN(NESTED_LOOP_ERROR);
    }
    ...
    if (!rnd_inited && ((error = table->file->ha_rnd_end()) || (error = table->file->ha_index_init(0, 0))))
    {
      table->file->print_error(error, MYF(0));
      DBUG_RETURN(NESTED_LOOP_ERROR);
    }
...
enum_nested_loop_state end_write_group(JOIN *join, JOIN_TAB *join_tab __attribute__((unused)), bool end_of_records)
{
  TABLE *table = join_tab->table;
  int idx = -1;
  ...
  if (!join->first_record || end_of_records || (idx = test_if_group_changed(join->group_fields)) >= 0)
  {
    if (join->first_record || (end_of_records && !join->group))
    {
      ...
      if (idx < send_group_parts)
      {
        ...
        if (!join_tab->having || join_tab->having->val_int())
        {
          ...
          if (unlikely(error) &&
              create_internal_tmp_table_from_heap(join->thd, table, join_tab->tmp_table_param->start_recinfo,
                                                  &join_tab->tmp_table_param->recinfo, error, 0, NULL))
            DBUG_RETURN(NESTED_LOOP_ERROR);
        }
        if (unlikely(join->rollup.state != ROLLUP::STATE_NONE))
        {
          if (unlikely(join->rollup_write_data((uint)(idx + 1), join_tab->tmp_table_param, table)))
          {
            DBUG_RETURN(NESTED_LOOP_ERROR);
          }
        }
    ...
    if (idx < (int)join->send_group_parts)
    {
      ...
      if (unlikely(copy_funcs(join_tab->tmp_table_param->items_to_copy, join->thd)))
        DBUG_RETURN(NESTED_LOOP_ERROR);
      if (unlikely(init_sum_functions(join->sum_funcs, join->sum_funcs_end[idx + 1])))
        DBUG_RETURN(NESTED_LOOP_ERROR);
...
bool test_if_ref(Item *root_cond, Item_field *left_item, Item *right_item)
{
  ...
  if (!field->table->const_table && join_tab && !join_tab->is_ref_for_hash_join() &&
      (!join_tab->first_inner || *join_tab->first_inner->on_expr_ref == root_cond))
  {
    /*
    ...
    */
    ...
    if (ref_item && (ref_item->eq(right_item, 1) || ref_item->real_item()->eq(right_item, 1)))
    {
      right_item = right_item->real_item();
...
static Item *make_cond_for_table(THD *thd, Item *cond, table_map tables, table_map used_table, int join_tab_idx_arg,
                                 bool exclude_expensive_cond __attribute__((unused)), bool retain_ref_cond)
{
  return make_cond_for_table_from_pred(thd, cond, cond, tables, used_table, join_tab_idx_arg, exclude_expensive_cond,
                                       retain_ref_cond, true);
}

static Item *make_cond_for_table_from_pred(THD *thd, Item *root_cond, Item *cond, table_map tables,
                                           table_map used_table, int join_tab_idx_arg,
                                           bool exclude_expensive_cond __attribute__((unused)), bool retain_ref_cond,
...

{
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC)
    {
      ...
      while ((item = li++))
      {
        ...
        */
        ...
        Item *fix = make_cond_for_table_from_pred(thd, root_cond, item, tables, used_table, join_tab_idx_arg,
                                                  exclude_expensive_cond, retain_ref_cond, false);
        if (fix)
    ...
    {  // Or list
      ...
      while ((item = li++))
      {
        Item *fix = make_cond_for_table_from_pred(thd, root_cond, item, tables, 0L, join_tab_idx_arg,
                                                  exclude_expensive_cond, retain_ref_cond, false);
        if (!fix)
          return (COND *)0;  // Always true
        new_cond->argument_list()->push_back(fix, thd->mem_root);
  ...
  /*
    ...
    of the test
  */
  if ((cond->marker == MARKER_CHECK_ON_READ && !retain_ref_cond) || (cond->used_tables() & ~tables))
...
*/
static COND *make_cond_after_sjm(THD *thd, Item *root_cond, Item *cond, table_map tables, table_map sjm_tables,
                                 bool inside_or_clause)
{
  /*
  ...
  */
  if (!inside_or_clause)
  {
    ...
    if ((!(cond_used_tables & ~tables) || !(cond_used_tables & ~sjm_tables)))
      return (COND *)0;  // Already checked
  }
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC)
    {
      ...
      while ((item = li++))
      {
        Item *fix = make_cond_after_sjm(thd, root_cond, item, tables, sjm_tables, inside_or_clause);
        if (fix)
          new_cond->argument_list()->push_back(fix, thd->mem_root);
      }
      switch (new_cond->argument_list()->elements)
  ...
  */
  ...
  if (cond->marker == MARKER_CHECK_ON_READ || (cond->used_tables() & ~(tables | sjm_tables)))
    return (COND *)0;  // Can't check this yet
  if (cond->marker == MARKER_PROCESSED || cond->eq_cmp_result() == Item::COND_OK)
    return cond;  // Not boolean op
  ...
  */
  if (((Item_func *)cond)->functype() == Item_func::EQ_FUNC)
  {
    Item *left_item = ((Item_func *)cond)->arguments()[0]->real_item();
    Item *right_item = ((Item_func *)cond)->arguments()[1]->real_item();
    if (left_item->type() == Item::FIELD_ITEM && test_if_ref(root_cond, (Item_field *)left_item, right_item))
    {
      cond->marker = MARKER_CHECK_ON_READ;
      return (COND *)0;
    }
    if (right_item->type() == Item::FIELD_ITEM && test_if_ref(root_cond, (Item_field *)right_item, left_item))
    {
      cond->marker = MARKER_CHECK_ON_READ;
      return (COND *)0;
    }
...
static int test_if_order_by_key(JOIN *join, ORDER *order, TABLE *table, uint idx, uint *used_key_parts)
{
  ...
  if ((table->file->ha_table_flags() & HA_PRIMARY_KEY_IN_READ_INDEX) && table->key_info[idx].ext_key_part_map &&
      pk != MAX_KEY && pk != idx)
  {
    have_pk_suffix = true;
  }
  ...
  for (; order; order = order->next, const_key_parts >>= 1)
  {
    ...
    */
    ...
    if (have_pk_suffix && reverse == 0 &&  // all were =const so far
        key_parts == table->key_info[idx].ext_key_parts &&
        table->const_key_parts[pk] == PREV_BITS(uint, table->key_info[pk].user_defined_key_parts))
    {
      key_parts = 0;
    ...
    if (key_part->field != field)
    {
      ...
      */
      if (item_field->item_equal && item_field->item_equal->contains(key_part->field))
  ...
  if (reverse == -1 && !(table->file->index_flags(idx, user_defined_kp - 1, 1) & HA_READ_PREV))
...
static bool list_contains_unique_index(TABLE *table, bool (*find_func)(Field *, void *), void *data)
{
  ...
  {
    if (keynr == table->s->primary_key || (table->key_info[keynr].flags & HA_NOSAME))
    {
      KEY *keyinfo = table->key_info + keynr;
      KEY_PART_INFO *key_part, *key_part_end;

      for (key_part = keyinfo->key_part, key_part_end = key_part + keyinfo->user_defined_key_parts;
...
static bool find_field_in_order_list(Field *field, void *data)
{
  ...
  for (ORDER *tmp_group = group; tmp_group; tmp_group = tmp_group->next)
  {
    ...
    if (item->type() == Item::FIELD_ITEM && ((Item_field *)item)->field->eq(field))
    {
      part_found = 1;
      break;
    }
...
static bool find_field_in_item_list(Field *field, void *data)
{
  ...
  while ((item = li++))
  {
    if (item->real_item()->type() == Item::FIELD_ITEM && ((Item_field *)(item->real_item()))->field->eq(field))
    {
      part_found = 1;
      break;
    }
...
void compute_part_of_sort_key_for_equals(JOIN *join, TABLE *table, Item_field *item_field, key_map *col_keys)
{
  col_keys->clear_all();
  ...
  else
  {
    ...
    */
    ...
    {
      if ((cur_item_eq->used_tables() & needed_tbl_map) && cur_item_eq->contains(item_field->field))
      {
        item_eq = cur_item_eq;
  ...
  if (item_eq)
  {
    ...
    while ((item = it++))
    {
      if (item->type() == Item::FIELD_ITEM && ((Item_field *)item)->field->table == table)
      {
                                    ...
                                    const key_map *map)
{
  ...
  */
  ...
  for (ORDER *tmp_order = order; tmp_order; tmp_order = tmp_order->next)
  {
    ...
    */
    key_map col_keys;
    compute_part_of_sort_key_for_equals(tab->join, table, (Item_field *)item, &col_keys);
  ...
  if (tab->ref.key >= 0 && tab->ref.key_parts)
  {
    ...
    */
    if (tab->type == JT_REF_OR_NULL || tab->type == JT_FT || tab->ref.uses_splitting)
      goto use_filesort;
  }
  else if (select && select->quick)  // Range found by opt_range
  {
    ...
    */
    ...
    if (quick_type == QUICK_SELECT_I::QS_TYPE_INDEX_MERGE || quick_type == QUICK_SELECT_I::QS_TYPE_INDEX_INTERSECT ||
        quick_type == QUICK_SELECT_I::QS_TYPE_ROR_UNION || quick_type == QUICK_SELECT_I::QS_TYPE_ROR_INTERSECT)
    {
      /*
  ...
  if (ref_key >= 0 && ref_key != MAX_KEY)
  {
    ...
    if (!usable_keys.is_set(ref_key))
    {
      ...
      */
      ...
      if ((new_ref_key = test_if_subkey(order, table, ref_key, ref_key_parts, &usable_keys)) < MAX_KEY)
      {
        ...
        */
        if ((table->opt_range_keys.is_set(new_ref_key) && table->opt_range[new_ref_key].key_parts > ref_key_parts) ||
            !(tab->ref.key >= 0))
        {
          /*
        ...
        else
        {
          ...
          */
          ...
          if (create_ref_for_key(tab->join, tab, keyuse, FALSE, (tab->join->const_table_map | OUTER_REF_TABLE_BIT)))
            goto use_filesort;
    ...
    if (usable_keys.is_set(ref_key) &&
        (order_direction = test_if_order_by_key(tab->join, order, table, ref_key, &used_key_parts)))
  ...
  }
  {
    ...
    test_if_cheaper_ordering(tab, order, table, usable_keys, ref_key, select_limit, &best_key, &best_key_direction,
                             &select_limit, &best_key_parts, &saved_best_key_parts);
    ...
    */
    if (best_key < 0 || ((select_limit >= table_records) &&
                         (tab->type == JT_ALL && tab->join->table_count > tab->join->const_tables + 1) &&
        ...
        table->opt_range_keys.is_set(best_key) && best_key != ref_key)
    {
      ...
      */
      ...
      select->test_quick_select(
          join->thd, tmp_map, 0,
          join->select_options & OPTION_FOUND_ROWS ? HA_POS_ERROR : join->unit->lim.get_select_limit(), TRUE, FALSE,
          FALSE, FALSE);
...
check_reverse_order:
  ...
  */
  if (!no_changes)  // We are allowed to update QEP
  {
    if (best_key >= 0)
    {
      bool quick_created = (select && select->quick && select->quick != save_quick);
      ...
      if (!quick_created)
      {
        ...
        tab->index = best_key;
        tab->read_first_record = order_direction > 0 ? join_read_first : join_read_last;
        ...
        */
        if (tab->rowid_filter && table->file->is_clustering_key(tab->index))
        {
    ...
    if (order_direction == -1)  // If ORDER BY ... DESC
    {
      ...
      else if (tab->type != JT_NEXT && tab->type != JT_REF_OR_NULL && tab->ref.key >= 0 &&
...
int create_sort_index(THD *thd, JOIN *join, JOIN_TAB *tab, Filesort *fsort)
{
  ...
  if (select && tab->ref.key >= 0)
  {
    if (!select->quick)
    {
      ...
      else
      {
        ...
        */
        if (!(select->quick =
                  (tab->type == JT_FT ? get_ft_select(thd, table, tab->ref.key)
    ...
    else
    {
      ...
      // Update ref value
      if (unlikely(cp_buffer_from_ref(thd, table, &tab->ref) && thd->is_fatal_error))
  ...
  if (file_sort)
  {
    tab->records = join->select_options & OPTION_FOUND_ROWS ? file_sort->found_rows : file_sort->return_rows;
...

{
  ...
  for (Field **ptr = first_field; *ptr; ptr++) keylength += (*ptr)->sort_length() + (*ptr)->maybe_null();
  ...
  */
  ...
  if (table->s->db_type() == heap_hton ||
      (!table->s->blob_fields &&
       ((ALIGN_SIZE(keylength) + HASH_OVERHEAD) * table->file->stats.records < thd->variables.sortbuff_size)))
    error = remove_dup_with_hash_index(join->thd, table, field_count, first_field, keylength, having);
  else
...
static int remove_dup_with_compare(THD *thd, TABLE *table, Field **first_field, Item *having)
{
  handler *file = table->file;
  ...
  for (;;)
  {
    ...
    {
      my_message(ER_OUTOFMEMORY, ER_THD(thd, ER_OUTOFMEMORY), MYF(ME_FATAL));
      error = 0;
      goto err;
    }
    store_record(table, record[1]);
                                      ...
                                      Item *having)
{
  ...
  if (!my_multi_malloc(key_memory_hash_index_key_buffer, MYF(MY_WME), &key_buffer,
                       (uint)((key_length + extra_length) * (long)file->stats.records), &field_lengths,
                       (uint)(field_count * sizeof(*field_lengths)), NullS))
    DBUG_RETURN(1);

  for (ptr = first_field, field_length = field_lengths; *ptr; ptr++) (*field_length++) = (*ptr)->sort_length();

  if (my_hash_init(key_memory_hash_index_key_buffer, &hash, &my_charset_bin, (uint)file->stats.records, 0, key_length,
...
static bool cmp_buffer_with_ref(THD *thd, TABLE *table, TABLE_REF *tab_ref)
{
  ...
  if (!tab_ref->disable_cache)
  {
    ...
    no_prev_key = TRUE;
  if ((tab_ref->key_err = cp_buffer_from_ref(thd, table, tab_ref)) || no_prev_key)
    return 1;
...
static bool find_order_in_list(THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables, ORDER *order,
                               List<Item> &fields, List<Item> &all_fields, bool is_group_field, bool add_to_all_fields,
                               bool from_window_spec)
{
  ...
  if (select_item != not_found_item)
  {
    ...
    */
    if (resolution == RESOLVED_BEHIND_ALIAS && order_item->fix_fields_if_needed_for_order_by(thd, order->item))
      return TRUE;
    ...
    {
      from_field = find_field_in_tables(thd, (Item_ident *)order_item, tables, NULL, ignored_tables_list_t(NULL),
    ...
    else
    {
      ...
      */
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NON_UNIQ_ERROR, ER_THD(thd, ER_NON_UNIQ_ERROR),
                          ((Item_ident *)order_item)->field_name.str, thd->where);
    }
  }
  else if (from_window_spec)
  {
    Item **found_item = find_item_in_list(order_item, all_fields, &counter, REPORT_EXCEPT_NOT_FOUND, &resolution,
  ...
  */
  if (order_item->fix_fields_if_needed_for_order_by(thd, order->item) || thd->is_error())
...
int setup_order(THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables, List<Item> &fields,
                List<Item> &all_fields, ORDER *order, bool from_window_spec)
{
  SELECT_LEX *select = thd->lex->current_select;
  enum_parsing_place context_analysis_place = thd->lex->current_select->context_analysis_place;
  thd->where = "order clause";
  const bool for_union = select->master_unit()->is_unit_op() && select == select->master_unit()->fake_select_lex;
  for (uint number = 1; order; order = order->next, number++)
  {
    if (find_order_in_list(thd, ref_pointer_array, tables, order, fields, all_fields, false, true, from_window_spec))
      return 1;
    ...
    */
    ...
    if ((from_window_spec && item->with_sum_func() && item->type() != Item::SUM_FUNC_ITEM) || item->with_window_func())
    {
      item->split_sum_func(thd, ref_pointer_array, all_fields, SPLIT_SUM_SELECT);
    }
  }
  return 0;
}
...
int setup_group(THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables, List<Item> &fields,
                List<Item> &all_fields, ORDER *order, bool *hidden_group_fields, bool from_window_spec)
{
  enum_parsing_place context_analysis_place = thd->lex->current_select->context_analysis_place;
  *hidden_group_fields = 0;
  ...
  {
    if (find_order_in_list(thd, ref_pointer_array, tables, ord, fields, all_fields, true, true, from_window_spec))
      return 1;
    ...
    if (from_window_spec && (*ord->item)->with_sum_func() && (*ord->item)->type() != Item::SUM_FUNC_ITEM)
      (*ord->item)->split_sum_func(thd, ref_pointer_array, all_fields, SPLIT_SUM_SELECT);
  }
  if (thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY && context_analysis_place == IN_GROUP_BY)
  {
    /*
    ...
    */
    ...
    while (field && (item = li++))
    {
      if (item->type() != Item::SUM_FUNC_ITEM && item->marker != MARKER_UNDEF_POS && !item->const_item() &&
          !(item->real_item()->type() == Item::FIELD_ITEM && item->used_tables() & OUTER_REF_TABLE_BIT))
      {
        while (field)
        {
...
ORDER *create_distinct_group(THD *thd, Ref_ptr_array ref_pointer_array, ORDER *order_list, List<Item> &fields,
                             List<Item> &all_fields, bool *all_order_by_fields_used)
{
  List_iterator<Item> li(fields);
  Item *item;
  ...
  *all_order_by_fields_used = 1;
  while ((item = li++)) item->marker = MARKER_UNUSED; /* Marker that field is not used */
  ...
  while ((item = li++))
  {
    if (!item->const_item() && !item->with_sum_func() && item->marker == MARKER_UNUSED)
    {
      /*
  ...
  while ((field = li++))
  {
    ...
    else if (real_type == Item::SUM_FUNC_ITEM)
    {
      if (!field->const_item())
      {
        ...
        if (!sum_item->depended_from() || sum_item->depended_from() == select_lex)
        {
...
static bool fill_cached_item_list(THD *thd, List<Cached_item> *list, ORDER *order,
                                  uint max_number_of_elements = UINT_MAX)
{
  for (; order && max_number_of_elements; order = order->next, max_number_of_elements--)
  {
...
static bool alloc_order_fields(JOIN *join, ORDER *order, uint max_number_of_elements)
{
  return fill_cached_item_list(join->thd, &join->order_fields, order, max_number_of_elements);
}
...
bool setup_copy_fields(THD *thd, TMP_TABLE_PARAM *param, Ref_ptr_array ref_pointer_array,
                       ...
                       List<Item> &all_fields)
{
  ...
  if (param->field_count && !(copy = param->copy_field = new (thd->mem_root) Copy_field[param->field_count]))
    goto err2;
  ...
  for (i = 0; (pos = li++); i++)
  {
    ...
    */
    if (real_pos->type() == Item::FIELD_ITEM &&
        !(real_pos != pos && ((Item_ref *)pos)->ref_type() == Item_ref::AGGREGATE_REF))
    {
      Item_field *item;
      ...
      else
      {
        ...
        */
        field = item->field;
        item->result_field = field->make_new_field(thd->mem_root, field->table, 1);
        /*
    ...
    else if ((real_pos->type() == Item::FUNC_ITEM || real_pos->real_type() == Item::SUBSELECT_ITEM ||
              real_pos->type() == Item::CACHE_ITEM || real_pos->type() == Item::COND_ITEM) &&
...
void copy_fields(TMP_TABLE_PARAM *param)
{
  ...
  Item_copy *item;
  while ((item = (Item_copy *)it++)) item->copy();
}
...
bool JOIN::make_sum_func_list(List<Item> &field_list, List<Item> &send_result_set_metadata, bool before_group_by)
{
  ...
  while ((item = it++))
  {
    if (item->type() == Item::SUM_FUNC_ITEM && !item->const_item() &&
        (!((Item_sum *)item)->depended_from() || ((Item_sum *)item)->depended_from() == select_lex))
      *func++ = (Item_sum *)item;
  }
...
static bool change_to_use_tmp_fields(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &res_selected_fields,
                                     List<Item> &res_all_fields, uint elements, List<Item> &all_fields)
{
  ...
  for (uint i = 0; (item = it++); i++)
  {
    ...
    if ((item->with_sum_func() && item_type != Item::SUM_FUNC_ITEM) || item->with_window_func())
      item_field = item;
    else if (item_type == Item::FIELD_ITEM || item_type == Item::DEFAULT_VALUE_ITEM)
    {
      if (!(item_field = item->get_tmp_table_item(thd)))
        DBUG_RETURN(true);
    }
    else if (item_type == Item::FUNC_ITEM && ((Item_func *)item)->functype() == Item_func::SUSERVAR_FUNC)
    {
      ...
      if (field != NULL)
      {
         ...
         */
        Item_func_set_user_var *suv = new (thd->mem_root) Item_func_set_user_var(thd, (Item_func_set_user_var *)item);
...
static bool change_refs_to_tmp_fields(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &res_selected_fields,
                                      List<Item> &res_all_fields, uint elements, List<Item> &all_fields)
{
  List_iterator_fast<Item> it(all_fields);
  Item *item, *new_item;
...
static bool prepare_sum_aggregators(THD *thd, Item_sum **func_ptr, bool need_distinct)
{
  ...
  while ((func = *(func_ptr++)))
  {
    if (func->set_aggregator(thd, need_distinct && func->has_with_distinct() ? Aggregator::DISTINCT_AGGREGATOR
...
bool copy_funcs(Item **func_ptr, const THD *thd)
{
  ...
  {
    if (func->type() == Item::FUNC_ITEM && ((Item_func *)func)->with_window_func())
      continue;
...
static bool add_ref_to_table_cond(THD *thd, JOIN_TAB *join_tab)
{
  ...
  if (join_tab->select)
  {
    ...
    if (join_tab->select->pre_idx_push_select_cond)
    {
      Item *new_cond = and_conds(thd, cond_copy, join_tab->select->pre_idx_push_select_cond);
      if (new_cond->fix_fields_if_needed(thd, &new_cond))
        error = 1;
      join_tab->pre_idx_push_select_cond = join_tab->select->pre_idx_push_select_cond = new_cond;
    }
    join_tab->set_select_cond(cond, __LINE__);
  }
  else if ((join_tab->select = make_select(join_tab->table, 0, 0, cond, (SORT_INFO *)0, 0, &error)))
    join_tab->set_select_cond(cond, __LINE__);
...
static bool change_group_ref(THD *thd, Item_func *expr, ORDER *group_list, bool *changed)
{
  if (expr->argument_count())
  {
    ...
    for (arg = expr->arguments(), arg_end = expr->arguments() + expr->argument_count(); arg != arg_end; arg++)
    {
      Item *item = *arg;
      if (item->type() == Item::FIELD_ITEM || item->type() == Item::REF_ITEM)
      {
        ...
        for (group_tmp = group_list; group_tmp; group_tmp = group_tmp->next)
        {
          ...
          {
            Item *new_item;
            if (!(new_item = new (thd->mem_root) Item_ref(thd, context, group_tmp->item, null_clex_str, item->name)))
...
bool JOIN::rollup_init()
{
  ...
  */
  ...
  Item_null_result **null_items = static_cast<Item_null_result **>(thd->alloc(sizeof(Item *) * send_group_parts));

  rollup.null_items = Item_null_array(null_items, send_group_parts);
  rollup.ref_pointer_arrays = static_cast<Ref_ptr_array *>(
      thd->alloc((sizeof(Ref_ptr_array) + all_fields.elements * sizeof(Item *)) * send_group_parts));
  rollup.fields = static_cast<List<Item> *>(thd->alloc(sizeof(List<Item>) * send_group_parts));
  ...
  */
  ...
  for (i = 0; i < send_group_parts; i++)
  {
    for (j = 0; j < fields_list.elements; j++) rollup.fields[i].push_back(rollup.null_items[i], thd->mem_root);
  }
  ...
  for (i = send_group_parts; i-- > idx;)
  {
    ...
    if ((!having || having->val_int()))
    {
      if (send_records < unit->lim.get_select_limit() && do_send_rows &&
          (res = result->send_data_with_check(rollup.fields[i], unit, send_records)) > 0)
        return 1;
      if (!res)
        send_records++;
    }
...
int JOIN::rollup_write_data(uint idx, TMP_TABLE_PARAM *tmp_table_param_arg, TABLE *table_arg)
{
  ...
  for (i = send_group_parts; i-- > idx;)
  {
    ...
    if ((!having || having->val_int()))
    {
      ...
      if (unlikely((write_error = table_arg->file->ha_write_tmp_row(table_arg->record[0]))))
      {
        if (create_internal_tmp_table_from_heap(thd, table_arg, tmp_table_param_arg->start_recinfo,
                                                &tmp_table_param_arg->recinfo, write_error, 0, NULL))
          return 1;
      }
    }
  }
...
int print_explain_message_line(select_result_sink *result, uint8 options, bool is_analyze, uint select_number,
                               const char *select_type, ha_rows *rows, const char *message)
{
  ...
  item_list.push_back(new (mem_root) Item_int(thd, (int32)select_number), mem_root);
  item_list.push_back(new (mem_root) Item_string_sys(thd, select_type), mem_root);
  /* `table` */
  ...
  for (uint i = 0; i < 5; i++) item_list.push_back(item_null, mem_root);
  ...
  if (rows)
  {
    ...
    item_list.push_back(new (mem_root) Item_string_sys(thd, rows_str.ptr(), rows_str.length()), mem_root);
  }
  else
    item_list.push_back(item_null, mem_root);
  ...
  /* `Extra` */
  if (message)
    item_list.push_back(new (mem_root) Item_string_sys(thd, message), mem_root);
  else
    item_list.push_back(item_null, mem_root);
...
void explain_append_mrr_info(QUICK_RANGE_SELECT *quick, String *res)
{
  ...
  len = h->multi_range_read_explain_info(quick->mrr_flags, mrr_str_buf, sizeof(mrr_str_buf));
  if (len > 0)
  {
...
///////////////////////////////////////////////////////////////////////////////
int append_possible_keys(MEM_ROOT *alloc, String_list &list, TABLE *table, key_map possible_keys)
{
  uint j;
...
bool JOIN_TAB::save_explain_data(Explain_table_access *eta, table_map prefix_tables, bool distinct_arg,
                                 JOIN_TAB *first_top_tab)
{
  ...
  */
  ...
  if (filesort)
  {
    if (!(eta->pre_join_sort =
              new (thd->mem_root) Explain_aggr_filesort(thd->mem_root, thd->lex->analyze_stmt, filesort)))
      return 1;
  }
  ...
  else
  {
    ...
    */
    ...
    {
      TABLE_LIST *view_child = real_table->view->first_select_lex()->table_list.first;
  ...
  /* "partitions" column */
  {
    ...
    if (!table->derived_select_number && (part_info = table->part_info))
    {  // TODO: all thd->mem_root here should be fixed
      make_used_partitions_str(thd->mem_root, part_info, &eta->used_partitions, eta->used_partitions_list);
  ...
  if ((type == JT_ALL || type == JT_HASH) && tab_select && tab_select->quick && use_quick != 2)
  {
  ...
  */
  ...
  {
    eta->hash_next_key.set(thd->mem_root, &table->key_info[index], table->key_info[index].key_length);
  ...
  if (!key_info)
  {
    if (table_list && /* SJM bushes don't have table_list */
        table_list->schema_table && table_list->schema_table->i_s_requested_object & OPTIMIZE_I_S_TABLE)
    {
      IS_table_read_plan *is_table_read_plan = table_list->is_table_read_plan;
      StringBuffer<64> key_name_buf;
      if (is_table_read_plan->trivial_show_command || is_table_read_plan->has_db_lookup_value())
      {
      ...
      if (is_table_read_plan->trivial_show_command || is_table_read_plan->has_table_lookup_value())
      {
        if (is_table_read_plan->trivial_show_command || is_table_read_plan->has_db_lookup_value())
          key_name_buf.append(',');
  ...
  /* "rows" */
  if (table_list /* SJM bushes don't have table_list */ && table_list->schema_table)
  {
  ...
  if ((tab_type == JT_NEXT || tab_type == JT_CONST) && table->covering_keys.is_set(index))
    key_read = 1;
  ...
  else
  {
    ...
    if (tab_select)
    {
      if (use_quick == 2)
      {
        ...
        if (eta->range_checked_fer)
          eta->range_checked_fer->append_possible_keys_stat(thd->mem_root, table, keys);
      }
      else if (tab_select->cond || (cache_select && cache_select->cond))
      {
        const COND *pushed_cond = table->file->pushed_cond;

        if ((table->file->ha_table_flags() & HA_CAN_TABLE_CONDITION_PUSHDOWN) && pushed_cond)
        {
    ...
    if (key_read)
    {
      ...
      {
        QUICK_GROUP_MIN_MAX_SELECT *qgs = (QUICK_GROUP_MIN_MAX_SELECT *)tab_select->quick;
    ...
    {
      explain_append_mrr_info((QUICK_RANGE_SELECT *)(tab_select->quick), &eta->mrr_type);
    ...
    else if (do_firstmatch)
    {
      ...
      else
      {
        ...
        if (prev_table->derived_select_number)
        {
          ...
          /* Derived table name generation */
          size_t len = my_snprintf(namebuf, sizeof(namebuf) - 1, "<derived%u>", prev_table->derived_select_number);
...
bool save_agg_explain_data(JOIN *join, Explain_select *xpl_sel)
{
  ...
  for (uint i = 0; i < join->aggr_tables; i++, join_tab++)
  {
    ...
    {
      Explain_aggr_node *new_node = join_tab->window_funcs_step->save_explain_plan(thd->mem_root, is_analyze);
      if (!new_node)
        return 1;
...
int JOIN::save_explain_data_intern(Explain_query *output, bool need_tmp_table_arg, bool need_order_arg,
                                   bool distinct_arg, const char *message)
{
  JOIN *join = this; /* Legacy: this code used to be a non-member function */
  DBUG_ENTER("JOIN::save_explain_data_intern");
  DBUG_PRINT("info", ("Select %p (%u), type %s, message %s", join->select_lex, join->select_lex->select_number,
  ...
  if (message)
  {
    if (!(explain = new (output->mem_root) Explain_select(output->mem_root, thd->lex->analyze_stmt)))
      DBUG_RETURN(1);
#ifndef DBUG_OFF
  ...
  else
  {
    ...
    explain = xpl_sel = new (output->mem_root) Explain_select(output->mem_root, thd->lex->analyze_stmt);
    if (!explain)
      DBUG_RETURN(1);
    ...
    xpl_sel->is_lateral =
        ((select_lex->get_linkage() == DERIVED_TABLE_TYPE) && (select_lex->uncacheable & UNCACHEABLE_DEPENDENT));
    if (select_lex->master_unit()->derived)
    ...
    for (JOIN_TAB *tab = first_explain_order_tab(join); tab; tab = next_explain_order_tab(join, tab))
    {
      ...
      Explain_table_access *eta = (new (output->mem_root) Explain_table_access(output->mem_root));
      ...
      if (tab->bush_root_tab != prev_bush_root_tab)
      {
        if (tab->bush_root_tab)
        {
          ...
          */
          if (!(cur_parent = new (output->mem_root) Explain_basic_join(output->mem_root)))
            DBUG_RETURN(1);

          JOIN_TAB *first_child = tab->bush_root_tab->bush_children->start;
          cur_parent->select_id = first_child->emb_sj_nest->sj_subq_pred->get_identifier();
        }
        else
        {
          /*
  ...
  */
  if (!select_lex->pushdown_select && select_lex->type != pushed_derived_text)
    for (SELECT_LEX_UNIT *tmp_unit = join->select_lex->first_inner_unit(); tmp_unit; tmp_unit = tmp_unit->next_unit())
...
bool mysql_explain_union(THD *thd, SELECT_LEX_UNIT *unit, select_result *result)
{
  ...
  if (unit->is_unit_op() || unit->fake_select_lex)
  {
    ...
    if (!(res = unit->prepare(unit->derived, result, SELECT_NO_UNLOCK | SELECT_DESCRIBE)))
    {
      if (!is_pushed_union)
        res = unit->exec();
    }
  ...
  else
  {
    ...
    unit->set_limit(unit->global_parameters());
    res = mysql_select(thd, first->table_list.first, first->item_list, first->where,
                       first->order_list.elements + first->group_list.elements, first->order_list.first,
                       first->group_list.first, first->having, thd->lex->proc_list.first,
                       first->options | thd->variables.option_bits | SELECT_DESCRIBE, result, unit, first);
  }
...
static void print_table_array(THD *thd, table_map eliminated_tables, String *str, TABLE_LIST **table, TABLE_LIST **end,
                              enum_query_type query_type)
{
  ...
  for (TABLE_LIST **tbl = table + 1; tbl < end; tbl++)
  {
    ...
    */
    if (eliminated_tables && ((curr->table && (curr->table->map & eliminated_tables)) ||
                              (curr->nested_join && !(curr->nested_join->used_tables & ~eliminated_tables))))
    {
...
{
  return eliminated_tables && ((tbl->table && (tbl->table->map & eliminated_tables)) ||
                               (tbl->nested_join && !(tbl->nested_join->used_tables & ~eliminated_tables)));
}
...
static void print_join(THD *thd, table_map eliminated_tables, String *str, List<TABLE_LIST> *tables,
                       enum_query_type query_type)
{
  ...
  */
  ...
  if (!(table = static_cast<TABLE_LIST **>(thd->alloc(sizeof(TABLE_LIST *) * tables_to_print))))
  ...
  */
  DBUG_ASSERT(!eliminated_tables ||
              !(((*table)->table && ((*table)->table->map & eliminated_tables)) ||
                ((*table)->nested_join && !((*table)->nested_join->used_tables & ~eliminated_tables))));
  /*
  ...
  */
  ...
  print_table_array(thd, eliminated_tables, str, table, table + tables_to_print, query_type);
  DBUG_VOID_RETURN;
}
...
void Index_hint::print(THD *thd, String *str)
{
  ...
  if (key_name.length)
  {
    if (thd && !system_charset_info->strnncoll((const uchar *)key_name.str, key_name.length,
                                               (const uchar *)primary_key_name.str, primary_key_name.length))
...
void TABLE_LIST::print(THD *thd, table_map eliminated_tables, String *str, enum_query_type query_type)
{
  if (nested_join)
  {
    str->append('(');
  ...
  {
    if (jtbm_subselect->engine->engine_type() == subselect_engine::SINGLE_SELECT_ENGINE)
    {
      /*
  ...
  else
  {
    ...
    if (view_name.str)
    {
      ...
      if (!(belong_to_view && belong_to_view->compact_view_format) && !(query_type & QT_ITEM_IDENT_SKIP_DB_NAMES))
      {
...
void st_select_lex::print(THD *thd, String *str, enum_query_type query_type)
{
  ...
  if ((query_type & QT_SHOW_SELECT_NUMBER) && thd->lex->all_selects_list && thd->lex->all_selects_list->link_next &&
      select_number != FAKE_SELECT_LEX_ID)
  {
    ...
    if (thd->lex->describe & DESCRIBE_EXTENDED2)
    {
      ...
      if (master_unit()->fake_select_lex && master_unit()->first_select() == this)
      {
  ...
  */
  ...
  while ((item = it++))
  {
    ...
    if ((is_subquery_function() && !item->is_explicit_name()) || !item->name.str)
    {
      /*
    ...
    else
    {
      ...
      */
      if (top_level || item->is_explicit_name() || !check_column_name(item->name.str))
...
bool JOIN::change_result(select_result *new_result, select_result *old_result)
{
  ...
  {
    result = new_result;
    if (result->prepare(fields_list, select_lex->master_unit()) || result->prepare2(this))
      DBUG_RETURN(true); /* purecov: inspected */
    DBUG_RETURN(false);
  }
...
void JOIN::set_allowed_join_cache_types()
{
  ...
  allowed_semijoin_with_cache = optimizer_flag(thd, OPTIMIZER_SWITCH_SEMIJOIN_WITH_CACHE);
  allowed_outer_join_with_cache = optimizer_flag(thd, OPTIMIZER_SWITCH_OUTER_JOIN_WITH_CACHE);
...
void JOIN::save_query_plan(Join_plan_state *save_to)
{
  ...
  memcpy((uchar *)save_to->best_positions, (uchar *)best_positions, sizeof(POSITION) * (table_count + 1));
...
void JOIN::restore_query_plan(Join_plan_state *restore_from)
{
  ...
  memcpy((uchar *)best_positions, (uchar *)restore_from->best_positions, sizeof(POSITION) * (table_count + 1));
  /* Restore SJM nests */
  List_iterator<TABLE_LIST> it(select_lex->sj_nests);
  TABLE_LIST *tlist;
...
JOIN::enum_reopt_result JOIN::reoptimize(Item *added_where, table_map join_tables, Join_plan_state *save_to)
{
  ...
  if (update_ref_and_keys(thd, &added_keyuse, join_tab, table_count, added_where, ~outer_join, select_lex, &sargables))
  {
  ...
  if (!keyuse.buffer &&
      my_init_dynamic_array(thd->mem_root->psi_key, &keyuse, sizeof(KEYUSE), 20, 64, MYF(MY_THREAD_SPECIFIC)))
  {
    delete_dynamic(&added_keyuse);
    return REOPT_ERROR;
  }
  ...
  if (save_to)
  {
    ...
    if (size_t e = keyuse.elements)
      memcpy(keyuse.buffer, save_to->keyuse.buffer, e * keyuse.size_of_element);
  }

  /* Add the new access methods to the keyuse array. */
  memcpy(keyuse.buffer + keyuse.elements * keyuse.size_of_element, added_keyuse.buffer,
...
void JOIN::cache_const_exprs()
{
  ...
  if (conds)
    conds->compile(thd, &Item::cache_const_expr_analyzer, (uchar **)&analyzer_arg, &Item::cache_const_expr_transformer,
                   (uchar *)&cache_flag);
  cache_flag = FALSE;
  if (having)
    having->compile(thd, &Item::cache_const_expr_analyzer, (uchar **)&analyzer_arg, &Item::cache_const_expr_transformer,
                    (uchar *)&cache_flag);

  for (JOIN_TAB *tab = first_depth_first_tab(this); tab; tab = next_depth_first_tab(this, tab))
  {
    if (*tab->on_expr_ref)
    {
      cache_flag = FALSE;
      (*tab->on_expr_ref)
          ->compile(thd, &Item::cache_const_expr_analyzer, (uchar **)&analyzer_arg, &Item::cache_const_expr_transformer,
                    (uchar *)&cache_flag);
    }
  }
}
...
static bool get_range_limit_read_cost(const JOIN_TAB *tab, const TABLE *table, ha_rows table_records, uint keynr,
    ...
    if (tab)
    {
      ...
      if (kp > 0)
      {
        ...
        */
        ...
        if (ref_rows > 0)
        {
          double tmp = cost_for_index_read(tab->join->thd, table, keynr, ref_rows, (ha_rows)tab->worst_seeks);
          if (tmp < best_cost)
          {
            best_cost = tmp;
...
static bool test_if_cheaper_ordering(const JOIN_TAB *tab, ORDER *order, TABLE *table, key_map usable_keys, int ref_key,
                                     ha_rows select_limit_arg, int *new_key, int *new_key_direction,
                                     ha_rows *new_select_limit, uint *new_used_key_parts, uint *saved_best_key_parts)
{
  ...
  */
  ...
  Json_writer_object trace_wrapper(thd);
  Json_writer_object trace_cheaper_ordering(thd, "reconsidering_access_paths_for_index_ordering");
  ...
  */
  if (ref_key >= 0 && ref_key != MAX_KEY && tab->type == JT_REF)
  {
    ...
    */
    if (tab->ref.const_ref_part_map == make_prev_keypart_map(tab->ref.key_parts) &&
        table->opt_range_keys.is_set(ref_key) && table->opt_range[ref_key].key_parts == tab->ref.key_parts)
  ...
  for (nr = 0; nr < table->s->keys; nr++)
  {
    ...
    if (keys.is_set(nr) && (direction = test_if_order_by_key(join, order, table, nr, &used_key_parts)))
    {
      /*
      ...
      */
      ...
      bool is_covering = (table->covering_keys.is_set(nr) || (table->file->index_flags(nr, 0, 1) & HA_CLUSTERED_INDEX));
      /*
        ...
        queries too.
      */
      if (is_covering || select_limit != HA_POS_ERROR || (ref_key < 0 && (group || table->force_index)))
      {
        double rec_per_key;
        ...
        if (group)
        {
          ...
          */
          ...
          rec_per_key = used_key_parts ? keyinfo->actual_rec_per_key(used_key_parts - 1) : 1;
          ...
          if (used_pk_parts)
          {
            ...
            /*
              ...
              they are equal to 1.
            */
            if (used_key_parts == pkinfo->user_defined_key_parts || pkinfo->rec_per_key[0] == 0)
              rec_per_key = 1;
            if (rec_per_key > 1)
            {
        ...
        */
        {
          ...
          // estimate.
          refkey_rows_estimate = MY_MIN(refkey_rows_estimate, ha_rows(table_records * table->cond_selectivity));
        }
        ...
        if (select_limit > refkey_rows_estimate)
          ...
          select_limit = (ha_rows)(select_limit * (double)table_records / refkey_rows_estimate);
        ...
        */
        ...
        if (get_range_limit_read_cost(tab, table, table_records, nr, select_limit, &range_scan_time))
        {
        ...
        if ((ref_key < 0 && (group || table->force_index || is_covering)) || index_scan_time < read_time)
        {
          ha_rows quick_records = table_records;
          ha_rows refkey_select_limit =
              (ref_key >= 0 && !is_hash_join_key_no(ref_key) && table->covering_keys.is_set(ref_key))
          ...
          if (best_key < 0 ||
              (select_limit <= MY_MIN(quick_records, best_records) ? keyinfo->user_defined_key_parts < best_key_parts
...
uint get_index_for_order(ORDER *order, TABLE *table, SQL_SELECT *select, ha_rows limit, ha_rows *scanned_limit,
                         bool *need_sort, bool *reverse)
{
  ...
  if (select && select->quick)
  {
    ...
    uint used_key_parts;
    switch (test_if_order_by_key(NULL, order, table, select->quick->index, &used_key_parts))
    {
      ...
      case -1:  // desired order, but opposite direction
      {
        ...
        if ((reverse_quick = select->quick->make_reverse(used_key_parts)))
        {
  ...
  {  // check if some index scan & LIMIT is more efficient than filesort
    ...
    */
    ...
    int key, direction;
    if (test_if_cheaper_ordering(NULL, order, table, table->keys_in_use_for_order_by, -1, limit, &key, &direction,
                                 &limit) &&
...
ulong check_selectivity(THD *thd, ulong rows_to_read, TABLE *table, List<COND_STATISTIC> *conds)
{
  ulong count = 0;
  COND_STATISTIC *cond;
...
bool AGGR_OP::prepare_tmp_table()
{
  TABLE *table = join_tab->table;
  ...
  {
    if (instantiate_tmp_table(table, join_tab->tmp_table_param->keyinfo, join_tab->tmp_table_param->start_recinfo,
                              &join_tab->tmp_table_param->recinfo, join->select_options))
      return true;
    (void)table->file->extra(HA_EXTRA_WRITE_CACHE);
  }
  /* If it wasn't already, start index scan for grouping using table index. */
  if (!table->file->inited && table->group && join_tab->tmp_table_param->sum_func_count && table->s->keys)
...
enum_nested_loop_state AGGR_OP::put_record(bool end_of_records)
{
  // Lasy tmp table creation/initialization
  if (!join_tab->table->file->inited)
    if (prepare_tmp_table())
      return NESTED_LOOP_ERROR;
  enum_nested_loop_state rc = (*write_func)(join_tab->join, join_tab, end_of_records);
  return rc;
}
...
enum_nested_loop_state AGGR_OP::end_send()
{
  enum_nested_loop_state rc = NESTED_LOOP_OK;
  TABLE *table = join_tab->table;
  JOIN *join = join_tab->join;
...
void JOIN_TAB::partial_cleanup()
{
  ...
  if (table->is_created())
  {
    table->file->ha_index_or_rnd_end();
    DBUG_PRINT("info",
               ("close index: %s.%s  alias: %s", table->s->db.str, table->s->table_name.str, table->alias.c_ptr()));
    if (aggr)
    {
      int tmp = 0;
      if ((tmp = table->file->extra(HA_EXTRA_NO_CACHE)))
...
void JOIN::make_notnull_conds_for_range_scans()
{
  ...
  if (impossible_where || !optimizer_flag(thd, OPTIMIZER_SWITCH_NOT_NULL_RANGE_SCAN))
  {
    /* Complementary range analysis is not needed */
    DBUG_VOID_RETURN;
  }

  if (conds && build_notnull_conds_for_range_scans(this, conds, conds->used_tables()))
  {
    /*
  ...
  while ((tbl = li++))
  {
    if (tbl->on_expr)
    {
      ...
      else if (build_notnull_conds_for_range_scans(this, tbl->on_expr, tbl->table->map))
      {
        /*
          Found a IS NULL conjunctive predicate for a null-rejected field
...
static bool build_notnull_conds_for_range_scans(JOIN *join, Item *cond, table_map allowed)
{
  THD *thd = join->thd;

  DBUG_ENTER("build_notnull_conds_for_range_scans");

  for (JOIN_TAB *s = join->join_tab; s < join->join_tab + join->table_count; s++)
  {
    /* Clear all needed bitmaps to mark found fields */
    if ((allowed & s->table->map) && !(s->table->map && join->const_table_map))
  ...
  */
  for (JOIN_TAB *s = join->join_tab; s < join->join_tab + join->table_count; s++)
  {
    ...
    if (!(allowed & tab->map) || (s->table->map && join->const_table_map))
      continue;
    ...
    for (Field **field_ptr = tab->field; *field_ptr; field_ptr++)
    {
      ...
      if (!field_item)
        continue;
      Item *isnotnull_item = new (thd->mem_root) Item_func_isnotnull(thd, field_item);
      if (!isnotnull_item)
        continue;
    ...
    switch (notnull_list.elements)
    {
      ...
      case 1:
        ...
        break;
      default:
        notnull_cond = new (thd->mem_root) Item_cond_and(thd, notnull_list);
    }
...
static void build_notnull_conds_for_inner_nest_of_outer_join(JOIN *join, TABLE_LIST *nest_tbl)
{
  ...
  if (used_tables && build_notnull_conds_for_range_scans(join, nest_tbl->on_expr, used_tables))
  {
  ...
  while ((tbl = li++))
  {
    if (tbl->on_expr)
    {
      ...
      else if (build_notnull_conds_for_range_scans(join, tbl->on_expr, tbl->table->map))
        tbl->on_expr = (Item *)Item_false;
    }
  }
}
...
void JOIN::init_join_cache_and_keyread()
{
       ...
       tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS))
  {
    ...
    switch (tab->type)
    {
      ...
      case JT_ALL:
        SQL_SELECT *select;
        select = tab->select ? tab->select : (tab->filesort ? tab->filesort->select : NULL);
        if (select && select->quick && select->quick->index != MAX_KEY &&
            table->covering_keys.is_set(select->quick->index) && !table->no_keyread)
          table->file->ha_start_keyread(select->quick->index);
        break;
      case JT_HASH_NEXT:
      case JT_NEXT:
        if ((tab->read_first_record == join_read_first || tab->read_first_record == join_read_last) &&
...
{
  JOIN_TAB *tab = table->reginfo.join_tab;
  for (Copy_field *cp = tab->read_record.copy_field; cp != tab->read_record.copy_field_end; cp++) (*cp->do_copy)(cp);
}
...
static void fix_items_after_optimize(THD *thd, SELECT_LEX *select_lex)
{
  ...
  while (Item *item = li++) item->fix_after_optimize(thd);
}
...
static bool set_limit_for_unit(THD *thd, SELECT_LEX_UNIT *unit, ha_rows lim)
{
  ...
  gpar->limit_params.select_limit = new (thd->mem_root) Item_int(thd, lim, MAX_BIGINT_WIDTH);
  if (gpar->limit_params.select_limit == 0)
    return true;  // EOM
...
bool JOIN::optimize_upper_rownum_func()
{
  ...
  if (select_lex->master_unit()->global_parameters()->limit_params.offset_limit != NULL)
  ...
  */
  if (outer_select == NULL || !outer_select->with_rownum || (outer_select->options & SELECT_DISTINCT) ||
      outer_select->table_list.elements != 1 || outer_select->where == NULL ||
...
static bool check_rownum_usage(Item_func *func_item, longlong *limit, bool *inv_order)
{
  Item *arg1, *arg2;
  *inv_order = 0;
  ...
  if (arg1->type() == Item::FUNC_ITEM && ((Item_func *)arg1)->functype() == Item_func::ROWNUM_FUNC)
  {
  ...
  {
    arg2 = func_item->arguments()[1]->real_item();
    if (arg2->type() == Item::FUNC_ITEM && ((Item_func *)arg2)->functype() == Item_func::ROWNUM_FUNC)
    {
      *limit = arg1->val_int();
      *inv_order = 1;
      return *limit <= 0 || (ulonglong)*limit >= HA_POS_ERROR;
...
static void optimize_rownum(THD *thd, SELECT_LEX_UNIT *unit, Item *cond)
{
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC)
    {
      List_iterator<Item> li(*((Item_cond *)cond)->argument_list());
      Item *item;
      while ((item = li++)) optimize_rownum(thd, unit, item);
    }
    DBUG_VOID_RETURN;
  }
