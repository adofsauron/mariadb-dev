...
static bool init_fields(THD *thd, TABLE_LIST *tables, struct st_find_field *find_fields, uint count)
{
  ...
  for (; count--; find_fields++)
  {
    ...
    Item_field *field = (new (thd->mem_root) Item_field(thd, context, {STRING_WITH_LEN("mysql")},
                                                        Lex_cstring_strlen(find_fields->table_name),
                                                        Lex_cstring_strlen(find_fields->field_name)));
    if (!(find_fields->field = find_field_in_tables(thd, field, tables, NULL, ignored_tables_list_t(NULL), 0,
                                                    REPORT_ALL_ERRORS, 1, TRUE)))
      DBUG_RETURN(1);
    bitmap_set_bit(find_fields->field->table->read_set, find_fields->field->field_index);
    /* To make life easier when setting values in keys */
    bitmap_set_bit(find_fields->field->table->write_set, find_fields->field->field_index);
  }
  DBUG_RETURN(0);
}
                  ...
                  String *name, String *description, String *example)
{
  ...
  if (init_read_record(&read_record_info, thd, topics, select, NULL, 1, 0, FALSE))
    DBUG_RETURN(0);
...
int search_keyword(THD *thd, TABLE *keywords, struct st_find_field *find_fields, SQL_SELECT *select, int *key_id)
{
  ...
  /* Should never happen. As this is part of help, we can ignore this */
  if (init_read_record(&read_record_info, thd, keywords, select, NULL, 1, 0, FALSE))
    DBUG_RETURN(0);
                           ...
                           List<String> *names, String *name, String *description, String *example)
{
  ...
  if ((iindex_topic = find_type(primary_key_name.str, &topics->s->keynames, FIND_TYPE_NO_PREFIX) - 1) < 0 ||
      (iindex_relations = find_type(primary_key_name.str, &relations->s->keynames, FIND_TYPE_NO_PREFIX) - 1) < 0)
  {
  ...
  if (topics->file->ha_index_init(iindex_topic, 1) || relations->file->ha_index_init(iindex_relations, 1))
  {
  ...
  int key_res = relations->file->ha_index_read_map(relations->record[0], buff, (key_part_map)1, HA_READ_KEY_EXACT);
       ...
       key_res = relations->file->ha_index_next(relations->record[0]))
  {
    ...
    if (!topics->file->ha_index_read_map(topics->record[0], topic_id_buff, (key_part_map)1, HA_READ_KEY_EXACT))
    {
      memorize_variant_topic(thd, topics, count, find_fields, names, name, description, example);
      count++;
    }
                      ...
                      List<String> *names, int16 *res_id)
{
  ...
  /* Should never happen. As this is part of help, we can ignore this */
  if (init_read_record(&read_record_info, thd, categories, select, NULL, 1, 0, FALSE))
    DBUG_RETURN(0);
  while (!read_record_info.read_record())
  {
...
void get_all_items_for_category(THD *thd, TABLE *items, Field *pfname, SQL_SELECT *select, List<String> *res)
{
  ...
  /* Should never happen. As this is part of help, we can ignore this */
  if (init_read_record(&read_record_info, thd, items, select, NULL, 1, 0, FALSE))
    DBUG_VOID_RETURN;
...
static void fill_answer_1_fields(THD *thd, List<Item> *field_list)
{
  ...
  field_list->push_back(new (mem_root) Item_empty_string(thd, "description", 1000), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "example", 1000), mem_root);
}
...
static bool send_answer_1_metadata(Protocol *protocol)
{
  ...
  return protocol->send_result_set_metadata(&field_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF);
}
...
static void fill_header_2_fields(THD *thd, List<Item> *field_list, bool for_category)
{
  MEM_ROOT *mem_root = thd->mem_root;
  if (for_category)
    field_list->push_back(new (mem_root) Item_empty_string(thd, "source_category_name", 64), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "name", 64), mem_root);
...
static int send_header_2(Protocol *protocol, bool for_category)
{
  ...
  DBUG_RETURN(protocol->send_result_set_metadata(&field_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF));
}
...
SQL_SELECT *prepare_simple_select(THD *thd, Item *cond, TABLE *table, int *error)
{
  ...
  if (unlikely(*error) || (likely(res) && unlikely(res->check_quick(thd, 0, HA_POS_ERROR))) ||
...
SQL_SELECT *prepare_select_for_name(THD *thd, const char *mask, size_t mlen, TABLE *table, Field *pfname, int *error)
{
  MEM_ROOT *mem_root = thd->mem_root;
  Item *cond = new (mem_root) Item_func_like(thd, new (mem_root) Item_field(thd, pfname),
                                             new (mem_root) Item_string(thd, mask, (uint)mlen, pfname->charset()),
                                             new (mem_root) Item_string_ascii(thd, "\\"), FALSE);
...
static bool init_items_for_help_command(THD *thd, SELECT_LEX *first_select_lex, TABLE_LIST (&tables)[M],
                                        st_find_field (&used_fields)[N])
{
  ...
  */
  first_select_lex->context.table_list = first_select_lex->context.first_name_resolution_table = &tables[0];

  if (setup_tables(thd, &first_select_lex->context, &first_select_lex->top_join_list, &tables[0], leaves, false, false))
    return true;

  memcpy((char *)used_fields, (char *)init_used_fields, sizeof(used_fields[0]) * N);
  if (init_fields(thd, &tables[0], used_fields, N))
    return true;

  for (size_t i = 0; i < M; i++) tables[i].table->file->init_table_handle_for_HANDLER();
...
bool mysqld_help_prepare(THD *thd, const char *mask, List<Item> *fields)
{
  ...
  */
  ...
  if (init_items_for_help_command(thd, thd->lex->first_select_lex(), tables, used_fields))
  ...
  /*
    ...
    for execution
  */
  if (!(select = prepare_select_for_name(thd, mask, mlen, tables[0].table, used_fields[help_topic_name].field, &error)))
  ...
  */
  int count_topics =
      search_topics(thd, tables[0].table, used_fields, select, &topics_list, &name, &description, &example);
  delete select;
  ...
  if (count_topics == 0)
  {
    ...
    */
    if (!(select =
              prepare_select_for_name(thd, mask, mlen, tables[3].table, used_fields[help_keyword_name].field, &error)))
      return cleanup_and_return(true);
  ...
  {
    if (!(select =
              prepare_select_for_name(thd, mask, mlen, tables[1].table, used_fields[help_category_name].field, &error)))
      return cleanup_and_return(true);

    List<String> categories_list;
    int16 category_id;
    int count_categories = search_categories(thd, tables[1].table, used_fields, select, &categories_list, &category_id);
    delete select;
  ...
  */
  ...
  if (count_topics == 0)
  {
    ...
    else
    {
      ...
      Item *cond_topic_by_cat = new (mem_root) Item_func_equal(thd, new (mem_root) Item_field(thd, topic_cat_id),
                                                               new (mem_root) Item_int(thd, (int32)category_id));
      Item *cond_cat_by_cat = new (mem_root) Item_func_equal(thd, new (mem_root) Item_field(thd, cat_cat_id),
                                                             new (mem_root) Item_int(thd, (int32)category_id));
