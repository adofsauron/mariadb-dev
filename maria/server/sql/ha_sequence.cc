...
#define SEQUENCE_ENABLED_TABLE_FLAGS (HA_STATS_RECORDS_IS_EXACT | HA_PERSISTENT_TABLE)
#define SEQUENCE_DISABLED_TABLE_FLAGS (HA_CAN_SQL_HANDLER | HA_CAN_INSERT_DELAYED | HA_BINLOG_STMT_CAPABLE)
...
ha_sequence::ha_sequence(handlerton *hton, TABLE_SHARE *share) : handler(hton, share), write_locked(0)
{
...
*/
ha_sequence::~ha_sequence() { delete file; }
...
int ha_sequence::open(const char *name, int mode, uint flags)
{
  ...
  if (likely(!(error = file->open(name, mode, flags))))
  {
    ...
    */
    ...
    internal_tmp_table = file->internal_tmp_table = MY_TEST(flags & HA_OPEN_INTERNAL_TABLE);
    reset_statistics();
    ...
    /*
      ...
      or FLUSH
    */
    if (!(flags & (HA_OPEN_FOR_CREATE | HA_OPEN_FOR_REPAIR | HA_OPEN_FOR_FLUSH)))
    {
...
int ha_sequence::create(const char *name, TABLE *form, HA_CREATE_INFO *create_info)
{
...
int ha_sequence::write_row(const uchar *buf)
{
  ...
  */
  ...
  if (!write_locked)  // If not from next_value()
  {
    ...
    */
    if (table->s->tmp_table == NO_TMP_TABLE &&
        thd->mdl_context.upgrade_shared_lock(table->mdl_ticket, MDL_EXCLUSIVE, thd->variables.lock_wait_timeout))
  ...
  as TOI. */
  if (WSREP_ON && WSREP(thd) && !thd->wsrep_trx().active() && wsrep_thd_is_local(thd))
...
handler::Table_flags ha_sequence::table_flags() const
{
  ...
  DBUG_RETURN((file->table_flags() & ~SEQUENCE_DISABLED_TABLE_FLAGS) | SEQUENCE_ENABLED_TABLE_FLAGS);
}
...
bool ha_sequence::check_if_incompatible_data(HA_CREATE_INFO *create_info, uint table_changes)
{
...
void ha_sequence::print_error(int error, myf errflag)
{
  ...
  switch (error)
  {
    ...
    {
      my_error(ER_SEQUENCE_INVALID_DATA, MYF(errflag), sequence_db, sequence_name);
      DBUG_VOID_RETURN;
    }
...
static handler *sequence_create_handler(handlerton *hton, TABLE_SHARE *share, MEM_ROOT *mem_root)
{
...
/*
  ...
  RETURN VALUES
    ...
    !=0         Failure
*/
static int sequence_end(handlerton *hton, ha_panic_function type __attribute__((unused)))
{
...
static int sequence_initialize(void *p)
{
  ...
  local_sequence_hton->flags =
      (HTON_NOT_USER_SELECTABLE | HTON_HIDDEN | HTON_TEMPORARY_NOT_SUPPORTED | HTON_ALTER_NOT_SUPPORTED |
#ifdef WITH_WSREP
...
static struct st_mysql_storage_engine sequence_storage_engine = {MYSQL_HANDLERTON_INTERFACE_VERSION};
