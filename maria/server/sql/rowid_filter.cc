...
inline double Range_rowid_filter_cost_info::lookup_cost(Rowid_filter_container_type cont_type)
{
  switch (cont_type)
  {
...
inline double Range_rowid_filter_cost_info::avg_access_and_eval_gain_per_row(Rowid_filter_container_type cont_type)
{
  return (1 + 1.0 / TIME_FOR_COMPARE) * (1 - selectivity) - lookup_cost(cont_type);
}
...
inline double Range_rowid_filter_cost_info::avg_adjusted_gain_per_row(double access_cost_factor)
{
...
inline void Range_rowid_filter_cost_info::set_adjusted_gain_param(double access_cost_factor)
{
...
void Range_rowid_filter_cost_info::init(Rowid_filter_container_type cont_type, TABLE *tab, uint idx)
{
...
Rowid_filter_container *Range_rowid_filter_cost_info::create_container()
{
  ...
  {
    case SORTED_ARRAY_CONTAINER:
      res = new (thd->mem_root) Rowid_filter_sorted_array((uint)est_elements, elem_sz);
      break;
    default:
      DBUG_ASSERT(0);
  }
...
static int compare_range_rowid_filter_cost_info_by_a(Range_rowid_filter_cost_info **filter_ptr_1,
...
void TABLE::prune_range_rowid_filters()
{
  ...
  */
  ...
  for (uint i = 0; i < range_rowid_filter_cost_info_elems; i++, filter_ptr_1++)
  {
    uint key_no = (*filter_ptr_1)->key_no;
    Range_rowid_filter_cost_info **filter_ptr_2 = filter_ptr_1 + 1;
    for (uint j = i + 1; j < range_rowid_filter_cost_info_elems; j++, filter_ptr_2++)
    {
  ...
  my_qsort(range_rowid_filter_cost_info_ptr, range_rowid_filter_cost_info_elems, sizeof(Range_rowid_filter_cost_info *),
  ...
  */
  ...
  Range_rowid_filter_cost_info **cand_filter_ptr = range_rowid_filter_cost_info_ptr;
  for (uint i = 0; i < range_rowid_filter_cost_info_elems; i++, cand_filter_ptr++)
  {
    bool is_pruned = false;
    Range_rowid_filter_cost_info **usable_filter_ptr = range_rowid_filter_cost_info_ptr;
    key_map abs_indep;
    abs_indep.clear_all();
    for (uint j = 0; j < i; j++, usable_filter_ptr++)
    {
      ...
      else
      {
        ...
        */
        ...
        memmove(usable_filter_ptr + 1, usable_filter_ptr, sizeof(Range_rowid_filter_cost_info *) * (i - j - 1));
    ...
    if (is_pruned)
    {
      ...
      memmove(cand_filter_ptr, cand_filter_ptr + 1,
              sizeof(Range_rowid_filter_cost_info *) * (range_rowid_filter_cost_info_elems - 1 - i));
...
static ulonglong get_max_range_rowid_filter_elems_for_table(THD *thd, TABLE *tab, Rowid_filter_container_type cont_type)
{
  switch (cont_type)
  {
    case SORTED_ARRAY_CONTAINER:
...
void TABLE::init_cost_info_for_usable_range_rowid_filters(THD *thd)
{
  ...
  */
  while ((key_no = it++) != key_map::Iterator::BITMAP_END)
  {
    ...
    if (file->is_clustering_key(key_no))  // !2
      continue;
    if (opt_range[key_no].rows > get_max_range_rowid_filter_elems_for_table(thd, this,
  ...
  */
  ...
  range_rowid_filter_cost_info_ptr = (Range_rowid_filter_cost_info **)thd->calloc(
      sizeof(Range_rowid_filter_cost_info *) * range_rowid_filter_cost_info_elems);
  range_rowid_filter_cost_info = new (thd->mem_root) Range_rowid_filter_cost_info[range_rowid_filter_cost_info_elems];
  ...
  Range_rowid_filter_cost_info *curr_filter_cost_info = range_rowid_filter_cost_info;
...
Range_rowid_filter_cost_info *TABLE::best_range_rowid_filter_for_partial_join(uint access_key_no, double records,
                                                                              double access_cost_factor)
{
  if (range_rowid_filter_cost_info_elems == 0 || covering_keys.is_set(access_key_no))
    return 0;
  ...
  */
  ...
  for (uint i = 0; i < range_rowid_filter_cost_info_elems; i++)
  {
    ...
    */
    if ((filter->key_no == access_key_no) || no_filter_usage.is_set(filter->key_no))
      continue;
...
bool Rowid_filter_sorted_array::check(void *ctxt, char *elem)
{
  ...
  {
    int m = (l + r) / 2;
    int cmp = refpos_order_cmp((void *)(table->file), refpos_container.get_pos(m), elem);
    if (cmp == 0)
      return true;
    if (cmp < 0)
      l = m + 1;
