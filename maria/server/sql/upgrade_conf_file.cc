...
static const char *removed_variables[] = {"aria_recover",
                                          "debug_crc_break",
...
static UINT get_system_acp()
{
  ...
  char str_cp[10];
  int cch = GetLocaleInfo(GetSystemDefaultLCID(), LOCALE_IDEFAULTANSICODEPAGE, str_cp, sizeof(str_cp));
...
#define MAX_STR_LEN MY_INI_SECTION_SIZE
  ...
  {
    if (WideCharToMultiByte(CP_UTF8, 0, utf16_buf, -1, utf8_buf, MAX_STR_LEN, 0, 0))
      return utf8_buf;
  }
  return 0;
}

int fix_section(const char *myini_path, const char *section_name, bool is_server)
{
  ...
  for (char *keyval = section_data; *keyval; keyval += strlen(keyval) + 1)
  {
    ...
    const char *v =
        (const char *)bsearch(varname, removed_variables, sizeof(removed_variables) / sizeof(removed_variables[0]),
...
{
  if (strncmp(name, "mysql", 5) && strncmp(name, "mariadb", 7) && strcmp(name, "client") &&
      strcmp(name, "client-server") && strcmp(name, "server"))
  {
    return false;
  }
...
int upgrade_config_file(const char *myini_path)
{
  ...
  int sz = GetPrivateProfileSectionNamesA(all_sections, MY_INI_SECTION_SIZE, myini_path);
  if (!sz)
    return 0;
