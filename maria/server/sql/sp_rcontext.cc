...
const LEX_CSTRING *Sp_rcontext_handler_local::get_name_prefix() const { return &empty_clex_str; }

const LEX_CSTRING *Sp_rcontext_handler_package_body::get_name_prefix() const
{
  static const LEX_CSTRING sp_package_body_variable_prefix_clex_str = {STRING_WITH_LEN("PACKAGE_BODY.")};
...
sp_rcontext::sp_rcontext(const sp_head *owner, const sp_pcontext *root_parsing_ctx, Field *return_value_fld,
                         bool in_sub_stmt)
    : end_partial_result_set(false),
      ...
      m_handler_call_stack(PSI_INSTRUMENT_MEM),
      m_ccount(0)
{
}
...
sp_rcontext *sp_rcontext::create(THD *thd, const sp_head *owner, const sp_pcontext *root_parsing_ctx,
                                 Field *return_value_fld, Row_definition_list &field_def_lst)
{
  SELECT_LEX *save_current_select;
  sp_rcontext *ctx = new (thd->mem_root) sp_rcontext(owner, root_parsing_ctx, return_value_fld, thd->in_sub_stmt);
  if (!ctx)
    return NULL;
  ...
  if (ctx->alloc_arrays(thd) || ctx->init_var_table(thd, field_def_lst) || ctx->init_var_items(thd, field_def_lst))
  {
    delete ctx;
    ctx = 0;
  }
...
bool Row_definition_list::adjust_formal_params_to_actual_params(THD *thd, List<Item> *args)
{
...
bool Row_definition_list::adjust_formal_params_to_actual_params(THD *thd, Item **args, uint arg_count)
{
  List_iterator<Spvar_definition> it(*this);
...
{
  {
    ...
    m_cstack.reset(static_cast<sp_cursor **>(thd->alloc(n * sizeof(sp_cursor *))), n);
  }

  {
    size_t n = m_root_parsing_ctx->get_num_case_exprs();
    m_case_expr_holders.reset(static_cast<Item_cache **>(thd->calloc(n * sizeof(Item_cache *))), n);
  }

  return !m_cstack.array() || !m_case_expr_holders.array();
}

bool sp_rcontext::init_var_table(THD *thd, List<Spvar_definition> &field_def_lst)
{
...
*/
static inline bool check_column_grant_for_type_ref(THD *thd, TABLE_LIST *table_list, const char *str, size_t length,
                                                   Field *fld)
{
...
bool Qualified_column_ident::resolve_type_ref(THD *thd, Column_definition *def)
{
  ...
  if ((table_list =
           lex.first_select_lex()->add_table_to_list(thd, this, NULL, 0, TL_READ_NO_INSERT, MDL_SHARED_READ)) &&
      !check_table_access(thd, SELECT_ACL, table_list, TRUE, UINT_MAX, FALSE) &&
      !open_tables_only_view_structure(thd, table_list, thd->mdl_context.has_locks()))
  {
    if (likely((src = lex.query_tables->table->find_field_by_name(&m_column))))
    {
      if (!(rc = check_column_grant_for_type_ref(thd, table_list, m_column.str, m_column.length, src)))
      {
...
*/
bool Table_ident::resolve_table_rowtype_ref(THD *thd, Row_definition_list &defs)
{
  Open_tables_backup open_tables_state_backup;
  thd->reset_n_backup_open_tables_state(&open_tables_state_backup);
      ...
      !open_tables_only_view_structure(thd, table_list, thd->mdl_context.has_locks()))
  {
    for (Field **src = lex.query_tables->table->field; *src; src++)
    {
      ...
      */
      ...
      Spvar_definition *def;
      if ((rc = check_column_grant_for_type_ref(thd, table_list, tmp.str, tmp.length, src[0])) ||
...
bool Row_definition_list::resolve_type_refs(THD *thd)
{
  ...
  while ((def = it++))
  {
    if (def->is_column_type_ref() && def->column_type_ref()->resolve_type_ref(thd, def))
      return true;
  }
  return false;
};

bool sp_rcontext::init_var_items(THD *thd, List<Spvar_definition> &field_def_lst)
{
  uint num_vars = m_root_parsing_ctx->max_var_index();

  m_var_items.reset(static_cast<Item_field **>(thd->alloc(num_vars * sizeof(Item *))), num_vars);
  ...
  for (uint idx = 0; idx < num_vars; ++idx, def = it++)
  {
    ...
    if (def->is_table_rowtype_ref())
    {
      ...
      if (!(m_var_items[idx] = item) || def->table_rowtype_ref()->resolve_table_rowtype_ref(thd, defs) ||
    ...
    {
      Item_field_row *item = new (thd->mem_root) Item_field_row(thd, field);
      if (!(m_var_items[idx] = item) || item->row_create_items(thd, def->row_field_definitions()))
        return true;
    }
    else
    {
...
bool Item_field_row::row_create_items(THD *thd, List<Spvar_definition> *list)
{
  ...
  {
    if (!(args[arg_count] = new (thd->mem_root) Item_field(thd, ptable[0]->field[arg_count])))
      return true;
  }
  return false;
}
...
void sp_rcontext::push_cursor(sp_cursor *c) { m_cstack[m_ccount++] = c; }
...
void sp_rcontext::pop_cursors(THD *thd, size_t count)
{
  ...
  while (count--) pop_cursor(thd);
}

bool sp_rcontext::push_handler(sp_instr_hpush_jump *entry) { return m_handlers.append(entry); }

void sp_rcontext::pop_handlers(size_t count)
{
  DBUG_ASSERT(m_handlers.elements() >= count);

  for (size_t i = 0; i < count; ++i) m_handlers.pop();
}

bool sp_rcontext::handle_sql_condition(THD *thd, uint *ip, const sp_instr *cur_spi)
{
  ...
  */
  ...
  {
    found_handler = cur_spi->m_ctx->find_handler(da->get_error_condition_identity());
    ...
    */
    if (!found_condition)
    {
      found_condition = new (callers_arena->mem_root) Sql_condition(
          callers_arena->mem_root, da->get_error_condition_identity(), da->message(), da->current_row_for_warning());
    }
  }
  else if (da->current_statement_warn_count())
  {
    ...
    while ((c = it++))
    {
      if (c->get_level() == Sql_condition::WARN_LEVEL_WARN || c->get_level() == Sql_condition::WARN_LEVEL_NOTE)
      {
  ...
  */
  ...
  uint continue_ip = handler_entry->get_handler()->type == sp_handler::CONTINUE ? cur_spi->get_cont_dest() : 0;
  ...
  Sql_condition_info *cond_info = new (callers_arena->mem_root) Sql_condition_info(found_condition, callers_arena);
  Handler_call_frame *frame = new (callers_arena->mem_root) Handler_call_frame(cond_info, continue_ip);
...
int sp_rcontext::set_variable_row_field(THD *thd, uint var_idx, uint field_idx, Item **value)
{
...
int sp_rcontext::set_variable_row_field_by_name(THD *thd, uint var_idx, const LEX_CSTRING &field_name, Item **value)
{
...
bool sp_rcontext::find_row_field_by_name_or_error(uint *field_idx, uint var_idx, const LEX_CSTRING &field_name)
{
  Virtual_tmp_table *vtable = virtual_tmp_table_for_row(var_idx);
  Field *row = m_var_table->field[var_idx];
  return vtable->sp_find_field_by_name_or_error(field_idx, row->field_name, field_name);
}

Item_cache *sp_rcontext::create_case_expr_holder(THD *thd, const Item *item) const
{
  Item_cache *holder;
...
bool sp_rcontext::set_case_expr(THD *thd, int case_expr_id, Item **case_expr_item_ptr)
{
  ...
  if (!m_case_expr_holders[case_expr_id] ||
      m_case_expr_holders[case_expr_id]->result_type() != case_expr_item->result_type())
  {
    m_case_expr_holders[case_expr_id] = create_case_expr_holder(thd, case_expr_item);
  }
...
int sp_cursor::open(THD *thd)
{
  ...
  {
    my_message(ER_SP_CURSOR_ALREADY_OPEN, ER_THD(thd, ER_SP_CURSOR_ALREADY_OPEN), MYF(0));
    return -1;
  }
...
int sp_cursor::fetch(THD *thd, List<sp_variable> *vars, bool error_on_no_data)
{
  ...
  if (vars->elements != result.get_field_count() &&
      (vars->elements != 1 || result.get_field_count() != thd->spcont->get_variable(vars->head()->offset)->cols()))
  {
    my_message(ER_SP_WRONG_NO_OF_FETCH_ARGS, ER_THD(thd, ER_SP_WRONG_NO_OF_FETCH_ARGS), MYF(0));
    return -1;
  }

  m_fetch_count++;
  DBUG_EXECUTE_IF("bug23032_emit_warning",
                  push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_UNKNOWN_ERROR, ER_THD(thd, ER_UNKNOWN_ERROR)););
...
int sp_cursor::Select_fetch_into_spvars::prepare(List<Item> &fields, SELECT_LEX_UNIT *u)
{
  /*
...
bool sp_cursor::Select_fetch_into_spvars::send_data_to_variable_list(List<sp_variable> &vars, List<Item> &items)
{
...
int sp_cursor::Select_fetch_into_spvars::send_data(List<Item> &items)
{
  ...
  */
  return spvar_list->elements == 1 && (item = thd->spcont->get_variable(spvar_list->head()->offset)) &&
                 item->type_handler() == &type_handler_row && item->cols() == items.elements
