...
static int get_provider_option_value(const char *opts, const char *opt_name, ulong *opt_value)
{
  int ret = 1;
  ulong opt_value_tmp;
...
{
  WSREP_DEBUG("refresh_provider_options: %s", (wsrep_provider_options) ? wsrep_provider_options : "null");

  try
  {
    std::string opts = Wsrep_server_state::instance().provider().options();
    wsrep_provider_options_init(opts.c_str());
    get_provider_option_value(wsrep_provider_options, (char *)"repl.max_ws_size", &wsrep_max_ws_size);
    return false;
  }
  catch (...)
  {
...
void wsrep_set_wsrep_on(THD *thd)
{
  ...
  WSREP_PROVIDER_EXISTS_ = wsrep_provider && strncasecmp(wsrep_provider, WSREP_NONE, FN_REFLEN);
...
bool wsrep_on_update(sys_var *self, THD *thd, enum_var_type var_type)
{
  ...
  if (var_type == OPT_GLOBAL)
  {
    if (thd->variables.wsrep_on && thd->wsrep_cs().state() == wsrep::client_state::s_none)
    {
      wsrep_open(thd);
...
bool wsrep_on_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  if (new_wsrep_on)
  {
    ...
    if (var->type == OPT_SESSION && !global_system_variables.wsrep_on)
    {
      my_message(ER_WRONG_ARGUMENTS,
                 "Can't enable @@session.wsrep_on, "
                 "while @@global.wsrep_on is disabled",
                 MYF(0));
      return true;
    }
  }
...
{
  thd->variables.wsrep_causal_reads = thd->variables.wsrep_sync_wait & WSREP_SYNC_WAIT_BEFORE_READ;

  // update global settings too
  global_system_variables.wsrep_causal_reads = global_system_variables.wsrep_sync_wait & WSREP_SYNC_WAIT_BEFORE_READ;
...
/*
  ...
  @return
    ...
    false                   Pass
*/
static bool wsrep_start_position_verify(const char *start_str)
{
  size_t start_len;
  wsrep_uuid_t uuid;
  ssize_t uuid_len;
  ...
  */
  ...
  {
    startptr++;
    uint32_t domain __attribute__((unused)) (parse_value<uint32_t>(&startptr, &endptr));
    if (*endptr != '-')
      return true;
    startptr++;
    uint32_t server __attribute__((unused)) (parse_value<uint32_t>(&startptr, &endptr));
    if (*endptr != '-')
      return true;
    startptr++;
    uint64_t seq __attribute__((unused)) (parse_value<uint64_t>(&startptr, &endptr));
  }
...
static bool wsrep_set_local_position(THD *thd, const char *const value, size_t length, bool const sst)
{
  char *endptr;
  char *startptr;
  wsrep_uuid_t uuid;
  ...
  if (!strcmp(start_pos_buf, WSREP_START_POSITION_ZERO) && !strcmp(wsrep_start_position, WSREP_START_POSITION_ZERO))
    goto set;
  else
...
bool wsrep_start_position_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  memcpy(start_pos_buf, var->save_result.string_value.str, var->save_result.string_value.length);
  start_pos_buf[var->save_result.string_value.length] = 0;

  WSREP_DEBUG("SST wsrep_start_position check for new position %s old %s", start_pos_buf, wsrep_start_position);

  // Verify the format.
  if (wsrep_start_position_verify(start_pos_buf))
    return true;
  ...
  */
  if (wsrep_set_local_position(thd, var->save_result.string_value.str, var->save_result.string_value.length, true))
    goto err;
...
bool wsrep_start_position_init(const char *val)
{
  ...
  {
    WSREP_ERROR("Bad initial value for wsrep_start_position: %s", (val ? val : ""));
    return true;
  }
...
bool wsrep_provider_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  memcpy(wsrep_provider_buf, var->save_result.string_value.str, var->save_result.string_value.length);
  wsrep_provider_buf[var->save_result.string_value.length] = 0;

  if (!wsrep_provider_verify(wsrep_provider_buf))
    return 0;
...
{
  WSREP_DEBUG("wsrep_provider_init: %s -> %s", (wsrep_provider) ? wsrep_provider : "null", (value) ? value : "null");
  if (NULL == value || wsrep_provider_verify(value))
  {
    WSREP_ERROR("Bad initial value for wsrep_provider: %s", (value ? value : ""));
    return;
  }

  if (wsrep_provider)
    my_free((void *)wsrep_provider);
...
bool wsrep_provider_options_update(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  {
    enum wsrep::provider::status ret = Wsrep_server_state::instance().provider().options(wsrep_provider_options);
    if (ret)
    {
...
bool wsrep_reject_queries_update(sys_var *self, THD *thd, enum_var_type type)
{
  switch (wsrep_reject_queries)
  {
    ...
    case WSREP_REJECT_ALL_KILL:
      ...
      break;
    default:
      WSREP_INFO("Unknown value for wsrep_reject_queries: %lu", wsrep_reject_queries);
      return true;
  }
  return false;
}
...
bool wsrep_debug_update(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  if ((!WSREP_ON || !Wsrep_server_state::instance().is_provider_loaded()) && wsrep_debug)
  {
    push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_VALUE_FOR_VAR,
...
bool wsrep_gtid_seq_no_check(sys_var *self, THD *thd, set_var *var)
{
...
bool wsrep_cluster_address_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  if ((!var->save_result.string_value.str) || (var->save_result.string_value.length >= sizeof(addr_buf)))  // safety
    goto err;
...
bool wsrep_cluster_address_update(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  */
  ...
  if (*tmp && wsrep_start_replication(tmp))
  {
    ...
    WSREP_DEBUG("Cluster address update creating %ld applier threads running %lu", wsrep_slave_threads,
...
{
  WSREP_DEBUG("wsrep_cluster_address_init: %s -> %s", (wsrep_cluster_address) ? wsrep_cluster_address : "null",
              (value) ? value : "null");
...
bool wsrep_cluster_name_update(sys_var *self, THD *thd, enum_var_type type) { return 0; }
...
bool wsrep_node_name_update(sys_var *self, THD *thd, enum_var_type type) { return 0; }
...
bool wsrep_node_address_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  memcpy(addr_buf, var->save_result.string_value.str, var->save_result.string_value.length);
...
bool wsrep_node_address_update(sys_var *self, THD *thd, enum_var_type type) { return 0; }
...
static void wsrep_slave_count_change_update()
{
  ...
  WSREP_DEBUG("Change on slave threads: New %ld old %lu difference %d", wsrep_slave_threads,
              wsrep_running_applier_threads, wsrep_slave_count_change);
}
...
bool wsrep_desync_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  if (wsrep_desync == new_wsrep_desync)
  {
    if (new_wsrep_desync)
    {
      push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_VALUE_FOR_VAR, "'wsrep_desync' is already ON.");
    }
    else
    {
      push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_VALUE_FOR_VAR, "'wsrep_desync' is already OFF.");
    }
    return false;
  }
  int ret = 1;
  if (new_wsrep_desync)
  {
    ...
    if (ret)
    {
      WSREP_WARN("SET desync failed %d for schema: %s, query: %s", ret, thd->db.str, wsrep_thd_query(thd));
  ...
  else
  {
    ...
    if (ret != WSREP_OK)
    {
      WSREP_WARN("SET resync failed %d for schema: %s, query: %s", ret, thd->get_db(), wsrep_thd_query(thd));
...
bool wsrep_desync_update(sys_var *self, THD *thd, enum_var_type type) { return false; }
...
{
  WSREP_DEBUG("wsrep_trx_fragment_size_update: %llu", thd->variables.wsrep_trx_fragment_size);
  ...
  {
    return thd->wsrep_cs().enable_streaming(wsrep_fragment_unit(thd->variables.wsrep_trx_fragment_unit),
...
{
  WSREP_DEBUG("wsrep_trx_fragment_unit_update: %lu", thd->variables.wsrep_trx_fragment_unit);
...
bool wsrep_max_ws_size_update(sys_var *self, THD *thd, enum_var_type)
{
  ...
  my_snprintf(max_ws_size_opt, sizeof(max_ws_size_opt), "repl.max_ws_size=%d", wsrep_max_ws_size);
...
bool wsrep_mode_check(sys_var *self, THD *thd, set_var *var) { return false; }
 ...
 */
static inline void wsrep_assign_to_mysql(SHOW_VAR *mysql, wsrep_stats_var *wsrep_var)
{
...
static void export_wsrep_status_to_mysql(THD *thd)
{
  ...
  {
    void *tmp = realloc(mysql_status_vars, (wsrep_status_len + 1) * sizeof(SHOW_VAR));
    if (!tmp)
    {
      sql_print_error(
          "Out of memory for wsrep status variables."
  ...
  /* @TODO: fix this: */
#else
  if (mysql_status_len < wsrep_status_len)
    wsrep_status_len = mysql_status_len;
#endif
...
int wsrep_show_status(THD *thd, SHOW_VAR *var, void *, system_status_var *, enum_var_type)
{
...
{
  WSREP_DEBUG("wsrep_gtid_domain_id_update: %llu", wsrep_gtid_domain_id);
