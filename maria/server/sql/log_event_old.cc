...
int Old_rows_log_event::do_apply_event(Old_rows_log_event *ev, rpl_group_info *rgi)
{
  ...
  */
    ...
    */
    ...
    if (unlikely(open_and_lock_tables(ev_thd, rgi->tables_to_lock, FALSE, 0)))
    {
      if (ev_thd->is_error())
      {
        ...
        */
        rli->report(ERROR_LEVEL, ev_thd->get_stmt_da()->sql_errno(), NULL, "Error '%s' on opening tables",
  ...
  if (table)
  {
    ...
    */
    ...
    while (error == 0 && row_start < ev->m_rows_end)
    {
      ...
      if (unlikely((error = do_prepare_row(ev_thd, rgi, table, row_start, &row_end))))
      ...
      switch (error)
      {
        ...
        default:
          rli->report(ERROR_LEVEL, ev_thd->get_stmt_da()->sql_errno(), NULL,
                      "Error in %s event: row application failed. %s", ev->get_type_str(),
    ...
    DBUG_EXECUTE_IF("stop_slave_middle_group", const_cast<Relay_log_info *>(rli)->abort_slave = 1;);
  ...
  { /* error has occurred during the transaction */
    rli->report(ERROR_LEVEL, ev_thd->get_stmt_da()->sql_errno(), NULL,
                ...
                "on table %s.%s. %s",
                ev->get_type_str(), table->s->db.str, table->s->table_name.str,
                ev_thd->is_error() ? ev_thd->get_stmt_da()->message() : "");
 ...
 */
static int copy_extra_record_fields(TABLE *table, size_t master_reclength, my_ptrdiff_t master_fields)
{
  ...
  DBUG_PRINT("info",
             ("Copying to %p "
              ...
              table->record[0], (ulong)master_fields, (ulong)master_reclength, table->s->fields, table->s->reclength));
  /*
  ...
  */
  ...
  if (master_reclength < table->s->reclength)
    memcpy(table->record[0] + master_reclength, table->record[1] + master_reclength,
  ...
  */
  {
    ...
    for (; *field_ptr; ++field_ptr)
    {
       ...
       */
      if ((*field_ptr)->maybe_null() && (*field_ptr)->is_null_in_record(reinterpret_cast<uchar *>(table->record[1])))
        (*field_ptr)->set_null();
      else
      ...
      switch ((*field_ptr)->real_type())
      {
        ...
        case MYSQL_TYPE_BIT:
          ...
          if (f->bit_len > 0)
          {
            ...
            uchar const bits = get_rec_bits(f->bit_ptr + offset, f->bit_ofs, f->bit_len);
 ...
 */
static int replace_record(THD *thd, TABLE *table, ulong const master_reclength, uint const master_fields)
{
  ...
  while (unlikely(error = table->file->ha_write_row(table->record[0])))
  {
    ...
    */
    ...
    /*
       ...
       the correct row.
     */
    if (last_uniq_key(table, keynum) && !table->file->referenced_by_foreign_key())
    {
...
static int find_and_fetch_row(TABLE *table, uchar *key)
{
  ...
  DBUG_PRINT("enter", ("table: %p, key: %p  record: %p", table, key, table->record[1]));
  ...
  if (table->s->keys > 0)
  {
    ...
    if (!table->file->inited && unlikely(error = table->file->ha_index_init(0, FALSE)))
    {
      table->file->print_error(error, MYF(0));
      DBUG_RETURN(error);
    }
    ...
    /*
      ...
      correctly.
    */
    my_ptrdiff_t const pos = table->s->null_bytes > 0 ? table->s->null_bytes - 1 : 0;
    table->record[1][pos] = 0xFF;
    if (unlikely((error = table->file->ha_index_read_map(table->record[1], key, HA_WHOLE_KEY, HA_READ_KEY_EXACT))))
    {
      table->file->print_error(error, MYF(0));
...
int Write_rows_log_event_old::do_prepare_row(THD *thd_arg, rpl_group_info *rgi, TABLE *table, uchar const *row_start,
                                             uchar const **row_end)
{
  ...
  int error;
  error = unpack_row_old(rgi, table, m_width, table->record[0], row_start, m_rows_end, &m_cols, row_end,
                         &m_master_reclength, table->write_set, PRE_GA_WRITE_ROWS_EVENT);
...
int Delete_rows_log_event_old::do_before_row_operations(TABLE *table)
{
  ...
  if (table->s->keys > 0)
  {
    m_memory = (uchar *)my_multi_malloc(key_memory_log_event_old, MYF(MY_WME), &m_after_image,
                                        (uint)table->s->reclength, &m_key, (uint)table->key_info->key_length, NullS);
  }
  else
  {
...
int Delete_rows_log_event_old::do_prepare_row(THD *thd_arg, rpl_group_info *rgi, TABLE *table, uchar const *row_start,
                                              uchar const **row_end)
{
  int error;
  DBUG_ASSERT(row_start && row_end);
  ...
  */
  ...
  error = unpack_row_old(rgi, table, m_width, table->record[0], row_start, m_rows_end, &m_cols, row_end,
                         &m_master_reclength, table->read_set, PRE_GA_DELETE_ROWS_EVENT);
  /*
...
int Update_rows_log_event_old::do_prepare_row(THD *thd_arg, rpl_group_info *rgi, TABLE *table, uchar const *row_start,
                                              uchar const **row_end)
{
  int error;
  DBUG_ASSERT(row_start && row_end);
  ...
  */
  ...
  error = unpack_row_old(rgi, table, m_width, table->record[0], row_start, m_rows_end, &m_cols, row_end,
                         &m_master_reclength, table->read_set, PRE_GA_UPDATE_ROWS_EVENT);
  row_start = *row_end;
  /* m_after_image is the after image for the update */
  error = unpack_row_old(rgi, table, m_width, m_after_image, row_start, m_rows_end, &m_cols, row_end,
                         &m_master_reclength, table->write_set, PRE_GA_UPDATE_ROWS_EVENT);
...
#ifndef MYSQL_CLIENT
Old_rows_log_event::Old_rows_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid, MY_BITMAP const *cols,
...
Old_rows_log_event::Old_rows_log_event(const uchar *buf, uint event_len, Log_event_type event_type,
...
#endif
{
  ...
  uchar const *const var_start = (const uchar *)buf + common_header_len + post_header_len;
  ...
  DBUG_PRINT("info",
             ("m_table_id: %lu  m_flags: %d  m_width: %lu  data_size: %zu", m_table_id, m_flags, m_width, data_size));
...
int Old_rows_log_event::get_data_size()
{
  ...
  DBUG_EXECUTE_IF("old_row_based_repl_4_byte_map_id_master",
                  return (int)(6 + no_bytes_in_map(&m_cols) + (end - buf) + m_rows_cur - m_rows_buf););
...
int Old_rows_log_event::do_add_row_data(uchar *row_data, size_t length)
{
  ...
  */
  ...
  DBUG_PRINT("enter", ("row_data: %p  length: %zu", row_data, length));
  /*
    Don't print debug messages when running valgrind since they can
    trigger false warnings.
   */
  ...
  if (static_cast<size_t>(m_rows_end - m_rows_cur) <= length)
  {
    ...
    my_ptrdiff_t const new_alloc = block_size * ((cur_size + length + block_size - 1) / block_size);
...
int Old_rows_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  */
  if (!thd->lock)
  {
    ...
    */
    ...
    if (unlikely((error = lock_tables(thd, rgi->tables_to_lock, rgi->tables_to_lock_count, 0))))
    {
      if (thd->is_slave_error || thd->is_fatal_error)
      {
        ...
        */
        ...
        rli->report(ERROR_LEVEL, actual_error, NULL, "Error '%s' in %s event: when locking tables",
                    (actual_error ? thd->net.last_error : "unexpected success or fatal error"), get_type_str());
        thd->is_fatal_error = 1;
      }
      else
      {
        rli->report(ERROR_LEVEL, error, NULL, "Error in %s event: when locking tables", get_type_str());
      }
      rgi->slave_close_thread_tables(thd);
      DBUG_RETURN(error);
    }
  ...
  TABLE *table = m_table = rgi->m_table_map.get_table(m_table_id);
    ...
    */
    ...
    while (error == 0 && m_curr_row < m_rows_end)
    {
      ...
      switch (error)
      {
        ...
        default:
          rli->report(ERROR_LEVEL, thd->net.last_errno, NULL, "Error in %s event: row application failed. %s",
  ...
  { /* error has occurred during the transaction */
    rli->report(ERROR_LEVEL, thd->net.last_errno, NULL,
                "Error in %s event: error during transaction execution "
                "on table %s.%s. %s",
                get_type_str(), table->s->db.str, table->s->table_name.str, thd->net.last_error);
  ...
  */
  if (table && (table->s->primary_key == MAX_KEY) && !use_trans_cache() && get_flags(STMT_END_F) == RLE_NO_FLAGS)
  {
    /*
  ...
  if (get_flags(STMT_END_F))
  {
    ...
    */
    ...
    if (unlikely((error = (binlog_error ? trans_rollback_stmt(thd) : trans_commit_stmt(thd)))))
      rli->report(ERROR_LEVEL, error, NULL,
                  "Error in %s event: commit of row events failed, "
                  "table `%s`.`%s`",
                  get_type_str(), m_table->s->db.str, m_table->s->table_name.str);
    error |= binlog_error;
...
Log_event::enum_skip_reason Old_rows_log_event::do_shall_skip(rpl_group_info *rgi)
{
  /*
...
int Old_rows_log_event::do_update_pos(rpl_group_info *rgi)
{
...
void Old_rows_log_event::pack_info(Protocol *protocol)
{
  ...
  size_t bytes = my_snprintf(buf, sizeof(buf), "table_id: %lu%s", m_table_id, flagstr);
...
bool Old_rows_log_event::print_helper(FILE *file, PRINT_EVENT_INFO *print_event_info, char const *const name)
{
  ...
  bool do_print_encoded = print_event_info->base64_output_mode != BASE64_OUTPUT_DECODE_ROWS &&
                          print_event_info->base64_output_mode != BASE64_OUTPUT_NEVER && !print_event_info->short_form;

  if (!print_event_info->short_form)
  {
    if (print_header(head, print_event_info, !do_print_encoded) ||
        my_b_printf(head, "\t%s: table id %lu%s\n", name, m_table_id, do_print_encoded ? " flags: STMT_END_F" : "") ||
...
int Old_rows_log_event::write_row(rpl_group_info *rgi, const bool overwrite)
{
  ...
  if (unlikely((error = prepare_record(table, m_width, TRUE /* check if columns have def. values */))))
    DBUG_RETURN(error);
...
Write_rows_log_event_old::Write_rows_log_event_old(THD *thd_arg, TABLE *tbl_arg, ulong tid_arg, MY_BITMAP const *cols,
                                                   bool is_transactional)
 ...
 */
#ifdef HAVE_REPLICATION
Write_rows_log_event_old::Write_rows_log_event_old(const uchar *buf, uint event_len,
                                                   const Format_description_log_event *description_event)
...
int Write_rows_log_event_old::do_after_row_operations(const Slave_reporting_capability *const, int error)
{
  int local_error = 0;
...
int Write_rows_log_event_old::do_exec_row(rpl_group_info *rgi)
{
...
#ifdef MYSQL_CLIENT
bool Write_rows_log_event_old::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  return Old_rows_log_event::print_helper(file, print_event_info, "Write_rows_old");
}
#endif
...
#ifndef MYSQL_CLIENT
Delete_rows_log_event_old::Delete_rows_log_event_old(THD *thd_arg, TABLE *tbl_arg, ulong tid, MY_BITMAP const *cols,
                                                     bool is_transactional)
    : Old_rows_log_event(thd_arg, tbl_arg, tid, cols, is_transactional), m_after_image(NULL), m_memory(NULL)
{
  // This constructor should not be reached.
  assert(0);
}
 ...
 */
#ifdef HAVE_REPLICATION
Delete_rows_log_event_old::Delete_rows_log_event_old(const uchar *buf, uint event_len,
                                                     const Format_description_log_event *description_event)
    : Old_rows_log_event(buf, event_len, PRE_GA_DELETE_ROWS_EVENT, description_event),
      m_after_image(NULL),
      m_memory(NULL)
{
}
...
int Delete_rows_log_event_old::do_after_row_operations(const Slave_reporting_capability *const, int error)
{
  /*error= ToDo:find out what this should really be, this triggers close_scan in nbd, returning error?*/
...
#ifdef MYSQL_CLIENT
bool Delete_rows_log_event_old::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  return Old_rows_log_event::print_helper(file, print_event_info, "Delete_rows_old");
}
#endif
 ...
 */
#ifdef HAVE_REPLICATION
Update_rows_log_event_old::Update_rows_log_event_old(const uchar *buf, uint event_len,
                                                     const Format_description_log_event *description_event)
    : Old_rows_log_event(buf, event_len, PRE_GA_UPDATE_ROWS_EVENT, description_event),
      m_after_image(NULL),
      m_memory(NULL)
{
}
...
int Update_rows_log_event_old::do_after_row_operations(const Slave_reporting_capability *const, int error)
{
  /*error= ToDo:find out what this should really be, this triggers close_scan in nbd, returning error?*/
  m_table->file->ha_index_or_rnd_end();
...
int Update_rows_log_event_old::do_exec_row(rpl_group_info *rgi)
{
  DBUG_ASSERT(m_table != NULL);
...
#ifdef MYSQL_CLIENT
bool Update_rows_log_event_old::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  return Old_rows_log_event::print_helper(file, print_event_info, "Update_rows_old");
}
#endif
