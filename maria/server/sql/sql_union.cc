...
bool mysql_union(THD *thd, LEX *lex, select_result *result, SELECT_LEX_UNIT *unit, ulonglong setup_tables_done_option)
{
  ...
  bool res;
  if (!(res = unit->prepare(unit->derived, result, SELECT_NO_UNLOCK | setup_tables_done_option)))
    res = unit->exec();
  res |= unit->cleanup();
  DBUG_RETURN(res);
}
...
void select_unit::change_select()
{
  ...
  DBUG_PRINT("enter", ("select in unit change: %u -> %u", curr_sel, current_select_number));
...
{
  if (step != INTERSECT_TYPE || (thd->lex->current_select->next_select() &&
...
int select_union_recursive::send_data(List<Item> &values)
{
  ...
  */
  if (thd->lex->sql_command != SQLCOM_CREATE_TABLE && thd->lex->sql_command != SQLCOM_INSERT_SELECT)
  ...
  if (rc == 0 && write_err != HA_ERR_FOUND_DUPP_KEY && write_err != HA_ERR_FOUND_DUPP_UNIQUE)
  {
    int err;
    ...
    {
      bool is_duplicate;
      rc = create_internal_tmp_table_from_heap(thd, incr_table, tmp_table_param.start_recinfo, &tmp_table_param.recinfo,
                                               err, 1, &is_duplicate);
    }
  }
...
bool select_unit::create_result_table(THD *thd_arg, List<Item> *column_types, bool is_union_distinct, ulonglong options,
                                      ...
                                      bool keep_row_order, uint hidden)
{
  ...
  if (!(table = create_tmp_table(thd_arg, &tmp_table_param, *column_types, (ORDER *)0, is_union_distinct, 1, options,
                                 HA_POS_ERROR, alias, !create_table, keep_row_order)))
    return TRUE;

  table->keys_in_use_for_query.clear_all();
  for (uint i = 0; i < table->s->fields; i++) table->field[i]->flags &= ~(PART_KEY_FLAG | PART_INDIRECT_KEY_FLAG);
...
bool select_union_recursive::create_result_table(THD *thd_arg, List<Item> *column_types, bool is_union_distinct,
                                                 ulonglong options, const LEX_CSTRING *alias, bool bit_fields_as_long,
                                                 bool create_table, bool keep_row_order, uint hidden)
{
  if (select_unit::create_result_table(thd_arg, column_types, is_union_distinct, options, &empty_clex_str,
                                       bit_fields_as_long, create_table, keep_row_order, hidden))
    return true;

  incr_table_param.init();
  incr_table_param.field_count = column_types->elements;
  incr_table_param.bit_fields_as_long = bit_fields_as_long;
  if (!(incr_table = create_tmp_table(thd_arg, &incr_table_param, *column_types, (ORDER *)0, false, 1, options,
                                      HA_POS_ERROR, &empty_clex_str, true, keep_row_order)))
    return true;

  incr_table->keys_in_use_for_query.clear_all();
  for (uint i = 0; i < table->s->fields; i++) incr_table->field[i]->flags &= ~(PART_KEY_FLAG | PART_INDIRECT_KEY_FLAG);
...
int select_unit::update_counter(Field *counter, longlong value)
{
  ...
  int error = table->file->ha_update_tmp_row(table->record[1], table->record[0]);
  return error;
}
...
{
  if (is_index_enabled && (curr_sl == curr_sl->master_unit()->union_distinct || !curr_sl->next_select()))
  {
...
int select_unit_ext::send_data(List<Item> &values)
{
  ...
  {
    case UNION_ALL:
      if (!is_index_enabled || (find_res = table->file->find_unique_row(table->record[0], 0)))
      {
        rc = write_record();
        /* no reaction with conversion */
        if (rc == -2)
...
bool select_unit_ext::send_eof()
{
  ...
  bool is_next_intersect_all = next_sl && next_sl->get_linkage() == INTERSECT_TYPE && !next_sl->distinct;
  bool need_unfold = (disable_index_if_needed(curr_sl) && !curr_sl->distinct);

  if (((curr_sl->distinct && !is_next_distinct) || curr_op_type == INTERSECT_ALL || is_next_intersect_all) &&
      !need_unfold)
  {
    if (!next_sl)
    ...
    do
    {
      ...
      if (need_update_row)
        error = table->file->ha_update_tmp_row(table->record[1], table->record[0]);
    } while (likely(!error));
    table->file->ha_rnd_end();
  }
...
void st_select_lex_unit::init_prepare_fake_select_lex(THD *thd_arg, bool first_execution)
{
  thd_arg->lex->current_select = fake_select_lex;
  fake_select_lex->table_list.link_in_list(&result_table_list, &result_table_list.next_local);
  ...
  */
  if ((fake_select_lex->changed_elements & TOUCHED_SEL_COND) && first_execution)
  {
    for (ORDER *order = global_parameters()->order_list.first; order; order = order->next)
      order->item = &order->item_ptr;
  }
}

bool st_select_lex_unit::prepare_join(THD *thd_arg, SELECT_LEX *sl, select_result *tmp_result,
                                      ulonglong additional_options, bool is_union_select)
{
  ...
  JOIN *join =
      new JOIN(thd_arg, sl->item_list, (sl->options | thd_arg->variables.option_bits | additional_options), tmp_result);
  if (!join)
    DBUG_RETURN(true);

  thd_arg->lex->current_select = sl;

  can_skip_order_by = (is_union_select && !(sl->braces && sl->limit_params.explicit_limit) && !thd->lex->with_rownum);

  saved_error = join->prepare(sl->table_list.first, (derived && derived->merged ? NULL : sl->where),
                              (can_skip_order_by ? 0 : sl->order_list.elements) + sl->group_list.elements,
                              can_skip_order_by ? NULL : sl->order_list.first, can_skip_order_by, sl->group_list.first,
                              sl->having, (is_union_select ? NULL : thd_arg->lex->proc_list.first), sl, this);
...
*/
bool st_select_lex_unit::join_union_type_handlers(THD *thd_arg, Type_holder *holders, uint count)
{
      ...
      else
      {
        ...
        if (holders[pos].aggregate_for_result(item_type_handler))
        {
          my_error(ER_ILLEGAL_PARAMETER_DATA_TYPES2_FOR_OPERATION, MYF(0), holders[pos].type_handler()->name().ptr(),
                   item_type_handler->name().ptr(), "UNION");
          DBUG_RETURN(true);
        }
      }
    }
...
*/
bool st_select_lex_unit::join_union_type_attributes(THD *thd_arg, Type_holder *holders, uint count)
{
  ...
  for (item_pos = 0, sl = first_sl; item_pos < count; sl = sl->next_select(), item_pos++)
  {
    Item *item_tmp;
    ...
    for (uint holder_pos = 0; (item_tmp = itx++); holder_pos++)
    {
      ...
      */
      DBUG_ASSERT(item_tmp->fixed() ||
                  (item_tmp->type() == Item::REF_ITEM && ((Item_ref *)(item_tmp))->ref_type() == Item_ref::OUTER_REF));
      if (!item_tmp->fixed())
...
*/
bool st_select_lex_unit::join_union_item_types(THD *thd_arg, List<Item> &types, uint count)
{
  DBUG_ENTER("st_select_lex_unit::join_union_select_list_types");
  SELECT_LEX *first_sl = first_select();
  Type_holder *holders;

  if (!(holders = new (thd_arg->mem_root) Type_holder[first_sl->item_list.elements]) ||
      join_union_type_handlers(thd_arg, holders, count) || join_union_type_attributes(thd_arg, holders, count))
    DBUG_RETURN(true);
  ...
  for (uint pos = 0; (item_tmp = it++); pos++)
  {
    ...
    */
    ...
    types.push_back(new (thd_arg->mem_root) Item_type_holder(thd_arg, item_tmp, holders[pos].type_handler(),
                                                             &holders[pos] /*Type_all_attributes*/, pos_maybe_null));
  }
...
bool st_select_lex_unit::prepare(TABLE_LIST *derived_arg, select_result *sel_result, ulonglong additional_options)
{
  ...
  bool have_except = false, have_intersect = false, have_except_all_or_intersect_all = false;
  ...
  if (is_recursive && (sl = first_sl->next_select()))
  {
    ...
    for (;; sl = next_sl)
    {
      ...
      {
        my_error(ER_NOT_SUPPORTED_YET, MYF(0), "mix of ALL and DISTINCT UNION operations in recursive CTE spec");
        DBUG_RETURN(TRUE);
      }
    }
  }
  ...
  */
  if (item && is_unit_op() && (item->is_in_predicate() || item->is_exists_predicate()))
  {
  ...
  if (is_union_select || is_recursive)
  {
    if ((single_tvc_wo_order && !fake_select_lex) ||
        (is_unit_op() && !union_needs_tmp_table() && !have_except && !have_intersect && !single_tvc))
    {
      SELECT_LEX *last = first_select();
      while (last->next_select()) last = last->next_select();
      if (!(tmp_result = union_result = new (thd->mem_root) select_union_direct(thd, sel_result, last)))
    ...
    else
    {
      ...
      {
        with_element->rec_result = new (thd->mem_root) select_union_recursive(thd);
        union_result = with_element->rec_result;
        if (fake_select_lex)
        {
          if (fake_select_lex->order_list.first || fake_select_lex->limit_params.explicit_limit)
          {
            my_error(ER_NOT_SUPPORTED_YET, MYF(0), "global ORDER_BY/LIMIT in recursive CTE spec");
            goto err;
          }
  ...
  if (!is_union_select && !is_recursive)
  {
    ...
    {
      if (prepare_join(thd, first_sl, tmp_result, additional_options, is_union_select))
        goto err;

      if (derived_arg && derived_arg->table && derived_arg->derived_type == VIEW_ALGORITHM_MERGE &&
  ...
  if (sl->tvc && sl->order_list.elements && !sl->tvc->to_be_wrapped_as_with_tail())
  {
  ...
  for (; sl; sl = sl->next_select(), union_part_count++)
  {
    if (sl->tvc)
    {
      if (sl->tvc->to_be_wrapped_as_with_tail() && !(thd->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW))

      {
        ...
        if (prepare_join(thd, sl, tmp_result, additional_options, is_union_select))
          goto err;
      }
      else if (sl->tvc->prepare(thd, sl, tmp_result, this))
        goto err;
    }
    else if (prepare_join(thd, sl, tmp_result, additional_options, is_union_select))
      goto err;
    ...
    */
    ...
    if (is_recursive)
    {
      ...
      if (!is_rec_result_table_created && (!sl->next_select() || sl->next_select() == with_element->first_recursive))
      {
        ulonglong create_options;
        create_options = (first_sl->options | thd->variables.option_bits | TMP_TABLE_ALL_COLUMNS);
        // Join data types for all non-recursive parts of a recursive UNION
        if (join_union_item_types(thd, types, union_part_count + 1))
          goto err;
        if (union_result->create_result_table(thd, &types, MY_TEST(union_distinct), create_options, &derived_arg->alias,
                                              false, instantiate_tmp_table, false, 0))
          goto err;
        if (have_except_all_or_intersect_all)
        {
          union_result->init();
        ...
        if (!derived_arg->table)
        {
          ...
          if ((!derived_arg->is_with_table_recursive_reference() || !derived_arg->derived_result) &&
              !(derived_arg->derived_result = new (thd->mem_root) select_unit(thd)))
            goto err;  // out of memory
          thd->create_tmp_table_for_derived = TRUE;

          res = derived_arg->derived_result->create_result_table(thd, &types, FALSE, create_options,
                                                                 &derived_arg->alias, FALSE, FALSE, FALSE, 0);
          ...
          if (derived_arg->is_with_table_recursive_reference())
          {
            ...
            derived_arg->with->rec_result->rec_table_refs.push_back(derived_arg);
          }
        }
  ...
  if (is_union_select)
  {
    ...
    */
    ...
    while ((type = tp++))
    {
      ...
      */
      if (type->type() == Item::TYPE_HOLDER &&
          type->type_handler()->union_element_finalize(static_cast<Item_type_holder *>(type)))
        goto err;
    }
    ...
    */
    if (global_parameters()->ftfunc_list->elements && global_parameters()->order_list.elements &&
    ...
    */
    ...
    if (!is_recursive)
    {
      ...
      bool error = union_result->create_result_table(
          thd, &types, MY_TEST(union_distinct) || have_except_all_or_intersect_all || have_intersect, create_options,
          &empty_clex_str, false, instantiate_tmp_table, false, hidden);
      union_result->addon_cnt = hidden;
      for (uint i = 0; i < hidden; i++) types.pop();
      if (unlikely(error))
        goto err;
    }
    ...
    if (!item_list.elements)
    {
      ...
      for (uint i = 0; i < hidden; i++) item_list.pop();
      ...
      if (fake_select_lex != NULL &&
          (thd->stmt_arena->is_stmt_prepare() || (thd->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW)))
      {
        ...
        if (!(fake_select_lex->join = new JOIN(thd, item_list, thd->variables.option_bits, result)))
        {
        ...
        (thd->stmt_arena->is_stmt_prepare() || (thd->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW)))
    {
      if (!fake_select_lex->join &&
          !(fake_select_lex->join = new JOIN(thd, item_list, thd->variables.option_bits, result)))
      {
        fake_select_lex->table_list.empty();
        DBUG_RETURN(TRUE);
      }
      saved_error = fake_select_lex->join->prepare(fake_select_lex->table_list.first, 0,
...
void st_select_lex_unit::optimize_bag_operation(bool is_outer_distinct)
{
  /*
    skip run optimize for:
      ...
      recursive
  */
  if ((thd->variables.sql_mode & MODE_ORACLE) || (thd->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW) ||
      (fake_select_lex != NULL && thd->stmt_arena->is_stmt_prepare()) || (with_element && with_element->is_recursive))
    return;
  ...
  */
  ...
  if (any_intersect_distinct || (intersect_end != NULL && intersect_end->distinct))
  {
  ...
  */
  if (disable_index && disable_index->linkage == INTERSECT_TYPE && intersect_end && intersect_end->distinct)
  ...
  {
    if (sl->is_unit_nest() && sl->first_inner_unit() && !sl->first_inner_unit()->bag_set_op_optimized)
    ...
    for (SELECT_LEX *sl = select_cursor; sl; sl = sl->next_select())
    {
      if (sl->tvc)
      {
        sl->tvc->select_options =
            (lim.is_unlimited() || sl->braces) ? sl->options & ~OPTION_FOUND_ROWS : sl->options | found_rows_for_union;
...
bool st_select_lex_unit::exec()
{
  ...
  if (executed && !uncacheable && !describe)
    DBUG_RETURN(FALSE);
  executed = 1;
  if (!(uncacheable & ~UNCACHEABLE_EXPLAIN) && item && !item->with_recursive_reference)
    item->make_const();
  ...
  {
    union_result->init();
    if (uncacheable & UNCACHEABLE_DEPENDENT && union_result->table && union_result->table->is_created())
    {
    ...
    for (SELECT_LEX *sl = select_cursor; sl; sl = sl->next_select())
    {
...

      {
        ...
        */
        if (sl->tvc)
        {
          sl->tvc->select_options = (lim.is_unlimited() || sl->braces) ? sl->options & ~OPTION_FOUND_ROWS
                                                                       : sl->options | found_rows_for_union;
          saved_error = sl->tvc->optimize(thd);
        }
        else
        {
          sl->join->select_options = (lim.is_unlimited() || sl->braces) ? sl->options & ~OPTION_FOUND_ROWS
                                                                        : sl->options | found_rows_for_union;
          saved_error = sl->join->optimize();
        }
      }
      if (likely(!saved_error))
      {
        ...
        if (sl->tvc)
          ...
          sl->join->exec();
        if (sl == union_distinct && !have_except_all_or_intersect_all && !(with_element && with_element->is_recursive))
        {
      ...
      if (found_rows_for_union && !sl->braces && !lim.is_unlimited())
      {
        /*
        ...
        */
        add_rows += (ulonglong)(thd->limit_found_rows - (ulonglong)((table->file->stats.records - records_at_start)));
      }
  ...
  DBUG_EXECUTE_IF("show_explain_probe_union_read", dbug_serve_apcs(thd, 1););
  {
    ...
    */
    ...
    if (fake_select_lex != NULL && likely(!thd->is_fatal_error))
    {
      ...
      if (!join)
      {
        ...
        */
        if (unlikely(!(fake_select_lex->join = new JOIN(thd, item_list, fake_select_lex->options, result))))
        {
          fake_select_lex->table_list.empty();
          goto err;
        }
        ...
        */
        ...
        saved_error = mysql_select(thd, &result_table_list, item_list, NULL, global_parameters()->order_list.elements,
                                   global_parameters()->order_list.first, NULL, NULL, NULL,
                                   fake_select_lex->options | SELECT_NO_UNLOCK, result, this, fake_select_lex);
      }
      else
      {
        if (describe)
...
bool st_select_lex_unit::exec_recursive()
{
  ...
  {
    if (!incr_table->is_created() &&
        instantiate_tmp_table(incr_table, tmp_table_param->keyinfo, tmp_table_param->start_recinfo,
                              &tmp_table_param->recinfo, 0))
      DBUG_RETURN(1);
  ...
  for (st_select_lex *sl = start; sl != end; sl = sl->next_select())
  {
    ...
    {
      for (TABLE_LIST *derived = with_element->derived_with_rec_ref.first; derived;
  ...
  while ((rec_tbl = li++))
  {
    ...
    saved_error = incr_table->insert_all_rows_into_tmp_table(thd, rec_table, tmp_table_param, !is_unrestricted);
  ...
  for (Item_subselect *sq = with_element->sq_with_rec_ref.first; sq; sq = sq->next_with_rec_ref)
  {
    sq->reset();
...
bool st_select_lex_unit::cleanup()
{
  ...
  if (with_element && with_element->is_recursive && union_result && with_element->rec_outer_references)
  {
    ...
    else
    {
      ...
      */
      ...
      while ((with_elem = with_elem->get_next_mutually_recursive()) != with_element)
...
bool st_select_lex_unit::change_result(select_result_interceptor *new_result, select_result_interceptor *old_result)
{
...
{
  for (; order; order = order->next) order->counter_used = 0;
}
...
bool st_select_lex::cleanup()
{
  ...
  DBUG_PRINT("info", ("select: %p (%u)  JOIN %p", this, select_number, join));
  ...
  if (join)
  {
    ...
    while ((tbl = ti++))
    {
      if (tbl->is_recursive_with_table() && !tbl->is_with_table_recursive_reference())
      {
        /*
  ...
  {
    if (lex_unit->with_element && lex_unit->with_element->is_recursive && lex_unit->with_element->rec_outer_references)
      continue;
...
void st_select_lex_unit::set_unique_exclude()
{
  for (SELECT_LEX *sl = first_select(); sl; sl = sl->next_select())
  {
    ...
    for (SELECT_LEX_UNIT *unit = sl->first_inner_unit(); unit; unit = unit->next_unit())
    {
