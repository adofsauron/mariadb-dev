...
static void signal_error_to_sql_driver_thread(THD *thd, rpl_group_info *rgi, int err);

static int rpt_handle_event(rpl_parallel_thread::queued_event *qev, struct rpl_parallel_thread *rpt)
{
  int err;
...
static void handle_queued_pos_update(THD *thd, rpl_parallel_thread::queued_event *qev)
{
  int cmp;
  Relay_log_info *rli;
  ...
  /*
    ...
    worker threads.
  */
  if ((thd->variables.option_bits & OPTION_BEGIN) && opt_using_transactions)
    return;
  ...
  if (e->stop_on_error_sub_id < (uint64)ULONGLONG_MAX || (e->force_abort && !rli->stop_for_until))
    return;
  ...
  else if (cmp == 0 && rli->group_relay_log_pos < qev->future_event_relay_log_pos)
    rli->group_relay_log_pos = qev->future_event_relay_log_pos;
  ...
  else if (cmp == 0 && rli->group_master_log_pos < qev->future_event_master_log_pos)
    rli->group_master_log_pos = qev->future_event_master_log_pos;
...
*/
static void wait_for_pending_deadlock_kill(THD *thd, rpl_group_info *rgi)
{
  PSI_stage_info old_stage;

  mysql_mutex_lock(&thd->LOCK_wakeup_ready);
  thd->ENTER_COND(&thd->COND_wakeup_ready, &thd->LOCK_wakeup_ready, &stage_waiting_for_deadlock_kill, &old_stage);
...
static void finish_event_group(rpl_parallel_thread *rpt, uint64 sub_id, rpl_parallel_entry *entry, rpl_group_info *rgi)
{
  THD *thd = rpt->thd;
  ...
  */
  ...
  if (entry->last_committed_sub_id < sub_id)
  {
    ...
    */
    ...
    while (tmp_gco && (!tmp_gco->next_gco || tmp_gco->last_sub_id > sub_id ||
  ...
  */
  ...
  DBUG_EXECUTE_IF("rpl_parallel_simulate_wait_at_retry", {
    if (rgi->current_gtid.seq_no == 1000)
    {
      ...
      debug_sync_set_action(thd, STRING_WITH_LEN("now WAIT_FOR proceed_by_1000"));
    }
  });
#endif
...
static void unlock_or_exit_cond(THD *thd, mysql_mutex_t *lock, bool *did_enter_cond, PSI_stage_info *old_stage)
{
  if (*did_enter_cond)
  {
    thd->EXIT_COND(old_stage);
...
static void register_wait_for_prior_event_group_commit(rpl_group_info *rgi, rpl_parallel_entry *entry)
{
  ...
  if (rgi->wait_commit_sub_id > entry->last_committed_sub_id)
  {
    ...
    */
    wait_for_commit *waitee = &rgi->wait_commit_group_info->commit_orderer;
...
*/
static bool do_gco_wait(rpl_group_info *rgi, group_commit_orderer *gco, bool *did_enter_cond, PSI_stage_info *old_stage)
{
  THD *thd = rgi->thd;
  ...
  if (wait_count > entry->count_committing_event_groups)
  {
    ...
    thd->ENTER_COND(&gco->COND_group_commit_orderer, &entry->LOCK_parallel_entry,
                    &stage_waiting_for_prior_transaction_to_start_commit, old_stage);
    *did_enter_cond = true;
    ...
    do
    {
      ...
      mysql_cond_wait(&gco->COND_group_commit_orderer, &entry->LOCK_parallel_entry);
...
static bool do_ftwrl_wait(rpl_group_info *rgi, bool *did_enter_cond, PSI_stage_info *old_stage)
{
  THD *thd = rgi->thd;
  rpl_parallel_entry *entry = rgi->parallel_entry;
  ...
  */
  ...
  {
    thd->ENTER_COND(&entry->COND_parallel_entry, &entry->LOCK_parallel_entry, &stage_waiting_for_ftwrl, old_stage);
    *did_enter_cond = true;
    thd->set_time_for_next_stage();
    do
    {
...
static int pool_mark_busy(rpl_parallel_thread_pool *pool, THD *thd)
{
  PSI_stage_info old_stage;
  int res = 0;
  ...
  */
  ...
  if (thd)
  {
    thd->ENTER_COND(&pool->COND_rpl_thread_pool, &pool->LOCK_rpl_thread_pool, &stage_waiting_for_rpl_thread_pool,
                    &old_stage);
    thd->set_time_for_next_stage();
  }
  while (pool->busy)
...
static void pool_mark_not_busy(rpl_parallel_thread_pool *pool)
{
  mysql_mutex_lock(&pool->LOCK_rpl_thread_pool);
...
void rpl_unpause_after_ftwrl(THD *thd)
{
  uint32 i;
...
*/
int rpl_pause_for_ftwrl(THD *thd)
{
  uint32 i;
  rpl_parallel_thread_pool *pool = &global_rpl_thread_pool;
  int err;
  ...
  */
  ...
  for (i = 0; i < pool->count; ++i)
  {
    ...
    */
    ...
    thd->ENTER_COND(&e->COND_parallel_entry, &e->LOCK_parallel_entry, &stage_waiting_for_ftwrl_threads_to_pause,
                    &old_stage);
    thd->set_time_for_next_stage();
    while (e->pause_sub_id < (uint64)ULONGLONG_MAX && e->last_committed_sub_id < e->pause_sub_id && !err)
    {
      if (unlikely(thd->check_killed()))
      {
        err = 1;
    ...
    */
    ...
    bool found = false;
    for (uint i = 0; i < mi_arr.elements() && !found; i++) found = mi_arr.at(i) == mi;
    if (!found)
    {
      mi_arr.append(mi);
...
#ifndef DBUG_OFF
static int dbug_simulate_tmp_error(rpl_group_info *rgi, THD *thd)
{
  if (rgi->current_gtid.domain_id == 0 && rgi->current_gtid.seq_no == 100 && rgi->retry_event_count == 4)
  {
    thd->clear_error();
    thd->get_stmt_da()->reset_diagnostics_area();
...
*/
static void convert_kill_to_deadlock_error(rpl_group_info *rgi)
{
  THD *thd = rgi->thd;
  int err_code;
  ...
  if ((rgi->speculation == rpl_group_info::SPECULATE_OPTIMISTIC && err_code != ER_PRIOR_COMMIT_FAILED) ||
      ((err_code == ER_QUERY_INTERRUPTED || err_code == ER_CONNECTION_KILLED) && rgi->killed_for_retry))
  {
    thd->clear_error();
    my_error(ER_LOCK_DEADLOCK, MYF(0));
    thd->reset_killed();
...
*/
static int is_group_ending(Log_event *ev, Log_event_type event_type)
{
  ...
  if (event_type == QUERY_EVENT)  // COMMIT/ROLLBACK are never compressed
  {
    ...
    if (qev->is_commit() || !strncmp(qev->query, STRING_WITH_LEN("XA COMMIT")) ||
...
static int retry_event_group(rpl_group_info *rgi, rpl_parallel_thread *rpt, rpl_parallel_thread::queued_event *orig_qev)
{
  IO_CACHE rlog;
  LOG_INFO linfo;
  File fd = (File)-1;
...
do_retry:
  ...
  */
  ...
  DBUG_EXECUTE_IF("rpl_parallel_simulate_wait_at_retry", {
    ...
    {
      debug_sync_set_action(thd, STRING_WITH_LEN("rpl_parallel_simulate_wait_at_retry WAIT_FOR proceed_by_1001"));
    }
  ...
  */
  ...
  {
    mysql_mutex_lock(&entry->LOCK_parallel_entry);
    if (entry->stop_on_error_sub_id == (uint64)ULONGLONG_MAX || DBUG_IF("simulate_mdev_12746") ||
  ...
  */
  ...
  description_event = read_relay_log_description_event(&rlog, cur_offset, &errmsg);
  ...
  do
  {
    ...
    for (;;)
    {
      ...
      ev = Log_event::read_log_event(&rlog, description_event, opt_slave_sql_verify_checksum);
      ...
      if ((err = rli->relay_log.find_log_pos(&linfo, log_name, 1)) || (err = rli->relay_log.find_next_log(&linfo, 1)))
      {
        char buff[22];
        sql_print_error("next log error: %d  offset: %s  log: %s", err, llstr(linfo.index_file_offset, buff), log_name);
        goto err;
      }
    ...
    mysql_mutex_lock(&rpt->LOCK_rpl_thread);
    qev = rpt->retry_get_qev(ev, orig_qev, log_name, old_offset, cur_offset - old_offset);
    mysql_mutex_unlock(&rpt->LOCK_rpl_thread);
    if (!qev)
    {
      delete ev;
    ...
    DBUG_EXECUTE_IF("rpl_parallel_simulate_infinite_temp_err_gtid_0_x_100", err = dbug_simulate_tmp_error(rgi, thd););
    if (!err)
      continue;
  ...
  check_retry:
    ...
    if (has_temporary_error(thd))
    {
      ...
      sql_print_error(
          "Slave worker thread retried transaction %lu time(s) "
...
pthread_handler_t handle_rpl_parallel_thread(void *arg)
{
  ...
  */
  ...
  while (rpt->delay_start) mysql_cond_wait(&rpt->COND_rpl_thread, &rpt->LOCK_rpl_thread);
  ...
  while (!rpt->stop)
  {
    ...
    thd->ENTER_COND(&rpt->COND_rpl_thread, &rpt->LOCK_rpl_thread, &stage_waiting_for_work_from_sql_thread, &old_stage);
    /*
    ...
    */
    while (!((events = rpt->event_queue) || (rpt->current_owner && !in_event_group) ||
             (rpt->current_owner && group_rgi->parallel_entry->force_abort) || rpt->stop))
    {
      if (!wait_count++)
        thd->set_time_for_next_stage();
  ...
  more_events:
    for (qev = events; qev; qev = next_qev)
    {
      ...
      else if (qev->typ == rpl_parallel_thread::queued_event::QUEUED_MASTER_RESTART)
      {
        if (in_event_group)
        {
          /*
          ...
          */
          ...
          finish_event_group(rpt, group_rgi->gtid_sub_id, qev->entry_for_queued, group_rgi);
      ...
      if ((event_type = qev->ev->get_type_code()) == GTID_EVENT)
      {
        ...
        if (rpt->channel_name_length)
          memcpy(rpt->channel_name, rgi->rli->mi->connection_name.str, rgi->rli->mi->connection_name.length);
        ...
        DBUG_EXECUTE_IF("rpl_parallel_scheduled_gtid_0_x_100", {
          if (rgi->current_gtid.domain_id == 0 && rgi->current_gtid.seq_no == 100)
          {
            debug_sync_set_action(thd, STRING_WITH_LEN("now SIGNAL scheduled_gtid_0_x_100"));
          }
        });
#endif
        ...
        if (unlikely(thd->wait_for_commit_ptr) && group_rgi != NULL)
        {
          ...
          */
          ...
          finish_event_group(rpt, group_rgi->gtid_sub_id, group_rgi->parallel_entry, group_rgi);
          rpt->loc_free_rgi(group_rgi);
        }
        ...
        */
        group_standalone = (0 != (static_cast<Gtid_log_event *>(qev->ev)->flags2 & Gtid_log_event::FL_STANDALONE));
        ...
        */
        ...
        unlock_or_exit_cond(thd, &entry->LOCK_parallel_entry, &did_enter_cond, &old_stage);

        thd->wait_for_commit_ptr = &rgi->commit_orderer;

        if (opt_gtid_ignore_duplicates && rgi->rli->mi->using_gtid != Master_info::USE_GTID_NO)
        {
          int res = rpl_global_gtid_slave_state->check_duplicate_gtid(&rgi->current_gtid, rgi);
          if (res < 0)
          {
            /* Error. */
            slave_output_error_info(rgi, thd);
        ...
        /*
          ...
          commit.
        */
        if (rgi->speculation == rpl_group_info::SPECULATE_WAIT && (err = thd->wait_for_prior_commit()))
        {
          slave_output_error_info(rgi, thd);
          signal_error_to_sql_driver_thread(thd, rgi, 1);
        }
      ...
      */
      if (likely(!rgi->worker_error) && !skip_event_group)
      {
        ...
        err = 0;
        DBUG_EXECUTE_IF(
            "rpl_parallel_simulate_temp_err_xid", if (event_type == XID_EVENT) {
              thd->clear_error();
              thd->get_stmt_da()->reset_diagnostics_area();
              my_error(ER_LOCK_DEADLOCK, MYF(0));
              err = 1;
        ...
        DBUG_EXECUTE_IF("rpl_parallel_simulate_temp_err_gtid_0_x_100", err = dbug_simulate_tmp_error(rgi, thd););
        if (unlikely(err))
        {
          convert_kill_to_deadlock_error(rgi);
      ...
      end_of_group = in_event_group && ((group_standalone && !Log_event::is_part_of_group(event_type)) || group_ending);
    ...
    */
    ...
    if (!in_event_group)
    {
      ...
      if (likely(rpt->current_entry) && unlikely(rpt->current_entry->force_abort))
...
static void dealloc_gco(group_commit_orderer *gco)
{
...
static int rpl_parallel_change_thread_count(rpl_parallel_thread_pool *pool, uint32 new_count, bool force)
{
  uint32 i;
  ...
  */
  if (!new_count && !force)
  {
    ...
    {
      DBUG_PRINT("warning", ("SQL threads running while trying to reset parallel pool"));
      pool_mark_not_busy(pool);
  ...
  */
  if (new_count && !my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME | MY_ZEROFILL), &new_list,
                                    new_count * sizeof(*new_list), &rpt_array, new_count * sizeof(*rpt_array), NULL))
  {
    my_error(ER_OUTOFMEMORY, MYF(0), (int(new_count * sizeof(*new_list) + new_count * sizeof(*rpt_array))));
    goto err;
  }
  ...
  for (i = 0; i < new_count; ++i)
  {
    ...
    mysql_mutex_init(key_LOCK_rpl_thread, &new_list[i]->LOCK_rpl_thread, MY_MUTEX_INIT_SLOW);
    mysql_cond_init(key_COND_rpl_thread, &new_list[i]->COND_rpl_thread, NULL);
    mysql_cond_init(key_COND_rpl_thread_queue, &new_list[i]->COND_rpl_thread_queue, NULL);
    mysql_cond_init(key_COND_rpl_thread_stop, &new_list[i]->COND_rpl_thread_stop, NULL);
    new_list[i]->pool = pool;
    if (mysql_thread_create(key_rpl_parallel_thread, &th, &connection_attrib, handle_rpl_parallel_thread, new_list[i]))
    {
      my_error(ER_OUT_OF_RESOURCES, MYF(0));
      goto err;
    }
  ...
  */
  for (i = 0; i < pool->count; ++i)
  {
    ...
    mysql_mutex_lock(&pool->LOCK_rpl_thread_pool);
    while ((rpt = pool->free_list) == NULL) mysql_cond_wait(&pool->COND_rpl_thread_pool, &pool->LOCK_rpl_thread_pool);
  ...
  for (i = 0; i < pool->count; ++i)
  {
    rpl_parallel_thread *rpt = pool->threads[i];
    mysql_mutex_lock(&rpt->LOCK_rpl_thread);
    while (rpt->running) mysql_cond_wait(&rpt->COND_rpl_thread, &rpt->LOCK_rpl_thread);
    mysql_mutex_unlock(&rpt->LOCK_rpl_thread);
  ...
  for (i = 0; i < pool->count; ++i)
  {
    ...
    while (!pool->threads[i]->running)
      mysql_cond_wait(&pool->threads[i]->COND_rpl_thread, &pool->threads[i]->LOCK_rpl_thread);
...
err:
  if (new_list)
  {
    while (new_free_list)
    {
      ...
      while (!new_free_list->running) mysql_cond_wait(&new_free_list->COND_rpl_thread, &new_free_list->LOCK_rpl_thread);
      while (new_free_list->running) mysql_cond_wait(&new_free_list->COND_rpl_thread, &new_free_list->LOCK_rpl_thread);
...
*/
int rpl_parallel_activate_pool(rpl_parallel_thread_pool *pool)
{
  int rc = 0;
  ...
  {
    pool_mark_not_busy(pool);
    rc = rpl_parallel_change_thread_count(pool, opt_slave_parallel_threads, 0);
    if (!rc)
    {
      if (pool->count)
      {
...
int rpl_parallel_inactivate_pool(rpl_parallel_thread_pool *pool)
{
  return rpl_parallel_change_thread_count(pool, 0, 0);
}

void rpl_parallel_thread::batch_free()
{
...
void rpl_parallel_thread::inuse_relaylog_refcount_update()
{
...
rpl_parallel_thread::queued_event *rpl_parallel_thread::get_qev_common(Log_event *ev, ulonglong event_size)
{
  queued_event *qev;
  mysql_mutex_assert_owner(&LOCK_rpl_thread);
...
rpl_parallel_thread::queued_event *rpl_parallel_thread::get_qev(Log_event *ev, ulonglong event_size,
                                                                Relay_log_info *rli)
{
...
rpl_parallel_thread::queued_event *rpl_parallel_thread::retry_get_qev(Log_event *ev, queued_event *orig_qev,
                                                                      const char *relay_log_name, ulonglong event_pos,
                                                                      ulonglong event_size)
{
  ...
  strcpy(qev->future_event_master_log_name, orig_qev->future_event_master_log_name);
  return qev;
}

void rpl_parallel_thread::loc_free_qev(rpl_parallel_thread::queued_event *qev)
{
  ...
  */
  if (++qev_free_pending >= QEV_BATCH_FREE || loc_qev_size >= opt_slave_parallel_max_queued / 3)
  {
...
void rpl_parallel_thread::free_qev(rpl_parallel_thread::queued_event *qev)
{
  mysql_mutex_assert_owner(&LOCK_rpl_thread);
...
rpl_group_info *rpl_parallel_thread::get_rgi(Relay_log_info *rli, Gtid_log_event *gtid_ev, rpl_parallel_entry *e,
                                             ulonglong event_size)
{
  rpl_group_info *rgi;
  mysql_mutex_assert_owner(&LOCK_rpl_thread);
  ...
  if ((rgi->deferred_events_collecting = rli->mi->rpl_filter->is_on()) && !rgi->deferred_events)
...
void rpl_parallel_thread::loc_free_rgi(rpl_group_info *rgi)
{
...
void rpl_parallel_thread::free_rgi(rpl_group_info *rgi)
{
  mysql_mutex_assert_owner(&LOCK_rpl_thread);
  DBUG_ASSERT(rgi->commit_orderer.waitee == NULL);
  rgi->free_annotate_event();
...
group_commit_orderer *rpl_parallel_thread::get_gco(uint64 wait_count, group_commit_orderer *prev, uint64 prior_sub_id)
{
  group_commit_orderer *gco;
  mysql_mutex_assert_owner(&LOCK_rpl_thread);
  ...
  mysql_cond_init(key_COND_group_commit_orderer, &gco->COND_group_commit_orderer, NULL);
...
void rpl_parallel_thread::loc_free_gco(group_commit_orderer *gco)
{
  if (!loc_gco_list)
...
rpl_parallel_thread_pool::rpl_parallel_thread_pool()
    : threads(0), free_list(0), count(0), inited(false), current_start_alters(0), busy(false), pfs_bkp{0, false, NULL}
{
}

int rpl_parallel_thread_pool::init(uint32 size)
{
  threads = NULL;
  free_list = NULL;
  count = 0;
  ...
  mysql_mutex_init(key_LOCK_rpl_thread_pool, &LOCK_rpl_thread_pool, MY_MUTEX_INIT_SLOW);
...
void rpl_parallel_thread_pool::destroy()
{
  deactivate();
  pfs_bkp.destroy();
  destroy_cond_mutex();
...
void rpl_parallel_thread_pool::deactivate()
{
  if (!inited)
    return;
...
void rpl_parallel_thread_pool::destroy_cond_mutex()
{
  if (!inited)
    return;
...
*/
struct rpl_parallel_thread *rpl_parallel_thread_pool::get_thread(rpl_parallel_thread **owner, rpl_parallel_entry *entry)
{
  rpl_parallel_thread *rpt;

  DBUG_ASSERT(count > 0);
  mysql_mutex_lock(&LOCK_rpl_thread_pool);
  while (unlikely(busy) || !(rpt = free_list)) mysql_cond_wait(&COND_rpl_thread_pool, &LOCK_rpl_thread_pool);
  free_list = rpt->next;
...
*/
void rpl_parallel_thread_pool::release_thread(rpl_parallel_thread *rpt)
{
...
void rpl_parallel_thread_pool::copy_pool_for_pfs(Relay_log_info *rli)
{
  if (pfs_bkp.inited)
  {
    for (uint i = 0; i < count; i++)
    {
      ...
      if (rpt->channel_name_length)
      {
        ...
        strmake(pfs_rpt->channel_name, rpt->channel_name, rpt->channel_name_length);
      }
      pfs_rpt->thd = rpt->thd;
      ...
      if (rli->err_thread_id && rpt->thd->thread_id == rli->err_thread_id)
      {
        ...
        strmake(pfs_rpt->last_error_message, rli->last_error().message, sizeof(rli->last_error().message));
...
*/
static bool handle_split_alter(rpl_parallel_entry *e, Gtid_log_event *gtid_ev, uint32 *idx,
                               // choose_thread_internal specific
                               bool *did_enter_cond, rpl_group_info *rgi, PSI_stage_info *old_stage)
{
      ...
      e->pending_start_alters)
  {
    ...
    */
    ...
    {
      if (!e->rpl_threads[j] || e->rpl_threads[j]->current_owner != &e->rpl_threads[j] ||
          !e->rpl_threads[j]->current_start_alter_id)
      {
  ...
  idx_found:
    ...
    if (flags_extra & Gtid_log_event::FL_START_ALTER_E1)
    {
      ...
      e->rpl_threads[e->rpl_thread_idx]->current_start_alter_domain_id = gtid_ev->domain_id;
      /*
  ...
  if (flags_extra & (Gtid_log_event::FL_COMMIT_ALTER_E1 | Gtid_log_event::FL_ROLLBACK_ALTER_E1))
  {
    // Free the corrosponding rpt current_start_alter_id
    for (uint i = 0; i < e->rpl_thread_max; i++)
    {
      if (e->rpl_threads[i] && e->rpl_threads[i]->current_start_alter_id == gtid_ev->sa_seq_no &&
...
*/
rpl_parallel_thread *rpl_parallel_entry::choose_thread(rpl_group_info *rgi, bool *did_enter_cond,
                                                       PSI_stage_info *old_stage, Gtid_log_event *gtid_ev)
{
  uint32 idx;
  ...
  if (gtid_ev)
  {
    ...
    if (gtid_ev->flags2 & (Gtid_log_event::FL_COMPLETED_XA | Gtid_log_event::FL_PREPARED_XA))
    {
      idx = my_hash_sort(&my_charset_bin, gtid_ev->xid.key(), gtid_ev->xid.key_length()) % rpl_thread_max;
    }
    rpl_thread_idx = idx;
  }
...
rpl_parallel_thread *rpl_parallel_entry::choose_thread_internal(uint idx, bool *did_enter_cond, rpl_group_info *rgi,
                                                                PSI_stage_info *old_stage)
{
    ...
    for (;;)
    {
      if (thr->current_owner != &rpl_threads[idx])
      {
        /*
          ...
          a new worker thread.
        */
        unlock_or_exit_cond(rli->sql_driver_thd, &thr->LOCK_rpl_thread, did_enter_cond, old_stage);
        thr = NULL;
        break;
      }
      ...
      else if (unlikely(rli->sql_driver_thd->check_killed(1)))
      {
        ...
        DBUG_EXECUTE_IF("rpl_parallel_wait_queue_max", {
          debug_sync_set_action(rli->sql_driver_thd, STRING_WITH_LEN("now SIGNAL wait_queue_killed"));
        };);
#endif
...
#ifdef ENABLED_DEBUG_SYNC
          DBUG_EXECUTE_IF("rpl_parallel_wait_queue_max", {
            debug_sync_set_action(rli->sql_driver_thd, STRING_WITH_LEN("now SIGNAL wait_queue_ready"));
          };);
#endif
          rli->sql_driver_thd->ENTER_COND(&thr->COND_rpl_thread_queue, &thr->LOCK_rpl_thread,
  ...
  if (!thr)
    rpl_threads[idx] = thr = global_rpl_thread_pool.get_thread(&rpl_threads[idx], this);

  return thr;
}

static void free_rpl_parallel_entry(void *element)
{
...
rpl_parallel::rpl_parallel() : current(NULL), sql_thread_stopping(false)
{
  my_hash_init(PSI_INSTRUMENT_ME, &domain_hash, &my_charset_bin, 32, offsetof(rpl_parallel_entry, domain_id),
               sizeof(uint32), NULL, free_rpl_parallel_entry, HASH_UNIQUE);
}
...
rpl_parallel::~rpl_parallel() { my_hash_free(&domain_hash); }

rpl_parallel_entry *rpl_parallel::find(uint32 domain_id, Relay_log_info *rli)
{
  struct rpl_parallel_entry *e;

  if (!(e = (rpl_parallel_entry *)my_hash_search(&domain_hash, (const uchar *)&domain_id, sizeof(domain_id))))
  {
    ...
    if (!my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME | MY_ZEROFILL), &e, sizeof(*e), &p, count * sizeof(*p), NULL))
    {
...
void rpl_parallel::wait_for_done(THD *thd, Relay_log_info *rli)
{
  struct rpl_parallel_entry *e;
  rpl_parallel_thread *rpt;
  uint32 i, j;
  ...
  */
  ...
  DBUG_EXECUTE_IF("rpl_parallel_wait_for_done_trigger",
                  { debug_sync_set_action(thd, STRING_WITH_LEN("now SIGNAL wait_for_done_waiting")); };);
#endif
...
*/
void rpl_parallel::stop_during_until()
{
  struct rpl_parallel_entry *e;
  uint32 i;
...
bool rpl_parallel::workers_idle()
{
  struct rpl_parallel_entry *e;
  uint32 i, max_i;
...
int rpl_parallel_entry::queue_master_restart(rpl_group_info *rgi, Format_description_log_event *fdev)
{
  uint32 idx;
...
int rpl_parallel::wait_for_workers_idle(THD *thd)
{
  uint32 i, max_i;
  ...
  */
  ...
  for (i = 0; i < max_i; ++i)
  {
    ...
    mysql_mutex_lock(&e->LOCK_parallel_entry);
    ++e->need_sub_id_signal;
    thd->ENTER_COND(&e->COND_parallel_entry, &e->LOCK_parallel_entry, &stage_waiting_for_workers_idle, &old_stage);
...
*/
bool process_gtid_for_restart_pos(Relay_log_info *rli, rpl_gtid *gtid)
{
  slave_connection_state::entry *gtid_entry;
  slave_connection_state *state = &rli->restart_gtid_pos;

  if (likely(state->count() == 0) || !(gtid_entry = state->find_entry(gtid->domain_id)))
    return false;
...
*/
static void abandon_worker_thread(THD *thd, rpl_parallel_thread *cur_thread, bool *did_enter_cond,
                                  PSI_stage_info *old_stage)
{
  unlock_or_exit_cond(thd, &cur_thread->LOCK_rpl_thread, did_enter_cond, old_stage);
...
int rpl_parallel::do_event(rpl_group_info *serial_rgi, Log_event *ev, ulonglong event_size)
{
  rpl_parallel_entry *e;
  ...
  if (unlikely(typ == ROTATE_EVENT))
  {
    ...
    if ((rev->server_id != global_system_variables.server_id || rli->replicate_same_server_id) &&
        !rev->is_relay_log_event() && !rli->is_in_group())
    {
      memcpy(rli->future_event_master_log_name, rev->new_log_ident, rev->ident_len + 1);
  ...
  */
  ...
  if (unlikely(!current) && typ != GTID_EVENT && !(unlikely(rli->gtid_skip_flag != GTID_SKIP_NOT) && is_group_event))
    return -1;
  ...
  {
    Format_description_log_event *fdev = static_cast<Format_description_log_event *>(ev);
    if (fdev->created)
    {
      /*
      ...
      */
      if (current->queue_master_restart(serial_rgi, fdev) || wait_for_workers_idle(rli->sql_driver_thd))
      {
        delete ev;
        return 1;
      }
  ...
  */
      ...
      else
      {
        ...
        if (typ == XID_EVENT || typ == XA_PREPARE_LOG_EVENT ||
            (typ == QUERY_EVENT &&  // COMMIT/ROLLBACK are never compressed
             (((Query_log_event *)ev)->is_commit() || ((Query_log_event *)ev)->is_rollback())))
          rli->gtid_skip_flag = GTID_SKIP_NOT;
      }
  ...
  if (typ == GTID_EVENT)
  {
    ...
    uint32 domain_id =
        (rli->mi->using_gtid == Master_info::USE_GTID_NO || rli->mi->parallel_mode <= SLAVE_PARALLEL_MINIMAL
             ? 0
             : gtid_ev->domain_id);
    ...
    if (process_gtid_for_restart_pos(rli, &gtid))
    {
      ...
      */
      rli->gtid_skip_flag =
          ((gtid_ev->flags2 & Gtid_log_event::FL_STANDALONE) ? GTID_SKIP_STANDALONE : GTID_SKIP_TRANSACTION);
      delete_or_keep_event_post_apply(serial_rgi, typ, ev);
      return 0;
    }
  }
  ...
  */
  cur_thread = e->choose_thread(serial_rgi, &did_enter_cond, &old_stage, gtid_ev);
  if (!cur_thread)
  {
  ...
  {
    abandon_worker_thread(rli->sql_driver_thd, cur_thread, &did_enter_cond, &old_stage);
    delete ev;
    return 1;
  }
  ...
  if (typ == GTID_EVENT)
  {
    ...
    */
    ...
    if (likely(gco))
    {
      ...
      if (mode <= SLAVE_PARALLEL_MINIMAL || !(gtid_flags & Gtid_log_event::FL_GROUP_COMMIT_ID) ||
      ...
      else if ((mode >= SLAVE_PARALLEL_OPTIMISTIC) && !(flags & group_commit_orderer::FORCE_SWITCH))
      {
        /*
        ...
        */
        ...
        if (!(gtid_flags & Gtid_log_event::FL_TRANSACTIONAL) ||
            ((!(gtid_flags & Gtid_log_event::FL_ALLOW_PARALLEL) || (gtid_flags & Gtid_log_event::FL_WAITED)) &&
  ...
  else if (!is_group_event)
  {
    ...
    */
    ...
    if (ev->is_relay_log_event())
      ...
      qev->future_event_master_log_pos = (static_cast<Rotate_log_event *>(ev))->pos;
    else
  ...
  */
  ...
  unlock_or_exit_cond(rli->sql_driver_thd, &cur_thread->LOCK_rpl_thread, &did_enter_cond, &old_stage);
  mysql_cond_signal(&cur_thread->COND_rpl_thread);
