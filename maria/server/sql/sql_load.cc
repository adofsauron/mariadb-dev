...
class READ_INFO : public Load_data_param
{
 ...
 public:
  ...
  READ_INFO(THD *thd, File file, const Load_data_param &param, String &field_term, String &line_start,
            String &line_term, String &enclosed, int escape, bool get_it_from_net, bool is_fifo);
  ~READ_INFO();
  int read_field();
  ...
  bool terminator(const Term_string &str) { return terminator(str.ptr(), str.length()); }
  bool terminator(int chr, const Term_string &str) { return str.initial_byte() == chr && terminator(str); }
...
static int read_xml_field(THD *thd, COPY_INFO &info, TABLE_LIST *table_list, List<Item> &fields_vars,
                          List<Item> &set_fields, List<Item> &set_values, READ_INFO &read_info, String &enclosed,
                          ulong skip_lines, bool ignore_check_option_errors);

#ifndef EMBEDDED_LIBRARY
static bool write_execute_load_query_log_event(THD *, const sql_exchange *, const char *, const char *, bool,
                                               enum enum_duplicates, bool, bool, int);
...
int mysql_load(THD *thd, const sql_exchange *ex, TABLE_LIST *table_list, List<Item> &fields_vars,
               List<Item> &set_fields, List<Item> &set_values, enum enum_duplicates handle_duplicates, bool ignore,
               bool read_file_from_client)
{
  ...
  */
  ...
  {
    my_message(ER_WRONG_FIELD_TERMINATORS, ER_THD(thd, ER_WRONG_FIELD_TERMINATORS), MYF(0));
    DBUG_RETURN(TRUE);
  }

  /* Report problems with non-ascii separators */
  if (!ex->escaped->is_ascii() || !ex->enclosed->is_ascii() || !ex->field_term->is_ascii() ||
  ...
  if (setup_tables_and_check_access(
          ...
          thd->lex->first_select_lex()->leaf_tables, FALSE, INSERT_ACL | UPDATE_ACL, INSERT_ACL | UPDATE_ACL, FALSE))
    DBUG_RETURN(-1);
  ...
  */
  ...
  {
    my_error(ER_UPDATE_TABLE_USED, MYF(0), table_list->table_name.str, "LOAD DATA");
    DBUG_RETURN(TRUE);
  }
  ...
  auto scope_cleaner = make_scope_exit([&fields_vars]() { fields_vars.empty(); });
  ...
  if (!fields_vars.elements)
  {
    ...
    {
      if (field_iterator.field() && field_iterator.field()->invisible > VISIBLE)
        continue;
      Item *item;
    ...
    /*
      ...
      in this case.
    */
    if (setup_fields(thd, Ref_ptr_array(), set_fields, MARK_COLUMNS_WRITE, 0, NULL, 0) ||
        setup_fields(thd, Ref_ptr_array(), set_values, MARK_COLUMNS_READ, 0, NULL, 0))
      DBUG_RETURN(TRUE);
  }
  else
  {  // Part field list
    ...
    if (setup_fields(thd, Ref_ptr_array(), fields_vars, MARK_COLUMNS_WRITE, 0, NULL, 0) ||
        setup_fields(thd, Ref_ptr_array(), set_fields, MARK_COLUMNS_WRITE, 0, NULL, 0) ||
        check_that_all_fields_are_given_values(thd, table, table_list))
      DBUG_RETURN(TRUE);
    /* Fix the expressions in SET clause */
    if (setup_fields(thd, Ref_ptr_array(), set_values, MARK_COLUMNS_READ, 0, NULL, 0))
      DBUG_RETURN(TRUE);
  }
  ...
  {
    my_message(ER_BLOBS_AND_NO_TERMINATED, ER_THD(thd, ER_BLOBS_AND_NO_TERMINATED), MYF(0));
    DBUG_RETURN(TRUE);
  }
...
#endif
  {
    ...
    if (thd->rgi_slave)
    {
      ...
      if (strncmp(thd->rgi_slave->rli->slave_patternload_file, name, thd->rgi_slave->rli->slave_patternload_file_size))
      {
        /*
    ...
    if (!thd->slave_thread && !((stat_info.st_mode & S_IFLNK) != S_IFLNK &&   // symlink
    ...
    if ((stat_info.st_mode & S_IFIFO) == S_IFIFO)
      is_fifo = 1;
#endif
    if ((file = mysql_file_open(key_file_load, name, O_RDONLY, MYF(MY_WME))) < 0)
  ...
  info.escape_char =
      (ex->escaped->length() && (ex->escaped_given() || !(thd->variables.sql_mode & MODE_NO_BACKSLASH_ESCAPES)))
          ? (*ex->escaped)[0]
          : INT_MAX;

  READ_INFO read_info(thd, file, param, *ex->field_term, *ex->line_start, *ex->line_term, *ex->enclosed,
  ...
  if (likely(!(error = MY_TEST(read_info.error))))
  {
    ...
    if (handle_duplicates == DUP_REPLACE && (!table->triggers || !table->triggers->has_delete_triggers()))
    ...
    if (table_list->table->validate_default_values_of_unset_fields(thd))
    {
      ...
      error = read_xml_field(thd, info, table_list, fields_vars, set_fields, set_values, read_info, *(ex->line_term),
                             skip_lines, ignore);
      ...
      error = read_sep_field(thd, info, table_list, fields_vars, set_fields, set_values, read_info, *ex->enclosed,
                             skip_lines, ignore);
    ...
    if (thd->locked_tables_mode <= LTM_LOCK_TABLES && table->file->ha_end_bulk_insert() && !error)
    {
  ...
  */
        ...
        */
        ...
        if (read_info.cache.wrote_create_file)
        {
          ...
          if (thd->transaction->stmt.modified_non_trans_table)
            (void)write_execute_load_query_log_event(thd, ex, table_list->db.str, table_list->table_name.str,
                                                     is_concurrent, handle_duplicates, ignore, transactional_table,
                                                     errcode);
          else
          {
  ...
  sprintf(name, ER_THD(thd, ER_LOAD_INFO), (ulong)info.records, (ulong)info.deleted,
          (ulong)(info.records - info.copied), (long)thd->get_stmt_da()->current_statement_warn_count());
    ...
    else
    {
      ...
      */
      ...
      if (read_info.cache.wrote_create_file)
      {
        int errcode = query_error_code(thd, killed_status == NOT_KILLED);
        error =
            write_execute_load_query_log_event(thd, ex, table_list->db.str, table_list->table_name.str, is_concurrent,
                                               handle_duplicates, ignore, transactional_table, errcode);
      }
...
err:
  DBUG_ASSERT(transactional_table || !(info.copied || info.deleted) || thd->transaction->stmt.modified_non_trans_table);
...
static bool write_execute_load_query_log_event(THD *thd, const sql_exchange *ex,
                                               ...
                                               const char *table_name_arg, bool is_concurrent,
                                               ...
                                               int errcode)
{
  ...
  String query_str(command_buffer, sizeof(command_buffer), system_charset_info);

  Load_log_event lle(thd, ex, tdb, table_name_arg, fv, is_concurrent, duplicates, ignore, transactional_table);
  ...
  */
  ...
  lle.print_query(thd, FALSE, (const char *)ex->cs ? ex->cs->cs_name.str : NULL, &query_str, &fname_start, &fname_end,
                  qualify_db);
  ...
  */
  ...
  Execute_load_query_log_event e(
      thd, load_data_query, query_str.length(), (uint)(fname_start - 1), (uint)fname_end,
                             ...
                             bool ignore_check_option_errors)
{
  ...
  while (!read_info.read_fixed_length())
  {
    ...
    if (progress_reports)
    {
      ...
      {
        time_to_report_progress += MY_HOW_OFTEN_TO_WRITE / 10;
        thd_progress_report(thd, thd->progress.counter, thd->progress.max_counter);
      }
    }
    if (skip_lines)
    {
    ...
    if (pos != read_info.row_end)
    {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_TOO_MANY_RECORDS,
                          ER_THD(thd, ER_WARN_TOO_MANY_RECORDS), thd->get_stmt_da()->current_row_for_warning());
    }

    if (thd->killed || fill_record_n_invoke_before_triggers(thd, table, set_fields, set_values,
                                                            ignore_check_option_errors, TRG_EVENT_INSERT))
      DBUG_RETURN(1);
                          ...
                          ulong skip_lines, bool ignore_check_option_errors)
{
  ...
  for (;; it.rewind())
  {
    ...
    if (unlikely(thd->killed) || unlikely(fill_record_n_invoke_before_triggers(
                                     thd, table, set_fields, set_values, ignore_check_option_errors, TRG_EVENT_INSERT)))
      DBUG_RETURN(1);
...
****************************************************************************/
static int read_xml_field(THD *thd, COPY_INFO &info, TABLE_LIST *table_list, List<Item> &fields_vars,
                          List<Item> &set_fields, List<Item> &set_values, READ_INFO &read_info, String &row_tag,
                          ulong skip_lines, bool ignore_check_option_errors)
{
  List_iterator_fast<Item> it(fields_vars);
  Item *item;
  ...
  for (;; it.rewind())
  {
    ...
    while ((tag = xmlit++))
    {
      DBUG_PRINT("read_xml_field", ("got tag:%i '%s' '%s'", tag->level, tag->field.c_ptr(), tag->value.c_ptr()));
    }
#endif
    ...
    while ((item = it++))
    {
      ...
      while (tag && strcmp(tag->field.c_ptr(), item->name.str) != 0) tag = xmlit++;

      Load_data_outvar *dst = item->get_load_data_outvar_or_error();
      DBUG_ASSERT(dst);
      if (!tag ? dst->load_data_set_null(thd, &read_info)
               : dst->load_data_set_value(thd, tag->value.ptr(), tag->value.length(), &read_info))
        DBUG_RETURN(1);
    }
...
READ_INFO::READ_INFO(THD *thd, File file_par, const Load_data_param &param, String &field_term, String &line_start,
                     String &line_term, String &enclosed_par, int escape, bool get_it_from_net, bool is_fifo)
    : Load_data_param(param),
      file(file_par),
      m_field_term(field_term),
      ...
      found_null(false)
{
  ...
  */
  ...
  uint length = MY_MAX(charset()->mbmaxlen, MY_MAX(m_field_term.length(), m_line_term.length())) + 1;
...
int READ_INFO::read_field()
{
  ...
  for (;;)
  {
    ...
    while (data.length() + charset()->mbmaxlen <= data.alloced_length())
    {
      ...
      else if (chr == m_field_term.initial_byte() && found_enclosed_char == INT_MAX)
      {
...
int READ_INFO::next_line()
{
  ...
  for (;;)
  {
    ...
    if (charset()->use_mb() && (chlen = charset()->charlen(buf, buf + 1)) != 1)
    {
      uint i;
      ...
      */
      if ((uchar)buf[0] == m_line_term.initial_byte() && i <= m_line_term.length() &&
          !memcmp(buf, m_line_term.ptr(), i))
      {
        ...
        */
        if (terminator(m_line_term.ptr() + i - 1, m_line_term.length() - i + 1))
          return 0;
      }
      /*
  ...
  for (chr = my_tospace(GET); chr != my_b_EOF;)
  {
    switch (chr)
    {
      case '<': /* read tag */
        ...
        if ((tag.length() == m_line_term.length() - 2) && (memcmp(tag.ptr(), m_line_term.ptr() + 1, tag.length()) == 0))
        {
          DBUG_PRINT("read_xml", ("start-of-row: %i %s %s", level, tag.c_ptr_safe(), m_line_term.ptr()));
        }
      ...
      case '>': /* end tag - read tag value */
        ...
        {
          DBUG_PRINT("read_xml", ("lev:%i tag:%s val:%s", level, tag.c_ptr_safe(), value.c_ptr_safe()));
      ...
      case '/': /* close tag */
        ...
        if ((tag.length() == m_line_term.length() - 2) && (memcmp(tag.ptr(), m_line_term.ptr() + 1, tag.length()) == 0))
        {
          DBUG_PRINT("read_xml", ("found end-of-row %i %s", level, tag.c_ptr_safe()));
      ...
      case '=': /* attribute name end - read the value */
        ...
        if (!strcmp(tag.c_ptr_safe(), "field") && !strcmp(attribute.c_ptr_safe(), "name"))
        {
          /*
        ...
        {
          DBUG_PRINT("read_xml", ("lev:%i att:%s val:%s", level + 1, attribute.c_ptr_safe(), value.c_ptr_safe()));
