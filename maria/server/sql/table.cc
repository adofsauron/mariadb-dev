...
bool TABLE::init_expr_arena(MEM_ROOT *mem_root)
{
  ...
  */
  expr_arena =
      new (alloc_root(mem_root, sizeof(Query_arena))) Query_arena(mem_root, Query_arena::STMT_CONVENTIONAL_EXECUTION);
...
struct extra2_fields
{
  ...
  void reset() { bzero((void *)this, sizeof(*this)); }
};

static Virtual_column_info *unpack_vcol_info_from_frm(THD *, TABLE *, String *, Virtual_column_info **, bool *);
...
static bool fix_type_pointers(const char ***typelib_value_names, uint **typelib_value_lengths, TYPELIB *point_to_type,
                              uint types, char *names, size_t names_length);

static field_index_t find_field(Field **fields, uchar *record, uint start, uint length);
...
Default_object_creation_ctx::Default_object_creation_ctx(THD *thd)
    : m_client_cs(thd->variables.character_set_client), m_connection_cl(thd->variables.collation_connection)
{
}

Default_object_creation_ctx::Default_object_creation_ctx(CHARSET_INFO *client_cs, CHARSET_INFO *connection_cl)
...
void Default_object_creation_ctx::change_env(THD *thd) const { thd->update_charset(m_client_cs, m_connection_cl); }
...
View_creation_ctx *View_creation_ctx::create(THD *thd, TABLE_LIST *view)
{
  ...
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_VIEW_NO_CREATION_CTX,
                        ER_THD(thd, ER_VIEW_NO_CREATION_CTX), view->db.str, view->table_name.str);
  ...
  invalid_creation_ctx =
      resolve_charset(view->view_client_cs_name.str, system_charset_info, &ctx->m_client_cs, MYF(utf8_flag));

  invalid_creation_ctx = resolve_collation(view->view_connection_cl_name.str, system_charset_info,
  ...
  if (invalid_creation_ctx)
  {
    sql_print_warning(
        ...
        view->db.str, view->table_name.str, (const char *)view->view_client_cs_name.str,
        (const char *)view->view_connection_cl_name.str);

    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_VIEW_INVALID_CREATION_CTX,
                        ER_THD(thd, ER_VIEW_INVALID_CREATION_CTX), view->db.str, view->table_name.str);
  }
...
static uchar *get_field_name(Field **buff, size_t *length, my_bool not_used __attribute__((unused)))
{
...
#ifdef WITH_WSREP
  if (db->str && my_strcasecmp(system_charset_info, db->str, "mysql") == 0 &&
...
TABLE_SHARE *alloc_table_share(const char *db, const char *table_name, const char *key, uint key_length)
{
  MEM_ROOT mem_root;
  TABLE_SHARE *share;
  ...
  path_length = build_table_filename(path, sizeof(path) - 1, db, table_name, "", 0);
  init_sql_alloc(key_memory_table_share, &mem_root, TABLE_ALLOC_BLOCK_SIZE, 0, MYF(0));
  if (multi_alloc_root(&mem_root, &share, sizeof(*share), &key_buff, key_length, &path_buff, path_length + 1, NULL))
  {
    ...
    if (key_length > 6 && table_alias_charset->strnncoll(key, 6, "mysql", 6) == 0)
      share->not_usable_by_query_cache = 1;

    init_sql_alloc(PSI_INSTRUMENT_ME, &share->stats_cb.mem_root, TABLE_ALLOC_BLOCK_SIZE, 0, MYF(0));

    memcpy((char *)&share->mem_root, (char *)&mem_root, sizeof(mem_root));
    mysql_mutex_init(key_TABLE_SHARE_LOCK_share, &share->LOCK_share, MY_MUTEX_INIT_SLOW);
    mysql_mutex_init(key_TABLE_SHARE_LOCK_ha_data, &share->LOCK_ha_data, MY_MUTEX_INIT_FAST);

    DBUG_EXECUTE_IF("simulate_big_table_id", if (last_table_id < UINT_MAX32) last_table_id = UINT_MAX32 - 1;);
    /*
    ...
    */
    do
    {
      share->table_map_id = last_table_id.fetch_add(1, std::memory_order_relaxed);
    } while (unlikely(share->table_map_id == ~0UL || share->table_map_id == 0));
  }
  DBUG_RETURN(share);
}
...
void init_tmp_table_share(THD *thd, TABLE_SHARE *share, const char *key, uint key_length, const char *table_name,
                          const char *path)
{
  ...
  */
  init_sql_alloc(key_memory_table_share, &share->mem_root, TABLE_ALLOC_BLOCK_SIZE, 0,
...
inline bool is_system_table_name(const char *name, size_t length)
{
  ...
  return (
      /* mysql.proc table */
      (length == 4 && my_tolower(ci, name[0]) == 'p' && my_tolower(ci, name[1]) == 'r' &&
       my_tolower(ci, name[2]) == 'o' && my_tolower(ci, name[3]) == 'c') ||

      (length > 4 &&
       (
           /* one of mysql.help* tables */
           (my_tolower(ci, name[0]) == 'h' && my_tolower(ci, name[1]) == 'e' && my_tolower(ci, name[2]) == 'l' &&
            my_tolower(ci, name[3]) == 'p') ||

           /* one of mysql.time_zone* tables */
           (my_tolower(ci, name[0]) == 't' && my_tolower(ci, name[1]) == 'i' && my_tolower(ci, name[2]) == 'm' &&
            my_tolower(ci, name[3]) == 'e') ||

           /* one of mysql.*_stat tables, but not mysql.innodb* tables*/
           ((my_tolower(ci, name[length - 5]) == 's' && my_tolower(ci, name[length - 4]) == 't' &&
             my_tolower(ci, name[length - 3]) == 'a' && my_tolower(ci, name[length - 2]) == 't' &&
             my_tolower(ci, name[length - 1]) == 's') &&
            !(my_tolower(ci, name[0]) == 'i' && my_tolower(ci, name[1]) == 'n' && my_tolower(ci, name[2]) == 'n' &&
              my_tolower(ci, name[3]) == 'o')) ||

           /* mysql.event table */
           (my_tolower(ci, name[0]) == 'e' && my_tolower(ci, name[1]) == 'v' && my_tolower(ci, name[2]) == 'e' &&
            my_tolower(ci, name[3]) == 'n' && my_tolower(ci, name[4]) == 't'))));
}
...
enum open_frm_error open_table_def(THD *thd, TABLE_SHARE *share, uint flags)
{
  ...
  DBUG_PRINT("enter",
             ("table: '%s'.'%s'  path: '%s'", share->db.str, share->table_name.str, share->normalized_path.str));

  share->error = OPEN_FRM_OPEN_ERROR;

  length = (uint)(strxmov(path, share->normalized_path.str, reg_ext, NullS) - path);
  if (flags & GTS_FORCE_DISCOVERY)
  {
    ...
    mysql_file_delete_with_symlink(key_file_partition_ddl_log, path2, PAR_EXT, MYF(0));
    file = -1;
  }
  else
  ...
  {
    share->error = my_errno == HA_ERR_FILE_TOO_SHORT ? OPEN_FRM_CORRUPTED : OPEN_FRM_READ_ERROR;
    goto err;
  }
  ...
  if (!(buf = (uchar *)my_malloc(PSI_INSTRUMENT_ME, frmlen, MYF(MY_THREAD_SPECIFIC | MY_WME))))
    goto err;

  memcpy(buf, head, sizeof(head));

  read_length = mysql_file_read(file, buf + sizeof(head), frmlen - sizeof(head), MYF(MY_WME));
...
static bool create_key_infos(const uchar *strpos, const uchar *frm_image_end, uint keys, KEY *keyinfo, uint new_frm_ver,
                             uint *ext_key_parts, TABLE_SHARE *share, uint len, KEY *first_keyinfo,
                             LEX_STRING *keynames)
{
  uint i, j, n_length;
  ...
  */
  ...
  for (i = 0; i < keys; i++, keyinfo++)
  {
    ...
    if (i == 0)
    {
      ...
      if (!(keyinfo = (KEY *)alloc_root(&share->mem_root, n_length + len)))
        return 1;
      bzero((char *)keyinfo, n_length);
      share->key_info = keyinfo;
      key_part = reinterpret_cast<KEY_PART_INFO *>(keyinfo + keys);

      if (!(rec_per_key = (ulong *)alloc_root(&share->mem_root, sizeof(ulong) * *ext_key_parts)))
        return 1;
  ...
  keynames->length = strnmov(keynames->str, (char *)strpos, frm_image_end - strpos) - keynames->str;
  ...
  for (keyinfo = share->key_info, i = 0; i < keys; i++, keyinfo++)
  {
    if (keyinfo->flags & HA_USES_COMMENT)
    {
      ...
      if (strpos + keyinfo->comment.length >= frm_image_end)
        return 1;
      keyinfo->comment.str = strmake_root(&share->mem_root, (char *)strpos, keyinfo->comment.length);
      strpos += keyinfo->comment.length;
    }
    DBUG_ASSERT(MY_TEST(keyinfo->flags & HA_USES_COMMENT) == (keyinfo->comment.length > 0));
  }
...
static uint enum_value_with_check(THD *thd, TABLE_SHARE *share, const char *name, uint value, uint limit)
{
  if (value < limit)
    return value;

  sql_print_warning("%s.frm: invalid value %d for the field %s", share->normalized_path.str, value, name);
  return 0;
}
...
static uint upgrade_collation(ulong mysql_version, uint cs_number)
{
  if (mysql_version >= 50300 && mysql_version <= 50399)
  {
    ...
    {
      case 149:
        return MY_PAGE2_COLLATION_ID_UCS2;  // ucs2_crotian_ci
      case 213:
        return MY_PAGE2_COLLATION_ID_UTF8;  // utf8_crotian_ci
    }
  }
  if ((mysql_version >= 50500 && mysql_version <= 50599) || (mysql_version >= 100000 && mysql_version <= 100005))
  {
    switch (cs_number)
    {
      case 149:
      ...
      case 213:
        return MY_PAGE2_COLLATION_ID_UTF8;  // utf8_crotian_ci
      case 214:
        return MY_PAGE2_COLLATION_ID_UTF32;  // utf32_croatian_ci
      case 215:
        return MY_PAGE2_COLLATION_ID_UTF16;  // utf16_croatian_ci
      case 245:
        return MY_PAGE2_COLLATION_ID_UTF8MB4;  // utf8mb4_croatian_ci
    }
  }
  return cs_number;
}
...
bool Column_definition_attributes::frm_unpack_numeric_with_dec(TABLE_SHARE *share, const uchar *buff)
{
...
bool Column_definition_attributes::frm_unpack_temporal_with_dec(TABLE_SHARE *share, uint intlen, const uchar *buff)
{
  frm_unpack_basic(buff);
...
bool Column_definition_attributes::frm_unpack_charset(TABLE_SHARE *share, const uchar *buff)
{
...
static void mysql57_calculate_null_position(TABLE_SHARE *share, uchar **null_pos, uint *null_bit_pos,
                                            const uchar *strpos, const uchar *vcol_screen_pos)
{
  ...
  for (uint i = 0; i < share->fields; i++, strpos += field_pack_length)
  {
    ...
    if ((strpos[10] & MYSQL57_GENERATED_FIELD))
    {
      ...
      vcol_screen_pos += (uint2korr(vcol_screen_pos + 1) + MYSQL57_GCOL_HEADER_SIZE);
      if (!stored_in_db)
        continue;
    }
...
/** Parse TABLE_SHARE::vcol_defs
  ...
  10.2+
    ...
    expression
*/
bool parse_vcol_defs(THD *thd, MEM_ROOT *mem_root, TABLE *table, bool *error_reported, vcol_init_mode mode)
{
  struct check_vcol_forward_refs
  {
    static bool check(Field *field, Virtual_column_info *vcol)
    {
      return vcol && vcol->expr->walk(&Item::check_field_expression_processor, 0, field);
    }
    ...
    {
      if (check(field, field->vcol_info) || check_constraint(field, field->check_constraint) ||
  ...
  if (check_constraint_ptr)
    memcpy(table->check_constraints + table->s->field_check_constraints, table->s->check_constraints,
  ...
  Sql_mode_instant_remove sms(thd, MODE_NO_BACKSLASH_ESCAPES | MODE_EMPTY_STRING_IS_NULL);
  ...
  while (pos < end)
  {
    ...
    else
    {
      ...
      */
      ...
      type = (*field_ptr)->vcol_info->stored_in_db ? VCOL_GENERATED_STORED : VCOL_GENERATED_VIRTUAL;
    ...
    switch (type)
    {
      ...
      case VCOL_GENERATED_STORED:
        vcol = unpack_vcol_info_from_frm(thd, table, &expr_str, &((*field_ptr)->vcol_info), error_reported);
        ...
        */
        ...
        break;
      case VCOL_DEFAULT:
        vcol = unpack_vcol_info_from_frm(thd, table, &expr_str, &((*field_ptr)->default_value), error_reported);
        *(dfield_ptr++) = *field_ptr;
        if (vcol && (vcol->flags & (VCOL_NON_DETERMINISTIC | VCOL_SESSION_FUNC)))
          table->s->non_determinstic_insert = true;
        break;
      case VCOL_CHECK_FIELD:
        vcol = unpack_vcol_info_from_frm(thd, table, &expr_str, &((*field_ptr)->check_constraint), error_reported);
        *check_constraint_ptr++ = (*field_ptr)->check_constraint;
        break;
      case VCOL_CHECK_TABLE:
        vcol = unpack_vcol_info_from_frm(thd, table, &expr_str, check_constraint_ptr, error_reported);
    ...
    if (field->flags & LONG_UNIQUE_HASH_FIELD)
    {
      ...
      for (uint i = 0; i < parts; i++)
      {
        ...
        if (keypart->key_part_flag & HA_PART_KEY_SEG)
        {
          ...
          list_item = new (mem_root)
              Item_func_left(thd, new (mem_root) Item_field(thd, keypart->field), new (mem_root) Item_int(thd, length));
          list_item->fix_fields(thd, NULL);
          keypart->field->vcol_info = table->field[keypart->field->field_index]->vcol_info;
        }
        else
...
static const Type_handler *old_frm_type_handler(uint pack_flag, uint interval_nr)
{
...
bool TABLE_SHARE::init_period_from_extra2(period_info_t *period, const uchar *data, const uchar *end)
{
...
static bool read_extra2_section_once(const uchar *extra2, size_t len, LEX_CUSTRING *section)
{
  if (section->str)
    return true;
  *section = {extra2, len};
...
static bool read_extra2(const uchar *frm_image, size_t len, extra2_fields *fields)
{
    ...
    while (extra2 + 3 <= e2end)
    {
      ...
      switch (type)
      {
        ...
        case EXTRA2_PERIOD_FOR_SYSTEM_TIME:
          fail = read_extra2_section_once(extra2, length, &fields->system_period) || length != 2 * frm_fieldno_size;
          break;
...
class Field_data_type_info_array
{
 public:
  class Elem
  {
   ...
   public:
    void set(const LEX_CSTRING &type_info) { m_type_info = type_info; }
    const LEX_CSTRING &type_info() const { return m_type_info; }
  };
 ...
 public:
  Field_data_type_info_array() : m_array(NULL), m_count(0) {}
...
int TABLE_SHARE::init_from_binary_frm_image(THD *thd, bool write, const uchar *frm_image, size_t frm_length,
                                            const uchar *par_image, size_t par_length)
{
  ...
  */
  ...
  tabledef_version.str = (uchar *)memdup_root(&mem_root, extra2.version.str, extra2.version.length);
  ...
  if (frm_length < FRM_HEADER_SIZE + len || !(pos = uint4korr(frm_image + FRM_HEADER_SIZE + len)))
    goto err;
  ...
  */
  if (legacy_db_type > DB_TYPE_UNKNOWN && legacy_db_type < DB_TYPE_FIRST_DYNAMIC)
  ...
  if (!frm_image[32])  // New frm file in 3.23
  {
    ...
    share->transactional =
        (ha_choice)enum_value_with_check(thd, share, "transactional", frm_image[39] & 3, HA_CHOICE_MAX);
    share->page_checksum =
        (ha_choice)enum_value_with_check(thd, share, "page_checksum", (frm_image[39] >> 2) & 3, HA_CHOICE_MAX);
    if (((ha_choice)enum_value_with_check(thd, share, "sequence", (frm_image[39] >> 4) & 3, HA_CHOICE_MAX)) ==
        HA_CHOICE_YES)
    {
    ...
    share->row_type = (enum row_type)enum_value_with_check(thd, share, "row_format", frm_image[40], ROW_TYPE_MAX);
  ...
  record_offset =
      (ulong)(uint2korr(frm_image + 6) +
              ((uint2korr(frm_image + 14) == 0xffff ? uint4korr(frm_image + 47) : uint2korr(frm_image + 14))));
  ...
  if ((n_length = uint4korr(frm_image + 55)))
  {
    ...
    if (!(share->connect_string.str =
              strmake_root(&share->mem_root, (char *)next_chunk + 2, share->connect_string.length)))
    {
      goto err;
    }
    ...
    if (next_chunk + 2 < buff_end)
    {
      ...
      if (tmp_plugin != NULL && !plugin_equals(tmp_plugin, se_plugin) && legacy_db_type != DB_TYPE_S3)
      {
        if (se_plugin)
        {
          /* bad file, legacy_db_type did not match the name */
          sql_print_warning("%s.frm is inconsistent: engine typecode %d, engine name %s (%d)",
                            share->normalized_path.str, legacy_db_type, plugin_name(tmp_plugin)->str,
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
      else if (str_db_type_length == 9 && !strncmp((char *)next_chunk + 2, "partition", 9))
      {
        /*
    ...
    if (create_key_infos(disk_buff + 6, frm_image_end, keys, keyinfo, new_frm_ver, &ext_key_parts, share, len,
                         &first_keyinfo, &keynames))
      goto err;
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
      if ((share->partition_info_buffer_size = share->partition_info_str_len = partition_info_str_len))
      {
        if (!(share->partition_info_str =
                  (char *)memdup_root(&share->mem_root, next_chunk + 4, partition_info_str_len + 1)))
        {
          goto err;
        }
      }
    ...
    for (i = 0; i < keys; i++, keyinfo++)
    {
      if (keyinfo->flags & HA_USES_PARSER)
      {
        ...
        {
          DBUG_PRINT("error", ("fulltext key uses parser that is not defined in .frm"));
          goto err;
        }
        parser_name.str = (char *)next_chunk;
        parser_name.length = strlen((char *)next_chunk);
        next_chunk += parser_name.length + 1;
        keyinfo->parser = my_plugin_lock_by_name(NULL, &parser_name, MYSQL_FTPARSER_PLUGIN);
        if (!keyinfo->parser)
        {
    ...
    if (forminfo[46] == (uchar)255)
    {
      ...
      if (!(share->comment.str = strmake_root(&share->mem_root, (char *)next_chunk + 2, share->comment.length)))
      {
        goto err;
      }
  ...
  share->virtual_fields = share->default_expressions = share->field_check_constraints = share->default_fields = 0;
  share->visible_fields = 0;
  share->stored_fields = share->fields;
  if (forminfo[46] != (uchar)255)
  {
    share->comment.length = (int)(forminfo[46]);
    share->comment.str = strmake_root(&share->mem_root, (char *)forminfo + 47, share->comment.length);
  }

  DBUG_PRINT(
      "info",
      ("i_count: %d  i_parts: %d  index: %d  n_length: %d  int_length: %d  com_length: %d  vcol_screen_length: %d",
       interval_count, interval_parts, keys, n_length, int_length, com_length, vcol_screen_length));
  ...
  */
  total_typelib_value_count = (share->fields + 1 /*end-of-values marker*/) +
                              (interval_parts + interval_count /*end-of-values markers*/) +
                              (keys + 1 /*end-of-values marker*/);

  if (!multi_alloc_root(&share->mem_root, &share->field, (uint)(share->fields + 1) * sizeof(Field *), &share->intervals,
                        (uint)interval_count * sizeof(TYPELIB), &share->check_constraints,
                        (uint)share->table_check_constraints * sizeof(Virtual_column_info *),
                        /*
                           This looks wrong: shouldn't it be (+2+interval_count)
                           instread of (+3) ?
                        */
                        &interval_array, (uint)(share->fields + interval_parts + keys + 3) * sizeof(char *),
                        &typelib_value_lengths, total_typelib_value_count * sizeof(uint *), &names,
                        (uint)(n_length + int_length), &comment_pos, (uint)com_length, &vcol_screen_pos,
                        vcol_screen_length, NullS))
  ...
  read_length =
      (uint)(share->fields * field_pack_length + pos + (uint)(n_length + int_length + com_length + vcol_screen_length));
  ...
  memcpy(comment_pos, disk_buff + read_length - com_length - vcol_screen_length, com_length);
  memcpy(vcol_screen_pos, disk_buff + read_length - vcol_screen_length, vcol_screen_length);

  if (fix_type_pointers(&interval_array, &typelib_value_lengths, &share->fieldnames, 1, names, n_length) ||
      share->fieldnames.count != share->fields)
    goto err;

  if (fix_type_pointers(&interval_array, &typelib_value_lengths, share->intervals, interval_count, names + n_length,
                        int_length))
    goto err;

  if (keynames.length &&
      (fix_type_pointers(&interval_array, &typelib_value_lengths, &share->keynames, 1, keynames.str, keynames.length) ||
       share->keynames.count != keys))
    goto err;

  /* Allocate handler */
  if (!(handler_file = get_new_handler(share, thd->mem_root, plugin_hton(se_plugin))))
    goto err;
  ...
  if (use_hash)
    use_hash = !my_hash_init(PSI_INSTRUMENT_ME, &share->name_hash, system_charset_info, share->fields, 0, 0,
                             (my_hash_get_key)get_field_name, 0, 0);

  if (share->mysql_version >= 50700 && share->mysql_version < 100000 && vcol_screen_length)
  {
    /*
    ...
    */
    ...
    mysql57_calculate_null_position(share, &mysql57_vcol_null_pos, &mysql57_vcol_null_bit_pos, strpos, vcol_screen_pos);
  }
  ...
  else
  {
    ...
    if (init_period_from_extra2(&vers, extra2.system_period.str,
      ...
      goto err;
    DBUG_PRINT("info", ("Columns with system versioning: [%d, %d]", vers.start_fieldno, vers.end_fieldno));
  ...
  if (extra2.application_period.str)
  {
    ...
    period.constr_name.str = strmake_root(&mem_root, (char *)pos, period.constr_name.length);
    pos += period.constr_name.length;

    if (init_period_from_extra2(&period, pos, end))
      goto err;
    if (extra2_str_size(period.name.length) + extra2_str_size(period.constr_name.length) + 2 * frm_fieldno_size !=
  ...
  if (extra2.without_overlaps.str)
  {
    ...
    if ((period.unique_keys + 1) * frm_keyno_size != extra2.without_overlaps.length)
      goto err;
  }

  if (extra2.field_data_type_info.length &&
      field_data_type_info_array.parse(old_root, share->fields, extra2.field_data_type_info))
    goto err;
  ...
  for (i = 0; i < share->fields; i++, strpos += field_pack_length, field_ptr++)
  {
    ...
    if (new_frm_ver >= 3)
    {
      ...
      if ((uchar)strpos[13] == (uchar)MYSQL_TYPE_VIRTUAL && likely(share->mysql_version >= 100000))
      {
        /*
        ...
        */
        ...
        uint vcol_expr_length = vcol_info_length - (uint)(FRM_VCOL_OLD_HEADER_SIZE(opt_interval_id));
      ...
      else
      {
        ...
        {
          if (field_type == 245 && share->mysql_version >= 50700)  // a.k.a MySQL 5.7 JSON
          {
        ...
        if (handler->Column_definition_attributes_frm_unpack(&attr, share, strpos, &extra2.gis))
          goto err;

        if (field_data_type_info_array.count())
        {
          const LEX_CSTRING &info = field_data_type_info_array.element(i).type_info();
          DBUG_EXECUTE_IF("frm_data_type_info",
                          push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                                              "DBUG: [%u] name='%s' type_info='%.*s'", i,
                                              share->fieldnames.type_names[i], (uint)info.length, info.str););

          if (info.length)
          {
            const Type_handler *h = Type_handler::handler_by_name_or_error(thd, info);
            /*
    ...
    else
    {
      ...
      recpos = uint2korr(strpos + 4), attr.pack_flag = uint2korr(strpos + 6);
      ...
      if (f_is_binary(attr.pack_flag))
      {
        ...
        */
        if (!f_is_blob(attr.pack_flag))
        {
          // 3.23 or 4.0 string
          myf utf8_flag = thd->get_utf8_flag();
          if (!(attr.charset = get_charset_by_csname(share->table_charset->cs_name.str, MY_CS_BINSORT, MYF(utf8_flag))))
    ...
    if (interval_nr && attr.charset->mbminlen > 1 && !interval_unescaped[interval_nr - 1])
    {
      /*
    ...
    if (handler->real_field_type() == MYSQL_TYPE_NEWDECIMAL && !share->mysql_version)
    {
      /*
      ...
      */
      ...
      attr.length = my_decimal_precision_to_length((uint)attr.length, decimals, f_is_dec(attr.pack_flag) == 0);
      sql_print_error(
          "Found incompatible DECIMAL field '%s' in %s; "
          "Please do \"ALTER TABLE '%s' FORCE\" to fix it!",
          share->fieldnames.type_names[i], share->table_name.str, share->table_name.str);
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_CRASHED_ON_USAGE,
                          "Found incompatible DECIMAL field '%s' in %s; "
                          "Please do \"ALTER TABLE '%s' FORCE\" to fix it!",
                          share->fieldnames.type_names[i], share->table_name.str, share->table_name.str);
      ...
      if (flags & VERS_SYSTEM_FIELD)
      {
        ...
        switch (field_type)
        {
          ...
          case MYSQL_TYPE_LONGLONG:
            ...
            /* Fallthrough */
          default:
            my_error(ER_VERS_FIELD_WRONG_TYPE, (field_type == MYSQL_TYPE_LONGLONG ? MYF(0) : MYF(ME_WARNING)),
                     fieldnames.type_names[i], (versioned == VERS_TIMESTAMP ? "TIMESTAMP(6)" : "BIGINT(20) UNSIGNED"),
                     table_name.str);
            goto err;
        }
      }
    ...
    *field_ptr = reg_field = attr.make_field(share, &share->mem_root, &addr, handler, &name, flags);
    if (!reg_field)  // Not supported field type
      goto err;

    if (attr.unireg_check == Field::TIMESTAMP_DNUN_FIELD || attr.unireg_check == Field::TIMESTAMP_DN_FIELD)
    {
    ...
    if (handler->real_field_type() == MYSQL_TYPE_BIT && !f_bit_as_char(attr.pack_flag))
    {
  ...
  *field_ptr = 0;  // End marker
  ...
  if (key_parts)
  {
    ...
    keyinfo = share->key_info;
    uint primary_key =
        my_strcasecmp(system_charset_info, share->keynames.type_names[0], primary_key_name.str) ? MAX_KEY : 0;
    KEY *key_first_info = NULL;

    if (primary_key >= MAX_KEY && keyinfo->flags & HA_NOSAME && keyinfo->algorithm != HA_KEY_ALG_LONG_HASH)
    {
      /*
      ...
      */
      ...
      for (i = 0; i < keyinfo->user_defined_key_parts; i++)
      {
        ...
        /*
          ...
          it to primary key.
        */
        if (!table_field->real_maybe_null() && table_field->type() == MYSQL_TYPE_BLOB &&
            table_field->field_length == key_part[i].length)
          continue;

        if (table_field->real_maybe_null() || table_field->key_length() != key_part[i].length)
        {
    ...
    */
      ...
      else
      {
        ...
        */
        for (i = 0; i < first_keyinfo.user_defined_key_parts; i++)
        {
          ...
          if (share->field[fieldnr - 1]->key_length() != keyinfo[0].key_part[i].length)
          {
    ...
    for (uint key = 0; key < keys; key++, keyinfo++)
    {
      ...
      keyinfo->cache_name =
          (uchar *)alloc_root(&share->mem_root, share->table_cache_key.length + keyinfo->name.length + 1);
      if (keyinfo->cache_name)  // If not out of memory
      {
        ...
        memcpy(pos + share->table_cache_key.length, keyinfo->name.str, keyinfo->name.length + 1);
      }

      if (ext_key_parts > share->key_parts && key)
      {
        KEY_PART_INFO *new_key_part = (keyinfo - 1)->key_part + (keyinfo - 1)->ext_key_parts;
        ...
        */
        for (i = 0; i < keyinfo->user_defined_key_parts; i++)
        {
          ...
          ext_key_length += keyinfo->key_part[i].length + len_null_byte + length_bytes;
        ...
        {
          memmove(new_key_part, keyinfo->key_part, sizeof(KEY_PART_INFO) * keyinfo->ext_key_parts);
      ...
      key_part = keyinfo->key_part;
      uint key_parts = share->use_ext_keys ? keyinfo->ext_key_parts : keyinfo->user_defined_key_parts;
      if (keyinfo->algorithm == HA_KEY_ALG_LONG_HASH)
        key_parts++;
      for (i = 0; i < key_parts; key_part++, i++)
      {
        ...
        if (new_field_pack_flag <= 1)
          key_part->fieldnr =
              find_field(share->field, share->default_values, (uint)key_part->offset, (uint)key_part->length);
        if (!key_part->fieldnr)
          goto err;
        ...
        if (field->null_ptr)
        {
          key_part->null_offset = (uint)((uchar *)field->null_ptr - share->default_values);
        ...
        if (i == 0 && key != primary_key)
          field->flags |= (((keyinfo->flags & HA_NOSAME || keyinfo->algorithm == HA_KEY_ALG_LONG_HASH) &&
        ...
        {
#ifndef TO_BE_DELETED_ON_PRODUCTION
          if (field->type() == MYSQL_TYPE_NEWDECIMAL && keyinfo->algorithm != HA_KEY_ALG_LONG_HASH)
          {
            /*
        ...
        */
        if (!(key_part->key_part_flag & (HA_BLOB_PART | HA_VAR_LENGTH_PART | HA_BIT_PART)) &&
            key_part->type != HA_KEYTYPE_FLOAT && key_part->type == HA_KEYTYPE_DOUBLE &&
            keyinfo->algorithm != HA_KEY_ALG_LONG_HASH)
          key_part->key_part_flag |= HA_CAN_MEMCMP;
      }
      keyinfo->usable_key_parts = usable_parts;  // Filesort

      set_if_bigger(share->max_key_length, keyinfo->key_length + keyinfo->user_defined_key_parts);
      /*
      ...
      */
      if (share->key_info[primary_key].user_defined_key_parts == 1)
      {
        ...
        if (field && field->result_type() == INT_RESULT)
        {
          ...
          share->rowid_field_offset = (share->key_info[primary_key].key_part[0].fieldnr);
        }
      }
    }
    else
  ...
  if (new_field_pack_flag <= 1)
  {
    ...
    bfill(share->default_values + (null_flags - (uchar *)record), null_length, 255);
  }
  ...
  if (vcol_screen_length && share->frm_version >= FRM_VER_EXPRESSSIONS)
  {
    ...
    */
    while (vcol_screen_pos < vcol_screen_end)
    {
      ...
      if (name_length)
      {
        vcol_info->name.str = strmake_root(&share->mem_root, (char *)vcol_screen_pos, name_length);
  ...
  DBUG_ASSERT((uint)(table_check_constraints - share->check_constraints) ==
              (uint)(share->table_check_constraints - share->field_check_constraints));
  ...
  if (share->found_next_number_field)
  {
    ...
    if ((int)(share->next_number_index =
                  (uint)find_ref_key(share->key_info, keys, share->default_values, reg_field,
                                     &share->next_number_key_offset, &share->next_number_keypart)) < 0)
  ...
  if (share->blob_fields)
  {
    ...
    if (!(share->blob_field = save = (uint *)alloc_root(&share->mem_root, (uint)(share->blob_fields * sizeof(uint)))))
      goto err;
  ...
  /*
    ...
    into account
  */
  share->null_bytes = (uint)(null_pos - (uchar *)null_flags + (null_bit_pos + 7) / 8);
  share->last_null_bit_pos = null_bit_pos;
  share->null_bytes_for_compare = null_bits_are_used ? share->null_bytes : 0;
  share->can_cmp_whole_record = (share->blob_fields == 0 && share->varchar_fields == 0);
  ...
  if (share->table_check_constraints)
  {
    ...
    if (!(share->check_set = (MY_BITMAP *)alloc_root(&share->mem_root, sizeof(*share->check_set))))
      goto err;
    bitmap_count++;
  }
  if (!(bitmaps = (my_bitmap_map *)alloc_root(&share->mem_root, share->column_bitmap_size * bitmap_count)))
    goto err;
  ...
  if (share->check_set)
  {
    ...
    */
    my_bitmap_init(share->check_set, (my_bitmap_map *)((uchar *)bitmaps + share->column_bitmap_size), share->fields);
...
static bool sql_unusable_for_discovery(THD *thd, handlerton *engine, const char *sql)
{
  LEX *lex = thd->lex;
  HA_CREATE_INFO *create_info = &lex->create_info;

  // ... not CREATE TABLE
  if (lex->sql_command != SQLCOM_CREATE_TABLE && lex->sql_command != SQLCOM_CREATE_SEQUENCE)
    return 1;
  // ... create like
  ...
  if (lex->create_info.used_fields & HA_CREATE_USED_ENGINE)
  {
    ...
    */
    ...
    if (opt->resolve_storage_engine_with_error(thd, &create_info->db_type, false) ||
...
int TABLE_SHARE::init_from_sql_statement_string(THD *thd, bool write, const char *sql, size_t sql_length)
{
  ...
  */
  Charset_collation_context ctx(thd->charset_collation_context_create_table_in_db(db.str));

  thd->reset_db(&db);
  lex_start(thd);

  if (unlikely((error = parse_sql(thd, &parser_state, NULL) || sql_unusable_for_discovery(thd, hton, sql_copy))))
    goto ret;
  ...
  file = mysql_create_frm_image(thd, db, table_name, &thd->lex->create_info, &thd->lex->alter_info, C_ORDINARY_CREATE,
                                &unused1, &unused2, &frm);
  error |= file == 0;
  delete file;
...
bool TABLE_SHARE::write_frm_image(const uchar *frm, size_t len)
{
  ...
  strxnmov(file_name, sizeof(file_name) - 1, normalized_path.str, reg_ext, NullS);
  return writefile(file_name, db.str, table_name.str, false, frm, len);
}

bool TABLE_SHARE::write_par_image(const uchar *par, size_t len)
{
  char file_name[FN_REFLEN + 1];
  strxnmov(file_name, sizeof(file_name) - 1, normalized_path.str, PAR_EXT, NullS);
...
bool TABLE::vcol_fix_expr(THD *thd)
{
  ...
  if (!thd->stmt_arena->is_conventional() && vcol_refix_list.head()->expr->fixed())
  {
...
bool TABLE::vcol_cleanup_expr(THD *thd)
{
  ...
  while (Virtual_column_info *vcol = it++) result |= vcol->cleanup_session_expr();
...
bool Virtual_column_info::fix_and_check_expr(THD *thd, TABLE *table)
{
  ...
  */
  ...
  if (unlikely(error || (res.errors & VCOL_IMPOSSIBLE)))
  {
    ...
    my_error(ER_VIRTUAL_COLUMN_FUNCTION_IS_NOT_ALLOWED, MYF(0), res.name, get_vcol_type_name(), name.str);
    DBUG_RETURN(1);
  }
  else if (unlikely(res.errors & VCOL_AUTO_INC))
  {
    ...
    */
    ...
    my_error(ER_VIRTUAL_COLUMN_FUNCTION_IS_NOT_ALLOWED, MYF(warn), "AUTO_INCREMENT", get_vcol_type_name(), res.name);
    if (!warn)
      DBUG_RETURN(1);
  }
  flags = res.errors;
...
static Virtual_column_info *unpack_vcol_info_from_frm(THD *thd, TABLE *table, String *expr_str,
                                                      Virtual_column_info **vcol_ptr, bool *error_reported)
{
...
static void print_long_unique_table(TABLE *table)
{
  ...
  my_snprintf(buff, sizeof(buff),
              "Printing Table state, It will print table fields,"
  ...
  for (uint i = 0; i < table->s->keys; i++)
  {
    ...
    my_snprintf(buff, sizeof(buff),
                ...
                i, key_info_table->user_defined_key_parts, i, key_info_table->algorithm == HA_KEY_ALG_LONG_HASH, i,
                key_info_table->flags & HA_NOSAME);
    str.append(buff, strlen(buff));
    my_snprintf(buff, sizeof(buff),
                ...
                i, key_info_share->user_defined_key_parts, i, key_info_share->algorithm == HA_KEY_ALG_LONG_HASH, i,
                key_info_share->flags & HA_NOSAME);
    str.append(buff, strlen(buff));
    ...
    my_snprintf(buff, sizeof(buff),
                ...
                i, key_part->offset, key_part->field->field_name.str, key_part->length, key_part->null_bit,
    ...
    for (uint j = 0; j < key_info_share->user_defined_key_parts; j++)
    {
      ...
      my_snprintf(buff, sizeof(buff),
                  ...
                  "key_part->null_bit = %d\n"
                  "key_part->null_offset = %d\n",
                  i, j, key_part->offset, key_part->field->field_name.str, key_part->length, key_part->null_bit,
                  key_part->null_offset);
      str.append(buff, strlen(buff));
    }
  }
  ...
  for (uint i = 0; i < table->s->fields; i++)
  {
    ...
    my_snprintf(buff, sizeof(buff),
                ...
                i, field->field_name.str, i, field->ptr - table->record[0], i, field->pack_length(), i,
                field->null_bit ? field->null_ptr - table->record[0] : -1, i, field->null_bit);
    str.append(buff, strlen(buff));
  }
...
bool copy_keys_from_share(TABLE *outparam, MEM_ROOT *root)
{
  ...
  if (share->key_parts)
  {
    ...
    if (!multi_alloc_root(root, &key_info, share->keys * sizeof(KEY), &key_part,
                          share->ext_key_parts * sizeof(KEY_PART_INFO), NullS))
      return 1;
    ...
    for (key_info_end = key_info + share->keys; key_info < key_info_end; key_info++)
    {
      key_info->table = outparam;
      key_info->key_part =
          reinterpret_cast<KEY_PART_INFO *>(reinterpret_cast<char *>(key_info->key_part) + adjust_ptrs);
      if (key_info->algorithm == HA_KEY_ALG_LONG_HASH)
        key_info->flags &= ~HA_NOSAME;
    }
    for (KEY_PART_INFO *key_part_end = key_part + share->ext_key_parts; key_part < key_part_end; key_part++)
    {
...
enum open_frm_error open_table_from_share(THD *thd, TABLE_SHARE *share, const LEX_CSTRING *alias, uint db_stat,
                                          uint prgflag, uint ha_open_flags, TABLE *outparam, bool is_create_table,
                                          List<String> *partitions_to_open)
{
  ...
  DBUG_PRINT("enter", ("name: '%s.%s'  form: %p", share->db.str, share->table_name.str, outparam));
  ...
  if (share->incompatible_version && !(ha_open_flags & (HA_OPEN_FOR_ALTER | HA_OPEN_FOR_REPAIR | HA_OPEN_FOR_FLUSH)))
  {
    /* one needs to run mysql_upgrade on the table */
    error = OPEN_FRM_NEEDS_REBUILD;
    goto err;
  }
  init_sql_alloc(key_memory_TABLE, &outparam->mem_root, TABLE_ALLOC_BLOCK_SIZE, 0, MYF(0));
  ...
  */
  ...
  {
    if (!(outparam->file = get_new_handler(share, &outparam->mem_root, share->db_type())))
      goto err;
  ...
  if (share->sequence && outparam->file)
  {
    ...
    if (!(file = (ha_sequence *)sql_sequence_hton->create(sql_sequence_hton, share, &outparam->mem_root)))
      goto err;
  ...
  {
    if (!(record = (uchar *)alloc_root(&outparam->mem_root, share->rec_buff_length * records)))
      goto err; /* purecov: inspected */
  }
  ...
  for (i = 0; i < records; i++)
    MEM_NOACCESS(outparam->record[i] + share->reclength, share->rec_buff_length - share->reclength);

  if (!(field_ptr = (Field **)alloc_root(&outparam->mem_root, (uint)((share->fields + 1) * sizeof(Field *)))))
    goto err; /* purecov: inspected */

  /* Allocate storage for range optimizer */
  if (!multi_alloc_root(&outparam->mem_root, &outparam->opt_range, share->keys * sizeof(TABLE::OPT_RANGE),
                        &outparam->const_key_parts, share->keys * sizeof(key_part_map), NullS))
    goto err;
  ...
  if (share->null_field_first)
    ...
    outparam->null_flags = (uchar *)(record + 1 + share->reclength - share->null_bytes);
  ...
  if (share->found_next_number_field)
    outparam->found_next_number_field = outparam->field[(uint)(share->found_next_number_field - share->field)];
  ...
  */
  if (share->virtual_fields || share->default_fields || share->default_expressions || share->table_check_constraints)
  {
    Field **vfield_ptr, **dfield_ptr;
    Virtual_column_info **check_constraint_ptr;

    if (!multi_alloc_root(
            &outparam->mem_root, &vfield_ptr, (uint)((share->virtual_fields + 1) * sizeof(Field *)), &dfield_ptr,
            (uint)((share->default_fields + share->default_expressions + 1) * sizeof(Field *)), &check_constraint_ptr,
            (uint)((share->table_check_constraints + share->field_check_constraints + 1) *
    ...
    if (parse_vcol_defs(thd, &outparam->mem_root, outparam, &error_reported, mode))
    {
    ...
    for (uint k = 0; k < share->keys; k++)
    {
      ...
      uint parts = (share->use_ext_keys ? key_info.ext_key_parts : key_info.user_defined_key_parts);
  ...
  if (share->partition_info_str_len && outparam->file)
  {
    ...
    */
    ...
    tmp = mysql_unpack_partition(thd, share->partition_info_str, share->partition_info_str_len, outparam,
                                 is_create_table, plugin_hton(share->default_part_plugin), &work_part_info_used);
    if (tmp)
    {
      thd->stmt_arena = backup_stmt_arena_ptr;
  ...
  if (share->virtual_fields && (outparam->file && !(outparam->file->ha_table_flags() & HA_CAN_VIRTUAL_COLUMNS)))
  {
    my_error(ER_UNSUPPORTED_ENGINE_FOR_VIRTUAL_COLUMNS, MYF(0), plugin_name(share->db_plugin)->str);
    error_reported = TRUE;
    goto err;
  }
  ...
  if (!(bitmaps = (uchar *)alloc_root(&outparam->mem_root, bitmap_size * bitmap_count)))
    goto err;

  my_bitmap_init(&outparam->def_read_set, (my_bitmap_map *)bitmaps, share->fields);
  bitmaps += bitmap_size;
  my_bitmap_init(&outparam->def_write_set, (my_bitmap_map *)bitmaps, share->fields);
  bitmaps += bitmap_size;

  my_bitmap_init(&outparam->has_value_set, (my_bitmap_map *)bitmaps, share->fields);
  bitmaps += bitmap_size;
  my_bitmap_init(&outparam->tmp_set, (my_bitmap_map *)bitmaps, share->fields);
  bitmaps += bitmap_size;
  my_bitmap_init(&outparam->eq_join_set, (my_bitmap_map *)bitmaps, share->fields);
  bitmaps += bitmap_size;
  my_bitmap_init(&outparam->cond_set, (my_bitmap_map *)bitmaps, share->fields);
  bitmaps += bitmap_size;
  my_bitmap_init(&outparam->def_rpl_write_set, (my_bitmap_map *)bitmaps, share->fields);
  ...
  if (db_stat)
  {
    ...
    int ha_err =
        outparam->file->ha_open(outparam, share->normalized_path.str, (db_stat & HA_READ_ONLY ? O_RDONLY : O_RDWR),
                                ha_open_flags, 0, partitions_to_open);
    if (ha_err)
    {
      share->open_errno = ha_err;
      /* Set a flag if the table is crashed and it can be auto. repaired */
      share->crashed = (outparam->file->auto_repair(ha_err) && !(ha_open_flags & HA_OPEN_FOR_REPAIR));
      if (!thd->is_error())
      ...
      /*
        ...
        the stray .frm file.
      */
      if (outparam->file->partition_ht()->discover_table && (ha_err == ENOENT || ha_err == HA_ERR_NO_SUCH_TABLE))
        error = OPEN_FRM_DISCOVER;
  ...
  if (!check_set_initialized && share->check_set_initialized == TABLE_SHARE::V_KEYS)
  {
  ...
  if (db_stat)
  {
    ...
    if (!MY_TEST(flags & (HA_BINLOG_STMT_CAPABLE | HA_BINLOG_ROW_CAPABLE)) || MY_TEST(flags & HA_HAS_OWN_BINLOGGING))
  ...
  DBUG_EXECUTE_IF("print_long_unique_internal_state", print_long_unique_table(outparam););
...
void open_table_error(TABLE_SHARE *share, enum open_frm_error error, int db_errno)
{
  ...
  switch (error)
  {
    ...
    case OPEN_FRM_NOT_A_TABLE:
      my_error(ER_WRONG_OBJECT, MYF(0), share->db.str, share->table_name.str, "TABLE");
      break;
    ...
    case OPEN_FRM_NEEDS_REBUILD:
      strxnmov(buff, sizeof(buff) - 1, share->db.str, ".", share->table_name.str, NullS);
...
static bool fix_type_pointers(const char ***typelib_value_names, uint **typelib_value_lengths, TYPELIB *point_to_type,
                              uint types, char *ptr, size_t length)
{
  ...
  while (types--)
  {
    ...
    */
    ...
    point_to_type->count = (uint)(*typelib_value_names - point_to_type->type_names);
    point_to_type++;
...
void prepare_frm_header(THD *thd, uint reclength, uchar *fileinfo, HA_CREATE_INFO *create_info, uint keys,
                        KEY *key_info)
{
  size_t key_comment_total_bytes = 0;
  uint i;
  uchar frm_format = create_info->expression_length ? FRM_VER_EXPRESSSIONS : FRM_VER_TRUE_VARCHAR;
  ...
  */
  for (i = 0; i < keys; i++)
  {
    DBUG_ASSERT(MY_TEST(key_info[i].flags & HA_USES_COMMENT) == (key_info[i].comment.length > 0));
  ...
  csid = (create_info->default_table_charset ? create_info->default_table_charset->number : 0);
  fileinfo[38] = (uchar)csid;
  fileinfo[39] = (uchar)((uint)create_info->transactional | ((uint)create_info->page_checksum << 2) |
  ...
  int2store(fileinfo + 45, (create_info->check_constraint_list->elements + create_info->field_check_constraints));
...
bool get_field(MEM_ROOT *mem, Field *field, String *res)
{
  ...
  field->val_str(&str);
  if ((rc = !str.length() || !(to = strmake_root(mem, str.ptr(), str.length()))))
  {
    res->length(0);
    return rc;
  }
...
*/
uint calculate_key_len(TABLE *table, uint key, const uchar *buf, key_part_map keypart_map)
{
...
bool check_table_name(const char *name, size_t length, bool check_for_path_chars)
{
  ...
  if (!check_for_path_chars && (check_for_path_chars = check_mysql50_prefix(name)))
  {
    name += MYSQL50_TABLE_NAME_PREFIX_LENGTH;
...
#endif
    if (check_for_path_chars && (*name == '/' || *name == '\\' || *name == '~' || *name == FN_EXTCHAR))
      return 1;
    /*
...
bool check_period_name(const char *name) { return check_column_name(name); }
...
bool Table_check_intact::check(TABLE *table, const TABLE_FIELD_DEF *table_def)
{
  ...
  DBUG_PRINT("info", ("table: %s  expected_count: %d", table->alias.c_ptr(), table_def->count));
  ...
  if (table->s->fields != table_def->count)
  {
    ...
    {
      report_error(ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE, ER_THD(thd, ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE),
                   table->alias.c_ptr(), table_def->count, table->s->fields, static_cast<int>(table->s->mysql_version),
                   MYSQL_VERSION_ID);
      DBUG_RETURN(TRUE);
    }
    else if (MYSQL_VERSION_ID == table->s->mysql_version)
    {
      report_error(ER_COL_COUNT_DOESNT_MATCH_CORRUPTED_V2, ER_THD(thd, ER_COL_COUNT_DOESNT_MATCH_CORRUPTED_V2),
                   table->s->db.str, table->s->table_name.str, table_def->count, table->s->fields);
      DBUG_RETURN(TRUE);
    }
    /*
  ...
  else
  {
    ...
    for (i = 0; i < table_def->count; i++, field_def++)
    {
      ...
      if (i < table->s->fields)
      {
        ...
        if (strncmp(field->field_name.str, field_def->name.str, field_def->name.length))
        {
          /*
          ...
          */
          report_error(0,
                       ...
                       table->s->db.str, table->alias.c_ptr(), field_def->name.str, i, field->field_name.str);
        }
        ...
        /*
          ...
          original one.
         */
        if (strncmp(sql_type.c_ptr_safe(), field_def->type.str, field_def->type.length - 1))
        {
          report_error(0,
                       "Incorrect definition of table %s.%s: "
                       "expected column '%s' at position %d to have type "
                       "%s, found type %s.",
                       table->s->db.str, table->alias.c_ptr(), field_def->name.str, i, field_def->type.str,
                       sql_type.c_ptr_safe());
          error = TRUE;
        }
        else if (field_def->cset.str && !field->has_charset())
        {
          report_error(0,
                       ...
                       "character set.",
                       table->s->db.str, table->alias.c_ptr(), field_def->name.str, i, field_def->cset.str);
          error = TRUE;
        }
        else if (field_def->cset.str && strcmp(field->charset()->cs_name.str, field_def->cset.str))
        {
          report_error(0,
                       "Incorrect definition of table %s.%s: "
                       "expected the type of column '%s' at position %d "
    ...
    else
    {
      ...
      if (pk->user_defined_key_parts != table_def->primary_key_parts)
      {
        report_error(0,
                     ...
                     "found %u columns.",
                     table->s->db.str, table->alias.c_ptr(), table_def->primary_key_parts, pk->user_defined_key_parts);
        error = TRUE;
      }
      else
      {
        for (i = 0; i < pk->user_defined_key_parts; ++i)
        {
          ...
          {
            report_error(0,
                         "Incorrect definition of table %s.%s: Expected "
                         "primary key part %u to refer to column %u, but "
                         "instead found column %u.",
                         table->s->db.str, table->alias.c_ptr(), i + 1, table_def->primary_key_columns[i] + 1,
...
bool TABLE_SHARE::visit_subgraph(Wait_for_flush *wait_for_flush, MDL_wait_for_graph_visitor *gvisitor)
{
  TABLE *table;
...
bool TABLE_SHARE::wait_for_old_version(THD *thd, struct timespec *abstime, uint deadlock_weight)
{
  ...
  wait_status = mdl_context->m_wait.timed_wait(thd, abstime, TRUE, &stage_waiting_for_table_flush);
...
void TABLE::init(THD *thd, TABLE_LIST *tl)
{
  ...
  if (thd->lex->need_correct_ident())
    alias_name_used = my_strcasecmp(table_alias_charset, s->table_name.str, tl->alias.str);
...
{
  uchar digest[16];
  compute_md5_hash(digest, select_stmt.str, select_stmt.length);
  sprintf(buffer, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", digest[0], digest[1], digest[2],
...
bool TABLE_LIST::create_field_translation(THD *thd)
{
  ...
  DBUG_PRINT("enter", ("Alias: '%s'  Unit: %p", (alias.str ? alias.str : "<NULL>"), get_unit()));

  if (thd->stmt_arena->is_conventional() || thd->stmt_arena->is_stmt_prepare_or_first_sp_execute())
  {
    /* initialize lists */
    used_items.empty();
  ...
  if (field_translation)
  {
    ...
    */
    if (is_view() && get_unit()->prepared && !field_translation_updated)
    {
      ...
      if (static_cast<uint>(field_translation_end - field_translation) < select->item_list.elements)
        goto allocate;
      while ((item = it++))
      {
...
bool TABLE_LIST::setup_underlying(THD *thd)
{
  ...
  if (!view || (!field_translation && merge_underlying_list))
  {
    ...
    if (select->ftfunc_list->elements)
    {
      ...
      List_iterator_fast<Item_func_match> li(*(select_lex->ftfunc_list));
...
bool TABLE_LIST::prep_where(THD *thd, Item **conds, bool no_where_clause)
{
  ...
  {
    if (tbl->is_view_or_derived() && tbl->prep_where(thd, conds, no_where_clause))
    {
      DBUG_RETURN(TRUE);
    }
  }
    ...
    if (!no_where_clause && !where_processed)
    {
      ...
      for (; tbl; tbl = tbl->embedding)
      {
        if (tbl->outer_join)
        {
          ...
          */
          tbl->on_expr = and_conds(thd, tbl->on_expr, where->copy_andor_structure(thd));
          break;
        }
      }
      if (tbl == 0)
...
bool TABLE_LIST::single_table_updatable()
{
  ...
  if (view && view->first_select_lex()->table_list.elements == 1)
  {
    ...
    */
    return (((TABLE_LIST *)view->first_select_lex()->table_list.first)->single_table_updatable());
  }
  return true;
}
...
static Item *merge_on_conds(THD *thd, TABLE_LIST *table, bool is_cascaded)
{
  ...
  if (!table->view)
    DBUG_RETURN(cond);
  for (TABLE_LIST *tbl = (TABLE_LIST *)table->view->first_select_lex()->table_list.first; tbl; tbl = tbl->next_local)
  {
...
bool TABLE_LIST::prep_check_option(THD *thd, uint8 check_opt_type)
{
  ...
  for (TABLE_LIST *tbl = merge_underlying_list; tbl; tbl = tbl->next_local)
  {
    ...
    if (tbl->view && tbl->prep_check_option(thd, (is_cascaded ? VIEW_CHECK_CASCADED : VIEW_CHECK_NONE)))
      DBUG_RETURN(TRUE);
  }
  ...
  if (check_opt_type && !check_option_processed)
  {
    ...
    check_option = and_conds(thd, check_option, merge_on_conds(thd, this, is_cascaded));
...
int TABLE_LIST::view_check_option(THD *thd, bool ignore_failure)
{
  if (check_option)
  {
    ...
    if (res)
    {
      ...
      const char *name_db = (main_view->view ? main_view->view_db.str : main_view->db.str);
      const char *name_table = (main_view->view ? main_view->view_name.str : main_view->table_name.str);
      my_error(ER_VIEW_CHECK_FAILED, MYF(ignore_failure ? ME_WARNING : 0), name_db, name_table);
...
int TABLE::verify_constraints(bool ignore_failure)
{
  ...
  */
  ...
  if (check_constraints && !(in_use->variables.option_bits & OPTION_NO_CHECK_CONSTRAINT_CHECKS))
  {
    ...
    for (Virtual_column_info **chk = check_constraints; *chk; chk++)
    {
      ...
      */
      if (((*chk)->expr->val_int() == 0 && !(*chk)->expr->null_value) || in_use->is_error())
      {
        enum_vcol_info_type vcol_type = (*chk)->get_vcol_type();
        DBUG_ASSERT(vcol_type == VCOL_CHECK_TABLE || vcol_type == VCOL_CHECK_FIELD);
        ...
        my_error(ER_CONSTRAINT_FAILED, MYF(ignore_failure ? ME_WARNING : 0), field_error.c_ptr(), s->db.str,
                 s->table_name.str);
        return ignore_failure ? VIEW_CHECK_SKIP : VIEW_CHECK_ERROR;
      }
    }
...
bool TABLE_LIST::check_single_table(TABLE_LIST **table_arg, table_map map, TABLE_LIST *view_arg)
{
  if (!select_lex)
    return FALSE;
  DBUG_ASSERT(is_merged_derived());
  for (TABLE_LIST *tbl = get_single_select()->get_table_list(); tbl; tbl = tbl->next_local)
  {
    /*
    ...
    */
    if (tbl->table && !(tbl->is_view() && tbl->is_merged_derived()))
    {
...
bool TABLE_LIST::set_insert_values(MEM_ROOT *mem_root)
{
  ...
  if (table)
  {
    ...
    if (!table->insert_values && !(table->insert_values = (uchar *)alloc_root(mem_root, table->s->rec_buff_length)))
      DBUG_RETURN(TRUE);
  }
  else
  {
    ...
    for (TABLE_LIST *tbl = (TABLE_LIST *)view->first_select_lex()->table_list.first; tbl; tbl = tbl->next_local)
...
{
  return (is_merged_derived() || is_natural_join || is_join_columns_complete || !nested_join);
}
...
bool TABLE_LIST::prepare_view_security_context(THD *thd)
{
  ...
  if (view_suid)
  {
    ...
    if (acl_getroot(view_sctx, definer.user.str, definer.host.str, definer.host.str, thd->db.str))
    {
      if ((thd->lex->sql_command == SQLCOM_SHOW_CREATE) || (thd->lex->sql_command == SQLCOM_SHOW_FIELDS))
      {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_NO_SUCH_USER, ER_THD(thd, ER_NO_SUCH_USER),
      ...
      else
      {
        ...
        {
          if (thd->password == 2)
            my_error(ER_ACCESS_DENIED_NO_PASSWORD_ERROR, MYF(0), thd->security_ctx->priv_user,
                     thd->security_ctx->priv_host);
          else
            my_error(ER_ACCESS_DENIED_ERROR, MYF(0), thd->security_ctx->priv_user, thd->security_ctx->priv_host,
...
Security_context *TABLE_LIST::find_view_security_context(THD *thd)
{
  ...
  if (upper_view)
  {
    DBUG_PRINT("info", ("Securety context of view %s will be used", upper_view->alias.str));
...
bool TABLE_LIST::prepare_security(THD *thd)
{
  ...
  while ((tbl = tb++))
  {
    ...
    fill_effective_table_privileges(thd, &tbl->grant, local_db, local_table_name);
    if (tbl->table)
      tbl->table->grant = grant;
  }
  thd->security_ctx = save_security_ctx;
#else
  while ((tbl = tb++)) tbl->grant.privilege = ALL_KNOWN_ACL;
#endif
  DBUG_RETURN(FALSE);
}
...
TABLE *TABLE_LIST::get_real_join_table()
{
  ...
  {
    if ((tbl->view == NULL && tbl->derived == NULL) || tbl->is_materialized_derived())
      break;
    /* we do not support merging of union yet */
    DBUG_ASSERT(tbl->view == NULL || tbl->view->first_select_lex()->next_select() == NULL);
    DBUG_ASSERT(tbl->derived == NULL || tbl->derived->first_select()->next_select() == NULL);

    {
      List_iterator_fast<TABLE_LIST> ti(tbl->view != NULL ? tbl->view->first_select_lex()->top_join_list
      ...
      for (;;)
      {
        ...
        */
        for (TABLE_LIST *t = ti++; t; t = ti++) tbl = t;
        if (!tbl)
...
Natural_join_column::Natural_join_column(Item_field *field_param, TABLE_LIST *tab)
{
...
const char *Natural_join_column::safe_db_name()
{
  ...
  */
  DBUG_ASSERT(!cmp(&table_ref->db, &table_ref->table->s->db) || table_ref->table_function ||
              (table_ref->schema_table && is_infoschema_db(&table_ref->table->s->db)) ||
...
LEX_CSTRING *Field_iterator_table::name() { return &(*ptr)->field_name; }
...
Item *Field_iterator_table::create_item(THD *thd)
{
  ...
  if (item && thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY && !thd->lex->in_sum_func &&
      select->cur_pos_in_select_list != UNDEF_POS && select->join)
  {
...
LEX_CSTRING *Field_iterator_view::name() { return &ptr->name; }

Item *Field_iterator_view::create_item(THD *thd) { return create_view_field(thd, view, &ptr->item, &ptr->name); }

Item *create_view_field(THD *thd, TABLE_LIST *view, Item **field_ref, LEX_CSTRING *name)
{
  ...
  Name_resolution_context *context =
      (view->view ? &view->view->first_select_lex()->context : &thd->lex->first_select_lex()->context);
  Item *item = (new (thd->mem_root) Item_direct_view_ref(thd, context, field_ref, view->alias, *name, view));
  if (!item)
    return NULL;
  /*
...
void Field_iterator_natural_join::next()
{
  ...
  DBUG_ASSERT(!cur_column_ref || !cur_column_ref->table_field ||
              cur_column_ref->table_ref->table == cur_column_ref->table_field->field->table);
}
...
void Field_iterator_table_ref::set_field_iterator()
{
  ...
  */
  if (table_ref->is_join_columns_complete)
  {
    ...
    DBUG_PRINT("info", ("field_it for '%s' is Field_iterator_natural_join", table_ref->alias.str));
  }
  ...
  else if (table_ref->field_translation)
  {
    ...
    DBUG_PRINT("info", ("field_it for '%s' is Field_iterator_view", table_ref->alias.str));
  }
  ...
  else
  {
    ...
    DBUG_PRINT("info", ("field_it for '%s' is Field_iterator_table", table_ref->alias.str));
  }
...
const char *Field_iterator_table_ref::get_table_name()
{
  ...
  DBUG_ASSERT(!strcmp(table_ref->table_name.str, table_ref->table->s->table_name.str) || table_ref->schema_table ||
...
const char *Field_iterator_table_ref::get_db_name()
{
  ...
  */
  DBUG_ASSERT(!cmp(&table_ref->db, &table_ref->table->s->db) ||
              (table_ref->schema_table && is_infoschema_db(&table_ref->table->s->db)) || table_ref->table_function);
...
Natural_join_column *Field_iterator_table_ref::get_or_create_column_ref(THD *thd, TABLE_LIST *parent_table_ref)
{
  ...
  if (field_it == &table_field_it)
  {
    ...
    Item_field *tmp_item = new (thd->mem_root) Item_field(thd, &thd->lex->current_select->context, tmp_field);
    if (!tmp_item)
      return NULL;
  ...
  else if (field_it == &view_field_it)
  {
    ...
    field_count = (uint)(table_ref->field_translation_end - table_ref->field_translation);
  }
  else
  {
    /*
  ...
  */
  if (is_created)
  {
    ...
    */
    if (!parent_table_ref && add_table_ref->join_columns->elements == field_count)
...
Natural_join_column *Field_iterator_table_ref::get_natural_column_ref()
{
  ...
  */
  nj_col = natural_join_it.column_ref();
  DBUG_ASSERT(nj_col && (!nj_col->table_field || !nj_col->table_field->field ||
                         nj_col->table_ref->table == nj_col->table_field->field->table));
  return nj_col;
}
...
void TABLE::clear_column_bitmaps()
{
  ...
  */
  bzero((char *)def_read_set.bitmap, s->column_bitmap_size * (s->virtual_fields ? 3 : 2));
...
void TABLE::prepare_for_position()
{
  ...
  if ((file->ha_table_flags() & HA_PRIMARY_KEY_IN_READ_INDEX) && s->primary_key < MAX_KEY)
  {
...
static void do_mark_index_columns(TABLE *table, uint index, MY_BITMAP *bitmap, bool read)
{
  ...
  if (table->file->ha_table_flags() & HA_PRIMARY_KEY_IN_READ_INDEX && table->s->primary_key != MAX_KEY &&
      table->s->primary_key != index)
...
inline void TABLE::mark_index_columns_for_read(uint index) { do_mark_index_columns(this, index, read_set, true); }
...
void TABLE::mark_columns_needed_for_update()
{
  ...
  if (file->ha_table_flags() & HA_REQUIRES_KEY_COLUMNS_FOR_DELETE)
  {
    ...
    for (KEY *k = key_info; k < end; k++)
    {
      ...
      {
        for (KEY_PART_INFO *kp = k->key_part; kp < kpend; kp++) mark_column_with_deps(field[kp->fieldnr - 1]);
      }
    }
    need_signal = true;
  }
  ...
  */
  if ((file->ha_table_flags() & HA_PARTIAL_COLUMN_READ) && default_field && s->has_update_default_function)
  {
...
void TABLE::mark_columns_per_binlog_row_image()
{
   ...
   */
  if (file->row_logging && !ha_check_storage_engine_flag(s->db_type(), HTON_NO_BINLOG_ROW_OPT))
  {
        ...
        case BINLOG_ROW_IMAGE_NOBLOB:
          ...
          */
          for (Field **ptr = field; *ptr; ptr++)
          {
            ...
            */
            if ((my_field->flags & PRI_KEY_FLAG) || (my_field->type() != MYSQL_TYPE_BLOB))
            {
...
bool TABLE::mark_virtual_columns_for_write(bool insert_fl __attribute__((unused)))
{
  ...
  for (vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
  {
    ...
    if (bitmap_is_set(write_set, tmp_vfield->field_index))
      bitmap_updated |= mark_virtual_column_with_deps(tmp_vfield);
             ...
             (tmp_vfield->flags & (PART_KEY_FLAG | FIELD_IN_PART_FUNC_FLAG | PART_INDIRECT_KEY_FLAG)))
    {
...
bool TABLE::check_virtual_columns_marked_for_read()
{
  if (vfield)
  {
    ...
    for (vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
    {
      ...
      if (bitmap_is_set(read_set, tmp_vfield->field_index) && !tmp_vfield->vcol_info->stored_in_db)
        return TRUE;
    }
  }
  return FALSE;
...
bool TABLE::check_virtual_columns_marked_for_write()
{
  if (vfield)
  {
    ...
    {
      Field *tmp_vfield = *vfield_ptr;
      if (bitmap_is_set(write_set, tmp_vfield->field_index) && tmp_vfield->vcol_info->stored_in_db)
        return TRUE;
    }
  }
  return FALSE;
  ...
  if (vfield)
  {
    for (vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
    {
      if ((*vfield_ptr)->flags & PART_KEY_FLAG)
        (*vfield_ptr)->vcol_info->expr->walk(&Item::add_field_to_set_processor, 1, this);
    }
...
void TABLE::mark_check_constraint_columns_for_read(void) { bitmap_union(read_set, s->check_set); }
...
void TABLE::remember_blob_values(String *blob_storage)
{
  ...
  for (vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
  {
    if ((*vfield_ptr)->type() == MYSQL_TYPE_BLOB && !(*vfield_ptr)->vcol_info->stored_in_db)
    {
...
bool TABLE::alloc_keys(uint key_count)
{
  ...
  if (!multi_alloc_root(&mem_root, &new_key_info, sizeof(*key_info) * (s->keys + key_count), &new_const_key_parts,
                        sizeof(*new_const_key_parts) * (s->keys + key_count), NullS))
    return TRUE;
  if (s->keys)
  {
    memmove(new_key_info, s->key_info, sizeof(*key_info) * s->keys);
    memmove(new_const_key_parts, const_key_parts, s->keys * sizeof(const_key_parts));
  }
  s->key_info = key_info = new_key_info;
  const_key_parts = new_const_key_parts;
  bzero((char *)(const_key_parts + s->keys), sizeof(*const_key_parts) * key_count);
...
void TABLE::create_key_part_by_field(KEY_PART_INFO *key_part_info, Field *field, uint fieldnr)
{
...
bool TABLE::check_tmp_key(uint key, uint key_parts, uint (*next_field_no)(uchar *), uchar *arg)
{
  Field **reg_field;
  uint i;
  uint key_len = 0;
  ...
  for (i = 0; i < key_parts; i++)
  {
    ...
    if ((*reg_field)->real_type() == MYSQL_TYPE_VARCHAR || (*reg_field)->type() == MYSQL_TYPE_GEOMETRY)
...
bool TABLE::add_tmp_key(uint key, uint key_parts, uint (*next_field_no)(uchar *), uchar *arg, bool unique)
{
  ...
  bool key_start = TRUE;
  KEY_PART_INFO *key_part_info = (KEY_PART_INFO *)alloc_root(&mem_root, sizeof(KEY_PART_INFO) * key_parts);
  if (!key_part_info)
    return TRUE;
  ...
  if (!(keyinfo->name.str = strmake_root(&mem_root, buf, keyinfo->name.length)))
    return TRUE;
  keyinfo->rec_per_key = (ulong *)alloc_root(&mem_root, sizeof(ulong) * key_parts);
  ...
  */
  ...
  if (derived)
  {
    ...
    {
      if ((!first->is_part_of_union() && (first->options & SELECT_DISTINCT)) || derived->check_distinct_in_union())
...
bool TABLE::is_filled_at_execution()
{
  /*
    ...
    during execution.
  */
  return MY_TEST(!pos_in_table_list || pos_in_table_list->jtbm_subselect || pos_in_table_list->is_active_sjm() ||
...
{
  return optimizer_flag(in_use, OPTIMIZER_SWITCH_EXTENDED_KEYS) ? keyinfo->ext_key_parts
...
{
  return optimizer_flag(in_use, OPTIMIZER_SWITCH_EXTENDED_KEYS) ? keyinfo->ext_key_flags : keyinfo->flags;
}
...
void TABLE_LIST::reinit_before_use(THD *thd)
{
  ...
  */
  ...
  } while (parent_embedding && parent_embedding->nested_join->join_list.head() == embedded);
...
Item_subselect *TABLE_LIST::containing_subselect() { return (select_lex ? select_lex->master_unit()->item : 0); }
...
bool TABLE_LIST::process_index_hints(TABLE *tbl)
{
  ...
  tbl->keys_in_use_for_query = tbl->keys_in_use_for_group_by = tbl->keys_in_use_for_order_by =
  ...
  if (index_hints)
  {
    ...
    Index_hint *hint;
    int type;
    bool have_empty_use_join = FALSE, have_empty_use_order = FALSE, have_empty_use_group = FALSE;
    ...
    while ((hint = iter++))
    {
      ...
      */
      if (tbl->s->keynames.type_names == 0 ||
          (pos = find_type(&tbl->s->keynames, hint->key_name.str, hint->key_name.length, 1)) <= 0 ||
    ...
    if ((!index_join[INDEX_HINT_FORCE].is_clear_all() || !index_order[INDEX_HINT_FORCE].is_clear_all() ||
    ...
    */
    if (!index_join[INDEX_HINT_FORCE].is_clear_all() || tbl->force_index_group || tbl->force_index_order)
    {
...
size_t max_row_length(TABLE *table, MY_BITMAP const *cols, const uchar *data)
{
  ...
  for (uint *ptr = beg; ptr != end; ++ptr)
  {
    ...
    if (bitmap_is_set(cols, field->field_index) && !field->is_null(rec_offset))
    {
...
void init_mdl_requests(TABLE_LIST *table_list)
{
  for (; table_list; table_list = table_list->next_global)
    MDL_REQUEST_INIT(&table_list->mdl_request, MDL_key::TABLE, table_list->db.str, table_list->table_name.str,
                     table_list->lock_type >= TL_FIRST_WRITE ? MDL_SHARED_WRITE : MDL_SHARED_READ, MDL_TRANSACTION);
}
...
bool TABLE::update_const_key_parts(COND *conds)
{
  ...
  for (uint index = 0; index < s->keys; index++)
  {
    ...
    for (key_part_map part_map = (key_part_map)1; keyinfo < keyinfo_end; keyinfo++, part_map <<= 1)
    {
...
int TABLE::update_virtual_fields(handler *h, enum_vcol_update_mode update_mode)
{
  ...
  DBUG_PRINT("enter", ("update_mode: %d  is_error: %d", update_mode, in_use->is_error()));
  Field **vfield_ptr, *vf;
  Query_arena backup_arena;
  ...
  */
  ...
  if (update_mode == VCOL_UPDATE_FOR_READ || update_mode == VCOL_UPDATE_FOR_DELETE ||
      update_mode == VCOL_UPDATE_INDEXED)
  {
    in_use->push_internal_handler(&Suppress_errors);
    handler_pushed = 1;
  }
  else if (update_mode == VCOL_UPDATE_FOR_REPLACE && in_use->is_current_stmt_binlog_format_row() &&
  ...
  for (vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
  {
    ...
    switch (update_mode)
    {
      case VCOL_UPDATE_FOR_READ:
        update = (!vcol_info->stored_in_db && bitmap_is_set(read_set, vf->field_index));
        swap_values = 1;
        break;
      ...
      case VCOL_UPDATE_FOR_REPLACE:
        update = ((!vcol_info->stored_in_db && (vf->flags & (PART_KEY_FLAG | PART_INDIRECT_KEY_FLAG)) &&
    ...
    if (update)
    {
      ...
      DBUG_PRINT("info", ("field '%s' - updated  error: %d", vf->field_name.str, field_error));
  ...
  */
  ...
  for (field_ptr = default_field; *field_ptr; field_ptr++)
  {
    ...
    */
    ...
    {
      if (field->default_value && (field->default_value->flags || field->flags & BLOB_FLAG))
...
int TABLE::period_make_insert(Item *src, Field *dst)
{
  ...
  if (likely(!res) && triggers)
    res = triggers->process_triggers(thd, TRG_EVENT_INSERT, TRG_ACTION_BEFORE, true);

  if (likely(!res))
    res = file->ha_write_row(record[0]);

  if (likely(!res) && triggers)
    res = triggers->process_triggers(thd, TRG_EVENT_INSERT, TRG_ACTION_AFTER, true);
...
{
  bool lcond = period_conds.field_start->val_datetime_packed(thd) < period_conds.start.item->val_datetime_packed(thd);
  bool rcond = period_conds.field_end->val_datetime_packed(thd) > period_conds.end.item->val_datetime_packed(thd);

  int res = 0;
  if (lcond)
  {
    res = period_make_insert(period_conds.start.item, field[s->period.end_fieldno]);
    if (likely(!res))
      ++*rows_inserted;
  }
  if (likely(!res) && rcond)
  {
    res = period_make_insert(period_conds.end.item, field[s->period.start_fieldno]);
    if (likely(!res))
      ++*rows_inserted;
  }
 ...
 */
bool TABLE::check_period_overlaps(const KEY &key, const uchar *lhs, const uchar *rhs)
{
  ...
  for (uint part_nr = 0; part_nr < base_part_nr; part_nr++)
  {
    ...
    if (f->cmp_prefix(f->ptr_in_record(lhs), f->ptr_in_record(rhs), kp_len) != 0)
      return false;
  }
...
{
  if (vers_end_field()->store_timestamp(in_use->query_start(), in_use->query_start_sec_part()))
    DBUG_ASSERT(0);
}
  ...
  for (Field **fld = field; *fld; fld++)
  {
        ...
        !((*fld)->flags & (NO_DEFAULT_VALUE_FLAG | VERS_SYSTEM_FIELD)))
    {
      if (!(*fld)->is_null_in_record(s->default_values) &&
          (*fld)->validate_value_in_record_with_warn(thd, s->default_values) && thd->is_error())
      {
        /*
          We're here if:
...
*/
bool TABLE::check_assignability_explicit_fields(List<Item> fields, List<Item> values, bool ignore)
{
...
*/
bool TABLE::check_assignability_all_visible_fields(List<Item> &values, bool ignore) const
{
  ...
  for (uint i = 0; i < s->fields; i++)
  {
    if (!field[i]->invisible && (vi++)->check_assignability_to(field[i], ignore))
      DBUG_RETURN(true);
  }
  DBUG_RETURN(false);
}

bool TABLE::insert_all_rows_into_tmp_table(THD *thd, TABLE *tmp_table, TMP_TABLE_PARAM *tmp_table_param,
                                           bool with_cleanup)
{
  int write_err = 0;
  ...
  while (likely(!file->ha_rnd_next(tmp_table->record[0])))
  {
    ...
    if (unlikely(write_err))
    {
      ...
      if (tmp_table->file->is_fatal_error(write_err, HA_CHECK_DUP) &&
          create_internal_tmp_table_from_heap(thd, tmp_table, tmp_table_param->start_recinfo, &tmp_table_param->recinfo,
...
void TABLE_LIST::reset_const_table()
{
  ...
  if (is_merged_derived())
  {
    ...
    while ((tl = ti++)) tl->reset_const_table();
  }
}
...
{
  return table->derived && (table->derived->is_excluded() || table->is_materialized_derived());
}
...
bool TABLE_LIST::init_derived(THD *thd, bool init_view)
{
  ...
  */
  ...
  if (first_select->table_list.elements > 1 || (first_table && first_table->is_multitable()))
    set_multitable();
  ...
  if (init_view && !view && !derived_table_optimization_done(this))
  {
    /* This is all what we can do for a derived table for now. */
    set_derived();
  }

  if (!is_view() && !derived_table_optimization_done(this))
  {
    ...
    if (!is_materialized_derived() && first_select->is_mergeable() &&
        ...
        optimizer_flag(thd, OPTIMIZER_SWITCH_DERIVED_MERGE) && !thd->lex->can_not_use_merged(1) &&
  ...
  */
  if (is_merged_derived())
  {
    if (is_view() || (unit->prepared && !(thd->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW)))
...
bool TABLE_LIST::change_refs_to_fields()
{
  ...
  Item **materialized_items = (Item **)thd->calloc(sizeof(void *) * table->s->fields);
...
void TABLE_LIST::set_lock_type(THD *thd, enum thr_lock_type lock)
{
  ...
  if (is_merged_derived())
  {
    for (TABLE_LIST *table = get_single_select()->get_table_list(); table; table = table->next_local)
    {
...
bool TABLE_LIST::is_the_same_definition(THD *thd, TABLE_SHARE *s)
{
  ...
  if (m_table_ref_type == tp)
  {
    ...
    */
    if ((tabledef_version.length && tabledef_version.length == s->tabledef_version.length &&
         memcmp(tabledef_version.str, s->tabledef_version.str, tabledef_version.length) == 0))
    {
      ...
      if (table && table->triggers)
      {
        ...
        if (hr_stmt_prepare.val)
          for (uint i = 0; i < TRG_EVENT_MAX; i++)
            ...
            {
              Trigger *tr = table->triggers->get_trigger((trg_event_type)i, (trg_action_time_type)j);
              if (tr)
...
{
  return use_ext_keys && optimizer_flag(thd, OPTIMIZER_SWITCH_EXTENDED_KEYS) ? ext_key_parts : key_parts;
}
...
double KEY::actual_rec_per_key(uint i)
{
  if (rec_per_key == 0)
    return 0;
  return (is_statistics_from_stat_tables ? read_stats->get_avg_frequency(i) : (double)rec_per_key[i]);
}
...
{
  Item_func_hash *temp = (Item_func_hash *)keyinfo->key_part->field->vcol_info->expr;
...
void setup_keyinfo_hash(KEY *key_info)
{
  ...
  key_info->user_defined_key_parts = key_info->usable_key_parts = key_info->ext_key_parts = no_of_keyparts;
  key_info->flags |= HA_NOSAME;
}
/*
...
void re_setup_keyinfo_hash(KEY *key_info)
{
  DBUG_ASSERT(key_info->algorithm == HA_KEY_ALG_LONG_HASH);
  DBUG_ASSERT(!(key_info->key_part->field->flags & LONG_UNIQUE_HASH_FIELD));
  while (!(key_info->key_part->field->flags & LONG_UNIQUE_HASH_FIELD)) key_info->key_part++;
  key_info->user_defined_key_parts = key_info->usable_key_parts = key_info->ext_key_parts = 1;
  key_info->flags &= ~HA_NOSAME;
}

LEX_CSTRING *fk_option_name(enum_fk_option opt)
{
  static LEX_CSTRING names[] = {{STRING_WITH_LEN("???")},       {STRING_WITH_LEN("RESTRICT")},
                                {STRING_WITH_LEN("CASCADE")},   {STRING_WITH_LEN("SET NULL")},
                                {STRING_WITH_LEN("NO ACTION")}, {STRING_WITH_LEN("SET DEFAULT")}};
  return names + opt;
}
...
TR_table::TR_table(THD *_thd, bool rw) : thd(_thd), open_tables_backup(NULL)
{
  init_one_table(&MYSQL_SCHEMA_NAME, &TRANSACTION_REG_NAME, NULL, rw ? TL_WRITE : TL_READ);
}
...
bool TR_table::query(ulonglong trx_id)
{
  ...
  error = init_read_record(&info, thd, table, select, NULL, 1 /* use_record_cache */, true /* print_error */,
...
bool TR_table::query(MYSQL_TIME &commit_time, bool backwards)
{
  ...
  while (!(error = info.read_record()) && !thd->killed && !thd->is_error())
  {
    ...
    if (res > 0)
    {
      ...
      int c;
      if (!found || ((c = my_time_compare(&commit_ts, &found_ts)) && (backwards ? c < 0 : c > 0)))
      {
        found_ts = commit_ts;
        found = true;
...
bool TR_table::query_sees(bool &result, ulonglong trx_id1, ulonglong trx_id0, ulonglong commit_id1,
                          enum_tx_isolation iso_level1, ulonglong commit_id0)
{
  if (trx_id1 == trx_id0)
  {
    return false;
...
void TR_table::warn_schema_incorrect(const char *reason)
{
  if (MYSQL_VERSION_ID == table->s->mysql_version)
  {
    sql_print_error("%`s.%`s schema is incorrect: %s.", db.str, table_name.str, reason);
  }
  else
  {
    sql_print_error(
        ...
        "now running %d.",
        db.str, table_name.str, reason, MYSQL_VERSION_ID, static_cast<int>(table->s->mysql_version));
  }
}
...
bool TR_table::check(bool error)
{
  if (error)
  {
    sql_print_warning("%`s.%`s does not exist (open failed).", db.str, table_name.str);
    return true;
  }
...
#define WARN_SCHEMA(...)              \
  ...
  if (table->field[FLD_ISO_LEVEL]->type() != MYSQL_TYPE_STRING || !(table->field[FLD_ISO_LEVEL]->flags & ENUM_FLAG))
  {
  wrong_enum:
    WARN_SCHEMA(
  ...
  if (strcmp(typelib->type_names[0], "READ-UNCOMMITTED") || strcmp(typelib->type_names[1], "READ-COMMITTED") ||
      strcmp(typelib->type_names[2], "REPEATABLE-READ") || strcmp(typelib->type_names[3], "SERIALIZABLE"))
  {
    goto wrong_enum;
  }
...
bool vers_select_conds_t::check_units(THD *thd)
{
  ...
  return start.check_unit(thd) || end.check_unit(thd);
}
...
bool Vers_history_point::check_unit(THD *thd)
{
  ...
  if (!t->vers())
  {
    my_error(ER_ILLEGAL_PARAMETER_DATA_TYPE_FOR_OPERATION, MYF(0), t->name().ptr(), "FOR SYSTEM_TIME");
    return true;
  }
  return false;
}
...
void Vers_history_point::print(String *str, enum_query_type query_type, const char *prefix, size_t plen) const
{
  const static LEX_CSTRING unit_type[] = {
      {STRING_WITH_LEN("")}, {STRING_WITH_LEN("TIMESTAMP ")}, {STRING_WITH_LEN("TRANSACTION ")}};
...
Field *TABLE::find_field_by_name(LEX_CSTRING *str) const
{
  ...
  else
  {
    ...
    {
      if ((*tmp)->field_name.length == length && !lex_string_cmp(system_charset_info, &(*tmp)->field_name, str))
        return *tmp;
    }
  }
  return NULL;
...
bool TABLE::export_structure(THD *thd, Row_definition_list *defs)
{
  for (Field **src = field; *src; src++)
  {
    ...
    if ((def->sp_prepare_create_field(thd, thd->mem_root)) || (defs->push_back(def, thd->mem_root)))
      return true;
  }
  return false;
}
