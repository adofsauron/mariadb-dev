    ...
    : str_(cmd ? strdup(cmd) : strdup("")), io_(NULL), err_(EINVAL), pid_(0)
{
  ...
  err_ = posix_spawnattr_setflags(&attr, POSIX_SPAWN_SETSIGDEF | POSIX_SPAWN_SETSIGMASK | POSIX_SPAWN_USEVFORK);
  if (err_)
  {
  ...
  err_ = posix_spawn_file_actions_adddup2(&fact, pipe_fds[child_end], close_fd);
  if (err_)
  {
    WSREP_ERROR("posix_spawn_file_actions_addup2() failed: %d (%s)", err_, strerror(err_));
    goto cleanup_fact;
  }
  ...
  if (err_)
  {
    WSREP_ERROR("posix_spawnp(%s) failed: %d (%s)", pargv[2], err_, strerror(err_));
...
cleanup_attr:
  ...
  if (err)
  {
    WSREP_ERROR("posix_spawnattr_destroy() failed: %d (%s)", err, strerror(err));
  }
...
int process::wait()
{
  if (pid_)
  {
    ...
    if (-1 == waitpid(pid_, &status, 0))
    {
      err_ = errno;
      assert(err_);
      WSREP_ERROR("Waiting for process failed: %s, PID(%ld): %d (%s)", str_, (long)pid_, err_, strerror(err_));
    }
    else
    {  // command completed, check exit status
      ...
      {  // command didn't complete with exit()
      ...
      if (err_)
      {
        ...
        WSREP_ERROR("Process completed with error: %s: %d (%s)", str_, err_, strerror(err_));
      }
...
unsigned int wsrep_check_ip(const char *const addr, bool *is_ipv6)
{
  ...
  *is_ipv6 = false;
  ...
  {
    WSREP_ERROR("getaddrinfo() failed on '%s': %d (%s)", addr, gai_ret, gai_strerror(gai_ret));
  }
...
size_t wsrep_guess_ip(char *buf, size_t buf_len)
{
  ...
  if (my_bind_addr_str && my_bind_addr_str[0] != '\0' && strcmp(my_bind_addr_str, "*") != 0)
  {
    bool unused;
  ...
  if (wsrep_node_address && wsrep_node_address[0] != '\0')
  {
    ...
    if (!addr.is_valid())
    {
      WSREP_WARN("Could not parse wsrep_node_address : %s", wsrep_node_address);
      ret = 0;
      goto done;
    }
