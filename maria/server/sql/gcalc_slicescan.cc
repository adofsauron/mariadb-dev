...
{
  return sprintf(str, "%s %d %d | %s %d %d%s", GCALC_SIGN(pi->node.shape.ix[0]) ? "-" : "",
                 FIRST_DIGIT(pi->node.shape.ix[0]), pi->node.shape.ix[1], GCALC_SIGN(pi->node.shape.iy[0]) ? "-" : "",
                 FIRST_DIGIT(pi->node.shape.iy[0]), pi->node.shape.iy[1], postfix);
}
...
static void GCALC_DBUG_PRINT_PI(const Gcalc_heap::Info *pi)
{
  ...
  if (pi->type == Gcalc_heap::nt_intersection)
  {
    ...
    GCALC_DBUG_PRINT(("intersection point %d %d", ic->edge_a->thread, ic->edge_b->thread));
#endif
    return;
  }
...
static void GCALC_DBUG_PRINT_SLICE(const char *header, const Gcalc_scan_iterator::point *slice)
{
  size_t nbuf;
  char buf[1024];
  nbuf = strlen(header);
...
#else
#define GCALC_DBUG_CHECK_COUNTER() \
  do                               \
  {                                \
  } while (0)
#define GCALC_DBUG_PRINT_PI(pi) \
  do                            \
  {                             \
  } while (0)
#define GCALC_DBUG_PRINT_SLICE(a, b) \
  do                                 \
  {                                  \
  } while (0)
#define GCALC_DBUG_PRINT_INTERSECTIONS(a) \
  do                                      \
  {                                       \
  } while (0)
#define GCALC_DBUG_PRINT_STATE(a) \
  do                              \
  {                               \
  } while (0)
...
Gcalc_dyn_list::~Gcalc_dyn_list() { cleanup(); }
...
static void do_add(Gcalc_internal_coord *result, int result_len, const Gcalc_internal_coord *a,
                   const Gcalc_internal_coord *b)
{
  ...
  do
  {
    if ((result[n_digit] = a[n_digit] + b[n_digit] + carry) >= GCALC_DIG_BASE)
    {
      carry = 1;
...
static void do_sub(Gcalc_internal_coord *result, int result_len, const Gcalc_internal_coord *a,
                   const Gcalc_internal_coord *b)
{
  int n_digit = result_len - 1;
  gcalc_digit_t carry = 0;
...
static int do_cmp(const Gcalc_internal_coord *a, const Gcalc_internal_coord *b, int len)
{
  int n_digit = 1;
...
static int de_check(long double a, long double b) { return de_weak_check(a, b, (long double)1e-9); }
#endif /*GCALC_CHECK_WITH_FLOAT*/

void gcalc_mul_coord(Gcalc_internal_coord *result, int result_len, const Gcalc_internal_coord *a, int a_len,
                     const Gcalc_internal_coord *b, int b_len)
{
  ...
  do
  {
    ...
    if (carry)
    {
      for (n_res = n_a; (result[n_res] += carry) >= GCALC_DIG_BASE; n_res--)
      {
...
#ifdef GCALC_CHECK_WITH_FLOAT
  GCALC_DBUG_ASSERT(
      de_check(gcalc_get_double(a, a_len) * gcalc_get_double(b, b_len), gcalc_get_double(result, result_len)));
#endif /*GCALC_CHECK_WITH_FLOAT*/
}

inline void gcalc_mul_coord1(Gcalc_coord1 result, const Gcalc_coord1 a, const Gcalc_coord1 b)
{
  return gcalc_mul_coord(result, GCALC_COORD_BASE2, a, GCALC_COORD_BASE, b, GCALC_COORD_BASE);
}

void gcalc_add_coord(Gcalc_internal_coord *result, int result_len, const Gcalc_internal_coord *a,
                     const Gcalc_internal_coord *b)
{
...
#ifdef GCALC_CHECK_WITH_FLOAT
  GCALC_DBUG_ASSERT(de_check(gcalc_get_double(a, result_len) + gcalc_get_double(b, result_len),
                             gcalc_get_double(result, result_len)));
#endif /*GCALC_CHECK_WITH_FLOAT*/
}

void gcalc_sub_coord(Gcalc_internal_coord *result, int result_len, const Gcalc_internal_coord *a,
                     const Gcalc_internal_coord *b)
{
...
inline void gcalc_sub_coord1(Gcalc_coord1 result, const Gcalc_coord1 a, const Gcalc_coord1 b)
{
  return gcalc_sub_coord(result, GCALC_COORD_BASE, a, b);
}

int gcalc_cmp_coord(const Gcalc_internal_coord *a, const Gcalc_internal_coord *b, int len)
{
  int n_digit = 0;
  int result = 0;
...
#ifdef GCALC_CHECK_WITH_FLOAT
  if (result == 0)
    GCALC_DBUG_ASSERT(de_check(gcalc_get_double(a, len), gcalc_get_double(b, len)));
  else if (result == 1)
    GCALC_DBUG_ASSERT(de_check(gcalc_get_double(a, len), gcalc_get_double(b, len)) ||
...
void Gcalc_scan_iterator::intersection_info::do_calc_t()
{
  ...
  GCALC_DBUG_ASSERT(!gcalc_is_zero(edge_a->dy, GCALC_COORD_BASE) || !gcalc_is_zero(edge_b->dy, GCALC_COORD_BASE));
...
void Gcalc_scan_iterator::intersection_info::do_calc_y()
{
  ...
  gcalc_mul_coord(a_tb, GCALC_COORD_BASE3, t_b, GCALC_COORD_BASE2, edge_a->pi->node.shape.iy, GCALC_COORD_BASE);
  gcalc_mul_coord(b_ta, GCALC_COORD_BASE3, t_a, GCALC_COORD_BASE2, edge_a->dy, GCALC_COORD_BASE);
...
void Gcalc_scan_iterator::intersection_info::do_calc_x()
{
  ...
  gcalc_mul_coord(a_tb, GCALC_COORD_BASE3, t_b, GCALC_COORD_BASE2, edge_a->pi->node.shape.ix, GCALC_COORD_BASE);
  gcalc_mul_coord(b_ta, GCALC_COORD_BASE3, t_a, GCALC_COORD_BASE2, edge_a->dx, GCALC_COORD_BASE);

  gcalc_add_coord(x_exp, GCALC_COORD_BASE3, a_tb, b_ta);
  x_calculated = 1;
}

static int cmp_node_isc(const Gcalc_heap::Info *node, const Gcalc_heap::Info *isc)
{
  ...
  gcalc_mul_coord(exp, GCALC_COORD_BASE3, inf->t_b, GCALC_COORD_BASE2, node->node.shape.iy, GCALC_COORD_BASE);
  ...
  inf->calc_x_exp();
  gcalc_mul_coord(exp, GCALC_COORD_BASE3, inf->t_b, GCALC_COORD_BASE2, node->node.shape.ix, GCALC_COORD_BASE);
  ...
  if (result < 0)
  {
    if (!de_check(int_x, node->node.shape.x) && node->node.shape.x > int_x)
      GCALC_DBUG_PRINT(("floatcheck cmp_nod_iscx failed %g < %LG", node->node.shape.x, int_x));
  }
  else if (result > 0)
  {
    if (!de_check(int_x, node->node.shape.x) && node->node.shape.x < int_x)
...
static int cmp_intersections(const Gcalc_heap::Info *i1, const Gcalc_heap::Info *i2)
{
  ...
  gcalc_mul_coord(exp_a, GCALC_COORD_BASE5, inf1->y_exp, GCALC_COORD_BASE3, inf2->t_b, GCALC_COORD_BASE2);
  gcalc_mul_coord(exp_b, GCALC_COORD_BASE5, inf2->y_exp, GCALC_COORD_BASE3, inf1->t_b, GCALC_COORD_BASE2);
  ...
  if (result < 0)
  {
    if (!de_check(y1, y2) && y2 > y1)
      GCALC_DBUG_PRINT(("floatcheck cmp_intersections_y failed %LG < %LG", y2, y1));
  }
  else if (result > 0)
  {
    if (!de_check(y1, y2) && y2 < y1)
  ...
  inf1->calc_x_exp();
  inf2->calc_x_exp();
  gcalc_mul_coord(exp_a, GCALC_COORD_BASE5, inf1->x_exp, GCALC_COORD_BASE3, inf2->t_b, GCALC_COORD_BASE2);
  gcalc_mul_coord(exp_b, GCALC_COORD_BASE5, inf2->x_exp, GCALC_COORD_BASE3, inf1->t_b, GCALC_COORD_BASE2);
  ...
  if (result < 0)
  {
    if (!de_check(x1, x2) && x2 > x1)
      GCALC_DBUG_PRINT(("floatcheck cmp_intersectionsx failed %LG < %LG", x2, x1));
  }
  else if (result > 0)
  {
    if (!de_check(x1, x2) && x2 < x1)
...
{
  double scale = 1e-2;
  while (scale < extent) scale *= (double)10;
...
void Gcalc_heap::free_point_info(Gcalc_heap::Info *i, Gcalc_dyn_list::Item **i_hook)
{
  if (m_hook == &i->next)
    m_hook = i_hook;
  *i_hook = i->next;
...
Gcalc_heap::Info *Gcalc_heap::new_point_info(double x, double y, gcalc_shape_info shape)
{
...
static Gcalc_heap::Info *new_intersection(Gcalc_heap *heap, Gcalc_scan_iterator::intersection_info *ii)
{
...
static Gcalc_heap::Info *new_eq_point(Gcalc_heap *heap, const Gcalc_heap::Info *p, Gcalc_scan_iterator::point *edge)
{
...
void Gcalc_heap::Info::calc_xy(double *x, double *y) const
{
  ...
  double t = (node.intersection.p3->node.shape.x - node.intersection.p1->node.shape.x) * b1_y -
             (node.intersection.p3->node.shape.y - node.intersection.p1->node.shape.y) * b1_x;
...
static int cmp_point_info(const Gcalc_heap::Info *i0, const Gcalc_heap::Info *i1)
{
...
int Gcalc_shape_transporter::int_single_point(gcalc_shape_info Info, double x, double y)
{
...
int Gcalc_shape_transporter::int_add_point(gcalc_shape_info Info, double x, double y)
{
...
Gcalc_scan_iterator::Gcalc_scan_iterator(size_t blk_size)
    : Gcalc_dyn_list(blk_size, sizeof(point) > sizeof(intersection_info) ? sizeof(point) : sizeof(intersection_info))
{
  state.slice = NULL;
...
int Gcalc_scan_iterator::point::cmp_dx_dy(const Gcalc_coord1 dx_a, const Gcalc_coord1 dy_a, const Gcalc_coord1 dx_b,
...
int Gcalc_scan_iterator::point::cmp_dx_dy(const Gcalc_heap::Info *p1, const Gcalc_heap::Info *p2,
...
#ifdef GCALC_CHECK_WITH_FLOAT
void Gcalc_scan_iterator::point::calc_x(long double *x, long double y, long double ix) const
{
  ...
  if (fabsl(ddy) < (long double)1e-20)
  {
    ...
    *x = (ddy * (long double)pi->node.shape.x + gcalc_get_double(dx, GCALC_COORD_BASE) * (y - pi->node.shape.y)) / ddy;
}
#endif /*GCALC_CHECK_WITH_FLOAT*/
...
int Gcalc_scan_iterator::arrange_event(int do_sorting, int n_intersections)
{
  ...
  *m_bottom_hook = NULL;
  for (sp = m_bottom_points; sp; sp = sp->get_next()) sp->ev_next = sp->get_next();

  for (sp = state.slice, sp_hook = &state.slice; sp; sp_hook = sp->next_ptr(), sp = sp->get_next())
  {
    if (sp->event)
    {
  ...
  for (sp = *(sp_hook = state.event_position_hook); sp && sp->event; sp_hook = sp->next_ptr(), sp = sp->get_next())
  {
    ev_counter++;
  ...
  {
    point *cur_p = sp;
    for (; cur_p; cur_p = cur_p->get_next()) GCALC_DBUG_ASSERT(!cur_p->event);
  }
#endif /*GCALC_DBUG_OFF*/
  ...
  else if (ev_counter > 1 && do_sorting)
  {
    point *cur_p;
    *sp_hook = NULL;
    sp = (point *)sort_list(compare_events, *state.event_position_hook, ev_counter);
    /* Find last item in the list, it's changed after the sorting. */
    for (cur_p = sp->get_next(); cur_p->get_next(); cur_p = cur_p->get_next())
    {
    }
...
int Gcalc_scan_iterator::step()
{
  ...
  if (m_bottom_points)
  {
    free_list((Gcalc_dyn_list::Item **)&m_bottom_points, (Gcalc_dyn_list::Item **)m_bottom_hook);
    m_bottom_points = NULL;
    m_bottom_hook = &m_bottom_points;
  }
  for (sp = *state.event_position_hook; sp != state.event_end; sp = sp->get_next()) sp->event = scev_none;
  ...
  {
#ifndef GCALC_DBUG_OFF
    if (m_cur_pi->type == Gcalc_heap::nt_intersection && m_cur_pi->get_next()->type == Gcalc_heap::nt_intersection &&
...
static int node_on_right(const Gcalc_heap::Info *node, const Gcalc_heap::Info *edge_a, const Gcalc_heap::Info *edge_b)
{
...
static int cmp_tops(const Gcalc_heap::Info *top_node, const Gcalc_heap::Info *edge_a, const Gcalc_heap::Info *edge_b)
{
  ...
  if (m_cur_pi->node.shape.left)
  {
    ...
    if (m_cur_pi->node.shape.right)
    {
      ...
      else if (cmp_res == 0)
      {
        ...
        {
          cmp_res =
              gcalc_cmp_coord1(m_cur_pi->node.shape.left->node.shape.ix, m_cur_pi->node.shape.right->node.shape.ix);
          if (cmp_res != 0)
          {
            if (cmp_res < 0)
            {
...
int Gcalc_scan_iterator::add_events_for_node(point *sp_node)
{
  ...
  for (; sp != sp_node; sp = sp->get_next())
  {
    ...
    if (sp->next_pi == sp_node->next_pi || gcalc_cmp_coord1(*sp->r_border, *sp_node->l_border) < 0)
      continue;
  ...
  for (; sp; sp = sp->get_next())
  {
    ...
    if (sp->next_pi == sp_node->next_pi || gcalc_cmp_coord1(*sp_node->r_border, *sp->l_border) < 0)
      continue;
    sp_pi_r = node_on_right(sp->next_pi, sp_node->pi, sp_node->next_pi);
    if (sp_pi_r > 0)
      continue;
...
int Gcalc_scan_iterator::add_intersection(point *sp_a, point *sp_b, Gcalc_heap::Info *pi_from)
{
  Gcalc_heap::Info *ii;
  ...
  if (!(i_calc = new_intersection_info(sp_a, sp_b)) || !(ii = new_intersection(m_heap, i_calc)))
    GCALC_DBUG_RETURN(1);

  ii->node.intersection.equal = 0;

  for (; pi_from->get_next() != sp_a->next_pi && pi_from->get_next() != sp_b->next_pi; pi_from = pi_from->get_next())
  {
...
void calc_t(Gcalc_coord2 t_a, Gcalc_coord2 t_b, Gcalc_coord1 dxa, Gcalc_coord1 dxb, const Gcalc_heap::Info *p1,
            const Gcalc_heap::Info *p2, const Gcalc_heap::Info *p3, const Gcalc_heap::Info *p4)
{
  Gcalc_coord1 a2_a1x, a2_a1y;
  Gcalc_coord2 x1y2, x2y1;
...
double Gcalc_scan_iterator::get_y() const
{
  if (state.pi->type == Gcalc_heap::nt_intersection)
  {
    ...
    calc_t(t_a, t_b, dxa, dya, state.pi->node.intersection.p1, state.pi->node.intersection.p2,
           state.pi->node.intersection.p3, state.pi->node.intersection.p4);

    gcalc_mul_coord(a_tb, GCALC_COORD_BASE3, t_b, GCALC_COORD_BASE2, state.pi->node.intersection.p1->node.shape.iy,
                    GCALC_COORD_BASE);
    gcalc_mul_coord(b_ta, GCALC_COORD_BASE3, t_a, GCALC_COORD_BASE2, dya, GCALC_COORD_BASE);

    gcalc_add_coord(y_exp, GCALC_COORD_BASE3, a_tb, b_ta);

    return (get_pure_double(y_exp, GCALC_COORD_BASE3) / get_pure_double(t_b, GCALC_COORD_BASE2)) / m_heap->coord_extent;
  }
  else
...
double Gcalc_scan_iterator::get_event_x() const
{
  if (state.pi->type == Gcalc_heap::nt_intersection)
  {
    ...
    gcalc_mul_coord(a_tb, GCALC_COORD_BASE3, t_b, GCALC_COORD_BASE2, state.pi->node.intersection.p1->node.shape.ix,
                    GCALC_COORD_BASE);
    gcalc_mul_coord(b_ta, GCALC_COORD_BASE3, t_a, GCALC_COORD_BASE2, dxa, GCALC_COORD_BASE);

    gcalc_add_coord(x_exp, GCALC_COORD_BASE3, a_tb, b_ta);

    return (get_pure_double(x_exp, GCALC_COORD_BASE3) / get_pure_double(t_b, GCALC_COORD_BASE2)) / m_heap->coord_extent;
  }
  else
...
double Gcalc_scan_iterator::get_pure_double(const Gcalc_internal_coord *d, int d_len)
{
  int n = 1;
  long double res = (long double)FIRST_DIGIT(d[0]);
  do
