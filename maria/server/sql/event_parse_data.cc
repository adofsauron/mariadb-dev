...
void Event_parse_data::check_if_in_the_past(THD *thd, my_time_t ltime_utc)
{
  ...
  */
  ...
  if (on_completion == Event_parse_data::ON_COMPLETION_DROP)
  {
    switch (thd->lex->sql_command)
    {
      case SQLCOM_CREATE_EVENT:
        push_warning(thd, Sql_condition::WARN_LEVEL_NOTE, ER_EVENT_CANNOT_CREATE_IN_THE_PAST,
  ...
  else if (status == Event_parse_data::ENABLED)
  {
    ...
    status_changed = true;
    push_warning(thd, Sql_condition::WARN_LEVEL_NOTE, ER_EVENT_EXEC_TIME_IN_THE_PAST,
...
int Event_parse_data::init_execute_at(THD *thd)
{
  ...
  DBUG_PRINT("info", ("starts_null && ends_null should be 1 is %d", (starts_null && ends_null)));
  ...
  */
  if (item_execute_at->get_date(thd, &ltime, TIME_NO_ZERO_DATE | thd->temporal_round_mode()))
    goto wrong_value;
...
int Event_parse_data::init_interval(THD *thd)
{
  ...
  switch (interval)
  {
    ...
    case INTERVAL_DAY_MINUTE:
      expression = (interval_tmp.day * 24 + interval_tmp.hour) * 60 + interval_tmp.minute;
      break;
    case INTERVAL_HOUR_SECOND: /* day is anyway 0 */
    case INTERVAL_DAY_SECOND:
      /* DAY_SECOND having problems because of leap seconds? */
      expression = ((interval_tmp.day * 24 + interval_tmp.hour) * 60 + interval_tmp.minute) * 60 + interval_tmp.second;
      break;
  ...
  if (interval_tmp.neg || expression == 0 || expression > EVEX_MAX_INTERVAL_VALUE)
  {
...
int Event_parse_data::init_starts(THD *thd)
{
  ...
  if (item_starts->get_date(thd, &ltime, TIME_NO_ZERO_DATE | thd->temporal_round_mode()))
    goto wrong_value;
  ...
  DBUG_PRINT("info", ("now: %ld  starts: %ld", (long)thd->query_start(), (long)ltime_utc));
...
int Event_parse_data::init_ends(THD *thd)
{
  ...
  if (item_ends->get_date(thd, &ltime, TIME_NO_ZERO_DATE | thd->temporal_round_mode()))
    goto error_bad_params;
...
bool Event_parse_data::check_parse_data(THD *thd)
{
  ...
  DBUG_PRINT("info",
             ("execute_at: %p  expr=%p  starts=%p  ends=%p", item_execute_at, item_expression, item_starts, item_ends));

  init_name(thd, identifier);

  init_definer(thd);

  ret = init_execute_at(thd) || init_interval(thd) || init_starts(thd) || init_ends(thd);
...
void Event_parse_data::check_originator_id(THD *thd)
{
  ...
  if ((WSREP(thd) && IF_WSREP(thd->wsrep_applier, 0)) || (thd->system_thread == SYSTEM_THREAD_SLAVE_SQL) ||
      (thd->system_thread == SYSTEM_THREAD_SLAVE_IO))
  {
    DBUG_PRINT("info", ("Invoked object status set to SLAVESIDE_DISABLED."));
    if ((status == Event_parse_data::ENABLED) || (status == Event_parse_data::DISABLED))
    {
