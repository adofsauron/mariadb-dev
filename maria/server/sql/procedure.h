...
class Item_proc_real : public Item_proc
{
 ...
 public:
  Item_proc_real(THD *thd, const char *name_par, uint dec) : Item_proc(thd, name_par)
  {
    decimals = dec;
...
class Item_proc_int : public Item_proc
{
 ...
 public:
  Item_proc_int(THD *thd, const char *name_par) : Item_proc(thd, name_par) { max_length = 11; }
  ...
  {
    int err;
    value = cs->strntoll(str, length, 10, NULL, &err);
  }
...
class Item_proc_string : public Item_proc
{
 ...
 public:
  Item_proc_string(THD *thd, const char *name_par, uint length) : Item_proc(thd, name_par)
  {
  ...
  void set(const char *str, uint length, CHARSET_INFO *cs) override { value.copy(str, length, cs); }
  double val_real() override
  {
    int err_not_used;
    char *end_not_used;
    CHARSET_INFO *cs = value.charset();
    return cs->strntod((char *)value.ptr(), value.length(), &end_not_used, &err_not_used);
  }
  ...
  String *val_str(String *) override { return null_value ? (String *)0 : &value; }
  my_decimal *val_decimal(my_decimal *) override;
