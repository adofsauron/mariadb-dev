...
Histogram_base *create_histogram(MEM_ROOT *mem_root, Histogram_type hist_type, THD *owner);
...
static const LEX_CSTRING stat_table_name[STATISTICS_TABLES] = {
    {STRING_WITH_LEN("table_stats")}, {STRING_WITH_LEN("column_stats")}, {STRING_WITH_LEN("index_stats")}};
...
static inline void init_table_list_for_stat_tables(TABLE_LIST *tables, bool for_write)
{
  uint i;
  ...
  for (i = 0; i < STATISTICS_TABLES; i++)
  {
    ...
    if (i < STATISTICS_TABLES - 1)
      tables[i].next_global = tables[i].next_local = tables[i].next_name_resolution_table = &tables[i + 1];
    if (i != 0)
...
static const TABLE_FIELD_TYPE table_stat_fields[TABLE_STAT_N_FIELDS] = {
    {{STRING_WITH_LEN("db_name")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("table_name")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("cardinality")}, {STRING_WITH_LEN("bigint(21)")}, {NULL, 0}},
};
static const uint table_stat_pk_col[] = {0, 1};
static const TABLE_FIELD_DEF table_stat_def = {TABLE_STAT_N_FIELDS, table_stat_fields, 2, table_stat_pk_col};

static const TABLE_FIELD_TYPE column_stat_fields[COLUMN_STAT_N_FIELDS] = {
    {{STRING_WITH_LEN("db_name")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("table_name")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("column_name")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("min_value")}, {STRING_WITH_LEN("varbinary(255)")}, {NULL, 0}},
    {{STRING_WITH_LEN("max_value")}, {STRING_WITH_LEN("varbinary(255)")}, {NULL, 0}},
    {{STRING_WITH_LEN("nulls_ratio")}, {STRING_WITH_LEN("decimal(12,4)")}, {NULL, 0}},
    {{STRING_WITH_LEN("avg_length")}, {STRING_WITH_LEN("decimal(12,4)")}, {NULL, 0}},
    {{STRING_WITH_LEN("avg_frequency")}, {STRING_WITH_LEN("decimal(12,4)")}, {NULL, 0}},
    {{STRING_WITH_LEN("hist_size")}, {STRING_WITH_LEN("tinyint(3)")}, {NULL, 0}},
    {{STRING_WITH_LEN("hist_type")},
     {STRING_WITH_LEN("enum('SINGLE_PREC_HB','DOUBLE_PREC_HB','JSON_HB')")},
     {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("histogram")}, {STRING_WITH_LEN("longblob")}, {NULL, 0}}};
static const uint column_stat_pk_col[] = {0, 1, 2};
static const TABLE_FIELD_DEF column_stat_def = {COLUMN_STAT_N_FIELDS, column_stat_fields, 3, column_stat_pk_col};

static const TABLE_FIELD_TYPE index_stat_fields[INDEX_STAT_N_FIELDS] = {
    {{STRING_WITH_LEN("db_name")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("table_name")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("index")}, {STRING_WITH_LEN("varchar(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("prefix_arity")}, {STRING_WITH_LEN("int(11)")}, {NULL, 0}},
    {{STRING_WITH_LEN("avg_frequency")}, {STRING_WITH_LEN("decimal(12,4)")}, {NULL, 0}}};
static const uint index_stat_pk_col[] = {0, 1, 2, 3};
static const TABLE_FIELD_DEF index_stat_def = {INDEX_STAT_N_FIELDS, index_stat_fields, 4, index_stat_pk_col};
...
static int open_stat_tables(THD *thd, TABLE_LIST *tables, bool for_write)
{
  ...
  if (!rc && (stat_table_intact.check(tables[TABLE_STAT].table, &table_stat_def) ||
...
*/
static inline int open_stat_table_for_ddl(THD *thd, TABLE_LIST *table, const LEX_CSTRING *stat_tab_name)
{
...
class Stat_table
{
 ...
 protected:
  ...
  void store_record_for_update() { store_record(stat_table, record[1]); }

  void store_record_for_lookup() { DBUG_ASSERT(record[0] == stat_table->record[0]); }

  bool update_record()
  {
    int err;
    if ((err = stat_file->ha_update_row(record[1], record[0])) && err != HA_ERR_RECORD_IS_THE_SAME)
      return TRUE;
  ...
  */
  ...
  bool find_stat()
  {
    ...
    return !stat_file->ha_index_read_idx_map(record[0], stat_key_idx, key, HA_WHOLE_KEY, HA_READ_KEY_EXACT);
  }
  ...
  */
  ...
  bool find_next_stat_for_prefix(uint prefix_parts)
  {
    ...
    for (uint i = 0; i < prefix_parts; i++) prefix_key_length += stat_key_info->key_part[i].store_length;
    key_copy(key, record[0], stat_key_info, prefix_key_length);
    key_part_map prefix_map = (key_part_map)((1 << prefix_parts) - 1);
    return !stat_file->ha_index_read_idx_map(record[0], stat_key_idx, key, prefix_map, HA_READ_KEY_EXACT);
  }
...
class Table_stat : public Stat_table
{
  ...
  */
  ...
  Table_stat(TABLE *stat, TABLE *tab) : Stat_table(stat, tab) { common_init_table_stat(); }
  ...
  */
  ...
  Table_stat(TABLE *stat, const LEX_CSTRING *db, const LEX_CSTRING *tab) : Stat_table(stat, db, tab)
  {
  ...
  */
  ...
  void set_key_fields()
  {
    ...
    table_name_field->store(table_name->str, table_name->length, system_charset_info);
  }
...
class Column_stat : public Stat_table
{
  ...
  */
  ...
  Column_stat(TABLE *stat, TABLE *tab) : Stat_table(stat, tab) { common_init_column_stat_table(); }
  ...
  */
  ...
  Column_stat(TABLE *stat, const LEX_CSTRING *db, const LEX_CSTRING *tab) : Stat_table(stat, db, tab)
  {
  ...
  */
  ...
  {
    set_full_table_name();
    column_name_field->store(col->field_name.str, col->field_name.length, system_charset_info);
    table_field = col;
  }
      ...
      else
      {
        ...
        switch (i)
        {
          ...
          case COLUMN_STAT_HIST_SIZE:
            ...
            // histogram!
            stat_field->store(stats->histogram ? stats->histogram->get_size() : 0);
            break;
  ...
  void get_stat_values()
  {
    ...
    if (find_stat())
    {
      ...
      for (uint i = COLUMN_STAT_MIN_VALUE; i <= COLUMN_STAT_HISTOGRAM; i++)
      {
        ...
        if (!stat_field->is_null() &&
            (i > COLUMN_STAT_MAX_VALUE || (i == COLUMN_STAT_MIN_VALUE && table_field->read_stats->min_value) ||
             (i == COLUMN_STAT_MAX_VALUE && table_field->read_stats->max_value)))
        {
          ...
          switch (i)
          {
            ...
            case COLUMN_STAT_HIST_TYPE:
            {
              ...
              */
              Histogram_type hist_type = (Histogram_type)(stat_field->val_int() - 1);
  ...
  */
  ...
  Histogram_base *load_histogram(MEM_ROOT *mem_root)
  {
    if (find_stat())
    {
      ...
      Histogram_type hist_type = table_field->read_stats->histogram_type_on_disk;

      Histogram_base *hist;
      if (!(hist = create_histogram(mem_root, hist_type, NULL)))
        return NULL;
      Field *field = table->field[table_field->field_index];
      if (!hist->parse(mem_root, db_name->str, table_name->str, field, hist_type, val.ptr(), val.length()))
      {
...
bool Histogram_binary::parse(MEM_ROOT *mem_root, const char *, const char *, Field *, Histogram_type type_arg,
...
*/
void Histogram_binary::serialize(Field *field) { field->store((char *)values, size, &my_charset_bin); }

void Histogram_binary::init_for_collection(MEM_ROOT *mem_root, Histogram_type htype_arg, ulonglong size_arg)
{
  type = htype_arg;
...
class Index_stat : public Stat_table
{
  ...
  */
  ...
  Index_stat(TABLE *stat, TABLE *tab) : Stat_table(stat, tab) { common_init_index_stat_table(); }
  ...
  */
  ...
  Index_stat(TABLE *stat, const LEX_CSTRING *db, const LEX_CSTRING *tab) : Stat_table(stat, db, tab)
  {
  ...
  */
  ...
  void set_index_prefix_key_fields(KEY *index_info)
  {
    ...
    index_name_field->store(index_name, index_info->name.length, system_charset_info);
  ...
  */
  ...
  void store_stat_fields()
  {
    ...
    double avg_frequency = table_key_info->collected_stats->get_avg_frequency(prefix_arity - 1);
    if (avg_frequency == 0)
      stat_field->set_null();
    else
    {
...
class Stat_table_write_iter
{
 ...
 public:
  Stat_table_write_iter(Stat_table *stat_table_arg)
      : owner(stat_table_arg), rowid_buf(NULL), rowid_size(owner->stat_file->ref_length)
  {
    my_b_clear(&io_cache);
  }
  ...
  */
  bool init(uint n_keyparts)
  {
    ...
    if (open_cached_file(&io_cache, mysql_tmpdir, TEMP_PREFIX, 1024, MYF(MY_WME)))
      return true;

    handler *h = owner->stat_file;
    uchar key[MAX_KEY_LENGTH];
    uint prefix_len = 0;
    for (uint i = 0; i < n_keyparts; i++) prefix_len += owner->stat_key_info->key_part[i].store_length;

    key_copy(key, owner->record[0], owner->stat_key_info, prefix_len);
    key_part_map prefix_map = (key_part_map)((1 << n_keyparts) - 1);
    h->ha_index_init(owner->stat_key_idx, false);
    int res = h->ha_index_read_map(owner->record[0], key, prefix_map, HA_READ_KEY_EXACT);
    if (res)
    {
...
class Histogram_binary_builder : public Histogram_builder
{
 ...
 public:
  Histogram_binary_builder(Field *col, uint col_len, ha_rows rows) : Histogram_builder(col, col_len, rows)
  {
  ...
  int next(void *elem, element_count elem_cnt) override
  {
    ...
    if (count > bucket_capacity * (curr_bucket + 1))
    {
      ...
      histogram->set_value(curr_bucket, column->pos_in_interval(min_value, max_value));
      curr_bucket++;
      while (curr_bucket != hist_width && count > bucket_capacity * (curr_bucket + 1))
      {
...
Histogram_builder *Histogram_binary::create_builder(Field *col, uint col_len, ha_rows rows)
{
...
int basic_stats_collector_walk(void *elem, element_count count, void *arg)
{
...
class Count_distinct_field : public Sql_alloc
{
  ...
  */
  ...
  Count_distinct_field(Field *field, size_t max_heap_table_size)
  {
    ...
    tree = new Unique((qsort_cmp2)simple_str_key_cmp, (void *)field, tree_key_length, max_heap_table_size, 1);
  }
  ...
  */
  void walk_tree()
  {
    ...
    tree->walk(table_field->table, basic_stats_collector_walk, (void *)&stats_collector);
  ...
  */
  bool walk_tree_with_histogram(ha_rows rows)
  {
    ...
    Histogram_builder *hist_builder = hist->create_builder(table_field, tree_key_length, rows);

    if (tree->walk(table_field->table, histogram_build_walk, (void *)hist_builder))
    {
      delete hist_builder;
      return true;  // Error
    }
    ...
    distincts_single_occurence = hist_builder->counters.get_count_single_occurence();
    delete hist_builder;
    return false;
  }

  ulonglong get_count_distinct() { return distincts; }

  ulonglong get_count_distinct_single_occurence() { return distincts_single_occurence; }

  /*
    @brief
    Get the pointer to the histogram built for table_field
  */
  Histogram_base *get_histogram() { return table_field->collected_stats->histogram; }
};
...
class Count_distinct_field_bit : public Count_distinct_field
{
 public:
  Count_distinct_field_bit(Field *field, size_t max_heap_table_size)
  {
    ...
    tree = new Unique((qsort_cmp2)simple_ulonglong_key_cmp, (void *)&tree_key_length, tree_key_length,
 ...
 public:
  ...
  Index_prefix_calc(THD *thd, TABLE *table, KEY *key_info)
      : index_table(table),
        ...
        is_partial_fields_present(false)
  {
    uint i;
    ...
    if ((uint)(table->key_info - key_info) == pk && table->key_info[pk].user_defined_key_parts == 1)
    {
      prefixes = 1;
    ...
    if ((calc_state = (Prefix_calc_state *)thd->alloc(sizeof(Prefix_calc_state) * key_parts)))
    {
      ...
      for (i = 0, state = calc_state; i < key_parts; i++, state++)
      {
        ...
        */
        ...
        if (!(state->last_prefix = new (thd->mem_root) Cached_item_field(thd, key_info->key_part[i].field)))
          break;
  ...
  */
  ...
  void get_avg_frequency()
  {
    ...
    for (i = 0, state = calc_state; i < prefixes; i++, state++)
    {
      if (i < prefixes)
      {
        double val = state->prefix_count == 0 ? 0 : (double)state->entry_count / state->prefix_count;
...
static void create_min_max_statistical_fields_for_table(TABLE *table)
{
  uint rec_buff_length = table->s->rec_buff_length;

  if ((table->collected_stats->min_max_record_buffers = (uchar *)alloc_root(&table->mem_root, 2 * rec_buff_length)))
  {
...
static void create_min_max_statistical_fields_for_table_share(THD *thd, TABLE_SHARE *table_share)
{
  ...
  if ((stats->min_max_record_buffers = (uchar *)alloc_root(&stats_cb->mem_root, 2 * rec_buff_length)))
  {
...
int alloc_statistics_for_table(THD *thd, TABLE *table)
{
  ...
  Table_statistics *table_stats = (Table_statistics *)alloc_root(&table->mem_root, sizeof(Table_statistics));

  Column_statistics_collected *column_stats =
      (Column_statistics_collected *)alloc_root(&table->mem_root, sizeof(Column_statistics_collected) * columns);

  uint keys = table->s->keys;
  Index_statistics *index_stats = (Index_statistics *)alloc_root(&table->mem_root, sizeof(Index_statistics) * keys);

  uint key_parts = table->s->ext_key_parts;
  ulonglong *idx_avg_frequency = (ulonglong *)alloc_root(&table->mem_root, sizeof(ulonglong) * key_parts);
  ...
  KEY *key_info, *end;
  for (key_info = table->key_info, end = key_info + table->s->keys; key_info < end; key_info++, index_stats++)
  {
...
void free_statistics_for_table(THD *thd, TABLE *table)
{
  for (Field **field_ptr = table->field; *field_ptr; field_ptr++)
  {
    ...
    if ((*field_ptr)->collected_stats && (*field_ptr)->collected_stats->histogram &&
...
static int alloc_statistics_for_table_share(THD *thd, TABLE_SHARE *table_share)
{
  ...
  if (!table_stats)
  {
    table_stats = (Table_statistics *)alloc_root(&stats_cb->mem_root, sizeof(Table_statistics));
    if (!table_stats)
      DBUG_RETURN(1);
  ...
  if (!column_stats)
  {
    column_stats = (Column_statistics *)alloc_root(&stats_cb->mem_root, sizeof(Column_statistics) * (fields + 1));
    if (column_stats)
    {
  ...
  if (!index_stats)
  {
    index_stats = (Index_statistics *)alloc_root(&stats_cb->mem_root, sizeof(Index_statistics) * keys);
    if (index_stats)
    {
      table_stats->index_stats = index_stats;
      for (key_info = table_share->key_info, end = key_info + keys; key_info < end; key_info++, index_stats++)
      {
  ...
  {
    idx_avg_frequency = (ulonglong *)alloc_root(&stats_cb->mem_root, sizeof(ulonglong) * key_parts);
    if (idx_avg_frequency)
    {
      memset(idx_avg_frequency, 0, sizeof(ulonglong) * key_parts);
      table_stats->idx_avg_frequency = idx_avg_frequency;
      for (key_info = table_share->key_info, end = key_info + keys; key_info < end; key_info++)
      {
...
inline void Column_statistics_collected::init(THD *thd, Field *table_field)
{
  ...
  {
    count_distinct = table_field->type() == MYSQL_TYPE_BIT
...
inline bool Column_statistics_collected::add()
{
  bool err = 0;
  if (column->is_null())
    nulls++;
  else
  {
    ...
    if (min_value && column->update_min(min_value, is_null(COLUMN_STAT_MIN_VALUE)))
      set_not_null(COLUMN_STAT_MIN_VALUE);
    if (max_value && column->update_max(max_value, is_null(COLUMN_STAT_MAX_VALUE)))
...
inline bool Column_statistics_collected::finish(MEM_ROOT *mem_root, ha_rows rows, double sample_fraction)
{
  double val;
  ...
  if (count_distinct)
  {
    ...
    ulonglong distincts_single_occurence = count_distinct->get_count_distinct_single_occurence();
    ...
    if (distincts)
    {
      ...
      */
      ...
      else
      {
        ...
        if (nulls)
          distincts += 1;
        double fraction_single_occurence = static_cast<double>(distincts_single_occurence) / rows;
        double total_number_of_rows = rows / sample_fraction;
        double estimate_total_distincts =
            total_number_of_rows / (distincts / (1.0 - (1.0 - sample_fraction) * fraction_single_occurence));
...
inline void Column_statistics_collected::cleanup()
{
  if (count_distinct)
  {
    delete count_distinct;
...
static int collect_statistics_for_index(THD *thd, TABLE *table, uint index)
{
  int rc = 0;
...
int collect_statistics_for_table(THD *thd, TABLE *table)
{
  ...
  if (thd->variables.sample_percentage == 0)
  {
    ...
    {
      sample_fraction =
          std::fmin((MIN_THRESHOLD_FOR_SAMPLING + 4096 * log(200 * file->records())) / file->records(), 1);
    }
  }
  ...
  */
  if (!rc)
  {
    ...
    table->collected_stats->cardinality = static_cast<ha_rows>(rows / sample_fraction);
  }
  ...
  for (field_ptr = table->field; *field_ptr; field_ptr++)
  {
    ...
    if (!rc)
    {
      rc = table_field->collected_stats->finish(&table->mem_root, rows, sample_fraction);
    }
    else
...
static int read_statistics_for_table(THD *thd, TABLE *table, TABLE_LIST *stat_tables)
{
  uint i;
  TABLE *stat_table;
  Field *table_field;
  ...
  Index_stat index_stat(stat_table, table);
  for (key_info = table_share->key_info, key_info_end = key_info + table_share->keys; key_info < key_info_end;
       key_info++)
  {
        ...
        key_info->read_stats->get_avg_frequency(key_info->user_defined_key_parts) == 0)
    {
      ...
      for (uint j = 0; j < pk_parts; j++)
      {
        if (!(ext_key_part_map & 1 << j))
        {
          ...
          {
            double avg_frequency = pk_key_info->read_stats->get_avg_frequency(j - 1);
            set_if_smaller(avg_frequency, 1);
            double val = pk_key_info->read_stats->get_avg_frequency(j) / avg_frequency;
...
void delete_stat_values_for_table_share(TABLE_SHARE *table_share)
{
  ...
  for (Field **field_ptr = table_share->field; *field_ptr; field_ptr++, column_stats++)
  {
...
static int read_histograms_for_table(THD *thd, TABLE *table, TABLE_LIST *stat_tables)
{
  ...
  if (stats_cb->start_histograms_load())
  {
    ...
    */
    ...
    for (Field **field_ptr = table->s->field; *field_ptr; field_ptr++)
    {
      ...
      {
        column_stat.set_key_fields(table_field);
        table_field->read_stats->histogram = column_stat.load_histogram(&stats_cb->mem_root);
      }
    }
...
static void dump_stats_from_share_to_table(TABLE *table)
{
  ...
  for (; key_info < key_info_end; key_info++, table_key_info++) table_key_info->read_stats = key_info->read_stats;

  Field **field_ptr = table_share->field;
  Field **table_field_ptr = table->field;
  for (; *field_ptr; field_ptr++, table_field_ptr++) (*table_field_ptr)->read_stats = (*field_ptr)->read_stats;
...
int read_statistics_for_tables(THD *thd, TABLE_LIST *tables)
{
        ...
        table_share->tmp_table == NO_TMP_TABLE)
    {
      if (table_share->table_category == TABLE_CATEGORY_USER)
      {
        if (table_share->stats_cb.stats_are_ready())
        {
          ...
          tl->table->histograms_are_read = table_share->stats_cb.histograms_are_ready();
          if (table_share->stats_cb.histograms_are_ready() || thd->variables.optimizer_use_condition_selectivity <= 3)
            continue;
        }
  ...
  */
  ...
  for (TABLE_LIST *tl = tables; tl; tl = tl->next_global)
  {
    TABLE_SHARE *table_share;
    if (!tl->is_view_or_derived() && tl->table && (table_share = tl->table->s) &&
        table_share->tmp_table == NO_TMP_TABLE && table_share->table_category == TABLE_CATEGORY_USER)
    {
      if (!tl->table->stats_is_read)
      {
...
int delete_statistics_for_index(THD *thd, TABLE *tab, KEY *key_info, bool ext_prefixes_only)
{
  int err;
  enum_binlog_format save_binlog_format;
  TABLE *stat_table;
...
int rename_table_in_stat_tables(THD *thd, const LEX_CSTRING *db, const LEX_CSTRING *tab, const LEX_CSTRING *new_db,
...
int rename_column_in_stat_tables(THD *thd, TABLE *tab, Field *col, const char *new_name)
{
  int err;
  enum_binlog_format save_binlog_format;
  TABLE *stat_table;
...
void set_statistics_for_table(THD *thd, TABLE *table)
{
  ...
  table->used_stat_records = (!check_eits_preferred(thd) || !table->stats_is_read || read_stats->cardinality_is_null)
                                 ? table->file->stats.records
                                 : read_stats->cardinality;
  ...
  */
  ...
  KEY *key_info, *key_info_end;
  for (key_info = table->key_info, key_info_end = key_info + table->s->keys; key_info < key_info_end; key_info++)
  {
    key_info->is_statistics_from_stat_tables =
        (check_eits_preferred(thd) && table->stats_is_read && key_info->read_stats->avg_frequency_is_inited() &&
...
double get_column_range_cardinality(Field *field, key_range *min_endp, key_range *max_endp, uint range_flag)
{
  double res;
  TABLE *table = field->table;
  Column_statistics *col_stats = field->read_stats;
  ...
  */
  ...
  bool nulls_incl = field->null_ptr && min_endp && min_endp->key[0] && !(range_flag & NEAR_MIN);
           ...
           !memcmp(min_endp->key, max_endp->key, min_endp->length))
  {
    ...
    else
    {
      ...
      res = avg_frequency;
      if (avg_frequency > 1.0 + 0.000001 && col_stats->min_max_values_are_provided())
      {
        Histogram_base *hist = col_stats->histogram;
        if (hist && hist->is_usable(thd))
        {
          res = col_non_nulls * hist->point_selectivity(field, min_endp, avg_frequency / col_non_nulls);
        }
      }
  ...
  else
  {
    if (col_stats->min_max_values_are_provided())
    {
      ...
      if (hist && hist->is_usable(thd))
      {
        sel = hist->range_selectivity(field, min_endp, max_endp, avg_frequency / col_non_nulls);
      ...
      else
      {
        ...
        {
          store_key_image_to_rec(field, (uchar *)min_endp->key, field->key_length());
          min_mp_pos = field->pos_in_interval(col_stats->min_value, col_stats->max_value);
        }
        else
          min_mp_pos = 0.0;
        if (max_endp)
        {
          store_key_image_to_rec(field, (uchar *)max_endp->key, field->key_length());
          max_mp_pos = field->pos_in_interval(col_stats->min_value, col_stats->max_value);
        }
        else
          max_mp_pos = 1.0;
...
double Histogram_binary::point_selectivity(Field *field, key_range *endpoint, double avg_sel)
{
  double sel;
  Column_statistics *col_stats = field->read_stats;
  store_key_image_to_rec(field, (uchar *)endpoint->key, field->key_length());
  double pos = field->pos_in_interval(col_stats->min_value, col_stats->max_value);
  ...
  uint max = min;
  while (max + 1 < get_width() && get_value(max + 1) == pos_value) max++;
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    double current_bucket_width = (max + 1 == get_width() ? 1.0 : (get_value(max) * inv_prec_factor)) -
...
double Histogram_binary::range_selectivity(Field *field, key_range *min_endp, key_range *max_endp, double avg_sel)
{
