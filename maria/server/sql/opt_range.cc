...
class SEL_TREE : public Sql_alloc
{
  ...
  */
  ...
  SEL_TREE(MEM_ROOT *root, size_t num_keys) : type(KEY), keys(root, num_keys), n_ror_scans(0) { keys_map.clear_all(); }
...
static ha_rows check_quick_select(PARAM *param, uint idx, bool index_only, SEL_ARG *tree, bool update_tbl_stats,
                                  uint *mrr_flags, uint *bufsize, Cost_estimate *cost, bool *is_ror_scan);

QUICK_RANGE_SELECT *get_quick_select(PARAM *param, uint index, SEL_ARG *key_tree, uint mrr_flags, uint mrr_buf_size,
                                     MEM_ROOT *alloc);
static TRP_RANGE *get_key_scans_params(PARAM *param, SEL_TREE *tree, bool index_read_must_be_used,
                                       bool for_range_access, double read_time);
static TRP_INDEX_INTERSECT *get_best_index_intersect(PARAM *param, SEL_TREE *tree, double read_time);
...
static TABLE_READ_PLAN *get_best_disjunct_quick(PARAM *param, SEL_IMERGE *imerge, double read_time,
                                                bool named_trace = false);
static TABLE_READ_PLAN *merge_same_index_scans(PARAM *param, SEL_IMERGE *imerge, TRP_INDEX_MERGE *imerge_trp,
                                               double read_time);
...
static SEL_ARG *key_or_with_limit(RANGE_OPT_PARAM *param, uint keyno, SEL_ARG *key1, SEL_ARG *key2);
static SEL_ARG *key_and_with_limit(RANGE_OPT_PARAM *param, uint keyno, SEL_ARG *key1, SEL_ARG *key2, uint clone_flag);
static bool get_range(SEL_ARG **e1, SEL_ARG **e2, SEL_ARG *root1);
bool get_quick_keys(PARAM *param, QUICK_RANGE_SELECT *quick, KEY_PART *key, SEL_ARG *key_tree, uchar *min_key,
                    uint min_key_flag, uchar *max_key, uint max_key_flag);
...
static SEL_ARG *enforce_sel_arg_weight_limit(RANGE_OPT_PARAM *param, uint keyno, SEL_ARG *sel_arg);
static bool sel_arg_and_weight_heuristic(RANGE_OPT_PARAM *param, SEL_ARG *key1, SEL_ARG *key2);

#include "opt_range_mrr.cc"

static bool sel_trees_have_common_keys(SEL_TREE *tree1, SEL_TREE *tree2, key_map *common_keys);
static void eliminate_single_tree_imerges(RANGE_OPT_PARAM *param, SEL_TREE *tree);

static bool sel_trees_can_be_ored(RANGE_OPT_PARAM *param, SEL_TREE *tree1, SEL_TREE *tree2, key_map *common_keys);
static bool sel_trees_must_be_ored(RANGE_OPT_PARAM *param, SEL_TREE *tree1, SEL_TREE *tree2, key_map common_keys);
static int and_range_trees(RANGE_OPT_PARAM *param, SEL_TREE *tree1, SEL_TREE *tree2, SEL_TREE *result);
static bool remove_nonrange_trees(PARAM *param, SEL_TREE *tree);
static void restore_nonrange_trees(RANGE_OPT_PARAM *param, SEL_TREE *tree, SEL_ARG **backup);
static void print_key_value(String *out, const KEY_PART_INFO *key_part, const uchar *key, uint length);
static void print_keyparts_name(String *out, const KEY_PART_INFO *key_part, uint n_keypart, key_part_map keypart_map);

static void trace_ranges(Json_writer_array *range_trace, PARAM *param, uint idx, SEL_ARG *keypart,
                         const KEY_PART_INFO *key_parts);

static void print_range(String *out, const KEY_PART_INFO *key_part, KEY_MULTI_RANGE *range, uint n_key_parts);

static void print_range_for_non_indexed_field(String *out, Field *field, KEY_MULTI_RANGE *range);
...
class SEL_IMERGE : public Sql_alloc
{
 ...
 public:
  ...
  int and_sel_tree(RANGE_OPT_PARAM *param, SEL_TREE *tree, SEL_IMERGE *new_imerge);
  int or_sel_tree_with_checks(RANGE_OPT_PARAM *param, uint n_init_trees, SEL_TREE *new_tree, bool is_first_check_pass,
                              bool *is_last_check_pass);
  int or_sel_imerge_with_checks(RANGE_OPT_PARAM *param, uint n_init_trees, SEL_IMERGE *imerge, bool is_first_check_pass,
                                bool *is_last_check_pass);
};
...
{
  for (SEL_TREE **or_tree = trees, **bound = trees_next; or_tree != bound; or_tree++)
  {
    key_map common_keys;
...
int SEL_IMERGE::and_sel_tree(RANGE_OPT_PARAM *param, SEL_TREE *tree, SEL_IMERGE *new_imerge)
{
  for (SEL_TREE **or_tree = trees; or_tree != trees_next; or_tree++)
  {
    SEL_TREE *res_or_tree = 0;
    SEL_TREE *and_tree = 0;
    if (!(res_or_tree = new SEL_TREE(param->mem_root, param->keys)) || !(and_tree = new SEL_TREE(tree, TRUE, param)))
      return (-1);
...
int SEL_IMERGE::or_sel_tree_with_checks(RANGE_OPT_PARAM *param, uint n_trees, SEL_TREE *tree, bool is_first_check_pass,
                                        bool *is_last_check_pass)
{
  bool was_ored = FALSE;
  *is_last_check_pass = is_first_check_pass;
  ...
  for (uint i = 0; i < n_trees; i++, or_tree++)
  {
    ...
    {
      bool must_be_ored = sel_trees_must_be_ored(param, *or_tree, tree, ored_keys);
    ...
    if (result)
    {
      ...
      if ((result->type == SEL_TREE::MAYBE) || (result->type == SEL_TREE::ALWAYS))
        return 1;
  ...
  if (is_first_check_pass && !*is_last_check_pass && !(tree = new SEL_TREE(tree, FALSE, param)))
    return (-1);
...
int SEL_IMERGE::or_sel_imerge_with_checks(RANGE_OPT_PARAM *param, uint n_trees, SEL_IMERGE *imerge,
                                          bool is_first_check_pass, bool *is_last_check_pass)
{
  *is_last_check_pass = TRUE;
  ...
  for (; tree < tree_end; tree++)
  {
    uint rc;
    bool is_last = TRUE;
    rc = or_sel_tree_with_checks(param, n_trees, *tree, is_first_check_pass, &is_last);
    if (!is_last)
      *is_last_check_pass = FALSE;
    if (rc)
      return rc;
...
SEL_IMERGE::SEL_IMERGE(SEL_IMERGE *arg, uint cnt, RANGE_OPT_PARAM *param) : Sql_alloc()
{
  ...
  for (SEL_TREE **tree = trees, **arg_tree = arg->trees; tree < trees_next; tree++, arg_tree++)
  {
...
inline void imerge_list_and_list(List<SEL_IMERGE> *im1, List<SEL_IMERGE> *im2) { im1->append(im2); }
...
int imerge_list_or_list(RANGE_OPT_PARAM *param, List<SEL_IMERGE> *im1, List<SEL_IMERGE> *im2)
{
  ...
  rc = imerge->or_sel_imerge_with_checks(param, elems, im2->head(), TRUE, &is_last_check_pass);
  if (rc)
  {
    if (rc == 1)
    {
  ...
  if (!is_last_check_pass)
  {
    ...
    {
      is_last_check_pass = TRUE;
      rc = new_imerge->or_sel_imerge_with_checks(param, elems, im2->head(), FALSE, &is_last_check_pass);
      if (!rc)
...
static int imerge_list_or_tree(RANGE_OPT_PARAM *param, List<SEL_IMERGE> *merges, SEL_TREE *tree)
{
  SEL_IMERGE *imerge;
  ...
  while ((imerge = it++))
  {
    ...
    {
      uint elems = (uint)(imerge->trees_next - imerge->trees);
      rc = imerge->or_sel_tree_with_checks(param, elems, or_tree, TRUE, &is_last_check_pass);
      if (!is_last_check_pass)
      {
        SEL_IMERGE *new_imerge = new (mem_root) SEL_IMERGE(imerge, elems, param);
        if (new_imerge)
        {
          rc1 = new_imerge->or_sel_tree_with_checks(param, elems, or_tree, FALSE, &is_last_check_pass);
          if (!rc1)
...
static int imerge_list_and_tree(RANGE_OPT_PARAM *param, List<SEL_IMERGE> *merges, SEL_TREE *tree, bool replace)
{
  SEL_IMERGE *imerge;
  ...
  while ((imerge = it++))
  {
    ...
    if (imerge->have_common_keys(param, tree) && new_imerge && !imerge->and_sel_tree(param, tree, new_imerge))
    {
...
SQL_SELECT *make_select(TABLE *head, table_map const_tables, table_map read_tables, COND *conds, SORT_INFO *filesort,
      ...
      dont_free(0)
{
  ...
  if (!no_alloc && !parent_alloc)
  {
    ...
    init_sql_alloc(key_memory_quick_range_select_root, &alloc, thd->variables.range_alloc_block_size, 0,
  ...
  my_init_dynamic_array2(PSI_INSTRUMENT_ME, &ranges, sizeof(QUICK_RANGE *), thd->alloc(sizeof(QUICK_RANGE *) * 16), 16,
...
bool QUICK_INDEX_SORT_SELECT::push_quick_back(QUICK_RANGE_SELECT *quick_sel_range)
{
    ...
    : cpk_quick(NULL), thd(thd_param), need_to_fetch_row(retrieve_full_rows), scans_inited(FALSE)
{
  ...
  last_rowid = (uchar *)alloc_root(parent_alloc ? parent_alloc : &alloc, head->file->ref_length);
}
...
int QUICK_RANGE_SELECT::init_ror_merged_scan(bool reuse_handler, MEM_ROOT *local_alloc)
{
...
/*
  ...
  RETURN
    ...
    other error code
*/
int QUICK_ROR_INTERSECT_SELECT::init_ror_merged_scan(bool reuse_handler, MEM_ROOT *local_alloc)
{
  ...
  if (need_to_fetch_row && unlikely(head->file->ha_rnd_init_with_error(false)))
  {
...
bool QUICK_ROR_INTERSECT_SELECT::push_quick_back(MEM_ROOT *local_alloc, QUICK_RANGE_SELECT *quick)
{
  QUICK_SELECT_WITH_RECORD *qr;
  if (!(qr = new QUICK_SELECT_WITH_RECORD) ||
      !(qr->key_tuple = (uchar *)alloc_root(local_alloc, quick->max_used_key_length)))
    return TRUE;
  qr->quick = quick;
...
int QUICK_ROR_UNION_SELECT::init()
{
  ...
  if (init_queue(&queue, quick_selects.elements, 0, FALSE, QUICK_ROR_UNION_SELECT_queue_cmp, (void *)this, 0, 0))
  {
...
SEL_ARG::SEL_ARG(Field *f, const uchar *min_value_arg, const uchar *max_value_arg)
    : min_flag(0),
      max_flag(0),
      maybe_flag(0),
      ...
      use_count(1),
      field(f),
      min_value((uchar *)min_value_arg),
      max_value((uchar *)max_value_arg),
      next(0),
      prev(0),
      next_key_part(0),
      color(BLACK),
...
SEL_ARG::SEL_ARG(Field *field_, uint8 part_, uchar *min_value_, uchar *max_value_, uint8 min_flag_, uint8 max_flag_,
                 uint8 maybe_flag_)
    : min_flag(min_flag_),
      max_flag(max_flag_),
...
{
 public:
  SEL_ARG_LE(const uchar *key, Field *field) : SEL_ARG(field, key, key)
  {
...
class SEL_ARG_LT : public SEL_ARG_LE
{
  ...
  */
  SEL_ARG_LT(const uchar *key, Field *field) : SEL_ARG_LE(key, field) { max_flag = NEAR_MAX; }
  /*
  ...
  */
  SEL_ARG_LT(THD *thd, const uchar *key, Field *field, Item *value) : SEL_ARG_LE(key, field)
  {
...
class SEL_ARG_GT : public SEL_ARG
{
 public:
  /*
    ...
    without any data rounding or truncation.
  */
  SEL_ARG_GT(const uchar *key, const KEY_PART *key_part, Field *field) : SEL_ARG(field, key, key)
  {
  ...
  /*
    ...
    (as some Field::store() do not report minor data changes).
  */
  SEL_ARG_GT(THD *thd, const uchar *key, const KEY_PART *key_part, Field *field, Item *value) : SEL_ARG(field, key, key)
  {
    // Don't use open ranges for partial key_segments
    if ((!(key_part->flag & HA_PART_KEY_SEG)) && (stored_field_cmp_to_item(thd, field, value) <= 0))
      min_flag = NEAR_MIN;
    max_flag = NO_MAX_RANGE;
  }
};
...
class SEL_ARG_GE : public SEL_ARG
{
 public:
  /*
    ...
    without any data rounding or truncation.
  */
  SEL_ARG_GE(const uchar *key, Field *field) : SEL_ARG(field, key, key) { max_flag = NO_MAX_RANGE; }
  /*
    Use this constructor if value->save_in_field() returned success,
    but we don't know if rounding or truncation happened
    (as some Field::store() do not report minor data changes).
  */
  SEL_ARG_GE(THD *thd, const uchar *key, const KEY_PART *key_part, Field *field, Item *value) : SEL_ARG(field, key, key)
  {
    // Don't use open ranges for partial key_segments
    if ((!(key_part->flag & HA_PART_KEY_SEG)) && (stored_field_cmp_to_item(thd, field, value) < 0))
      min_flag = NEAR_MIN;
...
SEL_ARG *SEL_ARG::clone(RANGE_OPT_PARAM *param, SEL_ARG *new_parent, SEL_ARG **next_arg)
{
  SEL_ARG *tmp;
  ...
  {
    if (!(tmp = new (param->mem_root) SEL_ARG(field, part, min_value, max_value, min_flag, max_flag, maybe_flag)))
      return 0;  // OOM
...
int SEL_ARG::sel_cmp(Field *field, uchar *a, uchar *b, uint8 a_flag, uint8 b_flag)
{
  int cmp;
...
class TABLE_READ_PLAN
{
  ...
  */
  virtual void trace_basic_info(PARAM *param, Json_writer_object *trace_object) const = 0;
};
...
class TRP_RANGE : public TABLE_READ_PLAN
{
 public:
  ...
  QUICK_SELECT_I *make_quick(PARAM *param, bool retrieve_full_rows, MEM_ROOT *parent_alloc)
  {
    ...
    QUICK_RANGE_SELECT *quick;
    if ((quick = get_quick_select(param, key_idx, key, mrr_flags, mrr_buf_size, parent_alloc)))
    {
  ...
  void trace_basic_info(PARAM *param, Json_writer_object *trace_object) const;
};

void TRP_RANGE::trace_basic_info(PARAM *param, Json_writer_object *trace_object) const
{
  ...
  trace_object->add("type", "range_scan").add("index", cur_key.name).add("rows", records);
...
void TRP_ROR_UNION::trace_basic_info(PARAM *param, Json_writer_object *trace_object) const
{
  THD *thd = param->thd;
  DBUG_ASSERT(trace_object->trace_started());
...
void TRP_INDEX_INTERSECT::trace_basic_info(PARAM *param, Json_writer_object *trace_object) const
{
  THD *thd = param->thd;
  DBUG_ASSERT(trace_object->trace_started());
...
void TRP_INDEX_MERGE::trace_basic_info(PARAM *param, Json_writer_object *trace_object) const
{
  THD *thd = param->thd;
  DBUG_ASSERT(trace_object->trace_started());
...
class TRP_GROUP_MIN_MAX : public TABLE_READ_PLAN
{
 ...
 public:
  TRP_GROUP_MIN_MAX(bool have_min_arg, bool have_max_arg, bool have_agg_distinct_arg,
                    KEY_PART_INFO *min_max_arg_part_arg, uint group_prefix_len_arg, uint used_key_parts_arg,
                    uint group_key_parts_arg, KEY *index_info_arg, uint index_arg, uint key_infix_len_arg,
                    uchar *key_infix_arg, SEL_TREE *tree_arg, SEL_ARG *index_tree_arg, uint param_idx_arg,
...
void TRP_GROUP_MIN_MAX::trace_basic_info(PARAM *param, Json_writer_object *trace_object) const
{
  THD *thd = param->thd;
  DBUG_ASSERT(trace_object->trace_started());
  ...
  if (index_tree)
  {
    trace_ranges(&trace_range, param, param_idx, index_tree, key_part);
  }
}
...
static int fill_used_fields_bitmap(PARAM *param)
{
  ...
  if (!(tmp = (my_bitmap_map *)alloc_root(param->mem_root, param->fields_bitmap_size)) ||
                                  ...
                                  bool only_single_index_range_scan)
{
  ...
  DBUG_PRINT("enter", ("keys_to_use: %lu  prev_tables: %lu  const_tables: %lu", (ulong)keys_to_use.to_ulonglong(),
  ...
  if (!keys_to_use.is_clear_all())
  {
    ...
    init_sql_alloc(key_memory_quick_range_select_root, &alloc, thd->variables.range_alloc_block_size, 0,
                   MYF(MY_THREAD_SPECIFIC));
    if (!(param.key_parts = (KEY_PART *)alloc_root(&alloc, sizeof(KEY_PART) * head->s->actual_n_key_parts(thd))) ||
    ...
    */
    ...
    for (idx = 0; idx < head->s->keys; idx++, key_info++)
    {
      ...
      {
        trace_idx_details.add("usable", false).add("cause", "not applicable");
        continue;
      }
      ...
      for (uint part = 0; part < n_key_parts; part++, key_parts++, key_part_info++)
      {
    ...
    if (!force_quick_range && !head->covering_keys.is_clear_all())
    {
      ...
      double key_read_time = (head->file->key_scan_time(key_for_use) + rows2double(records) / TIME_FOR_COMPARE);
      ...
      trace_cov.add("index", head->key_info[key_for_use].name).add("cost", key_read_time).add("chosen", chosen);
      if (!chosen)
    ...
    {
      {
        Json_writer_array trace_range_summary(thd, "setup_range_conditions");
        if (cond)
    ...
    if (tree)
    {
      ...
      */
      ...
      memcpy(&backup_keys[0], &tree->keys[0], sizeof(backup_keys[0]) * param.keys);

      remove_nonrange_trees(&param, tree);

      /* Get best 'range' plan and prepare data for making other plans */
      if (auto range_trp = get_key_scans_params(&param, tree, only_single_index_range_scan, true, best_read_time))
      {
        best_trp = range_trp;
      ...
      /*
        ...
        table deletes.
      */
      if ((thd->lex->sql_command != SQLCOM_DELETE) && optimizer_flag(thd, OPTIMIZER_SWITCH_INDEX_MERGE) &&
      ...
      */
      if (param.table->covering_keys.is_clear_all() && optimizer_flag(thd, OPTIMIZER_SWITCH_INDEX_MERGE) &&
          optimizer_flag(thd, OPTIMIZER_SWITCH_INDEX_MERGE_SORT_INTERSECT) && !only_single_index_range_scan)
      {
        if ((intersect_trp = get_best_index_intersect(&param, tree, best_read_time)))
        {
          best_trp = intersect_trp;
          best_read_time = best_trp->read_cost;
          set_if_smaller(param.table->opt_range_condition_rows, intersect_trp->records);
        }
      }

      if (optimizer_flag(thd, OPTIMIZER_SWITCH_INDEX_MERGE) && head->stat_records() != 0 &&
          !only_single_index_range_scan)
      {
        /* Try creating index_merge/ROR-union scan. */
        SEL_IMERGE *imerge;
        TABLE_READ_PLAN *best_conj_trp = NULL, *UNINIT_VAR(new_conj_trp); /* no empty index_merge lists possible */
        ...
        while ((imerge = it++))
        {
          ...
          if (new_conj_trp)
            set_if_smaller(param.table->opt_range_condition_rows, new_conj_trp->records);
          if (new_conj_trp && (!best_conj_trp || new_conj_trp->read_cost < best_conj_trp->read_cost))
          {
    ...
    */
    ...
    if (!only_single_index_range_scan)
    {
      ...
      {
        param.table->opt_range_condition_rows = MY_MIN(group_trp->records, head->stat_records());
  ...
  free_mem:
    ...
    {
      Json_writer_object trace_range_summary(thd, "chosen_range_access_summary");
      {
...
static bool create_key_parts_for_pseudo_indexes(RANGE_OPT_PARAM *param, MY_BITMAP *used_fields)
{
  Field **field_ptr;
  TABLE *table = param->table;
  uint parts = 0;
  ...
  for (field_ptr = table->field; *field_ptr; field_ptr++)
  {
    ...
    if (bitmap_is_set(used_fields, field->field_index) && is_eits_usable(field))
      parts++;
  }
  ...
  if (!(key_part = (KEY_PART *)alloc_root(param->mem_root, sizeof(KEY_PART) * parts)))
    return TRUE;
...
static double records_in_column_ranges(PARAM *param, uint idx, SEL_ARG *tree)
{
  THD *thd = param->thd;
...
static int cmp_quick_ranges(TABLE *table, uint *a, uint *b)
{
...
bool calculate_cond_selectivity_for_table(THD *thd, TABLE *table, Item **cond)
{
  ...
  QUICK_SELECT_I *quick;
  if ((quick = table->reginfo.join_tab->quick) && quick->get_type() == QUICK_SELECT_I::QS_TYPE_GROUP_MIN_MAX)
  {
  ...
  */
  ...
  my_qsort2(optimal_key_order, ranges, sizeof(optimal_key_order[0]), (qsort2_cmp)cmp_quick_ranges, table);
    ...
    {
      {
        ...
        double quick_cond_selectivity = (table->opt_range[keynr].rows / table_records);
        ...
        */
        ...
        if (i)
        {
          ...
          */
          ...
          selectivity_for_index.add("index_name", key_info->name).add("selectivity_from_index", quick_cond_selectivity);
          if (i != used_key_parts)
          {
            /*
            ...
            */
            ...
            selectivity_for_index.add("selectivity_multiplier", selectivity_mult);
          }
          /*
  ...
  */
  ...
  if (thd->variables.optimizer_use_condition_selectivity > 2 && !bitmap_is_clear_all(used_fields) &&
      thd->variables.use_stat_tables > 0 && table->stats_is_read)
  {
      ...
      if (key)
      {
        ...
        else
        {
          ...
          if (rows != DBL_MAX)
          {
            ...
            selectivity_for_column.add("selectivity_from_histogram", key->field->cond_selectivity);
          }
        }
      }
    }
    ...
    for (Field **field_ptr = table->field; *field_ptr; field_ptr++)
    {
      ...
      if (bitmap_is_set(used_fields, table_field->field_index) && table_field->cond_selectivity < 1.0)
      {
  ...
  ulong check_rows =
      MY_MIN(thd->variables.optimizer_selectivity_sampling_limit, (ulong)(table_records * SELECTIVITY_SAMPLING_SHARE));
  if (*cond && check_rows > SELECTIVITY_SAMPLING_THRESHOLD && thd->variables.optimizer_use_condition_selectivity > 4)
  {
    find_selective_predicates_list_processor_data *dt = (find_selective_predicates_list_processor_data *)alloc_root(
    ...
    if (dt->list.elements > 0)
    {
      ...
      if (check_rows > SELECTIVITY_SAMPLING_THRESHOLD)
      {
        ...
        while ((stat = it++))
        {
          ...
          DBUG_PRINT("info", ("The predicate selectivity : %g", (double)stat->positive / examined_rows));
          ...
          */
          if (stat->field_arg)
          {
            ...
            {
              for (Field *next_field = stat->field_arg->next_equal_field; next_field != stat->field_arg;
...
static int find_used_partitions_imerge(PART_PRUNE_PARAM *ppar, SEL_IMERGE *imerge);
static int find_used_partitions_imerge_list(PART_PRUNE_PARAM *ppar, List<SEL_IMERGE> &merges);
...
bool prune_partitions(THD *thd, TABLE *table, Item *pprune_cond)
{
  ...
  if (tree->merges.is_empty())
  {
    ...
    if (!tree->keys[0] || (-1 == (res = find_used_partitions(&prune_param, tree->keys[0]))))
      goto all_used;
  }
  else
  {
    if (tree->merges.elements == 1)
    {
      ...
      */
      if (-1 == (res = find_used_partitions_imerge(&prune_param, tree->merges.head())))
        goto all_used;
    }
    else
    {
      ...
      */
      if (-1 == (res = find_used_partitions_imerge_list(&prune_param, tree->merges)))
        goto all_used;
    }
  }
...
all_used:
  ...
  /*
    ...
    within that set.
  */
  bitmap_intersect(&prune_param.part_info->read_partitions, &prune_param.part_info->lock_partitions);
  /*
  ...
  */
  if (table->file->get_lock_type() == F_UNLCK && !partition_key_modified(table, table->write_set))
  {
    bitmap_copy(&prune_param.part_info->lock_partitions, &prune_param.part_info->read_partitions);
  }
...
static void store_selargs_to_rec(PART_PRUNE_PARAM *ppar, SEL_ARG **start, int num)
{
  ...
  for (SEL_ARG **end = start + num; start != end; start++)
  {
    ...
    store_key_image_to_rec(sel_arg->field, sel_arg->min_value, parts[sel_arg->part].length);
  }
}
...
static int find_used_partitions_imerge_list(PART_PRUNE_PARAM *ppar, List<SEL_IMERGE> &merges)
{
  ...
  if (!(bitmap_buf = (my_bitmap_map *)alloc_root(ppar->range_param.mem_root, bitmap_bytes)))
  {
    /*
  ...
  memcpy(ppar->part_info->read_partitions.bitmap, all_merges.bitmap, bitmap_bytes);
  return 1;
}
...
static int find_used_partitions(PART_PRUNE_PARAM *ppar, SEL_ARG *key_tree)
{
  ...
  *(ppar->arg_stack_end++) = key_tree;
  ...
  {
    if (ppar->part_info->get_part_iter_for_interval && key_tree->part <= ppar->last_part_partno)
    {
      ...
      key_tree->store_min(ppar->key[key_tree->part].store_length, &tmp_min_key, ppar->cur_min_flag);
      key_tree->store_max(ppar->key[key_tree->part].store_length, &tmp_max_key, ppar->cur_max_flag);
      uint flag;
          ...
          key_tree->next_key_part->type == SEL_ARG::KEY_RANGE)
      {
        ...
        */
        if ((tmp_min_key - min_key) == (tmp_max_key - max_key) &&
            (memcmp(min_key, max_key, (uint)(tmp_max_key - max_key)) == 0) && !key_tree->min_flag &&
            !key_tree->max_flag)
        {
          /* Set 'parameters' */
        ...
        if (!tmp_min_flag)
          key_tree->next_key_part->store_min_key(ppar->key, &tmp_min_key, &tmp_min_flag, ppar->last_part_partno, true);
        if (!tmp_max_flag)
          key_tree->next_key_part->store_max_key(ppar->key, &tmp_max_key, &tmp_max_flag, ppar->last_part_partno, false);
      ...
      */
      if (ppar->arg_stack[0]->part == 0 || ppar->part_info->part_type == VERSIONING_PARTITION)
      {
        ...
        res = ppar->part_info->get_part_iter_for_interval(
            ...
            (uint)(tmp_min_key - range_par->min_key), (uint)(tmp_max_key - range_par->max_key), flag, &ppar->part_iter);
        if (!res)
    ...
    if (key_tree_part == ppar->last_subpart_partno && (NULL != ppar->part_info->get_subpart_iter_for_interval))
    {
      PARTITION_ITERATOR subpart_iter;
      DBUG_EXECUTE("info", dbug_print_segment_range(key_tree, range_par->key_parts););
      res = ppar->part_info->get_subpart_iter_for_interval(ppar->part_info, TRUE, NULL, /* Currently not used here */
      ...
      while ((part_id = ppar->part_iter.get_next(&ppar->part_iter)) != NOT_A_PARTITION_ID)
      {
        for (uint i = 0; i < ppar->part_info->num_subparts; i++)
          if (bitmap_is_set(&ppar->subparts_bitmap, i))
            bitmap_set_bit(&ppar->part_info->read_partitions, part_id * ppar->part_info->num_subparts + i);
      }
      goto pop_and_go_right;
    }
    ...
    {
      if (key_tree_part == ppar->last_part_partno && ppar->cur_part_fields == ppar->part_fields &&
          ppar->part_info->get_part_iter_for_interval == NULL)
      {
        ...
        */
        ...
        DBUG_EXECUTE("info", dbug_print_singlepoint_range(ppar->arg_stack, ppar->part_fields););
        uint32 part_id;
        longlong func_value;
        /* Find in which partition the {const1, ...,constN} tuple goes */
        if (ppar->get_top_partition_id_func(ppar->part_info, &part_id, &func_value))
        {
      ...
      if (key_tree_part == ppar->last_subpart_partno && ppar->cur_subpart_fields == ppar->subpart_fields)
      {
        /*
          Ok, we've got "fieldN<=>constN"-type SEL_ARGs for all subpartitioning
          fields. Save all constN constants into table record buffer.
        */
        store_selargs_to_rec(ppar, ppar->arg_stack_end - ppar->subpart_fields, ppar->subpart_fields);
        DBUG_EXECUTE("info",
                     dbug_print_singlepoint_range(ppar->arg_stack_end - ppar->subpart_fields, ppar->subpart_fields););
        ...
        while ((part_id = ppar->part_iter.get_next(&ppar->part_iter)) != NOT_A_PARTITION_ID)
        {
          bitmap_set_bit(&part_info->read_partitions, part_id * part_info->num_subparts + subpart_id);
        }
...
static bool create_partition_index_description(PART_PRUNE_PARAM *ppar)
{
  ...
  used_part_fields = fields_ok_for_partition_index(part_info->part_field_array) ? part_info->num_part_fields : 0;
  used_subpart_fields =
      fields_ok_for_partition_index(part_info->subpart_field_array) ? part_info->num_subpart_fields : 0;
  ...
  ppar->last_subpart_partno = used_subpart_fields ? (int)(used_part_fields + used_subpart_fields - 1) : -1;
  ...
  if (!total_parts || !(key_part = (KEY_PART *)alloc_root(alloc, sizeof(KEY_PART) * total_parts)) ||
      !(ppar->arg_stack = (SEL_ARG **)alloc_root(alloc, sizeof(SEL_ARG *) * total_parts)) ||
      !(ppar->is_part_keypart = (my_bool *)alloc_root(alloc, sizeof(my_bool) * total_parts)) ||
      !(ppar->is_subpart_keypart = (my_bool *)alloc_root(alloc, sizeof(my_bool) * total_parts)))
    return TRUE;
  ...
  Field **field = (ppar->part_fields) ? part_info->part_field_array : part_info->subpart_field_array;
  ...
  for (uint part = 0; part < total_parts; part++, key_part++)
  {
    ...
    DBUG_PRINT("info", ("part %u length %u store_length %u", part, key_part->length, key_part->store_length));
  ...
  DBUG_EXECUTE("info", print_partitioning_index(range_par->key_parts, range_par->key_parts_end););
  return FALSE;
}
...
double get_sweep_read_cost(const PARAM *param, ha_rows records)
{
  ...
  if (param->table->file->pk_is_clustering_key(pk) || param->table->file->stats.block_size == 0 /* HEAP */)
  {
    /*
  ...
  else
  {
    /*
      ...
      cost for this.
    */
    double n_blocks = ceil(ulonglong2double(param->table->file->stats.data_file_length) / IO_SIZE);
...
static TABLE_READ_PLAN *get_best_disjunct_quick(PARAM *param, SEL_IMERGE *imerge, double read_time, bool named_trace)
{
  SEL_TREE **ptree;
  ...
  */
  for (ptree = imerge->trees, cur_child = range_scans; ptree != imerge->trees_next; ptree++, cur_child++)
  {
    ...
    trace_idx.add("index_to_merge", param->table->key_info[keynr_in_table].name).add("cumulated_cost", imerge_cost);
  }
  ...
  if (imerge_too_expensive || (imerge_cost > read_time) ||
      ((non_cpk_scan_records + cpk_scan_records >= param->table->stat_records()) && read_time != DBL_MAX))
  {
    /*
  ...
  */
  if (all_scans_rors && optimizer_flag(param->thd, OPTIMIZER_SWITCH_INDEX_MERGE_UNION))
  {
    roru_read_plans = (TABLE_READ_PLAN **)range_scans;
    trace_best_disjunct.add("use_roworder_union", true).add("cause", "always cheaper than non roworder retrieval");
  ...
  if (cpk_scan)
  {
     ...
     */
    double rid_comp_cost = (rows2double(non_cpk_scan_records) / TIME_FOR_COMPARE_ROWID);
    imerge_cost += rid_comp_cost;
    trace_best_disjunct.add("cost_of_mapping_rowid_in_non_clustered_pk_scan", rid_comp_cost);
  }
  ...
  if (imerge_cost > read_time || !optimizer_flag(param->thd, OPTIMIZER_SWITCH_INDEX_MERGE_SORT_UNION))
  {
  ...
  unique_calc_buff_size = Unique::get_cost_calc_buff_size((ulong)non_cpk_scan_records, param->table->file->ref_length,
...

  {
    const double dup_removal_cost =
        Unique::get_use_cost(param->imerge_cost_buff, (uint)non_cpk_scan_records, param->table->file->ref_length,
                             (size_t)param->thd->variables.sortbuff_size, TIME_FOR_COMPARE_ROWID, FALSE, NULL);
    imerge_cost += dup_removal_cost;
    trace_best_disjunct.add("cost_duplicate_removal", dup_removal_cost).add("total_cost", imerge_cost);
  }
  ...
  if (imerge_cost < read_time)
  {
    if ((imerge_trp = new (param->mem_root) TRP_INDEX_MERGE))
    {
      ...
      imerge_trp->records = MY_MIN(imerge_trp->records, param->table->stat_records());
    ...
    if (imerge_trp)
    {
      TABLE_READ_PLAN *trp = merge_same_index_scans(param, imerge, imerge_trp, limit_read_time);
      if (trp != imerge_trp)
        DBUG_RETURN(trp);
    }
  }
...
skip_to_ror_scan:
       ...
       ptree++, cur_child++, cur_roru_plan++)
  {
    ...
    */
    ...
    roru_intersect_part *= (*cur_roru_plan)->records / param->table->stat_records();
  }
  ...
  */
  roru_total_records -= (ha_rows)(roru_intersect_part * param->table->stat_records());
  ...
  */
  ...
  DBUG_PRINT("info", ("ROR-union: cost %g, %zu members", roru_total_cost, n_child_scans));
  trace_best_disjunct.add("index_roworder_union_cost", roru_total_cost).add("members", n_child_scans);
  TRP_ROR_UNION *roru;
                                               ...
                                               double read_time)
{
  ...
  for (tree = imerge->trees, cur_child = imerge_trp->range_scans; tree != imerge->trees_next; tree++, cur_child++)
  {
    DBUG_ASSERT(tree);
    ...
    else
    {
      ...
      for (uint i = 0; i < param->keys; i++) (*changed_tree)->keys[i] = NULL;
      (*changed_tree)->keys_map.clear_all();
      if (key)
        key->incr_refs();
      if ((*tree)->keys[key_idx])
        (*tree)->keys[key_idx]->incr_refs();
      if (((*changed_tree)->keys[key_idx] = key_or_with_limit(param, key_idx, key, (*tree)->keys[key_idx])))
...
static bool same_index_prefix(KEY *key1, KEY *key2, uint used_parts)
{
...
static bool create_fields_bitmap(PARAM *param, MY_BITMAP *fields_bitmap)
{
  my_bitmap_map *bitmap_buf;

  if (!(bitmap_buf = (my_bitmap_map *)alloc_root(param->mem_root, param->fields_bitmap_size)))
    return TRUE;
...
static int cmp_intersect_index_scan(INDEX_SCAN_INFO **a, INDEX_SCAN_INFO **b)
{
  return (*a)->records < (*b)->records ? -1 : (*a)->records == (*b)->records ? 0 : 1;
}

static inline void set_field_bitmap_for_index_prefix(MY_BITMAP *field_bitmap, KEY_PART_INFO *key_part,
                                                     uint used_key_parts)
{
  bitmap_clear_all(field_bitmap);
  for (KEY_PART_INFO *key_part_end = key_part + used_key_parts; key_part < key_part_end; key_part++)
  {
...
static inline ha_rows get_table_cardinality_for_index_intersect(TABLE *table)
{
...
static void print_keyparts(THD *thd, KEY *key, uint key_parts)
{
  DBUG_ASSERT(thd->trace_started());

  KEY_PART_INFO *part = key->key_part;
  Json_writer_array keyparts(thd, "keyparts");
  for (uint i = 0; i < key_parts; i++, part++) keyparts.add(part->field->field_name);
}

static ha_rows records_in_index_intersect_extension(PARTIAL_INDEX_INTERSECT_INFO *curr,
...
static bool prepare_search_best_index_intersect(PARAM *param, SEL_TREE *tree, COMMON_INDEX_INTERSECT_INFO *common,
                                                PARTIAL_INDEX_INTERSECT_INFO *init, double cutoff_cost)
{
  uint i;
  uint n_search_scans;
  double cost;
  ...
  common->table_cardinality = get_table_cardinality_for_index_intersect(table);
  ...
  if (!(common->search_scans = (INDEX_SCAN_INFO **)alloc_root(param->mem_root, sizeof(INDEX_SCAN_INFO *) * i)))
    return TRUE;
  ...
  for (i = 0, index_scan = tree->index_scans; i < n_index_scans; i++, index_scan++)
  {
    ...
    {
      idx_scan.add("chosen", "false").add("cause", "clustered index used for filtering");
      continue;
    }
  ...
  if (!(common->best_intersect = (INDEX_SCAN_INFO **)alloc_root(
            param->mem_root, sizeof(INDEX_SCAN_INFO *) * (i + MY_TEST(cpk_scan != NULL)))))
    return TRUE;

  size_t calc_cost_buff_size =
      Unique::get_cost_calc_buff_size((size_t)records_in_scans, common->key_size, common->max_memory_size);
  if (!(common->buff_elems = (uint *)alloc_root(param->mem_root, calc_cost_buff_size)))
    return TRUE;

  my_qsort(selected_index_scans, n_search_scans, sizeof(INDEX_SCAN_INFO *), (qsort_cmp)cmp_intersect_index_scan);

  Json_writer_array selected_idx_scans(thd, "selected_index_scans");
  if (cpk_scan)
  {
    PARTIAL_INDEX_INTERSECT_INFO curr;
    set_field_bitmap_for_index_prefix(&cpk_scan->used_fields, cpk_scan->key_info->key_part, cpk_scan->used_key_parts);
    ...
    for (scan_ptr = selected_index_scans; *scan_ptr; scan_ptr++)
    {
      ...
      (*scan_ptr)->filtered_out = records >= scan_records ? 0 : scan_records - records;
      if (thd->trace_started())
      {
        ...
        selected_idx.add("records", (*scan_ptr)->records).add("filtered_records", (*scan_ptr)->filtered_out);
      }
    }
  }
  else
...
static ha_rows records_in_index_intersect_extension(PARTIAL_INDEX_INTERSECT_INFO *curr, INDEX_SCAN_INFO *ext_index_scan)
{
  ...
  if (i)
  {
    ...
    if (ext_records < table_cardinality)
    {
      ...
      records = (ha_rows)((double)records / table_cardinality * ext_records);
      bitmap_copy(used_fields, curr_intersect_fields);
      key_part = key_info->key_part;
      for (uint j = 0; j < used_key_parts; j++, key_part++) bitmap_set_bit(used_fields, key_part->fieldnr - 1);
    }
  }
  return !better_selectivity ? records + 1 : !records ? 1 : records;
}
...
static inline double get_cpk_filter_cost(ha_rows filtered_records, INDEX_SCAN_INFO *cpk_scan, double compare_factor)
{
  return log((double)(cpk_scan->range_count + 1)) / (compare_factor * M_LN2) * filtered_records;
}
...
static bool check_index_intersect_extension(PARTIAL_INDEX_INTERSECT_INFO *curr, INDEX_SCAN_INFO *ext_index_scan,
                                            PARTIAL_INDEX_INTERSECT_INFO *next)
{
  ...
  if (idx && next->in_memory)
  {
    ...
    */
    ha_rows elems_in_tree = common_info->search_scans[0]->records - common_info->search_scans[0]->filtered_out;
    next->in_memory_cost +=
        Unique::get_search_cost(elems_in_tree, common_info->compare_factor) * ext_index_scan_records;
  ...
  else
  {
    ...
    cost = Unique::get_use_cost(buff_elems, (size_t)records_sent_to_unique, key_size, max_memory_size, compare_factor,
                                TRUE, &next->in_memory);
    if (records_filtered_out_by_cpk)
    {
      ...
      cost2 = Unique::get_use_cost(buff_elems, (size_t)records2, key_size, max_memory_size, compare_factor, TRUE,
                                   &in_memory2);
      cost2 += get_cpk_filter_cost(ext_index_scan_records, common_info->cpk_scan, compare_factor);
  ...
  if (next->use_cpk_filter)
  {
    ...
    bitmap_union(&ext_index_scan->used_fields, &common_info->cpk_scan->used_fields);
  }
...
static void find_index_intersect_best_extension(PARTIAL_INDEX_INTERSECT_INFO *curr)
{
  PARTIAL_INDEX_INTERSECT_INFO next;
  COMMON_INDEX_INTERSECT_INFO *common_info = curr->common_info;
  ...
  for (INDEX_SCAN_INFO **index_scan_ptr = rem_first_index_scan_ptr; *index_scan_ptr; index_scan_ptr++)
  {
...
static TRP_INDEX_INTERSECT *get_best_index_intersect(PARAM *param, SEL_TREE *tree, double read_time)
{
  ...
  if (prepare_search_best_index_intersect(param, tree, &common, &init, read_time))
    DBUG_RETURN(NULL);
  ...
  if (!(range_scans = (TRP_RANGE **)alloc_root(param->mem_root, sizeof(TRP_RANGE *) * count)))
    DBUG_RETURN(NULL);

  for (i = 0, cur_range = range_scans; i < count; i++)
  {
    index_scan = common.best_intersect[i];
    if ((*cur_range = new (param->mem_root) TRP_RANGE(index_scan->sel_arg, index_scan->idx, 0)))
    {
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if (!table->intersect_keys.is_set(index_scan->keynr))
    {
      for (uint j = 0; j < common.best_length; j++)
      {
        ...
        if (same_index_prefix(index_scan->key_info, scan->key_info, scan->used_key_parts))
        {
          table->intersect_keys.set_bit(index_scan->keynr);
          break;
        }
  ...
  if ((intersect_trp = new (param->mem_root) TRP_INDEX_INTERSECT))
  {
    ...
    trace_idx_interect.add("rows", intersect_trp->records).add("cost", intersect_trp->read_cost).add("chosen", true);
  }
...
void TRP_ROR_INTERSECT::trace_basic_info(PARAM *param, Json_writer_object *trace_object) const
{
  THD *thd = param->thd;
  DBUG_ASSERT(trace_object->trace_started());
  ...
  for (ROR_SCAN_INFO **cur_scan = first_scan; cur_scan != last_scan; cur_scan++)
  {
    ...
    trace_ranges(&trace_range, param, (*cur_scan)->idx, (*cur_scan)->sel_arg, key_part);
  }
}
...
static ROR_SCAN_INFO *make_ror_scan(const PARAM *param, int idx, SEL_ARG *sel_arg)
{
  ...
  if (my_bitmap_init(&ror_scan->covered_fields, bitmap_buf, param->table->s->fields))
    DBUG_RETURN(NULL);
  bitmap_clear_all(&ror_scan->covered_fields);

  KEY_PART_INFO *key_part = param->table->key_info[keynr].key_part;
  KEY_PART_INFO *key_part_end = key_part + param->table->key_info[keynr].user_defined_key_parts;
  for (; key_part != key_part_end; ++key_part)
  {
    if (bitmap_is_set(&param->needed_fields, key_part->fieldnr - 1))
      bitmap_set_bit(&ror_scan->covered_fields, key_part->fieldnr - 1);
  }
  ror_scan->index_read_cost = param->table->file->keyread_time(ror_scan->keynr, 1, ror_scan->records);
...
static ROR_INTERSECT_INFO *ror_intersect_init(const PARAM *param)
{
  ...
  if (!(info = (ROR_INTERSECT_INFO *)alloc_root(param->mem_root, sizeof(ROR_INTERSECT_INFO))))
    return NULL;
  info->param = param;
  if (!(buf = (my_bitmap_map *)alloc_root(param->mem_root, param->fields_bitmap_size)))
    return NULL;
...
{
  dst->param = src->param;
  memcpy(dst->covered_fields.bitmap, src->covered_fields.bitmap, no_bytes_in_map(&src->covered_fields));
...
static double ror_scan_selectivity(const ROR_INTERSECT_INFO *info, const ROR_SCAN_INFO *scan)
{
  ...
  bool cur_covered;
  bool prev_covered = MY_TEST(bitmap_is_set(&info->covered_fields, key_part->fieldnr - 1));
  key_range min_range;
  key_range max_range;
  ...
  for (sel_arg = scan->sel_arg; sel_arg; sel_arg = sel_arg->next_key_part)
  {
    ...
    cur_covered = MY_TEST(bitmap_is_set(&info->covered_fields, key_part[sel_arg->part].fieldnr - 1));
    if (cur_covered != prev_covered)
    {
      ...
      records = (info->param->table->file->records_in_range(scan->keynr, &min_range, &max_range, &pages));
      if (cur_covered)
      {
        /* uncovered -> covered */
  ...
  if (!prev_covered)
  {
    double tmp = rows2double(info->param->quick_rows[scan->keynr]) / rows2double(prev_records);
    DBUG_PRINT("info", ("Selectivity multiplier: %g", tmp));
    selectivity_mult *= tmp;
  }
...
static bool ror_intersect_add(ROR_INTERSECT_INFO *info, ROR_SCAN_INFO *ror_scan, Json_writer_object *trace_costs,
                              bool is_cpk_scan)
{
  double selectivity_mult = 1.0;
  ...
  DBUG_PRINT("info", ("Adding scan on %s", info->param->table->key_info[ror_scan->keynr].name.str));
  ...
  if (!info->is_covering)
  {
    double sweep_cost = get_sweep_read_cost(info->param, double2rows(info->out_rows));
                                                 ...
                                                 bool *are_all_covering)
{
  ...
  */
  ...
  if (!(tree->ror_scans = (ROR_SCAN_INFO **)alloc_root(param->mem_root, sizeof(ROR_SCAN_INFO *) * param->keys)))
    return NULL;
  cpk_no =
      (param->table->file->pk_is_clustering_key(param->table->s->primary_key) ? param->table->s->primary_key : MAX_KEY);
  ...
  for (idx = 0, cur_ror_scan = tree->ror_scans; idx < param->keys; idx++)
  {
    ...
    if (key_no != cpk_no && param->table->file->index_flags(key_no, 0, 0) & HA_CLUSTERED_INDEX)
    {
  ...
  */
  ...
  {
    Json_writer_object trace_idx(thd);
    trace_idx.add("index", param->table->key_info[(*cur_ror_scan)->keynr].name);

    /* S= S + first(R);  R= R - first(R); */
    if (!ror_intersect_add(intersect, *cur_ror_scan, &trace_idx, FALSE))
    {
      trace_idx.add("usable", false).add("cause", "does not reduce cost of intersect");
      cur_ror_scan++;
      continue;
    }
    ...
    *(intersect_scans_end++) = *(cur_ror_scan++);
    ...
    {
      trace_idx.add("chosen", false).add("cause", "does not reduce cost");
    }
  }
  trace_isect_idx.end();
  ...
  if (intersect_scans_best == intersect_scans)
  {
    ...
    trace_ror.add("chosen", false).add("cause", "does not increase selectivity");
    DBUG_RETURN(NULL);
  }
  ...
  */
  ...
  {
    if (ror_intersect_add(intersect, cpk_scan, &trace_cpk, TRUE) && (intersect->total_cost < min_cost))
    {
      trace_cpk.add("clustered_pk_scan_added_to_intersect", true).add("cumulated_cost", intersect->total_cost);
      intersect_best = intersect;  // just set pointer here
    }
    else
    {
      trace_cpk.add("clustered_pk_added_to_intersect", false).add("cause", "cost");
  ...
  else
  {
    trace_cpk.add("clustered_pk_added_to_intersect", false)
        .add("cause", cpk_scan ? "roworder is covering" : "no clustered pk index");
    cpk_scan = 0;  // Don't use cpk_scan
  }
  trace_cpk.end();
  ...
  {
    trace_ror.add("chosen", false).add("cause", (read_time > min_cost) ? "too few indexes to merge" : "cost");
  }
  DBUG_RETURN(trp);
}
...
static TRP_ROR_INTERSECT *get_best_covering_ror_intersect(PARAM *param, SEL_TREE *tree, double read_time)
{
  ...
  for (ROR_SCAN_INFO **scan = tree->ror_scans; scan != ror_scans_end; ++scan)
    (*scan)->key_components = param->table->key_info[(*scan)->keynr].user_defined_key_parts;
  ...
  */
  ...
  if (!covered_fields->bitmap)
    covered_fields->bitmap = (my_bitmap_map *)alloc_root(param->mem_root, param->fields_bitmap_size);
  if (!covered_fields->bitmap || my_bitmap_init(covered_fields, covered_fields->bitmap, param->table->s->fields))
    DBUG_RETURN(0);
  ...
  do
  {
    ...
    */
    ...
    DBUG_PRINT("info", ("Adding scan on %s", param->table->key_info[(*ror_scan_mark)->keynr].name.str));
                                       ...
                                       bool for_range_access, double read_time)
{
  ...
  */
  ...
  {
    tree->index_scans = (INDEX_SCAN_INFO **)alloc_root(param->mem_root, sizeof(INDEX_SCAN_INFO *) * param->keys);
  }
  ...
  for (idx = 0; idx < param->keys; idx++)
  {
    ...
    if (key)
    {
      ...
      bool read_index_only = index_read_must_be_used ? TRUE : (bool)param->table->covering_keys.is_set(keynr);

      Json_writer_object trace_idx(thd);
      trace_idx.add("index", param->table->key_info[keynr].name);

      found_records = check_quick_select(param, idx, read_index_only, key, for_range_access, &mrr_flags, &buf_size,
                                         &cost, &is_ror_scan);

      if (!for_range_access && !is_ror_scan && !optimizer_flag(param->thd, OPTIMIZER_SWITCH_INDEX_MERGE_SORT_UNION))
      {
      ...
      if (found_records != HA_POS_ERROR && tree->index_scans &&
          (index_scan = (INDEX_SCAN_INFO *)alloc_root(param->mem_root, sizeof(INDEX_SCAN_INFO))))
      {
        Json_writer_array trace_range(thd, "ranges");
      ...
      if (found_records != HA_POS_ERROR && read_time > (found_read_time = cost.total_cost()))
      {
  ...
  if (key_to_read)
  {
    if ((read_plan = new (param->mem_root) TRP_RANGE(key_to_read, best_idx, best_mrr_flags)))
    {
      ...
      DBUG_PRINT("info", ("Returning range plan for key %s, cost %g, records %lu",
                          param->table->key_info[param->real_keynr[best_idx]].name.str, read_plan->read_cost,
...
QUICK_SELECT_I *TRP_INDEX_INTERSECT::make_quick(PARAM *param, bool retrieve_full_rows, MEM_ROOT *parent_alloc)
{
  ...
  for (TRP_RANGE **range_scan = range_scans; range_scan != range_scans_end; range_scan++)
  {
    if (!(quick = (QUICK_RANGE_SELECT *)((*range_scan)->make_quick(param, FALSE, &quick_intersect->alloc))) ||
...
QUICK_SELECT_I *TRP_ROR_INTERSECT::make_quick(PARAM *param, bool retrieve_full_rows, MEM_ROOT *parent_alloc)
{
                                                       ...
                                                       (retrieve_full_rows ? (!is_covering) : FALSE), parent_alloc)))
  {
    ...
    for (ROR_SCAN_INFO **curr_scan = first_scan; curr_scan != last_scan; ++curr_scan)
    {
      if (!(quick = get_quick_select(param, (*curr_scan)->idx, (*curr_scan)->sel_arg,
                                     HA_MRR_USE_DEFAULT_IMPL | HA_MRR_SORTED, 0, alloc)) ||
    ...
    if (cpk_scan)
    {
      if (!(quick = get_quick_select(param, cpk_scan->idx, cpk_scan->sel_arg, HA_MRR_USE_DEFAULT_IMPL | HA_MRR_SORTED,
                                     0, alloc)))
      {
        delete quick_intrsect;
        DBUG_RETURN(NULL);
...
SEL_TREE *Item_bool_func::get_ne_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *lt_value, Item *gt_value)
{
  SEL_TREE *tree;
  tree = get_mm_parts(param, field, Item_func::LT_FUNC, lt_value);
  if (tree)
    tree = tree_or(param, tree, get_mm_parts(param, field, Item_func::GT_FUNC, gt_value));
  return tree;
}

SEL_TREE *Item_func_ne::get_func_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *value)
{
...
SEL_TREE *Item_func_between::get_func_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *value)
{
  SEL_TREE *tree;
  ...
  if (!value)
  {
    ...
    else
    {
      ...
      if (tree)
      {
        tree = tree_and(param, tree, get_mm_parts(param, field, Item_func::LE_FUNC, args[2]));
      }
    }
  }
  else
  {
    tree = get_mm_parts(param, field,
                        (negated ? (value == (Item *)1 ? Item_func::GT_FUNC : Item_func::LT_FUNC)
                                 : (value == (Item *)1 ? Item_func::LE_FUNC : Item_func::GE_FUNC)),
                        args[0]);
  }
  DBUG_RETURN(tree);
}

SEL_TREE *Item_func_in::get_func_mm_tree(RANGE_OPT_PARAM *param, Field *field, Item *value)
{
  SEL_TREE *tree = 0;
...
#define NOT_IN_IGNORE_THRESHOLD 1000
      ...
      for (; i < array->used_count; i++)
      {
        if (array->compare_elems(i, i - 1))
        {
          ...
          for (uint idx = 0; idx < param->keys; idx++)
          {
            ...
            if (((new_interval = tree2->keys[idx])) && (tree->keys[idx]) && ((last_val = tree->keys[idx]->last())))
            {
    ...
    else
    {
      ...
      if (tree)
      {
        ...
        {
          tree = tree_and(param, tree, get_ne_mm_tree(param, field, *arg, *arg));
        }
      }
    }
  }
  else
  {
    ...
    if (tree)
    {
      ...
      for (arg = args + 2, end = arg + arg_count - 2; arg < end; arg++)
      {
        tree = tree_or(param, tree, get_mm_parts(param, field, Item_func::EQ_FUNC, *arg));
      }
    }
  }
  DBUG_RETURN(tree);
...
SEL_TREE *Item_func_in::get_func_row_mm_tree(RANGE_OPT_PARAM *param, Item_row *key_row)
{
  ...
  for (uint i = 0; i < row_cols; i++, key_col_ptr++)
  {
    ...
    else if (!(key_col->used_tables() & (param_comp | param->current_table)) && !key_col->is_expensive())
    {
    ...
    if (col_comparators)
    {
      ...
      for (i = 0; i < row_cols; i++)
      {
        ...
        if (key_col_info->comparator)
        {
          ...
          if (!(arg_col->used_tables() & (param_comp | param->current_table)) && !arg_col->is_expensive() &&
    ...
    for (uint i = 0; i < row_cols; i++, arg_col_ptr++)
    {
      ...
      if (!(arg_col->used_tables() & (param_comp | param->current_table)) && !arg_col->is_expensive())
      {
        and_tree = tree_and(param, and_tree,
                            get_mm_parts(param, key_col_info->field, Item_func::EQ_FUNC, arg_col->real_item()));
      }
    }
    if (!and_tree)
    {
  ...
  if (omitted_tuples == argument_count() - 1)
  {
    ...
    res_tree = new (param->mem_root) SEL_TREE(SEL_TREE::IMPOSSIBLE, param->mem_root, param->keys);
  }
...
SEL_TREE *Item_bool_func::get_full_func_mm_tree(RANGE_OPT_PARAM *param, Item_field *field_item, Item *value)
{
...
SEL_TREE *Item_cond::get_mm_tree(RANGE_OPT_PARAM *param, Item **cond_ptr)
{
  ...
  if (tree)
  {
    if (tree->type == SEL_TREE::IMPOSSIBLE && param->remove_false_where_parts)
    {
    ...
    while ((item = li++))
    {
      ...
      if (new_tree && new_tree->type == SEL_TREE::IMPOSSIBLE && param->remove_false_where_parts)
      {
        /*
...
SEL_TREE *Item::get_mm_tree(RANGE_OPT_PARAM *param, Item **cond_ptr)
{
  ...
  */
  ...
  if ((ref_tables & param->current_table) || (ref_tables & ~(param->prev_tables | param->read_tables)))
    DBUG_RETURN(0);
  DBUG_RETURN(new (param->mem_root) SEL_TREE(SEL_TREE::MAYBE, param->mem_root, param->keys));
}

SEL_TREE *Item_func_between::get_mm_tree(RANGE_OPT_PARAM *param, Item **cond_ptr)
{
  ...
  */
  for (uint i = 1; i < arg_count; i++)
  {
    if (arguments()[i]->real_item()->type() == Item::FIELD_ITEM)
    {
      ...
      SEL_TREE *tmp = get_full_func_mm_tree(param, field_item, (Item *)(intptr)i);
      if (negated)
      {
...
SEL_TREE *Item_func_in::get_mm_tree(RANGE_OPT_PARAM *param, Item **cond_ptr)
{
  ...
  switch (key_item()->real_item()->type())
  {
    case Item::FIELD_ITEM:
      tree = get_full_func_mm_tree(param, (Item_field *)(key_item()->real_item()), NULL);
      break;
    case Item::ROW_ITEM:
      tree = get_func_row_mm_tree(param, (Item_row *)(key_item()->real_item()));
      break;
    default:
      DBUG_RETURN(0);
  }
...
static bool is_field_an_unique_index(RANGE_OPT_PARAM *param, Field *field)
{
  ...
  if (param->using_real_indexes)
  {
    ...
    while ((key_no = it++) != key_map::Iterator::BITMAP_END)
    {
      ...
      if (key_info->user_defined_key_parts == 1 && (key_info->flags & HA_NOSAME))
      {
        DBUG_RETURN(true);
      }
    }
...
SEL_TREE *Item_bool_func::get_mm_parts(RANGE_OPT_PARAM *param, Field *field, Item_func::Functype type, Item *value)
{
  ...
  if (value && (value_used_tables = value->used_tables()) & ~(param->prev_tables | param->read_tables))
    DBUG_RETURN(0);
  for (; key_part != end; key_part++)
  {
    ...
    {
      SEL_ARG *sel_arg = 0;
      if (!tree && !(tree = new (param->thd->mem_root) SEL_TREE(param->mem_root, param->keys)))
        DBUG_RETURN(0);  // OOM
...
SEL_ARG *Item_func_null_predicate::get_mm_leaf(RANGE_OPT_PARAM *param, Field *field, KEY_PART *key_part,
...
SEL_ARG *Item_func_like::get_mm_leaf(RANGE_OPT_PARAM *param, Field *field, KEY_PART *key_part, Item_func::Functype type,
                                     Item *value)
{
  ...
  if (param->using_real_indexes && !field->optimize_range(param->real_keynr[key_part->key], key_part->part))
    DBUG_RETURN(0);

  if (field->result_type() == STRING_RESULT && field->charset() != compare_collation())
    DBUG_RETURN(0);
  ...
  if (field->cmp_type() != STRING_RESULT || field->type_handler() == &type_handler_enum ||
  ...
  */
  ...
  if (field->charset()->like_range(res->ptr(), res->length(), escape, wild_one, wild_many, field_length,
                                   (char *)min_str + offset, (char *)max_str + offset, &min_length, &max_length))
...
SEL_ARG *Item_bool_func::get_mm_leaf(RANGE_OPT_PARAM *param, Field *field, KEY_PART *key_part,
                                     Item_func::Functype functype, Item *value)
{
  ...
  DBUG_RETURN(field->get_mm_leaf(param, key_part, this, functype_to_scalar_comparison_op(functype), value));
}

bool Field::can_optimize_scalar_range(const RANGE_OPT_PARAM *param, const KEY_PART *key_part,
                                      const Item_bool_func *cond, scalar_comparison_op op, const Item *value) const
{
  bool is_eq_func = op == SCALAR_CMP_EQ || op == SCALAR_CMP_EQUAL;
  if ((param->using_real_indexes && !optimize_range(param->real_keynr[key_part->key], key_part->part) && !is_eq_func) ||
...
SEL_ARG *Field::stored_field_make_mm_leaf_truncated(RANGE_OPT_PARAM *param, scalar_comparison_op op, Item *value)
{
  DBUG_ENTER("Field::stored_field_make_mm_leaf_truncated");
  if ((op == SCALAR_CMP_EQ || op == SCALAR_CMP_EQUAL) &&
      value->result_type() == item_cmp_type(result_type(), value->result_type()))
...
SEL_ARG *Field_num::get_mm_leaf(RANGE_OPT_PARAM *prm, KEY_PART *key_part, const Item_bool_func *cond,
...
SEL_ARG *Field_temporal::get_mm_leaf(RANGE_OPT_PARAM *prm, KEY_PART *key_part, const Item_bool_func *cond,
                                     scalar_comparison_op op, Item *value)
{
...
SEL_ARG *Field_date_common::get_mm_leaf(RANGE_OPT_PARAM *prm, KEY_PART *key_part, const Item_bool_func *cond,
                                        scalar_comparison_op op, Item *value)
{
...
SEL_ARG *Field_str::get_mm_leaf(RANGE_OPT_PARAM *prm, KEY_PART *key_part, const Item_bool_func *cond,
                                scalar_comparison_op op, Item *value)
{
...
SEL_ARG *Field::get_mm_leaf_int(RANGE_OPT_PARAM *prm, KEY_PART *key_part, const Item_bool_func *cond,
                                scalar_comparison_op op, Item *value, bool unsigned_field)
{
  ...
  if (err > 0)
  {
    if (value->result_type() != INT_RESULT)
      ...
      DBUG_RETURN(stored_field_make_mm_leaf_bounded_int(prm, key_part, op, value, unsigned_field));
  }
  if (value->result_type() != INT_RESULT)
    DBUG_RETURN(stored_field_make_mm_leaf(prm, key_part, op, value));
...
SEL_ARG *Field::stored_field_make_mm_leaf_bounded_int(RANGE_OPT_PARAM *param, KEY_PART *key_part,
                                                      scalar_comparison_op op, Item *value, bool unsigned_field)
{
  ...
  else if (op == SCALAR_CMP_GT && !unsigned_field && !value->unsigned_flag && item_val < 0)
...
SEL_ARG *Field::stored_field_make_mm_leaf(RANGE_OPT_PARAM *param, KEY_PART *key_part, scalar_comparison_op op,
                                          Item *value)
{
...
SEL_ARG *Field::stored_field_make_mm_leaf_exact(RANGE_OPT_PARAM *param, KEY_PART *key_part, scalar_comparison_op op,
                                                Item *value)
{
...
static int and_range_trees(RANGE_OPT_PARAM *param, SEL_TREE *tree1, SEL_TREE *tree2, SEL_TREE *result)
{
  ...
  while ((key_no = it++) != key_map::Iterator::BITMAP_END)
  {
    ...
    SEL_ARG *key;
    if ((result->keys[key_no] = key = key_and_with_limit(param, key_no, key1, key2, flag)))
    {
...
static bool is_key_infix(KEY_PART *key_init, KEY_PART *key_end, KEY_PART *inf_init, KEY_PART *inf_end)
{
  ...
  if (key_part == key_end)
    return false;
  for (key_part++, inf_part = inf_init + 1; key_part < key_end && inf_part < inf_end; key_part++, inf_part++)
  {
...
static bool sel_trees_must_be_ored(RANGE_OPT_PARAM *param, SEL_TREE *tree1, SEL_TREE *tree2, key_map oredable_keys)
{
  key_map tmp;
...
static void restore_nonrange_trees(RANGE_OPT_PARAM *param, SEL_TREE *tree, SEL_ARG **backup_keys)
{
  for (uint i = 0; i < param->keys; i++)
  {
    if (backup_keys[i])
...
static SEL_TREE *tree_or(RANGE_OPT_PARAM *param, SEL_TREE *tree1, SEL_TREE *tree2)
{
  ...
  {
    if (!result && !(result = new (param->mem_root) SEL_TREE(param->mem_root, param->keys)))
      DBUG_RETURN(result);
  }
  ...
  else if (!no_ranges1 && !no_ranges2 && !no_imerge_from_ranges)
  {
    ...
    if (!rt1 || !rt2 || result->merges.push_back(imerge_from_ranges) || imerge_from_ranges->or_sel_tree(param, rt1) ||
  ...
  if (!no_merges1 && !no_merges2 && !imerge_list_or_list(param, &tree1->merges, &tree2->merges))
  {
  ...
  for (uint i = 0; i < 2; i++)
  {
    ...
    if (rt && im && !merges.push_back(im) && !imerge_list_or_tree(param, &merges, rt))
...
static SEL_ARG *and_all_keys(RANGE_OPT_PARAM *param, SEL_ARG *key1, SEL_ARG *key2, uint clone_flag)
{
  SEL_ARG *next;
...
static SEL_ARG *key_and(RANGE_OPT_PARAM *param, SEL_ARG *key1, SEL_ARG *key2, uint clone_flag)
{
  ...
  if (key2->type == SEL_ARG::MAYBE_KEY)
  {
    ...
    if (key1->type == SEL_ARG::MAYBE_KEY)
    {  // Both are maybe key
      key1->next_key_part = key_and(param, key1->next_key_part, key2->next_key_part, clone_flag);
  ...
  while (e1 && e2)
  {
    ...
    if (cmp < 0)
    {
      ...
      continue;
    SEL_ARG *next = key_and(param, e1->next_key_part, e2->next_key_part, clone_flag);
    e1->incr_refs();
    e2->incr_refs();
  ...
  for (key2 = key2->first();;)
  {
    /*
      ...
      key1: [---] [-----] [----]
                  ...
                  tmp
    */
    if (key1->min_flag & NO_MIN_RANGE && key1->max_flag & NO_MAX_RANGE)
    {
      if (key1->maybe_flag)
      ...
      if (cmp == -2 && eq_tree(tmp->next_key_part, key2->next_key_part))
      {
        ...
        */
        ...
        {  // Only one key in tree
          if (key2->min_flag & NO_MIN_RANGE && key2->max_flag & NO_MAX_RANGE)
          {
            if (key2->maybe_flag)
              return new SEL_ARG(SEL_ARG::MAYBE_KEY);
            return 0;  // Always true OR
    ...
    */
    ...
    for (;;)
    {
      ...
      if ((cmp = tmp->cmp_max_to_max(&key2_cpy)) <= 0)
      {
        ...
        tmp->next_key_part = key_or(param, tmp->next_key_part, key2_cpy.next_key_part);
      ...
      else
      {
        ...
        */
        ...
        new_arg->next_key_part = key_or(param, tmp->next_key_part, key2_cpy.next_key_part);
        key1 = key1->insert(new_arg);
        break;
      }
    }
...
SEL_ARG *enforce_sel_arg_weight_limit(RANGE_OPT_PARAM *param, uint keyno, SEL_ARG *sel_arg)
{
  if (!sel_arg || sel_arg->type != SEL_ARG::KEY_RANGE || !param->thd->variables.optimizer_max_sel_arg_weight)
    return sel_arg;
  ...
  while (1)
  {
    if (likely(sel_arg->weight <= param->thd->variables.optimizer_max_sel_arg_weight))
      break;
...
SEL_ARG *SEL_ARG::tree_delete(SEL_ARG *key)
{
  ...
  */
  uint new_weight = root->weight - (1 + (key->next_key_part ? key->next_key_part->weight : 0));

  DBUG_ASSERT(root->weight >= (1 + (key->next_key_part ? key->next_key_part->weight : 0)));
...
void SEL_ARG::test_use_count(SEL_ARG *root)
{
    ...
    if (pos->next_key_part)
    {
      ...
      if (count > pos->next_key_part->use_count)
      {
        sql_print_information(
            "Use_count: Wrong count for key at %p: %lu "
            "should be %lu",
  ...
  if (e_count != elements)
    sql_print_warning("Wrong use count: %u (should be %u) for tree at %p", e_count, elements, this);
}
#endif
                                  ...
                                  uint *mrr_flags, uint *bufsize, Cost_estimate *cost, bool *is_ror_scan)
{
  ...
  *mrr_flags |= HA_MRR_NO_ASSOCIATION | HA_MRR_SORTED;
  ...
  // datatypes.
  if (index_only && (file->index_flags(keynr, param->max_key_parts, 1) & HA_KEYREAD_ONLY) &&
  ...
  */
  ...
  if (rows != HA_POS_ERROR)
  {
    ...
    if (update_tbl_stats)
    {
      ...
      param->table->opt_range_condition_rows = MY_MIN(param->table->opt_range_condition_rows, rows);
...
static bool is_key_scan_ror(PARAM *param, uint keynr, uint8 nparts)
{
  ...
  KEY_PART_INFO *key_part_end = (table_key->key_part + table_key->user_defined_key_parts);
  uint pk_number;
  ...
  {
    field_index_t fieldnr = (param->table->key_info[keynr].key_part[kp - table_key->key_part].fieldnr - 1);
  ...
  */
  ...
  KEY_PART_INFO *pk_part_end = pk_part + param->table->key_info[pk_number].user_defined_key_parts;
...
QUICK_RANGE_SELECT *get_quick_select(PARAM *param, uint idx, SEL_ARG *key_tree, uint mrr_flags, uint mrr_buf_size,
                                     MEM_ROOT *parent_alloc)
{
  QUICK_RANGE_SELECT *quick;
  ...
  if (param->table->key_info[param->real_keynr[idx]].flags & HA_SPATIAL)
    quick = new QUICK_RANGE_SELECT_GEOM(param->thd, param->table, param->real_keynr[idx], MY_TEST(parent_alloc),
                                        parent_alloc, &create_err);
  else
    quick = new QUICK_RANGE_SELECT(param->thd, param->table, param->real_keynr[idx], MY_TEST(parent_alloc), NULL,
                                   &create_err);

  if (quick)
  {
    if (create_err || get_quick_keys(param, quick, param->key[idx], key_tree, param->min_key, 0, param->max_key, 0))
    {
      delete quick;
      quick = 0;
    }
    else
    {
      ...
      quick->key_parts = (KEY_PART *)memdup_root(parent_alloc ? parent_alloc : &quick->alloc, (char *)param->key[idx],
                                                 sizeof(KEY_PART) * param->table->actual_n_key_parts(keyinfo));
    }
  }
  DBUG_RETURN(quick);
}

void SEL_ARG::store_next_min_max_keys(KEY_PART *key, uchar **cur_min_key, uint *cur_min_flag, uchar **cur_max_key,
                                      uint *cur_max_flag, int *min_part, int *max_part)
{
  ...
  if (!get_min_flag(key))
  {
    if (asc)
    {
      *min_part += next_key_part->store_min_key(key, cur_min_key, cur_min_flag, MAX_KEY, true);
    }
    else
    {
      uint tmp_flag = invert_min_flag(*cur_min_flag);
      *min_part += next_key_part->store_max_key(key, cur_min_key, &tmp_flag, MAX_KEY, true);
  ...
  if (!get_max_flag(key))
  {
    if (asc)
    {
      *max_part += next_key_part->store_max_key(key, cur_max_key, cur_max_flag, MAX_KEY, false);
    }
    else
    {
      uint tmp_flag = invert_max_flag(*cur_max_flag);
      *max_part += next_key_part->store_min_key(key, cur_max_key, &tmp_flag, MAX_KEY, false);
                    ...
                    uint min_key_flag, uchar *max_key, uint max_key_flag)
{
  ...
  {
    if (get_quick_keys(param, quick, key, next_tree, min_key, min_key_flag, max_key, max_key_flag))
      return 1;
  }
  uchar *tmp_min_key = min_key, *tmp_max_key = max_key;

  key_tree->store_min_max(key, key[key_tree->part].store_length, &tmp_min_key, min_key_flag, &tmp_max_key, max_key_flag,
                          &min_part, &max_part);

  if (key_tree->next_key_part && key_tree->next_key_part->type == SEL_ARG::KEY_RANGE &&
      key_tree->next_key_part->part == key_tree->part + 1)
  {  // const key as prefix
    if ((tmp_min_key - min_key) == (tmp_max_key - max_key) &&
        memcmp(min_key, max_key, (uint)(tmp_max_key - max_key)) == 0 && key_tree->min_flag == 0 &&
        key_tree->max_flag == 0)
    {
      ...
      if (get_quick_keys(param, quick, key, key_tree->next_key_part, tmp_min_key, min_key_flag, tmp_max_key,
                         max_key_flag))
        return 1;
    ...
    }
    {
      ...
      key_tree->store_next_min_max_keys(key, &tmp_min_key, &tmp_min_flag, &tmp_max_key, &tmp_max_flag, &min_part,
                                        &max_part);
      flag = tmp_min_flag | tmp_max_flag;
    }
  }
  else
  {
    ...
    else
    {
      ...
      flag = invert_min_flag(key_tree->min_flag) | invert_max_flag(key_tree->max_flag);
    }
  }
  ...
  */
  ...
  if (flag == 0)
  {
    ...
    if (length == (uint)(tmp_max_key - param->max_key) && !memcmp(param->min_key, param->max_key, length))
    {
      ...
      flag = EQ_RANGE;
      if ((table_key->flags & HA_NOSAME) && min_part == key_tree->part &&
  ...
  if (!(range = new (param->thd->mem_root)
            QUICK_RANGE(param->thd, param->min_key, (uint)(tmp_min_key - param->min_key),
                        min_part >= 0 ? make_keypart_map(min_part) : 0, param->max_key,
                        (uint)(tmp_max_key - param->max_key), max_part >= 0 ? make_keypart_map(max_part) : 0, flag)))
  ...
  if (next_tree != &null_element)
    return get_quick_keys(param, quick, key, next_tree, min_key, min_key_flag, max_key, max_key_flag);
  return 0;
}
...
static bool null_part_in_key(KEY_PART *key_part, const uchar *key, uint length)
{
  for (const uchar *end = key + length; key < end; key += key_part++->store_length)
  {
...
QUICK_RANGE_SELECT *get_quick_select_for_ref(THD *thd, TABLE *table, TABLE_REF *ref, ha_rows records)
{
  ...
  */
  ...
  if ((cp_buffer_from_ref(thd, table, ref) && unlikely(thd->is_fatal_error)) ||
      unlikely(!(range = new (alloc) QUICK_RANGE())))
    goto err;  // out of memory

  range->min_key = range->max_key = ref->key_buff;
  range->min_length = range->max_length = ref->key_length;
  range->min_keypart_map = range->max_keypart_map = make_prev_keypart_map(ref->key_parts);
  range->flag = EQ_RANGE;

  if (unlikely(
          !(quick->key_parts = key_part = (KEY_PART *)alloc_root(&quick->alloc, sizeof(KEY_PART) * ref->key_parts))))
    goto err;
  ...
  */
  if (ref->null_ref_key)
  {
    ...
    *ref->null_ref_key = 1;  // Set null byte then create a range
    if (!(null_range =
              new (alloc) QUICK_RANGE(thd, ref->key_buff, ref->key_length, make_prev_keypart_map(ref->key_parts),
                                      ref->key_buff, ref->key_length, make_prev_keypart_map(ref->key_parts), EQ_RANGE)))
      goto err;
  ...
  quick->mrr_flags = HA_MRR_NO_ASSOCIATION | (table->file->keyread_enabled() ? HA_MRR_INDEX_ONLY : 0);
  if (thd->lex->sql_command != SQLCOM_SELECT)
    quick->mrr_flags |= HA_MRR_USE_DEFAULT_IMPL;

  quick->mrr_buf_size = thd->variables.mrr_buff_size;
  if (table->file->multi_range_read_info(quick->index, 1, (uint)records, ~0, &quick->mrr_buf_size, &quick->mrr_flags,
                                         &cost))
    goto err;
...
int read_keys_and_merge_scans(THD *thd, TABLE *head, List<QUICK_RANGE_SELECT> quick_selects,
                              QUICK_RANGE_SELECT *pk_quick_select, READ_RECORD *read_record, bool intersection,
                              key_map *filtered_scans, Unique **unique_ptr)
{
  ...
  */
  ...
  if (unique == NULL)
  {
    ...
    DBUG_EXECUTE_IF("only_one_Unique_may_be_created", DBUG_SET("+d,index_merge_may_not_create_a_Unique"););

    unique = new Unique(refpos_order_cmp, (void *)file, file->ref_length, (size_t)thd->variables.sortbuff_size,
  ...
  for (;;)
  {
    ...
    if (with_cpk_filter && pk_quick_select->row_in_ranges() != intersection)
      continue;
  ...
  */
  ...
  if (init_read_record(read_record, thd, head, (SQL_SELECT *)0, &unique->sort, 1, 1, TRUE))
    result = 1;
  DBUG_RETURN(result);
...

{
  ...
  result = read_keys_and_merge_scans(thd, head, quick_selects, pk_quick_select, &read_record, FALSE, NULL, &unique);
  doing_pk_scan = FALSE;
  DBUG_RETURN(result);
}
...

{
  ...
  result = read_keys_and_merge_scans(thd, head, quick_selects, pk_quick_select, &read_record, TRUE, &filtered_scans,
                                     &unique);
  DBUG_RETURN(result);
}
...
int QUICK_ROR_INTERSECT_SELECT::get_next()
{
  ...
  key_copy(qr->key_tuple, record, head->key_info + quick->index, quick->max_used_key_length);
  ...
  while (last_rowid_count < quick_selects.elements)
  {
    ...
    do
    {
      DBUG_EXECUTE_IF("innodb_quick_report_deadlock", DBUG_SET("+d,innodb_report_deadlock"););
      if (unlikely((error = quick->get_next())))
      {
  ...
  if (!need_to_fetch_row)
  {
    ...
    while ((qr = quick_it++))
    {
      if (qr->quick != quick)
      {
        key_restore(record, qr->key_tuple, head->key_info + qr->quick->index, qr->quick->max_used_key_length);
      }
    }
  }
...
int QUICK_RANGE_SELECT::reset()
{
  ...
  if (mrr_buf_size && !mrr_buf_desc)
  {
    ...
    while (buf_size && !my_multi_malloc(key_memory_QUICK_RANGE_SELECT_mrr_buf_desc, MYF(MY_WME), &mrr_buf_desc,
                                        sizeof(*mrr_buf_desc), &mrange_buff, buf_size, NullS))
    {
  ...
  error = file->multi_range_read_init(&seq_funcs, (void *)this, (uint)ranges.elements, mrr_flags,
...
int QUICK_RANGE_SELECT::get_next_prefix(uint prefix_length, uint group_key_parts, uchar *cur_prefix)
{
  ...
  for (;;)
  {
    ...
    if (last_range)
    {
      ...
      result = file->ha_index_read_map(record, cur_prefix, keypart_map, HA_READ_AFTER_KEY);
    ...
    result =
        file->read_range_first(last_range->min_keypart_map ? &start_key : 0, last_range->max_keypart_map ? &end_key : 0,
                               MY_TEST(last_range->flag & EQ_RANGE), TRUE);
...
int QUICK_RANGE_SELECT_GEOM::get_next()
{
  ...
  for (;;)
  {
    ...
    if (last_range)
    {
      ...
      result = file->ha_index_next_same(record, last_range->min_key, last_range->min_length);
      if (result != HA_ERR_END_OF_FILE)
        DBUG_RETURN(result);
    }
    ...
    result = file->ha_index_read_map(record, last_range->min_key, last_range->min_keypart_map,
...
QUICK_SELECT_DESC::QUICK_SELECT_DESC(QUICK_RANGE_SELECT *q, uint used_key_parts_arg)
    : QUICK_RANGE_SELECT(*q), rev_it(rev_ranges), used_key_parts(used_key_parts_arg)
{
  QUICK_RANGE *r;
  /*
...
int QUICK_SELECT_DESC::get_next()
{
  ...
  for (;;)
  {
    ...
    if (last_range)
    {  // Already read through key
      result = ((last_range->flag & EQ_RANGE && used_key_parts <= head->key_info[index].user_defined_key_parts)
                    ? file->ha_index_next_same(record, last_range->min_key, last_range->min_length)
    ...
    start_key.flag = ((last_range->flag & NEAR_MIN)   ? HA_READ_AFTER_KEY
                      : (last_range->flag & EQ_RANGE) ? HA_READ_KEY_EXACT
                                                      : HA_READ_KEY_OR_NEXT);
    start_key.keypart_map = last_range->min_keypart_map;
    key_range end_key;
    end_key.key = (const uchar *)last_range->max_key;
    end_key.length = last_range->max_length;
    end_key.flag = (last_range->flag & NEAR_MAX ? HA_READ_BEFORE_KEY : HA_READ_AFTER_KEY);
    ...
    if (last_range->flag & EQ_RANGE && used_key_parts <= head->key_info[index].user_defined_key_parts)

    {
      result = file->ha_index_read_map(record, last_range->max_key, last_range->max_keypart_map, HA_READ_KEY_EXACT);
    }
    else
    {
      DBUG_ASSERT(last_range->flag & NEAR_MAX ||
                  (last_range->flag & EQ_RANGE && used_key_parts > head->key_info[index].user_defined_key_parts) ||
                  range_reads_after_key(last_range));
      result =
          file->ha_index_read_map(record, last_range->max_key, last_range->max_keypart_map,
                                  ((last_range->flag & NEAR_MAX) ? HA_READ_BEFORE_KEY : HA_READ_PREFIX_LAST_OR_PREV));
    }
    if (result)
    {
      if (result != HA_ERR_KEY_NOT_FOUND && result != HA_ERR_END_OF_FILE)
...
int QUICK_RANGE_SELECT::cmp_next(QUICK_RANGE *range_arg)
{
  ...
  for (uchar *key = range_arg->max_key, *end = key + range_arg->max_length; key < end; key += store_length, key_part++)
  {
    int cmp;
...
Explain_quick_select *QUICK_GROUP_MIN_MAX_SELECT::get_explain(MEM_ROOT *local_alloc)
{
  Explain_quick_select *res;
...
Explain_quick_select *QUICK_INDEX_SORT_SELECT::get_explain(MEM_ROOT *local_alloc)
{
  Explain_quick_select *res;
...
Explain_quick_select *QUICK_INDEX_INTERSECT_SELECT::get_explain(MEM_ROOT *local_alloc)
{
  Explain_quick_select *res;
  Explain_quick_select *child_explain;
...
Explain_quick_select *QUICK_ROR_INTERSECT_SELECT::get_explain(MEM_ROOT *local_alloc)
{
  Explain_quick_select *res;
  Explain_quick_select *child_explain;
...
Explain_quick_select *QUICK_ROR_UNION_SELECT::get_explain(MEM_ROOT *local_alloc)
{
  Explain_quick_select *res;
  Explain_quick_select *child_explain;
...
void QUICK_SELECT_I::add_key_and_length(String *key_names, String *used_lengths, bool *first)
{
  char buf[64];
  size_t length;
  KEY *key_info = head->key_info + index;
...
void QUICK_INDEX_INTERSECT_SELECT::add_keys_and_lengths(String *key_names, String *used_lengths)
{
  QUICK_RANGE_SELECT *quick;
  bool first = TRUE;
...
void QUICK_RANGE_SELECT::add_used_key_part_to_set()
{
  ...
  for (key_len = 0; key_len < max_used_key_length; key_len += (part++)->store_length)
  {
    /*
...
static bool get_sel_arg_for_keypart(Field *field, SEL_ARG *index_range_tree, SEL_ARG **cur_range);
static bool get_constant_key_infix(KEY *index_info, SEL_ARG *index_range_tree, KEY_PART_INFO *first_non_group_part,
                                   KEY_PART_INFO *min_max_arg_part, KEY_PART_INFO *last_part, THD *thd,
                                   uchar *key_infix, uint *key_infix_len, KEY_PART_INFO **first_non_infix_part);
...
static TRP_GROUP_MIN_MAX *get_best_group_min_max(PARAM *param, SEL_TREE *tree, double read_time)
{
  ...
  if (!join) /* This is not a select statement. */
    ...
    cause = "no index";
           ...
           join->conds->used_tables() & OUTER_REF_TABLE_BIT) /* Cannot execute with correlated conditions. */
  ...
  if (join->sum_funcs[0])
  {
    ...
    while ((min_max_item = *(func_ptr++)))
    {
      if (min_max_item->sum_func() == Item_sum::MIN_FUNC)
        ...
        have_max = TRUE;
      else if (is_agg_distinct && (min_max_item->sum_func() == Item_sum::COUNT_DISTINCT_FUNC ||
                                   min_max_item->sum_func() == Item_sum::SUM_DISTINCT_FUNC ||
                                   min_max_item->sum_func() == Item_sum::AVG_DISTINCT_FUNC))
        continue;
      else
      {
        trace_group.add("chosen", false).add("cause", "not applicable aggregate function");
        DBUG_RETURN(NULL);
      }
  ...
  {
    trace_group.add("chosen", false).add("cause", "have both agg distinct and min max");
    DBUG_RETURN(NULL);
  }
  ...
  if (join->select_distinct)
  {
    ...
    while ((item = select_items_it++))
    {
      ...
      {
        trace_group.add("chosen", false).add("cause", "distinct field is expression");
        DBUG_RETURN(NULL);
      }
    }
  }
  ...
  for (tmp_group = join->group_list; tmp_group; tmp_group = tmp_group->next)
  {
    ...
    {
      trace_group.add("chosen", false).add("cause", "group field is expression");
      DBUG_RETURN(NULL);
    }
    elements_in_group++;
  }
  ...
  */
  ...
  for (uint cur_param_idx = 0; cur_param_idx < param->keys; ++cur_param_idx)
  {
    ...
    */
    if (!table->covering_keys.is_set(cur_index) || !table->keys_in_use_for_group_by.is_set(cur_index))
    {
      cause = "not covering";
      goto next_index;
    }
    ...
    */
    if (cur_index_info->user_defined_key_parts == table->actual_n_key_parts(cur_index_info) && pk < MAX_KEY &&
        cur_index != pk && (table->file->ha_table_flags() & HA_PRIMARY_KEY_IN_READ_INDEX))
    {
    ...
    */
    ...
    /*
      ...
      to a GROUP query.
    */
    if ((!join->group && join->select_distinct) || is_agg_distinct)
    {
      if (!is_agg_distinct)
      {
      ...
      while (NULL != (item = (is_agg_distinct ? (Item *)agg_distinct_flds_it++ : select_items_it++)))
      {
    ...
    */
    if (cur_index_info->flags & HA_NOSAME && cur_group_key_parts == cur_index_info->user_defined_key_parts)
    {
    ...
    */
    ...
    first_non_group_part = (cur_group_key_parts < key_parts) ? cur_index_info->key_part + cur_group_key_parts : NULL;
    first_non_infix_part = min_max_arg_part ? (min_max_arg_part < last_part) ? min_max_arg_part : NULL : NULL;
    if (first_non_group_part && (!min_max_arg_part || (min_max_arg_part - first_non_group_part > 0)))
    {
      if (tree)
      {
        ...
        if (!get_constant_key_infix(cur_index_info, index_range_tree, first_non_group_part, min_max_arg_part, last_part,
                                    thd, cur_key_infix, &cur_key_infix_len, &first_non_infix_part))
        {
      ...
      else if (first_non_group_part && join->conds)
      {
        ...
        */
        ...
        if (join->conds->walk(&Item::find_item_in_field_list_processor, true, key_part_range))
        {
    ...
    */
    ...
    {
      cur_part = first_non_infix_part + (min_max_arg_part && (min_max_arg_part < last_part));
    ...
    */
    if (tree && min_max_arg_item)
    {
      ...
      if (get_sel_arg_for_keypart(min_max_arg_part->field, index_range_tree, &cur_range) ||
    ...
    key_infix_parts = cur_key_infix_len ? (uint)(first_non_infix_part - first_non_group_part) : 0;
    ...
    if (tree)
    {
      if ((cur_index_tree = tree->keys[cur_param_idx]))
      {
        ...
        {
          Json_writer_array trace_range(thd, "ranges");
          trace_ranges(&trace_range, param, cur_param_idx, cur_index_tree, cur_index_info->key_part);
        }
      }
      else
        cur_quick_prefix_records = HA_POS_ERROR;
    }
    cost_group_min_max(table, cur_index_info, cur_used_key_parts, cur_group_key_parts, tree, cur_index_tree,
  ...
  if (join->conds && min_max_arg_item &&
      !check_group_min_max_predicates(join->conds, min_max_arg_item,
                                      (index_info->flags & HA_SPATIAL) ? Field::itMBR : Field::itRAW, &has_min_max_fld,
                                      &has_other_fld))
  {
    trace_group.add("usable", false).add("cause", "unsupported predicate on agg attribute");
    DBUG_RETURN(NULL);
  }
  ...
  */
  ...
  {
    trace_group.add("usable", false).add("cause", "index is clustered");
    DBUG_RETURN(NULL);
  }

  /* The query passes all tests, so construct a new TRP object. */
  read_plan = new (param->mem_root)
      TRP_GROUP_MIN_MAX(have_min, have_max, is_agg_distinct, min_max_arg_part, group_prefix_len, used_key_parts,
                        group_key_parts, index_info, index, key_infix_len, (key_infix_len > 0) ? key_infix : NULL, tree,
                                           ...
                                           bool *has_min_max_arg, bool *has_other_arg)
{
    ...
    while ((and_or_arg = li++))
    {
      ...
      */
      if (!check_group_min_max_predicates(and_or_arg, min_max_arg_item,  // 1
                                          image_type, &has_min_max, &has_other) ||
  ...
  */
  ...
  if (unlikely(cond_type == Item::SUBSELECT_ITEM))
  {
    ...
    if (subs_cond->is_correlated)
    {
      ...
      List_iterator_fast<Item_subselect::Ref_to_outside> li(subs_cond->upper_refs);
  ...
  */
  ...
  for (uint arg_idx = 0; arg_idx < pred->argument_count(); arg_idx++)
  {
    ...
    {
      if (!check_group_min_max_predicates(cur_arg, min_max_arg_item, image_type, &has_min_max, &has_other))
        DBUG_RETURN(FALSE);
    }
...
static bool get_sel_arg_for_keypart(Field *field, SEL_ARG *keypart_tree, SEL_ARG **cur_range)
{
  ...
  for (SEL_ARG *cur_kp = first_kp; cur_kp; cur_kp = cur_kp->next)
  {
    ...
    {
      if (get_sel_arg_for_keypart(field, cur_kp->next_key_part, &curr_tree))
        return true;
    }
    /*
...
static inline uint get_field_keypart(KEY *index, Field *field)
{
  ...
  for (part = index->key_part, end = part + field->table->actual_n_key_parts(index); part < end; part++)
  {
                        ...
                        double *read_cost, ha_rows *records)
{
  ...
  keys_per_block =
      (uint)(table->file->stats.block_size * 3 / 4 / (index_info->key_length + table->file->ref_length) + 1);
  ...
  */
  const double tree_traversal_cost =
      ceil(log(static_cast<double>(table_records)) / log(static_cast<double>(keys_per_block))) * 1 /
      (2 * TIME_FOR_COMPARE);

  const double cpu_cost = num_groups * (tree_traversal_cost + 1 / TIME_FOR_COMPARE_IDX);

  *read_cost = io_cost + cpu_cost;
  *records = num_groups;

  DBUG_PRINT("info", ("table rows: %lu  keys/block: %u  keys/group: %lu  "
                      "result rows: %lu  blocks: %lu",
                      (ulong)table_records, keys_per_block, (ulong)keys_per_group, (ulong)*records, (ulong)num_blocks));
  DBUG_VOID_RETURN;
}
...
QUICK_SELECT_I *TRP_GROUP_MIN_MAX::make_quick(PARAM *param, bool retrieve_full_rows, MEM_ROOT *parent_alloc)
{
  ...
  quick = new QUICK_GROUP_MIN_MAX_SELECT(param->table, param->thd->lex->current_select->join, have_min, have_max,
                                         have_agg_distinct, min_max_arg_part, group_prefix_len, group_key_parts,
                                         used_key_parts, index_info, index, read_cost, records, key_infix_len,
  ...
  if (range_tree)
  {
    ...
    if (quick_prefix_records == HA_POS_ERROR)
      ...
      quick->quick_prefix_select =
          get_quick_select(param, param_idx, index_tree, HA_MRR_USE_DEFAULT_IMPL, 0, &quick->alloc);
...
QUICK_GROUP_MIN_MAX_SELECT::QUICK_GROUP_MIN_MAX_SELECT(
    TABLE *table, JOIN *join_arg, bool have_min_arg, bool have_max_arg, bool have_agg_distinct_arg,
    KEY_PART_INFO *min_max_arg_part_arg, uint group_prefix_len_arg, uint group_key_parts_arg, uint used_key_parts_arg,
    KEY *index_info_arg, uint use_index, double read_cost_arg, ha_rows records_arg, uint key_infix_len_arg,
    uchar *key_infix_arg, MEM_ROOT *parent_alloc, bool is_index_scan_arg)
    : file(table->file),
      join(join_arg),
      index_info(index_info_arg),
      group_prefix_len(group_prefix_len_arg),
...
int QUICK_GROUP_MIN_MAX_SELECT::init()
{
  ...
  */
  ...
  /*
    ...
    the buffer is compared using uint3korr (e.g. a Field_newdate field)
  */
  if (!(group_prefix = (uchar *)alloc_root(&alloc, real_prefix_len + min_max_arg_len + 1)))
    return 1;
  ...
  if (min_max_arg_part)
  {
    if (my_init_dynamic_array(PSI_INSTRUMENT_ME, &min_max_ranges, sizeof(QUICK_RANGE *), 16, 16,
                              MYF(MY_THREAD_SPECIFIC)))
      return 1;
...
bool QUICK_GROUP_MIN_MAX_SELECT::add_range(SEL_ARG *sel_range)
{
  ...
  range = new QUICK_RANGE(join->thd, sel_range->min_value, min_max_arg_len, make_keypart_map(sel_range->part),
                          sel_range->max_value, min_max_arg_len, make_keypart_map(sel_range->part), range_flag);
  if (!range)
    return TRUE;
...
{
  if (quick_prefix_select && group_prefix_len < quick_prefix_select->max_used_key_length)
  {
    DYNAMIC_ARRAY *arr;
    uint inx;
...
void QUICK_GROUP_MIN_MAX_SELECT::update_key_stat()
{
  ...
  if (min_max_ranges.elements > 0)
  {
    ...
    { /* Check if the right-most range has a lower boundary. */
      get_dynamic(&min_max_ranges, (uchar *)&cur_range, min_max_ranges.elements - 1);
  ...
  do
  {
    ...
    */
    if (!have_min && !have_max && key_infix_len > 0)
      result = file->ha_index_read_map(record, group_prefix, make_prev_keypart_map(real_key_parts), HA_READ_KEY_EXACT);

    result = have_min ? min_res : have_max ? max_res : result;
  } while ((result == HA_ERR_KEY_NOT_FOUND || result == HA_ERR_END_OF_FILE) && is_last_prefix != 0);
...
int QUICK_GROUP_MIN_MAX_SELECT::next_min()
{
  ...
  else
  {
    ...
    {
      if ((result =
               file->ha_index_read_map(record, group_prefix, make_prev_keypart_map(real_key_parts), HA_READ_KEY_EXACT)))
        DBUG_RETURN(result);
    }
    ...
    */
    if (min_max_arg_part && min_max_arg_part->field->is_null())
    {
      ...
      result = file->ha_index_read_map(record, tmp_key_buff, make_keypart_map(real_key_parts), HA_READ_AFTER_KEY);
      /*
...
int QUICK_GROUP_MIN_MAX_SELECT::next_max()
{
  ...
  if (min_max_ranges.elements > 0)
    ...
    result = file->ha_index_read_map(record, group_prefix, make_prev_keypart_map(real_key_parts), HA_READ_PREFIX_LAST);
  DBUG_RETURN(result);
}
...
static int index_next_different(bool is_index_scan, handler *file, KEY_PART_INFO *key_part, uchar *record,
                                const uchar *group_prefix, uint group_prefix_len, uint group_key_parts)
{
  if (is_index_scan)
  {
    int result = 0;
    ...
    return file->ha_index_read_map(record, group_prefix, make_prev_keypart_map(group_key_parts), HA_READ_AFTER_KEY);
}
...
int QUICK_GROUP_MIN_MAX_SELECT::next_prefix()
{
  ...
  if (quick_prefix_select)
  {
    ...
    if ((result = quick_prefix_select->get_next_prefix(group_prefix_len, group_key_parts, cur_prefix)))
      DBUG_RETURN(result);
    seen_first_key = TRUE;
  }
  else
  {
    ...
    else
    {
      ...
      result = index_next_different(is_index_scan, file, index_info->key_part, record, group_prefix, group_prefix_len,
                                    group_key_parts);
      if (result)
        DBUG_RETURN(result);
    }
...
*/
int QUICK_GROUP_MIN_MAX_SELECT::cmp_min_max_key(const uchar *key, uint16 length)
{
  /*
    Allocate a buffer.
  ...
  */
  ...
  int cmp_res = key_cmp(index_info->key_part, buffer, real_prefix_len + min_max_arg_len);
  my_afree(buffer);
  return cmp_res;
}
  ...
  { /* Search from the left-most range to the right. */
    ...
    */
    if (range_idx != 0 && !(cur_range->flag & NO_MAX_RANGE) &&
        (key_cmp(min_max_arg_part, (const uchar *)cur_range->max_key, min_max_arg_len) == 1))
      continue;
  ...
  { /* Search from the right-most range to the left. */
    ...
    */
    if (range_idx != min_max_ranges.elements && !(cur_range->flag & NO_MIN_RANGE) &&
        (key_cmp(min_max_arg_part, (const uchar *)cur_range->min_key, min_max_arg_len) == -1))
      continue;
    ...
    if (result)
    {
      if ((result == HA_ERR_KEY_NOT_FOUND || result == HA_ERR_END_OF_FILE) && (cur_range->flag & EQ_RANGE))
        continue; /* Check the next range. */
...
bool eq_ranges_exceeds_limit(RANGE_SEQ_IF *seq, void *seq_init_param, uint limit)
{
  KEY_MULTI_RANGE range;
  range_seq_t seq_it;
  uint count = 0;
...
static void print_sel_tree(PARAM *param, SEL_TREE *tree, key_map *tree_map, const char *msg)
{
  ...
  DBUG_PRINT("info", ("SEL_TREE: %p (%s)  scans: %s", tree, msg, tmp.c_ptr_safe()));
...
static void print_sel_arg_key(Field *field, const uchar *key, String *out)
{
...
void QUICK_RANGE_SELECT::dbug_dump(int indent, bool verbose)
{
  ...
  fprintf(DBUG_FILE, "%*squick range select, key %s, length: %d\n", indent, "", head->key_info[index].name.str,
          max_used_key_length);
...
void QUICK_ROR_INTERSECT_SELECT::dbug_dump(int indent, bool verbose)
{
  ...
  fprintf(DBUG_FILE, "%*smerged scans {\n", indent, "");
  while ((qr = it++)) qr->quick->dbug_dump(indent + 2, verbose);
  if (cpk_quick)
  {
    fprintf(DBUG_FILE, "%*sclustered PK quick:\n", indent, "");
...
static void print_range(String *out, const KEY_PART_INFO *key_part, KEY_MULTI_RANGE *range, uint n_key_parts)
{
  ...
  key_part_map keypart_map = range->start_key.keypart_map | range->end_key.keypart_map;
  ...
  if (flag & GEOM_FLAG)
  {
    ...
    */
    ...
    out->append(STRING_WITH_LEN(" "));
    print_key_value(out, key_part, range->start_key.key, range->start_key.length);
    return;
  }
...
static void print_key_value(String *out, const KEY_PART_INFO *key_part, const uchar *key, uint used_length)
{
  out->append(STRING_WITH_LEN("("));
  Field *field = key_part->field;
...
void print_keyparts_name(String *out, const KEY_PART_INFO *key_part, uint n_keypart, key_part_map keypart_map)
{
  uint i;
  out->append(STRING_WITH_LEN("("));
