...
typedef I_P_List<worker_thread_t,
                 I_P_List_adapter<worker_thread_t, &worker_thread_t::next_in_list, &worker_thread_t::prev_in_list>,
                 I_P_List_counter>
    worker_list_t;
...
struct TP_connection_generic : public TP_connection
{
  ...
  void init_vio(st_vio *vio) override { win_sock.init(vio); }
#endif
};

typedef I_P_List<TP_connection_generic,
                 I_P_List_adapter<TP_connection_generic, &TP_connection_generic::next_in_queue,
                                  &TP_connection_generic::prev_in_queue>,
                 I_P_List_counter, I_P_List_fast_push_back<TP_connection_generic>>
    connection_queue_t;
...
#define TP_INCREMENT_GROUP_COUNTER(group, var) \
  do                                           \
  {                                            \
    group->counters.var++;                     \
  } while (0)
