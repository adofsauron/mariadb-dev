...
static inline int access_denied_error_code(int passwd_used)
{
  ...
  return 0;
#else
  return passwd_used == 2 ? ER_ACCESS_DENIED_NO_PASSWORD_ERROR : ER_ACCESS_DENIED_ERROR;
#endif
}
...
bool mysql_grant(THD *thd, const char *db, List<LEX_USER> &user_list, privilege_t rights, bool revoke, bool is_proxy);
int mysql_table_grant(THD *thd, TABLE_LIST *table, List<LEX_USER> &user_list, List<LEX_COLUMN> &column_list,
                      privilege_t rights, bool revoke);
bool mysql_routine_grant(THD *thd, TABLE_LIST *table, const Sp_handler *sph, List<LEX_USER> &user_list,
...
bool check_grant_all_columns(THD *thd, privilege_t want_access, Field_iterator_table_ref *fields);
bool check_grant_routine(THD *thd, privilege_t want_access, TABLE_LIST *procs, const Sp_handler *sph, bool no_error);
 ...
 public:
  ...
  /**
    ...
    @return
      ...
      in save_priv.
  */
  virtual ACL_internal_access_result check(privilege_t want_access, privilege_t *save_priv) const = 0;
};
...
#ifndef DBUG_OFF
extern ulong role_global_merges, role_db_merges, role_table_merges, role_column_merges, role_routine_merges;
#endif
...
#endif
 public:
  Sql_cmd_grant(enum_sql_command command) : m_command(command) {}
...
#endif
 public:
  Sql_cmd_grant_table(enum_sql_command command, const Grant_privilege &grant) : Sql_cmd_grant_object(command, grant) {}
  bool execute(THD *thd);
};
...
class Sql_cmd_grant_sp : public Sql_cmd_grant_object
{
 ...
 public:
  Sql_cmd_grant_sp(enum_sql_command command, const Grant_privilege &grant, const Sp_handler &sph)
      : Sql_cmd_grant_object(command, grant), m_sph(sph)
  {
  }
  bool execute(THD *thd);
};
