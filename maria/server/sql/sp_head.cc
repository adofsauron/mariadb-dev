...
bool Item_splocal::append_for_log(THD *thd, String *str)
{
  ...
  */
  ...
  if (str->append(STRING_WITH_LEN(" NAME_CONST('")) || str->append(&m_name) || str->append(STRING_WITH_LEN("',")))
    return true;
...
bool Item_splocal::append_value_for_log(THD *thd, String *str)
{
  ...
  Item *item = this_item();
  String *str_value = item->type_handler()->print_item_value(thd, item, &str_value_holder);
  return (str_value ? str->append(*str_value) : str->append(NULL_clex_str));
}
...
bool Item_splocal_row_field::append_for_log(THD *thd, String *str)
{
  ...
  if (str->append(STRING_WITH_LEN(" NAME_CONST('")) || str->append(&m_name) || str->append('.') ||
      str->append(&m_field_name) || str->append(STRING_WITH_LEN("',")))
    return true;
  return append_value_for_log(thd, str) || str->append(')');
}
...
bool check_routine_name(const LEX_CSTRING *ident)
{
  DBUG_ASSERT(ident);
...
sp_head *sp_head::create(sp_package *parent, const Sp_handler *handler, enum_sp_aggregate_type agg_type)
{
  MEM_ROOT own_root;
  init_sql_alloc(key_memory_sp_head_main_root, &own_root, MEM_ROOT_BLOCK_SIZE, MEM_ROOT_PREALLOC, MYF(0));
  sp_head *sp;
...
void sp_head::destroy(sp_head *sp)
{
  if (sp)
  {
    ...
    DBUG_PRINT("info", ("mem_root %p moved to %p", &sp->mem_root, &own_root));
    delete sp;
...
sp_head::sp_head(MEM_ROOT *mem_root_arg, sp_package *parent, const Sp_handler *sph, enum_sp_aggregate_type agg_type)
      ...
      m_cont_level(0)
{
  ...
  m_lex.empty();
  my_init_dynamic_array(key_memory_sp_head_main_root, &m_instr, sizeof(sp_instr *), 16, 8, MYF(0));
  my_hash_init(key_memory_sp_head_main_root, &m_sptabs, system_charset_info, 0, 0, 0, sp_table_key, 0, 0);
  my_hash_init(key_memory_sp_head_main_root, &m_sroutines, system_charset_info, 0, 0, 0, sp_sroutine_key, 0, 0);

  DBUG_VOID_RETURN;
}

sp_package *sp_package::create(LEX *top_level_lex, const sp_name *name, const Sp_handler *sph)
{
  MEM_ROOT own_root;
  init_sql_alloc(key_memory_sp_head_main_root, &own_root, MEM_ROOT_BLOCK_SIZE, MEM_ROOT_PREALLOC, MYF(0));
  sp_package *sp;
...
sp_package::sp_package(MEM_ROOT *mem_root_arg, LEX *top_level_lex, const sp_name *name, const Sp_handler *sph)
...
bool sp_head::eq_routine_spec(const sp_head *sp) const
{
  ...
  return m_handler->type() == sp->m_handler->type() && m_pcont->context_var_count() == sp->m_pcont->context_var_count();
}
  ...
  */
  ...
  for (LEX *lex; (lex = it++);)
  {
    ...
    for (LEX *lex2; (lex2 = it2++);)
    {
      ...
      if (Sp_handler::eq_routine_name(lex2->sphead->m_name, lex->sphead->m_name) &&
    ...
    if (!found)
    {
      my_error(ER_PACKAGE_ROUTINE_IN_SPEC_NOT_DEFINED_IN_BODY, MYF(0), ErrConvDQName(lex->sphead).ptr());
      return true;
    }
  }
  return false;
...
bool sp_package::validate_private_routines(THD *thd)
{
  ...
  */
  ...
  for (LEX *lex; (lex = it++);)
  {
    ...
    if (!found)
    {
      my_error(ER_PACKAGE_ROUTINE_FORWARD_DECLARATION_NOT_DEFINED, MYF(0), ErrConvDQName(lex->sphead).ptr());
      return true;
    }
  }
  return false;
...
LEX *sp_package::LexList::find(const LEX_CSTRING &name, enum_sp_type type)
{
  ...
  for (LEX *lex; (lex = it++);)
  {
    DBUG_ASSERT(lex->sphead);
    const char *dot;
    if (lex->sphead->m_handler->type() == type && (dot = strrchr(lex->sphead->m_name.str, '.')))
    {
...
LEX *sp_package::LexList::find_qualified(const LEX_CSTRING &name, enum_sp_type type)
{
  List_iterator<LEX> it(*this);
  for (LEX *lex; (lex = it++);)
  {
    DBUG_ASSERT(lex->sphead);
    if (lex->sphead->m_handler->type() == type && Sp_handler::eq_routine_name(lex->sphead->m_name, name))
      return lex;
  }
  return NULL;
}
...
void sp_head::init_sp_name(const sp_name *spname)
{
...
{
  m_sp_share = MYSQL_GET_SP_SHARE(m_handler->type(), m_db.str, static_cast<uint>(m_db.length), m_name.str,
                                  static_cast<uint>(m_name.length));
}
...
void sp_head::set_stmt_end(THD *thd)
{
...
Field *sp_head::create_result_field(uint field_max_length, const LEX_CSTRING *field_name, TABLE *table) const
{
  Field *field;
  LEX_CSTRING name;
  ...
  */
  DBUG_ASSERT(field_max_length <= m_return_field_def.length ||
              ...
              (current_thd->stmt_arena->is_stmt_execute() && m_return_field_def.length == 8 &&
               (m_return_field_def.pack_flag & (FIELDFLAG_BLOB | FIELDFLAG_GEOM))));
...
int cmp_rqp_locations(Rewritable_query_parameter *const *a, Rewritable_query_parameter *const *b)
{
...
static bool subst_spvars(THD *thd, sp_instr *instr, LEX_STRING *query_str)
{
  ...
  for (Rewritable_query_parameter **rqp = rewritables.front(); rqp <= rewritables.back(); rqp++)
  {
    if (acc.append(*rqp))
      DBUG_RETURN(TRUE);
  }
  ...
  /*
    ...
    The query buffer layout is:
       buffer :==
            ...
            <flags>       Flags struct
  */
  size_t buf_len = (qbuf.length() + 1 + QUERY_CACHE_DB_LENGTH_SIZE + thd->db.length + QUERY_CACHE_FLAGS_SIZE + 1);
...
void Sp_handler_procedure::recursion_level_error(THD *thd, const sp_head *sp) const
{
  my_error(ER_SP_RECURSION_LIMIT, MYF(0), static_cast<int>(thd->variables.max_sp_recursion_depth), sp->m_name.str);
}
...
bool sp_head::execute(THD *thd, bool merge_da_on_success)
{
  ...
  LEX_STRING saved_cur_db_name = {saved_cur_db_name_buf, sizeof(saved_cur_db_name_buf)};
  ...
  Query_arena execute_arena(&execute_mem_root, STMT_INITIALIZED_FOR_SP), backup_arena;
  ...
  const uint status_backup_mask = SERVER_STATUS_CURSOR_EXISTS | SERVER_STATUS_LAST_ROW_SENT;
  ...
  init_sql_alloc(key_memory_sp_head_execute_root, &execute_mem_root, MEM_ROOT_BLOCK_SIZE, 0, MYF(0));
  ...
  if (m_next_cached_sp)
  {
    DBUG_PRINT("info", ("first free for %p ++: %p->%p  level: %lu  flags %x", m_first_instance, this, m_next_cached_sp,
                        m_next_cached_sp->m_recursion_level, m_next_cached_sp->m_flags));
  }
  /*
  ...
  */
  DBUG_ASSERT((m_next_cached_sp == 0 && m_first_instance->m_last_cached_sp == this) ||
  ...
  */
  ...
  if (m_db.length && (err_status = mysql_opt_change_db(thd, &m_db, &saved_cur_db_name, FALSE, &cur_db_changed)))
  {
    goto done;
  }
  ...
  */
  ...
  do
  {
    ...
    */
    ...
    thd->m_statement_psi =
        MYSQL_START_STATEMENT(&state, psi_info->m_key, thd->db.str, thd->db.length, thd->charset(), m_sp_share);
#endif
    ...
    if (WSREP(thd))
    {
      ...
      {
        WSREP_DEBUG("SP abort err status %d in sub %d trx state %d", err_status, thd->in_sub_stmt,
                    thd->wsrep_trx().state());
        err_status = 1;
      ...
      else
      {
        ...
        if (must_replay)
        {
          WSREP_DEBUG("MUST_REPLAY set after SP, err_status %d trx state: %d", err_status, thd->wsrep_trx().state());
        }
    ...
    */
    if (likely(!thd->is_fatal_error) && likely(!thd->killed_errno()) && ctx->handle_sql_condition(thd, &ip, i))
    {
      err_status = FALSE;
    }
  ...
  } while (!err_status && likely(!thd->killed) && likely(!thd->is_fatal_error) && !thd->spcont->pause_state);
  ...
  if (m_chistics.agg_type != GROUP_AGGREGATE || (m_chistics.agg_type == GROUP_AGGREGATE && thd->spcont->quit_func))
    ...
    */
    if (da->warning_info_changed(&sp_wi))
    {
      ...
      */
      ...
      if (i != NULL)
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_SP_STACK_TRACE, ER_THD(thd, ER_SP_STACK_TRACE),
                            i->m_lineno, m_qname.str != NULL ? m_qname.str : "anonymous block");
    }
  }

done:
  DBUG_PRINT("info", ("err_status: %d  killed: %d  is_slave_error: %d  report_error: %d", err_status, thd->killed,
                      thd->is_slave_error, thd->is_error()));
  ...
  */
  ...
  if (m_parent)
    m_parent->m_invoked_subroutine_count--;
  DBUG_PRINT("info", ("first free for %p --: %p->%p, level: %lu, flags %x", m_first_instance,
                      m_first_instance->m_first_free_instance, this, m_recursion_level, m_flags));
  /*
  ...
  */
  DBUG_ASSERT((m_first_instance->m_first_free_instance == 0 && this == m_first_instance->m_last_cached_sp &&
               m_next_cached_sp == 0) ||
              (m_first_instance->m_first_free_instance != 0 &&
               m_first_instance->m_first_free_instance == m_next_cached_sp &&
               m_first_instance->m_first_free_instance->m_recursion_level == m_recursion_level + 1));
...
bool set_routine_security_ctx(THD *thd, sp_head *sp, Security_context **save_ctx)
{
  *save_ctx = 0;
  if (sp->suid() != SP_IS_NOT_SUID &&
      sp->m_security_ctx.change_security_context(thd, &sp->m_definer.user, &sp->m_definer.host, &sp->m_db, save_ctx))
    return TRUE;
  ...
  /*
    ...
    TODO:
      ...
      statement that 'may' affect this.
  */
  if (*save_ctx && sp->check_execute_access(thd))
  {
...
bool sp_head::check_execute_access(THD *thd) const
{
  return m_parent ? m_parent->check_execute_access(thd)
                  : check_routine_access(thd, EXECUTE_ACL, &m_db, &m_name, m_handler, false);
}
...
sp_rcontext *sp_head::rcontext_create(THD *thd, Field *ret_value, Row_definition_list *defs, bool switch_security_ctx)
{
  ...
  if (switch_security_ctx && set_routine_security_ctx(thd, this, &save_security_ctx))
    return NULL;
#endif
...
sp_rcontext *sp_head::rcontext_create(THD *thd, Field *ret_value, List<Item> *args)
{
  DBUG_ASSERT(args);
...
sp_rcontext *sp_head::rcontext_create(THD *thd, Field *ret_value, Item **args, uint arg_count)
{
  Row_definition_list defs;
  m_pcont->retrieve_field_definitions(&defs);
...
bool sp_head::execute_trigger(THD *thd, const LEX_CSTRING *db_name, const LEX_CSTRING *table_name,
                              GRANT_INFO *grant_info)
{
  ...
  if (suid() != SP_IS_NOT_SUID &&
      m_security_ctx.change_security_context(thd, &m_definer.user, &m_definer.host, &m_db, &save_ctx))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  if (!(grant_info->privilege & TRIGGER_ACL))
  {
    ...
    my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), priv_desc, thd->security_ctx->priv_user,
  ...
  /*
    ...
    on subsequent executions of a trigger.
  */
  init_sql_alloc(key_memory_sp_head_call_root, &call_mem_root, MEM_ROOT_BLOCK_SIZE, 0, MYF(0));
...
bool sp_head::execute_function(THD *thd, Item **argp, uint argcount, Field *return_value_fld, sp_rcontext **func_ctx,
                               Query_arena *call_arena)
{
  ...
  */
  if (argcount != m_pcont->context_var_count())
  {
    ...
    */
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "FUNCTION", ErrConvDQName(this).ptr(), m_pcont->context_var_count(),
             argcount);
    DBUG_RETURN(TRUE);
  }
  /*
  ...
  */
  need_binlog_call = mysql_bin_log.is_open() && (thd->variables.option_bits & OPTION_BIN_LOG) &&
  ...
  */
  if (need_binlog_call)
  {
    ...
    for (arg_no = 0; arg_no < argcount; arg_no++)
    {
      ...
      str_value = item->type_handler()->print_item_value(thd, item, &str_value_holder);
      if (str_value)
  ...
  */
  ...
  if (need_binlog_call)
  {
    ...
    if (thd->binlog_evt_union.unioned_events)
    {
      ...
      Query_log_event qinfo(thd, binlog_buf.ptr(), binlog_buf.length(), thd->binlog_evt_union.unioned_events_trans,
                            FALSE, FALSE, errcode);
...
err_with_cleanup:
  ...
  /*
    ...
    messsages.
  */
  if (need_binlog_call && thd->spcont == NULL && !thd->binlog_evt_union.do_union)
...
bool sp_head::execute_procedure(THD *thd, List<Item> *args)
{
  ...
  {
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "PROCEDURE", ErrConvDQName(this).ptr(), params, args->elements);
    DBUG_RETURN(TRUE);
  }
  ...
  if (params > 0)
  {
    ...
    */
    ...
    DBUG_PRINT("info", (" %.*s: eval args done", (int)m_name.length, m_name.str));
  }
  ...
  */
  if (save_enable_slow_log &&
      ((!(m_flags & LOG_SLOW_STATEMENTS) && (thd->variables.log_slow_disabled_statements & LOG_SLOW_DISABLE_SP))))
  {
  ...
  */
  if (!(thd->variables.option_bits & OPTION_LOG_OFF) &&
      (!(m_flags & LOG_GENERAL_LOG) && (thd->variables.log_disabled_statements & LOG_DISABLE_SP)))
  {
  ...
  */
  ...
  /*
    ...
    messsages.
  */
  bool need_binlog_call = mysql_bin_log.is_open() && (thd->variables.option_bits & OPTION_BIN_LOG) &&
                          !thd->is_current_stmt_binlog_format_row();
  if (need_binlog_call && thd->spcont == NULL && !thd->binlog_evt_union.do_union)
    thd->issue_unsafe_warnings();
...
bool sp_head::bind_input_param(THD *thd, Item *arg_item, uint arg_no, sp_rcontext *nctx, bool is_function)
{
  ...
  {
    Settable_routine_parameter *srp = arg_item->get_settable_routine_parameter();
    ...
    if (is_function)
    {
      ...
      */
      if (thd->lex->sql_command == SQLCOM_SELECT || thd->lex->sql_command == SQLCOM_INSERT ||
  ...
  if (spvar->mode == sp_variable::MODE_OUT)
  {
    ...
    if (!null_item || nctx->set_parameter(thd, arg_no, &tmp_item))
    {
...
bool sp_head::bind_output_param(THD *thd, Item *arg_item, uint arg_no, sp_rcontext *octx, sp_rcontext *nctx)
{
...
bool sp_head::reset_lex(THD *thd, sp_lex_local *sublex)
{
...
bool sp_head::merge_lex(THD *thd, LEX *oldlex, LEX *sublex)
{
  ...
  DBUG_PRINT("info", ("sublex->get_stmt_unsafe_flags: 0x%x", sublex->get_stmt_unsafe_flags()));
...
int sp_head::push_backpatch(THD *thd, sp_instr *i, sp_label *lab, List<bp_t> *list, backpatch_instr_type itype)
{
...
int sp_head::push_backpatch(THD *thd, sp_instr *i, sp_label *lab)
{
  return push_backpatch(thd, i, lab, &m_backpatch, GOTO);
}

int sp_head::push_backpatch_goto(THD *thd, sp_pcontext *ctx, sp_label *lab)
{
...
void sp_head::backpatch(sp_label *lab)
{
  bp_t *bp;
  ...
  while ((bp = li++))
  {
    if (bp->lab == lab)
    {
      DBUG_PRINT("info", ("backpatch: (m_ip %d, label %p <%s>) to dest %d", bp->instr->m_ip, lab, lab->name.str, dest));
...
void sp_head::backpatch_goto(THD *thd, sp_label *lab, sp_label *lab_begin_block)
{
  bp_t *bp;
  uint dest = instructions();
  ...
  while ((bp = li++))
  {
    ...
    if (lex_string_cmp(system_charset_info, &bp->lab->name, &lab->name) == 0)
    {
      ...
      {
        DBUG_PRINT("info",
                   ("backpatch_goto: (m_ip %d, label %p <%s>) to dest %d", bp->instr->m_ip, lab, lab->name.str, dest));
        bp->instr->backpatch(dest, lab->ctx);
...
int sp_head::new_cont_backpatch(sp_instr_opt_meta *i)
{
  m_cont_level += 1;
  if (i)
  {
...
int sp_head::add_cont_backpatch(sp_instr_opt_meta *i)
{
  i->m_cont_dest = m_cont_level;
...
bool sp_head::sp_add_instr_cpush_for_cursors(THD *thd, sp_pcontext *pcontext)
{
  for (uint i = 0; i < pcontext->frame_cursor_count(); i++)
  {
    const sp_pcursor *c = pcontext->get_cursor_by_local_frame_offset(i);
    sp_instr_cpush *instr =
        new (thd->mem_root) sp_instr_cpush(instructions(), pcontext, c->lex(), pcontext->cursor_offset() + i);
...
void sp_head::set_chistics(const st_sp_chistics &chistics)
{
...
void sp_head::set_c_chistics(const st_sp_chistics &chistics)
{
...
void sp_head::set_info(longlong created, longlong modified, const st_sp_chistics &chistics, sql_mode_t sql_mode)
{
  m_created = created;
  m_modified = modified;
  set_chistics(chistics);
...
void sp_head::reset_thd_mem_root(THD *thd)
{
  ...
  DBUG_PRINT("info", ("mem_root %p moved to thd mem root %p", &mem_root, &thd->mem_root));
...
void sp_head::restore_thd_mem_root(THD *thd)
{
  ...
  */
  ...
  DBUG_PRINT("info", ("mem_root %p returned from thd mem root %p", &mem_root, &thd->mem_root));
...
bool check_show_routine_access(THD *thd, sp_head *sp, bool *full_access)
{
  ...
  *full_access = ((!check_table_access(thd, SELECT_ACL, &tables, FALSE, 1, TRUE) &&
                   (tables.grant.privilege & SELECT_ACL) != NO_ACL) ||
                  /* Check if user owns the routine. */
                  (!strcmp(sp->m_definer.user.str, thd->security_ctx->priv_user) &&
                   !strcmp(sp->m_definer.host.str, thd->security_ctx->priv_host)) ||
                  /* Check if current role or any of the sub-granted roles
                     own the routine. */
                  (sp->m_definer.host.length == 0 &&
                   (!strcmp(sp->m_definer.user.str, thd->security_ctx->priv_role) ||
                    check_role_is_granted(thd->security_ctx->priv_role, NULL, sp->m_definer.user.str))));
  if (!*full_access)
    return check_some_routine_access(thd, sp->m_db.str, sp->m_name.str, sp->m_handler);
  return 0;
}
...
void sp_head::show_create_routine_get_fields(THD *thd, const Sp_handler *sph, List<Item> *fields)
{
  ...
  fields->push_back(new (mem_root) Item_empty_string(thd, col1_caption, NAME_CHAR_LEN), mem_root);
  fields->push_back(new (mem_root) Item_empty_string(thd, "sql_mode", 256), mem_root);

  {
    ...
    */
    ...
    Item_empty_string *stmt_fld = new (mem_root) Item_empty_string(thd, col3_caption, 1024);
...
bool sp_head::show_create_routine(THD *thd, const Sp_handler *sph)
{
  const char *col1_caption = sph->show_create_routine_col1_caption();
  const char *col3_caption = sph->show_create_routine_col3_caption();
  ...
  fields.push_back(new (mem_root) Item_empty_string(thd, col1_caption, NAME_CHAR_LEN), thd->mem_root);
  fields.push_back(new (mem_root) Item_empty_string(thd, "sql_mode", (uint)sql_mode.length), thd->mem_root);

  {
    ...
    */
    ...
    Item_empty_string *stmt_fld =
        new (mem_root) Item_empty_string(thd, col3_caption, (uint)MY_MAX(m_defstr.length, 1024));
  ...
  fields.push_back(new (mem_root) Item_empty_string(thd, "collation_connection", MY_CS_COLLATION_NAME_SIZE),
                   thd->mem_root);

  fields.push_back(new (mem_root) Item_empty_string(thd, "Database Collation", MY_CS_CHARACTER_SET_NAME_SIZE),
                   thd->mem_root);

  if (protocol->send_result_set_metadata(&fields, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
  {
    DBUG_RETURN(TRUE);
  }
  ...
  if (full_access)
    protocol->store(m_defstr.str, m_defstr.length, m_creation_ctx->get_client_cs());
  else
    protocol->store_null();

  protocol->store(&m_creation_ctx->get_client_cs()->cs_name, system_charset_info);
  protocol->store(&m_creation_ctx->get_connection_cl()->coll_name, system_charset_info);
  protocol->store(&m_creation_ctx->get_db_cl()->coll_name, system_charset_info);
...
{
  sp_instr_jump *i = new (thd->mem_root) sp_instr_jump(instructions(), spcont, dest);
  return i == NULL || add_instr(i);
}

bool sp_head::add_instr_jump_forward_with_backpatch(THD *thd, sp_pcontext *spcont, sp_label *lab)
{
  sp_instr_jump *i = new (thd->mem_root) sp_instr_jump(instructions(), spcont);
  if (i == NULL || add_instr(i))
    return true;
...
bool sp_head::add_instr_freturn(THD *thd, sp_pcontext *spcont, Item *item, LEX *lex)
{
  sp_instr_freturn *i =
      new (thd->mem_root) sp_instr_freturn(instructions(), spcont, item, m_return_field_def.type_handler(), lex);
  if (i == NULL || add_instr(i))
    return true;
...
{
  sp_instr_preturn *i = new (thd->mem_root) sp_instr_preturn(instructions(), spcont);
  if (i == NULL || add_instr(i))
    return true;
  return false;
}
...
bool sp_head::replace_instr_to_nop(THD *thd, uint ip)
{
  ...
  sp_instr_jump *nop = new (thd->mem_root) sp_instr_jump(instr->m_ip, instr->m_ctx, instr->m_ip + 1);
  if (!nop)
    return true;
  delete instr;
...
int sp_head::show_routine_code(THD *thd)
{
  ...
  field_list.push_back(new (thd->mem_root) Item_uint(thd, "Pos", 9), thd->mem_root);
  // 1024 is for not to confuse old clients
  field_list.push_back(new (thd->mem_root) Item_empty_string(thd, "Instruction", MY_MAX(buffer.length(), 1024)),
                       thd->mem_root);
...
int sp_lex_keeper::reset_lex_and_exec_core(THD *thd, uint *nextp, bool open_tables, sp_instr *instr)
{
  ...
  */
  ...
  unsigned int parent_unsafe_rollback_flags = thd->transaction->stmt.m_unsafe_rollback_flags;
  ...
  */
  if ((thd->client_capabilities & CLIENT_SESSION_TRACK) && (thd->server_status & SERVER_SESSION_STATE_CHANGED))
    thd->lex->safe_to_cache_query = 0;
#endif

  Opt_trace_start ots(thd);
  ots.init(thd, m_lex->query_tables, SQLCOM_SELECT, &m_lex->var_list, NULL, 0, thd->variables.character_set_client);
...
int sp_lex_keeper::cursor_reset_lex_and_exec_core(THD *thd, uint *nextp, bool open_tables, sp_instr *instr)
{
...
int sp_instr::exec_open_and_lock_tables(THD *thd, TABLE_LIST *tables)
{
  ...
  */
  if (thd->open_temporary_tables(tables) || check_table_access(thd, SELECT_ACL, tables, FALSE, UINT_MAX, FALSE) ||
...
uint sp_instr::get_cont_dest() const { return (m_ip + 1); }
...
int sp_instr_stmt::execute(THD *thd, uint *nextp)
{
  ...
  if (!(res = alloc_query(thd, m_query.str, m_query.length)) && !(res = subst_spvars(thd, this, &m_query)))
  {
    /*
...
int sp_instr_stmt::exec_core(THD *thd, uint *nextp)
{
  MYSQL_QUERY_EXEC_START(thd->query(), thd->thread_id, thd->get_db(), &thd->security_ctx->priv_user[0],
                         (char *)thd->security_ctx->host_or_ip, 3);
...
sp_rcontext *sp_instr_set::get_rcontext(THD *thd) const { return m_rcontext_handler->get_rcontext(thd->spcont); }
...
void sp_instr_set::print(String *str)
{
  ...
  str->qs_append(' ');
  m_value->print(str, enum_query_type(QT_ORDINARY | QT_ITEM_ORIGINAL_FUNC_NULLIF));
}
...
int sp_instr_set_row_field::exec_core(THD *thd, uint *nextp)
{
  int res = get_rcontext(thd)->set_variable_row_field(thd, m_offset, m_field_offset, &m_value);
  delete_explain_query(thd->lex);
  *nextp = m_ip + 1;
  return res;
}

void sp_instr_set_row_field::print(String *str)
{
  ...
  const Column_definition *def = var->field_def.row_field_definitions()->elem(m_field_offset);
  DBUG_ASSERT(def);
...
int sp_instr_set_row_field_by_name::exec_core(THD *thd, uint *nextp)
{
  int res = get_rcontext(thd)->set_variable_row_field_by_name(thd, m_offset, m_field_name, &m_value);
  delete_explain_query(thd->lex);
  *nextp = m_ip + 1;
  return res;
}

void sp_instr_set_row_field_by_name::print(String *str)
{
  ...
  const LEX_CSTRING *prefix = m_rcontext_handler->get_name_prefix();
  DBUG_ASSERT(var);
  DBUG_ASSERT(var->field_def.is_table_rowtype_ref() || var->field_def.is_cursor_rowtype_ref());
...
PSI_statement_info sp_instr_set_trigger_field::psi_info = {0, "set_trigger_field", 0};
...
void sp_instr_set_trigger_field::print(String *str)
{
  ...
  trigger_field->print(str, enum_query_type(QT_ORDINARY | QT_ITEM_ORIGINAL_FUNC_NULLIF));
  str->append(STRING_WITH_LEN(":="));
  value->print(str, enum_query_type(QT_ORDINARY | QT_ITEM_ORIGINAL_FUNC_NULLIF));
}
...
uint sp_instr_opt_meta::get_cont_dest() const { return m_cont_dest; }
...
void sp_instr_jump_if_not::print(String *str)
{
  ...
  str->qs_append(STRING_WITH_LEN(") "));
  m_expr->print(str, enum_query_type(QT_ORDINARY | QT_ITEM_ORIGINAL_FUNC_NULLIF));
}

uint sp_instr_jump_if_not::opt_mark(sp_head *sp, List<sp_instr> *leads)
{
  sp_instr *i;
...
void sp_instr_jump_if_not::opt_move(uint dst, List<sp_instr> *bp)
{
  /*
...
int sp_instr_preturn::execute(THD *thd, uint *nextp)
{
...
void sp_instr_preturn::print(String *str) { str->append(STRING_WITH_LEN("preturn")); }
...
int sp_instr_agg_cfetch::execute(THD *thd, uint *nextp)
{
...
void sp_instr_agg_cfetch::print(String *str)
{
...
PSI_statement_info sp_instr_cursor_copy_struct::psi_info = {0, "cursor_copy_struct", 0};

int sp_instr_cursor_copy_struct::exec_core(THD *thd, uint *nextp)
{
...
int sp_instr_cursor_copy_struct::execute(THD *thd, uint *nextp)
{
...
void sp_instr_cursor_copy_struct::print(String *str)
{
...
int sp_instr_set_case_expr::execute(THD *thd, uint *nextp)
{
...
int sp_instr_set_case_expr::exec_core(THD *thd, uint *nextp)
{
  ...
  if (res && !thd->spcont->get_case_expr(m_case_expr_id))
  {
    ...
    */
    ...
    if (!null_item || thd->spcont->set_case_expr(thd, m_case_expr_id, &null_item))
    {
...
void sp_instr_set_case_expr::print(String *str)
{
  ...
  str->qs_append(' ');
  m_case_expr->print(str, enum_query_type(QT_ORDINARY | QT_ITEM_ORIGINAL_FUNC_NULLIF));
}
...
void sp_instr_set_case_expr::opt_move(uint dst, List<sp_instr> *bp)
{
  if (m_cont_dest > m_ip)
    bp->push_back(this);  // Forward
  else if (m_cont_optdest)
...
bool sp_head::merge_table_list(THD *thd, TABLE_LIST *table, LEX *lex_for_tmp_check)
{
  ...
  if ((lex_for_tmp_check->sql_command == SQLCOM_DROP_TABLE || lex_for_tmp_check->sql_command == SQLCOM_DROP_SEQUENCE) &&
  ...
  for (; table; table = table->next_global)
    if (!table->derived && !table->schema_table && !table->table_function)
    {
      ...
      */
      if ((tab = (SP_TABLE *)my_hash_search(&m_sptabs, (uchar *)tname.ptr(), tname.length())) ||
          ((tab = (SP_TABLE *)my_hash_search(&m_sptabs, (uchar *)tname.ptr(), temp_table_key_length)) && tab->temp))
      {
      ...
      else
      {
        ...
        if ((lex_for_tmp_check->sql_command == SQLCOM_CREATE_TABLE ||
            ...
            lex_for_tmp_check->query_tables == table && lex_for_tmp_check->tmp_table())
        {
          tab->temp = TRUE;
...
bool sp_head::add_used_tables_to_table_list(THD *thd, TABLE_LIST ***query_tables_last_ptr, TABLE_LIST *belong_to_view)
{
  uint i;
  ...
  */
  ...
  for (i = 0; i < m_sptabs.records; i++)
  {
    ...
    if (!(tab_buff = (char *)thd->alloc(ALIGN_SIZE(sizeof(TABLE_LIST)) * stab->lock_count)) ||
        !(key_buff = (char *)thd->memdup(stab->qname.str, stab->qname.length)))
      DBUG_RETURN(FALSE);
    ...
    for (uint j = 0; j < stab->lock_count; j++)
    {
      ...
      LEX_CSTRING table_name = {key_buff + stab->db_length + 1, stab->table_name_length};
      LEX_CSTRING alias = {table_name.str + table_name.length + 1, strlen(table_name.str + table_name.length + 1)};

      table->init_one_table_for_prelocking(&db_name, &table_name, &alias, stab->lock_type, TABLE_LIST::PRELOCK_ROUTINE,
                                           belong_to_view, stab->trg_event_map, query_tables_last_ptr,
...
TABLE_LIST *sp_add_to_query_tables(THD *thd, LEX *lex, const LEX_CSTRING *db, const LEX_CSTRING *name,
                                   thr_lock_type locktype, enum_mdl_type mdl_type)
{
  TABLE_LIST *table;
  ...
  MDL_REQUEST_INIT(&table->mdl_request, MDL_key::TABLE, table->db.str, table->table_name.str, mdl_type,
                   MDL_TRANSACTION);
...
bool sp_head::set_local_variable(THD *thd, sp_pcontext *spcont, const Sp_rcontext_handler *rh, sp_variable *spv,
                                 Item *val, LEX *lex, bool responsible_to_free_lex)
{
  ...
  sp_instr_set *sp_set =
      new (thd->mem_root) sp_instr_set(instructions(), spcont, rh, spv->offset, val, lex, responsible_to_free_lex);
...
bool sp_head::set_local_variable_row_field(THD *thd, sp_pcontext *spcont, const Sp_rcontext_handler *rh,
                                           sp_variable *spv, uint field_idx, Item *val, LEX *lex)
{
  if (!(val = adjust_assignment_source(thd, val, NULL)))
    return true;

  sp_instr_set_row_field *sp_set =
      new (thd->mem_root) sp_instr_set_row_field(instructions(), spcont, rh, spv->offset, field_idx, val, lex, true);
  return sp_set == NULL || add_instr(sp_set);
}

bool sp_head::set_local_variable_row_field_by_name(THD *thd, sp_pcontext *spcont, const Sp_rcontext_handler *rh,
                                                   sp_variable *spv, const LEX_CSTRING *field_name, Item *val, LEX *lex)
{
  if (!(val = adjust_assignment_source(thd, val, NULL)))
    return true;

  sp_instr_set_row_field_by_name *sp_set = new (thd->mem_root)
      sp_instr_set_row_field_by_name(instructions(), spcont, rh, spv->offset, *field_name, val, lex, true);
  return sp_set == NULL || add_instr(sp_set);
}

bool sp_head::add_open_cursor(THD *thd, sp_pcontext *spcont, uint offset, sp_pcontext *param_spcont,
                              List<sp_assignment_lex> *parameters)
{
  ...
  */
  DBUG_ASSERT((param_spcont ? param_spcont->context_var_count() : 0) == (parameters ? parameters->elements : 0));

  if (parameters && add_set_cursor_param_variables(thd, param_spcont, parameters))
    return true;

  sp_instr_copen *i = new (thd->mem_root) sp_instr_copen(instructions(), spcont, offset);
  return i == NULL || add_instr(i);
}

bool sp_head::add_for_loop_open_cursor(THD *thd, sp_pcontext *spcont, sp_variable *index, const sp_pcursor *pcursor,
                                       uint coffset, sp_assignment_lex *param_lex, Item_args *parameters)
{
  if (parameters && add_set_for_loop_cursor_param_variables(thd, pcursor->param_context(), param_lex, parameters))
    return true;

  sp_instr *instr_copy_struct =
      new (thd->mem_root) sp_instr_cursor_copy_struct(instructions(), spcont, coffset, pcursor->lex(), index->offset);
  if (instr_copy_struct == NULL || add_instr(instr_copy_struct))
    return true;

  sp_instr_copen *instr_copen = new (thd->mem_root) sp_instr_copen(instructions(), spcont, coffset);
  if (instr_copen == NULL || add_instr(instr_copen))
    return true;

  sp_instr_cfetch *instr_cfetch = new (thd->mem_root) sp_instr_cfetch(instructions(), spcont, coffset, false);
...
bool sp_head::add_set_for_loop_cursor_param_variables(THD *thd, sp_pcontext *param_spcont, sp_assignment_lex *param_lex,
                                                      Item_args *parameters)
{
  ...
  for (uint idx = 0; idx < parameters->argument_count(); idx++)
  {
    ...
    */
    ...
    if (set_local_variable(thd, param_spcont, &sp_rcontext_handler_local, spvar, parameters->arguments()[idx],
                           param_lex, last))
      return true;
  }
  return false;
...
bool sp_head::spvar_fill_row(THD *thd, sp_variable *spvar, Row_definition_list *defs)
{
...
bool sp_head::spvar_fill_type_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &table, const LEX_CSTRING &col)
{
...
bool sp_head::spvar_fill_type_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &db, const LEX_CSTRING &table,
...
bool sp_head::spvar_fill_table_rowtype_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &table)
{
  Table_ident *ref;
...
bool sp_head::spvar_fill_table_rowtype_reference(THD *thd, sp_variable *spvar, const LEX_CSTRING &db,
...
{
  return agg_type() == GROUP_AGGREGATE ? check_group_aggregate_instructions_require()
...
bool sp_head::check_standalone_routine_end_name(const sp_name *end_name) const
{
  if (end_name && !end_name->eq(this))
  {
    my_error(ER_END_IDENTIFIER_DOES_NOT_MATCH, MYF(0), ErrConvDQName(end_name).ptr(), ErrConvDQName(this).ptr());
    return true;
  }
  return false;
}

ulong sp_head::sp_cache_version() const { return m_parent ? m_parent->sp_cache_version() : m_sp_cache_version; }
