...
static TYPELIB deletable_extensions = {array_elements(del_exts) - 1, "del_exts", del_exts, NULL};

static bool find_db_tables_and_rm_known_files(THD *, MY_DIR *, const char *, const char *, TABLE_LIST **);

long mysql_rm_arc_files(THD *thd, MY_DIR *dirp, const char *org_path);
my_bool rm_dir_w_symlink(const char *org_path, my_bool send_error);
static void mysql_change_db_impl(THD *thd, LEX_CSTRING *new_db_name, privilege_t new_db_access,
                                 CHARSET_INFO *new_db_charset);
static bool mysql_rm_db_internal(THD *thd, const LEX_CSTRING *db, bool if_exists, bool silent);
...
static inline bool cmp_db_names(LEX_CSTRING *db1_name, const LEX_CSTRING *db2_name)
{
  return (db1_name->length == db2_name->length &&
          (db1_name->length == 0 || my_strcasecmp(table_alias_charset, db1_name->str, db2_name->str) == 0));
}
...
struct dbname_cache_t
{
 ...
 public:
  dbname_cache_t()
      : m_set(key_memory_dbnames_cache, table_alias_charset, 10, 0, sizeof(char *), (my_hash_get_key)get_key, my_free,
              0)
  {
  ...
  {
    auto len = strlen(s);
    auto ls = (LEX_STRING *)my_malloc(key_memory_dbnames_cache, sizeof(LEX_STRING) + strlen(s) + 1, 0);
  ...
  ~dbname_cache_t() { mysql_rwlock_destroy(&m_lock); }
};
...
extern "C" uchar *dboptions_get_key(my_dbopt_t *opt, size_t *length, my_bool not_used);

uchar *dboptions_get_key(my_dbopt_t *opt, size_t *length, my_bool not_used __attribute__((unused)))
{
...
static inline int write_to_binlog(THD *thd, const char *query, size_t q_len, const char *db, size_t db_len)
{
...
bool my_dboptions_cache_init(void)
{
  ...
  {
    dboptions_init = 1;
    error = my_hash_init(key_memory_dboptions_hash, &dboptions, table_alias_charset, 32, 0, 0,
                         (my_hash_get_key)dboptions_get_key, free_dbopt, 0);
  }
  dbname_cache_init();
  return error;
}
...
static my_bool get_dbopt(THD *thd, const char *dbname, Schema_specification_st *create)
{
  my_dbopt_t *opt;
  uint length;
  my_bool error = 1;
  ...
  if ((opt = (my_dbopt_t *)my_hash_search(&dboptions, (uchar *)dbname, length)))
  {
    ...
    {
      create->schema_comment = thd->make_clex_string(opt->comment.str, opt->comment.length);
    }
    error = 0;
  }
  mysql_rwlock_unlock(&LOCK_dboptions);
...
static my_bool put_dbopt(const char *dbname, Schema_specification_st *create)
{
  ...
  if (!(opt = (my_dbopt_t *)my_hash_search(&dboptions, (uchar *)dbname, length)))
  {
    ...
    if (!my_multi_malloc(key_memory_dboptions_hash, MYF(MY_WME | MY_ZEROFILL), &opt, (uint)sizeof(*opt), &tmp_name,
                         (uint)length + 1, &tmp_comment, (uint)DATABASE_COMMENT_MAXLEN + 1, NullS))
    {
      error = 1;
      goto end;
    }
...
static void del_dbopt(const char *path)
{
  my_dbopt_t *opt;
  mysql_rwlock_wrlock(&LOCK_dboptions);
  if ((opt = (my_dbopt_t *)my_hash_search(&dboptions, (const uchar *)path, strlen(path))))
...
static bool write_db_opt(THD *thd, const char *path, Schema_specification_st *create)
{
  File file;
  ...
  if (create->schema_comment)
  {
    if (validate_comment_length(thd, create->schema_comment, DATABASE_COMMENT_MAXLEN, ER_TOO_LONG_DATABASE_COMMENT,
                                thd->lex->name.str))
      return error;
  }

  if (thd->lex->sql_command == SQLCOM_ALTER_DB && (!create->schema_comment || !create->default_table_charset))
  {
  ...
  if ((file = mysql_file_create(key_file_dbopt, path, CREATE_MODE, O_RDWR | O_TRUNC, MYF(MY_WME))) >= 0)
  {
    ulong length;
    length =
        (ulong)(strxnmov(buf, sizeof(buf) - 1, "default-character-set=", create->default_table_charset->cs_name.str,
                         "\ndefault-collation=", create->default_table_charset->coll_name.str, "\n", NullS) -
                buf);

    if (create->schema_comment)
      length = (ulong)(strxnmov(buf + length, sizeof(buf) - 1 - length, "comment=", create->schema_comment->str, "\n",
                                NullS) -
                       buf);
...
bool load_db_opt(THD *thd, const char *path, Schema_specification_st *create)
{
  ...
  if ((file = mysql_file_open(key_file_dbopt, path, O_RDONLY | O_SHARE, MYF(0))) < 0)
    goto err1;
    ...
    *pos = 0;
    if ((pos = strchr(buf, '=')))
    {
      if (!strncmp(buf, "default-character-set", (pos - buf)))
      {
        ...
        */
        if (!(create->default_table_charset = get_charset_by_csname(pos + 1, MY_CS_PRIMARY, MYF(utf8_flag))) &&
            !(create->default_table_charset = get_charset_by_name(pos + 1, MYF(utf8_flag))))
        {
...
bool load_db_opt_by_name(THD *thd, const char *db_name, Schema_specification_st *db_create_info)
{
  ...
  */
  (void)build_table_filename(db_opt_path, sizeof(db_opt_path) - 1, db_name, "", MY_DB_OPT_FILE, 0);
...
static int mysql_create_db_internal(THD *thd, const LEX_CSTRING *db, const DDL_options_st &options,
                                    Schema_specification_st *create_info, bool silent)
{
  ...
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_DB_CREATE_EXISTS, ER_THD(thd, ER_DB_CREATE_EXISTS),
                        db->str);
    affected_rows = 0;
    goto not_silent;
  }
...
static bool mysql_alter_db_internal(THD *thd, const LEX_CSTRING *db, Schema_specification_st *create_info)
{
  char path[FN_REFLEN + 16];
  long result = 1;
  int error = 0;
...
int mysql_create_db(THD *thd, const LEX_CSTRING *db, DDL_options_st options, const Schema_specification_st *create_info)
{
  ...
  */
  ...
  if (thd->slave_thread && slave_ddl_exec_mode_options == SLAVE_EXEC_MODE_IDEMPOTENT)
    options.add(DDL_options::OPT_IF_NOT_EXISTS);
  return mysql_create_db_internal(thd, db, options, &tmp, false);
}

bool mysql_alter_db(THD *thd, const LEX_CSTRING *db, const Schema_specification_st *create_info)
{
  DBUG_ASSERT(create_info->default_table_charset);
  /*
...
void drop_database_objects(THD *thd, const LEX_CSTRING *path, const LEX_CSTRING *db, bool rm_mysql_schema)
{
...
static bool mysql_rm_db_internal(THD *thd, const LEX_CSTRING *db, bool if_exists, bool silent)
{
  ...
  if (!(dirp = my_dir(path, MYF(MY_DONT_SORT))))
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_DB_DROP_EXISTS, ER_THD(thd, ER_DB_DROP_EXISTS),
                          db->str);
      error = false;
      goto update_binlog;
    }
  ...
  */
  if ((rm_mysql_schema = (my_strcasecmp(system_charset_info, MYSQL_SCHEMA_NAME.str, db->str) == 0)))
  {
  ...
  if (lock_table_names(thd, tables, NULL, thd->variables.lock_wait_timeout, 0) || lock_db_routines(thd, dbnorm))
    goto exit;
  ...
  if (!rm_mysql_schema)
  {
    for (table = tables; table; table = table->next_local)
    {
      if (table->open_type == OT_BASE_ONLY || !thd->find_temporary_table(table))
  ...
  */
  ...
  for (table = tables; table; table = table->next_local) deleted_tables++;

  thd->push_internal_handler(&err_handler);
  if (!thd->killed &&
      !(tables && mysql_rm_table_no_locks(thd, tables, &rm_db, &ddl_log_state, true, false, true, false, true, false)))
  {
    ...
    */
    ...
    if (mysql_file_delete_with_symlink(key_file_misc, path, "", MYF(0)) && my_errno != ENOENT)
    {
...
update_binlog:
  ...
  else if (mysql_bin_log.is_open() && !silent)
  {
    ...
    for (tbl = tables; tbl; tbl = tbl->next_local)
    {
      ...
      tbl_name_len = my_snprintf(quoted_name, sizeof(quoted_name), "%`s", tbl->table_name.str);
    ...
    if (query_pos != query_data_start)  // If database was not empty
    {
      ...
      */
      ...
      thd->binlog_xid = thd->query_id;
      ddl_log_update_xid(&ddl_log_state, thd->binlog_xid);
      res = write_to_binlog(thd, query, (uint)(query_pos - 1 - query), db->str, db->length);
      thd->binlog_xid = 0;
      debug_crash_here("ddl_log_drop_after_binlog");
      if (res)
      {
...
static bool find_db_tables_and_rm_known_files(THD *thd, MY_DIR *dirp, const char *dbname, const char *path,
                                              TABLE_LIST **tables)
{
  ...
  for (size_t idx = 0; idx < files.elements(); idx++)
  {
    ...
    */
    if (lower_case_file_system)
      table_list->table_name.length = my_casedn_str(files_charset_info, (char *)table_list->table_name.str);
  ...
  *tables = tot_list;
  ...
  for (size_t idx = 0; idx < dirp->number_of_files && !thd->killed; idx++)
  {
    ...
    if (file->name[0] == 'a' && file->name[1] == 'r' && file->name[2] == 'c' && file->name[3] == '\0')
    {
      /* .frm archive:
    ...
    if (find_type(extension, &deletable_extensions, FIND_TYPE_NO_PREFIX) > 0)
    {
      ...
      */
      if (mysql_file_delete_with_symlink(key_file_misc, filePath, "", MYF(0)) && my_errno != ENOENT)
      {
...
my_bool rm_dir_w_symlink(const char *org_path, my_bool send_error)
{
  ...
  if (unlikely((error = my_readlink(tmp2_path, path, MYF(send_error ? MY_WME : 0))) < 0))
    DBUG_RETURN(1);
  if (likely(!error))
  {
...
uint mysql_change_db(THD *thd, const LEX_CSTRING *new_db_name, bool force_switch)
{
  ...
  */
  ...
  new_db_file_name.str = my_strndup(key_memory_THD_db, new_db_name->str, new_db_name->length, MYF(MY_WME));
  ...
  */
  ...
  {
    db_access = acl_get(sctx->host, sctx->ip, sctx->priv_user, new_db_file_name.str, FALSE) | sctx->master_access;
    if (sctx->priv_role[0])
    {
  ...
  if (!force_switch && !(db_access & DB_ACLS) && check_grant_db(thd, new_db_file_name.str))
  {
    my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), sctx->priv_user, sctx->priv_host, new_db_file_name.str);
    general_log_print(thd, COM_INIT_DB, ER_THD(thd, ER_DBACCESS_DENIED_ERROR), sctx->priv_user, sctx->priv_host,
  ...
  if (check_db_dir_existence(new_db_file_name.str))
  {
    if (force_switch)
    {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_BAD_DB_ERROR, ER_THD(thd, ER_BAD_DB_ERROR),
                          new_db_file_name.str);
...
bool mysql_opt_change_db(THD *thd, const LEX_CSTRING *new_db_name, LEX_STRING *saved_db_name, bool force_switch,
                         bool *cur_db_changed)
{
...
bool mysql_upgrade_db(THD *thd, const LEX_CSTRING *old_db)
{
  ...
  if ((old_db->length <= MYSQL50_TABLE_NAME_PREFIX_LENGTH) ||
      (strncmp(old_db->str, MYSQL50_TABLE_NAME_PREFIX, MYSQL50_TABLE_NAME_PREFIX_LENGTH) != 0))
  {
    my_error(ER_WRONG_USAGE, MYF(0), "ALTER DATABASE UPGRADE DATA DIRECTORY NAME", "name");
    DBUG_RETURN(1);
  }
  ...
  */
  ...
  build_table_filename(path, sizeof(path) - 1, old_db->str, "", MY_DB_OPT_FILE, 0);
  ...
  if (unlikely((error = mysql_create_db_internal(thd, &new_db, DDL_options(), &create_info, 1))))
    goto exit;
  ...
  if ((dirp = my_dir(path, MYF(MY_DONT_SORT))))
  {
    ...
    for (size_t idx = 0; idx < nfiles && !thd->killed; idx++)
    {
      ...
      *extension = '\0';
      ...
      table_str.length = filename_to_tablename(file->name, tname, sizeof(tname) - 1);
      table_str.str = (char *)thd->memdup(tname, table_str.length + 1);
      Table_ident *old_ident = new Table_ident(thd, old_db, &table_str, 0);
      Table_ident *new_ident = new Table_ident(thd, &new_db, &table_str, 0);
      if (!old_ident || !new_ident ||
          !sl->add_table_to_list(thd, old_ident, NULL, TL_OPTION_UPDATING, TL_IGNORE, MDL_EXCLUSIVE) ||
  ...
  if ((table_list = thd->lex->query_tables) && (error = mysql_rename_tables(thd, table_list, 1, 0)))
  {
    /*
  ...
  */
  ...
  if ((dirp = my_dir(path, MYF(MY_DONT_SORT))))
  {
    ...
    for (size_t idx = 0; idx < nfiles; idx++)
    {
      ...
      build_table_filename(oldname, sizeof(oldname) - 1, old_db->str, "", file->name, 0);
      build_table_filename(newname, sizeof(newname) - 1, new_db.str, "", file->name, 0);
  ...
  */
  ...
  {
    int errcode = query_error_code(thd, TRUE);
    Query_log_event qinfo(thd, thd->query(), thd->query_length(), FALSE, TRUE, TRUE, errcode);
    thd->clear_error();
...
bool check_db_dir_existence(const char *db_name)
{
  ...
  db_dir_path_len = build_table_filename(db_dir_path, sizeof(db_dir_path) - 1, db_name, "", "", 0);
