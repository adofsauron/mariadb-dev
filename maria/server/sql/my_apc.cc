...
void init_show_explain_psi_keys(void)
{
  ...
  PSI_server->register_cond("sql", show_explain_psi_conds, array_elements(show_explain_psi_conds));
}
#endif
...
bool Apc_target::make_apc_call(THD *caller_thd, Apc_call *call, int timeout_sec, bool *timed_out)
{
  bool res = TRUE;
  *timed_out = FALSE;
  ...
  if (enabled)
  {
    ...
    mysql_cond_init(key_show_explain_request_COND, &apc_request.COND_request, NULL);
    ...
    caller_thd->ENTER_COND(&apc_request.COND_request, LOCK_thd_kill_ptr, &stage_show_explain, &old_stage);
    /* todo: how about processing other errors here? */
    while (!apc_request.processed && (wait_res != ETIMEDOUT))
    {
      /* We own LOCK_thd_kill_ptr */
      wait_res = mysql_cond_timedwait(&apc_request.COND_request, LOCK_thd_kill_ptr, &abstime);
