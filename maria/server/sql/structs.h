...
{
  USER_AUTH *auth;
  bool has_auth() { return auth && (auth->plugin.length || auth->auth_str.length || auth->pwtext.length); }
};
...
class Discrete_intervals_list
{
  ...
  */
  ...
  void set_members(Discrete_interval *h, Discrete_interval *t, Discrete_interval *c, uint el)
  {
    head = h;
    tail = t;
    current = c;
 ...
 public:
  ...
  void empty_no_free() { set_members(NULL, NULL, NULL, 0); }
  void empty()
  {
...
struct DDL_options_st
{
 ...
 public:
  ...
  {
    return (DDL_options_st::Options)(((uint)m_options) & (OPT_IF_NOT_EXISTS | OPT_OR_REPLACE));
  }
  void init() { m_options = OPT_NONE; }
  void init(Options options) { m_options = options; }
  void set(Options other) { m_options = other; }
  void set(const DDL_options_st other) { m_options = other.m_options; }
  bool if_not_exists() const { return m_options & OPT_IF_NOT_EXISTS; }
  bool or_replace() const { return m_options & OPT_OR_REPLACE; }
  bool or_replace_slave_generated() const { return m_options & OPT_OR_REPLACE_SLAVE_GENERATED; }
  ...
  void add(const DDL_options_st::Options other) { m_options = (Options)((uint)m_options | (uint)other); }
  void add(const DDL_options_st &other) { add(other.m_options); }
...
class DDL_options : public DDL_options_st
{
 public:
  ...
  DDL_options(const DDL_options_st &options) { DDL_options_st::operator=(options); }
};
...
struct Lex_length_and_dec_st
{
 protected:
  ...
  static_assert(LEX_CHARSET_COLLATION_TYPE_BITS <= 8, "Lex_length_and_dec_st::m_collation_type bits check");
 ...
 public:
  ...
  uint8 dec() const { return m_dec; }
  bool has_explicit_length() const { return m_has_explicit_length; }
  bool has_explicit_dec() const { return m_has_explicit_dec; }
  bool length_overflowed() const { return m_length_overflowed; }
  bool dec_overflowed() const { return m_dec_overflowed; }
};
...
struct Lex_field_type_st : public Lex_length_and_dec_st
{
 ...
 public:
  ...
  void set(const Type_handler *handler, const Lex_length_and_dec_st &length_and_dec,
  ...
  void set(const Type_handler *handler, const Lex_column_charset_collation_attrs_st &coll)
  {
    m_handler = handler;
    m_ci = coll.charset_info();
  ...
  void set_handler_length_flags(const Type_handler *handler, const Lex_length_and_dec_st &length, uint32 flags);
  ...
  void set_handler(const Type_handler *handler) { m_handler = handler; }
  ...
  {
    return Lex_column_charset_collation_attrs(m_ci, (Lex_column_charset_collation_attrs_st::Type)m_collation_type);
  }
};
...
struct Lex_dyncol_type_st : public Lex_length_and_dec_st
{
 ...
 public:
  void set(int type, Lex_length_and_dec_st length_and_dec, CHARSET_INFO *cs = NULL)
  {
    m_type = type;
    m_ci = cs;
    Lex_length_and_dec_st::operator=(length_and_dec);
  ...
  bool set(int type, const Lex_column_charset_collation_attrs_st &collation, CHARSET_INFO *charset)
  {
...
{
 public:
  Lex_for_loop_bounds_intrange(int8 direction, class sp_assignment_lex *left_expr, class sp_assignment_lex *right_expr)
  {
...
struct Lex_for_loop_st
{
 public:
  ...
  void init(const Lex_for_loop_st &other) { *this = other; }
  bool is_for_loop_cursor() const { return m_target_bound == NULL; }
  bool is_for_loop_explicit_cursor() const { return is_for_loop_cursor() && !m_implicit_cursor; }
};
...
struct Lex_trim_st
{
 ...
 public:
  ...
  void set(trim_spec spec, Item *source) { set(spec, NULL, source); }
...
class Load_data_param
{
 ...
 public:
  Load_data_param(CHARSET_INFO *cs, bool is_fixed_length)
      : m_charset(cs), m_fixed_length(0), m_is_fixed_length(is_fixed_length), m_use_blobs(false)
  {
  }
...
class Load_data_outvar
{
 public:
  ...
  virtual bool load_data_set_value(THD *thd, const char *pos, uint length, const Load_data_param *param) = 0;
...
class Timeval : public timeval
{
 ...
 public:
  ...
  explicit Timeval(const timeval &tv) : timeval(tv) {}
};
