...
int Rpl_filter::parse_filter_rule(const char *spec, Add_filter add)
{
  int status = 0;
  char *arg, *ptr, *pstr;
  ...
  while (pstr)
  {
    ...
    /* Skip leading spaces.  */
    while (my_isspace(system_charset_info, *arg)) arg++;
...
int Rpl_filter::add_table_rule(HASH *h, const char *table_spec)
{
  ...
  TABLE_RULE_ENT *e = (TABLE_RULE_ENT *)my_malloc(key_memory_TABLE_RULE_ENT, sizeof(TABLE_RULE_ENT) + len, MYF(MY_WME));
  if (!e)
    return 1;
...
int Rpl_filter::add_wild_table_rule(DYNAMIC_ARRAY *a, const char *table_spec)
{
  const char *dot = strchr(table_spec, '.');
  if (!dot)
    return 1;
...
int Rpl_filter::add_string_pair_list(const char *spec)
{
...
int Rpl_filter::add_rewrite_db(const char *table_spec)
{
...
uchar *get_table_key(const uchar *a, size_t *len, my_bool __attribute__((unused)))
{
...
{
  my_hash_init(key_memory_TABLE_RULE_ENT, h, system_charset_info, TABLE_RULE_HASH_SIZE, 0, 0, get_table_key,
               free_table_ent, 0);
  *h_inited = 1;
}
...
{
  my_init_dynamic_array(key_memory_TABLE_RULE_ENT, a, sizeof(TABLE_RULE_ENT *), TABLE_RULE_ARR_SIZE,
...
TABLE_RULE_ENT *Rpl_filter::find_wild(DYNAMIC_ARRAY *a, const char *key, int len)
{
  ...
  for (i = 0; i < a->elements; i++)
  {
    ...
    if (!system_charset_info->wildcmp(key, key_end, (const char *)e->db, (const char *)(e->db + e->key_len), '\\',
                                      wild_one, wild_many))
      return e;
  }
...
void Rpl_filter::table_rule_ent_dynamic_array_to_str(String *s, DYNAMIC_ARRAY *a, bool inited)
{
  s->length(0);
  if (inited)
  {
...
void Rpl_filter::get_do_table(String *str) { table_rule_ent_hash_to_str(str, &do_table, do_table_inited); }

void Rpl_filter::get_ignore_table(String *str) { table_rule_ent_hash_to_str(str, &ignore_table, ignore_table_inited); }
...
bool Rpl_filter::rewrite_db_is_empty() { return rewrite_db.is_empty(); }

I_List<i_string_pair> *Rpl_filter::get_rewrite_db() { return &rewrite_db; }

void Rpl_filter::db_rewrite_rule_ent_list_to_str(String *str, I_List<i_string_pair> *list)
{
...
void Rpl_filter::db_rule_ent_list_to_str(String *str, I_List<i_string> *list)
{
...
void Rpl_filter::get_rewrite_db(String *str) { db_rewrite_rule_ent_list_to_str(str, get_rewrite_db()); }

void Rpl_filter::get_do_db(String *str) { db_rule_ent_list_to_str(str, get_do_db()); }
