...
#pragma GCC target("no-sse")
  ...
  */
  ...
  explicit Bitmap(uint prefix) { set_prefix(prefix); }
  void init(uint prefix) { set_prefix(prefix); }
  uint length() const { return width; }
  void set_bit(uint n) { buffer[bit_index(n)] |= bit_mask(n); }
  void clear_bit(uint n) { buffer[bit_index(n)] &= ~bit_mask(n); }
  bool is_set(uint n) const { return buffer[bit_index(n)] & bit_mask(n); }
  void set_prefix(uint prefix_size)
  {
    ...
    for (size_t i = 0; i < idx; i++) buffer[i] = ALL_BITS_SET;

    if (prefix_size % BITS_PER_ELEMENT)
      buffer[idx++] = last_element_mask(prefix_size);

    for (size_t i = idx; i < ARRAY_ELEMENTS; i++) buffer[i] = 0;
  }
  ...
  {
    for (size_t i = 0; i < ARRAY_ELEMENTS; i++) buffer[i] &= map2.buffer[i];
  }
  ...
  */
  void intersect_and_pad(ulonglong map2buff, bool pad_with_ones)
  {
    ...
    for (size_t i = 1; i < ARRAY_ELEMENTS; i++) buffer[i] = pad_with_ones ? ALL_BITS_SET : 0;
 ...
 public:
  void intersect(ulonglong map2buff) { intersect_and_pad(map2buff, 0); }
  /* Use highest bit for all bits above first element. */
  void intersect_extended(ulonglong map2buff) { intersect_and_pad(map2buff, (map2buff & (1ULL << 63))); }
  ...
  */
  ...
  ulonglong to_ulonglong() const { return buffer[0]; }
  uint bits_set()
  {
    uint res = 0;
    for (size_t i = 0; i < ARRAY_ELEMENTS; i++) res += my_count_bits(buffer[i]);
    return res;
  }
  class Iterator
  {
