...
double get_merge_cost(ha_rows num_elements, ha_rows num_buffers, uint elem_size)
{
  return 2.0 * ((double)num_elements * elem_size) / IO_SIZE +
         (double)num_elements * log((double)num_buffers) / (TIME_FOR_COMPARE_ROWID * M_LN2);
}
}  // namespace
...
double get_merge_many_buffs_cost_fast(ha_rows num_rows, ha_rows num_keys_per_buffer, uint elem_size)
{
  ...
  total_cost =
      ((num_buffers * num_keys_per_buffer * log(1.0 + num_keys_per_buffer) + last_n_elems * log(1.0 + last_n_elems)) /
  ...
  while (num_buffers >= MERGEBUFF2)
  {
    ...
    const ha_rows num_remaining_buffs = num_buffers - num_merge_calls * MERGEBUFF;

    // Cost of merge sort 'num_merge_calls'.
    total_cost += num_merge_calls * get_merge_cost(num_keys_per_buffer * MERGEBUFF, MERGEBUFF, elem_size);
    ...
    total_cost += get_merge_cost(last_n_elems, 1 + num_remaining_buffs, elem_size);
...
uchar *Filesort_buffer::alloc_sort_buffer(uint num_records, uint record_length)
{
  ...
  DBUG_EXECUTE_IF("alloc_sort_buffer_fail", DBUG_SET("+d,simulate_out_of_memory"););
  ...
  if (m_rawmem)
  {
    ...
    */
    if (buff_size > m_size_in_bytes)
    {
      ...
      */
      my_free(m_rawmem);
      if (!(m_rawmem = (uchar *)my_malloc(key_memory_Filesort_buffer_sort_keys, buff_size, MYF(MY_THREAD_SPECIFIC))))
      {
        m_size_in_bytes = 0;
        DBUG_RETURN(0);
      }
  ...
  m_record_pointers = reinterpret_cast<uchar **>(m_rawmem) + ((m_size_in_bytes / sizeof(uchar *)) - 1);
...
void Filesort_buffer::sort_buffer(const Sort_param *param, uint count)
{
  ...
  uchar **buffer = NULL;
  if (!param->using_packed_sortkeys() && radixsort_is_appliccable(count, param->sort_length) &&
      (buffer = (uchar **)my_malloc(PSI_INSTRUMENT_ME, count * sizeof(char *), MYF(MY_THREAD_SPECIFIC))))
  {
  ...
  my_qsort2(m_sort_keys, count, sizeof(uchar *), param->get_compare_function(), param->get_compare_argument(&size));
}
