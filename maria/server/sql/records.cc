...
bool init_read_record_idx(READ_RECORD *info, THD *thd, TABLE *table, bool print_error, uint idx, bool reverse)
{
  ...
  if (!table->file->inited && unlikely(error = table->file->ha_index_init(idx, 1)))
  {
    if (print_error)
...
bool init_read_record(READ_RECORD *info, THD *thd, TABLE *table, SQL_SELECT *select, SORT_INFO *filesort,
                      int use_record_cache, bool print_error, bool disable_rr_cache)
{
  IO_CACHE *tempfile;
  ...
  if (tempfile && !(select && select->quick))
  {
    ...
    */
    if (!disable_rr_cache && !using_addon_fields && thd->variables.read_rnd_buff_size &&
        !(table->file->ha_table_flags() & HA_FAST_KEY_READ) &&
        (table->db_stat & HA_READ_ONLY || table->reginfo.lock_type < TL_FIRST_WRITE) &&
        (ulonglong)table->s->reclength * (table->file->stats.records + table->file->stats.deleted) >
    ...
    if (using_addon_fields)
    {
      ...
      if (filesort->using_packed_addons())
      {
        info->read_record_func =
            using_packed_sortkeys ? rr_unpack_from_buffer<true, true> : rr_unpack_from_buffer<true, false>;
      }
      else
      {
        info->read_record_func =
            using_packed_sortkeys ? rr_unpack_from_buffer<false, true> : rr_unpack_from_buffer<false, false>;
      }
    }
    else
    {
      info->cache_end = (info->cache_pos + filesort->return_rows * info->ref_length);
  ...
  else if (table->file->keyread_enabled())
  {
    ...
    if (!table->file->inited && unlikely((error = table->file->ha_index_init(table->file->keyread, 1))))
    {
      if (print_error)
        table->file->print_error(error, MYF(0));
      DBUG_RETURN(1);
  ...
  else
  {
    ...
    if (!table->no_cache && (use_record_cache > 0 || (int)table->reginfo.lock_type <= (int)TL_READ_HIGH_PRIORITY ||
                             ...
                             (use_record_cache < 0 && !(table->file->ha_table_flags() & HA_NOT_DELETE_WITH_CACHE))))
      (void)table->file->extra_opt(HA_EXTRA_CACHE, thd->variables.read_buff_size);
  }
  /* Condition pushdown to storage engine */
  if ((table->file->ha_table_flags() & HA_CAN_TABLE_CONDITION_PUSHDOWN) && select && select->cond &&
...
static int rr_unpack_from_tempfile(READ_RECORD *info)
{
  ...
  if (Packed_addon_fields)
  {
    ...
    DBUG_PRINT("info", ("rr_unpack from %llu to %p sz %u", static_cast<ulonglong>(where), destination, res_length));
...
static int rr_unpack_from_buffer(READ_RECORD *info)
{
  ...
  uchar *record = info->sort_info->get_sorted_record(static_cast<uint>(info->unpack_counter));

  uint sort_length = Packed_sort_keys ? Sort_keys::read_sortkey_length(record) : info->sort_info->get_sort_length();
...
static int init_rr_cache(THD *thd, READ_RECORD *info)
{
  ...
  cache_records = thd->variables.read_rnd_buff_size / (info->reclength + STRUCT_LENGTH);
  rec_cache_size = cache_records * info->reclength;
  info->rec_cache_size = cache_records * info->ref_length;

  // We have to allocate one more byte to use uint3korr (see comments for it)
  if (cache_records <= 2 || !(info->cache = (uchar *)my_malloc_lock(rec_cache_size + cache_records * STRUCT_LENGTH + 1,
...
static int rr_from_cache(READ_RECORD *info)
{
  ...
  for (;;)
  {
    ...
    for (i = 0; i < length; i++)
    {
      ...
      if (unlikely((error = (int16)info->table->file->ha_rnd_pos(record_pos, info->ref_pos))))
      {
...
int read_record_func_for_rr_and_unpack(READ_RECORD *info)
{
  ...
  for (Copy_field *cp = info->copy_field; cp != info->copy_field_end; cp++) (*cp->do_copy)(cp);
