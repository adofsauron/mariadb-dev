...
Alter_info::Alter_info(const Alter_info &rhs, MEM_ROOT *mem_root)
    : drop_list(rhs.drop_list, mem_root),
      ...
      flags(rhs.flags),
      partition_flags(rhs.partition_flags),
...
void Alter_info::set_requested_algorithm(enum_alter_table_algorithm algo_val) { requested_algorithm = algo_val; }
...
bool Alter_info::supports_algorithm(THD *thd, const Alter_inplace_info *ha_alter_info)
{
  switch (ha_alter_info->inplace_supported)
  {
    ...
    case HA_ALTER_INPLACE_COPY_LOCK:
      ...
      {
        ha_alter_info->report_unsupported_error(algorithm_clause(thd), "ALGORITHM=INPLACE");
        return true;
      }
      return false;
    ...
    case HA_ALTER_INPLACE_NOCOPY_LOCK:
      ...
      {
        ha_alter_info->report_unsupported_error("ALGORITHM=INSTANT", "ALGORITHM=NOCOPY");
        return true;
      }
      return false;
    case HA_ALTER_INPLACE_NOT_SUPPORTED:
      ...
      {
        ha_alter_info->report_unsupported_error(algorithm_clause(thd), "ALGORITHM=COPY");
        return true;
      }
      return false;
    case HA_ALTER_ERROR:
...
bool Alter_info::supports_lock(THD *thd, const Alter_inplace_info *ha_alter_info)
{
  switch (ha_alter_info->inplace_supported)
  {
    case HA_ALTER_INPLACE_EXCLUSIVE_LOCK:
      ...
      if (requested_lock == Alter_info::ALTER_TABLE_LOCK_SHARED &&
          ...
          thd->variables.alter_algorithm == Alter_info::ALTER_TABLE_ALGORITHM_DEFAULT)
        return false;

      if (requested_lock == Alter_info::ALTER_TABLE_LOCK_SHARED || requested_lock == Alter_info::ALTER_TABLE_LOCK_NONE)
      {
...
{
  if (thd->slave_thread || thd->variables.vers_alter_history != VERS_ALTER_HISTORY_ERROR)
  {
    return false;
  }
  if (flags & (ALTER_PARSER_ADD_COLUMN | ALTER_PARSER_DROP_COLUMN | ALTER_CHANGE_COLUMN | ALTER_COLUMN_ORDER))
  {
    return true;
  }
...
Alter_info::enum_alter_table_algorithm Alter_info::algorithm(const THD *thd) const
{
...
Alter_table_ctx::Alter_table_ctx()
    : db(null_clex_str),
      table_name(null_clex_str),
      alias(null_clex_str),
      new_db(null_clex_str),
      new_name(null_clex_str),
      new_alias(null_clex_str)
{
}
...
Alter_table_ctx::Alter_table_ctx(THD *thd, TABLE_LIST *table_list, uint tables_opened_arg,
    ...
    : tables_opened(tables_opened_arg), new_db(*new_db_arg), new_name(*new_name_arg)
{
  ...
  */
  ...
  if (new_name.str)
  {
    ...
    if (!is_database_changed() && !my_strcasecmp(table_alias_charset, new_name.str, table_name.str))
    {
      /*
  ...
  tmp_name.length = my_snprintf(tmp_name_buff, sizeof(tmp_name_buff), "%s-alter-%lx-%llx", tmp_file_prefix, current_pid,
                                thd->thread_id);
  ...
  if (table_list->table->s->tmp_table == NO_TMP_TABLE)
  {
    ...
    build_table_filename(new_filename, sizeof(new_filename) - 1, new_db.str, new_name.str, reg_ext, 0);

    build_table_filename(tmp_path, sizeof(tmp_path) - 1, new_db.str, tmp_name.str, "", FN_IS_TMP);
  }
  else
  {
    /*
  ...
  storage_engine_name.length =
      ((strmake(storage_engine_buff, table_list->table->file->real_table_type(), sizeof(storage_engine_buff) - 1)) -
       storage_engine_buff);
...
void Alter_table_ctx::report_implicit_default_value_error(THD *thd, const TABLE_SHARE *s) const
{
  Create_field *error_field = implicit_default_value_error_field;
  const Type_handler *h = error_field->type_handler();
  thd->push_warning_truncated_value_for_field(Sql_condition::WARN_LEVEL_WARN, h->name().ptr(), h->default_value().ptr(),
                                              s ? s->db.str : nullptr, s ? s->table_name.str : nullptr,
...
bool Sql_cmd_alter_table::execute(THD *thd)
{
  ...
  if (used_engine)
  {
    if (resolve_storage_engine_with_error(thd, &lex->create_info.db_type, lex->create_info.tmp_table()))
  ...
  */
  if ((alter_info.partition_flags & ALTER_PARTITION_DROP) ||
      ...
      (alter_info.partition_flags & ALTER_PARTITION_CONVERT_OUT) || (alter_info.flags & ALTER_RENAME))
  ...
  if (check_access(thd, priv_needed, first_table->db.str, &first_table->grant.privilege, &first_table->grant.m_internal,
                   0, 0) ||
      check_access(thd, INSERT_ACL | CREATE_ACL, select_lex->db.str, &priv,
  ...
  if (create_info.merge_list)
  {
    ...
    */
    ...
    if (check_table_access(thd, SELECT_ACL | UPDATE_ACL | DELETE_ACL, create_info.merge_list, FALSE, UINT_MAX, FALSE))
      DBUG_RETURN(TRUE);
  }
  ...
  if (WSREP(thd) && (!thd->is_current_stmt_binlog_format_row() || !thd->find_temporary_table(first_table)))
  {
    ...
    WSREP_TO_ISOLATION_BEGIN_ALTER(lex->name.str ? select_lex->db.str : first_table->db.str,
                                   lex->name.str ? lex->name.str : first_table->table_name.str, first_table,
                                   &alter_info, &keys, used_engine ? &create_info : nullptr)
    {
  ...
  result =
      mysql_alter_table(thd, &select_lex->db, &lex->name, &create_info, first_table, &alter_info,
                        select_lex->order_list.elements, select_lex->order_list.first, lex->ignore, lex->if_exists());
...
bool Sql_cmd_discard_import_tablespace::execute(THD *thd)
{
  ...
  if (check_access(thd, ALTER_ACL, table_list->db.str, &table_list->grant.privilege, &table_list->grant.m_internal, 0,
                   0))
    return true;
