...
String *Item_func::val_str_from_val_str_ascii(String *str, String *ascii_buffer)
{
  ...
  if ((null_value = str->copy(res->ptr(), res->length(), &my_charset_latin1, collation.collation, &errors)))
    return 0;
...
String *Item_func_sha2::val_str_ascii(String *str)
{
  ...
  switch (digest_length)
  {
    ...
    default:
      ...
      {
        THD *thd = current_thd;
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_PARAMETERS_TO_NATIVE_FCT,
...
String *Item_aes_crypt::val_str(String *str2)
{
  ...
  if (sptr && user_key)  // we need both arguments to be not NULL
  {
    ...
    if (!str2->alloc(aes_length))  // Ensure that memory is free
    {
      ...
      if (!my_aes_crypt(MY_AES_ECB, what, (uchar *)sptr->ptr(), sptr->length(), (uchar *)str2->ptr(), &aes_length, rkey,
                        AES_KEY_LENGTH / 8, 0, 0))
      {
...
String *Item_func_to_base64::val_str_ascii(String *str)
{
  ...
  bool too_long = false;
  int length;
  if (!res || res->length() > (uint)my_base64_encode_max_arg_length() ||
      (too_long = ((uint)(length = my_base64_needed_encoded_length((int)res->length())) >
      ...
      str->alloc((uint)length))
  {
    ...
    {
      THD *thd = current_thd;
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
...
String *Item_func_from_base64::val_str(String *str)
{
       ...
       current_thd->variables.max_allowed_packet))
  {
    THD *thd = current_thd;
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                        ER_THD(thd, ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(), thd->variables.max_allowed_packet);
    goto err;
  }
  ...
  if ((length = my_base64_decode(res->ptr(), (int)res->length(), (char *)str->ptr(), &end_ptr, 0)) < 0 ||
      end_ptr < res->ptr() + res->length())
  {
    THD *thd = current_thd;
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_BAD_BASE64_DATA, ER_THD(thd, ER_BAD_BASE64_DATA),
                        (int)(end_ptr - res->ptr()));
    goto err;
  }
...
const char *histogram_types[] = {"SINGLE_PREC_HB", "DOUBLE_PREC_HB", "JSON_HB", 0};
static TYPELIB histogram_types_typelib = {array_elements(histogram_types), "histogram_types", histogram_types, NULL};
...
String *Item_func_decode_histogram::val_str(String *str)
{
  ...
  tmp.length(0);
  if (!(res = args[0]->val_str(&tmp)) || (type = find_type(res->c_ptr_safe(), &histogram_types_typelib, MYF(0))) <= 0)
  {
    null_value = 1;
    return 0;
  }
  ...
  for (i = 0; i < res->length(); i++)
  {
    ...
    size_t size = my_snprintf(numbuf, sizeof(numbuf), representation_by_type[type], val - prev);
    str->append(numbuf, size);
    str->append(',');
    prev = val;
  }
  /* show delta with max */
  size_t size = my_snprintf(numbuf, sizeof(numbuf), representation_by_type[type], 1.0 - prev);
  str->append(numbuf, size);
...
String *Item_func_concat::val_str(String *str)
{
  ...
  {
    if (!(res = args[i]->val_str(&tmp_value)) || append_value(thd, str, res))
      goto null;
  }
...
{
  uint concat_len;
  if ((concat_len = res->length() + app->length()) > thd->variables.max_allowed_packet)
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                        ER(ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(), thd->variables.max_allowed_packet);
    return true;
  }
...
bool Item_func_des_encrypt::fix_length_and_dec(THD *thd)
{
  ...
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_WARN_DEPRECATED_SYNTAX,
                      ER_THD(thd, ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT), func_name_cstring().str);
  return FALSE;
}
...
String *Item_func_des_encrypt::val_str(String *str)
{
  ...
  else
  {
    ...
    if (!EVP_BytesToKey(EVP_des_ede3_cbc(), EVP_md5(), NULL, (uchar *)keystr->ptr(), (int)keystr->length(), 1,
                        (uchar *)&keyblock, ivec))
      goto error;
  ...
  */
  ...
  // Real encryption
  bzero((char *)&ivec, sizeof(ivec));
  DES_ede3_cbc_encrypt((const uchar *)(tmp_arg.ptr()), (uchar *)(str->ptr() + 1), res_length, &keyschedule.ks1,
                       &keyschedule.ks2, &keyschedule.ks3, &ivec, TRUE);
  return str;

error:
  THD *thd = current_thd;
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, code, ER_THD(thd, code), "des_encrypt");
#else
  THD *thd = current_thd;
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_FEATURE_DISABLED, ER_THD(thd, ER_FEATURE_DISABLED),
...
String *Item_func_des_decrypt::val_str(String *str)
{
  ...
  if (arg_count == 1)  // If automatic uncompression
  {
    ...
    if (!(current_thd->security_ctx->master_access & PRIV_DES_DECRYPT_ONE_ARG) || key_number > 9)
      goto error;
  ...
  bzero((char *)&ivec, sizeof(ivec));
  DES_ede3_cbc_encrypt((const uchar *)res->ptr() + 1, (uchar *)(str->ptr()), length - 1, &keyschedule.ks1,
...
{
  THD *thd = current_thd;
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, code, ER_THD(thd, code), "des_decrypt");
}
wrong_key:
#else
  {
...
String *Item_func_concat_ws::val_str(String *str)
{
  ...
  for (i++; i < arg_count; i++)
  {
    ...
    if (!thd)
      thd = current_thd;
    if (res->length() + sep_str->length() + res2->length() > thd->variables.max_allowed_packet)
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                          ER_THD(thd, ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(),
                          thd->variables.max_allowed_packet);
    ...
    if (!is_const && res->alloced_length() >= res->length() + sep_str->length() + res2->length())
    {                         // Use old buffer
    ...
    {  // Two big const strings
      ...
      */
      ...
      if (tmp_value.copy(*res) || tmp_value.append(*sep_str) || tmp_value.append(*res2))
        goto null;
      res = &tmp_value;
      use_as_buff = str;
    }
...
String *Item_func_replace::val_str_internal(String *str, String *empty_string_for_null)
{
  DBUG_ASSERT(fixed());
    ...
    */
    ...
    while (ptr < end)
    {
      if (*ptr == *search)
      {
        ...
        if (res->length() - from_length + to_length > thd->variables.max_allowed_packet)
        {
          push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                              ER_THD(thd, ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(),
                              thd->variables.max_allowed_packet);
...
/*
  ...
  the item itself
*/
Item_func_sformat::Item_func_sformat(THD *thd, List<Item> &list) : Item_str_func(thd, list)
{
...
bool Item_func_sformat::fix_length_and_dec(THD *thd)
{
  ...
  if (Type_std_attributes::agg_item_collations(collation, func_name_cstring(), args, arg_count, flags, 1))
    return TRUE;
  ...
  for (uint i = 0; i < arg_count; i++)
  {
    ...
    if (args[i]->result_type() == STRING_RESULT &&
        Type_std_attributes::agg_item_set_converter(c, func_name_cstring(), args + i, 1, flags, 1))
      return TRUE;
  }
...
String *Item_func_sformat::val_str(String *res)
{
  ...
  try
  {
    auto text = fmt::vformat(fmt_arg->c_ptr_safe(), fmt::format_args(vargs, arg_count - 1));
    res->length(0);
    res->set_charset(collation.collation);
  ...
  {
    THD *thd = current_thd;
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, WARN_SFORMAT_ERROR, ER_THD(thd, WARN_SFORMAT_ERROR),
                        ex.what());
    null_value = true;
  }
  delete[] vargs;
...
*/
bool Item_func_regexp_replace::append_replacement(String *str, const LEX_CSTRING *source, const LEX_CSTRING *replace)
{
  ...
  for (;;)
  {
    ...
    if ((cnv = cs->mb_wc(&wc, (const uchar *)beg, (const uchar *)end)) < 1)
      break; /* End of line */
    beg += cnv;
...
String *Item_func_regexp_replace::val_str(String *str)
{
  ...
  if ((null_value = (args[0]->null_value || args[2]->null_value || re.recompile(args[1]))))
    return (String *)0;
...
String *Item_func_regexp_substr::val_str(String *str)
{
  ...
  if (str->append(source->ptr() + re.subpattern_start(0), re.subpattern_length(0), re.library_charset()))
    goto err;
...
String *Item_func_insert::val_str(String *str)
{
  ...
  */
  ...
  {
    THD *thd = current_thd;
    if ((ulonglong)(res->length() - length + res2->length()) > (ulonglong)thd->variables.max_allowed_packet)
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                          ER_THD(thd, ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(),
                          thd->variables.max_allowed_packet);
...
bool Item_func_insert::fix_length_and_dec(THD *thd)
{
  ...
  if (agg_arg_charsets_for_string_result(collation, args, 2, 3))
    return TRUE;
  char_length = ((ulonglong)args[0]->max_char_length() + (ulonglong)args[3]->max_char_length());
  fix_char_length_ulonglong(char_length);
  return FALSE;
}
...
String *Item_str_conv::val_str(String *str)
{
  ...
  if ((null_value = (!(res = args[0]->val_str(&tmp_value)) || str->alloc((alloced_length = res->length() * multiply)))))
    return 0;
...
String *Item_func_left::val_str(String *str)
{
  ...
  if ((length <= 0) && (!args[1]->unsigned_flag))
    return make_empty_result(str);
  if ((res->length() <= (ulonglong)length) || (res->length() <= (char_pos = res->charpos((int)length))))
    return res;
...
String *Item_func_substr::val_str(String *str)
{
  ...
  if ((arg_count == 3) && (length <= 0) && (length == 0 || !args[2]->unsigned_flag))
    return make_empty_result(str);
  ...
  if (res->use_mb())
  {
    ...
    for (pass = (count > 0); pass < 2; ++pass)
    {
      ...
      else
      {
        ...
        {
          ptr += delimiter_length;
          str->copy(res->ptr() + (ptr - res->ptr()), (uint32)(strend - ptr), collation.collation);
          return str;
        }
      }
    }
      ...
      for (offset = res->length(); offset;)
      {
        ...
        */
        if (!++count)
        {
          ...
          str->copy(res->ptr() + offset, res->length() - offset, collation.collation);
          return str;
        }
      }
      if (count)
...
bool Item_func_trim::fix_length_and_dec(THD *thd)
{
  ...
  else
  {
    ...
    if (agg_arg_charsets_for_string_result_with_comparison(collation, &args[1], 2, -1))
      return TRUE;
  }
  fix_char_length(args[0]->max_char_length());
  return FALSE;
...
Sql_mode_dependency Item_func_trim::value_depends_on_sql_mode() const
{
  ...
  if (arg_count == 1)  // RTRIM(expr)
    return (args[0]->value_depends_on_sql_mode() & Sql_mode_dependency(~0, ~MODE_PAD_CHAR_TO_FULL_LENGTH))
        .soft_to_hard();
  // TRIM(... FROM expr)
  ...
  if (trimstr->length() > trimstr->charset()->mbminlen || trimstr->numchars() > 1)
...
char *Item_func_password::alloc(THD *thd, const char *password, size_t pass_len, enum PW_Alg al)
{
  char *buff =
      (char *)thd->alloc((al == NEW) ? SCRAMBLED_PASSWORD_CHAR_LENGTH + 1 : SCRAMBLED_PASSWORD_CHAR_LENGTH_323 + 1);
  if (!buff)
    return NULL;
...
{
  max_length = args[0]->max_length;
  base_flags |= ((args[0]->base_flags | args[1]->base_flags) & item_base_t::MAYBE_NULL);
  collation.set(&my_charset_bin);
  /* Precompute the seed state if the item is constant. */
  seeded = args[1]->const_item() && (args[1]->result_type() == STRING_RESULT) && !seed();
  return FALSE;
}
...
void Item_func_decode::crypto_transform(String *res) { sql_crypt.decode((char *)res->ptr(), res->length()); }
...
String *Item_func_sqlerrm::val_str(String *str)
{
  ...
  if ((err = it++))
  {
    str->copy(err->get_message_text(), err->get_message_octet_length(), system_charset_info);
    return str;
  }
  str->copy(STRING_WITH_LEN("normal, successful completion"), system_charset_info);
  return str;
}
...
bool Item_func_user::init(const char *user, const char *host)
{
  ...
  if (user)
  {
    ...
    res_length = cs->cset->snprintf(cs, (char *)str_value.ptr(), (uint)res_length, "%s@%s", user, host);
...
bool Item_func_user::fix_fields(THD *thd, Item **ref)
{
  return (Item_func_sysconst::fix_fields(thd, ref) ||
          init(thd->main_security_ctx.user, thd->main_security_ctx.host_or_ip));
}
...
bool Item_func_current_user::fix_fields(THD *thd, Item **ref)
{
  ...
  Security_context *ctx = context && context->security_ctx ? context->security_ctx : thd->security_ctx;
...
bool Item_func_current_role::fix_fields(THD *thd, Item **ref)
{
  ...
  if (ctx->priv_role[0])
  {
    if (str_value.copy(ctx->priv_role, strlen(ctx->priv_role), system_charset_info))
      return 1;
    str_value.mark_as_const();
    null_value = 0;
...
Sql_mode_dependency Item_func_rpad::value_depends_on_sql_mode() const
{
  ...
  String *padstr = arg_count == 3 ? args[2]->val_str(&padstrbuf) : default_pad_str(&padstrbuf, collation.collation);
...
String *Item_func_lpad::val_str(String *str)
{
  ...
  if (!res || args[1]->null_value || !pad || ((count < 0) && !args[1]->unsigned_flag))
    goto err;
...
String *Item_func_conv::val_str(String *str)
{
  ...
  if (args[0]->null_value || args[1]->null_value || args[2]->null_value || from_base == INT_MIN || to_base == INT_MIN ||
      abs(to_base) > 36 || abs(to_base) < 2 || abs(from_base) > 36 || abs(from_base) < 2 || !(res->length()))
  {
    null_value = 1;
    return NULL;
  }
  ...
  {
    if (from_base < 0)
      dec = res->charset()->strntoll(res->ptr(), res->length(), -from_base, &endptr, &err);
    else
      dec = (longlong)res->charset()->strntoull(res->ptr(), res->length(), from_base, &endptr, &err);
  }

  if (!(ptr = longlong2str(dec, ans, to_base)) || str->copy(ans, (uint32)(ptr - ans), default_charset()))
  {
    null_value = 1;
    return NULL;
  }
...
String *Item_func_conv_charset::val_str(String *str)
{
  ...
  return ((null_value = args[0]->null_value || copier.copy_with_warn(collation.collation, str, arg->charset(),
                                                                     arg->ptr(), arg->length(), arg->length())))
             ? 0
             : str;
}
...
bool Item_func_set_collation::fix_length_and_dec(THD *thd)
{
  ...
  if (cl.merge_collation_override(m_set_collation))
    return true;
  collation.set(cl.collation().charset_info(), DERIVATION_EXPLICIT, args[0]->collation.repertoire);
  max_length = args[0]->max_length;
  return FALSE;
}
...
{
  return Item_func::eq(item, binary_cmp) && collation.collation == item->collation.collation;
}
...
String *Item_func_charset::val_str(String *str)
{
  ...
  null_value = 0;
  str->copy(cs->cs_name.str, cs->cs_name.length, &my_charset_latin1, collation.collation, &dummy_errors);
  return str;
}
...
String *Item_func_collation::val_str(String *str)
{
  ...
  null_value = 0;
  str->copy(cs->coll_name.str, cs->coll_name.length, &my_charset_latin1, collation.collation, &dummy_errors);
  return str;
}
  ...
  */
  if (!(max_length = result_length))
  {
    ...
    char_length = ((cs->state & MY_CS_STRNXFRM_BAD_NWEIGHTS) || !nweights)
                      ...
                      : nweights * my_count_bits_uint32(cs->levels_for_order);
...
String *Item_func_weight_string::val_str(String *str)
{
  ...
  if (args[0]->result_type() != STRING_RESULT || !(res = args[0]->val_str(&tmp_value)))
    goto nl;
  ...
  */
    ...
    else
    {
      ...
      */
      if (!(char_length = nweights))
        char_length = (weigth_flags & MY_STRXFRM_PAD_WITH_SPACE) ? res->numchars() : (res->length() / cs->mbminlen);
    }
  ...
  frm_length = cs->strnxfrm((char *)str->ptr(), tmp_length, nweights ? nweights : (uint)tmp_length, res->ptr(),
                            res->length(), weigth_flags);
...
String *Item_func_like_range::val_str(String *str)
{
  ...
  if (!res || args[0]->null_value || args[1]->null_value || nbytes < 0 || nbytes > MAX_BLOB_WIDTH ||
      min_str.alloc((size_t)nbytes) || max_str.alloc((size_t)nbytes))
    goto err;
  null_value = 0;

  if (cs->like_range(res->ptr(), res->length(), '\\', '_', '%', (size_t)nbytes, (char *)min_str.ptr(),
                     (char *)max_str.ptr(), &min_len, &max_len))
    goto err;
...
String *Item_load_file::val_str(String *str)
{
  ...
  (void)fn_format(path, file_name->c_ptr_safe(), mysql_real_data_home, "", MY_RELATIVE_PATH | MY_UNPACK_FILENAME);
  ...
  if (tmp_value.alloc((ulong)file_size))
    goto err;
  if ((file = mysql_file_open(key_file_loadfile, file_name->ptr(), O_RDONLY, MYF(0))) < 0)
    goto err;
  if (mysql_file_read(file, (uchar *)tmp_value.ptr(), (size_t)stat_info.st_size, MYF(MY_NABP)))
  {
...
String *Item_func_export_set::val_str(String *str)
{
  ...
  */
  ...
  const ulonglong max_total_length =
      num_set_values * MY_MAX(yes->length(), no->length()) + num_separators * sep->length();

  if (unlikely(max_total_length > max_allowed_packet))
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                        ER_THD(thd, ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(), max_allowed_packet);
    null_value = true;
    return NULL;
  }
...
bool Item_func_export_set::fix_length_and_dec(THD *thd)
{
  ...
  if (agg_arg_charsets_for_string_result(collation, args + 1, MY_MIN(4, arg_count) - 1))
    return TRUE;
...
toolong:
  push_warning_printf(current_thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                      ER_THD(current_thd, ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(), max_allowed_packet);
null:
  null_value = 1;
  return 0;
}
...
longlong Item_func_uncompressed_length::val_int()
{
  ...
  */
  ...
  {
    THD *thd = current_thd;
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_ZLIB_Z_DATA_ERROR, ER_THD(thd, ER_ZLIB_Z_DATA_ERROR));
    null_value = 1;
    return 0;
  }
...
longlong Item_func_crc32::val_int()
{
  ...
  return static_cast<longlong>(ulonglong{crc_func(uint32_t(crc), res->ptr(), res->length())});
}
...
String *Item_func_compress::val_str(String *str)
{
  ...
  */
  ...
  if (((uint32)(new_size + 5) <= res->length()) || str->alloc((uint32)new_size + 4 + 1))
  {
    null_value = 1;
    return 0;
  }
  ...
  if ((err = my_compress_buffer(body, &new_size, (const uchar *)res->ptr(), res->length())) != Z_OK)
  {
    THD *thd = current_thd;
...
String *Item_func_uncompress::val_str(String *str)
{
  ...
  {
    THD *thd = current_thd;
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_TOO_BIG_FOR_UNCOMPRESS,
                        ER_THD(thd, ER_TOO_BIG_FOR_UNCOMPRESS), static_cast<int>(thd->variables.max_allowed_packet));
    goto err;
  }
  if (str->alloc((uint32)new_size))
    goto err;

  if ((err = uncompress((Byte *)str->ptr(), &new_size, ((const Bytef *)res->ptr()) + 4, res->length() - 4)) == Z_OK)
  {
...
Item_func_dyncol_create::Item_func_dyncol_create(THD *thd, List<Item> &args, DYNCALL_CREATE_DEF *dfs)
    : Item_str_func(thd, args), defs(dfs), vals(0), keys_num(NULL), keys_str(NULL), names(FALSE), force_names(FALSE)
{
...
bool Item_func_dyncol_create::fix_fields(THD *thd, Item **ref)
{
  ...
  if (!res)
  {
    vals = (DYNAMIC_COLUMN_VALUE *)alloc_root(thd->mem_root, sizeof(DYNAMIC_COLUMN_VALUE) * (arg_count / 2));
    ...
    keys_num = (uint *)alloc_root(
        thd->mem_root, (sizeof(LEX_STRING) > sizeof(uint) ? sizeof(LEX_STRING) : sizeof(uint)) * (arg_count / 2));
...
bool Item_func_dyncol_create::prepare_arguments(THD *thd, bool force_names_arg)
{
  ...
  if (!(names || force_names))
  {
    for (i = 0; i < column_count; i++)
    {
      ...
      if (type == DYN_COL_STRING && args[valpos]->type() == Item::FUNC_ITEM &&
  ...
  for (i = 0; i < column_count; i++)
  {
    ...
    if (names || force_names)
    {
      ...
      if (res)
      {
        ...
        else
        {
          ...
          {
            keys_str[i].length =
                copy_and_convert(str, strlen, DYNCOL_UTF, res->ptr(), res->length(), res->charset(), &dummy_errors);
            keys_str[i].str = str;
          }
          else
            keys_str[i].length = 0;
    ...
    if (args[i * 2]->null_value)
    {
      ...
      for (; i < column_count; i++) vals[i].type = DYN_COL_NULL;
      return 1;
    }
    vals[i].type = type;
    switch (type)
    {
      ...
      case DYN_COL_STRING:
        ...
        if (res && (vals[i].x.string.value.str = thd->strmake(res->ptr(), res->length())))
        {
      ...
      case DYN_COL_DECIMAL:
        if ((dres = args[valpos]->val_decimal(&dtmp)))
        {
          ...
          memcpy(vals[i].x.decimal.buffer, dres->buf, sizeof(vals[i].x.decimal.buffer));
        }
        else
        {
      ...
      case DYN_COL_DATE:
        args[valpos]->get_date(thd, &vals[i].x.time_value, Datetime::Options(thd));
        break;
      case DYN_COL_TIME:
...
String *Item_func_dyncol_create::val_str(String *str)
{
  ...
  {
    if ((rc = ((names || force_names) ? mariadb_dyncol_create_many_named(&col, column_count, keys_str, vals, TRUE)
                                      : mariadb_dyncol_create_many_num(&col, column_count, keys_num, vals, TRUE))))
    {
...
void Item_func_dyncol_create::print_arguments(String *str, enum_query_type query_type)
{
  uint i;
  uint column_count = (arg_count / 2);
  for (i = 0; i < column_count; i++)
...
void Item_func_dyncol_create::print(String *str, enum_query_type query_type)
{
  DBUG_ASSERT((arg_count & 0x1) == 0);  // even number of arguments
...
String *Item_func_dyncol_add::val_str(String *str)
{
  ...
  if (args[arg_count - 1]->null_value ||
      init_dynamic_string(&col, NULL, res->length() + STRING_BUFFER_USUAL_SIZE, STRING_BUFFER_USUAL_SIZE))
    goto null;
  ...
  if ((rc = ((names || force_names) ? mariadb_dyncol_update_many_named(&col, column_count, keys_str, vals)
                                    : mariadb_dyncol_update_many_num(&col, column_count, keys_num, vals))))
  {
    dynamic_column_error_message(rc);
    mariadb_dyncol_free(&col);
    goto null;
...
void Item_func_dyncol_add::print(String *str, enum_query_type query_type)
{
  DBUG_ASSERT((arg_count & 0x1) == 1);  // odd number of arguments
...
bool Item_dyncol_get::get_dyn_value(THD *thd, DYNAMIC_COLUMN_VALUE *val, String *tmp)
{
  ...
  if ((rc = ((name == NULL) ? mariadb_dyncol_get_num(&dyn_str, (uint)num, val)
...
String *Item_dyncol_get::val_str(String *str_result)
{
  ...
  switch (val.type)
  {
    ...
    case DYN_COL_INT:
    case DYN_COL_UINT:
      str_result->set_int(val.x.long_value, MY_TEST(val.type == DYN_COL_UINT), &my_charset_latin1);
      break;
    case DYN_COL_DOUBLE:
    ...
    case DYN_COL_DYNCOL:
    case DYN_COL_STRING:
      if ((char *)tmp.ptr() <= val.x.string.value.str && (char *)tmp.ptr() + tmp.length() >= val.x.string.value.str)
      {
        /* value is allocated in tmp buffer; We have to make a copy */
        str_result->copy(val.x.string.value.str, val.x.string.value.length, val.x.string.charset);
      }
      else
      {
        /*
        ...
        */
        str_result->set(val.x.string.value.str, val.x.string.value.length, val.x.string.charset);
      }
      break;
    case DYN_COL_DECIMAL:
    {
      ...
      if (str_result->alloc(length))
        goto null;
      if ((res = decimal2string(&val.x.decimal.value, (char *)str_result->ptr(), &length, 0, 0, ' ')) != E_DEC_OK)
      {
        char buff[40];
        int len = sizeof(buff);
    ...
    case DYN_COL_TIME:
    {
      ...
      */
      if (str_result->alloc(MAX_DATE_STRING_REP_LENGTH) ||
          !(length = my_TIME_to_str(&val.x.time_value, (char *)str_result->ptr(), AUTO_SEC_PART_DIGITS)))
        goto null;
      str_result->set_charset(&my_charset_latin1);
      str_result->length(length);
      break;
...
longlong Item_dyncol_get::val_int()
{
  ...
  switch (val.type)
  {
    ...
    case DYN_COL_INT:
      ...
      return val.x.long_value;
    case DYN_COL_DOUBLE:
      return Converter_double_to_longlong_with_warn(thd, val.x.double_value, unsigned_flag).result();
    case DYN_COL_STRING:
    {
      int error;
      longlong num;
      ...
      {
        push_warning_printf(
            thd, Sql_condition::WARN_LEVEL_WARN, ER_BAD_DATA, ER_THD(thd, ER_BAD_DATA),
            ErrConvString(val.x.string.value.str, val.x.string.value.length, val.x.string.charset).ptr(),
    ...
    {
      longlong num;
      my_decimal2int(E_DEC_FATAL_ERROR, &val.x.decimal.value, unsigned_flag, &num);
      return num;
    }
    case DYN_COL_DATETIME:
    case DYN_COL_DATE:
...
double Item_dyncol_get::val_real()
{
  ...
  switch (val.type)
  {
    ...
    case DYN_COL_STRING:
    {
      ...
      char *end;
      double res =
          val.x.string.charset->strntod((char *)val.x.string.value.str, val.x.string.value.length, &end, &error);

      if (end != (char *)val.x.string.value.str + val.x.string.value.length || error)
      {
        push_warning_printf(
            thd, Sql_condition::WARN_LEVEL_WARN, ER_BAD_DATA, ER_THD(thd, ER_BAD_DATA),
            ErrConvString(val.x.string.value.str, val.x.string.value.length, val.x.string.charset).ptr(), "DOUBLE");
      }
      return res;
    }
    case DYN_COL_DECIMAL:
...
my_decimal *Item_dyncol_get::val_decimal(my_decimal *decimal_value)
{
  ...
  switch (val.type)
  {
    ...
    case DYN_COL_STRING:
    {
      const char *end;
      int rc;
      rc = str2my_decimal(0, val.x.string.value.str, val.x.string.value.length, val.x.string.charset, decimal_value,
                          &end);
      if (rc != E_DEC_OK || end != val.x.string.value.str + val.x.string.value.length)
      {
        push_warning_printf(
            thd, Sql_condition::WARN_LEVEL_WARN, ER_BAD_DATA, ER_THD(thd, ER_BAD_DATA),
            ErrConvString(val.x.string.value.str, val.x.string.value.length, val.x.string.charset).ptr(), "DECIMAL");
      }
      break;
    }
    case DYN_COL_DECIMAL:
  ...
  switch (val.type)
  {
    ...
    case DYN_COL_UINT:
      if (signed_value || val.x.ulong_value <= LONGLONG_MAX)
      {
        ...
        if (int_to_datetime_with_warn(thd, Longlong_hybrid(llval, !signed_value), ltime, fuzzydate, 0, 0 /* TODO */))
          goto null;
        return 0;
      }
      ...
      /* fall through */
    case DYN_COL_DOUBLE:
      if (double_to_datetime_with_warn(thd, val.x.double_value, ltime, fuzzydate, 0, 0 /* TODO */))
        goto null;
      return 0;
    case DYN_COL_DECIMAL:
      if (decimal_to_datetime_with_warn(thd, (my_decimal *)&val.x.decimal.value, ltime, fuzzydate, 0, 0 /* TODO */))
        goto null;
      return 0;
    case DYN_COL_STRING:
      if (str_to_datetime_with_warn(thd, &my_charset_numeric, val.x.string.value.str, val.x.string.value.length, ltime,
                                    fuzzydate))
        goto null;
      return 0;
    case DYN_COL_DATETIME:
...
Item_temptable_rowid::Item_temptable_rowid(TABLE *table_arg) : Item_str_func(table_arg->in_use), table(table_arg)
{
...
static size_t natsort_encode_length_max(size_t n) { return (n < 27) ? n / 9 + 1 : 26; }
...
static void natsort_encode_length(size_t n, String *out)
{
  ...
  size_t log10n = 0;
  for (size_t tmp = n / 10; tmp; tmp /= 10) log10n++;
...
*/
static NATSORT_ERR natsort_encode_numeric_string(const char *in, size_t n_digits, String *out)
{
  DBUG_ASSERT(in);
  DBUG_ASSERT(n_digits);

  if (out->length() + natsort_encode_length_max(n_digits - 1) + n_digits > out->alloced_length())
...
*/
static size_t natsort_max_key_size(size_t input_size) { return input_size + (input_size + 1) / 2; }
...
*/
static NATSORT_ERR to_natsort_key(const String *in, String *out, size_t max_key_size)
{
  size_t n_digits = 0;
  size_t n_lead_zeros = 0;
  size_t num_start;
  size_t reserve_length = std::min(natsort_max_key_size(in->length()) + MAX_BIGINT_WIDTH + 2, max_key_size);
  ...
  for (size_t pos = 0;; pos++)
  {
    ...
    if (!is_digit && (n_digits || n_lead_zeros))
    {
      ...
      NATSORT_ERR err = natsort_encode_numeric_string(in->ptr() + num_start, n_digits, out);
...
String *Item_func_natural_sort_key::val_str(String *out)
{
  ...
  */
  ...
  if (err != NATSORT_ERR::SUCCESS)
  {
    ...
    {
      push_warning_printf(current_thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                          ER(ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(), max_allowed_packet);
    }
    goto error_exit;
  }
...
bool Item_func_natural_sort_key::fix_length_and_dec(THD *thd)
{
  if (agg_arg_charsets_for_string_result(collation, args, 1))
    return true;
  DBUG_ASSERT(collation.collation != NULL);
  uint32 max_char_len = (uint32)natsort_max_key_size(args[0]->max_char_length());
  fix_char_length(max_char_len);

  set_maybe_null(args[0]->maybe_null() ||
                 max_char_len * collation.collation->mbmaxlen > current_thd->variables.max_allowed_packet);
  return false;
}
...
String *Item_func_wsrep_last_written_gtid::val_str_ascii(String *str)
{
  ...
  ssize_t gtid_len =
      my_snprintf((char *)gtid_str.ptr(), WSREP_MAX_WSREP_SERVER_GTID_STR_LEN + 1, "%u-%u-%llu",
                  wsrep_gtid_server.domain_id, wsrep_gtid_server.server_id, current_thd->wsrep_last_written_gtid_seqno);
  if (gtid_len < 0)
  {
    my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), func_name(), "wsrep_gtid_print failed");
    null_value = TRUE;
    return 0;
  }
...
String *Item_func_wsrep_last_seen_gtid::val_str_ascii(String *str)
{
  ...
  ssize_t gtid_len = my_snprintf((char *)gtid_str.ptr(), WSREP_MAX_WSREP_SERVER_GTID_STR_LEN + 1, "%u-%u-%llu",
                                 wsrep_gtid_server.domain_id, wsrep_gtid_server.server_id, wsrep_gtid_server.seqno());
  if (gtid_len < 0)
  {
    my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), func_name(), "wsrep_gtid_print failed");
    null_value = TRUE;
...
longlong Item_func_wsrep_sync_wait_upto::val_int()
{
  ...
  if (!(gtid_list = gtid_parse_string_to_list(gtid_str->ptr(), gtid_str->length(), &count)))
  {
  ...
  if (count == 1)
  {
    if (wsrep_check_gtid_seqno(gtid_list[0].domain_id, gtid_list[0].server_id, gtid_list[0].seq_no))
    {
