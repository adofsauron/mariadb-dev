...
Table_type dd_frm_type(THD *thd, char *path, LEX_CSTRING *engine_name, LEX_CSTRING *partition_engine_name,
    ...
    if (!engine_name->length && (n_length = uint4korr(frm_image + 55)))
    {
      ...
      if (next_chunk + 2 < buff_end)
      {
        ...
        if (len <= NAME_CHAR_LEN)
        {
          ...
          */
          strmake((char *)engine_name->str, (char *)next_chunk + 2, engine_name->length = len);
        }
      }
    }
...
bool dd_recreate_table(THD *thd, const char *db, const char *table_name)
{
  ...
  create_info.init();
  build_table_filename(path_buf, sizeof(path_buf) - 1, db, table_name, "", 0);
