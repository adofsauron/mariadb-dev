...
static bool admin_recreate_table(THD *thd, TABLE_LIST *table_list)
{
  ...
  */
  ...
  result_code = (thd->open_temporary_tables(table_list) || mysql_recreate_table(thd, table_list, false));
  reenable_binlog(thd);
  /*
...
static int send_check_errmsg(THD *thd, TABLE_LIST *table, const LEX_CSTRING *operator_name, const char *errmsg)
...
static int prepare_for_repair(THD *thd, TABLE_LIST *table_list, HA_CHECK_OPT *check_opt)
{
  ...
  const char **ext;
  MY_STAT stat_info;
  Open_table_context ot_ctx(thd, (MYSQL_OPEN_IGNORE_FLUSH | MYSQL_OPEN_HAS_MDL_LOCK | MYSQL_LOCK_IGNORE_TIMEOUT));
  ...
  if (!(table = table_list->table))
  {
    ...
    */
    ...
    if (lock_table_names(thd, table_list, table_list->next_global, thd->variables.lock_wait_timeout, 0))
      DBUG_RETURN(0);
    has_mdl_lock = TRUE;
    ...
    if (open_table_from_share(thd, share, &empty_clex_str, 0, 0, 0, &tmp_table, FALSE))
    {
  ...
  */
  ...
  {
    error = send_check_errmsg(thd, table_list, &msg_repair, "Cannot repair temporary table from .frm file");
    goto end;
  }
  ...
  */
  ...
  if (table->s->frm_version < FRM_VER_TRUE_VARCHAR && table->s->varchar_fields)
  {
  ...
  */
  ...
  my_snprintf(tmp, sizeof(tmp), "%s-%lx_%llx", from, current_pid, thd->thread_id);
  ...
  if (table_list->table)
  {
    ...
    */
    ...
    close_all_tables_for_name(thd, table_list->table->s, HA_EXTRA_NOT_USED, NULL);
    table_list->table = 0;
  }
  else
  {
  ...
  */
  ...
  {
    error = send_check_errmsg(thd, table_list, &msg_repair, "Failed renaming data file");
    goto end;
  }
  if (dd_recreate_table(thd, table_list->db.str, table_list->table_name.str))
    create_error = send_check_errmsg(thd, table_list, &msg_repair, "Failed generating table from .frm file");
  /*
  ...
  */
  ...
  {
    error = send_check_errmsg(thd, table_list, &msg_repair, "Failed restoring .MYD file");
    goto end;
  }
  if (create_error)
    goto end;
  ...
  else
  {
    ...
    */
    ...
    {
      error = send_check_errmsg(thd, table_list, &msg_repair, "Failed to open partially repaired table");
      goto end;
    }
  }
...
{
  return (sql_errno == ER_NO_SUCH_TABLE || sql_errno == ER_NO_SUCH_TABLE_IN_ENGINE || sql_errno == ER_FILE_NOT_FOUND ||
          sql_errno == ER_LOCK_WAIT_TIMEOUT || sql_errno == ER_LOCK_DEADLOCK || sql_errno == ER_CANT_LOCK_LOG_TABLE ||
...
static bool open_only_one_table(THD *thd, TABLE_LIST *table, bool repair_table_use_frm, bool is_view_operator_func)
{
  LEX *lex = thd->lex;
  ...
  */
  ...
  /*
    ...
    allowed.
  */
  if (lex->alter_info.partition_flags & ALTER_PARTITION_ADMIN || !is_view_operator_func)
  {
  ...
  else if ((lex->table_type != TABLE_TYPE_VIEW) && lex->sql_command == SQLCOM_REPAIR)
  {
    table->required_type = TABLE_TYPE_NORMAL;
  }

  if (lex->sql_command == SQLCOM_CHECK || lex->sql_command == SQLCOM_REPAIR || lex->sql_command == SQLCOM_ANALYZE ||
  ...
  if (!thd->locked_tables_mode && repair_table_use_frm)
  {
    ...
    */
    ...
    open_error = (thd->open_temporary_tables(table) || open_and_lock_tables(thd, table, TRUE, 0));
...
dbug_err:
  ...
  */
  if (save_next_global)
  {
    ...
    while (table_list_iterator->next_global) table_list_iterator = table_list_iterator->next_global;
...
static void send_read_only_warning(THD *thd, const LEX_CSTRING *msg_status, const LEX_CSTRING *table_name)
{
  Protocol *protocol = thd->protocol;
  char buf[MYSQL_ERRMSG_SIZE];
  size_t length;
  length = my_snprintf(buf, sizeof(buf), ER_THD(thd, ER_OPEN_AS_READONLY), table_name->str);
...
void fill_check_table_metadata_fields(THD *thd, List<Item> *fields)
{
  ...
  item = new (thd->mem_root) Item_empty_string(thd, "Msg_text", SQL_ADMIN_MSG_TEXT_SIZE);
  item->set_maybe_null();
  fields->push_back(item, thd->mem_root);
}
...
*/
static bool mysql_admin_table(THD *thd, TABLE_LIST *tables, HA_CHECK_OPT *check_opt, const LEX_CSTRING *operator_name,
                              thr_lock_type lock_type, bool org_open_for_modify, bool repair_table_use_frm,
                              uint extra_open_options, int (*prepare_func)(THD *, TABLE_LIST *, HA_CHECK_OPT *),
                              int (handler::*operator_func)(THD *, HA_CHECK_OPT *),
                              int(view_operator_func)(THD *, TABLE_LIST *, HA_CHECK_OPT *), bool is_cmd_replicated)
{
  TABLE_LIST *table;
  ...
  */
  ...
  for (table = tables; table; table = table->next_local)
  {
    ...
    table_name.length = strxmov(table_name_buff, db, ".", table->table_name.str, NullS) - table_name_buff;
    ...
    */
    table->mdl_request.set_type(lex->sql_command == SQLCOM_REPAIR ? MDL_SHARED_NO_READ_WRITE
                                : lock_type >= TL_FIRST_WRITE     ? MDL_SHARED_WRITE
                                                                  : MDL_SHARED_READ);
    ...
    while (1)
    {
      open_error = open_only_one_table(thd, table, repair_table_use_frm, (view_operator_func != NULL));
      thd->open_options &= ~extra_open_options;
      ...
      */
      ...
      table->table = NULL;
      thd->release_transactional_locks();
      MDL_REQUEST_INIT(&table->mdl_request, MDL_key::TABLE, table->db.str, table->table_name.str,
      ...
      */
      ...
      if (alter_info->partition_flags & ALTER_PARTITION_ADMIN)
      {
        ...
        if (set_part_state(alter_info, table->table->part_info, PART_ADMIN))
        {
          ...
          protocol->store(operator_name, system_charset_info);
          protocol->store(&error_clex_str, system_charset_info);
          length = my_snprintf(buff, sizeof(buff), ER_THD(thd, ER_PARTITION_DOES_NOT_EXIST));
    ...
    */
    if (!table->table)
    {
      ...
      if (thd->get_stmt_da()->is_warning_info_empty())
        push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_CHECK_NO_SUCH_TABLE, ER_THD(thd, ER_CHECK_NO_SUCH_TABLE));
      /* if it was a view will check md5 sum */
      if (table->view && view_check(thd, table, check_opt) == HA_ADMIN_WRONG_CHECKSUM)
        push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_VIEW_CHECKSUM, ER_THD(thd, ER_VIEW_CHECKSUM));
      if (thd->get_stmt_da()->is_error() && table_not_corrupt_error(thd->get_stmt_da()->sql_errno()))
        result_code = HA_ADMIN_FAILED;
      else
    ...
    if ((table->table->db_stat & HA_READ_ONLY) && open_for_modify && operator_func != &handler::ha_analyze)
    {
      /* purecov: begin inspected */
    ...
    */
    if (lock_type == TL_WRITE && table->mdl_request.type > MDL_SHARED_WRITE)
    {
      ...
      else
      {
        ...
        strmake(storage_engine_name, table->table->file->real_table_type(), sizeof(storage_engine_name) - 1);
        tabledef_version.str = tabledef_version_buff;
        if ((tabledef_version.length = table->table->s->tabledef_version.length))
          memcpy((char *)tabledef_version.str, table->table->s->tabledef_version.str, MY_UUID_SIZE);
    ...
    if (table->table->s->crashed && operator_func == &handler::ha_check)
    {
      ...
      protocol->store(STRING_WITH_LEN("Table is marked as crashed"), system_charset_info);
      if (protocol->write())
        goto err;
      /* purecov: end */
    }
    ...
    if (operator_func == &handler::ha_repair && !(check_opt->sql_flags & TT_USEFRM))
    {
      ...
      if (check_old_types == HA_ADMIN_NEEDS_ALTER || check_for_upgrade == HA_ADMIN_NEEDS_ALTER)
      {
    ...
    if (operator_func == &handler::ha_analyze)
    {
      ...
      if (lex->with_persistent_for_clause && tab->s->table_category != TABLE_CATEGORY_USER)
      {
      ...
      */
      collect_eis =
          (table->table->s->table_category == TABLE_CATEGORY_USER && !(lex->alter_info.flags & ALTER_PARTITION_ADMIN) &&
           (check_eits_collection_allowed(thd) || lex->with_persistent_for_clause));
    }
    ...
    if (compl_result_code == HA_ADMIN_OK && collect_eis)
    {
      ...
      */
      ...
      open_error = open_only_one_table(thd, table, repair_table_use_frm, FALSE);
      thd->open_options &= ~extra_open_options;
      ...
      if (unlikely(!open_error))
      {
        ...
        else
        {
          ...
          while ((column_name = it++))
          {
            if (tab->s->fieldnames.type_names == 0 ||
                (pos = find_type(&tab->s->fieldnames, column_name->str, column_name->length, 1)) <= 0)
            {
              compl_result_code = result_code = HA_ADMIN_INVALID;
              break;
            }
            ...
            if (type < MYSQL_TYPE_MEDIUM_BLOB || type > MYSQL_TYPE_BLOB)
              ...
              push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NO_EIS_FOR_FIELD,
                                  ER_THD(thd, ER_NO_EIS_FOR_FIELD), column_name->str);
          }
        ...
        else
        {
          ...
          while ((index_name = it++))
          {
            if (tab->s->keynames.type_names == 0 ||
                (pos = find_type(&tab->s->keynames, index_name->str, index_name->length, 1)) <= 0)
            {
              compl_result_code = result_code = HA_ADMIN_INVALID;
              break;
            }
        ...
        if (!(compl_result_code = alloc_statistics_for_table(thd, table->table)) &&
            !(compl_result_code = collect_statistics_for_table(thd, table->table)))
      ...
      else
      {
        ...
        protocol->store(operator_name, system_charset_info);
        protocol->store(&msg_status, system_charset_info);
        protocol->store(STRING_WITH_LEN("Engine-independent statistics collected"), system_charset_info);
        if (protocol->write())
          goto err;
      }
    }
  ...
  send_result:
    ...
    thd->clear_error();  // these errors shouldn't get client
    {
      ...
      while ((err = it++))
      {
        ...
        protocol->store(&table_name, system_charset_info);
        protocol->store(operator_name, system_charset_info);
        protocol->store(warning_level_names[err->get_level()].str, warning_level_names[err->get_level()].length,
                        system_charset_info);
  ...
  send_result_message:
    ...
    switch (result_code)
    {
      ...
      {
        char buf[MYSQL_ERRMSG_SIZE];
        size_t length = my_snprintf(buf, sizeof(buf), ER_THD(thd, ER_CHECK_NOT_IMPLEMENTED), operator_name->str);
      ...
      {
        char buf[MYSQL_ERRMSG_SIZE];
        size_t length = my_snprintf(buf, sizeof(buf), ER_THD(thd, ER_BAD_TABLE_ERROR), table_name.str);
        protocol->store(STRING_WITH_LEN("note"), system_charset_info);
        protocol->store(buf, length, system_charset_info);
      }
      break;
      ...
      case HA_ADMIN_REJECT:
        protocol->store(&msg_status, system_charset_info);
        protocol->store(STRING_WITH_LEN("Operation need committed state"), system_charset_info);
      ...
      case HA_ADMIN_TRY_ALTER:
      {
        ...
        if (alter_info->partition_flags & ALTER_PARTITION_ADMIN)
        {
          protocol->store(STRING_WITH_LEN("Table does not support optimize on partitions. All partitions "
                                          "will be rebuilt and analyzed."),
                          system_charset_info);
        }
        else
        {
          protocol->store(STRING_WITH_LEN("Table does not support optimize, doing recreate + analyze instead"),
                          system_charset_info);
        }
        if (protocol->write())
          goto err;
        ...
        if (!result_code)  // recreation went ok
        {
          ...
          if (!thd->open_temporary_tables(table) && (table->table = open_ltable(thd, table, lock_type, 0)))
          {
      ...
      {
        protocol->store(STRING_WITH_LEN("note"), system_charset_info);
        protocol->store(ER_THD(thd, ER_VIEW_CHECKSUM), strlen(ER_THD(thd, ER_VIEW_CHECKSUM)), system_charset_info);
        break;
      }
      ...
      case HA_ADMIN_NEEDS_ALTER:
      {
        ...
        const char *what_to_upgrade = table->view                                            ? "VIEW"
                                      : table->table->file->ha_table_flags() & HA_CAN_REPAIR ? "TABLE"
                                                                                             : 0;

        protocol->store(&error_clex_str, system_charset_info);
        if (what_to_upgrade)
          length = my_snprintf(buf, sizeof(buf), ER_THD(thd, ER_TABLE_NEEDS_UPGRADE), what_to_upgrade,
                               table->table_name.str);
        else
          length = my_snprintf(buf, sizeof(buf), ER_THD(thd, ER_TABLE_NEEDS_REBUILD), table->table_name.str);
        protocol->store(buf, length, system_charset_info);
        fatal_error = 1;
        break;
      }
      ...
      {
        char buf[MYSQL_ERRMSG_SIZE];
        size_t length = my_snprintf(buf, sizeof(buf), "Unknown - internal error %d during operation", result_code);
        protocol->store(&error_clex_str, system_charset_info);
        protocol->store(buf, length, system_charset_info);
        fatal_error = 1;
        break;
    ...
    */
    ...
    if (table->table && !table->view)
    {
      ...
      */
      const bool skip_flush = (operator_func == &handler::ha_analyze) &&
                              (table->table->file->ha_table_flags() & HA_ONLINE_ANALYZE) && !collect_eis;
      if (table->table->s->tmp_table)
      {
        /*
    ...
    if (storage_engine_name[0])
    {
      ...
      lex_string_set(&ddl_log.org_storage_engine_name, storage_engine_name);
  ...
  if (is_table_modified && is_cmd_replicated && (!opt_readonly || thd->slave_thread) && !thd->lex->no_write_to_binlog)
  {
...
bool mysql_assign_to_keycache(THD *thd, TABLE_LIST *tables, const LEX_CSTRING *key_cache_name)
{
  ...
  DBUG_RETURN(mysql_admin_table(thd, tables, &check_opt, &msg_assign_to_keycache, TL_READ_NO_INSERT, 0, 0, 0, 0,
...
bool mysql_preload_keys(THD *thd, TABLE_LIST *tables)
{
  ...
  */
  DBUG_RETURN(mysql_admin_table(thd, tables, 0, &msg_preload_keys, TL_READ_NO_INSERT, 0, 0, 0, 0,
...
bool Sql_cmd_analyze_table::execute(THD *thd)
{
  ...
  if (check_table_access(thd, SELECT_ACL | INSERT_ACL, first_table, FALSE, UINT_MAX, FALSE))
    goto error;
  if (thd->has_read_only_protection())
    goto error;

  WSREP_TO_ISOLATION_BEGIN_WRTCHK(NULL, NULL, first_table);
  res = mysql_admin_table(thd, first_table, &m_lex->check_opt, &msg_analyze, lock_type, 1, 0, 0, 0,
...
bool Sql_cmd_check_table::execute(THD *thd)
{
  ...
  res = mysql_admin_table(thd, first_table, &m_lex->check_opt, &msg_check, lock_type, 0, 0, HA_OPEN_FOR_REPAIR, 0,
...
bool Sql_cmd_optimize_table::execute(THD *thd)
{
  ...
  WSREP_TO_ISOLATION_BEGIN_WRTCHK(NULL, NULL, first_table);
  res = (specialflag & SPECIAL_NO_NEW_FUNC) ? mysql_recreate_table(thd, first_table, true)
                                            : mysql_admin_table(thd, first_table, &m_lex->check_opt, &msg_optimize,
                                                                TL_WRITE, 1, 0, 0, 0, &handler::ha_optimize, 0, true);
  m_lex->first_select_lex()->table_list.first = first_table;
  m_lex->query_tables = first_table;
...
bool Sql_cmd_repair_table::execute(THD *thd)
{
  ...
  if (check_table_access(thd, SELECT_ACL | INSERT_ACL, first_table, FALSE, UINT_MAX, FALSE))
    goto error; /* purecov: inspected */
  WSREP_TO_ISOLATION_BEGIN_WRTCHK(NULL, NULL, first_table);
  res = mysql_admin_table(thd, first_table, &m_lex->check_opt, &msg_repair, TL_WRITE, 1,
                          MY_TEST(m_lex->check_opt.sql_flags & TT_USEFRM), HA_OPEN_FOR_REPAIR, &prepare_for_repair,
