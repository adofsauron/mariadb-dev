...
class With_element_head : public Sql_alloc
{
  ...
  */
  ...
  With_element_head(LEX_CSTRING *name) : query_name(name)
  {
...
class With_element : public Sql_alloc
{
  ...
  */
  ...
  With_element(With_element_head *h, List<Lex_ident_sys> list, st_select_lex_unit *unit)
      : next(NULL),
        base_dep_map(0),
        derived_dep_map(0),
        sq_dep_map(0),
        work_dep_map(0),
        mutually_recursive(0),
        top_level_dep_map(0),
        sq_rec_ref(NULL),
        next_mutually_recursive(NULL),
        references(0),
        referenced(false),
        is_used_in_query(false),
        head(h),
        column_list(list),
        cycle_list(0),
        spec(unit),
        is_recursive(false),
        rec_outer_references(0),
        with_anchor(false),
        level(0),
        rec_result(NULL)
  {
  ...
  void set_tables_start_pos(TABLE_LIST **pos) { head->tables_pos.set_start_pos(pos); }
  void set_tables_end_pos(TABLE_LIST **pos) { head->tables_pos.set_end_pos(pos); }

  bool check_dependencies_in_spec();

  void check_dependencies_in_select(st_select_lex *sl, st_unit_ctxt_elem *ctxt, bool in_subq, table_map *dep_map);

  void check_dependencies_in_unit(st_select_lex_unit *unit, st_unit_ctxt_elem *ctxt, bool in_subq, table_map *dep_map);

  void check_dependencies_in_with_clause(With_clause *with_clause, st_unit_ctxt_elem *ctxt, bool in_subq,
                                         table_map *dep_map);

  void set_dependency_on(With_element *with_elem) { base_dep_map |= with_elem->get_elem_map(); }
  ...
  bool set_unparsed_spec(THD *thd, const char *spec_start, const char *spec_end, my_ptrdiff_t spec_offset);
  ...
  bool check_unrestricted_recursive(st_select_lex *sel, table_map &unrestricted, table_map &encountered);

  void print(THD *thd, String *str, enum_query_type query_type);

  With_clause *get_owner() { return owner; }

  bool contains_sq_with_recursive_reference() { return sq_dep_map & mutually_recursive; }

  bool no_rec_ref_on_top_level() { return !(top_level_dep_map & mutually_recursive); }

  table_map get_mutually_recursive() { return mutually_recursive; }

  With_element *get_next_mutually_recursive() { return next_mutually_recursive; }
  ...
  friend bool LEX::resolve_references_to_cte(TABLE_LIST *tables, TABLE_LIST **tables_last);
  friend bool LEX::resolve_references_to_cte_in_hanging_cte();
};
...
class With_clause : public Sql_alloc
{
 ...
 public:
  ...
  With_clause(bool recursive_fl, With_clause *emb_with_clause)
      : owner(NULL),
        embedding_with_clause(emb_with_clause),
        next_with_clause(NULL),
        dependencies_are_checked(false),
        unrestricted(0),
        with_prepared_anchor(0),
        cleaned(0),
        stabilized(0),
  ...
  void add_to_list(With_clause **ptr, With_clause **&last_next)
  {
    if (embedding_with_clause)
    {
      ...
      */
      while (*ptr != embedding_with_clause) ptr = &(*ptr)->next_with_clause;
      *ptr = this;
...
inline bool With_element::is_with_prepared_anchor() { return owner->with_prepared_anchor & get_elem_map(); }

inline void With_element::mark_as_with_prepared_anchor() { owner->with_prepared_anchor |= mutually_recursive; }

inline bool With_element::is_cleaned() { return owner->cleaned & get_elem_map(); }

inline void With_element::mark_as_cleaned() { owner->cleaned |= get_elem_map(); }

inline void With_element::reset_recursive_for_exec()
{
...
inline void With_element::cleanup_stabilized() { owner->stabilized &= ~mutually_recursive; }

inline void With_element::set_as_stabilized() { owner->stabilized |= get_elem_map(); }

inline bool With_element::is_stabilized() { return owner->stabilized & get_elem_map(); }

inline bool With_element::all_are_stabilized()
{
  return (owner->stabilized & mutually_recursive) == mutually_recursive;
}

inline void With_element::prepare_for_next_iteration()
{
...
inline void With_clause::attach_to(st_select_lex *select_lex)
{
...
inline void st_select_lex::set_with_clause(With_clause *with_clause)
{
