...
void sp_pcontext::init(uint var_offset, uint cursor_offset, int num_case_expressions)
{
...
sp_pcontext::sp_pcontext()
    : Sql_alloc(),
      ...
      m_conditions(PSI_INSTRUMENT_MEM),
      m_cursors(PSI_INSTRUMENT_MEM),
      m_handlers(PSI_INSTRUMENT_MEM),
      m_children(PSI_INSTRUMENT_MEM),
...
sp_pcontext::sp_pcontext(sp_pcontext *prev, sp_pcontext::enum_scope scope)
    : Sql_alloc(),
      ...
      m_scope(scope)
{
  init(prev->m_var_offset + prev->m_max_var_index, prev->current_cursor_count(), prev->get_num_case_exprs());
}

sp_pcontext::~sp_pcontext()
{
  for (size_t i = 0; i < m_children.elements(); ++i) delete m_children.at(i);
}
...
{
  return (lex_string_cmp(system_charset_info, &a->name, &b->name) == 0 && a->type == b->type);
}
...
sp_variable *sp_pcontext::find_variable(const LEX_CSTRING *name, bool current_scope_only) const
{
  ...
  while (i--)
  {
    ...
    if (system_charset_info->strnncoll(name->str, name->length, p->name.str, p->name.length) == 0)
    {
      return p;
    }
  }

  return (!current_scope_only && m_parent) ? m_parent->find_variable(name, false) : NULL;
}
...
{
  if (m_var_offset <= offset && m_vars.elements() && offset <= get_last_context_variable()->offset)
  {
...
{
  sp_variable *p = new (thd->mem_root) sp_variable(name, m_var_offset + m_max_var_index);
...
sp_label *sp_pcontext::push_label(THD *thd, const LEX_CSTRING *name, uint ip, sp_label::enum_type type,
                                  List<sp_label> *list)
{
  sp_label *label = new (thd->mem_root) sp_label(name, ip, type, this);
...
sp_label *sp_pcontext::find_goto_label(const LEX_CSTRING *name, bool recusive)
{
  ...
  */
  ...
  return m_parent && (m_scope == REGULAR_SCOPE) ? m_parent->find_goto_label(name) : NULL;
}
...
sp_label *sp_pcontext::find_label(const LEX_CSTRING *name)
{
  ...
  /*
    ...
    to labels from the parent context, as they are out of scope.
  */
  return (m_parent && (m_scope == REGULAR_SCOPE)) ? m_parent->find_label(name) : NULL;
}
...
sp_label *sp_pcontext::find_label_current_loop_start()
{
  ...
  return (m_parent && (m_scope == REGULAR_SCOPE)) ? m_parent->find_label_current_loop_start() : NULL;
}

bool sp_pcontext::add_condition(THD *thd, const LEX_CSTRING *name, sp_condition_value *value)
{
...
sp_condition_value *sp_pcontext::find_condition(const LEX_CSTRING *name, bool current_scope_only) const
{
  ...
  return (!current_scope_only && m_parent) ? m_parent->find_condition(name, false) : NULL;
}

sp_condition_value *sp_pcontext::find_declared_or_predefined_condition(THD *thd, const LEX_CSTRING *name) const
{
...
static sp_condition_value
    ...
    // Errors
    cond_invalid_cursor(ER_SP_CURSOR_NOT_OPEN, "24000"), cond_dup_val_on_index(ER_DUP_ENTRY, "23000"),
    cond_dup_val_on_index2(ER_DUP_ENTRY_WITH_KEY_NAME, "23000"), cond_too_many_rows(ER_TOO_MANY_ROWS, "42000");
...
sp_condition_value *sp_pcontext::find_predefined_condition(const LEX_CSTRING *name) const
{
...
bool sp_condition_value::matches(const Sql_condition_identity &value, const sp_condition_value *found_cv) const
{
  bool user_value_matched = !value.get_user_condition_value() || this == value.get_user_condition_value();

  switch (type)
  {
    case sp_condition_value::ERROR_CODE:
      return user_value_matched && value.get_sql_errno() == get_sql_errno() &&
             (!found_cv || found_cv->type > sp_condition_value::ERROR_CODE);

    case sp_condition_value::SQLSTATE:
      return user_value_matched && Sql_state::eq(&value) &&
             (!found_cv || found_cv->type > sp_condition_value::SQLSTATE);

    case sp_condition_value::WARNING:
      return user_value_matched &&
             (value.Sql_state::is_warning() || value.get_level() == Sql_condition::WARN_LEVEL_WARN) && !found_cv;

    case sp_condition_value::NOT_FOUND:
      return user_value_matched && value.Sql_state::is_not_found() && !found_cv;
    ...
    case sp_condition_value::EXCEPTION:
      ...
      */
      return (((current_thd->variables.sql_mode & MODE_ORACLE) ||
               (value.Sql_state::is_exception() && value.get_level() == Sql_condition::WARN_LEVEL_ERROR)) &&
              !found_cv);
  }
  return false;
}

sp_handler *sp_pcontext::find_handler(const Sql_condition_identity &value) const
{
...
bool sp_pcontext::add_cursor(const LEX_CSTRING *name, sp_pcontext *param_ctx, sp_lex_cursor *lex)
{
...
const sp_pcursor *sp_pcontext::find_cursor(const LEX_CSTRING *name, uint *poff, bool current_scope_only) const
{
  ...
  while (i--)
  {
    ...
    if (system_charset_info->strnncoll(name->str, name->length, n.str, n.length) == 0)
    {
  ...
  return (!current_scope_only && m_parent) ? m_parent->find_cursor(name, poff, false) : NULL;
}

void sp_pcontext::retrieve_field_definitions(List<Spvar_definition> *field_def_lst) const
{
  ...
  for (size_t i = 0; i < m_vars.elements(); ++i)
  {
    ...
    */
    for (; next_child < m_children.elements(); next_child++)
    {
      ...
      if (!child->context_var_count() || child->get_context_variable(0)->offset > var_def->offset)
        break;
      /*
...
{
  if (m_cursor_offset <= offset && offset < m_cursor_offset + m_cursors.elements())
  {
...
{
  if (param_count != (m_param_context ? m_param_context->context_var_count() : 0))
  {
...
const Spvar_definition *sp_variable::find_row_field(const LEX_CSTRING *var_name, const LEX_CSTRING *field_name,
                                                    uint *row_field_offset)
{
  if (!field_def.is_row())
  {
    my_printf_error(ER_UNKNOWN_ERROR, "'%s' is not a row variable", MYF(0), var_name->str);
    return NULL;
  }
