...
void engine_option_value::link(engine_option_value **start, engine_option_value **end)
{
  DBUG_ENTER("engine_option_value::link");
  DBUG_PRINT("enter",
             ("name: '%s' (%u)  value: '%s' (%u)", name.str, (uint)name.length, value.str, (uint)value.length));
  engine_option_value *opt;
  /* check duplicates to avoid writing them to frm*/
  for (opt = *start; opt && ((opt->parsed && !opt->value.str) ||
                             system_charset_info->strnncoll(name.str, name.length, opt->name.str, opt->name.length));
       opt = opt->next) /* no-op */
    ;
  if (opt)
  {
...
static bool report_wrong_value(THD *thd, const char *name, const char *val, bool suppress_warning)
{
  if (suppress_warning)
    return 0;

  if (!(thd->variables.sql_mode & MODE_IGNORE_BAD_TABLE_OPTIONS) && !thd->slave_thread)
  {
    my_error(ER_BAD_OPTION_VALUE, MYF(0), val, name);
    return 1;
  }

  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_BAD_OPTION_VALUE, ER_THD(thd, ER_BAD_OPTION_VALUE), val,
                      name);
  return 0;
}

static bool report_unknown_option(THD *thd, engine_option_value *val, bool suppress_warning)
{
  ...
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_UNKNOWN_OPTION, ER_THD(thd, ER_UNKNOWN_OPTION),
                      val->name.str);
  DBUG_RETURN(FALSE);
}
...
static bool set_one_value(ha_create_table_option *opt, THD *thd, const LEX_CSTRING *value, void *base,
                          bool suppress_warning, MEM_ROOT *root)
{
  DBUG_ENTER("set_one_value");
  DBUG_PRINT("enter", ("opt: %p type: %u name '%s' value: '%s'", opt, opt->type, opt->name,
  ...
  switch (opt->type)
  {
    ...
    case HA_OPTION_TYPE_ULL:
    {
      ...
      *val = getopt_ull_limit_value(*val, &optp, &unused);
      ...
      DBUG_RETURN(report_wrong_value(thd, opt->name, value->str, suppress_warning));
    }
    ...
    case HA_OPTION_TYPE_ENUM:
    {
      ...
      *val = (uint)opt->def_value;
      ...
      while (*start)
      {
        for (end = start; *end && *end != ','; end++) /* no-op */
          ;
        if (!system_charset_info->strnncoll(start, end - start, value->str, value->length))
        {
          *val = num;
          DBUG_RETURN(0);
        }
...
static const size_t ha_option_type_sizeof[] = {sizeof(ulonglong), sizeof(char *), sizeof(uint), sizeof(bool)};
...
bool parse_option_list(THD *thd, handlerton *hton, void *option_struct_arg, engine_option_value **option_list,
                       ha_create_table_option *rules, bool suppress_warning, MEM_ROOT *root)
{
  ...
  DBUG_PRINT("enter", ("struct: %p list: %p rules: %p suppress_warning: %u root: %p", *option_struct, *option_list,
                       rules, (uint)suppress_warning, root));

  if (rules)
  {
    for (opt = rules; opt->name; opt++)
      set_if_bigger(option_struct_size, opt->offset + ha_option_type_sizeof[opt->type]);
  ...
  for (opt = rules; rules && opt->name; opt++)
  {
    ...
    {
      last = val;
      if (system_charset_info->strnncoll(opt->name, opt->name_length, val->name.str, val->name.length))
        continue;
      ...
      if (set_one_value(opt, thd, &val->value, *option_struct, suppress_warning || val->parsed, root))
        DBUG_RETURN(TRUE);
      val->parsed = true;
      seen = true;
      break;
    ...
    if (!seen || (opt->var && !last->value.str))
    {
      ...
      */
      if (!suppress_warning && opt->var && (thd->lex->sql_command == SQLCOM_CREATE_TABLE || seen))
      {
        ...
        if (!sysvar->session_is_default(thd))
        {
          ...
          if ((str = sysvar->val_str(&sbuf, thd, OPT_SESSION, &null_clex_str)))
          {
            ...
            val = new (root) engine_option_value(name, default_val, opt->type != HA_OPTION_TYPE_ULL);
            if (!val)
              DBUG_RETURN(TRUE);
      ...
      set_one_value(opt, thd, &default_val, *option_struct, suppress_warning, root);
    }
  }
...
{
  return resolve_sysvars(hton, hton->table_options) || resolve_sysvars(hton, hton->field_options) ||
...
bool parse_engine_table_options(THD *thd, handlerton *ht, TABLE_SHARE *share)
{
  ...
  if (parse_option_list(thd, ht, &share->option_struct, &share->option_list, ht->table_options, TRUE, root))
    DBUG_RETURN(TRUE);

  for (Field **field = share->field; *field; field++)
  {
    if (parse_option_list(thd, ht, &(*field)->option_struct, &(*field)->option_list, ht->field_options, TRUE, root))
      DBUG_RETURN(TRUE);
  }

  for (uint index = 0; index < share->keys; index++)
  {
    if (parse_option_list(thd, ht, &share->key_info[index].option_struct, &share->key_info[index].option_list,
...
bool parse_engine_part_options(THD *thd, TABLE *table)
{
  ...
  {
    if (merge_engine_options(share->option_list, part_elem->option_list, &tmp_option_list, root))
      DBUG_RETURN(TRUE);

    if (!part_info->is_sub_partitioned())
    {
      ht = part_elem->engine_type;
      if (parse_option_list(thd, ht, &part_elem->option_struct, &tmp_option_list, ht->table_options, TRUE, root))
        DBUG_RETURN(TRUE);
    }
    else
    {
      ...
      while (partition_element *sub_part_elem = sub_it++)
      {
        ...
        if (parse_option_list(thd, ht, &sub_part_elem->option_struct, &tmp_option_list, ht->table_options, TRUE, root))
          DBUG_RETURN(TRUE);
      }
    }
  }
...
bool engine_options_differ(void *old_struct, void *new_struct, ha_create_table_option *rules)
{
  ha_create_table_option *opt;
  for (opt = rules; rules && opt->name; opt++)
  {
...
static uint option_list_frm_length(engine_option_value *opt)
{
  ...
  for (; opt; opt = opt->next) res += opt->frm_length();
...
uint engine_table_options_frm_length(engine_option_value *table_option_list, List<Create_field> &create_fields,
                                     uint keys, KEY *key_info)
{
  ...
  while ((field = it++)) res += option_list_frm_length(field->option_list);

  for (index = 0; index < keys; index++, key_info++) res += option_list_frm_length(key_info->option_list);
...
{
  for (; opt; opt = opt->next) buff = opt->frm_image(buff);
...
uchar *engine_table_options_frm_image(uchar *buff, engine_option_value *table_option_list,
                                      List<Create_field> &create_fields, uint keys, KEY *key_info)
{
  ...
  while ((field = it++)) buff = option_list_frm_image(buff, field->option_list);

  while (key_info < key_info_end) buff = option_list_frm_image(buff, (key_info++)->option_list);
...
/**
  ...
  @returns pointer to byte after last recorded in the buffer
*/
uchar *engine_option_value::frm_read(const uchar *buff, const uchar *buff_end, engine_option_value **start,
...
#define need_buff(N)          \
  ...
  engine_option_value *ptr = new (root) engine_option_value(name, value, len & FRM_QUOTED_VALUE);
  if (!ptr)
    return NULL;
  ptr->link(start, end);
...
bool engine_table_options_frm_read(const uchar *buff, size_t length, TABLE_SHARE *share)
{
  ...
  {
    if (!(buff = engine_option_value::frm_read(buff, buff_end, &share->option_list, &end, root)))
      DBUG_RETURN(TRUE);
  }
  buff++;
  ...
  for (count = 0; count < share->fields; count++)
  {
    while (buff < buff_end && *buff)
    {
      if (!(buff = engine_option_value::frm_read(buff, buff_end, &share->field[count]->option_list, &end, root)))
        DBUG_RETURN(TRUE);
    }
    buff++;
  }
  ...
  for (count = 0; count < share->keys; count++)
  {
    while (buff < buff_end && *buff)
    {
      if (!(buff = engine_option_value::frm_read(buff, buff_end, &share->key_info[count].option_list, &end, root)))
        DBUG_RETURN(TRUE);
    }
    buff++;
  }
  ...
  if (buff < buff_end)
    sql_print_warning(
        "Table '%s' was created in a later MariaDB version - "
...
/**
  ...
  @retval FALSE OK
*/
bool merge_engine_options(engine_option_value *source, engine_option_value *changes, engine_option_value **out,
                          MEM_ROOT *root)
{
...
bool is_engine_option_known(engine_option_value *opt, ha_create_table_option *rules)
{
  if (!rules)
    return false;
  ...
  {
    if (!system_charset_info->strnncoll(rules->name, rules->name_length, opt->name.str, opt->name.length))
      return true;
  }
  return false;
}
