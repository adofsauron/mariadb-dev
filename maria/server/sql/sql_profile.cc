...
PROF_MEASUREMENT::PROF_MEASUREMENT(QUERY_PROFILE *profile_arg, const char *status_arg) : profile(profile_arg)
{
  collect();
...
void PROF_MEASUREMENT::set_label(const char *status_arg, const char *function_arg, const char *file_arg,
                                 unsigned int line_arg)
{
  ...
  */
  ...
  allocated_status_memory = (char *)my_malloc(key_memory_PROFILE, sizes[0] + sizes[1] + sizes[2], MYF(0));
...
void QUERY_PROFILE::new_status(const char *status_arg, const char *function_arg, const char *file_arg,
                               unsigned int line_arg)
{
...
void PROFILING::finish_current_query_impl()
{
  ...
  if (enabled && /* ON at end? */
      (current->query_source != NULL) && (!current->entries.is_empty()))
  {
...
bool PROFILING::show_profiles()
{
  ...
  field_list.push_back(new (mem_root) Item_return_int(thd, "Query_ID", 10, MYSQL_TYPE_LONG), mem_root);
  field_list.push_back(new (mem_root) Item_return_int(thd, "Duration", TIME_FLOAT_DIGITS - 1, MYSQL_TYPE_DOUBLE),
                       mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Query", 40), mem_root);
  ...
  for (iterator = history.new_iterator(), idx = 1; iterator != NULL; iterator = history.iterator_next(iterator), idx++)
  {
    ...
    protocol->store_double(query_time_usecs / (1000.0 * 1000), (uint32)TIME_FLOAT_DIGITS - 1);
       ...
       history_iterator = history.iterator_next(history_iterator))
  {
    ...
    */
    ...
    for (entry_iterator = query->entries.new_iterator(); entry_iterator != NULL;
         entry_iterator = query->entries.iterator_next(entry_iterator), previous = entry, row_number++)
    {
      ...
      */
      ...
      /*
        ...
        previous status.
      */
      table->field[2]->store(previous->status, strlen(previous->status), system_charset_info);

      my_decimal duration_decimal;
      double2my_decimal(E_DEC_FATAL_ERROR, (entry->time_usecs - previous->time_usecs) / (1000.0 * 1000),
                        &duration_decimal);
      ...
      double2my_decimal(E_DEC_FATAL_ERROR,
                        RUSAGE_DIFF_USEC(entry->rusage.ru_utime, previous->rusage.ru_utime) / (1000.0 * 1000),
                        &cpu_utime_decimal);

      double2my_decimal(E_DEC_FATAL_ERROR,
                        RUSAGE_DIFF_USEC(entry->rusage.ru_stime, previous->rusage.ru_stime) / (1000.0 * 1000),
                        &cpu_stime_decimal);
      ...
      double2my_decimal(E_DEC_FATAL_ERROR, GetTimeDiffInSeconds(&entry->ftUser, &previous->ftUser), &cpu_utime_decimal);
      double2my_decimal(E_DEC_FATAL_ERROR, GetTimeDiffInSeconds(&entry->ftKernel, &previous->ftKernel),
                        &cpu_stime_decimal);
...
#ifdef HAVE_GETRUSAGE
      table->field[6]->store((uint32)(entry->rusage.ru_nvcsw - previous->rusage.ru_nvcsw));
      table->field[6]->set_notnull();
      table->field[7]->store((uint32)(entry->rusage.ru_nivcsw - previous->rusage.ru_nivcsw));
...
#ifdef HAVE_GETRUSAGE
      table->field[8]->store((uint32)(entry->rusage.ru_inblock - previous->rusage.ru_inblock));
      table->field[8]->set_notnull();
      table->field[9]->store((uint32)(entry->rusage.ru_oublock - previous->rusage.ru_oublock));
      table->field[9]->set_notnull();
#elif defined(_WIN32)
      ULONGLONG reads_delta = entry->io_count.ReadOperationCount - previous->io_count.ReadOperationCount;
      ULONGLONG writes_delta = entry->io_count.WriteOperationCount - previous->io_count.WriteOperationCount;
...
#ifdef HAVE_GETRUSAGE
      table->field[10]->store((uint32)(entry->rusage.ru_msgsnd - previous->rusage.ru_msgsnd), true);
      table->field[10]->set_notnull();
      table->field[11]->store((uint32)(entry->rusage.ru_msgrcv - previous->rusage.ru_msgrcv), true);
...
#ifdef HAVE_GETRUSAGE
      table->field[12]->store((uint32)(entry->rusage.ru_majflt - previous->rusage.ru_majflt), true);
      table->field[12]->set_notnull();
      table->field[13]->store((uint32)(entry->rusage.ru_minflt - previous->rusage.ru_minflt), true);
      ...
      */
      table->field[12]->store((uint32)(entry->mem_count.PageFaultCount - previous->mem_count.PageFaultCount), true);
      table->field[12]->set_notnull();
#else
...
#ifdef HAVE_GETRUSAGE
      table->field[14]->store((uint32)(entry->rusage.ru_nswap - previous->rusage.ru_nswap), true);
      ...
      {
        table->field[15]->store(previous->function, strlen(previous->function), system_charset_info);
...
void PROFILING::reset() { enabled = (thd->variables.option_bits & OPTION_PROFILING) != 0; }
