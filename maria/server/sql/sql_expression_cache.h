...
{
 public:
  enum expr_cache_state
  {
    UNINITED,
    STOPPED,
    OK
  };
  Expression_cache_tracker(Expression_cache *c) : cache(c), hit(0), miss(0), state(UNINITED) {}
...
class Expression_cache_tmptable : public Expression_cache
{
 public:
  ...
  virtual void update_tracker()
  {
    if (tracker)
    {
      tracker->set(hit, miss,
                   (inited ? (cache_table ? Expression_cache_tracker::OK : Expression_cache_tracker::STOPPED)
