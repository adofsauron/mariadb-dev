...
static uchar *get_sys_var_length(const sys_var *var, size_t *length, my_bool first)
{
...
int sys_var_init()
{
  ...
  if (my_hash_init(PSI_INSTRUMENT_ME, &system_variable_hash, system_charset_info, 700, 0, 0,
                   (my_hash_get_key)get_sys_var_length, 0, HASH_UNIQUE))
    goto error;
...
uint sys_var_elements() { return system_variable_hash.records; }
...
*/
sys_var::sys_var(sys_var_chain *chain, const char *name_arg, const char *comment, int flags_arg, ptrdiff_t off,
                 int getopt_id, enum get_opt_arg_type getopt_arg_type, SHOW_TYPE show_val_type_arg, longlong def_val,
                 PolyLock *lock, enum binlog_status_enum binlog_status_arg, on_check_function on_check_func,
                 on_update_function on_update_func, const char *substitute)
    : next(0),
      binlog_status(binlog_status_arg),
      value_origin(COMPILE_TIME),
      flags(flags_arg),
...
const uchar *sys_var::session_value_ptr(THD *thd, const LEX_CSTRING *base) const { return session_var_ptr(thd); }

const uchar *sys_var::global_value_ptr(THD *thd, const LEX_CSTRING *base) const { return global_var_ptr(); }

bool sys_var::check(THD *thd, set_var *var)
{
  if (unlikely((var->value && do_check(thd, var)) || (on_check && on_check(this, thd, var))))
  {
...
const uchar *sys_var::value_ptr(THD *thd, enum_var_type type, const LEX_CSTRING *base) const
{
  DBUG_ASSERT(base);
  if (type == OPT_GLOBAL || scope() == GLOBAL)
  {
...
#define case_for_integers(CMD)  \
  ...
  case SHOW_SLONG:              \
    do_num_val(long, CMD);      \
  case SHOW_SLONGLONG:          \
    do_num_val(longlong, CMD);  \
  case SHOW_UINT:               \
    do_num_val(uint, CMD);      \
  case SHOW_ULONG:              \
    do_num_val(ulong, CMD);     \
  case SHOW_ULONGLONG:          \
    do_num_val(ulonglong, CMD); \
  case SHOW_HA_ROWS:            \
    do_num_val(ha_rows, CMD);

#define case_for_double(CMD) \
  case SHOW_DOUBLE:          \
    do_num_val(double, CMD)
...
longlong sys_var::val_int(bool *is_null, THD *thd, enum_var_type type, const LEX_CSTRING *base)
{
  LEX_CSTRING sval;
  ...
  *is_null = false;
  ...
  longlong ret = 0;
  if (!(*is_null = !sval.str))
    ret = longlong_from_string_with_check(charset(thd), sval.str, sval.str + sval.length);
  return ret;
}

String *sys_var::val_str_nolock(String *str, THD *thd, const uchar *value)
{
  static LEX_CSTRING bools[] = {{STRING_WITH_LEN("OFF")}, {STRING_WITH_LEN("ON")}};
...
String *sys_var::val_str(String *str, THD *thd, enum_var_type type, const LEX_CSTRING *base)
{
  AutoWLock lock(&PLock_global_system_variables);
  const uchar *value = value_ptr(thd, type, base);
...
double sys_var::val_real(bool *is_null, THD *thd, enum_var_type type, const LEX_CSTRING *base)
{
  LEX_CSTRING sval;
  AutoWLock lock(&PLock_global_system_variables);
  const uchar *value = value_ptr(thd, type, base);
  *is_null = false;
  ...
  double ret = 0;
  if (!(*is_null = !sval.str))
    ret = double_from_string_with_check(charset(thd), sval.str, sval.str + sval.length);
  return ret;
}
...
void sys_var::do_deprecated_warning(THD *thd)
{
  if (deprecation_substitute != NULL)
  {
    ...
    */
    uint errmsg =
        deprecation_substitute[0] == '\0' ? ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT : ER_WARN_DEPRECATED_SYNTAX;
    if (thd)
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_DEPRECATED_SYNTAX, ER_THD(thd, errmsg), buf1,
...
static int show_cmp(SHOW_VAR *a, SHOW_VAR *b) { return strcmp(a->name, b->name); }
...
SHOW_VAR *enumerate_sys_vars(THD *thd, bool sorted, enum enum_var_type scope)
{
  ...
  if (result)
  {
    ...
    /* sort into order */
    if (sorted)
      my_qsort(result, show - result, sizeof(SHOW_VAR), (qsort_cmp)show_cmp);
...
int set_var::update(THD *thd) { return value ? var->update(thd, this) : var->set_default(thd, this); }

set_var::set_var(THD *thd, enum_var_type type_arg, sys_var *var_arg, const LEX_CSTRING *base_name_arg, Item *value_arg)
    : var(var_arg), type(type_arg), base(*base_name_arg)
{
  ...
  */
  if (value_arg && value_arg->type() == Item::FIELD_ITEM)
  {
    ...
    // names are utf8
    if (!(value = new (thd->mem_root) Item_string_sys(thd, item->field_name.str, (uint)item->field_name.length)))
...
int set_var_user::check(THD *thd)
{
  ...
  */
  return (user_var_item->fix_fields(thd, (Item **)0) || user_var_item->check(0)) ? -1 : 0;
}
...
int set_var_default_role::check(THD *thd)
{
  ...
  return acl_check_set_default_role(thd, real_user->host.str, real_user->user.str, real_role);
#else
  return 0;
#endif
}
...
int set_var_default_role::update(THD *thd)
{
  ...
  Reprepare_observer *save_reprepare_observer = thd->m_reprepare_observer;
  thd->m_reprepare_observer = 0;
  int res = acl_set_default_role(thd, real_user->host.str, real_user->user.str, real_role);
  thd->m_reprepare_observer = save_reprepare_observer;
  return res;
#else
  return 0;
...
int set_var_collation_client::check(THD *thd)
{
  ...
  {
    my_error(ER_WRONG_VALUE_FOR_VAR, MYF(0), "character_set_client", character_set_client->cs_name.str);
    return 1;
  }
  return 0;
}
...
{
  thd->update_charset(character_set_client, collation_connection, character_set_results);

  /* Mark client collation variables as changed */
  thd->session_tracker.sysvars.mark_as_changed(thd, Sys_character_set_client_ptr);
  thd->session_tracker.sysvars.mark_as_changed(thd, Sys_character_set_results_ptr);
  thd->session_tracker.sysvars.mark_as_changed(thd, Sys_character_set_connection_ptr);
  thd->session_tracker.state_change.mark_as_changed(thd);
...
*****************************************************************************/
static void store_value_ptr(Field *field, sys_var *var, String *str, const uchar *value_ptr)
{
  field->set_notnull();
...
static void store_var(Field *field, sys_var *var, enum_var_type scope, String *str)
{
  if (var->check_type(scope))
    return;

  store_value_ptr(field, var, str, var->value_ptr(field->table->in_use, scope, &null_clex_str));
}
...
int fill_sysvars(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (uint i = 0; i < system_variable_hash.records; i++)
  {
    ...
    if ((wild && wild_case_compare(system_charset_info, name_buffer, wild)) || (cond && !cond->val_int()))
      continue;
    ...
    // GLOBAL_VALUE_ORIGIN
    static const LEX_CSTRING origins[] = {{STRING_WITH_LEN("CONFIG")},       {STRING_WITH_LEN("COMMAND-LINE")},
                                          {STRING_WITH_LEN("AUTO")},         {STRING_WITH_LEN("SQL")},
                                          {STRING_WITH_LEN("COMPILE-TIME")}, {STRING_WITH_LEN("ENVIRONMENT")}};
    const LEX_CSTRING *origin = origins + var->value_origin;
    fields[3]->store(origin->str, origin->length, scs);

    // DEFAULT_VALUE
    const uchar *def = var->is_readonly() && var->option.id < 0 ? 0 : var->default_value_ptr(thd);
    if (def)
    ...
    static const LEX_CSTRING scopes[] = {
        {STRING_WITH_LEN("GLOBAL")}, {STRING_WITH_LEN("SESSION")}, {STRING_WITH_LEN("SESSION ONLY")}};
...
#define LONG_TYPE "BIGINT"
    ...
    // VARIABLE_COMMENT
    fields[7]->store(var->option.comment, strlen(var->option.comment), scs);
    ...
    // READ_ONLY
    static const LEX_CSTRING yesno[] = {{STRING_WITH_LEN("NO")}, {STRING_WITH_LEN("YES")}};
    ...
    if (var->value_origin == sys_var::CONFIG && has_file_acl)
    {
      ...
      fields[14]->store(var->origin_filename, strlen(var->origin_filename), files_charset_info);
    }
...
void set_sys_var_value_origin(void *ptr, enum sys_var::where here, const char *filename)
{
...
*/
static bool engine_list_next_item(const char **pos, const char *end_pos, const char **item_start, const char **item_end)
{
  if (*pos >= end_pos)
    return false;
  *item_start = *pos;
...
static bool resolve_engine_list_item(THD *thd, plugin_ref *list, uint32 *idx, const char *pos, const char *pos_end,
                                ...
                                bool temp_copy)
{
  ...
  for (;;)
  {
    ...
    if (idx >= count)
      break;
    if (resolve_engine_list_item(thd, res, &idx, item_start, item_end, error_on_unknown_engine, temp_copy))
      goto err;
  }
...
void free_engine_list(plugin_ref *list)
{
  ...
  if (!list)
    return;
  for (p = list; *p; ++p) plugin_unlock(NULL, *p);
  my_free(list);
}
...
plugin_ref *copy_engine_list(plugin_ref *list)
{
  ...
  for (i = 0; i < count; ++i) p[i] = my_plugin_lock(NULL, list[i]);
  p[i] = NULL;
  return p;
}
...
plugin_ref *temp_copy_engine_list(THD *thd, plugin_ref *list)
{
  ...
  for (i = 0; i < count; ++i) p[i] = my_plugin_lock(thd, list[i]);
  p[i] = NULL;
  return p;
}
...
char *pretty_print_engine_list(THD *thd, plugin_ref *list)
{
  ...
  size = 0;
  for (p = list; *p; ++p) size += plugin_name(*p)->length + 1;
