...
void Field::do_field_eq(Copy_field *copy) { memcpy(copy->to_ptr, copy->from_ptr, copy->from_length); }
...
static void do_copy_nullable_row_to_notnull(Copy_field *copy)
{
  if (*copy->null_row || (copy->from_null_ptr && (*copy->from_null_ptr & copy->from_bit)))
  {
    copy->to_field->set_warning(Sql_condition::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, 1);
    copy->to_field->reset();
  }
  else
  {
...
static void do_field_varbinary_pre50(Copy_field *copy)
{
  ...
  size_t length = my_lengthsp_8bit(&my_charset_bin, copy->tmp.ptr(), copy->from_field->field_length);

  copy->to_field->store(copy->tmp.ptr(), length, copy->tmp.charset());
}

void Field::do_field_int(Copy_field *copy)
{
  longlong value = copy->from_field->val_int();
  copy->to_field->store(value, MY_TEST(copy->from_field->flags & UNSIGNED_FLAG));
}
...
void Field::do_field_date(Copy_field *copy) { return do_field_temporal(copy, Date::Options(TIME_CONV_NONE)); }

void Field_time::do_field_time(Copy_field *copy) { return do_field_temporal(copy, Time::Options(current_thd)); }
...
static void do_cut_string(Copy_field *copy)
{
  ...
  if (cs->scan((char *)copy->from_ptr + copy->to_length, (char *)copy->from_ptr + copy->from_length, MY_SEQ_SPACES) <
...
{  // Shorter string field
  ...
  Well_formed_prefix prefix(cs, (char *)copy->from_ptr, (char *)from_end, copy->to_length / cs->mbmaxlen);
  ...
  if (unlikely(prefix.well_formed_error_pos() || cs->scan((char *)copy->from_ptr + copy_length, (char *)from_end,
  ...
  if (copy_length < copy->to_length)
    cs->fill((char *)copy->to_ptr + copy_length, copy->to_length - copy_length, ' ');
}
...
static void do_expand_binary(Copy_field *copy)
{
  CHARSET_INFO *cs = copy->from_field->charset();
  memcpy(copy->to_ptr, copy->from_ptr, copy->from_length);
  cs->fill((char *)copy->to_ptr + copy->from_length, copy->to_length - copy->from_length, '\0');
}
...
static void do_expand_string(Copy_field *copy)
{
  CHARSET_INFO *cs = copy->from_field->charset();
  memcpy(copy->to_ptr, copy->from_ptr, copy->from_length);
  cs->fill((char *)copy->to_ptr + copy->from_length, copy->to_length - copy->from_length, ' ');
}
...
static void do_varstring1(Copy_field *copy)
{
  ...
  if (length > copy->to_length - 1)
  {
    ...
    if (copy->from_field->table->in_use->count_cuted_fields > CHECK_FIELD_EXPRESSION && copy->to_field)
      copy->to_field->set_warning(Sql_condition::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, 1);
  }
...
Field::Copy_func *Field_temporal::get_copy_func(const Field *from) const
{
  ...
  if (from->type() == MYSQL_TYPE_BIT)
    return do_field_int;
  if (!eq_def(from) || (table->in_use->variables.sql_mode & (MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE)))
...
Field::Copy_func *Field_varstring::get_copy_func(const Field *from) const
{
  ...
  /*
    ...
    repairs data.
  */
  if (from->type() == MYSQL_TYPE_VAR_STRING && !from->has_charset() && !Field_varstring::has_charset())
    return do_field_varbinary_pre50;
  if (Field_varstring::real_type() != from->real_type() || Field_varstring::charset() != from->charset() ||
      length_bytes != ((const Field_varstring *)from)->length_bytes ||
      !compression_method() != !from->compression_method())
    return do_field_string;
  return length_bytes == 1 ? (from->charset()->mbmaxlen == 1 ? do_varstring1 : do_varstring1_mb)
...
Field::Copy_func *Field_string::get_copy_func(const Field *from) const
{
  if (from->type() == MYSQL_TYPE_BIT)
    return do_field_int;
  if (Field_string::type_handler() != from->type_handler() || Field_string::charset() != from->charset())
    return do_field_string;
  if (Field_string::pack_length() < from->pack_length())
    return (Field_string::charset()->mbmaxlen == 1 ? do_cut_string : do_cut_string_complex);
  if (Field_string::pack_length() > from->pack_length())
    return Field_string::charset() == &my_charset_bin ? do_expand_binary : do_expand_string;
  return get_identical_copy_func();
}
...
{
  return real_type() == from->real_type() && decimals() == from->decimals() && !sql_mode_for_dates(table->in_use);
}
...
*/
static int field_conv_incompatible(Field *to, Field *from) { return to->store_field(from); }
...
{
  return to->memcpy_field_possible(from) ? field_conv_memcpy(to, from) : field_conv_incompatible(to, from);
}

fast_field_copier Field::get_fast_field_copier(const Field *from)
{
  DBUG_ENTER("Field::get_fast_field_copier");
  DBUG_RETURN(memcpy_field_possible(from) ? &field_conv_memcpy : &field_conv_incompatible);
}
