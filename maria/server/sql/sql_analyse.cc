...
static bool prepare_param(THD *thd, Item **item, const char *proc_name, uint pos)
{
  ...
  if ((*item)->type_handler()->result_type() != INT_RESULT || !(*item)->basic_const_item() || (*item)->val_real() < 0)
  {
...
Procedure *proc_analyse_init(THD *thd, ORDER *param, select_result *result, List<Item> &field_list)
{
  ...
  if (!(pc->f_info = (field_info **)thd->alloc(sizeof(field_info *) * field_list.elements)))
    goto err;
    ...
    while ((item = it++))
    {
      ...
      switch (item->result_type())
      {
        case INT_RESULT:
          ...
          if (item->type() == Item::FIELD_ITEM && ((Item_field *)item)->field->type() == MYSQL_TYPE_LONGLONG &&
...
bool get_ev_num_info(EV_NUM_INFO *ev_info, NUM_INFO *info, const char *num)
{
  if (info->negative)
  {
    ...
    ev_info->llval = -(longlong)MY_MAX((ulonglong)-ev_info->llval, info->ullval);
...
void field_real::get_opt_type(String *answer, ha_rows total_rows __attribute__((unused)))
{
  ...
  {
    int len = (int)max_length - ((item->decimals >= FLOATING_POINT_DECIMALS) ? 0 : (item->decimals + 1));
...
void field_decimal::get_opt_type(String *answer, ha_rows total_rows __attribute__((unused)))
{
  ...
  length =
      snprintf(buff, sizeof(buff), "DECIMAL(%d, %d)", (int)(max_length - (item->decimals ? 1 : 0)), item->decimals);
  if (is_unsigned)
...
String *field_decimal::avg(String *s, ha_rows rows)
{
  ...
  avg_val.round_to(&rounded_avg, MY_MIN(sum[cur_sum].frac + prec_increment, DECIMAL_MAX_SCALE), HALF_UP);
...
String *field_decimal::std(String *s, ha_rows rows)
{
  ...
  s->set_real(((double)std_sqr <= 0.0 ? 0.0 : sqrt(std_sqr)), MY_MIN(item->decimals + prec_increment, NOT_FIXED_DEC),
              my_thd_charset);
...
int collect_decimal(uchar *element, element_count count, TREE_INFO *info)
{
...
bool analyse::change_columns(THD *thd, List<Item> &field_list)
{
  ...
  func_items[9] = new (mem_root) Item_proc_string(thd, "Optimal_fieldtype", MY_MAX(64, output_str_length));

  for (uint i = 0; i < array_elements(func_items); i++) field_list.push_back(func_items[i], thd->mem_root);
