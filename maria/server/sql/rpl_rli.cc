...
const char *const Relay_log_info::state_delaying_string =
...
Relay_log_info::Relay_log_info(bool is_slave_recovery, const char *thread_name)
    : Slave_reporting_capability(thread_name),
      ...
      mi(0),
      inuse_relaylog_list(0),
      last_inuse_relaylog(0),
      ...
      m_flags(0)
{
  ...
  relay_log.set_psi_keys(key_RELAYLOG_LOCK_index, key_RELAYLOG_COND_relay_log_updated,
                         key_RELAYLOG_COND_bin_log_updated, key_file_relaylog, key_file_relaylog_cache,
                         key_file_relaylog_index, key_file_relaylog_index_cache, key_RELAYLOG_COND_queue_busy,
  ...
  mysql_mutex_init(key_relay_log_info_data_lock, &data_lock, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_relay_log_info_log_space_lock, &log_space_lock, MY_MUTEX_INIT_FAST);
...
int Relay_log_info::init(const char *info_fname)
{
  ...
  if (fn_format(pattern, PREFIX_SQL_LOAD, pattern, "", MY_SAFE_PATH | MY_RETURN_REAL_PATH) == NullS)
  {
    mysql_mutex_unlock(&data_lock);
    sql_print_error("Unable to use slave's temporary directory %s", slave_load_tmpdir);
    DBUG_RETURN(1);
  }
  ...
  */

  {
    /* Reports an error and returns, if the --relay-log's path
       is a directory.*/
    if (opt_relay_logname && opt_relay_logname[strlen(opt_relay_logname) - 1] == FN_LIBCHAR)
    {
      mysql_mutex_unlock(&data_lock);
      sql_print_error(
          "Path '%s' is a directory name, please specify \
a file name for --relay-log option",
          opt_relay_logname);
      DBUG_RETURN(1);
    }
    ...
    /* Reports an error and returns, if the --relay-log-index's path
       is a directory.*/
    if (opt_relaylog_index_name && opt_relaylog_index_name[strlen(opt_relaylog_index_name) - 1] == FN_LIBCHAR)
    {
      mysql_mutex_unlock(&data_lock);
      sql_print_error(
          "Path '%s' is a directory name, please specify \
a file name for --relay-log-index option",
    ...
    ln = relay_log.generate_name(opt_relay_logname, "-relay-bin", 1, buf);
    /* We send the warning only at startup, not after every RESET SLAVE */
    if (!opt_relay_logname && !opt_relaylog_index_name && !name_warning_sent && !opt_bootstrap)
    {
      /*
    ...
    create_logfile_name_with_suffix(buf_relay_logname, sizeof(buf_relay_logname), ln, 1, &mi->cmp_connection_name);
    ln = buf_relay_logname;

    if (opt_relaylog_index_name)
    {
      buf_relaylog_index_name = buf_relaylog_index_name_buff;
      create_logfile_name_with_suffix(buf_relaylog_index_name_buff, sizeof(buf_relaylog_index_name_buff),
                                      opt_relaylog_index_name, 0, &mi->cmp_connection_name);
    }
    ...
    */
    ...
    if (relay_log.open_index_file(buf_relaylog_index_name, ln, TRUE) ||
        relay_log.open(ln, 0, 0, SEQ_READ_APPEND, (ulong)max_relay_log_size, 1, TRUE))
    {
  ...
  if (access(fname, F_OK))
  {
    ...
    */
    ...
    if ((info_fd = mysql_file_open(key_file_relay_log_info, fname, O_CREAT | O_RDWR | O_BINARY, MYF(MY_WME))) < 0)
    {
      sql_print_error(
    ...
    if (init_io_cache(&info_file, info_fd, LOG_BIN_IO_SIZE, READ_CACHE, 0L, 0, MYF(MY_WME)))
    {
    ...
    if (init_relay_log_pos(this, NullS, BIN_LOG_HEADER_SIZE, 0 /* no data lock */, &msg, 0))
    {
  ...
  else  // file exists
  {
    ...
    {
      int error = 0;
      if ((info_fd = mysql_file_open(key_file_relay_log_info, fname, O_RDWR | O_BINARY, MYF(MY_WME))) < 0)
      {
        sql_print_error(
            "\
Failed to open the existing relay log info file '%s' (errno %d)",
      ...
      else if (init_io_cache(&info_file, info_fd, LOG_BIN_IO_SIZE, READ_CACHE, 0L, 0, MYF(MY_WME)))
      {
        sql_print_error("Failed to create a cache on relay log info file '%s'", fname);
        error = 1;
      }
    ...
    */
    if (init_strvar_from_file(group_relay_log_name, sizeof(group_relay_log_name), &info_file, ""))
    {
    ...
    if (group_relay_log_name[0] != '\0' && *first_non_digit == '\0' && lines >= LINES_IN_RELAY_LOG_INFO_WITH_DELAY)
    {
    ...
    if (init_intvar_from_file(&relay_log_pos, &info_file, BIN_LOG_HEADER_SIZE) ||
        init_strvar_from_file(group_master_log_name, sizeof(group_master_log_name), &info_file, "") ||
        init_intvar_from_file(&master_log_pos, &info_file, 0) ||
        (lines >= LINES_IN_RELAY_LOG_INFO_WITH_DELAY && init_intvar_from_file(&sql_delay, &info_file, 0)))
    {
      msg = "Error reading slave log configuration";
      goto err;
    }
    ...
    if (init_relay_log_pos(this, group_relay_log_name, group_relay_log_pos, 0 /* no data lock*/, &msg, 0))
    {
      sql_print_error("Failed to open the relay log '%s' (relay_log_pos %llu)", group_relay_log_name,
                      group_relay_log_pos);
      goto err;
    }
  }

  DBUG_PRINT("info", ("my_b_tell(cur_log)=%llu event_relay_log_pos=%llu", my_b_tell(cur_log), event_relay_log_pos));
  ...
  */
  ...
  while (!found)
  {
    ...
    */
    ...
    if (!(ev = Log_event::read_log_event(cur_log, fdev, opt_slave_sql_verify_checksum)))
    {
      DBUG_PRINT("info", ("could not read event, cur_log->error=%d", cur_log->error));
                       ...
                       bool look_for_description_event)
{
  ...
  */
  if (pos > BIN_LOG_HEADER_SIZE) /* If pos<=4, we stay at 4 */
  {
    ...
    DBUG_PRINT("info", ("my_b_tell(rli->cur_log)=%llu rli->event_relay_log_pos=%llu", my_b_tell(rli->cur_log),
...
int Relay_log_info::wait_for_pos(THD *thd, String *log_name, longlong log_pos, longlong timeout)
{
  int event_count = 0;
  ...
  mysql_mutex_lock(&data_lock);
  thd->ENTER_COND(&data_cond, &data_lock, &stage_waiting_for_the_slave_thread_to_advance_position, &old_stage);
  /*
...
void Relay_log_info::inc_group_relay_log_pos(ulonglong log_pos, rpl_group_info *rgi, bool skip_lock)
{
  ...
  if (rgi->is_parallel_exec)
  {
    ...
    */
    if (rgi->last_master_timestamp && rgi->last_master_timestamp > last_master_timestamp)
...
int purge_relay_logs(Relay_log_info *rli, THD *thd, bool just_reset, const char **errmsg)
{
  ...
  */
  ...
  if (!rli->inited)
  {
    ...
    {
      ln = rli->relay_log.generate_name(opt_relay_logname, "-relay-bin", 1, name_buf);
      ...
      if (rli->relay_log.open_index_file(opt_relaylog_index_name, ln, TRUE))
      {
        sql_print_error(
            ...
            "log index file:%s.",
            rli->relay_log.get_index_fname());
        DBUG_RETURN(1);
      }
      mysql_mutex_lock(rli->relay_log.get_log_lock());
      if (rli->relay_log.open(ln, 0, 0, SEQ_READ_APPEND,
                              (ulong)(rli->max_relay_log_size ? rli->max_relay_log_size : max_binlog_size), 1, TRUE))
      {
        sql_print_error(
            "Unable to purge relay log files. Failed to open relay "
            "log file:%s.",
...
bool Relay_log_info::is_until_satisfied(Log_event *ev)
{
  ...
  {
    log_name = (mi->using_parallel() ? future_event_master_log_name : group_master_log_name);
  ...
  DBUG_PRINT("info",
             ("group_master_log_name='%s', group_master_log_pos=%llu", group_master_log_name, group_master_log_pos));
  DBUG_PRINT("info",
             ("group_relay_log_name='%s', group_relay_log_pos=%llu", group_relay_log_name, group_relay_log_pos));
  DBUG_PRINT("info", ("(%s) log_name='%s', log_pos=%llu", until_condition == UNTIL_MASTER_POS ? "master" : "relay",
                      log_name, log_pos));
  DBUG_PRINT("info", ("(%s) until_log_name='%s', until_log_pos=%llu",
                      until_condition == UNTIL_MASTER_POS ? "master" : "relay", until_log_name, until_log_pos));
  ...
  DBUG_RETURN(((until_log_names_cmp_result == UNTIL_LOG_NAMES_CMP_EQUAL && (log_pos >= until_log_pos && !in_trans)) ||
               until_log_names_cmp_result == UNTIL_LOG_NAMES_CMP_GREATER));
}

bool Relay_log_info::stmt_done(my_off_t event_master_log_pos, THD *thd, rpl_group_info *rgi)
{
  int error = 0;
  ...
  else
  {
    ...
    if (rpl_global_gtid_slave_state->record_and_update_gtid(thd, rgi))
    {
      report(WARNING_LEVEL, ER_CANNOT_UPDATE_GTID_STATE, rgi->gtid_info(),
             ...
             "inconsistent: %d: %s",
             "mysql", rpl_gtid_slave_state_table_name.str, thd->get_stmt_da()->sql_errno(),
...
int Relay_log_info::alloc_inuse_relaylog(const char *name)
{
  inuse_relaylog *ir;
  uint32 gtid_count;
  rpl_gtid *gtid_list;
  ...
  if (!(gtid_list = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*gtid_list) * gtid_count, MYF(MY_WME))))
  {
...
void Relay_log_info::free_inuse_relaylog(inuse_relaylog *ir)
{
  my_free(ir->relay_log_state);
  delete ir;
}

void Relay_log_info::reset_inuse_relaylog()
{
...
int Relay_log_info::update_relay_log_state(rpl_gtid *gtid_list, uint32 count)
{
  int res = 0;
  while (count)
  {
...
static int scan_one_gtid_slave_pos_table(THD *thd, HASH *hash, DYNAMIC_ARRAY *array, LEX_CSTRING *tablename,
                                         void **out_hton)
{
  TABLE_LIST tlist;
  ...
  for (;;)
  {
    ...
    DBUG_PRINT("info", ("Read slave state row: %u-%u-%lu sub_id=%lu", (unsigned)domain_id, (unsigned)server_id,
    ...
    if ((rec = my_hash_search(hash, (const uchar *)&domain_id, sizeof(domain_id))))
    {
    ...
    {
      if (!(entry = (struct gtid_pos_element *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*entry), MYF(MY_WME))))
      {
...
/*
  ...
  into HASH.
*/
static int scan_all_gtid_slave_pos_table(THD *thd, int (*cb)(THD *, LEX_CSTRING *, void *), void *cb_data)
{
  char path[FN_REFLEN];
  MY_DIR *dirp;
  ...
  else
  {
    ...
    {
      if (strncmp(files.at(i)->str, rpl_gtid_slave_state_table_name.str, rpl_gtid_slave_state_table_name.length) == 0)
      {
...
static int process_gtid_pos_table(THD *thd, LEX_CSTRING *table_name, void *hton, struct load_gtid_state_cb_data *data)
{
  struct rpl_slave_state::gtid_pos_table *p, *entry, **next_ptr;
  bool is_default = (strcmp(table_name->str, rpl_gtid_slave_state_table_name.str) == 0);
  ...
  */
  ...
  while (entry)
  {
    if (entry->table_hton == hton)
    {
      static const char *warning_msg =
          "Ignoring redundant table mysql.%s "
  ...
  p = rpl_global_gtid_slave_state->alloc_gtid_pos_table(table_name, hton, rpl_slave_state::GTID_POS_AVAILABLE);
  if (!p)
    return 1;
...
*/
static int gtid_pos_auto_create_tables(rpl_slave_state::gtid_pos_table **list_ptr)
{
  plugin_ref *auto_engines;
  int err = 0;
  mysql_mutex_lock(&LOCK_global_system_variables);
  for (auto_engines = opt_gtid_pos_auto_plugins; !err && auto_engines && *auto_engines; ++auto_engines)
  {
    ...
    table_case_convert(const_cast<char *>(table_name.str), static_cast<uint>(table_name.length));
    entry = rpl_global_gtid_slave_state->alloc_gtid_pos_table(&table_name, hton, rpl_slave_state::GTID_POS_AUTO_CREATE);
    if (!entry)
    {
      err = 1;
      break;
...
static int load_gtid_state_cb(THD *thd, LEX_CSTRING *table_name, void *arg)
{
  int err;
  load_gtid_state_cb_data *data = static_cast<load_gtid_state_cb_data *>(arg);
  void *hton;

  if ((err = scan_one_gtid_slave_pos_table(thd, data->hash, data->array, table_name, &hton)))
    return err;
...
int rpl_load_gtid_slave_state(THD *thd)
{
  ...
  my_hash_init(PSI_INSTRUMENT_ME, &hash, &my_charset_bin, 32,
               offsetof(gtid_pos_element, gtid) + offsetof(rpl_gtid, domain_id), sizeof(uint32), NULL, my_free,
               HASH_UNIQUE);
  if ((err = my_init_dynamic_array(PSI_INSTRUMENT_ME, &array, sizeof(gtid_pos_element), 0, 0, MYF(0))))
    goto end;
  array_inited = true;
  ...
  {
    my_error(ER_NO_SUCH_TABLE, MYF(0), "mysql", rpl_gtid_slave_state_table_name.str);
    mysql_mutex_unlock(&rpl_global_gtid_slave_state->LOCK_slave_state);
    err = 1;
    goto end;
  }
  ...
  for (i = 0; i < array.elements; ++i)
  {
    ...
    if ((err = rpl_global_gtid_slave_state->update(tmp_entry.gtid.domain_id, tmp_entry.gtid.server_id, tmp_entry.sub_id,
                                                   tmp_entry.gtid.seq_no, tmp_entry.hton, NULL)))
    {
      mysql_mutex_unlock(&rpl_global_gtid_slave_state->LOCK_slave_state);
      my_error(ER_OUT_OF_RESOURCES, MYF(0));
      goto end;
  ...
  for (i = 0; i < hash.records; ++i)
  {
    ...
    if (opt_bin_log && mysql_bin_log.bump_seq_no_counter_if_needed(entry->gtid.domain_id, entry->gtid.seq_no))
    {
      mysql_mutex_unlock(&rpl_global_gtid_slave_state->LOCK_slave_state);
      my_error(ER_OUT_OF_RESOURCES, MYF(0));
      goto end;
  ...
  rpl_global_gtid_slave_state->set_gtid_pos_tables_list(cb_data.table_list, cb_data.default_entry);
  cb_data.table_list = NULL;
  rpl_global_gtid_slave_state->loaded = true;
  mysql_mutex_unlock(&rpl_global_gtid_slave_state->LOCK_slave_state);

  /* Clear out no longer needed elements now. */
  old_gtids_list = rpl_global_gtid_slave_state->gtid_grab_pending_delete_list();
...
static int find_gtid_pos_tables_cb(THD *thd, LEX_CSTRING *table_name, void *arg)
{
  load_gtid_state_cb_data *data = static_cast<load_gtid_state_cb_data *>(arg);
  TABLE_LIST tlist;
  TABLE *table = NULL;
...
int find_gtid_slave_pos_tables(THD *thd)
{
  ...
  else
  {
    ...
    */
    ...
    auto old_entry = rpl_global_gtid_slave_state->gtid_pos_tables.load(std::memory_order_relaxed);
    while (old_entry)
    {
    ...
    while (new_entry)
    {
      ...
      old_entry = rpl_global_gtid_slave_state->gtid_pos_tables.load(std::memory_order_relaxed);
      while (old_entry)
      {
        if (new_entry->table_hton == old_entry->table_hton)
          break;
...
rpl_group_info::rpl_group_info(Relay_log_info *rli)
    : thd(0),
      wait_commit_sub_id(0),
      wait_commit_group_info(0),
      parallel_entry(0),
...
void rpl_group_info::cleanup_context(THD *thd, bool error)
{
  ...
  */
  thd->variables.option_bits &=
      ~(OPTION_NO_FOREIGN_KEY_CHECKS | OPTION_RELAXED_UNIQUE_CHECKS | OPTION_NO_CHECK_CONSTRAINT_CHECKS);
...
static void mark_start_commit_inner(rpl_parallel_entry *e, group_commit_orderer *gco, rpl_group_info *rgi)
{
...
void rpl_group_info::mark_start_commit()
{
...
*/
char *rpl_group_info::gtid_info()
{
  if (!gtid_sub_id || !current_gtid.seq_no)
    return NULL;
  my_snprintf(gtid_info_buf, sizeof(gtid_info_buf), "Gtid %u-%u-%llu", current_gtid.domain_id, current_gtid.server_id,
              current_gtid.seq_no);
...
*/
void rpl_group_info::unmark_start_commit()
{
  rpl_parallel_entry *e;
...
rpl_sql_thread_info::rpl_sql_thread_info(Rpl_filter *filter) : rpl_filter(filter) { cached_charset_invalidate(); }
...
bool Relay_log_info::flush()
{
  ...
  *pos++ = '\n';
  ...
  if (flush_io_cache(file))
    error = 1;
  if (sync_relayloginfo_period && !error && ++sync_counter >= sync_relayloginfo_period)
  {
