...
bool mysql_manager_submit(void (*action)(void *), void *data)
{
  ...
  cb = &cb_list;
  while (*cb) cb = &(*cb)->next;
  *cb = (struct handler_cb *)my_malloc(PSI_INSTRUMENT_ME, sizeof(struct handler_cb), MYF(MY_WME));
  if (!*cb)
    result = TRUE;
  else
  {
...
void start_handle_manager()
{
  ...
  abort_manager = false;
  {
    ...
    if ((err = mysql_thread_create(key_thread_handle_manager, &hThread, &connection_attrib, handle_manager, 0)))
...
void stop_handle_manager()
{
  ...
  if (manager_thread_in_use)
  {
    mysql_mutex_lock(&LOCK_manager);
    abort_manager = true;
    DBUG_PRINT("quit", ("initiate shutdown of handle manager thread: %lu", (ulong)manager_thread));
    mysql_cond_signal(&COND_manager);
    mysql_mutex_unlock(&LOCK_manager);
  }
  DBUG_VOID_RETURN;
