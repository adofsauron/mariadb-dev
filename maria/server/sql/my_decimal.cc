...
int decimal_operation_results(int result, const char *value, const char *type)
{
  ...
  switch (result)
  {
    case E_DEC_TRUNCATED:
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_DATA_TRUNCATED, ER_THD(thd, ER_DATA_TRUNCATED), value,
                          type);
      break;
    case E_DEC_OVERFLOW:
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_DATA_OVERFLOW, ER_THD(thd, ER_DATA_OVERFLOW), value,
                          type);
      break;
    case E_DEC_DIV_ZERO:
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_DIVISION_BY_ZERO, ER_THD(thd, ER_DIVISION_BY_ZERO));
      break;
    case E_DEC_BAD_NUM:
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_BAD_DATA, ER_THD(thd, ER_BAD_DATA), value, type);
      break;
    case E_DEC_OOM:
...
int my_decimal::to_string_native(String *str, uint fixed_prec, uint fixed_dec, char filler, uint mask) const
{
  /*
  ...
  */
  int length = (fixed_prec ? (fixed_prec + ((fixed_prec == fixed_dec) ? 1 : 0) + 1) : my_decimal_string_length(this));
  int result;
  if (str->alloc(length + 1))  // Alloc also space for \0
    return check_result(mask, E_DEC_OOM);
  result = decimal2string(this, (char *)str->ptr(), &length, (int)fixed_prec, fixed_dec, filler);
  str->length(length);
...
int my_decimal::to_binary(uchar *bin, int prec, decimal_digits_t scale, uint mask) const
{
...
int my_decimal2int(uint mask, const decimal_t *d, bool unsigned_flag, longlong *l, decimal_round_mode round_type)
{
  int res;
  my_decimal rounded;
  /* decimal_round can return only E_DEC_TRUNCATED */
  decimal_round(d, &rounded, 0, round_type);
  res = (unsigned_flag ? decimal2ulonglong(&rounded, (ulonglong *)l) : decimal2longlong(&rounded, l));
  if (res & mask)
  {
    ...
    decimal_operation_results(res, buff, unsigned_flag ? "UNSIGNED INT" : "INT");
  }
  return res;
}
