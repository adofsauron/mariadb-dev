...
static const std::string create_cluster_table_str = "CREATE TABLE IF NOT EXISTS " + wsrep_schema_str + "." +
                                                    cluster_table_str +
                                                    "("
...
static const std::string create_members_table_str = "CREATE TABLE IF NOT EXISTS " + wsrep_schema_str + "." +
                                                    members_table_str +
                                                    "("
...
static const std::string create_members_history_table_str = "CREATE TABLE IF NOT EXISTS " + wsrep_schema_str + "." +
...
static const std::string create_frag_table_str = "CREATE TABLE IF NOT EXISTS " + wsrep_schema_str + "." + sr_table_str +
                                                 "("
                                                 "node_uuid CHAR(36), "
                                                 "trx_id BIGINT, "
                                                 "seqno BIGINT, "
...
static const std::string create_allowlist_table_str = "CREATE TABLE IF NOT EXISTS " + wsrep_schema_str + "." +
                                                      allowlist_table_str +
                                                      "("
                                                      "ip CHAR(64) NOT NULL,"
                                                      "PRIMARY KEY (ip)"
...
static const std::string delete_from_cluster_table = "DELETE FROM " + wsrep_schema_str + "." + cluster_table_str;

static const std::string delete_from_members_table = "DELETE FROM " + wsrep_schema_str + "." + members_table_str;
...
static const std::string alter_cluster_table =
    "ALTER TABLE " + wsrep_schema_str + "." + cluster_table_str + " STATS_PERSISTENT=0";

static const std::string alter_members_table =
    "ALTER TABLE " + wsrep_schema_str + "." + members_table_str + " STATS_PERSISTENT=0";

#ifdef WSREP_SCHEMA_MEMBERS_HISTORY
static const std::string alter_members_history_table =
    "ALTER TABLE " + wsrep_schema_str + "." + members_history_table_str + " STATS_PERSISTENT=0";
#endif

static const std::string alter_frag_table =
    "ALTER TABLE " + wsrep_schema_str + "." + sr_table_str + " STATS_PERSISTENT=0";
...
class binlog_off
{
 public:
  binlog_off(THD *thd)
      : m_thd(thd), m_option_bits(thd->variables.option_bits), m_sql_log_bin(thd->variables.sql_log_bin)
  {
...
{
 public:
  wsrep_off(THD *thd) : m_thd(thd), m_wsrep_on(thd->variables.wsrep_on) { thd->variables.wsrep_on = 0; }
...
{
 public:
  thd_server_status(THD *thd, uint server_status, bool condition) : m_thd(thd), m_thd_server_status(thd->server_status)
  {
    if (condition)
      thd->server_status = server_status;
  }
  ~thd_server_status() { m_thd->server_status = m_thd_server_status; }
...
{
 public:
  thd_context_switch(THD *orig_thd, THD *cur_thd) : m_orig_thd(orig_thd), m_cur_thd(cur_thd)
  {
...
{
 public:
  sql_safe_updates(THD *thd) : m_thd(thd), m_option_bits(thd->variables.option_bits)
  {
    thd->variables.option_bits &= ~OPTION_SAFE_UPDATES;
  }
  ~sql_safe_updates() { m_thd->variables.option_bits = m_option_bits; }
...
static int execute_SQL(THD *thd, const char *sql, uint length)
{
  ...
  if (parser_state.init(thd, (char *)sql, length) == 0)
  {
    ...
    if (thd->is_error())
    {
      WSREP_WARN("Wsrep_schema::execute_sql() failed, %d %s\nSQL: %s", thd->get_stmt_da()->sql_errno(),
                 thd->get_stmt_da()->message(), sql);
      err = 1;
    }
...
static int open_table(THD *thd, const LEX_CSTRING *schema_name, const LEX_CSTRING *table_name,
                      enum thr_lock_type const lock_type, TABLE **table)
{
  ...
  *table = NULL;
  ...
  tables.init_one_table(schema_name, table_name, NULL, lock_type);
  thd->lex->query_tables_own_last = 0;

  // No need to open table if the query was bf aborted,
  // thd client will get ER_LOCK_DEADLOCK in the end.
  const bool interrupted =
      thd->killed || (thd->is_error() && (thd->get_stmt_da()->sql_errno() == ER_QUERY_INTERRUPTED));

  if (interrupted || !open_n_lock_single_table(thd, &tables, tables.lock_type, flags))
  {
    close_thread_tables(thd);
    DBUG_RETURN(1);
  }
...
static int open_for_write(THD *thd, const char *table_name, TABLE **table)
{
  ...
  if (Wsrep_schema_impl::open_table(thd, &schema_str, &table_str, TL_WRITE, table))
  {
    ...
    if (!interrupted)
    {
      WSREP_ERROR("Failed to open table %s.%s for writing", schema_str.str, table_name);
    }
    return 1;
  }
  empty_record(*table);
...
static void store(TABLE *table, uint field, const Wsrep_id &id)
{
  ...
  os << id;
  table->field[field]->store(os.str().c_str(), os.str().size(), &my_charset_bin);
}
...
static void store(TABLE *table, uint field, const std::string &str) { store(table, field, str.c_str(), str.size()); }
...
static int update_or_insert(TABLE *table)
{
  ...
  */
  if (table->s->primary_key >= MAX_KEY || !table->s->keys_in_use.is_set(table->s->primary_key))
  {
    WSREP_ERROR("No primary key for %s.%s", table->s->db.str, table->s->table_name.str);
    DBUG_RETURN(1);
  }
  ...
  */
  ...
  {
    WSREP_ERROR("Error allocating %ud bytes for key", table->s->max_unique_length);
    DBUG_RETURN(1);
  }

  key_copy((uchar *)key, table->record[0], table->key_info + table->s->primary_key, 0);

  if ((error = table->file->ha_index_read_idx_map(table->record[1], table->s->primary_key, (uchar *)key, HA_WHOLE_KEY,
                                                  HA_READ_KEY_EXACT)))
  {
    /*
    ...
    */
    ...
    {
      WSREP_ERROR("Error writing into %s.%s: %d", table->s->db.str, table->s->table_name.str, error);
      ret = 1;
    }
  }
  else if (!records_are_comparable(table) || compare_record(table))
  {
    ...
    */
    ...
    {
      WSREP_ERROR("Error updating record in %s.%s: %d", table->s->db.str, table->s->table_name.str, error);
      ret = 1;
    }
  }
...
static int insert(TABLE *table)
{
  ...
  */
  ...
  if ((error = table->file->ha_write_row(table->record[0])))
  {
    ...
    {
      WSREP_WARN("Duplicate key found when writing into %s.%s", table->s->db.str, table->s->table_name.str);
...
static int delete_row(TABLE *table)
{
  ...
  if (error)
  {
    WSREP_ERROR("Error deleting row from %s.%s: %d", table->s->db.str, table->s->table_name.str, error);
    return 1;
  }
  return 0;
}
...
static int open_for_read(THD *thd, const char *table_name, TABLE **table)
{
  LEX_CSTRING schema_str = {wsrep_schema_str.c_str(), wsrep_schema_str.length()};
  LEX_CSTRING table_str = {table_name, strlen(table_name)};
  if (Wsrep_schema_impl::open_table(thd, &schema_str, &table_str, TL_READ, table))
  {
    WSREP_ERROR("Failed to open table %s.%s for reading", schema_str.str, table_name);
    return 1;
  }
  empty_record(*table);
  (*table)->use_all_columns();
...
{
  int error;
  if ((error = table->file->ha_rnd_next(table->record[0])) && error != HA_ERR_END_OF_FILE)
  {
 ...
 */
static int scan_member(TABLE *table, const Wsrep_id &cluster_uuid, std::vector<Wsrep_view::member> &members)
{
  Wsrep_id member_id;
  ...
  if (scan(table, 0, member_id) || scan(table, 2, member_name, sizeof(member_name)) ||
  ...
  try
  {
    members.push_back(Wsrep_view::member(member_id, member_name, member_incoming));
  }
  catch (...)
  {
...
/*
  ...
  @return 0 in case of success, error code in case of error.
 */
static int init_for_index_scan(TABLE *table, const uchar *key, key_part_map map)
{
  int error;
  ...
  error = table->file->ha_index_read_map(table->record[0], key, map, HA_READ_KEY_EXACT);
  switch (error)
  {
    case 0:
...
int Wsrep_schema::init()
{
  ...
  if (Wsrep_schema_impl::execute_SQL(thd, create_cluster_table_str.c_str(), create_cluster_table_str.size()) ||
      Wsrep_schema_impl::execute_SQL(thd, create_members_table_str.c_str(), create_members_table_str.size()) ||
#ifdef WSREP_SCHEMA_MEMBERS_HISTORY
      Wsrep_schema_impl::execute_SQL(thd, create_members_history_table_str.c_str(),
                                     create_members_history_table_str.size()) ||
      Wsrep_schema_impl::execute_SQL(thd, alter_members_history_table.c_str(), alter_members_history_table.size()) ||
#endif /* WSREP_SCHEMA_MEMBERS_HISTORY */
      Wsrep_schema_impl::execute_SQL(thd, create_frag_table_str.c_str(), create_frag_table_str.size()) ||
      Wsrep_schema_impl::execute_SQL(thd, alter_cluster_table.c_str(), alter_cluster_table.size()) ||
      Wsrep_schema_impl::execute_SQL(thd, alter_members_table.c_str(), alter_members_table.size()) ||
      Wsrep_schema_impl::execute_SQL(thd, alter_frag_table.c_str(), alter_frag_table.size()) ||
      Wsrep_schema_impl::execute_SQL(thd, create_allowlist_table_str.c_str(), create_allowlist_table_str.size()))
  {
    ret = 1;
  }
  else
...
int Wsrep_schema::store_view(THD *thd, const Wsrep_view &view)
{
  ...
  */
  if (Wsrep_schema_impl::execute_SQL(thd, delete_from_cluster_table.c_str(), delete_from_cluster_table.size()) ||
      Wsrep_schema_impl::execute_SQL(thd, delete_from_members_table.c_str(), delete_from_members_table.size()))
  {
    goto out;
  }
  ...
  */
  Wsrep_schema_impl::init_stmt(thd);
  if (Wsrep_schema_impl::open_for_write(thd, members_table_str.c_str(), &members_table))
  {
  ...
  */
  Wsrep_schema_impl::init_stmt(thd);
  if (Wsrep_schema_impl::open_for_write(thd, cluster_member_history.c_str(), &members_history_table))
  {
    WSREP_ERROR("failed to open wsrep.members table");
    goto out;
  }
  ...
  for (size_t i = 0; i < view.members().size(); ++i)
  {
    ...
    Wsrep_schema_impl::store(members_history_table, 4, view.members()[i].name());
    Wsrep_schema_impl::store(members_history_table, 5, view.members()[i].incoming());
...
Wsrep_view Wsrep_schema::restore_view(THD *thd, const Wsrep_id &own_id) const
{
  ...
  if (((error = Wsrep_schema_impl::next_record(cluster_table)) != 0 ||
       Wsrep_schema_impl::scan(cluster_table, 0, cluster_uuid) || Wsrep_schema_impl::scan(cluster_table, 1, view_id) ||
       Wsrep_schema_impl::scan(cluster_table, 2, view_seqno) || Wsrep_schema_impl::scan(cluster_table, 3, proto_ver) ||
  ...
  */
  ...
  while (true)
  {
    ...
    {
      if (Wsrep_schema_impl::scan_member(members_table, cluster_uuid, members))
      {
        goto out;
      }
    }
...
out:
  ...
  if (end_members_scan)
    Wsrep_schema_impl::end_scan(members_table);
  ...
  if (0 == ret)
  {
    Wsrep_view ret_view(wsrep::gtid(cluster_uuid, Wsrep_seqno(view_seqno)), Wsrep_seqno(view_id), wsrep::view::primary,
...
int Wsrep_schema::append_fragment(THD *thd, const wsrep::id &server_id, wsrep::transaction_id transaction_id,
                                  wsrep::seqno seqno, int flags, const wsrep::const_buffer &data)
{
  DBUG_ENTER("Wsrep_schema::append_fragment");
  std::ostringstream os;
  os << server_id;
  WSREP_DEBUG("Append fragment(%llu) %s, %llu", thd->thread_id, os.str().c_str(), transaction_id.get());
...
int Wsrep_schema::update_fragment_meta(THD *thd, const wsrep::ws_meta &ws_meta)
{
  DBUG_ENTER("Wsrep_schema::update_fragment_meta");
  std::ostringstream os;
  os << ws_meta.server_id();
  WSREP_DEBUG("update_frag_seqno(%llu) %s, %llu, seqno %lld", thd->thread_id, os.str().c_str(),
 ...
 */
  ...
  if ((error = Wsrep_schema_impl::init_for_index_scan(frag_table, key, key_map)))
  {
    ...
    {
      WSREP_WARN("Record not found in %s.%s: %d", frag_table->s->db.str, frag_table->s->table_name.str, error);
    }
    Wsrep_schema_impl::finish_stmt(thd);
    thd->lex->restore_backup_query_tables_list(&query_tables_list_backup);
    my_free(key);
  ...
  if ((error = frag_table->file->ha_update_row(frag_table->record[1], frag_table->record[0])))
  {
    WSREP_ERROR("Error updating record in %s.%s: %d", frag_table->s->db.str, frag_table->s->table_name.str, error);
    Wsrep_schema_impl::finish_stmt(thd);
    thd->lex->restore_backup_query_tables_list(&query_tables_list_backup);
    DBUG_RETURN(1);
  }
...
static int remove_fragment(THD *thd, TABLE *frag_table, const wsrep::id &server_id,
                           wsrep::transaction_id transaction_id, wsrep::seqno seqno)
{
  WSREP_DEBUG("remove_fragment(%llu) trx %llu, seqno %lld", thd->thread_id, transaction_id.get(), seqno.get());
  int ret = 0;
  int error;
  uchar *key = NULL;
  key_part_map key_map = 0;
  ...
  */
  ...
  if ((error = Wsrep_schema_impl::init_for_index_scan(frag_table, key, key_map)))
  {
    if (error == HA_ERR_END_OF_FILE || error == HA_ERR_KEY_NOT_FOUND)
    {
      WSREP_DEBUG("Record not found in %s.%s:trx %llu, seqno %lld, error %d", frag_table->s->db.str,
                  frag_table->s->table_name.str, transaction_id.get(), seqno.get(), error);
    }
    ret = error;
  }
...
int Wsrep_schema::remove_fragments(THD *thd, const wsrep::id &server_id, wsrep::transaction_id transaction_id,
                                   const std::vector<wsrep::seqno> &fragments)
{
  ...
  {
    for (std::vector<wsrep::seqno>::const_iterator i = fragments.begin(); i != fragments.end(); ++i)
    {
      if (remove_fragment(thd, frag_table, server_id, transaction_id, *i))
      {
        ret = 1;
        break;
      }
  ...
  if (thd->wsrep_cs().mode() == wsrep::client_state::m_local && !thd->in_multi_stmt_transaction_mode())
  {
    /*
  ...
  {
    Wsrep_schema_impl::thd_server_status thd_server_status(thd, thd->server_status | SERVER_STATUS_IN_TRANS,
...
int Wsrep_schema::replay_transaction(THD *orig_thd, Relay_log_info *rli, const wsrep::ws_meta &ws_meta,
                                     const std::vector<wsrep::seqno> &fragments)
{
  ...
  for (std::vector<wsrep::seqno>::const_iterator i = fragments.begin(); i != fragments.end(); ++i)
  {
    ...
    int error = Wsrep_schema_impl::init_for_index_scan(frag_table, key, key_map);
    if (error)
    {
      WSREP_WARN("Failed to init streaming log table for index scan: %d", error);
      Wsrep_schema_impl::end_index_scan(frag_table);
      ret = 1;
      break;
    }
    ...
    WSREP_DEBUG("replay_fragment(%llu): seqno: %lld flags: %x", ws_meta.transaction_id().get(), i->get(), flags);
    String buf;
...
int Wsrep_schema::recover_sr_transactions(THD *orig_thd)
{
  ...
  storage_thd.thread_stack = (orig_thd ? orig_thd->thread_stack : (char *)&storage_thd);
  ...
  Wsrep_schema_impl::thd_context_switch thd_context_switch(orig_thd, &storage_thd);
  ...
  if (Wsrep_schema_impl::open_for_read(&storage_thd, cluster_table_str.c_str(), &cluster_table) ||
      Wsrep_schema_impl::init_for_scan(cluster_table))
  {
  ...
  */
  ...
  while (0 == error)
  {
    if ((error = Wsrep_schema_impl::next_record(frag_table)) == 0)
    {
      ...
      wsrep::ws_meta ws_meta(gtid, wsrep::stid(server_id, transaction_id, client_id), wsrep::seqno::undefined(), flags);

      wsrep::high_priority_service *applier;
      if (!(applier = server_state.find_streaming_applier(server_id, transaction_id)))
      {
        DBUG_ASSERT(wsrep::starts_transaction(flags));
        applier = wsrep_create_streaming_applier(&storage_thd, "recovery");
        server_state.start_streaming_applier(server_id, transaction_id, applier);
        applier->start_transaction(wsrep::ws_handle(transaction_id, 0), ws_meta);
      }
...
void Wsrep_schema::clear_allowlist()
{
  ...
  if (Wsrep_schema_impl::open_for_write(thd, allowlist_table_str.c_str(), &allowlist_table) ||
...
{
  Allowlist_check_arg(const std::string &value) : value(value), response(false) {}
  std::string value;
  bool response;
} Allowlist_check_arg;
...
static void *allowlist_check_thread(void *param)
{
   ...
   */
  Wsrep_schema_impl::init_stmt(&thd);
  if (Wsrep_schema_impl::open_for_read(&thd, allowlist_table_str.c_str(), &allowlist_table) ||
      Wsrep_schema_impl::init_for_scan(allowlist_table))
  {
    goto out;
  }
...
bool Wsrep_schema::allowlist_check(Wsrep_allowlist_key key, const std::string &value)
{
  ...
  ret = mysql_thread_create(0, /* Not instrumented */
                            &allowlist_check_thd, NULL, allowlist_check_thread, &arg);
  if (ret)
  {
    WSREP_ERROR("allowlist_check(): mysql_thread_create() failed: %d (%s)", ret, strerror(ret));
    return false;
  }
