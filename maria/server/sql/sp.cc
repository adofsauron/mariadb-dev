...
sp_cache **Sp_handler_procedure::get_cache(THD *thd) const { return &thd->sp_proc_cache; }

sp_cache **Sp_handler_function::get_cache(THD *thd) const { return &thd->sp_func_cache; }

sp_cache **Sp_handler_package_spec::get_cache(THD *thd) const { return &thd->sp_package_spec_cache; }

sp_cache **Sp_handler_package_body::get_cache(THD *thd) const { return &thd->sp_package_body_cache; }

ulong Sp_handler_procedure::recursion_depth(THD *thd) const { return thd->variables.max_sp_recursion_depth; }

bool Sp_handler::add_instr_freturn(THD *thd, sp_head *sp, sp_pcontext *spcont, Item *item, LEX *lex) const
{
  my_error(ER_SP_BADRETURN, MYF(0));
  return true;
}

bool Sp_handler::add_instr_preturn(THD *thd, sp_head *sp, sp_pcontext *spcont) const
{
  thd->parse_error();
  return true;
}

bool Sp_handler_function::add_instr_freturn(THD *thd, sp_head *sp, sp_pcontext *spcont, Item *item, LEX *lex) const
{
  return sp->add_instr_freturn(thd, spcont, item, lex);
}

bool Sp_handler_procedure::add_instr_preturn(THD *thd, sp_head *sp, sp_pcontext *spcont) const
{
...
const Sp_handler *Sp_handler_procedure::package_routine_handler() const { return &sp_handler_package_procedure; }

const Sp_handler *Sp_handler_function::package_routine_handler() const { return &sp_handler_package_function; }
...
static const TABLE_FIELD_TYPE proc_table_fields[MYSQL_PROC_FIELD_COUNT] = {
    ...
    {{STRING_WITH_LEN("type")}, {STRING_WITH_LEN("enum('FUNCTION','PROCEDURE')")}, {NULL, 0}},
    {{STRING_WITH_LEN("specific_name")}, {STRING_WITH_LEN("char(64)")}, {STRING_WITH_LEN("utf8mb3")}},
    {{STRING_WITH_LEN("language")}, {STRING_WITH_LEN("enum('SQL')")}, {NULL, 0}},
    {{STRING_WITH_LEN("sql_data_access")},
     {STRING_WITH_LEN("enum('CONTAINS_SQL','NO_SQL','READS_SQL_DATA','MODIFIES_SQL_DATA')")},
     {NULL, 0}},
    {{STRING_WITH_LEN("is_deterministic")}, {STRING_WITH_LEN("enum('YES','NO')")}, {NULL, 0}},
    {{STRING_WITH_LEN("security_type")}, {STRING_WITH_LEN("enum('INVOKER','DEFINER')")}, {NULL, 0}},
    ...
    {{STRING_WITH_LEN("sql_mode")},
     ...
     {NULL, 0}},
    {{STRING_WITH_LEN("comment")}, {STRING_WITH_LEN("text")}, {STRING_WITH_LEN("utf8mb3")}},
    ...
    {{STRING_WITH_LEN("aggregate")}, {STRING_WITH_LEN("enum('NONE','GROUP')")}, {NULL, 0}}};

static const TABLE_FIELD_DEF proc_table_def = {MYSQL_PROC_FIELD_COUNT, proc_table_fields, 0, (uint *)0};
...
class Stored_routine_creation_ctx : public Stored_program_creation_ctx, public Sql_alloc
{
 public:
  static Stored_routine_creation_ctx *load_from_db(THD *thd, const Database_qualified_name *name, TABLE *proc_tbl);

 public:
  virtual Stored_program_creation_ctx *clone(MEM_ROOT *mem_root)
  {
    return new (mem_root) Stored_routine_creation_ctx(m_client_cs, m_connection_cl, m_db_cl);
  }
 ...
 private:
  ...
  Stored_routine_creation_ctx(CHARSET_INFO *client_cs, CHARSET_INFO *connection_cl, CHARSET_INFO *db_cl)
...
bool load_charset(THD *thd, MEM_ROOT *mem_root, Field *field, CHARSET_INFO *dflt_cs, CHARSET_INFO **cs)
{
  LEX_CSTRING cs_name;
...
bool load_collation(THD *thd, MEM_ROOT *mem_root, Field *field, CHARSET_INFO *dflt_cl, CHARSET_INFO **cl)
{
  LEX_CSTRING cl_name;
...
Stored_routine_creation_ctx *Stored_routine_creation_ctx::load_from_db(THD *thd, const Database_qualified_name *name,
                                                                       TABLE *proc_tbl)
{
  ...
  if (load_charset(thd, thd->mem_root, proc_tbl->field[MYSQL_PROC_FIELD_CHARACTER_SET_CLIENT],
                   thd->variables.character_set_client, &client_cs))
  {
    sql_print_warning(
        "Stored routine '%s'.'%s': invalid value "
        "in column mysql.proc.character_set_client.",
        (const char *)db_name, (const char *)sr_name);

    invalid_creation_ctx = TRUE;
  }

  if (load_collation(thd, thd->mem_root, proc_tbl->field[MYSQL_PROC_FIELD_COLLATION_CONNECTION],
  ...
  if (load_collation(thd, thd->mem_root, proc_tbl->field[MYSQL_PROC_FIELD_DB_COLLATION], NULL, &db_cl))
  {
    sql_print_warning(
        "Stored routine '%s'.'%s': invalid value "
  ...
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_SR_INVALID_CREATION_CTX,
                        ER_THD(thd, ER_SR_INVALID_CREATION_CTX), (const char *)db_name, (const char *)sr_name);
  }
...
int Sp_handler::db_find_routine_aux(THD *thd, const Database_qualified_name *name, TABLE *table) const
{
  uchar key[MAX_KEY_LENGTH];  // db, name, optional key length type
  DBUG_ENTER("db_find_routine_aux");
  DBUG_PRINT("enter", ("type: %s  name: %.*s", type_str(), (int)name->m_name.length, name->m_name.str));
...
bool st_sp_chistics::read_from_mysql_proc_row(THD *thd, TABLE *table)
{
  ...
  if (table->field[MYSQL_PROC_FIELD_ACCESS]->val_str_nopad(thd->mem_root, &str))
    return true;
  ...
  if (table->field[MYSQL_PROC_FIELD_DETERMINISTIC]->val_str_nopad(thd->mem_root, &str))
    return true;
  detistic = str.str[0] == 'N' ? false : true;

  if (table->field[MYSQL_PROC_FIELD_SECURITY_TYPE]->val_str_nopad(thd->mem_root, &str))
    return true;
  suid = str.str[0] == 'I' ? SP_IS_NOT_SUID : SP_IS_SUID;

  if (table->field[MYSQL_PROC_FIELD_AGGREGATE]->val_str_nopad(thd->mem_root, &str))
    return true;
  ...
  if (table->field[MYSQL_PROC_FIELD_COMMENT]->val_str_nopad(thd->mem_root, &comment))
    return true;
...
{
  LEX_CSTRING str;
  if (table->field[MYSQL_PROC_FIELD_DEFINER]->val_str_nopad(thd->mem_root, &str))
    return true;
...
int Sp_handler::db_find_routine(THD *thd, const Database_qualified_name *name, sp_head **sphp) const
{
  TABLE *table;
  ...
  *sphp = 0;  // In case of errors
  ...
  if (chistics.read_from_mysql_proc_row(thd, table) || definer.read_from_mysql_proc_row(thd, table))
  {
    ret = SP_GET_FIELD_FAILED;
    goto done;
  }
  ...
  else if (table->field[MYSQL_PROC_FIELD_RETURNS]->val_str_nopad(thd->mem_root, &returns))
  {
    ret = SP_GET_FIELD_FAILED;
    goto done;
  }

  if (table->field[MYSQL_PROC_FIELD_BODY]->val_str_nopad(thd->mem_root, &body))
  {
    ret = SP_GET_FIELD_FAILED;
    goto done;
  }
  ...
  ret = db_load_routine(thd, name, sphp, sql_mode, params, returns, body, chistics, definer, created, modified, NULL,
                        creation_ctx);
done:
  /*
...
int Sp_handler::db_find_and_cache_routine(THD *thd, const Database_qualified_name *name, sp_head **sp) const
{
  int rc = db_find_routine(thd, name, sp);
  if (rc == SP_OK)
  {
    sp_cache_insert(get_cache(thd), *sp);
    DBUG_PRINT("info", ("added new: %p, level: %lu, flags %x", sp[0], sp[0]->m_recursion_level, sp[0]->m_flags));
  }
  return rc;
}
...
bool Silence_deprecated_warning::handle_condition(THD *, uint sql_errno, const char *,
                                                  Sql_condition::enum_warning_level *level, const char *,
                                                  Sql_condition **cond_hdl)
{
  *cond_hdl = NULL;
  if (sql_errno == ER_WARN_DEPRECATED_SYNTAX && *level == Sql_condition::WARN_LEVEL_WARN)
    return TRUE;
...
static sp_head *sp_compile(THD *thd, String *defstr, sql_mode_t sql_mode, sp_package *parent,
                           Stored_program_creation_ctx *creation_ctx)
{
  sp_head *sp;
...
class Bad_db_error_handler : public Internal_error_handler
{
 public:
  ...
  virtual bool handle_condition(THD *thd, uint sql_errno, const char *sqlstate,
                                Sql_condition::enum_warning_level *level, const char *message,
                                Sql_condition **cond_hdl);
...
bool Bad_db_error_handler::handle_condition(THD *thd, uint sql_errno, const char *sqlstate,
                                            Sql_condition::enum_warning_level *level, const char *message,
                                            Sql_condition **cond_hdl)
{
...
int Sp_handler::db_load_routine(THD *thd, const Database_qualified_name *name, sp_head **sphp, sql_mode_t sql_mode,
                                const LEX_CSTRING &params, const LEX_CSTRING &returns, const LEX_CSTRING &body,
                                const st_sp_chistics &chistics, const AUTHID &definer, longlong created,
                                longlong modified, sp_package *parent, Stored_program_creation_ctx *creation_ctx) const
{
  ...
  if (show_create_sp(thd, &defstr, null_clex_str, name->m_name, params, returns, body, chistics, definer, DDL_options(),
                     sql_mode))
  {
  ...
  */
  ...
  if (mysql_opt_change_db(thd, &name->m_db, &saved_cur_db_name, TRUE, &cur_db_changed))
  {
    ret = SP_INTERNAL_ERROR;
    thd->pop_internal_handler();
    goto end;
...

  {
    ...
    */
    ...
    if (cur_db_changed && mysql_change_db(thd, (LEX_CSTRING *)&saved_cur_db_name, TRUE))
    {
      ret = SP_INTERNAL_ERROR;
      goto end;
    }
...
void sp_returns_type(THD *thd, String &result, const sp_head *sp)
{
  TABLE table;
  TABLE_SHARE share;
  Field *field;
...
int Sp_handler::sp_drop_routine_internal(THD *thd, const Database_qualified_name *name, TABLE *table) const
{
  ...
  */
  ...
  MYSQL_DROP_SP(type(), name->m_db.str, static_cast<uint>(name->m_db.length), name->m_name.str,
                static_cast<uint>(name->m_name.length));
  DBUG_RETURN(SP_OK);
}

int Sp_handler::sp_find_and_drop_routine(THD *thd, TABLE *table, const Database_qualified_name *name) const
{
  int ret;
  if ((ret = db_find_routine_aux(thd, name, table)) != SP_OK)
    return ret;
...
int Sp_handler_package_spec::sp_find_and_drop_routine(THD *thd, TABLE *table, const Database_qualified_name *name) const
{
  int ret;
  if ((ret = db_find_routine_aux(thd, name, table)) != SP_OK)
    return ret;
...
bool Sp_handler::sp_create_routine(THD *thd, const sp_head *sp) const
{
  LEX *lex = thd->lex;
  bool ret = TRUE;
  TABLE *table;
  ...
  DBUG_PRINT("enter", ("type: %s  name: %.*s", type_str(), (int)sp->m_name.length, sp->m_name.str));
  ...
  */
  ...
  else
  {
    ...
    if (db_find_routine_aux(thd, sp, table) == SP_OK)
    {
      ...
      {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_SP_ALREADY_EXISTS,
                            ER_THD(thd, ER_SP_ALREADY_EXISTS), type_str(), sp->m_name.str);
    ...
    if (system_charset_info->numchars(sp->m_name.str, sp->m_name.str + sp->m_name.length) >
    ...
    store_failed = table->field[MYSQL_PROC_FIELD_DB]->store(sp->m_db, system_charset_info);

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_NAME]->store(sp->m_name, system_charset_info);

    if (sp->agg_type() != DEFAULT_AGGREGATE)
    {
      store_failed = store_failed || table->field[MYSQL_PROC_FIELD_AGGREGATE]->store((longlong)sp->agg_type(), TRUE);
    }

    store_failed = store_failed || table->field[MYSQL_PROC_MYSQL_TYPE]->store((longlong)type(), true);

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_SPECIFIC_NAME]->store(sp->m_name, system_charset_info);

    if (sp->daccess() != SP_DEFAULT_ACCESS)
    {
      store_failed = store_failed || table->field[MYSQL_PROC_FIELD_ACCESS]->store((longlong)sp->daccess(), TRUE);
    }

    store_failed =
        store_failed || table->field[MYSQL_PROC_FIELD_DETERMINISTIC]->store((longlong)(sp->detistic() ? 1 : 2), TRUE);

    if (sp->suid() != SP_IS_DEFAULT_SUID)
    {
      store_failed = store_failed || table->field[MYSQL_PROC_FIELD_SECURITY_TYPE]->store((longlong)sp->suid(), TRUE);
    }

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_PARAM_LIST]->store(sp->m_params, system_charset_info);
    ...
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_BODY]->store(sp->m_body, system_charset_info);

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_DEFINER]->store(definer, system_charset_info);

    table->field[MYSQL_PROC_FIELD_CREATED]->set_time();
    table->field[MYSQL_PROC_FIELD_MODIFIED]->set_time();

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_SQL_MODE]->store((longlong)org_sql_mode, TRUE);

    if (sp->comment().str)
    {
      store_failed = store_failed || table->field[MYSQL_PROC_FIELD_COMMENT]->store(sp->comment(), system_charset_info);
    }

    if (type() == SP_TYPE_FUNCTION && !trust_function_creators && mysql_bin_log.is_open())
    {
      if (!sp->detistic())
      {
        /*
        ...
        */
        enum enum_sp_data_access access =
            (sp->daccess() == SP_DEFAULT_ACCESS) ? SP_DEFAULT_ACCESS_MAPPING : sp->daccess();
    ...
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_CHARACTER_SET_CLIENT]->store(&thd->charset()->cs_name,
                                                                                              system_charset_info);

    table->field[MYSQL_PROC_FIELD_COLLATION_CONNECTION]->set_notnull();
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_COLLATION_CONNECTION]->store(
                                       &thd->variables.collation_connection->coll_name, system_charset_info);

    table->field[MYSQL_PROC_FIELD_DB_COLLATION]->set_notnull();
    store_failed =
        store_failed || table->field[MYSQL_PROC_FIELD_DB_COLLATION]->store(&db_cs->coll_name, system_charset_info);

    table->field[MYSQL_PROC_FIELD_BODY_UTF8]->set_notnull();
    store_failed =
        store_failed || table->field[MYSQL_PROC_FIELD_BODY_UTF8]->store(sp->m_body_utf8, system_charset_info);
...
log:
  if (mysql_bin_log.is_open())
  {
    ...
    if (show_create_sp(thd, &log_query, sp->m_explicit_name ? sp->m_db : null_clex_str, sp->m_name, sp->m_params,
                       returns, sp->m_body, sp->chistics(), thd->lex->definer[0], thd->lex->create_info, org_sql_mode))
    {
    ...
    if (thd->binlog_query(THD::STMT_QUERY_TYPE, log_query.ptr(), log_query.length(), FALSE, FALSE, FALSE, 0) > 0)
    {
...
static bool append_suid(String *buf, enum_sp_suid_behaviour suid)
{
  return suid == SP_IS_NOT_SUID && buf->append(STRING_WITH_LEN("    SQL SECURITY INVOKER\n"));
}

static bool append_comment(String *buf, const LEX_CSTRING &comment)
{
  if (!comment.length)
    return false;
...
static bool append_package_chistics(String *buf, const st_sp_chistics &chistics)
{
  return append_suid(buf, chistics.suid) || append_comment(buf, chistics.comment);
}

bool Sp_handler_package::show_create_sp(THD *thd, String *buf, const LEX_CSTRING &db, const LEX_CSTRING &name,
                                        const LEX_CSTRING &params, const LEX_CSTRING &returns, const LEX_CSTRING &body,
                                        const st_sp_chistics &chistics, const AUTHID &definer,
                                        const DDL_options_st ddl_options, sql_mode_t sql_mode) const
{
  Sql_mode_instant_set sms(thd, sql_mode);
  bool rc = buf->append(STRING_WITH_LEN("CREATE ")) ||
            (ddl_options.or_replace() && buf->append(STRING_WITH_LEN("OR REPLACE "))) ||
            append_definer(thd, buf, &definer.user, &definer.host) || buf->append(type_lex_cstring()) ||
            buf->append(' ') || (ddl_options.if_not_exists() && buf->append(STRING_WITH_LEN("IF NOT EXISTS "))) ||
            (db.length > 0 && (append_identifier(thd, buf, db.str, db.length) || buf->append('.'))) ||
            append_identifier(thd, buf, name.str, name.length) || append_package_chistics(buf, chistics) ||
            buf->append(' ') || buf->append(body.str, body.length);
  return rc;
}
...
int Sp_handler::sp_drop_routine(THD *thd, const Database_qualified_name *name) const
{
  TABLE *table;
  int ret;
...
int Sp_handler::sp_update_routine(THD *thd, const Database_qualified_name *name, const st_sp_chistics *chistics) const
{
  TABLE *table;
  int ret;
  ...
  if ((ret = db_find_routine_aux(thd, name, table)) == SP_OK)
  {
        ...
        (chistics->daccess == SP_CONTAINS_SQL || chistics->daccess == SP_MODIFIES_SQL_DATA))
    {
      ...
      ptr = get_field(thd->mem_root, table->field[MYSQL_PROC_FIELD_DETERMINISTIC]);
      if (ptr == NULL)
      {
        ret = SP_INTERNAL_ERROR;
        goto err;
      ...
      if (!is_deterministic)
      {
        my_message(ER_BINLOG_UNSAFE_ROUTINE, ER_THD(thd, ER_BINLOG_UNSAFE_ROUTINE), MYF(0));
        ret = SP_INTERNAL_ERROR;
        goto err;
      }
    }
    ...
    if (chistics->suid != SP_IS_DEFAULT_SUID)
      table->field[MYSQL_PROC_FIELD_SECURITY_TYPE]->store((longlong)chistics->suid, TRUE);
    if (chistics->daccess != SP_DEFAULT_ACCESS)
      table->field[MYSQL_PROC_FIELD_ACCESS]->store((longlong)chistics->daccess, TRUE);
    if (chistics->comment.str)
      table->field[MYSQL_PROC_FIELD_COMMENT]->store(chistics->comment, system_charset_info);
    if (chistics->agg_type != DEFAULT_AGGREGATE)
      table->field[MYSQL_PROC_FIELD_AGGREGATE]->store((longlong)chistics->agg_type, TRUE);
    if ((ret = table->file->ha_update_row(table->record[1], table->record[0])) && ret != HA_ERR_RECORD_IS_THE_SAME)
 ...
 public:
                        ...
                        const char *msg, Sql_condition **cond_hdl)
  {
    if (sql_errno == ER_NO_SUCH_TABLE || sql_errno == ER_NO_SUCH_TABLE_IN_ENGINE ||
        sql_errno == ER_CANNOT_LOAD_FROM_TABLE_V2 || sql_errno == ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE ||
...
bool lock_db_routines(THD *thd, const char *db)
{
  ...
  */
  ...
  if (!table->file->ha_index_read_map(table->record[0], keybuf, (key_part_map)1, HA_READ_KEY_EXACT))
  {
    do
    {
      char *sp_name = get_field(thd->mem_root, table->field[MYSQL_PROC_FIELD_NAME]);
      if (sp_name == NULL)  // skip invalid sp names (hand-edited mysql.proc?)
        continue;

      longlong sp_type = table->field[MYSQL_PROC_MYSQL_TYPE]->val_int();
      MDL_request *mdl_request = new (thd->mem_root) MDL_request;
      const Sp_handler *sph = Sp_handler::handler((enum_sp_type)sp_type);
      if (!sph)
        sph = &sp_handler_procedure;
      MDL_REQUEST_INIT(mdl_request, sph->get_mdl_type(), db, sp_name, MDL_EXCLUSIVE, MDL_TRANSACTION);
  ...
  DBUG_ASSERT(thd->mdl_context.is_lock_owner(MDL_key::BACKUP, "", "", MDL_BACKUP_DDL) &&
              thd->mdl_context.is_lock_owner(MDL_key::SCHEMA, db, "", MDL_EXCLUSIVE));
  DBUG_RETURN(thd->mdl_context.acquire_locks(&mdl_requests, thd->variables.lock_wait_timeout));
error:
  thd->commit_whole_transaction_and_close_tables();
  new_trans.restore_old_transaction();
  DBUG_RETURN(true);
...
bool Sp_handler::sp_show_create_routine(THD *thd, const Database_qualified_name *name) const
{
  ...
  */
  ...
  DBUG_EXECUTE_IF("cache_sp_in_show_create",
                  ...
                  sp->show_create_routine(thd, this); DBUG_RETURN(false););
...
sp_head *Sp_handler::sp_clone_and_link_routine(THD *thd, const Database_qualified_name *name, sp_head *sp) const
{
  ...
  uint parent_subroutine_count =
      !sp->m_parent ? 0
                    : sp->m_parent->m_routine_declarations.elements + sp->m_parent->m_routine_implementations.elements;
#endif
  ...
  */
  ...
  {
    DBUG_PRINT("info", ("first free: %p  level: %lu  flags %x", sp->m_first_free_instance,
                        sp->m_first_free_instance->m_recursion_level, sp->m_first_free_instance->m_flags));
  ...
  */
  ...
  rc = db_load_routine(thd, &lname, &new_sp, sp->m_sql_mode, sp->m_params, returns, sp->m_body, sp->chistics(),
                       sp->m_definer, sp->m_created, sp->m_modified, sp->m_parent, sp->get_creation_ctx());
  if (sp->m_parent)
  ...
  if (rc == SP_OK)
  {
    ...
    */
    DBUG_ASSERT(!sp->m_parent || parent_subroutine_count == sp->m_parent->m_routine_declarations.elements +
    ...
    DBUG_PRINT("info", ("added level: %p, level: %lu, flags %x", new_sp, new_sp->m_recursion_level, new_sp->m_flags));
    DBUG_RETURN(new_sp);
  }
  DBUG_RETURN(0);
}
...
sp_head *Sp_handler::sp_find_routine(THD *thd, const Database_qualified_name *name, bool cache_only) const
{
  DBUG_ENTER("Sp_handler::sp_find_routine");
  DBUG_PRINT("enter", ("name:  %.*s.%.*s  type: %s  cache only %d", (int)name->m_db.length, name->m_db.str,
                       (int)name->m_name.length, name->m_name.str, type_str(), cache_only));
...
sp_head *Sp_handler::sp_find_package_routine(THD *thd, const LEX_CSTRING pkgname_str,
                                             const Database_qualified_name *name, bool cache_only) const
{
...
sp_head *Sp_handler::sp_find_package_routine(THD *thd, const Database_qualified_name *name, bool cache_only) const
{
...
bool Sp_handler::sp_exist_routines(THD *thd, TABLE_LIST *routines) const
{
  TABLE_LIST *routine;
  ...
  for (routine = routines; routine; routine = routine->next_global)
  {
    ...
    thd->make_lex_string(&lex_name, routine->table_name.str, routine->table_name.length);
    name = new sp_name(&lex_db, &lex_name, true);
    sp_object_found = sp_find_routine(thd, name, false) != NULL;
    thd->get_stmt_da()->clear_warning_info(thd->query_id);
    if (!sp_object_found)
    {
      my_error(ER_SP_DOES_NOT_EXIST, MYF(0), "FUNCTION or PROCEDURE", routine->table_name.str);
      DBUG_RETURN(TRUE);
    }
  }
  DBUG_RETURN(FALSE);
...
extern "C" uchar *sp_sroutine_key(const uchar *ptr, size_t *plen, my_bool first)
{
...
bool sp_add_used_routine(Query_tables_list *prelocking_ctx, Query_arena *arena, const MDL_key *key,
                         const Sp_handler *handler, TABLE_LIST *belong_to_view)
{
  my_hash_init_opt(PSI_INSTRUMENT_ME, &prelocking_ctx->sroutines, system_charset_info,
                   Query_tables_list::START_SROUTINES_HASH_SIZE, 0, 0, sp_sroutine_key, 0, 0);

  if (!my_hash_search(&prelocking_ctx->sroutines, key->ptr(), key->length()))
  {
    Sroutine_hash_entry *rn = (Sroutine_hash_entry *)arena->alloc(sizeof(Sroutine_hash_entry));
...
int Sp_handler::sp_cache_routine_reentrant(THD *thd, const Database_qualified_name *name, sp_head **sp) const
{
  int ret;
...
static bool is_package_public_routine(THD *thd, const LEX_CSTRING &db, const LEX_CSTRING &package,
...
static bool is_package_public_routine_quick(THD *thd, const LEX_CSTRING &db, const LEX_CSTRING &pkgname,
                                            const LEX_CSTRING &name, enum_sp_type type)
{
...
static bool is_package_body_routine(THD *thd, sp_package *pkg, const LEX_CSTRING &name1, const LEX_CSTRING &name2,
                                    enum_sp_type type)
{
  return Sp_handler::eq_routine_name(pkg->m_name, name1) &&
         (pkg->m_routine_declarations.find(name2, type) || pkg->m_routine_implementations.find(name2, type));
}
...
bool Sp_handler::sp_resolve_package_routine_explicit(THD *thd, sp_head *caller, sp_name *name,
  ...
  */
  ...
  if (is_package_public_routine(thd, tmpdb, name->m_db, name->m_name, type()) ||
      ...
      (caller && caller->m_parent &&
       is_package_body_routine(thd, caller->m_parent, name->m_db, name->m_name, type())) ||
      // Check if a package initialization sections calls a private routine
      (caller && (pkg = caller->get_package()) && is_package_body_routine(thd, pkg, name->m_db, name->m_name, type())))
  {
    pkgname->m_db = tmpdb;
    pkgname->m_name = name->m_db;
    *pkg_routine_handler = package_routine_handler();
    return name->make_package_routine_name(thd->mem_root, tmpdb, name->m_db, name->m_name);
  }
  return false;
}
...
bool Sp_handler::sp_resolve_package_routine_implicit(THD *thd, sp_head *caller, sp_name *name,
                                                     const Sp_handler **pkg_routine_handler,
                                                     Database_qualified_name *pkgname) const
{
  sp_package *pkg;
  ...
  if (caller->m_parent)
  {
    ...
    */
    if (eq_routine_name(tmpname, name->m_name) ||
        ...
        is_package_public_routine_quick(thd, caller->m_db, pkgstr, name->m_name, type()))
    {
  ...
  if ((pkg = caller->get_package()) && pkg->m_routine_implementations.find(name->m_name, type()))
  {
    pkgname->m_db = caller->m_db;
    pkgname->m_name = caller->m_name;
    // Package initialization section is calling a non-qualified routine
    *pkg_routine_handler = package_routine_handler();
    return name->make_package_routine_name(thd->mem_root, caller->m_name, name->m_name);
  }
...
bool Sp_handler::sp_resolve_package_routine(THD *thd, sp_head *caller, sp_name *name,
                                            const Sp_handler **pkg_routine_handler,
                                            Database_qualified_name *pkgname) const
{
  ...
  return name->m_explicit_name ? sp_resolve_package_routine_explicit(thd, caller, name, pkg_routine_handler, pkgname)
                               : sp_resolve_package_routine_implicit(thd, caller, name, pkg_routine_handler, pkgname);
}
...
void Sp_handler::add_used_routine(Query_tables_list *prelocking_ctx, Query_arena *arena,
                                  const Database_qualified_name *rt) const
{
  ...
  prelocking_ctx->sroutines_list_own_elements = prelocking_ctx->sroutines_list.elements;
}
...
void sp_remove_not_own_routines(Query_tables_list *prelocking_ctx)
{
  ...
  for (not_own_rt = *prelocking_ctx->sroutines_list_own_last; not_own_rt; not_own_rt = next_rt)
  {
    /*
  ...
  *prelocking_ctx->sroutines_list_own_last = NULL;
  ...
  prelocking_ctx->sroutines_list.elements = prelocking_ctx->sroutines_list_own_elements;
}
...
bool sp_update_sp_used_routines(HASH *dst, HASH *src)
{
  for (uint i = 0; i < src->records; i++)
  {
    ...
    if (!my_hash_search(dst, (uchar *)rt->mdl_request.key.ptr(), rt->mdl_request.key.length()))
    {
...
void sp_update_stmt_used_routines(THD *thd, Query_tables_list *prelocking_ctx, HASH *src, TABLE_LIST *belong_to_view)
{
  for (uint i = 0; i < src->records; i++)
  {
    Sroutine_hash_entry *rt = (Sroutine_hash_entry *)my_hash_element(src, i);
    (void)sp_add_used_routine(prelocking_ctx, thd->stmt_arena, &rt->mdl_request.key, rt->m_handler, belong_to_view);
  }
}
...
void sp_update_stmt_used_routines(THD *thd, Query_tables_list *prelocking_ctx, SQL_I_List<Sroutine_hash_entry> *src,
...
int Sroutine_hash_entry::sp_cache_routine(THD *thd, bool lookup_only, sp_head **sp) const
{
...
int Sp_handler::sp_cache_routine(THD *thd, const Database_qualified_name *name, bool lookup_only, sp_head **sp) const
{
  int ret = 0;
  sp_cache **spc = get_cache(thd);
  ...
  switch ((ret = db_find_and_cache_routine(thd, name, sp)))
  {
    ...
    default:
      ...
      */
      if (!thd->is_error())
      {
        my_error(ER_SP_PROC_TABLE_CORRUPT, MYF(0), ErrConvDQName(name).ptr(), ret);
      }
      break;
  }
  DBUG_RETURN(ret);
...
int Sp_handler::sp_cache_package_routine(THD *thd, const LEX_CSTRING &pkgname_cstr, const Database_qualified_name *name,
                                         bool lookup_only, sp_head **sp) const
{
  ...
  sp_head *ph = NULL;
  int ret = sp_handler_package_body.sp_cache_routine(thd, &pkgname, lookup_only, &ph);
  if (!ret)
  {
...
int Sp_handler::sp_cache_package_routine(THD *thd, const Database_qualified_name *name, bool lookup_only,
                                         sp_head **sp) const
{
...
bool Sp_handler::show_create_sp(THD *thd, String *buf, const LEX_CSTRING &db, const LEX_CSTRING &name,
                                const LEX_CSTRING &params, const LEX_CSTRING &returns, const LEX_CSTRING &body,
                                const st_sp_chistics &chistics, const AUTHID &definer, const DDL_options_st ddl_options,
                                sql_mode_t sql_mode) const
{
  ...
  if (buf->alloc(100 + db.length + 1 + name.length + params.length + returns.length + chistics.comment.length +
                 10 /* length of " DEFINER= "*/ + agglen + USER_HOST_BUFF_SIZE))
    return true;
...
sp_head *Sp_handler::sp_load_for_information_schema(THD *thd, TABLE *proc_table, const LEX_CSTRING &db,
                                                    ...
                                                    bool *free_sp_head) const
{
  ...
  *free_sp_head = 0;
  ...
  LEX *old_lex = thd->lex, newlex;
  Stored_program_creation_ctx *creation_ctx = Stored_routine_creation_ctx::load_from_db(thd, &sp_name_obj, proc_table);
  defstr.set_charset(creation_ctx->get_client_cs());
  if (show_create_sp(thd, &defstr, sp_name_obj.m_db, sp_name_obj.m_name, params, returns,
                     empty_body_lex_cstring(sql_mode), Sp_chistics(), definer, DDL_options(), sql_mode))
    return 0;
