...
static Field_definition sequence_structure[] = {
    {"next_not_cached_value", 21, &type_handler_slonglong, {STRING_WITH_LEN("")}, FL},
    ...
    {"start_value",
     ...
     FL},
    {"increment", 21, &type_handler_slonglong, {STRING_WITH_LEN("increment value")}, FL},
    {"cache_size", 21, &type_handler_ulonglong, {STRING_WITH_LEN("")}, FL | UNSIGNED_FLAG},
    {"cycle_option",
     1,
     &type_handler_utiny,
     {STRING_WITH_LEN(
     ...
     FL | UNSIGNED_FLAG},
    {"cycle_count", 21, &type_handler_slonglong, {STRING_WITH_LEN("How many cycles have been done")}, FL},
...
bool sequence_definition::check_and_adjust(bool set_reserved_until)
{
  ...
  */
  ...
  max_increment = (real_increment ? llabs(real_increment) : MAX_AUTO_INCREMENT_VALUE);

  if (max_value >= start && max_value > min_value && start >= min_value && max_value != LONGLONG_MAX &&
      min_value != LONGLONG_MIN && cache >= 0 && cache < (LONGLONG_MAX - max_increment) / max_increment &&
      ((real_increment > 0 && reserved_until >= min_value) || (real_increment < 0 && reserved_until <= max_value)))
    DBUG_RETURN(FALSE);
...
bool check_sequence_fields(LEX *lex, List<Create_field> *fields)
{
  ...
  for (field_no = 0; (field = it++); field_no++)
  {
    ...
    if (my_strcasecmp(system_charset_info, field_def->field_name, field->field_name.str) ||
        field->flags != field_def->flags || field->type_handler() != field_def->type_handler ||
...
err:
  my_error(ER_SEQUENCE_INVALID_TABLE_STRUCTURE, MYF(0), lex->first_select_lex()->table_list.first->db.str,
...
bool prepare_sequence_fields(THD *thd, List<Create_field> *fields)
{
  ...
  for (field_info = sequence_structure; field_info->field_name; field_info++)
  {
    ...
    LEX_CSTRING field_name = {field_info->field_name, strlen(field_info->field_name)};
  ...
  */
  ...
  if (!temporary_table)
  {
    ...
    */
    ...
    table_list.init_one_table(&org_table_list->db, &org_table_list->table_name, NULL, TL_WRITE_DEFAULT);
    table_list.updating = 1;
    table_list.open_strategy = TABLE_LIST::OPEN_IF_EXISTS;
    table_list.open_type = OT_BASE_ONLY;

    DBUG_ASSERT(!thd->locked_tables_mode || (thd->variables.option_bits & OPTION_TABLE_LOCK));
    ...
    */
    ...
    error = open_and_lock_tables(thd, &table_list, FALSE, MYSQL_LOCK_IGNORE_TIMEOUT | MYSQL_OPEN_HAS_MDL_LOCK);
    thd->open_options &= ~HA_OPEN_FOR_CREATE;
  ...
  {
    uint save_unsafe_rollback_flags = thd->transaction->stmt.m_unsafe_rollback_flags;
...
SEQUENCE::~SEQUENCE() { mysql_rwlock_destroy(&mutex); }
    ...
    */
    if (table->mdl_ticket == 0)
    {
      ...
      */
      ...
      MDL_REQUEST_INIT(&mdl_request, MDL_key::TABLE, table->s->db.str, table->s->table_name.str, MDL_SHARED_READ,
                       MDL_EXPLICIT);
    ...
    if (!(lock = mysql_lock_tables(thd, &table, 1, MYSQL_LOCK_IGNORE_GLOBAL_READ_ONLY)))
    {
      if (mdl_lock_used)
...
void sequence_definition::adjust_values(longlong next_value)
{
  ...
  if (!(real_increment = increment))
  {
    ...
    if ((real_increment = global_system_variables.auto_increment_increment) != 1)
      offset = (global_system_variables.auto_increment_offset % global_system_variables.auto_increment_increment);
    ...
    */
    ...
    /*
      ...
      may overflow
    */
    if (next_free_value > max_value - to_add || next_free_value + to_add > max_value)
...
longlong SEQUENCE::next_value(TABLE *table, bool second_round, int *error)
{
  ...
  *error = 0;
  ...
  if ((real_increment > 0 && res_value < reserved_until) || (real_increment < 0 && res_value > reserved_until))
  {
    write_unlock(table);
  ...
  */
  ...
  if (real_increment > 0)
  {
    if (reserved_until > max_value - add_to || reserved_until + add_to > max_value)
    {
  ...
  {
    if (reserved_until + add_to < min_value || reserved_until < min_value - add_to)
    {
  ...
  if (unlikely((*error = write(table, thd->variables.binlog_row_image != BINLOG_ROW_IMAGE_MINIMAL))))
  {
...
err:
  write_unlock(table);
  my_error(ER_SEQUENCE_RUN_OUT, MYF(0), table->s->db.str, table->s->table_name.str);
...
bool SEQUENCE_LAST_VALUE::check_version(TABLE *table)
{
  ...
  return memcmp(table->s->tabledef_version.str, table_version, MY_UUID_SIZE) != 0;
}
...
int SEQUENCE::set_value(TABLE *table, longlong next_val, ulonglong next_round, bool is_used)
{
  int error = -1;
  ...
  if (round == next_round)
  {
    if (real_increment > 0 ? next_val < next_free_value : next_val > next_free_value)
      goto end;  // error = -1
  ...
  if ((real_increment > 0 ? next_free_value > reserved_until : next_free_value < reserved_until) || needs_to_be_stored)
  {
    reserved_until = next_free_value;
    if (write(table, thd->variables.binlog_row_image != BINLOG_ROW_IMAGE_MINIMAL))
    {
      reserved_until = org_reserved_until;
...
bool Sql_cmd_alter_sequence::execute(THD *thd)
{
  ...
  if (check_access(thd, ALTER_ACL, first_table->db.str, &first_table->grant.privilege, &first_table->grant.m_internal,
                   0, 0))
  ...
  if (WSREP_ON && WSREP(thd) &&
      wsrep_to_isolation_begin(thd, first_table->db.str, first_table->table_name.str, first_table))
    DBUG_RETURN(TRUE);
#endif /* WITH_WSREP */
  if (if_exists())
  ...
  if (unlikely(error))
  {
    if (trapped_errors)
    {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_UNKNOWN_SEQUENCES, ER_THD(thd, ER_UNKNOWN_SEQUENCES),
                          tbl_name.c_ptr_safe());
      my_ok(thd);
      DBUG_RETURN(FALSE);
    }
  ...
  {
    my_error(ER_SEQUENCE_INVALID_DATA, MYF(0), first_table->db.str, first_table->table_name.str);
    error = 1;
