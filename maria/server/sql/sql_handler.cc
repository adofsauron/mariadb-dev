...
static char *mysql_ha_hash_get_key(SQL_HANDLER *table, size_t *key_len, my_bool first __attribute__((unused)))
{
...
static void mysql_ha_hash_free(SQL_HANDLER *table) { delete table; }

static void mysql_ha_close_childs(THD *thd, TABLE_LIST *current_table_list, TABLE_LIST **next_global)
{
  ...
  for (table_list = *next_global; table_list; table_list = *next_global)
  {
    *next_global = table_list->next_global;
    DBUG_PRINT("info", ("table_name: %s.%s", table_list->table->s->db.str, table_list->table->s->table_name.str));
...
bool mysql_ha_open(THD *thd, TABLE_LIST *tables, SQL_HANDLER *reopen)
{
  ...
  DBUG_PRINT("enter",
             ("'%s'.'%s' as '%s'  reopen: %d", tables->db.str, tables->table_name.str, tables->alias.str, reopen != 0));
  ...
  {
    my_error(ER_WRONG_USAGE, MYF(0), "HANDLER OPEN", INFORMATION_SCHEMA_NAME.str);
  ...
  if (!my_hash_inited(&thd->handler_tables_hash))
  {
    ...
    */
    if (my_hash_init(key_memory_THD_handler_tables_hash, &thd->handler_tables_hash, &my_charset_latin1,
  ...
  {
    if (my_hash_search(&thd->handler_tables_hash, (uchar *)tables->alias.str, tables->alias.length + 1))
    {
  ...
  */
  MDL_REQUEST_INIT(&tables->mdl_request, MDL_key::TABLE, tables->db.str, tables->table_name.str, MDL_SHARED_READ,
                   MDL_TRANSACTION);
  ...
  */
  error = (thd->open_temporary_tables(tables) || open_tables(thd, &tables, &counter, 0));
  ...
  {
    my_error(ER_ILLEGAL_HA, MYF(0), table->file->table_type(), table->s->db.str, table->s->table_name.str);
    goto err;
  }

  DBUG_PRINT("info", ("clone_tickets start"));
  for (TABLE_LIST *table_list = tables; table_list; table_list = table_list->next_global)
  {
    DBUG_PRINT("info", ("table_list %s.%s", table_list->table->s->db.str, table_list->table->s->table_name.str));
    if (table_list->mdl_request.ticket && thd->mdl_context.has_lock(mdl_savepoint, table_list->mdl_request.ticket))
    {
  ...
  if (!reopen)
  {
    ...
    if (!(sql_handler = new SQL_HANDLER(thd)))
      goto err;
    init_alloc_root(PSI_INSTRUMENT_ME, &sql_handler->mem_root, 1024, 0, MYF(MY_THREAD_SPECIFIC));
    ...
    if (!(my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &sql_handler->base_data, (uint)sql_handler->db.length + 1,
                          &sql_handler->table_name.str, (uint)sql_handler->table_name.length + 1,
                          &sql_handler->handler_name.str, (uint)sql_handler->handler_name.length + 1, NullS)))
      goto err;
    sql_handler->db.str = sql_handler->base_data;
    memcpy((char *)sql_handler->db.str, tables->db.str, tables->db.length + 1);
    memcpy((char *)sql_handler->table_name.str, tables->table_name.str, tables->table_name.length + 1);
    memcpy((char *)sql_handler->handler_name.str, tables->alias.str, tables->alias.length + 1);
  ...
  if (!(sql_handler->lock = get_lock_data(thd, &sql_handler->table, 1, GET_LOCK_STORE_LOCKS)))
    goto err;
  ...
  {
    thd->mdl_context.set_lock_duration(sql_handler->mdl_request.ticket, MDL_EXPLICIT);
    thd->mdl_context.set_needs_thr_lock_abort(TRUE);
  }
  for (TABLE_LIST *table_list = tables->next_global; table_list; table_list = table_list->next_global)
  {
    DBUG_PRINT("info", ("table_list %s.%s", table_list->table->s->db.str, table_list->table->s->table_name.str));
    if (table_list->mdl_request.ticket)
    {
      thd->mdl_context.set_lock_duration(table_list->mdl_request.ticket, MDL_EXPLICIT);
      thd->mdl_context.set_needs_thr_lock_abort(TRUE);
    }
  }
...
bool mysql_ha_close(THD *thd, TABLE_LIST *tables)
{
  ...
  DBUG_PRINT("enter", ("'%s'.'%s' as '%s'", tables->db.str, tables->table_name.str, tables->alias.str));
  ...
  if ((my_hash_inited(&thd->handler_tables_hash)) &&
      (handler = (SQL_HANDLER *)my_hash_search(&thd->handler_tables_hash, (const uchar *)tables->alias.str,
...
static SQL_HANDLER *mysql_ha_find_handler(THD *thd, const LEX_CSTRING *name)
{
  SQL_HANDLER *handler;
  if ((my_hash_inited(&thd->handler_tables_hash)) &&
      (handler = (SQL_HANDLER *)my_hash_search(&thd->handler_tables_hash, (const uchar *)name->str, name->length + 1)))
  {
    DBUG_PRINT("info-in-hash", ("'%s'.'%s' as '%s' table: %p", handler->db.str, handler->table_name.str,
                                handler->handler_name.str, handler->table));
    if (!handler->table)
    {
      /* The handler table has been closed. Re-open it. */
      TABLE_LIST tmp;
      tmp.init_one_table(&handler->db, &handler->table_name, &handler->handler_name, TL_READ);
...
static bool mysql_ha_fix_cond_and_key(SQL_HANDLER *handler, enum enum_ha_read_modes mode, const char *keyname,
                                      ...
                                      bool in_prepare)
{
  ...
  if (cond)
  {
    ...
    if (ret || res.errors)
    {
      my_error(ER_GENERATED_COLUMN_FUNCTION_IS_NOT_ALLOWED, MYF(0), res.name, "WHERE", "HANDLER");
  ...
  if (keyname)
  {
    ...
    if (handler->keyno < 0 || my_strcasecmp(&my_charset_latin1, keyname, table->s->key_info[handler->keyno].name.str))
    {
      if ((handler->keyno = find_type(keyname, &table->s->keynames, FIND_TYPE_NO_PREFIX) - 1) < 0)
      {
        my_error(ER_KEY_DOES_NOT_EXISTS, MYF(0), keyname, handler->handler_name.str);
        return 1;
      }
    }
    ...
    if (mode == RKEY)
    {
      ...
      if ((c_key->flags & HA_SPATIAL) || c_key->algorithm == HA_KEY_ALG_FULLTEXT ||
          (ha_rkey_mode != HA_READ_KEY_EXACT &&
           (table->file->index_flags(handler->keyno, 0, TRUE) & (HA_READ_NEXT | HA_READ_PREV | HA_READ_RANGE)) == 0))
      {
        my_error(ER_KEY_DOESNT_SUPPORT, MYF(0), table->file->index_type(handler->keyno), keyinfo->name.str);
        return 1;
      }
      ...
      if (key_expr->elements < keyinfo->user_defined_key_parts &&
          (table->file->index_flags(handler->keyno, 0, TRUE) & HA_ONLY_WHOLE_INDEX))
      {
        my_error(ER_KEY_DOESNT_SUPPORT, MYF(0), table->file->index_type(handler->keyno), keyinfo->name.str);
        return 1;
      }
...
bool mysql_ha_read(THD *thd, TABLE_LIST *tables, enum enum_ha_read_modes mode, const char *keyname,
                   List<Item> *key_expr, enum ha_rkey_function ha_rkey_mode, Item *cond, ha_rows select_limit_cnt,
...
retry:
  ...
  if (handler->lock->table_count > 0)
  {
    ...
    for (pos = handler->lock->locks, end = handler->lock->locks + handler->lock->lock_count; pos < end; pos++)
    {
    ...
    */
    ...
    lock_error =
        mysql_lock_tables(thd, handler->lock, (table->s->tmp_table == NO_TMP_TABLE ? MYSQL_LOCK_NOT_TEMPORARY : 0));
  ...
  if (mysql_ha_fix_cond_and_key(handler, mode, keyname, key_expr, ha_rkey_mode, cond, 0))
    goto err;
  mode = handler->mode;
  keyno = handler->keyno;

  protocol->send_result_set_metadata(&handler->fields, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF);
  ...
  for (num_rows = 0; num_rows < select_limit_cnt;)
  {
    switch (mode)
    {
      ...
      case RFIRST:
        ...
        {
          if (likely(!(error = table->file->ha_index_or_rnd_end())) && likely(!(error = table->file->ha_rnd_init(1))))
            error = table->file->ha_rnd_next(table->record[0]);
        }
        mode = RNEXT;
        break;
      ...
      case RNEXT_SAME:
        ...
        DBUG_ASSERT(keyname != 0);
        error = table->file->ha_index_next_same(table->record[0], key, handler->key_len);
        break;
      case RKEY:
      {
        DBUG_ASSERT(keyname != 0);
        ...
        if (unlikely((error = table->file->ha_index_or_rnd_end())))
          break;
        key_copy(key, table->record[0], table->key_info + keyno, handler->key_len);
        if (unlikely(!(error = table->file->ha_index_init(keyno, 1))))
          error = table->file->ha_index_read_map(table->record[0], key, handler->keypart_map, ha_rkey_mode);
    ...
    if (unlikely(error))
    {
      if (error != HA_ERR_KEY_NOT_FOUND && error != HA_ERR_END_OF_FILE)
      {
        ...
        if (error != HA_ERR_RECORD_CHANGED && error != HA_ERR_WRONG_COMMAND)
          sql_print_error(
              "mysql_ha_read: Got error %d when reading "
              "table '%s'",
                                   ...
                                   List<Item> *key_expr, enum ha_rkey_function ha_rkey_mode, Item *cond)
{
  ...
  if (mysql_ha_fix_cond_and_key(handler, mode, keyname, key_expr, ha_rkey_mode, cond, 1))
    DBUG_RETURN(0);
  DBUG_RETURN(handler);
}
  ...
  for (uint i = 0; i < thd->handler_tables_hash.records; i++)
  {
    ...
    for (tables = first; tables; tables = tables->next_local)
    {
      if (tables->is_anonymous_derived_table())
        continue;
      if ((!tables->db.str[0] || !my_strcasecmp(&my_charset_latin1, hash_tables->db.str, tables->get_db_name())) &&
          !my_strcasecmp(&my_charset_latin1, hash_tables->table_name.str, tables->get_table_name()))
      {
...
void mysql_ha_flush(THD *thd)
{
  ...
  */
  ...
  for (uint i = 0; i < thd->handler_tables_hash.records; i++)
  {
    ...
    */
    if (hash_tables->table &&
        ((hash_tables->table->mdl_ticket && hash_tables->table->mdl_ticket->has_pending_conflicting_lock()) ||
         (!hash_tables->table->s->tmp_table && hash_tables->table->s->tdc->flushed)))
      mysql_ha_close_table(hash_tables);
  }
...
void mysql_ha_set_explicit_lock_duration(THD *thd)
{
  ...
  for (uint i = 0; i < thd->handler_tables_hash.records; i++)
  {
    ...
    if (hash_tables->table && hash_tables->table->mdl_ticket)
      thd->mdl_context.set_lock_duration(hash_tables->table->mdl_ticket, MDL_EXPLICIT);
  }
  DBUG_VOID_RETURN;
}
...
void mysql_ha_rm_temporary_tables(THD *thd)
{
  ...
  for (uint i = 0; i < thd->handler_tables_hash.records; i++)
  {
    TABLE_LIST *handler_table = reinterpret_cast<TABLE_LIST *>(my_hash_element(&thd->handler_tables_hash, i));
