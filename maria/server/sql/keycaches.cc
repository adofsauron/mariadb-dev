...
class NAMED_ILINK : public ilink
{
 public:
  ...
  NAMED_ILINK(I_List<NAMED_ILINK> *links, const char *name_arg, size_t name_length_arg, uchar *data_arg)
      : name_length(name_length_arg), data(data_arg)
  {
    name = my_strndup(key_memory_NAMED_ILINK_name, name_arg, name_length, MYF(MY_WME));
...
uchar *find_named(I_List<NAMED_ILINK> *list, const char *name, size_t length, NAMED_ILINK **found)
{
...
KEY_CACHE *create_key_cache(const char *name, size_t length)
{
  ...
  if ((key_cache = (KEY_CACHE *)my_malloc(key_memory_KEY_CACHE, sizeof(KEY_CACHE), MYF(MY_ZEROFILL | MY_WME))))
  {
...
Rpl_filter *get_rpl_filter(LEX_CSTRING *filter_name)
{
  ...
  return ((Rpl_filter *)find_named(&rpl_filters, filter_name->str, filter_name->length, 0));
}
...
void free_rpl_filter(const char *name, void *filter) { delete static_cast<Rpl_filter *>(filter); }

void free_all_rpl_filters() { rpl_filters.delete_elements(free_rpl_filter); }
