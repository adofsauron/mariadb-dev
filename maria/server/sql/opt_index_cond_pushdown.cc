...
bool uses_index_fields_only(Item *item, TABLE *tbl, uint keyno, bool other_tbls_ok)
{
  ...
  */
  if (item->type() == Item::FUNC_ITEM && ((Item_func *)item)->functype() == Item_func::TRIG_COND_FUNC)
    return FALSE;
  ...
  switch (item_type)
  {
    ...
    case Item::FIELD_ITEM:
    {
      ...
      */
      if (!field->part_of_key.is_set(keyno) || field->type() == MYSQL_TYPE_GEOMETRY || field->type() == MYSQL_TYPE_BLOB)
        return FALSE;
      ...
      if ((tbl->file->ha_table_flags() & HA_PRIMARY_KEY_IN_READ_INDEX) && tbl->s->primary_key != MAX_KEY &&
...
static Item *make_cond_for_index(THD *thd, Item *cond, TABLE *table, uint keyno, bool other_tbls_ok)
{
...
static Item *make_cond_remainder(THD *thd, Item *cond, TABLE *table, uint keyno, bool other_tbls_ok, bool exclude_index)
{
  if (exclude_index && uses_index_fields_only(cond, table, keyno, other_tbls_ok))
    return 0;
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    ...
    if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC)
    {
      ...
      while ((item = li++))
      {
        Item *fix = make_cond_remainder(thd, item, table, keyno, other_tbls_ok, exclude_index);
        if (fix)
        {
          new_cond->argument_list()->push_back(fix, thd->mem_root);
    ...
    else /* It's OR */
    {
      ...
      while ((item = li++))
      {
        Item *fix = make_cond_remainder(thd, item, table, keyno, other_tbls_ok, FALSE);
        if (!fix)
          return (COND *)0;
        new_cond->argument_list()->push_back(fix, thd->mem_root);
        tbl_map |= fix->used_tables();
...
void push_index_cond(JOIN_TAB *tab, uint keyno)
{
  ...
  */
  if ((tab->table->file->index_flags(keyno, 0, 1) & HA_DO_INDEX_COND_PUSHDOWN) &&
      ...
      tab->join->thd->lex->sql_command != SQLCOM_DELETE_MULTI && tab->type != JT_CONST && tab->type != JT_SYSTEM &&
      !tab->table->file->is_clustering_key(keyno))  // 6
  {
    DBUG_EXECUTE("where", print_where(tab->select_cond, "full cond", QT_ORDINARY););

    idx_cond = make_cond_for_index(tab->join->thd, tab->select_cond, tab->table, keyno, tab->icp_other_tables_ok);
    ...
    if (idx_cond)
    {
      ...
      */
      if (tab->use_join_cache &&
          ...
          */
          tab->icp_other_tables_ok && (idx_cond->used_tables() & ~(tab->table->map | tab->join->const_table_map)))
      ...
      */
      ...
      Item *row_cond = tab->idx_cond_fact_out ? make_cond_remainder(tab->join->thd, tab->select_cond, tab->table, keyno,
                                                                    tab->icp_other_tables_ok, TRUE)
                                              : tab->pre_idx_push_select_cond;

      DBUG_EXECUTE("where", print_where(row_cond, "remainder cond", QT_ORDINARY););
      ...
      if (row_cond)
      {
        ...
        {
          COND *new_cond = new (tab->join->thd->mem_root) Item_cond_and(tab->join->thd, row_cond, idx_remainder_cond);
      ...
      if (tab->select)
      {
        DBUG_EXECUTE("where", print_where(tab->select->cond, "select_cond", QT_ORDINARY););
