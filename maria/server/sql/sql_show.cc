...
static const LEX_CSTRING trg_action_time_type_names[] = {{STRING_WITH_LEN("BEFORE")}, {STRING_WITH_LEN("AFTER")}};

static const LEX_CSTRING trg_event_type_names[] = {
    {STRING_WITH_LEN("INSERT")}, {STRING_WITH_LEN("UPDATE")}, {STRING_WITH_LEN("DELETE")}};
...
static const LEX_CSTRING ha_choice_values[] = {{STRING_WITH_LEN("")}, {STRING_WITH_LEN("0")}, {STRING_WITH_LEN("1")}};

static void store_key_options(THD *, String *, TABLE *, KEY *);

static int show_create_view(THD *thd, TABLE_LIST *table, String *buff);
static int show_create_sequence(THD *thd, TABLE_LIST *table_list, String *packet);
...
static bool is_show_command(THD *thd) { return sql_command_flags[thd->lex->sql_command] & CF_STATUS_COMMAND; }
...
static const LEX_CSTRING maturity_name[] = {{STRING_WITH_LEN("Unknown")}, {STRING_WITH_LEN("Experimental")},
...
static my_bool show_plugins(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  table->field[0]->store(plugin_name(plugin)->str, plugin_name(plugin)->length, cs);

  table->field[1]->store(version_buf, make_version_string(version_buf, sizeof(version_buf), plug->version), cs);
  ...
  table->field[3]->store(plugin_type_names[plug->type].str, plugin_type_names[plug->type].length, cs);
  table->field[4]->store(version_buf, make_version_string(version_buf, sizeof(version_buf), *(uint *)plug->info), cs);

  if (plugin_dl)
  {
    table->field[5]->store(plugin_dl->dl.str, plugin_dl->dl.length, cs);
    table->field[5]->set_notnull();
    table->field[6]->store(version_buf, make_version_string(version_buf, sizeof(version_buf), plugin_dl->mariaversion),
                           cs);
  ...
  switch (plug->license)
  {
    case PLUGIN_LICENSE_GPL:
      table->field[9]->store(PLUGIN_LICENSE_GPL_STRING, strlen(PLUGIN_LICENSE_GPL_STRING), cs);
      break;
    case PLUGIN_LICENSE_BSD:
      table->field[9]->store(PLUGIN_LICENSE_BSD_STRING, strlen(PLUGIN_LICENSE_BSD_STRING), cs);
      break;
    default:
      table->field[9]->store(PLUGIN_LICENSE_PROPRIETARY_STRING, strlen(PLUGIN_LICENSE_PROPRIETARY_STRING), cs);
      break;
  }

  table->field[10]->store(global_plugin_typelib_names[plugin_load_option(plugin)],
                          strlen(global_plugin_typelib_names[plugin_load_option(plugin)]), cs);

  if (plug->maturity <= MariaDB_PLUGIN_MATURITY_STABLE)
    table->field[11]->store(maturity_name[plug->maturity].str, maturity_name[plug->maturity].length, cs);
  else
    table->field[11]->store("Unknown", 7, cs);

  if (plug->version_info)
  {
    table->field[12]->store(plug->version_info, strlen(plug->version_info), cs);
...
int fill_plugins(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  if (plugin_foreach_with_mask(thd, show_plugins, MYSQL_ANY_PLUGIN, ~PLUGIN_IS_FREED, table))
    DBUG_RETURN(1);
...
int fill_all_plugins(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (size_t i = 0; i < dirp->number_of_files; i++)
  {
    ...
    if (lookup.db_value.str)
    {
      ...
      {
        if (files_charset_info->wildcmp(dl.str, dlend, wstr, wend, wild_prefix, wild_one, wild_many))
          continue;
      }
      else
      {
        if (files_charset_info->strnncoll(dl.str, dl.length, lookup.db_value.str, lookup.db_value.length))
          continue;
      }
    }
...
bool mysqld_show_authors(THD *thd)
{
  ...
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Name", 40), mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Location", 40), mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Comment", 512), mem_root);
  ...
  for (authors = show_table_authors; authors->name; authors++)
  {
    ...
    protocol->store(authors->location, strlen(authors->location), system_charset_info);
    protocol->store(authors->comment, strlen(authors->comment), system_charset_info);
    if (protocol->write())
      DBUG_RETURN(TRUE);
  }
  my_eof(thd);
...
bool mysqld_show_contributors(THD *thd)
{
  ...
  {
    protocol->prepare_for_resend();
    protocol->store(contributors->name, strlen(contributors->name), system_charset_info);
    protocol->store(contributors->location, strlen(contributors->location), system_charset_info);
    protocol->store(contributors->comment, strlen(contributors->comment), system_charset_info);
    if (protocol->write())
      DBUG_RETURN(TRUE);
  }
  my_eof(thd);
...
bool mysqld_show_privileges(THD *thd)
{
  ...
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Privilege", 10), mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Context", 15), mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Comment", NAME_CHAR_LEN), mem_root);
  ...
  {
    protocol->prepare_for_resend();
    protocol->store(privilege->privilege, strlen(privilege->privilege), system_charset_info);
    protocol->store(privilege->context, strlen(privilege->context), system_charset_info);
    protocol->store(privilege->comment, strlen(privilege->comment), system_charset_info);
    if (protocol->write())
      DBUG_RETURN(TRUE);
  }
  my_eof(thd);
...
{
  return my_init_dynamic_array(key_memory_ignored_db, &ignore_db_dirs_array, sizeof(LEX_STRING *), 0, 0, MYF(0));
}
...
static uchar *db_dirs_hash_get_key(const uchar *data, size_t *len_ret, my_bool __attribute__((unused)))
{
...
bool push_ignored_db_dir(const char *path)
{
  ...
  if (!my_multi_malloc(key_memory_ignored_db, MYF(0), &new_elt, sizeof(LEX_STRING), &new_elt_buffer, path_len + 1,
                       NullS))
    return true;
...
static void dispose_db_dir(void *ptr) { my_free(ptr); }
...
void ignore_db_dirs_append(const char *dirname_arg)
{
  ...
  if (!my_multi_malloc(PSI_INSTRUMENT_ME, MYF(0), &new_entry, sizeof(LEX_STRING), &new_entry_buf, len + 1, NullS))
    return;
...
bool ignore_db_dirs_process_additions()
{
  ...
  if (my_hash_init(key_memory_ignored_db, &ignore_db_dirs_hash,
                   lower_case_table_names ? character_set_filesystem : &my_charset_bin, 0, 0, 0, db_dirs_hash_get_key,
  ...
  ptr = opt_ignore_db_dirs = (char *)my_malloc(key_memory_ignored_db, len + 1, MYF(0));
  if (!ptr)
    return true;
...
static inline bool is_in_ignore_db_dirs_list(const char *directory)
{
  return ignore_db_dirs_hash.records &&
         NULL != my_hash_search(&ignore_db_dirs_hash, (const uchar *)directory, strlen(directory));
}
...
bool db_name_is_in_ignore_db_dirs_list(const char *directory)
{
  char buff[FN_REFLEN];
  uint buff_len;
...
static find_files_result find_files(THD *thd, Dynamic_array<LEX_CSTRING *> *files, LEX_CSTRING *db, const char *path,
...
class Show_create_error_handler : public Internal_error_handler
{
  ...
  */
  explicit Show_create_error_handler(THD *thd, TABLE_LIST *top_view)
      : m_top_view(top_view), m_handling(FALSE), m_view_access_denied_message_ptr(NULL)
  {
    m_sctx = MY_TEST(m_top_view->security_ctx) ? m_top_view->security_ctx : thd->security_ctx;
  }
  ...
  */
  char *get_view_access_denied_message(THD *thd)
  {
    if (!m_view_access_denied_message_ptr)
    {
      ...
      my_snprintf(m_view_access_denied_message, MYSQL_ERRMSG_SIZE, ER_THD(thd, ER_TABLEACCESS_DENIED_ERROR),
                  "SHOW VIEW", m_sctx->priv_user, m_sctx->host_or_ip, m_top_view->get_db_name(),
                  m_top_view->get_table_name());
    }
    return m_view_access_denied_message_ptr;
  }

  bool handle_condition(THD *thd, uint sql_errno, const char * /* sqlstate */, Sql_condition::enum_warning_level *level,
    ...
    switch (sql_errno)
    {
      ...
      case ER_NO_SUCH_TABLE_IN_ENGINE:
        /* Established behavior: warn if underlying tables, columns, or functions
           are missing. */
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_VIEW_INVALID, ER_THD(thd, ER_VIEW_INVALID),
...
bool mysqld_show_create_get_fields(THD *thd, TABLE_LIST *table_list, List<Item> *field_list, String *buffer)
{
  bool error = TRUE;
  LEX *lex = thd->lex;
  MEM_ROOT *mem_root = thd->mem_root;
  DBUG_ENTER("mysqld_show_create_get_fields");
  DBUG_PRINT("enter", ("db: %s  table: %s", table_list->db.str, table_list->table_name.str));
  ...
  if (lex->table_type == TABLE_TYPE_VIEW)
  {
    if (check_table_access(thd, SELECT_ACL, table_list, FALSE, 1, FALSE))
    {
      ...
      my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), "SHOW", thd->security_ctx->priv_user, thd->security_ctx->host_or_ip,
  ...
  else
  {
    ...
    */
    DBUG_PRINT("debug", ("table_list->grant.privilege: %llx", (longlong)(table_list->grant.privilege)));
  ...
  {
    my_error(ER_WRONG_OBJECT, MYF(0), table_list->db.str, table_list->table_name.str, "VIEW");
    goto exit;
  }
  else if (lex->table_type == TABLE_TYPE_SEQUENCE &&
           (!table_list->table || table_list->table->s->table_type != TABLE_TYPE_SEQUENCE))
  {
    my_error(ER_NOT_SEQUENCE, MYF(0), table_list->db.str, table_list->table_name.str);
    goto exit;
  }
  ...
  if ((table_list->view                         ? show_create_view(thd, table_list, buffer)
       : lex->table_type == TABLE_TYPE_SEQUENCE ? show_create_sequence(thd, table_list, buffer)
                                                : show_create_table(thd, table_list, buffer, NULL, WITHOUT_DB_NAME)))
    goto exit;

  if (table_list->view)
  {
    field_list->push_back(new (mem_root) Item_empty_string(thd, "View", NAME_CHAR_LEN), mem_root);
    field_list->push_back(new (mem_root) Item_empty_string(thd, "Create View", MY_MAX(buffer->length(), 1024)),
                          mem_root);
    field_list->push_back(new (mem_root) Item_empty_string(thd, "character_set_client", MY_CS_CHARACTER_SET_NAME_SIZE),
                          mem_root);
    field_list->push_back(new (mem_root) Item_empty_string(thd, "collation_connection", MY_CS_COLLATION_NAME_SIZE),
                          mem_root);
  }
  else
  {
    field_list->push_back(new (mem_root) Item_empty_string(thd, "Table", NAME_CHAR_LEN), mem_root);
    // 1024 is for not to confuse old clients
    field_list->push_back(new (mem_root) Item_empty_string(thd, "Create Table", MY_MAX(buffer->length(), 1024)),
                          mem_root);
  }
  error = FALSE;
  ...
  */
  ...
  else
  {
    if (table_list->schema_table)
      protocol->store(table_list->schema_table->table_name, strlen(table_list->schema_table->table_name),
                      system_charset_info);
    else
      protocol->store(table_list->table->alias.ptr(), table_list->table->alias.length(), system_charset_info);
  }
  ...
  if (table_list->view)
  {
    ...
    protocol->store(&table_list->view_creation_ctx->get_client_cs()->cs_name, system_charset_info);

    protocol->store(&table_list->view_creation_ctx->get_connection_cl()->coll_name, system_charset_info);
  }
  else
    protocol->store(&buffer);
...
{
  MEM_ROOT *mem_root = thd->mem_root;
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Database", NAME_CHAR_LEN), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Create Database", 1024), mem_root);
}

bool mysqld_show_create_db(THD *thd, LEX_CSTRING *dbname, LEX_CSTRING *orig_dbname, const DDL_options_st &options)
{
  ...
  {
    db_access = acl_get(sctx->host, sctx->ip, sctx->priv_user, dbname->str, 0) | sctx->master_access;
    if (sctx->priv_role[0])
  ...
  if (!(db_access & DB_ACLS) && check_grant_db(thd, dbname->str))
  {
    ...
    my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), sctx->priv_user, sctx->host_or_ip, dbname->str);
    general_log_print(thd, COM_INIT_DB, ER_THD(thd, ER_DBACCESS_DENIED_ERROR), sctx->priv_user, sctx->host_or_ip,
                      orig_dbname->str);
    DBUG_RETURN(TRUE);
  }
#endif
  ...
  if (create.schema_comment)
  {
    ...
    append_unescaped(&buffer, create.schema_comment->str, create.schema_comment->length);
  }
...
void mysqld_list_fields(THD *thd, TABLE_LIST *table_list, const char *wild)
{
  ...
  if (open_normal_and_derived_tables(thd, table_list, MYSQL_OPEN_FORCE_SHARED_HIGH_PRIO_MDL, DT_INIT | DT_PREPARE))
    DBUG_VOID_RETURN;
  ...
  {
    if (!wild || !wild[0] || !wild_case_compare(system_charset_info, field->field_name.str, wild))
...
bool append_identifier(THD *thd, String *packet, const char *name, size_t length)
{
  ...
  */
  CHARSET_INFO *quote_charset =
      q == 0x60 && (packet->charset()->state & MY_CS_NONASCII) && packet->charset()->mbmaxlen == 1
          ? &my_charset_bin
          : system_charset_info;
  ...
  for (name_end = name + length; name < name_end;)
  {
    ...
    */
    if (char_length <= 0)
      char_length = 1;
    if (char_length == 1 && chr == (uchar)quote_char && packet->append(&quote_char, 1, quote_charset))
      return true;
...
{
  if (length && !is_keyword(name, (uint)length) && !require_quotes(name, (uint)length) &&
...
static void append_directory(THD *thd, String *packet, LEX_CSTRING *dir_type, const char *filename)
{
...
static bool get_field_default_value(THD *thd, Field *field, String *def_value, bool quoted)
{
  ...
  has_default = (field->default_value || (!(field->flags & NO_DEFAULT_VALUE_FLAG) && !field->vers_sys_field() &&
...
static void append_create_options(THD *thd, String *packet, engine_option_value *opt, bool check_options,
...
static void add_table_options(THD *thd, TABLE *table, Table_specification_st *create_info_arg, bool schema_table,
                              bool sequence, String *packet)
{
  ...
  bool check_options = (!(sql_mode & MODE_IGNORE_BAD_TABLE_OPTIONS) &&
                        (!create_info_arg || create_info_arg->used_fields & HA_CREATE_PRINT_ALL_OPTIONS));
  ...
  */
  if (!create_info_arg || (create_info_arg->used_fields & HA_CREATE_USED_ENGINE))
  {
  ...
  */
  ...
  if (share->table_charset && !(sql_mode & (MODE_MYSQL323 | MODE_MYSQL40)) && share->table_type != TABLE_TYPE_SEQUENCE)
  {
    /*
      IF   check_create_info
      THEN add DEFAULT CHARSET only if it was used when creating the table
    */
    if (!create_info_arg || (create_info_arg->used_fields & HA_CREATE_USED_DEFAULT_CHARSET))
    {
...
end_options:
  ...
  append_create_options(thd, packet, share->option_list, check_options, hton->table_options);
...
static void append_period(THD *thd, String *packet, const LEX_CSTRING &start, const LEX_CSTRING &end,
                          const LEX_CSTRING &period, bool ident)
{
  packet->append(STRING_WITH_LEN(",\n  PERIOD FOR "));
  if (ident)
...
int show_create_table(THD *thd, TABLE_LIST *table_list, String *packet, Table_specification_st *create_info_arg,
                      enum_with_db_name with_db_name)
{
  return show_create_table_ex(thd, table_list, NULL, NULL, packet, create_info_arg, with_db_name);
}
...
int show_create_table_ex(THD *thd, TABLE_LIST *table_list, const char *force_db, const char *force_name, String *packet,
                         Table_specification_st *create_info_arg, enum_with_db_name with_db_name)
{
  ...
  bool foreign_db_mode = sql_mode & (MODE_POSTGRESQL | MODE_ORACLE | MODE_MSSQL | MODE_DB2 | MODE_MAXDB | MODE_ANSI);
  bool limited_mysql_mode = sql_mode & (MODE_NO_FIELD_OPTIONS | MODE_MYSQL323 | MODE_MYSQL40);
  bool show_table_options = !(sql_mode & MODE_NO_TABLE_OPTIONS) && !foreign_db_mode;
  bool check_options = !(sql_mode & MODE_IGNORE_BAD_TABLE_OPTIONS) && !create_info_arg;
  handlerton *hton;
  int error = 0;
  ...
  if (create_info_arg && ((create_info_arg->or_replace() && !create_info_arg->or_replace_slave_generated()) ||
  ...
  else
  {
    ...
    {
      const LEX_CSTRING *const db = table_list->schema_table ? &INFORMATION_SCHEMA_NAME : &table->s->db;
  ...
  */
  ...
  for (ptr = table->field; (field = *ptr); ptr++)
  {
    ...
    else
    {
      ...
      if (field->unireg_check == Field::NEXT_NUMBER && !(sql_mode & MODE_NO_FIELD_OPTIONS))
        packet->append(STRING_WITH_LEN(" AUTO_INCREMENT"));
    }
    ...
    append_create_options(thd, packet, field->option_list, check_options, hton->field_options);
  ...
  if (period.name)
  {
    append_period(thd, packet, period.start_field(share)->field_name, period.end_field(share)->field_name, period.name,
                  true);
  }
  ...
  for (uint i = 0; i < share->keys; i++, key_info++)
  {
    ...
    for (uint j = 0; j < key_parts; j++, key_part++)
    {
      ...
      if (key_part->field && (key_part->length != table->field[key_part->fieldnr - 1]->key_length() &&
                              !(key_info->flags & (HA_FULLTEXT | HA_SPATIAL))))
      {
        packet->append_parenthesized((long)key_part->length / key_part->field->charset()->mbmaxlen);
      }
    ...
    append_create_options(thd, packet, key_info->option_list, check_options, hton->index_options);
  }
  ...
  if (table->versioned())
  {
    ...
    if (explicit_fields)
    {
      append_period(thd, packet, fs->field_name, fe->field_name, table->s->vers.name, false);
    }
    else
    {
  ...
  */
  ...
  if (show_table_options)
    add_table_options(thd, table, create_info_arg, table_list->schema_table != 0, 0, packet);
  ...
  {
    if (table->part_info &&
        !((table->s->db_type()->partition_flags() & HA_USE_AUTO_PARTITION) && table->part_info->is_auto_partitioned))
    {
      /*
      ...
      */
      ...
      char *part_syntax;
      if ((part_syntax =
               generate_partition_syntax(thd, table->part_info, &part_syntax_len, show_table_options, NULL, NULL)))
      {
        packet->append('\n');
...
{
  bool limited_mysql_mode = (thd->variables.sql_mode & (MODE_NO_FIELD_OPTIONS | MODE_MYSQL323 | MODE_MYSQL40)) != 0;
  ...
  if (!(thd->variables.sql_mode & MODE_NO_KEY_OPTIONS) && !limited_mysql_mode && !foreign_db_mode)
  {
    ...
    if (key_info->algorithm == HA_KEY_ALG_HASH || key_info->algorithm == HA_KEY_ALG_LONG_HASH)
    ...
    if ((key_info->flags & HA_USES_BLOCK_SIZE) && table->s->key_block_size != key_info->block_size)
    {
      packet->append(STRING_WITH_LEN(" KEY_BLOCK_SIZE="));
      end = longlong10_to_str(key_info->block_size, buff, 10);
      packet->append(buff, (uint)(end - buff));
    }
    DBUG_ASSERT(MY_TEST(key_info->flags & HA_USES_COMMENT) == (key_info->comment.length > 0));
...
static bool append_at_host(THD *thd, String *buffer, const LEX_CSTRING *host)
{
  if (!host->str || !host->str[0])
    return false;
  return buffer->append('@') || append_identifier(thd, buffer, host);
}
...
bool append_definer(THD *thd, String *buffer, const LEX_CSTRING *definer_user, const LEX_CSTRING *definer_host)
{
  return buffer->append(STRING_WITH_LEN("DEFINER=")) || append_identifier(thd, buffer, definer_user) ||
...
static int show_create_view(THD *thd, TABLE_LIST *table, String *buff)
{
  ...
  else
  {
    ...
    */
    ...
    {
      if (!tbl->is_derived() && cmp(&table->view_db, tbl->view ? &tbl->view_db : &tbl->db))
      {
  ...
  */
  table->view->unit.print(buff, enum_query_type(QT_VIEW_INTERNAL | QT_ITEM_ORIGINAL_FUNC_NULLIF));
...
{
  list_callback_arg(const char *u, THD *t, ulong m) : user(u), thd(t), max_query_length(m) {}
...
static my_bool list_callback(THD *tmp, list_callback_arg *arg)
{
  ...
  if ((tmp->vio_ok() || tmp->system_thread) &&
      (!arg->user || (!tmp->system_thread && tmp_sctx->user && !strcmp(tmp_sctx->user, arg->user))))
  {
    ...
    if (tmp->peer_port && (tmp_sctx->host || tmp_sctx->ip) && arg->thd->security_ctx->host_or_ip[0])
    {
      ...
      thd_info->host = arg->thd->strdup(tmp_sctx->host_or_ip[0] ? tmp_sctx->host_or_ip
    ...
    if ((got_thd_data = !trylock_short(&tmp->LOCK_thd_data)))
    {
      ...
      thd_info->proc_info = (char *)(tmp->killed >= KILL_QUERY ? "Killed" : 0);
      ...
      */
      if (tmp->progress.max_counter)
      {
        ...
        thd_info->progress = (((tmp->progress.stage / (double)max_stage) +
                               ((tmp->progress.counter / (double)tmp->progress.max_counter) / (double)max_stage)) *
                              100.0);
...
void mysqld_list_processes(THD *thd, const char *user, bool verbose)
{
  Item *field;
  List<Item> field_list;
  list_callback_arg arg(user, thd, verbose ? thd->variables.max_allowed_packet : PROCESS_LIST_WIDTH);
  Protocol *protocol = thd->protocol;
  MEM_ROOT *mem_root = thd->mem_root;
  DBUG_ENTER("mysqld_list_processes");

  field_list.push_back(new (mem_root) Item_int(thd, "Id", 0, MY_INT32_NUM_DECIMAL_DIGITS), mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "User", USERNAME_CHAR_LENGTH), mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Host", LIST_PROCESS_HOST_LEN), mem_root);
  field_list.push_back(field = new (mem_root) Item_empty_string(thd, "db", NAME_CHAR_LEN), mem_root);
  field->set_maybe_null();
  ;
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Command", 16), mem_root);
  field_list.push_back(field = new (mem_root) Item_return_int(thd, "Time", 7, MYSQL_TYPE_LONG), mem_root);
  field->unsigned_flag = 0;
  field_list.push_back(field = new (mem_root) Item_empty_string(thd, "State", 30), mem_root);
  field->set_maybe_null();
  ;
  field_list.push_back(field = new (mem_root) Item_empty_string(thd, "Info", arg.max_query_length), mem_root);
  field->set_maybe_null();
  ;
  if (!(thd->variables.old_behavior & OLD_MODE_NO_PROGRESS_INFO))
  {
    field_list.push_back(field = new (mem_root) Item_float(thd, "Progress", 0.0, 3, 7), mem_root);
  ...
  while (thread_info *thd_info = arg.thread_infos.get())
  {
    ...
    if (thd_info->proc_info)
      protocol->store(thd_info->proc_info, strlen(thd_info->proc_info), system_charset_info);
    else
    ...
    if (thd_info->query_string.length())
      protocol->store(thd_info->query_string.str(), thd_info->query_string.length(), thd_info->query_string.charset());
    else
      protocol->store_null();
    if (!(thd->variables.old_behavior & OLD_MODE_NO_PROGRESS_INFO))
...
void Show_explain_request::call_in_target_thread()
{
  ...
  */
  target_thd->set_n_backup_active_arena((Query_arena *)request_thd, &backup_arena);

  query_str.copy(target_thd->query(), target_thd->query_length(), target_thd->query_charset());
  ...
  /*
    ...
    fields).
  */
  if (target_thd->lex->print_explain(explain_buf, 0 /* explain flags*/, is_analyze, is_json_format, &printed_anything))
  {
...
int select_result_text_buffer::send_data(List<Item> &items) { return append_row(items, false /*send item values */); }
...
int select_result_text_buffer::append_row(List<Item> &items, bool send_names)
{
  ...
  if (!(row = (char **)thd->alloc(sizeof(char *) * n_columns)) || rows.push_back(row, thd->mem_root))
    return true;
...
int fill_show_explain_or_analyze(THD *thd, TABLE_LIST *table, COND *cond, bool json_format, bool is_analyze)
{
  ...
  calling_user = (thd->security_ctx->master_access & PRIV_STMT_SHOW_EXPLAIN) ? NullS : thd->security_ctx->priv_user;
  ...
  if ((tmp = find_thread_by_id(thread_id)))
  {
    ...
    */
    if (calling_user && (!tmp_sctx->user || strcmp(calling_user, tmp_sctx->user)))
    {
    ...
    */
    ...
    else
    {
      ...
      */
      ...
      {
        uint conv_length = 1 + tocs->mbmaxlen * explain_req.query_str.length() / fromcs->mbminlen;
        uint dummy_errors;
        char *to, *p;
        if (!(to = (char *)thd->alloc(conv_length + 1)))
          DBUG_RETURN(1);
        p = to;
        p += copy_and_convert(to, conv_length, tocs, explain_req.query_str.c_ptr(), explain_req.query_str.length(),
                              fromcs, &dummy_errors);
        *p = 0;
        warning_text = to;
      }
      ...
      push_warning(thd, Sql_condition::WARN_LEVEL_NOTE, ER_YES, warning_text);
    }
    DBUG_RETURN(bres);
  }
...
{
  return fill_show_explain_or_analyze(thd, table, cond, FALSE /* json_format */, FALSE /* is_analyze */);
}

int fill_show_explain_json(THD *thd, TABLE_LIST *table, COND *cond)
{
  return fill_show_explain_or_analyze(thd, table, cond, TRUE /* json_format */, FALSE /* is_analyze */);
}

int fill_show_analyze_tabular(THD *thd, TABLE_LIST *table, COND *cond)
{
  return fill_show_explain_or_analyze(thd, table, cond, FALSE /* json_format */, TRUE /* is_analyze */);
}

int fill_show_analyze_json(THD *thd, TABLE_LIST *table, COND *cond)
{
  return fill_show_explain_or_analyze(thd, table, cond, TRUE /* json_format */, TRUE /* is_analyze */);
}
...
static my_bool processlist_callback(THD *tmp, processlist_callback_arg *arg)
{
  ...
  bool got_thd_data;
  char *user =
      arg->thd->security_ctx->master_access & PRIV_STMT_SHOW_PROCESSLIST ? NullS : arg->thd->security_ctx->priv_user;

  if ((!tmp->vio_ok() && !tmp->system_thread) ||
      (user && (tmp->system_thread || !tmp_sctx->user || strcmp(tmp_sctx->user, user))))
    return 0;
  ...
  /* USER */
  val = tmp_sctx->user ? tmp_sctx->user : (tmp->system_thread ? "system user" : "unauthenticated user");
  arg->table->field[1]->store(val, strlen(val), cs);
  /* HOST */
  if (tmp->peer_port && (tmp_sctx->host || tmp_sctx->ip) && arg->thd->security_ctx->host_or_ip[0])
  {
    char host[LIST_PROCESS_HOST_LEN + 1];
    my_snprintf(host, LIST_PROCESS_HOST_LEN, "%s:%u", tmp_sctx->host_or_ip, tmp->peer_port);
    arg->table->field[2]->store(host, strlen(host), cs);
  }
  else
    arg->table->field[2]->store(tmp_sctx->host_or_ip, strlen(tmp_sctx->host_or_ip), cs);
  ...
  /* COMMAND */
  if ((val = (char *)(!got_thd_data ? "Busy" : (tmp->killed >= KILL_QUERY ? "Killed" : 0))))
    arg->table->field[4]->store(val, strlen(val), cs);
  else
    arg->table->field[4]->store(command_name[tmp->get_command()].str, command_name[tmp->get_command()].length, cs);
  ...
  if (got_thd_data)
  {
    if (tmp->query())
    {
      arg->table->field[7]->store(tmp->query(), MY_MIN(PROCESS_LIST_INFO_WIDTH, tmp->query_length()), cs);
      arg->table->field[7]->set_notnull();

      /* INFO_BINARY */
      arg->table->field[16]->store(tmp->query(), MY_MIN(PROCESS_LIST_INFO_WIDTH, tmp->query_length()), &my_charset_bin);
    ...
    */
    if ((max_counter = tmp->progress.max_counter))
    {
      ...
      arg->table->field[11]->store((double)tmp->progress.counter / (double)max_counter * 100.0);
    }
    mysql_mutex_unlock(&tmp->LOCK_thd_data);
  }
  ...
  /*
    ...
    than hide it.
  */
  arg->table->field[12]->store((longlong)tmp->status_var.local_memory_used, FALSE);
  arg->table->field[13]->store((longlong)tmp->status_var.max_local_memory_used, FALSE);
...
int fill_schema_processlist(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  if (!thd->killed && server_threads.iterate(processlist_callback, &arg))
    DBUG_RETURN(1);
  DBUG_RETURN(0);
}
...
int add_status_vars(SHOW_VAR *list)
{
  ...
  if (!all_status_vars.buffer &&  // array is not allocated yet - do it now
      my_init_dynamic_array(PSI_INSTRUMENT_ME, &all_status_vars, sizeof(SHOW_VAR), 250, 50, MYF(0)))
  {
    res = 1;
    goto err;
  }
  while (list->name) res |= insert_dynamic(&all_status_vars, (uchar *)list++);
...
const char *get_one_variable(THD *thd, const SHOW_VAR *variable, enum_var_type value_type, SHOW_TYPE show_type,
                             system_status_var *status_var, const CHARSET_INFO **charset, char *buff, size_t *length)
{
...
static bool show_status_array(THD *thd, const char *wild, SHOW_VAR *variables, enum enum_var_type scope,
                              struct system_status_var *status_var, const char *prefix, TABLE *table, bool ucase_names,
                              COND *cond)
{
  ...
  */
  ...
  for (; variables->name; variables++)
  {
    ...
    */
    for (var = variables; var->type == SHOW_FUNC || var->type == SHOW_SIMPLE_FUNC; var = &tmp)
      ((mysql_show_var_func)(var->value))(thd, &tmp, buff, status_var, scope);

    SHOW_TYPE show_type = var->type;
    if (show_type == SHOW_ARRAY)
    {
      show_status_array(thd, wild, (SHOW_VAR *)var->value, scope, status_var, name_buffer, table, ucase_names, cond);
    }
    else
    {
      if ((wild_checked || !(wild && wild[0] && wild_case_compare(system_charset_info, name_buffer, wild))) &&
          (!cond || cond->val_int()))
      {
        ...
        pos = get_one_variable(thd, var, scope, show_type, status_var, &charset, buff, &length);
...
bool schema_table_store_record(THD *thd, TABLE *table)
{
  ...
  if (unlikely((error = table->file->ha_write_tmp_row(table->record[0]))))
  {
    ...
    if (unlikely(
            create_internal_tmp_table_from_heap(thd, table, param->start_recinfo, &param->recinfo, error, 0, NULL)))
...
static int make_table_list(THD *thd, SELECT_LEX *sel, LEX_CSTRING *db_name, LEX_CSTRING *table_name)
{
...
bool get_lookup_value(THD *thd, Item_func *item_func, TABLE_LIST *table, LOOKUP_FIELD_VALUES *lookup_field_vals)
{
  ST_SCHEMA_TABLE *schema_table = table->schema_table;
  ST_FIELD_INFO *field_info = schema_table->fields_info;
  const char *field_name1 = schema_table->idx_field1 >= 0 ? field_info[schema_table->idx_field1].name().str : "";
  const char *field_name2 = schema_table->idx_field2 >= 0 ? field_info[schema_table->idx_field2].name().str : "";

  if (item_func->functype() == Item_func::EQ_FUNC || item_func->functype() == Item_func::EQUAL_FUNC)
  {
    ...
    if (item_func->arguments()[0]->real_item()->type() == Item::FIELD_ITEM && item_func->arguments()[1]->const_item())
    {
      idx_field = 0;
      idx_val = 1;
    }
    ...
    if (!cs->strnncollsp(field_name1, strlen(field_name1), item_field->field_name.str, item_field->field_name.length))
    {
      thd->make_lex_string(&lookup_field_vals->db_value, tmp_str->ptr(), tmp_str->length());
    }
    /* Lookup value is table name */
    else if (!cs->strnncollsp(field_name2, strlen(field_name2), item_field->field_name.str,
                              item_field->field_name.length))
    {
      thd->make_lex_string(&lookup_field_vals->table_value, tmp_str->ptr(), tmp_str->length());
    }
  }
  return 0;
}
...
bool calc_lookup_values_from_cond(THD *thd, COND *cond, TABLE_LIST *table, LOOKUP_FIELD_VALUES *lookup_field_vals)
{
  if (!cond)
    return 0;
  ...
  else if (cond->type() == Item::FUNC_ITEM && get_lookup_value(thd, (Item_func *)cond, table, lookup_field_vals))
    return 1;
  return 0;
}
...
bool uses_only_table_name_fields(Item *item, TABLE_LIST *table)
{
  ...
  else if (item->type() == Item::FIELD_ITEM)
  {
    ...
    const char *field_name2 = schema_table->idx_field2 >= 0 ? field_info[schema_table->idx_field2].name().str : "";
    if (table->table != item_field->field->table ||
        (cs->strnncollsp(field_name1, strlen(field_name1), item_field->field_name.str, item_field->field_name.length) &&
         cs->strnncollsp(field_name2, strlen(field_name2), item_field->field_name.str, item_field->field_name.length)))
      return 0;
  }
...
bool get_lookup_field_values(THD *thd, COND *cond, bool fix_table_name_case, TABLE_LIST *tables,
                             LOOKUP_FIELD_VALUES *lookup_field_values)
{
  ...
  switch (lex->sql_command)
  {
    case SQLCOM_SHOW_PLUGINS:
      if (lex->ident.str)
      {
        thd->make_lex_string(&lookup_field_values->db_value, lex->ident.str, lex->ident.length);
        break;
      }
      /* fall through */
    ...
    case SQLCOM_SHOW_DATABASES:
      if (wild)
      {
        thd->make_lex_string(&lookup_field_values->db_value, wild->ptr(), wild->length());
    ...
    case SQLCOM_SHOW_EVENTS:
      thd->make_lex_string(&lookup_field_values->db_value, lex->first_select_lex()->db.str,
                           lex->first_select_lex()->db.length);
      if (wild)
      {
        thd->make_lex_string(&lookup_field_values->table_value, wild->ptr(), wild->length());
  ...
  if (lower_case_table_names && !rc)
  {
    ...
    */
    if (lookup_field_values->db_value.str && lookup_field_values->db_value.str[0])
      my_casedn_str(system_charset_info, (char *)lookup_field_values->db_value.str);
    if (fix_table_name_case && lookup_field_values->table_value.str && lookup_field_values->table_value.str[0])
      my_casedn_str(system_charset_info, (char *)lookup_field_values->table_value.str);
  }
...
static int make_db_list(THD *thd, Dynamic_array<LEX_CSTRING *> *files, LOOKUP_FIELD_VALUES *lookup_field_vals)
{
  if (lookup_field_vals->wild_db_value)
  {
    ...
    */
    if (!lookup_field_vals->db_value.str ||
        !wild_case_compare(system_charset_info, INFORMATION_SCHEMA_NAME.str, lookup_field_vals->db_value.str))
    {
      if (files->append_val(&INFORMATION_SCHEMA_NAME))
        return 1;
    }
    return find_files(thd, files, 0, mysql_data_home, &lookup_field_vals->db_value);
  }
...
static my_bool add_schema_table(THD *thd, plugin_ref plugin, void *p_data)
{
  ...
  if ((file_name = thd->make_clex_string(schema_table->table_name, strlen(schema_table->table_name))) &&
      !file_list->append(file_name))
    DBUG_RETURN(0);
  DBUG_RETURN(1);
}

int schema_tables_add(THD *thd, Dynamic_array<LEX_CSTRING *> *files, const char *wild)
{
  ...
  for (; tmp_schema_table->table_name; tmp_schema_table++)
  {
    ...
    if ((file_name = thd->make_clex_string(tmp_schema_table->table_name, strlen(tmp_schema_table->table_name))) &&
  ...
  add_data.wild = wild;
  if (plugin_foreach(thd, add_schema_table, MYSQL_INFORMATION_SCHEMA_PLUGIN, &add_data))
    DBUG_RETURN(1);
...
static int make_table_name_list(THD *thd, Dynamic_array<LEX_CSTRING *> *table_names, LEX *lex,
                                LOOKUP_FIELD_VALUES *lookup_field_vals, LEX_CSTRING *db_name)
{
  char path[FN_REFLEN + 1];
  build_table_filename(path, sizeof(path) - 1, db_name->str, "", "", 0);
  if (!lookup_field_vals->wild_table_value && lookup_field_vals->table_value.str)
  {
    if (check_table_name(lookup_field_vals->table_value.str, lookup_field_vals->table_value.length, false))
    {
      /*
    ...
    {
      LEX_CSTRING *name;
      ST_SCHEMA_TABLE *schema_table = find_schema_table(thd, &lookup_field_vals->table_value);
      if (schema_table && !schema_table->hidden)
      {
        if (!(name = thd->make_clex_string(schema_table->table_name, strlen(schema_table->table_name))) ||
  ...
  */
  if (db_name == &INFORMATION_SCHEMA_NAME)
    return (schema_tables_add(thd, table_names, lookup_field_vals->table_value.str));

  if (check_db_name((LEX_STRING *)db_name))
    return 0;  // Impossible TABLE_SCHEMA name

  find_files_result res = find_files(thd, table_names, db_name, path, &lookup_field_vals->table_value);
...
static void get_table_engine_for_i_s(THD *thd, char *buf, TABLE_LIST *tl, LEX_CSTRING *db, LEX_CSTRING *table)
{
  ...
  {
    char path[FN_REFLEN];
    build_table_filename(path, sizeof(path) - 1, db->str, table->str, reg_ext, 0);
...
/**
  ...
  @retval TRUE  - Failure.
*/
static bool fill_schema_table_by_open(THD *thd, MEM_ROOT *mem_root, bool is_show_fields_or_keys, TABLE *table,
                                      ST_SCHEMA_TABLE *schema_table, LEX_CSTRING *orig_db_name,
                                      LEX_CSTRING *orig_table_name, Open_tables_backup *open_tables_state_backup,
                                      bool can_deadlock)
{
  Query_arena i_s_arena(mem_root, Query_arena::STMT_CONVENTIONAL_EXECUTION), backup_arena, *old_arena;
  ...
  */
  if (!thd->make_lex_string(&db_name, orig_db_name->str, orig_db_name->length) ||
      !thd->make_lex_string(&table_name, orig_table_name->str, orig_table_name->length))
    goto end;
  ...
  */
  if (!is_show_fields_or_keys && result && thd->is_error() &&
      (thd->get_stmt_da()->sql_errno() == ER_NO_SUCH_TABLE || thd->get_stmt_da()->sql_errno() == ER_WRONG_OBJECT ||
  ...
  else
  {
    ...
    result = schema_table->process_table(thd, table_list, table, result, orig_db_name, orig_table_name);
  }
...
static int fill_schema_table_names(THD *thd, TABLE_LIST *tables, LEX_CSTRING *db_name, LEX_CSTRING *table_name)
{
  TABLE *table = tables->table;
  if (db_name == &INFORMATION_SCHEMA_NAME)
  {
    table->field[3]->store(STRING_WITH_LEN("SYSTEM VIEW"), system_charset_info);
  }
  else if (tables->table_open_method != SKIP_OPEN_TABLE)
  {
    ...
    if (ha_table_exists(thd, db_name, table_name, NULL, NULL, &hton, &is_sequence))
    {
    ...
    if (unlikely(thd->is_error() && thd->get_stmt_da()->sql_errno() == ER_NO_SUCH_TABLE))
    {
      thd->clear_error();
      return 0;
    }
...
uint get_table_open_method(TABLE_LIST *tables, ST_SCHEMA_TABLE *schema_table, enum enum_schema_tables schema_table_idx)
{
  /*
  ...
  */
  if (schema_table->i_s_requested_object & OPTIMIZE_I_S_TABLE)
  {
    ...
    for (ptr = tables->table->field; (field = *ptr); ptr++)
    {
      ...
      star_table_open_method = MY_MIN(star_table_open_method, (uint)def.open_method());
...
{
  bool error;
  MDL_REQUEST_INIT(&table->mdl_request, MDL_key::TABLE, table->db.str, table->table_name.str, MDL_SHARED_HIGH_PRIO,
                   MDL_TRANSACTION);
...
static int fill_schema_table_from_frm(THD *thd, TABLE *table, ST_SCHEMA_TABLE *schema_table, LEX_CSTRING *db_name,
                                      LEX_CSTRING *table_name, Open_tables_backup *open_tables_state_backup,
                                      bool can_deadlock)
{
  TABLE_SHARE *share;
  TABLE tbl;
  ...
  */
  ...
  if (!table_list.mdl_request.ticket)
  {
    ...
    */
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_I_S_SKIPPED_TABLE,
                        ER_THD(thd, ER_WARN_I_S_SKIPPED_TABLE), table_list.db.str, table_list.table_name.str);
    return 0;
  }

  if (schema_table->i_s_requested_object & OPEN_TRIGGER_ONLY)
  {
    init_sql_alloc(key_memory_table_triggers_list, &tbl.mem_root, TABLE_ALLOC_BLOCK_SIZE, 0, MYF(0));
    if (!Table_triggers_list::check_n_load(thd, db_name, table_name, &tbl, 1))
    {
      table_list.table = &tbl;
      res = schema_table->process_table(thd, &table_list, table, res, db_name, table_name);
      delete tbl.triggers;
    }
  ...
  if (!share)
  {
    if (thd->get_stmt_da()->sql_errno() == ER_NO_SUCH_TABLE || thd->get_stmt_da()->sql_errno() == ER_WRONG_OBJECT ||
        thd->get_stmt_da()->sql_errno() == ER_NOT_SEQUENCE)
    {
      res = 0;
    }
    else
    {
      ...
      res = schema_table->process_table(thd, &table_list, table, true, db_name, table_name);
    }
    goto end;
  }
  ...
  if (!open_table_from_share(thd, share, table_name, 0, (EXTRA_RECORD | OPEN_FRM_FILE_ONLY), thd->open_options, &tbl,
                             FALSE))
  {
    tbl.s = share;
    table_list.table = &tbl;
...
class Warnings_only_error_handler : public Internal_error_handler
{
 public:
  ...
  {
    if (sql_errno == ER_TRG_NO_DEFINER || sql_errno == ER_TRG_NO_CREATION_CTX || sql_errno == ER_PARSE_ERROR)
      return true;
...
int get_all_tables(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  */
  ...
  {
    error = fill_schema_table_by_open(thd, thd->mem_root, TRUE, table, schema_table, &lsel->table_list.first->db,
                                      &lsel->table_list.first->table_name, &open_tables_state_backup, can_deadlock);
    goto err;
  }
  ...
  init_alloc_root(PSI_INSTRUMENT_ME, &tmp_mem_root, SHOW_ALLOC_BLOCK_SIZE, SHOW_ALLOC_BLOCK_SIZE, MY_THREAD_SPECIFIC);
  ...
  for (size_t i = 0; i < db_names.elements(); i++)
  {
    ...
    DBUG_ASSERT(db_name->length <= NAME_LEN);
#ifndef NO_EMBEDDED_ACCESS_CHECKS
    if (!(check_access(thd, SELECT_ACL, db_name->str, &thd->col_access, NULL, 0, 1) ||
...
#endif
    {
      ...
      int res = make_table_name_list(thd, &table_names, lex, &plan->lookup_field_vals, db_name);
        ...
        if (!partial_cond || partial_cond->val_int())
        {
          ...
          */
          if (!table_open_method && schema_table_idx == SCH_TABLES &&
              (!plan->lookup_field_vals.table_value.length || plan->lookup_field_vals.wild_table_value))
          {
          ...
          else if (schema_table_idx == SCH_TRIGGERS && db_name == &INFORMATION_SCHEMA_NAME)
          {
            continue;
          }
          else
          {
            if (!(table_open_method & ~OPEN_FRM_ONLY) && db_name != &INFORMATION_SCHEMA_NAME)
            {
              if (!fill_schema_table_from_frm(thd, table, schema_table, db_name, table_name, &open_tables_state_backup,
                                              can_deadlock))
                continue;
            }
            ...
            if (fill_schema_table_by_open(thd, &tmp_mem_root, FALSE, table, schema_table, db_name, table_name,
...
bool store_schema_schemata(THD *thd, TABLE *table, LEX_CSTRING *db_name, CHARSET_INFO *cs,
                           LEX_CSTRING *schema_comment = NULL)
{
  restore_record(table, s->default_values);
  table->field[0]->store(STRING_WITH_LEN("def"), system_charset_info);
...
int fill_schema_schemata(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  */
  ...
  if (get_lookup_field_values(thd, cond, true, tables, &lookup_field_vals))
    DBUG_RETURN(0);
  DBUG_PRINT("INDEX VALUES",
             ("db_name: %s  table_name: %s", lookup_field_vals.db_value.str, lookup_field_vals.table_value.str));
  ...
  */
  if (lookup_field_vals.db_value.str && !lookup_field_vals.wild_db_value &&
      (!db_names.elements() /* The database name was too long */ ||
       (db_names.at(0) != &INFORMATION_SCHEMA_NAME && verify_database_directory_exists(lookup_field_vals.db_value))))
    DBUG_RETURN(0);
  ...
  for (size_t i = 0; i < db_names.elements(); i++)
  {
    ...
    if (db_name == &INFORMATION_SCHEMA_NAME)
    {
      if (store_schema_schemata(thd, table, db_name, system_charset_info))
        DBUG_RETURN(1);
      continue;
    }
#ifndef NO_EMBEDDED_ACCESS_CHECKS
    if (sctx->master_access & (DB_ACLS | SHOW_DB_ACL) ||
        ...
        (sctx->priv_role[0] ? acl_get("", "", sctx->priv_role, db_name->str, false) : NO_ACL) ||
        !check_grant_db(thd, db_name->str))
#endif
    {
      load_db_opt_by_name(thd, db_name->str, &create);
      if (store_schema_schemata(thd, table, db_name, create.default_table_charset, create.schema_comment))
        DBUG_RETURN(1);
    }
  }
  DBUG_RETURN(0);
...
static int get_schema_tables_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, const LEX_CSTRING *db_name,
                                    const LEX_CSTRING *table_name)
{
  ...
  else
  {
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
    if (share->db_type() == partition_hton && share->partition_info_str_len)
    {
    ...
    */
    if (share->transactional != HA_CHOICE_UNDEF ||
        (share->db_type() && share->db_type()->flags & HTON_TRANSACTIONAL_AND_NON_TRANSACTIONAL && file))
    {
      uint choice = share->transactional;
      if (choice == HA_CHOICE_UNDEF)
        choice =
            ((file->ha_table_flags() & (HA_NO_TRANSACTIONS | HA_CRASH_SAFE)) == HA_NO_TRANSACTIONS ? HA_CHOICE_NO
                                                                                                   : HA_CHOICE_YES);
    ...
    if (file)
    {
      ...
      if (unlikely((info_error = file->info(HA_STATUS_VARIABLE | HA_STATUS_TIME | HA_STATUS_VARIABLE_EXTRA |
                                            HA_STATUS_AUTO)) != 0))
      {
      ...
      {
        table->field[10]->store((longlong)file->stats.max_data_file_length, TRUE);
        table->field[10]->set_notnull();
      }
      table->field[11]->store((longlong)file->stats.index_file_length, TRUE);
      if (file->stats.max_index_file_length)
      {
        table->field[21]->store((longlong)file->stats.max_index_file_length, TRUE);
        table->field[21]->set_notnull();
      }
      table->field[12]->store((longlong)file->stats.delete_length, TRUE);
      if (show_table->found_next_number_field)
      {
        table->field[13]->store((longlong)file->stats.auto_increment_value, TRUE);
        table->field[13]->set_notnull();
      }
      if (file->stats.create_time)
      {
        thd->variables.time_zone->gmt_sec_to_TIME(&time, (my_time_t)file->stats.create_time);
        table->field[14]->store_time(&time);
        table->field[14]->set_notnull();
      }
      if (file->stats.update_time)
      {
        thd->variables.time_zone->gmt_sec_to_TIME(&time, (my_time_t)file->stats.update_time);
        table->field[15]->store_time(&time);
        table->field[15]->set_notnull();
      }
      if (file->stats.check_time)
      {
        thd->variables.time_zone->gmt_sec_to_TIME(&time, (my_time_t)file->stats.check_time);
        table->field[16]->store_time(&time);
        table->field[16]->set_notnull();
      }
      if ((file->ha_table_flags() & (HA_HAS_OLD_CHECKSUM | HA_HAS_NEW_CHECKSUM)) && !file->stats.checksum_null)
      {
...
err:
  if (unlikely(res || info_error))
  {
    ...
    */
    ...
    push_warning(thd, Sql_condition::WARN_LEVEL_WARN, thd->get_stmt_da()->sql_errno(), error);
    thd->clear_error();
  }
...
static void store_column_type(TABLE *table, Field *field, CHARSET_INFO *cs, uint offset)
{
  const char *tmp_buff;
  ...
  */
  ...
  table->field[offset]->store(column_type.ptr(),
                              (tmp_buff ? (uint)(tmp_buff - column_type.ptr()) : column_type.length()), cs);

  Information_schema_character_attributes cattr = field->information_schema_character_attributes();
  ...
  */
  ...
  Information_schema_numeric_attributes num = field->information_schema_numeric_attributes();
...
*/
static bool print_anchor_data_type(const Spvar_definition *def, String *data_type)
{
...
*/
static bool print_anchor_dtd_identifier(THD *thd, const Spvar_definition *def, String *dtd_identifier)
{
  if (def->column_type_ref())
    return (thd->variables.sql_mode & MODE_ORACLE) ? def->column_type_ref()->append_to(thd, dtd_identifier) ||
  ...
  if (def->is_table_rowtype_ref())
    return (thd->variables.sql_mode & MODE_ORACLE) ? def->table_rowtype_ref()->append_to(thd, dtd_identifier) ||
...
*/
static void store_variable_type(THD *thd, const sp_variable *spvar, TABLE *tmptbl, TABLE_SHARE *tmpshare,
                                CHARSET_INFO *cs, TABLE *table, uint offset)
{
  if (spvar->field_def.is_explicit_data_type())
  {
    ...
    else
    {
      ...
      Field *field = spvar->field_def.make_field(tmpshare, thd->mem_root, &spvar->name);
      field->table = tmptbl;
      tmptbl->in_use = thd;
  ...
  else
  {
    ...
    else
    {
      ...
      table->field[offset + 8]->store(dtd_identifier.ptr(), dtd_identifier.length(), cs);
      table->field[offset + 8]->set_notnull();
    }
  }
}

static int get_schema_column_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, const LEX_CSTRING *db_name,
                                    const LEX_CSTRING *table_name)
{
  LEX *lex = thd->lex;
  ...
  for (; (field = *ptr); ptr++)
  {
    ...
    check_access(thd, SELECT_ACL, db_name->str, &tables->grant.privilege, 0, 0, MY_TEST(tables->schema_table));
    col_access = get_column_grant(thd, &tables->grant, db_name->str, table_name->str, field->field_name.str) & COL_ACLS;
    ...
    table->field[1]->store(db_name->str, db_name->length, cs);
    table->field[2]->store(table_name->str, table_name->length, cs);
    table->field[3]->store(field->field_name.str, field->field_name.length, cs);
    ...
    pos = (uchar *)((field->flags & PRI_KEY_FLAG)        ? "PRI"
                    ...
                    : (field->flags & MULTIPLE_KEY_FLAG) ? "MUL"
                                                         : "");
    table->field[16]->store((const char *)pos, strlen((const char *)pos), cs);
...
int fill_schema_charsets(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (cs = all_charsets; cs < all_charsets + array_elements(all_charsets); cs++)
  {
    ...
    if (tmp_cs && (tmp_cs->state & MY_CS_PRIMARY) && (tmp_cs->state & MY_CS_AVAILABLE) &&
        !(tmp_cs->state & MY_CS_HIDDEN) && !(wild && wild[0] && wild_case_compare(scs, tmp_cs->cs_name.str, wild)))
    {
      const char *comment;
      restore_record(table, s->default_values);
...
static my_bool iter_schema_engines(THD *thd, plugin_ref plugin, void *ptable)
{
  ...
  if (!(hton->flags & HTON_HIDDEN))
  {
    ...
    {
      LEX_CSTRING yesno[2] = {{STRING_WITH_LEN("NO")}, {STRING_WITH_LEN("YES")}};
      LEX_CSTRING *tmp;
      const char *option_name = default_type != hton ? yesno[1].str : "DEFAULT";
      restore_record(table, s->default_values);

      table->field[0]->store(name->str, name->length, scs);
      table->field[1]->store(option_name, strlen(option_name), scs);
      table->field[2]->store(plugin_decl(plugin)->descr, strlen(plugin_decl(plugin)->descr), scs);
...
int fill_schema_engines(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  if (plugin_foreach_with_mask(thd, iter_schema_engines, MYSQL_STORAGE_ENGINE_PLUGIN,
                               ~(PLUGIN_IS_FREED | PLUGIN_IS_DYING), tables->table))
    DBUG_RETURN(1);
  DBUG_RETURN(0);
}
...
int fill_schema_collation(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (cs = all_charsets; cs < all_charsets + array_elements(all_charsets); cs++)
  {
    CHARSET_INFO **cl;
    CHARSET_INFO *tmp_cs = cs[0];
    if (!tmp_cs || !(tmp_cs->state & MY_CS_AVAILABLE) || (tmp_cs->state & MY_CS_HIDDEN) ||
        !(tmp_cs->state & MY_CS_PRIMARY))
      continue;
    for (cl = all_charsets; cl < all_charsets + array_elements(all_charsets); cl++)
    {
      CHARSET_INFO *tmp_cl = cl[0];
      if (!tmp_cl || !(tmp_cl->state & MY_CS_AVAILABLE) || !my_charset_same(tmp_cs, tmp_cl))
        continue;
      if (!(wild && wild[0] && wild_case_compare(scs, tmp_cl->coll_name.str, wild)))
      {
        LEX_CSTRING context_collation_name = tmp_cl->get_collation_name(MY_COLLATION_NAME_MODE_CONTEXT);
        LEX_CSTRING full_collation_name = tmp_cl->get_collation_name(MY_COLLATION_NAME_MODE_FULL);
        ...
        */
        if (is_context && cmp(tmp_cl->cs_name, Lex_cstring(STRING_WITH_LEN("utf8mb4"))))
          continue;
        restore_record(table, s->default_values);
        ...
        else
        {
          ...
          table->field[3]->store(Show::Yes_or_empty::value(tmp_cl->default_flag()), scs);
        }
        table->field[4]->store(Show::Yes_or_empty::value(tmp_cl->compiled_flag()), scs);
...
int fill_schema_coll_charset_app(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (cs = all_charsets; cs < all_charsets + array_elements(all_charsets); cs++)
  {
    ...
    {
      CHARSET_INFO *tmp_cl = cl[0];
      if (!tmp_cl || !(tmp_cl->state & MY_CS_AVAILABLE) || (tmp_cl->state & MY_CS_HIDDEN) ||
      ...
      table->field[4]->store(Show::Yes_or_empty::value(tmp_cl->default_flag()), scs);
      if (schema_table_store_record(thd, table))
        return 1;
    }
  }
...
int check_proc_record(const CHARSET_INFO *name_field_charset, const LOOKUP_FIELD_VALUES *lookup, const LEX_CSTRING &db,
                      const LEX_CSTRING &name)
{
  ...
  {
    if ((my_ci_strnncoll(name_field_charset, (const uchar *)lookup->table_value.str, lookup->table_value.length,
...
int store_schema_params(THD *thd, TABLE *table, TABLE *proc_table, LOOKUP_FIELD_VALUES *lookup, bool full_access,
                        const char *sp_user)
{
  TABLE_SHARE share;
  TABLE tbl;
  ...
  sph = Sp_handler::handler_mysql_proc((enum_sp_type)proc_table->field[MYSQL_PROC_MYSQL_TYPE]->val_int());
  if (!sph || sph->type() == SP_TYPE_PACKAGE || sph->type() == SP_TYPE_PACKAGE_BODY)
    DBUG_RETURN(0);

  if (!full_access)
    full_access = !strcmp(sp_user, definer.str);
  if (!full_access && check_some_routine_access(thd, db.str, name.str, sph))
    DBUG_RETURN(0);

  proc_table->field[MYSQL_PROC_FIELD_PARAM_LIST]->val_str_nopad(thd->mem_root, &params);
  if (sph->type() == SP_TYPE_FUNCTION)
    proc_table->field[MYSQL_PROC_FIELD_RETURNS]->val_str_nopad(thd->mem_root, &returns);
  sp = sph->sp_load_for_information_schema(thd, proc_table, db, name, params, returns, sql_mode, &free_sp_head);
  if (sp)
  {
    Field *field;
    ...
    if (sph->type() == SP_TYPE_FUNCTION)
    {
      ...
      proc_table->field[MYSQL_PROC_MYSQL_TYPE]->val_str_nopad(thd->mem_root, &tmp_string);
      table->field[15]->store(tmp_string, cs);
      field = sp->m_return_field_def.make_field(&share, thd->mem_root, &empty_clex_str);
      field->table = &tbl;
      tbl.in_use = thd;
...
int store_schema_proc(THD *thd, TABLE *table, TABLE *proc_table, LOOKUP_FIELD_VALUES *lookup, bool full_access,
                      const char *sp_user)
{
  LEX *lex = thd->lex;
  CHARSET_INFO *cs = system_charset_info;
  ...
  if (!is_show_command(thd) || sph == Sp_handler::handler(lex->sql_command))
  {
    restore_record(table, s->default_values);
    if (!wild || !wild[0] || !wild_case_compare(system_charset_info, name.str, wild))
    {
      int enum_idx = (int)proc_table->field[MYSQL_PROC_FIELD_ACCESS]->val_int();
      table->field[3]->store(name, cs);

      copy_field_as_string(table->field[0], proc_table->field[MYSQL_PROC_FIELD_SPECIFIC_NAME]);
      table->field[1]->store(STRING_WITH_LEN("def"), cs);
      table->field[2]->store(db, cs);
      copy_field_as_string(table->field[4], proc_table->field[MYSQL_PROC_MYSQL_TYPE]);
      ...
      if (sph->type() == SP_TYPE_FUNCTION)
      {
        ...
        bool free_sp_head;
        proc_table->field[MYSQL_PROC_FIELD_RETURNS]->val_str_nopad(thd->mem_root, &returns);
        sp = sph->sp_load_for_information_schema(thd, proc_table, db, name, empty_clex_str /*params*/, returns,
                                                 (ulong)proc_table->field[MYSQL_PROC_FIELD_SQL_MODE]->val_int(),
                                                 &free_sp_head);
        if (sp)
        {
          char path[FN_REFLEN];
      ...
      if (full_access)
      {
        copy_field_as_string(table->field[15], proc_table->field[MYSQL_PROC_FIELD_BODY_UTF8]);
        table->field[15]->set_notnull();
      }
      table->field[14]->store(STRING_WITH_LEN("SQL"), cs);
      table->field[18]->store(STRING_WITH_LEN("SQL"), cs);
      copy_field_as_string(table->field[19], proc_table->field[MYSQL_PROC_FIELD_DETERMINISTIC]);
      table->field[20]->store(sp_data_access_name[enum_idx].str, sp_data_access_name[enum_idx].length, cs);
      copy_field_as_string(table->field[22], proc_table->field[MYSQL_PROC_FIELD_SECURITY_TYPE]);

      proc_table->field[MYSQL_PROC_FIELD_CREATED]->save_in_field(table->field[23]);
      proc_table->field[MYSQL_PROC_FIELD_MODIFIED]->save_in_field(table->field[24]);

      copy_field_as_string(table->field[25], proc_table->field[MYSQL_PROC_FIELD_SQL_MODE]);
      copy_field_as_string(table->field[26], proc_table->field[MYSQL_PROC_FIELD_COMMENT]);

      table->field[27]->store(definer, cs);
      copy_field_as_string(table->field[28], proc_table->field[MYSQL_PROC_FIELD_CHARACTER_SET_CLIENT]);
      copy_field_as_string(table->field[29], proc_table->field[MYSQL_PROC_FIELD_COLLATION_CONNECTION]);
      copy_field_as_string(table->field[30], proc_table->field[MYSQL_PROC_FIELD_DB_COLLATION]);
...
int fill_schema_proc(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  enum enum_schema_tables schema_table_idx = get_schema_table_idx(tables->schema_table);
  ...
  full_access = !check_table_access(thd, SELECT_ACL, &proc_tables, FALSE, 1, TRUE);
  ...
  if (lookup.db_value.str)
  {
    ...
    proc_table->field[0]->store(lookup.db_value.str, lookup.db_value.length, system_charset_info);
    if (lookup.table_value.str)
    {
      proc_table->field[1]->store(lookup.table_value.str, lookup.table_value.length, system_charset_info);
    ...
    res =
        proc_table->file->ha_index_read_map(proc_table->record[0], (const uchar *)keybuf.ptr(), keypart_map, find_flag);
  }
  else
  ...
  res = schema_table_idx == SCH_PROCEDURES ? store_schema_proc(thd, table, proc_table, &lookup, full_access, definer)
...
static int get_schema_stat_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, const LEX_CSTRING *db_name,
                                  const LEX_CSTRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  ...
  if (res)
  {
    if (thd->lex->sql_command != SQLCOM_SHOW_KEYS)
    {
      ...
      */
      if (unlikely(thd->is_error()))
        push_warning(thd, Sql_condition::WARN_LEVEL_WARN, thd->get_stmt_da()->sql_errno(),
  ...
  else if (!tables->view)
  {
    ...
    if (show_table->file)
    {
      ...
      show_table->file->info(HA_STATUS_VARIABLE | HA_STATUS_NO_LOCK | HA_STATUS_CONST | HA_STATUS_TIME);
      set_statistics_for_table(thd, show_table);
    }
    for (uint i = 0; i < show_table->s->keys; i++, key_info++)
    {
      if ((key_info->flags & HA_INVISIBLE_KEY) && !DBUG_IF("test_invisible_index"))
        continue;
      KEY_PART_INFO *key_part = key_info->key_part;
      LEX_CSTRING *str;
      ...
      {
        if (key_part->field->invisible >= INVISIBLE_SYSTEM && !DBUG_IF("test_completely_invisible"))
        {
          /*
        ...
        if (show_table->file)
        {
          ...
          else
          {
            ...
            */
            ...
            {
              ha_rows records = (ha_rows)((double)show_table->stat_records() / key_info->actual_rec_per_key(j));
        ...
        if (!(key_info->flags & HA_FULLTEXT) &&
            (key_part->field && key_part->length != show_table->s->field[key_part->fieldnr - 1]->key_length()))
        {
          table->field[10]->store((longlong)key_part->length / key_part->field->charset()->mbmaxlen, TRUE);
          table->field[10]->set_notnull();
        }
        ...
        DBUG_ASSERT(MY_TEST(key_info->flags & HA_USES_COMMENT) == (key_info->comment.length > 0));
        if (key_info->flags & HA_USES_COMMENT)
          table->field[15]->store(key_info->comment.str, key_info->comment.length, cs);
...
static int get_schema_views_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, const LEX_CSTRING *db_name,
                                   const LEX_CSTRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  char definer[USER_HOST_BUFF_SIZE];
  ...
  if (tables->view)
  {
    ...
    {
      if (!my_strcasecmp(system_charset_info, tables->definer.user.str, sctx->priv_user) &&
          !my_strcasecmp(system_charset_info, tables->definer.host.str, sctx->priv_host))
        tables->allowed_show = TRUE;
#ifndef NO_EMBEDDED_ACCESS_CHECKS
      else
      {
        if ((thd->col_access & (SHOW_VIEW_ACL | SELECT_ACL)) == (SHOW_VIEW_ACL | SELECT_ACL))
          tables->allowed_show = TRUE;
        else
        {
          TABLE_LIST table_list;
          ...
          if ((view_access & (SHOW_VIEW_ACL | SELECT_ACL)) == (SHOW_VIEW_ACL | SELECT_ACL))
            tables->allowed_show = TRUE;
        }
      }
#endif
    ...
    if (tables->allowed_show)
    {
      table->field[3]->store(tables->view_body_utf8.str, tables->view_body_utf8.length, cs);
    }
    ...
    */
    if (res == FALSE && table->pos_in_table_list->table_open_method & OPEN_FULL_TABLE)
    {
      updatable_view = 0;
    ...
    definer_len = (uint)(strxmov(definer, tables->definer.user.str, "@", tables->definer.host.str, NullS) - definer);
    ...
    table->field[8]->store(&tables->view_creation_ctx->get_client_cs()->cs_name, cs);
    table->field[9]->store(&tables->view_creation_ctx->get_connection_cl()->coll_name, cs);
    table->field[10]->store(view_algorithm(tables), cs);

    if (schema_table_store_record(thd, table))
      DBUG_RETURN(1);
    if (unlikely(res && thd->is_error()))
      push_warning(thd, Sql_condition::WARN_LEVEL_WARN, thd->get_stmt_da()->sql_errno(), thd->get_stmt_da()->message());
  }
  if (res)
    thd->clear_error();
  DBUG_RETURN(0);
...
static bool store_constraints(THD *thd, TABLE *table, const LEX_CSTRING *db_name, const LEX_CSTRING *table_name,
                              const char *key_name, size_t key_len, const char *con_type, size_t con_len)
{
  CHARSET_INFO *cs = system_charset_info;
  restore_record(table, s->default_values);
  table->field[0]->store(STRING_WITH_LEN("def"), cs);
...
static int get_check_constraints_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res,
                                        const LEX_CSTRING *db_name, const LEX_CSTRING *table_name)
{
  ...
  if (!tables->view)
  {
    ...
    for (uint i = 0; i < tables->table->s->table_check_constraints; i++)
    {
      ...
      table->field[0]->store(STRING_WITH_LEN("def"), system_charset_info);
      table->field[3]->store(check->name.str, check->name.length, system_charset_info);
      const char *tmp_buff;
      tmp_buff = (check->get_vcol_type() == VCOL_CHECK_FIELD ? "Column" : "Table");
...
static int get_schema_constraints_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res,
                                         const LEX_CSTRING *db_name, const LEX_CSTRING *table_name)
{
  ...
  else if (!tables->view)
  {
    ...
    for (uint i = 0; i < show_table->s->keys; i++, key_info++)
    {
      ...
      if (i == primary_key && !strcmp(key_info->name.str, primary_key_name.str))
      {
        if (store_constraints(thd, table, db_name, table_name, key_info->name.str, key_info->name.length,
    ...
    for (uint i = 0; i < show_table->s->table_check_constraints; i++)
    {
      ...
      if (store_constraints(thd, table, db_name, table_name, check->name.str, check->name.length,
    ...
    while ((f_key_info = it++))
    {
      if (store_constraints(thd, table, db_name, table_name, f_key_info->foreign_id->str,
                            strlen(f_key_info->foreign_id->str), "FOREIGN KEY", 11))
        DBUG_RETURN(1);
    }
  }
  DBUG_RETURN(res);
...
static bool store_trigger(THD *thd, Trigger *trigger, TABLE *table, const LEX_CSTRING *db_name,
                          const LEX_CSTRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  ...
  table->field[3]->store(trg_event_type_names[trigger->event].str, trg_event_type_names[trigger->event].length, cs);
  ...
  if (trigger->hr_create_time.val)
  {
    ...
    table->field[16]->set_notnull();
    thd->variables.time_zone->gmt_sec_to_TIME(&timestamp, (my_time_t)hrtime_to_time(trigger->hr_create_time));
...
static int get_schema_triggers_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, const LEX_CSTRING *db_name,
                                      const LEX_CSTRING *table_name)
{
  ...
  */
  ...
  if (!tables->view && tables->table->triggers)
  {
    ...
    for (event = 0; event < (int)TRG_EVENT_MAX; event++)
    {
      ...
      {
        Trigger *trigger;
        for (trigger = triggers->get_trigger((enum trg_event_type)event, (enum trg_action_time_type)timing); trigger;
...
static void store_key_column_usage(TABLE *table, const LEX_CSTRING *db_name, const LEX_CSTRING *table_name,
                                   const char *key_name, size_t key_len, const char *con_type, size_t con_len,
                                   longlong idx)
{
  CHARSET_INFO *cs = system_charset_info;
...
static int get_schema_key_column_usage_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res,
                                              const LEX_CSTRING *db_name, const LEX_CSTRING *table_name)
{
  ...
  else if (!tables->view)
  {
    ...
    for (uint i = 0; i < show_table->s->keys; i++, key_info++)
    {
      ...
      for (uint j = 0; j < key_info->user_defined_key_parts; j++, key_part++)
      {
        if (key_part->field)
        {
          f_idx++;
          restore_record(table, s->default_values);
          store_key_column_usage(table, db_name, table_name, key_info->name.str, key_info->name.length,
                                 key_part->field->field_name.str, key_part->field->field_name.length, (longlong)f_idx);
          if (schema_table_store_record(thd, table))
            DBUG_RETURN(1);
        }
      }
    ...
    while ((f_key_info = fkey_it++))
    {
      LEX_CSTRING *f_info;
      LEX_CSTRING *r_info;
      List_iterator_fast<LEX_CSTRING> it(f_key_info->foreign_fields), it1(f_key_info->referenced_fields);
      uint f_idx = 0;
      while ((f_info = it++))
      {
        r_info = it1++;
        f_idx++;
        restore_record(table, s->default_values);
        store_key_column_usage(table, db_name, table_name, f_key_info->foreign_id->str, f_key_info->foreign_id->length,
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
static void collect_partition_expr(THD *thd, List<const char> &field_list, String *str)
{
...
static void store_schema_partitions_record(THD *thd, TABLE *schema_table, TABLE *showing_table,
                                           partition_element *part_elem, handler *file, uint part_id)
{
  ...
  {
    thd->variables.time_zone->gmt_sec_to_TIME(&time, (my_time_t)stat_info.create_time);
    table->field[18]->store_time(&time);
    table->field[18]->set_notnull();
  }
  if (stat_info.update_time)
  {
    thd->variables.time_zone->gmt_sec_to_TIME(&time, (my_time_t)stat_info.update_time);
    table->field[19]->store_time(&time);
    table->field[19]->set_notnull();
  }
  if (stat_info.check_time)
  {
    thd->variables.time_zone->gmt_sec_to_TIME(&time, (my_time_t)stat_info.check_time);
  ...
  if (part_elem)
  {
    if (part_elem->part_comment)
      table->field[22]->store(part_elem->part_comment, strlen(part_elem->part_comment), cs);
    else
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
static int get_partition_column_description(THD *thd, partition_info *part_info, part_elem_value *list_value,
                                            String &tmp_str)
{
  ...
  for (i = 0; i < num_elements; i++)
  {
    ...
    else
    {
      ...
      const Type_handler *th = field->type_handler();
      th->partition_field_append_value(&val, item, field->charset(), PARTITION_VALUE_PRINT_MODE_SHOW);
      tmp_str.append(val);
    }
    if (i != num_elements - 1)
      tmp_str.append(',');
...
static int get_schema_partitions_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res,
                                        const LEX_CSTRING *db_name, const LEX_CSTRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  char buff[61];
  ...
  if (part_info)
  {
    ...
    {
      table->field[3]->store(part_elem->partition_name, strlen(part_elem->partition_name), cs);
      table->field[3]->set_notnull();
      ...
      if (part_info->part_type == RANGE_PARTITION)
      {
        if (part_info->column_list)
        {
          ...
          tmp_str.length(0);
          if (get_partition_column_description(thd, part_info, list_value, tmp_str))
            DBUG_RETURN(1);
      ...
      if (part_elem->subpartitions.elements)
      {
        ...
        {
          table->field[4]->store(subpart_elem->partition_name, strlen(subpart_elem->partition_name), cs);
          table->field[4]->set_notnull();
          /* SUBPARTITION_ORDINAL_POSITION */
          table->field[6]->store((longlong)++subpart_pos, TRUE);
          table->field[6]->set_notnull();

          store_schema_partitions_record(thd, table, show_table, subpart_elem, file, part_id);
          part_id++;
          if (schema_table_store_record(thd, table))
            DBUG_RETURN(1);
        }
      ...
      {
        store_schema_partitions_record(thd, table, show_table, part_elem, file, part_id);
        part_id++;
        if (schema_table_store_record(thd, table))
          DBUG_RETURN(1);
      }
...
int copy_event_to_schema_table(THD *thd, TABLE *sch_table, TABLE *event_table)
{
  ...
  /*
    ...
    has access.
  */
  if (thd->lex->sql_command != SQLCOM_SHOW_EVENTS && check_access(thd, EVENT_ACL, et.dbname.str, NULL, NULL, 0, 1))
    DBUG_RETURN(0);
  ...
  sch_table->field[ISE_EVENT_DEFINITION]->store(et.body_utf8.str, et.body_utf8.length, scs);
  ...
  if (et.expression)
  {
    ...
    if (Events::reconstruct_interval_expression(&show_str, et.interval, et.expression))
      DBUG_RETURN(1);
  ...
  switch (et.status)
  {
    ...
    case Event_parse_data::SLAVESIDE_DISABLED:
      sch_table->field[ISE_STATUS]->store(STRING_WITH_LEN("SLAVESIDE_DISABLED"), scs);
      break;
  ...
  sch_table->field[ISE_CLIENT_CS]->store(&et.creation_ctx->get_client_cs()->cs_name, scs);
  sch_table->field[ISE_CONNECTION_CL]->set_notnull();
  sch_table->field[ISE_CONNECTION_CL]->store(&et.creation_ctx->get_connection_cl()->coll_name, scs);
  sch_table->field[ISE_DB_CL]->set_notnull();
  sch_table->field[ISE_DB_CL]->store(&et.creation_ctx->get_db_cl()->coll_name, scs);
...
int fill_open_tables(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  if (!(open_list = list_open_tables(thd, thd->lex->first_select_lex()->db.str, wild)) && thd->is_fatal_error)
    DBUG_RETURN(1);
...
int fill_variables(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  if ((sorted_vars && lex->option_type == OPT_GLOBAL) || schema_table_idx == SCH_GLOBAL_VARIABLES)
    scope = OPT_GLOBAL;
  ...
  */
  if (scope == OPT_SESSION && (!thd->variables.dynamic_variables_ptr ||
                               global_system_variables.dynamic_variables_head > thd->variables.dynamic_variables_head))
    sync_dynamic_session_variables(thd, true);

  res = show_status_array(thd, wild, enumerate_sys_vars(thd, sorted_vars, scope), scope, NULL, "", tables->table,
...
int fill_i_s_sql_functions(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (uint i = 0; i < native_func_registry_array.count(); i++)
    if (add_symbol_to_table(native_func_registry_array.element(i).name.str, table))
      DBUG_RETURN(1);
...
int fill_status(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  res = show_status_array(thd, wild, (SHOW_VAR *)all_status_vars.buffer, scope, tmp1, "", tables->table,
                          upper_case_names, partial_cond);
  mysql_rwlock_unlock(&LOCK_all_status_vars);
  DBUG_RETURN(res);
}
...
static int get_referential_constraints_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res,
                                              const LEX_CSTRING *db_name, const LEX_CSTRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  LEX_CSTRING *s;
  ...
  if (!tables->view)
  {
    ...
    while ((f_key_info = it++))
    {
      ...
      table->field[2]->store(f_key_info->foreign_id->str, f_key_info->foreign_id->length, cs);
      table->field[3]->store(STRING_WITH_LEN("def"), cs);
      table->field[4]->store(f_key_info->referenced_db->str, f_key_info->referenced_db->length, cs);
      table->field[10]->store(f_key_info->referenced_table->str, f_key_info->referenced_table->length, cs);
      if (f_key_info->referenced_key_name)
      {
        table->field[5]->store(f_key_info->referenced_key_name->str, f_key_info->referenced_key_name->length, cs);
        table->field[5]->set_notnull();
      }
      else
        table->field[5]->set_null();
...
*/
static my_bool find_schema_table_in_plugin(THD *thd, plugin_ref plugin, void *p_table)
{
...
ST_SCHEMA_TABLE *find_schema_table(THD *thd, const LEX_CSTRING *table_name, bool *in_plugin)
{
  ...
  *in_plugin = false;
  ...
  {
    if (!my_strcasecmp(system_charset_info, schema_table->table_name, table_name->str))
      DBUG_RETURN(schema_table);
  }

  *in_plugin = true;
  schema_table_a.table_name = table_name->str;
  if (plugin_foreach(thd, find_schema_table_in_plugin, MYSQL_INFORMATION_SCHEMA_PLUGIN, &schema_table_a))
...
TABLE *create_schema_table(THD *thd, TABLE_LIST *table_list)
{
  ...
  for (; !fields->end_marker(); fields++) field_count++;
  ...
  if (!(table = create_tmp_table_for_schema(thd, tmp_table_param, *schema_table,
    ...
    DBUG_RETURN(0);
  my_bitmap_map *bitmaps = (my_bitmap_map *)thd->alloc(bitmap_buffer_size(field_count));
...
int make_schemata_old_format(THD *thd, ST_SCHEMA_TABLE *schema_table)
{
  ...
  if (!sel->item_list.elements)
  {
    ...
    Item_field *field = new (thd->mem_root) Item_field(thd, context, field_info->name());
...
int make_columns_old_format(THD *thd, ST_SCHEMA_TABLE *schema_table)
{
  ...
  for (; *field_num >= 0; field_num++)
  {
    ...
    if (!thd->lex->verbose && (*field_num == 14 || *field_num == 18 || *field_num == 19))
      continue;
    Item_field *field = new (thd->mem_root) Item_field(thd, context, field_info->name());
    if (field)
    {
  ...
  */
  if (table_list->schema_table_name.str)
    table->alias_name_used =
        my_strcasecmp(table_alias_charset, table_list->schema_table_name.str, table_list->alias.str);
...
int make_schema_select(THD *thd, SELECT_LEX *sel, ST_SCHEMA_TABLE *schema_table)
{
  ...
  */
  if (!thd->make_lex_string(&db, INFORMATION_SCHEMA_NAME.str, INFORMATION_SCHEMA_NAME.length))
    DBUG_RETURN(1);

  if (!thd->make_lex_string(&table, schema_table->table_name, strlen(schema_table->table_name)))
    DBUG_RETURN(1);

  if (schema_table->old_format(thd, schema_table))
    DBUG_RETURN(1);

  if (!sel->add_table_to_list(thd, new Table_ident(thd, &db, &table, 0), 0, 0, TL_READ, MDL_SHARED_READ))
    DBUG_RETURN(1);
...
static bool optimize_for_get_all_tables(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  schema_table_idx = get_schema_table_idx(schema_table);
  tables->table_open_method = get_table_open_method(tables, schema_table, schema_table_idx);
  ...
  */
  ...
  if (get_lookup_field_values(thd, cond, true, tables, &plan->lookup_field_vals))
  {
    plan->no_rows = true;
    goto end;
  }

  DBUG_PRINT("info", ("db_name='%s', table_name='%s'", plan->lookup_field_vals.db_value.str,
                      plan->lookup_field_vals.table_value.str));

  if (!plan->lookup_field_vals.wild_db_value && !plan->lookup_field_vals.wild_table_value)
  {
    /*
      if lookup value is empty string then
      it's impossible table name or db name
    */
    if ((plan->lookup_field_vals.db_value.str && !plan->lookup_field_vals.db_value.str[0]) ||
        (plan->lookup_field_vals.table_value.str && !plan->lookup_field_vals.table_value.str[0]))
    {
      plan->no_rows = true;
      goto end;
    }
  ...
  while (TABLE_LIST *table_list = tli++)
  {
    ...
    if (!table->is_created())
    {
      ...
      for (uint i = 0; i < table->s->fields; i++, from_recinfo++)
      {
        ...
        {
          field = new (thd->mem_root) Field_string(cur, 0, field->null_ptr, field->null_bit, Field::NONE,
...
bool optimize_schema_tables_reads(JOIN *join)
{
  ...
  JOIN_TAB *tab;
  for (tab = first_linear_tab(join, WITHOUT_BUSH_ROOTS, WITH_CONST_TABLES); tab;
...
bool get_schema_tables_result(JOIN *join, enum enum_schema_table_state executed_place)
{
    ...
    if (table_list->schema_table && thd->fill_information_schema_tables())
    {
      ...
      */
      bool is_subselect = &lex->unit != lex->current_select->master_unit() &&
                          lex->current_select->master_unit()->item && tab->select_cond &&
      ...
      */
      ...
      /* skip I_S optimizations specific to get_all_tables */
      if (lex->describe && (table_list->schema_table->fill_table != get_all_tables))
        continue;
      ...
      */
      if (table_list->schema_table_state && (!is_subselect || table_list->schema_table_state != executed_place))
        continue;
  ...
  if (unlikely(thd->is_error()))
  {
    ...
    */
    thd->get_stmt_da()->push_warning(thd, thd->get_stmt_da()->sql_errno(), thd->get_stmt_da()->get_sqlstate(),
                                     Sql_condition::WARN_LEVEL_ERROR, thd->get_stmt_da()->message());
  }
  else if (result)
...
static my_bool run_hton_fill_schema_table(THD *thd, plugin_ref plugin, void *arg)
{
  struct run_hton_fill_schema_table_args *args = (run_hton_fill_schema_table_args *)arg;
  handlerton *hton = plugin_hton(plugin);
  if (hton->fill_is_table)
    hton->fill_is_table(hton, thd, args->tables, args->cond, get_schema_table_idx(args->tables->schema_table));
  return false;
}
...
int hton_fill_schema_table(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  plugin_foreach(thd, run_hton_fill_schema_table, MYSQL_STORAGE_ENGINE_PLUGIN, &args);

  DBUG_RETURN(0);
}

static int store_key_cache_table_record(THD *thd, TABLE *table, const char *name, size_t name_length,
                                        KEY_CACHE *key_cache, uint partitions, uint partition_no)
{
...
int run_fill_key_cache_tables(const char *name, KEY_CACHE *key_cache, void *p)
{
  ...
  if (partitions)
  {
    ...
    {
      if (store_key_cache_table_record(thd, table, name, namelen, key_cache, partitions, i + 1))
        DBUG_RETURN(1);
    }
  }

  if (store_key_cache_table_record(thd, table, name, namelen, key_cache, partitions, 0))
    DBUG_RETURN(1);
  DBUG_RETURN(0);
}
...
ST_FIELD_INFO schema_fields_info[] = {Column("CATALOG_NAME", Catalog(), NOT_NULL),
...
ST_FIELD_INFO tables_fields_info[] = {
    ...
    Column("AVG_ROW_LENGTH", ULonglong(), NULLABLE, "Avg_row_length", OPEN_FULL_TABLE),
    Column("DATA_LENGTH", ULonglong(), NULLABLE, "Data_length", OPEN_FULL_TABLE),
    Column("MAX_DATA_LENGTH", ULonglong(), NULLABLE, "Max_data_length", OPEN_FULL_TABLE),
    ...
    Column("AUTO_INCREMENT", ULonglong(), NULLABLE, "Auto_increment", OPEN_FULL_TABLE),
    ...
    Column("CREATE_OPTIONS", Varchar(2048), NULLABLE, "Create_options", OPEN_FULL_TABLE),
    Column("TABLE_COMMENT", Varchar(TABLE_COMMENT_MAXLEN), NOT_NULL, "Comment", OPEN_FRM_ONLY),
    Column("MAX_INDEX_LENGTH", ULonglong(), NULLABLE, "Max_index_length", OPEN_FULL_TABLE),
...
ST_FIELD_INFO columns_fields_info[] = {
    ...
    Column("COLUMN_DEFAULT", Longtext(MAX_FIELD_VARCHARLENGTH), NULLABLE, "Default", OPEN_FRM_ONLY),
    ...
    Column("COLUMN_COMMENT", Varchar(COLUMN_COMMENT_MAXLEN), NOT_NULL, "Comment", OPEN_FRM_ONLY),
    Column("IS_GENERATED", Varchar(6), NOT_NULL, OPEN_FRM_ONLY),
    Column("GENERATION_EXPRESSION", Longtext(MAX_FIELD_VARCHARLENGTH), NULLABLE, OPEN_FRM_ONLY),
    CEnd()};

ST_FIELD_INFO charsets_fields_info[] = {Column("CHARACTER_SET_NAME", CSName(), NOT_NULL, "Charset"),
                                        Column("DEFAULT_COLLATE_NAME", CLName(), NOT_NULL, "Default collation"),
                                        Column("DESCRIPTION", Varchar(60), NOT_NULL, "Description"),
                                        Column("MAXLEN", SLonglong(3), NOT_NULL, "Maxlen"), CEnd()};

ST_FIELD_INFO collation_fields_info[] = {Column("COLLATION_NAME", CLName(), NOT_NULL, "Collation"),
...
ST_FIELD_INFO applicable_roles_fields_info[] = {
    Column("GRANTEE", Userhost(), NOT_NULL), Column("ROLE_NAME", Varchar(USERNAME_CHAR_LENGTH), NOT_NULL),
    Column("IS_GRANTABLE", Yes_or_empty(), NOT_NULL), Column("IS_DEFAULT", Yes_or_empty(), NULLABLE), CEnd()};
...
ST_FIELD_INFO engines_fields_info[] = {Column("ENGINE", Varchar(64), NOT_NULL, "Engine"),
...
ST_FIELD_INFO coll_charset_app_fields_info[] = {
    Column("COLLATION_NAME", CLName(), NOT_NULL),      Column("CHARACTER_SET_NAME", CSName(), NOT_NULL),
    Column("FULL_COLLATION_NAME", CLName(), NOT_NULL), Column("ID", SLonglong(MY_INT32_NUM_DECIMAL_DIGITS), NOT_NULL),
    Column("IS_DEFAULT", Yes_or_empty(), NOT_NULL),    CEnd()};

ST_FIELD_INFO proc_fields_info[] = {Column("SPECIFIC_NAME", Name(), NOT_NULL),
...
ST_FIELD_INFO stat_fields_info[] = {
    ...
    Column("INDEX_COMMENT", Varchar(INDEX_COMMENT_MAXLEN), NOT_NULL, "Index_comment", OPEN_FRM_ONLY),
    Column("IGNORED", Varchar(3), NOT_NULL, "Ignored", OPEN_FRM_ONLY),
    CEnd()};

ST_FIELD_INFO view_fields_info[] = {Column("TABLE_CATALOG", Catalog(), NOT_NULL, OPEN_FRM_ONLY),
                                    Column("TABLE_SCHEMA", Name(), NOT_NULL, OPEN_FRM_ONLY),
                                    Column("TABLE_NAME", Name(), NOT_NULL, OPEN_FRM_ONLY),
...
ST_FIELD_INFO user_privileges_fields_info[] = {
    Column("GRANTEE", Userhost(), NOT_NULL), Column("TABLE_CATALOG", Catalog(), NOT_NULL),
    Column("PRIVILEGE_TYPE", Name(), NOT_NULL), Column("IS_GRANTABLE", Yes_or_empty(), NOT_NULL), CEnd()};

ST_FIELD_INFO schema_privileges_fields_info[] = {
    Column("GRANTEE", Userhost(), NOT_NULL),          Column("TABLE_CATALOG", Catalog(), NOT_NULL),
    Column("TABLE_SCHEMA", Name(), NOT_NULL),         Column("PRIVILEGE_TYPE", Name(), NOT_NULL),
    Column("IS_GRANTABLE", Yes_or_empty(), NOT_NULL), CEnd()};

ST_FIELD_INFO table_privileges_fields_info[] = {Column("GRANTEE", Userhost(), NOT_NULL),
                                                Column("TABLE_CATALOG", Catalog(), NOT_NULL),
                                                Column("TABLE_SCHEMA", Name(), NOT_NULL),
...
ST_FIELD_INFO column_privileges_fields_info[] = {
    Column("GRANTEE", Userhost(), NOT_NULL),          Column("TABLE_CATALOG", Catalog(), NOT_NULL),
    Column("TABLE_SCHEMA", Name(), NOT_NULL),         Column("TABLE_NAME", Name(), NOT_NULL),
    Column("COLUMN_NAME", Name(), NOT_NULL),          Column("PRIVILEGE_TYPE", Name(), NOT_NULL),
    Column("IS_GRANTABLE", Yes_or_empty(), NOT_NULL), CEnd()};

ST_FIELD_INFO table_constraints_fields_info[] = {Column("CONSTRAINT_CATALOG", Catalog(), NOT_NULL, OPEN_FULL_TABLE),
...
ST_FIELD_INFO table_names_fields_info[] = {
    Column("TABLE_CATALOG", Catalog(), NOT_NULL), Column("TABLE_SCHEMA", Name(), NOT_NULL),
    Column("TABLE_NAME", Varchar(NAME_CHAR_LEN + MYSQL50_TABLE_NAME_PREFIX_LENGTH), NOT_NULL, "Tables_in_"),
    Column("TABLE_TYPE", Name(), NOT_NULL, "Table_type", OPEN_FRM_ONLY), CEnd()};

ST_FIELD_INFO open_tables_fields_info[] = {Column("Database", Name(), NOT_NULL, "Database"),
                                           Column("Table", Name(), NOT_NULL, "Table"),
                                           Column("In_use", SLonglong(1), NOT_NULL, "In_use"),
                                           Column("Name_locked", SLonglong(4), NOT_NULL, "Name_locked"), CEnd()};
...
ST_FIELD_INFO triggers_fields_info[] = {
    ...
    Column("CHARACTER_SET_CLIENT", CSName(), NOT_NULL, "character_set_client", OPEN_FRM_ONLY),
    Column("COLLATION_CONNECTION", CLName(), NOT_NULL, "collation_connection", OPEN_FRM_ONLY),
    Column("DATABASE_COLLATION", CLName(), NOT_NULL, "Database Collation", OPEN_FRM_ONLY), CEnd()};
...
ST_FIELD_INFO variables_fields_info[] = {Column("VARIABLE_NAME", Varchar(64), NOT_NULL, "Variable_name"),
                                         Column("VARIABLE_VALUE", Varchar(2048), NOT_NULL, "Value"), CEnd()};

ST_FIELD_INFO sysvars_fields_info[] = {Column("VARIABLE_NAME", Name(), NOT_NULL),
...
ST_FIELD_INFO processlist_fields_info[] = {
    ...
    Column("INFO", Longtext(PROCESS_LIST_INFO_WIDTH), NULLABLE, "Info"),
    Column("TIME_MS", Decimal(100 * (MY_INT64_NUM_DECIMAL_DIGITS + 1) + 3), NOT_NULL, "Time_ms"),
...
ST_FIELD_INFO plugin_fields_info[] = {Column("PLUGIN_NAME", Name(), NOT_NULL, "Name"),
...
ST_FIELD_INFO files_fields_info[] = {Column("FILE_ID", SLonglong(4), NOT_NULL),
...
{
  int i;
  for (i = 0; !Show::files_fields_info[i].end_marker(); i++) table->field[i]->set_null();
...
ST_FIELD_INFO parameters_fields_info[] = {Column("SPECIFIC_CATALOG", Catalog(), NOT_NULL, OPEN_FULL_TABLE),
...
ST_FIELD_INFO tablespaces_fields_info[] = {
    Column("TABLESPACE_NAME", Name(), NOT_NULL),           Column("ENGINE", Name(), NOT_NULL),
    Column("TABLESPACE_TYPE", Name(), NULLABLE),           Column("LOGFILE_GROUP_NAME", Name(), NULLABLE),
    Column("EXTENT_SIZE", ULonglong(), NULLABLE),          Column("AUTOEXTEND_SIZE", ULonglong(), NULLABLE),
    Column("MAXIMUM_SIZE", ULonglong(), NULLABLE),         Column("NODEGROUP_ID", ULonglong(), NULLABLE),
    Column("TABLESPACE_COMMENT", Varchar(2048), NULLABLE), CEnd()};

ST_FIELD_INFO keycache_fields_info[] = {Column("KEY_CACHE_NAME", Varchar(NAME_LEN), NOT_NULL),
...
ST_FIELD_INFO show_explain_json_fields_info[] = {
    Column("EXPLAIN", Longtext(MAX_FIELD_VARCHARLENGTH), NOT_NULL, "SHOW EXPLAIN"), CEnd()};

ST_FIELD_INFO show_analyze_tabular_fields_info[] = {
    Column("id", SLonglong(3), NULLABLE, "id"), Column("select_type", Varchar(19), NOT_NULL, "select_type"),
    Column("table", Name(), NULLABLE, "table"), Column("type", Varchar(15), NULLABLE, "type"),
    Column("possible_keys", Varchar(NAME_CHAR_LEN *MAX_KEY), NULLABLE, "possible_keys"),
    Column("key", Varchar(NAME_CHAR_LEN *MAX_KEY), NULLABLE, "key"),
    Column("key_len", Varchar(NAME_CHAR_LEN *MAX_KEY), NULLABLE, "key_len"),
    Column("ref", Varchar(NAME_CHAR_LEN *MAX_REF_PARTS), NULLABLE, "ref"),
    Column("rows", SLonglong(10), NULLABLE, "rows"), Column("r_rows", Varchar(NAME_CHAR_LEN), NULLABLE, "r_rows"),
    ...
    Column("filtered", Decimal(502), NULLABLE, "filtered"), Column("r_filtered", Decimal(502), NULLABLE, "r_filtered"),
    Column("Extra", Varchar(255), NOT_NULL, "Extra"), CEnd()};

ST_FIELD_INFO show_analyze_json_fields_info[] = {
    Column("ANALYZE", Longtext(MAX_FIELD_VARCHARLENGTH), NOT_NULL, "SHOW ANALYZE"), CEnd()};
...
ST_FIELD_INFO check_constraints_fields_info[] = {
    ...
    Column("CHECK_CLAUSE", Longtext(MAX_FIELD_VARCHARLENGTH), NOT_NULL, OPEN_FULL_TABLE),
    CEnd()};
...
ST_SCHEMA_TABLE schema_tables[] = {
    {"ALL_PLUGINS", Show::plugin_fields_info, 0, fill_all_plugins, make_old_format, 0, 5, -1, 0, 0},
    {"APPLICABLE_ROLES", Show::applicable_roles_fields_info, 0, fill_schema_applicable_roles, 0, 0, -1, -1, 0, 0},
    {"CHARACTER_SETS", Show::charsets_fields_info, 0, fill_schema_charsets, make_character_sets_old_format, 0, -1, -1,
     0, 0},
    {"CHECK_CONSTRAINTS", Show::check_constraints_fields_info, 0, get_all_tables, 0, get_check_constraints_record, 1, 2,
     0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"COLLATIONS", Show::collation_fields_info, 0, fill_schema_collation, make_old_format, 0, -1, -1, 0, 0},
    {"COLLATION_CHARACTER_SET_APPLICABILITY", Show::coll_charset_app_fields_info, 0, fill_schema_coll_charset_app, 0, 0,
     -1, -1, 0, 0},
    {"COLUMNS", Show::columns_fields_info, 0, get_all_tables, make_columns_old_format, get_schema_column_record, 1, 2,
     0, OPTIMIZE_I_S_TABLE | OPEN_VIEW_FULL},
    {"COLUMN_PRIVILEGES", Show::column_privileges_fields_info, 0, fill_schema_column_privileges, 0, 0, -1, -1, 0, 0},
    {"ENABLED_ROLES", Show::enabled_roles_fields_info, 0, fill_schema_enabled_roles, 0, 0, -1, -1, 0, 0},
    {"ENGINES", Show::engines_fields_info, 0, fill_schema_engines, make_old_format, 0, -1, -1, 0, 0},
#ifdef HAVE_EVENT_SCHEDULER
    {"EVENTS", Show::events_fields_info, 0, Events::fill_schema_events, make_old_format, 0, -1, -1, 0, 0},
#else
    {"EVENTS", Show::events_fields_info, 0, 0, make_old_format, 0, -1, -1, 0, 0},
#endif
    {"EXPLAIN", Show::show_explain_tabular_fields_info, 0, fill_show_explain_tabular, make_old_format, 0, -1, -1,
     TRUE /*hidden*/, 0},
    {"EXPLAIN_JSON", Show::show_explain_json_fields_info, 0, fill_show_explain_json, make_old_format, 0, -1, -1,
     TRUE /*hidden*/, 0},
    {"ANALYZE", Show::show_analyze_tabular_fields_info, 0, fill_show_analyze_tabular, make_old_format, 0, -1, -1,
     TRUE /*hidden*/, 0},
    {"ANALYZE_JSON", Show::show_analyze_json_fields_info, 0, fill_show_analyze_json, make_old_format, 0, -1, -1,
     TRUE /*hidden*/, 0},
    {"FILES", Show::files_fields_info, 0, hton_fill_schema_table, 0, 0, -1, -1, 0, 0},
    {"GLOBAL_STATUS", Show::variables_fields_info, 0, fill_status, make_old_format, 0, 0, -1, 0, 0},
    {"GLOBAL_VARIABLES", Show::variables_fields_info, 0, fill_variables, make_old_format, 0, 0, -1, 0, 0},
    {"KEYWORDS", Show::keywords_field_info, 0, fill_i_s_keywords, 0, 0, -1, -1, 0, 0},
    {"KEY_CACHES", Show::keycache_fields_info, 0, fill_key_cache_tables, 0, 0, -1, -1, 0, 0},
    {"KEY_COLUMN_USAGE", Show::key_column_usage_fields_info, 0, get_all_tables, 0, get_schema_key_column_usage_record,
     4, 5, 0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"OPEN_TABLES", Show::open_tables_fields_info, 0, fill_open_tables, make_old_format, 0, -1, -1, 1, 0},
    {"OPTIMIZER_TRACE", Show::optimizer_trace_info, 0, fill_optimizer_trace_info, NULL, NULL, -1, -1, false, 0},
    {"PARAMETERS", Show::parameters_fields_info, 0, fill_schema_proc, 0, 0, 1, 2, 0, 0},
    {"PARTITIONS", Show::partitions_fields_info, 0, get_all_tables, 0, get_schema_partitions_record, 1, 2, 0,
     OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"PLUGINS", Show::plugin_fields_info, 0, fill_plugins, make_old_format, 0, -1, -1, 0, 0},
    {"PROCESSLIST", Show::processlist_fields_info, 0, fill_schema_processlist, make_old_format, 0, -1, -1, 0, 0},
    {"PROFILING", Show::query_profile_statistics_info, 0, fill_query_profile_statistics_info,
     make_profile_table_for_show, NULL, -1, -1, false, 0},
    {"REFERENTIAL_CONSTRAINTS", Show::referential_constraints_fields_info, 0, get_all_tables, 0,
     get_referential_constraints_record, 1, 9, 0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"ROUTINES", Show::proc_fields_info, 0, fill_schema_proc, make_proc_old_format, 0, 2, 3, 0, 0},
    {"SCHEMATA", Show::schema_fields_info, 0, fill_schema_schemata, make_schemata_old_format, 0, 1, -1, 0, 0},
    {"SCHEMA_PRIVILEGES", Show::schema_privileges_fields_info, 0, fill_schema_schema_privileges, 0, 0, -1, -1, 0, 0},
    {"SESSION_STATUS", Show::variables_fields_info, 0, fill_status, make_old_format, 0, 0, -1, 0, 0},
    {"SESSION_VARIABLES", Show::variables_fields_info, 0, fill_variables, make_old_format, 0, 0, -1, 0, 0},
    {"STATISTICS", Show::stat_fields_info, 0, get_all_tables, make_old_format, get_schema_stat_record, 1, 2, 0,
     OPEN_TABLE_ONLY | OPTIMIZE_I_S_TABLE},
    {"SQL_FUNCTIONS", Show::sql_functions_field_info, 0, fill_i_s_sql_functions, 0, 0, -1, -1, 0, 0},
    {"SYSTEM_VARIABLES", Show::sysvars_fields_info, 0, fill_sysvars, make_old_format, 0, 0, -1, 0, 0},
    {"TABLES", Show::tables_fields_info, 0, get_all_tables, make_old_format, get_schema_tables_record, 1, 2, 0,
     OPTIMIZE_I_S_TABLE},
    {"TABLESPACES", Show::tablespaces_fields_info, 0, hton_fill_schema_table, 0, 0, -1, -1, 0, 0},
    {"TABLE_CONSTRAINTS", Show::table_constraints_fields_info, 0, get_all_tables, 0, get_schema_constraints_record, 3,
     4, 0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"TABLE_NAMES", Show::table_names_fields_info, 0, get_all_tables, make_table_names_old_format, 0, 1, 2, 1,
     OPTIMIZE_I_S_TABLE},
    {"TABLE_PRIVILEGES", Show::table_privileges_fields_info, 0, fill_schema_table_privileges, 0, 0, -1, -1, 0, 0},
    {"TRIGGERS", Show::triggers_fields_info, 0, get_all_tables, make_old_format, get_schema_triggers_record, 5, 6, 0,
     OPEN_TRIGGER_ONLY | OPTIMIZE_I_S_TABLE},
    {"USER_PRIVILEGES", Show::user_privileges_fields_info, 0, fill_schema_user_privileges, 0, 0, -1, -1, 0, 0},
    {"VIEWS", Show::view_fields_info, 0, get_all_tables, 0, get_schema_views_record, 1, 2, 0,
...
int initialize_schema_table(st_plugin_int *plugin)
{
  ...
  if (!(schema_table = (ST_SCHEMA_TABLE *)my_malloc(key_memory_ST_SCHEMA_TABLE, sizeof(ST_SCHEMA_TABLE),
...
static bool show_create_trigger_impl(THD *thd, Trigger *trigger)
{
  ...
  */
  ...
  fields.push_back(new (mem_root) Item_empty_string(thd, "Trigger", NAME_LEN), mem_root);
  fields.push_back(new (mem_root) Item_empty_string(thd, "sql_mode", (uint)trg_sql_mode_str.length), mem_root);

  {
    ...
    */
    ...
    Item_empty_string *stmt_fld = new (mem_root)
        Item_empty_string(thd, "SQL Original Statement", (uint)MY_MAX(trg_sql_original_stmt.length, 1024));
  ...
  fields.push_back(new (mem_root) Item_empty_string(thd, "character_set_client", MY_CS_CHARACTER_SET_NAME_SIZE),
                   mem_root);

  fields.push_back(new (mem_root) Item_empty_string(thd, "collation_connection", MY_CS_COLLATION_NAME_SIZE), mem_root);

  fields.push_back(new (mem_root) Item_empty_string(thd, "Database Collation", MY_CS_COLLATION_NAME_SIZE), mem_root);

  static const Datetime zero_datetime(Datetime::zero());
  Item_datetime_literal *tmp = (new (mem_root) Item_datetime_literal(thd, &zero_datetime, 2));
  ...
  p->store(trigger->name.str, trigger->name.length, system_charset_info);

  p->store(trg_sql_mode_str.str, trg_sql_mode_str.length, system_charset_info);

  p->store(trg_sql_original_stmt.str, trg_sql_original_stmt.length, trg_client_cs);
...
bool show_create_trigger(THD *thd, const sp_name *trg_name)
{
  ...
  */
  if (open_tables(thd, &lst, &num_tables, MYSQL_OPEN_FORCE_SHARED_HIGH_PRIO_MDL))
  {
    my_error(ER_TRG_CANT_OPEN_TABLE, MYF(0), (const char *)trg_name->m_db.str, (const char *)lst->table_name.str);
  ...
  if (!trigger)
  {
    my_error(ER_TRG_CORRUPTED_FILE, MYF(0), (const char *)trg_name->m_db.str, (const char *)lst->table_name.str);
    goto exit;
  }
...
class IS_internal_schema_access : public ACL_internal_schema_access
{
 public:
  ...
  ACL_internal_access_result check(privilege_t want_access, privilege_t *save_priv) const;

  const ACL_internal_table_access *lookup(const char *name) const;
};

ACL_internal_access_result IS_internal_schema_access::check(privilege_t want_access, privilege_t *save_priv) const
{
  ...
  *save_priv |= SELECT_ACL;
  ...
  return want_access ? ACL_INTERNAL_ACCESS_CHECK_GRANT : ACL_INTERNAL_ACCESS_GRANTED;
}
...
{
  ACL_internal_schema_registry::register_schema(&INFORMATION_SCHEMA_NAME, &is_internal_schema_access);
}
...
extern "C" char *thd_get_error_context_description(THD *thd, char *buffer, unsigned int length,
                                                   unsigned int max_query_len)
{
  ...
  len = my_snprintf(header, sizeof(header), "MariaDB thread id %u, OS thread handle %lu, query id %llu",
