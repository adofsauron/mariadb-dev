...
class Mrr_index_reader : public Mrr_reader
{
 ...
 public:
  virtual int init(handler *h_arg, RANGE_SEQ_IF *seq_funcs, void *seq_init_param, uint n_ranges, uint mode,
                   Key_parameters *key_par, Lifo_buffer *key_buffer, Buffer_manager *buf_manager_arg) = 0;
...
{
 public:
  int init(handler *h_arg, RANGE_SEQ_IF *seq_funcs, void *seq_init_param, uint n_ranges, uint mode,
           Key_parameters *key_par, Lifo_buffer *key_buffer, Buffer_manager *buf_manager_arg);
  int get_next(range_id_t *range_info);
  ...
  {
    return (file->mrr_funcs.skip_record && file->mrr_funcs.skip_record(file->mrr_iter, range_id, rowid));
  }
};
...
class Mrr_ordered_index_reader : public Mrr_index_reader
{
 public:
  ...
  {
    return (mrr_funcs.skip_record && mrr_funcs.skip_record(mrr_iter, range_info, rowid));
  }

  bool skip_index_tuple(range_id_t range_info)
  {
    return (mrr_funcs.skip_index_tuple && mrr_funcs.skip_index_tuple(mrr_iter, range_info));
  }

  bool set_interruption_temp_buffer(uint rowid_length, uint key_len, uint saved_pk_len, uchar **space_start,
                                    uchar *space_end);
...
{
 public:
  int init(handler *file, Mrr_index_reader *index_reader, uint mode, Lifo_buffer *buf, Rowid_filter *filter);
  int get_next(range_id_t *range_info);
  int refill_buffer(bool initial);
...
class DsMrr_impl
{
 public:
  ...
  DsMrr_impl() : secondary_file(NULL), rowid_filter(NULL){};
  ...
  int dsmrr_init(handler *h_arg, RANGE_SEQ_IF *seq_funcs, void *seq_init_param, uint n_ranges, uint mode,
                 HANDLER_BUFFER *buf);
  void dsmrr_close();
  int dsmrr_next(range_id_t *range_info);

  ha_rows dsmrr_info(uint keyno, uint n_ranges, uint keys, uint key_parts, uint *bufsz, uint *flags,
                     Cost_estimate *cost);

  ha_rows dsmrr_info_const(uint keyno, RANGE_SEQ_IF *seq, void *seq_init_param, uint n_ranges, uint *bufsz, uint *flags,
                           Cost_estimate *cost);
  ...
  */
  ...
  bool get_disk_sweep_mrr_cost(uint keynr, ha_rows rows, uint flags, uint *buffer_size, uint extra_mem_overhead,
                               Cost_estimate *cost);
