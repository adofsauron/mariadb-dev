...
Master_info::Master_info(LEX_CSTRING *connection_name_arg, bool is_slave_recovery)
    : Slave_reporting_capability("I/O"),
      ...
      inited(0),
      abort_slave(0),
      slave_running(MYSQL_SLAVE_NOT_RUN),
      slave_run_id(0),
      ...
      master_id(0),
      prev_master_id(0),
      using_gtid(USE_GTID_SLAVE_POS),
      events_queued_since_last_gtid(0),
      gtid_reconnect_event_skip_count(0),
      gtid_event_seen(false),
      in_start_all_slaves(0),
      in_stop_all_slaves(0),
      in_flush_all_relay_logs(0),
      users(0),
      killed(0),
      total_ddl_groups(0),
      total_non_trans_groups(0),
      total_trans_groups(0)
{
  char *tmp;
  host[0] = 0;
  user[0] = 0;
  ...
  */
  connection_name.length = cmp_connection_name.length = connection_name_arg->length;
  if ((connection_name.str = tmp =
           (char *)my_malloc(PSI_INSTRUMENT_ME, connection_name_arg->length * 2 + 2, MYF(MY_WME))))
  {
  ...
  */
  rpl_filter = get_or_create_rpl_filter(connection_name.str, connection_name.length);
  copy_filter_setting(rpl_filter, global_rpl_filter);

  parallel_mode = rpl_filter->get_parallel_mode();

  my_init_dynamic_array(PSI_INSTRUMENT_ME, &ignore_server_ids, sizeof(global_system_variables.server_id), 16, 16,
                        MYF(0));
  bzero((char *)&file, sizeof(file));
  mysql_mutex_init(key_master_info_run_lock, &run_lock, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_master_info_data_lock, &data_lock, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_master_info_start_stop_lock, &start_stop_lock, MY_MUTEX_INIT_SLOW);
  /*
  ...
  */
  mysql_mutex_init(key_master_info_start_alter_lock, &start_alter_lock, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_master_info_start_alter_list_lock, &start_alter_list_lock, MY_MUTEX_INIT_FAST);
...
void Master_info::wait_until_free()
{
  ...
  killed = 1;
  while (users) mysql_cond_wait(&sleep_cond, &sleep_lock);
...
/**
   ...
   than the second
*/
static int change_master_id_cmp(const void *id1, const void *id2) { return (*(ulong *)id1 - *(ulong *)id2); }
...
bool Master_info::shall_ignore_server_id(ulong s_id)
{
  if (likely(ignore_server_ids.elements == 1))
    ...
    return bsearch((const ulong *)&s_id, ignore_server_ids.buffer, ignore_server_ids.elements, sizeof(ulong),
...
const char *Master_info::using_gtid_astext(enum enum_using_gtid arg)
{
  switch (arg)
  {
    case USE_GTID_NO:
...
void init_master_log_pos(Master_info *mi)
{
  ...
  */
  ...
  DBUG_ASSERT(mi->heartbeat_period > (float)0.001 || mi->heartbeat_period == 0);
...
/**
  ...
  @retval 1                           EOF
*/
static int read_mi_key_from_file(char *key, int max_size, IO_CACHE *f, bool *found_equal)
{
  int i = 0, c;
  ...
  */
  ...
  if (access(fname, F_OK))
  {
    ...
    */
    ...
    if ((fd = mysql_file_open(key_file_master_info, fname, O_CREAT | O_RDWR | O_BINARY, MYF(MY_WME))) < 0)
    {
      sql_print_error(
...
file '%s')",
  ...
  else  // file exists
  {
    ...
    {
      if ((fd = mysql_file_open(key_file_master_info, fname, O_RDWR | O_BINARY, MYF(MY_WME))) < 0)
      {
        sql_print_error(
...
file '%s')",
    ...
    */
    ...
    if (init_intvar_from_file(&master_log_pos, &mi->file, 4) ||
        ...
        init_strvar_from_file(mi->password, SCRAMBLED_PASSWORD_CHAR_LENGTH + 1, &mi->file, 0) ||
    ...
    if (lines >= LINES_IN_MASTER_INFO_WITH_SSL)
    {
      if (init_intvar_from_file(&ssl, &mi->file, 0) ||
          init_strvar_from_file(mi->ssl_ca, sizeof(mi->ssl_ca), &mi->file, 0) ||
          init_strvar_from_file(mi->ssl_capath, sizeof(mi->ssl_capath), &mi->file, 0) ||
          init_strvar_from_file(mi->ssl_cert, sizeof(mi->ssl_cert), &mi->file, 0) ||
          init_strvar_from_file(mi->ssl_cipher, sizeof(mi->ssl_cipher), &mi->file, 0) ||
          init_strvar_from_file(mi->ssl_key, sizeof(mi->ssl_key), &mi->file, 0))
        goto errwithmsg;
      ...
      */
       ...
       */
      if (lines >= LINE_FOR_MASTER_BIND && init_strvar_from_file(buf, sizeof(buf), &mi->file, ""))
        goto errwithmsg;
      /*
      ...
      */
      ...
      /* reserved */
      if (lines >= LINE_FOR_MASTER_UUID && init_strvar_from_file(buf, sizeof(buf), &mi->file, ""))
        goto errwithmsg;

      /* Starting from 5.5 the master_retry_count may be in the repository. */
      if (lines >= LINE_FOR_MASTER_RETRY_COUNT && init_strvar_from_file(buf, sizeof(buf), &mi->file, ""))
        goto errwithmsg;

      if (lines >= LINE_FOR_SSL_CRLPATH &&
          (init_strvar_from_file(mi->ssl_crl, sizeof(mi->ssl_crl), &mi->file, "") ||
           init_strvar_from_file(mi->ssl_crlpath, sizeof(mi->ssl_crlpath), &mi->file, "")))
        goto errwithmsg;
      ...
      if (lines >= LINE_FOR_LAST_MYSQL_FUTURE)
      {
        ...
        */
        while (!read_mi_key_from_file(buf, sizeof(buf), &mi->file, &got_eq))
        {
          ...
          {
            if (mi->domain_id_filter.init_ids(&mi->file, Domain_id_filter::DO_DOMAIN_IDS))
            {
          ...
          else if (got_eq && !seen_ignore_domain_ids && !strcmp(buf, "ignore_domain_ids"))
          {
            if (mi->domain_id_filter.init_ids(&mi->file, Domain_id_filter::IGNORE_DOMAIN_IDS))
            {
              sql_print_error(
                  "Failed to initialize master info "
                  "ignore_domain_ids");
              goto errwithmsg;
            }
...
int flush_master_info(Master_info *mi, bool flush_relay_log_cache, bool need_lock_relay_log)
{
  ...
  */
  ...
  {
    ignore_server_ids_buf = (char *)my_malloc(
        PSI_INSTRUMENT_ME, (sizeof(global_system_variables.server_id) * 3 + 1) * (1 + mi->ignore_server_ids.elements),
        MYF(MY_WME));
    if (!ignore_server_ids_buf)
      DBUG_RETURN(1); /* error */
    ulong cur_len = sprintf(ignore_server_ids_buf, "%zu", mi->ignore_server_ids.elements);
  ...
  do_domain_ids_buf = mi->domain_id_filter.as_string(Domain_id_filter::DO_DOMAIN_IDS);
  ...
  ignore_domain_ids_buf = mi->domain_id_filter.as_string(Domain_id_filter::IGNORE_DOMAIN_IDS);
  ...
  */
  ...
  my_b_printf(file,
              ...
              "ignore_domain_ids=%s\n"
              "END_MARKER\n",
              LINES_IN_MASTER_INFO, mi->master_log_name, llstr(mi->master_log_pos, lbuf), mi->host, mi->user,
              mi->password, mi->port, mi->connect_retry, (int)(mi->ssl), mi->ssl_ca, mi->ssl_capath, mi->ssl_cert,
              mi->ssl_cipher, mi->ssl_key, mi->ssl_verify_server_cert, heartbeat_buf, "", ignore_server_ids_buf, "", 0,
              mi->ssl_crl, mi->ssl_crlpath, mi->using_gtid, do_domain_ids_buf, ignore_domain_ids_buf);
  err = flush_io_cache(file);
  if (sync_masterinfo_period && !err && ++(mi->sync_counter) >= sync_masterinfo_period)
  {
...
void create_logfile_name_with_suffix(char *res_file_name, size_t length, const char *info_file, bool append,
                                     LEX_CSTRING *suffix)
{
  char buff[MAX_CONNECTION_NAME + 1], res[MAX_CONNECTION_NAME * MAX_FILENAME_MBWIDTH + 1], *p;
  ...
  if (suffix->length != 0 && p <= res_file_name + length - 1)
  {
    ...
    res_length = strconvert(system_charset_info, buff, from_length, &my_charset_filename, res, sizeof(res), &errors);

    ext_pos = (size_t)(ext - info_file);
    length -= (suffix->length - ext_pos); /* Leave place for extension */
    p = res_file_name + ext_pos;
    *p++ = '-'; /* Add separator */
    p = strmake(p, res, MY_MIN((size_t)(length - (p - res_file_name)), res_length));
...
Master_info_index::Master_info_index()
{
  ...
  */
  fn_format(index_file_name, master_info_file, mysql_data_home, "", MY_UNPACK_FILENAME);
  filename_length = strlen(index_file_name) + 1; /* Count 0 byte */
  dir_length = dirname_length(index_file_name);
  bmove_upp((uchar *)index_file_name + filename_length + 6, (uchar *)index_file_name + filename_length,
...
bool Master_info_index::init_all_master_info()
{
  ...
  if ((index_file_nr = my_open(index_file_name, O_RDWR | O_CREAT | O_BINARY, MYF(MY_WME | ME_ERROR_LOG))) < 0 ||
      my_sync(index_file_nr, MYF(MY_WME)) ||
      init_io_cache(&index_file, index_file_nr, IO_SIZE, READ_CACHE, my_seek(index_file_nr, 0L, MY_SEEK_END, MYF(0)), 0,
                    MYF(MY_WME | MY_WAIT_IF_FULL)))
  {
    if (index_file_nr >= 0)
      my_close(index_file_nr, MYF(0));

    sql_print_error("Creation of Master_info index file '%s' failed", index_file_name);
    DBUG_RETURN(1);
  }

  /* Initialize Master_info Hash Table */
  if (my_hash_init(PSI_INSTRUMENT_ME, &master_info_hash, system_charset_info, MAX_REPLICATION_THREAD, 0, 0,
                   (my_hash_get_key)get_key_master_info, (my_hash_free_key)free_key_master_info, HASH_UNIQUE))
  {
  ...
  while (!init_strvar_from_file(sign, sizeof(sign), &index_file, NULL))
  {
    ...
    if (!(mi = new Master_info(&connection_name, relay_log_recovery)) || mi->error())
    {
      delete mi;
      goto error;
    }
    ...
    create_logfile_name_with_suffix(buf_master_info_file, sizeof(buf_master_info_file), master_info_file, 0,
                                    &mi->cmp_connection_name);
    create_logfile_name_with_suffix(buf_relay_log_info_file, sizeof(buf_relay_log_info_file), relay_log_info_file, 0,
                                    &mi->cmp_connection_name);
    if (global_system_variables.log_warnings > 1)
      sql_print_information("Reading Master_info: '%s'  Relay_info:'%s'", buf_master_info_file,
                            buf_relay_log_info_file);

    mi->lock_slave_threads();
    if (init_master_info(mi, buf_master_info_file, buf_relay_log_info_file, 0, thread_mask))
    {
      err_num++;
      sql_print_error("Initialized Master_info from '%s' failed", buf_master_info_file);
      if (!master_info_index->get_master_info(&connection_name, Sql_condition::WARN_LEVEL_NOTE))
      {
      ...
      else
      {
        ...
        sql_print_error(ER_THD_OR_DEFAULT(current_thd, ER_CONNECTION_ALREADY_EXISTS), (int)connection_name.length,
                        connection_name.str, (int)connection_name.length, connection_name.str);
        mi->unlock_slave_threads();
        delete mi;
      }
      continue;
    ...
    else
    {
      ...
      if (global_system_variables.log_warnings > 1)
        sql_print_information("Initialized Master_info from '%s'", buf_master_info_file);
      if (master_info_index->get_master_info(&connection_name, Sql_condition::WARN_LEVEL_NOTE))
      {
      ...
      {
        if (start_slave_threads(current_thd, 1 /* need mutex */, 1 /* wait for start*/, mi, buf_master_info_file,
                                buf_relay_log_info_file, SLAVE_IO | SLAVE_SQL))
        {
          sql_print_error("Failed to create slave threads for connection '%.*s'", (int)connection_name.length,
                          connection_name.str);
          continue;
        }
        if (global_system_variables.log_warnings)
          sql_print_information("Started replication for '%.*s'", (int)connection_name.length, connection_name.str);
      }
      mi->unlock_slave_threads();
    }
  }
...
bool Master_info_index::write_master_name_to_index_file(LEX_CSTRING *name, bool do_sync)
{
  ...
  if (my_b_write(&index_file, (uchar *)name->str, name->length) || my_b_write(&index_file, (uchar *)"\n", 1) ||
      flush_io_cache(&index_file) || (do_sync && my_sync(index_file.file, MYF(MY_WME))))
  {
    sql_print_error("Write of new Master_info for '%.*s' to index file failed", (int)name->length, name->str);
    DBUG_RETURN(1);
  }
  ...
  */
  if (unlikely(!master_info_index))
  {
    if (warning != Sql_condition::WARN_LEVEL_NOTE)
      my_error(WARN_NO_MASTER_INFO, MYF(warning == Sql_condition::WARN_LEVEL_WARN ? ME_WARNING : 0),
...
Master_info *Master_info_index::get_master_info(const LEX_CSTRING *connection_name,
                                                Sql_condition::enum_warning_level warning)
{
  ...
  size_t buff_length;
  DBUG_ENTER("get_master_info");
  DBUG_PRINT("enter", ("connection_name: '%.*s'", (int)connection_name->length, connection_name->str));
  ...
  mi = (Master_info *)my_hash_search(&master_info_hash, (uchar *)buff, buff_length);
...
bool Master_info_index::check_duplicate_master_info(LEX_CSTRING *name_arg, const char *host, uint port)
{
  Master_info *mi;
  ...
  if ((mi = master_info_index->get_master_info(name_arg, Sql_condition::WARN_LEVEL_NOTE)))
  {
    if (!host)
      host = mi->host;
    if (!port)
  ...
  for (uint i = 0; i < master_info_hash.records; ++i)
  {
    ...
    {
      my_error(ER_CONNECTION_ALREADY_EXISTS, MYF(0), (int)name_arg->length, name_arg->str,
               (int)tmp_mi->connection_name.length, tmp_mi->connection_name.str);
      DBUG_RETURN(TRUE);
    }
  }
  DBUG_RETURN(FALSE);
...
bool Master_info_index::add_master_info(Master_info *mi, bool write_to_file)
{
  ...
  */
  if (unlikely(abort_loop) || !my_hash_insert(&master_info_hash, (uchar *)mi))
  {
    if (global_system_variables.log_warnings > 2)
      sql_print_information("Added new Master_info '%.*s' to hash table", (int)mi->connection_name.length,
  ...
  sql_print_error("Adding new entry '%.*s' to master_info failed", (int)mi->connection_name.length,
                  mi->connection_name.str);
  return TRUE;
}
...
bool Master_info_index::remove_master_info(Master_info *mi, bool clear_log_files)
{
  ...
  if (clear_log_files)
  {
    ...
    create_logfile_name_with_suffix(tmp_name, sizeof(tmp_name), master_info_file, 0, &mi->cmp_connection_name);
    my_delete(tmp_name, MYF(0));
    /* Delete relay-log-'connection'.info */
    create_logfile_name_with_suffix(tmp_name, sizeof(tmp_name), relay_log_info_file, 0, &mi->cmp_connection_name);
    my_delete(tmp_name, MYF(0));
  }
  ...
  if (!my_hash_delete(&master_info_hash, (uchar *)mi))
  {
    ...
    if ((index_file_nr = my_open(index_file_name, O_RDWR | O_CREAT | O_TRUNC | O_BINARY, MYF(MY_WME))) < 0 ||
                      ...
                      0, MYF(MY_WME | MY_WAIT_IF_FULL)))
    {
      ...
      sql_print_error(
          "Create of Master Info Index file '%s' failed with "
          "error: %M",
...
bool Master_info_index::start_all_slaves(THD *thd)
{
  ...
  for (uint i = 0; i < master_info_hash.records;)
  {
    ...
    */
    if (!((mi->slave_running == MYSQL_SLAVE_NOT_RUN || !mi->rli.slave_running) && *mi->host) || mi->in_start_all_slaves)
    {
      i++;
      continue;
    }
    ...
    if (unlikely(error))
    {
      my_error(ER_CANT_START_STOP_SLAVE, MYF(0), "START", (int)mi->connection_name.length, mi->connection_name.str);
      result = 1;
      if (error < 0)  // fatal error
        break;
    }
    else if (thd)
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_SLAVE_STARTED, ER_THD(thd, ER_SLAVE_STARTED),
...
bool Master_info_index::stop_all_slaves(THD *thd)
{
  ...
  for (uint i = 0; i < master_info_hash.records;)
  {
    ...
    Master_info *mi;
    mi = (Master_info *)my_hash_element(&master_info_hash, i);
    if (!(mi->slave_running != MYSQL_SLAVE_NOT_RUN || mi->rli.slave_running) || mi->in_stop_all_slaves)
    {
      i++;
      continue;
    }
    ...
    if (unlikely(error))
    {
      my_error(ER_CANT_START_STOP_SLAVE, MYF(0), "STOP", (int)mi->connection_name.length, mi->connection_name.str);
      result = 1;
      if (error < 0)  // Fatal error
        break;
    }
    else
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_SLAVE_STOPPED, ER_THD(thd, ER_SLAVE_STOPPED),
                          (int)mi->connection_name.length, mi->connection_name.str);
    /* Restart from first element as master_info_hash may have changed */
    i = 0;
    continue;
...
Domain_id_filter::Domain_id_filter() : m_filter(false)
{
  ...
  {
    my_init_dynamic_array(PSI_INSTRUMENT_ME, &m_domain_ids[i], sizeof(ulong), 16, 16, MYF(0));
  }
}
...
void Domain_id_filter::do_filter(ulong domain_id)
{
  ...
  if (do_domain_ids->elements > 0)
  {
    if (likely(do_domain_ids->elements == 1))
      m_filter = ((*(ulong *)dynamic_array_ptr(do_domain_ids, 0)) != domain_id);
    else
      m_filter = (bsearch((const ulong *)&domain_id, do_domain_ids->buffer, do_domain_ids->elements, sizeof(ulong),
                          change_master_id_cmp) == NULL);
  }
  else if (ignore_domain_ids->elements > 0)
  {
    if (likely(ignore_domain_ids->elements == 1))
      m_filter = ((*(ulong *)dynamic_array_ptr(ignore_domain_ids, 0)) == domain_id);
    else
      m_filter = (bsearch((const ulong *)&domain_id, ignore_domain_ids->buffer, ignore_domain_ids->elements,
...
/**
  ...
  XID_EVENT.
*/
void Domain_id_filter::reset_filter() { m_filter = false; }
...
/**
  ...
  @retval false                     Success
          true                      Error
*/
bool Domain_id_filter::update_ids(DYNAMIC_ARRAY *do_ids, DYNAMIC_ARRAY *ignore_ids, bool using_gtid)
{
  ...
  {
    ignore_list_empty = (m_domain_ids[IGNORE_DOMAIN_IDS].elements > 0) ? false : true;
  }

  if (!do_list_empty && !ignore_list_empty)
  {
    sql_print_error(
        "Both DO_DOMAIN_IDS & IGNORE_DOMAIN_IDS lists can't be "
        "non-empty at the same time");
    return true;
  }

  if (using_gtid == Master_info::USE_GTID_NO && (!do_list_empty || !ignore_list_empty))
  {
    sql_print_error(
        "DO_DOMAIN_IDS or IGNORE_DOMAIN_IDS lists can't be "
...
void update_change_master_ids(DYNAMIC_ARRAY *new_ids, DYNAMIC_ARRAY *old_ids)
{
  ...
  for (uint i = 0; i < new_ids->elements; i++)
  {
    ...
    if (bsearch((const ulong *)&id, old_ids->buffer, old_ids->elements, sizeof(ulong), change_master_id_cmp) == NULL)
    {
