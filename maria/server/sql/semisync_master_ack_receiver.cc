...
bool Ack_receiver::start()
{
  ...
  if (m_status == ST_DOWN)
  {
    ...
    if (DBUG_IF("rpl_semisync_simulate_create_thread_failure") || pthread_attr_init(&attr) != 0 ||
        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE) != 0 ||
#ifndef _WIN32
        pthread_attr_setscope(&attr, PTHREAD_SCOPE_SYSTEM) != 0 ||
#endif
        mysql_thread_create(key_thread_ack_receiver, &m_pid, &attr, ack_receive_handler, this))
    {
      sql_print_error(
          "Failed to start semi-sync ACK receiver thread, "
...
#else
      DBUG_PRINT("info", ("fd count %u, max_fd %d", slave_count, (int)listener.get_max_fd()));
#endif
    }
    ...
    if (ret <= 0)
    {
      ...
      if (ret == -1 && errno != EINTR)
        sql_print_information(
            "Failed to wait on semi-sync sockets, "
            "error: errno=%d",
            socket_errno);
      ...
      if (listener.is_socket_active(slave))
      {
        ...
        */
        ...
        if (likely(len != packet_error))
          repl_semisync_master.report_reply_packet(slave->server_id(), net.read_pos, len);
