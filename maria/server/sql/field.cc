...
#define BLOB_PACK_LENGTH_TO_MAX_LENGH(arg) ((ulong)((1LL << MY_MIN(arg, 4) * 8) - 1))

// Column marked for read or the field set to read out of record[0]
bool Field::marked_for_read() const
{
  return !table || (!table->read_set || bitmap_is_set(table->read_set, field_index) ||
...
{
  return (!table || (!table->write_set || bitmap_is_set(table->write_set, field_index) ||
                     (!(ptr >= table->record[0] && ptr < table->record[0] + table->s->reclength))));
}
...
const int FIELDTYPE_NUM = FIELDTYPE_TEAR_FROM + (FIELDTYPE_LAST - FIELDTYPE_TEAR_TO);

static inline int merge_type2index(enum_field_types merge_type)
{
  DBUG_ASSERT(merge_type < FIELDTYPE_TEAR_FROM || merge_type > FIELDTYPE_TEAR_TO);
...
const Type_handler *Type_handler::aggregate_for_result_traditional(const Type_handler *a, const Type_handler *b)
{
  if (a == b)
  {
    /*
  ...
  enum_field_types res = field_types_merge_rules[merge_type2index(ta)][merge_type2index(tb)];
  return Type_handler::get_handler_by_real_type(res);
}

bool Field::check_assignability_from(const Type_handler *from, bool ignore) const
{
  /*
  ...
  */
  Type_handler_hybrid_field_type th(type_handler()->type_handler_for_item_field());
  if (th.aggregate_for_result(from->type_handler_for_item_field()))
  {
    bool error =
        (!ignore && get_thd()->is_strict_mode()) || (type_handler()->is_scalar_type() != from->is_scalar_type());
    /*
    ...
    */
    if (table->s->db.str && table->s->table_name.str)
      my_printf_error(ER_ILLEGAL_PARAMETER_DATA_TYPES2_FOR_OPERATION,
                      "Cannot cast '%s' as '%s' in assignment of %`s.%`s.%`s", MYF(error ? 0 : ME_WARNING),
                      from->name().ptr(), type_handler()->name().ptr(), table->s->db.str, table->s->table_name.str,
                      field_name.str);
    else
      my_printf_error(ER_ILLEGAL_PARAMETER_DATA_TYPES2_FOR_OPERATION, "Cannot cast '%s' as '%s' in assignment of %`s",
                      MYF(error ? 0 : ME_WARNING), from->name().ptr(), type_handler()->name().ptr(), field_name.str);
    return error;
  }
  return false;
}
...
static bool test_if_important_data(CHARSET_INFO *cs, const char *str, const char *strend)
{
...
*/
uint Field::make_packed_sort_key_part(uchar *buff, const SORT_FIELD_ATTR *sort_field)
{
  if (maybe_null())
  {
    if (is_null())
...
uint Field_longstr::make_packed_sort_key_part(uchar *buff, const SORT_FIELD_ATTR *sort_field)
{
  if (maybe_null())
  {
    if (is_null())
...
uchar *Field_longstr::pack_sort_string(uchar *to, const SORT_FIELD_ATTR *sort_field)
{
...
{
  return pos_in_interval_for_double(val_real(), min->val_real(), max->val_real());
}

static inline ulonglong char_prefix_to_ulonglong(uchar *src)
{
...
{
  return pos_in_interval_for_string(charset(), ptr + data_offset, data_length(), min->ptr + data_offset,
                                    min->data_length(), max->ptr + data_offset, max->data_length());
}
...
bool Field::can_be_substituted_to_equal_item(const Context &ctx, const Item_equal *item_equal)
{
...
bool Field::cmp_is_done_using_type_handler_of_this(const Item_bool_func *cond, const Item *item) const
{
  /*
...
*/
bool Field::can_optimize_keypart_ref(const Item_bool_func *cond, const Item *item) const
{
  DBUG_ASSERT(cmp_type() != STRING_RESULT);
...
/*
  This handles all numeric and BIT data types.
*/
bool Field::can_optimize_group_min_max(const Item_bool_func *cond, const Item *const_item) const
{
  DBUG_ASSERT(cmp_type() != STRING_RESULT);
  DBUG_ASSERT(cmp_type() != TIME_RESULT);
...
*/
bool Field::can_optimize_range(const Item_bool_func *cond, const Item *item, bool is_eq_func) const
{
...
void Field::load_data_set_value(const char *pos, uint length, CHARSET_INFO *cs)
{
  /*
...
void Field::error_generated_column_function_is_not_allowed(THD *thd, bool error) const
{
  StringBuffer<64> tmp;
  vcol_info->expr->print(
      &tmp, (enum_query_type)(QT_TO_SYSTEM_CHARSET | QT_ITEM_IDENT_SKIP_DB_NAMES | QT_ITEM_IDENT_SKIP_TABLE_NAMES));
  my_error(ER_GENERATED_COLUMN_FUNCTION_IS_NOT_ALLOWED, MYF(error ? 0 : ME_WARNING), tmp.c_ptr_safe(),
           vcol_info->get_vcol_type_name(), const_cast<const char *>(field_name.str));
}
...
bool Field::check_vcol_sql_mode_dependency(THD *thd, vcol_init_mode mode) const
{
  ...
  if ((flags & PART_KEY_FLAG) != 0 || stored_in_db())
  {
    ...
    Sql_mode_dependency dep =
        (valdep | Sql_mode_dependency(0, cnvdep)) & Sql_mode_dependency(~0, ~can_handle_sql_mode_dependency_on_store());
    if (dep)
    {
...
*/
Field_num::Field_num(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg, utype unireg_check_arg,
...
void Field_num::prepend_zeros(String *value) const
{
  ...
  if ((diff = (int)(field_length - value->length())) > 0)
  {
    ...
    if (likely(!error))
    {
      bmove_upp((uchar *)value->ptr() + field_length, (uchar *)value->ptr() + value->length(), value->length());
...
sql_mode_t Field_num::can_handle_sql_mode_dependency_on_store() const { return MODE_PAD_CHAR_TO_FULL_LENGTH; }

Item *Field_num::get_equal_zerofill_const_item(THD *thd, const Context &ctx, Item *const_item)
{
  switch (ctx.subst_constraint())
  {
    case IDENTITY_SUBST:
...
Value_source::Warn_filter::Warn_filter(const THD *thd)
    : m_want_warning_edom(!thd->no_errors), m_want_note_truncated_spaces(!thd->no_errors)
{
}
...
*/
void Value_source::Converter_string_to_number::check_edom_and_truncation(THD *thd, Warn_filter filter, const char *type,
                                                                         CHARSET_INFO *cs, const char *str,
                                                                         size_t length) const
{
  DBUG_ASSERT(str <= m_end_of_num);
  DBUG_ASSERT(m_end_of_num <= str + length);
  if (m_edom || (m_end_of_num < str + length && !check_if_only_end_space(cs, m_end_of_num, str + length)))
  {
    ...
    if (filter.want_warning_edom())
    {
      ...
      */
      ...
      push_warning_printf(wthd, Sql_condition::WARN_LEVEL_WARN, ER_TRUNCATED_WRONG_VALUE,
                          ER_THD(wthd, ER_TRUNCATED_WRONG_VALUE), type, ErrConvString(str, length, cs).ptr());
    }
  }
  else if (m_end_of_num < str + length)
  {
    ...
    {
      THD *wthd = thd ? thd : current_thd;
      push_warning_printf(wthd, Sql_condition::WARN_LEVEL_NOTE, ER_TRUNCATED_WRONG_VALUE,
                          ER_THD(wthd, ER_TRUNCATED_WRONG_VALUE), type, ErrConvString(str, length, cs).ptr());
    }
  }
}
...
int Field_num::check_edom_and_important_data_truncation(const char *type, bool edom, CHARSET_INFO *cs, const char *str,
                                                        size_t length, const char *end)
{
...
int Field_num::check_edom_and_truncation(const char *type, bool edom, CHARSET_INFO *cs, const char *str, size_t length,
                                         const char *end)
{
  int rc = check_edom_and_important_data_truncation(type, edom, cs, str, length, end);
...
bool Field_num::get_int(CHARSET_INFO *cs, const char *from, size_t len, longlong *rnd, ulonglong unsigned_max,
                        longlong signed_min, longlong signed_max)
{
  char *end;
  int error;

  *rnd = (longlong)cs->strntoull10rnd(from, len, unsigned_flag, &end, &error);
  if (unsigned_flag)
  {
    if ((((ulonglong)*rnd > unsigned_max) && (*rnd = (longlong)unsigned_max)) || error == MY_ERRNO_ERANGE)
    {
      goto out_of_range;
    }
  }
  ...
  if (get_thd()->count_cuted_fields > CHECK_FIELD_EXPRESSION && check_int(cs, from, len, end, error))
    return 1;
...
double Field_real::get_double(const char *str, size_t length, CHARSET_INFO *cs, int *error)
{
  char *end;
  ...
  if (unlikely(*error))
  {
    ...
    *error = 1;
           ...
           check_edom_and_truncation("double", str == end, cs, str, length, end))
    *error = 1;
  return nr;
}
...
String *Field::val_int_as_str(String *val_buffer, bool unsigned_val)
{
  ...
  if (val_buffer->alloc(MY_INT64_NUM_DECIMAL_DIGITS))
    return 0;
  length = (uint)(cs->longlong10_to_str)((char *)val_buffer->ptr(), MY_INT64_NUM_DECIMAL_DIGITS,
...
Field::Field(uchar *ptr_arg, uint32 length_arg, uchar *null_ptr_arg, uchar null_bit_arg, utype unireg_check_arg,
...
bool Field_num::send_numeric_zerofill_str(Protocol_text *protocol, protocol_send_type_t send_type)
{
  DBUG_ASSERT(marked_for_read());
  StringBuffer<MAX_FIELD_WIDTH> tmp(&my_charset_latin1);
  val_str(&tmp);
  return protocol->store_numeric_zerofill_str(tmp.ptr(), tmp.length(), send_type);
}
...
*/
bool Field::compatible_field_size(uint field_metadata, const Relay_log_info *rli_arg __attribute__((unused)),
                                  uint16 mflags __attribute__((unused)), int *order_var) const
{
  uint const source_size = pack_length_from_metadata(field_metadata);
  uint const destination_size = row_pack_length();
  DBUG_PRINT("debug",
             ("real_type: %d, source_size: %u, destination_size: %u", real_type(), source_size, destination_size));
  *order_var = compare(source_size, destination_size);
  return true;
}

int Field::store(const char *to, size_t length, CHARSET_INFO *cs, enum_check_fields check_level)
{
  Check_level_instant_set tmp_level(get_thd(), check_level);
  return store(to, length, cs);
}

int Field::store_text(const char *to, size_t length, CHARSET_INFO *cs, enum_check_fields check_level)
{
  Check_level_instant_set tmp_level(get_thd(), check_level);
...
{
  val_str(val);
  size_t length = Well_formed_prefix(val->charset(), val->ptr(), MY_MIN(val->length(), field->field_length)).length();
...
*/
const uchar *Field::unpack(uchar *to, const uchar *from, const uchar *from_end, uint param_data)
{
...
void Field::make_send_field(Send_field *field)
{
  if (orig_table && orig_table->s->db.str && *orig_table->s->db.str)
  {
    ...
    if (orig_table->pos_in_table_list && orig_table->pos_in_table_list->schema_table)
      field->org_table_name = Lex_cstring_strlen(orig_table->pos_in_table_list->schema_table->table_name);
    else
...
bool Field_int::get_date(MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  return int_to_datetime_with_warn(get_thd(), nr, ltime, fuzzydate, table->s, field_name.str);
}
...
Field_str::Field_str(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg, utype unireg_check_arg,
...
bool Field_str::test_if_equality_guarantees_uniqueness(const Item *item) const
{
  ...
  */
  if (!field_charset()->propagate(0, 0) || item->cmp_type() != STRING_RESULT)
    return false;
  /*
...
bool Field_str::can_be_substituted_to_equal_item(const Context &ctx, const Item_equal *item_equal)
{
  DBUG_ASSERT(item_equal->compare_type_handler()->cmp_type() == STRING_RESULT);
  switch (ctx.subst_constraint())
  {
    ...
    case IDENTITY_SUBST:
      return ((charset()->state & MY_CS_BINSORT) && (charset()->state & MY_CS_NOPAD));
  }
  return false;
}
...
void Field_str::change_charset(const DTCollation &new_cs)
{
  ...
  field_length = (field_length * new_cs.collation->mbmaxlen) / m_collation.collation->mbmaxlen;
  m_collation = new_cs;
}
...
uint Field::fill_cache_field(CACHE_FIELD *copy)
{
  ...
  {
    copy->type = pack_length() - row_pack_length() == 1 ? CACHE_VARSTR1 : CACHE_VARSTR2;
    store_length = 0;
  }
  else
  {
...
bool Field::get_date(MYSQL_TIME *to, date_mode_t mode)
{
  ...
  Temporal_hybrid *t = new (to) Temporal_hybrid(get_thd(), &warn, val_str(&tmp), mode);
...
int Field::store_time_dec(const MYSQL_TIME *ltime, uint dec)
{
  ...
  return store(buff, length, (charset()->state & MY_CS_NONASCII) ? &my_charset_latin1 : charset());
}
...
Field *Field::make_new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type __attribute__((unused)))
{
  Field *tmp;
...
Field *Field::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uint32 length, uchar *new_null_ptr,
                            uint new_null_bit)
{
  Field *tmp;
...
Field *Field::create_tmp_field(MEM_ROOT *mem_root, TABLE *new_table, bool maybe_null_arg)
{
  Field *new_field;
...
Field *Field::clone(MEM_ROOT *root, TABLE *new_table)
{
  ...
  {
    tmp->init(new_table);
    tmp->move_field_offset((my_ptrdiff_t)(new_table->record[0] - new_table->s->default_values));
  }
  return tmp;
}
...
bool Field_null::is_equal(const Column_definition &new_field) const
{
  ...
  return new_field.type_handler() == type_handler() && new_field.charset == field_charset() &&
...
Field_row::~Field_row() { delete m_table; }
...
bool Field_row::sp_prepare_and_store_item(THD *thd, Item **value)
{
  ...
  */
  Item *src;
  if (!(src = thd->sp_fix_func_item(value)) || src->cmp_type() != ROW_RESULT || src->cols() != m_table->s->fields)
  {
  ...
  */
  ...
  if (expo_sign_char == '-')
  {
    while (frac_digits_added_zeros-- > 0)
    {
      if (pos == right_wall)
      {
        if (get_thd()->count_cuted_fields > CHECK_FIELD_EXPRESSION && !is_cuted_fields_incr)
...
int Field_decimal::store(double nr)
{
  ...
  {
    to = ptr;
    for (size_t i = field_length - length; i-- > 0;) *to++ = fyllchar;
...
void Field_decimal::sql_type(String &res) const
{
  ...
  if (dec)
    tmp--;
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "decimal(%d,%d)/*old*/", tmp, dec));
  add_zerofill_and_unsigned(res);
}

Field *Field_decimal::make_new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type)
{
  if (keep_type)
    return Field_real::make_new_field(root, new_table, keep_type);

  Field *field = new (root) Field_new_decimal(NULL, field_length, maybe_null() ? (uchar *)"" : 0, 0, NONE, &field_name,
                                              dec, flags & ZEROFILL_FLAG, unsigned_flag);
  if (field)
...
static decimal_digits_t get_decimal_precision(uint len, decimal_digits_t dec, bool unsigned_val)
{
...
Field_new_decimal::Field_new_decimal(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg,
                                     enum utype unireg_check_arg, const LEX_CSTRING *field_name_arg,
                                     decimal_digits_t dec_arg, bool zero_arg, bool unsigned_arg)
    : Field_num(ptr_arg, len_arg, null_ptr_arg, null_bit_arg, unireg_check_arg, field_name_arg,
...
bool Field_new_decimal::store_value(const my_decimal *decimal_value, int *native_error)
{
  DBUG_ASSERT(marked_for_write_or_computed());
  int error = 0;
  ...
  *native_error = decimal_value->to_binary(ptr, precision, dec, E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW);
  ...
  DBUG_EXECUTE("info", print_decimal_buff(decimal_value, (uchar *)ptr, bin_size););
  DBUG_RETURN(error);
}
...
int Field_new_decimal::store(const char *from, size_t length, CHARSET_INFO *charset_arg)
{
  DBUG_ASSERT(marked_for_write_or_computed());
  ...
  const char *end;
  int err = str2my_decimal(E_DEC_FATAL_ERROR & ~(E_DEC_OVERFLOW | E_DEC_BAD_NUM), from, length, charset_arg,
                           &decimal_value, &end);
  ...
  {
    if (check_edom_and_important_data_truncation("decimal", err && err != E_DEC_TRUNCATED, charset_arg, from, length,
                                                 end))
    {
      if (!thd->abort_on_warning)
      {
  ...
  */
  if (thd->count_cuted_fields > CHECK_FIELD_EXPRESSION &&
      (err == E_DEC_TRUNCATED || err2 == E_DEC_TRUNCATED || end < from + length))
    set_note(WARN_DATA_TRUNCATED, 1);
  DBUG_RETURN(0);
}
...
int Field_new_decimal::cmp(const uchar *a, const uchar *b) const { return memcmp(a, b, bin_size); }

void Field_new_decimal::sort_string(uchar *buff, uint length) { memcpy(buff, ptr, length); }
...
Binlog_type_info Field_new_decimal::binlog_type_info() const
{
  ...
  return Binlog_type_info(Field_new_decimal::type(), precision + (decimals() << 8), 2, binlog_signedness());
}
...
uint Field_new_decimal::pack_length_from_metadata(uint field_metadata) const
{
  ...
  uint const source_size = my_decimal_get_binary_size(source_precision, source_decimal);
  return (source_size);
}

bool Field_new_decimal::compatible_field_size(uint field_metadata, const Relay_log_info *__attribute__((unused)),
                                              uint16 mflags __attribute__((unused)), int *order_var) const
{
  uint const source_precision = (field_metadata >> 8U) & 0x00ff;
  uint const source_decimal = field_metadata & 0x00ff;
...
bool Field_new_decimal::is_equal(const Column_definition &new_field) const
{
  return ((new_field.type_handler() == type_handler()) &&
          ...
          (new_field.length == max_display_length()) && (new_field.decimals == dec));
}
...
/**
   ...
   @return  New pointer into memory based on from + length of the data
*/
const uchar *Field_new_decimal::unpack(uchar *to, const uchar *from, const uchar *from_end, uint param_data)
{
  if (param_data == 0)
  ...
  if ((from_pack_len && (from_pack_len < length)) || (from_precision < precision) || (from_decimal < decimals()))
  {
    /*
...
Item *Field_new_decimal::get_equal_const_item(THD *thd, const Context &ctx, Item *const_item)
{
  ...
  {
    case IDENTITY_SUBST:
      if (const_item->field_type() != MYSQL_TYPE_NEWDECIMAL || const_item->decimal_scale() != decimals())
      {
        VDec val(const_item);
        if (val.is_null())
        {
        ...
        */
        ...
        return new (thd->mem_root) Item_decimal(thd, field_name.str, &tmp, decimals(), field_length);
      }
      break;
    case ANY_SUBST:
      break;
...
void Field_int::sql_type(String &res) const
{
  ...
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "%.*s(%d)", (int)name.length(), name.ptr(),
                                (int)field_length));
  add_zerofill_and_unsigned(res);
}
...
String *Field_int::val_str_from_long(String *val_buffer, uint max_char_length, int radix, long nr)
{
...
int Field_longlong::store(const char *from, size_t len, CHARSET_INFO *cs)
{
  ...
  else if (get_thd()->count_cuted_fields > CHECK_FIELD_EXPRESSION && check_int(cs, from, len, end, error))
    error = 1;
  else
    error = 0;
  int8store(ptr, tmp);
...
String *Field_longlong::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  ...
  length = (uint)(cs->longlong10_to_str)(to, mlength, unsigned_flag ? 10 : -10, j);
  val_buffer->length(length);
  if (zerofill)
...
{
  DBUG_ASSERT(marked_for_write_or_computed());
  int error = truncate_double(&nr, field_length, not_fixed ? NOT_FIXED_DEC : dec, unsigned_flag, FLT_MAX);
  if (unlikely(error))
  {
    set_warning(ER_WARN_DATA_OUT_OF_RANGE, 1);
...
{
  return Field_float::store(unsigned_val ? ulonglong2double((ulonglong)nr) : (double)nr);
}
...
{
  DBUG_ASSERT(marked_for_write_or_computed());
  int error = truncate_double(&nr, field_length, not_fixed ? NOT_FIXED_DEC : dec, unsigned_flag, DBL_MAX);
  if (unlikely(error))
  {
    set_warning(ER_WARN_DATA_OUT_OF_RANGE, 1);
    if (error < 0)  // Wrong double value
...
{
  return Field_double::store(unsigned_val ? ulonglong2double((ulonglong)nr) : (double)nr);
}
...
int truncate_double(double *nr, uint field_length, decimal_digits_t dec, bool unsigned_flag, double max_value)
{
  ...
  if (dec < FLOATING_POINT_DECIMALS)
  {
    ...
    for (; order > step; order -= step) max_value_by_dec *= log_10[step];
...
Value_source::Converter_double_to_longlong::Converter_double_to_longlong(double nr, bool unsigned_flag) : m_error(false)
{
  nr = rint(nr);
  if (unsigned_flag)
  {
...
void Value_source::Converter_double_to_longlong::push_warning(THD *thd, double nr, bool unsigned_flag)
{
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_DATA_OVERFLOW, ER_THD(thd, ER_DATA_OVERFLOW),
                      ErrConvDouble(nr).ptr(), unsigned_flag ? "UNSIGNED INT" : "INT");
}

int Field_real::store_time_dec(const MYSQL_TIME *ltime, uint dec_arg) { return store(TIME_to_double(ltime)); }
...
bool Field_real::get_date(MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  DBUG_ASSERT(marked_for_read());
  double nr = val_real();
  return double_to_datetime_with_warn(get_thd(), nr, ltime, fuzzydate, table->s, field_name.str);
}

Item *Field_real::get_equal_const_item(THD *thd, const Context &ctx, Item *const_item)
{
  if (flags & ZEROFILL_FLAG)
    return Field_num::get_equal_zerofill_const_item(thd, ctx, const_item);
  switch (ctx.subst_constraint())
...
void Field_real::sql_type(String &res) const
{
  ...
  {
    CHARSET_INFO *cs = res.charset();
    res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "%.*s(%d,%d)", (int)name.length(),
                                  name.ptr(), (int)field_length, dec));
  }
  add_zerofill_and_unsigned(res);
}
...
Field_timestamp::Field_timestamp(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg,
...
sql_mode_t Field_timestamp::conversion_depends_on_sql_mode(THD *thd, Item *expr) const
{
  return expr->datetime_precision(thd) > decimals() ? MODE_TIME_ROUND_FRACTIONAL : 0;
}
...
my_time_t Field_timestamp0::get_timestamp(const uchar *pos, ulong *sec_part) const
{
  DBUG_ASSERT(marked_for_read());
  *sec_part = 0;
...
bool Field_timestamp0::val_native(Native *to)
{
  ...
  return Timestamp_or_zero_datetime(Timestamp(sec, 0), sec == 0).to_native(to, 0);
}
...
date_conv_mode_t Timestamp::sql_mode_for_timestamp(THD *thd)
{
  ...
  return date_conv_mode_t((thd->variables.sql_mode & MODE_NO_ZERO_DATE) | MODE_NO_ZERO_IN_DATE);
}
...
int Field_timestamp::store_timestamp_dec(const timeval &ts, uint dec)
{
  ...
  if (ts.tv_sec == 0 && ts.tv_usec == 0 && get_thd()->variables.sql_mode & (ulonglong)TIME_NO_ZERO_DATE)
...
int Field_timestamp::zero_time_stored_return_code_with_warning()
{
  ...
  {
    ErrConvString s(STRING_WITH_LEN("0000-00-00 00:00:00.000000") - (decimals() ? 6 - decimals() : 7),
                    system_charset_info);
...
longlong Field_timestamp::val_int(void)
{
  ...
  return ltime.year * 10000000000LL + ltime.month * 100000000LL + ltime.day * 1000000L + ltime.hour * 10000L +
...
String *Field_timestamp::val_str(String *val_buffer, String *val_ptr)
{
  ...
  *to = 0;
  ...
  if ((dec = decimals()))
  {
    ...
    for (int i = dec; i > 0; i--, sec_part /= 10) buf[i] = (char)(sec_part % 10) + '0';
    buf[0] = '.';
    buf[dec + 1] = 0;
  }
  return val_buffer;
...
bool Field_timestamp::validate_value_in_record(THD *thd, const uchar *record) const
{
  ...
  switch (bytes)
  {
    case 1:
      *to = (uchar)num;
      break;
    case 2:
      shortstore(to, (ushort)num);
      break;
    case 3:
      int3store(to, num); /* Sic!*/
      break;
    case 4:
      longstore(to, (ulong)num);
      break;
    case 8:
      longlongstore(to, num);
      break;
    default:
      DBUG_ASSERT(0);
  }
...
my_time_t Field_timestamp_hires::get_timestamp(const uchar *pos, ulong *sec_part) const
{
  DBUG_ASSERT(marked_for_read());
...
bool Field_timestamp_hires::val_native(Native *to)
{
  ...
  return Timestamp_or_zero_datetime(Timestamp(tm), tm.tv_sec == 0).to_native(to, dec);
}
...
double Field_timestamp_with_dec::val_real(void)
{
  ...
  return ltime.year * 1e10 + ltime.month * 1e8 + ltime.day * 1e6 + ltime.hour * 1e4 + ltime.minute * 1e2 +
         ltime.second + ltime.second_part * 1e-6;
}
...
int Field_timestamp_hires::cmp(const uchar *a_ptr, const uchar *b_ptr) const
{
  ...
  return ((uint32)a < (uint32)b)   ? -1
         : ((uint32)a > (uint32)b) ? 1
         : a_sec_part < b_sec_part ? -1
         : a_sec_part > b_sec_part ? 1
                                   : 0;
}
...
void Field_timestampf::store_TIMEVAL(const timeval &tm) { my_timestamp_to_binary(&tm, ptr, dec); }
...
bool Field_timestampf::is_max()
{
  ...
  DBUG_RETURN(mi_sint4korr(ptr) == TIMESTAMP_MAX_VALUE && mi_sint3korr(ptr + 4) == TIME_MAX_SECOND_PART);
}

my_time_t Field_timestampf::get_timestamp(const uchar *pos, ulong *sec_part) const
{
  struct timeval tm;
...
/*************************************************************/
sql_mode_t Field_temporal::can_handle_sql_mode_dependency_on_store() const { return MODE_PAD_CHAR_TO_FULL_LENGTH; }

bool Field_temporal::is_equal(const Column_definition &new_field) const
{
  return new_field.type_handler() == type_handler() && new_field.length == max_display_length();
}

void Field_temporal::set_warnings(Sql_condition::enum_warning_level trunc_level, const ErrConv *str, int was_cut,
                                  const char *typestr)
{
  /*
    error code logic:
...
void Field_temporal::sql_type_dec_comment(String &res, const Name &name, uint dec, const Name &comment) const
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "%.*s(%u)%s%.*s%s", (uint)name.length(),
                                name.ptr(), dec, comment.length() ? " /* " : "", (uint)comment.length(), comment.ptr(),
                                comment.length() ? " */" : ""));
}

void Field_temporal::sql_type_comment(String &res, const Name &name, const Name &comment) const
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "%.*s%s%.*s%s", (uint)name.length(),
                                name.ptr(), comment.length() ? " /* " : "", (uint)comment.length(), comment.ptr(),
                                comment.length() ? " */" : ""));
}
...
*/
int Field_datetime::store_TIME_with_warning(const Datetime *dt, const ErrConv *str, int was_cut)
{
  DBUG_ASSERT(marked_for_write_or_computed());
  // Handle totally bad values
  if (!dt->is_valid_datetime())
...
bool Field_temporal_with_date::validate_value_in_record(THD *thd, const uchar *record) const
{
  DBUG_ASSERT(!is_null_in_record(record));
  MYSQL_TIME ltime;
...
bool Field_temporal::can_optimize_keypart_ref(const Item_bool_func *cond, const Item *value) const
{
  return true;  // Field is of TIME_RESULT, which supersedes everything else.
}

bool Field_temporal::can_optimize_group_min_max(const Item_bool_func *cond, const Item *const_item) const
{
  return true;  // Field is of TIME_RESULT, which supersedes everything else.
}

Item *Field_temporal::get_equal_const_item_datetime(THD *thd, const Context &ctx, Item *const_item)
{
  switch (ctx.subst_constraint())
  {
    case IDENTITY_SUBST:
      if ((const_item->field_type() != MYSQL_TYPE_DATETIME && const_item->field_type() != MYSQL_TYPE_TIMESTAMP) ||
    ...
    case ANY_SUBST:
      if (!is_temporal_type_with_date(const_item->field_type()))
      {
        ...
        if (!dt.is_valid_datetime())
          return NULL;
        return new (thd->mem_root) Item_datetime_literal_for_invalid_dates(
            thd, &dt, dt.get_mysql_time()->second_part ? TIME_SECOND_PART_DIGITS : 0);
      }
      break;
  }
  return const_item;
...
****************************************************************************/
sql_mode_t Field_time::conversion_depends_on_sql_mode(THD *thd, Item *expr) const
{
  return expr->time_precision(thd) > decimals() ? MODE_TIME_ROUND_FRACTIONAL : 0;
}

int Field_time::store_TIME_with_warning(const Time *t, const ErrConv *str, int warn)
{
  DBUG_ASSERT(marked_for_write_or_computed());
  // Handle totally bad values
  if (!t->is_valid_time())
...
void Field_time0::store_TIME(const MYSQL_TIME *ltime)
{
  ...
  long tmp = (ltime->day * 24L + ltime->hour) * 10000L + (ltime->minute * 100 + ltime->second);
  if (ltime->neg)
    tmp = -tmp;
  int3store(ptr, tmp);
}
...
Field *Field_time::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uint32 length, uchar *new_null_ptr,
                                 uint new_null_bit)
{
  THD *thd = get_thd();
  Field_time *res = (Field_time *)Field::new_key_field(root, new_table, new_ptr, length, new_null_ptr, new_null_bit);
...
String *Field_time::val_str(String *str, String *unused __attribute__((unused)))
{
  DBUG_ASSERT(marked_for_read());
  MYSQL_TIME ltime;
...
bool Field_time::can_be_substituted_to_equal_item(const Context &ctx, const Item_equal *item_equal)
{
  DBUG_ASSERT(item_equal->compare_type_handler()->cmp_type() != STRING_RESULT);
  switch (ctx.subst_constraint())
  {
...
Item *Field_time::get_equal_const_item(THD *thd, const Context &ctx, Item *const_item)
{
  /*
  ...
  */
  Time::datetime_to_time_mode_t mode = (thd->variables.old_behavior & OLD_MODE_ZERO_DATE_TIME_CAST)
      ...
      if (const_item->field_type() != MYSQL_TYPE_TIME)
      {
        ...
        */
        return new (thd->mem_root)
            Item_time_literal(thd, &tm, tm.get_mysql_time()->second_part ? TIME_SECOND_PART_DIGITS : 0);
      }
      break;
    case IDENTITY_SUBST:
      if (const_item->field_type() != MYSQL_TYPE_TIME || const_item->decimals != decimals())
      {
        /*
        ...
        */
        Time tm(thd, const_item, Time::Options(TIME_TIME_ONLY, thd, mode), decimals());
        if (!tm.is_valid_time())
          return NULL;
...
int Field_year::store(const char *from, size_t len, CHARSET_INFO *cs)
{
  ...
  if (nr < 0 || (nr >= 100 && nr <= 1900) || nr > 2155 || error == MY_ERRNO_ERANGE)
  {
    *ptr = 0;
    set_warning(ER_WARN_DATA_OUT_OF_RANGE, 1);
    return 1;
  ...
  if (thd->count_cuted_fields > CHECK_FIELD_EXPRESSION && (error = check_int(cs, from, len, end, error)))
  {
...
bool Field_year::get_date(MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  if (tmp || field_length != 4)
    tmp += 1900;
  return int_to_datetime_with_warn(get_thd(), Longlong_hybrid(tmp * 10000, true), ltime, fuzzydate, table->s,
                                   field_name.str);
}
...
int Field_date_common::store_TIME_with_warning(const Datetime *dt, const ErrConv *str, int was_cut)
{
  DBUG_ASSERT(marked_for_write_or_computed());
  // Handle totally bad values
  if (!dt->is_valid_datetime())
...
bool Field_date::get_TIME(MYSQL_TIME *ltime, const uchar *pos, date_mode_t fuzzydate) const
{
  DBUG_ASSERT(marked_for_read());
...
String *Field_date::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  ...
  uint length = (uint)my_date_to_str(&ltime, const_cast<char *>(val_buffer->ptr()));
  val_buffer->length(length);
  val_buffer->set_charset(&my_charset_numeric);
...
void Field_date::sql_type(String &res) const { res.set_ascii(STRING_WITH_LEN("date")); }
...
bool Field_newdate::get_TIME(MYSQL_TIME *ltime, const uchar *pos, date_mode_t fuzzydate) const
{
  DBUG_ASSERT(marked_for_read());
...
Item *Field_newdate::get_equal_const_item(THD *thd, const Context &ctx, Item *const_item)
{
  switch (ctx.subst_constraint())
  {
    case ANY_SUBST:
...
sql_mode_t Field_datetime::conversion_depends_on_sql_mode(THD *thd, Item *expr) const
{
  return expr->datetime_precision(thd) > decimals() ? MODE_TIME_ROUND_FRACTIONAL : 0;
}
...
String *Field_datetime0::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  val_buffer->alloc(field_length + 1);
  val_buffer->length(field_length);
...
bool Field_datetime0::get_TIME(MYSQL_TIME *ltime, const uchar *pos, date_mode_t fuzzydate) const
{
  DBUG_ASSERT(marked_for_read());
...
int Field_datetime::set_time()
{
  ...
  if (decimals())
    store_datetime(Datetime(thd, Timeval(thd->query_start(), thd->query_start_sec_part())).trunc(decimals()));
  else
...
String *Field_datetime_with_dec::val_str(String *str, String *unused __attribute__((unused)))
{
  MYSQL_TIME ltime;
  get_date(&ltime, date_mode_t(0));
  str->alloc(field_length + 1);
...
bool Field_datetime_hires::get_TIME(MYSQL_TIME *ltime, const uchar *pos, date_mode_t fuzzydate) const
{
  DBUG_ASSERT(marked_for_read());
...
bool Field_datetimef::get_TIME(MYSQL_TIME *ltime, const uchar *pos, date_mode_t fuzzydate) const
{
  DBUG_ASSERT(marked_for_read());
...
bool Field_longstr::check_string_copy_error(const String_copier *copier, const char *end, CHARSET_INFO *cs)
{
  const char *pos;
  char tmp[32];
...
int Field_longstr::report_if_important_data(const char *pstr, const char *end, bool count_spaces)
{
  THD *thd;
  if ((pstr < end) && (thd = get_thd())->count_cuted_fields > CHECK_FIELD_EXPRESSION)
  {
...
int Field_string::store(const char *from, size_t length, CHARSET_INFO *cs)
{
  ...
  rc = well_formed_copy_with_check((char *)ptr, field_length, cs, from, length, Field_string::char_length(), false,
                                   &copy_length);

  /* Append spaces if the string was shorter than the field. */
  if (copy_length < field_length)
    field_charset()->fill((char *)ptr + copy_length, field_length - copy_length, field_charset()->pad_char);
...
int Field_str::store(longlong nr, bool unsigned_val)
{
  char buff[64];
  uint length;
  length = (uint)(field_charset()->longlong10_to_str)(buff, sizeof(buff), (unsigned_val ? 10 : -10), nr);
...
{
  DBUG_ASSERT(!compression_method());
  return new_field.type_handler() == type_handler() && new_field.char_length == char_length() &&
         new_field.charset == field_charset() && new_field.length == max_display_length();
}
...
bool Field_longstr::cmp_to_string_with_same_collation(const Item_bool_func *cond, const Item *item) const
{
  return cmp_is_done_using_type_handler_of_this(cond, item) && charset() == cond->compare_collation();
}

bool Field_longstr::cmp_to_string_with_stricter_collation(const Item_bool_func *cond, const Item *item) const
{
  return cmp_is_done_using_type_handler_of_this(cond, item) &&
         (charset() == cond->compare_collation() || cond->compare_collation()->state & MY_CS_BINSORT);
}

bool Field_longstr::can_optimize_keypart_ref(const Item_bool_func *cond, const Item *item) const
{
  DBUG_ASSERT(cmp_type() == STRING_RESULT);
  return cmp_to_string_with_stricter_collation(cond, item);
}

bool Field_longstr::can_optimize_hash_join(const Item_bool_func *cond, const Item *item) const
{
  DBUG_ASSERT(cmp_type() == STRING_RESULT);
  return cmp_to_string_with_same_collation(cond, item);
}

bool Field_longstr::can_optimize_group_min_max(const Item_bool_func *cond, const Item *const_item) const
{
  /*
...
bool Field_longstr::can_optimize_range(const Item_bool_func *cond, const Item *item, bool is_eq_func) const
{
  return is_eq_func ? cmp_to_string_with_stricter_collation(cond, item) : cmp_to_string_with_same_collation(cond, item);
}
...
*/
Field_string::Warn_filter_string::Warn_filter_string(const THD *thd, const Field_string *field)
    : Warn_filter(!thd->no_errors, !thd->no_errors && field->field_charset() == &my_charset_bin)
{
}
...
double Field_string::val_real(void)
{
  DBUG_ASSERT(marked_for_read());
  THD *thd = get_thd();
  return Converter_strntod_with_warn(get_thd(), Warn_filter_string(thd, this), Field_string::charset(),
...
longlong Field_string::val_int(void)
{
  DBUG_ASSERT(marked_for_read());
  THD *thd = get_thd();
  return Converter_strntoll_with_warn(thd, Warn_filter_string(thd, this), Field_string::charset(), (const char *)ptr,
                                      field_length)
      .result();
}
...
my_decimal *Field_string::val_decimal(my_decimal *decimal_value)
{
  DBUG_ASSERT(marked_for_read());
  THD *thd = get_thd();
  Converter_str2my_decimal_with_warn(thd, Warn_filter_string(thd, this), E_DEC_FATAL_ERROR & ~E_DEC_BAD_NUM,
                                     Field_string::charset(), (const char *)ptr, field_length, decimal_value);
  return decimal_value;
}
...
static bool check_field_for_37426(const void *param_arg)
{
  ...
  DBUG_PRINT("debug", ("Field %s - type: %d, size: %d", param->field->field_name.str, param->field->real_type(),
                       param->field->row_pack_length()));
  return param->field->row_pack_length() > 255;
}
#endif

bool Field_string::compatible_field_size(uint field_metadata, const Relay_log_info *rli_arg,
                                         uint16 mflags __attribute__((unused)), int *order_var) const
{
#ifdef HAVE_REPLICATION
  const Check_field_param check_param = {this};
  if (rpl_master_has_bug(rli_arg, 37426, TRUE, check_field_for_37426, &check_param))
...
{
  return field_charset()->coll->strnncollsp_nchars(field_charset(), a_ptr, field_length, b_ptr, field_length,
...
void Field_string::sort_string(uchar *to, uint length)
{
  ...
  size_t tmp =
#endif
      field_charset()->strnxfrm(to, length, char_length() * field_charset()->strxfrm_multiply, ptr, field_length,
...
void Field_string::sql_type(String &res) const
{
  ...
  length =
      cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "%s(%d)%s",
                         (type() == MYSQL_TYPE_VAR_STRING ? (has_charset() ? "varchar" : "varbinary")
                                                          : (has_charset() ? "char" : "binary")),
                         (int)field_length / charset()->mbmaxlen, type() == MYSQL_TYPE_VAR_STRING ? "/*old*/" : "");
  res.length(length);
...
void Field_string::sql_rpl_type(String *res) const
{
  ...
  if (Field_string::has_charset())
  {
    size_t length = cs->cset->snprintf(cs, (char *)res->ptr(), res->alloced_length(),
                                       "char(%u octets) character set %s", field_length, charset()->cs_name.str);
    res->length(length);
  }
  else
...
/**
   ...
   @return  New pointer into memory based on from + length of the data
*/
const uchar *Field_string::unpack(uchar *to, const uchar *from, const uchar *from_end, uint param_data)
{
  return StringPack(field_charset(), field_length).unpack(to, from, from_end, param_data);
}
...
Binlog_type_info_fixed_string::Binlog_type_info_fixed_string(uchar type_code, uint32 octets, CHARSET_INFO *cs)
    : Binlog_type_info(type_code, 0, 2, cs)
{
  ...
  m_metadata = (type_code ^ ((octets & 0x300) >> 4)) + (((uint)(octets & 0xFF)) << 8);
}

Binlog_type_info Field_string::binlog_type_info() const
{
  DBUG_ASSERT(Field_string::type() == binlog_type());
  return Binlog_type_info_fixed_string(Field_string::binlog_type(), field_length, charset());
}
...
uint Field_string::max_packed_col_length(uint max_length) { return StringPack::max_packed_col_length(max_length); }

uint Field_string::get_key_image(uchar *buff, uint length, const uchar *ptr_arg, imagetype type_arg) const
{
  size_t bytes = field_charset()->charpos((char *)ptr_arg, (char *)ptr_arg + field_length, length / mbmaxlen());
  memcpy(buff, ptr_arg, bytes);
  if (bytes < length)
    field_charset()->fill((char *)buff + bytes, length - bytes, field_charset()->pad_char);
  return (uint)bytes;
}

Field *Field_string::make_new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type)
{
  Field *field;
  if (type() != MYSQL_TYPE_VAR_STRING || keep_type)
    field = Field::make_new_field(root, new_table, keep_type);
  else if ((field = new (root) Field_varstring(field_length, maybe_null(), &field_name, new_table->s, charset())))
  {
    /*
...
{
  return field_length >= MIN_STRING_LENGTH_TO_PACK_ROWS ? FIELD_SKIP_ENDSPACE : FIELD_NORMAL;
}
...
bool Field_varstring::memcpy_field_possible(const Field *from) const
{
  return (Field_str::memcpy_field_possible(from) && !compression_method() == !from->compression_method() &&
          ...
          (table->file && !(table->file->ha_table_flags() & HA_RECORD_MUST_BE_CLEAN_ON_WRITE)));
}
...
int Field_varstring::store(const char *from, size_t length, CHARSET_INFO *cs)
{
  ...
  rc = well_formed_copy_with_check((char *)get_data(), field_length, cs, from, length, Field_varstring::char_length(),
                                   true, &copy_length);
...
double Field_varstring::val_real(void)
{
  DBUG_ASSERT(marked_for_read());
  THD *thd = get_thd();
  return Converter_strntod_with_warn(thd, Warn_filter(thd), Field_varstring::charset(), (const char *)get_data(),
                                     get_length())
      .result();
}
...
longlong Field_varstring::val_int(void)
{
  DBUG_ASSERT(marked_for_read());
  THD *thd = get_thd();
  return Converter_strntoll_with_warn(thd, Warn_filter(thd), Field_varstring::charset(), (const char *)get_data(),
                                      get_length())
      .result();
}
...
my_decimal *Field_varstring::val_decimal(my_decimal *decimal_value)
{
  DBUG_ASSERT(marked_for_read());
  THD *thd = get_thd();
  Converter_str2my_decimal_with_warn(thd, Warn_filter(thd), E_DEC_FATAL_ERROR & ~E_DEC_BAD_NUM,
                                     Field_varstring::charset(), (const char *)get_data(), get_length(), decimal_value);
  return decimal_value;
}
...
{
  return protocol->store((const char *)get_data(), get_length(), field_charset());
}
...
int Field_varstring::cmp(const uchar *a_ptr, const uchar *b_ptr) const
{
  ...
  diff = field_charset()->strnncollsp(a_ptr + length_bytes, a_length, b_ptr + length_bytes, b_length);
  return diff;
}

int Field_varstring::cmp_prefix(const uchar *a_ptr, const uchar *b_ptr, size_t prefix_len) const
{
  ...
  return field_charset()->coll->strnncollsp_nchars(field_charset(), a_ptr + length_bytes, a_length,
...
int Field_varstring::key_cmp(const uchar *key_ptr, uint max_key_length) const
{
  ...
  return field_charset()->strnncollsp(ptr + length_bytes, length, key_ptr + HA_KEY_BLOB_LENGTH, uint2korr(key_ptr));
}
...
{
  return field_charset()->strnncollsp(a + HA_KEY_BLOB_LENGTH, uint2korr(a), b + HA_KEY_BLOB_LENGTH, uint2korr(b));
}
...
void Field_varstring::sort_string(uchar *to, uint length)
{
  ...
  size_t rc =
#endif
      field_charset()->strnxfrm(to, length, char_length() * field_charset()->strxfrm_multiply, (const uchar *)buf.ptr(),
                                buf.length(), MY_STRXFRM_PAD_WITH_SPACE | MY_STRXFRM_PAD_TO_MAXLEN);
...
void Field_varstring::sql_type(String &res) const
{
  ...
  length = cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "%s(%u)",
                              (has_charset() ? "varchar" : "varbinary"), (uint)char_length());
  res.length(length);
  if ((thd->variables.sql_mode & (MODE_MYSQL323 | MODE_MYSQL40)) && has_charset() && (charset()->state & MY_CS_BINSORT))
    res.append(STRING_WITH_LEN(" binary"));
}
...
void Field_varstring::sql_rpl_type(String *res) const
{
  ...
  {
    size_t length = cs->cset->snprintf(cs, (char *)res->ptr(), res->alloced_length(),
                                       "varchar(%u octets) character set %s", field_length, charset()->cs_name.str);
    res->length(length);
  }
  else
...
/**
   ...
   @return  New pointer into memory based on from + length of the data
*/
const uchar *Field_varstring::unpack(uchar *to, const uchar *from, const uchar *from_end, uint param_data)
{
  uint length;
  uint l_bytes = (param_data && (param_data < field_length)) ? (param_data <= 255) ? 1 : 2 : length_bytes;
...
uint Field_varstring::get_key_image(uchar *buff, uint length, const uchar *ptr_arg, imagetype type_arg) const
{
  String val;
...
int Field_varstring::cmp_binary(const uchar *a_ptr, const uchar *b_ptr, uint32 max_length) const
{
...
Field *Field_varstring::make_new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type)
{
  Field_varstring *res = (Field_varstring *)Field::make_new_field(root, new_table, keep_type);
  if (res)
    res->length_bytes = length_bytes;
  return res;
}

Field *Field_varstring::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uint32 length,
                                      uchar *new_null_ptr, uint new_null_bit)
{
  Field_varstring *res;
  if ((res = (Field_varstring *)Field::new_key_field(root, new_table, new_ptr, length, new_null_ptr, new_null_bit)))
  {
...
{
  return new_field.type_handler() == type_handler() && new_field.length == field_length &&
         new_field.char_length == char_length() && !new_field.compression_method() == !compression_method() &&
...
int Field_longstr::compress(char *to, uint to_length, const char *from, uint length, uint max_length, uint *out_length,
                            CHARSET_INFO *cs, size_t nchars)
{
  ...
  if (String::needs_conversion_on_storage(length, cs, field_charset()) || max_length < length)
  {
    ...
    rc = well_formed_copy_with_check(buf, max_length, cs, from, length, nchars, true, &buf_length);
  }
  else
  {
  ...
  else
  {
    ...
    else
    {
      ...
      rc = well_formed_copy_with_check(to + 1, to_length - 1, cs, from, length, nchars, true, &buf_length);
    }
...
String *Field_longstr::uncompress(String *val_buffer, String *val_ptr, const uchar *from, uint from_length) const
{
  if (from_length)
  {
    ...
    {
      if (!compression_methods[method].uncompress(val_buffer, from, from_length, field_length))
      {
...
int Field_varstring_compressed::store(const char *from, size_t length, CHARSET_INFO *cs)
{
  DBUG_ASSERT(marked_for_write_or_computed());
  uint compressed_length;
  int rc =
      compress((char *)get_data(), field_length, from, (uint)length, Field_varstring_compressed::max_display_length(),
               &compressed_length, cs, Field_varstring_compressed::char_length());
...
double Field_varstring_compressed::val_real(void)
{
  ...
  String buf;
  val_str(&buf, &buf);
  return Converter_strntod_with_warn(thd, Warn_filter(thd), field_charset(), buf.ptr(), buf.length()).result();
}
...
longlong Field_varstring_compressed::val_int(void)
{
  ...
  String buf;
  val_str(&buf, &buf);
  return Converter_strntoll_with_warn(thd, Warn_filter(thd), field_charset(), buf.ptr(), buf.length()).result();
}

int Field_varstring_compressed::cmp(const uchar *a_ptr, const uchar *b_ptr) const
{
  String a, b;
  uint a_length, b_length;
...
{
  return Binlog_type_info(Field_varstring_compressed::binlog_type(), field_length, 2, charset());
}
...
Field_blob::Field_blob(uchar *ptr_arg, uchar *null_ptr_arg, uchar null_bit_arg, enum utype unireg_check_arg,
...
int Field_blob::copy_value(Field_blob *from)
{
  ...
  if (packlength < from->packlength)
  {
    ...
    length = (uint32)Well_formed_prefix(field_charset(), (const char *)data, length).length();
    rc = report_if_important_data((const char *)data + length, (const char *)data + from->get_length(), true);
  }
  store_length(length);
...
int Field_blob::store(const char *from, size_t length, CHARSET_INFO *cs)
{
  ...
  */
  ...
  {
    copy_length = my_copy_with_hex_escaping(field_charset(), tmp, new_length, from, length);
    Field_blob::store_length(copy_length);
    bmove(ptr + packlength, (uchar *)&tmp, sizeof(char *));
    return 0;
  }
  rc = well_formed_copy_with_check((char *)value.ptr(), (uint)new_length, cs, from, length, length, true, &copy_len);
...
double Field_blob::val_real(void)
{
  ...
  if (!blob)
    return 0.0;
  THD *thd = get_thd();
  return Converter_strntod_with_warn(thd, Warn_filter(thd), Field_blob::charset(), blob, get_length(ptr)).result();
}
...
longlong Field_blob::val_int(void)
{
  ...
  if (!blob)
    return 0;
  THD *thd = get_thd();
  return Converter_strntoll_with_warn(thd, Warn_filter(thd), Field_blob::charset(), blob, get_length(ptr)).result();
}
...
my_decimal *Field_blob::val_decimal(my_decimal *decimal_value)
{
  ...
  THD *thd = get_thd();
  Converter_str2my_decimal_with_warn(thd, Warn_filter(thd), E_DEC_FATAL_ERROR & ~E_DEC_BAD_NUM, Field_blob::charset(),
                                     blob, length, decimal_value);
  return decimal_value;
}

int Field_blob::cmp(const uchar *a, uint32 a_length, const uchar *b, uint32 b_length) const
{
...
int Field_blob::cmp_prefix(const uchar *a_ptr, const uchar *b_ptr, size_t prefix_len) const
{
  uchar *blob1, *blob2;
  memcpy(&blob1, a_ptr + packlength, sizeof(char *));
  memcpy(&blob2, b_ptr + packlength, sizeof(char *));
  size_t a_len = get_length(a_ptr), b_len = get_length(b_ptr);
  return field_charset()->coll->strnncollsp_nchars(field_charset(), blob1, a_len, blob2, b_len,
                                                   prefix_len / field_charset()->mbmaxlen);
}

int Field_blob::cmp_binary(const uchar *a_ptr, const uchar *b_ptr, uint32 max_length) const
{
  char *a, *b;
  uint diff;
  uint32 a_length, b_length;
...
uint Field_blob::get_key_image_itRAW(const uchar *ptr_arg, uchar *buff, uint length) const
{
  size_t blob_length = get_length(ptr_arg);
  const uchar *blob = get_ptr(ptr_arg);
  size_t local_char_length = length / mbmaxlen();
  local_char_length = field_charset()->charpos(blob, blob + blob_length, local_char_length);
...
int Field_blob::key_cmp(const uchar *key_ptr, uint max_key_length) const
{
  ...
  local_char_length = cs->charpos(blob1, blob1 + blob_length, local_char_length);
  set_if_smaller(blob_length, local_char_length);
  return Field_blob::cmp(blob1, (uint32)blob_length, key_ptr + HA_KEY_BLOB_LENGTH, uint2korr(key_ptr));
}

int Field_blob::key_cmp(const uchar *a, const uchar *b) const
{
  return Field_blob::cmp(a + HA_KEY_BLOB_LENGTH, uint2korr(a), b + HA_KEY_BLOB_LENGTH, uint2korr(b));
}

Field *Field_blob::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uint32 length, uchar *new_null_ptr,
                                 uint new_null_bit)
{
  Field_varstring *res = new (root)
      Field_varstring(new_ptr, length, 2, new_null_ptr, new_null_bit, Field::NONE, &field_name, table->s, charset());
  res->init(new_table);
  return res;
}
...
Binlog_type_info Field_blob::binlog_type_info() const
{
  ...
  return Binlog_type_info(Field_blob::type(), pack_length_no_ptr(), 1, charset());
}

uint32 Field_blob::sort_length() const
{
  return packlength == 4 ? UINT_MAX32 : (uint32)field_length + sort_suffix_length();
}

uint32 Field_blob::sort_suffix_length() const { return field_charset() == &my_charset_bin ? packlength : 0; }
...
void Field_blob::sort_string(uchar *to, uint length)
{
  ...
  else
  {
    ...
    size_t rc =
#endif
        field_charset()->strnxfrm(to, length, length, (const uchar *)buf.ptr(), buf.length(),
                                  MY_STRXFRM_PAD_WITH_SPACE | MY_STRXFRM_PAD_TO_MAXLEN);
    DBUG_ASSERT(rc == length);
  }
}
...
const Type_handler *Field_blob::type_handler() const
{
  ...
  */
  if (Type_handler_json_common::has_json_valid_constraint(this))
    return Type_handler_json_common::json_blob_type_handler_by_length_bytes(packlength);
...
const uchar *Field_blob::unpack(uchar *to, const uchar *from, const uchar *from_end, uint param_data)
{
  DBUG_ENTER("Field_blob::unpack");
  DBUG_PRINT("enter", ("to: %p; from: %p; param_data: %u", to, from, param_data));
  uint const master_packlength = param_data > 0 ? param_data & 0xFF : packlength;
...
{
  return new_field.type_handler() == type_handler() && !new_field.compression_method() == !compression_method() &&
         new_field.pack_length == pack_length() && new_field.charset == field_charset();
}
...
int Field_blob_compressed::store(const char *from, size_t length, CHARSET_INFO *cs)
{
  DBUG_ASSERT(marked_for_write_or_computed());
  uint compressed_length;
  ...
  rc = compress((char *)value.ptr(), to_length, tmp.ptr(), (uint)length, max_length, &compressed_length, cs,
                (uint)length);
...
{
  return Binlog_type_info(Field_blob_compressed::binlog_type(), pack_length_no_ptr(), 1, charset());
}
...
sql_mode_t Field_enum::can_handle_sql_mode_dependency_on_store() const { return MODE_PAD_CHAR_TO_FULL_LENGTH; }
...
void Field_enum::store_type(ulonglong value) { store_lowendian(value, ptr, packlength); }
...
longlong Field_enum::val_int(const uchar *real_ptr) const { return read_lowendian(real_ptr, packlength); }
...
Binlog_type_info Field_enum::binlog_type_info() const
{
  ...
  return Binlog_type_info(Field_enum::type(), real_type() + (pack_length() << 8), 2, charset(),
                          (TYPELIB *)get_typelib(), NULL);
}
...
Field *Field_enum::make_new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type)
{
  Field_enum *res = (Field_enum *)Field::make_new_field(root, new_table, keep_type);
  if (res)
...
Binlog_type_info Field_set::binlog_type_info() const
{
  ...
  return Binlog_type_info(Field_set::type(), real_type() + (pack_length() << 8), 2, charset(), NULL,
                          (TYPELIB *)get_typelib());
}
...
static bool compare_type_names(CHARSET_INFO *charset, const TYPELIB *t1, const TYPELIB *t2)
{
  for (uint i = 0; i < t1->count; i++)
    if (charset->strnncoll(t1->type_names[i], t1->type_lengths[i], t2->type_names[i], t2->type_lengths[i]))
      return FALSE;
  return TRUE;
}
...
bool Field_enum::is_equal(const Column_definition &new_field) const
{
  ...
  */
  if (new_field.type_handler() != type_handler() || new_field.charset != field_charset() ||
...
const uchar *Field_enum::unpack(uchar *to, const uchar *from, const uchar *from_end, uint param_data)
{
...
bool Field_enum::can_optimize_range_or_keypart_ref(const Item_bool_func *cond, const Item *item) const
{
...
Field_bit::Field_bit(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg, uchar *bit_ptr_arg,
      ...
      bytes_in_rec(len_arg / 8)
{
  ...
  DBUG_PRINT("enter", ("ptr_arg: %p, null_ptr_arg: %p, len_arg: %u, bit_len: %u, bytes_in_rec: %u", ptr_arg,
                       null_ptr_arg, len_arg, bit_len, bytes_in_rec));
  flags |= UNSIGNED_FLAG;
  /*
...
{
  static DTCollation tmp(&my_charset_bin, DERIVATION_IMPLICIT, MY_REPERTOIRE_UNICODE30);
  return tmp;
}
...
size_t Field_bit::do_last_null_byte() const
{
  ...
  */
  DBUG_PRINT("test", ("bit_ofs: %d, bit_len: %d  bit_ptr: %p", bit_ofs, bit_len, bit_ptr));
  uchar *result;
  if (bit_len == 0)
    result = null_ptr;
...
Field *Field_bit::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uint32 length, uchar *new_null_ptr,
                                uint new_null_bit)
{
  Field_bit *res;
  if ((res = (Field_bit *)Field::new_key_field(root, new_table, new_ptr, length, new_null_ptr, new_null_bit)))
  {
...
int Field_bit::store(double nr) { return Field_bit::store((longlong)nr, FALSE); }
...
*/
int Field_bit::cmp_prefix(const uchar *a, const uchar *b, size_t prefix_len) const
{
...
bool Field_bit::compatible_field_size(uint field_metadata, const Relay_log_info *__attribute__((unused)), uint16 mflags,
                                      int *order_var) const
{
  ...
  DBUG_PRINT("debug", ("from_bit_len: %u, to_bit_len: %u", from_bit_len, to_bit_len));
  /*
...
{
  CHARSET_INFO *cs = res.charset();
  size_t length = cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "bit(%d)", (int)field_length);
  res.length(length);
}
...
/**
   ...
   @return  New pointer into memory based on from + length of the data
*/
const uchar *Field_bit::unpack(uchar *to, const uchar *from, const uchar *from_end, uint param_data)
{
  DBUG_ENTER("Field_bit::unpack");
  DBUG_PRINT("enter", ("to: %p, from: %p, param_data: 0x%x", to, from, param_data));
  DBUG_PRINT("debug", ("bit_ptr: %p, bit_len: %u, bit_ofs: %u", bit_ptr, bit_len, bit_ofs));
  uint const from_len = (param_data >> 8U) & 0x00ff;
  uint const from_bit_len = param_data & 0x00ff;
  DBUG_PRINT("debug", ("from_len: %u, from_bit_len: %u", from_len, from_bit_len));
  /*
...
Field_bit_as_char::Field_bit_as_char(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg,
                                     enum utype unireg_check_arg, const LEX_CSTRING *field_name_arg)
    : Field_bit(ptr_arg, len_arg, null_ptr_arg, null_bit_arg, 0, 0, unireg_check_arg, field_name_arg)
{
  flags |= UNSIGNED_FLAG;
  bit_len = 0;
...
bool Column_definition::create_interval_from_interval_list(MEM_ROOT *mem_root, bool reuse_interval_list_values)
{
  ...
  int comma_length = charset->wc_mb(',', (uchar *)comma_buf, (uchar *)comma_buf + sizeof(comma_buf));
  DBUG_ASSERT(comma_length >= 0 && comma_length <= (int)sizeof(comma_buf));

  if (!multi_alloc_root(mem_root, &tmpint->type_names, sizeof(char *) * (interval_list.elements + 1),
                        &tmpint->type_lengths, sizeof(uint) * (interval_list.elements + 1), NullS))
    goto err;  // EOM
  ...
  for (uint i = 0; i < interval_list.elements; i++)
  {
    ...
    String *tmp = it++;
    LEX_CSTRING value;
    if (String::needs_conversion(tmp->length(), tmp->charset(), charset, &dummy))
    {
      uint cnv_errs;
    ...
    {
      value.str = reuse_interval_list_values ? tmp->ptr() : strmake_root(mem_root, tmp->ptr(), tmp->length());
    ...
    {
      if (charset->instr(value.str, value.length, comma_buf, comma_length, NULL, 0))
      {
...
bool Column_definition::prepare_interval_field(MEM_ROOT *mem_root, bool reuse_interval_list_values)
{
  DBUG_ENTER("Column_definition::prepare_interval_field");
  DBUG_ASSERT(real_field_type() == MYSQL_TYPE_ENUM || real_field_type() == MYSQL_TYPE_SET);
  /*
  ...
  */
  if (interval_list.elements && create_interval_from_interval_list(mem_root, reuse_interval_list_values))
    DBUG_RETURN(true);
...
bool Column_definition::set_attributes(THD *thd, const Lex_field_type_st &def, column_definition_type_t type)
{
  ...
  return type_handler()->Column_definition_set_attributes(thd, this, def, type);
}

void Column_definition_attributes::set_length_and_dec(const Lex_length_and_dec_st &type)
{
  if (type.has_explicit_length())
    length = type.length_overflowed() ? (ulonglong)UINT_MAX32 + 1 : (ulonglong)type.length();
...
void Column_definition::create_length_to_internal_length_newdecimal()
{
  ...
  decimal_digit_t prec = get_decimal_precision((uint)length, decimals, flags & UNSIGNED_FLAG);
...

{
  ...
  */
  ...
  {
    my_error(ER_GENERATED_COLUMN_FUNCTION_IS_NOT_ALLOWED, MYF(0), res.name, vcol_type_name(type), name->str);
    return TRUE;
  }
  /*
...
bool Column_definition::fix_attributes_real(uint default_length)
{
  ...
  {
    my_error(ER_TOO_BIG_SCALE, MYF(0), field_name.str, static_cast<uint>(FLOATING_POINT_DECIMALS - 1));
    return true;
  }
  return check_length(ER_TOO_BIG_DISPLAYWIDTH, MAX_FIELD_CHARLENGTH);
}
...
bool Column_definition::fix_attributes_decimal()
{
  ...
  {
    my_error(ER_TOO_BIG_SCALE, MYF(0), field_name.str, static_cast<uint>(NOT_FIXED_DEC - 1));
    return true;
  }
  my_decimal_trim(&length, &decimals);
  if (length > DECIMAL_MAX_PRECISION)
  {
    my_error(ER_TOO_BIG_PRECISION, MYF(0), field_name.str, DECIMAL_MAX_PRECISION);
    return true;
  }
  if (length < decimals)
  {
  ...
  length = my_decimal_precision_to_length((uint)length, decimals, flags & UNSIGNED_FLAG);
...
{
  return check_constraint && check_expression(check_constraint, &field_name, VCOL_CHECK_FIELD);
}
...
bool Column_definition::check(THD *thd)
{
  ...
  if (vcol_info)
  {
    ...
    if (check_expression(vcol_info, &field_name,
                         vcol_info->stored_in_db ? VCOL_GENERATED_STORED : VCOL_GENERATED_VIRTUAL))
      DBUG_RETURN(TRUE);
  }
  ...
  if (default_value && !default_value->expr->basic_const_item() && mysql_timestamp_type() == MYSQL_TIMESTAMP_DATETIME &&
      default_value->expr->type() == Item::FUNC_ITEM)
  {
    ...
    */
    ...
    if (fn->functype() == Item_func::NOW_FUNC && (fn->decimals == 0 || fn->decimals >= length))
    {
      default_value = 0;
  ...
  if (on_update)
  {
    if (mysql_timestamp_type() != MYSQL_TIMESTAMP_DATETIME || on_update->decimals < length)
    {
      my_error(ER_INVALID_ON_UPDATE, MYF(0), field_name.str);
      DBUG_RETURN(TRUE);
    }
    unireg_check = unireg_check == Field::NONE ? Field::TIMESTAMP_UN_FIELD : Field::TIMESTAMP_DNUN_FIELD;
  }
  ...
  */
  if (!default_value && unireg_check == Field::NONE && (flags & NOT_NULL_FLAG))
  {
    ...
    */
    if (((thd->variables.option_bits & OPTION_EXPLICIT_DEF_TIMESTAMP) || !is_timestamp_type()) && !vers_sys_field())
    {
      flags |= NO_DEFAULT_VALUE_FLAG;
    }
  }

  if ((flags & AUTO_INCREMENT_FLAG) && !type_handler()->type_can_have_auto_increment_attribute())
  {
...
Field *Column_definition_attributes::make_field(TABLE_SHARE *share, MEM_ROOT *mem_root, const Record_addr *rec,
                                                const Type_handler *handler, const LEX_CSTRING *field_name,
                                                uint32 flags) const
{
  DBUG_ASSERT(length <= UINT_MAX32);
  DBUG_PRINT("debug", ("field_type: %s, field_length: %u, interval: %p, pack_flag: %s%s%s%s%s", handler->name().ptr(),
                       (uint)length, interval, FLAGSTR(pack_flag, FIELDFLAG_BINARY),
                       FLAGSTR(pack_flag, FIELDFLAG_INTERVAL), FLAGSTR(pack_flag, FIELDFLAG_NUMBER),
                       FLAGSTR(pack_flag, FIELDFLAG_PACK), FLAGSTR(pack_flag, FIELDFLAG_BLOB)));

  Record_addr addr(rec->ptr(), f_maybe_null(pack_flag) ? rec->null() : Bit_addr());
  /*
  ...
  */
  ...
  if (f_maybe_null(pack_flag))
    bit.inc();
  return handler->make_table_field_from_def(share, mem_root, field_name, addr, bit, this, flags);
}
...
Column_definition_attributes::Column_definition_attributes(const Type_all_attributes &attr)
...
Column_definition::Column_definition(THD *thd, Field *old_field, Field *orig_field)
    : Column_definition_attributes(old_field)
{
  ...
  */
  if (!(flags & (NO_DEFAULT_VALUE_FLAG | BLOB_FLAG)) && old_field->ptr != NULL && orig_field != NULL)
  {
    ...
    if (!default_value && !orig_field->is_null_in_record(dv))
    {
      ...
      default_value->expr = new (thd->mem_root) Item_string(thd, pos, res->length(), charset);
...
*/
void Column_definition::redefine_stage1_common(const Column_definition *dup_field, const handler *file)
{
...
uint32 Field_blob::char_length() const { return Field_blob::character_octet_length(); }
...
{
  return (default_value && (!default_value->expr->basic_const_item() || (flags & BLOB_FLAG)));
}
...
bool Column_definition::set_compressed_deprecated(THD *thd, const char *method)
{
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_DEPRECATED_SYNTAX,
                      ER_THD(thd, ER_WARN_DEPRECATED_SYNTAX), "<data type> <character set clause> ... COMPRESSED...",
                      "'<data type> COMPRESSED... <character set clause> ...'");
  return set_compressed(method);
}

bool Column_definition::set_compressed_deprecated_column_attribute(THD *thd, const char *pos, const char *method)
{
  ...
  if (sql_type == MYSQL_TYPE_VARCHAR || sql_type == MYSQL_TYPE_TINY_BLOB || sql_type == MYSQL_TYPE_BLOB ||
      sql_type == MYSQL_TYPE_MEDIUM_BLOB || sql_type == MYSQL_TYPE_LONG_BLOB)
...
bool Field::set_warning(Sql_condition::enum_warning_level level, uint code, int cut_increment, ulong current_row) const
{
  /*
  ...
  */
  ...
  if (thd->count_cuted_fields > CHECK_FIELD_EXPRESSION)
  {
    ...
    push_warning_printf(thd, level, code, ER_THD(thd, code), field_name.str,
                        current_row ? current_row : thd->get_stmt_da()->current_row_for_warning());
    return 0;
  }
...
void Field::set_datetime_warning(Sql_condition::enum_warning_level level, uint code, const ErrConv *str,
                                 const char *typestr, int cuted_increment) const
{
  THD *thd = get_thd();
  if (thd->really_abort_on_warning() && level >= Sql_condition::WARN_LEVEL_WARN)
  {
    ...
    */
    thd->push_warning_wrong_or_truncated_value(level, false, typestr, str->ptr(), table->s->db.str,
...
void Field::set_warning_truncated_wrong_value(const char *type_arg, const char *value)
{
  THD *thd = get_thd();
  const char *db_name;
  ...
  */
  ...
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                      ER_THD(thd, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), type_arg, value, db_name, table_name,
                      field_name.str, static_cast<ulong>(thd->get_stmt_da()->current_row_for_warning()));
}
...
key_map Field::get_possible_keys()
{
  ...
  return (table->pos_in_table_list->is_materialized_derived() ? part_of_key : key_start);
}
...
bool Field::validate_value_in_record_with_warn(THD *thd, const uchar *record)
{
  ...
  if ((rc = validate_value_in_record(thd, record)))
  {
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_INVALID_DEFAULT_VALUE_FOR_FIELD,
                        ER_THD(thd, ER_INVALID_DEFAULT_VALUE_FOR_FIELD), ErrConvString(&tmp).ptr(), field_name.str);
  }
...
bool Field::save_in_field_default_value(bool view_error_processing)
{
  ...
  */
  ...
  if (unlikely(flags & NO_DEFAULT_VALUE_FLAG && real_type() != MYSQL_TYPE_ENUM))
  {
    if (reset())
    {
      my_message(ER_CANT_CREATE_GEOMETRY_OBJECT, ER_THD(thd, ER_CANT_CREATE_GEOMETRY_OBJECT), MYF(0));
      return true;
    }
    ...
    if (view_error_processing)
    {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NO_DEFAULT_FOR_VIEW_FIELD,
                          ER_THD(thd, ER_NO_DEFAULT_FOR_VIEW_FIELD), view->view_db.str, view->view_name.str);
    }
    else
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NO_DEFAULT_FOR_FIELD,
                          ER_THD(thd, ER_NO_DEFAULT_FOR_FIELD), field_name.str);
    }
    return true;
  }
  set_default();
  return !is_null() && validate_value_in_record_with_warn(thd, table->record[0]) && thd->is_error();
}
...
bool Field::save_in_field_ignore_value(bool view_error_processing)
{
  ...
  if (com == SQLCOM_INSERT || com == SQLCOM_REPLACE || com == SQLCOM_INSERT_SELECT || com == SQLCOM_REPLACE_SELECT ||
      com == SQLCOM_LOAD)
...
void Field::print_key_part_value(String *out, const uchar *key, uint32 length)
{
