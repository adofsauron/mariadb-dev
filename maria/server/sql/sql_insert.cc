...
static bool delayed_get_table(THD *thd, MDL_request *grl_protection_request, TABLE_LIST *table_list);
static int write_delayed(THD *thd, TABLE *table, enum_duplicates duplic, LEX_STRING query, bool ignore, bool log_on);
static void end_delayed_insert(THD *thd);
pthread_handler_t handle_delayed_insert(void *arg);
static void unlink_blobs(TABLE *table);
#endif
static bool check_view_insertability(THD *thd, TABLE_LIST *view);
static int binlog_show_create_table(THD *thd, TABLE *table, Table_specification_st *create_info);
...
static bool check_view_single_update(List<Item> &fields, List<Item> *values, TABLE_LIST *view, table_map *map,
                                     bool insert)
{
  ...
  */
  ...
  {
    it.init(*values);
    while ((item = it++)) tables |= item->view_used_tables(view);
  }
...
error:
  my_error(ER_VIEW_MULTIUPDATE, MYF(0), view->view_db.str, view->view_name.str);
  return TRUE;
}
...
static int check_insert_fields(THD *thd, TABLE_LIST *table_list, List<Item> &fields, List<Item> &values,
                               bool check_unique, bool fields_and_values_from_different_maps, table_map *map)
{
  ...
  {  // Part field list
    ...
    */
    ...
    res = setup_fields(thd, Ref_ptr_array(), fields, MARK_COLUMNS_WRITE, 0, NULL, 0);
    ...
    {
      if (check_view_single_update(fields, fields_and_values_from_different_maps ? (List<Item> *)0 : &values,
                                   table_list, map, true))
        DBUG_RETURN(-1);
    ...
    {
      my_error(ER_FIELD_SPECIFIED_TWICE, MYF(0), thd->dup_field->field_name.str);
      DBUG_RETURN(-1);
    }
  }
...
{
  if ((field->flags & (NO_DEFAULT_VALUE_FLAG | VERS_ROW_START | VERS_ROW_END)) == NO_DEFAULT_VALUE_FLAG &&
      field->real_type() != MYSQL_TYPE_ENUM)
  {
    ...
    if (view)
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NO_DEFAULT_FOR_VIEW_FIELD,
                          ER_THD(thd, ER_NO_DEFAULT_FOR_VIEW_FIELD), table_list->view_db.str,
                          table_list->view_name.str);
    }
    else
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NO_DEFAULT_FOR_FIELD,
                          ER_THD(thd, ER_NO_DEFAULT_FOR_FIELD), field->field_name.str);
    }
...
static int check_update_fields(THD *thd, TABLE_LIST *insert_table_list, List<Item> &update_fields,
                               List<Item> &update_values, bool fields_and_values_from_different_maps, table_map *map)
{
  ...
  if (table->found_next_number_field)
  {
    /*
      ...
      by update_fields
    */
    autoinc_mark = bitmap_test_and_clear(table->write_set, table->found_next_number_field->field_index);
  }

  thd->lex->sql_command = SQLCOM_UPDATE;

  /* Check the fields we are going to modify */
  if (setup_fields(thd, Ref_ptr_array(), update_fields, MARK_COLUMNS_WRITE, 0, NULL, 0))
  {
  ...
  if (insert_table_list->is_view() && insert_table_list->is_merged_derived() &&
      check_view_single_update(update_fields, fields_and_values_from_different_maps ? (List<Item> *)0 : &update_values,
  ...
  if (table->found_next_number_field)
  {
    if (bitmap_is_set(table->write_set, table->found_next_number_field->field_index))
      table->next_number_field_updated = TRUE;

    if (autoinc_mark)
      bitmap_set_bit(table->write_set, table->found_next_number_field->field_index);
  }
...
static void upgrade_lock_type(THD *thd, thr_lock_type *lock_type, enum_duplicates duplic)
{
  if (duplic == DUP_UPDATE || (duplic == DUP_REPLACE && *lock_type == TL_WRITE_CONCURRENT_INSERT))
  {
  ...
  if (*lock_type == TL_WRITE_DELAYED)
  {
    ...
    */
    ...
    if (thd->slave_thread)
    {
      ...
      *lock_type = (duplic == DUP_UPDATE || duplic == DUP_REPLACE) ? TL_WRITE : TL_WRITE_CONCURRENT_INSERT;
      return;
    }

    bool log_on = (thd->variables.option_bits & OPTION_BIN_LOG);
    if (WSREP_BINLOG_FORMAT(global_system_variables.binlog_format) == BINLOG_FORMAT_STMT && log_on &&
...
static bool open_and_lock_for_insert_delayed(THD *thd, TABLE_LIST *table_list)
{
  ...
  */
  ...
  MDL_REQUEST_INIT(&protection_request, MDL_key::BACKUP, "", "", MDL_BACKUP_DML, MDL_STATEMENT);

  if (thd->mdl_context.acquire_lock(&protection_request, thd->variables.lock_wait_timeout))
    DBUG_RETURN(TRUE);

  if (thd->mdl_context.acquire_lock(&table_list->mdl_request, thd->variables.lock_wait_timeout))
    /*
...
static int create_insert_stmt_from_insert_delayed(THD *thd, String *buf)
{
  ...
  if (buf->append(thd->query(), thd->query_length()) ||
      buf->replace(thd->lex->keyword_delayed_begin_offset,
                   thd->lex->keyword_delayed_end_offset - thd->lex->keyword_delayed_begin_offset, NULL, 0))
    return 1;
  return 0;
}
...
static void save_insert_query_plan(THD *thd, TABLE_LIST *table_list)
{
  ...
  for (SELECT_LEX_UNIT *unit = thd->lex->first_select_lex()->first_inner_unit(); unit; unit = unit->next_unit())
  {
    if (unit->explainable())
...
Field **TABLE::field_to_fill() { return triggers && triggers->nullable_fields() ? triggers->nullable_fields() : field; }
                  ...
                  select_result *result)
{
  ...
  */
  if (table_list->lock_type == TL_WRITE_DELAYED && thd->locked_tables_mode &&
      find_locked_table(thd->open_tables, table_list->db.str, table_list->table_name.str))
  {
    my_error(ER_DELAYED_INSERT_TABLE_LOCKED, MYF(0), table_list->table_name.str);
    DBUG_RETURN(TRUE);
  }
  ...
  if ((res = mysql_prepare_insert(thd, table_list, fields, values, update_fields, update_values, duplic, ignore,
                                  &unused_conds, FALSE)))
  {
    retval = thd->is_error();
    if (res < 0)
  ...
  */
  if (values->elements && table_list->table->check_assignability_opt_fields(fields, *values, ignore))
    goto abort;
  ...
  while ((values = its++))
  {
    ...
    {
      my_error(ER_WRONG_VALUE_COUNT_ON_ROW, MYF(0), thd->get_stmt_da()->current_row_for_warning());
      goto abort;
    }
  ...
  */
  ...
  if (thd->rgi_slave && (info.handle_duplicates == DUP_UPDATE) && (table->next_number_field != NULL) &&
      rpl_master_has_bug(thd->rgi_slave->rli, 24432, TRUE, NULL, NULL))
    goto abort;
#endif

  error = 0;
  if (duplic == DUP_REPLACE && (!table->triggers || !table->triggers->has_delete_triggers()))
...
#endif /* EMBEDDED_LIBRARY */
  {
    ...
    */
    if (thd->locked_tables_mode <= LTM_LOCK_TABLES && values_list.elements > 1)
    {
      using_bulk_insert = 1;
  ...
  {
    if (table->triggers && table->triggers->has_triggers(TRG_EVENT_INSERT, TRG_ACTION_BEFORE))
    {
  ...
  /*
    ...
    metadata.
  */
  if (returning && result->send_result_set_metadata(returning->item_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
    goto values_loop_end;
    ...
    while ((values = its++))
    {
      ...
      if (fields.elements || !value_count)
      {
        ...
        */
        restore_record(table, s->default_values);  // Get empty record
        table->reset_default_fields();
        if (unlikely(fill_record_n_invoke_before_triggers(thd, table, fields, *values, 0, TRG_EVENT_INSERT)))
        {
        ...
        */
        ...
        else
        {
          ...
          */
          ...
          {
            table->record[0][share->null_bytes - 1] = share->default_values[share->null_bytes - 1];
          }
        }
        table->reset_default_fields();
        if (unlikely(
                fill_record_n_invoke_before_triggers(thd, table, table->field_to_fill(), *values, 0, TRG_EVENT_INSERT)))
        {
          if (values_list.elements != 1 && !thd->is_error())
          {
            info.records++;
      ...
      */
      if (table->triggers && table->triggers->has_triggers(TRG_EVENT_INSERT, TRG_ACTION_BEFORE))
      {
        ...
        {
          if (unlikely(!(*f)->has_explicit_value() && has_no_default_value(thd, *f, table_list)))
          {
            error = 1;
            goto values_loop_end;
          }
  ...
  */
...
#endif
  {
    ...
    */
    ...
    if (error <= 0 || thd->transaction->stmt.modified_non_trans_table || thd->log_current_statement() ||
        was_insert_delayed)
    {
      if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
      {
        ...
        StatementBinlog stmt_binlog(thd,
                                    table->versioned(VERS_TRX_ID) || thd->binlog_need_stmt_format(transactional_table));
        /* bug#22725:
        ...
        if (was_insert_delayed && table_list->lock_type == TL_WRITE)
        {
          ...
          else if (thd->binlog_query(THD::ROW_QUERY_TYPE, log_query.c_ptr(), log_query.length(), transactional_table,
                                     FALSE, FALSE, errcode) > 0)
            error = 1;
        }
        else if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(), thd->query_length(), transactional_table, FALSE,
                                   FALSE, errcode) > 0)
          error = 1;
      }
    }
  ...
  */
  id = (thd->first_successful_insert_id_in_cur_stmt > 0)
           ...
           : (thd->arg_of_last_insert_id_function
                  ...
                  : ((table->next_number_field && info.copied) ? table->next_number_field->val_int() : 0));
  ...
  DBUG_PRINT("info", ("touched: %llu  copied: %llu  updated: %llu  deleted: %llu", (ulonglong)info.touched,
                      (ulonglong)info.copied, (ulonglong)info.updated, (ulonglong)info.deleted));
      ...
      (!(thd->variables.option_bits & OPTION_WARNINGS) || !thd->cuted_fields))
  {
    ...
    */
    if (returning)
      ...
      my_ok(thd,
            info.copied + info.deleted + ((thd->client_capabilities & CLIENT_FOUND_ROWS) ? info.touched : info.updated),
            id);
  }
  else
  {
    char buff[160];
    ha_rows updated = ((thd->client_capabilities & CLIENT_FOUND_ROWS) ? info.touched : info.updated);
    ...
    if (ignore)
      ...
      sprintf(buff, ER_THD(thd, ER_INSERT_INFO), (ulong)info.records, (ulong)(info.deleted + updated),
              (long)thd->get_stmt_da()->current_statement_warn_count());
    if (returning)
      result->send_eof();
    else
...
abort:
  ...
  if (lock_type == TL_WRITE_DELAYED)
  {
    ...
    */
    for (Field **ptr = table_list->table->field; *ptr; ptr++) (*ptr)->free();
 ...
 */
int check_duplic_insert_without_overlaps(THD *thd, TABLE *table, enum_duplicates duplic)
{
...
static bool mysql_prepare_insert_check_table(THD *thd, TABLE_LIST *table_list, List<Item> &fields, bool select_insert)
{
  ...
  */
  ...
  if (setup_tables_and_check_access(
          thd, &thd->lex->first_select_lex()->context, &thd->lex->first_select_lex()->top_join_list, table_list,
          thd->lex->first_select_lex()->leaf_tables, select_insert, INSERT_ACL, SELECT_ACL, TRUE))
    DBUG_RETURN(TRUE);
...
int mysql_prepare_insert(THD *thd, TABLE_LIST *table_list, List<Item> &fields, List_item *values,
                         List<Item> &update_fields, List<Item> &update_values, enum_duplicates duplic, bool ignore,
                         COND **where, bool select_insert)
{
  ...
  DBUG_PRINT("enter", ("table_list: %p  view: %d", table_list, (int)insert_into_view));
  ...
  if (values)
  {
    ...
    res = setup_returning_fields(thd, table_list) ||
          setup_fields(thd, Ref_ptr_array(), *values, MARK_COLUMNS_READ, 0, NULL, 0) ||
          check_insert_fields(thd, context->table_list, fields, *values, !insert_into_view, 0, &map);

    if (!res)
      res = setup_fields(thd, Ref_ptr_array(), update_values, MARK_COLUMNS_READ, 0, NULL, 0);

    if (!res && duplic == DUP_UPDATE)
    {
      select_lex->no_wrap_view_item = TRUE;
      res = check_update_fields(thd, context->table_list, update_fields, update_values, false, &map) ||
            /*
              Check that all col=expr pairs are compatible for assignment in
              INSERT INTO t1 VALUES (...)
                ON DUPLICATE KEY UPDATE col=expr [, col=expr];
            */
            TABLE::check_assignability_explicit_fields(update_fields, update_values, ignore);
  ...
  if (!select_insert)
  {
    ...
    if ((duplicate = unique_table(thd, table_list, table_list->next_global, CHECK_DUP_ALLOW_DIFFERENT_ALIAS)))
    {
...
int vers_insert_history_row(TABLE *table)
{
  ...
  if (table->vfield && table->update_virtual_fields(table->file, VCOL_UPDATE_FOR_READ))
    ...
    while (unlikely(error = table->file->ha_write_row(table->record[0])))
    {
      ...
      */
      ...
      if (table->file->is_fatal_error(error, HA_CHECK_ALL))
        goto err;
      is_duplicate_key_error = table->file->is_fatal_error(error, HA_CHECK_ALL & ~HA_CHECK_DUP);
      ...
      */
      ...
      else
      {
        ...
        if ((error = (table->file->ha_index_read_idx_map(table->record[1], key_nr, (uchar *)key, keypart_map,
                                                         HA_READ_KEY_EXACT))))
          goto err;
      }
      if (table->vfield)
      ...
      if (info->handle_duplicates == DUP_UPDATE)
      {
        ...
        */
        ...
        if (fill_record_n_invoke_before_triggers(thd, table, *info->update_fields, *info->update_values, info->ignore,
                                                 TRG_EVENT_UPDATE))
          goto before_trg_err;

        bool different_records = (!records_are_comparable(table) || compare_record(table));
        /*
...
int check_that_all_fields_are_given_values(THD *thd, TABLE *entry, TABLE_LIST *table_list)
{
  ...
  {
    if (!bitmap_is_set(write_set, (*field)->field_index) && has_no_default_value(thd, *field, table_list))
      err = 1;
  }
...
class delayed_row : public ilink
{
 public:
  ...
  delayed_row(LEX_STRING const query_arg, enum_duplicates dup_arg, bool ignore_arg, bool log_query_arg)
      : record(0),
        dup(dup_arg),
        ignore(ignore_arg),
  ...
  */
  ...
  Delayed_insert(SELECT_LEX *current_select)
      : locks_in_memory(0),
        ...
        status(0),
        retry(0),
        handler_thread_initialized(FALSE),
        group_count(0)
  {
    ...
    */
    ...
    delayed_lock = global_system_variables.low_priority_updates ? TL_WRITE_LOW_PRIORITY : TL_WRITE;
    DBUG_VOID_RETURN;
  }
  ~Delayed_insert()
  {
...
static Delayed_insert *find_handler(THD *thd, TABLE_LIST *table_list)
{
  ...
  while ((di = it++))
  {
    if (!cmp(&table_list->db, &di->table_list.db) && !cmp(&table_list->table_name, &di->table_list.table_name))
    {
      di->lock();
      break;
    }
    ...
    */
    if (!(di = find_handler(thd, table_list)))
    {
      ...
      */
      ...
      di->thd.set_query(my_strndup(PSI_INSTRUMENT_ME, table_list->table_name.str, table_list->table_name.length,
                                   MYF(MY_WME | ME_FATAL)),
      ...
      */
      MDL_REQUEST_INIT(&di->grl_protection, MDL_key::BACKUP, "", "", MDL_BACKUP_DML, MDL_STATEMENT);
      ...
      if ((error = mysql_thread_create(key_thread_delayed_insert, &di->thd.real_id, &connection_attrib,
      ...
      */
      ...
      while (!di->handler_thread_initialized || (!di->thd.killed && !di->table && !thd->killed))
      {
      ...
      if (di->thd.killed)
      {
        if (di->thd.is_error() && !di->retry)
        {
          /*
            ...
            INSERT THREAD.
          */
          my_message(di->thd.get_stmt_da()->sql_errno(), di->thd.get_stmt_da()->message(), MYF(0));
        }
        di->unlock();
        goto end_create;
      }
...
TABLE *Delayed_insert::get_local_table(THD *client_thd)
{
    ...
    if (thd.killed)
    {
      ...
      */
      if (thd.is_error())
      {
        ...
        */
        my_message(thd.get_stmt_da()->sql_errno(), thd.get_stmt_da()->message(), MYF(0));
      }
      goto error;
    }
  }
  ...
  */
  ...
  if (!multi_alloc_root(client_thd->mem_root, &copy_tmp, sizeof(*table), &field,
                        (uint)(share->fields + 1) * sizeof(Field **), &default_fields,
                        (share->default_fields + share->default_expressions + 1) * sizeof(Field *), &virtual_fields,
                        (share->virtual_fields + 1) * sizeof(Field *), &record, (uint)share->reclength, &bitmap,
                        (uint)share->column_bitmap_size * 4, NullS))
    goto error;
  ...
  *field = 0;
  ...
  if (share->virtual_fields || share->default_expressions || share->default_fields)
  {
    bool error_reported = FALSE;
    if (unlikely(parse_vcol_defs(client_thd, client_thd->mem_root, copy, &error_reported,
  ...
  copy->def_write_set.bitmap = ((my_bitmap_map *)(bitmap + share->column_bitmap_size));
  bitmaps_used = 2;
  if (share->default_fields || share->default_expressions)
  {
    my_bitmap_init(&copy->has_value_set, (my_bitmap_map *)(bitmap + bitmaps_used * share->column_bitmap_size),
                   share->fields);
  }
...
static int write_delayed(THD *thd, TABLE *table, enum_duplicates duplic, LEX_STRING query, bool ignore, bool log_on)
{
  ...
  {
    char *str;
    if (!(str = my_strndup(PSI_INSTRUMENT_ME, query.str, query.length, MYF(MY_WME))))
      goto err;
    query.str = str;
  }
  ...
  if (!(row->record =
            (char *)my_malloc(PSI_INSTRUMENT_ME, table->s->reclength + user_len + host_len + ip_length, MYF(MY_WME))))
    goto err;
  ...
  */
  ...
  if ((forced_auto_inc = thd->auto_inc_intervals_forced.get_next()))
  {
    ...
    DBUG_PRINT("delayed", ("transmitting auto_inc: %lu", (ulong)row->forced_insert_id));
  }
...
{
 public:
  virtual bool handle_routine(THD *thd, Query_tables_list *prelocking_ctx, Sroutine_hash_entry *rt, sp_head *sp,
...
bool Delayed_prelocking_strategy::handle_table(THD *thd, Query_tables_list *prelocking_ctx, TABLE_LIST *table_list,
                                               bool *need_prelocking)
{
...
bool Delayed_prelocking_strategy::handle_routine(THD *thd, Query_tables_list *prelocking_ctx, Sroutine_hash_entry *rt,
...
bool Delayed_prelocking_strategy::handle_view(THD *thd, Query_tables_list *prelocking_ctx, TABLE_LIST *table_list,
                                              bool *need_prelocking)
{
...
bool Delayed_insert::open_and_lock_table()
{
  ...
  */
  if (!(table = open_n_lock_single_table(&thd, &table_list, TL_WRITE_DELAYED,
                                         MYSQL_OPEN_IGNORE_GLOBAL_READ_LOCK | MYSQL_OPEN_IGNORE_REPAIR,
                                         &prelocking_strategy)))
  {
    ...
    for (;;)
    {
      if (thd->killed)
      {
        ...
        */
        ...
        mysql_mutex_unlock(&LOCK_delayed_insert);
        mysql_mutex_lock(&di->mutex);
        if (!lock_count && !di->tables_in_use && !di->stacked_inserts && !thd->lock)
          break;  // Time to die
      }

      /* Shouldn't wait if killed or an insert is waiting. */
      DBUG_PRINT("delayed", ("thd->killed: %d  di->status: %d  di->stacked_inserts: %d", thd->killed, di->status,
                             di->stacked_inserts));
      if (!thd->killed && !di->status && !di->stacked_inserts)
      {
        ...
        while (!thd->killed && !di->status)
        {
          ...
          if (error && error != EINTR && error != ETIMEDOUT)
          {
            ...
            DBUG_PRINT("error", ("Got error %d from mysql_cond_timedwait", error));
          }
#endif
      ...
      */
      DBUG_EXECUTE_IF("write_delay_wakeup", if (!thd->killed && di->stacked_inserts) my_sleep(500000););

      if (di->tables_in_use && !thd->lock && (!thd->killed || di->stacked_inserts))
      {
        thd->set_query_id(next_query_id());
        /*
      ...
      if (di->stacked_inserts)
      {
        ...
        while ((row = it++))
        {
          ...
          {
            mysql_audit_external_lock_ex(&di->thd, row->thread_id, row->user, row->host, row->ip, row->query_id,
                                         di->table->s, F_WRLCK);
...
bool Delayed_insert::handle_inserts(void)
{
  ...
  if (thr_upgrade_write_delay_lock(*thd.lock->locks, delayed_lock, thd.variables.lock_wait_timeout))
  {
    /*
    ...
    */
    my_error(ER_DELAYED_CANT_CHANGE_LOCK, MYF(ME_FATAL | ME_ERROR_LOG), table->s->table_name.str);
    goto err;
  }
  ...
  */
  ...
  while ((row = rows.get()))
  {
    ...
    */
    ...
    DBUG_PRINT("delayed",
               ("query: '%s'  length: %lu", row->query.str ? row->query.str : "[NULL]", (ulong)row->query.length));
    if (log_query)
    {
      /*
      ...
      */
      DBUG_ASSERT(!mysql_bin_log.is_open() || thd.is_current_stmt_binlog_format_row());
    ...
    {
      DBUG_PRINT("delayed", ("received auto_inc: %lu", (ulong)row->forced_insert_id));
    ...
    if (info.handle_duplicates == DUP_REPLACE && (!table->triggers || !table->triggers->has_delete_triggers()))
    {
      table->file->extra(HA_EXTRA_WRITE_CAN_REPLACE);
      using_opt_replace = 1;
    }
    ...
    if (unlikely(table->vfield))
    {
      ...
      */
      tmp_error = table->update_virtual_fields(table->file, VCOL_UPDATE_FOR_WRITE);
    }
    ...
    */
    if (group_count++ >= max_rows && (row = rows.head()) && (!(row->log_query & using_bin_log)))
    {
      group_count = 0;
      if (stacked_inserts || tables_in_use)  // Let these wait a while
      {
        ...
        if (thr_reschedule_write_lock(*thd.lock->locks, thd.variables.lock_wait_timeout))
        {
  ...
  */
  if (table->file->row_logging && thd.binlog_flush_pending_rows_event(TRUE, table->file->row_logging_has_trans))
    goto err;
...
int mysql_insert_select_prepare(THD *thd, select_result *sel_res)
{
  ...
  */
  ...
  if ((res = mysql_prepare_insert(thd, lex->query_tables, lex->field_list, 0, lex->update_list, lex->value_list,
                                  lex->duplicates, lex->ignore, &select_lex->where, TRUE)))
    DBUG_RETURN(res);
  ...
  */
  ...
  insert_tables = select_lex->insert_tables;
  while ((table = ti++) && insert_tables--) ti.remove();

  DBUG_RETURN(0);
}

select_insert::select_insert(THD *thd_arg, TABLE_LIST *table_list_par, TABLE *table_par, List<Item> *fields_par,
                             List<Item> *update_fields, List<Item> *update_values, enum_duplicates duplic,
                             bool ignore_check_option_errors, select_result *result)
...
int select_insert::prepare(List<Item> &values, SELECT_LEX_UNIT *u)
{
  ...
  */
  ...
  res = (setup_returning_fields(thd, table_list) ||
         ...
         check_insert_fields(thd, table_list, *fields, values, !insert_into_view, 1, &map));
  ...
  if (!res)
  {
    ...
    */
    res = table_list->table->check_assignability_opt_fields(*fields, values, lex->ignore);
  }
  ...
  if (info.handle_duplicates == DUP_UPDATE && !res)
  {
    ...
    res = res || check_update_fields(thd, context->table_list, *info.update_fields, *info.update_values,
                                     /*
    ...
    */
    ...
    if (lex->first_select_lex()->group_list.elements == 0 && !lex->first_select_lex()->with_sum_func)
    {
      /*
    ...
    res = res || setup_fields(thd, Ref_ptr_array(), *info.update_values, MARK_COLUMNS_READ, 0, NULL, 0) ||
          /*
            Check that all col=expr pairs are compatible for assignment in
              INSERT INTO t1 SELECT ... FROM t2
                ON DUPLICATE KEY UPDATE col=expr [, col=expr]
          */
          TABLE::check_assignability_explicit_fields(*info.update_fields, *info.update_values, lex->ignore);
    if (!res)
    {
      /*
      ...
      */
      ...
      while ((item = li++))
      {
        item->transform(thd, &Item::update_value_transformer, (uchar *)lex->current_select);
      }
    }
  ...
  */
  ...
  else if (!(lex->current_select->options & OPTION_BUFFER_RESULT) && thd->locked_tables_mode <= LTM_LOCK_TABLES)
  {
    /*
  ...
  res = (table_list->prepare_where(thd, 0, TRUE) || table_list->prepare_check_option(thd));
...
int select_insert::prepare2(JOIN *)
{
  ...
  if (thd->lex->describe)
    DBUG_RETURN(0);
  if (thd->lex->current_select->options & OPTION_BUFFER_RESULT && thd->locked_tables_mode <= LTM_LOCK_TABLES)
    table->file->ha_start_bulk_insert((ha_rows)0);

  /* Same as the other variants of INSERT */
  if (sel_result && sel_result->send_result_set_metadata(thd->lex->returning()->item_list,
...
int select_insert::send_data(List<Item> &values)
{
  ...
  store_values(values);
  if (table->default_field && unlikely(table->update_default_fields(info.ignore)))
    DBUG_RETURN(1);
...
void select_insert::store_values(List<Item> &values)
{
  ...
  if (fields->elements)
    fill_record_n_invoke_before_triggers(thd, table, *fields, values, 1, TRG_EVENT_INSERT);
  else
    fill_record_n_invoke_before_triggers(thd, table, table->field_to_fill(), values, 1, TRG_EVENT_INSERT);
...
bool select_insert::prepare_eof()
{
  ...
  DBUG_PRINT("enter", ("trans_table: %d, table_type: '%s'", trans_table, table->file->table_type()));
  ...
  DBUG_ASSERT(trans_table || !changed || thd->transaction->stmt.modified_non_trans_table);
  ...
  */
  if ((WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open()) &&
      (likely(!error) || thd->transaction->stmt.modified_non_trans_table || thd->log_current_statement()))
  {
    int errcode = 0;
    int res;
    if (likely(!error))
    ...
    StatementBinlog stmt_binlog(thd, !can_rollback_data() && thd->binlog_need_stmt_format(trans_table));
    res = thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(), thd->query_length(), trans_table, FALSE, FALSE, errcode);
    if (res > 0)
    {
      table->file->ha_release_auto_increment();
      DBUG_RETURN(true);
...
bool select_insert::send_ok_packet()
{
  ...
  if (info.ignore)
    my_snprintf(message, sizeof(message), ER(ER_INSERT_INFO), (ulong)info.records, (ulong)(info.records - info.copied),
                (long)thd->get_stmt_da()->current_statement_warn_count());
  else
    my_snprintf(message, sizeof(message), ER(ER_INSERT_INFO), (ulong)info.records, (ulong)(info.deleted + info.updated),
                (long)thd->get_stmt_da()->current_statement_warn_count());
...
void select_insert::abort_result_set()
{
  ...
  if (table && table->file->is_open())
  {
    ...
    */
    ...
    transactional_table = table->file->has_transactions_and_rollback();
    if (thd->transaction->stmt.modified_non_trans_table || thd->log_current_statement())
    {
      ...
      if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
      {
        StatementBinlog stmt_binlog(thd, !can_rollback_data() && thd->binlog_need_stmt_format(transactional_table));
        int errcode = query_error_code(thd, thd->killed == NOT_KILLED);
        int res;
        /* error of writing binary log is ignored */
        res = thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(), thd->query_length(), transactional_table, FALSE,
                                FALSE, errcode);
        binary_logged = res == 0 || !table->s->tmp_table;
      }
      if (changed)
...
TABLE *select_create::create_table_from_items(THD *thd, List<Item> *items, MYSQL_LOCK **lock)
{
  ...
  while ((item = it++))
  {
    Field *tmp_field = item->create_field_for_create_select(thd->mem_root, &tmp_table);
    ...
    Create_field *cr_field = new (thd->mem_root) Create_field(thd, tmp_field, table_field);
  ...
  if (create_info->check_fields(thd, alter_info, table_list->table_name, table_list->db, select_field_count))
    DBUG_RETURN(NULL);

  DEBUG_SYNC(thd, "create_table_select_before_create");

  /* Check if LOCK TABLES + CREATE OR REPLACE of existing normal table*/
  if (thd->locked_tables_mode && table_list->table && !create_info->tmp_table())
  {
    /* Remember information about the locked table */
    create_info->pos_in_locked_tables = table_list->table->pos_in_locked_tables;
  ...
  */
  ...
  if (!mysql_create_table_no_lock(thd, &ddl_log_state_create, &ddl_log_state_rm, &table_list->db,
                                  &table_list->table_name, create_info, alter_info, NULL, select_field_count,
                                  table_list))
  {
      ...
      */
      if (open_table(thd, table_list, &ot_ctx))
      {
        quick_rm_table(thd, create_info->db_type, &table_list->db,
                       table_case_name(create_info, &table_list->table_name), 0);
      }
      /* Restore */
  ...
  */
  if (unlikely(!((*lock) = mysql_lock_tables(thd, &table, 1, 0)) || postlock(thd, &table)))
  {
  ...
  if (!table->s->tmp_table)
    ...
    */
#ifdef HAVE_REPLICATION
  if (thd->slave_thread && opt_replicate_annotate_row_events && thd->is_current_stmt_binlog_format_row())
...
int select_create::prepare(List<Item> &_values, SELECT_LEX_UNIT *u)
{
  ...
  */
  if (!thd->lex->tmp_table() && thd->is_current_stmt_binlog_format_row() && mysql_bin_log.is_open())
  {
  ...
  if (!(table = create_table_from_items(thd, &values, &extra_lock)))
  {
    if (create_info->or_replace())
    {
      ...
      log_drop_table(thd, &table_list->db, &table_list->table_name, &create_info->org_storage_engine_name,
                     create_info->db_type == partition_hton, &create_info->org_tabledef_version, thd->lex->tmp_table());
    }
...
static int binlog_show_create_table(THD *thd, TABLE *table, Table_specification_st *create_info)
{
  ...
  */
  ...
  result = show_create_table(thd, &tmp_table_list, &query, create_info, WITH_DB_NAME);
  DBUG_ASSERT(result == 0); /* show_create_table() always return 0 */

  if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
  {
    int errcode = query_error_code(thd, thd->killed == NOT_KILLED);
    result = thd->binlog_query(THD::STMT_QUERY_TYPE, query.ptr(), query.length(),
                               /* is_trans */ TRUE,
                               /* direct */ FALSE,
                               /* suppress_use */ FALSE, errcode) > 0;
  }
#ifdef WITH_WSREP
...
bool binlog_create_table(THD *thd, TABLE *table, bool replace)
{
  ...
  if (thd->variables.binlog_format == BINLOG_FORMAT_ROW && table->s->tmp_table)
    return 0;
  ...
  */
  ...
  create_info.used_fields |= (HA_CREATE_USED_ENGINE | HA_CREATE_USED_DEFAULT_CHARSET);
...
bool binlog_drop_table(THD *thd, TABLE *table)
{
  ...
  return thd->binlog_query(THD::STMT_QUERY_TYPE, query.ptr(), query.length(),
                           /* is_trans */ TRUE,
                           /* direct */ FALSE,
                           /* suppress_use */ TRUE, 0) > 0;
}

void select_create::store_values(List<Item> &values)
{
  fill_record_n_invoke_before_triggers(thd, table, field, values, 1, TRG_EVENT_INSERT);
}
...
bool select_create::send_eof()
{
  ...
  */
  ...
  if (table->s->tmp_table)
  {
    /*
      ...
      statement.
    */
    if (thd->find_tmp_table_share(table->s->table_cache_key.str, table->s->table_cache_key.length))
    {
  ...
  */
  ...
  {
#ifdef WITH_WSREP
    if (WSREP(thd) && table->file->ht->db_type == DB_TYPE_INNODB)
    {
      ...
      WSREP_DEBUG("CTAS key append for trx: %" PRIu64 " thd %llu query %lld ", thd->wsrep_trx_id(), thd->thread_id,
                  thd->query_id);
      ...
      */
      ...
      wsrep_prepare_keys_for_isolation(thd, table_list->db.str, table_list->table_name.str, table_list, &key_arr);
      int rcode = wsrep_thd_append_key(thd, key_arr.keys, key_arr.keys_len, WSREP_SERVICE_KEY_EXCLUSIVE);
    ...
    if (WSREP(thd))
    {
      ...
      {
        WSREP_DEBUG("select_create commit failed, thd: %llu err: %s %s", thd->thread_id,
    ...
    if ((ddl_log.org_partitioned = (create_info->db_type == partition_hton)))
      ...
      lex_string_set(&ddl_log.org_storage_engine_name, ha_resolve_storage_engine_name(create_info->db_type));
    ...
    *m_plock = NULL;
    ...
    if (create_info->pos_in_locked_tables)
    {
      ...
      */
      ...
      if (!thd->locked_tables_list.restore_lock(thd, create_info->pos_in_locked_tables, table, lock))
...
void select_create::abort_result_set()
{
  ...
  */
  ...
  if (table)
  {
    ...
    bool table_creation_was_logged = (!tmp_table || table->s->table_creation_was_logged);
    if (tmp_table)
    {
      DBUG_ASSERT(saved_tmp_table_share);
      thd->restore_tmp_table_share(saved_tmp_table_share);
    ...
    if (table->file->inited && (info.ignore || info.handle_duplicates != DUP_ERROR) &&
    ...
    if (thd->log_current_statement())
    {
      if (mysql_bin_log.is_open())
      {
        ...
        if (table_creation_was_logged)
        {
          ...
          debug_crash_here("ddl_log_create_before_binlog");
          log_drop_table(thd, &table_list->db, &table_list->table_name, &create_info->org_storage_engine_name,
                         create_info->db_type == partition_hton, &create_info->tabledef_version, tmp_table);
          debug_crash_here("ddl_log_create_after_binlog");
          thd->binlog_xid = 0;
        }
      }
