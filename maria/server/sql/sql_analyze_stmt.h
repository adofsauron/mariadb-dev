...
class Exec_time_tracker
{
  ...
  */
  ...
  double get_time_ms() const
  {
    ...
    return 1000.0 * static_cast<double>(cycles) / static_cast<double>(sys_timer_info.cycles.frequency);
  }
...
class Gap_time_tracker
{
 ...
 public:
  ...
  void log_time(ulonglong start, ulonglong end) { cycles += end - start; }
...
class Time_and_counter_tracker : public Exec_time_tracker
{
  ...
  */
  void stop_tracking(THD *thd) { cycles_stop_tracking(thd); }
};
...
class Table_access_tracker
{
 public:
  Table_access_tracker()
      : r_scans(0),
        r_rows(0), /*r_rows_after_table_cond(0),*/
  ...
  double get_avg_rows() const { return r_scans ? static_cast<double>(r_rows) / static_cast<double>(r_scans) : 0; }

  double get_filtered_after_where() const
  {
    return r_rows > 0 ? static_cast<double>(r_rows_after_where) / static_cast<double>(r_rows) : 1.0;
  }
...
class Filesort_tracker : public Sql_alloc
{
 public:
  ...
  inline void report_row_numbers(ha_rows examined_rows, ha_rows sorted_rows, ha_rows returned_rows)
  {
  ...
  inline void report_merge_passes_at_start(ulong passes) { sort_passes -= passes; }
  ...
  inline void report_sort_keys_format(bool sort_keys_packed) { r_sort_keys_packed = sort_keys_packed; }
  ...
  {
    return static_cast<double>(r_examined_rows) / static_cast<double>(get_r_loops());
  }
  double get_avg_returned_rows() const
  {
    return static_cast<double>(r_output_rows) / static_cast<double>(get_r_loops());
  }
  double get_r_filtered() const
  {
    return r_examined_rows > 0 ? static_cast<double>(r_sorted_rows) / static_cast<double>(r_examined_rows) : 1.0;
  }
...
class Rowid_filter_tracker : public Sql_alloc
{
 ...
 public:
  Rowid_filter_tracker(bool do_timing)
      : time_tracker(do_timing), container_buff_size(0), container_elements(0), n_checks(0), n_positive_checks(0)
  {
  }

  inline void start_tracking(THD *thd) { ANALYZE_START_TRACKING(thd, &time_tracker); }

  inline void stop_tracking(THD *thd) { ANALYZE_STOP_TRACKING(thd, &time_tracker); }

  /* Save container buffer size in bytes */
  inline void report_container_buff_size(uint elem_size) { container_buff_size = container_elements * elem_size / 8; }

  Time_and_counter_tracker *get_time_tracker() { return &time_tracker; }

  double get_time_fill_container_ms() const { return time_tracker.get_time_ms(); }
  ...
  {
    return static_cast<double>(n_positive_checks) / static_cast<double>(n_checks);
  }
