...
class Loose_scan_opt
{
 ...
 public:
  ...
  void init(JOIN *join, JOIN_TAB *s, table_map remaining_tables)
  {
    ...
    */
        ...
        optimizer_flag(join->thd, OPTIMIZER_SWITCH_LOOSE_SCAN))
    {
      ...
      bound_sj_equalities = get_bound_sj_equalities(s->emb_sj_nest, remaining_tables);
      try_loosescan = TRUE;
      DBUG_PRINT("info", ("Will try LooseScan scan, bound_map=%llx", (longlong)bound_sj_equalities));
    }
  }
  ...
  void check_ref_access_part1(JOIN_TAB *s, uint key, KEYUSE *start_key, table_map found_part)
  {
    /*
    ...
    */
    ...
    {
      if (s->quick && s->quick->index == key && s->quick->get_type() == QUICK_SELECT_I::QS_TYPE_RANGE)
      {
      ...
      */
          ...
          s->table->covering_keys.is_set(key))
      {
        ...
        double read_time = s->table->file->keyread_time(key, 1, (ha_rows)records);
  ...
  void check_ref_access_part2(uint key, KEYUSE *start_key, double records, double read_time,
                              table_map ref_depend_map_arg)
  {
  ...
  void check_range_access(JOIN *join, uint idx, QUICK_SELECT_I *quick)
  {
    ...
    if (quick_uses_applicable_index && idx == join->const_tables && quick->read_time < best_loose_scan_cost)
    {
  ...
  void save_to_position(JOIN_TAB *tab, POSITION *pos)
  {
    ...
    if (best_loose_scan_cost != DBL_MAX)
    {
      ...
      DBUG_PRINT("info", ("Produced a LooseScan plan, key %s, %s", tab->table->key_info[best_loose_scan_key].name.str,
                          best_loose_scan_start_key ? "(ref access)" : "(range/index access)"));
    }
  }
};
