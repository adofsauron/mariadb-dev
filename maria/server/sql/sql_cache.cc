...
#define RW_WLOCK(M)                                          \
  {                                                          \
    DBUG_PRINT("lock", ("rwlock wlock %p", (M)));            \
    if (!mysql_rwlock_wrlock(M))                             \
...
#define RW_RLOCK(M)                                          \
  {                                                          \
    DBUG_PRINT("lock", ("rwlock rlock %p", (M)));            \
    if (!mysql_rwlock_rdlock(M))                             \
...
#define RW_UNLOCK(M)                                          \
  {                                                           \
    DBUG_PRINT("lock", ("rwlock unlock %p", (M)));            \
    if (!mysql_rwlock_unlock(M))                              \
...
#define BLOCK_LOCK_WR(B)                                  \
  {                                                       \
    DBUG_PRINT("lock", ("%d LOCK_WR %p", __LINE__, (B))); \
    B->query()->lock_writing();                           \
  }
#define BLOCK_LOCK_RD(B)                                  \
  {                                                       \
    DBUG_PRINT("lock", ("%d LOCK_RD %p", __LINE__, (B))); \
    B->query()->lock_reading();                           \
  }
#define BLOCK_UNLOCK_WR(B)                                  \
  {                                                         \
    DBUG_PRINT("lock", ("%d UNLOCK_WR %p", __LINE__, (B))); \
    B->query()->unlock_writing();                           \
  }
#define BLOCK_UNLOCK_RD(B)                                  \
  {                                                         \
    DBUG_PRINT("lock", ("%d UNLOCK_RD %p", __LINE__, (B))); \
...
struct Query_cache_wait_state
{
  ...
  Query_cache_wait_state(THD *thd, const char *func, const char *file, unsigned int line)
      : m_thd(thd), m_old_stage(), m_func(func), m_file(file), m_line(line)
  {
    if (m_thd)
      set_thd_stage_info(m_thd, &stage_waiting_for_query_cache_lock, &m_old_stage, m_func, m_file, m_line);
  }
...
inline bool is_white_space(char c) { return (query_state_map[(uint)((uchar)c)] == MY_LEX_SKIP); }
...
static void make_base_query(String *new_query, const char *query, size_t query_length, size_t additional_length)
{
  char *buffer;
  ...
  while (query < query_end)
  {
    ...
    switch (current)
    {
      ...
      case '/':    // Start of comment ?
        ...
        */
        if (query[0] != '*' || query[1] == '!' || (query[1] == 'M' && query[2] == '!'))
          break;
    ...
    else
    {
      ...
      */
      ...
      else if (mode == TIMEOUT)
      {
        ...
        int res = mysql_cond_timedwait(&COND_cache_status_changed, &structure_guard_mutex, &waittime);
        if (res == ETIMEDOUT)
          break;
      }
      else
...
#endif
  DBUG_ASSERT(m_cache_lock_status == Query_cache::LOCKED || m_cache_lock_status == Query_cache::LOCKED_NO_WAIT);
...
{
  while (is_white_space(*sql)) sql++;

  if (my_toupper(system_charset_info, sql[0]) == 'S' && my_toupper(system_charset_info, sql[1]) == 'Q' &&
      my_toupper(system_charset_info, sql[2]) == 'L' && my_toupper(system_charset_info, sql[3]) == '_' &&
      my_toupper(system_charset_info, sql[4]) == 'N' && my_toupper(system_charset_info, sql[5]) == 'O' &&
      my_toupper(system_charset_info, sql[6]) == '_' && my_toupper(system_charset_info, sql[7]) == 'C' &&
      my_toupper(system_charset_info, sql[8]) == 'A' && my_toupper(system_charset_info, sql[9]) == 'C' &&
      my_toupper(system_charset_info, sql[10]) == 'H' && my_toupper(system_charset_info, sql[11]) == 'E' &&
...
{
  return (Query_cache_block *)(((uchar *)this) - ALIGN_SIZE(sizeof(Query_cache_block_table) * n) -
...
void Query_cache_block::init(size_t block_length)
{
  ...
  DBUG_PRINT("qcache", ("init block: %p  length: %zu", this, block_length));
  length = block_length;
  used = 0;
...
void Query_cache_block::destroy()
{
  ...
  DBUG_PRINT("qcache", ("destroy block %p, type %d", this, type));
  type = INCOMPLETE;
  DBUG_VOID_RETURN;
}
...
uchar *Query_cache_block::data(void) { return (uchar *)(((uchar *)this) + headers_len()); }
...
{
  return ((Query_cache_block_table *)(((uchar *)this) + ALIGN_SIZE(sizeof(Query_cache_block)) +
...
extern "C"
{
  uchar *query_cache_table_get_key(const uchar *record, size_t *length, my_bool not_used __attribute__((unused)))
  {
    ...
    *length = (table_block->used - table_block->headers_len() - ALIGN_SIZE(sizeof(Query_cache_table)));
    return (((uchar *)table_block->data()) + ALIGN_SIZE(sizeof(Query_cache_table)));
  }
}
...
void Query_cache_query::init_n_lock()
{
  ...
  lock_writing();
  DBUG_PRINT("qcache", ("inited & locked query for block %p", (uchar *)this - ALIGN_SIZE(sizeof(Query_cache_block))));
  DBUG_VOID_RETURN;
}
...
void Query_cache_query::unlock_n_destroy()
{
  ...
  DBUG_PRINT("qcache",
             ("destroyed & unlocked query for block %p", (uchar *)this - ALIGN_SIZE(sizeof(Query_cache_block))));
  /*
...
extern "C"
{
  uchar *query_cache_query_get_key(const uchar *record, size_t *length, my_bool not_used)
  {
    ...
    *length = (query_block->used - query_block->headers_len() - ALIGN_SIZE(sizeof(Query_cache_query)));
    return (((uchar *)query_block->data()) + ALIGN_SIZE(sizeof(Query_cache_query)));
  }
}
...
*/
void query_cache_insert(void *thd_arg, const char *packet, size_t length, unsigned pkt_nr)
{
  THD *thd = (THD *)thd_arg;
  ...
  */
  ...
  query_cache.insert(thd, &thd->query_cache_tls, packet, (size_t)length, pkt_nr);
}
...
void Query_cache::insert(THD *thd, Query_cache_tls *query_cache_tls, const char *packet, size_t length, unsigned pkt_nr)
{
  ...
  */
  if (!append_result_data(&result, length, (uchar *)packet, query_block))
  {
...
#ifdef EMBEDDED_LIBRARY
  insert(thd, query_cache_tls, (char *)thd, emb_count_querycache_size(thd), 0);
#endif
...
*/
extern "C" void mysql_query_cache_invalidate4(THD *thd, const char *key, unsigned key_length, int using_trx)
{
...
Query_cache::Query_cache(size_t query_cache_limit_arg, size_t min_allocation_unit_arg, size_t min_result_data_size_arg,
    ...
    : query_cache_size(0),
      ...
      initialized(0)
{
  size_t min_needed = (ALIGN_SIZE(sizeof(Query_cache_block)) + ALIGN_SIZE(sizeof(Query_cache_block_table)) +
...
size_t Query_cache::resize(size_t query_cache_size_arg)
{
  ...
  DBUG_PRINT("qcache", ("from %zu to %zu", query_cache_size, query_cache_size_arg));
...
void Query_cache::store_query(THD *thd, TABLE_LIST *tables_used)
{
  ...
  */
  ...
  DBUG_ASSERT(thd->base_query.is_alloced() || thd->base_query.ptr() == thd->query());
  ...
  if ((local_tables = is_cacheable(thd, thd->lex, tables_used, &tables_type)))
  {
    ...
    flags.client_protocol_41 = MY_TEST(thd->client_capabilities & CLIENT_PROTOCOL_41);
    flags.client_extended_metadata = MY_TEST(thd->client_capabilities & MARIADB_CLIENT_EXTENDED_METADATA);
    flags.client_depr_eof = MY_TEST(thd->client_capabilities & CLIENT_DEPRECATE_EOF);
    /*
    ...
    */
    ...
    flags.more_results_exists = MY_TEST(thd->server_status & SERVER_MORE_RESULTS_EXISTS);
...
def_week_frmt: %zu, in_trans: %d, autocommit: %d",
         (int)flags.client_long_flag, (int)flags.client_protocol_41, (int)flags.client_extended_metadata,
         (int)flags.client_depr_eof, (int)flags.protocol_type, (int)flags.more_results_exists, flags.pkt_nr,
         flags.character_set_client_num, flags.character_set_results_num, flags.collation_connection_num,
         (ulonglong)flags.limit, flags.time_zone, flags.sql_mode, flags.max_sort_length, flags.group_concat_max_len,
         flags.div_precision_increment, flags.default_week_format, (int)flags.in_trans, (int)flags.autocommit));
    ...
    */
    ...
    if (thd->db.length)
    {
      memcpy((char *)(query + query_length + 1 + QUERY_CACHE_DB_LENGTH_SIZE), thd->db.str, thd->db.length);
      DBUG_PRINT("qcache", ("database: %s  length: %u", thd->db.str, (unsigned)thd->db.length));
    }
    else
    {
      DBUG_PRINT("qcache", ("No active database"));
    }
    tot_length = (query_length + thd->db.length + 1 + QUERY_CACHE_DB_LENGTH_SIZE + QUERY_CACHE_FLAGS_SIZE);
    /*
      We should only copy structure (don't use it location directly)
      because of alignment issue
    */
    memcpy((void *)(query + (tot_length - QUERY_CACHE_FLAGS_SIZE)), &flags, QUERY_CACHE_FLAGS_SIZE);

    /* Check if another thread is processing the same query? */
    Query_cache_block *competitor = (Query_cache_block *)my_hash_search(&queries, (uchar *)query, tot_length);
    DBUG_PRINT("qcache", ("competitor %p", competitor));
    if (competitor == 0)
    {
      /* Query is not in cache and no one is working with it; Store it */
      Query_cache_block *query_block;
      query_block = write_block_data(tot_length, (uchar *)query, ALIGN_SIZE(sizeof(Query_cache_query)),
                                     Query_cache_block::QUERY, local_tables);
      if (query_block != 0)
      {
        DBUG_PRINT("qcache", ("query block %p allocated, %zu", query_block, query_block->used));
...
*/
static bool send_data_in_chunks(NET *net, const uchar *packet, size_t len)
{
  /*
...
size_t build_normalized_name(char *buff, size_t bufflen, const char *db, size_t db_len, const char *table_name,
                             size_t table_len, size_t suffix_len)
{
  uint errors;
  size_t length;
  ...
  (*pos++) = FN_LIBCHAR;
  length = strconvert(system_charset_info, db, db_len, &my_charset_filename, pos, bufflen - 3, &errors);
  pos += length;
  (*pos++) = FN_LIBCHAR;
  length =
      strconvert(system_charset_info, table_name, table_len, &my_charset_filename, pos, (uint)(end - pos), &errors);
  pos += length;
...
int Query_cache::send_result_to_client(THD *thd, char *org_sql, uint query_length)
{
  ...
  */
      ...
      switch (current)
      {
        case '/':
          ...
          if (*sql == '!')
          {
            ...
            */
            sql++;
            while (my_isdigit(system_charset_info, *sql)) sql++;
          }
          else
          {
            while (sql++ < sql_end)
  ...
  if ((my_toupper(system_charset_info, sql[0]) != 'S' || my_toupper(system_charset_info, sql[1]) != 'E' ||
      ...
      (my_toupper(system_charset_info, sql[0]) != 'W' || my_toupper(system_charset_info, sql[1]) != 'I' ||
  ...
  */
  ...
  tot_length = (query_length + 1 + QUERY_CACHE_DB_LENGTH_SIZE + thd->db.length + QUERY_CACHE_FLAGS_SIZE);

  if (thd->db.length)
  {
    memcpy((uchar *)sql + query_length + 1 + QUERY_CACHE_DB_LENGTH_SIZE, thd->db.str, thd->db.length);
    DBUG_PRINT("qcache", ("database: '%s'  length: %u", thd->db.str, (uint)thd->db.length));
  }
  else
  {
    DBUG_PRINT("qcache", ("No active database"));
...
def_week_frmt: %zu, in_trans: %d, autocommit: %d",
                        (int)flags.client_long_flag, (int)flags.client_protocol_41, (int)flags.client_extended_metadata,
                        (int)flags.client_depr_eof, (int)flags.protocol_type, (int)flags.more_results_exists,
                        flags.pkt_nr, flags.character_set_client_num, flags.character_set_results_num,
                        flags.collation_connection_num, (ulonglong)flags.limit, flags.time_zone, flags.sql_mode,
                        flags.max_sort_length, flags.group_concat_max_len, flags.div_precision_increment,
                        flags.default_week_format, (int)flags.in_trans, (int)flags.autocommit));
...
lookup:
  ...
  query_block = (Query_cache_block *)my_hash_search(&queries, (uchar *)sql, tot_length);
  ...
  if (thd->in_multi_stmt_transaction_mode() && (query->tables_type() & HA_CACHE_TBL_TRANSACT))
  {
  ...
  for (; block_table != block_table_end; block_table++)
  {
    ...
    */
    if ((tmptable = thd->find_tmp_table_share_w_base_key((char *)table->data(), table->key_length())))
    {
      DBUG_PRINT("qcache", ("Temporary table detected: '%s.%s'", tmptable->db.str, tmptable->table_name.str));
      unlock();
      /*
    ...
    {
      DBUG_PRINT("qcache", ("probably no SELECT access to %s.%s =>  return to normal processing", table_list.db.str,
    ...
    {
      DBUG_PRINT("qcache", ("Need to check column privileges for %s.%s", table_list.db.str, table_list.alias.str));
      BLOCK_UNLOCK_RD(query_block);
      thd->query_cache_is_applicable = 0;  // Query can't be cached
      thd->lex->safe_to_cache_query = 0;   // For prepared statements
    ...
    if (table->callback())
    {
      ...
      qcache_se_key_len =
          build_normalized_name(qcache_se_key_name, sizeof(qcache_se_key_name), table->db(), db_length, table->table(),
                                table->key_length() - db_length - 2 - table->suffix_length(), table->suffix_length());

      if (!(*table->callback())(thd, qcache_se_key_name, (uint)qcache_se_key_len, &engine_data))
      {
        DBUG_PRINT("qcache", ("Handler does not allow caching for %.*s", (int)qcache_se_key_len, qcache_se_key_name));
        BLOCK_UNLOCK_RD(query_block);
        if (engine_data != table->engine_data())
        {
          DBUG_PRINT("qcache", ("Handler require invalidation queries of %.*s %llu-%llu", (int)qcache_se_key_len,
                                qcache_se_key_name, engine_data, table->engine_data()));
          invalidate_table_internal(thd, (uchar *)table->db(), table->key_length());
        }
        else
        {
          /*
      ...
      DBUG_PRINT("qcache", ("handler allow caching %s,%s", table_list.db.str, table_list.alias.str));
  }
  ...
  */
  ...
  do
  {
    DBUG_PRINT("qcache", ("Results  (len: %zu  used: %zu  headers: %u)", result_block->length, result_block->used,
                          (uint)(result_block->headers_len() + ALIGN_SIZE(sizeof(Query_cache_result)))));
...
void Query_cache::invalidate_locked_for_write(THD *thd, TABLE_LIST *tables_used)
{
  ...
  for (; tables_used; tables_used = tables_used->next_local)
  {
    ...
    if (tables_used->lock_type >= TL_FIRST_WRITE && tables_used->table)
    {
...
void Query_cache::invalidate(THD *thd, const char *key, size_t key_length, my_bool using_transactions)
{
...
void Query_cache::init()
{
  ...
  mysql_mutex_init(key_structure_guard_mutex, &structure_guard_mutex, MY_MUTEX_INIT_FAST);
  mysql_cond_init(key_COND_cache_status_changed, &COND_cache_status_changed, NULL);
  m_cache_lock_status = Query_cache::UNLOCKED;
...
size_t Query_cache::init_cache()
{
  ...
  approx_additional_data_size = (sizeof(Query_cache) + sizeof(uchar *) * (def_query_hash_size + def_table_hash_size));
  ...
  */
  ...
  if (!(cache = (uchar *)my_malloc_lock(query_cache_size + additional_data_size, MYF(0))))
    goto err;
  ...
  {
    DBUG_PRINT("warning", ("coudn't mark query cache memory as " DONTDUMP_STR ": %s", strerror(errno)));
  }
#endif

  DBUG_PRINT("qcache",
             ("cache length %zu, min unit %zu, %zu bins", query_cache_size, min_allocation_unit, mem_bin_num));
  ...
  */
  ...
  (void)my_hash_init(key_memory_Query_cache, &queries, &my_charset_bin, def_query_hash_size, 0, 0,
  ...
  */
  (void)my_hash_init(key_memory_Query_cache, &tables, &my_charset_bin, def_table_hash_size, 0, 0,
  ...
  /*
    ...
    comparison.
  */
  (void)my_hash_init(PSI_INSTRUMENT_ME, &tables, lower_case_table_names ? &my_charset_bin : files_charset_info,
...
void Query_cache::free_cache()
{
  ...
  DBUG_ASSERT(m_cache_lock_status == LOCKED_NO_WAIT || m_cache_status == DISABLE_REQUEST);
  ...
  {
    DBUG_PRINT("warning", ("coudn't mark query cache memory as " DODUMP_STR ": %s", strerror(errno)));
  }
#endif
  my_free(cache);
  make_disabled();
...
void Query_cache::free_query_internal(Query_cache_block *query_block)
{
  ...
  DBUG_PRINT("qcache", ("free query %p %zu bytes result", query_block, query_block->query()->length()));
...
Query_cache_block *Query_cache::write_block_data(size_t data_len, uchar *data, size_t header_len,
                                                 Query_cache_block::block_type type, TABLE_COUNTER_TYPE ntab)
{
  ...
  DBUG_PRINT("qcache", ("data: %zd, header: %zd, all header: %zd", data_len, header_len, all_headers_len));
  Query_cache_block *block = allocate_block(MY_MAX(align_len, min_allocation_unit), 1, 0);
  if (block != 0)
  {
    block->type = type;
    block->n_tables = ntab;
...
my_bool Query_cache::append_result_data(Query_cache_block **current_block, size_t data_len, uchar *data,
                                        Query_cache_block *query_block)
{
  DBUG_ENTER("Query_cache::append_result_data");
  DBUG_PRINT("qcache", ("append %zu bytes to %p query", data_len, query_block));

  if (query_block->query()->add(data_len) > query_cache_limit)
  {
    DBUG_PRINT("qcache", ("size limit reached %zu > %zu", query_block->query()->length(), query_cache_limit));
    DBUG_RETURN(0);
  }
  if (*current_block == 0)
  {
  ...
  DBUG_PRINT("qcache", ("lastblock %p len %zu used %zu", last_block, last_block->length, last_block->used));
  my_bool success = 1;
  ...
  */
  ...
  if (last_block_free_space < data_len && append_next_free_block(last_block, MY_MAX(tail, append_min)))
    last_block_free_space = last_block->length - last_block->used;
  // If no space in last block (even after join) allocate new block
  if (last_block_free_space < data_len)
  {
    DBUG_PRINT("qcache", ("allocate new block for %zu bytes", data_len - last_block_free_space));
    Query_cache_block *new_block = 0;
    success =
        write_result_data(&new_block, data_len - last_block_free_space,
                          (uchar *)(((uchar *)data) + last_block_free_space), query_block, Query_cache_block::RES_CONT);
    /*
  ...
  if (success && last_block_free_space > 0)
  {
    ...
    DBUG_PRINT("qcache", ("use free space %zub at block %p to copy %zub", last_block_free_space, last_block, to_copy));
...
my_bool Query_cache::write_result_data(Query_cache_block **result_block, size_t data_len, uchar *data,
  ...
  */
  ...
  if (success)
  {
    ...
    do
    {
      ...
      DBUG_PRINT("qcache", ("write %zu byte in block %p", length, block));
...
inline size_t Query_cache::get_min_append_result_data_size() { return min_result_data_size; }

/*
  Allocate one or more blocks to hold data
*/
my_bool Query_cache::allocate_data_chain(Query_cache_block **result_block, size_t data_len,
                                         Query_cache_block *query_block, my_bool first_block_arg)
{
  size_t all_headers_len = (ALIGN_SIZE(sizeof(Query_cache_block)) + ALIGN_SIZE(sizeof(Query_cache_result)));
  size_t min_size = (first_block_arg ? get_min_first_result_data_size() : get_min_append_result_data_size());
  Query_cache_block *prev_block = NULL;
  Query_cache_block *new_block;
  DBUG_ENTER("Query_cache::allocate_data_chain");
  DBUG_PRINT("qcache", ("data_len %zu, all_headers_len %zu", data_len, all_headers_len));
  ...
  do
  {
    ...
    if (!(new_block = allocate_block(MY_MAX(min_size, align_len), min_result_data_size == 0,
    ...
    DBUG_PRINT("qcache", ("Block len %zu used %zu", new_block->length, new_block->used));
...
void Query_cache::invalidate_table_internal(THD *thd, uchar *key, size_t key_length)
{
  Query_cache_block *table_block = (Query_cache_block *)my_hash_search(&tables, key, key_length);
  if (table_block)
  {
    Query_cache_block_table *list_root = table_block->table(0);
    invalidate_query_block_list(thd, list_root);
...
void Query_cache::invalidate_query_block_list(THD *thd, Query_cache_block_table *list_root)
{
...
TABLE_COUNTER_TYPE
Query_cache::register_tables_from_list(THD *thd, TABLE_LIST *tables_used, TABLE_COUNTER_TYPE counter,
                                       Query_cache_block_table **block_table)
{
  ...
  {
    if (tables_used->is_anonymous_derived_table() || tables_used->table_function)
    {
    ...
    if (tables_used->view)
    {
      ...
      */
      if (!insert_table(thd, key_length, key, (*block_table), tables_used->view_db.length, 0, HA_CACHE_TBL_NONTRANSACT,
                        0, 0, TRUE))
        goto err_cleanup;
      /*
    ...
    {
      DBUG_PRINT("qcache",
                 ("table: %s  db: %s  openinfo:  %p  keylen: %zu  key: %p", tables_used->table->s->table_name.str,
                  tables_used->table->s->table_cache_key.str, tables_used->table,
                  tables_used->table->s->table_cache_key.length, tables_used->table->s->table_cache_key.str));

      if (!insert_table(thd, tables_used->table->s->table_cache_key.length, tables_used->table->s->table_cache_key.str,
                        (*block_table), tables_used->db.length, 0, tables_used->table->file->table_cache_type(),
                        tables_used->callback_func, tables_used->engine_data, TRUE))
        goto err_cleanup;

      if (tables_used->table->file->register_query_cache_dependant_tables(thd, this, block_table, &n))
        DBUG_RETURN(0);
    }
  }
...
my_bool Query_cache::register_all_tables(THD *thd, Query_cache_block *block, TABLE_LIST *tables_used,
                                         TABLE_COUNTER_TYPE tables_arg)
{
  TABLE_COUNTER_TYPE n;
  DBUG_PRINT("qcache", ("register tables block %p, n %d, header %x", block, (int)tables_arg,
...
my_bool Query_cache::insert_table(THD *thd, size_t key_len, const char *key, Query_cache_block_table *node,
                                  size_t db_length, uint8 suffix_length_arg, uint8 cache_type,
                                  qc_engine_callback callback, ulonglong engine_data, my_bool hash)
{
  DBUG_ENTER("Query_cache::insert_table");
  DBUG_PRINT("qcache", ("insert table node %p, len %zu", node, key_len));

  Query_cache_block *table_block = (hash ? (Query_cache_block *)my_hash_search(&tables, (uchar *)key, key_len) : NULL);

  if (table_block && table_block->table()->engine_data() != engine_data)
  {
    DBUG_PRINT("qcache", ("Handler require invalidation queries of %s.%s %llu-%llu", table_block->table()->db(),
  ...
  if (table_block == 0)
  {
    DBUG_PRINT("qcache", ("new table block from %p (%u)", key, (int)key_len));
    table_block =
        write_block_data(key_len, (uchar *)key, ALIGN_SIZE(sizeof(Query_cache_table)), Query_cache_block::TABLE, 1);
    if (table_block == 0)
    {
    ...
    */
    ...
    if (hash && my_hash_insert(&tables, (const uchar *)table_block))
    {
...
Query_cache_block *Query_cache::allocate_block(size_t len, my_bool not_less, size_t min)
{
  DBUG_ENTER("Query_cache::allocate_block");
  DBUG_PRINT("qcache", ("len %zu, not less %d, min %zu", len, not_less, min));

  if (len >= MY_MIN(query_cache_size, query_cache_limit))
  {
    DBUG_PRINT("qcache", ("Query cache hase only %zu memory and limit %zu", query_cache_size, query_cache_limit));
...
Query_cache_block *Query_cache::get_free_block(size_t len, my_bool not_less, size_t min)
{
  Query_cache_block *block = 0, *first = 0;
  DBUG_ENTER("Query_cache::get_free_block");
  DBUG_PRINT("qcache", ("length %zu, not_less %d, min %zu", len, (int)not_less, min));
...
void Query_cache::free_memory_block(Query_cache_block *block)
{
  ...
  DBUG_PRINT("qcache", ("first_block %p, block %p, pnext %p pprev %p", first_block, block, block->pnext, block->pprev));
...
void Query_cache::split_block(Query_cache_block *block, size_t len)
{
  ...
  DBUG_PRINT("qcache", ("split %p (%zu) new %p", block, len, new_block));
  DBUG_VOID_RETURN;
}
...
Query_cache_block *Query_cache::join_free_blocks(Query_cache_block *first_block_arg, Query_cache_block *block_in_list)
{
  ...
  DBUG_PRINT("qcache", ("join first %p, pnext %p, in list %p", first_block_arg, first_block_arg->pnext, block_in_list));
...
my_bool Query_cache::append_next_free_block(Query_cache_block *block, size_t add_size)
{
  Query_cache_block *next_block = block->pnext;
  DBUG_ENTER("Query_cache::append_next_free_block");
  DBUG_PRINT("enter", ("block %p, add_size %zu", block, add_size));
...
void Query_cache::exclude_from_free_memory_list(Query_cache_block *free_block)
{
  ...
  free_memory_blocks--;
  DBUG_PRINT("qcache", ("exclude block %p, bin %p", free_block, bin));
  DBUG_VOID_RETURN;
}
...
void Query_cache::insert_into_free_memory_list(Query_cache_block *free_block)
{
  ...
  *bin_ptr = bins + idx;
  (*bin_ptr)->number++;
  DBUG_PRINT("qcache", ("insert block %p, bin[%d] %p", free_block, idx, *bin_ptr));
  DBUG_VOID_RETURN;
}
...
uint Query_cache::find_bin(size_t size)
{
  ...
  size_t bin = steps[left].idx - ((size - steps[left].size) / steps[left].increment);

  DBUG_PRINT("qcache", ("bin %zu step %zu, size %zu step size %zu", bin, left, size, steps[left].size));
...
void Query_cache::double_linked_list_exclude(Query_cache_block *point, Query_cache_block **list_pointer)
{
  ...
  DBUG_PRINT("qcache", ("excluding block %p, list %p", point, list_pointer));
  if (point->next == point)
...
TABLE_COUNTER_TYPE
Query_cache::process_and_count_tables(THD *thd, TABLE_LIST *tables_used, uint8 *tables_type)
{
  ...
  for (; tables_used; tables_used = tables_used->next_global)
  {
    ...
    */
    if (tables_used->grant.want_privilege && tables_used->belong_to_view == NULL)
    {
    ...
    if (tables_used->derived || tables_used->table_function)
    {
      ...
      table_count--;
      DBUG_PRINT("qcache", (tables_used->table_function ? "table function skipped" : "derived table skipped"));
      continue;
    }

    DBUG_PRINT("qcache", ("table: %s  db:  %s  type: %u", tables_used->table->s->table_name.str,
                          tables_used->table->s->db.str, tables_used->table->s->db_type()->db_type));
    ...
    */
    table_count += tables_used->table->file->count_query_cache_dependant_tables(tables_type);
...
TABLE_COUNTER_TYPE
Query_cache::is_cacheable(THD *thd, LEX *lex, TABLE_LIST *tables_used, uint8 *tables_type)
{
      ...
      qc_is_able_to_intercept_result(thd))
  {
    DBUG_PRINT("qcache", ("options: %lx  %lx  type: %u", (long)OPTION_TO_QUERY_CACHE,
                          (long)lex->first_select_lex()->options, (int)thd->variables.query_cache_type));

    if (!(table_count = process_and_count_tables(thd, tables_used, tables_type)))
      DBUG_RETURN(0);

    if (thd->in_multi_stmt_transaction_mode() && ((*tables_type) & HA_CACHE_TBL_TRANSACT))
    {
  ...
  DBUG_PRINT("qcache", ("not interesting query: %d or not cacheable, options %lx %lx  type: %u  net->vio present: %u",
                        (int)lex->sql_command, (long)OPTION_TO_QUERY_CACHE, (long)lex->first_select_lex()->options,
                        (int)thd->variables.query_cache_type, (uint)MY_TEST(qc_is_able_to_intercept_result(thd))));
  DBUG_RETURN(0);
}
...
my_bool Query_cache::ask_handler_allowance(THD *thd, TABLE_LIST *tables_used)
{
  ...
  for (; tables_used; tables_used = tables_used->next_global)
  {
    ...
    if (!(table = tables_used->table))
      continue;
    handler = table->file;
    if (!handler->register_query_cache_table(thd, table->s->normalized_path.str, (uint)table->s->normalized_path.length,
                                             &tables_used->callback_func, &tables_used->engine_data))
    {
      DBUG_PRINT("qcache", ("Handler does not allow caching for %s", table->s->normalized_path.str));
      /*
        As this can change from call to call, don't reset set
        thd->lex->safe_to_cache_query
      */
...
my_bool Query_cache::move_by_type(uchar **border, Query_cache_block **before, size_t *gap, Query_cache_block *block)
{
  ...
  switch (block->type)
  {
    ...
    case Query_cache_block::TABLE:
    {
      ...
      DBUG_PRINT("qcache", ("list_root: %p tnext %p tprev %p tprev->next %p tnext->prev %p", list_root, tnext, tprev,
      ...
      *before = new_block;
      ...
      DBUG_PRINT("qcache", ("moved %zu bytes to %p, new gap at %p", len, new_block, *border));
      break;
    }
      ...
      for (TABLE_COUNTER_TYPE j = 0; j < n_tables; j++)
      {
        ...
        if ((beg_of_table_table <= block_table->next) && (block_table->next < end_of_table_table))
          ((Query_cache_block_table *)(beg_of_new_table_table +
                                       (((uchar *)block_table->next) - ((uchar *)beg_of_table_table))))
              ->prev = block_table;
        else
        ...
        if ((beg_of_table_table <= block_table->prev) && (block_table->prev < end_of_table_table))
          ((Query_cache_block_table *)(beg_of_new_table_table +
                                       (((uchar *)block_table->prev) - ((uchar *)beg_of_table_table))))
              ->next = block_table;
        else
    ...
    {
      DBUG_PRINT("qcache", ("block %p RES* (%d)", block, (int)block->type));
      if (*border == 0)
        break;
      ...
      *before = new_block;
          ...
          *gap + free_space > min_allocation_unit && new_block->length - free_space > min_allocation_unit)
      {
        *border -= free_space;
        *gap += free_space;
        DBUG_PRINT("qcache", ("rest of result free space added to gap (%zu)", *gap));
    ...
    default:
      DBUG_PRINT("error", ("unexpected block type %d, block %p", (int)block->type, block));
      ok = 0;
  }
  DBUG_RETURN(ok);
}
    ...
    do
    {
      ...
      if (header->result() != 0 && header->result()->type == Query_cache_block::RESULT && header->length() > join_limit)
      {
        ...
        if (new_result_block != 0)
        {
          ...
          DBUG_PRINT("qcache",
                     ("new block %zu/%zu (%zu)", new_result_block->length, new_result_block->used, header->length()));
          ...
          do
          {
            size_t len = (result_block->used - result_block->headers_len() - ALIGN_SIZE(sizeof(Query_cache_result)));
            DBUG_PRINT("loop", ("add block %zu/%zu (%zu)", result_block->length, result_block->used, len));
...
uint Query_cache::filename_2_table_key(char *key, const char *path, uint32 *db_length)
{
  ...
  *db_length = (uint32)(filename - dbname) - 1;
  ...
  DBUG_RETURN((uint)(strmake(strmake(key, dbname, MY_MIN(*db_length, NAME_LEN)) + 1, filename, NAME_LEN) - key) + 1);
}
...
void Query_cache::bins_dump()
{
  ...
  DBUG_PRINT("qcache", ("mem_bin_num=%zu, mem_bin_steps=%zu", mem_bin_num, mem_bin_steps));
  DBUG_PRINT("qcache", ("-------------------------"));
  DBUG_PRINT("qcache", ("      size idx       step"));
  DBUG_PRINT("qcache", ("-------------------------"));
  for (i = 0; i < mem_bin_steps; i++)
  {
    DBUG_PRINT("qcache", ("%10zu %3zd %10zu", steps[i].size, steps[i].idx, steps[i].increment));
  }
  DBUG_PRINT("qcache", ("-------------------------"));
  ...
  {
    DBUG_PRINT("qcache", ("%10zu %3d %p", bins[i].size, bins[i].number, &(bins[i])));
    if (bins[i].free_blocks)
    {
      Query_cache_block *block = bins[i].free_blocks;
      do
      {
        DBUG_PRINT("qcache", ("\\-- %zu %p %p %p %p %p", block->length, block, block->next, block->prev, block->pnext,
                              block->pprev));
        block = block->next;
...
void Query_cache::cache_dump()
{
  ...
  do
  {
    DBUG_PRINT("qcache", ("%10zu %10zu %1d %2d %p %p %p %p %p", i->length, i->used, (int)i->type, i->n_tables, i,
                          i->next, i->prev, i->pnext, i->pprev));
    i = i->pnext;
  } while (i != first_block);
  DBUG_PRINT("qcache", ("-------------------------------------"));
}
...
void Query_cache::queries_dump()
{
  ...
  if (queries_blocks != 0)
  {
    ...
    do
    {
      ...
      DBUG_PRINT("qcache", ("F: %u  C: %u L: %llu  T: '%s' (%zu)  '%s'  '%s'", flags.client_long_flag,
                            flags.character_set_client_num, flags.limit, flags.time_zone->get_name()->ptr(), len, str,
                            strend(str) + 1));
      DBUG_PRINT("qcache", ("-b- %p %p %p %p %p", block, block->next, block->prev, block->pnext, block->pprev));
      ...
      if (header->result())
      {
        ...
        do
        {
          DBUG_PRINT("qcache", ("-r- %u %zu/%zu %p %p %p %p %p", (uint)result_block->type, result_block->length,
                                result_block->used, result_block, result_block->next, result_block->prev,
                                result_block->pnext, result_block->pprev));
          result_block = result_block->next;
...
my_bool Query_cache::check_integrity(bool locked)
{
  ...
  do
  {
    ...
    DBUG_PRINT("qcache", ("block %p, type %u...", block, (uint)block->type));
    // Check allignment
    if ((((size_t)block) % ALIGN_SIZE(1)) != (((size_t)first_block) % ALIGN_SIZE(1)))
    {
      DBUG_PRINT("error", ("block %p do not aligned by %d", block, (int)ALIGN_SIZE(1)));
      result = 1;
    }
    ...
    if (block->pnext == first_block)  // Is it last block?
    {
      ...
      {
        DBUG_PRINT("error", ("block %p, type %u, ended at %p, but cache ended at %p", block, (uint)block->type,
                             (((uchar *)block) + block->length), (((uchar *)first_block) + query_cache_size)));
        result = 1;
      }
    }
    else if (((uchar *)block) + block->length != ((uchar *)block->pnext))
    {
      DBUG_PRINT("error", ("block %p, type %u, ended at %p, but next block beginning at %p", block, (uint)block->type,
                           (((uchar *)block) + block->length), ((uchar *)block->pnext)));
    }
    if (block->type == Query_cache_block::FREE)
      free += block->length;
    else
    ...
    switch (block->type)
    {
      case Query_cache_block::FREE:
      {
        ...
        if (((uchar *)bin) < ((uchar *)bins) || ((uchar *)bin) >= ((uchar *)first_block))
        {
          DBUG_PRINT("error", ("free block %p have bin pointer %p beyaond of bins array bounds [%p,%p]", block, bin,
                               bins, first_block));
          result = 1;
        }
        else
        {
          size_t idx = (((uchar *)bin) - ((uchar *)bins)) / sizeof(Query_cache_memory_bin);
      ...
      case Query_cache_block::QUERY:
      {
        ...
        for (TABLE_COUNTER_TYPE j = 0; j < block->n_tables; j++)
        {
          ...
          Query_cache_block_table *block_table_root =
              (Query_cache_block_table *)(((uchar *)block_table->parent) - ALIGN_SIZE(sizeof(Query_cache_block_table)));
      ...
      case Query_cache_block::RESULT:
      {
        ...
        {
          DBUG_PRINT("error", ("result block %p have query block pointer %p beyaond of block pool bounds [%p,%p]",
                               block, query_block, first_block, (((uchar *)first_block) + query_cache_size)));
          result = 1;
        }
        else
        {
      ...
      default:
        DBUG_PRINT("error", ("block %p have incorrect type %u", block, block->type));
        result = 1;
    }
  ...
  {
    DBUG_PRINT("error",
               ("used memory (%zu) + free memory (%zu) !=  query_cache_size (%zu)", used, free, query_cache_size));
    result = 1;
  }

  if (free != free_memory)
  {
    DBUG_PRINT("error", ("free memory (%zu) != free_memory (%zu)", free, free_memory));
    result = 1;
  }
  ...
  if ((block = queries_blocks))
  {
    do
    {
      ...
      if (((uchar *)block) != val)
      {
        DBUG_PRINT("error", ("block %p found in queries hash like %p", block, val));
      }
      if (in_blocks(block))
        result = 1;
  ...
  if ((block = tables_blocks))
  {
    do
    {
      ...
      if (((uchar *)block) != val)
      {
        DBUG_PRINT("error", ("block %p found in tables hash like %p", block, val));
      }
  ...
  for (i = 0; i < mem_bin_num; i++)
  {
    if ((block = bins[i].free_blocks))
    {
      ...
      {
        DBUG_PRINT("error", ("bins[%d].number= %d, but bin have %d blocks", i, bins[i].number, count));
        result = 1;
      }
    }
  }
...
my_bool Query_cache::in_blocks(Query_cache_block *point)
{
    ...
    if (block->pprev->pnext != block)
    {
      DBUG_PRINT("error",
                 ("block %p in physical list is incorrect linked, prev block %p referred as next to %p (check from %p)",
                  block, block->pprev, block->pprev->pnext, point));
      // back trace
      for (; block != point; block = block->pnext) DBUG_PRINT("error", ("back trace %p", block));
      result = 1;
      goto err1;
    }
    block = block->pprev;
  ...
  {
    DBUG_PRINT("error", ("block %p (%p<-->%p) not owned by pysical list", block, block->pprev, block->pnext));
    return 1;
  }
    ...
    if (block->pnext->pprev != block)
    {
      DBUG_PRINT("error",
                 ("block %p in physicel list is incorrect linked, next block %p referred as prev to %p (check from %p)",
                  block, block->pnext, block->pnext->pprev, point));
      // back trace
      for (; block != point; block = block->pprev) DBUG_PRINT("error", ("back trace %p", block));
      result = 1;
      goto err2;
    }
    block = block->pnext;
...
my_bool Query_cache::in_list(Query_cache_block *root, Query_cache_block *point, const char *name)
{
    ...
    if (block->prev->next != block)
    {
      DBUG_PRINT("error",
                 ("block %p in list '%s' %p is incorrect linked, prev block %p referred as next to %p (check from %p)",
                  block, name, root, block->prev, block->prev->next, point));
      // back trace
      for (; block != point; block = block->next) DBUG_PRINT("error", ("back trace %p", block));
      result = 1;
      goto err1;
    }
    block = block->prev;
  ...
  if (block != root)
  {
    DBUG_PRINT("error", ("block %p (%p<-->%p) not owned by list '%s' %p", block, block->prev, block->next, name, root));
    return 1;
  }
err1:
  // forward
    ...
    if (block->next->prev != block)
    {
      DBUG_PRINT("error",
                 ("block %p in list '%s' %p is incorrect linked, next block %p referred as prev to %p (check from %p)",
                  block, name, root, block->next, block->next->prev, point));
      // back trace
      for (; block != point; block = block->prev) DBUG_PRINT("error", ("back trace %p", block));
      result = 1;
      goto err2;
    }
    block = block->next;
...
void dump_node(Query_cache_block_table *node, const char *call, const char *descr)
{
  ...
  DBUG_PRINT("qcache", ("%s: %s: node block: %p", call, descr, node->block()));
  DBUG_PRINT("qcache", ("%s: %s: next: %p", call, descr, node->next));
  DBUG_PRINT("qcache", ("%s: %s: prev: %p", call, descr, node->prev));
}
...
my_bool Query_cache::in_table_list(Query_cache_block_table *root, Query_cache_block_table *point, const char *name)
{
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error", ("table %p(%p) in list '%s' %p(%p) is incorrect linked, prev table %p(%p) referred as next "
                           "to %p(%p) (check from %p(%p))",
                           table, table->block(), name, root, root->block(), table->prev, table->prev->block(),
                           table->prev->next, table->prev->next->block(), point, point->block()));
      // back trace
      for (; table != point; table = table->next) DBUG_PRINT("error", ("back trace %p(%p)", table, table->block()));
      result = 1;
      goto err1;
    }
    table = table->prev;
  ...
  {
    DBUG_PRINT("error",
               ("table %p(%p) (%p(%p)<-->%p(%p)) not owned by list '%s' %p(%p)", table, table->block(), table->prev,
                table->prev->block(), table->next, table->next->block(), name, root, root->block()));
    return 1;
  }
err1:
  // forward
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error", ("table %p(%p) in list '%s' %p(%p) is incorrect linked, next table %p(%p) referred as prev "
                           "to %p(%p) (check from %p(%p))",
                           table, table->block(), name, root, root->block(), table->next, table->next->block(),
                           table->next->prev, table->next->prev->block(), point, point->block()));
      // back trace
      for (; table != point; table = table->prev) DBUG_PRINT("error", ("back trace %p(%p)", table, table->block()));
      result = 1;
      goto err2;
    }
    table = table->next;
