...
{
 public:
  Poll_socket_listener(const Slave_ilist &slaves) : m_slaves(slaves) {}

  bool listen_on_sockets() { return poll(m_fds.data(), m_fds.size(), 1000 /*1 Second timeout*/); }

  bool is_socket_active(const Slave *slave) { return m_fds[slave->m_fds_index].revents & POLLIN; }
...
{
 public:
  Select_socket_listener(const Slave_ilist &slaves) : m_slaves(slaves), m_max_fd(INVALID_SOCKET) {}
  ...
  bool is_socket_active(const Slave *slave) { return FD_ISSET(slave->sock_fd(), &m_fds); }

  void clear_socket_info(const Slave *slave) { FD_CLR(slave->sock_fd(), &m_init_fds); }
  ...
  uint init_slave_sockets()
  {
    ...
    while ((slave = it++))
    {
      ...
      {
        sql_print_error(
            "Semisync slave socket fd is %u. "
            "select() cannot handle if the socket fd is "
            "greater than %u (FD_SETSIZE).",
