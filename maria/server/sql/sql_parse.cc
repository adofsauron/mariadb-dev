...
static bool wsrep_mysql_parse(THD *thd, char *rawbuf, uint length, Parser_state *parser_state);
...
inline bool all_tables_not_ok(THD *thd, TABLE_LIST *tables)
{
  ...
  return rpl_filter->is_on() && tables && !thd->spcont && !rpl_filter->tables_ok(thd->db.str, tables);
}
#endif
  ...
  switch (lex->sql_command)
  {
    ...
    case SQLCOM_CREATE_SEQUENCE:
      /*
        ...
        master and slave.
      */
      skip = (lex->tmp_table() || (thd->variables.option_bits & OPTION_GTID_BEGIN));
      break;
...
void init_update_queries(void)
{
  ...
  */
  sql_command_flags[SQLCOM_CREATE_TABLE] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_AUTO_COMMIT_TRANS |
                                           CF_REPORT_PROGRESS | CF_CAN_GENERATE_ROW_EVENTS | CF_SCHEMA_CHANGE;
  sql_command_flags[SQLCOM_CREATE_SEQUENCE] =
      (CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_AUTO_COMMIT_TRANS | CF_SCHEMA_CHANGE);
  sql_command_flags[SQLCOM_CREATE_INDEX] =
      CF_CHANGES_DATA | CF_AUTO_COMMIT_TRANS | CF_ADMIN_COMMAND | CF_REPORT_PROGRESS;
  sql_command_flags[SQLCOM_ALTER_TABLE] = CF_CHANGES_DATA | CF_WRITE_LOGS_COMMAND | CF_AUTO_COMMIT_TRANS |
                                          CF_REPORT_PROGRESS | CF_INSERTS_DATA | CF_ADMIN_COMMAND;
  sql_command_flags[SQLCOM_ALTER_SEQUENCE] =
      CF_CHANGES_DATA | CF_WRITE_LOGS_COMMAND | CF_AUTO_COMMIT_TRANS | CF_SCHEMA_CHANGE | CF_ADMIN_COMMAND;
  ...
  sql_command_flags[SQLCOM_LOAD] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS | CF_REPORT_PROGRESS | CF_INSERTS_DATA;
  ...
  sql_command_flags[SQLCOM_UPDATE] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                     CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_UPDATES_DATA |
                                     CF_PS_ARRAY_BINDING_SAFE;
  sql_command_flags[SQLCOM_UPDATE_MULTI] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                           CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_UPDATES_DATA |
                                           CF_PS_ARRAY_BINDING_SAFE;
  sql_command_flags[SQLCOM_INSERT] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                     CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_INSERTS_DATA |
                                     CF_PS_ARRAY_BINDING_SAFE | CF_PS_ARRAY_BINDING_OPTIMIZED;
  sql_command_flags[SQLCOM_INSERT_SELECT] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                            CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_INSERTS_DATA;
  sql_command_flags[SQLCOM_DELETE] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                     CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_DELETES_DATA |
                                     CF_PS_ARRAY_BINDING_SAFE;
  sql_command_flags[SQLCOM_DELETE_MULTI] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                           CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_DELETES_DATA;
  sql_command_flags[SQLCOM_REPLACE] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_INSERTS_DATA |
                                      CF_PS_ARRAY_BINDING_SAFE | CF_PS_ARRAY_BINDING_OPTIMIZED;
  sql_command_flags[SQLCOM_REPLACE_SELECT] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
                                             CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_INSERTS_DATA;
  sql_command_flags[SQLCOM_SELECT] =
      CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS | CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED;
  ...
  */
  sql_command_flags[SQLCOM_SET_OPTION] =
      CF_REEXECUTION_FRAGILE | CF_AUTO_COMMIT_TRANS | CF_CAN_GENERATE_ROW_EVENTS | CF_OPTIMIZER_TRACE;  // (1)
  ...
  */
  sql_command_flags[SQLCOM_REPAIR] =
      CF_WRITE_LOGS_COMMAND | CF_AUTO_COMMIT_TRANS | CF_REPORT_PROGRESS | CF_ADMIN_COMMAND;
...
int bootstrap(MYSQL_FILE *file)
{
  ...
  thd->security_ctx->user = (char *)my_strdup(key_memory_MPVIO_EXT_auth_info, "boot", MYF(MY_WME));
  thd->security_ctx->priv_user[0] = thd->security_ctx->priv_host[0] = thd->security_ctx->priv_role[0] = 0;
  /*
    ...
    */
    if (rc != READ_BOOTSTRAP_SUCCESS)
    {
      ...
      */
      ...
      char *err_ptr = buffer + (length <= MAX_BOOTSTRAP_ERROR_LEN ? 0 : (length - MAX_BOOTSTRAP_ERROR_LEN));
      switch (rc)
      {
        ...
        case READ_BOOTSTRAP_QUERY_SIZE:
          my_printf_error(ER_UNKNOWN_ERROR,
                          ...
                          MYF(0), MAX_BOOTSTRAP_QUERY_SIZE, err_ptr);
          break;
    ...
    query = (char *)thd->memdup_w_gap(buffer, length + 1,
                                      thd->db.length + 1 + QUERY_CACHE_DB_LENGTH_SIZE + QUERY_CACHE_FLAGS_SIZE);
    size_t db_len = 0;
...
{
  return (command == COM_STMT_FETCH || command == COM_STMT_SEND_LONG_DATA || command == COM_STMT_CLOSE);
}
#endif /* WITH_WSREP */
#ifndef EMBEDDED_LIBRARY
static enum enum_server_command fetch_command(THD *thd, char *packet)
{
  enum enum_server_command command = (enum enum_server_command)(uchar)packet[0];
  DBUG_ENTER("fetch_command");

  if (command >= COM_END || (command >= COM_MDB_GAP_BEG && command <= COM_MDB_GAP_END))
    command = COM_END;  // Wrong command

  DBUG_PRINT("info", ("Command on %s = %d (%s)", vio_description(thd->net.vio), command, command_name[command].str));
  DBUG_RETURN(command);
}
...
dispatch_command_return do_command(THD *thd, bool blocking)
{
  ...
  */
  if (unlikely(wsrep_service_started) && wsrep_before_command(thd, wsrep_command_no_result(command)))
  {
    /*
    ...
    */
    ...
    if (command != COM_STMT_CLOSE && command != COM_STMT_EXECUTE && command != COM_QUIT)
    {
  ...
  if (WSREP(thd))
  {
     ...
     */
    if (!(thd->wsrep_applier) && (!wsrep_ready_get() || wsrep_reject_queries != WSREP_REJECT_NONE) &&
        (server_command_flags[command] & CF_SKIP_WSREP_CHECK) == 0)
    {
      my_message(ER_UNKNOWN_COM_ERROR, "WSREP has not yet prepared node for application use", MYF(0));
      thd->protocol->end_statement();
...
resume:
  return_value = dispatch_command(command, thd, packet + 1, (uint)(packet_length - 1), blocking);
...
static void wsrep_copy_query(THD *thd)
{
  ...
  thd->wsrep_retry_query = (char *)my_malloc(PSI_INSTRUMENT_ME, thd->wsrep_retry_query_len + 1, MYF(0));
...
*/
dispatch_command_return dispatch_command(enum enum_server_command command, THD *thd, char *packet, uint packet_length,
                                         bool blocking)
{
  NET *net = &thd->net;
  bool error = 0;
  ...
  DBUG_PRINT("info", ("command: %d %s", command, (command_name[command].str != 0 ? command_name[command].str : "<?>")));
  ...
  */
  ...
  if (unlikely(thd->security_ctx->password_expired && command != COM_QUERY && command != COM_PING &&
               command != COM_QUIT && command != COM_STMT_PREPARE && command != COM_STMT_EXECUTE &&
  ...
  switch (command)
  {
    case COM_INIT_DB:
    {
      ...
      if (unlikely(
              thd->copy_with_error(system_charset_info, (LEX_STRING *)&tmp, thd->charset(), packet, packet_length)))
        break;
      ...
      */
      ...
      if (auth_rc)
      {
        ...
        *thd->security_ctx = save_security_ctx;
        ...
        thd->update_charset(save_character_set_client, save_collation_connection, save_character_set_results);
    ...
    case COM_QUERY:
    {
      ...
      MYSQL_QUERY_START(thd->query(), thd->thread_id, thd->get_db(), &thd->security_ctx->priv_user[0],
      ...
      if (WSREP(thd))
      {
        if (wsrep_mysql_parse(thd, thd->query(), thd->query_length(), &parser_state))
        {
          WSREP_DEBUG("Deadlock error for: %s", thd->query());
      ...
      while (!thd->killed && (parser_state.m_lip.found_semicolon != NULL) && !thd->is_error())
      {
        ...
        */
        ...
        /* DTRACE begin */
        MYSQL_QUERY_START(beginning_of_next_stmt, thd->thread_id, thd->get_db(), &thd->security_ctx->priv_user[0],
                          (char *)thd->security_ctx->host_or_ip);

        /* PSI begin */
        thd->m_digest = &thd->m_digest_state;

        thd->m_statement_psi = MYSQL_START_STATEMENT(&thd->m_statement_state, com_statement_info[command].m_key,
    ...
    case COM_FIELD_LIST:  // This isn't actually needed
      ...
      my_message(ER_NOT_ALLOWED_COMMAND, ER_THD(thd, ER_NOT_ALLOWED_COMMAND), MYF(0)); /* purecov: inspected */
      break;
#else
    {
      ...
      */
      ...
      thd->convert_string(&table_name, system_charset_info, packet, arg_length, thd->charset());
      ...
      */
      ...
      thd->lex->first_select_lex()->table_list.link_in_list(&table_list, &table_list.next_local);
      ...
      general_log_print(thd, command, "%s %s", table_list.table_name.str, fields);
      ...
      */
      ...
      if (debug_simulate)
      {
        ...
        */
        bool res;
        set_current_thd(0);
        res = reload_acl_and_cache(NULL, options | REFRESH_FAST, NULL, &not_used);
        set_current_thd(thd);
        if (res)
          break;
      }
    ...
    case COM_STATISTICS:
    {
      ...
      if (!(current_global_status_var = (STATUS_VAR *)thd->alloc(sizeof(STATUS_VAR))))
        break;
      general_log_print(thd, command, NullS);
      ...
      *current_global_status_var = global_status_var;
      ...
      size_t length =
          ...
          my_snprintf(buff, buff_len - 1,
                      ...
                      uptime, THD_count::value(), (ulong)thd->query_id, current_global_status_var->long_query_count,
                      current_global_status_var->opened_tables, tc_records(), (uint)(queries_per_second1000 / 1000),
    ...
    case COM_PROCESS_INFO:
      ...
      if (!thd->security_ctx->priv_user[0] && check_global_access(thd, PRIV_COM_PROCESS_INFO))
        break;
      general_log_print(thd, command, NullS);
      mysqld_list_processes(
          thd, thd->security_ctx->master_access & PRIV_COM_PROCESS_INFO ? NullS : thd->security_ctx->priv_user, 0);
      break;
...
resume:
  ...
  */
  if (unlikely(wsrep_service_started))
  {
    ...
    if (wsrep_current_error(thd) && !wsrep_command_no_result(command))
    {
      ...
      wsrep_override_error(thd, wsrep_current_error(thd), wsrep_current_error_status(thd));
  ...
  if (do_end_of_statement)
  {
    DBUG_ASSERT(thd->derived_tables == NULL &&
                (thd->open_tables == NULL || (thd->locked_tables_mode == LTM_LOCK_TABLES)));
  ...
  mysql_audit_general(thd, MYSQL_AUDIT_GENERAL_STATUS,
                      thd->get_stmt_da()->is_error() ? thd->get_stmt_da()->sql_errno() : 0, command_name[command].str);
  ...
  */
  if (command == COM_QUERY && thd->lex->sql_command != SQLCOM_EXECUTE)
    log_slow_statement(thd);
  else
    delete_explain_query(thd->lex);
...
void log_slow_statement(THD *thd)
{
  ...
  */
  ...
  DBUG_EXECUTE_IF("simulate_slow_query", {
    if (thd->get_command() == COM_QUERY || thd->get_command() == COM_STMT_EXECUTE)
      thd->server_status |= SERVER_QUERY_WAS_SLOW;
  });

  if ((thd->server_status & (SERVER_QUERY_NO_INDEX_USED | SERVER_QUERY_NO_GOOD_INDEX_USED)) &&
      !(thd->query_plan_flags & QPLAN_STATUS) && !slow_filter_masked(thd, QPLAN_NOT_USING_INDEX))
  {
      ...
      thd->get_examined_row_count() >= thd->variables.min_examined_row_limit)
  {
    ...
    */
    if ((thd->query_plan_flags & QPLAN_ADMIN) && (thd->variables.log_slow_disabled_statements & LOG_SLOW_DISABLE_ADMIN))
      goto end;
    ...
    */
    if (thd->variables.log_slow_rate_limit > 1 && (global_query_id % thd->variables.log_slow_rate_limit) != 0)
      goto end;
    ...
    */
    ...
    slow_log_print(thd, thd->query(), thd->query_length(), thd->utime_after_query);
  }
...
int prepare_schema_table(THD *thd, LEX *lex, Table_ident *table_ident, enum enum_schema_tables schema_table_idx)
{
  ...
  switch (schema_table_idx)
  {
...
#else
    {
      if (lex->first_select_lex()->db.str == NULL && lex->copy_db_to(&lex->first_select_lex()->db))
      {
        DBUG_RETURN(1);
      }
      ...
      if (lower_case_table_names == 1)
        lex->first_select_lex()->db.str = thd->strdup(lex->first_select_lex()->db.str);
...
#else
    {
      ...
      if (!schema_select_lex->add_table_to_list(thd, table_ident, 0, 0, TL_READ, MDL_SHARED_READ))
        DBUG_RETURN(1);
  ...
  /* We must allocate some extra memory for query cache
    ...
    The query buffer layout is:
       buffer :==
            ...
            <flags>       Flags struct
  */
  if (!(query = (char *)thd->memdup_w_gap(packet, packet_length,
                                          1 + thd->db.length + QUERY_CACHE_DB_LENGTH_SIZE + QUERY_CACHE_FLAGS_SIZE)))
    return TRUE;
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    bool curuserhost =
        curuser && d->host.str && !my_strcasecmp(system_charset_info, d->host.str, thd->security_ctx->priv_host);
    if (!curuserhost && !currole && check_global_access(thd, PRIV_DEFINER_CLAUSE, false))
      DBUG_RETURN(TRUE);
  }
...
static bool __attribute__((noinline)) lock_tables_open_and_lock_tables(THD *thd, TABLE_LIST *tables)
{
...
retry:
  ...
  for (table = tables; table; table = table->next_global)
  {
    if (!table->placeholder())
    {
      ...
      else if (table->mdl_request.type == MDL_SHARED_READ && !table->prelocking_placeholder &&
               table->table->file->lock_count() == 0)
      {
        ...
        */
        ...
        lock_type =
            table->table->mdl_ticket->get_type() == MDL_SHARED_WRITE ? MDL_SHARED_NO_READ_WRITE : MDL_SHARED_READ_ONLY;

        bool result =
            thd->mdl_context.upgrade_shared_lock(table->table->mdl_ticket, lock_type, thd->variables.lock_wait_timeout);
    ...
    /*
       ...
       to TL_READ
     */
    if (table->belong_to_view && check_single_table_access(thd, PRIV_LOCK_TABLES, table, 1))
    {
      ...
      else
      {
        ...
        if (error)
        {
          my_error(ER_VIEW_INVALID, MYF(0), table->belong_to_view->view_db.str, table->belong_to_view->view_name.str);
          goto err;
        }
      }
    }
  ...
  if (lock_tables(thd, tables, counter, 0) || thd->locked_tables_list.init_locked_tables(thd))
    goto err;
...
static int __attribute__((noinline)) mysql_create_routine(THD *thd, LEX *lex)
{
  ...
  */
  ...
  if (check_access(thd, CREATE_PROC_ACL, lex->sphead->m_db.str, NULL, NULL, 0, 0))
    return true;

  /* Checking the drop permissions if CREATE OR REPLACE is used */
  if (lex->create_info.or_replace())
  {
    if (check_routine_access(thd, ALTER_PROC_ACL, &lex->sphead->m_db, &lex->sphead->m_name,
  ...
  if (!lex->sphead->m_handler->sp_create_routine(thd, lex->sphead))
  {
    ...
    */
    ...
    {
      security_context.change_security_context(thd, &thd->lex->definer->user, &thd->lex->definer->host,
                                               &thd->lex->sphead->m_db, &backup);
      restore_backup_context = true;
    }

    if (sp_automatic_privileges && !opt_noacl &&
        check_routine_access(thd, DEFAULT_CREATE_PROC_ACLS, &lex->sphead->m_db, name,
                             Sp_handler::handler(lex->sql_command), 1))
    {
      if (sp_grant_privileges(thd, lex->sphead->m_db.str, name->str, Sp_handler::handler(lex->sql_command)))
        push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_PROC_AUTO_GRANT_FAIL,
...
*/
static bool prepare_db_action(THD *thd, privilege_t want_access, LEX_CSTRING *dbname)
{
  ...
  */
  ...
  if (thd->slave_thread)
  {
    ...
    if (!rpl_filter->db_ok(dbname->str) || !rpl_filter->db_ok_with_wild_table(dbname->str))
    {
...
bool Sql_cmd_call::execute(THD *thd)
{
  ...
  */
  if (!(sp = m_handler->sp_find_routine(thd, m_name, true)))
  {
    ...
    */
    if (check_routine_access(thd, EXECUTE_ACL, &m_name->m_db, &m_name->m_name, &sp_handler_procedure, false))
      return true;
    /*
    ...
    */
    if (!sp_cache_lookup(&thd->sp_proc_cache, m_name))
      my_error(ER_SP_DOES_NOT_EXIST, MYF(0), "PROCEDURE", ErrConvDQName(m_name).ptr());
    return true;
  }
  else
  {
...
bool run_set_statement_if_requested(THD *thd, LEX *lex)
{
  if (!lex->stmt_var_list.is_empty() && !thd->slave_thread)
  {
      ...
      else
      {
        switch (v->var->option.var_type & GET_TYPE_MASK)
        {
          ...
          case GET_LL:
          {
            ...
            o = new set_var(
                thd, v->type, v->var, &v->base,
                (null_value ? (Item *)new (mem_root) Item_null(thd) : (Item *)new (mem_root) Item_int(thd, val)));
          }
          break;
          case GET_UINT:
          case GET_ULONG:
          case GET_ULL:
          {
            ...
            o = new set_var(
                thd, v->type, v->var, &v->base,
                (null_value ? (Item *)new (mem_root) Item_null(thd) : (Item *)new (mem_root) Item_uint(thd, val)));
          }
          break;
          case GET_DOUBLE:
          {
            ...
            o = new set_var(
                thd, v->type, v->var, &v->base,
                (null_value ? (Item *)new (mem_root) Item_null(thd) : (Item *)new (mem_root) Item_float(thd, val, 1)));
          }
          break;
          default:
          case GET_NO_ARG:
          ...
          case GET_STR_ALLOC:
          {
            ...
            if (val)
            {
              Item_string *str = new (mem_root) Item_string(thd, v->var->charset(thd), val->ptr(), val->length());
              o = new set_var(thd, v->type, v->var, &v->base, str);
            }
            else
              o = new set_var(thd, v->type, v->var, &v->base, new (mem_root) Item_null(thd));
          }
          break;
        }
      }
    ...
    if (thd->is_error() || sql_set_variables(thd, &lex->stmt_var_list, false))
    {
      if (!thd->is_error())
...
int mysql_execute_command(THD *thd, bool is_called_from_prepared_stmt)
{
  ...
  DBUG_ASSERT(!(sql_command_flags[lex->sql_command] & CF_INSERTS_DATA) || first_table->for_insert_data);

  if (thd->security_ctx->password_expired && lex->sql_command != SQLCOM_SET_OPTION &&
  ...
  */
  ...
  if (unlikely(thd->slave_thread))
  {
    if (lex->sql_command == SQLCOM_DROP_TRIGGER)
    {
      ...
      */
      ...
      if (!all_tables)
      {
        ...
        */
        if (!thd->lex->create_info.if_exists() && !(thd->variables.option_bits & OPTION_IF_EXISTS))
          DBUG_RETURN(0);
        /*
    ...
    */
    if (!(lex->sql_command == SQLCOM_UPDATE_MULTI) && !(lex->sql_command == SQLCOM_SET_OPTION) &&
        !((lex->sql_command == SQLCOM_DROP_TABLE || lex->sql_command == SQLCOM_DROP_SEQUENCE) && lex->tmp_table() &&
          lex->if_exists()) &&
  ...
  thd->get_binlog_format(&orig_binlog_format, &orig_current_stmt_binlog_format);
#ifdef WITH_WSREP
  if (WSREP(thd))
  {
    /*
    ...
    */
    ...
    if (lex->sql_command == SQLCOM_UNLOCK_TABLES && thd->wsrep_converted_lock_session)
    {
     ...
     */
    if (!(thd->wsrep_applier) && !(wsrep_ready_get() && wsrep_reject_queries == WSREP_REJECT_NONE) &&
        !(thd->variables.wsrep_dirty_reads && (sql_command_flags[lex->sql_command] & CF_CHANGES_DATA) == 0) &&
        !wsrep_tables_accessible_when_detached(all_tables) && lex->sql_command != SQLCOM_SET_OPTION &&
  ...
  thd->progress.report_to_client = MY_TEST(sql_command_flags[lex->sql_command] & CF_REPORT_PROGRESS);
  ...
  */
  ...
  ots.init(thd, all_tables, lex->sql_command, &lex->var_list, thd->query(), thd->query_length(),
  ...
  */
  if (!(sql_command_flags[lex->sql_command] &
        (CF_CAN_GENERATE_ROW_EVENTS | CF_FORCE_ORIGINAL_BINLOG_FORMAT | CF_STATUS_COMMAND)))
  ...
  if (stmt_causes_implicit_commit(thd, CF_IMPLICIT_COMMIT_BEGIN))
  {
    ...
    */
    ...
    if (!(thd->variables.option_bits & OPTION_GTID_BEGIN))
    {
      ...
      if (commit_failed)
      {
        WSREP_DEBUG("implicit commit failed, MDL released: %lld", (longlong)thd->thread_id);
        goto error;
      }
    }
    ...
    if (WSREP_NNULL(thd) && wsrep_thd_is_local(thd) && wsrep_after_statement(thd))
    {
      goto error;
    }
#endif /* WITH_WSREP */
  ...
  */
  if (thd->tx_read_only && (sql_command_flags[lex->sql_command] & CF_DISALLOW_IN_RO_TRANS))
  {
  ...
  */
  ...
  if (WSREP_NNULL(thd) && wsrep_thd_is_local(thd) && !wsrep_check_mode_before_cmd_execute(thd))
    goto error;
  ...
  */
  if (WSREP_NNULL(thd) && wsrep_thd_is_local(thd) && lex->sql_command != SQLCOM_BEGIN &&
      lex->sql_command != SQLCOM_CALL && lex->sql_command != SQLCOM_EXECUTE &&
  ...
  switch (lex->sql_command)
  {
    ...
    {
      if (!thd->security_ctx->priv_user[0] && check_global_access(thd, PRIV_STMT_SHOW_EXPLAIN))
        break;
      ...
      */
      ...
      {
        my_message(ER_SET_CONSTANTS_ONLY, ER_THD(thd, ER_SET_CONSTANTS_ONLY), MYF(0));
        goto error;
      }

      Item **it = lex->value_list.head_ref();
      if (!(*it)->basic_const_item() || (*it)->fix_fields_if_needed_for_scalar(lex->thd, it))
      {
        my_message(ER_SET_CONSTANTS_ONLY, ER_THD(thd, ER_SET_CONSTANTS_ONLY), MYF(0));
        goto error;
      }
    ...
    case SQLCOM_SELECT:
    {
      ...
      */
      privilege_t privileges_requested = lex->exchange ? SELECT_ACL | FILE_ACL : SELECT_ACL;
    ...
    case SQLCOM_SHOW_WARNS:
    {
      res = mysqld_show_warnings(
          thd, (ulong)((1L << (uint)Sql_condition::WARN_LEVEL_NOTE) | (1L << (uint)Sql_condition::WARN_LEVEL_WARN) |
                       (1L << (uint)Sql_condition::WARN_LEVEL_ERROR)));
      break;
    }
    case SQLCOM_SHOW_ERRORS:
    {
      res = mysqld_show_warnings(thd, (ulong)(1L << (uint)Sql_condition::WARN_LEVEL_ERROR));
      break;
    }
    ...
    {
      DBUG_ASSERT(first_table == all_tables && first_table != 0);
      if (check_access(thd, INDEX_ACL, first_table->db.str, &first_table->grant.privilege,
    ...
    case SQLCOM_CHANGE_MASTER:
    {
      ...
      */
      ...
      mi = master_info_index->get_master_info(&lex_mi->connection_name, Sql_condition::WARN_LEVEL_NOTE);
      ...
      {
        mi->rpl_filter = get_or_create_rpl_filter(lex_mi->connection_name.str, lex_mi->connection_name.length);
      }
      ...
      */
      {
        ...
        res = mysql_alter_table(thd, &first_table->db, &first_table->table_name, &create_info, first_table, &alter_info,
    ...
    case SQLCOM_SLAVE_START:
    {
      ...
      */
      if (unlikely((mi = get_master_info(&lex_mi->connection_name, Sql_condition::WARN_LEVEL_ERROR))))
      {
        if (load_error)
        {
          /*
    ...
    case SQLCOM_SLAVE_STOP:
    {
      ...
      */
      if (thd->locked_tables_mode || thd->in_active_multi_stmt_transaction() || thd->global_read_lock.is_acquired())
      {
        my_message(ER_LOCK_OR_ACTIVE_TRANSACTION, ER_THD(thd, ER_LOCK_OR_ACTIVE_TRANSACTION), MYF(0));
        goto error;
      }
    ...
    case SQLCOM_SHOW_CREATE:
    {
      ...
      DBUG_PRINT("debug", ("lex->only_view: %d, table: %s.%s", lex->table_type == TABLE_TYPE_VIEW, first_table->db.str,
    ...
    case SQLCOM_UPDATE:
    {
      ...
      */
      ...
      res = up_result = mysql_update(thd, all_tables, select_lex->item_list, lex->value_list, select_lex->where,
                                     select_lex->order_list.elements, select_lex->order_list.first,
                                     unit->lim.get_select_limit(), lex->ignore, &found, &updated);
    ...
    case SQLCOM_UPDATE_MULTI:
    {
      ...
      */
      ...
      else
      {
        ...
        if (res)
          break;
        if (opt_readonly && !(thd->security_ctx->master_access & PRIV_IGNORE_READ_ONLY) &&
...
#endif
      {
        ...
        res = mysql_multi_update(thd, all_tables, &select_lex->item_list, &lex->value_list, select_lex->where,
                                 select_lex->options, lex->duplicates, lex->ignore, unit, select_lex, &result_obj);
        if (result_obj)
        {
          MYSQL_MULTI_UPDATE_DONE(res, result_obj->num_found(), result_obj->num_updated());
    ...
    case SQLCOM_INSERT:
    {
      ...
      */
      if (first_table->view && !first_table->contain_auto_increment)
        thd->first_successful_insert_id_in_cur_stmt = thd->first_successful_insert_id_in_prev_stmt;
      ...
      DBUG_EXECUTE_IF("after_mysql_insert", {
        ...
        DBUG_ASSERT(!debug_sync_set_action(thd, STRING_WITH_LEN(act1)));
    ...
    case SQLCOM_INSERT_SELECT:
    {
      ...
      /*
        ...
        deterministic. Waiting for BUG#42415
      */
      if (lex->sql_command == SQLCOM_INSERT_SELECT && lex->duplicates == DUP_UPDATE)
      ...
      if (!(res = open_and_lock_tables(thd, all_tables, TRUE, 0)))
      {
        ...
        */
        ...
        if (!res && (sel_result = new (thd->mem_root)
                         select_insert(thd, first_table, first_table->table, &lex->field_list, &lex->update_list,
                                       &lex->value_list, lex->duplicates, lex->ignore, result)))
        {
          if (lex->analyze_stmt)
          ...
          /*
            ...
            the unlock procedure.
          */
          if (!res && first_table->lock_type == TL_WRITE_CONCURRENT_INSERT && thd->lock)
          {
    ...
    case SQLCOM_DELETE:
    {
      ...
      res = mysql_delete(thd, all_tables, select_lex->where, &select_lex->order_list, unit->lim.get_select_limit(),
                         select_lex->options, lex->result ? lex->result : sel_result);
    ...
    case SQLCOM_DELETE_MULTI:
    {
      ...
      {
        result = new (thd->mem_root) multi_delete(thd, aux_tables, lex->table_count_update);
        if (likely(result))
        {
          if (unlikely(select_lex->vers_setup_conds(thd, aux_tables)))
            goto multi_delete_error;
          res = mysql_select(thd, select_lex->get_table_list(), select_lex->item_list, select_lex->where, 0,
    ...
    case SQLCOM_DROP_TABLE:
    {
      ...
      */
      ...
      if (WSREP(thd))
      {
        for (TABLE_LIST *table = all_tables; table; table = table->next_global)
        {
          if (!lex->tmp_table() && (!thd->is_current_stmt_binlog_format_row() || !is_temporary_table(table)))
          {
      ...
      res = mysql_rm_table(thd, first_table, lex->if_exists(), lex->tmp_table(), lex->table_type == TABLE_TYPE_SEQUENCE,
                           0);
    ...
    case SQLCOM_SHOW_PROCESSLIST:
      if (!thd->security_ctx->priv_user[0] && check_global_access(thd, PRIV_STMT_SHOW_PROCESSLIST))
        break;
      mysqld_list_processes(
          thd, (thd->security_ctx->master_access & PRIV_STMT_SHOW_PROCESSLIST ? NullS : thd->security_ctx->priv_user),
          lex->verbose);
      break;
    ...
    {
      DBUG_ASSERT(first_table == all_tables && first_table != 0);
      privilege_t privilege = (lex->duplicates == DUP_REPLACE ? INSERT_ACL | DELETE_ACL : INSERT_ACL) |
      ...
      if (lex->local_file)
      {
        ...
        {
          my_message(ER_LOAD_INFILE_CAPABILITY_DISABLED, ER_THD(thd, ER_LOAD_INFILE_CAPABILITY_DISABLED), MYF(0));
          goto error;
        }
      }
    ...
    case SQLCOM_UNLOCK_TABLES:
      ...
      */
      ...
      if (thd->global_read_lock.is_acquired() && thd->current_backup_stage == BACKUP_FINISHED)
    ...
    {
      if (prepare_db_action(thd, lex->create_info.or_replace() ? (CREATE_ACL | DROP_ACL) : CREATE_ACL, &lex->name))
        break;

      if ((res =
               lex->create_info.resolve_to_charset_collation_context(thd, thd->charset_collation_context_create_db())))
        break;

      WSREP_TO_ISOLATION_BEGIN(lex->name.str, NULL, NULL);

      res = mysql_create_db(thd, &lex->name, lex->create_info, &lex->create_info);
      break;
    }
    case SQLCOM_DROP_DB:
    {
    ...
    case SQLCOM_ALTER_DB_UPGRADE:
    {
      ...
      {
        rpl_filter = thd->system_thread_info.rpl_sql_info->rpl_filter;
        if (!rpl_filter->db_ok(db->str) || !rpl_filter->db_ok_with_wild_table(db->str))
        {
          res = 1;
          my_message(ER_SLAVE_IGNORED_TABLE, ER_THD(thd, ER_SLAVE_IGNORED_TABLE), MYF(0));
          break;
    ...
    case SQLCOM_ALTER_DB:
    {
      ...
      if ((res = lex->create_info.resolve_to_charset_collation_context(
               thd, thd->charset_collation_context_alter_db(lex->name.str))))
        break;
    ...
    case SQLCOM_ALTER_EVENT:
      ...
      do
      {
        ...
        switch (lex->sql_command)
        {
          ...
          case SQLCOM_ALTER_EVENT:
            res = Events::update_event(thd, lex->event_parse_data, lex->spname ? &lex->spname->m_db : NULL,
    ...
    case SQLCOM_SHOW_CREATE_EVENT:
      WSREP_SYNC_WAIT(thd, WSREP_SYNC_WAIT_BEFORE_SHOW);
      res = Events::show_create_event(thd, &lex->spname->m_db, &lex->spname->m_name);
      break;
    case SQLCOM_DROP_EVENT:
      if (!(res = Events::drop_event(thd, &lex->spname->m_db, &lex->spname->m_name, lex->if_exists())))
        my_ok(thd);
      break;
#else
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "embedded server");
    ...
    {
      if (check_access(thd, lex->create_info.or_replace() ? (INSERT_ACL | DELETE_ACL) : INSERT_ACL, "mysql", NULL, NULL,
                       1, 0))
        break;
#ifdef HAVE_DLOPEN
      WSREP_TO_ISOLATION_BEGIN(WSREP_MYSQL_DB, NULL, NULL);
    ...
    case SQLCOM_CREATE_ROLE:
    {
      ...
      if (!(res = mysql_create_user(thd, lex->users_list, lex->sql_command == SQLCOM_CREATE_ROLE)))
        my_ok(thd);
      break;
    }
    ...
    {
      if (check_access(thd, DELETE_ACL, "mysql", NULL, NULL, 1, 1) && check_global_access(thd, CREATE_USER_ACL))
        break;

      WSREP_TO_ISOLATION_BEGIN(WSREP_MYSQL_DB, NULL, NULL);

      /* Conditionally writes to binlog */
      if (!(res = mysql_drop_user(thd, lex->users_list, lex->sql_command == SQLCOM_DROP_ROLE)))
        my_ok(thd);
      break;
    }
    ...
    {
      if (check_access(thd, UPDATE_ACL, "mysql", NULL, NULL, 1, 1) && check_global_access(thd, CREATE_USER_ACL))
        break;
    ...
    case SQLCOM_GRANT_ROLE:
    {
      ...
      if (!(res = mysql_grant_role(thd, lex->users_list, lex->sql_command != SQLCOM_GRANT_ROLE)))
        my_ok(thd);
      break;
    }
#endif /*!NO_EMBEDDED_ACCESS_CHECKS*/
    ...
    case SQLCOM_FLUSH:
    {
      ...
      if (first_table && lex->type & (REFRESH_READ_LOCK | REFRESH_FOR_EXPORT))
      {
        ...
        if (check_table_access(thd, PRIV_LOCK_TABLES, all_tables, FALSE, UINT_MAX, FALSE))
          goto error;
      ...
      if (lex->type &
           ...
           */
           REFRESH_RELAY_LOG | REFRESH_SLOW_LOG | REFRESH_GENERAL_LOG | REFRESH_ENGINE_LOG | REFRESH_ERROR_LOG |
#ifdef HAVE_QUERY_CACHE
           REFRESH_QUERY_CACHE_FREE |
#endif /* HAVE_QUERY_CACHE */
           REFRESH_STATUS | REFRESH_USER_RESOURCES))
      {
    ...
    case SQLCOM_KILL:
    {
      ...
      if (lex->kill_type == KILL_TYPE_ID || lex->kill_type == KILL_TYPE_QUERY)
      {
        ...
        sql_kill_user(thd, get_current_user(thd, lex->users_list.head()), lex->kill_signal);
      break;
    }
    ...
    case SQLCOM_HA_READ:
      ...
      */
      ...
      res = mysql_ha_read(thd, first_table, lex->ha_read_mode, lex->ident.str, lex->insert_list, lex->ha_rkey_mode,
                          select_lex->where, unit->lim.get_select_limit(), unit->lim.get_offset_limit());
      break;
    ...
    case SQLCOM_BEGIN:
      ...
      {
        thd->release_transactional_locks();
        WSREP_DEBUG("BEGIN failed, MDL released: %lld", (longlong)thd->thread_id);
    ...
    case SQLCOM_COMMIT:
    {
      DBUG_ASSERT(thd->lock == NULL || thd->locked_tables_mode == LTM_LOCK_TABLES);
      bool tx_chain = (lex->tx_chain == TVL_YES || (thd->variables.completion_type == 1 && lex->tx_chain != TVL_NO));
      bool tx_release =
          (lex->tx_release == TVL_YES || (thd->variables.completion_type == 2 && lex->tx_release != TVL_NO));
      bool commit_failed = trans_commit(thd);
      thd->release_transactional_locks();
      if (commit_failed)
      {
        WSREP_DEBUG("COMMIT failed, MDL released: %lld", (longlong)thd->thread_id);
        goto error;
      }
    ...
    case SQLCOM_ROLLBACK:
    {
      ...
      if (rollback_failed)
      {
        WSREP_DEBUG("rollback failed, MDL released: %lld", (longlong)thd->thread_id);
        goto error;
      }
      /* Begin transaction with the same isolation level. */
      if (tx_chain)
    ...
    case SQLCOM_COMPOUND:
    {
      ...
      sp->m_sp_share = MYSQL_GET_SP_SHARE(sp->m_handler->type(), sp->m_db.str, static_cast<uint>(sp->m_db.length),
    ...
    case SQLCOM_SHOW_PACKAGE_BODY_CODE:
    {
      ...
      const Sp_handler *sph = Sp_handler::handler(lex->sql_command);
      WSREP_SYNC_WAIT(thd, WSREP_SYNC_WAIT_BEFORE_SHOW);
      if (sph->sp_resolve_package_routine(thd, thd->lex->sphead, lex->spname, &sph, &pkgname))
        return true;
      ...
      if (!sp || sp->show_routine_code(thd))
      {
        ...
        my_error(ER_SP_DOES_NOT_EXIST, MYF(0), sph->type_str(), lex->spname->m_name.str);
        goto error;
      }
      break;
#else
      my_error(ER_FEATURE_DISABLED, MYF(0), "SHOW PROCEDURE|FUNCTION CODE", "--with-debug");
      goto error;
    ...
    case SQLCOM_XA_START:
      ...
      if (WSREP_ON)
      {
        my_error(ER_NOT_SUPPORTED_YET, MYF(0), "XA transactions with Galera replication");
        break;
      }
#endif /* WITH_WSREP */
    ...
    case SQLCOM_XA_COMMIT:
    {
      ...
      if (commit_failed)
      {
        WSREP_DEBUG("XA commit failed, MDL released: %lld", (longlong)thd->thread_id);
        goto error;
      }
      /*
    ...
    case SQLCOM_XA_ROLLBACK:
    {
      ...
      if (rollback_failed)
      {
        WSREP_DEBUG("XA rollback failed, MDL released: %lld", (longlong)thd->thread_id);
        goto error;
      }
      /*
    ...
    case SQLCOM_INSTALL_PLUGIN:
      if (!(res = mysql_install_plugin(thd, &thd->lex->comment, &thd->lex->ident)))
        my_ok(thd);
      break;
    case SQLCOM_UNINSTALL_PLUGIN:
      if (!(res = mysql_uninstall_plugin(thd, &thd->lex->comment, &thd->lex->ident)))
        my_ok(thd);
      break;
    ...
    case SQLCOM_ALTER_SERVER:
    {
      ...
      {
        DBUG_PRINT("info", ("problem altering server <%s>", lex->server_options.server_name.str));
    ...
    case SQLCOM_DROP_SERVER:
    {
      ...
      if ((err_code = drop_server(thd, &lex->server_options)))
      {
        ...
        {
          DBUG_PRINT("info", ("problem dropping server %s", lex->server_options.server_name.str));
    ...
    case SQLCOM_GRANT:
      ...
      DBUG_PRINT("result", ("res: %d  killed: %d  is_error(): %d", res, thd->killed, thd->is_error()));
      break;
    default:
...
finish:
  ...
  DBUG_ASSERT(!thd->in_active_multi_stmt_transaction() || thd->in_multi_stmt_transaction_mode());

  lex->unit.cleanup();

  /* close/reopen tables that were marked to need reopen under LOCK TABLES */
  if (unlikely(thd->locked_tables_list.some_table_marked_for_reopen) && !thd->lex->requires_prelocking())
  ...
  if (!thd->in_sub_stmt)
  {
    ...
    if (unlikely(thd->is_error()) || (thd->variables.option_bits & OPTION_MASTER_SQL_ERROR))
    {
  ...
  if (!(sql_command_flags[lex->sql_command] &
        (CF_CAN_GENERATE_ROW_EVENTS | CF_FORCE_ORIGINAL_BINLOG_FORMAT | CF_STATUS_COMMAND)))
    thd->set_binlog_format(orig_binlog_format, orig_current_stmt_binlog_format);
  ...
  */
  if (WSREP(thd) && !thd->in_sub_stmt && !thd->in_active_multi_stmt_transaction() &&
      thd->mdl_context.has_transactional_locks())
  {
    WSREP_DEBUG("Forcing release of transactional locks for thd: %lld", (longlong)thd->thread_id);
    thd->release_transactional_locks();
  }
  ...
  */
  if (!thd->in_active_multi_stmt_transaction() && !thd->in_sub_stmt && thd->wsrep_trx().active() &&
...
static bool execute_sqlcom_select(THD *thd, TABLE_LIST *all_tables)
{
  ...
  /* assign global limit variable if limit is not given */
  {
    ...
    if (!param->limit_params.explicit_limit)
      param->limit_params.select_limit = new (thd->mem_root) Item_int(thd, (ulonglong)thd->variables.select_limit);
  }
      ...
      */
      ...
      if (likely(!res))
      {
        ...
        */
        ...
        if (lex->explain_json)
        {
          lex->explain->print_explain_json(result, lex->analyze_stmt, false /* is_show_cmd */);
        }
        else
        {
          lex->explain->print_explain(result, thd->lex->describe, thd->lex->analyze_stmt);
...
static bool __attribute__((noinline)) execute_show_status(THD *thd, TABLE_LIST *all_tables)
{
  bool res;
  system_status_var old_status_var = thd->status_var;
  thd->initial_status_var = &old_status_var;
  WSREP_SYNC_WAIT(thd, WSREP_SYNC_WAIT_BEFORE_SHOW);
  if (!(res = check_table_access(thd, SELECT_ACL, all_tables, FALSE, UINT_MAX, FALSE)))
    res = execute_sqlcom_select(thd, all_tables);
  ...
  */
  ...
  memcpy(&thd->status_var, &old_status_var, offsetof(STATUS_VAR, last_cleared_system_status_var));
...
static TABLE *find_temporary_table_for_rename(THD *thd, TABLE_LIST *first_table, TABLE_LIST *cur_table)
{
  TABLE_LIST *table;
  TABLE *res = 0;
  bool found = 0;
  ...
  for (table = first_table; table != cur_table; table = table->next_local->next_local)
  {
...
static bool __attribute__((noinline)) check_rename_table(THD *thd, TABLE_LIST *first_table, TABLE_LIST *all_tables)
{
  DBUG_ASSERT(first_table == all_tables && first_table != 0);
  TABLE_LIST *table;
  for (table = first_table; table; table = table->next_local->next_local)
  {
    if (check_access(thd, ALTER_ACL | DROP_ACL, table->db.str, &table->grant.privilege, &table->grant.m_internal, 0,
                     0) ||
        check_access(thd, INSERT_ACL | CREATE_ACL, table->next_local->db.str, &table->next_local->grant.privilege,
    ...
    */
    ...
    if (check_grant(thd, ALTER_ACL | DROP_ACL, &old_list, FALSE, 1, FALSE) ||
        (!test_all_bits(table->next_local->grant.privilege, INSERT_ACL | CREATE_ACL) &&
         check_grant(thd, INSERT_ACL | CREATE_ACL, &new_list, FALSE, 1, FALSE)))
      return 1;
  }
...
static bool __attribute__((noinline)) generate_incident_event(THD *thd)
{
  if (mysql_bin_log.is_open())
  {
    ...
    DBUG_EXECUTE_IF("incident_database_resync_on_replace", incident = INCIDENT_LOST_EVENTS;);
    if (incident)
    {
...
#else
static bool generate_incident_event(THD *thd) { return 0; }
#endif

static int __attribute__((noinline)) show_create_db(THD *thd, LEX *lex)
{
  char db_name_buff[NAME_LEN + 1];
  LEX_CSTRING db_name;
  DBUG_EXECUTE_IF("4x_server_emul", my_error(ER_UNKNOWN_ERROR, MYF(0)); return 1;);
...
static bool __attribute__((noinline)) alter_routine(THD *thd, LEX *lex)
{
  int sp_result;
  const Sp_handler *sph = Sp_handler::handler(lex->sql_command);
  if (check_routine_access(thd, ALTER_PROC_ACL, &lex->spname->m_db, &lex->spname->m_name, sph, 0))
    return 1;
  /*
  ...
  */
  ...
  switch (sp_result)
  {
    ...
    case SP_KEY_NOT_FOUND:
      my_error(ER_SP_DOES_NOT_EXIST, MYF(0), sph->type_str(), ErrConvDQName(lex->spname).ptr());
      return 1;
    default:
      my_error(ER_SP_CANT_ALTER, MYF(0), sph->type_str(), ErrConvDQName(lex->spname).ptr());
      return 1;
  }
  return 0; /* purecov: deadcode */
}

static bool __attribute__((noinline)) drop_routine(THD *thd, LEX *lex)
{
  int sp_result;
#ifdef HAVE_DLOPEN
  if (lex->sql_command == SQLCOM_DROP_FUNCTION && !lex->spname->m_explicit_name)
  {
    ...
    if (!lex->spname->m_db.str)
    {
      if (lex->if_exists())
      {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_SP_DOES_NOT_EXIST,
                            ER_THD(thd, ER_SP_DOES_NOT_EXIST), "FUNCTION (UDF)", lex->spname->m_name.str);
        my_ok(thd);
        return 0;
      }
      my_error(ER_SP_DOES_NOT_EXIST, MYF(0), "FUNCTION (UDF)", lex->spname->m_name.str);
      return 1;
    }
  ...
  */
  ...
  if (sp_result != SP_KEY_NOT_FOUND && sp_automatic_privileges && !opt_noacl &&
      sp_revoke_privileges(thd, lex->spname->m_db.str, lex->spname->m_name.str, Sp_handler::handler(lex->sql_command)))
  {
    push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_PROC_AUTO_REVOKE_FAIL, ER_THD(thd, ER_PROC_AUTO_REVOKE_FAIL));
  ...
  switch (sp_result)
  {
    ...
    case SP_KEY_NOT_FOUND:
      ...
      if (lex->if_exists())
      {
        ...
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_SP_DOES_NOT_EXIST,
                            ER_THD(thd, ER_SP_DOES_NOT_EXIST), sph->type_str(), ErrConvDQName(lex->spname).ptr());
        if (res)
          return 1;
        my_ok(thd);
        return 0;
      ...
      return 1;
    default:
      my_error(ER_SP_DROP_FAILED, MYF(0), sph->type_str(), ErrConvDQName(lex->spname).ptr());
      return 1;
  }
...
bool check_access(THD *thd, privilege_t want_access, const char *db, privilege_t *save_priv,
                  GRANT_INTERNAL_INFO *grant_internal_info, bool dont_check_global_grants, bool no_errors)
{
  ...
  */
  ...
  DBUG_PRINT("enter", ("db: %s  want_access: %llx  master_access: %llx", db ? db : "", (longlong)want_access,
      ...
      switch (access->check(want_access, save_priv))
      {
        ...
        case ACL_INTERNAL_ACCESS_DENIED:
          if (!no_errors)
          {
            ...
            my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), sctx->priv_user, sctx->priv_host, db);
          }
          DBUG_RETURN(TRUE);
  ...
  if (unlikely(((want_access & ~sctx->master_access) & ~DB_ACLS) || (!db && dont_check_global_grants)))
  {  // We can never grant this
    DBUG_PRINT("error", ("No possible access"));
    if (!no_errors)
    {
      status_var_increment(thd->status_var.access_denied_errors);
      my_error(access_denied_error_code(thd->password), MYF(0), sctx->priv_user, sctx->priv_host,
               (thd->password ? ER_THD(thd, ER_YES) : ER_THD(thd, ER_NO))); /* purecov: tested */
    }
  ...
  if (db && (!thd->db.str || db_is_pattern || strcmp(db, thd->db.str)))
  {
    ...
    db_access = sctx->db_access;
  DBUG_PRINT("info", ("db_access: %llx  want_access: %llx", (longlong)db_access, (longlong)want_access));
  ...
  */
  bool need_table_or_column_check = (want_access & (TABLE_ACLS | PROC_ACLS | db_access)) == want_access;
  ...
  */
  if ((db_access & want_access) == want_access || (!dont_check_global_grants && need_table_or_column_check))
  {
    /*
...
bool check_single_table_access(THD *thd, privilege_t privilege, TABLE_LIST *tables, bool no_errors)
{
  if (tables->derived)
    return 0;
  ...
  if (check_access(thd, privilege, db_name, &tables->grant.privilege, &tables->grant.m_internal, 0, no_errors))
    return 1;

  /* Show only 1 table for check_grant */
  if (!(tables->belong_to_view && (thd->lex->sql_command == SQLCOM_SHOW_FIELDS)) &&
...
bool check_one_table_access(THD *thd, privilege_t privilege, TABLE_LIST *all_tables)
{
  ...
  if ((subselects_tables = all_tables->next_global))
  {
    ...
    */
    ...
    if (subselects_tables && (check_table_access(thd, SELECT_ACL, subselects_tables, FALSE, UINT_MAX, FALSE)))
      return 1;
  }
  return 0;
}
...
static bool check_show_access(THD *thd, TABLE_LIST *table)
{
  ...
  */
  ...
  {
    case SCH_SCHEMATA:
      return (specialflag & SPECIAL_SKIP_SHOW_DB) && check_global_access(thd, SHOW_DB_ACL);
    ...
    case SCH_EVENTS:
    {
      ...
      if (check_access(thd, SELECT_ACL, dst_db_name, &thd->col_access, NULL, FALSE, FALSE))
        return TRUE;
    ...
    case SCH_STATISTICS:
    {
      ...
      */
      ...
      if (check_access(thd, SELECT_ACL, dst_table->db.str, &dst_table->grant.privilege, &dst_table->grant.m_internal,
                       FALSE, FALSE))
...
bool check_table_access(THD *thd, privilege_t requirements, TABLE_LIST *tables,
                        bool any_combination_of_privileges_will_do, uint number, bool no_errors)
{
  ...
  */
  for (; i < number && tables != first_not_own_table && tables; tables = tables->next_global, i++)
  {
    TABLE_LIST *const table_ref = tables->correspondent_table ? tables->correspondent_table : tables;
    ...
    DBUG_PRINT("info", ("derived: %d  view: %d", table_ref->derived != 0, table_ref->view != 0));
    ...
    if (table_ref->sequence)
    {
      ...
      */
      want_access = ((table_ref->lock_type >= TL_FIRST_WRITE) ? INSERT_ACL : SELECT_ACL);
    }

    if (check_access(thd, want_access, table_ref->get_db_name(), &table_ref->grant.privilege,
                     &table_ref->grant.m_internal, 0, no_errors))
      return 1;
  }
  return check_grant(thd, requirements, org_tables, any_combination_of_privileges_will_do, number, no_errors);
}

bool check_routine_access(THD *thd, privilege_t want_access, const LEX_CSTRING *db, const LEX_CSTRING *name,
                          const Sp_handler *sph, bool no_errors)
{
  ...
  */
  ...
  else if (check_access(thd, want_access, db->str, &tables->grant.privilege, &tables->grant.m_internal, 0, no_errors))
    return TRUE;
...
bool check_some_routine_access(THD *thd, const char *db, const char *name, const Sp_handler *sph)
{
  ...
  */
  if (thd->security_ctx->master_access & SHOW_PROC_ACLS)
    return FALSE;
  if (!check_access(thd, SHOW_PROC_ACLS, db, &save_priv, NULL, 0, 1) || (save_priv & SHOW_PROC_ACLS))
    return FALSE;
...
bool check_some_access(THD *thd, privilege_t want_access, TABLE_LIST *table)
{
  ...
  for (ulonglong bit = 1; bit < (ulonglong)want_access; bit <<= 1)
  {
    if (bit & want_access)
    {
      ...
      if (!check_access(thd, access, table->db.str, &table->grant.privilege, &table->grant.m_internal, 0, 1) &&
...
*/
bool check_fk_parent_table_access(THD *thd, HA_CREATE_INFO *create_info, Alter_info *alter_info, const char *create_db)
{
  Key *key;
  ...
  while ((key = key_iterator++))
  {
    if (key->type == Key::FOREIGN_KEY)
    {
      ...
      LEX_CSTRING db_name;
      LEX_CSTRING table_name = {fk_key->ref_table.str, fk_key->ref_table.length};
      ...
      {
        if (!(db_name.str = (char *)thd->memdup(fk_key->ref_db.str, fk_key->ref_db.length + 1)))
          return true;
      ...
      else
      {
        if (!thd->db.str)
        {
          ...
          if (!(db_name.str = (char *)thd->memdup(create_db, db_name.length + 1)))
            return true;
      ...
      {
        char *name;
        table_name.str = name = (char *)thd->memdup(fk_key->ref_table.str, fk_key->ref_table.length + 1);
      ...
      */
      if (check_some_access(thd, privileges, &parent_table) || parent_table.grant.want_privilege)
      {
        my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), "REFERENCES", thd->security_ctx->priv_user,
                 thd->security_ctx->host_or_ip, db_name.str, table_name.str);
        return true;
      }
    }
  }
    ...
    check_stack_overrun(THD *thd, long margin, uchar *buf __attribute__((unused)))
{
  ...
  if ((stack_used = available_stack_size(thd->thread_stack, &stack_used)) >= (long)(my_thread_stack_size - margin))
  {
...
bool my_yyoverflow(short **yyss, YYSTYPE **yyvs, size_t *yystacksize)
{
  ...
  *yystacksize = set_zone((int)(*yystacksize) * 2, MY_YACC_INIT, MY_YACC_MAX);
  if (!(state->yacc_yyvs = (uchar *)my_realloc(key_memory_bison_stack, state->yacc_yyvs, *yystacksize * sizeof(**yyvs),
                                               MYF(MY_ALLOW_ZERO_PTR | MY_FREE_ON_ERROR))) ||
      !(state->yacc_yyss = (uchar *)my_realloc(key_memory_bison_stack, state->yacc_yyss, *yystacksize * sizeof(**yyss),
                                               MYF(MY_ALLOW_ZERO_PTR | MY_FREE_ON_ERROR))))
    return 1;
  if (old_info)
  {
...
void THD::reset_for_next_command(bool do_clear_error)
{
  ...
  */
  DBUG_ASSERT(binlog_table_maps == 0 || locked_tables_mode == LTM_LOCK_TABLES);
  ...
  */
  ...
  main_lex.stmt_lex = &main_lex;
  main_lex.current_select_number = 0;
  /*
  ...
  */
  ...
  /*
    ...
    the cluster.
  */
  if (WSREP_NNULL(this) && wsrep_thd_is_local(this) && !slave_thread && wsrep_auto_increment_control)
  {
    variables.auto_increment_offset = global_system_variables.auto_increment_offset;
    variables.auto_increment_increment = global_system_variables.auto_increment_increment;
  }
#endif /* WITH_WSREP */
...
bool mysql_new_select(LEX *lex, bool move_down, SELECT_LEX *select_lex)
{
  THD *thd = lex->thd;
...
void mysql_init_multi_delete(LEX *lex)
{
  ...
  lex->unit.lim.clear();
  lex->first_select_lex()->table_list.save_and_clear(&lex->auxiliary_table_list);
  lex->query_tables = 0;
  lex->query_tables_last = &lex->query_tables;
}

#ifdef WITH_WSREP
static void wsrep_prepare_for_autocommit_retry(THD *thd, char *rawbuf, uint length, Parser_state *parser_state)
{
  thd->clear_error();
  close_thread_tables(thd);
  ...
  /* DTRACE begin */
  MYSQL_QUERY_START(rawbuf, thd->thread_id, thd->get_db(), &thd->security_ctx->priv_user[0],
                    (char *)thd->security_ctx->host_or_ip);

  /* Performance Schema Interface instrumentation, begin */
  thd->m_statement_psi = MYSQL_REFINE_STATEMENT(thd->m_statement_psi, com_statement_info[thd->get_command()].m_key);
  MYSQL_SET_STATEMENT_TEXT(thd->m_statement_psi, thd->query(), thd->query_length());
...
static bool wsrep_mysql_parse(THD *thd, char *rawbuf, uint length, Parser_state *parser_state)
{
  bool is_autocommit = !thd->in_multi_stmt_transaction_mode() && !thd->wsrep_applier;
  ...
  do
  {
     ...
     */
    if (((thd->get_stmt_da()->is_error() && thd->get_stmt_da()->sql_errno() == ER_QUERY_INTERRUPTED) ||
    ...
    if (wsrep_after_statement(thd) && is_autocommit && thd_is_connection_alive(thd))
    {
      thd->reset_for_next_command();
      thd->reset_kill_query();
...
#endif
        WSREP_DEBUG("wsrep retrying AC query: %lu  %s", thd->wsrep_retry_counter, wsrep_thd_query(thd));
      ...
      {
        WSREP_DEBUG("%s, thd: %llu is_AC: %d, retry: %lu - %lu SQL: %s", wsrep_thd_transaction_state_str(thd),
                    thd->thread_id, is_autocommit, thd->wsrep_retry_counter, thd->variables.wsrep_retry_autocommit,
                    wsrep_thd_query(thd));
        my_error(ER_LOCK_DEADLOCK, MYF(0));
        thd->reset_kill_query();
  ...
  if (thd->wsrep_retry_query)
  {
    WSREP_DEBUG(
        ...
        wsrep_thd_transaction_state_str(thd), thd->get_stmt_da()->is_sent(), thd->killed,
        thd->get_stmt_da()->is_error() ? thd->get_stmt_da()->sql_errno() : 0, thd->wsrep_retry_query);
    my_free(thd->wsrep_retry_query);
        ...
        if (likely(!thd->is_error()))
        {
          ...
          */
          if (found_semicolon && (ulong)(found_semicolon - thd->query()))
            thd->set_query(thd->query(), (uint32)(found_semicolon - thd->query() - 1), thd->charset());
          ...
          MYSQL_QUERY_EXEC_START(thd->query(), thd->thread_id, thd->get_db(), &thd->security_ctx->priv_user[0],
                                 (char *)thd->security_ctx->host_or_ip, 0);
...
bool mysql_test_parse_for_slave(THD *thd, char *rawbuf, uint length)
{
  ...
  if (likely(!(error = parser_state.init(thd, rawbuf, length))))
  {
    ...
    if (!parse_sql(thd, &parser_state, NULL, true) && all_tables_not_ok(thd, lex->first_select_lex()->table_list.first))
...
TABLE_LIST *st_select_lex::add_table_to_list(THD *thd, Table_ident *table, LEX_CSTRING *alias, ulong table_options,
                                             ...
                                             LEX_STRING *option)
{
  ...
  DBUG_PRINT("enter",
             ("Table '%s' (%p)  Select %p (%u)", (alias ? alias->str : table->table.str), table, this, select_number));
  ...
  if (unlikely(table->is_derived_table() == FALSE && table->db.str && !(table_options & TL_OPTION_TABLE_FUNCTION) &&
  ...
  if (!alias) /* Alias is case sensitive */
  {
    ...
    {
      my_message(ER_DERIVED_MUST_HAVE_ALIAS, ER_THD(thd, ER_DERIVED_MUST_HAVE_ALIAS), MYF(0));
      DBUG_RETURN(0);
    }
  ...
  if (lower_case_table_names)
  {
    if (table->table.length)
      table->table.length = my_casedn_str(files_charset_info, (char *)table->table.str);
  ...
  if (!ptr->derived && is_infoschema_db(&ptr->db))
  {
    if (ptr->updating &&
        ...
        lex->sql_command != SQLCOM_CHECK && lex->sql_command != SQLCOM_CHECKSUM)
    {
      my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), thd->security_ctx->priv_user, thd->security_ctx->priv_host,
 ...
 */
  ...
  if (lock_type != TL_IGNORE && !ptr->sequence)
  {
    ...
    {
      if (unlikely(!my_strcasecmp(table_alias_charset, alias_str.str, tables->alias.str) &&
                   (tables->db.str == any_db.str || ptr->db.str == any_db.str || !cmp(&ptr->db, &tables->db)) &&
                   !tables->sequence))
      {
  ...
  */
  ...
  {
    MDL_REQUEST_INIT(&ptr->mdl_request, MDL_key::TABLE, ptr->db.str, ptr->table_name.str, mdl_type, MDL_TRANSACTION);
  }
  DBUG_RETURN(ptr);
}
...
bool st_select_lex::init_nested_join(THD *thd)
{
  ...
  if (unlikely(!(ptr = (TABLE_LIST *)thd->calloc(ALIGN_SIZE(sizeof(TABLE_LIST)) + sizeof(NESTED_JOIN)))))
    DBUG_RETURN(1);
...
bool st_select_lex::add_cross_joined_table(TABLE_LIST *left_op, TABLE_LIST *right_op, bool straight_fl)
{
  DBUG_ENTER("add_cross_joined_table");
  THD *thd = parent_lex->thd;
  if (!(right_op->nested_join && (right_op->nested_join->nest_type & JOIN_OP_NEST)))
  {
    /*
  ...
  */
  if (unlikely(!(cj_nest = (TABLE_LIST *)thd->calloc(ALIGN_SIZE(sizeof(TABLE_LIST)) + sizeof(NESTED_JOIN)))))
    DBUG_RETURN(true);
  cj_nest->nested_join = ((NESTED_JOIN *)((uchar *)cj_nest + ALIGN_SIZE(sizeof(TABLE_LIST))));
  ...
  for (;;)
  {
    ...
    if (tbl->nested_join && tbl->nested_join->nest_type & JOIN_OP_NEST)
    {
...
bool st_select_lex::add_window_def(THD *thd, LEX_CSTRING *win_name, LEX_CSTRING *win_ref,
                                   SQL_I_List<ORDER> win_partition_list, SQL_I_List<ORDER> win_order_list,
                                   Window_frame *win_frame)
{
  SQL_I_List<ORDER> *win_part_list_ptr = new (thd->mem_root) SQL_I_List<ORDER>(win_partition_list);
  SQL_I_List<ORDER> *win_order_list_ptr = new (thd->mem_root) SQL_I_List<ORDER>(win_order_list);
  if (!(win_part_list_ptr && win_order_list_ptr))
    return true;
  Window_def *win_def =
      new (thd->mem_root) Window_def(win_name, win_ref, win_part_list_ptr, win_order_list_ptr, win_frame);
  group_list = thd->lex->save_group_list;
  order_list = thd->lex->save_order_list;
  if (parsing_place != SELECT_LIST)
  {
    fields_in_window_functions += win_part_list_ptr->elements + win_order_list_ptr->elements;
  }
  win_def->win_spec_number = window_specs.elements;
  return (win_def == NULL || window_specs.push_back(win_def));
}

bool st_select_lex::add_window_spec(THD *thd, LEX_CSTRING *win_ref, SQL_I_List<ORDER> win_partition_list,
                                    SQL_I_List<ORDER> win_order_list, Window_frame *win_frame)
{
  SQL_I_List<ORDER> *win_part_list_ptr = new (thd->mem_root) SQL_I_List<ORDER>(win_partition_list);
  SQL_I_List<ORDER> *win_order_list_ptr = new (thd->mem_root) SQL_I_List<ORDER>(win_order_list);
  if (!(win_part_list_ptr && win_order_list_ptr))
    return true;
  Window_spec *win_spec = new (thd->mem_root) Window_spec(win_ref, win_part_list_ptr, win_order_list_ptr, win_frame);
  group_list = thd->lex->save_group_list;
  order_list = thd->lex->save_order_list;
  if (parsing_place != SELECT_LIST)
  {
...
void st_select_lex::set_lock_for_tables(thr_lock_type lock_type, bool for_update, bool skip_locked_arg)
{
  DBUG_ENTER("set_lock_for_tables");
  DBUG_PRINT("enter", ("lock_type: %d  for_update: %d  skip_locked %d", lock_type, for_update, skip_locked));
  ...
  for (TABLE_LIST *tables = table_list.first; tables; tables = tables->next_local)
  {
    ...
    if (tables->db.length)
      tables->mdl_request.set_type((lock_type >= TL_FIRST_WRITE) ? MDL_SHARED_WRITE : MDL_SHARED_READ);
  }
  DBUG_VOID_RETURN;
}
...
uint kill_one_thread(THD *thd, longlong id, killed_state kill_signal, killed_type type)
{
  THD *tmp;
    ...
    */
...
#endif /* WITH_WSREP */
    {
      ...
      if (tmp->wsrep_aborter && tmp->wsrep_aborter != thd->thread_id)
      {
        ...
        WSREP_DEBUG("victim %lld has wsrep aborter: %lu, skipping awake()", id, tmp->wsrep_aborter);
        error = 0;
      }
      else
#endif /* WITH_WSREP */
      {
        WSREP_DEBUG(
            "kill_one_thread victim: %lld wsrep_aborter %lu"
            " by signal %d",
      ...
      error = (type == KILL_TYPE_QUERY ? ER_KILL_QUERY_DENIED_ERROR : ER_KILL_DENIED_ERROR);
...
static my_bool kill_threads_callback(THD *thd, kill_threads_callback_arg *arg)
{
  if (thd->security_ctx->user)
  {
    ...
    */
    ...
    {
      if (!(arg->thd->security_ctx->master_access & PRIV_KILL_OTHER_USER_PROCESS) &&
...
static uint kill_threads_for_user(THD *thd, LEX_USER *user, killed_state kill_signal, ha_rows *rows)
{
  ...
  *rows = 0;
  ...
  DBUG_PRINT("enter", ("user: %s  signal: %u", user->user.str, (uint)kill_signal));
...
static void __attribute__((noinline)) sql_kill_user(THD *thd, LEX_USER *user, killed_state state)
{
  uint error;
  ha_rows rows;
#ifdef WITH_WSREP
...
bool append_file_to_dir(THD *thd, const char **filename_ptr, const LEX_CSTRING *table_name)
{
  ...
  if (strlen(*filename_ptr) + table_name->length >= FN_REFLEN - 1 || !test_if_hard_path(*filename_ptr))
  {
  ...
  if (unlikely(!(ptr = (char *)thd->alloc((size_t)(end - buff) + table_name->length + 1))))
...
*/
Item *all_any_subquery_creator(THD *thd, Item *left_expr, chooser_compare_func_creator cmp, bool all,
                               SELECT_LEX *select_lex)
{
  ...
  if ((cmp == &comp_ne_creator) && all)  // <> ALL <=> NOT IN
    return new (thd->mem_root) Item_func_not(thd, new (thd->mem_root) Item_in_subselect(thd, left_expr, select_lex));

  Item_allany_subselect *it = new (thd->mem_root) Item_allany_subselect(thd, left_expr, cmp, select_lex, all);
  if (all) /* ALL */
...
bool multi_update_precheck(THD *thd, TABLE_LIST *tables)
{
  ...
  */
  for (table = tables; table; table = table->next_local)
  {
    ...
    if (table->derived)
      table->grant.privilege = SELECT_ACL;
    else if ((check_access(thd, UPDATE_ACL, table->db.str, &table->grant.privilege, &table->grant.m_internal, 0, 1) ||
              check_grant(thd, UPDATE_ACL, table, FALSE, 1, TRUE)) &&
             (check_access(thd, SELECT_ACL, table->db.str, &table->grant.privilege, &table->grant.m_internal, 0, 0) ||
  ...
  */
  if (lex->first_select_lex() != lex->all_selects_list)
  {
    ...
    for (table = tables; table; table = table->next_global)
    {
      ...
      {
        if (check_access(thd, SELECT_ACL, table->db.str, &table->grant.privilege, &table->grant.m_internal, 0, 0) ||
            check_grant(thd, SELECT_ACL, table, FALSE, 1, FALSE))
          DBUG_RETURN(TRUE);
      }
    }
...
bool multi_delete_precheck(THD *thd, TABLE_LIST *tables)
{
  ...
  */
  ...
  {
    my_message(ER_UPDATE_WITHOUT_KEY_IN_SAFE_MODE, ER_THD(thd, ER_UPDATE_WITHOUT_KEY_IN_SAFE_MODE), MYF(0));
    DBUG_RETURN(TRUE);
  }
  DBUG_RETURN(FALSE);
}
...
static TABLE_LIST *multi_delete_table_match(LEX *lex, TABLE_LIST *tbl, TABLE_LIST *tables)
{
  ...
  for (TABLE_LIST *elem = tables; elem; elem = elem->next_local)
  {
    ...
    if (tbl->is_fqtn && elem->is_fqtn)
      res = (my_strcasecmp(table_alias_charset, tbl->table_name.str, elem->table_name.str) || cmp(&tbl->db, &elem->db));
...
bool update_precheck(THD *thd, TABLE_LIST *tables)
{
  ...
  if (thd->lex->first_select_lex()->item_list.elements != thd->lex->value_list.elements)
  {
    my_message(ER_WRONG_VALUE_COUNT, ER_THD(thd, ER_WRONG_VALUE_COUNT), MYF(0));
    DBUG_RETURN(TRUE);
  }
...
bool insert_precheck(THD *thd, TABLE_LIST *tables)
{
  ...
  */
  privilege_t privilege = (INSERT_ACL | (lex->duplicates == DUP_REPLACE ? DELETE_ACL : NO_ACL) |
...
bool create_table_precheck(THD *thd, TABLE_LIST *tables, TABLE_LIST *create_table)
{
  ...
  */
  ...
  if (check_access(thd, want_priv, create_table->db.str, &create_table->grant.privilege,
  ...
  if (lex->create_info.merge_list)
  {
    ...
    */
    ...
    if (check_table_access(thd, SELECT_ACL | UPDATE_ACL | DELETE_ACL, lex->create_info.merge_list, FALSE, UINT_MAX,
                           FALSE))
      goto err;
  }

  if (want_priv != CREATE_TMP_ACL && check_grant(thd, want_priv, create_table, FALSE, 1, FALSE))
    goto err;

  if (select_lex->item_list.elements)
  {
    /* Check permissions for used tables in CREATE TABLE ... SELECT */
    if (tables && check_table_access(thd, SELECT_ACL, tables, FALSE, UINT_MAX, FALSE))
      goto err;
  }
  ...
  if (check_fk_parent_table_access(thd, &lex->create_info, &lex->alter_info, create_table->db.str))
    goto err;
...
static bool lock_tables_precheck(THD *thd, TABLE_LIST *tables)
{
  ...
  for (TABLE_LIST *table = tables; table != first_not_own_table && table; table = table->next_global)
  {
    if (is_temporary_table(table))
      continue;

    if (check_table_access(thd, PRIV_LOCK_TABLES, table, FALSE, 1, FALSE))
      return TRUE;
  }
...
bool check_string_byte_length(const LEX_CSTRING *str, uint err_msg, size_t max_byte_length)
{
  if (str->length <= max_byte_length)
    return FALSE;

  my_error(ER_WRONG_STRING_LENGTH, MYF(0), str->str, err_msg ? ER(err_msg) : "", max_byte_length);
...
bool check_string_char_length(const LEX_CSTRING *str, uint err_msg, size_t max_char_length, CHARSET_INFO *cs,
                              bool no_error)
{
  Well_formed_prefix prefix(cs, str->str, str->length, max_char_length);
  if (likely(!prefix.well_formed_error_pos() && str->length == prefix.length()))
    return FALSE;

  if (!no_error)
  {
    ErrConvString err(str->str, str->length, cs);
    my_error(ER_WRONG_STRING_LENGTH, MYF(0), err.ptr(), err_msg ? ER(err_msg) : "", max_char_length);
  }
  return TRUE;
}
...
extern "C"
{
  int path_starts_from_data_home_dir(const char *path)
  {
    ...
    {
      if (dir_len > mysql_unpacked_real_data_home_len && path[mysql_unpacked_real_data_home_len] != FN_LIBCHAR)
        DBUG_RETURN(0);

      if (lower_case_file_system)
      {
        if (!default_charset_info->strnncoll(path, mysql_unpacked_real_data_home_len, mysql_unpacked_real_data_home,
      ...
      else if (!memcmp(path, mysql_unpacked_real_data_home, mysql_unpacked_real_data_home_len))
      {
        DBUG_PRINT("error", ("Path is part of mysql_real_data_home"));
        DBUG_RETURN(1);
      }
...
bool parse_sql(THD *thd, Parser_state *parser_state, Object_creation_ctx *creation_ctx, bool do_pfs_digest)
{
  ...
  */
  ...
  DBUG_ASSERT(!mysql_parse_status || thd->is_error() || thd->get_internal_handler());
...
void LEX::mark_first_table_as_inserting()
{
  ...
  DBUG_PRINT("info", ("table_list: %p  name: %s  db: %s  command: %u", t, t->table_name.str, t->db.str, sql_command));
  DBUG_VOID_RETURN;
}
