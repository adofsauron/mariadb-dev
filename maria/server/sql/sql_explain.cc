...
const char *unit_operation_text[4] = {"UNIT RESULT", "UNION RESULT", "INTERSECT RESULT", "EXCEPT RESULT"};
...
static void print_json_array(Json_writer *writer, const char *title, String_list &list)
{
  List_iterator_fast<char> it(list);
  const char *name;
  writer->add_member(title).start_array();
  while ((name = it++)) writer->add_str(name);
  writer->end_array();
}
...
Explain_query::~Explain_query()
{
  ...
  delete insert_plan;
  uint i;
  for (i = 0; i < unions.elements(); i++) delete unions.at(i);
  for (i = 0; i < selects.elements(); i++) delete selects.at(i);
}
...
int Explain_query::print_explain(select_result_sink *output, uint8 explain_flags, bool is_analyze)
{
  if (upd_del_plan)
  {
...
int Explain_query::print_explain_json(select_result_sink *output, bool is_analyze, bool is_show_cmd,
                                      ulonglong query_time_in_progress_ms)
{
  ...
  */
  if (is_analyze)
  {
    ...
    {
      writer.add_member("r_query_time_in_progress_ms").add_ull(query_time_in_progress_ms);
    }
...
void Explain_query::print_query_optimization_json(Json_writer *writer)
{
  if (optimization_time_tracker.has_timed_statistics())
  {
    ...
    writer->add_member("r_total_time_ms").add_double(optimization_time_tracker.get_time_ms());
    writer->end_object();
  }
}
...
void Explain_query::send_explain_json_to_output(Json_writer *writer, select_result_sink *output)
{
  ...
  THD *thd = output->thd;
  item_list.push_back(new (thd->mem_root) Item_string(thd, buf->ptr(), buf->length(), cs), thd->mem_root);
...
{
  item_list->push_back(new (thd->mem_root) Item_string_sys(thd, str), thd->mem_root);
}

static void push_string(THD *thd, List<Item> *item_list, String *str)
{
  item_list->push_back(new (thd->mem_root) Item_string_sys(thd, str->ptr(), str->length()), thd->mem_root);
}

static void push_string_list(THD *thd, List<Item> *item_list, String_list &lines, String *buf)
{
...
static int print_explain_row(select_result_sink *result, uint8 options, bool is_analyze, uint select_number,
                             const char *select_type, const char *table_name, const char *partitions,
                             enum join_type jtype, String_list *possible_keys, const char *index, const char *key_len,
                             const char *ref, ha_rows *rows, double *r_rows, double r_filtered, const char *extra)
{
  THD *thd = result->thd;
  ...
  item_list.push_back(new (mem_root) Item_string_sys(thd, table_name), mem_root);
  if (options & DESCRIBE_PARTITIONS)
  {
    if (partitions)
    {
      item_list.push_back(new (mem_root) Item_string_sys(thd, partitions), mem_root);
    }
    else
  ...
  item_list.push_back(new (mem_root) Item_string_sys(thd, jtype_str), mem_root);
  ...
  */
  ...
  if (is_analyze)
  {
    if (r_rows)
    {
      ...
      item_list.push_back(new (mem_root) Item_string_sys(thd, r_rows_str.ptr(), r_rows_str.length()), mem_root);
    }
    else
      item_list.push_back(item_null, mem_root);
  }
  ...
  /* 'r_filtered' */
  if (is_analyze)
    item_list.push_back(new (mem_root) Item_float(thd, r_filtered, 2), mem_root);
...
uint Explain_union::make_union_table_name(char *buf)
{
  ...
  for (; childno < union_members.elements() && len + lastop + 5 < NAME_LEN; childno++)
  {
    len += lastop;
    lastop = (uint)my_snprintf(buf + len, NAME_LEN - len, "%u,", union_members.at(childno));
  }
...
int Explain_union::print_explain(Explain_query *query, select_result_sink *output, uint8 explain_flags, bool is_analyze)
{
  THD *thd = output->thd;
  MEM_ROOT *mem_root = thd->mem_root;
  ...
  item_list.push_back(new (mem_root) Item_string_sys(thd, table_name_buffer, len), mem_root);
  ...
  item_list.push_back(new (mem_root) Item_string_sys(thd, extra_buf.ptr(), extra_buf.length()), mem_root);
...
void Explain_union::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
...
int Explain_node::print_explain_for_children(Explain_query *query, select_result_sink *output, uint8 explain_flags,
                                             bool is_analyze)
{
...
bool Explain_basic_join::add_table(Explain_table_access *tab, Explain_query *query)
{
  if (!join_tabs)
  {
    n_join_tabs = 0;
    if (!(join_tabs =
              ((Explain_table_access **)alloc_root(query->mem_root, sizeof(Explain_table_access *) * MAX_TABLES))))
      return true;
  }
...
{
  return (type != Explain_node::EXPLAIN_NODE_DERIVED && type != Explain_node::EXPLAIN_NODE_NON_MERGED_SJ);
}

void Explain_node::print_explain_json_for_children(Explain_query *query, Json_writer *writer, bool is_analyze,
                                                   bool no_tmp_tbl)
{
  Json_writer_nesting_guard guard(writer);
...
bool Explain_node::print_explain_json_cache(Json_writer *writer, bool is_analyze)
{
  if (cache_tracker)
  {
    ...
    {
      writer->add_member("state").add_str(Expression_cache_tracker::state_str[cache_tracker->state]);
    }
...
Explain_basic_join::~Explain_basic_join()
{
  if (join_tabs)
  {
    for (uint i = 0; i < n_join_tabs; i++) delete join_tabs[i];
  }
}

int Explain_select::print_explain(Explain_query *query, select_result_sink *output, uint8 explain_flags,
                                  bool is_analyze)
{
  THD *thd = output->thd;
  MEM_ROOT *mem_root = thd->mem_root;
  ...
  {
    print_explain_message_line(output, explain_flags, is_analyze, select_id /*select number*/, select_type,
                               NULL, /* rows */
                               NULL);
  }
  else if (message)
  {
    ...
    item_list.push_back(new (mem_root) Item_int(thd, (int32)select_id), mem_root);
    item_list.push_back(new (mem_root) Item_string_sys(thd, select_type), mem_root);
    for (uint i = 0; i < 7; i++) item_list.push_back(item_null, mem_root);
    if (explain_flags & DESCRIBE_PARTITIONS)
      item_list.push_back(item_null, mem_root);
  ...
  else
  {
    ...
    {
      join_tabs[i]->print_explain(output, explain_flags, is_analyze, select_id, select_type, using_tmp, using_fs);
      if (i == 0)
      {
        /*
...
int Explain_basic_join::print_explain(Explain_query *query, select_result_sink *output, uint8 explain_flags,
                                      bool is_analyze)
{
  for (uint i = 0; i < n_join_tabs; i++)
  {
    if (join_tabs[i]->print_explain(output, explain_flags, is_analyze, select_id, "MATERIALIZED" /*select_type*/,
                                    FALSE /*using temporary*/, FALSE /*using filesort*/))
      return 1;
  }
  return 0;
}
...
void Explain_select::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
  Json_writer_nesting_guard guard(writer);

  bool started_cache = print_explain_json_cache(writer, is_analyze);

  if (message || select_type == pushed_derived_text || select_type == pushed_select_text)
  {
    writer->add_member("query_block").start_object();
    ...
    writer->add_member("message").add_str(select_type == pushed_derived_text  ? "Pushed derived"
                                          : select_type == pushed_select_text ? "Pushed select"
                                                                              : message);
    writer->end_object();
  ...
  else
  {
    ...
    Explain_basic_join::print_explain_json_interns(query, writer, is_analyze, no_tmp_tbl);

    for (; started_objects; started_objects--) writer->end_object();
...
Explain_aggr_filesort::Explain_aggr_filesort(MEM_ROOT *mem_root, bool is_analyze, Filesort *filesort)
    : tracker(is_analyze)
{
  child = NULL;
...
void Explain_aggr_filesort::print_json_members(Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
  char item_buf[256];
...
void Explain_aggr_window_funcs::print_json_members(Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
...
void Explain_basic_join::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
  writer->add_member("query_block").start_object();
  writer->add_member("select_id").add_ll(select_id);
...
void Explain_basic_join::print_explain_json_interns(Explain_query *query, Json_writer *writer, bool is_analyze,
                                                    bool no_tmp_tbl)
{
  {
...
void Explain_table_access::push_extra(enum explain_extra_tag extra_tag) { extra_tags.append(extra_tag); }
...
{
  CHARSET_INFO *cs = system_charset_info;
  bool is_hj = (type == JT_HASH || type == JT_HASH_NEXT || type == JT_HASH_RANGE || type == JT_HASH_INDEX_MERGE);
...
void Explain_table_access::fill_key_len_str(String *key_len_str, bool is_json) const
{
  bool is_hj = (type == JT_HASH || type == JT_HASH_NEXT || type == JT_HASH_RANGE || type == JT_HASH_INDEX_MERGE);
...
bool Explain_index_use::set(MEM_ROOT *mem_root, KEY *key, uint key_len_arg)
{
  ...
  {
    if (!key_parts_list.append_str(mem_root, key->key_part[i].field->field_name.str))
      return 1;
...
int Explain_table_access::print_explain(select_result_sink *output, uint8 explain_flags, bool is_analyze,
                                        uint select_id, const char *select_type, bool using_temporary,
                                        bool using_filesort)
{
  ...
  else
  {
    ...
    item_list.push_back(new (mem_root) Item_string_sys(thd, join_type_buf.ptr(), join_type_buf.length()), mem_root);
  }
  ...
  if (rows_set)
  {
    ...
    if (rowid_filter)
    {
      ...
      rows_str.append_ulonglong((ulonglong)(round(rowid_filter->selectivity * 100.0)));
      rows_str.append(STRING_WITH_LEN("%)"));
    }
    item_list.push_back(new (mem_root) Item_string_sys(thd, rows_str.ptr(), rows_str.length()), mem_root);
  }
  ...
  if (is_analyze)
  {
    ...
    else
    {
      ...
      if (rowid_filter)
      {
        ...
        r_rows_str.append_ulonglong((ulonglong)(rowid_filter->tracker->get_r_selectivity_pct() * 100.0));
        r_rows_str.append(STRING_WITH_LEN("%)"));
      }
      item_list.push_back(new (mem_root) Item_string_sys(thd, r_rows_str.ptr(), r_rows_str.length()), mem_root);
    }
  ...
  if (is_analyze)
  {
    ...
    else
    {
      ...
      if (bka_type.is_using_jbuf())
        r_filtered *= jbuf_tracker.get_filtered_after_where();
      item_list.push_back(new (mem_root) Item_float(thd, r_filtered * 100.0, 2), mem_root);
    }
  }
...
void Explain_table_access::tag_to_json(Json_writer *writer, enum explain_extra_tag tag, bool no_tmp_tbl)
{
  switch (tag)
  {
...
static void add_json_keyset(Json_writer *writer, const char *elem_name, String_list *keyset)
{
  if (!keyset->is_empty())
    print_json_array(writer, elem_name, *keyset);
}

void Explain_rowid_filter::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze)
{
  Json_writer_nesting_guard guard(writer);
  ...
  if (is_analyze)
  {
    ...
    writer->add_member("r_selectivity_pct").add_double(tracker->get_r_selectivity_pct() * 100.0);
    writer->add_member("r_buffer_size").add_double((double)(tracker->get_container_buff_size()));
    writer->add_member("r_filling_time_ms").add_double(tracker->get_time_fill_container_ms());
  }
  writer->end_object();  // rowid_filter
}

void Explain_table_access::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze,
                                              bool no_tmp_tbl)
{
  ...
  if (bka_type.is_using_jbuf())
  {
    ...
    writer->add_member("buffer_type").add_str(bka_type.incremental ? "incremental" : "flat");
  ...
  if (derived_select_number)
  {
    ...
    if (node->get_type() == Explain_node::EXPLAIN_SELECT && ((Explain_select *)node)->is_lateral)
    {
...
void Explain_table_access::append_tag_name(String *str, enum explain_extra_tag tag)
{
  switch (tag)
  {
    ...
    case ET_RANGE_CHECKED_FOR_EACH_RECORD:
    {
      ...
      str->append(
          STRING_WITH_LEN("Range checked for each "
                          "record (index map: 0x"));
...
{
  if (quick_type == QUICK_SELECT_I::QS_TYPE_RANGE || quick_type == QUICK_SELECT_I::QS_TYPE_RANGE_DESC ||
...
int Explain_delete::print_explain(Explain_query *query, select_result_sink *output, uint8 explain_flags,
                                  bool is_analyze)
{
  if (deleting_all_rows)
  {
    ...
    int res =
        print_explain_message_line(output, explain_flags, is_analyze, 1 /*select number*/, select_type, &rows, msg);
    return res;
  }
  else
  {
    return Explain_update::print_explain(query, output, explain_flags, is_analyze);
  }
}

void Explain_delete::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
  Json_writer_nesting_guard guard(writer);
...
int Explain_update::print_explain(Explain_query *query, select_result_sink *output, uint8 explain_flags,
                                  bool is_analyze)
{
  ...
  {
    const char *msg = impossible_where ? STR_IMPOSSIBLE_WHERE : STR_NO_ROWS_AFTER_PRUNING;
    int res =
        print_explain_message_line(output, explain_flags, is_analyze, 1 /*select number*/, select_type, NULL, /* rows */
                                   msg);
    return res;
  }
  ...
  */
  ...
  print_explain_row(output, explain_flags, is_analyze, 1, /* id */
                    select_type, table_name.c_ptr(), used_partitions_set ? used_partitions.c_ptr() : NULL, jtype,
                    &possible_keys, key_buf.length() ? key_buf.c_ptr() : NULL,
                    key_len_buf.length() ? key_len_buf.c_ptr() : NULL,
                    NULL, /* 'ref' is always NULL in single-table EXPLAIN DELETE */
                    &rows, tracker.has_scans() ? &r_rows : NULL, r_filtered, extra_str.c_ptr_safe());

  return print_explain_for_children(query, output, explain_flags, is_analyze);
}

void Explain_update::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
  Json_writer_nesting_guard guard(writer);
  ...
  {
    writer->add_member("r_total_time_ms").add_double(command_tracker.get_time_ms());
  }
  ...
  if (is_analyze)
  {
    ...
    {
      writer->add_member("r_total_time_ms").add_double(table_tracker.get_time_ms());
    }
  }
...
int Explain_insert::print_explain(Explain_query *query, select_result_sink *output, uint8 explain_flags,
                                  bool is_analyze)
{
  const char *select_type = "INSERT";
  print_explain_row(output, explain_flags, is_analyze, 1, /* id */
                    select_type, table_name.c_ptr(),
                    NULL,  // partitions
                    JT_ALL,
                    NULL,   // possible_keys
                    NULL,   // key
...
void Explain_insert::print_explain_json(Explain_query *query, Json_writer *writer, bool is_analyze, bool no_tmp_tbl)
{
  Json_writer_nesting_guard guard(writer);
...
int Explain_range_checked_fer::append_possible_keys_stat(MEM_ROOT *alloc, TABLE *table, key_map possible_keys)
{
  uint j;
  multi_alloc_root(alloc, &keys_stat, sizeof(ha_rows) * table->s->keys, &keys_stat_names,
                   sizeof(char *) * table->s->keys, NULL);
  ...
  for (j = 0; j < table->s->keys; j++)
  {
    ...
    {
      if (!(keys_stat_names[j] = key_set.append_str(alloc, table->key_info[j].name.str)))
        return 1;
    }
    else
...
void Explain_range_checked_fer::print_json(Json_writer *writer, bool is_analyze)
{
