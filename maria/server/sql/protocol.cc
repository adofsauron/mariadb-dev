...
CHARSET_INFO *Protocol::character_set_results() const { return thd->variables.character_set_results; }
...
#ifndef EMBEDDED_LIBRARY
bool Protocol::net_store_data_cs(const uchar *from, size_t length, CHARSET_INFO *from_cs, CHARSET_INFO *to_cs)
#else
bool Protocol_binary::net_store_data_cs(const uchar *from, size_t length, CHARSET_INFO *from_cs, CHARSET_INFO *to_cs)
#endif
{
  uint dummy_errors;
  ...
  if (conv_length > 250)
  {
    ...
    */
    return (convert->copy((const char *)from, length, from_cs, to_cs, &dummy_errors) ||
  ...
  to += copy_and_convert(to, conv_length, to_cs, (const char *)from, length, from_cs, &dummy_errors);
...
bool Protocol::net_send_error(THD *thd, uint sql_errno, const char *err, const char *sqlstate)
{
  bool error;
...
#ifndef EMBEDDED_LIBRARY
bool Protocol::net_send_ok(THD *thd, uint server_status, uint statement_warn_count, ulonglong affected_rows,
                           ulonglong id, const char *message, bool is_eof)
{
  NET *net = &thd->net;
  ...
  */
  ...
  if (thd->client_capabilities & CLIENT_PROTOCOL_41)
  {
    DBUG_PRINT("info", ("affected_rows: %lu  id: %lu  status: %u  warning_count: %u", (ulong)affected_rows, (ulong)id,
                        (uint)(server_status & 0xffff), (uint)statement_warn_count));
...
bool Protocol::net_send_eof(THD *thd, uint server_status, uint statement_warn_count)
{
  NET *net = &thd->net;
  bool error = FALSE;
  ...
  */
  if ((thd->client_capabilities & CLIENT_DEPRECATE_EOF) && (thd->get_command() != COM_BINLOG_DUMP))
  {
    error = net_send_ok(thd, server_status, statement_warn_count, 0, 0, NULL, true);
    DBUG_RETURN(error);
  }
...
bool Protocol::net_send_error_packet(THD *thd, uint sql_errno, const char *err, const char *sqlstate)

{
  ...
  */
  if (!net->pkt_nr &&
      (sql_errno == ER_CONNECTION_KILLED || sql_errno == ER_SERVER_SHUTDOWN || sql_errno == ER_QUERY_INTERRUPTED))
  {
    net->pkt_nr = 1;
  }

  ret = net_write_command(net, (uchar)255, (uchar *)"", 0, (uchar *)buff, length);
...
void Protocol::end_statement()
{
  ...
  */
  ...
  if (WSREP(thd) && thd->wsrep_trx().state() == wsrep::transaction::s_replaying)
  {
  ...
  switch (thd->get_stmt_da()->status())
  {
    case Diagnostics_area::DA_ERROR:
      ...
      error = send_error(thd->get_stmt_da()->sql_errno(), thd->get_stmt_da()->message(),
                         thd->get_stmt_da()->get_sqlstate());
      break;
    case Diagnostics_area::DA_EOF:
    case Diagnostics_area::DA_EOF_BULK:
      error = send_eof(thd->server_status, thd->get_stmt_da()->statement_warn_count());
      break;
    case Diagnostics_area::DA_OK:
    case Diagnostics_area::DA_OK_BULK:
      error =
          send_ok(thd->server_status, thd->get_stmt_da()->statement_warn_count(), thd->get_stmt_da()->affected_rows(),
                  thd->get_stmt_da()->last_insert_id(), thd->get_stmt_da()->message());
      break;
...
bool Protocol::send_ok(uint server_status, uint statement_warn_count, ulonglong affected_rows, ulonglong last_insert_id,
                       const char *message)
{
  DBUG_ENTER("Protocol::send_ok");
  const bool retval =
      net_send_ok(thd, server_status, statement_warn_count, affected_rows, last_insert_id, message, false);
  DBUG_RETURN(retval);
}
...
bool Protocol::send_error(uint sql_errno, const char *err_msg, const char *sql_state)
{
...
void net_send_progress_packet(THD *thd)
{
  ...
  *pos++ = (uchar)MY_MAX(thd->progress.max_stage, thd->progress.stage + 1);
  ...
  pos += 3;
  pos = net_store_data(pos, (const uchar *)proc_info, MY_MIN(length, sizeof(buff) - 7));
  net_write_command(&thd->net, (uchar)255, progress_header, sizeof(progress_header), (uchar *)buff, (uint)(pos - buff));
  DBUG_VOID_RETURN;
}
...
bool Protocol_text::store_field_metadata(const THD *thd, const Send_field &field, CHARSET_INFO *charset_for_protocol,
                                         uint fieldnr)
{
  ...
  if (thd->client_capabilities & CLIENT_PROTOCOL_41)
  {
    ...
    if (store_ident(def) || store_ident(field.db_name) || store_ident(field.table_name) ||
        store_ident(field.org_table_name) || store_ident(field.col_name) || store_ident(field.org_col_name))
      return true;
    ...
    *pos++ = 12;  // Length of packed fields
    ...
    else
    {
      ...
      DBUG_ASSERT(id <= UINT_MAX16);
      int2store(pos, (uint16)id);
      uint32 field_length = field.max_octet_length(charset_for_protocol, thd_charset);
  ...
  {
    if (store_ident(field.table_name) || store_ident(field.col_name) || packet->realloc(packet->length() + 10))
      return true;
    pos = (char *)packet->end();
    pos[0] = 3;
...
static uint32 calc_metadata_hash(THD *thd, List<Item> *list)
{
  ...
  while ((item = it++))
  {
    ...
    */
    ...
    for (const auto &chunk : data) crc32_c = my_crc32c(crc32_c, chunk.str, chunk.length);
  }
...
static bool metadata_columns_changed(send_column_info_state &state, THD *thd, List<Item> &list)
{
...
static bool should_send_column_info(THD *thd, List<Item> *list, uint flags)
{
  ...
  DBUG_ASSERT(cmd == COM_STMT_EXECUTE || cmd == COM_STMT_PREPARE || cmd == COM_STMT_BULK_EXECUTE);
...
bool Protocol::send_result_set_metadata(List<Item> *list, uint flags)
{
...
#ifndef DBUG_OFF
    field_handlers = (const Type_handler **)thd->alloc(sizeof(field_handlers[0]) * list->elements);
#endif
  ...
  if (flags & SEND_EOF)
  {
    ...
    if (!(thd->client_capabilities & CLIENT_DEPRECATE_EOF))
    {
      ...
      */
      if (write_eof_packet(thd, &thd->net, thd->server_status, thd->get_stmt_da()->current_statement_warn_count()))
        DBUG_RETURN(1);
    }
  }
...
err:
  my_message(ER_OUT_OF_RESOURCES, ER_THD(thd, ER_OUT_OF_RESOURCES), MYF(0)); /* purecov: inspected */
...
bool Protocol::write()
{
  ...
  DBUG_RETURN(my_net_write(&thd->net, (uchar *)packet->ptr(), packet->length()));
}
#endif /* EMBEDDED_LIBRARY */
...
bool Protocol_text::store_field_metadata_for_list_fields(const THD *thd, Field *fld, const TABLE_LIST *tl, uint pos)
{
  Send_field field = tl->view ? Send_field(fld, tl->view_db, tl->view_name) : Send_field(fld);
...
bool Protocol::store_string_aux(const char *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs)
{
...
bool Protocol_text::store_str(const char *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs)
{
#ifndef DBUG_OFF
  DBUG_PRINT("info", ("Protocol_text::store field %u : %.*b", field_pos, (int)length, (length == 0 ? "" : from)));
...
bool Protocol_text::store_numeric_zerofill_str(const char *from, size_t length, protocol_send_type_t send_type)
{
#ifndef DBUG_OFF
  DBUG_PRINT("info", ("Protocol_text::store_numeric_zerofill_str field %u : %.*b", field_pos, (int)length,
                      (length == 0 ? "" : from)));
  DBUG_ASSERT(field_handlers == 0 || field_pos < field_count);
...
#endif
  char buff[22];
  size_t length = (size_t)(int10_to_str((long int)from, buff, (from < 0) ? -10 : 10) - buff);
  return store_numeric_string_aux(buff, length);
}
...
#endif
  char buff[22];
  size_t length = (size_t)(longlong10_to_str(from, buff, unsigned_flag ? 10 : -10) - buff);
  return store_numeric_string_aux(buff, length);
}
...
bool Protocol_binary::store_str(const char *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs)
{
  field_pos++;
  return store_string_aux(from, length, fromcs, tocs);
}
...
bool Protocol_binary::store_decimal(const my_decimal *d)
{
  ...
  StringBuffer<DECIMAL_MAX_STR_LENGTH> str;
  (void)d->to_string(&str);
  return store_str(str.ptr(), str.length(), str.charset(), thd->variables.character_set_results);
}
...
bool Protocol_binary::store_datetime(MYSQL_TIME *tm, int decimals)
{
  ...
  DBUG_ASSERT(decimals == AUTO_SEC_PART_DIGITS || (decimals >= 0 && decimals <= TIME_SECOND_PART_DIGITS));
