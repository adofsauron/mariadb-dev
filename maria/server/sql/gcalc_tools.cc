...
gcalc_shape_info Gcalc_function::add_new_shape(uint32 shape_id, shape_type shape_kind)
{
...
int Gcalc_function::reserve_shape_buffer(uint n_shapes) { return shapes_buffer.reserve(n_shapes * 4, 512); }

/*
  Specify how many operations we're going to have.
*/

int Gcalc_function::reserve_op_buffer(uint n_ops) { return function_buffer.reserve(n_ops * 4, 512); }
...
int Gcalc_function::count_internal(const char *cur_func, uint set_type, const char **end)
{
  ...
  {
    result = count_internal(cur_func, (set_type == 1) ? set_type : next_func, &cur_func);
    goto exit;
  }
...
{
  for (uint i = 0; i < n_shapes; i++) i_states[i] = 0;
}

void Gcalc_function::clear_b_states()
{
  for (uint i = 0; i < n_shapes; i++) b_states[i] = 0;
}
...
int Gcalc_function::check_function(Gcalc_scan_iterator &scan_it)
{
  ...
  while (scan_it.more_points())
  {
    ...
    for (; events; events = events->get_next())
    {
      ...
      if (events->event == scev_thread || events->event == scev_end ||
          (get_shape_kind(si) == Gcalc_function::shape_polygon))
        set_b_state(si);
      else if (events->event == scev_single_point || get_shape_kind(si) == Gcalc_function::shape_line)
        set_i_state(si);
    }
    ...
    {
      ++pit;
      if (pit.point() != scan_it.get_event_end() && eq_start->cmp_dx_dy(pit.point()) == 0)
        continue;
...
{
  gcalc_shape_info si;
  return m_fn->single_shape_op(Gcalc_function::shape_point, &si) || int_single_point(si, x, y);
}
...
int Gcalc_operation_transporter::add_point(double x, double y) { return int_add_point(m_si, x, y); }
...
int Gcalc_result_receiver::complete_shape()
{
  ...
  else
  {
    ...
    if ((cur_shape == Gcalc_function::shape_polygon || cur_shape == Gcalc_function::shape_hole) && prev_x == first_x &&
        prev_y == first_y)
    {
      n_points--;
...
{
  return start_shape(Gcalc_function::shape_point) || add_point(x, y) || complete_shape();
}
...
int Gcalc_result_receiver::get_result_typeid()
{
  ...
  switch (common_shapetype)
  {
    case Gcalc_function::shape_polygon:
      return (n_shapes - n_holes == 1) ? Geometry::wkb_polygon : Geometry::wkb_multipolygon;
    ...
    case Gcalc_function::shape_line:
      return (n_shapes == 1) ? Geometry::wkb_linestring : Geometry::wkb_multilinestring;
    default:
      GCALC_DBUG_ASSERT(0);
  }
  return 0;
...
int Gcalc_result_receiver::move_hole(uint32 dest_position, uint32 source_position, uint32 *position_shift)
{
  char *ptr;
  int source_len;
  ...
  *position_shift = source_len = buffer.length() - source_position;
  ...
  ptr = (char *)buffer.ptr();
  memmove(ptr + dest_position + source_len, ptr + dest_position, buffer.length() - dest_position);
...
Gcalc_operation_reducer::Gcalc_operation_reducer(const Gcalc_operation_reducer &gor)
    : Gcalc_dyn_list(gor),
#ifndef GCALC_DBUG_OFF
      n_res_points(0),
#endif /*GCALC_DBUG_OFF*/
...
Gcalc_operation_reducer::Gcalc_operation_reducer(Gcalc_function *fn, modes mode, size_t blk_size)
    : Gcalc_dyn_list(blk_size, sizeof(res_point)), m_res_hook((Gcalc_dyn_list::Item **)&m_result)
{
  init(fn, mode);
}
...
Gcalc_operation_reducer::res_point *Gcalc_operation_reducer::add_res_point(Gcalc_function::shape_type type)
{
...
int Gcalc_operation_reducer::add_line(int incoming, active_thread *t, const Gcalc_scan_iterator::point *p)
{
  line *l = new_line();
...
int Gcalc_operation_reducer::add_poly_border(int incoming, active_thread *t, int prev_state,
...
int Gcalc_operation_reducer::continue_range(active_thread *t, const Gcalc_heap::Info *p, const Gcalc_heap::Info *p_next)
{
...
inline int Gcalc_operation_reducer::continue_i_range(active_thread *t, const Gcalc_heap::Info *ii)
{
  res_point *rp = add_res_point(t->rp->type);
...
int Gcalc_operation_reducer::end_couple(active_thread *t0, active_thread *t1, const Gcalc_heap::Info *p)
{
  res_point *rp0, *rp1;
  GCALC_DBUG_ENTER("Gcalc_operation_reducer::end_couple");
  GCALC_DBUG_ASSERT(t0->rp->type == t1->rp->type);
  if (!(rp0 = add_res_point(t0->rp->type)) || !(rp1 = add_res_point(t0->rp->type)))
    GCALC_DBUG_RETURN(1);
  rp0->down = t0->rp;
  rp1->down = t1->rp;
  rp1->glue = rp0;
...
int Gcalc_operation_reducer::count_slice(Gcalc_scan_iterator *si)
{
  ...
  for (; pi.point() != si->get_event_position(); ++pi, cur_t_hook = (active_thread **)&(*cur_t_hook)->next)
  {
    active_thread *cur_t = *cur_t_hook;
    if (cur_t->enabled() && cur_t->rp->type == Gcalc_function::shape_polygon)
    {
      prev_state ^= 1;
  ...
  if (events->simple_event())
  {
    ...
    switch (events->event)
    {
      case scev_point:
      {
        if (cur_t->enabled() && continue_range(cur_t, events->pi, events->next_pi))
          GCALC_DBUG_RETURN(1);
        break;
      }
      case scev_end:
  ...
  for (; events; events = events->get_next())
  {
    ...
    if (events->event == scev_thread || events->event == scev_two_threads)
    {
  ...
  while (eq_start != si->get_event_end())
  {
    ...
    if (pi.point() != si->get_event_end() && eq_start->cmp_dx_dy(pi.point()) == 0)
      continue;

    for (cur_eq = eq_start; cur_eq != pi.point(); cur_eq = cur_eq->get_next()) m_fn->set_b_state(cur_eq->get_shape());
    in_state = m_fn->count();
  ...
  if (!sav_prev_state && !m_poly_borders && !m_lines)
  {
    ...
    for (events = si->get_events(); events; events = events->get_next()) m_fn->set_b_state(events->get_shape());
  ...
  if (m_poly_borders)
  {
    *m_poly_borders_hook = NULL;
    while (m_poly_borders)
    {
      ...
      if (connect_threads(pb1->incoming, pb2->incoming, pb1->t, pb2->t, pb1->p, pb2->p, prev_range, si,
  ...
  {
    *m_lines_hook = NULL;
    if (m_lines->get_next() && !m_lines->get_next()->get_next())
    {
      if (connect_threads(m_lines->incoming, m_lines->get_next()->incoming, m_lines->t, m_lines->get_next()->t,
                          m_lines->p, m_lines->get_next()->p, NULL, si, Gcalc_function::shape_line))
        GCALC_DBUG_RETURN(1);
    }
    else
    {
...
Gcalc_operation_reducer::poly_border *Gcalc_operation_reducer::get_pair_border(poly_border *b1)
{
...
int Gcalc_operation_reducer::connect_threads(int incoming_a, int incoming_b, active_thread *ta, active_thread *tb,
                                             const Gcalc_scan_iterator::point *pa, const Gcalc_scan_iterator::point *pb,
                                             active_thread *prev_range, const Gcalc_scan_iterator *si,
                                             Gcalc_function::shape_type s_t)
{
  ...
  if (incoming_a && incoming_b)
  {
    ...
    if (!(rpa = add_res_point(ta->rp->type)) || !(rpb = add_res_point(ta->rp->type)))
      GCALC_DBUG_RETURN(1);
    rpa->down = ta->rp;
    rpb->down = tb->rp;
    rpb->glue = rpa;
  ...
  if (Gcalc_scan_iterator::point::cmp_dx_dy(ta->p1, ta->p2, pb->pi, pb->next_pi) != 0)
  {
    if (si->intersection_step() ? continue_i_range(tb, si->get_cur_pi())
...
int Gcalc_operation_reducer::start_line(active_thread *t, const Gcalc_scan_iterator::point *p,
...
int Gcalc_operation_reducer::end_line(active_thread *t, const Gcalc_scan_iterator *si)
{
...
inline int Gcalc_operation_reducer::get_single_result(res_point *res, Gcalc_result_receiver *storage)
{
  ...
  else if (storage->single_point(res->pi->node.shape.x, res->pi->node.shape.y))
    GCALC_DBUG_RETURN(1);
  free_result(res);
  GCALC_DBUG_RETURN(0);
}

int Gcalc_operation_reducer::get_result_thread(res_point *cur, Gcalc_result_receiver *storage, int move_upward,
...
int Gcalc_operation_reducer::get_polygon_result(res_point *cur, Gcalc_result_receiver *storage,
                                                res_point *first_poly_node)
{
...
int Gcalc_operation_reducer::get_line_result(res_point *cur, Gcalc_result_receiver *storage)
{
  res_point *next;
  ...
  while (m_result)
  {
    ...
    if (shape == Gcalc_function::shape_polygon)
    {
      if (m_result->outer_poly)
      {
        ...
        if (get_polygon_result(m_result, storage, m_result->outer_poly->first_poly_node) ||
            storage->move_hole(insert_position, hole_position, &position_shift))
          GCALC_DBUG_RETURN(1);
        for (cur_poly = polygons; cur_poly && *cur_poly->after_poly_position >= insert_position;
