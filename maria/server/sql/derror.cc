...
#include <mysqld_ername.h>
    ...
    size_t all_errors = 0;
    for (size_t i = 0; i < MAX_ERROR_RANGES; i++) all_errors += errors_per_range[i];

    if (!(original_error_messages = (const char ***)my_malloc(
              PSI_NOT_INSTRUMENTED, (all_errors + MAX_ERROR_RANGES) * sizeof(void *), MYF(MY_ZEROFILL))))
      DBUG_RETURN(TRUE);
    ...
    {
      original_error_messages[i] = original_error_messages[i - 1] + errors_per_range[i - 1];
    }

    for (uint i = 0; i < array_elements(english_msgs); i++)
    {
      uint id = english_msgs[i].id;
      original_error_messages[id / ERRORS_PER_RANGE - 1][id % ERRORS_PER_RANGE] = english_msgs[i].fmt;
    }
  }
  ...
  for (uint i = 0; i < MAX_ERROR_RANGES; i++)
  {
    if (errors_per_range[i])
    {
      if (my_error_register(get_server_errmsgs, (i + 1) * ERRORS_PER_RANGE,
                            (i + 1) * ERRORS_PER_RANGE + errors_per_range[i] - 1))
      {
        my_free(original_error_messages);
        original_error_messages = 0;
        DBUG_RETURN(TRUE);
...
void free_error_messages()
{
  ...
  for (uint i = 0; i < MAX_ERROR_RANGES; i++)
  {
    if (errors_per_range[i])
    {
      my_error_unregister((i + 1) * ERRORS_PER_RANGE, (i + 1) * ERRORS_PER_RANGE + errors_per_range[i] - 1);
...
static bool check_error_mesg(const char *file_name, const char **errmsg)
{
  ...
  */
  if (errmsg[ER_LAST_MYSQL_ERROR_MESSAGE - 1 - ER_ERROR_FIRST][0] == 0 ||
      (errors_per_range[0] < ER_ERROR_LAST_SECTION_2 - ER_ERROR_FIRST + 1) || errors_per_range[1] != 0 ||
      (errors_per_range[2] < ER_ERROR_LAST_SECTION_4 - ER_ERROR_FIRST_SECTION_4 + 1) ||
      (errors_per_range[3] < ER_ERROR_LAST - ER_ERROR_FIRST_SECTION_5 + 1))
  {
    sql_print_error(
        "Error message file '%s' is probably from and older "
...
static File open_error_msg_file(const char *file_name, const char *language, uint error_messages,
                                struct st_msg_file *ret)
{
  int error_pos = 0;
  File file;
  char name[FN_REFLEN];
  ...
  if ((file = mysql_file_open(key_file_ERRMSG, fn_format(name, file_name, lang_path, "", 4),
                              O_RDONLY | O_SHARE | O_BINARY, MYF(0))) < 0)
  {
    ...
    */
    if ((file = mysql_file_open(key_file_ERRMSG, fn_format(name, file_name, lc_messages_dir, "", 4),
                                O_RDONLY | O_SHARE | O_BINARY, MYF(0))) < 0)
      goto err;
    if (global_system_variables.log_warnings > 2)
    {
      sql_print_warning("An old style --language or -lc-message-dir value with language specific part detected: %s",
                        lc_messages_dir);
  ...
  if (mysql_file_read(file, (uchar *)head, 32, MYF(MY_NABP)))
    goto err;
  error_pos = 2;
  if (head[0] != (uchar)254 || head[1] != (uchar)254 || head[2] != 2 || head[3] != 4)
  ...
  if (unlikely(ret->max_error < error_messages || ret->sections != MAX_ERROR_RANGES))
  {
    sql_print_error(
        "\
...
err:
  sql_print_error((error_pos == 2)
                      ...
                      : ((error_pos == 1) ? "Can't read from messagefile '%s'" : "Can't find messagefile '%s'"),
                  name);
  if (file != FERR)
    (void)mysql_file_close(file, MYF(MY_WME));
  DBUG_RETURN(FERR);
...
bool read_texts(const char *file_name, const char *language, const char ****data)
{
  ...
  if (unlikely((file = open_error_msg_file(file_name, language, error_messages, &msg_file)) == FERR))
    DBUG_RETURN(1);

  if (!(*data = (const char ***)my_malloc(
            key_memory_errmsgs,
            (size_t)((MAX_ERROR_RANGES + 1) * sizeof(char **) + MY_MAX(msg_file.text_length, msg_file.errors * 2) +
  ...
  if (mysql_file_read(file, buff, (size_t)(msg_file.errors + msg_file.sections) * 2, MYF(MY_NABP | MY_WME)))
    goto err;
