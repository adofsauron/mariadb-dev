...
class sp_variable : public Sql_alloc
{
 ...
 public:
  sp_variable(const LEX_CSTRING *name_arg, uint offset_arg)
      : Sql_alloc(), name(*name_arg), mode(MODE_IN), offset(offset_arg), default_value(NULL)
  {
  }
  /*
  ...
  */
  const Spvar_definition *find_row_field(const LEX_CSTRING *var_name, const LEX_CSTRING *field_name,
...
class sp_label : public Sql_alloc
{
 ...
 public:
  sp_label(const LEX_CSTRING *_name, uint _ip, enum_type _type, sp_pcontext *_ctx)
      : Sql_alloc(), name(*_name), ip(_ip), type(_type), ctx(_ctx)
  {
  }
};
...
class sp_condition_value : public Sql_alloc, public Sql_state_errno
{
 ...
 public:
  sp_condition_value(uint _mysqlerr)
      : Sql_alloc(), Sql_state_errno(_mysqlerr), m_is_user_defined(false), type(ERROR_CODE)
  {
  }

  sp_condition_value(uint _mysqlerr, const char *_sql_state)
      : Sql_alloc(), Sql_state_errno(_mysqlerr, _sql_state), m_is_user_defined(false), type(ERROR_CODE)
  {
  }

  sp_condition_value(const char *_sql_state, bool is_user_defined = false)
      : Sql_alloc(), Sql_state_errno(0, _sql_state), m_is_user_defined(is_user_defined), type(SQLSTATE)
  {
  }

  sp_condition_value(enum_type _type) : Sql_alloc(), m_is_user_defined(false), type(_type)
  {
  ...
  */
  bool matches(const Sql_condition_identity &identity, const sp_condition_value *found_cv) const;
...
{
 public:
  sp_condition_value_user_defined() : sp_condition_value("45000", true) {}
};
...
class sp_condition : public Sql_alloc
{
 ...
 public:
  sp_condition(const LEX_CSTRING *name_arg, sp_condition_value *value_arg)
      : Sql_alloc(), name(*name_arg), value(value_arg)
  {
  }
  sp_condition(const char *name_arg, size_t name_length_arg, sp_condition_value *value_arg) : value(value_arg)
  {
    name.str = name_arg;
  ...
  {
    return system_charset_info->strnncoll(name.str, name.length, str->str, str->length) == 0;
  }
};
...
class sp_pcursor : public LEX_CSTRING
{
 ...
 public:
  sp_pcursor(const LEX_CSTRING *name, class sp_pcontext *param_ctx, class sp_lex_cursor *lex)
...
class sp_handler : public Sql_alloc
{
 ...
 public:
  ...
  sp_handler(enum_type _type) : Sql_alloc(), type(_type) {}
};
...
class sp_pcontext : public Sql_alloc
{
 ...
 public:
  ...
  sp_pcontext *child_context(uint i) const { return i < m_children.elements() ? m_children.at(i) : NULL; }
  ...
  uint current_var_count() const { return m_var_offset + (uint)m_vars.elements(); }

  /// @return the number of variables in this context alone.
  uint context_var_count() const { return (uint)m_vars.elements(); }
  ...
  */
  ...
  int register_case_expr() { return m_num_case_exprs++; }

  int get_num_case_exprs() const { return m_num_case_exprs; }

  bool push_case_expr_id(int case_expr_id) { return m_case_expr_ids.append(case_expr_id); }

  void pop_case_expr_id() { m_case_expr_ids.pop(); }
  ...
  sp_label *push_label(THD *thd, const LEX_CSTRING *name, uint ip, sp_label::enum_type type, List<sp_label> *list);

  sp_label *push_label(THD *thd, const LEX_CSTRING *name, uint ip, sp_label::enum_type type)
  {
    return push_label(thd, name, ip, type, &m_labels);
  }

  sp_label *push_goto_label(THD *thd, const LEX_CSTRING *name, uint ip, sp_label::enum_type type)
  {
  ...
  sp_label *find_goto_label(const LEX_CSTRING *name) { return find_goto_label(name, true); }
  ...
  bool add_condition(THD *thd, const LEX_CSTRING *name, sp_condition_value *value);

  /// See comment for find_variable() above.
  sp_condition_value *find_condition(const LEX_CSTRING *name, bool current_scope_only) const;

  sp_condition_value *find_declared_or_predefined_condition(THD *thd, const LEX_CSTRING *name) const;

  bool declare_condition(THD *thd, const LEX_CSTRING *name, sp_condition_value *val)
  {
  ...
  bool add_cursor(const LEX_CSTRING *name, sp_pcontext *param_ctx, class sp_lex_cursor *lex);

  /// See comment for find_variable() above.
  const sp_pcursor *find_cursor(const LEX_CSTRING *name, uint *poff, bool current_scope_only) const;

  const sp_pcursor *find_cursor_with_error(const LEX_CSTRING *name, uint *poff, bool current_scope_only) const
  {
  ...
  const sp_pcursor *get_cursor_by_local_frame_offset(uint offset) const { return &m_cursors.at(offset); }

  uint cursor_offset() const { return m_cursor_offset; }

  uint frame_cursor_count() const { return (uint)m_cursors.elements(); }

  uint max_cursor_index() const { return m_max_cursor_index + (uint)m_cursors.elements(); }

  uint current_cursor_count() const { return m_cursor_offset + (uint)m_cursors.elements(); }

  void set_for_loop(const Lex_for_loop_st &for_loop) { m_for_loop.init(for_loop); }
  const Lex_for_loop_st &for_loop() { return m_for_loop; }
