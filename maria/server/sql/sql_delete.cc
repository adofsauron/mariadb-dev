...
Explain_delete *Delete_plan::save_explain_delete_data(MEM_ROOT *mem_root, THD *thd)
{
  ...
  Explain_delete *explain = new (mem_root) Explain_delete(mem_root, thd->lex->analyze_stmt);
  if (!explain)
    return 0;
  ...
  else
  {
    ...
    if (Update_plan::save_explain_data_intern(mem_root, explain, thd->lex->analyze_stmt))
      return 0;
  }
...
Explain_update *Update_plan::save_explain_update_data(MEM_ROOT *mem_root, THD *thd)
{
  Explain_query *query = thd->lex->explain;
  Explain_update *explain = new (mem_root) Explain_update(mem_root, thd->lex->analyze_stmt);
  if (!explain)
    return 0;
...
bool Update_plan::save_explain_data_intern(MEM_ROOT *mem_root, Explain_update *explain, bool is_analyze)
{
  explain->select_type = "SIMPLE";
  ...
  /* Partitions */
  {
    ...
    {
      make_used_partitions_str(mem_root, part_info, &explain->used_partitions, explain->used_partitions_list);
  ...
  append_possible_keys(mem_root, explain->possible_keys, table, possible_keys);
  ...
  else
  {
    if (index != MAX_KEY)
    {
      explain->key.set(mem_root, &table->key_info[index], table->key_info[index].key_length);
    }
  }
  explain->rows = scanned_rows;

  if (select && select->quick && select->quick->get_type() == QUICK_SELECT_I::QS_TYPE_RANGE)
  {
    explain_append_mrr_info((QUICK_RANGE_SELECT *)select->quick, &explain->mrr_type);
  }
...
static bool record_should_be_deleted(THD *thd, TABLE *table, SQL_SELECT *sel, Explain_delete *explain,
...
static int update_portion_of_time(THD *thd, TABLE *table, const vers_select_conds_t &period_conds, bool *inside_period)
{
  ...
  *inside_period = !lcond && !rcond;
  ...
  DBUG_ASSERT(!table->triggers || !table->triggers->has_triggers(TRG_EVENT_INSERT, TRG_ACTION_BEFORE));

  int res = 0;
  Item *src = lcond ? period_conds.start.item : period_conds.end.item;
  uint dst_fieldno = lcond ? table->s->period.end_fieldno : table->s->period.start_fieldno;
  ...
  if (likely(!res) && lcond && rcond)
    res = table->period_make_insert(period_conds.end.item, table->field[table->s->period.start_fieldno]);
...
bool mysql_delete(THD *thd, TABLE_LIST *table_list, COND *conds, SQL_I_List<ORDER> *order_list, ha_rows limit,
                  ulonglong options, select_result *result)
{
  ...
  ORDER *order = (ORDER *)((order_list && order_list->elements) ? order_list->first : NULL);
  ...
  if (order)
  {
    ...
    if (select_lex->setup_ref_array(thd, order_list->elements) ||
        setup_order(thd, select_lex->ref_pointer_array, &tables, fields, all_fields, order))
    {
  ...
  safe_update = (thd->variables.option_bits & OPTION_SAFE_UPDATES) && !thd->lex->describe;
  ...
  */
  ...
  if (!returning && !using_limit && const_cond_result && (!thd->is_current_stmt_binlog_format_row() && !has_triggers) &&
  ...
  if (order)
  {
    ...
    else
    {
      ...
      table->no_keyread = 1;
      query_plan.index =
          get_index_for_order(order, table, select, limit, &scanned_limit, &query_plan.using_filesort, &reverse);
      table->no_keyread = 0;
  ...
  */
  ...
  DBUG_EXECUTE_IF("show_explain_probe_delete_exec_start", dbug_serve_apcs(thd, 1););
  ...
  */
  ...
  if ((table->file->ha_table_flags() & HA_CAN_DIRECT_UPDATE_AND_DELETE) && !has_triggers && !binlog_is_row &&
      !returning && !table_list->has_period())
  {
    ...
    if (!table->check_virtual_columns_marked_for_read())
    {
      ...
      if (!use_direct_delete && (select->cond->used_tables() & ~RAND_TABLE_BIT) == table->map)
      {
  ...
  */
  ...
  {
    if (result->send_result_set_metadata(returning->item_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
      goto cleanup;
  }
  ...
  if (!delete_while_scanning)
  {
    ...
    */
    ...
    deltempfile =
        new (thd->mem_root) Unique(refpos_order_cmp, table->file, table->file->ref_length, MEM_STRIP_BUF_SIZE);
    ...
    while (!(error = info.read_record()) && !thd->killed && !thd->is_error())
    {
      if (record_should_be_deleted(thd, table, select, explain, delete_history))
      {
        ...
        if (unlikely((error = deltempfile->unique_add((char *)table->file->ref))))
        {
          error = 1;
          goto terminate_delete;
        }
    ...
    if (unlikely(deltempfile->get(table)) || unlikely(table->file->ha_index_or_rnd_end()) ||
        unlikely(init_read_record(&info, thd, table, 0, &deltempfile->sort, 0, 1, false)))
    {
      error = 1;
      goto terminate_delete;
    }
  ...
  */
  portion_of_time_through_update =
      !(table->triggers && table->triggers->has_triggers(TRG_EVENT_INSERT, TRG_ACTION_BEFORE)) && !table->versioned() &&
  ...
  while (likely(!(error = info.read_record())) && likely(!thd->killed) && likely(!thd->is_error()))
  {
    thd->get_stmt_da()->inc_current_row_for_warning();
    if (delete_while_scanning)
      delete_record = record_should_be_deleted(thd, table, select, explain, delete_history);
    if (delete_record)
    {
      if (!delete_history && table->triggers &&
          table->triggers->process_triggers(thd, TRG_EVENT_DELETE, TRG_ACTION_BEFORE, FALSE))
      {
        error = 1;
        break;
      }
      ...
      if (table_list->has_period() && portion_of_time_through_update)
      {
        ...
        error = update_portion_of_time(thd, table, table_list->period_conditions, &need_delete);
      ...
      if (likely(!error))
      {
        deleted++;
        if (!delete_history && table->triggers &&
            table->triggers->process_triggers(thd, TRG_EVENT_DELETE, TRG_ACTION_AFTER, FALSE))
        {
          error = 1;
          break;
        }
      ...
      {
        table->file->print_error(error, MYF(thd->lex->ignore ? ME_WARNING : 0));
        if (thd->is_error())
        {
          error = 1;
          break;
  ...
  */
  ...
  if (likely((error < 0) || thd->transaction->stmt.modified_non_trans_table || thd->log_current_statement()))
  {
    if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
    {
      ...
      */
      int log_result =
          thd->binlog_query(query_type, thd->query(), thd->query_length(), transactional_table, FALSE, FALSE, errcode);
  ...
  if (likely(error < 0) || (thd->lex->ignore && !thd->is_error() && !thd->is_fatal_error))
  {
...
int mysql_prepare_delete(THD *thd, TABLE_LIST *table_list, Item **conds, bool *delete_while_scanning)
{
  ...
  *delete_while_scanning = true;
  ...
  if (setup_tables_and_check_access(thd, &select_lex->context, &select_lex->top_join_list, table_list,
                                    select_lex->leaf_tables, FALSE, DELETE_ACL, SELECT_ACL, TRUE))
    DBUG_RETURN(TRUE);
  ...
  *conds = select_lex->where;
  ...
  if (setup_returning_fields(thd, table_list) || setup_conds(thd, table_list, select_lex->leaf_tables, conds) ||
      setup_ftfuncs(select_lex))
    DBUG_RETURN(TRUE);
  if (!table_list->single_table_updatable() || check_key_in_view(thd, table_list))
  {
    my_error(ER_NON_UPDATABLE_TABLE, MYF(0), table_list->alias.str, "DELETE");
    DBUG_RETURN(TRUE);
  }
   ...
   */
  if (table_list->has_period() || unique_table(thd, table_list, table_list->next_global, 0))
...
int mysql_multi_delete_prepare(THD *thd)
{
  ...
  */
  if (setup_tables_and_check_access(thd, &thd->lex->first_select_lex()->context,
                                    &thd->lex->first_select_lex()->top_join_list, lex->query_tables,
                                    lex->first_select_lex()->leaf_tables, FALSE, DELETE_ACL, SELECT_ACL, FALSE))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  for (target_tbl = (TABLE_LIST *)aux_tables; target_tbl; target_tbl = target_tbl->next_local)
  {
    ...
    {
      my_error(ER_NON_UPDATABLE_TABLE, MYF(0), target_tbl->table_name.str, "DELETE");
      DBUG_RETURN(TRUE);
    }
  }
    ...
    */
    {
      ...
      if ((duplicate = unique_table(thd, target_tbl->correspondent_table, lex->query_tables, 0)))
      {
...
bool multi_delete::initialize_tables(JOIN *join)
{
  ...
  for (walk = delete_tables; walk; walk = walk->next_local)
  {
    ...
    if (delete_while_scanning && unique_table(thd, tbl, join->tables_list, 0))
    {
      /*
...
void multi_delete::abort_result_set()
{
  ...
  if (error_handled || (!thd->transaction->stmt.modified_non_trans_table && !deleted))
    DBUG_VOID_RETURN;
  ...
  */
  if (do_delete && normal_tables &&
      (table_being_deleted != delete_tables || !table_being_deleted->table->file->has_transactions_and_rollback()))
  {
    /*
...
int multi_delete::do_deletes()
{
  ...
  for (uint counter = 0; table_being_deleted; table_being_deleted = table_being_deleted->next_local, counter++)
  {
    TABLE *table = table_being_deleted->table;
    int local_error;
    ...
    local_error = do_table_deletes(table, &tempfiles[counter]->sort, thd->lex->ignore);
...
*/
int multi_delete::do_table_deletes(TABLE *table, SORT_INFO *sort_info, bool ignore)
{
  int local_error = 0;
  READ_RECORD info;
  ...
  if (unlikely(init_read_record(&info, thd, table, NULL, sort_info, 0, 1, FALSE)))
    DBUG_RETURN(1);

  bool will_batch = !table->file->start_bulk_delete();
  while (likely(!(local_error = info.read_record())) && likely(!thd->killed))
  {
    if (table->triggers && unlikely(table->triggers->process_triggers(thd, TRG_EVENT_DELETE, TRG_ACTION_BEFORE, FALSE)))
    {
      local_error = 1;
      break;
    }
  ...
  */
  ...
  if (likely((local_error == 0) || thd->transaction->stmt.modified_non_trans_table) || thd->log_current_statement())
  {
    if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
    {
      ...
      StatementBinlog stmt_binlog(thd, thd->binlog_need_stmt_format(transactional_tables));
      if (unlikely(thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(), thd->query_length(), transactional_tables,
