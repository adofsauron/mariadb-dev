...
bool is_parallel_retry_error(rpl_group_info *rgi, int err)
{
  ...
  if (rgi->speculation == rpl_group_info::SPECULATE_OPTIMISTIC)
    return true;
  if (rgi->killed_for_retry && (err == ER_QUERY_INTERRUPTED || err == ER_CONNECTION_KILLED))
    return true;
...
*/
static void inline slave_rows_error_report(enum loglevel level, int ha_error, rpl_group_info *rgi, THD *thd,
                                           TABLE *table, const char *type, const char *log_name, my_off_t pos)
{
  ...
  */
  ...
  for (err = it++, slider = buff; err && slider < buff_end - 1; slider += len, err = it++)
  {
    len = my_snprintf(slider, buff_end - slider, " %s, Error_code: %d;", err->get_message_text(), err->get_sql_errno());
  }
...
#if defined(HAVE_REPLICATION)
static void set_thd_db(THD *thd, Rpl_filter *rpl_filter, const char *db, uint32 db_len)
{
...
{
  uint actual_error = (thd->is_error() ? thd->get_stmt_da()->sql_errno() : 0);

  if (actual_error == 0)
  {
    table->file->print_error(error, MYF(0));
    actual_error = (thd->is_error() ? thd->get_stmt_da()->sql_errno() : ER_UNKNOWN_ERROR);
...
static void pretty_print_str(String *packet, const char *str, int len)
{
  ...
  while (str < end)
  {
    ...
    switch ((c = *str++))
    {
      case '\n':
        ...
        break;
      case '\r':
        packet->append(STRING_WITH_LEN("\\r"));
        break;
      case '\\':
        packet->append(STRING_WITH_LEN("\\\\"));
        break;
      case '\b':
        packet->append(STRING_WITH_LEN("\\b"));
        break;
      case '\t':
        packet->append(STRING_WITH_LEN("\\t"));
        break;
...
static char *load_data_tmp_prefix(char *name, LEX_CSTRING *connection_name)
{
  ...
  if (connection_name->length)
  {
    ...
    *name++ = '-';
    ...
    buf_length = strconvert(system_charset_info, connection_name->str, FN_REFLEN, &my_charset_filename, name, FN_REFLEN,
                            &errors);
    name += buf_length;
    *name++ = '-';
  }
...
int append_query_string(CHARSET_INFO *csinfo, String *to, const char *str, size_t len, bool no_backslash)
{
  char *beg, *ptr;
  my_bool overflow;
...
Log_event::Log_event(THD *thd_arg, uint16 flags_arg, bool using_trans)
    : log_pos(0), temp_buf(0), exec_time(0), thd(thd_arg), checksum_alg(BINLOG_CHECKSUM_ALG_UNDEF)
{
  ...
  flags = flags_arg | (thd->variables.option_bits & OPTION_SKIP_REPLICATION ? LOG_EVENT_SKIP_REPLICATION_F : 0);
}
...
Log_event::enum_skip_reason Log_event::do_shall_skip(rpl_group_info *rgi)
{
  Relay_log_info *rli = rgi->rli;
  DBUG_PRINT("info", ("ev->server_id: %lu, ::server_id: %lu,"
                      " rli->replicate_same_server_id: %d,"
                      " rli->slave_skip_counter: %llu",
                      (ulong)server_id, (ulong)global_system_variables.server_id, rli->replicate_same_server_id,
                      rli->slave_skip_counter));
  if ((server_id == global_system_variables.server_id &&
       !(rli->replicate_same_server_id || (flags & LOG_EVENT_ACCEPT_OWN_F))) ||
      (rli->slave_skip_counter == 1 && rli->is_in_group()) ||
      (flags & LOG_EVENT_SKIP_REPLICATION_F && opt_replicate_events_marked_for_skip != RPL_SKIP_REPLICATE))
...
void Log_event::pack_info(Protocol *protocol) { protocol->store("", 0, &my_charset_bin); }
...
void Log_event::init_show_field_list(THD *thd, List<Item> *field_list)
{
  ...
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Log_name", 20), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Pos", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG),
                        mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Event_type", 20), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Server_id", 10, MYSQL_TYPE_LONG), mem_root);
  field_list->push_back(
      new (mem_root) Item_return_int(thd, "End_log_pos", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Info", 20), mem_root);
}
...
my_bool Log_event::need_checksum()
{
  ...
  */
  ...
  else
  {
    ...
    checksum_alg = ret ? (enum_binlog_checksum_alg)binlog_checksum_options : BINLOG_CHECKSUM_ALG_OFF;
  }
  /*
  ...
  */
  ...
  DBUG_ASSERT(get_type_code() != FORMAT_DESCRIPTION_EVENT || ret || data_written == 0);

  DBUG_ASSERT(!ret || ((checksum_alg == binlog_checksum_options ||
                        /*
                        ...
                        */
                        get_type_code() == ROTATE_EVENT || get_type_code() == START_ENCRYPTION_EVENT ||
  ...
  DBUG_ASSERT(((get_type_code() != ROTATE_EVENT && get_type_code() != STOP_EVENT) ||
               get_type_code() != FORMAT_DESCRIPTION_EVENT) ||
...
int Log_event_writer::encrypt_and_write(const uchar *pos, size_t len)
{
  ...
  dstsize = encryption_encrypted_length((uint)len, ENCRYPTION_KEY_SYSTEM_DATA, crypto->key_version);
...
int Log_event_writer::write_header(uchar *pos, size_t len)
{
  ...
  */
  ...
  if (ctx)
  {
    ...
    if (encryption_ctx_init(ctx, crypto->key, crypto->key_length, iv, sizeof(iv),
...
bool Log_event::write_header(size_t event_data_length)
{
  ...
  DBUG_PRINT("enter",
             ("filepos: %lld  length: %zu type: %d", (longlong)writer->pos(), event_data_length, (int)get_type_code()));
...
#if defined(HAVE_REPLICATION)
inline Log_event::enum_skip_reason Log_event::continue_group(rpl_group_info *rgi)
{
...
void Query_log_event::pack_info(Protocol *protocol)
{
  ...
  if (flags2 & (OPTION_NO_FOREIGN_KEY_CHECKS | OPTION_AUTO_IS_NULL | OPTION_RELAXED_UNIQUE_CHECKS |
                OPTION_NO_CHECK_CONSTRAINT_CHECKS | OPTION_IF_EXISTS | OPTION_INSERT_HISTORY))
  {
...
*/
static void store_str_with_code_and_len(uchar **dst, const char *src, uint len, uint code)
{
  /*
...
bool Query_log_event::write()
{
  ...
  */
  ...
  {
    store_str_with_code_and_len(&start, catalog, catalog_len, Q_CATALOG_NZ_CODE);
    /*
  ...
  if (time_zone_len)
  {
    ...
    store_str_with_code_and_len(&start, time_zone_str, time_zone_len, Q_TIME_ZONE_CODE);
  }
  ...
  if (gtid_flags_extra)
  {
    ...
    *start++ = gtid_flags_extra;
    if (gtid_flags_extra & (Gtid_log_event::FL_COMMIT_ALTER_E1 | Gtid_log_event::FL_ROLLBACK_ALTER_E1))
    {
  ...
  */
  ...
  return write_header(event_length) || write_data(buf, QUERY_HEADER_LEN) || write_post_header_for_derived() ||
         write_data(start_of_status, (uint)status_vars_len) || write_data(db, db_len + 1) || write_data(query, q_len) ||
         write_footer();
}
...
bool Query_compressed_log_event::write()
{
  ...
  if (buffer && !binlog_buf_compress((uchar *)query, buffer, q_len, &compressed_size))
  {
    /*
...
/**
  ...
  to the log.
*/
Query_log_event::Query_log_event() : Log_event(), data_buf(0)
{
  memset(&user, 0, sizeof(user));
  memset(&host, 0, sizeof(host));
}
...
Query_log_event::Query_log_event(THD *thd_arg, const char *query_arg, size_t query_length, bool using_trans,
    ...
    : Log_event(thd_arg,
                ((thd_arg->used & THD::THREAD_SPECIFIC_USED) ? LOG_EVENT_THREAD_SPECIFIC_F : 0) |
      ...
      sa_seq_no(0)
{
  ...
  */
  flags2 = (uint32)(thd_arg->variables.option_bits & (OPTIONS_WRITTEN_TO_BIN_LOG & ~OPTION_NOT_AUTOCOMMIT));
  ...
  */
  ...
  if (!direct)
  {
    switch (lex->sql_command)
    {
      ...
      case SQLCOM_CREATE_SEQUENCE:
         ...
         */
        trx_cache = (lex->first_select_lex()->item_list.elements && thd->is_current_stmt_binlog_format_row()) ||
                    (thd->variables.option_bits & OPTION_GTID_BEGIN);
        use_cache = (lex->tmp_table() && thd->in_multi_stmt_transaction_mode()) || trx_cache;
        break;
  ...
  DBUG_PRINT("info",
             ("Query_log_event has flags2: %lu  sql_mode: %llu  cache_tye: %d", (ulong)flags2, sql_mode, cache_type));
}

Query_compressed_log_event::Query_compressed_log_event(THD *thd_arg, const char *query_arg, ulong query_length,
                                                       bool using_trans, bool direct, bool suppress_use, int errcode)
    : Query_log_event(thd_arg, query_arg, query_length, using_trans, direct, suppress_use, errcode), query_buf(0)
{
}

#if defined(HAVE_REPLICATION)

int Query_log_event::do_apply_event(rpl_group_info *rgi) { return do_apply_event(rgi, query, q_len); }
...
bool test_if_equal_repl_errors(int expected_error, int actual_error)
{
  ...
  switch (expected_error)
  {
    ...
    case ER_AUTOINC_READ_FAILED:
      return (actual_error == ER_DUP_ENTRY || actual_error == ER_DUP_ENTRY_WITH_KEY_NAME ||
              actual_error == ER_DUP_KEY || actual_error == ER_AUTOINC_READ_FAILED ||
...
static start_alter_info *get_new_start_alter_info(THD *thd)
{
  ...
  if (!(info = (start_alter_info *)my_malloc(PSI_INSTRUMENT_ME, sizeof(start_alter_info), MYF(MY_WME))))
  {
...
/*
  ...
  -1 as error.
*/
int Query_log_event::handle_split_alter_query_log_event(rpl_group_info *rgi, bool &skip_error_check)
{
  int rc = 0;
  ...
  if (gtid_flags_extra & Gtid_log_event::FL_START_ALTER_E1)
  {
    ...
    if (!rgi->rpt || rgi->reserved_start_alter_thread || WSREP(thd))
    {
      ...
      */
      ...
      Write_log_with_flags wlwf(thd, Gtid_log_event::FL_START_ALTER_E1, true /* wsrep to isolation end */);
#ifdef WITH_WSREP
  ...
  if (is_CA)
  {
    DBUG_EXECUTE_IF("rpl_slave_stop_CA_before_binlog", {
      ...
      debug_sync_set_action(thd, STRING_WITH_LEN("now WAIT_FOR proceed_CA_1"));
    });
  }
  ...
  mysql_mutex_lock(&mi->start_alter_list_lock);
  {
    ...
    {
      if (info->sa_seq_no == rgi->gtid_ev_sa_seq_no && info->domain_id == rgi->current_gtid.domain_id)
      {
  ...
  if (info->state != start_alter_state::COMPLETED)
  {
    ...
    */
    ...
    DBUG_ASSERT(info->state == start_alter_state::COMPLETED || !info->direct_commit_alter);

    while (info->state != start_alter_state::COMPLETED) mysql_cond_wait(&info->start_alter_cond, &mi->start_alter_lock);
  }
  else
  {
...
write_binlog:
  ...
  if (!is_CA)
  {
    if (((info && info->error) || error_code) && global_system_variables.log_warnings > 2)
    {
      sql_print_information(
          "Query '%s' having %d error code on master "
          "is rolled back%s",
          query, error_code, !(info && info->error) ? "." : ";");
      if (info && info->error)
        sql_print_information("its execution on slave %sproduced %d error.", info->error == error_code ? "re" : "",
                              info->error);
    }
  }
  {
    thd->lex->sql_command = SQLCOM_ALTER_TABLE;
    Write_log_with_flags wlwf(thd, is_CA ? Gtid_log_event::FL_COMMIT_ALTER_E1 : Gtid_log_event::FL_ROLLBACK_ALTER_E1,
                              true);
#ifdef WITH_WSREP
    if (WSREP(thd) && wsrep_thd_is_local(thd) && wsrep_to_isolation_begin(thd, db, NULL, NULL, NULL, NULL, NULL))
      rc = -1;
#endif
    if (rc != -1 && write_bin_log(thd, false, thd->query(), thd->query_length()))
      rc = -1;
  }
...
*/
int Query_log_event::do_apply_event(rpl_group_info *rgi, const char *query_arg, uint32 q_len_arg)
{
  ...
  */
  ...
  size_t valid_len = Well_formed_prefix(system_charset_info, db, db_len, NAME_LEN).length();

  if (valid_len != db_len)
  {
    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER_THD(thd, ER_SLAVE_FATAL_ERROR),
  ...
  */
  if (is_trans_keyword() || rpl_filter->db_ok(thd->db.str))
  {
    ...
    thd->set_query_and_id((char *)query_arg, q_len_arg, thd->charset(), next_query_id());
    thd->variables.pseudo_thread_id = thread_id;  // for temp tables
    DBUG_PRINT("query", ("%s", thd->query()));

    if (unlikely(!(expected_error = !is_rb_alter ? error_code : 0)) || ignored_error_code(expected_error) ||
        !unexpected_error_code(expected_error))
    {
      ...
      if (flags2_inited)
      {
        ...
        thd->variables.option_bits = (flags2 & mask) | (thd->variables.option_bits & ~mask);
      }
      /*
      ...
      */
      if (sql_mode_inited)
        thd->variables.sql_mode = (sql_mode_t)((thd->variables.sql_mode & MODE_NO_DIR_IN_CREATE) |
      ...
      if (lc_time_names_number)
      {
        if (!(thd->variables.lc_time_names = my_locale_by_number(lc_time_names_number)))
        {
          my_printf_error(ER_UNKNOWN_ERROR, "Unknown locale: '%d'", MYF(0), lc_time_names_number);
      ...
      */
      if (current_stmt_is_commit)
      {
        ...
        if (rgi->gtid_pending)
        {
          ...
          if (unlikely(rpl_global_gtid_slave_state->record_gtid(thd, &gtid, sub_id, true, false, &hton)))
          {
            ...
            if (!is_parallel_retry_error(rgi, errcode))
              rli->report(ERROR_LEVEL, ER_CANNOT_UPDATE_GTID_STATE, rgi->gtid_info(),
                          ...
                          "%s.%s: %d: %s",
                          "mysql", rpl_gtid_slave_state_table_name.str, errcode, thd->get_stmt_da()->message());
            sub_id = 0;
      ...
      */
      if (unlikely(expected_error) && (ignored_error_code(expected_error) || concurrency_error_code(expected_error)))
      {
      ...
      int sa_result = 0;
      bool is_2p_alter = gtid_flags_extra & (Gtid_log_event::FL_START_ALTER_E1 | Gtid_log_event::FL_COMMIT_ALTER_E1 |
      ...
      if (sa_result == 0)
      {
        ...
        if (!parser_state.init(thd, thd->query(), thd->query_length()))
        {
          ...
          thd->m_statement_psi = MYSQL_START_STATEMENT(&thd->m_statement_state, stmt_info_rpl.m_key, thd->db.str,
          ...
          if (thd->slave_thread)
          {
            ...
            */
            thd->variables.sql_log_slow =
                !MY_TEST(global_system_variables.log_slow_disabled_statements & LOG_SLOW_DISABLE_SLAVE);
          }
      ...
      {
        rli->report(ERROR_LEVEL, expected_error, rgi->gtid_info(), "TODO start alter error");
        thd->is_slave_error = 1;
        goto end;
      }
      thd->variables.option_bits &= ~OPTION_MASTER_SQL_ERROR;
...
START SLAVE; . Query: '%s'",
    ...
    if (likely(!thd->is_error()) || thd->get_stmt_da()->sql_errno() != ER_SLAVE_IGNORED_TABLE)
  ...
  compare_errors:
    ...
    */
    if ((thd->lex->sql_command == SQLCOM_DROP_TABLE || thd->lex->sql_command == SQLCOM_DROP_SEQUENCE) &&
        thd->lex->tmp_table() && thd->is_error() && thd->get_stmt_da()->sql_errno() == ER_BAD_TABLE_ERROR &&
        !expected_error)
    ...
    */
    actual_error = thd->is_error() ? thd->get_stmt_da()->sql_errno() : skip_error_check ? expected_error : 0;
    DBUG_PRINT("info", ("expected_error: %d  sql_errno: %d", expected_error, actual_error));

    if ((unlikely(expected_error) && !test_if_equal_repl_errors(expected_error, actual_error) &&
        ...
        !ignored_error_code(actual_error) && !ignored_error_code(expected_error))
    {
      rli->report(ERROR_LEVEL, 0, rgi->gtid_info(),
                  ...
                  actual_error ? thd->get_stmt_da()->message() : "no error", actual_error, print_slave_db_safe(db),
                  query_arg);
      thd->is_slave_error = 1;
    }
    /*
    ...
    */
    else if ((test_if_equal_repl_errors(expected_error, actual_error) && !concurrency_error_code(expected_error)) ||
             ignored_error_code(actual_error))
    {
      DBUG_PRINT("info", ("error ignored"));
      thd->clear_error(1);
      if (actual_error == ER_QUERY_INTERRUPTED || actual_error == ER_CONNECTION_KILLED)
        thd->reset_killed();
    }
    /*
    ...
    else if (unlikely(thd->is_slave_error || thd->is_fatal_error))
    {
      if (!is_parallel_retry_error(rgi, actual_error))
        rli->report(ERROR_LEVEL, actual_error, rgi->gtid_info(),
                    ...
                    (actual_error ? thd->get_stmt_da()->message() : "unexpected success or fatal error"), thd->get_db(),
                    query_arg);
      thd->is_slave_error = 1;
#ifdef WITH_WSREP
...

  {
    ...
    */
    DBUG_EXECUTE_IF(
        "stop_slave_middle_group", if (!current_stmt_is_commit && is_begin() == 0) {
...
Log_event::enum_skip_reason Query_log_event::do_shall_skip(rpl_group_info *rgi)
{
  Relay_log_info *rli = rgi->rli;
  ...
  */
  if (flags & LOG_EVENT_SKIP_REPLICATION_F && opt_replicate_events_marked_for_skip != RPL_SKIP_REPLICATE)
  ...
  if (rli->slave_skip_counter > 0)
  {
    ...
    if (is_commit() || is_rollback())
    {
      ...
      DBUG_RETURN(Log_event::EVENT_SKIP_COUNT);
           ...
           thd->wsrep_mysql_replicated > 0 && (is_begin() || is_commit()))
  {
...
bool Query_log_event::peek_is_commit_rollback(const uchar *event_start, size_t event_len,
                                              enum enum_binlog_checksum_alg checksum_alg)
{
  if (checksum_alg == BINLOG_CHECKSUM_ALG_CRC32)
  {
    ...
    DBUG_ASSERT(checksum_alg == BINLOG_CHECKSUM_ALG_UNDEF || checksum_alg == BINLOG_CHECKSUM_ALG_OFF);
...
Start_log_event_v3::Start_log_event_v3() : Log_event(), created(0), binlog_version(BINLOG_VERSION), dont_set_created(0)
{
...
bool Start_log_event_v3::write()
{
  ...
  return write_header(sizeof(buff)) || write_data(buff, sizeof(buff)) || write_footer();
}
...
int Start_log_event_v3::do_apply_event(rpl_group_info *rgi)
{
  ...
  switch (binlog_version)
  {
    ...
    default:
      ...
      */
      rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER_THD(thd, ER_SLAVE_FATAL_ERROR), "Binlog version not supported");
      DBUG_RETURN(1);
  }
  DBUG_RETURN(error);
}
...
bool Format_description_log_event::write()
{
  ...
  */
  ...
  size_t rec_size = sizeof(buff) + BINLOG_CHECKSUM_ALG_DESC_LEN + number_of_event_types;
  int2store(buff + ST_BINLOG_VER_OFFSET, binlog_version);
...
#endif
  uint8 checksum_byte = (uint8)(need_checksum() ? checksum_alg : BINLOG_CHECKSUM_ALG_OFF);
  /*
  ...
  */
  ...
  ret = write_header(rec_size) || write_data(buff, sizeof(buff)) ||
        write_data(post_header_len, number_of_event_types) || write_data(&checksum_byte, sizeof(checksum_byte)) ||
        write_footer();
  if (no_checksum)
    checksum_alg = BINLOG_CHECKSUM_ALG_OFF;
  return ret;
...
static void check_and_remove_stale_alter(Relay_log_info *rli)
{
  ...
  while ((info = info_iterator++))
  {
    ...
    sql_print_warning(
        "ALTER query started at %u-%u-%llu could not "
        "be completed because of unexpected master server "
        "or its binlog change",
        info->sa_seq_no,  // todo:gtid
        0, 0);
    info_iterator.remove();
    mysql_mutex_lock(&mi->start_alter_lock);
    info->state = start_alter_state::ROLLBACK_ALTER;
...
Log_event::enum_skip_reason Format_description_log_event::do_shall_skip(rpl_group_info *rgi)
{
...
#if defined(HAVE_REPLICATION)
bool Load_log_event::print_query(THD *thd, bool need_db, const char *cs, String *buf, my_off_t *fn_start,
...
bool Load_log_event::write_data_body()
{
  ...
  {
    if (write_data(field_lens, num_fields) || write_data(fields, field_block_len))
      return 1;
  }
  return (write_data(table_name, table_name_len + 1) || write_data(db, db_len + 1) || write_data(fname, fname_len));
}

Load_log_event::Load_log_event(THD *thd_arg, const sql_exchange *ex, const char *db_arg, const char *table_name_arg,
                               List<Item> &fields_arg, bool is_concurrent_arg, enum enum_duplicates handle_dup,
                               bool ignore, bool using_trans)
    : Log_event(thd_arg, (thd_arg->used & THD::THREAD_SPECIFIC_USED) ? LOG_EVENT_THREAD_SPECIFIC_F : 0, using_trans),
      thread_id(thd_arg->thread_id),
      slave_proxy_id((ulong)thd_arg->variables.pseudo_thread_id),
      num_fields(0),
      fields(0),
...
void Load_log_event::set_fields(const char *affected_db, List<Item> &field_list, Name_resolution_context *context)
{
  ...
  for (i = 0; i < num_fields; i++)
  {
    ...
    field_list.push_back(new (thd->mem_root) Item_field(thd, context, Lex_cstring_strlen(affected_db),
                                                        Lex_cstring_strlen(table_name), field_name),
                         thd->mem_root);
    field += field_lens[i] + 1;
  }
}
...
int Load_log_event::do_apply_event(NET *net, rpl_group_info *rgi, bool use_rli_only_for_errors)
{
  Relay_log_info const *rli = rgi->rli;
  Rpl_filter *rpl_filter = rli->mi->rpl_filter;
  ...
  if (rpl_filter->db_ok(thd->db.str))
  {
    ...
    else
    {
      ...
      */
      ...
      set_fields(tables.db.str, field_list, &thd->lex->first_select_lex()->context);
      ...
      */
      ...
      if (thd->open_temporary_tables(&tables) ||
          mysql_load(thd, &ex, &tables, field_list, tmp_list, tmp_list, handle_dup, ignore, net != 0))
        thd->is_slave_error = 1;
      if (thd->cuted_fields)
      {
        ...
        sql_print_warning(
            ...
            (char *)table_name, log_pos, RPL_LOG_NAME, (ulong)thd->cuted_fields, thd->get_db());
      }
      if (net)
...
error:
  ...
  */
  ...
  DBUG_EXECUTE_IF("LOAD_DATA_INFILE_has_fatal_error", thd->is_slave_error = 0; thd->is_fatal_error = 1;);
...
Error '%s' running LOAD DATA INFILE on table '%s'. Default database: '%s'",
  ...
  if (unlikely(thd->is_fatal_error))
  {
    ...
    my_snprintf(buf, sizeof(buf),
                ...
                (char *)table_name, remember_db);

    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, rgi->gtid_info(), ER_THD(thd, ER_SLAVE_FATAL_ERROR), buf);
    DBUG_RETURN(1);
  }
...
Rotate_log_event::Rotate_log_event(const char *new_log_ident_arg, uint ident_len_arg, ulonglong pos_arg, uint flags_arg)
    : Log_event(),
      ...
      flags(flags_arg)
{
  ...
  DBUG_PRINT("enter", ("new_log_ident: %s  pos: %llu  flags: %lu", new_log_ident_arg, pos_arg, (ulong)flags));
...
bool Rotate_log_event::write()
{
  ...
  return (write_header(ROTATE_HEADER_LEN + ident_len) || write_data(buf, ROTATE_HEADER_LEN) ||
          write_data(new_log_ident, (uint)ident_len) || write_footer());
}
...
int Rotate_log_event::do_update_pos(rpl_group_info *rgi)
{
  ...
  DBUG_PRINT("info",
             ("server_id=%lu; ::server_id=%lu", (ulong)this->server_id, (ulong)global_system_variables.server_id));
  ...
  */
  if ((server_id != global_system_variables.server_id || rli->replicate_same_server_id) && !is_relay_log_event() &&
      !rli->is_in_group() && !rgi->is_parallel_exec)
  {
...
Log_event::enum_skip_reason Rotate_log_event::do_shall_skip(rpl_group_info *rgi)
{
...
Log_event::enum_skip_reason Binlog_checkpoint_log_event::do_shall_skip(rpl_group_info *rgi)
{
  enum_skip_reason reason = Log_event::do_shall_skip(rgi);
...
Binlog_checkpoint_log_event::Binlog_checkpoint_log_event(const char *binlog_file_name_arg, uint binlog_file_len_arg)
    : Log_event(),
      binlog_file_name(my_strndup(PSI_INSTRUMENT_ME, binlog_file_name_arg, binlog_file_len_arg, MYF(MY_WME))),
...
bool Binlog_checkpoint_log_event::write()
{
  ...
  return write_header(BINLOG_CHECKPOINT_HEADER_LEN + binlog_file_len) ||
         write_data(buf, BINLOG_CHECKPOINT_HEADER_LEN) || write_data(binlog_file_name, binlog_file_len) ||
         write_footer();
}
...
Gtid_log_event::Gtid_log_event(THD *thd_arg, uint64 seq_no_arg, uint32 domain_id_arg, bool standalone, uint16 flags_arg,
                               bool is_transactional, uint64 commit_id_arg, bool has_xid, bool ro_1pc)
    : Log_event(thd_arg, flags_arg, is_transactional),
      seq_no(seq_no_arg),
      commit_id(commit_id_arg),
      domain_id(domain_id_arg),
      ...
      extra_engines(0)
{
  ...
  if (thd_arg->transaction->stmt.trans_did_wait() || thd_arg->transaction->all.trans_did_wait())
    flags2 |= FL_WAITED;
  if (thd_arg->transaction->stmt.trans_did_ddl() || thd_arg->transaction->stmt.has_created_dropped_temp_table() ||
      thd_arg->transaction->stmt.trans_executed_admin_cmd() || thd_arg->transaction->all.trans_did_ddl() ||
  ...
  if (is_transactional)
  {
    if (xid_state.is_explicit_XA() &&
        (thd->lex->sql_command == SQLCOM_XA_PREPARE || xid_state.get_state_code() == XA_PREPARED))
    {
      DBUG_ASSERT(!(thd->lex->sql_command == SQLCOM_XA_COMMIT && thd->lex->xa_opt == XA_ONE_PHASE));

      flags2 |= thd->lex->sql_command == SQLCOM_XA_PREPARE ? FL_PREPARED_XA : FL_COMPLETED_XA;
      xid.set(xid_state.get_xid());
    }
    /* count non-zero extra recoverable engines; total = extra + 1 */
    if (has_xid)
    {
      DBUG_ASSERT(ha_count_rw_2pc(thd_arg, thd_arg->in_multi_stmt_transaction_mode()));

      extra_engines = ha_count_rw_2pc(thd_arg, thd_arg->in_multi_stmt_transaction_mode()) - 1;
    }
    else if (ro_1pc)
    {
...
*/
bool Gtid_log_event::peek(const uchar *event_start, size_t event_len, enum enum_binlog_checksum_alg checksum_alg,
                          uint32 *domain_id, uint32 *server_id, uint64 *seq_no, uchar *flags2,
...
bool Gtid_log_event::write()
{
  ...
  return write_header(write_len) || write_data(buf, write_len) || write_footer();
}
...
*/
int Gtid_log_event::make_compatible_event(String *packet, bool *need_dummy_event, ulong ev_offset,
                                          enum enum_binlog_checksum_alg checksum_alg)
{
  uchar flags2;
...
void Gtid_log_event::pack_info(Protocol *protocol)
{
  ...
  char *p;
  p = strmov(buf, (flags2 & FL_STANDALONE ? "GTID " : flags2 & FL_PREPARED_XA ? "XA START " : "BEGIN GTID "));
...
int Gtid_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  {
    if (mysql_bin_log.check_strict_gtid_sequence(this->domain_id, this->server_id, this->seq_no))
      return 1;
  }
  ...
  if (flags2 & FL_PREPARED_XA)
  {
    ...
    thd->set_query_and_id(buf_xa, static_cast<uint32>(strlen(buf_xa)), &my_charset_bin, next_query_id());
  ...
  {
    thd->set_query_and_id(gtid_begin_string, sizeof(gtid_begin_string) - 1, &my_charset_bin, next_query_id());
...
int Gtid_log_event::do_update_pos(rpl_group_info *rgi)
{
  rgi->inc_event_relay_log_pos();
  return 0;
}

Log_event::enum_skip_reason Gtid_log_event::do_shall_skip(rpl_group_info *rgi)
{
  Relay_log_info *rli = rgi->rli;
  /*
    An event skipped due to @@skip_replication must not be counted towards the
...
Gtid_list_log_event::Gtid_list_log_event(rpl_binlog_state *gtid_set, uint32 gl_flags_)
    : count(gtid_set->count()), gl_flags(gl_flags_), list(0), sub_id_list(0)
{
  cache_type = EVENT_NO_CACHE;
  /* Failure to allocate memory will be caught by is_valid() returning false. */
  if (count < (1 << 28) &&
      (list = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, count * sizeof(*list) + (count == 0), MYF(MY_WME))))
    gtid_set->get_gtid_list(list, count);
}

Gtid_list_log_event::Gtid_list_log_event(slave_connection_state *gtid_set, uint32 gl_flags_)
    : count(gtid_set->count()), gl_flags(gl_flags_), list(0), sub_id_list(0)
{
  cache_type = EVENT_NO_CACHE;
  /* Failure to allocate memory will be caught by is_valid() returning false. */
      ...
      (list = (rpl_gtid *)my_malloc(PSI_INSTRUMENT_ME, count * sizeof(*list) + (count == 0), MYF(MY_WME))))
  {
    ...
    if (gl_flags & FLAG_IGN_GTIDS)
    {
      ...
      if (!(sub_id_list = (uint64 *)my_malloc(PSI_INSTRUMENT_ME, count * sizeof(uint64), MYF(MY_WME))))
      {
        my_free(list);
        list = NULL;
        return;
      ...
      for (i = 0; i < count; ++i)
      {
        if (!(sub_id_list[i] = rpl_global_gtid_slave_state->next_sub_id(list[i].domain_id)))
        {
          my_free(list);
          my_free(sub_id_list);
          list = NULL;
...
bool Gtid_list_log_event::write()
{
  char buf[128];
  ...
  if (to_packet(&packet))
    return true;
  return write_header(get_data_size()) || write_data(packet.ptr(), packet.length()) || write_footer();
}

int Gtid_list_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  if (gl_flags & FLAG_IGN_GTIDS)
  {
    ...
    for (i = 0; i < count; ++i)
    {
      if ((ret = rpl_global_gtid_slave_state->record_gtid(thd, &list[i], sub_id_list[i], false, false, &hton)))
        return ret;
      rpl_global_gtid_slave_state->update_state_hash(sub_id_list[i], &list[i], hton, NULL);
    }
  }
  ret = Log_event::do_apply_event(rgi);
  if (rli->until_condition == Relay_log_info::UNTIL_GTID && (gl_flags & FLAG_UNTIL_REACHED))
  {
    char str_buf[128];
...
Log_event::enum_skip_reason Gtid_list_log_event::do_shall_skip(rpl_group_info *rgi)
{
  enum_skip_reason reason = Log_event::do_shall_skip(rgi);
  if (reason == EVENT_SKIP_COUNT)
    reason = EVENT_SKIP_NOT;
...
void Gtid_list_log_event::pack_info(Protocol *protocol)
{
  char buf_mem[1024];
  String buf(buf_mem, sizeof(buf_mem), system_charset_info);
  uint32 i;
  ...
  */
  ...
  buf.append(STRING_WITH_LEN("["));
  first = true;
  for (i = 0; i < count; ++i) rpl_slave_state_tostring_helper(&buf, &list[i], &first);
  buf.append(STRING_WITH_LEN("]"));
...
bool Intvar_log_event::write()
{
  ...
  return write_header(sizeof(buf)) || write_data(buf, sizeof(buf)) || write_footer();
}
...
Log_event::enum_skip_reason Intvar_log_event::do_shall_skip(rpl_group_info *rgi)
{
  /*
...
Log_event::enum_skip_reason Rand_log_event::do_shall_skip(rpl_group_info *rgi)
{
  /*
    It is a common error to set the slave skip counter to 1 instead of
    2 when recovering from an insert which used a auto increment,
...
int Xid_apply_log_event::do_record_gtid(THD *thd, rpl_group_info *rgi, bool in_trans, void **out_hton)
{
  int err = 0;
  Relay_log_info const *rli = rgi->rli;

  rgi->gtid_pending = false;
  err = rpl_global_gtid_slave_state->record_gtid(thd, &rgi->current_gtid, rgi->gtid_sub_id, in_trans, false, out_hton);
    ...
    */
    if (!is_parallel_retry_error(rgi, ec))
      rli->report(ERROR_LEVEL, ER_CANNOT_UPDATE_GTID_STATE, rgi->gtid_info(),
                  ...
                  "%s.%s: %d: %s",
                  "mysql", rpl_gtid_slave_state_table_name.str, ec, thd->get_stmt_da()->message());
    thd->is_slave_error = 1;
  }
...
int Xid_apply_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  */
  enum enum_sql_command cmd = !thd->transaction->xid_state.is_explicit_XA() ? SQLCOM_COMMIT : SQLCOM_XA_PREPARE;
  status_var_increment(thd->status_var.com_stat[cmd]);

  return res;
}

Log_event::enum_skip_reason Xid_apply_log_event::do_shall_skip(rpl_group_info *rgi)
{
...
#ifdef WITH_WSREP
  else if (wsrep_mysql_replication_bundle && WSREP(thd) && opt_slave_domain_parallel_threads == 0)
  {
    if (++thd->wsrep_mysql_replicated < (int)wsrep_mysql_replication_bundle)
    {
      WSREP_DEBUG("skipping wsrep commit %d", thd->wsrep_mysql_replicated);
...
bool Xid_log_event::write()
{
  ...
  return write_header(sizeof(xid)) || write_data((uchar *)&xid, sizeof(xid)) || write_footer();
}
...
void XA_prepare_log_event::pack_info(Protocol *protocol)
{
  ...
  sprintf(query, (one_phase ? "XA COMMIT %s ONE PHASE" : "XA PREPARE %s"), m_xid.serialize());
...
int XA_prepare_log_event::do_commit()
{
  int res;
  xid_t xid;
  xid.set(m_xid.formatID, m_xid.data, m_xid.gtrid_length, m_xid.data + m_xid.gtrid_length, m_xid.bqual_length);
...
bool XA_prepare_log_event::write()
{
  ...
  return write_header(sizeof(one_phase_byte) + xid_subheader_no_data + static_cast<XID *>(xid)->gtrid_length +
                      static_cast<XID *>(xid)->bqual_length) ||
         write_data(data, sizeof(data)) ||
         write_data((uchar *)static_cast<XID *>(xid)->data,
                    static_cast<XID *>(xid)->gtrid_length + static_cast<XID *>(xid)->bqual_length) ||
         write_footer();
}
...
#if defined(HAVE_REPLICATION)
static bool user_var_append_name_part(THD *thd, String *buf, const char *name, size_t name_len)
{
  return buf->append('@') || append_identifier(thd, buf, name, name_len) || buf->append('=');
}
...
void User_var_log_event::pack_info(Protocol *protocol)
{
  if (is_null)
  {
    ...
    String buf(buf_mem, sizeof(buf_mem), system_charset_info);
    buf.length(0);
    if (user_var_append_name_part(protocol->thd, &buf, name, name_len) || buf.append(NULL_clex_str))
      return;
    ...
    switch (type)
    {
      case REAL_RESULT:
      {
        ...
        if (user_var_append_name_part(protocol->thd, &buf, name, name_len) ||
            buf.append(buf2, my_gcvt(real_val, MY_GCVT_ARG_DOUBLE, MY_GCVT_MAX_FIELD_WIDTH, buf2, NULL)))
          return;
        protocol->store(buf.ptr(), buf.length(), &my_charset_bin);
        break;
      }
      case INT_RESULT:
      {
        ...
        if (user_var_append_name_part(protocol->thd, &buf, name, name_len) ||
            buf.append(
                buf2,
                longlong10_to_str(uint8korr(val), buf2, ((flags & User_var_log_event::UNSIGNED_F) ? 10 : -10)) - buf2))
          return;
        protocol->store(buf.ptr(), buf.length(), &my_charset_bin);
        break;
      }
      case DECIMAL_RESULT:
      {
        ...
        if (user_var_append_name_part(protocol->thd, &buf, name, name_len) || buf.append(str))
          return;
        protocol->store(buf.ptr(), buf.length(), &my_charset_bin);
      ...
      case STRING_RESULT:
      {
        ...
        char buf_mem[FN_REFLEN + 512 + 1 + 15 + MY_CS_CHARACTER_SET_NAME_SIZE + MY_CS_COLLATION_NAME_SIZE];
        String buf(buf_mem, sizeof(buf_mem), system_charset_info);
        CHARSET_INFO *cs;
        buf.length(0);
        ...
        else
        {
          size_t old_len;
          char *beg, *end;
          if (user_var_append_name_part(protocol->thd, &buf, name, name_len) || buf.append('_') ||
              buf.append(cs->cs_name) || buf.append(' '))
            return;
          old_len = buf.length();
          if (buf.reserve(old_len + val_len * 2 + 3 + sizeof(" COLLATE ") + MY_CS_COLLATION_NAME_SIZE))
            return;
          beg = const_cast<char *>(buf.ptr()) + old_len;
          end = str_to_hex(beg, val, val_len);
          buf.length(old_len + (end - beg));
          if (buf.append(STRING_WITH_LEN(" COLLATE ")) || buf.append(cs->coll_name))
            return;
        }
        protocol->store(buf.ptr(), buf.length(), &my_charset_bin);
        break;
...
bool User_var_log_event::write()
{
  ...
  return write_header(event_length) || write_data(buf, sizeof(buf)) || write_data(name, name_len) ||
         write_data(buf1, buf1_length) || write_data(pos, val_len) || write_data(&flags, unsigned_len) ||
         write_footer();
}
...
int User_var_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  {
    rgi->rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER_THD(thd, ER_SLAVE_FATAL_ERROR),
  ...
  */
   ...
   */
  e->update_hash((void *)val, val_len, type, charset, (flags & User_var_log_event::UNSIGNED_F));
  if (!is_deferred())
...
Log_event::enum_skip_reason User_var_log_event::do_shall_skip(rpl_group_info *rgi)
{
  /*
...
Create_file_log_event::Create_file_log_event(THD *thd_arg, sql_exchange *ex, const char *db_arg,
                                             const char *table_name_arg, List<Item> &fields_arg, bool is_concurrent_arg,
                                             enum enum_duplicates handle_dup, bool ignore, uchar *block_arg,
                                             uint block_len_arg, bool using_trans)
    : Load_log_event(thd_arg, ex, db_arg, table_name_arg, fields_arg, is_concurrent_arg, handle_dup, ignore,
                     using_trans),
      fake_base(0),
      block(block_arg),
      event_buf(0),
...
bool Create_file_log_event::write_data_body()
{
  ...
  if ((res = Load_log_event::write_data_body()) || fake_base)
    return res;
  return write_data("", 1) || write_data(block, block_len);
}
...
int Create_file_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  ext = slave_load_file_stem(fname_buf, file_id, server_id, ".info", &rli->mi->connection_name);
  /* old copy may exist already */
  mysql_file_delete(key_file_log_event_info, fname_buf, MYF(0));
  if ((fd = mysql_file_create(key_file_log_event_info, fname_buf, CREATE_MODE,
                              O_WRONLY | O_BINARY | O_EXCL | O_NOFOLLOW, MYF(MY_WME))) < 0 ||
      init_io_cache(&file, fd, IO_SIZE, WRITE_CACHE, (my_off_t)0, 0, MYF(MY_WME | MY_NABP)))
  {
    rli->report(ERROR_LEVEL, my_errno, rgi->gtid_info(), "Error in Create_file event: could not open file '%s'",
                fname_buf);
    goto err;
  }
  ...
  if (write_base())
  {
    ...
    rli->report(ERROR_LEVEL, my_errno, rgi->gtid_info(), "Error in Create_file event: could not write to file '%s'",
                fname_buf);
    goto err;
  }
  end_io_cache(&file);
  ...
  {
    rli->report(ERROR_LEVEL, my_errno, rgi->gtid_info(), "Error in Create_file event: write to '%s' failed", fname_buf);
    goto err;
  }
...
Append_block_log_event::Append_block_log_event(THD *thd_arg, const char *db_arg, uchar *block_arg, uint block_len_arg,
                                               bool using_trans)
...
bool Append_block_log_event::write()
{
  ...
  return write_header(APPEND_BLOCK_HEADER_LEN + block_len) || write_data(buf, APPEND_BLOCK_HEADER_LEN) ||
         write_data(block, block_len) || write_footer();
}
...
int Append_block_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  slave_load_file_stem(fname, file_id, server_id, ".data", &rli->mi->cmp_connection_name);
  if (get_create_or_append())
  {
    ...
    */
    ...
    if ((fd = mysql_file_create(key_file_log_event_data, fname, CREATE_MODE, O_WRONLY | O_BINARY | O_EXCL | O_NOFOLLOW,
                                MYF(MY_WME))) < 0)
    {
      rli->report(ERROR_LEVEL, my_errno, rgi->gtid_info(), "Error in %s event: could not create file '%s'",
                  get_type_str(), fname);
      goto err;
    }
  }
  else if ((fd = mysql_file_open(key_file_log_event_data, fname, O_WRONLY | O_APPEND | O_BINARY | O_NOFOLLOW,
                                 MYF(MY_WME))) < 0)
  {
    rli->report(ERROR_LEVEL, my_errno, rgi->gtid_info(), "Error in %s event: could not open file '%s'", get_type_str(),
                fname);
    goto err;
  }

  DBUG_EXECUTE_IF("remove_slave_load_file_before_write", { my_delete(fname, MYF(0)); });

  if (mysql_file_write(fd, (uchar *)block, block_len, MYF(MY_WME + MY_NABP)))
  {
    rli->report(ERROR_LEVEL, my_errno, rgi->gtid_info(), "Error in %s event: write to '%s' failed", get_type_str(),
                fname);
    goto err;
  }
  error = 0;
...
int Delete_file_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  Relay_log_info const *rli = rgi->rli;
  char *ext = slave_load_file_stem(fname, file_id, server_id, ".data", &rli->mi->cmp_connection_name);
...
int Execute_load_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  ext = slave_load_file_stem(fname, file_id, server_id, ".info", &rli->mi->cmp_connection_name);
  if ((fd = mysql_file_open(key_file_log_event_info, fname, O_RDONLY | O_BINARY | O_NOFOLLOW, MYF(MY_WME))) < 0 ||
      init_io_cache(&file, fd, IO_SIZE, READ_CACHE, (my_off_t)0, 0, MYF(MY_WME | MY_NABP)))
  {
    rli->report(ERROR_LEVEL, my_errno, rgi->gtid_info(), "Error in Exec_load event: could not open file '%s'", fname);
    goto err;
  }
  if (!(lev = (Load_log_event *)Log_event::read_log_event(&file, rli->relay_log.description_event_for_exec,
  ...
  */
  ...
  if (lev->do_apply_event(0, rgi, 1))
  {
    ...
    */
    ...
    if (tmp)
    {
      rli->report(ERROR_LEVEL, rli->last_error().number, rgi->gtid_info(), "%s. Failed executing load from '%s'", tmp,
                  fname);
      my_free(tmp);
    }
    goto err;
...
Begin_load_query_log_event::Begin_load_query_log_event(THD *thd_arg, const char *db_arg, uchar *block_arg,
                                                       uint block_len_arg, bool using_trans)
...
Log_event::enum_skip_reason Begin_load_query_log_event::do_shall_skip(rpl_group_info *rgi)
{
  /*
...
Execute_load_query_log_event::Execute_load_query_log_event(THD *thd_arg, const char *query_arg, ulong query_length_arg,
                                                           ...
                                                           bool direct, bool suppress_use, int errcode)
    : Query_log_event(thd_arg, query_arg, query_length_arg, using_trans, direct, suppress_use, errcode),
      file_id(thd_arg->file_id),
...
void Execute_load_query_log_event::pack_info(Protocol *protocol)
{
  ...
  if (db && db_len)
  {
    if (buf.append(STRING_WITH_LEN("use ")) || append_identifier(protocol->thd, &buf, db, db_len) ||
        buf.append(STRING_WITH_LEN("; ")))
      return;
  }
  if (query && q_len && buf.append(query, q_len))
    return;
  if (buf.append(STRING_WITH_LEN(" ;file_id=")) || buf.append_ulonglong(file_id))
    return;
  protocol->store(buf.ptr(), buf.length(), &my_charset_bin);
}

int Execute_load_query_log_event::do_apply_event(rpl_group_info *rgi)
{
  char *p;
  char *buf;
  char *fname;
  ...
  buf = (char *)my_malloc(PSI_INSTRUMENT_ME, q_len + 1 - (fn_pos_end - fn_pos_start) + (FN_REFLEN + 10) + 10 + 8 + 5,
                          MYF(MY_WME));
  ...
  if (buf == NULL)
  {
    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, rgi->gtid_info(), ER_THD(rgi->thd, ER_SLAVE_FATAL_ERROR),
                "Not enough memory");
    return 1;
  }
  ...
  p = slave_load_file_stem(p, file_id, server_id, ".data", &rli->mi->cmp_connection_name);
...
static bool write_str(Log_event_writer *writer, const char *str, uint length)
{
  uchar tmp[1];
  tmp[0] = (uchar)length;
  return (writer->write_data(tmp, sizeof(tmp)) || writer->write_data((uchar *)str, length));
}
...
bool sql_ex_info::write_data(Log_event_writer *writer)
{
  if (new_format())
  {
    return write_str(writer, field_term, field_term_len) || write_str(writer, enclosed, enclosed_len) ||
           write_str(writer, line_term, line_term_len) || write_str(writer, line_start, line_start_len) ||
           write_str(writer, escaped, escaped_len) || writer->write_data((uchar *)&opt_flags, 1);
  }
  else
  {
    uchar old_ex[7];
...
Rows_log_event::Rows_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid, MY_BITMAP const *cols, bool is_transactional,
...
#endif
{
   ...
   */
  DBUG_ASSERT((tbl_arg && tbl_arg->s && tid != ~0UL) || (!tbl_arg && !cols && tid == ~0UL));
  ...
  if (likely(!my_bitmap_init(&m_cols, m_width <= sizeof(m_bitbuf) * 8 ? m_bitbuf : NULL, m_width)))
  {
...
int Rows_log_event::do_add_row_data(uchar *row_data, size_t length)
{
  ...
  */
  ...
  if (static_cast<size_t>(m_rows_end - m_rows_cur) <= length)
  {
    ...
    DBUG_EXECUTE_IF("simulate_too_big_row_case1", cur_size = UINT_MAX32 - (block_size * 10);
                    length = UINT_MAX32 - (block_size * 10););
    DBUG_EXECUTE_IF("simulate_too_big_row_case2", cur_size = UINT_MAX32 - (block_size * 10); length = block_size * 10;);
    DBUG_EXECUTE_IF("simulate_too_big_row_case3", cur_size = block_size * 10; length = UINT_MAX32 - (block_size * 10););
    DBUG_EXECUTE_IF("simulate_too_big_row_case4", cur_size = UINT_MAX32 - (block_size * 10);
                    length = (block_size * 10) - block_size + 1;);
    size_t remaining_space = UINT_MAX32 - cur_size;
    /* Check that the new data fits within remaining space and we can add
       block_size without wrapping.
     */
    if (cur_size > UINT_MAX32 || length > remaining_space || ((length + block_size) > remaining_space))
    {
      sql_print_error(
    ...
    size_t const new_alloc = block_size * ((cur_size + length + block_size - 1) / block_size);

    uchar *const new_buf =
        (uchar *)my_realloc(PSI_INSTRUMENT_ME, m_rows_buf, new_alloc, MYF(MY_ALLOW_ZERO_PTR | MY_WME));
    if (unlikely(!new_buf))
  ...
  */
  if (!thd->lock)
  {
    ...
    */
    ...
    for (TABLE_LIST *tables = rgi->tables_to_lock; tables; tables = tables->next_global)
    {
    ...
    if (unlikely(open_and_lock_tables(thd, rgi->tables_to_lock, FALSE, 0)))
    {
      ...
      if (WSREP(thd))
      {
        WSREP_WARN(
            ...
            thd->get_stmt_da()->sql_errno(), thd->is_fatal_error, thd->wsrep_cs().mode(), thd->wsrep_trx().state(),
            (long long)wsrep_thd_trx_seqno(thd));
      }
#endif /* WITH_WSREP */
      if (thd->is_error() && !is_parallel_retry_error(rgi, error = thd->get_stmt_da()->sql_errno()))
      {
        /*
        ...
        */
        rli->report(ERROR_LEVEL, error, rgi->gtid_info(), "Error executing row event: '%s'",
                    (error ? thd->get_stmt_da()->message() : "unexpected success or fatal error"));
        thd->is_slave_error = 1;
      }
    ...
    */
    ...
    {
      DBUG_PRINT("debug", ("Checking compatibility of tables to lock - tables_to_lock: %p", rgi->tables_to_lock));
           ...
           table_list_ptr = table_list_ptr->next_global, i++)
      {
        ...
        */
        ...
        {
          DBUG_PRINT("debug", ("Table: %s.%s is not compatible with master", ptr->table->s->db.str,
        ...
        DBUG_PRINT("debug", ("Table: %s.%s is compatible with master"
                             " - conv_table: %p",
                             ptr->table->s->db.str, ptr->table->s->table_name.str, conv_table));
    ...
    for (uint i = 0; ptr && (i < rgi->tables_to_lock_count); ptr = ptr->next_global, i++)
    {
      ...
      */
      ...
      {
        ptr->table->master_had_triggers = ((RPL_TABLE_LIST *)ptr)->master_had_triggers;
      }
    }
  ...
  DBUG_PRINT("debug", ("m_table:%p, m_table_id: %llu%s", m_table, m_table_id,
  ...
  if (table)
  {
    ...
    */
    ...
    if (get_general_type_code() == DELETE_ROWS_EVENT || get_general_type_code() == UPDATE_ROWS_EVENT)
    ...
    MY_BITMAP *after_image = ((get_general_type_code() == UPDATE_ROWS_EVENT) ? &m_cols_ai : &m_cols);
    ...
    */
    ...
    do
    {
      ...
      if (unlikely(error))
      {
        ...
        bool idempotent_error = (idempotent_error_code(error) && (slave_exec_mode == SLAVE_EXEC_MODE_IDEMPOTENT));
        bool ignored_error = (idempotent_error == 0 ? ignored_error_code(actual_error) : 0);
        ...
        {
          if (global_system_variables.log_warnings)
            slave_rows_error_report(WARNING_LEVEL, error, rgi, thd, table, get_type_str(), RPL_LOG_NAME, log_pos);
          thd->clear_error(1);
          error = 0;
      ...
      */
      ...
      DBUG_PRINT("info", ("curr_row: %p; curr_row_end: %p; rows_end:%p", m_curr_row, m_curr_row_end, m_rows_end));
    ...
    { /**
      ...
      */
      DBUG_EXECUTE_IF(
          ...
          if (thd->transaction->all.modified_non_trans_table) const_cast<Relay_log_info *>(rli)->abort_slave = 1;);
    }
  ...
  if (unlikely(error))
  {
    slave_rows_error_report(ERROR_LEVEL, error, rgi, thd, table, get_type_str(), RPL_LOG_NAME, log_pos);
    /*
  ...
  if (get_flags(STMT_END_F))
  {
    if (unlikely((error = rows_event_stmt_cleanup(rgi, thd))))
      slave_rows_error_report(ERROR_LEVEL, thd->is_error() ? 0 : error, rgi, thd, table, get_type_str(), RPL_LOG_NAME,
                              log_pos);
    if (thd->slave_thread)
      free_root(thd->mem_root, MYF(MY_KEEP_PREALLOC));
  }
...
Log_event::enum_skip_reason Rows_log_event::do_shall_skip(rpl_group_info *rgi)
{
  /*
 ...
 */
int Rows_log_event::do_update_pos(rpl_group_info *rgi)
{
  Relay_log_info *rli = rgi->rli;
  int error = 0;
...
bool Rows_log_event::write_data_body()
{
  ...
  */
  ...
  {
    DBUG_DUMP("m_cols_ai", (uchar *)m_cols_ai.bitmap, no_bytes_in_map(&m_cols_ai));
    res = res || write_data((uchar *)m_cols_ai.bitmap, no_bytes_in_map(&m_cols_ai));
  }
...
bool Rows_log_event::write_compressed()
{
  ...
  if (m_rows_buf &&
      !binlog_buf_compress(m_rows_buf_tmp, m_rows_buf, (uint32)(m_rows_cur_tmp - m_rows_buf_tmp), &comlen))
  {
...
void Rows_log_event::pack_info(Protocol *protocol)
{
  ...
  size_t bytes = my_snprintf(buf, sizeof(buf), "table_id: %llu%s", m_table_id, flagstr);
...
Annotate_rows_log_event::Annotate_rows_log_event(THD *thd, bool using_trans, bool direct)
...
bool Annotate_rows_log_event::write_data_header() { return 0; }

bool Annotate_rows_log_event::write_data_body() { return write_data(m_query_txt, m_query_len); }
...
#if defined(HAVE_REPLICATION)
Log_event::enum_skip_reason Annotate_rows_log_event::do_shall_skip(rpl_group_info *rgi) { return continue_group(rgi); }
#endif
 ...
 */
Table_map_log_event::Table_map_log_event(THD *thd, TABLE *tbl, ulong tid, bool is_transactional)
      ...
      m_optional_metadata(NULL)
{
  ...
  */
  DBUG_ASSERT((tbl->s->db.str == 0) || (tbl->s->db.str[tbl->s->db.length] == 0));
  DBUG_ASSERT(tbl->s->table_name.str[tbl->s->table_name.length] == 0);

  binlog_type_info_array = (Binlog_type_info *)thd->alloc(m_table->s->fields * sizeof(Binlog_type_info));
  for (uint i = 0; i < m_table->s->fields; i++) binlog_type_info_array[i] = m_table->field[i]->binlog_type_info();
  ...
  if ((m_memory = (uchar *)my_malloc(PSI_INSTRUMENT_ME, m_colcnt, MYF(MY_WME))))
  {
    ...
    for (unsigned int i = 0; i < m_table->s->fields; ++i) m_coltype[i] = binlog_type_info_array[i].m_type_code;
  ...
  */
  ...
  m_meta_memory = (uchar *)my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &m_null_bits, num_null_bytes,
                                           &m_field_metadata, (m_colcnt * 2), NULL);
...
/*
  ...
  @return OK_TO_PROCESS
          ...
          SAME_ID_MAPPING_SAME_TABLE
            ...
            rli->tables_to_lock.
*/
static enum_tbl_map_status check_table_map(rpl_group_info *rgi, RPL_TABLE_LIST *table_list)
{
  ...
  else
  {
    ...
    for (uint i = 0; ptr && (i < rgi->tables_to_lock_count); ptr = static_cast<RPL_TABLE_LIST *>(ptr->next_local), i++)
    {
      if (ptr->table_id == table_list->table_id)
      {
        if (cmp(&ptr->db, &table_list->db) || cmp(&ptr->alias, &table_list->table_name) ||
...
int Table_map_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  if (!(memory = my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &table_list, (uint)sizeof(RPL_TABLE_LIST), &db_mem,
                                 (uint)NAME_LEN + 1, &tname_mem, (uint)NAME_LEN + 1, NullS)))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  DBUG_PRINT("debug", ("table: %s is mapped to %llu", table_list->table_name.str, table_list->table_id));
  table_list->master_had_triggers = ((m_flags & TM_BIT_HAS_TRIGGERS_F) ? 1 : 0);
  DBUG_PRINT("debug", ("table->master_had_triggers=%d", (int)table_list->master_had_triggers));
  ...
  if (tblmap_status == OK_TO_PROCESS)
  {
    ...
    */
    new (&table_list->m_tabledef)
        table_def(m_coltype, m_colcnt, m_field_metadata, m_field_metadata_size, m_null_bits, m_flags);
...
Log_event::enum_skip_reason Table_map_log_event::do_shall_skip(rpl_group_info *rgi)
{
  /*
    If the slave skip counter is 1, then we should not start executing
    on the next event.
...
bool Table_map_log_event::write_data_body()
{
  ...
  */
  ...
  return write_data(dbuf, sizeof(dbuf)) || write_data(m_dbnam, m_dblen + 1) || write_data(tbuf, sizeof(tbuf)) ||
             write_data(m_tblnam, m_tbllen + 1) || write_data(cbuf, (size_t)(cbuf_end - cbuf)) ||
             write_data(m_coltype, m_colcnt) || write_data(mbuf, (size_t)(mbuf_end - mbuf)) ||
             write_data(m_field_metadata, m_field_metadata_size),
         write_data(m_null_bits, (m_colcnt + 7) / 8) ||
             write_data((const uchar *)m_metadata_buf.ptr(), m_metadata_buf.length());
}
...
*/
static inline bool write_tlv_field(String &str_buf, enum Table_map_log_event::Optional_metadata_field_type type,
                                   uint length, const uchar *value)
{
...
static inline bool write_tlv_field(String &str_buf, enum Table_map_log_event::Optional_metadata_field_type type,
                                   const String &value)
{
  return write_tlv_field(str_buf, type, value.length(), reinterpret_cast<const uchar *>(value.ptr()));
}
...
void Table_map_log_event::init_metadata_fields()
{
  ...
  if (binlog_row_metadata == BINLOG_ROW_METADATA_NO_LOG)
    DBUG_VOID_RETURN;
  if (init_signedness_field() || init_charset_field(&is_character_field, DEFAULT_CHARSET, COLUMN_CHARSET) ||
  ...
  if (binlog_row_metadata == BINLOG_ROW_METADATA_FULL)
  {
    if ((!DBUG_IF("dont_log_column_name") && init_column_name_field()) ||
        ...
        init_set_str_value_field() || init_enum_str_value_field() || init_primary_key_field())
      m_metadata_buf.length(0);
  }
  DBUG_VOID_RETURN;
}
...
bool Table_map_log_event::init_charset_field(bool (*include_type)(Binlog_type_info *, Field *),
                                             ...
                                             Optional_metadata_field_type column_charset_type)
{
  ...
  for (std::map<uint, uint>::iterator it = collation_map.begin(); it != collation_map.end(); it++)
  {
...
{
  char buf[256];
  size_t bytes = my_snprintf(buf, sizeof(buf), "table_id: %llu (%s.%s)", m_table_id, m_dbnam, m_tblnam);
  protocol->store(buf, bytes, &my_charset_bin);
}
#endif
...
/*
  Constructor used to build an event for writing to the binary log.
 */
Write_rows_log_event::Write_rows_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid_arg, bool is_transactional)
    : Rows_log_event(thd_arg, tbl_arg, tid_arg, tbl_arg->rpl_write_set, is_transactional, WRITE_ROWS_EVENT_V1)
{
}

Write_rows_compressed_log_event::Write_rows_compressed_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid_arg,
...
bool Write_rows_compressed_log_event::write() { return Rows_log_event::write_compressed(); }
...
int Write_rows_log_event::do_after_row_operations(const Slave_reporting_capability *const, int error)
{
  int local_error = 0;
...
bool Rows_log_event::process_triggers(trg_event_type event, trg_action_time_type time_type, bool old_row_is_record1)
{
  bool result;
  ...
  {
    result = m_table->triggers->process_triggers(thd, event, time_type, old_row_is_record1);
  }
  else
    result = m_table->triggers->process_triggers(thd, event, time_type, old_row_is_record1);
...
int Rows_log_event::write_row(rpl_group_info *rgi, const bool overwrite)
{
  ...
  */
  ...
  if (invoke_triggers && unlikely(process_triggers(TRG_EVENT_INSERT, TRG_ACTION_BEFORE, TRUE)))
  {
  ...
  if (table->s->sequence)
    ...
    while (unlikely(error = table->file->ha_write_row(table->record[0])))
    {
      ...
      else
      {
        ...
        error = table->file->ha_index_read_idx_map(table->record[1], keynum, (const uchar *)key.get(), HA_WHOLE_KEY,
                                                   HA_READ_KEY_EXACT);
        if (unlikely(error))
        {
      ...
      */
       ...
       */
      if (last_uniq_key(table, keynum) && !invoke_triggers && !table->file->referenced_by_foreign_key())
      {
      ...
      else
      {
        ...
        if (invoke_triggers && unlikely(process_triggers(TRG_EVENT_DELETE, TRG_ACTION_BEFORE, TRUE)))
        ...
        else
        {
          ...
          if (invoke_triggers && unlikely(process_triggers(TRG_EVENT_DELETE, TRG_ACTION_AFTER, TRUE)))
            DBUG_RETURN(HA_ERR_GENERIC);  // in case if error is not set yet
        }
  ...
  if (invoke_triggers && unlikely(process_triggers(TRG_EVENT_INSERT, TRG_ACTION_AFTER, TRUE)))
    error = HA_ERR_GENERIC;  // in case if error is not set yet
...
int Write_rows_log_event::do_exec_row(rpl_group_info *rgi)
{
  ...
  my_snprintf(msg, sizeof(msg), "Write_rows_log_event::write_row() on table %c%s%c", quote_char, table_name,
              quote_char);
  ...
  my_snprintf(thd->wsrep_info, sizeof(thd->wsrep_info) - 1, "Write_rows_log_event::write_row(%lld) on table %c%s%c",
...
{
  return trg2bit(TRG_EVENT_INSERT) | trg2bit(TRG_EVENT_UPDATE) | trg2bit(TRG_EVENT_DELETE);
}
#endif
...
static bool record_compare(TABLE *table)
{
  ...
  /**
    ...
    */
  if ((table->s->blob_fields + table->s->varchar_fields + table->s->null_fields) == 0)
  {
  ...
  */
  for (i = 0, key = m_table->key_info; i < m_table->s->keys; i++, key++)
  {
    ...
    */
    ...
    DBUG_PRINT("info", ("Index %s rec_per_key[%u]= %lu", key->name.str, last_part, key->rec_per_key[last_part]));
...
*/
static inline void issue_long_find_row_warning(Log_event_type type, const char *table_name, bool is_index_scan,
                                               rpl_group_info *rgi)
{
  if ((global_system_variables.log_warnings > 1 && !rgi->is_long_find_row_note_printed()))
  {
    ulonglong now = microsecond_interval_timer();
    ulonglong stmt_ts = rgi->get_row_stmt_start_timestamp();

    DBUG_EXECUTE_IF("inject_long_find_row_note", stmt_ts -= (LONG_FIND_ROW_THRESHOLD * 2 * HRTIME_RESOLUTION););
...
{
  return rgi->speculation != rpl_group_info::SPECULATE_OPTIMISTIC ? HA_ERR_KEY_NOT_FOUND : HA_ERR_RECORD_CHANGED;
}
...
int Rows_log_event::find_row(rpl_group_info *rgi)
{
  ...
  */
  ...
  if ((table->file->ha_table_flags() & HA_PRIMARY_KEY_REQUIRED_FOR_POSITION) && table->s->primary_key < MAX_KEY)
  {
    /*
  ...
  if (m_key_info)
  {
    DBUG_PRINT("info", ("locating record using key #%u [%s] (index_read)", m_key_nr, m_key_info->name.str));
    /* We use this to test that the correct key is used in test cases. */
    DBUG_EXECUTE_IF("slave_crash_if_wrong_index", if (0 != strcmp(m_key_info->name.str, "expected_key")) abort(););

    /* The key is active: search the table using the index */
    if (!table->file->inited && (error = table->file->ha_index_init(m_key_nr, FALSE)))
    {
    ...
    */
    if (table->s->null_bytes > 0)
      table->record[0][table->s->null_bytes - 1] |= 256U - (1U << table->s->last_null_bit_pos);

    if (unlikely((error = table->file->ha_index_read_map(table->record[0], m_key, HA_WHOLE_KEY, HA_READ_KEY_EXACT))))
    {
  ...
  if (is_table_scan || is_index_scan)
    issue_long_find_row_warning(get_general_type_code(), m_table->alias.c_ptr(), is_index_scan, rgi);
  DBUG_RETURN(error);
}
...
Delete_rows_log_event::Delete_rows_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid, bool is_transactional)
...
bool Delete_rows_compressed_log_event::write() { return Rows_log_event::write_compressed(); }
...
int Delete_rows_log_event::do_before_row_operations(const Slave_reporting_capability *const)
{
  ...
  if ((m_table->file->ha_table_flags() & HA_PRIMARY_KEY_REQUIRED_FOR_POSITION) && m_table->s->primary_key < MAX_KEY)
  {
    /*
...
int Delete_rows_log_event::do_after_row_operations(const Slave_reporting_capability *const, int error)
{
...
int Delete_rows_log_event::do_exec_row(rpl_group_info *rgi)
{
  ...
  const char *table_name = m_table->s->table_name.str;
  char quote_char = get_quote_char_for_identifier(thd, STRING_WITH_LEN(table_name));
  my_snprintf(msg, sizeof(msg), "Delete_rows_log_event::find_row() on table %c%s%c", quote_char, table_name,
              quote_char);
  thd->reset_db(&m_table->s->db);
  message = msg;
  const bool invoke_triggers = (m_table->triggers && do_invoke_trigger());
...
#ifdef WSREP_PROC_INFO
  my_snprintf(thd->wsrep_info, sizeof(thd->wsrep_info) - 1, "Delete_rows_log_event::find_row(%lld) on table %c%s%c",
              (long long)wsrep_thd_trx_seqno(thd), quote_char, table_name, quote_char);
  message = thd->wsrep_info;
#endif /* WSREP_PROC_INFO */
  ...
  if (likely(!(error = find_row(rgi))))
  {
    ...
    */
    my_snprintf(msg, sizeof(msg), "Delete_rows_log_event::ha_delete_row() on table %c%s%c", quote_char, table_name,
                quote_char);
    message = msg;
#ifdef WSREP_PROC_INFO
    snprintf(thd->wsrep_info, sizeof(thd->wsrep_info) - 1, "Delete_rows_log_event::ha_delete_row(%lld) on table %c%s%c",
             (long long)wsrep_thd_trx_seqno(thd), quote_char, table_name, quote_char);
    message = thd->wsrep_info;
#endif
    thd_proc_info(thd, message);

    if (invoke_triggers && unlikely(process_triggers(TRG_EVENT_DELETE, TRG_ACTION_BEFORE, FALSE)))
      error = HA_ERR_GENERIC;  // in case if error is not set yet
    if (likely(!error))
    {
    ...
    if (invoke_triggers && likely(!error) && unlikely(process_triggers(TRG_EVENT_DELETE, TRG_ACTION_AFTER, FALSE)))
      error = HA_ERR_GENERIC;  // in case if error is not set yet
    m_table->file->ha_index_or_rnd_end();
  }
  thd->reset_db(&tmp_db);
...
#if defined(HAVE_REPLICATION)
uint8 Delete_rows_log_event::get_trg_event_map() { return trg2bit(TRG_EVENT_DELETE); }
#endif
...
Update_rows_log_event::Update_rows_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid, bool is_transactional)
    : Rows_log_event(thd_arg, tbl_arg, tid, tbl_arg->read_set, is_transactional, UPDATE_ROWS_EVENT_V1)
{
  init(tbl_arg->rpl_write_set);
}

Update_rows_compressed_log_event::Update_rows_compressed_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid,
                                                                   bool is_transactional)
...
bool Update_rows_compressed_log_event::write() { return Rows_log_event::write_compressed(); }

void Update_rows_log_event::init(MY_BITMAP const *cols)
{
  /* if my_bitmap_init fails, caught in is_valid() */
  if (likely(!my_bitmap_init(&m_cols_ai, m_width <= sizeof(m_bitbuf_ai) * 8 ? m_bitbuf_ai : NULL, m_width)))
  {
    /* Cols can be zero if this is a dummy binrows event */
    if (likely(cols != NULL))
    {
...
int Update_rows_log_event::do_after_row_operations(const Slave_reporting_capability *const, int error)
{
...
int Update_rows_log_event::do_exec_row(rpl_group_info *rgi)
{
  const bool invoke_triggers = (m_table->triggers && do_invoke_trigger());
  const char *tmp = thd->get_proc_info();
  DBUG_ASSERT(m_table != NULL);
  ...
  const char *table_name = m_table->s->table_name.str;
  char quote_char = get_quote_char_for_identifier(thd, STRING_WITH_LEN(table_name));
  my_snprintf(msg, sizeof(msg), "Update_rows_log_event::find_row() on table %c%s%c", quote_char, table_name,
              quote_char);
  thd->reset_db(&m_table->s->db);
  message = msg;

#ifdef WSREP_PROC_INFO
  my_snprintf(thd->wsrep_info, sizeof(thd->wsrep_info) - 1, "Update_rows_log_event::find_row(%lld) on table %c%s%c",
              (long long)wsrep_thd_trx_seqno(thd), quote_char, table_name, quote_char);
  message = thd->wsrep_info;
#endif /* WSREP_PROC_INFO */
  ...
  m_curr_row = m_curr_row_end;
  my_snprintf(msg, sizeof(msg), "Update_rows_log_event::unpack_current_row() on table %c%s%c", quote_char, table_name,
              quote_char);
  message = msg;
#ifdef WSREP_PROC_INFO
  my_snprintf(thd->wsrep_info, sizeof(thd->wsrep_info) - 1,
              "Update_rows_log_event::unpack_current_row(%lld) on table %c%s%c", (long long)wsrep_thd_trx_seqno(thd),
  ...
  my_snprintf(msg, sizeof(msg), "Update_rows_log_event::ha_update_row() on table %c%s%c", quote_char, table_name,
              quote_char);
  message = msg;
#ifdef WSREP_PROC_INFO
  my_snprintf(thd->wsrep_info, sizeof(thd->wsrep_info) - 1,
              "Update_rows_log_event::ha_update_row(%lld) on table %c%s%c", (long long)wsrep_thd_trx_seqno(thd),
              quote_char, table_name, quote_char);
  message = thd->wsrep_info;
#endif /* WSREP_PROC_INFO */

  thd_proc_info(thd, message);
  if (invoke_triggers && unlikely(process_triggers(TRG_EVENT_UPDATE, TRG_ACTION_BEFORE, TRUE)))
  {
    error = HA_ERR_GENERIC;  // in case if error is not set yet
    goto err;
  }
  ...
  if (invoke_triggers && likely(!error) && unlikely(process_triggers(TRG_EVENT_UPDATE, TRG_ACTION_AFTER, TRUE)))
    error = HA_ERR_GENERIC;  // in case if error is not set yet
...
#if defined(HAVE_REPLICATION)
uint8 Update_rows_log_event::get_trg_event_map() { return trg2bit(TRG_EVENT_UPDATE); }
#endif
...
void Incident_log_event::pack_info(Protocol *protocol)
{
  ...
  if (m_message.length > 0)
    bytes = my_snprintf(buf, sizeof(buf), "#%d (%s)", m_incident, description());
  else
    bytes = my_snprintf(buf, sizeof(buf), "#%d (%s): %s", m_incident, description(), m_message.str);
  protocol->store(buf, bytes, &my_charset_bin);
}
...
Log_event *wsrep_read_log_event(char **arg_buf, size_t *arg_buf_len,
                                const Format_description_log_event *description_event)
{
  ...
  res = Log_event::read_log_event(head, data_len, &error, description_event, false);
...
int Incident_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  rli->report(ERROR_LEVEL, ER_SLAVE_INCIDENT, NULL, ER_THD(rgi->thd, ER_SLAVE_INCIDENT), description(),
              m_message.length > 0 ? m_message.str : "<none>");
  DBUG_RETURN(1);
}
#endif

bool Incident_log_event::write_data_header()
{
...
bool Incident_log_event::write_data_body()
{
  uchar tmp[1];
...
void Ignorable_log_event::pack_info(Protocol *protocol)
{
  char buf[256];
  size_t bytes;
  bytes = my_snprintf(buf, sizeof(buf), "# Ignorable event type %d (%s)", number, description);
  protocol->store(buf, bytes, &my_charset_bin);
}
...
bool event_that_should_be_ignored(const uchar *buf)
{
  ...
  if (event_type == GTID_LOG_EVENT || event_type == ANONYMOUS_GTID_LOG_EVENT ||
      event_type == PREVIOUS_GTIDS_LOG_EVENT || event_type == TRANSACTION_CONTEXT_EVENT ||
      event_type == VIEW_CHANGE_EVENT || (uint2korr(buf + FLAGS_OFFSET) & LOG_EVENT_IGNORABLE_F))
    return 1;
  return 0;
}
