...
static struct my_option my_long_options[] = {
    ...
    {"datadir", 'd', "Data directory of the new database", &opt_datadir, &opt_datadir, 0, GET_STR, REQUIRED_ARG, 0, 0,
     0, 0, 0, 0},
    {"service", 'S', "Name of the Windows service", &opt_service, &opt_service, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0,
     0},
    {"password", 'p', "Root password", &opt_password, &opt_password, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"port", 'P', "mysql port", &opt_port, &opt_port, 0, GET_INT, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"socket", 'W', "named pipe name (if missing, it will be set the same as service)", &opt_socket, &opt_socket, 0,
     GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"default-user", 'D', "Create default user", &opt_default_user, &opt_default_user, 0, GET_BOOL, OPT_ARG, 0, 0, 0, 0,
     0, 0},
    {"allow-remote-root-access", 'R', "Allows remote access from network for user root", &opt_allow_remote_root_access,
     &opt_allow_remote_root_access, 0, GET_BOOL, OPT_ARG, 0, 0, 0, 0, 0, 0},
    {"skip-networking", 'N', "Do not use TCP connections, use pipe instead", &opt_skip_networking, &opt_skip_networking,
     0, GET_BOOL, OPT_ARG, 0, 0, 0, 0, 0, 0},
    {"innodb-page-size", 'i', "Page size for innodb", &opt_innodb_page_size, &opt_innodb_page_size, 0, GET_INT,
     REQUIRED_ARG, DEFAULT_INNODB_PAGE_SIZE, 1 * 1024, 64 * 1024, 0, 0, 0},
    {"silent", 's', "Print less information", &opt_silent, &opt_silent, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"verbose-bootstrap", 'o', "Include mysqld bootstrap output", &opt_verbose_bootstrap, &opt_verbose_bootstrap, 0,
     GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"large-pages", 'l', "Use large pages", &opt_large_pages, &opt_large_pages, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"config", 'c', "my.ini config template file", &opt_config, &opt_config, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0,
     0},
...
int main(int argc, char **argv)
{
  ...
  if (!datadir && opt_config)
  {
    ...
    {
      auto ret =
          GetPrivateProfileStringA(section, "datadir", NULL, default_datadir, sizeof(default_datadir) - 1, opt_config);
      if (ret)
      {
...
static void get_basedir(char *basedir, int size, const char *mysqld_path, char slash)
{
...
static void write_myini_str(const char *key, const char *val, const char *section = "mysqld")
{
  ...
  {
    die("Can't write to ini file key=%s, val=%s, section=%s, Windows error %u", key, val, section, GetLastError());
  }
}
...
static int create_myini()
{
  ...
  enable_named_pipe = (my_bool)((opt_socket && opt_socket[0]) || opt_skip_networking);
...
static int handle_user_privileges(const char *account_name, const wchar_t *privilege_name, bool add)
{
  ...
  auto status = LsaOpenPolicy(0, &attr, POLICY_LOOKUP_NAMES | POLICY_CREATE_ACCOUNT, &lsa_handle);
  if (status)
  {
  ...
  BOOL ok = LookupAccountNameA(0, account_name, sid, &cbSid, domain_name, &cbDomain, &name_use);
  if (!ok)
  {
    verbose("LsaOpenPolicy returned %lu", LsaNtStatusToWinError(status));
    return 1;
  ...
  if (privilege_name)
  {
    ...
    if (add)
    {
      ...
      if (status)
      {
        verbose("LsaAddAccountRights returned %lu/%lu", status, LsaNtStatusToWinError(status));
        return 1;
      }
    }
    else
    {
      ...
      if (status)
      {
        verbose("LsaRemoveRights returned %lu/%lu", LsaNtStatusToWinError(status));
        return 1;
      }
    }
  }
...
static int register_service(const char *datadir, const char *user, const char *passwd)
{
  ...
  my_snprintf(buf, sizeof(buf) - 1, "\"%s\" \"--defaults-file=%s%smy.ini\" \"%s\"", mysqld_path, datadir,
  ...
  /* Create the service. */
  sc_service = CreateService(sc_manager, opt_service, opt_service, SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS,
                             SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, buf, NULL, NULL, NULL, user, passwd);
...
static int set_directory_permissions(const char *dir, const char *os_user, DWORD permission)
{
  struct
  {
  ...
  if (os_user)
  {
    ...
    */
    ...
    if (wellKnownSidType != WinNullSid)
    {
      ...
      if (!CreateWellKnownSid(wellKnownSidType, NULL, pSid, &size))
      {
        return 1;
      }
  ...
  else
  {
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &token))
    {
      ...
      if (GetTokenInformation(token, TokenUser, &tokenInfoBuffer, length, &length))
      {
...
static void set_permissions(const char *datadir, const char *service_user)
{
  ...
  */
  set_directory_permissions(datadir, NULL, FILE_GENERIC_READ | FILE_GENERIC_WRITE);
  if (!service_user)
    return;
...
*/
  ...
  {
    auto subdir = snprintf(path, sizeof(path), "%s\\%s", basedir, all_subdirs[i].subdir);
    if (access(path, 0) == 0)
    {
  ...
  if (access(path, 0) == 0)
  {
    set_directory_permissions(path, service_user, FILE_GENERIC_READ | FILE_GENERIC_EXECUTE);
  }
}
...
static int create_db_instance(const char *datadir)
{
  ...
  */
  ...
  if (GetNamedSecurityInfoA(datadir, SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, 0, 0, 0, 0, &pSD) == ERROR_SUCCESS)
  {
    char *string_sd = NULL;
    if (ConvertSecurityDescriptorToStringSecurityDescriptor(pSD, SDDL_REVISION_1, DACL_SECURITY_INFORMATION, &string_sd,
                                                            0))
    {
  ...
  */
  ...
  if (service_created)
  {
    ...
    if (strncmp(service_user.c_str(), "NT SERVICE\\", sizeof("NT SERVICE\\") - 1))
    {
