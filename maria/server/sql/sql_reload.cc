...
bool reload_acl_and_cache(THD *thd, unsigned long long options, TABLE_LIST *tables, int *write_to_binlog)
{
  bool result = 0;
    ...
    */
    ...
    if (mysql_bin_log.is_open())
    {
      DYNAMIC_ARRAY *drop_gtid_domain =
          (thd && (thd->lex->delete_gtid_domain.elements > 0)) ? &thd->lex->delete_gtid_domain : NULL;
  ...
  if (options & REFRESH_RELAY_LOG)
  {
    ...
    */
    ...
    else if (!(mi = (get_master_info(&connection_name, Sql_condition::WARN_LEVEL_ERROR))))
    {
      result = 1;
    }
    else
  ...
  DBUG_ASSERT(!thd || thd->locked_tables_mode || !thd->mdl_context.has_locks() || thd->handler_tables_hash.records ||
              thd->ull_hash.records || thd->global_read_lock.is_acquired() || thd->mdl_backup_lock ||
  ...
  */
      ...
      if (thd && thd->locked_tables_mode)
      {
        ...
        */
        ...
        else
        {
          ...
          */
          if (thd->open_tables && !thd->mdl_context.is_lock_owner(MDL_key::BACKUP, "", "", MDL_BACKUP_DDL))
          {
  ...
  if (options & REFRESH_SSL)
  {
    if (reinit_ssl())
      result = 1;
#ifdef WITH_WSREP
    if (!result && WSREP_ON && wsrep_reload_ssl())
    {
...
bool flush_tables_with_read_lock(THD *thd, TABLE_LIST *all_tables)
{
  ...
  */
  ...
  if (thd->lex->type & REFRESH_READ_LOCK)
  {
    ...
    */
    ...
    for (auto table_list = all_tables; table_list; table_list = table_list->next_global)
  ...
  */
  if (open_and_lock_tables(thd, all_tables, FALSE, MYSQL_OPEN_SKIP_SCOPED_MDL_LOCK, &lock_tables_prelocking_strategy))
  ...
  if (thd->lex->type & (REFRESH_FOR_EXPORT | REFRESH_READ_LOCK))
  {
    ...
    {
      if (table_list->belong_to_view && check_single_table_access(thd, PRIV_LOCK_TABLES, table_list, FALSE))
      {
      ...
      if (table_list->is_view_or_derived())
        continue;
      if (thd->lex->type & REFRESH_FOR_EXPORT && table_list->table &&
          !(table_list->table->file->ha_table_flags() & HA_CAN_EXPORT))
      {
        my_error(ER_ILLEGAL_HA, MYF(0), table_list->table->file->table_type(), table_list->db.str,
                 table_list->table_name.str);
        goto error_reset_bits;
      }
      if (thd->lex->type & REFRESH_READ_LOCK && table_list->table && table_list->table->file->extra(HA_EXTRA_FLUSH))
        goto error_reset_bits;
    }
  }
