...
bool Item_window_func::resolve_window_name(THD *thd)
{
  if (window_spec)
  {
  ...
  */
  List<Window_spec> curr_window_specs = List<Window_spec>(thd->lex->current_select->window_specs);
  List_iterator_fast<Window_spec> it(curr_window_specs);
  Window_spec *win_spec;
  while ((win_spec = it++))
  {
    const char *win_spec_name = win_spec->name();
    if (win_spec_name && my_strcasecmp(system_charset_info, ref_name, win_spec_name) == 0)
    {
      window_spec = win_spec;
      break;
    }
...
void Item_window_func::update_used_tables()
{
  used_tables_cache = 0;
...
bool Item_window_func::fix_fields(THD *thd, Item **ref)
{
  DBUG_ASSERT(fixed() == 0);

  if (!thd->lex->current_select || (thd->lex->current_select->context_analysis_place != SELECT_LIST &&
...
void Item_window_func::split_sum_func(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &fields, uint flags)
{
...
bool Item_window_func::check_result_type_of_order_item()
{
  switch (window_func()->sum_func())
  {
    case Item_sum::PERCENTILE_CONT_FUNC:
    {
      ...
      if (rtype != REAL_RESULT && rtype != INT_RESULT && rtype != DECIMAL_RESULT && rtype != TIME_RESULT)
      {
    ...
    case Item_sum::PERCENTILE_DISC_FUNC:
    {
      ...
      Item_sum_percentile_disc *func = static_cast<Item_sum_percentile_disc *>(window_func());
...
bool Item_sum_hybrid_simple::fix_fields(THD *thd, Item **ref)
{
  ...
  if (check_sum_func(thd, ref))
    return TRUE;
  for (uint i = 0; i < arg_count; i++) orig_args[i] = args[i];
...
String *Item_sum_hybrid_simple::val_str(String *str)
{
  DBUG_ASSERT(fixed());
  if (null_value)
    return 0;
...
Field *Item_sum_hybrid_simple::create_tmp_field(MEM_ROOT *root, bool group, TABLE *table)
{
  DBUG_ASSERT(0);
  return NULL;
}
...
void Item_sum_hybrid_simple::update_field() { DBUG_ASSERT(0); }
