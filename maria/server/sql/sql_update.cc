...
{
  return !table->versioned() && (((table->file->ha_table_flags() & HA_PARTIAL_COLUMN_READ) == 0) ||
...
bool compare_record(const TABLE *table)
{
  ...
  if (table->file->ha_table_flags() & HA_PARTIAL_COLUMN_READ || table->s->has_update_default_function)
  {
    /*
  ...
  */
  ...
  /* Compare null bits */
  if (memcmp(table->null_flags, table->null_flags + table->s->rec_buff_length, table->s->null_bytes_for_compare))
    return TRUE;  // Diff in NULL value
  /* Compare updated fields */
  for (Field **ptr = table->field; *ptr; ptr++)
  {
    Field *field = *ptr;
    if (field->has_explicit_value() && !field->vcol_info && field->cmp_binary_offset(table->s->rec_buff_length))
      return TRUE;
  }
  return FALSE;
}
...
static bool check_fields(THD *thd, TABLE_LIST *table, List<Item> &items, bool update_view)
{
  Item *item;
  if (update_view)
  {
  ...
  if (thd->variables.sql_mode & MODE_SIMULTANEOUS_ASSIGNMENT)
  {
    ...
    while ((item = it++))
    {
      ...
      {
        my_error(ER_UPDATED_COLUMN_ONLY_ONCE, MYF(0), *(f->table_name), f->field_name.str);
        return TRUE;
      }
  ...
  if (table->has_period())
  {
    ...
    {
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "updating and querying the same temporal periods table");
    ...
    for (List_iterator_fast<Item> it(items); (item = it++);)
    {
      ...
      {
        my_error(ER_PERIOD_COLUMNS_UPDATED, MYF(0), item->name.str, period.name.str);
        return true;
      }
    }
  }
...
static void prepare_record_for_error_message(int error, TABLE *table)
{
  ...
  */
  if ((error != HA_ERR_FOUND_DUPP_KEY) || !(table->file->ha_table_flags() & HA_PARTIAL_COLUMN_READ))
    DBUG_VOID_RETURN;
  ...
  */
  ...
  if ((error = table->file->ha_index_or_rnd_end()) || (error = table->file->ha_rnd_init(0)))
  {
...
static int cut_fields_for_portion_of_time(THD *thd, TABLE *table, const vers_select_conds_t &period_conds)
{
...
int mysql_update(THD *thd, TABLE_LIST *table_list, List<Item> &fields, List<Item> &values, COND *conds, uint order_num,
                 ORDER *order, ha_rows limit, bool ignore, ha_rows *found_return, ha_rows *updated_return)
{
  bool using_limit = limit != HA_POS_ERROR;
  bool safe_update = (thd->variables.option_bits & OPTION_SAFE_UPDATES) && !thd->lex->describe;
  ...
  if (setup_fields_with_no_wrap(thd, Ref_ptr_array(), fields, MARK_COLUMNS_WRITE, 0, 0))
  ...
  if (table_list->table->check_assignability_explicit_fields(fields, values, ignore))
    DBUG_RETURN(true);
  ...
  if (unlikely(error || !limit || thd->is_error() || (select && select->check_quick(thd, safe_update, limit))))
  {
    query_plan.set_impossible_where();
    if (thd->lex->describe || thd->lex->analyze_stmt)
      goto produce_explain_and_leave;
  ...
  else
  {
    ...
    table->no_keyread = 1;
    query_plan.index = get_index_for_order(order, table, select, limit, &scanned_limit, &need_sort, &reverse);
    table->no_keyread = 0;
    if (!need_sort)
    ...
    if (select && select->quick)
    {
      ...
      used_key_is_modified = (!select->quick->unique_key_range() && select->quick->is_keys_used(table->write_set));
    }
    else
    {
      if (need_sort)
  ...
  */
  ...
  if (used_key_is_modified || order || partition_key_modified(table, table->write_set))
  {
    if (order && need_sort)
  ...
  */
  ...
  DBUG_EXECUTE_IF("show_explain_probe_update_exec_start", dbug_serve_apcs(thd, 1););

  has_triggers = (table->triggers && (table->triggers->has_triggers(TRG_EVENT_UPDATE, TRG_ACTION_BEFORE) ||
                                      table->triggers->has_triggers(TRG_EVENT_UPDATE, TRG_ACTION_AFTER)));

  if (table_list->has_period())
    has_triggers =
        table->triggers && (table->triggers->has_triggers(TRG_EVENT_INSERT, TRG_ACTION_BEFORE) ||
                            table->triggers->has_triggers(TRG_EVENT_INSERT, TRG_ACTION_AFTER) || has_triggers);
  ...
  */
  DBUG_PRINT("info", ("HA_CAN_DIRECT_UPDATE_AND_DELETE: %s",
                      (table->file->ha_table_flags() & HA_CAN_DIRECT_UPDATE_AND_DELETE) ? "TRUE" : "FALSE"));
  DBUG_PRINT("info", ("using_io_buffer: %s", query_plan.using_io_buffer ? "TRUE" : "FALSE"));
  DBUG_PRINT("info", ("ignore: %s", ignore ? "TRUE" : "FALSE"));
  DBUG_PRINT("info", ("virtual_columns_marked_for_read: %s",
                      table->check_virtual_columns_marked_for_read() ? "TRUE" : "FALSE"));
  DBUG_PRINT("info", ("virtual_columns_marked_for_write: %s",
                      table->check_virtual_columns_marked_for_write() ? "TRUE" : "FALSE"));
  if ((table->file->ha_table_flags() & HA_CAN_DIRECT_UPDATE_AND_DELETE) && !has_triggers && !binlog_is_row &&
      !query_plan.using_io_buffer && !ignore && !table->check_virtual_columns_marked_for_read() &&
      !table->check_virtual_columns_marked_for_write())
  {
    ...
    if (!use_direct_update && (select->cond->used_tables() & ~RAND_TABLE_BIT) == table->map)
    {
    ...
    if (use_direct_update && !table->file->info_push(INFO_KIND_UPDATE_FIELDS, &fields) &&
        !table->file->info_push(INFO_KIND_UPDATE_VALUES, &values) && !table->file->direct_update_rows_init(&fields))
    {
    ...
    */
    if (query_plan.using_filesort)
    {
      ...
      */
      ...
      Filesort_tracker *fs_tracker = thd->lex->explain->get_upd_del_plan()->filesort_tracker;
    ...
    else
    {
      ...
      */
      ...
      if (query_plan.index == MAX_KEY || (select && select->quick))
        ...
        error = init_read_record_idx(&info, thd, table, 1, query_plan.index, reverse);
      ...
      while (likely(!(error = info.read_record())) && likely(!thd->killed))
      {
        ...
        if (!select || (error = select->skip_record(thd)) > 0)
        {
          ...
          table->file->position(table->record[0]);
          if (unlikely(my_b_write(&tempfile, table->file->ref, table->file->ref_length)))
          {
...
update_begin:
  ...
  */
  ...
  if ((table->file->ha_table_flags() & HA_CAN_FORCE_BULK_UPDATE) &&
      !table->prepare_triggers_for_update_stmt_or_event() && !thd->lex->with_rownum)
  ...
  */
  ...
  while (!(error = info.read_record()) && !thd->killed)
  {
    ...
    if (!select || select->skip_record(thd) > 0)
    {
      ...
      if (table_list->has_period())
        cut_fields_for_portion_of_time(thd, table, table_list->period_conditions);

      if (fill_record_n_invoke_before_triggers(thd, table, fields, values, 0, TRG_EVENT_UPDATE))
        break; /* purecov: inspected */
      ...
      if (need_update)
      {
        if (table->versioned(VERS_TIMESTAMP) && thd->lex->sql_command == SQLCOM_DELETE)
        ...
        if (will_batch)
        {
          ...
          */
          ...
          error = table->file->ha_bulk_update_row(table->record[1], table->record[0], &dup_key_found);
          limit += dup_key_found;
        ...
        if (unlikely(error) && (!ignore || table->file->is_fatal_error(error, HA_CHECK_ALL)))
        {
          goto error;
        }
      }
      ...
      if (!--limit && using_limit)
      {
        ...
        */
        if (will_batch && ((error = table->file->exec_bulk_update(&dup_key_found)) || dup_key_found))
        {
          if (error)
          {
  ...
  */
  ...
  DBUG_EXECUTE_IF("simulate_kill_bug27571", { thd->set_killed(KILL_QUERY); };);
  error = (killed_status == NOT_KILLED) ? error : 1;

  if (likely(error) && will_batch && (loc_error = table->file->exec_bulk_update(&dup_key_found)))
  /*
...
update_end:
  ...
  */
  if (likely(error < 0) || thd->transaction->stmt.modified_non_trans_table || thd->log_current_statement())
  {
    if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
    {
      ...
      if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(), thd->query_length(), transactional_table, FALSE, FALSE,
                            errcode) > 0)
      {
  ...
  id = thd->arg_of_last_insert_id_function ? thd->first_successful_insert_id_in_prev_stmt : 0;

  if (likely(error < 0) && likely(!thd->lex->analyze_stmt))
  {
    char buff[MYSQL_ERRMSG_SIZE];
    if (!table->versioned(VERS_TIMESTAMP) && !table_list->has_period())
      my_snprintf(buff, sizeof(buff), ER_THD(thd, ER_UPDATE_INFO), (ulong)found, (ulong)updated,
                  (ulong)thd->get_stmt_da()->current_statement_warn_count());
    else
      my_snprintf(buff, sizeof(buff), ER_THD(thd, ER_UPDATE_INFO_WITH_SYSTEM_VERSIONING), (ulong)found, (ulong)updated,
                  (ulong)rows_inserted, (ulong)thd->get_stmt_da()->current_statement_warn_count());
    my_ok(thd, (thd->client_capabilities & CLIENT_FOUND_ROWS) ? found : updated, id, buff);
...
bool mysql_prepare_update(THD *thd, TABLE_LIST *table_list, Item **conds, uint order_num, ORDER *order)
{
  ...
  if (table_list->has_period() && select_lex->period_setup_conds(thd, table_list))
    DBUG_RETURN(true);

  DBUG_ASSERT(table_list->table);
  // conds could be cached from previous SP call
  DBUG_ASSERT(!table_list->vers_conditions.need_setup() || !*conds || thd->stmt_arena->is_stmt_execute());
  ...
  */
  ...
  if (setup_tables_and_check_access(thd, &select_lex->context, &select_lex->top_join_list, table_list,
                                    select_lex->leaf_tables, FALSE, UPDATE_ACL, SELECT_ACL, TRUE) ||
...
static table_map get_table_map(List<Item> *items)
{
  ...
  while ((item = (Item_field *)item_it++)) map |= item->all_used_tables();
...
/**
  ...
  @return
    ...
    false  otherwise.
*/
static bool unsafe_key_update(List<TABLE_LIST> leaves, table_map tables_for_update)
{
  ...
  while ((tl = it++))
  {
    if (!tl->is_jtbm() && (tl->table->map & tables_for_update))
    {
      ...
      bool primkey_clustered = (table1->file->pk_is_clustering_key(table1->s->primary_key));
      ...
      while ((tl2 = it2++))
      {
        ...
        */
        ...
        if (tl2 != tl && table2->map & tables_for_update && table1->s == table2->s)
        {
          // A table is updated through two aliases
          if (table_partitioned &&
              (partition_key_modified(table1, table1->write_set) || partition_key_modified(table2, table2->write_set)))
          {
            // Partitioned key is updated
            my_error(ER_MULTI_UPDATE_KEY_CONFLICT, MYF(0), tl->top_table()->alias.str, tl2->top_table()->alias.str);
            return true;
          }
...
static bool multi_update_check_table_access(THD *thd, TABLE_LIST *table, table_map tables_for_update, bool *updated_arg)
{
  if (table->view)
  {
    bool updated = false;
    ...
    */
    DBUG_ASSERT(table->merge_underlying_list || (!table->updatable && !(table->table->map & tables_for_update)));

    for (TABLE_LIST *tbl = table->merge_underlying_list; tbl; tbl = tbl->next_local)
    {
    ...
    if (check_table_access(thd, updated ? UPDATE_ACL : SELECT_ACL, table, FALSE, 1, FALSE))
      return true;
    *updated_arg |= updated;
...
bool Multiupdate_prelocking_strategy::handle_end(THD *thd)
{
  ...
  if (mysql_handle_derived(lex, DT_INIT) || mysql_handle_derived(lex, DT_MERGE_FOR_INSERT) ||
  ...
  */
  ...
  while ((tl = ti++))
  {
    ...
    if (table->map & tables_for_update)
    {
      ...
      {
        my_error(ER_NON_UPDATABLE_TABLE, MYF(0), tl->top_table()->alias.str, "UPDATE");
        DBUG_RETURN(1);
      }

      DBUG_PRINT("info", ("setting table `%s` for update", tl->top_table()->alias.str));
      /*
  ...
  */
  ...
  for (tl = table_list; tl; tl = tl->next_local)
  {
    ...
    if (tl->is_jtbm())
      continue;
    if (tl->is_merged_derived() && tl->check_single_table(&for_update, tables_for_update, tl))
    {
...
int mysql_multi_update_prepare(THD *thd)
{
  ...
  */
  ...
  if (thd->lex->sql_command == SQLCOM_UPDATE_MULTI)
  {
    if (open_tables(thd, &table_list, &table_count,
                    thd->stmt_arena->is_stmt_prepare() ? MYSQL_OPEN_FORCE_SHARED_MDL : 0, &prelocking_strategy))
      DBUG_RETURN(TRUE);
  }
  else
  {
  ...
  if (!thd->stmt_arena->is_stmt_prepare() && lock_tables(thd, table_list, table_count, 0))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  while ((tl = ti++))
  {
    ...
    DBUG_PRINT("info", ("table: %s  want_privilege: %llx", tl->alias.str, (longlong)table->grant.want_privilege));
  }
  /*
                        ...
                        SELECT_LEX *select_lex, multi_update **result)
{
  ...
  if (!(*result = new (thd->mem_root) multi_update(thd, table_list, &thd->lex->first_select_lex()->leaf_tables, fields,
                                                   values, handle_duplicates, ignore)))
  {
    DBUG_RETURN(TRUE);
  }
  ...
  if (setup_tables(thd, &select_lex->context, &select_lex->top_join_list, table_list, select_lex->leaf_tables, FALSE,
                   FALSE))
    DBUG_RETURN(1);
  ...
  res = mysql_select(thd, table_list, total_list, conds, select_lex->order_list.elements, select_lex->order_list.first,
                     NULL, NULL, NULL, options | SELECT_NO_JOIN_CACHE | SELECT_NO_UNLOCK | OPTION_SETUP_TABLES_DONE,
...
multi_update::multi_update(THD *thd_arg, TABLE_LIST *table_list, List<TABLE_LIST> *leaves_list, List<Item> *field_list,
...

{
  ...
  */
  ...
  int error = setup_fields(thd, Ref_ptr_array(), *values, MARK_COLUMNS_READ, 0, NULL, 0) ||
  ...
  */
  ...
  while ((table_ref = ti++))
  {
    ...
    if (tables_to_update & table->map)
    {
      TABLE_LIST *tl = (TABLE_LIST *)thd->memdup(table_ref, sizeof(*tl));
      if (!tl)
        DBUG_RETURN(1);
...
void multi_update::prepare_to_read_rows()
{
  ...
  */
  ...
  for (TABLE_LIST *tl = update_tables; tl; tl = tl->next_local) tl->table->mark_columns_needed_for_update();
}
...
static bool safe_update_on_fly(THD *thd, JOIN_TAB *join_tab, TABLE_LIST *table_ref, TABLE_LIST *all_tables)
{
...
bool multi_update::initialize_tables(JOIN *join)
{
  ...
  */
  ...
  for (table_ref = update_tables; table_ref; table_ref = table_ref->next_local)
  {
    ...
    */
    if (table_ref->check_option && !join->select_lex->uncacheable)
    {
      ...
      SELECT_LEX *sl;
      for (tmp_unit = join->select_lex->first_inner_unit(); tmp_unit; tmp_unit = tmp_unit->next_unit())
      {
  ...
  loop_end:
    ...
    {
      table_map unupdated_tables = table_ref->check_option->used_tables() & ~first_table_for_update->map;
    ...
    */
    ...
    do
    {
      ...
      */
      ...
      Item_temptable_rowid *item = new (thd->mem_root) Item_temptable_rowid(tbl);
      if (!item)
        DBUG_RETURN(1);
    ...
    tmp_tables[cnt] = create_tmp_table(thd, tmp_param, temp_fields, (ORDER *)&group, 0, 0, TMP_TABLE_ALL_COLUMNS,
...
multi_update::~multi_update()
{
  ...
  DBUG_ASSERT(trans_safe || !updated || thd->transaction->all.modified_non_trans_table);
}
    ...
    */
    ...
    if (table == table_to_update)
    {
      ...
      */
      ...
      if (fill_record_n_invoke_before_triggers(thd, table, *fields_for_table[offset], *values_for_table[offset], 0,
                                               TRG_EVENT_UPDATE))
        DBUG_RETURN(1);
      /*
    ...
    else
    {
      ...
      DBUG_ASSERT(1 + unupdated_check_opt_tables.elements == tmp_table_param[offset].func_count);
      fill_record(thd, tmp_table, tmp_table->field + 1 + unupdated_check_opt_tables.elements, *values_for_table[offset],
                  TRUE, FALSE);
      ...
      if (unlikely(error))
      {
        ...
        {
          if (create_internal_tmp_table_from_heap(thd, tmp_table, tmp_table_param[offset].start_recinfo,
                                                  &tmp_table_param[offset].recinfo, error, 1, NULL))
          {
            do_update = 0;
...
void multi_update::abort_result_set()
{
  ...
  if (unlikely(error_handled || (!thd->transaction->stmt.modified_non_trans_table && !updated)))
    return;
    ...
    */
    if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
    {
      ...
      */
      ...
      (void)thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(), thd->query_length(), transactional_tables, FALSE,
                              FALSE, errcode);
    }
    thd->transaction->all.modified_non_trans_table = TRUE;
  }
  ...
  for (cur_table = update_tables; cur_table; cur_table = cur_table->next_local)
  {
    ...
    */
    ...
    for (;;)
    {
      ...
      if (unlikely((local_error = tmp_table->file->ha_rnd_next(tmp_table->record[0]))))
      {
      ...
      do
      {
        ...
        if (unlikely((local_error = tbl->file->ha_rnd_pos(tbl->record[0], (uchar *)rowid.ptr()))))
        {
          err_table = tbl;
          goto err;
        }
      ...
      if (table->vfield && unlikely(table->update_virtual_fields(table->file, VCOL_UPDATE_INDEXED_FOR_UPDATE)))
        goto err2;
      ...
      if (!can_compare_record || compare_record(table))
      {
        ...
        if (unlikely((local_error = table->file->ha_update_row(table->record[1], table->record[0]))) &&
            local_error != HA_ERR_RECORD_IS_THE_SAME)
        {
          if (!ignore || table->file->is_fatal_error(local_error, HA_CHECK_ALL))
          {
            err_table = table;
            goto err;
          }
...
bool multi_update::send_eof()
{
  ...
  */
  ...
  if (likely(local_error == 0 || thd->transaction->stmt.modified_non_trans_table) || thd->log_current_statement())
  {
    if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open())
    {
      int errcode = 0;
      ...
      if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(), thd->query_length(), transactional_tables, FALSE, FALSE,
                            errcode) > 0)
  ...
  if (unlikely(local_error))
  {
    ...
    if (thd->killed == NOT_KILLED && !thd->get_stmt_da()->is_set())
    {
      ...
      */
      my_message(ER_UNKNOWN_ERROR, "An error occurred in multi-table update", MYF(0));
    }
    DBUG_RETURN(TRUE);
  }
