...
static const LEX_CSTRING generated_by_server = {STRING_WITH_LEN(" /* generated by server */")};
...
static bool make_unique_constraint_name(THD *, LEX_CSTRING *, const char *, List<Virtual_column_info> *, uint *);
static const char *make_unique_invisible_field_name(THD *, const char *, List<Create_field> *);
static int copy_data_between_tables(THD *, TABLE *, TABLE *, List<Create_field> &, bool, uint, ORDER *, ha_rows *,
                                    ha_rows *, Alter_info::enum_enable_or_disable, Alter_table_ctx *);
static int append_system_key_parts(THD *thd, HA_CREATE_INFO *create_info, Key *key);
static int mysql_prepare_create_table(THD *, HA_CREATE_INFO *, Alter_info *, uint *, handler *, KEY **, uint *, int,
                                      const LEX_CSTRING db, const LEX_CSTRING table_name);
static uint blob_length_by_type(enum_field_types type);
static bool fix_constraints_names(THD *, List<Virtual_column_info> *, const HA_CREATE_INFO *);
static bool wait_for_master(THD *thd);
static int process_master_state(THD *thd, int alter_result, uint64 &start_alter_id, bool if_exists);
static bool write_bin_log_start_alter_rollback(THD *thd, uint64 &start_alter_id, bool &partial_alter, bool if_exists);
...
static char *add_identifier(THD *thd, char *to_p, const char *end_p, const char *name, size_t name_len)
{
  ...
  res = strconvert(&my_charset_filename, conv_name, name_len, system_charset_info, conv_string, FN_REFLEN, &errors);
  ...
  quote = (likely(thd) ? get_quote_char_for_identifier(thd, conv_name, res - 1) : '`');
  ...
  */
  ...
  while ((tmp_p = strchr(tmp_p, '#')))
  {
    ...
    switch (tmp_p[0])
    {
      ...
      case 'T':
      case 't':
        if ((tmp_p[1] == 'M' || tmp_p[1] == 'm') && (tmp_p[2] == 'P' || tmp_p[2] == 'p') && tmp_p[3] == '#' &&
            !tmp_p[4])
        {
      ...
      case 'R':
      case 'r':
        if ((tmp_p[1] == 'E' || tmp_p[1] == 'e') && (tmp_p[2] == 'N' || tmp_p[2] == 'n') && tmp_p[3] == '#' &&
            !tmp_p[4])
        {
  ...
  if (db_name)
  {
    ...
    {
      to_p = strnmov(to_p, ER_THD_OR_DEFAULT(thd, ER_DATABASE_NAME), end_p - to_p);
      *(to_p++) = ' ';
  ...
  if (part_name)
  {
    ...
    if (part_type != NORMAL_PART_NAME)
    {
      if (part_type == TEMP_PART_NAME)
        to_p = strnmov(to_p, ER_THD_OR_DEFAULT(thd, ER_TEMPORARY_NAME), end_p - to_p);
      else
        to_p = strnmov(to_p, ER_THD_OR_DEFAULT(thd, ER_RENAMED_NAME), end_p - to_p);
      to_p = strnmov(to_p, " ", end_p - to_p);
    }
    to_p = strnmov(to_p, ER_THD_OR_DEFAULT(thd, ER_PARTITION_NAME), end_p - to_p);
    *(to_p++) = ' ';
    to_p = add_identifier(thd, to_p, end_p, part_name, part_name_len);
    if (subpart_name)
    {
      to_p = strnmov(to_p, ", ", end_p - to_p);
      to_p = strnmov(to_p, ER_THD_OR_DEFAULT(thd, ER_SUBPARTITION_NAME), end_p - to_p);
      *(to_p++) = ' ';
...
uint filename_to_tablename(const char *from, char *to, size_t to_length, bool stay_quiet)
{
  uint errors;
  size_t res;
  ...
  res = strconvert(&my_charset_filename, from, FN_REFLEN, system_charset_info, to, to_length, &errors);
...
{
  return (name[0] == '#' && !strncmp(name, MYSQL50_TABLE_NAME_PREFIX, MYSQL50_TABLE_NAME_PREFIX_LENGTH));
}
...
uint check_n_cut_mysql50_prefix(const char *from, char *to, size_t to_length)
{
  if (check_mysql50_prefix(from))
    return (uint)(strmake(to, from + MYSQL50_TABLE_NAME_PREFIX_LENGTH, to_length - 1) - to);
  return 0;
}
...
uint tablename_to_filename(const char *from, char *to, size_t to_length)
{
  ...
  length = strconvert(system_charset_info, from, FN_REFLEN, &my_charset_filename, to, to_length, &errors);
...
uint build_table_filename(char *buff, size_t bufflen, const char *db, const char *table_name, const char *ext,
                          uint flags)
{
  char dbbuff[FN_REFLEN];
  char tbbuff[FN_REFLEN];
  DBUG_ENTER("build_table_filename");
  DBUG_PRINT("enter", ("db: '%s'  table_name: '%s'  ext: '%s'  flags: %x", db, table_name, ext, flags));
  ...
  /*
    ...
    file exists.
  */
  if (!(flags & FN_IS_TMP) && is_prefix(table_name, tmp_file_prefix) && strlen(table_name) < NAME_CHAR_LEN &&
...
uint build_tmptable_filename(THD *thd, char *buff, size_t bufflen)
{
  ...
  my_snprintf(p, bufflen - (p - buff), "/%s-temptable-%lx-%llx-%x", tmp_file_prefix, current_pid, thd->thread_id,
              thd->tmp_table++);
                                     ...
                                     uint flags)
{
  ...
  build_table_filename(buff, bufflen, db_name, table_name, "", flags & FN_IS_TMP);
}
...
uint build_table_shadow_filename(char *buff, size_t bufflen, ALTER_PARTITION_PARAM_TYPE *lpt, bool backup)
{
  char tmp_name[FN_REFLEN];
  my_snprintf(tmp_name, sizeof(tmp_name), "%s-%s-%lx-%s", tmp_file_prefix, backup ? "backup" : "shadow",
              (ulong)current_thd->thread_id, lpt->table_name.str);
  return build_table_filename(buff, bufflen, lpt->db.str, tmp_name, "", FN_IS_TMP);
}
...
bool mysql_write_frm(ALTER_PARTITION_PARAM_TYPE *lpt, uint flags)
{
  ...
  */
  ...
  {
    if (mysql_prepare_create_table(lpt->thd, lpt->create_info, lpt->alter_info, &lpt->db_options, lpt->table->file,
                                   &lpt->key_info_buffer, &lpt->key_count, C_ALTER_TABLE, lpt->db, lpt->table_name))
    {
      DBUG_RETURN(TRUE);
    }
#ifdef WITH_PARTITION_STORAGE_ENGINE
    {
      ...
      {
        part_syntax_buf =
            generate_partition_syntax_for_frm(lpt->thd, part_info, &syntax_len, lpt->create_info, lpt->alter_info);
        if (!part_syntax_buf)
          DBUG_RETURN(TRUE);
    ...
    LEX_CUSTRING frm = build_frm_image(lpt->thd, lpt->table_name, lpt->create_info, lpt->alter_info->create_list,
                                       lpt->key_count, lpt->key_info_buffer, lpt->table->file);
    if (!frm.str)
    {
      error = 1;
      goto end;
    ...
    int error = writefile(shadow_frm_name, lpt->db.str, lpt->table_name.str, lpt->create_info->tmp_table(), frm.str,
                          frm.length);
    my_free(const_cast<uchar *>(frm.str));

    if (unlikely(error) ||
        unlikely(lpt->table->file->ha_create_partitioning_metadata(shadow_path, NULL, CHF_CREATE_FLAG)))
    {
  ...
  if (flags & WFRM_WRITE_CONVERTED_TO)
  {
    ...
    if (ddl_log_create_table(part_info, create_info->db_type, &new_path, &alter_ctx->new_db, &alter_ctx->new_name,
                             true) ||
        ERROR_INJECT("create_before_create_frm"))
      DBUG_RETURN(TRUE);

    if (mysql_prepare_create_table(thd, create_info, lpt->alter_info, &lpt->db_options, file, &lpt->key_info_buffer,
                                   &lpt->key_count, C_ALTER_TABLE, alter_ctx->new_db, alter_ctx->new_name))
      DBUG_RETURN(TRUE);

    lpt->create_info->table_options = lpt->db_options;
    LEX_CUSTRING frm = build_frm_image(thd, alter_ctx->new_name, create_info, lpt->alter_info->create_list,
                                       lpt->key_count, lpt->key_info_buffer, file);
    if (unlikely(!frm.str))
      DBUG_RETURN(TRUE);
    ...
    error = writefile(frm_name, alter_ctx->new_db.str, alter_ctx->new_name.str, create_info->tmp_table(), frm.str,
                      frm.length);
  ...
  if (flags & WFRM_BACKUP_ORIGINAL)
  {
    ...
    if (write_log_replace_frm(lpt, part_info->list->entry_pos, (const char *)bak_path, (const char *)path) ||
        ddl_log_write_execute_entry(part_info->list->entry_pos, &part_info->execute_entry))
    {
    ...
    if (mysql_file_rename(key_file_frm, frm_name, bak_frm_name, MYF(MY_WME)))
      DBUG_RETURN(TRUE);
    if (lpt->table->file->ha_create_partitioning_metadata(bak_path, path, CHF_RENAME_FLAG))
      DBUG_RETURN(TRUE);
  }
    ...
    */
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
         || lpt->table->file->ha_create_partitioning_metadata(path, shadow_path, CHF_DELETE_FLAG) ||
         ddl_log_increment_phase(part_info->main_entry->entry_pos) || (ddl_log_sync(), FALSE)
#endif
             ))
    {
      error = 1;
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
        || lpt->table->file->ha_create_partitioning_metadata(path, shadow_path, CHF_RENAME_FLAG)
#endif
    )
    {
      error = 1;
    ...
    if (part_info && (flags & WFRM_KEEP_SHARE))
    {
      ...
      if (share->partition_info_buffer_size < syntax_len + 1)
      {
        ...
        if (!(tmp_part_syntax_str = (char *)strmake_root(&share->mem_root, part_syntax_buf, syntax_len)))
        {
          error = 1;
          goto err;
        }
        ...
        memcpy((char *)share->partition_info_str, part_syntax_buf, syntax_len + 1);
...
int write_bin_log(THD *thd, bool clear_error, char const *query, ulong query_length, bool is_trans)
{
  int error = 0;
  if (mysql_bin_log.is_open())
  {
    ...
    if (clear_error)
    {
      if (global_system_variables.log_warnings > 2)
      {
        ...
        if (err_clear)
          sql_print_warning(
              "Error code %d of query '%s' is cleared at its "
              "binary logging.",
              err_clear, query);
      }
      thd->clear_error();
    ...
    error = thd->binlog_query(THD::STMT_QUERY_TYPE, query, query_length, is_trans, FALSE, FALSE, errcode) > 0;
    thd_proc_info(thd, 0);
  }
  return error;
}
...
int write_bin_log_with_if_exists(THD *thd, bool clear_error, bool is_trans, bool add_if_exists, bool commit_alter)
{
  int result;
  ...
  result = write_bin_log(thd, clear_error, thd->query(), thd->query_length(), is_trans);
  if (commit_alter)
  {
                    ...
                    bool dont_log_query)
{
    ...
    if (!thd->locked_tables_mode)
    {
      if (drop_sequence)
      {
        ...
        */
        ...
        {
          if (table->open_type == OT_TEMPORARY_OR_BASE && is_temporary_table(table) && !table->table->s->sequence)
          {
      ...
      for (table = tables; table; table = table->next_local)
      {
        ...
        else
        {
          /*
            ...
            in its elements.
          */
          table->table = find_table_for_mdl_upgrade(thd, table->db.str, table->table_name.str, NULL);
          if (!table->table)
            DBUG_RETURN(true);
  ...
  error = mysql_rm_table_no_locks(thd, tables, &thd->db, (DDL_LOG_STATE *)0, if_exists, drop_temporary, false,
                                  drop_sequence, dont_log_query, false);
...
*/
static uint32 get_comment(THD *thd, uint32 comment_pos, const char **comment_start)
{
...
int mysql_rm_table_no_locks(THD *thd, TABLE_LIST *tables, const LEX_CSTRING *current_db, DDL_LOG_STATE *ddl_log_state,
                            ...
                            bool dont_log_query, bool dont_free_locks)
{
  ...
  const LEX_CSTRING *object_to_drop = ((drop_sequence) ? &SEQUENCE_clex_str : &TABLE_clex_str);
  ...
  */
  ...
  for (table = tables; table; table = table->next_local)
  {
    ...
    DBUG_PRINT("table", ("table_l: '%s'.'%s'  table: %p  s: %p", db.str, table_name.str, table->table,
    ...
    */
    DBUG_ASSERT(!(thd->locked_tables_mode && table->open_type != OT_BASE_ONLY && thd->find_temporary_table(table) &&
                  table->mdl_request.ticket != NULL));

    if (drop_sequence && table->table && table->table->s->table_type != TABLE_TYPE_SEQUENCE)
    {
      if (if_exists)
      {
        char buff[FN_REFLEN];
        ...
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_NOT_SEQUENCE2, ER_THD(thd, ER_NOT_SEQUENCE2),
                            tbl_name.c_ptr_safe());
    ...
    if ((drop_temporary && if_exists) || temporary_table_was_dropped)
    {
      ...
      */
      if (!dont_log_query && table_creation_was_logged)
      {
        ...
        String *built_ptr_query = (is_trans ? &built_trans_tmp_query : &built_non_trans_tmp_query);
        /*
          ...
          query.
        */
        if (thd->db.str == NULL || cmp(&db, &thd->db) || is_drop_tmp_if_exists_added)
        {
    ...
    else if (!drop_temporary)
    {
      ...
      DBUG_ASSERT(thd->mdl_context.is_lock_owner(MDL_key::TABLE, db.str, table_name.str, MDL_SHARED));

      alias = (lower_case_table_names == 2) ? table->alias : table_name;
      /* remove .frm file and engine files */
      path_length = build_table_filename(path, sizeof(path) - 1, db.str, alias.str, reg_ext, 0);
...

    {
      ...
      table_type = dd_frm_type(thd, path, &engine, &partition_engine_name, &version);
    ...
    else
    {
      ...
      if (thd->locked_tables_mode == LTM_LOCK_TABLES || thd->locked_tables_mode == LTM_PRELOCKED_UNDER_LOCK_TABLES)
      {
        ...
        close_all_tables_for_name(thd, table->table->s, HA_EXTRA_PREPARE_FOR_DROP, NULL);
        table->table = 0;
      }
      else
        tdc_remove_table(thd, db.str, table_name.str);
      ...
      DBUG_ASSERT(thd->mdl_context.is_lock_owner(MDL_key::TABLE, db.str, table_name.str, MDL_EXCLUSIVE));
      ...
      */
      ...
      if ((likely(!error) || non_existing_table_error(error)) && !access(path, F_OK))
      {
        ...
        else if (unlikely(mysql_file_delete(key_file_frm, path, !error ? MYF(MY_WME) : MYF(MY_WME | MY_IGNORE_ENOENT))))
        {
          frm_delete_error = my_errno;
    ...
    */
    ...
    {
      if (Table_triggers_list::drop_all_triggers(thd, &db, &table_name, MYF(MY_WME | MY_IGNORE_ENOENT)))
        error = error ? error : -1;
    }
  ...
  report_error:
    if (error)
    {
      ...
      uint is_note = (if_exists && (was_view || wrong_drop_sequence) ? ME_NOTE : 0);
    ...
    */
    ...
    {
      PSI_CALL_drop_table_share(temporary_table_was_dropped, db.str, (uint)db.length, table_name.str,
      ...
      if (!is_temporary)
      {
        ...
        if ((ddl_log.org_partitioned = (partition_engine_name.str != 0)))
          ...
          lex_string_set(&ddl_log.org_storage_engine_name, ha_resolve_storage_engine_name(hton));
    ...
    if (!dont_log_query && (!error || table_dropped || non_existing_table_error(error)))
    {
    ...
    DBUG_PRINT("table", ("table: %p  s: %p", table->table, table->table ? table->table->s : NULL));
  }
...
err:
  if (unknown_tables.length() > 1)
  {
    ...
    unknown_tables.chop();
    my_error((drop_sequence ? ER_UNKNOWN_SEQUENCES : ER_BAD_TABLE_ERROR), MYF(is_note), unknown_tables.c_ptr_safe());
  }
  error = thd->is_error();
  ...
  */
  ...
  if (non_trans_tmp_table_deleted || trans_tmp_table_deleted || non_tmp_table_deleted)
  {
    ...
    if (!dont_log_query && mysql_bin_log.is_open())
    {
      ...
      if (non_trans_tmp_table_deleted)
      {
        ...
        error |=
            (thd->binlog_query(THD::STMT_QUERY_TYPE, built_non_trans_tmp_query.ptr(),
                               built_non_trans_tmp_query.length(), FALSE, FALSE, is_drop_tmp_if_exists_added, 0) > 0);
      }
      if (trans_tmp_table_deleted)
      {
        ...
        error |= (thd->binlog_query(THD::STMT_QUERY_TYPE, built_trans_tmp_query.ptr(), built_trans_tmp_query.length(),
                                    TRUE, FALSE, is_drop_tmp_if_exists_added, 0) > 0);
      }
      if (non_tmp_table_deleted)
      {
        ...
        error |= (thd->binlog_query(THD::STMT_QUERY_TYPE, built_query.ptr(), built_query.length(), TRUE, FALSE, FALSE,
                                    0) > 0);
        thd->binlog_xid = 0;
      }
  ...
  if (!drop_temporary)
  {
    ...
    */
    if (thd->locked_tables_mode)
    {
      if (thd->lock && thd->lock->table_count == 0 && non_temp_tables_count > 0 && !dont_free_locks)
      {
...
bool log_drop_table(THD *thd, const LEX_CSTRING *db_name, const LEX_CSTRING *table_name,
                    const LEX_CSTRING *handler_name, bool partitioned, const LEX_CUSTRING *id, bool temporary_table)
{
  ...
  if (mysql_bin_log.is_open())
  {
    ...
    query.append(
        STRING_WITH_LEN("/* Generated to handle "
    ...
    */
    error = thd->binlog_query(THD::STMT_QUERY_TYPE, query.ptr(), query.length(), FALSE, FALSE, temporary_table, 0) > 0;
  }
  if (!temporary_table)
  {
    backup_log_info ddl_log;
                    ...
                    const char *table_path)
{
  ...
  size_t path_length = table_path ? (strxnmov(path, pathmax, table_path, NullS) - path)
  ...
  if (likely(error == 0))
  {
    PSI_CALL_drop_table_share(flags & FN_IS_TMP, db->str, (uint)db->length, table_name->str, (uint)table_name->length);
  }
...
static int sort_keys(KEY *a, KEY *b)
{
  ...
  */
  if (a->algorithm == HA_KEY_ALG_LONG_HASH && b->algorithm == HA_KEY_ALG_LONG_HASH)
...
bool check_duplicates_in_interval(const char *set_or_name, const char *name, const TYPELIB *typelib, CHARSET_INFO *cs,
  ...
  *dup_val_count = 0;
  ...
  for (; tmp.count > 1; cur_value++, cur_length++)
  {
    ...
    if (find_type2(&tmp, (const char *)*cur_value, *cur_length, cs))
    {
      ...
      {
        my_error(ER_DUPLICATED_VALUE_IN_TYPE, MYF(0), name, err.ptr(), set_or_name);
        return 1;
      }
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_DUPLICATED_VALUE_IN_TYPE,
                          ER_THD(thd, ER_DUPLICATED_VALUE_IN_TYPE), name, err.ptr(), set_or_name);
      (*dup_val_count)++;
    }
  }
  return 0;
...
*/
bool Column_definition::prepare_charset_for_string(const Column_derived_attributes *dattr)
{
  CHARSET_INFO *tmp = charset_collation_attrs().resolved_to_character_set(dattr->charset());
  if (!tmp)
    return true;
  charset = tmp;
  /*
...
bool Column_definition::prepare_stage2_blob(handler *file, ulonglong table_flags, uint field_flags)
{
  ...
  pack_flag = field_flags | pack_length_to_packflag(pack_length - portable_sizeof_char_ptr);
...
bool Column_definition::prepare_stage2_typelib(const char *type_name, uint field_flags, uint *dup_val_count)
{
  pack_flag = pack_length_to_packflag(pack_length) | field_flags;
  if (charset->state & MY_CS_BINSORT)
    pack_flag |= FIELDFLAG_BINARY;
  return check_duplicates_in_interval(type_name, field_name.str, interval, charset, dup_val_count);
}

uint Column_definition::pack_flag_numeric() const
{
  return (FIELDFLAG_NUMBER | (flags & UNSIGNED_FLAG ? 0 : FIELDFLAG_DECIMAL) |
...
bool Column_definition::prepare_stage2(handler *file, ulonglong table_flags)
{
  ...
  */
  ...
  if (!(flags & NOT_NULL_FLAG) || (vcol_info)) /* Make virtual columns allow NULL values */
...
void promote_first_timestamp_column(List<Create_field> *column_definitions)
{
  ...
  for (Create_field &column_definition : *column_definitions)
  {
        ...
        column_definition.unireg_check == Field::TIMESTAMP_OLD_FIELD)  // Legacy
    {
      ...
      if (first && (column_definition.flags & NOT_NULL_FLAG) != 0 &&  // NOT NULL,
          column_definition.default_value == NULL &&                  // no constant default,
          column_definition.unireg_check == Field::NONE &&            // no function default
          column_definition.vcol_info == NULL && column_definition.period == NULL &&
...
{
  return a.length == b.length && a.asc == b.asc && !lex_string_cmp(system_charset_info, &a.field_name, &b.field_name);
}
...
*/
static void check_duplicate_key(THD *thd, const Key *key, const KEY *key_info, const List<Key> *key_list)
{
  /*
  ...
  */
  ...
  for (const Key &k : *key_list)
  {
    ...
    if (k.generated || (key->type != k.type) || (key->key_create_info.algorithm != k.key_create_info.algorithm) ||
    ...
    if (std::equal(key->columns.begin(), key->columns.end(), k.columns.begin(), key_cmp))
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_DUP_INDEX, ER_THD(thd, ER_DUP_INDEX),
                          key_info->name.str);
      return;
    }
  }
...
bool Column_definition::prepare_stage1_typelib(THD *thd, MEM_ROOT *mem_root, handler *file, ulonglong table_flags)
{
  /*
...
bool Column_definition::prepare_stage1_string(THD *thd, MEM_ROOT *mem_root, handler *file, ulonglong table_flags)
{
  ...
  */
  if (!(flags & BLOB_FLAG) && default_value && default_value->expr->basic_const_item() &&
...
bool Column_definition::prepare_stage1_bit(THD *thd, MEM_ROOT *mem_root, handler *file, ulonglong table_flags)
{
...
bool Column_definition::prepare_stage1(THD *thd, MEM_ROOT *mem_root, handler *file, ulonglong table_flags,
                                       const Column_derived_attributes *derived_attr)
{
  return type_handler()->Column_definition_prepare_stage1(thd, mem_root, this, file, table_flags, derived_attr);
}

bool Column_definition::prepare_stage1_convert_default(THD *thd, MEM_ROOT *mem_root, CHARSET_INFO *cs)
{
...
bool Column_definition::prepare_stage1_check_typelib_default()
{
  ...
  else
  {
    ...
    if (real_field_type() == MYSQL_TYPE_SET)
    {
      char *not_used;
      uint not_used2;
      find_set(interval, def->ptr(), def->length(), charset, &not_used, &not_used2, &not_found);
    }
...
*/
int mysql_add_invisible_field(THD *thd, List<Create_field> *field_list, const char *field_name,
                              Type_handler *type_handler, field_visibility_t invisible, Item *default_value)
{
  ...
  {
    if ((new_name = make_unique_invisible_field_name(thd, field_name, field_list)))
    {
...
{
  buf->length = my_snprintf((char *)buf->str, LONG_HASH_FIELD_NAME_LENGTH, "DB_ROW_HASH_%u", num);
}
...
*/
static Create_field *add_hash_field(THD *thd, List<Create_field> *create_list, KEY *key_info)
{
...
Key *mysql_add_invisible_index(THD *thd, List<Key> *key_list, LEX_CSTRING *field_name, enum Key::Keytype type)
{
  Key *key = new (thd->mem_root) Key(type, &null_clex_str, HA_KEY_ALG_UNDEF, false, DDL_options(DDL_options::OPT_NONE));
  key->columns.push_back(new (thd->mem_root) Key_part_spec(field_name, 0, true), thd->mem_root);
  key_list->push_back(key, thd->mem_root);
  return key;
}

bool Type_handler_string::Key_part_spec_init_ft(Key_part_spec *part, const Column_definition &def) const
{
  /*
...
bool Type_handler_varchar::Key_part_spec_init_ft(Key_part_spec *part, const Column_definition &def) const
{
  part->length = 0;
  return !Charset(def.charset).is_good_for_ft();
}

bool Type_handler_blob_common::Key_part_spec_init_ft(Key_part_spec *part, const Column_definition &def) const
{
  /*
...
static bool key_add_part_check_null(const handler *file, KEY *key_info, const Column_definition *sql_field,
...
static int mysql_prepare_create_table(THD *thd, HA_CREATE_INFO *create_info, Alter_info *alter_info, uint *db_options,
                                      handler *file, KEY **key_info_buffer, uint *key_count, int create_table_mode,
                                      const LEX_CSTRING db, const LEX_CSTRING table_name)
{
  const char *key_name;
  ...
  const Column_bulk_alter_attributes battr(create_info->alter_table_convert_to_charset);
  DBUG_ENTER("mysql_prepare_create_table");

  DBUG_EXECUTE_IF("test_pseudo_invisible", {
    mysql_add_invisible_field(thd, &alter_info->create_list, "invisible", &type_handler_slong, INVISIBLE_SYSTEM,
  ...
  DBUG_EXECUTE_IF("test_invisible_index", {
    ...
    mysql_add_invisible_index(thd, &alter_info->key_list, &temp, Key::MULTIPLE);
  });
  LEX_CSTRING *connect_string = &create_info->connect_string;
  if (connect_string->length != 0 && connect_string->length > CONNECT_STRING_MAXLEN &&
      (system_charset_info->charpos(connect_string->str, (connect_string->str + connect_string->length),
                                    CONNECT_STRING_MAXLEN) < connect_string->length))
  {
    my_error(ER_WRONG_STRING_LENGTH, MYF(0), connect_string->str, "CONNECTION", CONNECT_STRING_MAXLEN);
    DBUG_RETURN(TRUE);
  }
  ...
  if (create_info->sequence)
  {
    ...
    {
      my_error(ER_ILLEGAL_HA_CREATE_OPTION, MYF(0), file->table_type(), SEQUENCE_clex_str.str);
      DBUG_RETURN(TRUE);
    }
  ...
  for (field_no = 0; (sql_field = it++); field_no++)
  {
    ...
    */
    ...
    if (sql_field->prepare_stage1(thd, thd->mem_root, file, file->ha_table_flags(), &dattr))
      DBUG_RETURN(true);

    DBUG_ASSERT(sql_field->charset);

    if (sql_field->real_field_type() == MYSQL_TYPE_BIT && file->ha_table_flags() & HA_CAN_BIT_FIELD)
    ...
    {
      if (lex_string_cmp(system_charset_info, &sql_field->field_name, &dup_field->field_name) == 0)
      {
        /*
          If this was a CREATE ... SELECT statement, accept a field
          redefinition if we are changing a field in the SELECT part
        */
        if (field_no < select_field_pos || dup_no >= select_field_pos || dup_field->invisible >= INVISIBLE_SYSTEM)
        {
    ...
    if ((sql_field->flags & BLOB_FLAG) ||
        (sql_field->real_field_type() == MYSQL_TYPE_VARCHAR && create_info->row_type != ROW_TYPE_FIXED))
  ...
  while ((sql_field = it++))
  {
    ...
    if (MTYP_TYPENR(sql_field->unireg_check) == Field::NEXT_NUMBER)
      auto_increment++;
    if (parse_option_list(thd, create_info->db_type, &sql_field->option_struct, &sql_field->option_list,
                          create_info->db_type->field_options, FALSE, thd->mem_root))
      DBUG_RETURN(TRUE);
    /*
  ...
  *key_count = 0;
  ...
  {
    DBUG_PRINT("info", ("key name: '%s'  type: %d", key->name.str ? key->name.str : "(none)", key->type));
    if (key->type == Key::FOREIGN_KEY)
    {
      ...
      if (fk_key->ref_columns.elements)
      {
        ...
        {
          my_error(ER_WRONG_FK_DEF, MYF(0), (fk_key->name.str ? fk_key->name.str : "foreign key without name"),
    ...
    if (key->type != Key::FOREIGN_KEY)
    {
      while ((key2 = key_iterator2++) != key)
      {
        ...
        */
        if ((key2->type != Key::FOREIGN_KEY && key2->name.str != ignore_key && !foreign_key_prefix(key, key2)))
        {
    ...
    if (key->name.str && !tmp_table && (key->type != Key::PRIMARY) &&
        !my_strcasecmp(system_charset_info, key->name.str, primary_key_name.str))
    {
  ...
  while ((key = key_iterator++))
  {
    ...
    if (key->type == Key::PRIMARY)
    {
      if (primary_key)
      {
        my_message(ER_MULTIPLE_PRI_KEY, ER_THD(thd, ER_MULTIPLE_PRI_KEY), MYF(0));
        DBUG_RETURN(true);
      }
    ...
    else if (!(key_name = key->name.str))
    {
      ...
      it.rewind();
      while ((sql_field = it++) && lex_string_cmp(system_charset_info, &field_name, &sql_field->field_name))
        ;
      if (sql_field)
        field_name = sql_field->field_name;
      key_name = make_unique_key_name(thd, field_name.str, *key_info_buffer, key_info);
    }
  ...
  for (; (key = key_iterator++); key_number++)
  {
    ...
    if (parse_option_list(thd, create_info->db_type, &key_info->option_struct, &key_info->option_list,
                          create_info->db_type->index_options, FALSE, thd->mem_root))
      DBUG_RETURN(TRUE);
    ...
    */
    key_info->block_size =
        (key->key_create_info.block_size ? key->key_create_info.block_size : create_info->key_block_size);
    ...
    */
    if (key_info->block_size && ((key->key_create_info.flags & HA_USES_BLOCK_SIZE) || create_info->key_block_size))
    ...
    for (uint column_nr = 0; (column = cols++); column_nr++)
    {
      ...
      */
      if (!sql_field || (sql_field->invisible > INVISIBLE_USER && !column->generated))
      {
        my_error(ER_KEY_COLUMN_DOES_NOT_EXIST, MYF(0), column->field_name.str);
        DBUG_RETURN(TRUE);
      }
      if (sql_field->invisible > INVISIBLE_USER && !(sql_field->flags & VERS_SYSTEM_FIELD) && !key->invisible &&
      ...
      {
        if (!lex_string_cmp(system_charset_info, &column->field_name, &dup_column->field_name))
        {
      ...
      {
        case Key::FULLTEXT:
          if (sql_field->type_handler()->Key_part_spec_init_ft(column, *sql_field) ||
        ...
        case Key::SPATIAL:
          if (sql_field->type_handler()->Key_part_spec_init_spatial(column, *sql_field) ||
              sql_field->check_vcol_for_key(thd))
            DBUG_RETURN(TRUE);
          if (!(sql_field->flags & NOT_NULL_FLAG))
          {
            my_message(ER_SPATIAL_CANT_HAVE_NULL, ER_THD(thd, ER_SPATIAL_CANT_HAVE_NULL), MYF(0));
            DBUG_RETURN(TRUE);
          }
          break;
        ...
        case Key::PRIMARY:
          ...
          if (sql_field->type_handler()->Key_part_spec_init_primary(column, *sql_field, file))
            DBUG_RETURN(TRUE);
          if (!(sql_field->flags & NOT_NULL_FLAG))
          {
        ...
        case Key::MULTIPLE:
          if (sql_field->type_handler()->Key_part_spec_init_multiple(column, *sql_field, file) ||
              sql_field->check_vcol_for_key(thd) || key_add_part_check_null(file, key_info, sql_field, column))
            DBUG_RETURN(TRUE);
          break;

        case Key::FOREIGN_KEY:
          if (sql_field->type_handler()->Key_part_spec_init_foreign(column, *sql_field, file) ||
              sql_field->check_vcol_for_key(thd) || key_add_part_check_null(file, key_info, sql_field, column))
            DBUG_RETURN(TRUE);
          break;

        case Key::UNIQUE:
          if (sql_field->type_handler()->Key_part_spec_init_unique(column, *sql_field, file, &is_hash_field_needed) ||
              sql_field->check_vcol_for_key(thd) || key_add_part_check_null(file, key_info, sql_field, column))
            DBUG_RETURN(TRUE);
          break;
      }
      ...
      uint key_part_length = sql_field->type_handler()->calc_key_length(*sql_field);
      ...
      if (column->length)
      {
        ...
        {
          key_part_length =
              MY_MIN(column->length, blob_length_by_type(sql_field->real_field_type()) * sql_field->charset->mbmaxlen);
          if (key_part_length > max_key_length || key_part_length > file->max_key_part_length())
          {
            if (key->type == Key::MULTIPLE)
            {
              key_part_length = MY_MIN(max_key_length, file->max_key_part_length());
              /* not a critical problem */
              push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_TOO_LONG_KEY, ER_THD(thd, ER_TOO_LONG_KEY),
                                  key_part_length);
      ...
      else if (key_part_length == 0 && (sql_field->flags & NOT_NULL_FLAG) && !is_hash_field_needed)
      {
        my_error(ER_WRONG_KEY_COLUMN, MYF(0), file->table_type(), column->field_name.str);
        DBUG_RETURN(TRUE);
      }
      if (key_part_length > file->max_key_part_length() && key->type != Key::FULLTEXT)
      {
        if (key->type == Key::MULTIPLE)
        {
      ...
      if (key->type == Key::UNIQUE && key_part_length > MY_MIN(max_key_length, file->max_key_part_length()))
      ...
      if (!((*db_options) & HA_OPTION_NO_PACK_KEYS) && !((create_info->table_options & HA_OPTION_NO_PACK_KEYS)) &&
          (key_part_length >= KEY_DEFAULT_PACK_LENGTH) && !is_hash_field_needed)
      {
        key_info->flags |= sql_field->type_handler()->KEY_pack_flags(column_nr);
      }
      /* Check if the key segment is partial, set the key flag accordingly */
      if (key_part_length != sql_field->type_handler()->calc_key_length(*sql_field) &&
    ...
    if (key_length > max_key_length && key->type != Key::FULLTEXT && !is_hash_field_needed)
    {
    ...
    if (is_hash_field_needed)
    {
      ...
      {
        my_error(ER_NO_AUTOINCREMENT_WITH_UNIQUE, MYF(0), sql_field->field_name.str, key_info->name.str);
        DBUG_RETURN(TRUE);
      }
      if (key_info->algorithm != HA_KEY_ALG_UNDEF && key_info->algorithm != HA_KEY_ALG_HASH)
      {
        my_error(ER_TOO_LONG_KEY, MYF(0), max_key_length);
        DBUG_RETURN(TRUE);
      }
    ...
    if (is_hash_field_needed ||
        (key_info->algorithm == HA_KEY_ALG_HASH && key->type != Key::PRIMARY && key_info->flags & HA_NOSAME &&
         !(file->ha_table_flags() & HA_CAN_HASH_KEYS) && file->ha_table_flags() & HA_CAN_VIRTUAL_COLUMNS))
    {
      Create_field *hash_fld = add_hash_field(thd, &alter_info->create_list, key_info);
      if (!hash_fld)
        DBUG_RETURN(TRUE);
    ...
    if (validate_comment_length(thd, &key->key_create_info.comment, INDEX_COMMENT_MAXLEN, ER_TOO_LONG_INDEX_COMMENT,
                                key_info->name.str))
      DBUG_RETURN(TRUE);
    ...
    if (key_info->without_overlaps)
    {
      ...
      while ((key2 = key_iterator2++))
      {
        ...
        for (Key_part_spec &kp : key->columns)
        {
          ...
          {
            if (!lex_string_cmp(system_charset_info, &kp.field_name, &kp2.field_name))
            {
  ...
  if (!unique_key && !primary_key && !create_info->sequence && (file->ha_table_flags() & HA_REQUIRE_PRIMARY_KEY))
  {
    my_message(ER_REQUIRES_PRIMARY_KEY, ER_THD(thd, ER_REQUIRES_PRIMARY_KEY), MYF(0));
    DBUG_RETURN(TRUE);
  }
  if (auto_increment > 0)
  {
  ...
  */
  if (!(create_info->options & HA_SKIP_KEY_SORT))
  {
    ...
    */
    my_qsort((uchar *)*key_info_buffer, *key_count, sizeof(KEY), (qsort_cmp)sort_keys);
  }
  ...
  while ((sql_field = it++))
  {
    ...
    */
    if (!sql_field->default_value && !sql_field->has_default_function() && (sql_field->flags & NOT_NULL_FLAG) &&
        (!sql_field->is_timestamp_type() || (thd->variables.option_bits & OPTION_EXPLICIT_DEF_TIMESTAMP)) &&
    ...
    if (thd->variables.sql_mode & MODE_NO_ZERO_DATE && !sql_field->default_value && !sql_field->vcol_info &&
        !sql_field->vers_sys_field() && sql_field->is_timestamp_type() &&
        !(thd->variables.option_bits & OPTION_EXPLICIT_DEF_TIMESTAMP) && (sql_field->flags & NOT_NULL_FLAG) &&
    ...
    if (sql_field->invisible == INVISIBLE_USER && sql_field->flags & NOT_NULL_FLAG &&
        sql_field->flags & NO_DEFAULT_VALUE_FLAG && !sql_field->vers_sys_field())
    {
      my_error(ER_INVISIBLE_NOT_NULL_WITHOUT_DEFAULT, MYF(0), sql_field->field_name.str);
      DBUG_RETURN(TRUE);
    }
    ...
    if (create_simple)
    {
      ...
      */
      if (sql_field->vcol_info && sql_field->vcol_info->expr &&
          sql_field->vcol_info->expr->walk(&Item::check_table_name_processor, false, (void *)&walk_prm))
      {
      ...
      if (sql_field->default_value &&
          sql_field->default_value->expr->walk(&Item::check_table_name_processor, false, (void *)&walk_prm))
      {
      ...
      if (sql_field->check_constraint &&
          sql_field->check_constraint->expr->walk(&Item::check_table_name_processor, false, (void *)&walk_prm))
      {
  ...
  create_info->check_constraint_list = &alter_info->check_constraint_list;
  {
    ...
    while ((check = c_it++))
    {
      if (create_simple && check->expr->walk(&Item::check_table_name_processor, false, (void *)&walk_prm))
      {
        my_error(ER_BAD_FIELD_ERROR, MYF(0), walk_prm.field.c_ptr(), "CHECK");
        DBUG_RETURN(TRUE);
      }
...

      {
        ...
        List_iterator_fast<Virtual_column_info> dup_it(alter_info->check_constraint_list);
        const Virtual_column_info *dup_check;
        while ((dup_check = dup_it++) && dup_check != check)
        {
          if (!lex_string_cmp(system_charset_info, &check->name, &dup_check->name))
          {
      ...
      while (const Key *key = key_it++)
      {
        ...
        */
        if (key->type != Key::PRIMARY && key->type != Key::UNIQUE && key->type != Key::FOREIGN_KEY)
          continue;

        if (check->name.length == key->name.length &&
            my_strcasecmp(system_charset_info, check->name.str, key->name.str) == 0)
        {
          my_error(ER_DUP_CONSTRAINT_NAME, MYF(0), "CHECK", check->name.str);
          DBUG_RETURN(TRUE);
        }
      ...
      if (check_string_char_length(&check->name, 0, NAME_CHAR_LEN, system_charset_info, 1))
      {
  ...
  if (file->partition_ht() != maria_hton && create_info->transactional && !file->has_transaction_manager())
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_ILLEGAL_HA_CREATE_OPTION,
                        ER_THD(thd, ER_ILLEGAL_HA_CREATE_OPTION), file->engine_name()->str,
                        create_info->transactional == HA_CHOICE_YES ? "TRANSACTIONAL=1" : "TRANSACTIONAL=0");
...
#ifndef DBUG_OFF
  DBUG_EXECUTE_IF("key", Debug_key::print_keys(thd, "prep_create_table: ", *key_info_buffer, *key_count););
#endif
...
bool validate_comment_length(THD *thd, LEX_CSTRING *comment, size_t max_len, uint err_code, const char *name)
{
  ...
  size_t tmp_len = Well_formed_prefix(system_charset_info, *comment, max_len).length();
  if (tmp_len < comment->length)
  {
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, err_code, ER_THD(thd, err_code), name,
...
bool Column_definition::prepare_blob_field(THD *thd)
{
  ...
  if (length > MAX_FIELD_VARCHARLENGTH && !(flags & BLOB_FLAG))
  {
    ...
    flags |= BLOB_FLAG;
    my_snprintf(warn_buff, sizeof(warn_buff), ER_THD(thd, ER_AUTO_CONVERT), field_name.str,
                (charset == &my_charset_bin) ? "VARBINARY" : "VARCHAR", (charset == &my_charset_bin) ? "BLOB" : "TEXT");
    push_warning(thd, Sql_condition::WARN_LEVEL_NOTE, ER_AUTO_CONVERT, warn_buff);
  }

  if ((flags & BLOB_FLAG) && length)
  {
    if (real_field_type() == FIELD_TYPE_BLOB || real_field_type() == FIELD_TYPE_TINY_BLOB ||
...
bool Column_definition::sp_prepare_create_field(THD *thd, MEM_ROOT *mem_root)
{
  ...
  return prepare_stage1(thd, mem_root, NULL, HA_CAN_GEOMETRY, &dattr) || prepare_stage2(NULL, HA_CAN_GEOMETRY);
}
...
static int append_system_key_parts(THD *thd, HA_CREATE_INFO *create_info, Key *key)
{
  ...
  int result = 0;
  if (create_info->versioned() && (key->type == Key::PRIMARY || key->type == Key::UNIQUE))
  {
    Key_part_spec *key_part = NULL;
    List_iterator<Key_part_spec> part_it(key->columns);
    while ((key_part = part_it++))
    {
      if (row_start_field.streq(key_part->field_name) || row_end_field.streq(key_part->field_name))
        break;
    }
    if (!key_part)
    {
      key->columns.push_back(new (thd->mem_root) Key_part_spec(&row_end_field, 0, true));
      result++;
    }
  }
  ...
  if (key->without_overlaps)
  {
    ...
    if (!create_info->period_info.is_set() || !key->period.streq(create_info->period_info.name))
    {
    ...
    {
      if (period_start.streq(key_part->field_name) || period_end.streq(key_part->field_name))
      {
        my_error(ER_KEY_CONTAINS_PERIOD_FIELDS, MYF(0), key->name.str, key_part->field_name.str);
        return -1;
      }
    }
    const auto &period = create_info->period_info.period;
    key->columns.push_back(new (thd->mem_root) Key_part_spec(&period.end, 0, true));
    key->columns.push_back(new (thd->mem_root) Key_part_spec(&period.start, 0, true));
    result += 2;
  }
                                ...
                                KEY **key_info, uint *key_count, LEX_CUSTRING *frm)
{
  ...
  if (unlikely(!(file = get_new_handler((TABLE_SHARE *)0, thd->mem_root, create_info->db_type))))
    DBUG_RETURN(NULL);
  ...
  if (part_info)
  {
    ...
    */
    ...
    while ((part_elem = part_it++))
    {
      ...
      {
        LEX_CSTRING comment = {part_elem->part_comment, strlen(part_elem->part_comment)};
        if (validate_comment_length(thd, &comment, TABLE_PARTITION_COMMENT_MAXLEN, ER_TOO_LONG_TABLE_PARTITION_COMMENT,
      ...
      if (part_elem->subpartitions.elements)
      {
        ...
        while ((subpart_elem = sub_it++))
        {
          ...
          {
            LEX_CSTRING comment = {subpart_elem->part_comment, strlen(subpart_elem->part_comment)};
            if (validate_comment_length(thd, &comment, TABLE_PARTITION_COMMENT_MAXLEN,
                                        ER_TOO_LONG_TABLE_PARTITION_COMMENT, subpart_elem->partition_name))
              DBUG_RETURN(NULL);
            /* cut comment length. Safe to do in all cases */
    ...
    if ((part_engine_type == partition_hton) && part_info->default_engine_type)
    {
      /*
    ...
    if (part_info->check_partition_info(thd, &engine_type, file, create_info, FALSE))
      goto err;
    ...
    */
    part_syntax_buf = generate_partition_syntax_for_frm(thd, part_info, &syntax_len, create_info, alter_info);
    if (!part_syntax_buf)
      goto err;
    part_info->part_info_string = part_syntax_buf;
    part_info->part_info_len = syntax_len;
        ...
        create_info->db_type == partition_hton)
    {
      ...
      */
      DBUG_PRINT("info", ("db_type: %s", ha_resolve_storage_engine_name(create_info->db_type)));
      delete file;
      create_info->db_type = partition_hton;
      ...
      */
      if (part_info->use_default_num_partitions && part_info->num_parts &&
          (int)part_info->num_parts != file->get_default_no_partitions(create_info))
      {
        uint i;
        List_iterator<partition_element> part_it(part_info->partitions);
        part_it++;
        ...
        for (i = 1; i < part_info->partitions.elements; i++) (part_it++)->part_state = PART_TO_BE_DROPPED;
               ...
               (int)part_info->num_subparts != file->get_default_no_partitions(create_info))
      {
        DBUG_ASSERT(thd->lex->sql_command != SQLCOM_CREATE_TABLE);
    ...
    else if (create_info->db_type != engine_type)
    {
      ...
      */
      delete file;
      if (unlikely(!(file = get_new_handler((TABLE_SHARE *)0, thd->mem_root, engine_type))))
        DBUG_RETURN(NULL);
    }
  }
  /*
  ...
  */
  ...
  if (mysql_prepare_create_table(thd, create_info, alter_info, &db_options, file, key_info, key_count,
                                 create_table_mode, db, table_name))
    goto err;
  create_info->table_options = db_options;

  *frm = build_frm_image(thd, table_name, create_info, alter_info->create_list, *key_count, *key_info, file);
...
static int create_table_impl(THD *thd, DDL_LOG_STATE *ddl_log_state_create, DDL_LOG_STATE *ddl_log_state_rm,
                             const LEX_CSTRING &orig_db, const LEX_CSTRING &orig_table_name, const LEX_CSTRING &db,
                             const LEX_CSTRING &table_name, const LEX_CSTRING &path, const DDL_options_st options,
                             HA_CREATE_INFO *create_info, Alter_info *alter_info, int create_table_mode, bool *is_trans,
                             KEY **key_info, uint *key_count, LEX_CUSTRING *frm)
{
  LEX_CSTRING *alias;
  handler *file = 0;
  ...
  DBUG_PRINT("enter",
             ("db: '%s'  table: '%s'  tmp: %d  path: %s", db.str, table_name.str, internal_tmp_table, path.str));
  ...
  if (fix_constraints_names(thd, &alter_info->check_constraint_list, create_info))
    DBUG_RETURN(1);
  ...
  {
    if (unlikely(error_if_data_home_dir(create_info->data_file_name, "DATA DIRECTORY")) ||
        unlikely(error_if_data_home_dir(create_info->index_file_name, "INDEX DIRECTORY")) ||
  ...
  if (create_info->tmp_table())
  {
    ...
    */
    TABLE *tmp_table = thd->find_temporary_table(db.str, table_name.str, THD::TMP_TABLE_ANY);
  ...
  else
  {
    ...
    if (!internal_tmp_table &&
        ha_table_exists(thd, &db, &table_name, &create_info->org_tabledef_version, NULL, &db_type))
    {
      ...
      if (options.or_replace())
      {
        ...
        */
        ...
        if (mysql_rm_table_no_locks(thd, &table_list, &thd->db, ddl_log_state_rm, 0, 0, 0, 0, 1, 1))
          goto err;
        ...
        */
        thd->variables.option_bits |= OPTION_BINLOG_THIS;
        create_info->table_was_deleted = 1;
        lex_string_set(&create_info->org_storage_engine_name, ha_resolve_storage_engine_name(db_type));
        DBUG_EXECUTE_IF("send_kill_after_delete", thd->set_killed(KILL_QUERY););
        /*
          Restart statement transactions for the case of CREATE ... SELECT.
        */
        if (thd->lex->first_select_lex()->item_list.elements && restart_trans_for_tables(thd, thd->lex->query_tables))
          goto err;
      }
      else if (options.if_not_exists())
      {
  ...
  if (create_table_mode == C_ASSISTED_DISCOVERY)
  {
    ...
    */
    if (ddl_log_state_create)
      ddl_log_create_table(ddl_log_state_create, (handlerton *)0, &path, &db, &table_name, 1);
  ...
  {
    if (ddl_log_state_create)
      ddl_log_create_table(ddl_log_state_create, create_info->db_type, &path, &db, &table_name, frm_only);
    debug_crash_here("ddl_log_create_before_create_frm");

    file = mysql_create_frm_image(thd, orig_db, orig_table_name, create_info, alter_info, create_table_mode, key_info,
                                  key_count, frm);
    /*
    ...
    */
    ...
    if (!frm_only)
    {
      ...
      if (ha_create_table(thd, path.str, db.str, table_name.str, create_info, frm, 0))
      {
  ...
  {
    TABLE *table = thd->create_and_open_tmp_table(frm, path.str, db.str, table_name.str, false);
...
warn:
  error = -1;
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_TABLE_EXISTS_ERROR, ER_THD(thd, ER_TABLE_EXISTS_ERROR),
                      alias->str);
  goto err;
}
...
int mysql_create_table_no_lock(THD *thd, DDL_LOG_STATE *ddl_log_state_create, DDL_LOG_STATE *ddl_log_state_rm,
                               const LEX_CSTRING *db, const LEX_CSTRING *table_name,
                               ...
                               int create_table_mode, TABLE_LIST *table_list)
{
  ...
  else
  {
    ...
    path_length = build_table_filename(path, sizeof(path) - 1, db->str, alias->str, "", 0);
  ...
  res = create_table_impl(thd, ddl_log_state_create, ddl_log_state_rm, *db, *table_name, *db, *table_name, cpath,
                          *create_info, create_info, alter_info, create_table_mode, is_trans, &not_used_1, &not_used_2,
                          &frm);
  my_free(const_cast<uchar *>(frm.str));
  ...
  if (!res && create_info->sequence)
  {
    ...
    if (res)
    {
      ...
      if (!mysql_rm_table_no_locks(thd, table_list, &thd->db, (DDL_LOG_STATE *)0, 1, create_info->tmp_table(), false,
                                   true /* Sequence*/, true /* Don't log_query */, true /* Don't free locks */))
      {
        /*
...
static bool mysql_create_table(THD *thd, TABLE_LIST *create_table, Table_specification_st *create_info,
                               Alter_info *alter_info)
{
  ...
  if (result)
  {
    if (thd->slave_thread && !thd->is_error() && create_table->db_type &&
  ...
  if (mysql_create_table_no_lock(thd, &ddl_log_state_create, &ddl_log_state_rm, &create_table->db,
                                 &create_table->table_name, create_info, alter_info, &is_trans, create_table_mode,
                                 create_table) > 0)
  {
    result = 1;
    goto err;
  ...
  */
  if (thd->locked_tables_mode && pos_in_locked_tables && create_info->or_replace())
  {
...
err:
  ...
  {
    if (unlikely(result) && create_info->table_was_deleted && pos_in_locked_tables)
    {
      /*
    ...
    if (unlikely(write_bin_log(thd, result ? FALSE : TRUE, thd->query(), thd->query_length(), is_trans)))
      result = 1;
...
*/
static bool check_if_field_name_exists(const char *name, List<Create_field> *fields)
{
  Create_field *fld;
...
static char *make_unique_key_name(THD *thd, const char *field_name, KEY *start, KEY *end)
{
...
static bool make_unique_constraint_name(THD *thd, LEX_CSTRING *name, const char *own_name_base,
                                        List<Virtual_column_info> *vcol, uint *nr)
{
  ...
  for (int round = 0;; round++)
  {
    ...
    while ((check = it++))
    {
      if (check->name.str && !my_strcasecmp(system_charset_info, buff, check->name.str))
        break;
    }
...
*/
static const char *make_unique_invisible_field_name(THD *thd, const char *field_name, List<Create_field> *fields)
{
...
bool operator!=(const MYSQL_TIME &lhs, const MYSQL_TIME &rhs)
{
  return lhs.year != rhs.year || lhs.month != rhs.month || lhs.day != rhs.day || lhs.hour != rhs.hour ||
         lhs.minute != rhs.minute || lhs.second_part != rhs.second_part || lhs.neg != rhs.neg ||
                        ...
                        const LEX_CSTRING *new_db, const LEX_CSTRING *new_name, LEX_CUSTRING *id, uint flags)
{
  ...
  DBUG_ASSERT(base);
  DBUG_PRINT("enter", ("old: '%s'.'%s'  new: '%s'.'%s'", old_db->str, old_name->str, new_db->str, new_name->str));
  ...
  build_table_filename(from, sizeof(from) - 1, old_db->str, old_name->str, "", flags & FN_FROM_IS_TMP);
  length = build_table_filename(to, sizeof(to) - 1, new_db->str, new_name->str, "", flags & FN_TO_IS_TMP);
  // Check if we hit FN_REFLEN bytes along with file extension.
  ...
  {
    build_lower_case_table_filename(lc_from, sizeof(lc_from) - 1, old_db, old_name, flags & FN_FROM_IS_TMP);
    build_lower_case_table_filename(lc_to, sizeof(lc_from) - 1, new_db, new_name, flags & FN_TO_IS_TMP);
    from_base = lc_from;
    to_base = lc_to;
  }
  ...
  if (likely(error == 0))
  {
    PSI_CALL_drop_table_share(flags & FN_FROM_IS_TMP, old_db->str, (uint)old_db->length, old_name->str,
                              (uint)old_name->length);
  }
...
static bool mysql_create_like_table(THD *thd, TABLE_LIST *table, TABLE_LIST *src_table,
                                    Table_specification_st *create_info)
{
...
#ifdef WITH_WSREP
  if (WSREP(thd) && !thd->wsrep_applier && wsrep_create_like_table(thd, table, src_table, create_info))
  {
    DBUG_RETURN(res);
  }
#endif
  ...
  */
  ...
  if (mysql_prepare_alter_table(thd, src_table->table, &local_create_info, &local_alter_info, &local_alter_ctx))
    goto err;
#ifdef WITH_PARTITION_STORAGE_ENGINE
  ...
  */
  ...
  if (src_table->table->versioned() &&
      local_create_info.vers_info.fix_create_like(local_alter_info, local_create_info, *src_table, *table))
  {
    goto err;
  }
  ...
  res = ((create_res = mysql_create_table_no_lock(thd, &ddl_log_state_create, &ddl_log_state_rm, &table->db,
                                                  &table->table_name, &local_create_info, &local_alter_info, &is_trans,
                                                  C_ORDINARY_CREATE, table)) > 0);
  ...
  else
  {
    ...
    */
    DBUG_ASSERT((create_info->tmp_table()) || create_res < 0 ||
                thd->mdl_context.is_lock_owner(MDL_key::TABLE, table->db.str, table->table_name.str, MDL_EXCLUSIVE) ||
                (thd->locked_tables_mode && pos_in_locked_tables && create_info->if_not_exists()));
  }
  ...
  */
  force_generated_create = (((src_table->table->file->partition_ht()->flags & HTON_TABLE_MAY_NOT_EXIST_ON_SLAVE) &&
        ...
        */
        if (!table->view)
        {
          ...
          */
          ...
          int result __attribute__((unused)) = show_create_table(thd, table, &query, create_info, WITH_DB_NAME);
  ...
  {
    DBUG_PRINT("info", ("res: %d  tmp_table: %d  create_info->table: %p", res, create_info->tmp_table(),
                        local_create_info.table));
    if (create_info->tmp_table())
    {
      thd->transaction->stmt.mark_created_temp_table();
  ...
  if (do_logging)
  {
    ...
    if (res && create_info->table_was_deleted)
    {
      ...
      */
      ...
      log_drop_table(thd, &table->db, &table->table_name, &create_info->org_storage_engine_name,
                     create_info->db_type == partition_hton, &create_info->org_tabledef_version,
                     create_info->tmp_table());
    }
    else if (res != 2)  // Table was not dropped
    {
      if (write_bin_log(thd, res ? FALSE : TRUE, thd->query(), thd->query_length(), is_trans))
        res = 1;
    }
    debug_crash_here("ddl_log_create_after_binlog");
    thd->binlog_xid = 0;
...
int mysql_discard_or_import_tablespace(THD *thd, TABLE_LIST *table_list, bool discard)
{
  ...
  */
  ...
  if (open_and_lock_tables(thd, table_list, FALSE, 0, &alter_prelocking_strategy))
  {
...
static bool is_candidate_key(KEY *key)
{
  ...
  if (!(key->flags & HA_NOSAME) || (key->flags & HA_NULL_PART_KEY) || (key->flags & HA_KEY_HAS_PART_KEY_SEG))
    return false;
...
static bool handle_if_exists_options(THD *thd, TABLE *table, Alter_info *alter_info, Table_period_info *period_info)
{
  Field **f_ptr;
  ...
  /* Handle ADD COLUMN IF NOT EXISTS. */
  {
    ...
    while ((sql_field = it++))
    {
      ...
      */
      ...
      {
        if (lex_string_cmp(system_charset_info, &sql_field->field_name, &(*f_ptr)->field_name) == 0)
      ...
      }
      {
        ...
        */
        ...
        {
          if (lex_string_cmp(system_charset_info, &sql_field->field_name, &chk_field->field_name) == 0)
            goto drop_create_field;
        }
      }
      continue;
    drop_create_field:
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_DUP_FIELDNAME, ER_THD(thd, ER_DUP_FIELDNAME),
  ...
  /* Handle MODIFY COLUMN IF EXISTS. */
  {
    ...
    while ((sql_field = it++))
    {
      ...
      */
      for (f_ptr = table->field; *f_ptr; f_ptr++)
      {
        if (lex_string_cmp(system_charset_info, &sql_field->change, &(*f_ptr)->field_name) == 0)
        {
          break;
        }
      }
      ...
      {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_BAD_FIELD_ERROR, ER_THD(thd, ER_BAD_FIELD_ERROR),
  ...
  /* Handle ALTER/RENAME COLUMN IF EXISTS. */
  {
    ...
    while ((acol = it++))
    {
      ...
      */
      for (f_ptr = table->field; *f_ptr; f_ptr++)
      {
        if (my_strcasecmp(system_charset_info, acol->name.str, (*f_ptr)->field_name.str) == 0)
          break;
      }
      if (unlikely(*f_ptr == NULL))
      {
  ...
  /* Handle DROP COLUMN/KEY IF EXISTS. */
  {
    ...
    while ((drop = drop_it++))
    {
      ...
      if (drop->type == Alter_drop::COLUMN)
      {
        ...
        */
        for (f_ptr = table->field; *f_ptr; f_ptr++)
        {
          if (my_strcasecmp(system_charset_info, drop->name, (*f_ptr)->field_name.str) == 0)
          {
            remove_drop = FALSE;
            break;
          }
      ...
      else if (drop->type == Alter_drop::CHECK_CONSTRAINT)
      {
        ...
        {
          if (my_strcasecmp(system_charset_info, drop->name, table->check_constraints[i]->name.str) == 0)
          {
            remove_drop = FALSE;
            break;
          }
      ...
      else /* Alter_drop::KEY and Alter_drop::FOREIGN_KEY */
      {
        ...
        if (drop->type != Alter_drop::FOREIGN_KEY)
        {
          ...
          {
            if (my_strcasecmp(system_charset_info, drop->name, table->key_info[n_key].name.str) == 0)
            {
              remove_drop = FALSE;
              break;
            }
        ...
        else
        {
          ...
          {
            if (my_strcasecmp(system_charset_info, f_key->foreign_id->str, drop->name) == 0)
            {
              remove_drop = FALSE;
              break;
            }
      ...
      if (!remove_drop)
      {
        ...
        */
        ...
        {
          if (drop->type == chk_drop->type && my_strcasecmp(system_charset_info, drop->name, chk_drop->name) == 0)
          {
            remove_drop = TRUE;
            break;
          }
      ...
      if (remove_drop)
      {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_CANT_DROP_FIELD_OR_KEY,
                            ER_THD(thd, ER_CANT_DROP_FIELD_OR_KEY), drop->type_name(), drop->name);
        drop_it.remove();
      }
      else
        left_flags |= cur_flag;
    ...
    alter_info->flags &= ~(ALTER_PARSER_DROP_COLUMN | ALTER_DROP_INDEX | ALTER_DROP_FOREIGN_KEY);
  ...
  /* Handle RENAME KEY IF EXISTS. */
  {
    ...
    while ((rename_key = rename_key_it++))
    {
      ...
      {
        if (my_strcasecmp(system_charset_info, rename_key->old_name.str, table->key_info[n_key].name.str) == 0)
        {
          exists = true;
          break;
        }
      ...
      if (exists)
        continue;
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_KEY_DOES_NOT_EXISTS,
                          ER_THD(thd, ER_KEY_DOES_NOT_EXISTS), rename_key->old_name.str, table->s->table_name.str);
  ...
  /* Handle ALTER KEY IF EXISTS. */
  {
    ...
    while ((aii = ignor_it++))
    {
      ...
      for (uint n_key = 0; n_key < table->s->keys; n_key++)
      {
        if (my_strcasecmp(system_charset_info, aii->name(), table->key_info[n_key].name.str) == 0)
        {
          exists = true;
          break;
        }
      ...
      if (exists)
        continue;
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_KEY_DOES_NOT_EXISTS,
                          ER_THD(thd, ER_KEY_DOES_NOT_EXISTS), aii->name(), table->s->table_name.str);
      ignor_it.remove();
    }
  }
  ...
  /* ALTER TABLE ADD FOREIGN KEY IF NOT EXISTS */
  {
    ...
    while ((key = key_it++))
    {
      ...
      bool dup_primary_key = key->type == Key::PRIMARY && table->s->primary_key != MAX_KEY &&
      ...
      if (key->type != Key::FOREIGN_KEY)
      {
        for (n_key = 0; n_key < table->s->keys; n_key++)
        {
          if (my_strcasecmp(system_charset_info, keyname, table->key_info[n_key].name.str) == 0)
          {
            goto remove_key;
          }
        }
      ...
      else
      {
        ...
        while ((f_key = fk_key_it++))
        {
          if (my_strcasecmp(system_charset_info, f_key->foreign_id->str, keyname) == 0)
            goto remove_key;
        }
      }

      {
        ...
        while ((chk_key = chk_it++) && chk_key != key)
        {
          ...
          if (key->type == chk_key->type && my_strcasecmp(system_charset_info, keyname, chkname) == 0)
            goto remove_key;
        }
      }
      continue;
    ...
    remove_key:
      ...
      {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_DUP_KEYNAME,
                            ER_THD(thd, dup_primary_key ? ER_MULTIPLE_PRI_KEY : ER_DUP_KEYNAME), keyname);
        key_it.remove();
        if (key->type == Key::FOREIGN_KEY)
        {
      ...
      else
      {
        ...
        Alter_drop::drop_type type = (key->type == Key::FOREIGN_KEY) ? Alter_drop::FOREIGN_KEY : Alter_drop::KEY;
  ...
  if (tab_part_info)
  {
    ...
    if ((alter_info->partition_flags & ALTER_PARTITION_ADD) && thd->lex->create_info.if_not_exists())
    {
      ...
      if (alt_part_info)
      {
        ...
        while ((pe = new_part_it++))
        {
          ...
          {
            push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_SAME_NAME_PARTITION,
                                ER_THD(thd, ER_SAME_NAME_PARTITION), pe->partition_name);
    ...
    if ((alter_info->partition_flags & ALTER_PARTITION_DROP) && thd->lex->if_exists())
    {
      ...
      while ((name = names_it++))
      {
        ...
        while ((part_elem = part_it++))
        {
          if (my_strcasecmp(system_charset_info, part_elem->partition_name, name) == 0)
            break;
        }
        if (!part_elem)
        {
          push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_PARTITION_DOES_NOT_EXIST,
  ...
  /* ADD CONSTRAINT IF NOT EXISTS. */
  {
    ...
    while ((check = it++))
    {
      if (!(check->flags & VCOL_CHECK_CONSTRAINT_IF_NOT_EXISTS) && check->name.length)
        continue;
      check->flags = 0;
      for (c = share->field_check_constraints; c < share->table_check_constraints; c++)
      {
        Virtual_column_info *dup = table->check_constraints[c];
        if (dup->name.length == check->name.length &&
            lex_string_cmp(system_charset_info, &check->name, &dup->name) == 0)
        {
          push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_DUP_CONSTRAINT_NAME,
                              ER_THD(thd, ER_DUP_CONSTRAINT_NAME), "CHECK", check->name.str);
          it.remove();
  ...
  if (period_info->create_if_not_exists && table->s->period.name && table->s->period.name.streq(period_info->name))
  {
...
static bool fix_constraints_names(THD *thd, List<Virtual_column_info> *check_constraint_list,
                                  const HA_CREATE_INFO *create_info)
{
  ...
  while ((check = it++))
  {
    if (!check->name.length)
    {
      ...
      const char *own_name_base = create_info->period_info.constr == check ? create_info->period_info.name.str : NULL;

      if (make_unique_constraint_name(thd, &check->name, own_name_base, check_constraint_list, &nr))
        DBUG_RETURN(TRUE);
    }
  }
  DBUG_RETURN(FALSE);
...
Compare_keys compare_keys_but_name(const KEY *table_key, const KEY *new_key, Alter_info *alter_info, const TABLE *table,
                                   const KEY *const new_pk, const KEY *const old_pk)
{
  if (table_key->algorithm != new_key->algorithm)
    return Compare_keys::NotEqual;

  if ((table_key->flags & HA_KEYFLAG_MASK) != (new_key->flags & HA_KEYFLAG_MASK))
    return Compare_keys::NotEqual;
  ...
  */
  ...
  if (engine_options_differ(table_key->option_struct, new_key->option_struct, table->file->ht->index_options))
    return Compare_keys::NotEqual;

  Compare_keys result = Compare_keys::Equal;

  for (const KEY_PART_INFO *key_part = table_key->key_part, *new_part = new_key->key_part,
       ...
       key_part < end; key_part++, new_part++)
  {
    ...
    */
    const Create_field &new_field = *alter_info->create_list.elem(new_part->fieldnr);

    if (!new_field.field || new_field.field->field_index != key_part->fieldnr - 1)
    {
      return Compare_keys::NotEqual;
    }
    ...
    */
    ...
    auto compare =
        table->file->compare_key_parts(*table->field[key_part->fieldnr - 1], new_field, *key_part, *new_part);
...
static bool fill_alter_inplace_info(THD *thd, TABLE *table, bool varchar, Alter_inplace_info *ha_alter_info)
{
  Field **f_ptr, *field;
  ...
  if (!(ha_alter_info->index_drop_buffer = (KEY **)thd->alloc(sizeof(KEY *) * table->s->keys)) ||
      !(ha_alter_info->index_add_buffer = (uint *)thd->alloc(sizeof(uint) * alter_info->key_list.elements)) ||
      ha_alter_info->rename_keys.reserve(ha_alter_info->index_add_count) ||
      !(ha_alter_info->index_altered_ignorability_buffer =
            (KEY_PAIR *)thd->alloc(sizeof(KEY_PAIR) * alter_info->alter_index_ignorability_list.elements)))
    DBUG_RETURN(true);
  ...
  */
  ...
  alter_table_operations flags_to_remove = ALTER_ADD_INDEX | ALTER_DROP_INDEX | ALTER_PARSER_ADD_COLUMN |
  ...
  */
  ...
  for (f_ptr = table->field; (field = *f_ptr); f_ptr++, field_stored_index += field->stored_in_db())
  {
    ...
    if (new_field)
    {
      ...
      */
      ...
      if (!is_equal)
      {
        ...
        else
        {
          ...
          ha_alter_info->handler_flags |= field->stored_in_db() ? ALTER_STORED_COLUMN_TYPE : ALTER_VIRTUAL_COLUMN_TYPE;
          ...
          if (table->s->tmp_table == NO_TMP_TABLE)
          {
            ...
            for (uint i = 0; i < table->s->keys; i++, key_info++)
            {
              ...
              for (uint j = 0; j < key_parts; j++)
              {
                ...
                {
                  delete_statistics_for_index(thd, table, key_info, j >= key_info->user_defined_key_parts);
                  break;
                }
              }
            }
      ...
      if (field->vcol_info || new_field->vcol_info)
      {
        ...
        if (field->stored_in_db() != new_field->stored_in_db())
          ha_alter_info->handler_flags |= (ALTER_STORED_COLUMN_TYPE | ALTER_VIRTUAL_COLUMN_TYPE);
        if (field->vcol_info && new_field->vcol_info)
        {
          ...
          if ((ha_alter_info->handler_flags & ALTER_COLUMN_DEFAULT) && !(ha_alter_info->handler_flags & alter_expr))
          { /*
            ...
            */
            if (field->vcol_info->expr->walk(&Item::check_func_default_processor, 0, 0))
            {
              ha_alter_info->handler_flags |= alter_expr;
              value_changes = true;
            }
          ...
          if (field->vcol_info->is_in_partitioning_expr() || field->flags & PART_KEY_FLAG || field->stored_in_db())
          {
            if (value_changes)
      ...
      if (cmp(&field->field_name, &new_field->field_name))
      {
        ...
        rename_column_in_stat_tables(thd, table, field, new_field->field_name.str);
      }

      /* Check that NULL behavior is same for old and new fields */
      if ((new_field->flags & NOT_NULL_FLAG) != (uint)(field->flags & NOT_NULL_FLAG))
      {
      ...
      */
      ...
      if (engine_options_differ(field->option_struct, new_field->option_struct, table->file->ht->field_options))
      {
        ha_alter_info->handler_flags |= ALTER_COLUMN_OPTION;
        ha_alter_info->create_info->fields_option_struct[f_ptr - table->field] = new_field->option_struct;
      }
    }
    else
    {
  ...
  if (maybe_alter_vcol)
  {
    ...
    */
    if (ha_alter_info->handler_flags &
        (ALTER_STORED_COLUMN_TYPE | ALTER_VIRTUAL_COLUMN_TYPE | ALTER_COLUMN_NOT_NULLABLE | ALTER_COLUMN_OPTION))
      ha_alter_info->handler_flags |= ALTER_COLUMN_VCOL;
  }
  ...
  */
  const KEY *const new_pk =
      (ha_alter_info->key_count > 0 &&
       (!my_strcasecmp(system_charset_info, ha_alter_info->key_info_buffer->name.str, primary_key_name.str) ||
        is_candidate_key(ha_alter_info->key_info_buffer)))
          ? ha_alter_info->key_info_buffer
          : NULL;
  const KEY *const old_pk = table->s->primary_key == MAX_KEY ? NULL : table->key_info + table->s->primary_key;

  DBUG_PRINT("info", ("index count old: %d  new: %d", table->s->keys, ha_alter_info->key_count));
  ...
  */
  ...
  for (table_key = table->key_info; table_key < table_key_end; table_key++)
  {
    ...
    {
      if (!lex_string_cmp(system_charset_info, &table_key->name, &new_key->name))
        break;
    }
    if (new_key >= new_key_end)
    {
      /* Key not found. Add the key to the drop buffer. */
      ha_alter_info->index_drop_buffer[ha_alter_info->index_drop_count++] = table_key;
      DBUG_PRINT("info", ("index dropped: '%s'", table_key->name.str));
      continue;
    }

    switch (compare_keys_but_name(table_key, new_key, alter_info, table, new_pk, old_pk))
    {
  ...
  */
  for (new_key = ha_alter_info->key_info_buffer; new_key < new_key_end; new_key++)
  {
    ...
    if (table_key >= table_key_end)
    {
      ...
      ha_alter_info->create_info->indexes_option_struct[table_key - table->key_info] = new_key->option_struct;
  }
  ...
  for (uint i = 0; i < ha_alter_info->index_add_count; i++)
  {
    ...
    for (uint j = 0; j < ha_alter_info->index_drop_count; j++)
    {
      ...
      if (compare_keys_but_name(old_key, new_key, alter_info, table, new_pk, old_pk) != Compare_keys::Equal)
      {
        continue;
      }

      DBUG_ASSERT(lex_string_cmp(system_charset_info, &old_key->name, &new_key->name));

      ha_alter_info->handler_flags |= ALTER_RENAME_INDEX;
      ha_alter_info->rename_keys.push_back(Alter_inplace_info::Rename_key_pair(old_key, new_key));

      --ha_alter_info->index_add_count;
      --ha_alter_info->index_drop_count;
      memmove(add_buffer + i, add_buffer + i + 1, sizeof(add_buffer[0]) * (ha_alter_info->index_add_count - i));
      memmove(drop_buffer + j, drop_buffer + j + 1, sizeof(drop_buffer[0]) * (ha_alter_info->index_drop_count - j));
      --i;  // this index once again
      break;
    }
  }

  List_iterator<Alter_index_ignorability> ignorability_index_it(alter_info->alter_index_ignorability_list);
  ...
  */
  my_qsort(ha_alter_info->index_add_buffer, ha_alter_info->index_add_count, sizeof(uint), (qsort_cmp)compare_uint);

  /* Now let us calculate flags for storage engine API. */

  /* Figure out what kind of indexes we are dropping. */
  KEY **dropped_key;
  KEY **dropped_key_end = ha_alter_info->index_drop_buffer + ha_alter_info->index_drop_count;

  for (dropped_key = ha_alter_info->index_drop_buffer; dropped_key < dropped_key_end; dropped_key++)
  {
...
static void update_altered_table(const Alter_inplace_info &ha_alter_info, TABLE *altered_table)
{
  ...
  */
  for (add_key_idx = 0; add_key_idx < ha_alter_info.index_add_count; add_key_idx++)
  {
    key = ha_alter_info.key_info_buffer + ha_alter_info.index_add_buffer[add_key_idx];
...
bool mysql_compare_tables(TABLE *table, Alter_info *alter_info, HA_CREATE_INFO *create_info, bool *metadata_equal)
{
  ...
  */
  ...
  int create_table_mode = table->s->tmp_table == NO_TMP_TABLE ? C_ORDINARY_CREATE : C_ALTER_TABLE;
  if (mysql_prepare_create_table(thd, create_info, &tmp_alter_info, &db_options, table->file, &key_info_buffer,
                                 &key_count, create_table_mode, table->s->db, table->s->table_name))
    DBUG_RETURN(1);

  /* Some very basic checks. */
  if (table->s->fields != alter_info->create_list.elements || table->s->db_type() != create_info->db_type ||
      table->s->tmp_table || (table->s->row_type != create_info->row_type))
    DBUG_RETURN(false);
  ...
  for (Field **f_ptr = table->field; *f_ptr; f_ptr++)
  {
    ...
    if ((tmp_new_field->flags & NOT_NULL_FLAG) != (uint)(field->flags & NOT_NULL_FLAG))
      DBUG_RETURN(false);
    ...
    */
    if (create_info->row_type == ROW_TYPE_DYNAMIC || create_info->row_type == ROW_TYPE_PAGE ||
        (tmp_new_field->flags & BLOB_FLAG) ||
        (tmp_new_field->real_field_type() == MYSQL_TYPE_VARCHAR && create_info->row_type != ROW_TYPE_FIXED))
      create_info->table_options |= HA_OPTION_PACK_RECORD;

    /* Check if field was renamed */
    if (lex_string_cmp(system_charset_info, &field->field_name, &tmp_new_field->field_name))
      DBUG_RETURN(false);
  ...
  for (table_key = table->key_info; table_key < table_key_end; table_key++)
  {
    ...
    for (table_part = table_key->key_part, new_part = new_key->key_part; table_part < table_part_end;
         table_part++, new_part++)
    {
      /*
      ...
      */
      if ((table_part->length != new_part->length) || (table_part->fieldnr - 1 != new_part->fieldnr) ||
                                    ...
                                    Alter_info::enum_enable_or_disable keys_onoff)
{
  ...
  DBUG_PRINT("enter", ("table=%p were_disabled=%d on_off=%d", table, indexes_were_disabled, keys_onoff));
  ...
  if (unlikely(error))
  {
    ...
    {
      THD *thd = table->in_use;
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_ILLEGAL_HA, ER_THD(thd, ER_ILLEGAL_HA),
                          table->file->table_type(), table->s->db.str, table->s->table_name.str);
      error = 0;
    }
    else
...
static bool is_inplace_alter_impossible(TABLE *table, HA_CREATE_INFO *create_info, const Alter_info *alter_info)
{
  ...
  */
  if (table->s->mysql_version > 50700 && table->s->mysql_version < 100000 && table->s->virtual_fields)
    DBUG_RETURN(TRUE);
...
static bool notify_tabledef_changed(TABLE_LIST *table_list)
{
  ...
  if (table->file->partition_ht()->notify_tabledef_changed)
  {
    ...
    tmp_db.length = tablename_to_filename(table_list->db.str, db_buff, sizeof(db_buff));
    tmp_table.length = tablename_to_filename(table_list->table_name.str, table_buff, sizeof(table_buff));
    if ((hton->notify_tabledef_changed)(hton, &tmp_db, &tmp_table, table->s->frm_image, &table->s->tabledef_version,
                                        table->file))
    {
...
static bool mysql_inplace_alter_table(THD *thd, TABLE_LIST *table_list, TABLE *table, TABLE *altered_table,
                                      Alter_inplace_info *ha_alter_info, MDL_request *target_mdl_request,
                                      DDL_LOG_STATE *ddl_log_state, TRIGGER_RENAME_PARAM *trigger_param,
                                      Alter_table_ctx *alter_ctx, bool &partial_alter, uint64 &start_alter_id,
                                      bool if_exists)
{
  Open_table_context ot_ctx(thd, MYSQL_OPEN_REOPEN | MYSQL_OPEN_IGNORE_KILLED);
  ...
  const enum_alter_inplace_result inplace_supported = ha_alter_info->inplace_supported;
  ...
  */
  if (inplace_supported == HA_ALTER_INPLACE_EXCLUSIVE_LOCK ||
      ((inplace_supported == HA_ALTER_INPLACE_COPY_NO_LOCK || inplace_supported == HA_ALTER_INPLACE_COPY_LOCK ||
        inplace_supported == HA_ALTER_INPLACE_NOCOPY_NO_LOCK || inplace_supported == HA_ALTER_INPLACE_NOCOPY_LOCK ||
      ...
      alter_info->requested_lock == Alter_info::ALTER_TABLE_LOCK_EXCLUSIVE)
  {
    ...
    */
    close_all_tables_for_name(thd, table->s,
                              alter_ctx->is_table_renamed() ? HA_EXTRA_PREPARE_FOR_RENAME : HA_EXTRA_NOT_USED, table);
    /*
  ...
  else if (inplace_supported == HA_ALTER_INPLACE_COPY_LOCK || inplace_supported == HA_ALTER_INPLACE_COPY_NO_LOCK ||
  ...
  */
  if ((inplace_supported == HA_ALTER_INPLACE_SHARED_LOCK ||
      ...
      thd->mdl_context.upgrade_shared_lock(table->mdl_ticket, MDL_SHARED_NO_WRITE, thd->variables.lock_wait_timeout))
    goto cleanup;
  ...
  if (table->s->tmp_table == NO_TMP_TABLE)
  {
    if (write_bin_log_start_alter(thd, partial_alter, start_alter_id, if_exists))
      goto cleanup;
  }
  else if (start_alter_id)
  {
    ...
    my_error(ER_INCONSISTENT_SLAVE_TEMP_TABLE, MYF(0), thd->query(), table_list->db.str, table_list->table_name.str);
    goto cleanup;
  }

  DBUG_EXECUTE_IF("start_alter_kill_after_binlog", { DBUG_SUICIDE(); });
  ...
  */
  ddl_log_update_unique_id(ddl_log_state, table->file->table_version());
  /*
  ...
  */
      ...
      (alter_info->requested_lock != Alter_info::ALTER_TABLE_LOCK_EXCLUSIVE))
  {
    ...
    if (inplace_supported == HA_ALTER_INPLACE_COPY_LOCK || inplace_supported == HA_ALTER_INPLACE_NOCOPY_LOCK ||
        alter_info->requested_lock == Alter_info::ALTER_TABLE_LOCK_SHARED)
  ...
  */
  ...
  DBUG_EXECUTE_IF("alter_table_rollback_new_index", {
    table->file->ha_commit_inplace_alter_table(altered_table, ha_alter_info, false);
  ...
  */
  ...
  if (!(table->file->partition_ht()->flags & HTON_REQUIRES_NOTIFY_TABLEDEF_CHANGED_AFTER_COMMIT) &&
  ...
  */
  ...
  close_all_tables_for_name(thd, table->s,
                            alter_ctx->is_table_renamed() ? HA_EXTRA_PREPARE_FOR_RENAME : HA_EXTRA_NOT_USED, NULL);
  ...
  */
  /*
    ...
    such error interception).
  */
  if (mysql_rename_table(db_type, &alter_ctx->new_db, &alter_ctx->tmp_name, &alter_ctx->db, &alter_ctx->alias,
                         &alter_ctx->tmp_id, FN_FROM_IS_TMP | NO_HA_TABLE) ||
      thd->is_error())
  {
  ...
  {
    DBUG_ASSERT(!tdc_share_is_cached(thd, alter_ctx->db.str, alter_ctx->table_name.str));
    if (mysql_rename_table(db_type, &alter_ctx->db, &alter_ctx->table_name, &alter_ctx->new_db, &alter_ctx->new_alias,
                           &alter_ctx->tmp_id, 0))
    {
      /*
    ...
    if (Table_triggers_list::change_table_name(thd, trigger_param, &alter_ctx->db, &alter_ctx->alias,
                                               &alter_ctx->table_name, &alter_ctx->new_db, &alter_ctx->new_alias))
    {
      /*
        If the rename of trigger files fails, try to rename the table
        back so we at least have matching table and trigger files.
      */
      (void)mysql_rename_table(db_type, &alter_ctx->new_db, &alter_ctx->new_alias, &alter_ctx->db, &alter_ctx->alias,
                               &alter_ctx->id, NO_FK_CHECKS);
      ddl_log_disable_entry(ddl_log_state);
      DBUG_RETURN(true);
    }
    rename_table_in_stat_tables(thd, &alter_ctx->db, &alter_ctx->alias, &alter_ctx->new_db, &alter_ctx->new_alias);
...
static inline void append_drop_column(THD *thd, String *str, Field *field)
{
  if (str->length())
...
static inline void rename_field_in_list(Create_field *field, List<const char> *field_list)
{
                               ...
                               Alter_table_ctx *alter_ctx)
{
  ...
  /*
    ...
    altered here.
  */
  List<Alter_index_ignorability> alter_index_ignorability_list(alter_info->alter_index_ignorability_list,
                                                               thd->mem_root);
  ...
  uint db_create_options = (table->s->db_create_options & ~(HA_OPTION_PACK_RECORD));
  ...
  */
  ...
  if (create_info->resolve_to_charset_collation_context(thd, thd->charset_collation_context_alter_table(table->s)))
    DBUG_RETURN(true);
  ...
  if ((create_info->fields_option_struct = (ha_field_option_struct **)thd->calloc(sizeof(void *) * table->s->fields)) ==
          NULL ||
      (create_info->indexes_option_struct = (ha_index_option_struct **)thd->calloc(sizeof(void *) * table->s->keys)) ==
          NULL)
    DBUG_RETURN(1);

  if (merge_engine_options(table->s->option_list, create_info->option_list, &create_info->option_list, thd->mem_root))
    DBUG_RETURN(1);
  ...
  */
  ...
  for (f_ptr = table->field; (field = *f_ptr); f_ptr++)
  {
    ...
    while ((drop = drop_it++))
    {
      if (drop->type == Alter_drop::COLUMN && !my_strcasecmp(system_charset_info, field->field_name.str, drop->name))
        break;
    }
    /*
      DROP COLULMN xxx
    ...
    */
    if (drop && field->invisible < INVISIBLE_SYSTEM &&
        !(field->flags & VERS_SYSTEM_FIELD && !(alter_info->flags & ALTER_DROP_SYSTEM_VERSIONING)))
    {
    ...
    if (field->invisible == INVISIBLE_SYSTEM && field->flags & VERS_SYSTEM_FIELD)
    {
      vers_system_invisible = true;
    }
    /* invisible versioning column is dropped automatically on DROP SYSTEM VERSIONING */
    if (!drop && field->invisible >= INVISIBLE_SYSTEM && field->flags & VERS_SYSTEM_FIELD &&
    ...
    while ((def = def_it++))
    {
      if (def->change.str && !lex_string_cmp(system_charset_info, &field->field_name, &def->change))
        break;
    }
    ...
    else if (alter_info->flags & ALTER_DROP_SYSTEM_VERSIONING && field->flags & VERS_SYSTEM_FIELD &&
    ...
    else if (drop && field->invisible < INVISIBLE_SYSTEM && field->flags & VERS_SYSTEM_FIELD &&
             !(alter_info->flags & ALTER_DROP_SYSTEM_VERSIONING))
    {
      ...
      if (field->flags & VERS_ROW_START)
        create_info->vers_info.period.start = create_info->vers_info.as_row.start = def->field_name =
            Vers_parse_info::default_start;

      else
        create_info->vers_info.period.end = create_info->vers_info.as_row.end = def->field_name =
            Vers_parse_info::default_end;
      new_create_list.push_back(def, thd->mem_root);
      dropped_sys_vers_fields |= field->flags;
      drop_it.remove();
    }
    else if (field->invisible < INVISIBLE_SYSTEM)
    {
      ...
      */
      ...
      {
        if (!my_strcasecmp(system_charset_info, field->field_name.str, alter->name.str))
          break;
      }
  ...
  */
  if (alter_info->flags & ALTER_RENAME_COLUMN)
  {
    ...
    while ((alter = alter_it++))
    {
      if (alter->is_rename())
      {
        my_error(ER_BAD_FIELD_ERROR, MYF(0), alter->name.str, table->s->table_name.str);
        goto err;
      }
    }
    for (f_ptr = table->field; (field = *f_ptr); f_ptr++)
    {
      if (field->vcol_info)
        field->vcol_info->expr->walk(&Item::rename_fields_processor, 1, &column_rename_param);
      if (field->check_constraint)
        field->check_constraint->expr->walk(&Item::rename_fields_processor, 1, &column_rename_param);
      if (field->default_value)
        field->default_value->expr->walk(&Item::rename_fields_processor, 1, &column_rename_param);
    }
#ifdef WITH_PARTITION_STORAGE_ENGINE
    if (thd->work_part_info)
    {
      ...
      if (part_info->part_expr)
        part_info->part_expr->walk(&Item::rename_fields_processor, 1, &column_rename_param);
      if (part_info->subpart_expr)
        part_info->subpart_expr->walk(&Item::rename_fields_processor, 1, &column_rename_param);
  ...
  if ((dropped_sys_vers_fields || alter_info->flags & ALTER_DROP_PERIOD) &&
      dropped_sys_vers_fields != VERS_SYSTEM_FIELD && !vers_system_invisible)
  {
    StringBuffer<NAME_LEN * 3> tmp;
  ...
  while ((def = def_it++))  // Add new columns
  {
    ...
    if (def->change.str && !def->field)
    {
      ...
      */
      ...
      while ((find = find_it++))
      {
        if (!my_strcasecmp(system_charset_info, find->field_name.str, def->field_name.str))
          break;
      }
      ...
      {
        my_error(ER_BAD_FIELD_ERROR, MYF(0), def->change.str, table->s->table_name.str);
        goto err;
      }
    }
    /*
    ...
    */
    ...
    else
    {
      ...
      else
      {
        ...
        while ((find = find_it++))
        {
          if (!lex_string_cmp(system_charset_info, &def->after, &find->field_name))
            break;
        }
        if (unlikely(!find))
        {
          my_error(ER_BAD_FIELD_ERROR, MYF(0), def->after.str, table->s->table_name.str);
          goto err;
        }
    ...
    */
    ...
    while ((alter = alter_it++))
    {
      if (!my_strcasecmp(system_charset_info, def->field_name.str, alter->name.str))
        break;
    }
    if (alter)
    {
  ...
  {
    my_error(ER_BAD_FIELD_ERROR, MYF(0), alter_info->alter_list.head()->name.str, table->s->table_name.str);
    goto err;
  }
  if (unlikely(!new_create_list.elements))
  {
    my_message(ER_CANT_REMOVE_ALL_FIELDS, ER_THD(thd, ER_CANT_REMOVE_ALL_FIELDS), MYF(0));
    goto err;
  }
  ...
  */
  for (uint i = 0; i < table->s->keys; i++, key_info++)
  {
    ...
    const bool explicit_pk = primary_key && !my_strcasecmp(system_charset_info, key_name, primary_key_name.str);
        ...
        if (primary_key)
        {
          ...
          for (uint j = 0; j < table->s->keys; j++, tab_key_info++)
          {
            if (tab_key_info->user_defined_key_parts != tab_key_info->ext_key_parts)
              (void)delete_statistics_for_index(thd, table, tab_key_info, TRUE);
          }
        }
      }
      drop_it.remove();
    ...
    List_iterator<Alter_index_ignorability> ignorability_index_it(alter_index_ignorability_list);
    ...
    while ((rename_key = rename_key_it++))
    {
      if (!my_strcasecmp(system_charset_info, key_name, rename_key->old_name.str))
      {
        ...
        else if (!my_strcasecmp(system_charset_info, rename_key->new_name.str, primary_key_name.str))
        {
    ...
    for (uint j = 0; j < key_parts_nr; j++, key_part++)
    {
      ...
      while ((cfield = field_it++))
      {
        ...
        {
          if (!my_strcasecmp(system_charset_info, key_part_name, cfield->change.str))
            break;
        }
        else if (!my_strcasecmp(system_charset_info, key_part_name, cfield->field_name.str))
          break;
      }
      if (!cfield)
      {
      ...
      if (implicit_pk && !alter_ctx->modified_primary_key && !(cfield->flags & NOT_NULL_FLAG))
        alter_ctx->modified_primary_key = true;
      ...
      if (cfield->field)  // Not new field
      {
        ...
        if (!cfield->field->type_handler()->type_can_have_key_part() ||
            ...
            (cfield->length &&
             (((cfield->real_field_type() >= MYSQL_TYPE_TINY_BLOB && cfield->real_field_type() <= MYSQL_TYPE_BLOB)
                   ? blob_length_by_type(cfield->real_field_type())
                   : cfield->length) < key_part_length / kfield->charset()->mbmaxlen)))
          key_part_length = 0;  // Use whole field
      }
      key_part_length /= kfield->charset()->mbmaxlen;
      Key_part_spec *kps = new (thd->mem_root) Key_part_spec(&cfield->field_name, key_part_length, true);
    ...
    if (table->s->tmp_table == NO_TMP_TABLE)
    {
      if (delete_index_stat)
        (void)delete_statistics_for_index(thd, table, key_info, FALSE);
      else if (alter_ctx->modified_primary_key && key_info->user_defined_key_parts != key_info->ext_key_parts)
    ...
    if (key_parts.elements)
    {
      ...
      */
      ...
      while ((index_ignorability = ignorability_index_it++))
      {
        ...
        if (!my_strcasecmp(system_charset_info, key_name, name))
        {
          if (table->s->primary_key <= MAX_KEY && table->key_info + table->s->primary_key == key_info)
          {
      ...
      key = new (thd->mem_root) Key(key_type, &tmp_name, &key_create_info, MY_TEST(key_info->flags & HA_GENERATED_KEY),
  ...
  }
  {
    ...
    {
      if (key->type == Key::FOREIGN_KEY && ((Foreign_key *)key)->validate(new_create_list))
        goto err;
      new_key_list.push_back(key, thd->mem_root);
      if (key->name.str && !my_strcasecmp(system_charset_info, key->name.str, primary_key_name.str))
      {
        my_error(ER_WRONG_NAME_FOR_INDEX, MYF(0), key->name.str);
        goto err;
      }
  ...
  if (table->s->period.name)
  {
    ...
    {
      found = drop->type == Alter_drop::PERIOD && table->s->period.name.streq(drop->name);
    }
  ...
  if (table->s->table_check_constraints)
  {
    ...
    for (uint i = share->field_check_constraints; i < share->table_check_constraints; i++)
    {
      ...
      if (share->period.constr_name.streq(check->name.str))
      {
        if (!drop_period && !keep)
        {
          my_error(ER_PERIOD_CONSTRAINT_DROP, MYF(0), check->name.str, share->period.name.str);
          goto err;
        }
      ...
      if (keep && dropped_fields)
      {
        ...
        else if (bitmap_is_overlapping(dropped_fields, table->read_set))
        {
          ...
          my_error(ER_BAD_FIELD_ERROR, MYF(0), table->field[field_nr]->field_name.str, "CHECK");
          goto err;
        }
      }
      if (keep)
    ...
    while ((f_key = fk_key_it++))
    {
      ...
      while ((check = c_it++))
      {
        ...
        if (check->name.length == f_key->foreign_id->length &&
            my_strcasecmp(system_charset_info, f_key->foreign_id->str, check->name.str) == 0)
        {
          my_error(ER_DUP_CONSTRAINT_NAME, MYF(0), "CHECK", check->name.str);
          goto err;
        }
    ...
    while ((drop = drop_it++))
    {
      switch (drop->type)
      {
        ...
        case Alter_drop::PERIOD:
          my_error(ER_CANT_DROP_FIELD_OR_KEY, MYF(0), drop->type_name(), alter_info->drop_list.head()->name);
          goto err;
        case Alter_drop::FOREIGN_KEY:
  ...
  {
    my_error(ER_KEY_DOES_NOT_EXISTS, MYF(0), rename_key_list.head()->old_name.str, table->s->table_name.str);
    goto err;
  }

  if (alter_index_ignorability_list.elements)
  {
    my_error(ER_KEY_DOES_NOT_EXISTS, MYF(0), alter_index_ignorability_list.head()->name(), table->s->table_name.str);
    goto err;
  }
  ...
  if ((create_info->table_options & (HA_OPTION_STATS_PERSISTENT | HA_OPTION_NO_STATS_PERSISTENT)) ||
...
static Create_field *get_field_by_old_name(Alter_info *alter_info, const char *old_name)
{
  ...
  while ((new_field = new_field_it++))
  {
    if (new_field->field && (my_strcasecmp(system_charset_info, new_field->field->field_name.str, old_name) == 0))
      break;
  }
  return new_field;
}
...
static enum fk_column_change_type fk_check_column_changes(THD *thd, Alter_info *alter_info,
                                                          List<LEX_CSTRING> &fk_columns, const char **bad_column_name)
{
  ...
  *bad_column_name = NULL;
  ...
  while ((column = column_it++))
  {
    ...
    if (new_field)
    {
      ...
      if (lex_string_cmp(system_charset_info, &old_field->field_name, &new_field->field_name))
      {
        /*
      ...
      if ((old_field->is_equal(*new_field) == IS_EQUAL_NO) ||
          ((new_field->flags & NOT_NULL_FLAG) && !(old_field->flags & NOT_NULL_FLAG)))
      {
...
static bool fk_prepare_copy_alter_table(THD *thd, TABLE *table, Alter_info *alter_info, Alter_table_ctx *alter_ctx)
{
  ...
  */
  ...
  while ((f_key = fk_parent_key_it++))
  {
    ...
    while ((drop = drop_it++))
    {
      ...
      */
      if ((drop->type == Alter_drop::FOREIGN_KEY) &&
          (my_strcasecmp(system_charset_info, f_key->foreign_id->str, drop->name) == 0) &&
          (lex_string_cmp(table_alias_charset, f_key->foreign_db, &table->s->db) == 0) &&
          (lex_string_cmp(table_alias_charset, f_key->foreign_table, &table->s->table_name) == 0))
  ...
  */
  if (!fk_parent_key_list.is_empty() && !(thd->variables.option_bits & OPTION_NO_FOREIGN_KEY_CHECKS))
  ...
  while ((f_key = fk_parent_key_it++))
  {
    ...
    changes = fk_check_column_changes(thd, alter_info, f_key->referenced_fields, &bad_column_name);
    ...
    switch (changes)
    {
      ...
      case FK_COLUMN_DATA_CHANGE:
      {
        ...
        strxnmov(buff, sizeof(buff) - 1, f_key->foreign_db->str, ".", f_key->foreign_table->str, NullS);
        my_error(ER_FK_COLUMN_CANNOT_CHANGE_CHILD, MYF(0), bad_column_name, f_key->foreign_id->str, buff);
        DBUG_RETURN(true);
      }
      case FK_COLUMN_RENAMED:
        my_error(ER_ALTER_OPERATION_NOT_SUPPORTED_REASON, MYF(0), "ALGORITHM=COPY",
                 ER_THD(thd, ER_ALTER_OPERATION_NOT_SUPPORTED_REASON_FK_RENAME), "ALGORITHM=INPLACE");
        DBUG_RETURN(true);
      case FK_COLUMN_DROPPED:
      {
        ...
        my_error(ER_FK_COLUMN_CANNOT_DROP_CHILD, MYF(0), bad_column_name, f_key->foreign_id->str, buff.c_ptr());
        DBUG_RETURN(true);
      }
      default:
        DBUG_ASSERT(0);
  ...
  */
  ...
  while ((f_key = fk_key_it++))
  {
    ...
    changes = fk_check_column_changes(thd, alter_info, f_key->foreign_fields, &bad_column_name);
    ...
    switch (changes)
    {
      case FK_COLUMN_NO_CHANGE:
        ...
        break;
      case FK_COLUMN_DATA_CHANGE:
        my_error(ER_FK_COLUMN_CANNOT_CHANGE, MYF(0), bad_column_name, f_key->foreign_id->str);
        DBUG_RETURN(true);
      case FK_COLUMN_RENAMED:
        my_error(ER_ALTER_OPERATION_NOT_SUPPORTED_REASON, MYF(0), "ALGORITHM=COPY",
                 ER_THD(thd, ER_ALTER_OPERATION_NOT_SUPPORTED_REASON_FK_RENAME), "ALGORITHM=INPLACE");
        DBUG_RETURN(true);
      case FK_COLUMN_DROPPED:
        my_error(ER_FK_COLUMN_CANNOT_DROP, MYF(0), bad_column_name, f_key->foreign_id->str);
        DBUG_RETURN(true);
      default:
        DBUG_ASSERT(0);
    }
  ...
  */
  if (alter_info->flags & ALTER_ADD_FOREIGN_KEY)
  {
    ...
    while (Key *key = fk_list_it++)
    {
      ...
      char dbuf[NAME_LEN];
      char tbuf[NAME_LEN];
      const char *ref_db = (fk->ref_db.str ? fk->ref_db.str : alter_ctx->new_db.str);
      ...
      MDL_REQUEST_INIT(&mdl_request, MDL_key::TABLE, ref_db, ref_table, MDL_SHARED_NO_WRITE, MDL_TRANSACTION);
...
/**
  ...
  @return Operation status
    ...
    @retval true            Failure
*/
static bool simple_tmp_rename_or_index_change(THD *thd, TABLE_LIST *table_list,
                                              Alter_info::enum_enable_or_disable keys_onoff, Alter_table_ctx *alter_ctx)
{
  ...
  if (keys_onoff != Alter_info::LEAVE_AS_IS)
  {
    ...
    error = alter_table_manage_keys(table, table->file->indexes_are_disabled(), keys_onoff);
  }
  ...
  if (likely(!error) && alter_ctx->is_table_renamed())
  {
    ...
    */
    error = thd->rename_temporary_table(table, &alter_ctx->new_db, &alter_ctx->new_alias);
  }
...
static bool simple_rename_or_index_change(THD *thd, TABLE_LIST *table_list,
                                          Alter_info::enum_enable_or_disable keys_onoff,
                                          TRIGGER_RENAME_PARAM *trigger_param, Alter_table_ctx *alter_ctx)
{
  TABLE *table = table_list->table;
  ...
  int error = 0;
  bool partitioned;
  enum ha_extra_function extra_func = thd->locked_tables_mode ? HA_EXTRA_NOT_USED : HA_EXTRA_FORCE_REOPEN;
  ...
  if ((table_version.length = table->s->tabledef_version.length))
    memcpy((char *)table_version.str, table->s->tabledef_version.str, table_version.length);
  partitioned = table->file->partition_engine();
  storage_engine.length =
      (strmake((char *)storage_engine.str, table->file->real_table_type(), sizeof(storage_engine_buff) - 1) -
       storage_engine.str);
  ...
  if (likely(!error) && alter_ctx->is_table_renamed())
  {
    ...
    */
    ...
    (void)ddl_log_rename_table(&ddl_log_state, old_db_type, &alter_ctx->db, &alter_ctx->table_name, &alter_ctx->new_db,
                               &alter_ctx->new_alias);
    if (mysql_rename_table(old_db_type, &alter_ctx->db, &alter_ctx->table_name, &alter_ctx->new_db,
                           &alter_ctx->new_alias, &table_version, 0))
      error = -1;
    if (!error)
      ddl_log_update_phase(&ddl_log_state, DDL_RENAME_PHASE_TRIGGER);
    debug_crash_here("ddl_log_alter_before_rename_triggers");
    if (!error &&
        Table_triggers_list::change_table_name(thd, trigger_param, &alter_ctx->db, &alter_ctx->alias,
                                               &alter_ctx->table_name, &alter_ctx->new_db, &alter_ctx->new_alias))
    {
      (void)mysql_rename_table(old_db_type, &alter_ctx->new_db, &alter_ctx->new_alias, &alter_ctx->db,
                               &alter_ctx->table_name, &table_version, NO_FK_CHECKS);
      ddl_log_disable_entry(&ddl_log_state);
      error = -1;
    }
    /*
    ...
    */
    if (error == 0)
      (void)rename_table_in_stat_tables(thd, &alter_ctx->db, &alter_ctx->table_name, &alter_ctx->new_db,
                                        &alter_ctx->new_alias);
    debug_crash_here("ddl_log_alter_after_rename_triggers");
  }
...
{
  table->file->ha_create_partitioning_metadata(table->s->normalized_path.str, 0, CHF_DELETE_FLAG);
  deletefrm(table->s->normalized_path.str);
  cleanup_table_after_inplace_alter_keep_files(table);
}

static int create_table_for_inplace_alter(THD *thd, const Alter_table_ctx &alter_ctx, LEX_CUSTRING *frm,
                                          TABLE_SHARE *share, TABLE *table)
{
  init_tmp_table_share(thd, share, alter_ctx.new_db.str, 0, alter_ctx.new_name.str, alter_ctx.get_tmp_path());
  if (share->init_from_binary_frm_image(thd, true, frm->str, frm->length) ||
      open_table_from_share(thd, share, &alter_ctx.new_name, 0, EXTRA_RECORD, thd->open_options, table, false))
  {
    free_table_share(share);
...
static bool wait_for_master(THD *thd)
{
  ...
  DBUG_ASSERT(!info->direct_commit_alter || info->state == start_alter_state::ROLLBACK_ALTER);
...
{
  DBUG_ASSERT(!(thd->rgi_slave && (thd->rgi_slave->gtid_ev_flags_extra & Gtid_log_event::FL_START_ALTER_E1)) ||
              !thd->rgi_slave->direct_commit_alter);
  return thd->rgi_slave && (thd->rgi_slave->gtid_ev_flags_extra & Gtid_log_event::FL_START_ALTER_E1)
                       ...
                       uint order_num, ORDER *order, bool ignore, bool if_exists)
{
  ...
  */
  ...
  /*
    ...
    inplace alter table.
  */
  Alter_inplace_info::inplace_alter_table_commit_callback *inplace_alter_table_committed = 0;
  ...
  */
  ...
  if (table_kind)
  {
    ...
    {
    unsupported:
      my_error(ER_UNSUPORTED_LOG_ENGINE, MYF(0), hton_name(create_info->db_type)->str);
      DBUG_RETURN(true);
    }

    if (create_info->db_type == maria_hton && create_info->transactional != HA_CHOICE_NO)
      goto unsupported;
  ...
  backup_name.length = my_snprintf(backup_name_buff, sizeof(backup_name_buff) - 1, "%s-backup-%lx-%llx",
  ...
  */
  ...
  error = open_tables(thd, &table_list, &tables_opened, 0, &alter_prelocking_strategy);
  thd->open_options &= ~HA_OPEN_FOR_ALTER;
  ...
  if (table->versioned())
  {
    if (handlerton *hton1 = create_info->db_type)
    {
      ...
      if (hton1 != hton2 && (ha_check_storage_engine_flag(hton1, HTON_NATIVE_SYS_VERSIONING) ||
                             ha_check_storage_engine_flag(hton2, HTON_NATIVE_SYS_VERSIONING)))
      {
        my_error(ER_VERS_ALTER_ENGINE_PROHIBITED, MYF(0), table_list->db.str, table_list->table_name.str);
        DBUG_RETURN(true);
      }
    }
    if (alter_info->vers_prohibited(thd))
    {
      my_error(ER_VERS_ALTER_NOT_ALLOWED, MYF(0), table_list->db.str, table_list->table_name.str);
      DBUG_RETURN(true);
    }
  }
  ...
  */
  if (table->file->check_if_updates_are_ignored("ALTER"))
  {
    ...
    */
    ...
    if (thd->mdl_context.upgrade_shared_lock(mdl_ticket, MDL_EXCLUSIVE, thd->variables.lock_wait_timeout))
      DBUG_RETURN(1);
    quick_rm_table(thd, table->file->ht, &table_list->db, &table_list->table_name, NO_HA_TABLE, 0);
    goto end_inplace;
  }
  if (!if_exists && (table->file->partition_ht()->flags & HTON_TABLE_MAY_NOT_EXIST_ON_SLAVE))
  {
    /*
  ...
  */
  if ((thd->locked_tables_mode == LTM_LOCK_TABLES || thd->locked_tables_mode == LTM_PRELOCKED_UNDER_LOCK_TABLES) &&
      (create_info->used_fields & HA_CREATE_USED_UNION) && (table->s->tmp_table == NO_TMP_TABLE))
  {
  ...
  if (alter_ctx.is_table_renamed())
  {
    ...
    else
    {
      ...
      MDL_REQUEST_INIT(&target_mdl_request, MDL_key::TABLE, alter_ctx.new_db.str, alter_ctx.new_name.str, MDL_EXCLUSIVE,
                       MDL_TRANSACTION);
      ...
      */
      ...
      {
        MDL_REQUEST_INIT(&target_db_mdl_request, MDL_key::SCHEMA, alter_ctx.new_db.str, "", MDL_INTENTION_EXCLUSIVE,
                         MDL_TRANSACTION);
  ...
  if ((create_info->db_type != table->s->db_type() || (alter_info->partition_flags & ALTER_PARTITION_INFO)) &&
  ...
  */
  ...
  DBUG_PRINT("info", ("old type: %s  new type: %s", ha_resolve_storage_engine_name(old_db_type),
                      ha_resolve_storage_engine_name(new_db_type)));
  if (ha_check_storage_engine_flag(old_db_type, HTON_ALTER_NOT_SUPPORTED))
  {
    DBUG_PRINT("info", ("doesn't support alter"));
    my_error(ER_ILLEGAL_HA, MYF(0), hton_name(old_db_type)->str, alter_ctx.db.str, alter_ctx.table_name.str);
    DBUG_RETURN(true);
  }
  ...
  {
    DBUG_PRINT("info", ("doesn't support alter"));
    my_error(ER_ILLEGAL_HA, MYF(0), hton_name(new_db_type)->str, alter_ctx.new_db.str, alter_ctx.new_name.str);
    DBUG_RETURN(true);
  }
...

        {
          ...
          for (n_key = 0; n_key < table->s->keys; n_key++)
          {
            if ((table->key_info[n_key].flags & HA_NOSAME) &&
                my_strcasecmp(system_charset_info, drop->name, table->key_info[n_key].name.str) == 0)
            {
  ...
  if (handle_if_exists_options(thd, table, alter_info, &create_info->period_info) ||
      fix_constraints_names(thd, &alter_info->check_constraint_list, create_info))
    DBUG_RETURN(true);

  /* Check if rename of triggers are supported */
  if (alter_ctx.is_table_renamed() &&
      Table_triggers_list::prepare_for_rename(thd, &trigger_param, &alter_ctx.db, &alter_ctx.alias,
                                              &alter_ctx.table_name, &alter_ctx.new_db, &alter_ctx.new_alias))
    DBUG_RETURN(true);
  ...
  */
  ...
  {
    my_snprintf(alter_ctx.tmp_buff, sizeof(alter_ctx.tmp_buff), ER_THD(thd, ER_INSERT_INFO), 0L, 0L,
  ...
  */
  if (!(alter_info->flags & ~(ALTER_RENAME | ALTER_KEYS_ONOFF)) && alter_info->partition_flags == 0 &&
      alter_info->algorithm(thd) != Alter_info::ALTER_TABLE_ALGORITHM_COPY)  // No need to touch frm.
  {
    bool res;
    ...
    if (!table->s->tmp_table)
    {
      ...
      if (alter_info->requested_lock != Alter_info::ALTER_TABLE_LOCK_DEFAULT &&
          alter_info->requested_lock != Alter_info::ALTER_TABLE_LOCK_EXCLUSIVE)
      {
        my_error(ER_ALTER_OPERATION_NOT_SUPPORTED, MYF(0), "LOCK=NONE/SHARED", "LOCK=EXCLUSIVE");
        DBUG_RETURN(true);
      }
      res = simple_rename_or_index_change(thd, table_list, alter_info->keys_onoff, &trigger_param, &alter_ctx);
    }
    else
    {
      res = simple_tmp_rename_or_index_change(thd, table_list, alter_info->keys_onoff, &alter_ctx);
    }
    DBUG_RETURN(res);
  }
  ...
  */
  ...
  if (create_info->check_fields(thd, alter_info, table_list->table_name, table_list->db) ||
  ...
  if (fast_alter_partition)
  {
    ...
    */
    if (alter_info->requested_lock != Alter_info::ALTER_TABLE_LOCK_DEFAULT)
    {
      my_error(ER_ALTER_OPERATION_NOT_SUPPORTED_REASON, MYF(0), "LOCK=NONE/SHARED/EXCLUSIVE",
               ER_THD(thd, ER_ALTER_OPERATION_NOT_SUPPORTED_REASON_PARTITION), "LOCK=DEFAULT");
      DBUG_RETURN(true);
    }
    else if (alter_info->algorithm(thd) != Alter_info::ALTER_TABLE_ALGORITHM_DEFAULT)
    {
      my_error(ER_ALTER_OPERATION_NOT_SUPPORTED_REASON, MYF(0), "ALGORITHM=COPY/INPLACE",
               ER_THD(thd, ER_ALTER_OPERATION_NOT_SUPPORTED_REASON_PARTITION), "ALGORITHM=DEFAULT");
      DBUG_RETURN(true);
    }
    ...
    */
    if ((thd->mdl_context.upgrade_shared_lock(mdl_ticket, MDL_SHARED_NO_WRITE, thd->variables.lock_wait_timeout)) ||
    ...
    DBUG_RETURN(fast_alter_partition_table(thd, table, alter_info, &alter_ctx, create_info, table_list));
  }
#endif
  ...
  */
  if ((thd->variables.alter_algorithm == Alter_info::ALTER_TABLE_ALGORITHM_COPY &&
      ...
      IF_PARTITIONING((partition_changed && !(old_db_type->partition_flags() & HA_USE_AUTO_PARTITION)), 0))
  {
    if (alter_info->algorithm(thd) == Alter_info::ALTER_TABLE_ALGORITHM_INPLACE)
    {
  ...
  /*
    ...
    rebuild.
  */
  if (new_db_type == old_db_type && create_info->used_fields & HA_CREATE_USED_ENGINE)
  ...
  */
  ...
  if (!table->s->tmp_table)
  {
    ...
    if (ddl_log_alter_table(&ddl_log_state, old_db_type, &alter_ctx.db, &alter_ctx.table_name, new_db_type,
                            table->file->partition_ht(), &alter_ctx.new_db, &alter_ctx.tmp_name, &path_to_frm,
                            &tmp_table, &create_info->tabledef_version, table->file->table_version(),
                            alter_ctx.is_table_renamed()) ||
        ddl_log_store_query(thd, &ddl_log_state, thd->query(), thd->query_length()))
    {
      error = 1;
      goto err_cleanup;
    }
  ...
  */
  error =
      create_table_impl(thd, (DDL_LOG_STATE *)0, (DDL_LOG_STATE *)0, alter_ctx.db, alter_ctx.table_name,
                        alter_ctx.new_db, alter_ctx.tmp_name, alter_ctx.get_tmp_cstring_path(), thd->lex->create_info,
                        create_info, alter_info, C_ALTER_TABLE_FRM_ONLY, NULL, &key_info, &key_count, &frm);
  reenable_binlog(thd);
  ...
  {
    Alter_inplace_info ha_alter_info(create_info, alter_info, key_info, key_count,
                                     IF_PARTITIONING(thd->work_part_info, NULL), ignore, alter_ctx.error_if_not_empty);
    ...
    alter_ctx.tmp_storage_engine_name_partitioned = table->file->partition_engine();
    alter_ctx.tmp_storage_engine_name.length =
        (strmake((char *)alter_ctx.tmp_storage_engine_name.str, table->file->real_table_type(),
    ...
    */
    ...
    if (!(ha_alter_info.handler_flags & ~(ALTER_COLUMN_ORDER | ALTER_RENAME_COLUMN | ALTER_INDEX_ORDER)))
    {
      /*
        ...
        TODO: Now case-change index name is treated as noop which is not quite
              correct.
      */
      table->file->ha_create_partitioning_metadata(alter_ctx.get_tmp_path(), NULL, CHF_DELETE_FLAG);
      goto end_inplace;
    }
    ...
    if (create_table_for_inplace_alter(thd, alter_ctx, &frm, &altered_share, &altered_table))
      goto err_new_table_cleanup;
    /*
    ...
    */
    ...
    /*
      ...
      ALTER TABLE.
    */
    altered_table.column_bitmaps_set_no_signal(&altered_table.s->all_set, &altered_table.s->all_set);
    ...
    if (altered_table.default_field && altered_table.update_default_fields(true))
    {
    ...
    // Ask storage engine whether to use copy or in-place
    {
      ...
      ha_alter_info.inplace_supported = table->file->check_if_supported_inplace_alter(&altered_table, &ha_alter_info);
    }
    ...
    if (alter_info->supports_algorithm(thd, &ha_alter_info) || alter_info->supports_lock(thd, &ha_alter_info))
    {
      cleanup_table_after_inplace_alter(&altered_table);
      goto err_new_table_cleanup;
    }
    ...
    if (use_inplace)
    {
      ...
      */
      ...
      int res = mysql_inplace_alter_table(thd, table_list, table, &altered_table, &ha_alter_info, &target_mdl_request,
                                          &ddl_log_state, &trigger_param, &alter_ctx, partial_alter, start_alter_id,
                                          if_exists);
      thd->count_cuted_fields = org_count_cuted_fields;
      inplace_alter_table_committed = ha_alter_info.inplace_alter_table_committed;
      inplace_alter_table_committed_argument = ha_alter_info.inplace_alter_table_committed_argument;
      if (res)
      {
        cleanup_table_after_inplace_alter(&altered_table);
        goto err_cleanup;
  ...
  if (!table->s->tmp_table)
  {
    ...
    if (alter_info->requested_lock == Alter_info::ALTER_TABLE_LOCK_NONE)
    {
      my_error(ER_ALTER_OPERATION_NOT_SUPPORTED_REASON, MYF(0), "LOCK=NONE",
               ER_THD(thd, ER_ALTER_OPERATION_NOT_SUPPORTED_REASON_COPY), "LOCK=SHARED");
      goto err_new_table_cleanup;
    }
    ...
    */
    if (alter_info->requested_lock != Alter_info::ALTER_TABLE_LOCK_EXCLUSIVE &&
        thd->mdl_context.upgrade_shared_lock(mdl_ticket, MDL_SHARED_NO_WRITE, thd->variables.lock_wait_timeout))
      goto err_new_table_cleanup;
  ...
  DBUG_EXECUTE_IF("start_alter_delay_master",
                  { debug_sync_set_action(thd, STRING_WITH_LEN("now wait_for alter_cont")); });
  // It's now safe to take the table level lock.
  if (lock_tables(thd, table_list, alter_ctx.tables_opened, MYSQL_LOCK_USE_MALLOC))
    goto err_new_table_cleanup;

  ddl_log_update_phase(&ddl_log_state, DDL_ALTER_TABLE_PHASE_CREATED);

  if (ha_create_table(thd, alter_ctx.get_tmp_path(), alter_ctx.new_db.str, alter_ctx.new_name.str, create_info, &frm,
                      frm_is_created))
    goto err_new_table_cleanup;
  ...
  new_table = thd->create_and_open_tmp_table(&frm, alter_ctx.get_tmp_path(), alter_ctx.new_db.str,
  ...
  */
    ...
    */
        ...
        thd->binlog_table_should_be_logged(&new_table->s->db))
    {
      ...
      */
      ...
      res = (binlog_drop_table(thd, table) || binlog_create_table(thd, new_table, 1));
    ...
    if (copy_data_between_tables(thd, table, new_table, alter_info->create_list, ignore, order_num, order, &copied,
                                 &deleted, alter_info->keys_onoff, &alter_ctx))
      goto err_new_table_cleanup;
  }
  else
  {
    if (!table->s->tmp_table && wait_while_table_is_used(thd, table, HA_EXTRA_FORCE_REOPEN))
      goto err_new_table_cleanup;
    THD_STAGE_INFO(thd, stage_manage_keys);
    alter_table_manage_keys(table, table->file->indexes_are_disabled(), alter_info->keys_onoff);
  ...
  if (table->s->tmp_table != NO_TMP_TABLE)
  {
    ...
    new_table->s->table_creation_was_logged = table->s->table_creation_was_logged;
    /* Remove link to old table and rename the new one */
    thd->drop_temporary_table(table, NULL, true);
    /* Should pass the 'new_name' as we store table name in the cache */
    if (thd->rename_temporary_table(new_table, &alter_ctx.new_db, &alter_ctx.new_name))
      goto err_new_table_cleanup;
    ...
    /* We don't replicate alter table statement on temporary tables */
    if (!thd->is_current_stmt_binlog_format_row() && table_creation_was_logged && !binlog_as_create_select)
    {
      int tmp_error;
      thd->binlog_xid = thd->query_id;
      ddl_log_update_xid(&ddl_log_state, thd->binlog_xid);
  ...
  alter_ctx.tmp_storage_engine_name_partitioned = new_table->file->partition_engine();
  alter_ctx.tmp_storage_engine_name.length =
      (strmake((char *)alter_ctx.tmp_storage_engine_name.str, new_table->file->real_table_type(),
               sizeof(alter_ctx.tmp_storage_engine_buff) - 1) -
       alter_ctx.tmp_storage_engine_name.str);
  ...
  */
  engine_changed =
      ((new_table->file->ht != table->file->ht) &&
       ((!(new_table->file->ha_table_flags() & HA_FILE_BASED) || !(table->file->ha_table_flags() & HA_FILE_BASED))) &&
       !(table->file->ha_table_flags() & HA_REUSES_FILE_NAMES) &&
       !(new_table->file->ha_table_flags() & HA_REUSES_FILE_NAMES));
  /*
  ...
  */
  ...
  DBUG_PRINT("info", ("is_table_renamed: %d  engine_changed: %d", alter_ctx.is_table_renamed(), engine_changed));
  ...
  */
  if (!alter_ctx.is_table_renamed() || alter_ctx.fk_error_if_delete_row)
  {
    ...
    */
    if (mysql_rename_table(old_db_type, &alter_ctx.db, &alter_ctx.table_name, &alter_ctx.db, &backup_name,
                           &alter_ctx.id, FN_TO_IS_TMP | (engine_changed ? NO_HA_TABLE | NO_PAR_TABLE : 0)))
    {
      // Rename to temporary name failed, delete the new table, abort ALTER.
      (void)quick_rm_table(thd, new_db_type, &alter_ctx.new_db, &alter_ctx.tmp_name, FN_IS_TMP);
      goto err_with_mdl;
    }
  }
  else
  {
    ...
    PSI_CALL_drop_table_share(0, alter_ctx.db.str, (int)alter_ctx.db.length, alter_ctx.table_name.str,
                              (int)alter_ctx.table_name.length);
  }
  ...
  if (mysql_rename_table(new_db_type, &alter_ctx.new_db, &alter_ctx.tmp_name, &alter_ctx.new_db, &alter_ctx.new_alias,
                         &alter_ctx.tmp_id, FN_FROM_IS_TMP))
  {
    ...
    if (!alter_ctx.is_table_renamed() || alter_ctx.fk_error_if_delete_row)
    {
      ...
      (void)mysql_rename_table(old_db_type, &alter_ctx.db, &backup_name, &alter_ctx.db, &alter_ctx.alias, &alter_ctx.id,
                               FN_FROM_IS_TMP | NO_FK_CHECKS | (engine_changed ? NO_HA_TABLE | NO_PAR_TABLE : 0));
    }
    goto err_with_mdl;
  }
  ...
  if (alter_ctx.is_table_renamed())
  {
                                               ...
                                               &alter_ctx.table_name, &alter_ctx.new_db, &alter_ctx.new_alias))
    {
      ...
      (void)quick_rm_table(thd, new_db_type, &alter_ctx.new_db, &alter_ctx.new_alias, 0);
      // Restore the backup of the original table to the old name.
      (void)mysql_rename_table(old_db_type, &alter_ctx.db, &backup_name, &alter_ctx.db, &alter_ctx.alias, &alter_ctx.id,
                               FN_FROM_IS_TMP | NO_FK_CHECKS | (engine_changed ? NO_HA_TABLE | NO_PAR_TABLE : 0));
      goto err_with_mdl;
...
end_inplace:
  ...
  DBUG_ASSERT(!(mysql_bin_log.is_open() && thd->is_current_stmt_binlog_format_row() && (create_info->tmp_table())));
  ...
  else if (!binlog_as_create_select)
  {
    ...
    thd->binlog_xid = thd->query_id;
    ddl_log_update_xid(&ddl_log_state, thd->binlog_xid);
    tmp_error = write_bin_log_with_if_exists(thd, true, false, log_if_exists, partial_alter);
    thd->binlog_xid = 0;
    if (tmp_error)
      goto err_cleanup;
  }
...
end_temporary:
  ...
  my_snprintf(alter_ctx.tmp_buff, sizeof(alter_ctx.tmp_buff), ER_THD(thd, ER_INSERT_INFO), (ulong)(copied + deleted),
              (ulong)deleted, (ulong)thd->get_stmt_da()->current_statement_warn_count());
...
err_new_table_cleanup:
  ...
  /*
    ...
    Report error here.
  */
  if (unlikely(alter_ctx.error_if_not_empty && thd->get_stmt_da()->current_row_for_warning()))
  {
    Abort_on_warning_instant_set aws(thd, true);
    alter_ctx.report_implicit_default_value_error(thd, new_table ? new_table->s : table->s);
  }
  ...
  if (new_table)
  {
    ...
    (void)quick_rm_table(thd, new_db_type, &alter_ctx.new_db, &alter_ctx.tmp_name,
                         (FN_IS_TMP | (no_ha_table ? NO_HA_TABLE : 0)), alter_ctx.get_tmp_path());
...
err_cleanup:
  ...
  if (partial_alter || start_alter_id)
    write_bin_log_start_alter_rollback(thd, start_alter_id, partial_alter, if_exists);
  DBUG_RETURN(true);
...
static int copy_data_between_tables(THD *thd, TABLE *from, TABLE *to, List<Create_field> &create, bool ignore,
                                    uint order_num, ORDER *order, ha_rows *copied, ha_rows *deleted,
                                    Alter_info::enum_enable_or_disable keys_onoff, Alter_table_ctx *alter_ctx)
{
  int error = 1;
  ...
  if (order)
  {
    if (to->s->primary_key != MAX_KEY && to->file->ha_table_flags() & HA_TABLE_SCAN_ON_INDEX)
    {
      char warn_buff[MYSQL_ERRMSG_SIZE];
      ...
      push_warning(thd, Sql_condition::WARN_LEVEL_WARN, ER_UNKNOWN_ERROR, warn_buff);
    }
    else
    {
      ...
      if (thd->lex->first_select_lex()->setup_ref_array(thd, order_num) ||
          setup_order(thd, thd->lex->first_select_lex()->ref_pointer_array, &tables, fields, all_fields, order))
        goto err;
  ...
  if (init_read_record(&info, thd, from, (SQL_SELECT *)0, file_sort, 1, 1, FALSE))
    goto err;
    ...
    if (unlikely(error))
    {
      ...
      else
      {
        ...
        if (unlikely(alter_ctx->fk_error_if_delete_row))
        {
          ...
          */
          my_error(ER_FK_CANNOT_DELETE_PARENT, MYF(0), alter_ctx->fk_error_id, alter_ctx->fk_error_table);
          break;
        }
        ...
        else
        {
          ...
          if ((int)key_nr >= 0)
          {
            ...
            if (key_nr == 0 && to->s->keys > 0 && (to->key_info[0].key_part[0].field->flags & AUTO_INCREMENT_FLAG))
              err_msg = ER_THD(thd, ER_DUP_ENTRY_AUTOINCREMENT_CASE);
            print_keydup_error(to, key_nr >= to->s->keys ? NULL : &to->key_info[key_nr], err_msg, MYF(0));
          }
          else
            to->file->print_error(error, MYF(0));
          break;
...
err:
  ...
  *deleted = delete_count;
  ...
  if (to->file->ha_external_unlock(thd))
    error = 1;
  if (error < 0 && !from->s->tmp_table && to->file->extra(HA_EXTRA_PREPARE_FOR_RENAME))
    error = 1;
...
bool mysql_recreate_table(THD *thd, TABLE_LIST *table_list, bool table_copy)
{
  ...
  bool res = mysql_alter_table(thd, &null_clex_str, &null_clex_str, &create_info, table_list, &alter_info, 0,
                               (ORDER *)0, 0, 0);
...
void fill_checksum_table_metadata_fields(THD *thd, List<Item> *fields)
{
  ...
  item = new (thd->mem_root) Item_int(thd, "Checksum", (longlong)1, MY_INT64_NUM_DECIMAL_DIGITS);
  item->set_maybe_null();
  fields->push_back(item, thd->mem_root);
}
...
bool mysql_checksum_table(THD *thd, TABLE_LIST *tables, HA_CHECK_OPT *check_opt)
{
  ...
  */
  ...
  for (table = tables; table; table = table->next_local)
  {
    ...
    table_name.length = strxmov(table_name_buff, table->db.str, ".", table->table_name.str, NullS) - table_name_buff;
    ...
    if (thd->open_temporary_tables(table) || open_and_lock_tables(thd, table, FALSE, 0))
    {
      t = NULL;
    }
    else
    ...
    else
    {
      ...
      if (!(check_opt->flags & T_EXTEND) && (((t->file->ha_table_flags() & HA_HAS_OLD_CHECKSUM) &&
...
bool check_engine(THD *thd, const char *db_name, const char *table_name, HA_CREATE_INFO *create_info)
{
  ...
  *new_engine = ha_checktype(thd, req_engine, no_substitution);
  ...
  if (!(thd->lex->sql_command == SQLCOM_ALTER_TABLE && !(create_info->used_fields & HA_CREATE_USED_ENGINE)))
    enf_engine = thd->variables.enforced_table_plugin ? plugin_hton(thd->variables.enforced_table_plugin) : NULL;
  ...
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_WARN_USING_OTHER_HANDLER,
                        ER_THD(thd, ER_WARN_USING_OTHER_HANDLER), ha_resolve_storage_engine_name(*new_engine),
                        table_name);
  }
  if (create_info->tmp_table() && ha_check_storage_engine_flag(*new_engine, HTON_TEMPORARY_NOT_SUPPORTED))
  {
    my_error(ER_ILLEGAL_HA_CREATE_OPTION, MYF(0), hton_name(*new_engine)->str, "TEMPORARY");
    *new_engine = 0;
    DBUG_RETURN(true);
  }
  lex_string_set(&create_info->new_storage_engine_name, ha_resolve_storage_engine_name(*new_engine));
  DBUG_RETURN(false);
}
...
bool Sql_cmd_create_table_like::execute(THD *thd)
{
  ...
  if (lex->create_info.resolve_to_charset_collation_context(
          thd, thd->charset_collation_context_create_table_in_db(first_table->db.str)))
    DBUG_RETURN(true);
  ...
  if (used_engine)
  {
    ...
    if (!lex->create_info.db_type)  // Not found, but substitution is allowed
    {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_USING_OTHER_HANDLER,
                          ER_THD(thd, ER_WARN_USING_OTHER_HANDLER), hton_name(lex->create_info.db_type)->str,
  ...
  */
  ...
  if (append_file_to_dir(thd, &create_info.data_file_name, &create_table->table_name) ||
      append_file_to_dir(thd, &create_info.index_file_name, &create_table->table_name))
    goto end_with_restore_list;
  ...
  */
  ...
  if (select_lex->item_list.elements || select_lex->tvc)  // With select or TVC
  {
    ...
    */
     ...
     */
    if (thd->query_name_consts && mysql_bin_log.is_open() && thd->wsrep_binlog_format() == BINLOG_FORMAT_STMT &&
    ...
    */
    ...
    {
      my_error(ER_WRONG_OBJECT, MYF(0), create_table->db.str, create_table->table_name.str, "BASE TABLE");
      res = 1;
      goto end_with_restore_list;
    }
    ...
    if (create_info.or_replace() && !create_info.tmp_table())
    {
      if (TABLE_LIST *duplicate = unique_table(thd, lex->query_tables, lex->query_tables->next_global,
    ...
    }
    {
      ...
      */
      if ((result =
               new (thd->mem_root) select_create(thd, create_table, &create_info, &alter_info, select_lex->item_list,
                                                 lex->duplicates, lex->ignore, select_tables)))
      {
        /*
  ...
  else
  {
    ...
    {
      /* CREATE TABLE ... LIKE ... */
      res = mysql_create_like_table(thd, create_table, select_tables, &create_info);
    }
    else
    {
      if (create_info.fix_create_fields(thd, &alter_info, *create_table) ||
          create_info.check_fields(thd, &alter_info, create_table->table_name, create_table->db))
        goto end_with_restore_list;
      ...
      */
      if (WSREP(thd))
      {
        ...
        if (!check_engine(thd, create_table->db.str, create_table->table_name.str, &create_info) &&
            (!thd->is_current_stmt_binlog_format_row() || !create_info.tmp_table()))
        {
#ifdef WITH_WSREP
          WSREP_TO_ISOLATION_BEGIN_ALTER(create_table->db.str, create_table->table_name.str, first_table, &alter_info,
                                         NULL, &create_info)
          {
...
bool HA_CREATE_INFO::resolve_to_charset_collation_context(THD *thd,
                                                          ...
                                                          const Charset_collation_context &ctx)
{
  ...
  */
  Lex_table_charset_collation_attrs_st default_cscl =
      !convert_cscl.is_empty() && default_cscl_arg.is_empty() ? convert_cscl : default_cscl_arg;
  ...
  else
  {
    ...
    if (!(default_table_charset = default_cscl.resolved_to_context(ctx)))
      return true;
  }
  ...
  {
    // Make sure we don't do double resolution in direct SQL execution
    DBUG_ASSERT(!alter_table_convert_to_charset || thd->stmt_arena->is_stmt_execute());
    if (!(alter_table_convert_to_charset = convert_cscl.resolved_to_context(ctx)))
      return true;
  }
  return false;
}
