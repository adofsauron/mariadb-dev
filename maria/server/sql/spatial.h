...
struct MBR
{
  ...
  MBR(const MBR &mbr1, const MBR &mbr2) : xmin(mbr1.xmin), ymin(mbr1.ymin), xmax(mbr1.xmax), ymax(mbr1.ymax)
  {
    add_mbr(&mbr2);
  }
  ...
  */
  ...
  int overlaps(const MBR *mbr)
  {
    ...
    */
    ...
    MBR intersection(MY_MAX(xmin, mbr->xmin), MY_MAX(ymin, mbr->ymin), MY_MIN(xmax, mbr->xmax),
                     MY_MIN(ymax, mbr->ymax));

    return (d == intersection.dimension());
  }

  int valid() const { return xmin <= xmax && ymin <= ymax; }
};
...
class Geometry
{
 public:
  ...
  class Class_info
  {
   public:
    ...
    Class_info(const char *name, const char *gejson_name, int type_id, create_geom_t create_func);
  };
  ...
  virtual uint init_from_wkb(const char *wkb, uint len, wkbByteOrder bo, String *res) = 0;
  virtual uint init_from_opresult(String *bin, const char *opres, uint res_len)
  {
    return init_from_wkb(opres + 4, UINT_MAX32, wkb_ndr, bin) + 4;
  }
  virtual bool init_from_json(json_engine_t *je, bool er_on_3D, String *wkb) { return true; }

  virtual bool get_data_as_wkt(String *txt, const char **end) const = 0;
  virtual bool get_data_as_json(String *txt, uint max_dec_digits, const char **end) const = 0;
 ...
 public:
  ...
  static Geometry *construct(Geometry_buffer *buffer, const char *data, uint32 data_len);
  static Geometry *create_from_wkt(Geometry_buffer *buffer, Gis_read_stream *trs, String *wkt, bool init_stream = 1);
  static Geometry *create_from_wkb(Geometry_buffer *buffer, const char *wkb, uint32 len, String *res);
  static Geometry *create_from_json(Geometry_buffer *buffer, json_engine_t *je, bool er_on_3D, String *res);
  static Geometry *create_from_opresult(Geometry_buffer *g_buf, String *res, Gcalc_result_receiver &rr);
  ...
  const char *get_data_ptr() const { return m_data; }
 ...
 protected:
  ...
  {
    return ((type_id < wkb_point) || (type_id > wkb_last)) ? NULL : ci_collection[type_id];
  }
  ...
  */
  inline bool no_data(const char *cur_data, size_t data_amount) const { return (cur_data + data_amount > m_data_end); }
  ...
  */
  inline bool not_enough_points(const char *data, uint32 expected_points, uint32 extra_point_space = 0) const
  {
    return (m_data_end < data || (expected_points > ((m_data_end - data) / (POINT_DATA_SIZE + extra_point_space))));
  }
  const char *m_data;
...
class Gis_point : public Geometry
{
 public:
  ...
  bool get_data_as_json(String *txt, uint max_dec_digits, const char **end) const;
  ...
  double calculate_haversine(const Geometry *g, const double sphere_radius, int *error);
  int spherical_distance_multipoints(Geometry *g, const double r, double *result, int *error);
};
...
class Gis_multi_point : public Geometry
{
  ...
  static const uint32 max_n_points =
      (uint32)(UINT_MAX32 - WKB_HEADER_SIZE - 4 /* n_points */) / (WKB_HEADER_SIZE + POINT_DATA_SIZE);
 ...
 public:
  ...
  int store_shapes(Gcalc_shape_transporter *trn) const;
  const Class_info *get_class_info() const;
  int spherical_distance_multipoints(Geometry *g, const double r, double *res, int *error);
};
...
struct Geometry_buffer : public my_aligned_storage<sizeof(Gis_point), MY_ALIGNOF(Gis_point)>
{
};
