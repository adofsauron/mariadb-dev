...
struct list_node : public Sql_alloc
{
  list_node *next;
  void *info;
  list_node(const void *info_par, list_node *next_par) : next(next_par), info(const_cast<void *>(info_par)) {}
...
class base_list : public Sql_alloc
{
 ...
 public:
  ...
  /**
    ...
    it in any new code.
  */
  inline base_list(const base_list &tmp) : Sql_alloc() { *this = tmp; }
  /**
  ...
  */
  ...
  {
    list_node *node = first;
    for (; node != &end_of_list && (!(*eq)(node->info, info)); node = node->next)
      ;
    if (node == &end_of_list)
...
class base_list_iterator
{
 ...
 public:
  ...
  inline void *peek() { return (*el)->info; }
  ...
  inline bool at_end() { return current == &end_of_list; }
  friend class error_list_iterator;
};
...
class List : public base_list
{
 public:
  ...
  inline List(const List<T> &tmp, MEM_ROOT *mem_root) : base_list(tmp, mem_root) {}
  inline bool push_back(T *a) { return base_list::push_back(a); }
  inline bool push_back(T *a, MEM_ROOT *mem_root) { return base_list::push_back((void *)a, mem_root); }
  inline bool push_front(T *a) { return base_list::push_front(a); }
  inline bool push_front(T *a, MEM_ROOT *mem_root) { return base_list::push_front((void *)a, mem_root); }
  ...
  inline bool add_unique(T *a, bool (*eq)(T *a, T *b)) { return base_list::add_unique(a, (List_eq *)eq); }
  inline bool copy(const List<T> *list, MEM_ROOT *root) { return base_list::copy(list, root); }
  ...
  class Iterator
  {
   public:
    ...
    bool operator==(const typename List<T>::iterator &rhs) { return node == rhs.node; }
...
template <class T>
inline void bubble_sort(List<T> *list_to_sort, int (*sort_func)(T *a, T *b, void *arg), void *arg)
{
  bool swap;
  T **ref1 = 0;
  T **ref2 = 0;
...
struct ilink
{
  ...
  {
    return (void *)my_malloc(PSI_INSTRUMENT_ME, (uint)size, MYF(MY_WME | MY_FAE | ME_FATAL));
  }
  static void operator delete(void *ptr_arg, size_t) { my_free(ptr_arg); }
  ...
  inline void assert_linked() { DBUG_ASSERT(prev != 0 && next != 0); }
  inline void assert_not_linked() { DBUG_ASSERT(prev == 0 && next == 0); }
...
class i_string_pair : public ilink
{
 public:
  ...
  i_string_pair(const char *key_arg, const char *val_arg) : key(key_arg), val(val_arg) {}
};
...
class I_List : private base_ilist
{
 public:
  ...
  inline void move_elements_to(I_List<T> *new_owner) { base_ilist::move_elements_to(new_owner); }
#ifndef _lint
...
template <typename T>
inline void list_copy_and_replace_each_value(List<T> &list, MEM_ROOT *mem_root)
{
