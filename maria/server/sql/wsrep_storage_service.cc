...
{
 public:
  Wsrep_on(THD *thd) : m_thd(thd), m_wsrep_on(thd->variables.wsrep_on) { thd->variables.wsrep_on = TRUE; }
...
Wsrep_storage_service::Wsrep_storage_service(THD *thd)
    : wsrep::storage_service(), wsrep::high_priority_context(thd->wsrep_cs()), m_thd(thd)
{
...
int Wsrep_storage_service::start_transaction(const wsrep::ws_handle &ws_handle)
{
  ...
  DBUG_PRINT("info", ("Wsrep_storage_service::start_transcation(%llu, %p)", m_thd->thread_id, m_thd));
  m_thd->set_wsrep_next_trx_id(ws_handle.transaction_id().get());
  DBUG_RETURN(m_thd->wsrep_cs().start_transaction(wsrep::transaction_id(m_thd->wsrep_next_trx_id())) ||
...
int Wsrep_storage_service::append_fragment(const wsrep::id &server_id, wsrep::transaction_id transaction_id, int flags,
                                           const wsrep::const_buffer &data, const wsrep::xid &xid WSREP_UNUSED)
{
  DBUG_ENTER("Wsrep_storage_service::append_fragment");
  DBUG_ASSERT(m_thd == current_thd);
  DBUG_PRINT("info", ("Wsrep_storage_service::append_fragment(%llu, %p)", m_thd->thread_id, m_thd));
  int ret = wsrep_schema->append_fragment(m_thd, server_id, transaction_id, wsrep::seqno(-1), flags, data);
  DBUG_RETURN(ret);
}
...
int Wsrep_storage_service::update_fragment_meta(const wsrep::ws_meta &ws_meta)
{
  ...
  DBUG_ASSERT(m_thd == current_thd);
  DBUG_PRINT("info", ("Wsrep_storage_service::update_fragment_meta(%llu, %p)", m_thd->thread_id, m_thd));
...
int Wsrep_storage_service::remove_fragments()
{
  ...
  int ret = wsrep_schema->remove_fragments(m_thd, m_thd->wsrep_trx().server_id(), m_thd->wsrep_trx().id(),
                                           m_thd->wsrep_sr().fragments());
  DBUG_RETURN(ret);
}

int Wsrep_storage_service::commit(const wsrep::ws_handle &ws_handle, const wsrep::ws_meta &ws_meta)
{
  DBUG_ENTER("Wsrep_storage_service::commit");
  DBUG_ASSERT(m_thd == current_thd);
  DBUG_PRINT("info", ("Wsrep_storage_service::commit(%llu, %p)", m_thd->thread_id, m_thd));
  WSREP_DEBUG("Storage service commit: %llu, %lld", ws_meta.transaction_id().get(), ws_meta.seqno().get());
  int ret = 0;
  ...
  else if (ret)
  {
    ...
    m_thd->wsrep_cs().prepare_for_ordering(wsrep::ws_handle(), wsrep::ws_meta(), false);
...
int Wsrep_storage_service::rollback(const wsrep::ws_handle &ws_handle, const wsrep::ws_meta &ws_meta)
{
  DBUG_ENTER("Wsrep_storage_service::rollback");
  DBUG_ASSERT(m_thd == current_thd);
  DBUG_PRINT("info", ("Wsrep_storage_service::rollback(%llu, %p)", m_thd->thread_id, m_thd));
  int ret = (m_thd->wsrep_cs().prepare_for_ordering(ws_handle, ws_meta, false) || trans_rollback(m_thd));
  m_thd->wsrep_cs().after_applying();
  m_thd->release_transactional_locks();
  DBUG_RETURN(ret);
}
