...
static bool pack_header(THD *, uchar *, List<Create_field> &, HA_CREATE_INFO *, ulong, handler *);
...
static bool pack_fields(uchar **, List<Create_field> &, HA_CREATE_INFO *, ulong);
...
static uchar *extra2_write(uchar *pos, enum extra2_frm_value_type type, const LEX_CSTRING &str)
{
  *pos++ = type;
  return extra2_write_str(pos, str);
}

static uchar *extra2_write(uchar *pos, enum extra2_frm_value_type type, const LEX_CUSTRING &str)
{
  return extra2_write(pos, type, *reinterpret_cast<const LEX_CSTRING *>(&str));
}

static uchar *extra2_write_field_properties(uchar *pos, List<Create_field> &create_fields)
{
...
static uchar *extra2_write_index_properties(uchar *pos, const KEY *keyinfo, uint keys)
{
  *pos++ = EXTRA2_INDEX_FLAGS;
  pos = extra2_write_len(pos, keys);
  for (uint i = 0; i < keys; i++)
  {
    *pos++ = keyinfo[i].is_ignored ? EXTRA2_IGNORED_KEY : EXTRA2_DEFAULT_INDEX_FLAGS;
  }
  return pos;
}

static field_index_t get_fieldno_by_name(HA_CREATE_INFO *create_info, List<Create_field> &create_fields,
...
static inline bool has_extra2_field_flags(List<Create_field> &create_fields)
{
  List_iterator<Create_field> it(create_fields);
...
static uint gis_field_options_image(uchar *buff, List<Create_field> &create_fields)
{
  uint image_size = 0;
  List_iterator<Create_field> it(create_fields);
  Create_field *field;
  while ((field = it++))
  {
    ...
    image_size += field->type_handler()->Column_definition_gis_options_image(cbuf, *field);
  }
  return image_size;
}
...
{
  static uchar *store_length(uchar *pos, ulonglong length) { return net_store_length(pos, length); }
  ...
  static uint store_length_required_length(ulonglong length) { return net_length_size(length); }
 ...
 public:
  ...
  bool append(uint fieldnr, const Column_definition &def)
  {
    ...
    if (def.type_handler()->Column_definition_data_type_info_image(&type_info, def) ||
        type_info.length() > 0xFFFF /*Some reasonable limit*/)
      return true;  // Error
    if (!type_info.length())
      return false;
    size_t need_length =
        store_length_required_length(fieldnr) + store_length_required_length(type_info.length()) + type_info.length();
...
LEX_CUSTRING build_frm_image(THD *thd, const LEX_CSTRING &table, HA_CREATE_INFO *create_info,
                             List<Create_field> &create_fields, uint keys, KEY *key_info, handler *db_file)
{
  ...
  error = pack_header(thd, forminfo, create_fields, create_info, (ulong)data_offset, db_file);
  if (unlikely(error))
    DBUG_RETURN(frm);
  ...
  /* str_db_type */
  create_info->extra_size = (uint)(2 + str_db_type.length + 2 + create_info->connect_string.length);
  /*
    Partition:
  ...
  */
  ...
  options_len = engine_table_options_frm_length(create_info->option_list, create_fields, keys, key_info);
  ...
  DBUG_PRINT("info", ("Field data type info length: %u", (uint)field_data_type_info_image.length()));
  DBUG_EXECUTE_IF("frm_data_type_info", push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                                                            "build_frm_image: Field data type info length: %u",
                                                            (uint)field_data_type_info_image.length()););

  if (validate_comment_length(thd, &create_info->comment, TABLE_COMMENT_MAXLEN, ER_TOO_LONG_TABLE_COMMENT, table.str))
    DBUG_RETURN(frm);
  /*
  ...
  */
  ...
  {
    strmake((char *)forminfo + 47, create_info->comment.str ? create_info->comment.str : "",
  ...
  */
  ...
  if (frm.length > FRM_MAX_SIZE || create_info->expression_length > UINT_MAX32)
  {
    my_error(ER_TABLE_DEFINITION_TOO_BIG, MYF(0), table.str);
    DBUG_RETURN(frm);
  }

  frm_ptr = (uchar *)my_malloc(PSI_INSTRUMENT_ME, frm.length, MYF(MY_WME | MY_ZEROFILL | MY_THREAD_SPECIFIC));
  if (!frm_ptr)
    DBUG_RETURN(frm);
  ...
  pos = extra2_write(pos, EXTRA2_TABLEDEF_VERSION, create_info->tabledef_version);

  if (part_info)
    pos = extra2_write(pos, EXTRA2_DEFAULT_PART_ENGINE, *hton_name(part_info->default_engine_type));

  if (options_len)
  {
    *pos++ = EXTRA2_ENGINE_TABLEOPTS;
    pos = extra2_write_len(pos, options_len);
    pos = engine_table_options_frm_image(pos, create_info->option_list, create_fields, keys, key_info);
  }
  ...
  if (create_info->period_info.is_set())
  {
    *pos++ = EXTRA2_APPLICATION_TIME_PERIOD;
    ...
    store_frm_fieldno(pos, get_fieldno_by_name(create_info, create_fields, create_info->period_info.period.start));
    pos += frm_fieldno_size;
    store_frm_fieldno(pos, get_fieldno_by_name(create_info, create_fields, create_info->period_info.period.end));
    pos += frm_fieldno_size;
  ...
  if (create_info->versioned())
  {
    ...
    *pos++ = 2 * frm_fieldno_size;
    store_frm_fieldno(pos, get_fieldno_by_name(create_info, create_fields, create_info->vers_info.as_row.start));
    pos += frm_fieldno_size;
    store_frm_fieldno(pos, get_fieldno_by_name(create_info, create_fields, create_info->vers_info.as_row.end));
    pos += frm_fieldno_size;
  }
  ...
  fileinfo[26] = (uchar)MY_TEST((create_info->max_rows == 1) && (create_info->min_rows == 1) && (keys == 0));
  ...
  pos += key_buff_length;
  if (make_empty_rec(thd, pos, create_info->table_options, create_fields, reclength, data_offset))
    goto err;
...
static uint pack_keys(uchar *keybuff, uint key_count, KEY *keyinfo, ulong data_offset, uint e_unique_hash_extra_parts)
{
  ...
  for (key = keyinfo, end = keyinfo + key_count; key != end; key++)
  {
    ...
    DBUG_PRINT("loop",
               ("flags: %lu  key_parts: %d  key_part: %p", key->flags, key->user_defined_key_parts, key->key_part));
    ...
    for (key_part = key->key_part, key_part_end = key_part + key->user_defined_key_parts; key_part != key_part_end;
         key_part++)

    {
      uint offset;
      DBUG_PRINT("loop", ("field: %d  startpos: %lu  length: %d", key_part->fieldnr, key_part->offset + data_offset,
                          key_part->length));
...
static bool pack_expression(String *buf, Virtual_column_info *vcol, uint field_nr, enum_vcol_info_type type)
{
...
static bool pack_vcols(String *buf, List<Create_field> &create_fields, List<Virtual_column_info> *check_constraint_list)
{
  List_iterator<Create_field> it(create_fields);
  Create_field *field;
  ...
  for (uint field_nr = 0; (field = it++); field_nr++)
  {
    if (field->vcol_info && field->vcol_info->expr)
      if (pack_expression(buf, field->vcol_info, field_nr,
                          field->vcol_info->stored_in_db ? VCOL_GENERATED_STORED : VCOL_GENERATED_VIRTUAL))
        return 1;
    ...
    if (field->check_constraint)
      if (pack_expression(buf, field->check_constraint, field_nr, VCOL_CHECK_FIELD))
        return 1;
  }
...
static bool pack_header(THD *thd, uchar *forminfo, List<Create_field> &create_fields, HA_CREATE_INFO *create_info,
                        ulong data_offset, handler *file)
{
  ...
  while ((field = it++))
  {
    if (validate_comment_length(thd, &field->comment, COLUMN_COMMENT_MAXLEN, ER_TOO_LONG_FIELD_COMMENT,
    ...
    */
    if (field->real_field_type() == MYSQL_TYPE_TIMESTAMP && field->unireg_check != Field::NONE && !time_stamp_pos)
    ...
    if (field->interval)
    {
      ...
      if (field->charset->mbminlen > 1)
      {
        ...
        *tmpint = *field->save_interval;
        tmpint->type_names = (const char **)thd->alloc(sizeof(char *) * (count + 1));
  ...
  length = n_length + create_fields.elements * FCOMP + FRM_FORMINFO_SIZE + int_length + com_length +
           create_info->expression_length;
...
static bool pack_fields(uchar **buff_arg, List<Create_field> &create_fields, HA_CREATE_INFO *create_info,
                        ulong data_offset)
{
  uchar *buff = *buff_arg;
  uint int_count;
...
static bool make_empty_rec_store_default(THD *thd, Field *regfield, Create_field *field)
{
...
static bool make_empty_rec(THD *thd, uchar *buff, uint table_options, List<Create_field> &create_fields, uint reclength,
                           ulong data_offset)
{
  int error = false;
  uint null_count;
  ...
  while ((field = it++))
  {
    Record_addr addr(buff + field->offset + data_offset, null_pos + null_count / 8, null_count & 7);
    Column_definition_attributes tmp(*field);
    tmp.interval = field->save_interval ? field->save_interval : field->interval;
    /* regfield don't have to be deleted as it's allocated on THD::mem_root */
    Field *regfield =
        tmp.make_field(&share, thd->mem_root, &addr, field->type_handler(), &field->field_name, field->flags);
    if (!regfield)
    {
      error = true;
    ...
    if (field->real_field_type() == MYSQL_TYPE_BIT && !f_bit_as_char(field->pack_flag))
