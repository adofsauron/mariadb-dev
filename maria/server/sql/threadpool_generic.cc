...
#define EPOLLRDHUP 0
#endif
static TP_file_handle io_poll_create() { return epoll_create(1); }
...
*/
int io_poll_wait(TP_file_handle pollfd, native_event *native_events, int maxevents, int timeout_ms)
{
  int ret;
  do
  {
...
static void *native_event_get_userdata(native_event *event) { return event->data.ptr; }
...
TP_file_handle io_poll_create() { return kqueue(); }

int io_poll_start_read(TP_file_handle pollfd, TP_file_handle fd, void *data, void *)
{
  struct kevent ke;
  MY_EV_SET(&ke, fd, EVFILT_READ, EV_ADD | EV_ONESHOT, 0, 0, data);
...
int io_poll_wait(TP_file_handle pollfd, struct kevent *events, int maxevents, int timeout_ms)
{
  ...
  do
  {
    ret = kevent(pollfd, 0, 0, events, maxevents, (timeout_ms >= 0) ? &ts : NULL);
  } while (ret == -1 && errno == EINTR);
  return ret;
}

static void *native_event_get_userdata(native_event *event) { return (void *)event->udata; }

#elif defined(__sun)

static TP_file_handle io_poll_create() { return port_create(); }
...
int io_poll_wait(TP_file_handle pollfd, native_event *events, int maxevents, int timeout_ms)
{
  ...
  do
  {
    ret = port_getn(pollfd, events, maxevents, &nget, (timeout_ms >= 0) ? &ts : NULL);
  } while (ret == -1 && errno == EINTR);
  DBUG_ASSERT(nget < INT_MAX);
  return (int)nget;
}

static void *native_event_get_userdata(native_event *event) { return event->portev_user; }

#elif defined(_WIN32)

static TP_file_handle io_poll_create() { return CreateIoCompletionPort(INVALID_HANDLE_VALUE, 0, 0, 0); }
...
int io_poll_disassociate_fd(TP_file_handle pollfd, TP_file_handle fd)
{
  static pNtSetInformationFile my_NtSetInformationFile =
      (pNtSetInformationFile)GetProcAddress(GetModuleHandle("ntdll"), "NtSetInformationFile");
...
static void *native_event_get_userdata(native_event *event) { return (void *)event->lpCompletionKey; }
...
static void *timer_thread(void *param)
{
  ...
  timer->next_timeout_check.store(std::numeric_limits<uint64_t>::max(), std::memory_order_relaxed);
  ...
  for (;;)
  {
    ...
    if (err == ETIMEDOUT)
    {
      ...
      if (timer->next_timeout_check.load(std::memory_order_relaxed) <= timer->current_microtime)
      {
...
static void start_timer(pool_timer_t *timer)
{
  ...
  mysql_thread_create(key_timer_thread, &timer->timer_thread_id, NULL, timer_thread, timer);
  DBUG_VOID_RETURN;
}
...
*/
static TP_connection_generic *listener(worker_thread_t *current_thread, thread_group_t *thread_group)
{
  DBUG_ENTER("listener");
...
static int create_worker(thread_group_t *thread_group, bool due_to_stall)
{
  ...
  if (tp_stats.num_worker_threads >= threadpool_max_threads && thread_group->thread_count >= 2)
  {
    err = 1;
  ...
  err = mysql_thread_create(key_worker_thread, &thread_id, thread_group->pthread_attr, worker_main, thread_group);
  if (!err)
  {
...
static int wake_or_create_thread(thread_group_t *thread_group, bool due_to_stall)
{
  ...
  ulonglong time_since_last_thread_created = (now - thread_group->last_thread_creation_time);
...
static int wake_listener(thread_group_t *thread_group)
{
  ...
  /* Wake listener */
  if (io_poll_associate_fd(thread_group->pollfd, thread_group->shutdown_pipe[0], NULL, NULL))
  {
    return -1;
  }
  char c = 0;
...
{
  return (thread_group->active_thread_count >= 1 + (int)threadpool_oversubscribe && !thread_group->stalled);
}
...
TP_connection_generic *get_event(worker_thread_t *current_thread, thread_group_t *thread_group,
...
void wait_begin(thread_group_t *thread_group)
{
  ...
  if ((thread_group->active_thread_count == 0) && (!is_queue_empty(thread_group) || !thread_group->listener))
  {
    /*
...
TP_connection *TP_pool_generic::new_connection(CONNECT *c) { return new (std::nothrow) TP_connection_generic(c); }
...
void TP_pool_generic::resume(TP_connection *c) { add(c); }
...
static void set_next_timeout_check(ulonglong abstime)
{
  ...
  while (abstime < old)
  {
    if (pool_timer.next_timeout_check.compare_exchange_weak(old, abstime, std::memory_order_relaxed,
...
static size_t get_group_id(my_thread_id tid) { return size_t(tid % group_count); }
...
#ifdef _WIN32
  fd = (c->vio_type == VIO_TYPE_NAMEDPIPE) ? c->pipe : (TP_file_handle)mysql_socket_getfd(c->sock);
#else
  fd = mysql_socket_getfd(c->sock);
#endif

  /* Assign connection to a group. */
  thread_group_t *group = &all_groups[get_group_id(c->thread_id)];
  thread_group = group;
...
void TP_connection_generic::set_io_timeout(int timeout_sec)
{
  ...
  */
  ...
  abs_wait_timeout = pool_timer.current_microtime + 1000LL * pool_timer.tick_interval + 1000000LL * timeout_sec;
...
static int change_group(TP_connection_generic *c, thread_group_t *old_group, thread_group_t *new_group)
{
  int ret = 0;
...
int TP_pool_generic::init()
{
  ...
  all_groups = (thread_group_t *)my_malloc(PSI_INSTRUMENT_ME, sizeof(thread_group_t) * threadpool_max_size,
...
TP_pool_generic::~TP_pool_generic()
{
  ...
  */
  int timeout_ms = 5000;
  while (all_groups && timeout_ms--) my_sleep(1000);
