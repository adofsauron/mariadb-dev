...
class Dep_value : public Sql_alloc
{
 public:
  ...
  virtual Dep_module *get_next_unbound_module(Dep_analysis_context *dac, Iterator iter) = 0;
...
{
 public:
  Dep_value_field(Dep_value_table *table_arg, Field *field_arg) : table(table_arg), field(field_arg) {}
...
const size_t Dep_value_field::iterator_size = ALIGN_SIZE(sizeof(Dep_value_field::Module_iter));
...
{
 public:
  Dep_value_table(TABLE *table_arg) : table(table_arg), fields(NULL), keys(NULL), pseudo_key(NULL) {}
...
const size_t Dep_value_table::iterator_size = ALIGN_SIZE(sizeof(Dep_value_table::Module_iter));

const size_t Dep_value::iterator_size = MY_MAX(Dep_value_table::iterator_size, Dep_value_field::iterator_size);
...
const size_t Dep_module_expr::iterator_size = ALIGN_SIZE(sizeof(Dep_module_expr::Value_iter));
...
const size_t Dep_module_key::iterator_size = ALIGN_SIZE(sizeof(Dep_module_key::Value_iter));
...
{
 public:
  Dep_module_pseudo_key(Dep_value_table *table_arg, MY_BITMAP *exposed_fields, uint exposed_fields_num)
...
const size_t Dep_module_pseudo_key::iterator_size = ALIGN_SIZE(sizeof(Dep_module_pseudo_key::Value_iter));

const size_t Dep_module::iterator_size =
    MY_MAX(Dep_module_expr::iterator_size, MY_MAX(Dep_module_key::iterator_size, Dep_module_pseudo_key::iterator_size));
...
{
 public:
  Dep_module_goal(uint n_children) { unbound_args = n_children; }
...
class Dep_analysis_context
{
 ...
 private:
  void create_unique_pseudo_key_if_needed(TABLE_LIST *table_list, Dep_value_table *tbl_dep);
  int find_field_in_list(List<Item> &fields_list, Item *field);
};

void eliminate_tables(JOIN *join);

static bool eliminate_tables_for_list(JOIN *join, List<TABLE_LIST> *join_list, table_map tables_in_list, Item *on_expr,
                                      table_map tables_used_elsewhere, Json_writer_array *trace_eliminate_tables);
static bool check_func_dependency(JOIN *join, table_map dep_tables, List_iterator<TABLE_LIST> *it, TABLE_LIST *oj_tbl,
                                  Item *cond);
static void build_eq_mods_for_cond(THD *thd, Dep_analysis_context *dac, Dep_module_expr **eq_mod, uint *and_level,
                                   Item *cond);
static void check_equality(Dep_analysis_context *dac, Dep_module_expr **eq_mod, uint and_level, Item_bool_func *cond,
                           Item *left, Item *right);
static Dep_module_expr *merge_eq_mods(Dep_module_expr *start, Dep_module_expr *new_fields, Dep_module_expr *end,
                                      uint and_level);
static void mark_as_eliminated(JOIN *join, TABLE_LIST *tbl, Json_writer_array *trace_eliminate_tables);
static void add_module_expr(Dep_analysis_context *dac, Dep_module_expr **eq_mod, uint and_level,
                            Dep_value_field *field_val, Item *right, List<Dep_value_field> *mult_equal_fields);
...
void eliminate_tables(JOIN *join)
{
  ...
  used_tables = (join->conds ? join->conds->used_tables() : 0) | (join->having ? join->having->used_tables() : 0);

  /*
    For "INSERT ... SELECT ... ON DUPLICATE KEY UPDATE column = val"
    we should also take into account tables mentioned in "val".
  */
  if (join->thd->lex->sql_command == SQLCOM_INSERT_SELECT && join->select_lex == thd->lex->first_select_lex())
  {
  ...
  while ((item = it++)) used_tables |= item->used_tables();
  ...
  if (join->select_lex == thd->lex->first_select_lex())
  {
    ...
    if (thd->lex->sql_command == SQLCOM_UPDATE_MULTI)
    {
      ...
      while ((item = it2++)) used_tables |= item->used_tables();
    }

    if (thd->lex->sql_command == SQLCOM_DELETE_MULTI)
    {
      TABLE_LIST *tbl;
      for (tbl = (TABLE_LIST *)thd->lex->auxiliary_table_list.first; tbl; tbl = tbl->next_local)
      {
  ...
  if (all_tables & ~used_tables)
  {
    ...
    eliminate_tables_for_list(join, join->join_list, all_tables, NULL, used_tables, &trace_eliminated_tables);
  }
  DBUG_VOID_RETURN;
}
...
static bool eliminate_tables_for_list(JOIN *join, List<TABLE_LIST> *join_list, table_map list_tables, Item *on_expr,
                                      table_map tables_used_elsewhere, Json_writer_array *trace_eliminate_tables)
{
  TABLE_LIST *tbl;
  ...
  while ((tbl = it++))
  {
    if (tbl->on_expr)
    {
      table_map outside_used_tables = tables_used_elsewhere | tables_used_on_left;
      if (on_expr)
        outside_used_tables |= on_expr->used_tables();
      if (tbl->nested_join)
      {
        /* This is  "... LEFT JOIN (join_nest) ON cond" */
        if (eliminate_tables_for_list(join, &tbl->nested_join->join_list, tbl->nested_join->used_tables, tbl->on_expr,
                                      outside_used_tables, trace_eliminate_tables))
        {
      ...
      else
      {
        ...
        if (!(tbl->table->map & outside_used_tables) &&
            check_func_dependency(join, tbl->table->map, NULL, tbl, tbl->on_expr))
        {
          mark_as_eliminated(join, tbl, trace_eliminate_tables);
        }
        else
  ...
  {
    it.rewind();
    return check_func_dependency(join, list_tables & ~join->eliminated_tables, &it, NULL, on_expr);
  }
                                  ...
                                  Item *cond)
{
  ...
  */
  dac.n_equality_mods_alloced = join->thd->lex->current_select->max_equal_elems +
...
class Field_dependency_recorder : public Field_enumerator
{
 public:
  ...
  void visit_field(Item_field *item)
  {
    ...
    {
      for (Dep_value_field *field_dep = tbl_dep->fields; field_dep; field_dep = field_dep->next_table_field)
      {
...
bool Dep_analysis_context::setup_equality_modules_deps(List<Dep_module> *bound_modules)
{
  THD *thd = current_thd;
  ...
  */
  uint offset = 0;
  for (Dep_value_table **tbl_dep = table_deps; tbl_dep < table_deps + MAX_TABLES; tbl_dep++)
  {
    if (*tbl_dep)
    {
      for (Dep_value_field *field_dep = (*tbl_dep)->fields; field_dep; field_dep = field_dep->next_table_field)
  ...
  void *buf;
  if (!(buf = thd->alloc(bitmap_buffer_size(offset))) || my_bitmap_init(&expr_deps, (my_bitmap_map *)buf, offset))
  {
  ...
  */
  ...
  for (Dep_module_expr *eq_mod = equality_mods; eq_mod < equality_mods + n_equality_mods; eq_mod++)
  {
    ...
    if (eq_mod->field)
    {
      ...
      eq_mod->expr->walk(&Item::enumerate_field_refs_processor, FALSE, &deps_recorder);
    }
    else
    {
...
static int compare_field_values(Dep_value_field *a, Dep_value_field *b, void *unused)
{
  uint a_ratio = a->field->table->tablenr * MAX_FIELDS + a->field->field_index;

  uint b_ratio = b->field->table->tablenr * MAX_FIELDS + b->field->field_index;
...
static void build_eq_mods_for_cond(THD *thd, Dep_analysis_context *ctx, Dep_module_expr **eq_mod, uint *and_level,
                                   Item *cond)
{
  if (cond->type() == Item_func::COND_ITEM)
  {
    ...
    {
      Item *item;
      while ((item = li++)) build_eq_mods_for_cond(thd, ctx, eq_mod, and_level, item);

      for (Dep_module_expr *mod_exp = ctx->equality_mods + orig_offset; mod_exp != *eq_mod; mod_exp++)
      {
    ...
    else
    {
      ...
      while ((item = li++))
      {
        ...
        *eq_mod = merge_eq_mods(ctx->equality_mods + orig_offset, start_key_fields, *eq_mod, ++(*and_level));
      }
    }
    return;
  }
  ...
  switch (cond_func->functype())
  {
    case Item_func::BETWEEN:
    {
      ...
      if (!func->negated && (fld = args[0]->real_item())->type() == Item::FIELD_ITEM &&
    ...
    case Item_func::ISNULL_FUNC:
    {
      ...
      if (tmp)
        check_equality(ctx, eq_mod, *and_level, (Item_func_isnull *)cond_func, args[0], tmp);
      break;
    }
    ...
    for (Dep_module_expr *old = start; old != first_free; old++)
    {
      if (old->field == new_fields->field)
      {
        if (!old->field)
        {
          ...
          */
          ...
          if (old->expr && new_fields->expr &&
              old->expr->eq_by_collation(new_fields->expr, eq_field->binary(), eq_field->charset()))
          {
            /* Ok, keep */
          }
          else
        ...
        else if (!new_fields->expr->const_item())
        {
          ...
          */
          if (old->expr->eq(new_fields->expr, old->field->field->binary()))
          {
            old->level = and_level;
          }
        }
        else if (old->expr->eq_by_collation(new_fields->expr, old->field->field->binary(),
...
static void check_equality(Dep_analysis_context *ctx, Dep_module_expr **eq_mod, uint and_level, Item_bool_func *cond,
                           Item *left, Item *right)
{
  if ((left->used_tables() & ctx->usable_tables) && !(right->used_tables() & RAND_TABLE_BIT) &&
...
static void add_module_expr(Dep_analysis_context *ctx, Dep_module_expr **eq_mod, uint and_level,
                            Dep_value_field *field_val, Item *right, List<Dep_value_field> *mult_equal_fields)
{
  if (*eq_mod == ctx->equality_mods + ctx->n_equality_mods_alloced)
  {
    ...
    */
    ...
    for (int i = 0; i < *eq_mod - ctx->equality_mods; i++) new_arr[i] = ctx->equality_mods[i];
...
Dep_value_table *Dep_analysis_context::create_table_value(TABLE_LIST *table_list)
{
  Dep_value_table *tbl_dep;
...
void Dep_analysis_context::create_unique_pseudo_key_if_needed(TABLE_LIST *table_list, Dep_value_table *tbl_dep)
{
  ...
  */
  if (first_select && first_select->join && first_select->group_list.elements > 0)
  {
    auto max_possible_elements = first_select->join->fields_list.elements;
    void *buf;
    MY_BITMAP *exposed_fields = (MY_BITMAP *)current_thd->alloc(sizeof(MY_BITMAP));
    if (!(buf = current_thd->alloc(bitmap_buffer_size(max_possible_elements))) ||
        my_bitmap_init(exposed_fields, (my_bitmap_map *)buf, max_possible_elements))
    ...
    bool valid = true;
    for (auto cur_group = first_select->group_list.first; cur_group; cur_group = cur_group->next)
    {
    ...
    {
      Dep_module_pseudo_key *pseudo_key;
      pseudo_key = new Dep_module_pseudo_key(tbl_dep, exposed_fields, exposed_fields_count);
...
Dep_module *Dep_value_table::get_next_unbound_module(Dep_analysis_context *dac, char *iter)
{
...
Dep_value *Dep_module_expr::get_next_unbound_value(Dep_analysis_context *dac, char *buf)
{
  Dep_value *res;
  if (field)
  {
...
Dep_value *Dep_module_key::get_next_unbound_value(Dep_analysis_context *dac, Dep_module::Iterator iter)
{
...
Dep_value *Dep_module_pseudo_key::get_next_unbound_value(Dep_analysis_context *dac, Dep_module::Iterator iter)
{
  Dep_value *res = ((Value_iter *)iter)->table;
  ((Value_iter *)iter)->table = NULL;
  return res;
...
bool Dep_module_pseudo_key::covers_field(int field_no) { return bitmap_is_set(exposed_fields_map, field_no) > 0; }
...
Dep_module *Dep_value_field::get_next_unbound_module(Dep_analysis_context *dac, Dep_value::Iterator iter)
{
  Module_iter *di = (Module_iter *)iter;
  ...
  */
  while (key_dep && (key_dep->is_applicable() || !field->part_of_key_not_clustered.is_set(key_dep->keyno)))
  {
  ...
  if (pseudo_key_dep && !pseudo_key_dep->is_applicable() && pseudo_key_dep->covers_field(field->field_index))
  {
  ...
  */
  ...
  while (eq_no < dac->n_equality_mods &&
         (!bitmap_is_set(&dac->expr_deps, bitmap_offset + eq_no) || dac->equality_mods[eq_no].is_applicable()))
  {
    eq_no++;
  }
...
static void mark_as_eliminated(JOIN *join, TABLE_LIST *tbl, Json_writer_array *trace_eliminate_tables)
{
  ...
  */
  if (tbl->nested_join)
  {
    ...
    while ((child = it++)) mark_as_eliminated(join, child, trace_eliminate_tables);
  }
...
void Dep_analysis_context::dbug_print_deps()
{
  ...
  for (Dep_module_expr *eq_mod = equality_mods; eq_mod != equality_mods + n_equality_mods; eq_mod++)
  {
    ...
    if (eq_mod->field)
    {
      fprintf(DBUG_FILE, "  equality%ld: %s -> %s.%s\n", (long)(eq_mod - equality_mods), str.c_ptr(),
              eq_mod->field->table->table->alias.c_ptr(), eq_mod->field->field->field_name.str);
    }
    else
    {
      fprintf(DBUG_FILE, "  equality%ld: multi-equality", (long)(eq_mod - equality_mods));
    }
  }
  fprintf(DBUG_FILE, "\n");
  ...
  for (uint i = 0; i < MAX_TABLES; i++)
  {
    ...
    if ((table_dep = table_deps[i]))
    {
      ...
      /* Print fields */
      for (Dep_value_field *field_dep = table_dep->fields; field_dep; field_dep = field_dep->next_table_field)
      {
        fprintf(DBUG_FILE, "    field %s.%s ->", table_dep->table->alias.c_ptr(), field_dep->field->field_name.str);
