...
{
  static int sel_cmp(Field *field, uchar *a, uchar *b, uint8 a_flag, uint8 b_flag);
  ...
  */
  bool all_same(const SEL_ARG *arg) const
  {
    ...
    for (; cur_arg && cmp_arg && cur_arg->is_same(cmp_arg); cur_arg = cur_arg->next, cmp_arg = cmp_arg->next)
      ;
    if (cur_arg || cmp_arg)
      return false;
    return true;
  ...
  {  // Get overlapping range
    ...
    return new (thd->mem_root)
        SEL_ARG(field, part, new_min, new_max, flag_min, flag_max, MY_TEST(maybe_flag && arg->maybe_flag));
  }
  ...
  void store_min_max(KEY_PART *kp, uint length, uchar **min_key, uint min_flag, uchar **max_key, uint max_flag,
  ...
  /*
    ...
    account.
  */
  uint get_min_flag(KEY_PART *kp) { return (kp[part].flag & HA_REVERSE_SORT) ? invert_max_flag(max_flag) : min_flag; }
  /*
    Get the flag for range's starting endpoint, taking index order into
    account.
  */
  uint get_max_flag(KEY_PART *kp) { return (kp[part].flag & HA_REVERSE_SORT) ? invert_min_flag(min_flag) : max_flag; }
  /* Get the previous interval, taking index order into account */
  inline SEL_ARG *index_order_prev(KEY_PART *kp) { return (kp[part].flag & HA_REVERSE_SORT) ? next : prev; }
  /* Get the next interval, taking index order into account */
  inline SEL_ARG *index_order_next(KEY_PART *kp) { return (kp[part].flag & HA_REVERSE_SORT) ? prev : next; }

  /*
    Produce a single multi-part interval, taking key part ordering into
    account.
  */
  void store_next_min_max_keys(KEY_PART *key, uchar **cur_min_key, uint *cur_min_flag, uchar **cur_max_key,
  ...
  */
  int store_min_key(KEY_PART *key, uchar **range_key, uint *range_key_flag, uint last_part, bool start_key)
  {
    ...
    uint res = key_tree->store_min(key[key_tree->part].store_length, range_key, *range_key_flag);
    // add flags only if a key_part is written to the buffer
    if (!res)
      return 0;
    *range_key_flag |= key_tree->min_flag;
    SEL_ARG *nkp = key_tree->next_key_part;
    if (nkp && nkp->type == SEL_ARG::KEY_RANGE && key_tree->part != last_part && nkp->part == key_tree->part + 1 &&
        !(*range_key_flag & (NO_MIN_RANGE | NEAR_MIN)))
    {
      const bool asc = !(key[key_tree->part].flag & HA_REVERSE_SORT);
      if (start_key == asc)
      {
        res += nkp->store_min_key(key, range_key, range_key_flag, last_part, start_key);
      }
      else
      {
        uint tmp_flag = invert_min_flag(*range_key_flag);
        res += nkp->store_max_key(key, range_key, &tmp_flag, last_part, start_key);
  ...
  int store_max_key(KEY_PART *key, uchar **range_key, uint *range_key_flag, uint last_part, bool start_key)
  {
    ...
    uint res = key_tree->store_max(key[key_tree->part].store_length, range_key, *range_key_flag);
    if (!res)
      return 0;
    *range_key_flag |= key_tree->max_flag;
        ...
        !(*range_key_flag & (NO_MAX_RANGE | NEAR_MAX)))
    {
      ...
      {
        res += nkp->store_max_key(key, range_key, range_key_flag, last_part, start_key);
      }
      else
      {
        uint tmp_flag = invert_max_flag(*range_key_flag);
        res += nkp->store_min_key(key, range_key, &tmp_flag, last_part, start_key);
...
{
 public:
  SEL_ARG_IMPOSSIBLE(Field *field) : SEL_ARG(field, 0, 0) { type = SEL_ARG::IMPOSSIBLE; }
};
...
class RANGE_OPT_PARAM
{
  ...
  */
  ...
  {
    return thd->killed || thd->is_fatal_error || thd->is_error() || alloced_sel_args > SEL_ARG::MAX_SEL_ARGS;
  }
};
...
class QUICK_RANGE : public Sql_alloc
{
 public:
  ...
  QUICK_RANGE(THD *thd, const uchar *min_key_arg, uint min_length_arg, key_part_map min_keypart_map_arg,
              const uchar *max_key_arg, uint max_length_arg, key_part_map max_keypart_map_arg, uint flag_arg)
  ...
  */
  void make_min_endpoint(key_range *kr)
  {
    ...
    kr->flag = ((flag & NEAR_MIN) ? HA_READ_AFTER_KEY : (flag & EQ_RANGE) ? HA_READ_KEY_EXACT : HA_READ_KEY_OR_NEXT);
  }
...
class QUICK_SELECT_I
{
  ...
  */
  ...
  void add_key_and_length(String *key_names, String *used_lengths, bool *first);
  ...
  */
  ...
  virtual void replace_handler(handler *new_file) { DBUG_ASSERT(0); /* Only supported in QUICK_RANGE_SELECT */ }
...
class QUICK_RANGE_SELECT : public QUICK_SELECT_I
{
 ...
 public:
  ...
  QUICK_RANGE_SELECT(THD *thd, TABLE *table, uint index_arg, bool no_alloc, MEM_ROOT *parent_alloc, bool *create_err);
  ~QUICK_RANGE_SELECT();
  virtual QUICK_RANGE_SELECT *clone(bool *create_error)
  {
    return new QUICK_RANGE_SELECT(thd, head, index, no_alloc, parent_alloc, create_error);
  }
 ...
 private:
  ...
  friend QUICK_RANGE_SELECT *get_quick_select_for_ref(THD *thd, TABLE *table, struct st_table_ref *ref,
                                                      ha_rows records);
  friend bool get_quick_keys(PARAM *param, QUICK_RANGE_SELECT *quick, KEY_PART *key, SEL_ARG *key_tree, uchar *min_key,
                             uint min_key_flag, uchar *max_key, uint max_key_flag);
  friend QUICK_RANGE_SELECT *get_quick_select(PARAM *, uint idx, SEL_ARG *key_tree, uint mrr_flags, uint mrr_buf_size,
                                              MEM_ROOT *alloc);
  ...
  friend int read_keys_and_merge_scans(THD *thd, TABLE *head, List<QUICK_RANGE_SELECT> quick_selects,
...
class QUICK_RANGE_SELECT_GEOM : public QUICK_RANGE_SELECT
{
 public:
  QUICK_RANGE_SELECT_GEOM(THD *thd, TABLE *table, uint index_arg, bool no_alloc, MEM_ROOT *parent_alloc,
                          bool *create_err)
      : QUICK_RANGE_SELECT(thd, table, index_arg, no_alloc, parent_alloc, create_err){};
  virtual QUICK_RANGE_SELECT *clone(bool *create_error)
  {
    DBUG_ASSERT(0);
    return new QUICK_RANGE_SELECT_GEOM(thd, head, index, no_alloc, parent_alloc, create_error);
  }
...
class QUICK_INDEX_MERGE_SELECT : public QUICK_INDEX_SORT_SELECT
{
 ...
 public:
  QUICK_INDEX_MERGE_SELECT(THD *thd_arg, TABLE *table) : QUICK_INDEX_SORT_SELECT(thd_arg, table) {}
...
class QUICK_INDEX_INTERSECT_SELECT : public QUICK_INDEX_SORT_SELECT
{
 ...
 public:
  QUICK_INDEX_INTERSECT_SELECT(THD *thd_arg, TABLE *table) : QUICK_INDEX_SORT_SELECT(thd_arg, table) {}
...
class QUICK_GROUP_MIN_MAX_SELECT : public QUICK_SELECT_I
{
 ...
 public:
  QUICK_GROUP_MIN_MAX_SELECT(TABLE *table, JOIN *join, bool have_min, bool have_max, bool have_agg_distinct,
                             KEY_PART_INFO *min_max_arg_part, uint group_prefix_len, uint group_key_parts,
                             uint used_key_parts, KEY *index_info, uint use_index, double read_cost, ha_rows records,
                             uint key_infix_len, uchar *key_infix, MEM_ROOT *parent_alloc, bool is_index_scan);
...
class QUICK_SELECT_DESC : public QUICK_RANGE_SELECT
{
 public:
  ...
  {
    DBUG_ASSERT(0);
    return new QUICK_SELECT_DESC(this, used_key_parts);
  }
  int get_next();
...
class SQL_SELECT : public Sql_alloc
{
  ...
  */
  ...
  bool check_quick(THD *thd, bool force_quick_range, ha_rows limit)
  {
    key_map tmp;
    tmp.set_all();
    return test_quick_select(thd, tmp, 0, limit, force_quick_range, FALSE, FALSE, FALSE) < 0;
  }
  /*
    RETURN
  ...
  */
  ...
  int test_quick_select(THD *thd, key_map keys, table_map prev_tables, ha_rows limit, bool force_quick_range,
                        bool ordered_output, bool remove_false_parts_of_where, bool only_single_index_range_scan);
};
...
class SQL_SELECT_auto
{
 ...
 public:
  ...
  ~SQL_SELECT_auto() { delete select; }
  SQL_SELECT_auto &operator=(SQL_SELECT *_select)
  {
    select = _select;
    return *this;
  }
  operator SQL_SELECT *() const { return select; }
  SQL_SELECT *operator->() const { return select; }
  operator bool() const { return select; }
};
...
class FT_SELECT : public QUICK_RANGE_SELECT
{
 public:
  ...
  {
    DBUG_ASSERT(0);
    return new FT_SELECT(thd, head, index, create_error);
  }
