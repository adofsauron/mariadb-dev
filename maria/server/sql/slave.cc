...
log '%s' at position %llu%s",
     "COM_REGISTER_SLAVE", "Slave I/O thread killed during or after reconnect"},
    {"Waiting to reconnect after a failed binlog dump request", "Slave I/O thread killed while retrying master dump",
     "Reconnecting after a failed binlog dump request",
     "failed dump request, reconnecting to try again, log '%s' at position %llu%s", "COM_BINLOG_DUMP",
...
static int terminate_slave_thread(THD *, mysql_mutex_t *, mysql_cond_t *, volatile uint *, bool);
...
static void set_slave_max_allowed_packet(THD *thd, MYSQL *mysql)
{
  ...
  */
  mysql->options.max_allowed_packet = slave_max_allowed_packet + MAX_LOG_EVENT_HEADER;
  DBUG_VOID_RETURN;
}
...
static PSI_thread_info all_slave_threads[] = {{&key_thread_slave_io, "slave_io", PSI_FLAG_GLOBAL},
...
*/
static const char gtid_pos_table_definition1[] = "CREATE TABLE ";
...
*/
static bool build_gtid_pos_create_query(THD *thd, String *query, LEX_CSTRING *table_name, LEX_CSTRING *engine_name)
{
  bool err = false;
  err |= query->append(gtid_pos_table_definition1, sizeof(gtid_pos_table_definition1) - 1);
  err |= append_identifier(thd, query, table_name);
  err |= query->append(gtid_pos_table_definition2, sizeof(gtid_pos_table_definition2) - 1);
  err |= append_identifier(thd, query, engine_name);
  return err;
}

static int gtid_pos_table_creation(THD *thd, plugin_ref engine, LEX_CSTRING *table_name)
{
  int err;
  StringBuffer<sizeof(gtid_pos_table_definition1) + sizeof(gtid_pos_table_definition1) + 2 * FN_REFLEN> query;
  ...
  thd->set_query_and_id(query.c_ptr(), query.length(), thd->charset(), next_query_id());
  ...
  sql_print_warning(
      "The automatically created table '%s' name may not be "
...
static void bg_gtid_pos_auto_create(void *hton)
{
  ...
  */
  ...
  for (auto_engines = opt_gtid_pos_auto_plugins; auto_engines && *auto_engines; ++auto_engines)
  {
  ...
  entry = rpl_global_gtid_slave_state->gtid_pos_tables.load(std::memory_order_relaxed);
  while (entry)
  {
    if (entry->table_hton == hton && entry->state == rpl_slave_state::GTID_POS_CREATE_REQUESTED)
      break;
    entry = entry->next;
  }
  if (entry)
  ...
  if (err)
  {
    sql_print_error("Error auto-creating GTID position table `mysql.%s`: %s Error_code: %d", table_name.str,
                    thd->get_stmt_da()->message(), thd->get_stmt_da()->sql_errno());
    thd->clear_error();
    goto end;
  }
  ...
  while (entry)
  {
    if (entry->table_hton == hton && entry->state == rpl_slave_state::GTID_POS_CREATE_IN_PROGRESS)
    {
...
*/
void slave_background_gtid_pending_delete_request(void) { mysql_manager_submit(bg_gtid_delete_pending, NULL); }
...
int init_slave()
{
  ...
  while (!slave_background_thread_gtid_loaded)
    mysql_cond_wait(&global_rpl_thread_pool.COND_rpl_thread_pool, &global_rpl_thread_pool.LOCK_rpl_thread_pool);
  mysql_mutex_unlock(&global_rpl_thread_pool.LOCK_rpl_thread_pool);
  ...
  */
  ...
  if (!(active_mi = new Master_info(&default_master_connection_name, relay_log_recovery)) || active_mi->error())
  {
    delete active_mi;
    active_mi = 0;
  ...
  */
  ...
  if (init_master_info(active_mi, master_info_file, relay_log_info_file, 1, (SLAVE_IO | SLAVE_SQL)))
  {
  ...
  if (active_mi->host[0] && !opt_skip_slave_start)
  {
    ...
    error = start_slave_threads(0, /* No active thd */
                                1 /* need mutex */, 1 /* wait for start*/, active_mi, master_info_file,
...
int init_recovery(Master_info *mi, const char **errmsg)
{
  ...
  {
    mi->master_log_pos = MY_MAX(BIN_LOG_HEADER_SIZE, rli->group_master_log_pos);
    strmake_buf(mi->master_log_name, rli->group_master_log_name);

    sql_print_warning("Recovery from master pos %ld and file %s.", (ulong)mi->master_log_pos, mi->master_log_name);
...
static void make_slave_transaction_retry_errors_printable(void)
{
  ...
  *buff = 0;
  DBUG_PRINT("exit", ("error_names: '%s'", slave_transaction_retry_error_names));
  DBUG_VOID_RETURN;
}
...
bool init_slave_transaction_retry_errors(const char *arg)
{
  ...
  if (unlikely(!(slave_transaction_retry_errors =
                     (uint *)my_once_alloc(sizeof(int) * slave_transaction_retry_error_length, MYF(MY_WME)))))
    DBUG_RETURN(1);
...
int terminate_slave_threads(Master_info *mi, int thread_mask, bool skip_lock)
{
  ...
  if (thread_mask & (SLAVE_SQL | SLAVE_FORCE_ALL))
  {
    ...
    if (unlikely((error = terminate_slave_thread(mi->rli.sql_driver_thd, sql_lock, &mi->rli.stop_cond,
                                                 &mi->rli.slave_running, skip_lock))) &&
        !force_all)
      DBUG_RETURN(error);
    retval = error;
  ...
  if (thread_mask & (SLAVE_IO | SLAVE_FORCE_ALL))
  {
    ...
    mi->abort_slave = 1;
    if (unlikely(
            (error = terminate_slave_thread(mi->io_thd, io_lock, &mi->stop_cond, &mi->slave_running, skip_lock))) &&
        !force_all)
      DBUG_RETURN(error);
    if (!retval)
      retval = error;
    ...
    if (mi->rli.relay_log.is_open() && my_sync(mi->rli.relay_log.get_log_file()->file, MYF(MY_WME)))
      retval = ER_ERROR_DURING_FLUSH_LOGS;
 ...
 */
static int terminate_slave_thread(THD *thd, mysql_mutex_t *term_lock, mysql_cond_t *term_cond,
                                  volatile uint *slave_running, bool skip_lock)
{
    ...
    volatile uint *slave_running, volatile ulong *slave_run_id, Master_info *mi)
{
  ...
  if (unlikely((error = mysql_thread_create(thread_key, &th, &connection_attrib, h_func, (void *)mi))))
  {
  ...
  */
  ...
  if (start_cond && cond_lock)  // caller has cond_lock
  {
    ...
    while (start_id == *slave_run_id)
    {
      ...
      thd->ENTER_COND(start_cond, cond_lock, &stage_waiting_for_slave_thread_to_start, &saved_stage);
      /*
                        ...
                        const char *master_info_fname, const char *slave_info_fname, int thread_mask)
{
  ...
  /*
    ...
    removed.
  */
  if (mi->using_gtid != Master_info::USE_GTID_NO && !mi->slave_running && !mi->rli.slave_running)
  {
    /*
    ...
    */
    ...
    error = rpl_load_gtid_state(&mi->gtid_current_pos, mi->using_gtid == Master_info::USE_GTID_CURRENT_POS);
  ...
  if (likely(!error) && likely((thread_mask & SLAVE_IO)))
    error = start_slave_thread(
        ...
        key_thread_slave_io,
#endif
        handle_slave_io, lock_io, lock_cond_io, cond_io, &mi->slave_running, &mi->slave_run_id, mi);
...
#endif
        handle_slave_sql, lock_sql, lock_cond_sql, cond_sql, &mi->rli.slave_running, &mi->rli.slave_run_id, mi);
    if (unlikely(error))
...
static bool sql_slave_killed(rpl_group_info *rgi)
{
  ...
  if (rli->sql_driver_thd->killed || rli->abort_slave)
  {
    ...
    */
    ...
    if ((thd->transaction->all.modified_non_trans_table || (thd->variables.option_bits & OPTION_BINLOG_THIS_TRX)) &&
        rli->is_in_group())
    {
      char msg_stopped[] =
      ...
      DBUG_PRINT("info",
                 ("modified_non_trans_table: %d  OPTION_BEGIN: %d  "
                  "OPTION_BINLOG_THIS_TRX: %d  is_in_group: %d",
                  thd->transaction->all.modified_non_trans_table, MY_TEST(thd->variables.option_bits & OPTION_BEGIN),
                  MY_TEST(thd->variables.option_bits & OPTION_BINLOG_THIS_TRX), rli->is_in_group()));
      ...
      if (rli->abort_slave)
      {
        ...
        */
        ...
        ret = difftime(my_time(0), rgi->last_event_start_time) <= SLAVE_WAIT_GROUP_DONE ? FALSE : TRUE;

        DBUG_EXECUTE_IF("stop_slave_middle_group", DBUG_EXECUTE_IF("incomplete_group_in_relay_log",
        ...
        {
          rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, rgi->gtid_info(), ER_THD(thd, ER_SLAVE_FATAL_ERROR),
                      msg_stopped);
        }
      }
      else
...
bool net_request_file(NET *net, const char *fname)
{
  ...
  DBUG_RETURN(net_write_command(net, 251, (uchar *)fname, strlen(fname), (uchar *)"", 0));
}
...
int init_dynarray_intvar_from_file(DYNAMIC_ARRAY *arr, IO_CACHE *f)
{
  ...
  if (read_size + 1 == sizeof(buf) && buf[sizeof(buf) - 2] != '\n')
  {
    ...
    */
    ...
    buf_act = (char *)my_malloc(key_memory_Rpl_info_file_buffer, max_size, MYF(MY_WME));
    memcpy(buf_act, buf, read_size);
    snd_size = my_b_gets(f, buf_act + read_size, max_size - read_size);
    if (snd_size == 0 || ((snd_size + 1 == max_size - read_size) && buf_act[max_size - 2] != '\n'))
    {
      /*
...
{
  if (errorno == CR_CONNECTION_ERROR || errorno == CR_CONN_HOST_ERROR || errorno == CR_SERVER_GONE_ERROR ||
      errorno == CR_SERVER_LOST || errorno == ER_CON_COUNT_ERROR || errorno == ER_CONNECTION_KILLED ||
      errorno == ER_NEW_ABORTING_CONNECTION || errorno == ER_NET_READ_INTERRUPTED || errorno == ER_SERVER_SHUTDOWN)
    return TRUE;
#ifdef WITH_WSREP
...
static int get_master_version_and_clock(MYSQL *mysql, Master_info *mi)
{
  ...
  */
  ...
  {
    errmsg = err_buff2;
    snprintf(err_buff2, sizeof(err_buff2), "Master reported unrecognized MariaDB version: %s", mysql->server_version);
    err_code = ER_SLAVE_FATAL_ERROR;
    sprintf(err_buff, ER_DEFAULT(err_code), err_buff2);
  }
  else
  {
    DBUG_EXECUTE_IF("mock_mariadb_primary_v5_in_get_master_version", version = 5;);
    ...
    */
    switch (version)
    {
      ...
      case 2:
        errmsg = err_buff2;
        snprintf(err_buff2, sizeof(err_buff2), "Master reported unrecognized MariaDB version: %s",
  ...
  */
  mi->rli.relay_log.description_event_for_queue->checksum_alg = mi->rli.relay_log.relay_log_checksum_alg;

  DBUG_ASSERT(mi->rli.relay_log.description_event_for_queue->checksum_alg != BINLOG_CHECKSUM_ALG_UNDEF);
  DBUG_ASSERT(mi->rli.relay_log.relay_log_checksum_alg != BINLOG_CHECKSUM_ALG_UNDEF);
  /*
  ...
  */
  ...
  if (!mysql_real_query(mysql, STRING_WITH_LEN("SELECT UNIX_TIMESTAMP()")) &&
      (master_res = mysql_store_result(mysql)) && (master_row = mysql_fetch_row(master_res)))
  {
  ...
  {
    mi->report(WARNING_LEVEL, mysql_errno(mysql), NULL, "Get master clock failed with error: %s", mysql_error(mysql));
    goto network_err;
  }
  else
  {
  ...
  */
  ...
  master_res = NULL;
  master_row = NULL;
  if (!mysql_real_query(mysql, STRING_WITH_LEN("SHOW VARIABLES LIKE 'SERVER_ID'")) &&
      (master_res = mysql_store_result(mysql)) && (master_row = mysql_fetch_row(master_res)))
  {
    if ((global_system_variables.server_id == (mi->master_id = strtoul(master_row[1], 0, 10))) &&
...
not always make sense; please check the manual before using it).";
  ...
  else if (mysql_errno(mysql))
  {
    ...
    {
      mi->report(WARNING_LEVEL, mysql_errno(mysql), NULL, "Get master SERVER_ID failed with error: %s",
                 mysql_error(mysql));
      goto network_err;
    }
    /* Fatal error */
...
maybe it is a *VERY OLD MASTER*.");
  ...
  */
  ...
  {
    master_res = NULL;
    if (!mysql_real_query(mysql, STRING_WITH_LEN("SELECT @@GLOBAL.COLLATION_SERVER")) &&
        (master_res = mysql_store_result(mysql)) && (master_row = mysql_fetch_row(master_res)))
    {
      if (strcmp(master_row[0], global_system_variables.collation_server->coll_name.str))
      {
        errmsg =
...
be equal for the Statement-format replication to work";
    ...
    {
      mi->report(WARNING_LEVEL, mysql_errno(mysql), NULL, "Get master COLLATION_SERVER failed with error: %s",
                 mysql_error(mysql));
      goto network_err;
    }
  ...
  */
  if (version == 4)
  {
    ...
    if (!mysql_real_query(mysql, STRING_WITH_LEN("SELECT @@GLOBAL.TIME_ZONE")) &&
        (master_res = mysql_store_result(mysql)) && (master_row = mysql_fetch_row(master_res)))
    {
      if (strcmp(master_row[0], global_system_variables.time_zone->get_name()->ptr()))
      {
        errmsg =
            "The slave I/O thread stops because master and slave have \
...
be equal for the Statement-format replication to work";
    ...
    {
      mi->report(ERROR_LEVEL, err_code, NULL, "Get master TIME_ZONE failed with error: %s", mysql_error(mysql));
      goto network_err;
    }
...
when it try to get the value of TIME_ZONE global variable from master.";
  ...
  if (mi->heartbeat_period != 0.0)
  {
    ...
    */
    my_snprintf(query, sizeof(query), query_format, (ulonglong)(mi->heartbeat_period * 1000000000UL));
    ...
    if (mysql_real_query(mysql, query, (ulong)strlen(query)))
    {
      ...
      if (is_network_error(mysql_errno(mysql)))
      {
        ...
        mi->report(WARNING_LEVEL, mysql_errno(mysql), NULL,
                   "SET @master_heartbeat_period to master failed with error: %s", mysql_error(mysql));
  ...
  */
  DBUG_EXECUTE_IF("simulate_slave_unaware_checksum", mi->checksum_alg_before_fd = BINLOG_CHECKSUM_ALG_OFF;
                  goto past_checksum;);
  {
    int rc;
    ...
    */
    ...
    {
      mysql_free_result(mysql_store_result(mysql));
      if (!mysql_real_query(mysql, STRING_WITH_LEN("SELECT @master_binlog_checksum")) &&
          (master_res = mysql_store_result(mysql)) && (master_row = mysql_fetch_row(master_res)) &&
          (master_row[0] != NULL))
      {
        mi->checksum_alg_before_fd =
            (enum_binlog_checksum_alg)(find_type(master_row[0], &binlog_checksum_typelib, 1) - 1);
      ...
      {
        mi->report(WARNING_LEVEL, mysql_errno(mysql), NULL, "Get master BINLOG_CHECKSUM failed with error: %s",
                   mysql_error(mysql));
        goto network_err;
      }
      else
...
past_checksum:
  ...
  */
  ...
  {
    if (unlikely(mysql_real_query(mysql, STRING_WITH_LEN("SET skip_replication=1"))))
    {
      err_code = mysql_errno(mysql);
  ...
  {
    int rc =
        DBUG_IF("simulate_slave_capability_old_53")
            ? mysql_real_query(
                  mysql,
                  STRING_WITH_LEN("SET @mariadb_slave_capability=" STRINGIFY_ARG(MARIA_SLAVE_CAPABILITY_ANNOTATE)))
            : mysql_real_query(
                  mysql, STRING_WITH_LEN("SET @mariadb_slave_capability=" STRINGIFY_ARG(MARIA_SLAVE_CAPABILITY_MINE)));
    if (unlikely(rc))
    {
      err_code = mysql_errno(mysql);
      if (is_network_error(err_code))
      {
        mi->report(ERROR_LEVEL, err_code, NULL, "Setting @mariadb_slave_capability failed with error: %s",
                   mysql_error(mysql));
        goto network_err;
      }
      else
...
after_set_capability:
  ...
  if (mi->using_gtid != Master_info::USE_GTID_NO)
  {
    ...
    */
    if (mysql_real_query(mysql, STRING_WITH_LEN("SELECT @@GLOBAL.gtid_domain_id")) ||
        !(master_res = mysql_store_result(mysql)) || !(master_row = mysql_fetch_row(master_res)))
    {
      err_code = mysql_errno(mysql);
      if (is_network_error(err_code))
      {
        mi->report(ERROR_LEVEL, err_code, NULL, "Get master @@GLOBAL.gtid_domain_id failed with error: %s",
                   mysql_error(mysql));
        goto network_err;
      }
      else
    ...
    query_str.append(STRING_WITH_LEN("SET @slave_connect_state='"), system_charset_info);
    if (mi->gtid_current_pos.append_to_string(&query_str))
    {
      err_code = ER_OUTOFMEMORY;
      errmsg =
          "The slave I/O thread stops because a fatal out-of-memory "
    ...
    if (unlikely(rc))
    {
      ...
      if (is_network_error(err_code))
      {
        mi->report(ERROR_LEVEL, err_code, NULL, "Setting @slave_connect_state failed with error: %s",
                   mysql_error(mysql));
        goto network_err;
      }
      else
    ...
    query_str.length(0);
    if (query_str.append(STRING_WITH_LEN("SET @slave_gtid_strict_mode="), system_charset_info) ||
    ...
    if (unlikely(rc))
    {
      ...
      if (is_network_error(err_code))
      {
        mi->report(ERROR_LEVEL, err_code, NULL, "Setting @slave_gtid_strict_mode failed with error: %s",
                   mysql_error(mysql));
        goto network_err;
      }
      else
    ...
    query_str.length(0);
    if (query_str.append(STRING_WITH_LEN("SET @slave_gtid_ignore_duplicates="), system_charset_info) ||
    ...
    {
      query_str.length(0);
      query_str.append(STRING_WITH_LEN("SET @slave_until_gtid='"), system_charset_info);
      ...
      if (unlikely(rc))
      {
        ...
        if (is_network_error(err_code))
        {
          mi->report(ERROR_LEVEL, err_code, NULL, "Setting @slave_until_gtid failed with error: %s",
                     mysql_error(mysql));
          goto network_err;
        }
        else
  ...
  else
  {
    ...
    */
    ...
    quote_length = escape_quotes_for_mysql(&my_charset_bin, quote_buf, sizeof(quote_buf), mi->master_log_name,
                                           strlen(mi->master_log_name), &overflow);
    query.append(quote_buf, quote_length);
    query.append(STRING_WITH_LEN("',"));
    query.append_ulonglong(mi->master_log_pos);
    query.append(')');

    if (!mysql_real_query(mysql, query.c_ptr_safe(), query.length()) && (master_res = mysql_store_result(mysql)) &&
        (master_row = mysql_fetch_row(master_res)) && (master_row[0] != NULL))
    {
      rpl_global_gtid_slave_state->load(mi->io_thd, master_row[0], strlen(master_row[0]), false, false);
    }
    else if (check_io_slave_killed(mi, NULL))
      goto slave_killed_err;
    else if (is_network_error(mysql_errno(mysql)))
    {
      mi->report(WARNING_LEVEL, mysql_errno(mysql), NULL, "Get master GTID position failed with error: %s",
                 mysql_error(mysql));
      goto network_err;
    }
    else
...
static bool wait_for_relay_log_space(Relay_log_info *rli)
{
  ...
  thd->ENTER_COND(&rli->log_space_cond, &rli->log_space_lock, &stage_waiting_for_relay_log_space, &old_stage);
  while (rli->log_space_limit < rli->log_space_total && !(slave_killed = io_slave_killed(mi)) &&
  ...
  if (ignore_log_space_limit)
  {
#ifndef DBUG_OFF
    {
      DBUG_PRINT("info", ("log_space_limit=%llu log_space_total=%llu "
                          ...
                          rli->log_space_limit, uint64(rli->log_space_total), (int)rli->ignore_log_space_limit,
...
static void write_ignored_events_info_to_relay_log(THD *thd, Master_info *mi)
{
  ...
  if (rli->ign_master_log_name_end[0] || rli->ign_gtids.count())
  {
    ...
    {
      rev = new Rotate_log_event(rli->ign_master_log_name_end, 0, rli->ign_master_log_pos_end,
                                 Rotate_log_event::DUP_NAME);
      rli->ign_master_log_name_end[0] = 0;
      if (unlikely(!(bool)rev))
        mi->report(ERROR_LEVEL, ER_SLAVE_CREATE_EVENT_FAILURE, NULL, ER_THD(thd, ER_SLAVE_CREATE_EVENT_FAILURE),
    ...
    if (rli->ign_gtids.count())
    {
      ...
      glev = new Gtid_list_log_event(&rli->ign_gtids, Gtid_list_log_event::FLAG_IGN_GTIDS);
      rli->ign_gtids.reset();
    ...
    if (rev)
    {
      ...
      if (unlikely(rli->relay_log.append(rev)))
        mi->report(ERROR_LEVEL, ER_SLAVE_RELAY_LOG_WRITE_FAILURE, NULL, ER_THD(thd, ER_SLAVE_RELAY_LOG_WRITE_FAILURE),
...
int register_slave_on_master(MYSQL *mysql, Master_info *mi, bool *suppress_warnings)
{
  ...
  *suppress_warnings = FALSE;
  ...
  {
    sql_print_warning(
        "The length of report_host is %zu. "
  ...
  {
    sql_print_warning(
        "The length of report_user is %zu. "
        "It is larger than the max length(%d), so this "
        "slave cannot be registered to the master.",
  ...
  {
    sql_print_warning(
        "The length of report_password is %zu. "
        "It is larger than the max length(%d), so this "
        "slave cannot be registered to the master.",
  ...
  int4store(pos, global_system_variables.server_id);
  pos += 4;
  ...
  if (simple_command(mysql, COM_REGISTER_SLAVE, buf, (ulong)(pos - buf), 0))
  {
    ...
    {
      char buf[256];
      my_snprintf(buf, sizeof(buf), "%s (Errno: %d)", mysql_error(mysql), mysql_errno(mysql));
      mi->report(ERROR_LEVEL, ER_SLAVE_MASTER_COM_FAILURE, NULL, ER(ER_SLAVE_MASTER_COM_FAILURE), "COM_REGISTER_SLAVE",
                 buf);
    }
    DBUG_RETURN(1);
  }
...
bool show_master_info(THD *thd, Master_info *mi, bool full)
{
  ...
  if (thd->protocol->send_result_set_metadata(&field_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
    DBUG_RETURN(TRUE);
...
void show_master_info_get_fields(THD *thd, List<Item> *field_list, bool full, size_t gtid_pos_length)
{
  ...
  if (full)
  {
    ...
    field_list->push_back(new (mem_root) Item_empty_string(thd, "Slave_SQL_State", 30), mem_root);
  }

  field_list->push_back(new (mem_root) Item_empty_string(thd, "Slave_IO_State", 30), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_Host", sizeof(mi->host)), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_User", sizeof(mi->user)), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Master_Port", 7, MYSQL_TYPE_LONG), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Connect_Retry", 10, MYSQL_TYPE_LONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_Log_File", FN_REFLEN), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Read_Master_Log_Pos", 10, MYSQL_TYPE_LONGLONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Relay_Log_File", FN_REFLEN), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Relay_Log_Pos", 10, MYSQL_TYPE_LONGLONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Relay_Master_Log_File", FN_REFLEN), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Slave_IO_Running", 3), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Slave_SQL_Running", 3), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Rewrite_DB", 23), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Do_DB", 20), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Ignore_DB", 20), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Do_Table", 20), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Ignore_Table", 23), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Wild_Do_Table", 24), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Wild_Ignore_Table", 28), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Last_Errno", 4, MYSQL_TYPE_LONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Last_Error", 20), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Skip_Counter", 10, MYSQL_TYPE_LONG), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Exec_Master_Log_Pos", 10, MYSQL_TYPE_LONGLONG), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Relay_Log_Space", 10, MYSQL_TYPE_LONGLONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Until_Condition", 6), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Until_Log_File", FN_REFLEN), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Until_Log_Pos", 10, MYSQL_TYPE_LONGLONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_Allowed", 7), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_CA_File", sizeof(mi->ssl_ca)), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_CA_Path", sizeof(mi->ssl_capath)), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_Cert", sizeof(mi->ssl_cert)), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_Cipher", sizeof(mi->ssl_cipher)), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_Key", sizeof(mi->ssl_key)), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Seconds_Behind_Master", 10, MYSQL_TYPE_LONGLONG),
                        mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_Verify_Server_Cert", 3), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Last_IO_Errno", 4, MYSQL_TYPE_LONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Last_IO_Error", 20), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Last_SQL_Errno", 4, MYSQL_TYPE_LONG), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Last_SQL_Error", 20), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Ignore_Server_Ids", FN_REFLEN), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Master_Server_Id", sizeof(ulong), MYSQL_TYPE_LONG),
                        mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_Crl", sizeof(mi->ssl_crl)), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Master_SSL_Crlpath", sizeof(mi->ssl_crlpath)), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Using_Gtid", sizeof("Current_Pos") - 1), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Gtid_IO_Pos", 30), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Do_Domain_Ids", FN_REFLEN), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Replicate_Ignore_Domain_Ids", FN_REFLEN), mem_root);
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Parallel_Mode", sizeof("conservative") - 1), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "SQL_Delay", 10, MYSQL_TYPE_LONG));
  field_list->push_back(new (mem_root) Item_return_int(thd, "SQL_Remaining_Delay", 8, MYSQL_TYPE_LONG));
  field_list->push_back(new (mem_root) Item_empty_string(thd, "Slave_SQL_Running_State", 20));
  field_list->push_back(new (mem_root) Item_return_int(thd, "Slave_DDL_Groups", 20, MYSQL_TYPE_LONGLONG), mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Slave_Non_Transactional_Groups", 20, MYSQL_TYPE_LONGLONG),
                        mem_root);
  field_list->push_back(new (mem_root) Item_return_int(thd, "Slave_Transactional_Groups", 20, MYSQL_TYPE_LONGLONG),
                        mem_root);

  if (full)
  {
    field_list->push_back(new (mem_root) Item_return_int(thd, "Retried_transactions", 10, MYSQL_TYPE_LONG), mem_root);
    field_list->push_back(new (mem_root) Item_return_int(thd, "Max_relay_log_size", 10, MYSQL_TYPE_LONGLONG), mem_root);
    field_list->push_back(new (mem_root) Item_return_int(thd, "Executed_log_entries", 10, MYSQL_TYPE_LONG), mem_root);
    field_list->push_back(new (mem_root) Item_return_int(thd, "Slave_received_heartbeats", 10, MYSQL_TYPE_LONG),
                          mem_root);
    field_list->push_back(new (mem_root) Item_float(thd, "Slave_heartbeat_period", 0.0, 3, 10), mem_root);
    field_list->push_back(new (mem_root) Item_empty_string(thd, "Gtid_Slave_Pos", (uint)gtid_pos_length), mem_root);
  }
  DBUG_VOID_RETURN;
}
...
static const LEX_CSTRING slave_running[] = {
    {STRING_WITH_LEN("No")}, {STRING_WITH_LEN("Connecting")}, {STRING_WITH_LEN("Preparing")}, {STRING_WITH_LEN("Yes")}};
...
static bool send_show_master_info_data(THD *thd, Master_info *mi, bool full, String *gtid_pos)
{
  ...
  if (mi->host[0])
  {
    ...
    */
    if (full)
      protocol->store(mi->connection_name.str, mi->connection_name.length, &my_charset_bin);
    ...
    protocol->store(mi->master_log_name, strlen(mi->master_log_name), &my_charset_bin);
    protocol->store((ulonglong)mi->master_log_pos);
    msg = (mi->rli.group_relay_log_name + dirname_length(mi->rli.group_relay_log_name));
    protocol->store(msg, strlen(msg), &my_charset_bin);
    protocol->store((ulonglong)mi->rli.group_relay_log_pos);
    protocol->store(mi->rli.group_master_log_name, strlen(mi->rli.group_master_log_name), &my_charset_bin);
    ...
    protocol->store_string_or_null(mi->rli.last_error().message, &my_charset_bin);
    ...
    msg = (mi->rli.until_condition == Relay_log_info::UNTIL_NONE
               ...
               : (mi->rli.until_condition == Relay_log_info::UNTIL_MASTER_POS
                      ? "Master"
                      : (mi->rli.until_condition == Relay_log_info::UNTIL_RELAY_POS ? "Relay" : "Gtid")));
    protocol->store(msg, strlen(msg), &my_charset_bin);
    ...
    */
    if ((mi->slave_running == MYSQL_SLAVE_RUN_READING) && mi->rli.slave_running)
    {
      long time_diff;
      bool idle;
    ...
    protocol->store(mi->ssl_verify_server_cert ? &msg_yes : &msg_no, &my_charset_bin);
    ...
    // Using_Gtid
    protocol->store_string_or_null(mi->using_gtid_astext(mi->using_gtid), &my_charset_bin);
    // Gtid_IO_Pos
    {
    ...
    const char *slave_sql_running_state = mi->rli.sql_driver_thd ? mi->rli.sql_driver_thd->proc_info : "";
...
static int cmp_mi_by_name(const Master_info **arg1, const Master_info **arg2)
{
  return my_strcasecmp(system_charset_info, (*arg1)->connection_name.str, (*arg2)->connection_name.str);
}
...
bool show_all_master_info(THD *thd)
{
  ...
  if (!master_info_index || !(elements = master_info_index->master_info_hash.records))
    goto end;
  ...
  */
  ...
  {
    tmp[i] = (Master_info *)my_hash_element(&master_info_index->master_info_hash, i);
  }
...
void set_slave_thread_options(THD *thd)
{
  ...
  */
  ulonglong options = (thd->variables.option_bits | OPTION_BIG_SELECTS | OPTION_BIN_LOG);
...
void set_slave_thread_default_charset(THD *thd, rpl_group_info *rgi)
{
  ...
  thd->update_charset(global_system_variables.character_set_client, global_system_variables.collation_connection);
...
static int init_slave_thread(THD *thd, Master_info *mi, SLAVE_THD_TYPE thd_type)
{
  DBUG_ENTER("init_slave_thread");
  int simulate_error __attribute__((unused)) = 0;
  DBUG_EXECUTE_IF("simulate_io_slave_error_on_init", simulate_error |= (1 << SLAVE_THD_IO););
  DBUG_EXECUTE_IF("simulate_sql_slave_error_on_init", simulate_error |= (1 << SLAVE_THD_SQL););
  ...
  if (my_net_init(&thd->net, 0, thd, MYF(MY_THREAD_SPECIFIC)) || IF_DBUG(simulate_error & (1 << thd_type), 0))
  {
    thd->cleanup();
    DBUG_RETURN(-1);
  }
...
static int request_dump(THD *thd, MYSQL *mysql, Master_info *mi, bool *suppress_warnings)
{
  ...
  *suppress_warnings = FALSE;
  ...
  if (simple_command(mysql, COM_BINLOG_DUMP, buf, len + 10, 1))
  {
    ...
    */
    if (mysql_errno(mysql) == ER_NET_READ_INTERRUPTED || mysql_errno(mysql) == ER_NET_ERROR_ON_WRITE)
      *suppress_warnings = TRUE;  // Suppress reconnect warning
    else
  ...
  */
  ...
  if (unlikely(len == packet_error || (long)len < 1))
  {
    if (mysql_errno(mysql) == ER_NET_READ_INTERRUPTED)
    {
      ...
      */
      *suppress_warnings = global_system_variables.log_warnings < 2 ? TRUE : FALSE;
    }
    else
    {
  ...
  DBUG_PRINT("exit", ("len: %lu  net->read_pos[4]: %d", len, mysql->net.read_pos[4]));
  DBUG_RETURN(len - 1);
}
...
int sql_delay_event(Log_event *ev, THD *thd, rpl_group_info *rgi)
{
  ...
  if (sql_delay && type != ROTATE_EVENT && type != FORMAT_DESCRIPTION_EVENT && type != START_EVENT_V3)
  {
    // The time when we should execute the event.
    time_t sql_delay_end = ev->when + rli->mi->clock_diff_with_master + sql_delay;
    // The current time.
    ...
    DBUG_PRINT("info", ("sql_delay= %lu "
                        ...
                        "sql_delay_end= %llu "
                        "nap_time= %ld",
                        sql_delay, (long)ev->when, rli->mi->clock_diff_with_master, (long)now, (ulonglong)sql_delay_end,
                        (long)nap_time));
...
*/
static int apply_event_and_update_pos_setup(Log_event *ev, THD *thd, rpl_group_info *rgi)
{
  DBUG_ENTER("apply_event_and_update_pos_setup");

  DBUG_PRINT("exec_event",
             ("%s(type_code: %d; server_id: %d)", ev->get_type_str(), ev->get_type_code(), ev->server_id));
  DBUG_PRINT("info", ("thd->options: '%s%s%s'  rgi->last_event_start_time: %lu",
                      FLAGSTR(thd->variables.option_bits, OPTION_NOT_AUTOCOMMIT),
                      FLAGSTR(thd->variables.option_bits, OPTION_BEGIN),
                      FLAGSTR(thd->variables.option_bits, OPTION_GTID_BEGIN), (ulong)rgi->last_event_start_time));
  ...
  */
  ...
  thd->variables.option_bits = (thd->variables.option_bits & ~OPTION_SKIP_REPLICATION) |
 ...
 */
static int apply_event_and_update_pos_apply(Log_event *ev, THD *thd, rpl_group_info *rgi, int reason)
{
  int exec_res = 0;
  Relay_log_info *rli = rgi->rli;
  ...
  DBUG_EXECUTE_IF("inject_slave_sql_before_apply_event", {
    DBUG_ASSERT(!debug_sync_set_action(thd, STRING_WITH_LEN("now WAIT_FOR continue")));
    ...
    if (exec_res)
    {
      ...
      switch (thd->wsrep_trx().state())
      {
        case wsrep::transaction::s_must_replay:
          ...
          break;
        default:
          WSREP_DEBUG("SQL apply failed, res %d conflict state: %s", exec_res, wsrep_thd_transaction_state_str(thd));
          rli->abort_slave = 1;
          rli->report(ERROR_LEVEL, ER_UNKNOWN_COM_ERROR, rgi->gtid_info(), "Node has dropped from cluster");
          break;
      }
      mysql_mutex_unlock(&thd->LOCK_thd_data);
    }
  ...
  */
  static const char *const explain[] = {// EVENT_SKIP_NOT,
                                        "not skipped",
                                        // EVENT_SKIP_IGNORE,
  ...
  DBUG_PRINT("info",
             ("OPTION_BEGIN: %d  IN_STMT: %d  IN_TRANSACTION: %d", MY_TEST(thd->variables.option_bits & OPTION_BEGIN),
              rli->get_flag(Relay_log_info::IN_STMT), rli->get_flag(Relay_log_info::IN_TRANSACTION)));
  DBUG_PRINT("skip_event", ("%s event was %s", ev->get_type_str(), explain[reason]));
#endif

  DBUG_PRINT("info", ("apply_event error = %d", exec_res));
  if (exec_res == 0)
  {
    if (thd->rgi_slave && (thd->rgi_slave->gtid_ev_flags_extra & Gtid_log_event::FL_START_ALTER_E1) &&
    ...
    {
      DBUG_PRINT("info", ("group %llu %s", rli->group_relay_log_pos, rli->group_relay_log_name));
      DBUG_PRINT("info", ("event %llu %s", rli->event_relay_log_pos, rli->event_relay_log_name));
    }
#endif
    /*
...
/*
  ...
  rli->data_lock held.
*/
int apply_event_and_update_pos_for_parallel(Log_event *ev, THD *thd, rpl_group_info *rgi)
{
  int rc = 0;
  ulong retries = 0;
  ...
  */
  do
  {
    ...
    if (rc && is_sa)
    {
      is_sa_temp_err = is_parallel_retry_error(rgi, thd->get_stmt_da()->sql_errno());
    }
    ...
    */
    ...
    if (global_system_variables.log_warnings > 2)
    {
      ...
      sql_print_information(
          ...
          "legacy mode",
          static_cast<Query_log_event *>(ev)->query, gtid->domain_id, gtid->server_id, gtid->seq_no,
...
inline void update_state_of_relay_log(Relay_log_info *rli, Log_event *ev)
{
  ...
  if (typ == XID_EVENT || typ == XA_PREPARE_LOG_EVENT)
    rli->clear_flag(Relay_log_info::IN_TRANSACTION);
  if (typ == GTID_EVENT && !(((Gtid_log_event *)ev)->flags2 & Gtid_log_event::FL_STANDALONE))
  {
    /* This GTID_EVENT will generate a BEGIN event */
    rli->set_flag(Relay_log_info::IN_TRANSACTION);
  }

  DBUG_PRINT("info", ("event: %u  IN_STMT: %d  IN_TRANSACTION: %d", (uint)typ, rli->get_flag(Relay_log_info::IN_STMT),
...
static int exec_relay_log_event(THD *thd, Relay_log_info *rli, rpl_group_info *serial_rgi)
{
  ...
  */
  ...
  if (ev)
  {
     ...
     */
    if (!rli->mi->using_parallel() && !(ev->is_artificial_event() || ev->is_relay_log_event() || (ev->when == 0)))
    {
    ...
    */
    if ((rli->until_condition == Relay_log_info::UNTIL_MASTER_POS ||
        ...
        (ev->server_id != global_system_variables.server_id || rli->replicate_same_server_id) &&
    ...
    { /**
      ...
      */
      DBUG_EXECUTE_IF(
          ...
          if ((typ == XID_EVENT) ||
              (LOG_EVENT_IS_QUERY(typ) && strcmp("COMMIT", ((Query_log_event *)ev)->query) == 0)) {
    ...
    if (typ == GTID_EVENT)
    {
      ...
      */
      ...
      if (opt_gtid_ignore_duplicates && rli->mi->using_gtid != Master_info::USE_GTID_NO)
      {
        int res = rpl_global_gtid_slave_state->check_duplicate_gtid(&serial_rgi->current_gtid, serial_rgi);
        if (res < 0)
        {
          sql_print_error("Error processing GTID event: %s",
                          "slave SQL "
          ...
          */
          if (serial_rgi->trans_retries < slave_trans_retries)
          {
            ...
            else
            {
              ...
              slave_sleep(thd,
                          MY_MAX(MY_MIN(serial_rgi->trans_retries, MAX_SLAVE_RETRY_PAUSE), slave_trans_retry_interval),
            ...
            sql_print_error(
                ...
                slave_trans_retries);
                 ...
                 (opt_using_transactions && rli->group_relay_log_pos == rli->event_relay_log_pos))
        {
          /*
          ...
          DBUG_PRINT("info", ("Resetting retry counter, rgi->trans_retries: %lu", serial_rgi->trans_retries));
        }
      }
...
#ifdef ENABLED_DEBUG_SYNC
    DBUG_EXECUTE_IF(
        "pause_sql_thread_on_fde", if (ev && typ == FORMAT_DESCRIPTION_EVENT) {
          DBUG_ASSERT(
              !debug_sync_set_action(thd, STRING_WITH_LEN("now SIGNAL paused_on_fde WAIT_FOR sql_thread_continue")));
        });
#endif
  ...
  mysql_mutex_unlock(&rli->data_lock);
  rli->report(ERROR_LEVEL, ER_SLAVE_RELAY_LOG_READ_FAILURE, NULL, ER_THD(thd, ER_SLAVE_RELAY_LOG_READ_FAILURE),
              "\
...
static int try_to_reconnect(THD *thd, MYSQL *mysql, Master_info *mi, uint *retry_count, bool suppress_warnings,
                            const char *messages[SLAVE_RECON_MSG_MAX])
{
  ...
  if (!suppress_warnings)
  {
    ...
    my_snprintf(buf, sizeof(buf), messages[SLAVE_RECON_MSG_FAILED], IO_RPL_LOG_NAME, mi->master_log_pos,
                tmp.c_ptr_safe());
    /*
    ...
    */
    ...
    {
      mi->report(WARNING_LEVEL, ER_SLAVE_MASTER_COM_FAILURE, NULL, ER_THD(thd, ER_SLAVE_MASTER_COM_FAILURE),
...
pthread_handler_t handle_slave_io(void *arg)
{
  ...
  DBUG_PRINT("master_info", ("log_file_name: '%s'  position: %llu", mi->master_log_name, mi->master_log_pos));
  ...
  if (rpl_load_gtid_slave_state(thd))
  {
    mi->report(ERROR_LEVEL, thd->get_stmt_da()->sql_errno(), NULL,
               "Unable to load replication GTID slave state from mysql.%s: %s", rpl_gtid_slave_state_table_name.str,
  ...
  thd->variables.wsrep_on = 0;
  if (DBUG_IF("failed_slave_start") || repl_semisync_slave.slave_start(mi))
  {
    mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, NULL, ER_THD(thd, ER_SLAVE_FATAL_ERROR),
  ...
  {
    mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, NULL, ER_THD(thd, ER_SLAVE_FATAL_ERROR), "error in mysql_init()");
    goto err;
  }
  ...
  if (!safe_connect(thd, mysql, mi))
  {
    if (mi->using_gtid == Master_info::USE_GTID_NO)
      sql_print_information(
          ...
          mi->user, mi->host, mi->port, IO_RPL_LOG_NAME, mi->master_log_pos);
    else
    {
      StringBuffer<100> tmp;
      mi->gtid_current_pos.to_string(&tmp);
...
connected:
  ...
  if (ret == 2)
  {
    ...
    /*
      ...
      problem
    */
    if (try_to_reconnect(thd, mysql, mi, &retry_count, suppress_warnings, reconnect_messages[SLAVE_RECON_ACT_REG]))
      goto err;
    goto connected;
  }
  ...
  while (!io_slave_killed(mi))
  {
    ...
    if (request_dump(thd, mysql, mi, &suppress_warnings))
    {
      ...
      if (check_io_slave_killed(mi, NullS) ||
          try_to_reconnect(thd, mysql, mi, &retry_count, suppress_warnings, reconnect_messages[SLAVE_RECON_ACT_DUMP]))
        goto err;
      goto connected;
    }
...
slave_max_allowed_packet",
                slave_max_allowed_packet);
            mi->report(ERROR_LEVEL, ER_NET_PACKET_TOO_LARGE, NULL, "%s",
          ...
          case ER_MASTER_FATAL_ERROR_READING_BINLOG:
            mi->report(ERROR_LEVEL, ER_MASTER_FATAL_ERROR_READING_BINLOG, NULL,
                       ER_THD(thd, ER_MASTER_FATAL_ERROR_READING_BINLOG), mysql_error_number, mysql_error(mysql));
            goto err;
          case ER_OUT_OF_RESOURCES:
            sql_print_error(
                "\
Stopping slave I/O thread due to out-of-memory error from master");
            mi->report(ERROR_LEVEL, ER_OUT_OF_RESOURCES, NULL, "%s", ER_THD(thd, ER_OUT_OF_RESOURCES));
            goto err;
        }
      ...
      mi->semi_ack = 0;
      if (repl_semisync_slave.slave_read_sync_header((const uchar *)mysql->net.read_pos + 1, event_len, &(mi->semi_ack),
                                                     &event_buf, &event_len))
      {
        mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, NULL, ER_THD(thd, ER_SLAVE_FATAL_ERROR),
      ...
      if (mi->using_gtid == Master_info::USE_GTID_NO &&
          ...
          */
          (!repl_semisync_slave.get_slave_enabled() ||
           (!(mi->semi_ack & SEMI_SYNC_SLAVE_DELAY_SYNC) || (mi->semi_ack & (SEMI_SYNC_NEED_ACK)))) &&
          (DBUG_IF("failed_flush_master_info") || flush_master_info(mi, TRUE, TRUE)))
      {
        sql_print_error("Failed to flush master info file");
        goto err;
      }
...
err:
  ...
  if (mi->using_gtid == Master_info::USE_GTID_NO)
    sql_print_information(
        "Slave I/O thread exiting, read up to log '%s', "
        "position %llu, master %s:%d",
        IO_RPL_LOG_NAME, mi->master_log_pos, mi->host, mi->port);
  else
  {
    StringBuffer<100> tmp;
    mi->gtid_current_pos.to_string(&tmp);
    sql_print_information(
        ...
        IO_RPL_LOG_NAME, mi->master_log_pos, tmp.c_ptr_safe(), mi->host, mi->port);
  }
...
void slave_output_error_info(rpl_group_info *rgi, THD *thd)
{
  ...
  */
  ...
  if (unlikely(thd->is_error()))
  {
    ...
    DBUG_PRINT("info", ("thd->get_stmt_da()->sql_errno()=%d; rli->last_error.number=%d",
    ...
    if (last_errno == 0)
    {
      ...
      */
      rli->report(ERROR_LEVEL, thd->get_stmt_da()->sql_errno(), rgi->gtid_info(), "%s", errmsg);
    }
    else if (last_errno != thd->get_stmt_da()->sql_errno())
    {
       ...
       */
      sql_print_error("Slave (additional info): %s Error_code: %d", errmsg, thd->get_stmt_da()->sql_errno());
    }
  }
...
wsrep_restart_point:
  ...
  */
  ...
  if (opt_slave_parallel_threads > 0 && rpl_parallel_activate_pool(&global_rpl_thread_pool))
  {
  ...
  if (init_slave_thread(thd, mi, SLAVE_THD_SQL))
  {
    ...
    */
    mysql_cond_broadcast(&rli->start_cond);
    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, NULL, "Failed during slave thread initialization");
    goto err_during_init;
  }
  ...
  */
  ...
  rli->last_seen_gtid = serial_rgi->current_gtid;
  if (mi->using_gtid != Master_info::USE_GTID_NO && mi->using_parallel() && rli->restart_gtid_pos.count() > 0)
  {
    /*
  ...
  {
    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, NULL, "Error initializing relay log position: %s", errmsg);
  ...
  DBUG_PRINT("master_info",
             ("log_file_name: %s  position: %llu", rli->group_master_log_name, rli->group_master_log_pos));
  if (global_system_variables.log_warnings)
  {
    StringBuffer<100> tmp;
    if (mi->using_gtid != Master_info::USE_GTID_NO)
    {
      tmp.append(STRING_WITH_LEN("; GTID position '"));
      rpl_append_gtid_state(&tmp, mi->using_gtid == Master_info::USE_GTID_CURRENT_POS);
      tmp.append(STRING_WITH_LEN("'"));
    }
    sql_print_information(
        ...
        "position: %llu%s",
        RPL_LOG_NAME, rli->group_master_log_pos, rli->group_relay_log_name, rli->group_relay_log_pos, tmp.c_ptr_safe());
  }

  if (check_temp_dir(rli->slave_patternload_file))
  {
    check_temp_dir_result = thd->get_stmt_da()->sql_errno();
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->sql_errno(), NULL, "Unable to use slave's temporary directory %s - %s",
  ...
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->sql_errno(), NULL,
                "Error processing replication GTID position tables: %s", thd->get_stmt_da()->message());
    goto err;
  }
  ...
  */
      ...
      rli->is_until_satisfied(NULL))
  {
    sql_print_information(
        ...
        rli->until_condition == Relay_log_info::UNTIL_MASTER_POS ? "binlog" : "relaylog");
    mysql_mutex_unlock(&rli->data_lock);
    goto err;
  }
  mysql_mutex_unlock(&rli->data_lock);
  ...
  while (!sql_slave_killed(serial_rgi))
  {
    ...
    if (saved_skip && rli->slave_skip_counter == 0)
    {
      ...
      sql_print_information(
          ...
          "relay_log_file='%s', relay_log_pos='%ld', master_log_name='%s', "
          "master_log_pos='%ld'%s ",
          (ulong)saved_skip, saved_log_name, (ulong)saved_log_pos, saved_master_log_name, (ulong)saved_master_log_pos,
          saved_skip_gtid_pos.c_ptr_safe(), rli->group_relay_log_name, (ulong)rli->group_relay_log_pos,
          rli->group_master_log_name, (ulong)rli->group_master_log_pos, tmp.c_ptr_safe());
      saved_skip = 0;
...
err:
  ...
  if (rli->stop_for_until && rli->until_condition != Relay_log_info::UNTIL_GTID)
  {
    if (global_system_variables.log_warnings > 2)
      sql_print_information(
          "Slave SQL thread UNTIL stop was requested at position "
          "%llu in %s %s file",
  ...
  /* Thread stopped. Print the current replication position to the log */
  {
    ...
    sql_print_information(
        "Slave SQL thread exiting, replication stopped in "
        "log '%s' at position %llu%s, master: %s:%d",
        RPL_LOG_NAME, rli->group_master_log_pos, tmp.c_ptr_safe(), mi->host, mi->port);
  }
#ifdef WITH_WSREP
...
*/
static int queue_binlog_ver_1_event(Master_info *mi, const uchar *buf, ulong event_len)
{
  const char *errmsg = 0;
  ulong inc_pos;
  bool ignore_event = 0;
  ...
  */
  ...
  {
    if (unlikely(!(tmp_buf = (uchar *)my_malloc(key_memory_binlog_ver_1_event, event_len + 1, MYF(MY_WME)))))
    {
      mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, NULL, ER(ER_SLAVE_FATAL_ERROR), "Memory allocation failed");
      DBUG_RETURN(1);
    }
  ...
  */
  Log_event *ev = Log_event::read_log_event(buf, event_len, &errmsg, mi->rli.relay_log.description_event_for_queue, 0);
  if (unlikely(!ev))
  {
    sql_print_error(
...
*/
static int queue_binlog_ver_3_event(Master_info *mi, const uchar *buf, ulong event_len)
{
  const char *errmsg = 0;
  ulong inc_pos;
  char *tmp_buf = 0;
...
static int queue_event(Master_info *mi, const uchar *buf, ulong event_len)
{
  ...
  */
  enum enum_binlog_checksum_alg checksum_alg = mi->checksum_alg_before_fd != BINLOG_CHECKSUM_ALG_UNDEF
                                                   ? mi->checksum_alg_before_fd
                                                   : mi->rli.relay_log.relay_log_checksum_alg;
  ...
  DBUG_ASSERT(checksum_alg == BINLOG_CHECKSUM_ALG_OFF || checksum_alg == BINLOG_CHECKSUM_ALG_UNDEF ||
  ...
  */
  ...
  DBUG_EXECUTE_IF(
      "corrupt_queue_event", if (buf[EVENT_TYPE_OFFSET] != FORMAT_DESCRIPTION_EVENT) {
  ...
  */
  DBUG_EXECUTE_IF(
      "dbug.rows_events_to_delay_relay_logging",
      if ((buf[EVENT_TYPE_OFFSET] == WRITE_ROWS_EVENT_V1 || buf[EVENT_TYPE_OFFSET] == WRITE_ROWS_EVENT) &&
  ...
  switch (buf[EVENT_TYPE_OFFSET])
  {
    ...
    case ROTATE_EVENT:
    {
      Rotate_log_event rev(buf, checksum_alg != BINLOG_CHECKSUM_ALG_OFF ? event_len - BINLOG_CHECKSUM_LEN : event_len,
                           mi->rli.relay_log.description_event_for_queue);
      bool master_changed = false;
      bool maybe_crashed = false;
      // Exclude server start scenario
      if ((mi->prev_master_id && mi->master_id) && (mi->prev_master_id != mi->master_id))
        master_changed = true;
      if ((mi->master_log_name[0] != '\0') && (strcmp(rev.new_log_ident, mi->master_log_name) != 0))
        maybe_crashed = true;

      if (unlikely((mi->gtid_reconnect_event_skip_count && master_changed) || maybe_crashed) &&
          unlikely(!mi->gtid_event_seen) && rev.is_artificial_event())
      {
        ...
        */
        ...
        if (mi->gtid_reconnect_event_skip_count)
        {
          if (master_changed)
            sql_print_warning(
                "The server_id of master server changed in the "
                ...
                "received: %lu",
                mi->last_queued_gtid.domain_id, mi->last_queued_gtid.server_id, mi->last_queued_gtid.seq_no,
                mi->prev_master_id, mi->master_id);
          else
            sql_print_warning(
                "Unexpected change of master binlog file name in "
        ...
        mysql_mutex_lock(log_lock);
        if (likely(!rli->relay_log.write_event(&fdle) && !rli->relay_log.flush_and_sync(NULL)))
        {
          rli->relay_log.harvest_bytes_written(&rli->log_space_total);
        }
        else
      ...
      */
          ...
          mi->rli.relay_log.relay_log_checksum_alg != BINLOG_CHECKSUM_ALG_OFF)
      {
        ...
        int4store(&rot_buf[EVENT_LEN_OFFSET], uint4korr(&rot_buf[EVENT_LEN_OFFSET]) + BINLOG_CHECKSUM_LEN);
        rot_crc = my_checksum(rot_crc, (const uchar *)rot_buf, event_len - BINLOG_CHECKSUM_LEN);
    ...
    case FORMAT_DESCRIPTION_EVENT:
    {
      ...
      */
      ...
      if (!(tmp = (Format_description_log_event *)Log_event::read_log_event(
                buf, event_len, &errmsg, mi->rli.relay_log.description_event_for_queue, 1)))
      {
        error = ER_SLAVE_RELAY_LOG_WRITE_FAILURE;
        goto err;
      }
    ...
    case HEARTBEAT_LOG_EVENT:
    {
      ...
      */
      if (memcmp(mi->master_log_name, hb.get_log_ident(), hb.get_ident_len()) || mi->master_log_pos > hb.log_pos)
      {
    ...
    case GTID_LIST_EVENT:
    {
      ...
      if (!(tmp = Log_event::read_log_event(buf, event_len, &errmsg, mi->rli.relay_log.description_event_for_queue,
      ...
      */
      ...
      if (mi->rli.until_condition == Relay_log_info::UNTIL_GTID && flags & Gtid_list_log_event::FLAG_UNTIL_REACHED)
      {
        char str_buf[128];
        String str(str_buf, sizeof(str_buf), system_charset_info);
        mi->rli.until_gtid_pos.to_string(&str);
        sql_print_information(
            "Slave I/O thread stops because it reached its"
    ...
    case GTID_EVENT:
    {
      ...
      if (Gtid_log_event::peek(buf, event_len, checksum_alg, &event_gtid.domain_id, &event_gtid.server_id,
                               &event_gtid.seq_no, &gtid_flag, rli->relay_log.description_event_for_queue))
      {
        error = ER_SLAVE_RELAY_LOG_WRITE_FAILURE;
        goto err;
      }
      ...
      if (unlikely(mi->gtid_reconnect_event_skip_count))
      {
        if (likely(!mi->gtid_event_seen))
        {
          ...
          */
              ...
              event_gtid.seq_no != mi->last_queued_gtid.seq_no)
          {
            ...
            first = true;
            rpl_slave_state_tostring_helper(&error_msg, &mi->last_queued_gtid, &first);
          ...
          if (global_system_variables.log_warnings > 1)
          {
            ...
            rpl_slave_state_tostring_helper(&gtid_text, &mi->last_queued_gtid, &first);
            sql_print_information(
                "Slave IO thread is reconnected to "
        ...
        else
        {
          ...
          error = ER_SLAVE_RELAY_LOG_WRITE_FAILURE;
          sql_print_error(
              "Slave IO thread has received a new Gtid_log_event "
              ...
              gtid_text.ptr(), mi->gtid_reconnect_event_skip_count, mi->events_queued_since_last_gtid);
          goto err;
        }
      }
      mi->gtid_event_seen = true;
      ...
      */
      if (unlikely(mi->events_queued_since_last_gtid > 0))
      {
        ...
        */
        IF_DBUG(rpl_gtid *gtid_in_slave_state = mi->gtid_current_pos.find(mi->last_queued_gtid.domain_id), );

        // Slave gtid state must not have updated yet to the last received gtid.
        DBUG_ASSERT((mi->using_gtid == Master_info::USE_GTID_NO || !opt_gtid_strict_mode) ||
                    (!gtid_in_slave_state || !(*gtid_in_slave_state == mi->last_queued_gtid)));

        DBUG_EXECUTE_IF("slave_discard_xid_for_gtid_0_x_1000", {
          /* Inject an event group that is missing its XID commit event. */
          if ((mi->last_queued_gtid.domain_id == 0 && mi->last_queued_gtid.seq_no == 1000) ||
              (mi->last_queued_gtid.domain_id == 1 && mi->last_queued_gtid.seq_no == 32))
          {
            sql_print_warning(
      ...
      {
        if (mi->gtid_reconnect_event_skip_count == mi->events_queued_since_last_gtid)
        {
    ...
    dbug_gtid_accept:
      DBUG_EXECUTE_IF("slave_discard_gtid_0_x_1002", {
        if (mi->last_queued_gtid.server_id == 27697 && mi->last_queued_gtid.seq_no == 1002)
        {
      ...
      mi->last_queued_gtid_standalone = (gtid_flag & Gtid_log_event::FL_STANDALONE) != 0;
      ...
      */
      mi->do_accept_own_server_id = (s_id == global_system_variables.server_id && rpl_semi_sync_slave_enabled &&
                                     opt_gtid_strict_mode && mi->using_gtid != Master_info::USE_GTID_NO &&
                                     !mysql_bin_log.check_strict_gtid_sequence(
                                         event_gtid.domain_id, event_gtid.server_id, event_gtid.seq_no, true));
      // ...} eof else_likely
    }
    break;
    /*
    ...
    */
    case QUERY_COMPRESSED_EVENT:
      ...
      if (query_event_uncompress(rli->relay_log.description_event_for_queue, checksum_alg == BINLOG_CHECKSUM_ALG_CRC32,
                                 buf, event_len, new_buf_arr, sizeof(new_buf_arr), &is_malloc, &new_buf, &event_len))
      {
        char llbuf[22];
    ...
    case DELETE_ROWS_COMPRESSED_EVENT_V1:
      inc_pos = event_len;
      {
        if (row_log_event_uncompress(rli->relay_log.description_event_for_queue,
                                     checksum_alg == BINLOG_CHECKSUM_ALG_CRC32, buf, event_len, new_buf_arr,
                                     sizeof(new_buf_arr), &is_malloc, &new_buf, &event_len))
        {
          char llbuf[22];
          error = ER_BINLOG_UNCOMPRESS_ERROR;
          error_msg.append(STRING_WITH_LEN("binlog uncompress error, master log_pos: "));
    ...
    {
      is_rows_event = true;
      mi->rows_event_tracker.update(mi->master_log_name, mi->master_log_pos, buf,
                                    mi->rli.relay_log.description_event_for_queue);

      DBUG_EXECUTE_IF("simulate_stmt_end_rows_event_loss", { mi->rows_event_tracker.stmt_end_seen = false; });
    }
      goto default_action;
    ...
    case XID_EVENT:
      DBUG_EXECUTE_IF("slave_discard_xid_for_gtid_0_x_1000", {
        if (mi->last_queued_gtid.server_id == 27697 && mi->last_queued_gtid.seq_no == 1000)
        {
          DBUG_SET("-d,slave_discard_xid_for_gtid_0_x_1000");
          goto skip_relay_logging;
        }
    ...
    default_action:
      ...
      DBUG_EXECUTE_IF("kill_slave_io_before_commit", {
        if ((uchar)buf[EVENT_TYPE_OFFSET] == XID_EVENT ||
            ((uchar)buf[EVENT_TYPE_OFFSET] ==
                 QUERY_EVENT && /* QUERY_COMPRESSED_EVENT would never be commmit or rollback */
      ...
      if (mi->using_gtid != Master_info::USE_GTID_NO)
      {
        if (likely(mi->gtid_event_seen))
        {
          if (unlikely(mi->gtid_reconnect_event_skip_count))
          {
            if (!got_gtid_event && mi->gtid_reconnect_event_skip_count == mi->events_queued_since_last_gtid)
          ...
          else if (Log_event::is_group_event((Log_event_type)(uchar)buf[EVENT_TYPE_OFFSET]))
          {
        ...
        gtid_not_start:
          ...
          error = ER_SLAVE_RELAY_LOG_WRITE_FAILURE;
          sql_print_error(
              "The current group of events starts with "
              "a non-GTID %s event; "
              "the last seen GTID is %u-%u-%llu",
              Log_event::get_type_str((Log_event_type)(uchar)buf[EVENT_TYPE_OFFSET]), mi->last_queued_gtid);
          goto err;
        }
      }
  ...
  */
  if (((uchar)buf[EVENT_TYPE_OFFSET] != HEARTBEAT_LOG_EVENT) && !is_rows_event &&
      mi->rows_event_tracker.check_and_report(mi->master_log_name, mi->master_log_pos))
  {
    error = ER_SLAVE_RELAY_LOG_WRITE_FAILURE;
    goto err;
  }
  ...
  */
  if (inc_pos > 0 && event_len >= LOG_POS_OFFSET + 4 &&
  ...
  */
  ...
  {
    mi->master_log_pos += inc_pos;
    if ((uchar)buf[EVENT_TYPE_OFFSET] == FORMAT_DESCRIPTION_EVENT && s_id == mi->master_id)
    {
      /*
      ...
      */
      ...
      if (rli->relay_log.append_no_lock(rli->relay_log.description_event_for_queue))
        error = ER_SLAVE_RELAY_LOG_WRITE_FAILURE;
      else
        rli->relay_log.harvest_bytes_written(&rli->log_space_total);
    }
      ...
      */
      ...
      if (rli->relay_log.append_no_lock(&fake_rev))
        ...
        rli->relay_log.harvest_bytes_written(&rli->log_space_total);
  ...
  else if ((s_id == global_system_variables.server_id &&
            !(mi->rli.replicate_same_server_id || mi->do_accept_own_server_id)) ||
           ...
           */
           (mi->ignore_server_ids.elements > 0 && mi->shall_ignore_server_id(s_id) &&
           ...
           */
           (mi->domain_id_filter.is_group_filtered() &&
            Log_event::is_group_event((Log_event_type)(uchar)buf[EVENT_TYPE_OFFSET])))
  {
    /*
    ...
    */
    if (!(s_id == global_system_variables.server_id && !mi->rli.replicate_same_server_id) ||
    ...
    DBUG_PRINT("info", ("master_log_pos: %lu, event originating from %u server, ignored", (ulong)mi->master_log_pos,
                        uint4korr(buf + SERVER_ID_OFFSET)));
  }
  else
  {
    if (mi->do_accept_own_server_id)
    {
      int2store(const_cast<uchar *>(buf + FLAGS_OFFSET), uint2korr(buf + FLAGS_OFFSET) | LOG_EVENT_ACCEPT_OWN_F);
      if (checksum_alg != BINLOG_CHECKSUM_ALG_OFF)
      {
        ha_checksum crc = 0;

        crc = my_checksum(crc, (const uchar *)buf, event_len - BINLOG_CHECKSUM_LEN);
  ...
  if (likely(!error) && mi->using_gtid != Master_info::USE_GTID_NO && mi->events_queued_since_last_gtid > 0 &&
      ((mi->last_queued_gtid_standalone && (LOG_EVENT_IS_QUERY((Log_event_type)(uchar)buf[EVENT_TYPE_OFFSET]) ||
                                            (uchar)buf[EVENT_TYPE_OFFSET] == INCIDENT_EVENT)) ||
       (!mi->last_queued_gtid_standalone &&
        ((uchar)buf[EVENT_TYPE_OFFSET] == XID_EVENT || (uchar)buf[EVENT_TYPE_OFFSET] == XA_PREPARE_LOG_EVENT ||
         ((uchar)buf[EVENT_TYPE_OFFSET] ==
              QUERY_EVENT && /* QUERY_COMPRESSED_EVENT would never be commmit or rollback */
          Query_log_event::peek_is_commit_rollback(buf, event_len, checksum_alg))))))
  {
    ...
    */
    if (gtid_skip_enqueue)
    {
      ...
      rpl_slave_state_tostring_helper(&gtid_text, &mi->last_queued_gtid, &first);
      sql_print_error(
          "Slave IO thread received a terminal event from "
...
err:
  ...
  */
  if (unlikely(error) && error != ER_SLAVE_RELAY_LOG_WRITE_FAILURE)
    mi->report(ERROR_LEVEL, error, NULL, ER_DEFAULT(error), error_msg.ptr());
...
static int connect_to_master(THD *thd, MYSQL *mysql, Master_info *mi, bool reconnect, bool suppress_warnings)
{
  ...
  if (mi->ssl)
  {
    ...
    mysql_options(mysql, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, &mi->ssl_verify_server_cert);
    mysql_options(mysql, MYSQL_OPT_SSL_CRLPATH, mi->ssl_crlpath[0] ? mi->ssl_crlpath : 0);
    mysql_options(mysql, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, &mi->ssl_verify_server_cert);
  }
#endif
  ...
  */
  ...
  else
  {
    sql_print_information(
        ...
        default_charset_info->cs_name.str, default_client_charset_info->cs_name.str);
    mysql_options(mysql, MYSQL_SET_CHARSET_NAME, default_client_charset_info->cs_name.str);
  }
                    ...
                    : mysql_real_connect(mysql, mi->host, mi->user, mi->password, 0, mi->port, 0, client_flag) == 0))
  {
    ...
    if ((int)mysql_errno(mysql) != last_errno)
    {
      ...
      mi->report(ERROR_LEVEL, last_errno, NULL,
                 ...
                 (reconnect ? "reconnecting" : "connecting"), mi->user, mi->host, mi->port, mi->connect_retry,
                 master_retry_count, mysql_error(mysql));
    }
    /*
  ...
  if (!slave_was_killed)
  {
    ...
    else
    {
      ...
      general_log_print(thd, COM_CONNECT_OUT, "%s@%s:%d", mi->user, mi->host, mi->port);
    }
#ifdef SIGNAL_WITH_VIO_CLOSE
...
MYSQL *rpl_connect_master(MYSQL *mysql)
{
  ...
  */
  ...
  if (mi->user == NULL || mi->user[0] == 0 || io_slave_killed(mi) ||
      !mysql_real_connect(mysql, mi->host, mi->user, mi->password, 0, mi->port, 0, 0))
  {
    if (!io_slave_killed(mi))
      sql_print_error("rpl_connect_master: error connecting to master: %s (server_error: %d)", mysql_error(mysql),
                      mysql_errno(mysql));
  ...
  while (!sql_slave_killed(rgi))
  {
    ...
    */
...
#ifndef DBUG_OFF
    {
      ...
      DBUG_PRINT("info", ("my_b_tell(cur_log)=%llu rli->event_relay_log_pos=%llu", my_b_tell(cur_log),
                          rli->event_relay_log_pos));
      DBUG_ASSERT(my_b_tell(cur_log) >= BIN_LOG_HEADER_SIZE);
      DBUG_ASSERT(rli->mi->using_parallel() || my_b_tell(cur_log) == rli->event_relay_log_pos);
    }
#endif
    /*
      ...
      if (hot_log)
      {
        ...
        */
        ...
        if (rli->ign_master_log_name_end[0])
        {
          ...
          ev = new Rotate_log_event(rli->ign_master_log_name_end, 0, rli->ign_master_log_pos_end,
                                    Rotate_log_event::DUP_NAME);
          rli->ign_master_log_name_end[0] = 0;
          mysql_mutex_unlock(log_lock);
          if (unlikely(!ev))
        ...
        if (rli->ign_gtids.count() && !rli->is_in_group())
        {
          ...
          */
          ...
          ev = new Gtid_list_log_event(&rli->ign_gtids, Gtid_list_log_event::FLAG_IGN_GTIDS);
          rli->ign_gtids.reset();
          mysql_mutex_unlock(log_lock);
          if (unlikely(!ev))
          {
            errmsg =
                "Slave SQL thread failed to create a Gtid_list event "
                        ...
                        const void *param)
{
  ...
  static struct st_version_range_for_one_bug versions_for_all_bugs[] = {
      {24432, {5, 0, 24}, {5, 0, 38}}, {24432, {5, 1, 12}, {5, 1, 17}}, {33029, {5, 0, 0}, {5, 0, 58}},
      {33029, {5, 1, 0}, {5, 1, 12}},  {37426, {5, 1, 0}, {5, 1, 26}},
  };
  const Version &master_ver = rli->relay_log.description_event_for_exec->server_version_split;

  for (uint i = 0; i < sizeof(versions_for_all_bugs) / sizeof(*versions_for_all_bugs); i++)
  {
    const Version &introduced_in = versions_for_all_bugs[i].introduced_in;
    const Version &fixed_in = versions_for_all_bugs[i].fixed_in;
    if ((versions_for_all_bugs[i].bug_id == bug_id) && introduced_in <= master_ver && fixed_in > master_ver &&
        (pred == NULL || (*pred)(param)))
    {
      ...
      rli->report(ERROR_LEVEL, ER_UNKNOWN_ERROR, NULL,
                  ...
                  " restarted.",
                  rli->relay_log.description_event_for_exec->server_version, bug_id, fixed_in[0], fixed_in[1],
                  fixed_in[2]);
      return TRUE;
    }
  }
...
static bool get_row_event_stmt_end(const uchar *buf, const Format_description_log_event *fdle)
{
...
void Rows_event_tracker::update(const char *file_name, my_off_t pos, const uchar *buf,
...
/**
  ...
  @return true  when the Rows- event group integrity found compromised,
                false otherwise.
*/
bool Rows_event_tracker::check_and_report(const char *file_name, my_off_t pos)
{
  if (last_seen)
  {
    ...
    {
      sql_print_error(
          "Slave IO thread did not receive an expected "
          ...
          binlog_file_name, first_seen, binlog_file_name, last_seen, file_name, pos);
      return true;
    }
    reset();
  }
