...
{
  Format_description_log_event *fd_event = rli->relay_log.description_event_for_exec;
  ...
  */
  if (fd_event && fd_event->event_type_permutation)
  {
    ...
    DBUG_PRINT("info", ("converting event type %d to %d (%s)", type, new_type,
  ...
  switch (type)
  {
    ...
    case FORMAT_DESCRIPTION_EVENT:
      ...
      */
      if (!fd_event)
        if (!(rli->relay_log.description_event_for_exec = new Format_description_log_event(4)))
        {
...
int binlog_defragment(THD *thd)
{
  ...
  for (uint k = 0; k < 2; k++)
  {
    entry[k] = (user_var_entry *)my_hash_search(&thd->user_vars, (uchar *)name[k].str, name[k].length);
  ...
  for (uint k = 0; k < 2; k++)
  {
    memcpy(const_cast<char *>(thd->lex->comment.str) + gathered_length, entry[k]->value, entry[k]->length);
    gathered_length += entry[k]->length;
  }
  for (uint k = 0; k < 2; k++) update_hash(entry[k], true, NULL, 0, STRING_RESULT, &my_charset_bin, 0);
...
void mysql_client_binlog_statement(THD *thd)
{
  ...
  DBUG_PRINT("info", ("binlog base64: '%*s'", (int)(thd->lex->comment.length < 2048 ? thd->lex->comment.length : 2048),
                      thd->lex->comment.str));
  ...
  */
  ...
  if (!(buf = (uchar *)my_malloc(key_memory_binlog_statement_buffer, decoded_len, MYF(MY_WME))))
  {
    my_error(ER_OUTOFMEMORY, MYF(ME_FATAL), 1);
    goto end;
  }
  ...
  {
    char const *endptr = 0;
    int bytes_decoded = my_base64_decode(strptr, coded_len, buf, &endptr, MY_BASE64_DECODE_ALLOW_MULTIPLE_CHUNKS);
    ...
    */
    DBUG_PRINT("info", ("bytes_decoded: %d  strptr: %p  endptr: %p ('%c':%d)", bytes_decoded, strptr, endptr, *endptr,
                        *endptr));
#endif
    ...
    */
    ...
    DBUG_PRINT("info", ("binlog base64 decoded_len: %lu  bytes_decoded: %d", (ulong)decoded_len, bytes_decoded));
    ...
    for (uchar *bufptr = buf; bytes_decoded > 0;)
    {
      ...
      */
      ...
      if (bytes_decoded < EVENT_LEN_OFFSET + 4 ||
          (event_len = uint4korr(bufptr + EVENT_LEN_OFFSET)) > (uint)bytes_decoded)
      {
        my_error(ER_SYNTAX_ERROR, MYF(0));
        goto end;
      }
      DBUG_PRINT("info", ("event_len=%lu, bytes_decoded=%d", event_len, bytes_decoded));

      if (check_event_type(bufptr[EVENT_TYPE_OFFSET], rli))
        goto end;

      ev = Log_event::read_log_event(bufptr, event_len, &error, rli->relay_log.description_event_for_exec, 0);
      ...
      */
#if !defined(MYSQL_CLIENT) && defined(HAVE_REPLICATION)
      ulonglong save_skip_replication = thd->variables.option_bits & OPTION_SKIP_REPLICATION;
      ...
      thd->variables.option_bits = (thd->variables.option_bits & ~OPTION_SKIP_REPLICATION) | save_skip_replication;
#else
      err = 0;
#endif
      /*
