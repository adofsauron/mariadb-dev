...
static udf_func *add_udf(LEX_CSTRING *name, Item_result ret, const char *dl, Item_udftype typ);
static void del_udf(udf_func *udf);
static void *find_udf_dl(const char *dl);
static bool find_udf_everywhere(THD *thd, const LEX_CSTRING &name, TABLE *table);
...
extern "C" uchar *get_hash_key(const uchar *buff, size_t *length, my_bool not_used __attribute__((unused)))
{
...
static PSI_rwlock_info all_udf_rwlocks[] = {{&key_rwlock_THR_LOCK_udf, "THR_LOCK_udf", PSI_FLAG_GLOBAL}};

static PSI_memory_info all_udf_memory[] = {{&key_memory_udf_mem, "udf_mem", PSI_FLAG_GLOBAL}};
...
void udf_init()
{
  ...
  if (!new_thd || my_hash_init(key_memory_udf_mem, &udf_hash, system_charset_info, 32, 0, 0, get_hash_key, NULL, 0))
  {
  ...
  {
    sql_print_error(
        "Could not initialize init_read_record; udf's not "
        "loaded");
    goto end;
  }
  ...
  while (!(error = read_record_info.read_record()))
  {
    ...
    */
    if (!name.str || !dl_name || check_valid_path(dl_name, strlen(dl_name)) ||
        check_string_char_length(&name, 0, NAME_CHAR_LEN, system_charset_info, 1))
    {
      sql_print_error("Invalid row in mysql.func table for function '%.64s'", safe_str(name.str));
      continue;
    }
    ...
    if (dl == NULL)
    {
      ...
      if (!(dl = dlopen(dlpath, RTLD_NOW)))
      {
        ...
        sql_print_error(ER_THD(new_thd, ER_CANT_OPEN_LIBRARY), tmp->dl, errno, my_dlerror(dlpath));
...
static udf_func *add_udf(LEX_CSTRING *name, Item_result ret, const char *dl, Item_udftype type)
{
...
static bool find_udf_in_table(const LEX_CSTRING &exact_name, TABLE *table)
{
  ...
  return (!table->file->ha_index_read_idx_map(table->record[0], 0, (uchar *)table->field[0]->ptr, HA_WHOLE_KEY,
                                              HA_READ_KEY_EXACT));
}
...
{
  TABLE_LIST tables;
  tables.init_one_table(&MYSQL_SCHEMA_NAME, &MYSQL_FUNC_NAME, &MYSQL_FUNC_NAME, TL_WRITE);
...
int mysql_create_function(THD *thd, udf_func *udf)
{
  ...
  */
  ...
  if ((u_d = (udf_func *)my_hash_search(&udf_hash, (uchar *)udf->name.str, udf->name.length)))
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_UDF_EXISTS, ER_THD(thd, ER_UDF_EXISTS),
                          udf->name.str);
  ...
  if (!(dl = find_udf_dl(udf->dl)))
  {
    ...
    if (!(dl = dlopen(dlpath, RTLD_NOW)))
    {
      my_error(ER_CANT_OPEN_LIBRARY, MYF(0), udf->dl, errno, my_dlerror(dlpath));
...
enum drop_udf_result mysql_drop_function(THD *thd, const LEX_CSTRING *udf_name)
{
  ...
  if (!(udf = (udf_func *)my_hash_search(&udf_hash, (uchar *)udf_name->str, (uint)udf_name->length)))
  {
