...
extern "C" int wsrep_is_wsrep_xid(const void *xid_ptr)
{
  const XID *xid = static_cast<const XID *>(xid_ptr);
  return (
      xid->formatID == 1 && xid->bqual_length == 0 &&
...
const unsigned char *wsrep_xid_uuid(const xid_t *xid)
{
  ...
  if (wsrep_is_wsrep_xid(xid))
    return reinterpret_cast<const unsigned char *>(xid->data + WSREP_XID_UUID_OFFSET);
  else
...
wsrep::seqno wsrep_xid_seqno(const XID &xid) { return wsrep::seqno(wsrep_xid_seqno(&xid)); }
...
static my_bool set_SE_checkpoint(THD *unused, plugin_ref plugin, void *arg)
{
  ...
  if (hton->set_checkpoint)
  {
    ...
    WSREP_DEBUG("Set WSREPXid for InnoDB:  %s:%lld", uuid_str, (long long)wsrep_xid_seqno(xid));
...
{
  return plugin_foreach(NULL, set_SE_checkpoint, MYSQL_STORAGE_ENGINE_PLUGIN, &xid);
}
...
static my_bool get_SE_checkpoint(THD *unused, plugin_ref plugin, void *arg)
{
  ...
  if (hton->get_checkpoint)
  {
    ...
    WSREP_DEBUG("Read WSREPXid from InnoDB:  %s:%lld", uuid_str, (long long)wsrep_xid_seqno(xid));
  }
  return FALSE;
}
...
{
  return plugin_foreach(NULL, get_SE_checkpoint, MYSQL_STORAGE_ENGINE_PLUGIN, &xid);
}
...
void wsrep_sort_xid_array(XID *array, int len) { std::sort(array, array + len, Wsrep_xid_cmp()); }
