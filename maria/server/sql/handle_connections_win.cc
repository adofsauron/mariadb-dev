...
struct Listener
{
  ...
  */
  ...
  @param wait_handle - usually, event handle or INVALID_HANDLE_VALUE
                       @see wait_handle
  */
  Listener(HANDLE handle, HANDLE wait_handle) : m_handle(handle), m_overlapped() { m_overlapped.hEvent = wait_handle; }
  ...
  */
  HANDLE wait_handle() { return m_overlapped.hEvent; }
  ...
  */
  void completion_callback()
  {
    ...
    DWORD bytes;
    return completion_callback(GetOverlappedResult(wait_handle(), &m_overlapped, &bytes, FALSE));
  }

  /** Cancel an in-progress IO. Useful for threadpool-bound IO */
  void cancel() { CancelIoEx(m_handle, &m_overlapped); }
...
struct Socket_Listener : public Listener
{
  ...
  */
  static void CALLBACK tp_accept_completion_callback(PTP_CALLBACK_INSTANCE, PVOID context, PVOID, ULONG io_result,
                                                     ULONG_PTR, PTP_IO io)
  {
  ...
  */
  Socket_Listener(MYSQL_SOCKET listen_socket, PTP_CALLBACK_ENVIRON callback_environ)
      : Listener((HANDLE)listen_socket.fd, 0), m_client_socket(INVALID_SOCKET), m_listen_socket(listen_socket)
  {
    if (callback_environ)
    {
      /* Accept executed in threadpool. */
      m_tp_io = CreateThreadpoolIo(m_handle, tp_accept_completion_callback, this, callback_environ);
    }
    else
    {
  ...
  */
  ...
  {
  retry:
    m_client_socket = socket(m_listen_socket.address_family, SOCK_STREAM, IPPROTO_TCP);
    ...
    BOOL ret = my_AcceptEx((SOCKET)m_handle, m_client_socket, m_buffer, 0, sizeof(sockaddr_storage) + 16,
                           sizeof(sockaddr_storage) + 16, &bytes_received, &m_overlapped);
  ...
  void completion_callback(bool success)
  {
    ...
    my_GetAcceptExSockaddrs(m_buffer, 0, sizeof(sockaddr_storage) + 16, sizeof(sockaddr_storage) + 16, &local_addr,
                            &local_addr_len, &remote_addr, &remote_addr_len);

    s_client.m_psi = PSI_SOCKET_CALL(init_socket)(key_socket_client_connection, (const my_socket *)&m_listen_socket.fd,
    ...
    if (setsockopt(s_client.fd, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char *)&m_listen_socket.fd,
                   sizeof(m_listen_socket.fd)))
    {
      if (!abort_loop)
      {
  ...
  */
  static void init_winsock_extensions()
  {
    ...
    {
      if (WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, guids[i], sizeof(GUID), funcs[i], sizeof(void *), &bytes, 0,
                   0) == -1)
      {
...
#define EVERYONE_PIPE_ACCESS_MASK                                                                        \
  (FILE_READ_DATA | FILE_READ_EA | FILE_READ_ATTRIBUTES | READ_CONTROL | SYNCHRONIZE | FILE_WRITE_DATA | \
...
#define SECURITY_MAX_SID_STRING_CHARACTERS 187
  ...
  */
  ...
  if (!GetTokenInformation(token, TokenUser, &token_buffer, (DWORD)sizeof(token_buffer), &tmp))
    goto fail;

  CloseHandle(token);

  char *current_user_string_sid;
  if (!ConvertSidToStringSid(token_buffer.token_user.User.Sid, &current_user_string_sid))
    goto fail;

  snprintf(sddl_string, sizeof(sddl_string), SDDL_FMT, (unsigned int)EVERYONE_PIPE_ACCESS_MASK,
           current_user_string_sid);
  LocalFree(current_user_string_sid);

  if (ConvertStringSecurityDescriptorToSecurityDescriptor(sddl_string, SDDL_REVISION_1,
                                                          &pipe_security.lpSecurityDescriptor, 0))
    return;
...
struct Pipe_Listener : public Listener
{
  ...
  Pipe_Listener()
      : Listener(create_named_pipe(), CreateEvent(0, FALSE, FALSE, 0)), m_tp_env(get_threadpool_win_callback_environ())
  {
  }
  ...
  */
  static HANDLE create_named_pipe()
  {
    ...
    HANDLE pipe_handle =
        CreateNamedPipe(pipe_name, open_mode, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, PIPE_UNLIMITED_INSTANCES,
                        (int)global_system_variables.net_buffer_length, (int)global_system_variables.net_buffer_length,
                        NMPWAIT_USE_DEFAULT_WAIT, &pipe_security);
  ...
  /* Threadpool callback.*/
  static void CALLBACK tp_create_pipe_connection(PTP_CALLBACK_INSTANCE, void *Context)
  {
    tp_win_callback_prolog();
  ...
  static void cleanup() { LocalFree(pipe_security.lpSecurityDescriptor); }
};
...
void network_init_win()
{
  ...
  if (mysqld_unix_port[0] && !opt_bootstrap && opt_enable_named_pipe)
  {
    ...
    */
    for (int i = 0; i < NUM_PIPE_LISTENERS; i++) all_listeners.push_back(new Pipe_Listener());
  }
  ...
  for (uint i = 0; i < listen_sockets.elements(); i++)
  {
    ...
    SetFileCompletionNotificationModes((HANDLE)sock->fd, FILE_SKIP_SET_EVENT_ON_HANDLE);
    all_listeners.push_back(new Socket_Listener(*sock, get_threadpool_win_callback_environ()));
  }
...
void handle_connections_win()
{
  ...
  if (wait_events.size() > MAXIMUM_WAIT_OBJECTS)
  {
    sql_print_warning(
        ...
        "\"extra-port\".",
        static_cast<ulong>(wait_events.size()));
  ...
  for (;;)
  {
    ...
    DWORD idx = WaitForMultipleObjects((DWORD)wait_events.size(), wait_events.data(), FALSE, INFINITE);
