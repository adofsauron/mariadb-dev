...
{
  if (lex_string_eq(name, STRING_WITH_LEN("GLOBAL")) || lex_string_eq(name, STRING_WITH_LEN("LOCAL")) ||
...
*/
static inline bool test_if_sum_overflows_ull(ulonglong arg1, ulonglong arg2) { return ULONGLONG_MAX - arg1 < arg2; }
...
void Item_args::set_arguments(THD *thd, List<Item> &list)
{
  ...
  Item *item;
  for (arg_count = 0; (item = li++);) args[arg_count++] = item;
}

Item_args::Item_args(THD *thd, const Item_args *other) : arg_count(other->arg_count)
{
  if (arg_count <= 2)
  {
    args = tmp_arg;
...
void Item_func::sync_with_sum_func_and_with_field(List<Item> &list)
{
  List_iterator_fast<Item> li(list);
  Item *item;
  while ((item = li++)) with_flags |= item->with_flags;
}
...
bool Item_func::check_argument_types_like_args0() const
{
  ...
  for (uint i = 1; i < arg_count; i++)
  {
    ...
    {
      my_error(ER_ILLEGAL_PARAMETER_DATA_TYPES2_FOR_OPERATION, MYF(0), args[0]->type_handler()->name().ptr(),
...
bool Item_func::check_argument_types_or_binary(const Type_handler *handler, uint start, uint end) const
{
...
bool Item_func::check_argument_types_traditional_scalar(uint start, uint end) const
{
  for (uint i = start; i < end; i++)
  {
    DBUG_ASSERT(i < arg_count);
...
bool Item_func::check_argument_types_can_return_int(uint start, uint end) const
{
  for (uint i = start; i < end; i++)
  {
    DBUG_ASSERT(i < arg_count);
...
bool Item_func::check_argument_types_can_return_real(uint start, uint end) const
{
  for (uint i = start; i < end; i++)
  {
    DBUG_ASSERT(i < arg_count);
...
bool Item_func::check_argument_types_can_return_text(uint start, uint end) const
{
  for (uint i = start; i < end; i++)
  {
    DBUG_ASSERT(i < arg_count);
...
bool Item_func::check_argument_types_can_return_str(uint start, uint end) const
{
  for (uint i = start; i < end; i++)
  {
    DBUG_ASSERT(i < arg_count);
...
bool Item_func::check_argument_types_can_return_date(uint start, uint end) const
{
  for (uint i = start; i < end; i++)
  {
    DBUG_ASSERT(i < arg_count);
...
bool Item_func::check_argument_types_can_return_time(uint start, uint end) const
{
  for (uint i = start; i < end; i++)
  {
    DBUG_ASSERT(i < arg_count);
...
void Item_func::quick_fix_field()
{
  Item **arg, **arg_end;
  if (arg_count)
  {
...
bool Item_func::eval_not_null_tables(void *opt_arg)
{
  Item **arg, **arg_end;
  not_null_tables_cache = 0;
  if (arg_count)
...
bool Item_func::find_not_null_fields(table_map allowed)
{
...
void Item_func::fix_after_pullout(st_select_lex *new_parent, Item **ref, bool merge)
{
  Item **arg, **arg_end;
...
Item *Item_func::compile(THD *thd, Item_analyzer analyzer, uchar **arg_p, Item_transformer transformer, uchar *arg_t)
{
  ...
  if (*arg_p && arg_count)
  {
    ...
    for (arg = args, arg_end = args + arg_count; arg != arg_end; arg++)
    {
      ...
      */
      uchar *arg_v = *arg_p;
      Item *new_item = (*arg)->compile(thd, analyzer, &arg_v, transformer, arg_t);
...
void Item_args::propagate_equal_fields(THD *thd, const Item::Context &ctx, COND_EQUAL *cond)
{
  uint i;
  for (i = 0; i < arg_count; i++) args[i]->propagate_equal_fields_and_change_item_tree(thd, ctx, cond, &args[i]);
}

Sql_mode_dependency Item_args::value_depends_on_sql_mode_bit_or() const
{
  Sql_mode_dependency res;
  for (uint i = 0; i < arg_count; i++) res |= args[i]->value_depends_on_sql_mode();
  return res;
}
...
void Item_func::split_sum_func(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &fields, uint flags)
{
  Item **arg, **arg_end;
  DBUG_ENTER("Item_func::split_sum_func");

  for (arg = args, arg_end = args + arg_count; arg != arg_end; arg++)
    (*arg)->split_sum_func2(thd, ref_pointer_array, fields, arg, flags | SPLIT_SUM_SKIP_REGISTERED);
  DBUG_VOID_RETURN;
}
...
bool Item_func::eq(const Item *item, bool binary_cmp) const
{
  ...
  */
  ...
  if ((func_type = functype()) != item_func->functype() || arg_count != item_func->arg_count ||
      (func_type != Item_func::FUNC_SP && func_name() != item_func->func_name()) ||
      (func_type == Item_func::FUNC_SP && my_strcasecmp(system_charset_info, func_name(), item_func->func_name())))
    return 0;
...
{
  bool rc = Item_hybrid_func::type_handler()->Item_hybrid_func_fix_attributes(current_thd, func_name_cstring(), this,
                                                                              this, items, nitems);
...
bool Item_num_op::fix_type_handler(const Type_aggregator *aggregator)
{
  ...
  if (!aggregate_for_num_op(aggregator, h0, h1))
    return false;
  my_error(ER_ILLEGAL_PARAMETER_DATA_TYPES2_FOR_OPERATION, MYF(0), h0->name().ptr(), h1->name().ptr(), func_name());
  return true;
}
...
my_decimal *Item_func_hybrid_field_type::val_decimal_from_int_op(my_decimal *dec)
{
  longlong result = int_op();
  if (null_value)
    return NULL;
...
my_decimal *Item_func_hybrid_field_type::val_decimal_from_real_op(my_decimal *dec)
{
...
{
  MYSQL_TIME ltime;
  if (date_op_with_null_check(current_thd, &ltime) || (null_value = str->alloc(MAX_DATE_STRING_REP_LENGTH)))
    return (String *)0;
...
my_decimal *Item_func_hybrid_field_type::val_decimal_from_date_op(my_decimal *dec)
{
  MYSQL_TIME ltime;
  if (date_op_with_null_check(current_thd, &ltime))
  {
...
{
  MYSQL_TIME ltime;
  if (time_op_with_null_check(current_thd, &ltime) || (null_value = my_TIME_to_str(&ltime, str, decimals)))
    return NULL;
  return str;
}
...
{
  MYSQL_TIME ltime;
  return time_op_with_null_check(current_thd, &ltime) ? 0 : TIME_to_double(&ltime);
}

longlong Item_func_hybrid_field_type::val_int_from_time_op()
{
  MYSQL_TIME ltime;
  return time_op_with_null_check(current_thd, &ltime) ? 0 : TIME_to_ulonglong(&ltime);
}

my_decimal *Item_func_hybrid_field_type::val_decimal_from_time_op(my_decimal *dec)
{
  MYSQL_TIME ltime;
...
my_decimal *Item_func_hybrid_field_type::val_decimal_from_str_op(my_decimal *decimal_value)
{
  String *res = str_op_with_null_check(&str_value);
...
err:
  THD *thd = current_thd;
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE,
                      ER_THD(thd, ER_WARN_DATA_OUT_OF_RANGE), name.str, thd->get_stmt_da()->current_row_for_warning());
  return dec;
}
...
void Item_decimal_typecast::print(String *str, enum_query_type query_type)
{
  ...
  uint precision = my_decimal_length_to_precision(max_length, decimals, unsigned_flag);
  str->append(STRING_WITH_LEN("cast("));
  args[0]->print(str, query_type);
...
double Item_real_typecast::val_real_with_truncate(double max_value)
{
  ...
  if (unlikely((error = truncate_double(&tmp, max_length, decimals, false /*unsigned_flag*/, max_value))))
  {
    /*
    ...
    */
    THD *thd = current_thd;
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_WARN_DATA_OUT_OF_RANGE,
                        ER_THD(thd, ER_WARN_DATA_OUT_OF_RANGE), name.str, (ulong)1);
    if (error < 0)
    {
...
my_decimal *Item_func_plus::decimal_op(my_decimal *decimal_value)
{
  ...
  if (!(null_value =
            (val.has_null() || check_decimal_overflow(my_decimal_add(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, decimal_value,
                                                                     val.m_a.ptr(), val.m_b.ptr())) > 3)))
    return decimal_value;
  return 0;
}
...
my_decimal *Item_func_minus::decimal_op(my_decimal *decimal_value)
{
  VDec2_lazy val(args[0], args[1]);
  if (!(null_value =
            (val.has_null() || check_decimal_overflow(my_decimal_sub(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, decimal_value,
                                                                     val.m_a.ptr(), val.m_b.ptr())) > 3)))
    return decimal_value;
  return 0;
}
...
my_decimal *Item_func_mul::decimal_op(my_decimal *decimal_value)
{
  VDec2_lazy val(args[0], args[1]);
  if (!(null_value =
            (val.has_null() || check_decimal_overflow(my_decimal_mul(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, decimal_value,
                                                                     val.m_a.ptr(), val.m_b.ptr())) > 3)))
    return decimal_value;
  return 0;
}
...
{
  decimals = MY_MIN(args[0]->decimal_scale() + args[1]->decimal_scale(), DECIMAL_MAX_SCALE);
...
my_decimal *Item_func_div::decimal_op(my_decimal *decimal_value)
{
  ...
  if ((err = check_decimal_overflow(my_decimal_div(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW & ~E_DEC_DIV_ZERO, decimal_value,
                                                   val.m_a.ptr(), val.m_b.ptr(), prec_increment))) > 3)
  {
...
void Item_func_div::result_precision()
{
  ...
  */
  uint precision = MY_MIN(args[0]->decimal_precision() + args[1]->divisor_precision_increment() + prec_increment,
...
longlong Item_func_int_div::val_int()
{
  ...
  */
  if (args[0]->result_type() != INT_RESULT || args[1]->result_type() != INT_RESULT)
  {
    ...
    int err;
    my_decimal tmp;
    if ((err = my_decimal_div(E_DEC_FATAL_ERROR & ~E_DEC_DIV_ZERO, &tmp, val.m_a.ptr(), val.m_b.ptr(), 0)) > 3)
    {
      if (err == E_DEC_DIV_ZERO)
        signal_divide_by_null();
      return 0;
    ...
    longlong res;
    if (my_decimal2int(E_DEC_FATAL_ERROR, &truncated, unsigned_flag, &res) & E_DEC_OVERFLOW)
...
my_decimal *Item_func_mod::decimal_op(my_decimal *decimal_value)
{
  ...
  if ((null_value = val.has_null()))
    return 0;
  switch (my_decimal_mod(E_DEC_FATAL_ERROR & ~E_DEC_DIV_ZERO, decimal_value, val.m_a.ptr(), val.m_b.ptr()))
  {
...
longlong Item_func_neg::int_op()
{
  ...
  if ((null_value = args[0]->null_value))
    return 0;
  if (args[0]->unsigned_flag && (ulonglong)value > (ulonglong)LONGLONG_MAX + 1)
    return raise_integer_overflow();
  ...
  */
  if (args[0]->const_item())
  {
    ...
    if ((ulonglong)val >= (ulonglong)LONGLONG_MIN &&
        ((ulonglong)val != (ulonglong)LONGLONG_MIN || !args[0]->is_of_type(CONST_ITEM, INT_RESULT)))
    {
      /*
...
bool Item_func_neg::fix_length_and_dec(THD *thd)
{
  ...
  DBUG_PRINT("info", ("name %s", func_name()));
  if (args[0]->cast_to_int_type_handler()->Item_func_neg_fix_length_and_dec(this))
    DBUG_RETURN(TRUE);
  DBUG_PRINT("info", ("Type: %s", type_handler()->name().ptr()));
  DBUG_RETURN(FALSE);
}
...
bool Item_func_abs::fix_length_and_dec(THD *thd)
{
  ...
  DBUG_PRINT("info", ("name %s", func_name()));
  if (args[0]->cast_to_int_type_handler()->Item_func_abs_fix_length_and_dec(this))
    DBUG_RETURN(TRUE);
  DBUG_PRINT("info", ("Type: %s", type_handler()->name().ptr()));
  DBUG_RETURN(FALSE);
}
...
class Func_handler_shift_left_int_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
    DBUG_ASSERT(item->fixed());
    return item->arguments()[0]->to_longlong_null() << item->arguments()[1]->to_longlong_null();
  }
};

class Func_handler_shift_left_decimal_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
    DBUG_ASSERT(item->fixed());
    return VDec(item->arguments()[0]).to_xlonglong_null() << item->arguments()[1]->to_longlong_null();
  }
};
...
class Func_handler_shift_right_int_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
...
class Func_handler_shift_right_decimal_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
...
class Func_handler_bit_neg_int_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
...
class Func_handler_bit_neg_decimal_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
...
void Item_func_int_val::fix_length_and_dec_int_or_decimal()
{
  ...
  */
  ...
  uint length_increase =
      args[0]->decimals > 0 && (mode == CEILING || (mode == FLOOR && !args[0]->unsigned_flag)) ? 1 : 0;
  ...
  */
  ...
  {
    unsigned_flag = args[0]->unsigned_flag;
    fix_char_length(my_decimal_precision_to_length_no_truncation(precision, 0, unsigned_flag));
    set_handler(&type_handler_newdecimal);
  }
  else
  {
...
{
  VDec value(args[0]);
  if (!(null_value = (value.is_null() || value.round_to(decimal_value, 0, CEILING) > 1)))
    return decimal_value;
  return 0;
}
...
longlong Item_func_floor::int_op()
{
  switch (args[0]->result_type())
  {
    ...
    case DECIMAL_RESULT:
    {
      ...
      return (!(dec = Item_func_floor::decimal_op(&dec_buf))) ? 0 : dec->to_longlong(unsigned_flag);
    }
    default:
      break;
  }
...
{
  VDec value(args[0]);
  if (!(null_value = (value.is_null() || value.round_to(decimal_value, 0, FLOOR) > 1)))
    return decimal_value;
  return 0;
}
...
void Item_func_round::fix_length_and_dec_decimal(uint decimals_to_set)
{
  ...
  int precision = args[0]->decimal_precision() + length_increase - decimals_delta;
...
void Item_func_round::fix_arg_double()
{
  ...
  {
    Longlong_hybrid dec = args[1]->to_longlong_hybrid();
    fix_length_and_dec_double(args[1]->null_value ? NOT_FIXED_DEC : dec.to_uint(NOT_FIXED_DEC));
  }
  else
    fix_length_and_dec_double(args[0]->decimals);
}

void Item_func_round::fix_arg_temporal(const Type_handler *h, uint int_part_length)
{
  set_handler(h);
  if (args[1]->can_eval_in_optimize())
  {
    Longlong_hybrid_null dec = args[1]->to_longlong_hybrid_null();
    fix_attributes_temporal(int_part_length, dec.is_null() ? args[0]->decimals : dec.to_uint(TIME_SECOND_PART_DIGITS));
  }
  else
    fix_attributes_temporal(int_part_length, args[0]->decimals);
}

void Item_func_round::fix_arg_time() { fix_arg_temporal(&type_handler_time2, MIN_TIME_WIDTH); }
...
/**
  ...
  @param [IN] preferred_attrs - The preferred data type attributes for
                                simple cases.
*/
void Item_func_round::fix_arg_int(const Type_handler *preferred, const Type_std_attributes *preferred_attrs,
...
longlong Item_func_round::int_op()
{
  ...
  if (truncate)
    value = (unsigned_flag) ? ((ulonglong)value / tmp) * tmp : (value / tmp) * tmp;
  else
    value = (unsigned_flag || value >= 0) ? my_unsigned_round((ulonglong)value, tmp)
...
my_decimal *Item_func_round::decimal_op(my_decimal *decimal_value)
{
  ...
  if (!(null_value = (value.is_null() || args[1]->null_value ||
                      value.round_to(decimal_value, (int)dec, truncate ? TRUNCATE : HALF_UP) > 1)))
    return decimal_value;
  return 0;
}
...
{
  DBUG_ASSERT(args[0]->type_handler()->mysql_timestamp_type() == MYSQL_TIMESTAMP_TIME);
  Time::Options_for_round opt(truncate ? TIME_FRAC_TRUNCATE : TIME_FRAC_ROUND);
  Longlong_hybrid_null dec = args[1]->to_longlong_hybrid_null();
  Time *tm = new (to) Time(thd, args[0], opt, dec.to_uint(TIME_SECOND_PART_DIGITS));
...
{
  DBUG_ASSERT(args[0]->type_handler()->mysql_timestamp_type() == MYSQL_TIMESTAMP_DATETIME);
  Datetime::Options opt(thd, truncate ? TIME_FRAC_TRUNCATE : TIME_FRAC_ROUND);
  Longlong_hybrid_null dec = args[1]->to_longlong_hybrid_null();
  Datetime *tm = new (to) Datetime(thd, args[0], opt, dec.to_uint(TIME_SECOND_PART_DIGITS));
...
void Item_func_rand::seed_random(Item *arg)
{
  ...
  */
  ...
  my_rnd_init(rand, (uint32)(tmp * 0x10001L + 55555555L), (uint32)(tmp * 0x10000001L));
}
...
bool Item_func_rand::fix_fields(THD *thd, Item **ref)
{
  ...
  {  // Only use argument once in query
    ...
    */
    if (!rand && !(rand = (struct my_rnd_struct *)thd->stmt_arena->alloc(sizeof(*rand))))
      return TRUE;
  }
  else
  {
...
{
  bool rc = Item_func_min_max::type_handler()->Item_func_min_max_fix_attributes(current_thd, this, items, nitems);
  DBUG_ASSERT(!rc || current_thd->is_error());
  return rc;
}
...
bool Item_func_min_max::get_date_native(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  if (!(fuzzydate & TIME_TIME_ONLY) &&
      unlikely((null_value = check_date_with_warn(thd, ltime, fuzzydate, MYSQL_TIMESTAMP_ERROR))))
    return true;
...
bool Item_func_min_max::val_native(THD *thd, Native *native)
{
  ...
  for (uint i = 0; i < arg_count; i++)
  {
    if (val_native_with_conversion_from_item(thd, args[i], i == 0 ? native : &cur, handler))
      return true;
    if (i > 0)
    {
...
longlong Item_func_locate::val_int()
{
  ...
  if (!cmp_collation.collation->instr(a->ptr() + start, (uint)(a->length() - start), b->ptr(), b->length(), &match, 1))
    return 0;
      ...
      if ((symbol_len = cs->mb_wc(&wc, (uchar *)str_end, (uchar *)real_end)) > 0)
      {
        ...
        if (is_separator || is_last_item)
        {
          ...
          if (is_last_item && !is_separator)
            str_end = substr_end;
          if (!cs->strnncoll(str_begin, (uint)(str_end - str_begin), find_str, find_str_len))
...
class Func_handler_bit_count_int_to_slong : public Item_handled_func::Handler_slong2
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
...
class Func_handler_bit_count_decimal_to_slong : public Item_handled_func::Handler_slong2
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
...
bool Item_func_bit_count::fix_length_and_dec(THD *thd)
{
  ...
  set_func_handler(args[0]->cmp_type() == INT_RESULT ? (const Handler *)&ha_int_to_slong
...
bool udf_handler::fix_fields(THD *thd, Item_func_or_sum *func, uint arg_count, Item **arguments)
{
  uchar buff[STACK_BUFF_ALLOC];  // Max argument in function
  ...
  {
    if (!(f_args.arg_type = (Item_result *)thd->alloc(f_args.arg_count * sizeof(Item_result))))
    ...
    if (!buffers ||
        !multi_alloc_root(thd->mem_root, &f_args.args, arg_count * sizeof(char *), &f_args.lengths,
                          arg_count * sizeof(long), &f_args.maybe_null, arg_count * sizeof(char), &num_buffer,
                          arg_count * sizeof(double), &f_args.attributes, arg_count * sizeof(char *),
                          &f_args.attribute_lengths, arg_count * sizeof(long), NullS))
      goto err_exit;
  }
  ...
  initid.ptr = 0;
  for (uint i1 = 0; i1 < arg_count; i1++) buffers[i1].set_thread_specific();
  ...
  if (unlikely(error))
  {
    my_error(ER_CANT_INITIALIZE_UDF, MYF(0), u_d->name.str, ER_THD(thd, ER_UNKNOWN_ERROR));
    DBUG_RETURN(TRUE);
  }
  DBUG_RETURN(FALSE);
}
...
longlong Item_master_pos_wait::val_int()
{
  ...
  if (arg_count >= 4)
  {
    ...
    {
      my_error(ER_WRONG_ARGUMENTS, MYF(ME_WARNING), "MASTER_CONNECTION_NAME");
      goto err;
    }
  }
  else
  ...
  mi->release();
#endif
  DBUG_PRINT("exit", ("event_count: %d  null_value: %d", event_count, (int)null_value));
...
uchar *ull_get_key(const uchar *ptr, size_t *length, my_bool not_used __attribute__((unused)))
{
...
bool Lock_wait_timeout_handler::handle_condition(THD *thd, uint sql_errno, const char * /* sqlstate */,
                                                 Sql_condition::enum_warning_level * /* level */, const char *message,
                                                 Sql_condition ** /* cond_hdl */)
{
...
longlong Item_func_get_lock::val_int()
{
  ...
  */
  ...
  if (args[1]->null_value || (!args[1]->unsigned_flag && ((longlong)timeout < 0)))
  {
    char buf[22];
    if (args[1]->null_value)
      strmov(buf, "NULL");
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_VALUE_FOR_TYPE, ER(ER_WRONG_VALUE_FOR_TYPE),
  ...
  MDL_REQUEST_INIT(&ull_request, MDL_key::USER_LOCK, res->c_ptr_safe(), "", MDL_SHARED_NO_WRITE, MDL_EXPLICIT);
  MDL_key *ull_key = &ull_request.key;

  if ((ull = (User_level_lock *)my_hash_search(&thd->ull_hash, ull_key->ptr(), ull_key->length())))
  {
    /* Recursive lock */
    ull->refs++;
    null_value = 0;
  ...
  ull = (User_level_lock *)my_malloc(key_memory_User_level_lock, sizeof(User_level_lock),
...
longlong Item_func_release_lock::val_int()
{
  ...
  if (!my_hash_inited(&thd->ull_hash) ||
      !(ull = (User_level_lock *)my_hash_search(&thd->ull_hash, ull_key.ptr(), ull_key.length())))
  {
...
longlong Item_func_last_insert_id::val_int()
{
  ...
  return static_cast<longlong>(thd->read_first_successful_insert_id_in_prev_stmt());
}
...
longlong Item_func_benchmark::val_int()
{
  ...
  if (args[0]->null_value || (!args[0]->unsigned_flag && (((longlong)loop_count) < 0)))
  {
    if (!args[0]->null_value)
    {
      char buff[22];
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_VALUE_FOR_TYPE,
                          ER_THD(thd, ER_WRONG_VALUE_FOR_TYPE), "count", buff, "benchmark");
    }
...
longlong Item_func_sleep::val_int()
{
  ...
  */
  ...
  DBUG_EXECUTE_IF("sleep_inject_query_done_debug_sync",
                  { debug_sync_set_action(thd, STRING_WITH_LEN("dispatch_command_end SIGNAL query_done")); };);
#endif
...
user_var_entry *get_variable(HASH *hash, LEX_CSTRING *name, bool create_if_not_exists)
{
  ...
  if (!(entry = (user_var_entry *)my_hash_search(hash, (uchar *)name->str, name->length)) && create_if_not_exists)
  {
    ...
    if (!(entry = (user_var_entry *)my_malloc(key_memory_user_var_entry, size,
                                              MYF(MY_WME | ME_FATAL | MY_THREAD_SPECIFIC))))
      return 0;
...
bool Item_func_set_user_var::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  ...
  if (!m_var_entry->charset() || !null_item)
    m_var_entry->set_charset(args[0]->collation.derivation == DERIVATION_NUMERIC ? &my_charset_numeric
                                                                                 : args[0]->collation.collation);
  collation.set(m_var_entry->charset(),
                args[0]->collation.derivation == DERIVATION_NUMERIC ? DERIVATION_NUMERIC : DERIVATION_IMPLICIT);
  switch (args[0]->result_type())
  {
    case STRING_RESULT:
    case TIME_RESULT:
      set_handler(type_handler_long_blob.type_handler_adjusted_to_max_octet_length(max_length, collation.collation));
      break;
    case REAL_RESULT:
      set_handler(&type_handler_double);
      break;
    case INT_RESULT:
      set_handler(Type_handler::type_handler_long_or_longlong(max_char_length(), unsigned_flag));
      break;
    case DECIMAL_RESULT:
      set_handler(&type_handler_newdecimal);
      break;
  ...
  if (thd->lex->current_select)
  {
    ...
    */
    ...
    TABLE_LIST *derived;
    for (derived = unit->derived; derived; derived = unit->derived)
    {
...
bool Item_func_set_user_var::fix_length_and_dec(THD *thd)
{
  ...
  else
  {
    ...
    fix_length_and_charset(args[0]->max_char_length(), args[0]->collation.collation);
  }
  unsigned_flag = args[0]->unsigned_flag;
  return FALSE;
}
...
bool Item_func_set_user_var::register_field_in_read_map(void *arg)
{
  if (result_field)
  {
    ...
    if (result_field->vcol_info)
      return result_field->vcol_info->expr->walk(&Item::register_field_in_read_map, 1, arg);
  }
  return 0;
}
  ...
  else
  {
    ...
    else
    {
      ...
      if (entry->length != length)
      {
        ...
        if (entry->value == pos)
          entry->value = 0;
        entry->value = (char *)my_realloc(key_memory_user_var_entry_value, entry->value, length,
                                          MYF(MY_ALLOW_ZERO_PTR | MY_WME | ME_FATAL | MY_THREAD_SPECIFIC));
        if (!entry->value)
          return 1;
      }
    }
...
bool Item_func_set_user_var::update_hash(void *ptr, size_t length, Item_result res_type, CHARSET_INFO *cs,
                                         bool unsigned_arg)
{
  /*
  ...
  */
  ...
  if (::update_hash(m_var_entry, null_value, ptr, length, res_type, cs, unsigned_arg))
  {
    null_value = 1;
    return 1;
  }
...
String *user_var_entry::val_str(bool *null_value, String *str, uint decimals) const
{
  if ((*null_value = (value == 0)))
    return (String *)0;
...
bool Item_func_set_user_var::check(bool use_result_field)
{
  ...
  switch (result_type())
  {
    case REAL_RESULT:
    {
      save_result.vreal = use_result_field ? result_field->val_real() : args[0]->val_real();
      break;
    }
    case INT_RESULT:
    {
      save_result.vint = use_result_field ? result_field->val_int() : args[0]->val_int();
      unsigned_flag = (use_result_field ? ((Field_num *)result_field)->unsigned_flag : args[0]->unsigned_flag);
      break;
    }
    case STRING_RESULT:
    {
      save_result.vstr = use_result_field ? result_field->val_str(&value) : args[0]->val_str(&value);
      break;
    }
    case DECIMAL_RESULT:
    {
      save_result.vdec =
          use_result_field ? result_field->val_decimal(&decimal_buff) : args[0]->val_decimal(&decimal_buff);
      break;
    }
    case ROW_RESULT:
    case TIME_RESULT:
...
bool Item_func_set_user_var::update()
{
  ...
  switch (result_type())
  {
    case REAL_RESULT:
    {
      res = update_hash((void *)&save_result.vreal, sizeof(save_result.vreal), REAL_RESULT, &my_charset_numeric, 0);
      break;
    }
    case INT_RESULT:
    {
      res = update_hash((void *)&save_result.vint, sizeof(save_result.vint), INT_RESULT, &my_charset_numeric,
                        unsigned_flag);
      break;
    }
    case STRING_RESULT:
    ...
    case DECIMAL_RESULT:
    {
      if (!save_result.vdec)  // Null value
        ...
        res = update_hash((void *)save_result.vdec, sizeof(my_decimal), DECIMAL_RESULT, &my_charset_numeric, 0);
      break;
    }
    case ROW_RESULT:
    case TIME_RESULT:
...
void Item_func_set_user_var::print_as_stmt(String *str, enum_query_type query_type)
{
...
int Item_func_set_user_var::save_in_field(Field *field, bool no_conversions, bool can_use_result_field)
{
  bool use_result_field = (!can_use_result_field ? 0 : (result_field && result_field != field));
  int error;
...
static int get_var_with_binlog(THD *thd, enum_sql_command sql_command, LEX_CSTRING *name, user_var_entry **out_entry)
{
  ...
  */
  ...
  if (!var_entry)
  {
    ...
    */
    ...
    thd->lex = &lex_tmp;
    lex_start(thd);
    tmp_var_list.push_back(new (thd->mem_root) set_var_user(new (thd->mem_root) Item_func_set_user_var(
                               thd, name, new (thd->mem_root) Item_null(thd))),
                           thd->mem_root);
    /* Create the variable if the above allocations succeeded */
    if (unlikely(thd->is_fatal_error) || unlikely(sql_set_variables(thd, &tmp_var_list, false)))
    {
      thd->lex = sav_lex;
      goto err;
    }
  ...
  */
  ...
  if (unlikely(!(user_var_event = (BINLOG_USER_VAR_EVENT *)alloc_root(thd->user_var_events_alloc, size))))
    goto err;
...
{
  return (!m_var_entry || current_thd->query_id != m_var_entry->update_query_id);
}
...
bool Item_func_get_user_var::set_value(THD *thd, sp_rcontext * /*ctx*/, Item **it)
{
...
bool Item_user_var_as_out_param::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  entry->set_charset(thd->lex->exchange->cs ? thd->lex->exchange->cs : thd->variables.collation_database);
...
{
  ::update_hash(entry, FALSE, (void *)str, length, STRING_RESULT, cs, 0 /* unsigned_arg */);
}
...
bool Item_user_var_as_out_param::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  DBUG_ASSERT(0);
  return true;
}

void Item_user_var_as_out_param::load_data_print_for_log_event(THD *thd, String *str) const
{
  str->append('@');
  append_identifier(thd, str, &org_name);
}

Item_func_get_system_var::Item_func_get_system_var(THD *thd, sys_var *var_arg, enum_var_type var_type_arg,
...
bool Item_func_get_system_var::is_written_to_binlog() { return var->is_written_to_binlog(var_type); }
...
bool Item_func_get_system_var::fix_length_and_dec(THD *thd)
{
  ...
  if (var->check_type(var_type))
  {
    ...
    {
      my_error(ER_INCORRECT_GLOBAL_LOCAL_VAR, MYF(0), var->name.str, var_type == OPT_GLOBAL ? "SESSION" : "GLOBAL");
      return TRUE;
    }
  ...
  switch (var->show_type())
  {
    ...
    case SHOW_CHAR_PTR:
      ...
      cptr = var->show_type() == SHOW_CHAR
                 ? reinterpret_cast<const char *>(var->value_ptr(thd, var_type, &component))
                 : *reinterpret_cast<const char *const *>(var->value_ptr(thd, var_type, &component));
      if (cptr)
        max_length = (uint32)system_charset_info->numchars(cptr, cptr + strlen(cptr));
    ...
    {
      mysql_mutex_lock(&LOCK_global_system_variables);
      const LEX_STRING *ls = reinterpret_cast<const LEX_STRING *>(var->value_ptr(current_thd, var_type, &component));
      max_length = (uint32)system_charset_info->numchars(ls->str, ls->str + ls->length);
      mysql_mutex_unlock(&LOCK_global_system_variables);
      collation.set(system_charset_info, DERIVATION_SYSCONST);
      max_length *= system_charset_info->mbmaxlen;
      decimals = NOT_FIXED_DEC;
...
bool Item_func_match::init_search(THD *thd, bool no_order)
{
  ...
  {
    List<Item> fields;
    fields.push_back(new (thd->mem_root) Item_string(thd, " ", 1, cmp_collation.collation), thd->mem_root);
  ...
  {
    uint dummy_errors;
    if (search_value.copy(ft_tmp->ptr(), ft_tmp->length(), ft_tmp->charset(), cmp_collation.collation, &dummy_errors))
      DBUG_RETURN(1);
    ft_tmp = &search_value;
  }
...
bool Item_func_match::fix_fields(THD *thd, Item **ref)
{
  ...
  for (uint i = 1; i < arg_count; i++)
  {
    ...
    */
    if (!thd->stmt_arena->is_stmt_execute() && item->type() != Item::FIELD_ITEM)
    {
...
bool Item_func_match::fix_index()
{
  ...
  */
  ...
  for (keynr = 0; keynr <= mkeys; keynr++)
  {
    ...
    if (max_cnt < arg_count - 1 || max_cnt < table->key_info[ft_to_key[keynr]].user_defined_key_parts)
      continue;
...
double Item_func_match::val_real()
{
  ...
  if (key == NO_SUCH_KEY)
  {
    ...
    if ((null_value = (a == 0)) || !a->length())
      DBUG_RETURN(0);
    DBUG_RETURN(ft_handler->please->find_relevance(ft_handler, (uchar *)a->ptr(), a->length()));
  }
...
class Func_handler_bit_xor_int_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
...
class Func_handler_bit_xor_dec_to_ulonglong : public Item_handled_func::Handler_ulonglong
{
 public:
  Longlong_null to_longlong_null(Item_handled_func *item) const
  {
    DBUG_ASSERT(item->fixed());
    return VDec(item->arguments()[0]).to_xlonglong_null() ^ VDec(item->arguments()[1]).to_xlonglong_null();
  }
};
...
Item *get_system_var(THD *thd, enum_var_type var_type, const LEX_CSTRING *name, const LEX_CSTRING *component)
{
  ...
  return new (thd->mem_root) Item_func_get_system_var(thd, var, var_type, &component_name, NULL, 0);
}
...
Item_func_sp::Item_func_sp(THD *thd, Name_resolution_context *context_arg, sp_name *name, const Sp_handler *sph)
    : Item_func(thd), Item_sp(thd, context_arg, name), m_handler(sph)
{
  set_maybe_null();
}

Item_func_sp::Item_func_sp(THD *thd, Name_resolution_context *context_arg, sp_name *name_arg, const Sp_handler *sph,
                           List<Item> &list)
...
{
  return Item_sp::func_name_cstring(current_thd, m_handler == &sp_handler_package_function);
}
...
bool Item_func_sp::is_expensive() { return !m_sp->detistic() || current_thd->locked_tables_mode < LTM_LOCK_TABLES; }
...
void Item_func_sp::make_send_field(THD *thd, Send_field *tmp_field)
{
...
longlong Item_func_sqlcode::val_int()
{
  DBUG_ASSERT(fixed());
  DBUG_ASSERT(!null_value);
  Diagnostics_area::Sql_condition_iterator it = current_thd->get_stmt_da()->sql_conditions();
...
bool Item_func_sp::fix_fields(THD *thd, Item **ref)
{
   ...
   */
  if (!(thd->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW) || (thd->lex->sql_command == SQLCOM_CREATE_VIEW))
  {
    ...
    */
    res = sp ? sp->check_execute_access(thd)
             : check_routine_access(thd, EXECUTE_ACL, &m_name->m_db, &m_name->m_name, &sp_handler_function, false);
...
{
  uuid_value = ((((ulonglong)global_system_variables.server_id) << 56) + (((ulonglong)server_start_time) << 24));
}
...
longlong Item_func_uuid_short::val_int() { return (longlong)server_uuid_value(); }
...
void Item_func_last_value::evaluate_sideeffects()
{
  ...
  for (uint i = 0; i < arg_count - 1; i++) args[i]->val_int();
}
...
sp_cursor *Cursor_ref::get_open_cursor_or_error()
{
  ...
  {
    my_message(ER_SP_CURSOR_NOT_OPEN, ER_THD(thd, ER_SP_CURSOR_NOT_OPEN), MYF(0));
    return NULL;
  }
  return c;
}
...
longlong Item_func_nextval::val_int()
{
  ...
  if (!(entry = ((SEQUENCE_LAST_VALUE *)my_hash_search(&thd->sequences, (uchar *)key, length))))
  {
...
Item_func_rownum::Item_func_rownum(THD *thd) : Item_longlong_func(thd), accepted_rows(0)
{
  /*
  ...
  */
  ...
  if (sql_command_flags[thd->lex->sql_command] & (CF_UPDATES_DATA | CF_DELETES_DATA))
...
void Item_func_rownum::fix_after_optimize(THD *thd) { accepted_rows = &select->join->accepted_rows; }
...
void fix_rownum_pointers(THD *thd, SELECT_LEX *select_lex, ha_rows *ptr)
{
  ...
  while (Item *item = li++)
  {
    if (item->type() == Item::FUNC_ITEM && ((Item_func *)item)->functype() == Item_func::ROWNUM_FUNC)
