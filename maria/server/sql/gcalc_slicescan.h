...
#else
#define GCALC_DBUG_PRINT(b) \
  do                        \
  {                         \
  } while (0)
#define GCALC_DBUG_ENTER(a) \
  do                        \
  {                         \
  } while (0)
#define GCALC_DBUG_RETURN(r) return (r)
#define GCALC_DBUG_VOID_RETURN \
  do                           \
  {                            \
  } while (0)
#define GCALC_DBUG_ASSERT(r) \
  do                         \
  {                          \
  } while (0)
...
#define GCALC_DECL_TERMINATED_STATE(varname) volatile int *varname;
...
class Gcalc_dyn_list
{
 ...
 protected:
  ...
  inline Item *ptr_add(Item *ptr, int n_items) { return (Item *)(((char *)ptr) + n_items * m_sizeof_item); }
};
...
class Gcalc_heap : public Gcalc_dyn_list
{
 public:
  ...
  class Info : public Gcalc_dyn_list::Item
  {
   public:
    ...
    bool is_bottom() const
    {
      ...
      return !node.shape.left;
    }
    bool is_top() const
    {
      GCALC_DBUG_ASSERT(type == nt_shape_node);
      return node.shape.top_node;
    }
    bool is_single_node() const { return is_bottom() && is_top(); }
  ...
  Gcalc_heap(size_t blk_size = 8192) : Gcalc_dyn_list(blk_size, sizeof(Info)), m_hook(&m_first), m_n_points(0) {}

  Gcalc_heap(const Gcalc_heap &gh) : Gcalc_dyn_list(gh), m_hook(&m_first), m_n_points(0) {}
  ...
  Info *new_intersection(const Info *p1, const Info *p2, const Info *p3, const Info *p4);
...
class Gcalc_shape_transporter
{
 ...
 public:
  Gcalc_shape_transporter(Gcalc_heap *heap) : m_shape_started(0), m_heap(heap) {}
  ...
  int start_simple_poly() { return start_poly() || start_ring(); }
  int complete_simple_poly() { return complete_ring() || complete_poly(); }
...
class Gcalc_scan_iterator : public Gcalc_dyn_list
{
 public:
  class point : public Gcalc_dyn_list::Item
  {
   public:
    ...
    inline const point *c_get_next() const { return (const point *)next; }
    ...
    static int cmp_dx_dy(const Gcalc_coord1 dx_a, const Gcalc_coord1 dy_a, const Gcalc_coord1 dx_b,
                         const Gcalc_coord1 dy_b);
    static int cmp_dx_dy(const Gcalc_heap::Info *p1, const Gcalc_heap::Info *p2, const Gcalc_heap::Info *p3,
  ...
  {
   public:
    inline const event_point *get_next() const { return (const event_point *)ev_next; }
    int simple_event() const
    {
      return !ev_next ? (event & (scev_point | scev_end)) : (!ev_next->ev_next && event == scev_two_ends);
    }
  };
 ...
 public:
  ...
  bool more_trapezoids() { return m_cur_pi && m_cur_pi->next; }

  const point *get_bottom_points() const { return m_bottom_points; }
  const point *get_event_position() const { return *state.event_position_hook; }
  const point *get_event_end() const { return state.event_end; }
  const event_point *get_events() const
  {
    return (const event_point *)(*state.event_position_hook == state.event_end ? m_bottom_points
                                                                               : *state.event_position_hook);
  }
  ...
  int intersection_step() const { return state.pi->type == Gcalc_heap::nt_intersection; }
  const Gcalc_heap::Info *get_cur_pi() const { return state.pi; }
 ...
 private:
  ...
  int insert_top_node();
  int add_intersection(point *sp_a, point *sp_b, Gcalc_heap::Info *pi_from);
...
class Gcalc_trapezoid_iterator
{
 ...
 public:
  Gcalc_trapezoid_iterator(const Gcalc_scan_iterator *scan_i) : sp0(scan_i->get_b_slice()), sp1(scan_i->get_t_slice())
  {
  }
  ...
  const Gcalc_scan_iterator::point *rt() const { return sp1->c_get_next(); }
...
class Gcalc_point_iterator
{
 ...
 public:
  Gcalc_point_iterator(const Gcalc_scan_iterator *scan_i) : sp(scan_i->get_b_slice()) {}
  ...
  inline void restart(const Gcalc_scan_iterator *scan_i) { sp = scan_i->get_b_slice(); }
};
