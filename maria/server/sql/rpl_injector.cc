...
inline injector::transaction::transaction(MYSQL_BIN_LOG *log, THD *thd) : m_state(START_STATE), m_thd(thd)
{
  /*
  ...
  */
  ...
  m_start_pos.m_file_name = my_strdup(key_memory_binlog_pos, log_info.log_file_name, MYF(0));
...
int injector::record_incident(THD *thd, Incident incident, const LEX_CSTRING *message)
{
