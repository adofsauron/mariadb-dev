...
typedef my_bool (*qc_engine_callback)(THD *thd, const char *table_key, uint key_length, ulonglong *engine_data);
...
struct Query_cache_query
{
  ...
  inline uchar *query() { return (((uchar *)this) + ALIGN_SIZE(sizeof(Query_cache_query))); }
  /**
...
struct Query_cache_table
{
  ...
  */
  ...
  inline uchar *data() { return (uchar *)(((uchar *)this) + ALIGN_SIZE(sizeof(Query_cache_table))); }
};
...
struct Query_cache_result
{
  ...
  inline uchar *data() { return (uchar *)(((uchar *)this) + ALIGN_SIZE(sizeof(Query_cache_result))); }
...
extern "C"
{
  ...
  uchar *query_cache_table_get_key(const uchar *record, size_t *length, my_bool not_used);
}
...
class Query_cache
{
 public:
  ...
  /* statistics */
  size_t free_memory, queries_in_cache, hits, inserts, refused, free_memory_blocks, total_blocks, lowmem_prunes;
  ...
  */
  ...
  my_bool allocate_data_chain(Query_cache_block **result_block, size_t data_len, Query_cache_block *query_block,
                              my_bool first_block);
  ...
  TABLE_COUNTER_TYPE
  register_tables_from_list(THD *thd, TABLE_LIST *tables_used, TABLE_COUNTER_TYPE counter,
                            Query_cache_block_table **block_table);
  my_bool register_all_tables(THD *thd, Query_cache_block *block, TABLE_LIST *tables_used, TABLE_COUNTER_TYPE tables);
  void unlink_table(Query_cache_block_table *node);
  Query_cache_block *get_free_block(size_t len, my_bool not_less, size_t min);
  ...
  my_bool append_next_free_block(Query_cache_block *block, size_t add_size);
  ...
  my_bool move_by_type(uchar **border, Query_cache_block **before, size_t *gap, Query_cache_block *i);
  ...
  */
  ...
  void make_disabled();
  void free_cache();
  Query_cache_block *write_block_data(size_t data_len, uchar *data, size_t header_len,
                                      Query_cache_block::block_type type, TABLE_COUNTER_TYPE ntab = 0);
  my_bool append_result_data(Query_cache_block **result, size_t data_len, uchar *data, Query_cache_block *parent);
  my_bool write_result_data(Query_cache_block **result, size_t data_len, uchar *data, Query_cache_block *parent,
  ...
  Query_cache_block *allocate_block(size_t len, my_bool not_less, size_t min);
  /*
    If query is cacheable return number tables in query
    (query without tables not cached)
  */
  TABLE_COUNTER_TYPE is_cacheable(THD *thd, LEX *lex, TABLE_LIST *tables_used, uint8 *tables_type);
 ...
 public:
  Query_cache(size_t query_cache_limit = ULONG_MAX, size_t min_allocation_unit = QUERY_CACHE_MIN_ALLOCATION_UNIT,
  ...
  inline bool is_disable_in_progress(void) { return m_cache_status == DISABLE_REQUEST; }
  ...
  */
  ...
  void invalidate(THD *thd, const char *key, size_t key_length, my_bool using_transactions);
  ...
  void flush();
  void pack(THD *thd, size_t join_limit = QUERY_CACHE_PACK_LIMIT, uint iteration_limit = QUERY_CACHE_PACK_ITERATION);

  void destroy();

  void insert(THD *thd, Query_cache_tls *query_cache_tls, const char *packet, size_t length, unsigned pkt_nr);
  my_bool insert_table(THD *thd, size_t key_len, const char *key, Query_cache_block_table *node, size_t db_length,
                       uint8 suffix_length_arg, uint8 cache_type, qc_engine_callback callback, ulonglong engine_data,
                       my_bool hash);
...
#define query_cache_maybe_disabled(T) (T->variables.query_cache_type == 0 || query_cache.query_cache_size == 0)
#define query_cache_is_cacheable_query(L) (((L)->sql_command == SQLCOM_SELECT) && (L)->safe_to_cache_query)
#else
