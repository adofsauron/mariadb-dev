...
class Opt_trace_stmt
{
  ...
  */
  ...
  bool is_enabled() { return I_S_disabled == 0; }
...
class Opt_trace_context
{
 public:
  ...
  void start(THD *thd, TABLE_LIST *tbl, enum enum_sql_command sql_command, const char *query, size_t query_length,
             const CHARSET_INFO *query_charset, ulong max_mem_size_arg);
  void end();
  ...
  */
  ...
  bool empty() { return static_cast<uint>(traces.elements()) == 0; }

  bool is_started() { return current_trace && current_trace->is_enabled(); }
