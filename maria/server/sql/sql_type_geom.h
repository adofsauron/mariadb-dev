...
class Type_handler_geometry : public Type_handler_string_result
{
 public:
  ...
  static bool check_type_geom_or_binary(const LEX_CSTRING &opname, const Item *item);
  static bool check_types_geom_or_binary(const LEX_CSTRING &opname, Item *const *args, uint start, uint end);
 ...
 public:
  ...
  virtual Item *create_typecast_item(THD *thd, Item *item, const Type_cast_attributes &attr) const override;
  const Type_handler *type_handler_frm_unpack(const uchar *buffer) const override;
  bool is_binary_compatible_geom_super_type_for(const Type_handler_geometry *th) const
  {
    return geometry_type() == GEOM_GEOMETRY || geometry_type() == th->geometry_type();
  }
  ...
  uint Column_definition_gis_options_image(uchar *buff, const Column_definition &def) const override;
  ...
  bool Key_part_spec_init_foreign(Key_part_spec *part, const Column_definition &def,
                                  const handler *file) const override;
  bool Key_part_spec_init_spatial(Key_part_spec *part, const Column_definition &def) const override;
...
class Type_collection_geometry : public Type_collection
{
  const Type_handler *aggregate_common(const Type_handler *a, const Type_handler *b) const
  {
    if (a == b)
      return a;
    if (dynamic_cast<const Type_handler_geometry *>(a) && dynamic_cast<const Type_handler_geometry *>(b))
      return &type_handler_geometry;
    return NULL;
  }
  const Type_handler *aggregate_if_null(const Type_handler *a, const Type_handler *b) const
  {
    return a == &type_handler_null ? b : b == &type_handler_null ? a : NULL;
  }
  const Type_handler *aggregate_if_long_blob(const Type_handler *a, const Type_handler *b) const
  {
...
class Field_geom : public Field_blob
{
 ...
 public:
  ...
  {
    GEOM_STORAGE_WKB = 0,
    GEOM_STORAGE_BINARY = 1
  };
  enum storage_type storage;

  Field_geom(uchar *ptr_arg, uchar *null_ptr_arg, uchar null_bit_arg, enum utype unireg_check_arg,
             const LEX_CSTRING *field_name_arg, TABLE_SHARE *share, uint blob_pack_length,
             const Type_handler_geometry *gth, uint field_srid)
  ...
  enum ha_base_keytype key_type() const override { return HA_KEYTYPE_VARBINARY2; }
  const Type_handler *type_handler() const override { return m_type_handler; }
  const Type_handler_geometry *type_handler_geom() const { return m_type_handler; }
  void set_type_handler(const Type_handler_geometry *th) { m_type_handler = th; }
  enum_field_types type() const override { return MYSQL_TYPE_GEOMETRY; }
  ...
  {
    const Type_handler_geometry *fth = dynamic_cast<const Type_handler_geometry *>(from->type_handler());
  ...
  {
    const Type_handler_geometry *fth = dynamic_cast<const Type_handler_geometry *>(from->type_handler());
    return fth && m_type_handler->is_binary_compatible_geom_super_type_for(fth) && !table->copy_blobs;
  }
