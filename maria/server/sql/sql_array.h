...
class Bounds_checked_array
{
 public:
  ...
  Bounds_checked_array(Element_type *el, size_t size_arg) : m_array(el), m_size(size_arg) {}
...
class Dynamic_array
{
 ...
 public:
  ...
  Dynamic_array(MEM_ROOT *root, size_t prealloc = 16, size_t increment = 16)
  {
    ...
    init_dynamic_array2(root->psi_key, &array, sizeof(Elem), init_buffer, prealloc, increment, MYF(0));
  }
  ...
  */
  ...
  const Elem &at(size_t idx) const { return *(((Elem *)array.buffer) + idx); }
  ...
  /// @returns pointer to last element
  const Elem *back() const { return ((const Elem *)array.buffer) + array.elements - 1; }

  size_t size() const { return array.elements; }

  const Elem *end() const { return back() + 1; }

  /// @returns pointer to n-th element
  Elem *get_pos(size_t idx) { return ((Elem *)array.buffer) + idx; }

  /// @returns pointer to n-th element
  const Elem *get_pos(size_t idx) const { return ((const Elem *)array.buffer) + idx; }

  /**
     @retval false ok
     @retval true  OOM, @c my_error() has been called.
  */
  bool append(const Elem &el) { return insert_dynamic(&array, &el); }

  bool append_val(Elem el) { return (insert_dynamic(&array, (uchar *)&el)); }

  bool push(Elem &el) { return append(el); }

  /// Pops the last element. Does nothing if array is empty.
  Elem &pop() { return *((Elem *)pop_dynamic(&array)); }
  ...
  size_t elements() const { return array.elements; }
  ...
  void set(size_t idx, const Elem &el) { set_dynamic(&array, &el, idx); }

  void freeze() { freeze_size(&array); }

  bool reserve(size_t new_size) { return allocate_dynamic(&array, new_size); }
  ...
  void free_memory() { delete_dynamic(&array); }

  typedef int (*CMP_FUNC)(const Elem *el1, const Elem *el2);

  void sort(CMP_FUNC cmp_func) { my_qsort(array.buffer, array.elements, sizeof(Elem), (qsort_cmp)cmp_func); }
