...
static int thr_lock_errno_to_mysql[] = {0, ER_LOCK_ABORTED, ER_LOCK_WAIT_TIMEOUT, ER_LOCK_DEADLOCK};
...
static int lock_tables_check(THD *thd, TABLE **tables, uint count, uint flags)
{
  ...
  system_count = 0;
  ignore_read_only = (thd->security_ctx->master_access & PRIV_IGNORE_READ_ONLY) != NO_ACL;
  ...
  for (i = 0; i < count; i++)
  {
    ...
    */
    if (t->s->require_write_privileges() && !log_table_write_query)
    {
      /*
      ...
      */
      if ((t->reginfo.lock_type >= TL_FIRST_WRITE) || (thd->lex->sql_command == SQLCOM_LOCK_TABLES))
      {
    ...
    */
    DBUG_ASSERT(t->s->tmp_table || thd->mdl_context.is_lock_owner(
                                       MDL_key::TABLE, t->s->db.str, t->s->table_name.str,
                                       t->reginfo.lock_type >= TL_FIRST_WRITE ? MDL_SHARED_WRITE : MDL_SHARED_READ));
    ...
    */
    ...
    {
      if (t->reginfo.lock_type >= TL_FIRST_WRITE && !ignore_read_only && opt_readonly && !thd->slave_thread)
      {
...
void reset_lock_data(MYSQL_LOCK *sql_lock, bool unlock)
{
  ...
  for (ldata = sql_lock->locks, ldata_end = ldata + sql_lock->lock_count; ldata < ldata_end; ldata++)
  ...
  if (thd->variables.session_track_transaction_info > TX_TRACK_NONE)
  {
    while (count--)
    {
      if (TABLE *t = tables[count])
        thd->session_tracker.transaction_info.add_trx_state(thd, t->reginfo.lock_type,
...
MYSQL_LOCK *mysql_lock_tables(THD *thd, TABLE **tables, uint count, uint flags)
{
  ...
  if (!(thd->variables.option_bits & OPTION_TABLE_LOCK) && !(flags & MYSQL_LOCK_USE_MALLOC))
...
{
  int rc = 1;
  ulong timeout = (flags & MYSQL_LOCK_IGNORE_TIMEOUT) ? LONG_TIMEOUT : thd->variables.lock_wait_timeout;
  ...
  memmove(sql_lock->locks + sql_lock->lock_count, sql_lock->locks, sql_lock->lock_count * sizeof(*sql_lock->locks));

  /* Lock on the copied half of the lock data array. */
  rc = thr_lock_errno_to_mysql[(int)thr_multi_lock(sql_lock->locks + sql_lock->lock_count, sql_lock->lock_count,
...
static int lock_external(THD *thd, TABLE **tables, uint count)
{
  ...
  for (i = 1; i <= count; i++, tables++)
  {
    ...
    if ((*tables)->db_stat & HA_READ_ONLY ||
        ((*tables)->reginfo.lock_type >= TL_READ && (*tables)->reginfo.lock_type < TL_FIRST_WRITE))
      lock_type = F_RDLCK;
...
int mysql_unlock_tables(THD *thd, MYSQL_LOCK *sql_lock)
{
  return mysql_unlock_tables(thd, sql_lock,
                             (thd->variables.option_bits & OPTION_TABLE_LOCK) || !(sql_lock->flags & GET_LOCK_ON_THD));
}
...
int mysql_unlock_some_tables(THD *thd, TABLE **table, uint count, uint flag)
{
  int error;
  MYSQL_LOCK *sql_lock;
  if (!(sql_lock = get_lock_data(thd, table, count, GET_LOCK_UNLOCK | GET_LOCK_ON_THD | flag)))
  ...
  if (locked)
  {
    ...
    for (i = 0; i < locked->table_count; i++)
    {
      if (locked->table[i] == table)
      {
        ...
        bmove((char *)(locked->table + i), (char *)(locked->table + i + 1), (old_tables - i) * sizeof(TABLE *));

        lock_data_end = table->lock_data_start + table->lock_count;
        /* Move down all lock data pointers above 'table->lock_data_end-1' */
        bmove((char *)(locked->locks + table->lock_data_start), (char *)(locked->locks + lock_data_end),
...
bool mysql_lock_abort_for_thread(THD *thd, TABLE *table)
{
  ...
  if ((locked = get_lock_data(thd, &table, 1, GET_LOCK_UNLOCK | GET_LOCK_ON_THD)))
  {
    ...
    {
      if (thr_abort_locks_for_thread(locked->locks[i]->lock, table->in_use->thread_id))
        result = TRUE;
    }
  }
  DBUG_RETURN(result);
...
MYSQL_LOCK *mysql_lock_merge(MYSQL_LOCK *a, MYSQL_LOCK *b, THD *thd)
{
  ...
  DBUG_PRINT("enter", ("a->lock_count: %u  b->lock_count: %u", a->lock_count, b->lock_count));

  const size_t lock_size = sizeof(*sql_lock) + sizeof(THR_LOCK_DATA *) * ((a->lock_count + b->lock_count) * 2) +
  ...
  {
    sql_lock = (MYSQL_LOCK *)my_malloc(key_memory_MYSQL_LOCK, lock_size, MYF(MY_WME));
    if (!sql_lock)
      DBUG_RETURN(0);
    sql_lock->flags = 0;
  }
  ...
  */
  for (table = sql_lock->table + a->table_count, end_table = table + b->table_count; table < end_table; table++)
  {
  ...
  */
  ...
  memcpy(sql_lock->locks + sql_lock->lock_count, sql_lock->locks, sql_lock->lock_count * sizeof(*sql_lock->locks));
  thr_merge_locks(sql_lock->locks + sql_lock->lock_count, a->lock_count, b->lock_count);
...
MYSQL_LOCK *get_lock_data(THD *thd, TABLE **table_ptr, uint count, uint flags)
{
  ...
  for (i = lock_count = table_count = 0; i < count; i++)
  {
    ...
    if ((likely(!t->s->tmp_table) || (t->s->tmp_table == TRANSACTIONAL_TMP_TABLE)) &&
  ...
  */
  size_t amount = sizeof(*sql_lock) + sizeof(THR_LOCK_DATA *) * lock_count * 2 + sizeof(table_ptr) * table_count;
  if (!(sql_lock = (MYSQL_LOCK *)(flags & GET_LOCK_ON_THD ? thd->alloc(amount)
                                                          : my_malloc(key_memory_MYSQL_LOCK, amount, MYF(0)))))
    DBUG_RETURN(0);
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if (!((likely(!table->s->tmp_table) || (table->s->tmp_table == TRANSACTIONAL_TMP_TABLE)) &&
    ...
    locks_start = locks;
    locks =
        table->file->store_lock(thd, locks, (flags & GET_LOCK_ACTION_MASK) == GET_LOCK_UNLOCK ? TL_IGNORE : lock_type);
  ...
  */
  ...
  DBUG_PRINT("info", ("sql_lock->table_count %d sql_lock->lock_count %d", sql_lock->table_count, sql_lock->lock_count));
  DBUG_RETURN(sql_lock);
}
...
bool lock_schema_name(THD *thd, const char *db)
{
  ...
  MDL_REQUEST_INIT(&mdl_request, MDL_key::SCHEMA, db, "", MDL_EXCLUSIVE, MDL_TRANSACTION);

  mdl_requests.push_front(&mdl_request);
  mdl_requests.push_front(&global_request);

  if (thd->mdl_context.acquire_locks(&mdl_requests, thd->variables.lock_wait_timeout))
    return TRUE;
...
bool lock_object_name(THD *thd, MDL_key::enum_mdl_namespace mdl_type, const char *db, const char *name)
{
  ...
  if (thd->has_read_only_protection())
    return TRUE;
  MDL_REQUEST_INIT(&global_request, MDL_key::BACKUP, "", "", MDL_BACKUP_DDL, MDL_STATEMENT);
  MDL_REQUEST_INIT(&schema_request, MDL_key::SCHEMA, db, "", MDL_INTENTION_EXCLUSIVE, MDL_TRANSACTION);
  MDL_REQUEST_INIT(&mdl_request, mdl_type, db, name, MDL_EXCLUSIVE, MDL_TRANSACTION);
...
bool Global_read_lock::lock_global_read_lock(THD *thd)
{
  ...
  if (!m_state)
  {
    ...
    */
    ...
    DBUG_ASSERT(!thd->mdl_context.is_lock_owner(MDL_key::BACKUP, "", "", MDL_BACKUP_FTWRL1));
    DBUG_ASSERT(!thd->mdl_context.is_lock_owner(MDL_key::BACKUP, "", "", MDL_BACKUP_FTWRL2));
    MDL_REQUEST_INIT(&mdl_request, MDL_key::BACKUP, "", "", MDL_BACKUP_FTWRL1, MDL_EXPLICIT);

    do
    {
      mdl_deadlock_handler.init();
      thd->push_internal_handler(&mdl_deadlock_handler);
      result = thd->mdl_context.acquire_lock(&mdl_request, thd->variables.lock_wait_timeout);
...
void Global_read_lock::unlock_global_read_lock(THD *thd)
{
  ...
  if (m_state == GRL_ACQUIRED_AND_BLOCKS_COMMIT && wsrep_locked_seqno != WSREP_SEQNO_UNDEFINED)
  {
    ...
    if (server_state.state() == Wsrep_server_state::s_donor ||
        (WSREP_NNULL(thd) && server_state.state() != Wsrep_server_state::s_synced))
    {
      server_state.resume();
      wsrep_locked_seqno = WSREP_SEQNO_UNDEFINED;
    }
    else if (WSREP_NNULL(thd) && server_state.state() == Wsrep_server_state::s_synced)
    {
      THD_STAGE_INFO(thd, stage_waiting_flow);
      WSREP_DEBUG("unlock_global_read_lock: waiting for flow control for %s", wsrep_thd_query(thd));
...
bool Global_read_lock::make_global_read_lock_block_commit(THD *thd)
{
  ...
  */
  ...
  if (thd->mdl_context.upgrade_shared_lock(m_mdl_global_read_lock, MDL_BACKUP_FTWRL2, thd->variables.lock_wait_timeout))
    DBUG_RETURN(TRUE);
