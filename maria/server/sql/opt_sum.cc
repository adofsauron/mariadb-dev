...
static int get_index_min_value(TABLE *table, TABLE_REF *ref, Item_field *item_field, uint range_fl, uint prefix_len)
{
  int error;
  ...
  else
  {
    ...
    */
    if (!(range_fl & NEAR_MIN))
      ...
      */
      error = table->file->ha_index_read_map(table->record[0], ref->key_buff, make_prev_keypart_map(ref->key_parts),
                                             HA_READ_KEY_OR_NEXT);
    else
    {
      /*
      ...
      */
      ...
      /*
         ...
         the skipped tuple.
      */
      if (item_field->field->real_maybe_null() && ref->key_buff[prefix_len] == 1 &&
          /*
            ...
            NULL.
          */
          (error == HA_ERR_KEY_NOT_FOUND || key_cmp_if_same(table, ref->key_buff, ref->key, prefix_len)))
      {
...
static int get_index_max_value(TABLE *table, TABLE_REF *ref, uint range_fl)
{
  return (ref->key_length
              ? table->file->ha_index_read_map(table->record[0], ref->key_buff, make_prev_keypart_map(ref->key_parts),
  ...
  */
  while ((tl = ti++))
  {
    ...
    /*
      ...
      get row count
    */
    if (!(tl->table->file->ha_table_flags() & HA_STATS_RECORDS_IS_EXACT) || tl->schema_table)
    {
      maybe_exact_count &= MY_TEST(!tl->schema_table && (tl->table->file->ha_table_flags() & HA_HAS_RECORDS));
      is_exact_count = FALSE;
      count = 1;  // ensure count != 0
    }
    else if (tl->is_materialized_derived() || tl->jtbm_subselect)
    {
      /*
      ...
      switch (item_sum->sum_func())
      {
        ...
        case Item_sum::MAX_FUNC:
        {
          ...
          */
          ...
          if (((expr->used_tables() & OUTER_REF_TABLE_BIT) == 0) && expr->real_item()->type() == Item::FIELD_ITEM)
          {
            ...
            */
            if (table->file->inited || (outer_tables & table->map) ||
                !find_key_for_maxmin(is_max, &ref, item_field->field, conds, &range_fl, &prefix_len))
            {
              const_result = 0;
              break;
            }
            ...
            if (!error && reckey_in_range(is_max, &ref, item_field->field, conds, range_fl, prefix_len))
          ...
          item_sum->set_aggregator(
              thd, item_sum->has_with_distinct() ? Aggregator::DISTINCT_AGGREGATOR : Aggregator::SIMPLE_AGGREGATOR);
          /*
...
{
  if (item1->cmp_type() == STRING_RESULT && item2->cmp_type() == STRING_RESULT)
  {
                          ...
                          key_part_map *key_part_used, uint *range_fl, uint *prefix_len)
{
  ...
  if (!(cond_used_tables & field->table->map) && MY_TEST(cond_used_tables & ~PSEUDO_TABLE_BITS))
  {
  ...
  if (!is_null_safe_eq && !is_null && (args[1]->is_null() || (between && args[2]->is_null())))
    DBUG_RETURN(FALSE);
  ...
  {
    if ((!is_null && !cond->val_int()) || (is_null && !MY_TEST(part->field->is_null())))
...
static bool find_key_for_maxmin(bool max_fl, TABLE_REF *ref, Field *field, COND *cond, uint *range_fl, uint *prefix_len)
{
  ...
  for (keyinfo = table->key_info, keyinfo_end = keyinfo + table->s->keys; keyinfo != keyinfo_end; keyinfo++, idx++)
  {
    ...
    *prefix_len = 0;
    ...
    for (part = keyinfo->key_part; part != part_end; part++, jdx++, key_part_to_use = (key_part_to_use << 1) | 1)
    {
      ...
      if ((part_field->flags & BLOB_FLAG) || part->length < part_field->key_length())
        break;
