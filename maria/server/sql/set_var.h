...
class sys_var : protected Value_source  // for double_from_string_with_check
{
 ...
 public:
  sys_var(sys_var_chain *chain, const char *name_arg, const char *comment, int flag_args, ptrdiff_t off, int getopt_id,
          enum get_opt_arg_type getopt_arg_type, SHOW_TYPE show_val_type_arg, longlong def_val, PolyLock *lock,
          enum binlog_status_enum binlog_status_arg, on_check_function on_check_func, on_update_function on_update_func,
  ...
  */
  ...
  virtual CHARSET_INFO *charset(THD *thd) const { return system_charset_info; }
  ...
  */
  ...
  bool check_update_type(const Item *item)
  {
    ...
    switch (option.var_type & GET_TYPE_MASK)
    {
      ...
      case GET_LL:
      case GET_ULL:
        return type != INT_RESULT && (type != DECIMAL_RESULT || item->decimals != 0);
      case GET_STR:
      case GET_STR_ALLOC:
  ...
  {
    DBUG_ASSERT(parse_flags == GETOPT_ONLY_HELP || parse_flags == PARSE_EARLY || parse_flags == 0);
  ...
  */
  ...
  virtual const uchar *default_value_ptr(THD *thd) const { return (uchar *)&option.def_value; }

  virtual bool on_check_access_global(THD *thd) const;
  virtual bool on_check_access_session(THD *thd) const { return false; }
  ...
  */
  ...
  void *max_var_ptr() { return scope() == SESSION ? (((uchar *)&max_system_variables) + offset) : 0; }
...
class set_var : public set_var_base
{
 public:
  ...
  set_var(THD *thd, enum_var_type type_arg, sys_var *var_arg, const LEX_CSTRING *base_name_arg, Item *value_arg);
...
class set_var_default_role : public set_var_base
{
 ...
 public:
  set_var_default_role(LEX_USER *user_arg, LEX_CSTRING role_arg) : user(user_arg), role(role_arg) {}
  int check(THD *thd);
  int update(THD *thd);
};
...
sys_var *find_sys_var(THD *thd, const char *str, size_t length = 0, bool throw_error = false);
...
void set_sys_var_value_origin(void *ptr, enum sys_var::where here, const char *filename = NULL);
...
bool sql_mode_string_representation(THD *thd, sql_mode_t sql_mode, LEX_CSTRING *ls);
int default_regex_flags_pcre(THD *thd);

extern sys_var *Sys_autocommit_ptr, *Sys_last_gtid_ptr, *Sys_character_set_client_ptr,
    *Sys_character_set_connection_ptr, *Sys_character_set_results_ptr;
