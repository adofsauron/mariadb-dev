...
int show_create_table_ex(THD *thd, TABLE_LIST *table_list, const char *forced_db, const char *forced_name,
                         String *packet, Table_specification_st *create_info_arg, enum_with_db_name with_db_name);
...
bool mysqld_show_create_db(THD *thd, LEX_CSTRING *db_name, LEX_CSTRING *orig_db_name, const DDL_options_st &options);
...
{
  bool unused;
  return find_schema_table(thd, table_name, &unused);
}
...
class IS_table_read_plan : public Sql_alloc
{
  ...
  */
  ...
  bool has_db_lookup_value() { return (lookup_field_vals.db_value.length && !lookup_field_vals.wild_db_value); }
  bool has_table_lookup_value()
  {
    return (lookup_field_vals.table_value.length && !lookup_field_vals.wild_table_value);
  }
};
