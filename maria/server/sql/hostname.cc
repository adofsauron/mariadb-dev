...
bool hostname_cache_init()
{
  ...
  if (!(hostname_cache = new Hash_filo<Host_entry>(key_memory_host_cache_hostname, host_cache_size, key_offset,
                                                   HOST_ENTRY_KEY_SIZE, NULL, (my_hash_free_key)free, &my_charset_bin)))
    return 1;
...
Host_entry *hostname_cache_first() { return hostname_cache->first(); }
...
static void add_hostname_impl(const char *ip_key, const char *hostname, bool validated, Host_errors *errors,
                              ulonglong now)
{
  Host_entry *entry;
  bool need_add = false;
  ...
  if (validated)
  {
    if (hostname != NULL)
    {
      ...
      DBUG_PRINT("info", ("Adding/Updating '%s' -> '%s' (validated) to the hostname cache...'", (const char *)ip_key,
                          (const char *)entry->m_hostname));
    }
    else
    {
      entry->m_hostname_length = 0;
      DBUG_PRINT("info", ("Adding/Updating '%s' -> NULL (validated) to the hostname cache...'", (const char *)ip_key));
    }
  ...
  else
  {
    ...
    DBUG_PRINT("info",
               ("Adding/Updating '%s' -> NULL (not validated) to the hostname cache...'", (const char *)ip_key));
  }
...
static void add_hostname(const char *ip_key, const char *hostname, bool validated, Host_errors *errors)
{
...
int ip_to_hostname(struct sockaddr_storage *ip_storage, const char *ip_string, const char **hostname,
                   uint *connect_errors)
{
  ...
  DBUG_PRINT("info", ("IP address: '%s'; family: %d.", (const char *)ip_string, (int)ip->sa_family));
      ...
      */
      if (entry->m_host_validated)
      {
        if (entry->m_hostname_length)
          *hostname = my_strdup(key_memory_host_cache_hostname, entry->m_hostname, MYF(0));

        DBUG_PRINT("info", ("IP (%s) has been found in the cache. "
                            "Hostname: '%s'",
                            (const char *)ip_key, (const char *)(*hostname ? *hostname : "null")));
  ...
  */
  ...
  DBUG_EXECUTE_IF("getnameinfo_fake_long_host", {
    strcpy(hostname_buffer,
           "host5678901_345678902_345678903_345678904_345678905_345678906_345678907_345678908_345678909_345678910_"
           "345678911_345678912_345678913_345678914_345678915_345678916_345678917_345678918_345678919_345678920_"
           "345678921_345678922_345678923_345678924_345678925_345");
    err_code = 0;
  });
  ...
  */
  ...
  if (err_code)
  {
    ...
    DBUG_PRINT("error", ("IP address '%s' could not be resolved: %s", (const char *)ip_key,
                         (const char *)gai_strerror(err_code)));

    sql_print_warning("IP address '%s' could not be resolved: %s", (const char *)ip_key,
                      (const char *)gai_strerror(err_code));
  ...
  */
  ...
  if (!is_hostname_valid(hostname_buffer))
  {
    DBUG_PRINT("error", ("IP address '%s' has been resolved "
                         ...
                         "IPv4-address itself.",
                         (const char *)ip_key, (const char *)hostname_buffer));
  ...
  */
  ...
  DBUG_PRINT("info", ("Getting IP addresses for hostname '%s'...", (const char *)hostname_buffer));
  ...
  */
  ...
  if (err_code != 0)
  {
    sql_print_warning("Host name '%s' could not be resolved: %s", (const char *)hostname_buffer,
                      (const char *)gai_strerror(err_code));
  ...
  DBUG_PRINT("info", ("The following IP addresses found for '%s':", (const char *)hostname_buffer));
  ...
  for (struct addrinfo *addr_info = addr_info_list; addr_info; addr_info = addr_info->ai_next)
  {
    ...
    {
      err_status =
          vio_get_normalized_ip_string(addr_info->ai_addr, (int)addr_info->ai_addrlen, ip_buffer, sizeof(ip_buffer));
    ...
    if (strcasecmp(ip_key, ip_buffer) == 0)
    {
      ...
      *hostname = my_strdup(key_memory_host_cache_hostname, hostname_buffer, MYF(0));
  ...
  if (!*hostname)
  {
    ...
    sql_print_warning("Hostname '%s' does not resolve to '%s'.", (const char *)hostname_buffer, (const char *)ip_key);
    sql_print_information("Hostname '%s' has the following IP addresses:", (const char *)hostname_buffer);
