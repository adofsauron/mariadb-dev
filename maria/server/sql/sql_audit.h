...
static inline bool mysql_audit_general_enabled() { return mysql_global_audit_mask[0] & MYSQL_AUDIT_GENERAL_CLASSMASK; }
...
static inline bool mysql_audit_table_enabled() { return mysql_global_audit_mask[0] & MYSQL_AUDIT_TABLE_CLASSMASK; }

#else
static inline void mysql_audit_notify(THD *thd, uint event_class, const void *event) {}
...
static inline unsigned int strlen_uint(const char *s) { return (uint)strlen(s); }

static inline unsigned int safe_strlen_uint(const char *s) { return (uint)safe_strlen(s); }
...
static inline uint make_user_name(THD *thd, char *buf)
{
  ...
  char *end =
      strxnmov(buf, MAX_USER_HOST_SIZE, sctx->priv_user[0] ? sctx->priv_user : "", "[", sctx->user ? sctx->user : "",
...
static inline void mysql_audit_general_log(THD *thd, time_t time, const char *user, uint userlen, const char *cmd,
                                           uint cmdlen, const char *query, uint querylen)
{
...
*/
static inline void mysql_audit_general(THD *thd, uint event_subtype, int error_code, const char *msg)
{
...
static inline void mysql_audit_notify_connection_connect(THD *thd)
{
  if (mysql_audit_connection_enabled())
  {
    ...
    event.status = thd->get_stmt_da()->is_error() ? thd->get_stmt_da()->sql_errno() : 0;
...
static inline void mysql_audit_notify_connection_disconnect(THD *thd, int errcode)
{
  if (mysql_audit_connection_enabled())
  {
    const Security_context *sctx = thd->security_ctx;
...
static inline void mysql_audit_notify_connection_change_user(THD *thd, const Security_context *old_ctx)
{
  if (mysql_audit_connection_enabled())
  {
    mysql_event_connection event;
...
static inline void mysql_audit_external_lock_ex(THD *thd, my_thread_id thread_id, const char *user, const char *host,
                                                const char *ip, query_id_t query_id, TABLE_SHARE *share, int lock)
{
...
static inline void mysql_audit_external_lock(THD *thd, TABLE_SHARE *share, int lock)
{
  mysql_audit_external_lock_ex(thd, thd->thread_id, thd->security_ctx->user, thd->security_ctx->host,
                               thd->security_ctx->ip, thd->query_id, share, lock);
}

static inline void mysql_audit_create_table(TABLE *table)
{
...
static inline void mysql_audit_drop_table(THD *thd, TABLE_LIST *table)
{
  if (mysql_audit_table_enabled())
  {
    const Security_context *sctx = thd->security_ctx;
...
static inline void mysql_audit_rename_table(THD *thd, const LEX_CSTRING *old_db, const LEX_CSTRING *old_tb,
...
static inline void mysql_audit_alter_table(THD *thd, TABLE_LIST *table)
{
  if (mysql_audit_table_enabled())
  {
    const Security_context *sctx = thd->security_ctx;
