...
Wsrep_client_service::Wsrep_client_service(THD *thd, Wsrep_client_state &client_state)
...
bool Wsrep_client_service::interrupted(wsrep::unique_lock<wsrep::mutex> &lock WSREP_UNUSED) const
{
  ...
  if (ret)
  {
    WSREP_DEBUG("wsrep state is interrupted, THD::killed %d trx state %d", m_thd->killed, m_thd->wsrep_trx().state());
  }
  return ret;
}
...
int Wsrep_client_service::prepare_data_for_replication()
{
  ...
  if (transactional_cache || stmt_cache)
  {
    ...
    if (transactional_cache && wsrep_write_cache(m_thd, transactional_cache, &transactional_data_len))
    {
      WSREP_ERROR("rbr write fail, data_len: %zu", data_len);
  ...
  if (data_len == 0)
  {
    if (m_thd->get_stmt_da()->is_ok() && m_thd->get_stmt_da()->affected_rows() > 0 && !binlog_filter->is_on() &&
        !m_thd->wsrep_trx().is_streaming())
    {
      WSREP_DEBUG(
          ...
          "changed tables: %d, "
          "sql_log_bin: %d",
          wsrep_thd_query(m_thd), m_thd->get_stmt_da()->affected_rows(), stmt_has_updated_trans_table(m_thd),
...
void Wsrep_client_service::cleanup_transaction()
{
  ...
  if (WSREP_EMULATE_BINLOG(m_thd))
    wsrep_thd_binlog_trx_reset(m_thd);
  m_thd->wsrep_affected_rows = 0;
}

int Wsrep_client_service::prepare_fragment_for_replication(wsrep::mutable_buffer &buffer, size_t &log_position)
{
  DBUG_ASSERT(m_thd == current_thd);
  THD *thd = m_thd;
  ...
  if (length > 0)
  {
    do
    {
      ...
      {
        WSREP_WARN("transaction size limit (%lu) exceeded: %zu", wsrep_max_ws_size, total_length);
        ret = 1;
        goto cleanup;
      }
...
int Wsrep_client_service::remove_fragments()
{
  ...
  if (wsrep_schema->remove_fragments(m_thd, Wsrep_server_state::instance().id(), m_thd->wsrep_trx().id(),
...
enum wsrep::provider::status Wsrep_client_service::replay()
{
  ...
  replayer_thd->prior_thr_create_utime = replayer_thd->start_utime = microsecond_interval_timer();
  ...
  enum wsrep::provider::status ret;
  {
    ...
    ret = provider.replay(replayer_thd->wsrep_trx().ws_handle(), &replayer_service);
...
void Wsrep_client_service::wait_for_replayers(wsrep::unique_lock<wsrep::mutex> &lock)
{
  ...
  while (wsrep_replaying > 0 && !wsrep_is_bf_aborted(m_thd))
  {
    ...
    mysql_cond_timedwait(&COND_wsrep_replaying, &LOCK_wsrep_replaying, &wait_time);
  }
  mysql_mutex_unlock(&LOCK_wsrep_replaying);
  lock.lock();
}
