...
bool Window_spec::check_window_names(List_iterator_fast<Window_spec> &it)
{
  ...
  while ((win_spec = it++) && win_spec != this)
  {
    ...
    if (ref_name && my_strcasecmp(system_charset_info, ref_name, win_spec_name) == 0)
    {
      if (partition_list->elements)
      {
        my_error(ER_PARTITION_LIST_IN_REFERENCING_WINDOW_SPEC, MYF(0), ref_name);
        return true;
      }
...
void Window_spec::print(String *str, enum_query_type query_type)
{
  str->append('(');
...
void Window_spec::print_partition(String *str, enum_query_type query_type)
{
...
void Window_spec::print_order(String *str, enum_query_type query_type)
{
  if (order_list->first)
  {
...
{
  if ((top_bound->is_unbounded() && top_bound->precedence_type == Window_frame_bound::FOLLOWING) ||
      (bottom_bound->is_unbounded() && bottom_bound->precedence_type == Window_frame_bound::PRECEDING) ||
...
void Window_frame::print(String *str, enum_query_type query_type)
{
  switch (units)
  {
    case UNITS_ROWS:
...
void Window_frame_bound::print(String *str, enum_query_type query_type)
{
...
int setup_windows(THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables, List<Item> &fields,
                  List<Item> &all_fields, List<Window_spec> &win_specs, List<Item_window_func> &win_funcs)
{
  Window_spec *win_spec;
  ...
  */
  ...
  while ((win_spec = it++))
  {
    ...
    if (win_spec->check_window_names(itp) ||
        setup_group(thd, ref_pointer_array, tables, fields, all_fields, win_spec->partition_list->first,
                    &hidden_group_fields, true) ||
        setup_order(thd, ref_pointer_array, tables, fields, all_fields, win_spec->order_list->first, true) ||
        (win_spec->window_frame && win_spec->window_frame->check_frame_bounds()))
    {
      DBUG_RETURN(1);
    }

    if (win_spec->window_frame && win_spec->window_frame->exclusion != Window_frame::EXCL_NONE)
    {
    ...
    */
    if (win_spec->window_frame && win_spec->window_frame->units == Window_frame::UNITS_RANGE)
    {
      ...
      */
      Window_frame_bound *bounds[] = {win_spec->window_frame->top_bound, win_spec->window_frame->bottom_bound, NULL};
      for (Window_frame_bound **pbound = &bounds[0]; *pbound; pbound++)
      {
        if (!(*pbound)->is_unbounded() && ((*pbound)->precedence_type == Window_frame_bound::FOLLOWING ||
    ...
    if (win_spec->window_frame && win_spec->window_frame->units == Window_frame::UNITS_ROWS)
    {
      Window_frame_bound *bounds[] = {win_spec->window_frame->top_bound, win_spec->window_frame->bottom_bound, NULL};
      for (Window_frame_bound **pbound = &bounds[0]; *pbound; pbound++)
      {
...
ORDER *st_select_lex::find_common_window_func_partition_fields(THD *thd)
{
  ...
  if (window_specs.elements == 1 && common_fields.elements == first_partition_elements)
  ...
  for (ord = first_wf->window_spec->partition_list->first, item = li++; ord; ord = ord->next)
  {
    if (item != *ord->item)
      continue;
...
static int compare_order_elements(ORDER *ord1, int weight1, ORDER *ord2, int weight2)
{
  ...
  {
    DBUG_ASSERT(((Item_field *)item1)->field->table == ((Item_field *)item2)->field->table);
    cmp = ((Item_field *)item1)->field->field_index - ((Item_field *)item2)->field->field_index;
  }
...
static int compare_order_lists(SQL_I_List<ORDER> *part_list1, int spec_number1, SQL_I_List<ORDER> *part_list2,
                               int spec_number2)
{
...
static int compare_window_frame_bounds(Window_frame_bound *win_frame_bound1, Window_frame_bound *win_frame_bound2,
                                       bool is_bottom_bound)
{
  int res;
  if (win_frame_bound1->precedence_type != win_frame_bound2->precedence_type)
  {
    res = win_frame_bound1->precedence_type > win_frame_bound2->precedence_type ? CMP_GT : CMP_LT;
    if (is_bottom_bound)
      res = -res;
    return res;
  }
  ...
  if (!win_frame_bound1->is_unbounded() && !win_frame_bound2->is_unbounded())
  {
    ...
    {
      res = strcmp(win_frame_bound1->offset->name.str, win_frame_bound2->offset->name.str);
...
static int compare_window_frames(Window_frame *win_frame1, Window_frame *win_frame2)
{
  int cmp;
  ...
  cmp = compare_window_frame_bounds(win_frame1->top_bound, win_frame2->top_bound, false);
  if (cmp)
    return cmp;

  cmp = compare_window_frame_bounds(win_frame1->bottom_bound, win_frame2->bottom_bound, true);
  if (cmp)
    return cmp;
...
static int compare_window_spec_joined_lists(Window_spec *win_spec1, Window_spec *win_spec2)
{
  win_spec1->join_partition_and_order_lists();
  win_spec2->join_partition_and_order_lists();
  int cmp = compare_order_lists(win_spec1->partition_list, win_spec1->win_spec_number, win_spec2->partition_list,
...
static int compare_window_funcs_by_window_specs(Item_window_func *win_func1, Item_window_func *win_func2, void *arg)
{
  int cmp;
  ...
  if (win_spec1 == win_spec2)
    return CMP_EQ;
  cmp = compare_order_lists(win_spec1->partition_list, win_spec1->win_spec_number, win_spec2->partition_list,
                            win_spec2->win_spec_number);
  if (cmp == CMP_EQ)
  {
    /*
    ...
    */
    ...
    cmp = compare_order_lists(win_spec1->order_list, win_spec1->win_spec_number, win_spec2->order_list,
                              win_spec2->win_spec_number);
    ...
    */
    ...
    cmp = compare_window_frames(win_spec1->window_frame, win_spec2->window_frame);
  ...
  if (CMP_LT_C <= cmp && cmp <= CMP_GT_C)
    cmp = win_spec1->partition_list->elements < win_spec2->partition_list->elements ? CMP_GT_C : CMP_LT_C;

  return cmp;
}

typedef int (*Item_window_func_cmp)(Item_window_func *f1, Item_window_func *f2, void *arg);
/*
  @brief
...
static void order_window_funcs_by_window_specs(List<Item_window_func> *win_func_list)
{
  if (win_func_list->elements == 0)
    return;

  bubble_sort<Item_window_func>(win_func_list, compare_window_funcs_by_window_specs, NULL);

  List_iterator_fast<Item_window_func> it(*win_func_list);
  Item_window_func *prev = it++;
  prev->marker = (MARKER_SORTORDER_CHANGE | MARKER_PARTITION_CHANGE | MARKER_FRAME_CHANGE);
  ...
  while ((curr = it++))
  {
          ...
          win_spec_prev->order_list == win_spec_curr->order_list))
    {
      int cmp;
      if (win_spec_prev->partition_list == win_spec_curr->partition_list)
        cmp = compare_order_lists(win_spec_prev->order_list, win_spec_prev->win_spec_number, win_spec_curr->order_list,
                                  win_spec_curr->win_spec_number);
      else
        cmp = compare_window_spec_joined_lists(win_spec_prev, win_spec_curr);
      if (!(CMP_LT_C <= cmp && cmp <= CMP_GT_C))
      {
        curr->marker = (MARKER_SORTORDER_CHANGE | MARKER_PARTITION_CHANGE | MARKER_FRAME_CHANGE);
      }
...
class Partition_read_cursor : public Table_read_cursor
{
  ...
  */
  int next()
  {
    ...
    if ((res = Table_read_cursor::next()) || (res = fetch()))
    {
...
class Frame_cursor : public Sql_alloc
{
 public:
  ...
  bool add_sum_func(Item_sum *item) { return sum_functions.push_back(item); }
  /*
...
{
 public:
  bool add_cursor(Frame_cursor *cursor) { return cursors.push_back(cursor); }

  void initialize_cursors(READ_RECORD *info)
  {
    List_iterator_fast<Frame_cursor> iter(cursors);
    Frame_cursor *fc;
    while ((fc = iter++)) fc->init(info);
  }
  ...
  void notify_cursors_partition_changed(ha_rows rownum)
  {
    List_iterator_fast<Frame_cursor> iter(cursors);
    Frame_cursor *cursor;
    while ((cursor = iter++)) cursor->pre_next_partition(rownum);

    iter.rewind();
    while ((cursor = iter++)) cursor->next_partition(rownum);
  }
  ...
  void notify_cursors_next_row()
  {
    List_iterator_fast<Frame_cursor> iter(cursors);
    Frame_cursor *cursor;
    while ((cursor = iter++)) cursor->pre_next_row();

    iter.rewind();
    while ((cursor = iter++)) cursor->next_row();
  }
...
class Frame_range_n_top : public Frame_cursor
{
 ...
 public:
  Frame_range_n_top(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list, bool is_preceding_arg,
                    Item *n_val_arg)
      : cursor(thd, partition_list), n_val(n_val_arg), item_add(NULL), is_preceding(is_preceding_arg)
  {
  ...
  void init(READ_RECORD *info) { cursor.init(info); }
  ...
  void next_partition(ha_rows rownum) { walk_till_non_peer(); }
  ...
  ha_rows get_curr_rownum() const { return cursor.get_rownum(); }
...
class Frame_range_n_bottom : public Frame_cursor
{
 ...
 public:
  Frame_range_n_bottom(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list,
...
class Frame_range_current_row_bottom : public Frame_cursor
{
 ...
 public:
  Frame_range_current_row_bottom(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list)
  ...
  void pre_next_row() { dont_move = !peer_tracker.check_if_next_group(); }
...
class Frame_range_current_row_top : public Frame_cursor
{
 ...
 public:
  Frame_range_current_row_top(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list)
      : bound_tracker(thd, partition_list), cursor(), peer_tracker(thd, order_list), move(false)
  {
  }
...
{
 public:
  Frame_unbounded_preceding(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list) {}
  ...
  void next_row() { /* Do nothing, UNBOUNDED PRECEDING frame end doesn't move. */ }

  ha_rows get_curr_rownum() const { return curr_rownum; }
...
class Frame_unbounded_following : public Frame_cursor
{
 ...
 public:
  Frame_unbounded_following(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list)
      : cursor(thd, partition_list)
  {
  }
  ...
  void pre_next_partition(ha_rows rownum) { cursor.on_next_partition(rownum); }
  ...
  void next_row() { /* Do nothing, UNBOUNDED FOLLOWING frame end doesn't move */ }
...
{
 public:
  Frame_unbounded_following_set_count(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list)
  ...
  void next_partition(ha_rows rownum)
  {
    ...
    while (!cursor.next()) num_rows_in_partition++;
 ...
 protected:
  void set_win_funcs_row_count(ha_rows num_rows_in_partition)
  {
    List_iterator_fast<Item_sum> it(sum_functions);
    Item_sum *item;
    while ((item = it++)) item->set_partition_row_count(num_rows_in_partition);
  }
};

class Frame_unbounded_following_set_count_no_nulls : public Frame_unbounded_following_set_count
{
 public:
  Frame_unbounded_following_set_count_no_nulls(THD *thd, SQL_I_List<ORDER> *partition_list,
...
{
 public:
  Frame_rows_current_row_top() : Frame_n_rows_preceding(true /*top*/, 0 /* n_rows */) {}
};
...
class Frame_n_rows_following : public Frame_cursor
{
 ...
 public:
  Frame_n_rows_following(THD *thd, SQL_I_List<ORDER> *partition_list, SQL_I_List<ORDER> *order_list,
                         bool is_top_bound_arg, ha_rows n_rows_arg)
      : is_top_bound(is_top_bound_arg), n_rows(n_rows_arg), cursor(thd, partition_list)
  {
  }
...
{
 public:
  Frame_scan_cursor(const Frame_cursor &top_bound, const Frame_cursor &bottom_bound)
  ...
  void next_partition(ha_rows rownum) { compute_values_for_current_row(); }

  void pre_next_row() { clear_sum_functions(); }
 ...
 private:
  ...
  {
    if (top_bound.is_outside_computation_bounds() || bottom_bound.is_outside_computation_bounds())
      return;
...
class Frame_positional_cursor : public Frame_cursor
{
 public:
  ...
  Frame_positional_cursor(const Frame_cursor &position_cursor, const Frame_cursor &top_bound,
                          const Frame_cursor &bottom_bound, Item &offset, bool negative_offset)
      : position_cursor(position_cursor),
        top_bound(&top_bound),
  ...
  void next_partition(ha_rows rownum) { save_positional_value(); }
  ...
  void next_row() { save_positional_value(); }

  ha_rows get_curr_rownum() const { return position_cursor.get_curr_rownum(); }
 ...
 private:
  ...
  bool position_is_within_bounds()
  {
    ...
    if (position_cursor.is_outside_computation_bounds() || top_bound->is_outside_computation_bounds() ||
  ...
  */
  void get_current_position()
  {
    ...
    if (offset)
    {
      if (offset_value < 0 && position + offset_value > position)
      {
        overflowed = true;
      }
      if (offset_value > 0 && position + offset_value < position)
...
Frame_cursor *get_frame_cursor(THD *thd, Window_spec *spec, bool is_top_bound)
{
  ...
  if (!frame)
  {
    ...
    */
    if (is_top_bound)
      return new Frame_unbounded_preceding(thd, spec->partition_list, spec->order_list);
    else
      return new Frame_range_current_row_bottom(thd, spec->partition_list, spec->order_list);
  }

  Window_frame_bound *bound = is_top_bound ? frame->top_bound : frame->bottom_bound;

  if (bound->precedence_type == Window_frame_bound::PRECEDING ||
      bound->precedence_type == Window_frame_bound::FOLLOWING)
  {
    bool is_preceding = (bound->precedence_type == Window_frame_bound::PRECEDING);
    ...
    if (bound->offset == NULL) /* this is UNBOUNDED */
    {
      ...
      return new Frame_unbounded_following(thd, spec->partition_list, spec->order_list);
    }
    ...
    if (frame->units == Window_frame::UNITS_ROWS)
    {
      ...
      return new Frame_n_rows_following(thd, spec->partition_list, spec->order_list, is_top_bound, n_rows);
    }
    else
    {
      if (is_top_bound)
        return new Frame_range_n_top(thd, spec->partition_list, spec->order_list, is_preceding, bound->offset);

      return new Frame_range_n_bottom(thd, spec->partition_list, spec->order_list, is_preceding, bound->offset);
    }
  }
  ...
  if (bound->precedence_type == Window_frame_bound::CURRENT)
  {
    ...
    {
      if (is_top_bound)
        return new Frame_range_current_row_top(thd, spec->partition_list, spec->order_list);
...
static void add_special_frame_cursors(THD *thd, Cursor_manager *cursor_manager, Item_window_func *window_func)
{
  ...
  switch (item_sum->sum_func())
  {
    case Item_sum::CUME_DIST_FUNC:
      fc = new Frame_unbounded_preceding(thd, spec->partition_list, spec->order_list);
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      fc = new Frame_range_current_row_bottom(thd, spec->partition_list, spec->order_list);
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      break;
    ...
    {
      Frame_cursor *bottom_bound = new Frame_unbounded_following(thd, spec->partition_list, spec->order_list);
      Frame_cursor *top_bound = new Frame_unbounded_preceding(thd, spec->partition_list, spec->order_list);
      ...
      fc = new Frame_positional_cursor(*current_row_pos, *top_bound, *bottom_bound, *item_sum->get_arg(1),
                                       negative_offset);
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      break;
    ...
    case Item_sum::FIRST_VALUE_FUNC:
    {
      ...
      fc = new Frame_positional_cursor(*top_bound, *top_bound, *bottom_bound, *offset_item, false);
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      break;
    }
    case Item_sum::LAST_VALUE_FUNC:
    {
      ...
      Item *offset_item = new (thd->mem_root) Item_int(thd, 0);
      offset_item->fix_fields(thd, &offset_item);
      fc = new Frame_positional_cursor(*bottom_bound, *top_bound, *bottom_bound, *offset_item, false);
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      break;
    }
    case Item_sum::NTH_VALUE_FUNC:
    {
      ...
      Item *offset_func = new (thd->mem_root) Item_func_minus(thd, item_sum->get_arg(1), int_item);
      offset_func->fix_fields(thd, &offset_func);
      fc = new Frame_positional_cursor(*top_bound, *top_bound, *bottom_bound, *offset_func, false);
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      break;
    }
    ...
    case Item_sum::PERCENTILE_DISC_FUNC:
    {
      ...
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      fc = new Frame_unbounded_following(thd, spec->partition_list, spec->order_list);
      fc->add_sum_func(item_sum);
      cursor_manager->add_cursor(fc);
      break;
    }
...
*/
void get_window_functions_required_cursors(THD *thd, List<Item_window_func> &window_functions,
  ...
  while ((item_win_func = it++))
  {
    ...
    */
    if (item_win_func->requires_partition_size())
    {
      ...
      {
        fc = new Frame_unbounded_following_set_count_no_nulls(thd, item_win_func->window_spec->partition_list,
                                                              item_win_func->window_spec->order_list);
      }
      else
      {
        fc = new Frame_unbounded_following_set_count(thd, item_win_func->window_spec->partition_list,
                                                     item_win_func->window_spec->order_list);
      }
    ...
    /*
      ...
      add to it.
    */
    if (item_win_func->is_frame_prohibited() || item_win_func->requires_special_cursors())
    {
    ...
    Frame_cursor *frame_bottom = get_frame_cursor(thd, item_win_func->window_spec, false);
    Frame_cursor *frame_top = get_frame_cursor(thd, item_win_func->window_spec, true);
    ...
    */
    ...
    if (is_computed_with_remove(sum_func->sum_func()) && !sum_func->supports_removal())
    {
      frame_bottom->set_no_action();
      frame_top->set_no_action();
      Frame_cursor *scan_cursor = new Frame_scan_cursor(*frame_top, *frame_bottom);
...
*/
static bool save_window_function_values(List<Item_window_func> &window_functions, TABLE *tbl, uchar *rowid_buf)
{
  ...
  while (Item_window_func *item_win = iter++) item_win->save_in_field(item_win->result_field, true);
...
*/
bool compute_window_func(THD *thd, List<Item_window_func> &window_functions, List<Cursor_manager> &cursor_managers,
                         TABLE *tbl, SORT_INFO *filesort_result)
{
  ...
  if (init_read_record(&info, current_thd, tbl, NULL /*select*/, filesort_result, 0, 1, FALSE))
    return true;

  Cursor_manager *cursor_manager;
  while ((cursor_manager = iter_cursor_managers++)) cursor_manager->initialize_cursors(&info);
  ...
  {
    Group_bound_tracker *tracker = new Group_bound_tracker(thd, win_func->window_spec->partition_list);
...
bool Window_func_runner::add_function_to_run(Item_window_func *win_func)
{
  ...
  switch (type)
  {
    ...
    case Item_sum::GROUP_CONCAT_FUNC:
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "GROUP_CONCAT() aggregate as window function");
      return true;
    case Item_sum::SUM_DISTINCT_FUNC:
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "SUM(DISTINCT) aggregate as window function");
      return true;
    case Item_sum::AVG_DISTINCT_FUNC:
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "AVG(DISTINCT) aggregate as window function");
      return true;
    case Item_sum::COUNT_DISTINCT_FUNC:
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "COUNT(DISTINCT) aggregate as window function");
      return true;
    case Item_sum::JSON_ARRAYAGG_FUNC:
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "JSON_ARRAYAGG() aggregate as window function");
      return true;
    case Item_sum::JSON_OBJECTAGG_FUNC:
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "JSON_OBJECTAGG() aggregate as window function");
      return true;
    default:
      break;
  }
...
bool Window_func_runner::exec(THD *thd, TABLE *tbl, SORT_INFO *filesort_result)
{
  ...
  bool is_error = compute_window_func(thd, window_functions, cursor_managers, tbl, filesort_result);
  while ((win_func = it++))
  {
...
bool Window_funcs_sort::setup(THD *thd, SQL_SELECT *sel, List_iterator<Item_window_func> &it, JOIN_TAB *join_tab)
{
  Window_spec *spec;
  ...
  do
  {
    ...
    int win_func_order_elements = spec->partition_list->elements + spec->order_list->elements;
  ...
  */
  ...
  ORDER *sort_order = concat_order_lists(thd->mem_root, spec->partition_list->first, spec->order_list->first);
  if (sort_order == NULL)  // No partition or order by clause.
  {
    ...
    Item_field *item = new (thd->mem_root) Item_field(thd, join_tab->table->field[0]);
    if (item)
...
bool Window_funcs_computation::setup(THD *thd, List<Item_window_func> *window_funcs, JOIN_TAB *tab)
{
  ...
  */
  ...
  while (iter.peek())
  {
    if (!(srt = new Window_funcs_sort()) || srt->setup(thd, sel, iter, tab))
    {
      return true;
    }
...
Explain_aggr_window_funcs *Window_funcs_computation::save_explain_plan(MEM_ROOT *mem_root, bool is_analyze)
{
  ...
  while ((srt = it++))
  {
    Explain_aggr_filesort *eaf = new Explain_aggr_filesort(mem_root, is_analyze, srt->filesort);
    if (!eaf)
      return 0;
