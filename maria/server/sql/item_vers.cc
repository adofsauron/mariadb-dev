...
Item_func_trt_ts::Item_func_trt_ts(THD *thd, Item *a, TR_table::field_id_t _trt_field)
    : Item_datetimefunc(thd, a), trt_field(_trt_field)
{
  decimals = 6;
  null_value = true;
...
bool Item_func_trt_ts::get_date(THD *thd, MYSQL_TIME *res, date_mode_t fuzzydate)
{
  DBUG_ASSERT(thd);
  DBUG_ASSERT(args[0]);
  if (args[0]->result_type() != INT_RESULT)
  {
    my_error(ER_ILLEGAL_PARAMETER_DATA_TYPE_FOR_OPERATION, MYF(0), args[0]->type_handler()->name().ptr(), func_name());
    return true;
  }
...
Item_func_trt_id::Item_func_trt_id(THD *thd, Item *a, TR_table::field_id_t _trt_field, bool _backwards)
    : Item_longlong_func(thd, a), trt_field(_trt_field), backwards(_backwards)
{
  decimals = 0;
  unsigned_flag = 1;
  null_value = true;
...
Item_func_trt_id::Item_func_trt_id(THD *thd, Item *a, Item *b, TR_table::field_id_t _trt_field)
    : Item_longlong_func(thd, a, b), trt_field(_trt_field), backwards(false)
{
  decimals = 0;
  unsigned_flag = 1;
  null_value = true;
...
longlong Item_func_trt_id::get_by_trx_id(ulonglong trx_id)
{
  THD *thd = current_thd;
  DBUG_ASSERT(thd);
...
longlong Item_func_trt_id::get_by_commit_ts(MYSQL_TIME &commit_ts, bool backwards)
{
  THD *thd = current_thd;
  DBUG_ASSERT(thd);
...
Item_func_trt_trx_sees::Item_func_trt_trx_sees(THD *thd, Item *a, Item *b) : Item_bool_func(thd, a, b), accept_eq(false)
{
  null_value = true;
  DBUG_ASSERT(arg_count == 2 && args[0] && args[1]);
}

longlong Item_func_trt_trx_sees::val_int()
{
  THD *thd = current_thd;
  DBUG_ASSERT(thd);
