...
String *Item_func_geometry_from_wkb::val_str(String *str)
{
  ...
  str->q_append(srid);
  if ((null_value = (args[0]->null_value || !Geometry::create_from_wkb(&buffer, wkb->ptr(), wkb->length(), str))))
    return 0;
  return str;
}
...
String *Item_func_geometry_from_json::val_str(String *str)
{
  ...
  if (arg_count > 1 && !args[1]->null_value)
  {
    ...
    if (options > 4 || options < 1)
    {
      ...
      my_error(ER_WRONG_VALUE_FOR_TYPE, MYF(0), "option", sv->c_ptr_safe(), "ST_GeomFromGeoJSON");
      null_value = 1;
      return 0;
    }
  }
  ...
  json_scan_start(&je, js->charset(), (const uchar *)js->ptr(), (const uchar *)js->end());
  ...
  if ((null_value = !Geometry::create_from_json(&buffer, &je, options == 1, str)))
  {
    ...
    {
      THD *thd = current_thd;
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, code, ER_THD(thd, code));
    }
    return 0;
  }
  return str;
...
String *Item_func_as_wkt::val_str_ascii(String *str)
{
  ...
  if ((null_value = (args[0]->null_value || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())))))
    return 0;
...
String *Item_func_as_wkb::val_str(String *str)
{
  ...
  if ((null_value = (args[0]->null_value || !(Geometry::construct(&buffer, swkb->ptr(), swkb->length())))))
    return 0;
...
String *Item_func_geometry_type::val_str_ascii(String *str)
{
  ...
  str->copy(geom->get_class_info()->m_name.str, geom->get_class_info()->m_name.length, &my_charset_latin1);
  return str;
}
...
int Item_func_boundary::Transporter::single_point(double x, double y) { return 0; }
...
int Item_func_convexhull::add_node_to_line(ch_node **p_cur, int dir, const Gcalc_heap::Info *pi)
{
  ch_node *new_node;
  ch_node *cur = *p_cur;
  ...
  while (cur->prev)
  {
    int v_sign = Gcalc_scan_iterator::point::cmp_dx_dy(cur->prev->pi, cur->pi, cur->pi, pi);
    if (v_sign * dir < 0)
      break;
    new_node = cur;
    cur = cur->prev;
...
String *Item_func_convexhull::val_str(String *str_value)
{
  ...
  left_cur->next = 0;
  if (left_first->get_next()->get_next() == NULL && right_cur->prev->prev == NULL)
  {
    ...
    if (res_receiver.start_shape(Gcalc_function::shape_line) ||
        ...
        res_receiver.add_point(left_cur->pi->node.shape.x, left_cur->pi->node.shape.y) || res_receiver.complete_shape())
...
String *Item_func_convexhull::val_str(String *str_value)
{
  ...
  {
    /* Single point. */
    if (res_receiver.single_point(scan_it.get_events()->pi->x, scan_it.get_events()->pi->y))
      goto mem_error;
    goto build_result;
  }
  ...
  while (scan_it.more_points())
  {
...

    {
      ...
      if (!pit.point() || pit.point()->event || scan_it.get_event_position() == pit.point()->c_get_next())
      {
  ...
  if (left_first->get_next()->get_next() == NULL && right_cur->prev->prev == NULL)
  {
    ...
    if (res_receiver.start_shape(Gcalc_function::shape_line) ||
        ...
        res_receiver.add_point(left_cur->pi->x, left_cur->pi->y) || res_receiver.complete_shape())
...
String *Item_func_point::val_str(String *str)
{
  ...
  if ((null_value =
           (args[0]->null_value || args[1]->null_value || str->alloc(4 /*SRID*/ + 1 + 4 + SIZEOF_STORED_DOUBLE * 2))))
    return 0;
...
SEL_ARG *Item_func_spatial_rel::get_mm_leaf(RANGE_OPT_PARAM *param, Field *field, KEY_PART *key_part,
...
longlong Item_func_spatial_mbr_rel::val_int()
{
  ...
  if ((null_value = (args[0]->null_value || args[1]->null_value ||
                     ...
                     !(g2 = Geometry::construct(&buffer2, res2->ptr(), res2->length())) || g1->get_mbr(&mbr1, &dummy) ||
                     !mbr1.valid() || g2->get_mbr(&mbr2, &dummy) || !mbr2.valid())))
    return 0;
...
static double count_edge_t(const Gcalc_heap::Info *ea, const Gcalc_heap::Info *eb, const Gcalc_heap::Info *v,
                           double &ex, double &ey, double &vx, double &vy, double &e_sqrlen)
{
...
static double distance_to_line(double ex, double ey, double vx, double vy, double e_sqrlen)
{
  return fabs(vx * ey - vy * ex) / sqrt(e_sqrlen);
}

static double distance_points(const Gcalc_heap::Info *a, const Gcalc_heap::Info *b)
{
...
static Gcalc_function::op_type op_matrix(int n)
{
  switch (n)
  {
    ...
    case 2:
      return (Gcalc_function::op_type)((int)Gcalc_function::op_not | (int)Gcalc_function::op_union);
  };
  GCALC_DBUG_ASSERT(FALSE);
  return Gcalc_function::op_any;
}

static int setup_relate_func(Geometry *g1, Geometry *g2, Gcalc_operation_transporter *trn, Gcalc_function *func,
                             const char *mask)
{
...
class Geometry_ptr_with_buffer_and_mbr
{
 public:
  ...
  bool construct(Item *item, String *tmp_value)
  {
    ...
    return item->null_value || !(geom = Geometry::construct(&buffer, res->ptr(), res->length())) ||
           geom->get_mbr(&mbr, &c_end) || !mbr.valid();
  }
  int store_shapes(Gcalc_shape_transporter *trn) const { return geom->store_shapes(trn); }
};
...
longlong Item_func_spatial_relate::val_int()
{
  ...
  if ((null_value =
           (g1.construct(args[0], &tmp_value1) || g2.construct(args[1], &tmp_value2) || func.reserve_op_buffer(1))))
    DBUG_RETURN(0);
  ...
  if ((null_value = args[2]->null_value || matrix->length() != 9 ||
                    setup_relate_func(g1.geom, g2.geom, &trn, &func, matrix->ptr())))
    goto exit;
  ...
  switch (spatial_rel)
  {
    case SP_CONTAINS_FUNC:
      if (!g1.mbr.contains(&g2.mbr))
        goto exit;
      func.add_operation(Gcalc_function::v_find_f | Gcalc_function::op_not | Gcalc_function::op_difference, 2);
    ...
    case SP_EQUALS_FUNC:
      if (!g1.mbr.contains(&g2.mbr))
        goto exit;
      func.add_operation(Gcalc_function::v_find_f | Gcalc_function::op_not | Gcalc_function::op_symdifference, 2);
      null_value = g1.store_shapes(&trn) || g2.store_shapes(&trn);
      break;
    case SP_DISJOINT_FUNC:
      func.add_operation(Gcalc_function::v_find_f | Gcalc_function::op_not | Gcalc_function::op_intersection, 2);
      null_value = g1.store_shapes(&trn) || g2.store_shapes(&trn);
      break;
    case SP_INTERSECTS_FUNC:
      if (!g1.mbr.intersects(&g2.mbr))
        goto exit;
      func.add_operation(Gcalc_function::v_find_t | Gcalc_function::op_intersection, 2);
      null_value = g1.store_shapes(&trn) || g2.store_shapes(&trn);
      break;
    case SP_OVERLAPS_FUNC:
    case SP_CROSSES_FUNC:
      ...
      if (func.reserve_op_buffer(7))
        break;
      func.add_operation(Gcalc_function::op_intersection, 2);
      func.add_operation(Gcalc_function::v_find_t | Gcalc_function::op_difference, 2);
      func.repeat_expression(shape_a);
      func.repeat_expression(shape_b);
      func.add_operation(Gcalc_function::v_find_t | Gcalc_function::op_difference, 2);
      func.repeat_expression(shape_b);
    ...
    case SP_TOUCHES_FUNC:
      ...
      shape_a = func.get_next_expression_pos();
      if ((null_value = g1.store_shapes(&trn)) || func.reserve_op_buffer(1))
        break;
      func.add_operation(Gcalc_function::op_internals, 1);
      shape_b = func.get_next_expression_pos();
      if ((null_value = g2.store_shapes(&trn)) || func.reserve_op_buffer(1))
        break;
      func.add_operation(Gcalc_function::v_find_t | Gcalc_function::op_intersection, 2);
      func.repeat_expression(shape_a);
      func.repeat_expression(shape_b);
...
String *Item_func_spatial_operation::val_str(String *str_value)
{
  ...
  if ((null_value = (g1.construct(args[0], &tmp_value1) || g2.construct(args[1], &tmp_value2))))
  {
    str_value = 0;
    goto exit;
  }
  ...
  if (operation.count_all(&collector) || operation.get_result(&res_receiver))
    goto exit;
...
static double n_sinus[SINUSES_CALCULATED + 1] = {0,
                                                 0.04906767432741802,
                                                 0.0980171403295606,
                                                 0.1467304744553618,
                                                 0.1950903220161283,
...
static int fill_half_circle(Gcalc_shape_transporter *trn, double x, double y, double ax, double ay)
{
  double n_sin, n_cos;
  double x_n, y_n;
...
static int fill_gap(Gcalc_shape_transporter *trn, double x, double y, double ax, double ay, double bx, double by,
                    double d, bool *empty_gap)
{
...
static void calculate_perpendicular(double x1, double y1, double x2, double y2, double d, double *ex, double *ey,
                                    double *px, double *py)
{
  double q;
  *ex = x1 - x2;
...
int Item_func_buffer::Transporter::add_edge_buffer(double x3, double y3, bool round_p1, bool round_p2)
{
  ...
  if (e1e2 < 0)
  {
    ...
    if (fill_gap(&trn, x2, y2, -p1_x, -p1_y, p2_x, p2_y, m_d, &empty_gap1) || trn.add_point(x2 + p2_x, y2 + p2_y) ||
  ...
  else
  {
    x_n = x2 - p2_x * cos1 - p2_y * sin1;
    y_n = y2 - p2_y * cos1 + p2_x * sin1;
    if (trn.add_point(x_n, y_n) || trn.add_point(x2 - p2_x, y2 - p2_y) ||
        fill_gap(&trn, x2, y2, -p2_x, -p2_y, p1_x, p1_y, m_d, &empty_gap2))
      return 1;
    empty_gap1 = false;
  }
  if ((!empty_gap2 && trn.add_point(x2 + p1_x, y2 + p1_y)) || trn.add_point(x1 + p1_x, y1 + p1_y))
    return 1;

  if (round_p1 && fill_half_circle(&trn, x1, y1, p1_x, p1_y))
    return 1;

  if (trn.add_point(x1 - p1_x, y1 - p1_y) || (!empty_gap1 && trn.add_point(x2 - p1_x, y2 - p1_y)))
    return 1;
...
int Item_func_buffer::Transporter::add_last_edge_buffer()
{
  ...
  if (trn.add_point(x1 + p1_x, y1 + p1_y) || trn.add_point(x1 - p1_x, y1 - p1_y) ||
      trn.add_point(x2 - p1_x, y2 - p1_y) || fill_half_circle(&trn, x2, y2, -p1_x, -p1_y) ||
...
int Item_func_buffer::Transporter::add_point_buffer(double x, double y)
{
  ...
  if (trn.start_simple_poly())
    return 1;
  if (trn.add_point(x - m_d, y) || fill_half_circle(&trn, x, y, -m_d, 0.0) || trn.add_point(x + m_d, y) ||
...
{
  return complete() || Gcalc_operation_transporter::complete_ring();
}
...
String *Item_func_buffer::val_str(String *str_value)
{
  ...
  null_value = 1;
  if (args[0]->null_value || args[1]->null_value || !(g = Geometry::construct(&buffer, obj->ptr(), obj->length())) ||
      g->get_mbr(&mbr, &c_end))
    goto mem_error;
...
longlong Item_func_isempty::val_int()
{
  ...
  null_value = args[0]->null_value || !(Geometry::construct(&buffer, swkb->ptr(), swkb->length()));
...
longlong Item_func_issimple::val_int()
{
  ...
  while (scan_it.more_points())
  {
    ...
    if ((next_ev && !next_ev->get_next()) && (ev->event & (scev_thread | scev_end)) &&
...
longlong Item_func_isclosed::val_int()
{
  ...
  null_value = 0;
  if (!swkb || args[0]->null_value || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) ||
...
longlong Item_func_dimension::val_int()
{
  ...
  null_value = (!swkb || args[0]->null_value || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) ||
...
longlong Item_func_numinteriorring::val_int()
{
  ...
  null_value =
      (!swkb || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) || geom->num_interior_ring(&num));
  return (longlong)num;
}
...
longlong Item_func_numgeometries::val_int()
{
  ...
  null_value =
      (!swkb || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) || geom->num_geometries(&num));
  return (longlong)num;
}
...
double Item_func_x::val_real()
{
  ...
  null_value = (!swkb || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) || geom->get_x(&res));
  return res;
}
...
double Item_func_y::val_real()
{
  ...
  null_value = (!swkb || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) || geom->get_y(&res));
  return res;
}
...
double Item_func_area::val_real()
{
  ...
  null_value =
      (!swkb || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) || geom->area(&res, &dummy));
  return res;
}
...
double Item_func_glength::val_real()
{
  ...
  null_value =
      (!swkb || !(geom = Geometry::construct(&buffer, swkb->ptr(), swkb->length())) || geom->geom_length(&res, &end));
  return res;
}
...
longlong Item_func_srid::val_int()
{
  ...
  null_value = (!swkb || !Geometry::construct(&buffer, swkb->ptr(), swkb->length()));
  if (null_value)
    return 0;
...
double Item_func_distance::val_real()
{
  ...
  {
    if (((Gis_point *)g1)->get_xy(&x1, &y1) || ((Gis_point *)g2)->get_xy(&x2, &y2))
      goto mem_error;
    ex = x2 - x1;
    ey = y2 - y1;
  ...
  count_distance:
    ...
    for (dist_point = collector.get_first(); dist_point; dist_point = dist_point->get_next())
    {
      ...
      if (dist_point->type != Gcalc_heap::nt_shape_node || dist_point->node.shape.shape < obj2_si)
        continue;

      /* if we have an edge to check */
      if (dist_point->node.shape.left)
      {
        t = count_edge_t(dist_point, dist_point->node.shape.left, cur_point, ex, ey, vx, vy, e_sqrlen);
        if ((t > 0.0) && (t < 1.0))
        {
      ...
      if (cur_point_edge)
      {
        t = count_edge_t(cur_point, cur_point->node.shape.left, dist_point, ex, ey, vx, vy, e_sqrlen);
        if ((t > 0.0) && (t < 1.0))
        {
          cur_distance = distance_to_line(ex, ey, vx, vy, e_sqrlen);
          if (distance > cur_distance)
...
double Item_func_sphere_distance::spherical_distance_points(Geometry *g1, Geometry *g2, const double r)
{
  double res = 0.0;
      ...
      else
      {
        ...
        else
        {
          ...
          if (g1->get_data_size() != GET_SIZE_ERROR)
            static_cast<Gis_point *>(g2)->spherical_distance_multipoints((Gis_multi_point *)g1, r, &res, &error);
        }
      }
      break;
      ...
      if (g1->get_class_info()->m_type_id == Geometry::wkb_point)
      {
        ...
        else
        {
          if (g2->get_data_size() != GET_SIZE_ERROR)
            ...
            static_cast<Gis_point *>(g1)->spherical_distance_multipoints((Gis_multi_point *)g2, r, &res, &error);
        }
      }
      else
      {
        // Multipoints in g1 and g2 - no optimization
        static_cast<Gis_multi_point *>(g1)->spherical_distance_multipoints((Gis_multi_point *)g2, r, &res, &error);
      }
      break;
...
handle_error:
  if (error > 0)
    my_error(ER_STD_OUT_OF_RANGE_ERROR, MYF(0), "Longitude should be [-180,180]", "ST_Distance_Sphere");
  else if (error < 0)
    my_error(ER_STD_OUT_OF_RANGE_ERROR, MYF(0), "Latitude should be [-90,90]", "ST_Distance_Sphere");
  return res;
}
...
String *Item_func_pointonsurface::val_str(String *str)
{
  ...
  null_value = 1;
  if ((args[0]->null_value || !(g = Geometry::construct(&buffer, res->ptr(), res->length())) ||
       g->get_mbr(&mbr, &c_end)))
    goto mem_error;
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_area(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_as_wkb(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_as_wkt(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_centroid(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_convexhull(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_pointonsurface(thd, arg1); }
...
class Create_func_mbr_contains : public Create_func_arg2
{
 public:
  ...
  {
    return new (thd->mem_root) Item_func_spatial_mbr_rel(thd, arg1, arg2, Item_func::SP_CONTAINS_FUNC);
  }
...
class Create_func_contains : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_CONTAINS_FUNC);
  }
...
class Create_func_crosses : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_CROSSES_FUNC);
  }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_dimension(thd, arg1); }
...
class Create_func_mbr_disjoint : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_mbr_rel(thd, arg1, arg2, Item_func::SP_DISJOINT_FUNC);
  }
...
class Create_func_disjoint : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_DISJOINT_FUNC);
  }
...
Item *Create_func_distance_sphere::create_native(THD *thd, const LEX_CSTRING *name, List<Item> *item_list)
{
  int arg_count = 0;
...
class Create_func_endpoint : public Create_func_arg1
{
 public:
  ...
  {
    return new (thd->mem_root) Item_func_spatial_decomp(thd, arg1, Item_func::SP_ENDPOINT);
  }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_envelope(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_boundary(thd, arg1); }
...
class Create_func_mbr_equals : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_mbr_rel(thd, arg1, arg2, Item_func::SP_EQUALS_FUNC);
  }
...
class Create_func_equals : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_EQUALS_FUNC);
  }
...
class Create_func_exteriorring : public Create_func_arg1
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override
  {
    return new (thd->mem_root) Item_func_spatial_decomp(thd, arg1, Item_func::SP_EXTERIORRING);
  }
...
Item *Create_func_geometry_from_text::create_native(THD *thd, const LEX_CSTRING *name, List<Item> *item_list)
{
  Item *func = NULL;
  int arg_count = 0;
...
Item *Create_func_geometry_from_wkb::create_native(THD *thd, const LEX_CSTRING *name, List<Item> *item_list)
{
  Item *func = NULL;
  int arg_count = 0;
...
Item *Create_func_geometry_from_json::create_native(THD *thd, const LEX_CSTRING *name, List<Item> *item_list)
{
  Item *func = NULL;
  int arg_count = 0;
  ...
  switch (arg_count)
  {
    ...
    case 3:
    {
      ...
      func = new (thd->mem_root) Item_func_geometry_from_json(thd, json, options, srid);
      break;
    }
    default:
    {
...
Item *Create_func_as_geojson::create_native(THD *thd, const LEX_CSTRING *name, List<Item> *item_list)
{
  Item *func = NULL;
  int arg_count = 0;
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_geometry_type(thd, arg1); }
...
class Create_func_geometryn : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_decomp_n(thd, arg1, arg2, Item_func::SP_GEOMETRYN);
  }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_gis_debug(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_glength(thd, arg1); }
...
class Create_func_interiorringn : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_decomp_n(thd, arg1, arg2, Item_func::SP_INTERIORRINGN);
  }
...
class Create_func_mbr_intersects : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_mbr_rel(thd, arg1, arg2, Item_func::SP_INTERSECTS_FUNC);
  }
...
class Create_func_intersects : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_INTERSECTS_FUNC);
  }
...
class Create_func_intersection : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_operation(thd, arg1, arg2, Gcalc_function::op_intersection);
  }
...
class Create_func_difference : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_operation(thd, arg1, arg2, Gcalc_function::op_difference);
  }
...
class Create_func_union : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_operation(thd, arg1, arg2, Gcalc_function::op_union);
  }
...
class Create_func_symdifference : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_operation(thd, arg1, arg2, Gcalc_function::op_symdifference);
  }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_isclosed(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_isring(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_isempty(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_issimple(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_numgeometries(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_numinteriorring(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_numpoints(thd, arg1); }
...
class Create_func_mbr_overlaps : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_mbr_rel(thd, arg1, arg2, Item_func::SP_OVERLAPS_FUNC);
  }
...
class Create_func_overlaps : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_OVERLAPS_FUNC);
  }
...
class Create_func_pointn : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_decomp_n(thd, arg1, arg2, Item_func::SP_POINTN);
  }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_srid(thd, arg1); }
...
class Create_func_startpoint : public Create_func_arg1
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override
  {
    return new (thd->mem_root) Item_func_spatial_decomp(thd, arg1, Item_func::SP_STARTPOINT);
  }
...
class Create_func_touches : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_TOUCHES_FUNC);
  }
...
class Create_func_mbr_within : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_mbr_rel(thd, arg1, arg2, Item_func::SP_WITHIN_FUNC);
  }
...
class Create_func_within : public Create_func_arg2
{
 public:
  Item *create_2_arg(THD *thd, Item *arg1, Item *arg2) override
  {
    return new (thd->mem_root) Item_func_spatial_precise_rel(thd, arg1, arg2, Item_func::SP_WITHIN_FUNC);
  }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_x(thd, arg1); }
...
{
 public:
  Item *create_1_arg(THD *thd, Item *arg1) override { return new (thd->mem_root) Item_func_y(thd, arg1); }
...
Native_func_registry_array native_func_registry_array_geom(func_array_geom, array_elements(func_array_geom));
