...
{
  return MY_MAX(1024, (size_t)MY_MIN(thd->variables.tmp_memory_table_size, thd->variables.max_heap_table_size));
}
...
static void store_bit_fields_as_bigint_in_tempory_table(List<Item> *list)
{
  ...
  while ((item = li++))
  {
    if (item->type() == Item::FIELD_ITEM && ((Item_field *)item)->field->type() == FIELD_TYPE_BIT)
...
bool Item_sum::init_sum_func_check(THD *thd)
{
  ...
  if (!curr_sel || !(thd->lex->allow_sum_func.is_overlapping(curr_sel->name_visibility_map)))
  {
    my_message(ER_INVALID_GROUP_FUNC_USE, ER_THD(thd, ER_INVALID_GROUP_FUNC_USE), MYF(0));
    return TRUE;
  }
...
bool Item_sum::check_sum_func(THD *thd, Item **ref)
{
  ...
  */
  if (with_window_func())
  {
    my_message(ER_SUM_FUNC_WITH_WINDOW_FUNC_AS_ARG, ER_THD(thd, ER_SUM_FUNC_WITH_WINDOW_FUNC_AS_ARG), MYF(0));
    return TRUE;
  }
  ...
  */
  ...
  else if (max_arg_level >= 0 || !(allow_sum_func.is_set(nest_level)))
  {
    /*
    ...
    */
    if (register_sum_func(thd, ref))
      return TRUE;
    invalid = aggr_level < 0 && !(allow_sum_func.is_set(nest_level));
  ...
  if (outer_fields.elements)
  {
    ...
    */
    ...
    while ((field = of++))
    {
      ...
      if (sel->nest_level > aggr_level && (sel->agg_func_used()) && !sel->group_list.elements)
      {
...
bool Item_sum::register_sum_func(THD *thd, Item **ref)
{
  SELECT_LEX *sl;
  nesting_map allow_sum_func = thd->lex->allow_sum_func;
  for (sl = thd->lex->current_select->context.outer_select(); sl && sl->nest_level > max_arg_level;
       sl = sl->context.outer_select())
  {
    if (aggr_level < 0 && (allow_sum_func.is_set(sl->nest_level)))
    {
  ...
  if (aggr_level >= 0)
  {
    ...
    */
    for (sl = thd->lex->current_select; sl && sl != aggr_sel && sl->master_unit()->item;
  ...
  {
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_WARN_AGGFUNC_DEPENDENCE,
                        ER_THD(thd, ER_WARN_AGGFUNC_DEPENDENCE), func_name(), thd->lex->current_select->select_number,
...
bool Item_sum::collect_outer_ref_processor(void *param)
{
  ...
  SELECT_LEX *ds;
  if ((ds = depended_from()) && ds->nest_level_base == prm->nest_level_base && ds->nest_level < prm->nest_level)
  {
    if (prm->collect)
  ...
  if (sum_item && sum_item->result_field)  // If not a const sum func
  {
    ...
    for (uint i = 0; i < sum_item->arg_count; i++)
    {
      ...
      if (!arg->const_item())
      {
        ...
        {
          auto item_field = new (thd->mem_root) Item_field(thd, result_field_tmp++);
          if (item_field)
...
Item *Item_sum::result_item(THD *thd, Field *field) { return new (thd->mem_root) Item_field(thd, field); }

bool Item_sum::check_vcol_func_processor(void *arg)
{
  return mark_unsupported_function(func_name(), is_aggr_sum_func() ? ")" : "()", arg, VCOL_IMPOSSIBLE);
}
...
int simple_raw_key_cmp(void *arg, const void *key1, const void *key2) { return memcmp(key1, key2, *(uint *)arg); }

static int item_sum_distinct_walk_for_count(void *element, element_count num_of_dups, void *item)
{
...
bool Aggregator_distinct::setup(THD *thd)
{
  ...
  */
  ...
  if (item_sum->setup(thd))
    return TRUE;
  if (item_sum->sum_func() == Item_sum::COUNT_FUNC || item_sum->sum_func() == Item_sum::COUNT_DISTINCT_FUNC)
  {
    List<Item> list;
    ...
    */
    ...
    if (!(table = create_tmp_table(thd, tmp_table_param, list, (ORDER *)0, 1, 0,
                                   (select_lex->options | thd->variables.option_bits), HA_POS_ERROR, &empty_clex_str)))
      return TRUE;
      ...
      */
      ...
      for (tree_key_length = 0; field < field_end; ++field)
      {
        ...
        if ((type == MYSQL_TYPE_VARCHAR) ||
            (!f->binary() && (type == MYSQL_TYPE_STRING || type == MYSQL_TYPE_VAR_STRING)))
        {
          all_binary = FALSE;
          break;
        }
      ...
      DBUG_ASSERT(tree == 0);
      tree = (new (thd->mem_root) Unique(compare_key, cmp_arg, tree_key_length, item_sum->ram_limitation(thd)));
      /*
  ...
  else
  {
    ...
    */
    ...
    Field *field = arg->type_handler()->make_num_distinct_aggregator_field(thd->mem_root, arg);
    ...
    */
    tree = (new (thd->mem_root)
                Unique(simple_raw_key_cmp, &tree_key_length, tree_key_length, item_sum->ram_limitation(thd)));
...
bool Item_sum_min_max::fix_fields(THD *thd, Item **ref)
{
...
bool Item_sum_hybrid::fix_length_and_dec_string()
{
  ...
  else
  {
    ...
    set_handler(type_handler_varchar.type_handler_adjusted_to_max_octet_length(max_length, collation.collation));
  }
  return false;
}
...
Field *Item_sum_min_max::create_tmp_field(MEM_ROOT *root, bool group, TABLE *table)
{
...
Item_sum_sp::Item_sum_sp(THD *thd, Name_resolution_context *context_arg, sp_name *name_arg, sp_head *sp,
                         List<Item> &list)
...
Item_sum_sp::Item_sum_sp(THD *thd, Item_sum_sp *item) : Item_sum(thd, item), Item_sp(thd, item)
{
  base_flags |= (item->base_flags & item_base_t::MAYBE_NULL);
  quick_group = item->quick_group;
}

bool Item_sum_sp::fix_fields(THD *thd, Item **ref)
{
  DBUG_ASSERT(fixed() == 0);
  if (init_sum_func_check(thd))
    return TRUE;
...
bool Item_sum_sp::add() { return execute_impl(current_thd, args, arg_count); }
...
bool Item_sum_sp::fix_length_and_dec(THD *thd)
{
...
LEX_CSTRING Item_sum_sp::func_name_cstring() const { return Item_sp::func_name_cstring(current_thd, false); }

Item *Item_sum_sp::copy_or_same(THD *thd)
{
  Item_sum_sp *copy_item = new (thd->mem_root) Item_sum_sp(thd, this);
  copy_item->init_result_field(thd, max_length, maybe_null(), &copy_item->null_value, &copy_item->name);
  return copy_item;
}
...
bool Item_sum_sum::fix_length_and_dec(THD *thd)
{
  ...
  set_maybe_null();
  null_value = 1;
  if (args[0]->cast_to_int_type_handler()->Item_sum_sum_fix_length_and_dec(this))
    DBUG_RETURN(TRUE);
  DBUG_PRINT("info", ("Type: %s (%d, %d)", type_handler()->name().ptr(), max_length, (int)decimals));
  DBUG_RETURN(FALSE);
}
...
void Item_sum_sum::add_helper(bool perform_removal)
{
  ...
  if (result_type() == DECIMAL_RESULT)
  {
    if (unlikely(direct_added))
    {
      ...
      {
        my_decimal_add(E_DEC_FATAL_ERROR, dec_buffs + (curr_dec_buff ^ 1), &direct_sum_decimal,
    ...
    else
    {
      ...
      if (!aggr->arg_is_null(true))
      {
        if (perform_removal)
        {
          if (count > 0)
          {
            my_decimal_sub(E_DEC_FATAL_ERROR, dec_buffs + (curr_dec_buff ^ 1), dec_buffs + curr_dec_buff, val);
            count--;
          }
          else
            DBUG_VOID_RETURN;
...
{
  return use_distinct_values ? table->field[0]->val_decimal(value) : item_sum->args[0]->val_decimal(value);
}

double Aggregator_distinct::arg_val_real()
{
  return use_distinct_values ? table->field[0]->val_real() : item_sum->args[0]->val_real();
}
...
void Item_sum_avg::fix_length_and_dec_double()
{
  ...
  decimals = MY_MIN(args[0]->decimals + prec_increment, FLOATING_POINT_DECIMALS);
...
bool Item_sum_avg::fix_length_and_dec(THD *thd)
{
  ...
  set_maybe_null();
  null_value = 1;
  if (args[0]->cast_to_int_type_handler()->Item_sum_avg_fix_length_and_dec(this))
    DBUG_RETURN(TRUE);
  DBUG_PRINT("info", ("Type: %s (%d, %d)", type_handler()->name().ptr(), max_length, (int)decimals));
  DBUG_RETURN(FALSE);
}
...
Field *Item_sum_avg::create_tmp_field(MEM_ROOT *root, bool group, TABLE *table)
{
  if (group)
  {
    ...
    */
    Field *field =
        new (root) Field_string(((result_type() == DECIMAL_RESULT) ? dec_bin_size : sizeof(double)) + sizeof(longlong),
...
Item *Item_sum_std::result_item(THD *thd, Field *field) { return new (thd->mem_root) Item_std_field(thd, this); }
...
Item_sum_variance::Item_sum_variance(THD *thd, Item_sum_variance *item)
    : Item_sum_double(thd, item), m_stddev(item->m_stddev), sample(item->sample), prec_increment(item->prec_increment)
{
}
...
void Item_sum_variance::fix_length_and_dec_decimal()
{
  ...
  decimals = MY_MIN(args[0]->decimals + prec_increment, FLOATING_POINT_DECIMALS - 1);
  max_length = my_decimal_precision_to_length_no_truncation(precision, decimals, unsigned_flag);
}
...
/**
  ...
  pass around.
*/
Field *Item_sum_variance::create_tmp_field(MEM_ROOT *root, bool group, TABLE *table)
{
  Field *field;
  if (group)
  {
    /*
      ...
      and unpack on access.
    */
    field = new (root) Field_string(Stddev::binary_size(), 0, &name, &my_charset_bin);
  }
  else
...
void Item_sum_variance::clear() { m_stddev = Stddev(); }
...
bool Item_sum_min_max::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  DBUG_ASSERT(fixed());
  if (null_value)
    return true;
...
String *Item_sum_min_max::val_str(String *str)
{
...
{
  return result_type() == DECIMAL_RESULT ? (Item_avg_field *)new (thd->mem_root) Item_avg_field_decimal(thd, this)
...
void Arg_comparator::min_max_update_field_native(THD *thd, Field *field, Item *item, int cmp_sign)
{
...
void Item_sum_min_max::min_max_update_native_field()
{
...
void Item_sum_min_max::min_max_update_str_field()
{
...
void Item_sum_min_max::min_max_update_real_field()
{
  double nr, old_nr;
...
void Item_sum_min_max::min_max_update_int_field()
{
  longlong nr, old_nr;
...
*/
void Item_sum_min_max::min_max_update_decimal_field()
{
...
double Item_sum_udf_float::val_real()
{
  ...
  DBUG_PRINT("enter", ("result_type: %d  arg_count: %d", args[0]->result_type(), arg_count));
...
extern "C" int group_concat_key_cmp_with_distinct(void *arg, const void *key1, const void *key2)
{
  ...
  for (uint i = 0; i < item_func->arg_count_field; i++)
  {
    ...
    */
    ...
    uint offset = (field->offset(field->table->record[0]) - field->table->s->null_bytes);
...
int group_concat_key_cmp_with_distinct_with_nulls(void *arg, const void *key1_arg, const void *key2_arg)
{
  Item_func_group_concat *item_func = (Item_func_group_concat *)arg;
  ...
  */
  ...
  if (field->is_null_in_record((uchar *)key1_arg) && field->is_null_in_record((uchar *)key2_arg))
    return 0;
...
extern "C" int group_concat_key_cmp_with_order(void *arg, const void *key1, const void *key2)
{
...
int group_concat_key_cmp_with_order_with_nulls(void *arg, const void *key1_arg, const void *key2_arg)
{
  Item_func_group_concat *grp_item = (Item_func_group_concat *)arg;
  ORDER **order_item, **end;
...
static void report_cut_value_error(THD *thd, uint row_count, const char *fname)
{
  ...
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_CUT_VALUE_GROUP_CONCAT,
                      ER_THD(thd, ER_CUT_VALUE_GROUP_CONCAT), row_count, fname_upper);
  my_afree(fname_upper);
}

void Item_func_group_concat::cut_max_length(String *result, uint old_length, uint max_length) const
{
  ...
  */
  size_t add_length =
      Well_formed_prefix(collation.collation, ptr + old_length, ptr + max_length, result->length()).length();
...
extern "C" int dump_leaf_key(void *key_arg, element_count count __attribute__((unused)), void *item_arg)
{
  ...
  for (; arg < arg_end; arg++)
  {
    ...
    else
    {
      ...
      if (field)
      {
        ...
        res = item->get_str_from_field(*arg, field, &tmp, key, offset + item->get_null_bytes());
      }
      else
        res = item->get_str_from_item(*arg, &tmp);
    }
...
Item_func_group_concat::Item_func_group_concat(THD *thd, Name_resolution_context *context_arg, bool distinct_arg,
                                               List<Item> *select_list, const SQL_I_List<ORDER> &order_list,
                                               String *separator_arg, bool limit_clause, Item *row_limit_arg,
                                               Item *offset_limit_arg)
    : Item_sum(thd),
      tmp_table_param(0),
      ...
      original(0)
{
  ...
  */
  if (!(args = (Item **)thd->alloc(sizeof(Item *) * arg_count * 2 + sizeof(ORDER *) * arg_count_order)))
    return;
  ...
  if (arg_count_order)
  {
    ...
    for (ORDER *order_item = order_list.first; order_item != NULL; order_item = order_item->next)
    {
...
Item_func_group_concat::Item_func_group_concat(THD *thd, Item_func_group_concat *item)
    : Item_sum(thd, item),
      ...
      offset_limit(item->offset_limit),
      limit_clause(item->limit_clause),
      copy_offset_limit(item->copy_offset_limit),
      copy_row_limit(item->copy_row_limit),
      original(item)
{
  quick_group = item->quick_group;
  ...
  */
  ORDER *tmp;
  if (!(tmp = (ORDER *)thd->alloc(sizeof(ORDER *) * arg_count_order + sizeof(ORDER) * arg_count_order)))
    return;
...
extern "C" int copy_to_tree(void *key, element_count count __attribute__((unused)), void *arg)
{
...
bool Item_func_group_concat::repack_tree(THD *thd)
{
  ...
  init_tree(&st.tree, (size_t)MY_MIN(thd->variables.max_heap_table_size, thd->variables.sortbuff_size / 16), 0, size,
            get_comparator_function_for_order_by(), NULL, (void *)this, MYF(MY_THREAD_SPECIFIC));
...
bool Item_func_group_concat::add(bool exclude_nulls)
{
  ...
  for (uint i = 0; i < arg_count_field; i++)
  {
    ...
    if (field)
    {
      if (field->is_null_in_record((const uchar *)table->record[0]) && exclude_nulls)
  ...
  if (row_eligible && tree)
  {
    ...
    if ((tree_len >> GCONCAT_REPACK_FACTOR) > thd->variables.group_concat_max_len && tree->elements_in_tree > 1)
      if (repack_tree(thd))
        return 1;
...
bool Item_func_group_concat::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  ...
  if (agg_arg_charsets_for_string_result(collation, args, arg_count - arg_count_order))
    return 1;

  result.set_charset(collation.collation);
  result_field = 0;
  null_value = 1;
  max_length = (uint32)MY_MIN(
      (ulonglong)thd->variables.group_concat_max_len / collation.collation->mbminlen * collation.collation->mbmaxlen,
      UINT_MAX32);

  uint32 offset;
  if (separator->needs_conversion(separator->length(), separator->charset(), collation.collation, &offset))
  {
    ...
    if (!(buf = (char *)thd->stmt_arena->alloc(buflen)) ||
        !(new_separator = new (thd->stmt_arena->mem_root) String(buf, buflen, collation.collation)))
      return TRUE;

    conv_length = copy_and_convert(buf, buflen, collation.collation, separator->ptr(), separator->length(),
...
bool Item_func_group_concat::setup(THD *thd)
{
  ...
  */
  if (arg_count_order)
  {
    ...
    DBUG_ASSERT(context);
    if (setup_order(thd, Ref_ptr_array(ref_pointer_array, n_elems), context->table_list, list, all_fields, *order))
      DBUG_RETURN(TRUE);
    /*
    ...
    */
    Item *item = new (thd->mem_root) Item_uint(thd, thd->variables.group_concat_max_len);
  ...
  */
  ...
  if (arg_count_order)
  {
    ...
    /*
      ...
      create this tree.
    */
    init_tree(tree, (size_t)MY_MIN(thd->variables.max_heap_table_size, thd->variables.sortbuff_size / 16), 0,
              tree_key_length + get_null_bytes(), get_comparator_function_for_order_by(), NULL, (void *)this,
              MYF(MY_THREAD_SPECIFIC));
    tree_len = 0;
  }

  if (distinct)
    unique_filter = (new (thd->mem_root) Unique(get_comparator_function_for_distinct(), (void *)this,
                                                tree_key_length + get_null_bytes(), ram_limitation(thd)));
  if ((row_limit && row_limit->cmp_type() != INT_RESULT) || (offset_limit && offset_limit->cmp_type() != INT_RESULT))
  {
...
String *Item_func_group_concat::val_str(String *str)
{
  ...
  if (table && table->blob_storage && table->blob_storage->is_truncated_value())
  {
...
{
  return skip_nulls() ? group_concat_key_cmp_with_distinct : group_concat_key_cmp_with_distinct_with_nulls;
}
...
{
  return skip_nulls() ? group_concat_key_cmp_with_order : group_concat_key_cmp_with_order_with_nulls;
}
...
{
  return skip_nulls() ? table->record[0] + table->s->null_bytes : table->record[0];
}
...
uint Item_func_group_concat::get_null_bytes() { return skip_nulls() ? 0 : table->s->null_bytes; }
