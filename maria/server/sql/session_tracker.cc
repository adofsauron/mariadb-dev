...
{
  sysvar_node_st *node;
  if (!(node = (sysvar_node_st *)my_malloc(PSI_INSTRUMENT_ME, sizeof(sysvar_node_st),
                                           MYF(MY_WME | (mysqld_server_initialized ? MY_THREAD_SPECIFIC : 0)))))
    return true;
...
/**
  ...
  @return
    ...
    false                   Success
*/
bool Session_sysvars_tracker::vars_list::parse_var_list(THD *thd, LEX_STRING var_list, bool throw_error,
                                                        CHARSET_INFO *char_set)
{
  ...
  for (;;)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WRONG_VALUE_FOR_VAR,
                          "%.*s is not a valid system variable and will"
                          "be ignored.",
                          (int)var.length, token);
    }
    else
      return true;
...
bool sysvartrack_validate_value(THD *thd, const char *str, size_t len)
{
  ...
  if (!var_list.str)
  {
    my_error(ER_WRONG_VALUE_FOR_VAR, MYF(0), "session_track_system_variables", "NULL");
    return false;
  }
  if (var_list.length == 0 || !strcmp(var_list.str, "*"))
  {
    return false;
  }
...
bool Session_sysvars_tracker::vars_list::construct_var_list(char *buf, size_t buf_len)
{
  LEX_CSTRING **names;
  uint idx;
  size_t left = buf_len;
...
void Session_sysvars_tracker::init(THD *thd)
{
  ...
  DBUG_ASSERT(thd->variables.session_track_system_variables == global_system_variables.session_track_system_variables);
  DBUG_ASSERT(global_system_variables.session_track_system_variables);
  thd->variables.session_track_system_variables = my_strdup(
      PSI_INSTRUMENT_ME, global_system_variables.session_track_system_variables, MYF(MY_WME | MY_THREAD_SPECIFIC));
}
...
bool Session_sysvars_tracker::enable(THD *thd)
{
  orig_list.reinit();
  m_parsed = false;
  m_enabled = thd->variables.session_track_system_variables && *thd->variables.session_track_system_variables;
  reset_changed();
  return false;
}
...
bool Session_sysvars_tracker::update(THD *thd, set_var *var)
{
  ...
  void *copy = var->save_result.string_value.str
                   ? my_memdup(PSI_INSTRUMENT_ME, var->save_result.string_value.str,
                               (length = var->save_result.string_value.length + 1), MYF(MY_WME | MY_THREAD_SPECIFIC))
                   : my_strdup(PSI_INSTRUMENT_ME, "", MYF(MY_WME | MY_THREAD_SPECIFIC));
  if (!copy)
    return true;

  if (tool_list.parse_var_list(thd, var->save_result.string_value, true, thd->charset()))
  {
    my_free(copy);
    return true;
  }
  ...
  orig_list.construct_var_list(thd->variables.session_track_system_variables, length);
  return false;
}
...
bool Session_sysvars_tracker::vars_list::store(THD *thd, String *buf)
{
  for (ulong i = 0; i < m_registered_sysvars.records; i++)
  {
    ...
    const char *value = get_one_variable(thd, &show, OPT_SESSION, SHOW_SYS, NULL, &charset, val_buf, &val_length);
    mysql_mutex_unlock(&LOCK_global_system_variables);

    if (is_plugin)
      mysql_mutex_unlock(&LOCK_plugin);

    length = net_length_size(svar->name.length) + svar->name.length + net_length_size(val_length) + val_length;

    compile_time_assert(SESSION_TRACK_SYSTEM_VARIABLES < 251);
    if (unlikely((1 + net_length_size(length) + length + buf->length() >= MAX_PACKET_LENGTH) ||
                 buf->reserve(1 + net_length_size(length) + length, EXTRA_ALLOC)))
      return true;
...
uchar *Session_sysvars_tracker::sysvars_get_key(const char *entry, size_t *length,
...
{
  for (ulong i = 0; i < m_registered_sysvars.records; i++) at(i)->m_changed = false;
}
...
{
  DBUG_ASSERT(global_system_variables.session_track_system_variables);
  if (sysvartrack_validate_value(0, global_system_variables.session_track_system_variables,
...
bool Current_schema_tracker::store(THD *thd, String *buf)
{
  ...
  */
  ...
  if (unlikely((1 + 1 + length + buf->length() >= MAX_PACKET_LENGTH) || buf->reserve(1 + 1 + length, EXTRA_ALLOC)))
    return true;
...
static LEX_CSTRING isol[] = {{STRING_WITH_LEN("READ UNCOMMITTED")},
...
bool Transaction_state_tracker::store(THD *thd, String *buf)
{
  ...
  {
    if (unlikely((11 + buf->length() >= MAX_PACKET_LENGTH) || buf->reserve(11, EXTRA_ALLOC)))
      return true;
    ...
    buf->q_append((char)((tx_curr_state & TX_EXPLICIT) ? 'T' : ((tx_curr_state & TX_IMPLICIT) ? 'I' : '_')));
    buf->q_append((char)((tx_curr_state & TX_READ_UNSAFE) ? 'r' : '_'));
    buf->q_append((char)(((tx_curr_state & TX_READ_TRX) || (tx_curr_state & TX_WITH_SNAPSHOT)) ? 'R' : '_'));
  ...
  if ((thd->variables.session_track_transaction_info == TX_TRACK_CHISTICS) && (tx_changed & TX_CHG_CHISTICS))
  {
    bool is_xa = thd->transaction->xid_state.is_explicit_XA();
    size_t start;

    /* 2 length by 1 byte and code */
    if (unlikely((1 + 1 + 1 + 110 + buf->length() >= MAX_PACKET_LENGTH) || buf->reserve(1 + 1 + 1, EXTRA_ALLOC)))
      return true;
...
enum_tx_state Transaction_state_tracker::calc_trx_state(THD *thd, thr_lock_type l, bool has_trx)
{
  enum_tx_state s;
...
void Transaction_state_tracker::add_trx_state(THD *thd, uint add)
{
  ...
  */
  else if (!(tx_curr_state & (TX_EXPLICIT | TX_IMPLICIT)) && (thd->variables.option_bits & OPTION_NOT_AUTOCOMMIT) &&
           (add & (TX_READ_TRX | TX_READ_UNSAFE | TX_WRITE_TRX | TX_WRITE_UNSAFE)))
...
void Transaction_state_tracker::set_read_flags(THD *thd, enum enum_tx_read_flags flags)
{
...
void Transaction_state_tracker::set_isol_level(THD *thd, enum enum_tx_isol_level level)
{
...
{
  if (unlikely((1 + 1 + 1 + buf->length() >= MAX_PACKET_LENGTH) || buf->reserve(1 + 1 + 1, EXTRA_ALLOC)))
    return true;
...
bool User_variables_tracker::store(THD *thd, String *buf)
{
  for (ulong i = 0; i < m_changed_user_variables.size(); i++)
  {
    ...
    buf->q_net_store_length(length);
    buf->q_net_store_data(reinterpret_cast<const uchar *>(var->name.str), var->name.length);
    if (!null_value)
      buf->q_net_store_data(reinterpret_cast<const uchar *>(value_str.ptr()), value_str.length());
    else
    {
