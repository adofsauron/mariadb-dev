...
extern "C"
{
  uint no_get_key(uint, uint, uchar *, uint *) { return ENCRYPTION_KEY_VERSION_INVALID; }
  uint no_key(uint) { return ENCRYPTION_KEY_VERSION_INVALID; }
  uint zero_size(uint, uint) { return 0; }

  static int ctx_init(void *ctx, const unsigned char *key, unsigned int klen, const unsigned char *iv,
                      unsigned int ivlen, int flags, unsigned int key_id, unsigned int key_version)
  {
    return my_aes_crypt_init(ctx, MY_AES_CBC, flags, key, klen, iv, ivlen);
  }

  static unsigned int get_length(unsigned int slen, unsigned int key_id, unsigned int key_version)
  {
    return my_aes_get_size(MY_AES_CBC, slen);
  }

  uint ctx_size(unsigned int, unsigned int) { return MY_AES_CTX_SIZE; }
...
int initialize_encryption_plugin(st_plugin_int *plugin)
{
  ...
  */
  ...
  encryption_handler.encryption_ctx_init_func = handle->crypt_ctx_init ? handle->crypt_ctx_init : ctx_init;
  ...
  encryption_handler.encryption_key_get_func = handle->get_key;

  encryption_handler.encryption_key_get_latest_version_func = handle->get_latest_key_version;  // must be the last
...
******************************************************************/
static uint scheme_get_key(st_encryption_scheme *scheme, st_encryption_scheme_key *key)
{
  if (scheme->locker)
  ...
  uint rc = encryption_key_get(scheme->key_id, key->version, global_key, &global_key_len);
  if (rc)
    goto ret;

  /* Now generate the local key by encrypting IV using the global key */
  rc = my_aes_crypt(MY_AES_ECB, ENCRYPTION_FLAG_ENCRYPT | ENCRYPTION_FLAG_NOPAD, scheme->iv, sizeof(scheme->iv),
                    key->key, &key_len, global_key, global_key_len, NULL, 0);
  ...
  for (uint i = array_elements(scheme->key) - 1; i; i--) scheme->key[i] = scheme->key[i - 1];
...
int do_crypt(const unsigned char *src, unsigned int slen, unsigned char *dst, unsigned int *dlen,
             struct st_encryption_scheme *scheme, unsigned int key_version, unsigned int i32_1, unsigned int i32_2,
             unsigned long long i64, int flag)
{
  compile_time_assert(ENCRYPTION_SCHEME_KEY_INVALID == (int)ENCRYPTION_KEY_VERSION_INVALID);
  ...
  if (key_version == ENCRYPTION_KEY_VERSION_INVALID || key_version == ENCRYPTION_KEY_NOT_ENCRYPTED)
  ...
  return encryption_crypt(src, slen, dst, dlen, key.key, sizeof(key.key), iv, sizeof(iv), flag, scheme->key_id,
                          key_version);
}

int encryption_scheme_encrypt(const unsigned char *src, unsigned int slen, unsigned char *dst, unsigned int *dlen,
                              struct st_encryption_scheme *scheme, unsigned int key_version, unsigned int i32_1,
                              unsigned int i32_2, unsigned long long i64)
{
  return do_crypt(src, slen, dst, dlen, scheme, key_version, i32_1, i32_2, i64,
                  ENCRYPTION_FLAG_NOPAD | ENCRYPTION_FLAG_ENCRYPT);
}

int encryption_scheme_decrypt(const unsigned char *src, unsigned int slen, unsigned char *dst, unsigned int *dlen,
                              struct st_encryption_scheme *scheme, unsigned int key_version, unsigned int i32_1,
                              unsigned int i32_2, unsigned long long i64)
{
  return do_crypt(src, slen, dst, dlen, scheme, key_version, i32_1, i32_2, i64,
                  ENCRYPTION_FLAG_NOPAD | ENCRYPTION_FLAG_DECRYPT);
}
