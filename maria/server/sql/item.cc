...
bool cmp_items(Item *a, Item *b) { return a->eq(b, FALSE); }
...
{
  if (thd->lex->in_sum_func && thd->lex->in_sum_func->nest_level >= select->nest_level)
    set_if_bigger(thd->lex->in_sum_func->max_sum_func_level, select->nest_level - 1);
}

MEM_ROOT *get_thd_memroot(THD *thd) { return thd->mem_root; }
...
longlong Item::val_datetime_packed_result(THD *thd)
{
  ...
  if ((null_value = time_to_datetime_with_warn(thd, &ltime, &tmp, TIME_CONV_NONE)))
    return 0;
...
String *Item::val_str_ascii(String *str)
{
  ...
  {
    if ((null_value = str->copy(res->ptr(), res->length(), collation.collation, &my_charset_latin1, &errors)))
      return 0;
  }
...
String *Item::val_str(String *str, String *converter, CHARSET_INFO *cs)
{
  ...
  uint errors;
  if ((null_value = converter->copy(res->ptr(), res->length(), collation.collation, cs, &errors)))
    return (String *)0;
...
longlong Item::val_int_from_str(int *error)
{
  ...
  */
  ...
  Converter_strtoll10_with_warn cnv(NULL, Warn_filter_all(), res->charset(), res->ptr(), res->length());
  *error = cnv.error();
  return cnv.result();
}
...
Item::Item(THD *thd) : name(null_clex_str), orig_name(0), is_expensive_cache(-1)
{
  DBUG_ASSERT(thd);
...
Item::Item() : name(null_clex_str), orig_name(0), is_expensive_cache(-1)
{
...
void Item::print_parenthesised(String *str, enum_query_type query_type, enum precedence parent_prec)
{
...
void Item::print(String *str, enum_query_type query_type) { str->append(full_name_cstring()); }
...
Item_ident::Item_ident(THD *thd, Name_resolution_context *context_arg, const LEX_CSTRING &db_name_arg,
    ...
    : Item_result_field(thd),
      ...
      can_be_depended(TRUE),
      alias_name_used(FALSE)
{
  name = field_name_arg;
}

Item_ident::Item_ident(THD *thd, TABLE_LIST *view_arg, const LEX_CSTRING &field_name_arg)
    : Item_result_field(thd),
      orig_db_name(null_clex_str),
      orig_table_name(view_arg->table_name),
      orig_field_name(field_name_arg),
      /* TODO: suspicious use of first_select_lex */
      context(&view_arg->view->first_select_lex()->context),
      db_name(null_clex_str),
      table_name(view_arg->alias),
      field_name(field_name_arg),
      cached_table(NULL),
      depended_from(NULL),
      cached_field_index(NO_CACHED_FIELD_INDEX),
...
bool Item_ident::collect_outer_ref_processor(void *param)
{
  ...
  if (depended_from && depended_from->nest_level_base == prm->nest_level_base &&
...
bool Item_field::collect_item_field_processor(void *arg)
{
  ...
  DBUG_PRINT("info", ("%s", field->field_name.str ? field->field_name.str : "noname"));
...
void Item_ident::undeclared_spvar_error() const
{
  /*
    ...
    Print the leftmost name in the error:
      ...
      SET var=a.b.c;  -> a
  */
  my_error(ER_SP_UNDECLARED_VAR, MYF(0), db_name.str ? db_name.str : table_name.str ? table_name.str : field_name.str);
}
...
bool Item_field::rename_fields_processor(void *arg)
{
  ...
  {
    if (def->change.str &&
        (!db_name.str || !db_name.str[0] || !my_strcasecmp(table_alias_charset, db_name.str, rename->db_name.str)) &&
...
bool Item_field::register_field_in_read_map(void *arg)
{
  ...
  if (field->vcol_info && !bitmap_fast_test_and_set(field->table->read_set, field->field_index))
  {
...
bool Item_field::check_field_expression_processor(void *arg)
{
  ...
  {
    if (field == org_field || (!org_field->vcol_info && field->vcol_info) ||
        (((field->vcol_info && org_field->vcol_info) || (!field->vcol_info && !org_field->vcol_info)) &&
         field->field_index >= org_field->field_index))
    {
      my_error(ER_EXPRESSION_REFERS_TO_UNINIT_FIELD, MYF(0), org_field->field_name.str, field->field_name.str);
      return 1;
    }
  }
  return 0;
...
bool Item_field::update_vcol_processor(void *arg)
{
  ...
  if (field->vcol_info && !bitmap_fast_test_and_set(map, field->field_index))
  {
...
bool Item::check_type_or_binary(const LEX_CSTRING &opname, const Type_handler *expect) const
{
  const Type_handler *handler = type_handler();
  if (handler == expect || (handler->is_general_purpose_string_type() && collation.collation == &my_charset_bin))
    return false;
  my_error(ER_ILLEGAL_PARAMETER_DATA_TYPE_FOR_OPERATION, MYF(0), handler->name().ptr(), opname.str);
  return true;
}
...
#define MY_NAME_BINARY_VALUE 0x200000
  ...
  */
  static int my_wc_mb_item_name(CHARSET_INFO *cs, my_wc_t wc, uchar *str, uchar *end)
  {
  ...
  */
  static int my_mb_wc_item_name(CHARSET_INFO *cs, my_wc_t *pwc, const uchar *str, const uchar *end)
  {
...
static LEX_CSTRING make_name(THD *thd, const char *str, size_t length, CHARSET_INFO *cs, size_t max_octet_length)
{
  uint errors;
  ...
  uint32 cnv_length =
      my_convert_using_func(dst, dst_nbytes, system_charset_info, my_wc_mb_item_name, str, length,
                            cs == &my_charset_bin ? system_charset_info : cs, my_mb_wc_item_name, &errors);
  dst[cnv_length] = '\0';
...
void Item::set_name(THD *thd, const char *str, size_t length, CHARSET_INFO *cs)
{
  ...
  if (str != str_start && is_explicit_name())
  {
    ...
    strmake(buff, str_start, MY_MIN(sizeof(buff) - 1, length + (int)(str - str_start)));

    if (length == 0)
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NAME_BECOMES_EMPTY,
                          ER_THD(thd, ER_NAME_BECOMES_EMPTY), buff);
    else
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_REMOVED_SPACES, ER_THD(thd, ER_REMOVED_SPACES), buff);
  }
  name = make_name(thd, str, length, cs, MAX_ALIAS_NAME - 1);
}

void Item::set_name_no_truncate(THD *thd, const char *str, uint length, CHARSET_INFO *cs)
{
...
*/
Item *Item::const_charset_converter(THD *thd, CHARSET_INFO *tocs, bool lossless, const char *func_name)
{
  ...
  {
    if (collation.collation == &my_charset_bin && tocs != &my_charset_bin && !this->check_well_formed_result(s, true))
      return NULL;
    return this;
  }
  ...
  uint conv_errors;
  Item_string *conv =
      (func_name ? new (mem_root) Item_static_string_func(thd, Lex_cstring_strlen(func_name), s, tocs, &conv_errors,
                                                          collation.derivation, collation.repertoire)
                 : new (mem_root) Item_string(thd, s, tocs, &conv_errors, collation.derivation, collation.repertoire));
  ...
  if (s->charset() == &my_charset_bin && tocs != &my_charset_bin && !conv->check_well_formed_result(true))
    return NULL;
  return conv;
}
...
Item *Item_param::safe_charset_converter(THD *thd, CHARSET_INFO *tocs)
{
  ...
  */
  return !const_item() || state == NULL_VALUE ? this : const_charset_converter(thd, tocs, true);
}
...
bool Item::get_date_from_int(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  return null_value ||
         int_to_datetime_with_warn(thd, value, ltime, fuzzydate, field_table_or_null(), field_name_or_null());
}
...
bool Item::get_date_from_real(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  return null_value ||
         double_to_datetime_with_warn(thd, value, ltime, fuzzydate, field_table_or_null(), field_name_or_null());
}
...
bool Item::get_date_from_string(THD *thd, MYSQL_TIME *to, date_mode_t mode)
{
  ...
  Temporal::Warn_push warn(thd, s ? s->db.str : nullptr, s ? s->table_name.str : nullptr, field_name_or_null(), to,
                           mode);
...
const MY_LOCALE *Item::locale_from_val_str()
{
  ...
  const MY_LOCALE *lc;
  if (!locale_name || !(lc = my_locale_by_name(locale_name->c_ptr_safe())))
  {
    THD *thd = current_thd;
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_UNKNOWN_LOCALE, ER_THD(thd, ER_UNKNOWN_LOCALE),
...
#ifndef DBUG_OFF
static inline void mark_unsupported_func(const char *where, const char *processor_name)
{
  char buff[64];
...
{
  Item::vcol_func_processor_result *res = (Item::vcol_func_processor_result *)store;
...
Query_fragment::Query_fragment(THD *thd, sp_head *sphead, const char *start, const char *end)
{
...
bool Item_sp_variable::val_native(THD *thd, Native *to) { return val_native_from_item(thd, this_item(), to); }
...
Item_splocal::Item_splocal(THD *thd, const Sp_rcontext_handler *rh, const LEX_CSTRING *sp_var_name, uint sp_var_idx,
                           const Type_handler *handler, uint pos_in_q, uint len_in_q)
...
Item_field *Item_splocal::get_variable(sp_rcontext *ctx) const { return get_rcontext(ctx)->get_variable(m_var_idx); }
...
{
  return get_rcontext(ctx)->set_variable_row_field(thd, m_var_idx, m_field_idx, it);
}
...
bool Item_splocal_row_field_by_name::fix_fields(THD *thd, Item **it)
{
  DBUG_ASSERT(fixed() == 0);
  m_thd = thd;
  if (get_rcontext(thd->spcont)->find_row_field_by_name_or_error(&m_field_idx, m_var_idx, m_field_name))
    return true;
  Item *item = get_variable(thd->spcont)->element_index(m_field_idx);
  set_handler(item->type_handler());
...
void Item_splocal_row_field_by_name::print(String *str, enum_query_type)
{
  ...
  if (str->reserve(m_name.length + 2 * m_field_name.length + prefix->length + 16))
    return;
...
Item_case_expr::Item_case_expr(THD *thd, uint case_expr_id)
    : Item_sp_variable(thd, &str_case_expr), m_case_expr_id(case_expr_id)
{
}
...
bool Item_name_const::val_native(THD *thd, Native *to) { return val_native_from_item(thd, value_item, to); }
    ...
    : Item_fixed_hybrid(thd), value_item(val), name_item(name_arg)
{
  ...
  set_maybe_null();
  if (name_item->basic_const_item() && (name_str = name_item->val_str(&name_buffer)))  // Can't have a NULL name
...
Item::Type Item_name_const::type() const
{
  ...
  */
  ...
  if (value_type == FUNC_ITEM)
  {
    ...
    */
    DBUG_ASSERT(((Item_func *)value_item)->functype() == Item_func::NEG_FUNC ||
                ((Item_func *)value_item)->functype() == Item_func::COLLATE_FUNC);
...
{
  if (value_item->fix_fields_if_needed(thd, &value_item) || name_item->fix_fields_if_needed(thd, &name_item) ||
      !value_item->const_item() || !name_item->const_item())
  {
...
{
 public:
  Item_aggregate_ref(THD *thd, Name_resolution_context *context_arg, Item **item, const LEX_CSTRING &table_name_arg,
...
void Item::split_sum_func2(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &fields, Item **ref, uint split_flags)
{
  if (unlikely(type() == SUM_FUNC_ITEM))
  {
    /* An item of type Item_sum is registered if ref_by != 0 */
    if ((split_flags & SPLIT_SUM_SKIP_REGISTERED) && ((Item_sum *)this)->ref_by)
      return;
  }
  ...
  else if (type() == FUNC_ITEM && ((Item_func *)this)->functype() == Item_func::ROWNUM_FUNC)
  {
  }
  else
  {
    /* Not a SUM() function */
    if (!with_sum_func() && !with_rownum_func() && !(split_flags & SPLIT_SUM_SELECT))
    {
      /*
    ...
    if (likely(with_sum_func() ||
               (type() == FUNC_ITEM && (((Item_func *)this)->functype() == Item_func::ISNOTNULLTEST_FUNC ||
  ...
  */
  ...
  {
    if (!(item_ref = (new (thd->mem_root) Item_direct_ref(thd, &thd->lex->current_select->context,
                                                          &ref_pointer_array[el], null_clex_str, name))))
      return;  // fatal_error is set
  }
  else
  {
    if (!(item_ref = (new (thd->mem_root) Item_aggregate_ref(thd, &thd->lex->current_select->context,
                                                             &ref_pointer_array[el], null_clex_str, name))))
      return;  // fatal_error is set
  }
...
static bool left_is_superset(const DTCollation *left, const DTCollation *right)
{
      ...
      (left->derivation < right->derivation ||
       (left->derivation == right->derivation &&
        (!(right->collation->state & MY_CS_UNICODE) ||
         ...
         (left->collation->state & MY_CS_UNICODE_SUPPLEMENT && !(right->collation->state & MY_CS_UNICODE_SUPPLEMENT) &&
  ...
  if (right->repertoire == MY_REPERTOIRE_ASCII &&
      (left->derivation < right->derivation ||
       (left->derivation == right->derivation && !(left->repertoire == MY_REPERTOIRE_ASCII))))
    return TRUE;
...
bool DTCollation::aggregate(const DTCollation &dt, uint flags)
{
  if (!my_charset_same(collation, dt.collation))
  {
    ...
    */
    ...
    else if ((flags & MY_COLL_ALLOW_SUPERSET_CONV) && left_is_superset(this, &dt))
    {
      /* Do nothing */
    }
    else if ((flags & MY_COLL_ALLOW_SUPERSET_CONV) && left_is_superset(&dt, this))
    {
      set(dt);
    }
  ...
  else
  {
    ...
    else
    {
      ...
      if (collation->state & MY_CS_BINSORT && dt.collation->state & MY_CS_BINSORT)
        return 1;
      ...
      THD *thd = current_thd;
      myf utf8_flag = thd ? thd->get_utf8_flag() : global_system_variables.old_behavior & OLD_MODE_UTF8_IS_UTF8MB3;
      CHARSET_INFO *bin = get_charset_by_csname(collation->cs_name.str, MY_CS_BINSORT, MYF(utf8_flag));
...
{
  my_error(ER_CANT_AGGREGATE_2COLLATIONS, MYF(0), c1.collation->coll_name.str, c1.derivation_name(),
           c2.collation->coll_name.str, c2.derivation_name(), fname);
}

static void my_coll_agg_error(DTCollation &c1, DTCollation &c2, DTCollation &c3, const char *fname)
{
  my_error(ER_CANT_AGGREGATE_3COLLATIONS, MYF(0), c1.collation->coll_name.str, c1.derivation_name(),
           c2.collation->coll_name.str, c2.derivation_name(), c3.collation->coll_name.str, c3.derivation_name(), fname);
}
...
static void my_coll_agg_error(Item **args, uint count, const char *fname, int item_sep)
{
  if (count == 2)
    ...
    my_coll_agg_error(args[0]->collation, args[item_sep]->collation, args[2 * item_sep]->collation, fname);
  else
    my_error(ER_CANT_AGGREGATE_NCOLLATIONS, MYF(0), fname);
}

bool Type_std_attributes::agg_item_collations(DTCollation &c, const LEX_CSTRING &fname, Item **av, uint count,
...
bool Type_std_attributes::agg_item_set_converter(const DTCollation &coll, const LEX_CSTRING &fname, Item **args,
...
Item *Item_func_or_sum::build_clone(THD *thd)
{
  ...
  if (arg_count > 2)
  {
    copy_args = static_cast<Item **>(alloc_root(thd->mem_root, sizeof(Item *) * arg_count));
...
{
  dummy_table = (TABLE *)thd->calloc(sizeof(TABLE) + sizeof(TABLE_SHARE) + sizeof(Query_arena));
...
Item_sp::Item_sp(THD *thd, Item_sp *item)
    : context(item->context), m_name(item->m_name), m_sp(item->m_sp), func_ctx(NULL), sp_result_field(NULL)
{
  dummy_table = (TABLE *)thd->calloc(sizeof(TABLE) + sizeof(TABLE_SHARE) + sizeof(Query_arena));
  dummy_table->s = (TABLE_SHARE *)(dummy_table + 1);
  sp_query_arena = new (dummy_table->s + 1) Query_arena();
...
Item_sp::func_name_cstring(THD *thd, bool is_package_function) const
{
  ...
  size_t len =
      (((m_name->m_explicit_name ? m_name->m_db.length : 0) + m_name->m_name.length) * 2 +  // characters*quoting
...
*/
bool Item_sp::sp_check_access(THD *thd)
{
...
/**
   ...
   @retval TRUE if an error occurred.
*/
bool Item_sp::execute_impl(THD *thd, Item **args, uint arg_count)
{
  Sub_statement_state statement_state;
  Security_context *save_security_ctx = thd->security_ctx;
  enum enum_sp_data_access access =
      (m_sp->daccess() == SP_DEFAULT_ACCESS) ? SP_DEFAULT_ACCESS_MAPPING : m_sp->daccess();
  ...
  */
  ...
  if (unlikely(!m_sp->detistic() && !trust_function_creators &&
               ...
               (mysql_bin_log.is_open() && thd->variables.binlog_format == BINLOG_FORMAT_STMT)))
  {
  ...
  */
  m_sp->agg_type();
  DBUG_ASSERT(m_sp->agg_type() == GROUP_AGGREGATE || (m_sp->agg_type() == NOT_AGGREGATE && !func_ctx));
  if (!func_ctx)
  {
    init_sql_alloc(key_memory_sp_head_call_root, &sp_mem_root, MEM_ROOT_BLOCK_SIZE, 0, MYF(0));
    *sp_query_arena = Query_arena(&sp_mem_root, Query_arena::STMT_INITIALIZED_FOR_SP);
  }

  bool err_status = m_sp->execute_function(thd, args, arg_count, sp_result_field, &func_ctx, sp_query_arena);
  /*
...
bool Item_sp::init_result_field(THD *thd, uint max_length, uint maybe_null, bool *null_value, LEX_CSTRING *name)
{
...
Item *Item_ref::build_clone(THD *thd)
{
  ...
  if (unlikely(!copy) || unlikely(!(copy->ref = (Item **)alloc_root(thd->mem_root, sizeof(Item *)))) ||
...
Item_field::Item_field(THD *thd, Field *f)
    : Item_ident(thd, 0, null_clex_str, Lex_cstring_strlen(*f->table_name), f->field_name),
      item_equal(0),
...
Item_field::Item_field(THD *thd, Name_resolution_context *context_arg, Field *f)
    : Item_ident(thd, context_arg, f->table->s->db, Lex_cstring_strlen(*f->table_name), f->field_name),
      item_equal(0),
      have_privileges(NO_ACL),
      any_privileges(0)
{
...
Item_field::Item_field(THD *thd, Name_resolution_context *context_arg, const LEX_CSTRING &db_arg,
                       const LEX_CSTRING &table_name_arg, const LEX_CSTRING &field_name_arg)
...
void Item_field::set_field(Field *field_par)
{
  ...
  if (field->table->s->tmp_table == SYSTEM_TMP_TABLE || field->table->s->tmp_table == INTERNAL_TMP_TABLE)
...
bool Item_field::load_data_set_no_data(THD *thd, const Load_data_param *param)
{
  ...
  */
  thd->cuted_fields++;
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_WARN_TOO_FEW_RECORDS,
                      ER_THD(thd, ER_WARN_TOO_FEW_RECORDS), thd->get_stmt_da()->current_row_for_warning());
  return false;
}
...
LEX_CSTRING Item_ident::full_name_cstring() const
{
  ...
  if (db_name.str && db_name.str[0])
  {
    ...
    length = (strxmov(tmp, db_name.str, ".", table_name.str, ".", field_name.str, NullS) - tmp);
  }
  else
  {
    if (!table_name.str[0])
    ...
    THD *thd = current_thd;
    tmp = (char *)thd->alloc((uint)table_name.length + field_name.length + 2);
...
void Item_ident::print(String *str, enum_query_type query_type)
{
  ...
  bool use_db_name = use_table_name && db_name.str && db_name.str[0] && !alias_name_used;

  if (use_db_name && (query_type & QT_ITEM_IDENT_SKIP_DB_NAMES))
    use_db_name = !thd->db.str || strcmp(thd->db.str, db_name.str);

  if (use_db_name)
    use_db_name = !(cached_table && cached_table->belong_to_view && cached_table->belong_to_view->compact_view_format);
  ...
  if (use_table_name && (query_type & QT_ITEM_IDENT_SKIP_TABLE_NAMES))
  {
    ...
    */
    if (!context)
      ...
      use_db_name = use_table_name = false;
    else if (!context->last_name_resolution_table && !context->first_name_resolution_table->next_name_resolution_table)
      use_db_name = use_table_name = false;
  }
...
bool Item_field::val_native(THD *thd, Native *to) { return val_native_from_field(field, to); }

bool Item_field::val_native_result(THD *thd, Native *to) { return val_native_from_field(result_field, to); }
...
void Item_field::save_result(Field *to) { save_field_in_field(result_field, &null_value, to, TRUE); }
...
bool Item_field::eq(const Item *item, bool binary_cmp) const
{
  ...
  /*
    ...
    ER_NON_UNIQ_ERROR).
  */
  return (!lex_string_cmp(system_charset_info, &item_field->name, &field_name) &&
          (!item_field->table_name.str || !table_name.str ||
           (!my_strcasecmp(table_alias_charset, item_field->table_name.str, table_name.str) &&
            (!item_field->db_name.str || !db_name.str ||
             (item_field->db_name.str && !strcmp(item_field->db_name.str, db_name.str))))));
}
...
void Item_field::fix_after_pullout(st_select_lex *new_parent, Item **ref, bool merge)
{
...
void Item_field::set_refers_to_temp_table(bool value) { refers_to_temp_table = value; }
...
bool Item_basic_value::eq(const Item *item, bool binary_cmp) const
{
  ...
  */
  bool res = (c0 = get_item_const()) && (c1 = item->get_item_const()) &&
  ...
  DBUG_EXECUTE_IF("Item_basic_value", push_warning_printf(current_thd, Sql_condition::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                                                          "%seq=%d a=%s b=%s", binary_cmp ? "bin_" : "", (int)res,
...
Item_int::Item_int(THD *thd, const char *str_arg, size_t length) : Item_num(thd)
{
...
Item_uint::Item_uint(THD *thd, const char *str_arg, size_t length) : Item_int(thd, str_arg, length)
{
  unsigned_flag = 1;
}

Item_uint::Item_uint(THD *thd, const char *str_arg, longlong i, uint length) : Item_int(thd, str_arg, i, length)
{
  unsigned_flag = 1;
}

Item_decimal::Item_decimal(THD *thd, const char *str_arg, size_t length, CHARSET_INFO *charset) : Item_num(thd)
{
  str2my_decimal(E_DEC_FATAL_ERROR, str_arg, length, charset, &decimal_value);
  name.str = str_arg;
  name.length = safe_strlen(str_arg);
  decimals = (uint8)decimal_value.frac;
  max_length = my_decimal_precision_to_length_no_truncation(decimal_value.intg + decimals, decimals, unsigned_flag);
}

Item_decimal::Item_decimal(THD *thd, longlong val, bool unsig) : Item_num(thd)
{
...
Item_decimal::Item_decimal(THD *thd, double val, int precision, int scale) : Item_num(thd)
{
...
Item_decimal::Item_decimal(THD *thd, const char *str, const my_decimal *val_arg, uint decimal_par, uint length)
    : Item_num(thd)
{
...
Item_decimal::Item_decimal(THD *thd, const my_decimal *value_par) : Item_num(thd)
{
...
Item_decimal::Item_decimal(THD *thd, const uchar *bin, int precision, int scale)
    : Item_num(thd), decimal_value(bin, precision, scale)
{
  decimals = (uint8)decimal_value.frac;
...
{
  return new (thd->mem_root) Item_decimal(thd, name.str, &decimal_value, decimals, max_length);
}
...
{
  return new (thd->mem_root) Item_float(thd, name.str, value, decimals, max_length);
}

void Item_string::print(String *str, enum_query_type query_type)
{
  const bool print_introducer = !(query_type & QT_WITHOUT_INTRODUCERS) && is_cs_specified();
  if (print_introducer)
  {
    str->append('_');
...
double Item_string::val_real() { return double_from_string_with_check(&str_value); }

/**
  @todo
  Give error if we wanted a signed integer and we got an unsigned one
*/
longlong Item_string::val_int() { return longlong_from_string_with_check(&str_value); }
...
Item *Item_null::safe_charset_converter(THD *thd, CHARSET_INFO *tocs) { return this; }

Item *Item_null::clone_item(THD *thd) { return new (thd->mem_root) Item_null(thd, name.str); }

Item_basic_constant *Item_null::make_string_literal_concat(THD *thd, const LEX_CSTRING *str)
{
  ...
  if (str->length)
  {
    ...
    return new (thd->mem_root) Item_string(thd, str->str, (uint)str->length, cs, DERIVATION_COERCIBLE, repertoire);
  }
  return this;
}
...
Item_param::Item_param(THD *thd, const LEX_CSTRING *name_arg, uint pos_in_query_arg, uint len_in_query_arg)
...
void Item_param::set_decimal(const char *str, ulong length)
{
  ...
  collation = DTCollation_numeric();
  max_length = my_decimal_precision_to_length_no_truncation(value.m_decimal.precision(), decimals, unsigned_flag);
  base_flags &= ~item_base_t::MAYBE_NULL;
  null_value = 0;
  DBUG_VOID_RETURN;
}
...
void Item_param::set_decimal(const my_decimal *dv, bool unsigned_arg)
{
  ...
  max_length = my_decimal_precision_to_length(value.m_decimal.intg + decimals, decimals, unsigned_flag);
  base_flags &= ~item_base_t::MAYBE_NULL;
  null_value = 0;
}
...
void Item_param::set_time(const MYSQL_TIME *tm, uint32 max_length_arg, uint decimals_arg)
{
...
*/
void Item_param::set_time(MYSQL_TIME *tm, timestamp_type time_type, uint32 max_length_arg)
{
  ...
  if (check_datetime_range(&value.time))
  {
    ...
    make_truncated_value_warning(current_thd, Sql_condition::WARN_LEVEL_WARN, &str, time_type, NULL, NULL, NULL);
    set_zero_time(&value.time, time_type);
  }
  base_flags &= ~item_base_t::MAYBE_NULL;
  null_value = 0;
  fix_temporal(max_length_arg, tm->second_part > 0 ? TIME_SECOND_PART_DIGITS : 0);
  DBUG_VOID_RETURN;
}

bool Item_param::set_str(const char *str, ulong length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs)
{
  ...
  */
  value.m_string_ptr.set(value.m_string.ptr(), value.m_string.length(), value.m_string.charset());
  state = SHORT_DATA_VALUE;
...
void Item_param::CONVERSION_INFO::set(THD *thd, CHARSET_INFO *fromcs)
{
  ...
  */
  uint32 dummy_offset;
  final_character_set_of_str_value = String::needs_conversion(0, fromcs, tocs, &dummy_offset) ? tocs : fromcs;
}

bool Item_param::CONVERSION_INFO::convert(THD *thd, String *str)
{
  return thd->convert_string(str, character_set_of_placeholder, final_character_set_of_str_value);
}
  ...
  */
  switch (state)
  {
    ...
    case NULL_VALUE:
      return set_field_to_null_with_conversions(field, no_conversions);
    case DEFAULT_VALUE:
      return field->save_in_field_default_value(field->table->pos_in_table_list->top_table() !=
                                                field->table->pos_in_table_list);
    case IGNORE_VALUE:
      return field->save_in_field_ignore_value(field->table->pos_in_table_list->top_table() !=
                                               field->table->pos_in_table_list);
    case NO_VALUE:
...
{
  my_message(ER_INVALID_DEFAULT_PARAM, ER_THD(current_thd, ER_INVALID_DEFAULT_PARAM), MYF(0));
}
...
bool Item_param::get_date(THD *thd, MYSQL_TIME *res, date_mode_t fuzzydate)
{
  ...
  */
  DBUG_ASSERT(type_handler()->result_type() == value.type_handler()->result_type());
  if (state == SHORT_DATA_VALUE && value.type_handler()->cmp_type() == TIME_RESULT)
  {
    *res = value.time;
    return 0;
  }
...
double Item_param::PValue::val_real(const Type_std_attributes *attr) const
{
  switch (type_handler()->cmp_type())
  {
    case REAL_RESULT:
      return real;
    case INT_RESULT:
      return attr->unsigned_flag ? (double)(ulonglong)integer : (double)integer;
    case DECIMAL_RESULT:
...
my_decimal *Item_param::PValue::val_decimal(my_decimal *dec, const Type_std_attributes *attr)
{
  switch (type_handler()->cmp_type())
  {
    case DECIMAL_RESULT:
...
String *Item_param::PValue::val_str(String *str, const Type_std_attributes *attr)
{
  switch (type_handler()->cmp_type())
  {
    case STRING_RESULT:
    ...
    case TIME_RESULT:
    {
      if (str->reserve(MAX_DATE_STRING_REP_LENGTH))
        return NULL;
      str->length((uint)my_TIME_to_str(&time, (char *)str->ptr(), attr->decimals));
...
const String *Item_param::value_query_val_str(THD *thd, String *str) const
{
  switch (value.type_handler()->cmp_type())
  {
    ...
    {
      str->length(0);
      append_query_string(value.cs_info.character_set_client, str, value.m_string.ptr(), value.m_string.length(),
...
{
  bool rc = FALSE;
  if ((state == SHORT_DATA_VALUE || state == LONG_DATA_VALUE) && value.type_handler()->cmp_type() == STRING_RESULT)
  {
...
Item *Item_param::value_clone_item(THD *thd)
{
  ...
  {
    case INT_RESULT:
      return (unsigned_flag ? new (mem_root) Item_uint(thd, name.str, value.integer, max_length)
                            : new (mem_root) Item_int(thd, name.str, value.integer, max_length));
    case REAL_RESULT:
      return new (mem_root) Item_float(thd, name.str, value.real, decimals, max_length);
    case DECIMAL_RESULT:
      return 0;  // Should create Item_decimal. See MDEV-11361.
    case STRING_RESULT:
      return new (mem_root) Item_string(thd, name, Lex_cstring(value.m_string.ptr(), value.m_string.length()),
                                        value.m_string.charset(), collation.derivation, collation.repertoire);
    case TIME_RESULT:
      break;
    case ROW_RESULT:
      DBUG_ASSERT(0);
...
Item *Item_param::clone_item(THD *thd)
{
  // There's no "default". See comments in Item_param::save_in_field().
  switch (state)
  {
...
bool Item_param::set_value(THD *thd, sp_rcontext *ctx, Item **it)
{
  ...
  */
  if (arg->save_in_value(thd, &tmp) || set_value(thd, arg, &tmp, arg->type_handler()))
  {
    set_null();
    return false;
  }
...
double Item_copy_string::val_real()
{
  ...
  return (null_value ? 0.0
                     : str_value.charset()->strntod((char *)str_value.ptr(), str_value.length(), &end_not_used,
                                                    &err_not_used));
}
...
{
  int err;
  return null_value ? 0 : str_value.charset()->strntoll(str_value.ptr(), str_value.length(), 10, (char **)0, &err);
}
...
static bool mark_as_dependent(THD *thd, SELECT_LEX *last, SELECT_LEX *current, Item_ident *resolved_item,
                              Item_ident *mark_item, bool suppress_warning_output)
{
  DBUG_ENTER("mark_as_dependent");
  DBUG_PRINT("info", ("current select: %d (%p)  last: %d (%p)", current->select_number, current,
  ...
  {
    const char *db_name = (resolved_item->db_name.str ? resolved_item->db_name.str : "");
    const char *table_name = (resolved_item->table_name.str ? resolved_item->table_name.str : "");
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_WARN_FIELD_RESOLVED,
                        ER_THD(thd, ER_WARN_FIELD_RESOLVED), db_name, (db_name[0] ? "." : ""), table_name,
                        (table_name[0] ? "." : ""), resolved_item->field_name.str, current->select_number,
                        last->select_number);
  }
  DBUG_RETURN(FALSE);
}
...
void mark_select_range_as_dependent(THD *thd, SELECT_LEX *last_select, SELECT_LEX *current_sel, Field *found_field,
                                    Item *found_item, Item_ident *resolved_item, bool suppress_warning_output)
{
  /*
  ...
  */
  ...
  prev_subselect_item->const_item_cache = 0;
  mark_as_dependent(thd, last_select, current_sel, resolved_item, dependent, suppress_warning_output);
}
...
static Item **find_field_in_group_list(Item *find_item, ORDER *group_list)
{
  ...
  for (ORDER *cur_group = group_list; cur_group; cur_group = cur_group->next)
  {
    ...
    if ((*(cur_group->item))->name.str && !table_name.str && (*(cur_group->item))->is_explicit_name() &&
        !lex_string_cmp(system_charset_info, &(*(cur_group->item))->name, &field_name))
    {
      ++cur_match_degree;
    }
    ...
    else if ((*(cur_group->item))->type() == Item::FIELD_ITEM || (*(cur_group->item))->type() == Item::REF_ITEM)
    {
      ...
      if (!lex_string_cmp(system_charset_info, l_field_name, &field_name))
        ++cur_match_degree;
      else
        continue;
...
static Item **resolve_ref_in_select_and_group(THD *thd, Item_ident *ref, SELECT_LEX *select)
{
  ...
  */
  if (!(select_ref =
            find_item_in_list(ref, *(select->get_item_list()), &counter, REPORT_EXCEPT_NOT_FOUND, &resolution)))
  ...
  if (select->having_fix_field && !ref->with_sum_func() && group_list)
  {
        ...
        (!select->having_fix_field_for_pushed_cond))
    {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_NON_UNIQ_ERROR, ER_THD(thd, ER_NON_UNIQ_ERROR),
                          ref->full_name(), thd->where);
    }
  }

  if (thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY && select->having_fix_field && select_ref != not_found_item &&
      !group_by_ref && !ref->alias_name_used)
  {
    /*
      Report the error if fields was found only in the SELECT item list and
      the strict mode is enabled.
    */
    my_error(ER_NON_GROUPING_FIELD_USED, MYF(0), ref->name.str, "HAVING");
    return NULL;
  }
  if (select_ref != not_found_item || group_by_ref)
  {
    if (select_ref != not_found_item && !ambiguous_fields)
    {
      ...
      {
        my_error(ER_ILLEGAL_REFERENCE, MYF(0), ref->name.str, "forward reference in item list");
        return NULL;
      }
...
static bool is_outer_table(TABLE_LIST *table, SELECT_LEX *select)
{
  ...
  if (table->belong_to_view && table->belong_to_view->select_lex == select)
    return FALSE;

  for (tl = select->master_unit()->derived; tl && tl->is_merged_derived();
...
int Item_field::fix_outer_field(THD *thd, Field **from_field, Item **reference)
{
  ...
  table_list = (cached_table                                     ? cached_table
                : field_found && (*from_field) != view_ref_found ? (*from_field)->table->pos_in_table_list
                                                                 : 0);
  /*
  ...
  */
  DBUG_ASSERT(outer_context || !*from_field || *from_field == not_found_field);
  for (; outer_context; outer_context = outer_context->outer_context)
  {
    ...
    */
    if (field_found && outer_context->select_lex != table_list->select_lex)
      continue;
    /*
    ...
    */
    if (field_found ||
        (*from_field = find_field_in_tables(thd, this, outer_context->first_name_resolution_table,
                                            outer_context->last_name_resolution_table, outer_context->ignored_tables,
                                            reference, IGNORE_EXCEPT_NON_UNIQUE, TRUE, TRUE)) != not_found_field)
    {
      if (*from_field)
      {
        if (thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY && select->cur_pos_in_select_list != UNDEF_POS)
        {
          /*
          ...
          else
          {
            ...
            */
            DBUG_ASSERT(select->master_unit()->outer_select() == NULL &&
                        (thd->lex->sql_command != SQLCOM_SELECT && thd->lex->sql_command != SQLCOM_UPDATE_MULTI &&
        ...
        if (*from_field != view_ref_found)
        {
          ...
          if (!last_checked_context->select_lex->having_fix_field && select->group_list.elements &&
          ...
          */
          if (thd->lex->in_sum_func && thd->lex == context->select_lex->parent_lex &&
              thd->lex->in_sum_func->nest_level >= select->nest_level)
          {
            ...
            mark_as_dependent(thd, last_checked_context->select_lex, context->select_lex, this,
                              ((ref_type == REF_ITEM || ref_type == FIELD_ITEM) ? (Item_ident *)(*reference) : 0),
                              false);
            return 0;
          }
        }
        else
        {
          ...
          mark_as_dependent(thd, last_checked_context->select_lex, context->select_lex, this,
                            ((ref_type == REF_ITEM || ref_type == FIELD_ITEM) ? (Item_ident *)(*reference) : 0), false);
          if (thd->lex->in_sum_func && thd->lex == context->select_lex->parent_lex &&
              thd->lex->in_sum_func->nest_level >= select->nest_level)
          {
            set_if_bigger(thd->lex->in_sum_func->max_arg_level, select->nest_level);
  ...
  if (ref == not_found_item && *from_field == not_found_field)
  {
    ...
    else
    {
      ...
      find_field_in_tables(thd, this, context->first_name_resolution_table, context->last_name_resolution_table,
                           context->ignored_tables, reference, REPORT_ALL_ERRORS, !any_privileges, TRUE);
    }
    return -1;
  }
  else if (ref != not_found_item)
  {
    ...
    *ref = NULL;  // Don't call set_properties()
    rf = (place == IN_HAVING
              ? new (thd->mem_root) Item_ref(thd, context, ref, table_name, field_name, alias_name_used)
              : (!select->group_list.elements
                     ? new (thd->mem_root) Item_direct_ref(thd, context, ref, table_name, field_name, alias_name_used)
    ...
    *ref = save;
    ...
    {
      outer_context->select_lex->inner_refs_list.push_back((Item_outer_ref *)rf, thd->mem_root);
    ...
    */
    ...
    mark_as_dependent(thd, last_checked_context->select_lex, context->select_lex, rf, rf, false);
  ...
  else
  {
    ...
    */
    set_max_sum_func_level(thd, select);
    mark_as_dependent(thd, last_checked_context->select_lex, context->select_lex, this, (Item_ident *)*reference,
                      false);
    if (last_checked_context->select_lex->having_fix_field)
    {
      Item_ref *rf;
      rf = new (thd->mem_root) Item_ref(thd, context, (*from_field)->table->s->db,
                                        Lex_cstring_strlen((*from_field)->table->alias.c_ptr()), field_name);
      if (!rf)
        return -1;
      thd->change_item_tree(reference, rf);
      /*
...
bool Item_field::fix_fields(THD *thd, Item **reference)
{
  ...
  if (!field)  // If field is not checked
  {
    ...
    */
    ...
    if ((from_field =
             find_field_in_tables(thd, this, context->first_name_resolution_table, context->last_name_resolution_table,
                                  context->ignored_tables, reference,
                                  thd->lex->use_only_table_context ? REPORT_ALL_ERRORS : IGNORE_EXCEPT_NON_UNIQUE,
                                  !any_privileges, TRUE)) == not_found_field)
    {
      int ret;
      ...
      if (select && select->is_item_list_lookup)
      {
        uint counter;
        enum_resolution_type resolution;
        Item **res = find_item_in_list(this, select->item_list, &counter, REPORT_EXCEPT_NOT_FOUND, &resolution);
        if (!res)
          return 1;
        if (resolution == RESOLVED_AGAINST_ALIAS)
        ...
        if (res != (Item **)not_found_item)
        {
          if ((*res)->type() == Item::FIELD_ITEM)
          {
            ...
            */
            ...
            if (unlikely(new_field == NULL))
            {
              ...
              my_error(ER_BAD_FIELD_ERROR, MYF(0), (*res)->name.str, thd->where);
              return (1);
            }
          ...
          else
          {
            ...
            */
            Item_ref *rf = new (thd->mem_root) Item_ref(thd, context, db_name, table_name, field_name);
            if (!rf)
              return 1;
            ...
            thd->change_item_tree(reference,
                                  select->context_analysis_place == IN_GROUP_BY && alias_name_used ? *rf->ref : rf);
    ...
    table_list = (cached_table                   ? cached_table
                  : from_field != view_ref_found ? from_field->table->pos_in_table_list
                                                 : 0);
    if (!outer_fixed && table_list && table_list->select_lex && context->select_lex &&
    ...
    if (!thd->lex->current_select->no_wrap_view_item && thd->lex->in_sum_func && thd->lex == select->parent_lex &&
        thd->lex->in_sum_func->nest_level == select->nest_level)
      set_if_bigger(thd->lex->in_sum_func->max_arg_level, select->nest_level);
    /*
  ...
  if (any_privileges)
  {
    ...
    if (!(have_privileges = (get_column_grant(thd, &field->table->grant, db, tab, field_name.str) & VIEW_ANY_ACL)))
    {
...
#endif
  base_flags |= item_base_t::FIXED;
  if (thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY && !outer_fixed && !thd->lex->in_sum_func && select &&
...
mark_non_agg_field:
  /*
    ...
    or virtual fields.
  */
  if (fixed() && (thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY) && field->table->pos_in_table_list)
  {
    /*
    ...
    */
    ...
    else
    {
      ...
      else if (thd->lex->in_sum_func->nest_level != select->nest_level)
        select_lex->set_non_agg_field_used(true);
    }
  }
  return FALSE;
...
Item *Item_field::propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *arg)
{
  ...
  if (item_equal && item_equal == param->item_equal)
  {
    ...
    if (const_item2)
    {
      /*
        ...
        item_equal.
      */
      DBUG_ASSERT(type_handler_for_comparison()->cmp_type() == item_equal->compare_type_handler()->cmp_type());
      return const_item2;
    }
    Item_ident *subst = (Item_ident *)(item_equal->get_first(param->context_tab, this));
    if (subst)
    {
...
void Item::init_make_send_field(Send_field *tmp_field, const Type_handler *h)
{
  ...
  tmp_field->flags = (maybe_null() ? 0 : NOT_NULL_FLAG) | (my_binary_compare(charset_for_protocol()) ? BINARY_FLAG : 0);
  tmp_field->set_handler(h);
  tmp_field->length = max_length;
  tmp_field->decimals = decimals;
  if (unsigned_flag)
    tmp_field->flags |= UNSIGNED_FLAG;
  static_cast<Send_field_extended_metadata>(*tmp_field) = Send_field_extended_metadata();
  h->Item_append_extended_type_info(tmp_field, this);
}

void Item::make_send_field(THD *thd, Send_field *tmp_field) { init_make_send_field(tmp_field, type_handler()); }
...
String *Item::check_well_formed_result(String *str, bool send_error)
{
  ...
  if (unlikely(wlen < str->length()))
  {
    ...
    if (send_error)
    {
      my_error(ER_INVALID_CHARACTER_STRING, MYF(0), cs->cs_name.str, hexbuf);
      return 0;
    }
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN, ER_INVALID_CHARACTER_STRING,
                        ER_THD(thd, ER_INVALID_CHARACTER_STRING), cs->cs_name.str, hexbuf);
  }
  return str;
}
...
*/
bool String_copier_for_item::copy_with_warn(CHARSET_INFO *dstcs, String *dst, CHARSET_INFO *srccs, const char *src,
                                            uint32 src_length, uint32 nchars)
{
  ...
  if (unlikely(pos = well_formed_error_pos()))
  {
    ...
    push_warning_printf(m_thd, Sql_condition::WARN_LEVEL_WARN, ER_INVALID_CHARACTER_STRING,
                        ER_THD(m_thd, ER_INVALID_CHARACTER_STRING),
                        srccs == &my_charset_bin ? dstcs->cs_name.str : srccs->cs_name.str, err.ptr());
    return false;
  }
  if (unlikely(pos = cannot_convert_error_pos()))
  {
    ...
    push_warning_printf(m_thd, Sql_condition::WARN_LEVEL_WARN, ER_CANNOT_CONVERT_CHARACTER,
                        ER_THD(m_thd, ER_CANNOT_CONVERT_CHARACTER), srccs->cs_name.str, buf, dstcs->cs_name.str);
    return false;
  }
  return false;
}
...
fast_field_copier Item_field::setup_fast_field_copier(Field *to) { return to->get_fast_field_copier(field); }
...
void Item_field::save_org_in_field(Field *to, fast_field_copier fast_field_copier_func)
{
  DBUG_ENTER("Item_field::save_org_in_field");
  DBUG_PRINT("enter", ("setup: %p  data: %p", to, fast_field_copier_func));
...
bool Item::save_in_param(THD *thd, Item_param *param) { return param->set_from_item(thd, this); }
...
Item_basic_constant *Item_string::make_string_literal_concat(THD *thd, const LEX_CSTRING *str)
{
...
Item *Item_string::make_odbc_literal(THD *thd, const LEX_CSTRING *typestr)
{
  ...
  if (collation.repertoire == MY_REPERTOIRE_ASCII && str_value.length() < MAX_DATE_STRING_REP_LENGTH * 4 &&
...
void Item_datetime::set(longlong packed, enum_mysql_timestamp_type ts_type) { unpack_time(packed, &ltime, ts_type); }
...
longlong Item_datetime::val_int() { return TIME_to_ulonglong(&ltime); }
...
Item *Item_int_with_ref::clone_item(THD *thd)
{
  ...
  /*
    ...
    parameter markers.
  */
  return (ref->unsigned_flag ? new (thd->mem_root) Item_uint(thd, ref->name.str, ref->val_int(), ref->max_length)
                             : new (thd->mem_root) Item_int(thd, ref->name.str, ref->val_int(), ref->max_length));
}
...
Item *Item_uint::clone_item(THD *thd) { return new (thd->mem_root) Item_uint(thd, name.str, value, max_length); }
...
Item_float::Item_float(THD *thd, const char *str_arg, size_t length) : Item_num(thd)
{
  int error;
  char *end_not_used;
...
decimal_digits_t Item_hex_hybrid::decimal_precision() const
{
  ...
  {          // HEX                                 DEC
    ...
    case 1:
      return 3;  // 0xFF                                255
    case 2:
      return 5;  // 0xFFFF                            65535
    case 3:
      return 8;  // 0xFFFFFF                       16777215
    case 4:
      return 10;  // 0xFFFFFFFF                   4294967295
    case 5:
      return 13;  // 0xFFFFFFFFFF              1099511627775
    case 6:
      return 15;  // 0xFFFFFFFFFFFF          281474976710655
    case 7:
      return 17;  // 0xFFFFFFFFFFFFFF      72057594037927935
  }
...
Item_bin_string::Item_bin_string(THD *thd, const char *str, size_t str_length) : Item_hex_hybrid(thd)
{
...
Item *Item_date_literal::clone_item(THD *thd) { return new (thd->mem_root) Item_date_literal(thd, &cached_time); }
...
bool Item_date_literal::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  ...
  return (null_value = check_date_with_warn(thd, ltime, fuzzydate, MYSQL_TIMESTAMP_ERROR));
}
    ...
    */
    if (const_item() &&
        !(basic_const_item() || item->basic_const_item() ||
          ...
          item->type() == Item::FIELD_ITEM || item->type() == SUBSELECT_ITEM || item->type() == CACHE_ITEM ||
          /*
...
*/
bool Item::find_item_processor(void *arg) { return (this == ((Item *)arg)); }

bool Item_field::send(Protocol *protocol, st_value *buffer) { return protocol->store(result_field); }

Item *Item::propagate_equal_fields_and_change_item_tree(THD *thd, const Context &ctx, COND_EQUAL *cond, Item **place)
{
...
Item *Item_field::update_value_transformer(THD *thd, uchar *select_arg)
{
  ...
  if (field->table != select->context.table_list->table && type() != Item::TRIGGER_FIELD_ITEM)
  {
    ...
    ref = new (thd->mem_root) Item_ref(thd, &select->context, &ref_pointer_array[el], table_name, field_name);
    return ref;
  }
  return this;
}
...
Item *Item::build_pushable_cond(THD *thd, Pushdown_checker checker, uchar *arg)
{
  bool is_multiple_equality =
      type() == Item::FUNC_ITEM && ((Item_func *)this)->functype() == Item_func::MULT_EQUAL_FUNC;
  ...
  if (type() == Item::COND_ITEM)
  {
    ...
    while ((item = li++))
    {
      ...
      if (fix->type() == Item::COND_ITEM && ((Item_cond *)fix)->functype() == Item_func::COND_AND_FUNC)
        is_fix_needed = true;
  ...
  else if (is_multiple_equality)
  {
    ...
    Item *new_cond = NULL;
    if (((Item_equal *)this)->create_pushable_equalities(thd, &equalities, checker, arg, true) ||
...
static Item *get_field_item_for_having(THD *thd, Item *item, st_select_lex *sel)
{
  ...
  if (field_item)
  {
    Item_ref *ref = new (thd->mem_root) Item_ref(thd, &sel->context, field_item->field_name);
    return ref;
  }
  DBUG_ASSERT(0);
  return NULL;
...
Item *Item_direct_view_ref::derived_field_transformer_for_having(THD *thd, uchar *arg)
{
  st_select_lex *sel = (st_select_lex *)arg;
  context = &sel->context;
  ...
  table_map tab_map = sel->master_unit()->derived->table->map;
  if ((item_equal && !(item_equal->used_tables() & tab_map)) || !item_equal)
    return this;
...
static Item *find_producing_item(Item *item, st_select_lex *sel)
{
  ...
  if (field_item)
  {
    ...
    for (uint i = 0; i <= field_no; i++) producing_item = li++;
...
Item *Item_field::grouping_field_transformer_for_where(THD *thd, uchar *arg)
{
  ...
  if (gr_field)
  {
    Item *producing_clone = gr_field->corresponding_item->build_clone(thd);
    if (producing_clone)
      producing_clone->marker |= MARKER_SUBSTITUTION;
    return producing_clone;
  }
...
Item *Item_direct_view_ref::grouping_field_transformer_for_where(THD *thd, uchar *arg)
{
  if ((*ref)->marker & MARKER_SUBSTITUTION)
  {
    this->marker |= MARKER_SUBSTITUTION;
...
void Item_field::print(String *str, enum_query_type query_type)
{
  ...
  */
  if (!(refers_to_temp_table &&                       // (1)
      ...
      field &&
      field->table->const_table && !(query_type & (QT_NO_DATA_EXPANSION | QT_VIEW_INTERNAL)))
  {
    print_value(str);
    return;
  }
...
Item_ref::Item_ref(THD *thd, Name_resolution_context *context_arg, Item **item, const LEX_CSTRING &table_name_arg,
                   const LEX_CSTRING &field_name_arg, bool alias_name_used_arg)
    : Item_ident(thd, context_arg, null_clex_str, table_name_arg, field_name_arg), ref(item), reference_trough_name(0)
{
...
class Dependency_marker : public Field_enumerator
{
 public:
  ...
  virtual void visit_field(Item_field *item)
  {
    ...
    st_select_lex *sel;
    for (sel = current_select; sel; sel = (sel->context.outer_context ? sel->context.outer_context->select_lex : NULL))
    {
...
Item_ref::Item_ref(THD *thd, TABLE_LIST *view_arg, Item **item, const LEX_CSTRING &field_name_arg,
                   bool alias_name_used_arg)
    : Item_ident(thd, view_arg, field_name_arg), ref(item), reference_trough_name(0)
{
  alias_name_used = alias_name_used_arg;
  /*
...
bool Item_ref::fix_fields(THD *thd, Item **reference)
{
    ...
    if (ref == not_found_item) /* This reference was not resolved. */
    {
      ...
      */
        ...
        */
        if ((place != IN_HAVING || (!select->with_sum_func && select->group_list.elements == 0)))
        {
          ...
          */
          from_field = find_field_in_tables(thd, this, outer_context->first_name_resolution_table,
                                            outer_context->last_name_resolution_table, outer_context->ignored_tables,
                                            reference, IGNORE_EXCEPT_NON_UNIQUE, TRUE, TRUE);
          if (!from_field)
            goto error;
          if (from_field == view_ref_found)
          {
            ...
            mark_as_dependent(thd, last_checked_context->select_lex, context->select_lex, this,
                              ((refer_type == REF_ITEM || refer_type == FIELD_ITEM) ? (Item_ident *)(*reference) : 0),
                              false);
            /*
          ...
          {
            if (cached_table && cached_table->select_lex && outer_context->select_lex &&
                cached_table->select_lex != outer_context->select_lex)
            {
              ...
              */
              do
              {
                ...
                select = outer_context->select_lex;
                prev_subselect_item = last_checked_context->select_lex->master_unit()->item;
                last_checked_context = outer_context;
      ...
      if (from_field != not_found_field)
      {
        ...
        mark_as_dependent(thd, last_checked_context->select_lex, current_sel, fld, fld, false);
        /*
          A reference is resolved to a nest level that's outer or the same as
          the nest level of the enclosing set function : adjust the value of
          max_arg_level for the function if it's needed.
        */
        if (thd->lex->in_sum_func && thd->lex == context->select_lex->parent_lex &&
            thd->lex->in_sum_func->nest_level >= last_checked_context->select_lex->nest_level)
          set_if_bigger(thd->lex->in_sum_func->max_arg_level, last_checked_context->select_lex->nest_level);
        return FALSE;
      }
      if (unlikely(ref == 0))
      {
      ...
      DBUG_ASSERT(*ref && (*ref)->fixed());
      mark_as_dependent(thd, last_checked_context->select_lex, context->select_lex, this, this, false);
      /*
        A reference is resolved to a nest level that's outer or the same as
        the nest level of the enclosing set function : adjust the value of
        max_arg_level for the function if it's needed.
  ...
  */
  if (!((*ref)->type() == REF_ITEM && ((Item_ref *)(*ref))->ref_type() == OUTER_REF) &&
      (((*ref)->with_sum_func() && name.str &&
        !(current_sel->get_linkage() != GLOBAL_OPTIONS_TYPE && current_sel->having_fix_field)) ||
       !(*ref)->fixed()))
  {
    my_error(ER_ILLEGAL_REFERENCE, MYF(0), name.str,
...
void Item_ref::set_properties()
{
  ...
  */
  ...
  base_flags |= (item_base_t::FIXED | ((*ref)->base_flags & item_base_t::MAYBE_NULL));
...
Item *Item_ref::compile(THD *thd, Item_analyzer analyzer, uchar **arg_p, Item_transformer transformer, uchar *arg_t)
{
...
void Item_ref::print(String *str, enum_query_type query_type)
{
  if (ref)
  {
    if ((*ref)->type() != Item::CACHE_ITEM && (*ref)->type() != Item::WINDOW_FUNC_ITEM && ref_type() != VIEW_REF &&
...
{
  return result_field ? val_native_from_field(result_field, to) : val_native(thd, to);
}
...
bool Item_ref::val_native(THD *thd, Native *to) { return val_native_from_item(thd, *ref, to); }
...
bool Item_direct_ref::val_native(THD *thd, Native *to) { return val_native_from_item(thd, *ref, to); }
...
Item_cache_wrapper::~Item_cache_wrapper() { DBUG_ASSERT(expr_cache == 0); }
    ...
    : Item_result_field(thd), orig_item(item_arg), expr_cache(NULL), expr_value(NULL)
{
  ...
  base_flags |= (item_base_t::FIXED | (orig_item->base_flags & item_base_t::MAYBE_NULL));
...
bool Item_cache_wrapper::fix_fields(THD *thd __attribute__((unused)), Item **it __attribute__((unused)))
{
  DBUG_ASSERT(orig_item->fixed());
  DBUG_ASSERT(fixed());
  return FALSE;
...
Expression_cache_tracker *Item_cache_wrapper::init_tracker(MEM_ROOT *mem_root)
{
  if (expr_cache)
  {
    Expression_cache_tracker *tracker = new (mem_root) Expression_cache_tracker(expr_cache);
    if (tracker)
...
void Item_outer_ref::fix_after_pullout(st_select_lex *new_parent, Item **ref_arg, bool merge)
{
...
void Item_ref::fix_after_pullout(st_select_lex *new_parent, Item **refptr, bool merge)
{
...
{
  List_iterator_fast<Item_outer_ref> *it = ((List_iterator_fast<Item_outer_ref> *)arg);
...
Item *Item_direct_view_ref::propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *cond)
{
  Item *field_item = real_item();
  if (field_item->type() != FIELD_ITEM)
    return this;
...
Item *Item_ref::propagate_equal_fields(THD *thd, const Context &ctx, COND_EQUAL *cond)
{
  Item *field_item = real_item();
  if (field_item->type() != FIELD_ITEM)
    return this;
...
{
  return used_tables() == tab_map || (item_equal && (item_equal->used_tables() & tab_map));
}

bool Item_field::excl_dep_on_grouping_fields(st_select_lex *sel)
{
...
bool Item_args::excl_dep_on_grouping_fields(st_select_lex *sel)
{
  for (uint i = 0; i < arg_count; i++)
  {
    if (args[i]->type() == Item::FUNC_ITEM && ((Item_func *)args[i])->functype() == Item_func::UDF_FUNC)
      return false;
...
bool Item_default_value::fix_fields(THD *thd, Item **items)
{
  ...
  */
  ...
  {
    my_error(ER_NO_DEFAULT_FOR_FIELD, MYF(0), field_arg->field->field_name.str);
    goto error;
  }
  ...
  if (def_field->default_value && (def_field->default_value->flags || (def_field->flags & BLOB_FLAG)))
  {
    ...
    def_field->move_field_offset((my_ptrdiff_t)(def_field->table->s->default_values - def_field->table->record[0]));
  set_field(def_field);
  return FALSE;
...
bool Item_default_value::get_date_result(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  calculate();
...
Item *Item_default_value::transform(THD *thd, Item_transformer transformer, uchar *args)
{
  DBUG_ASSERT(!thd->stmt_arena->is_stmt_prepare());
  DBUG_ASSERT(arg);
...
{
  return item->type() == INSERT_VALUE_ITEM && ((Item_insert_value *)item)->arg->eq(arg, binary_cmp);
}
...
bool Item_insert_value::fix_fields(THD *thd, Item **items)
{
  ...
  if (field_arg->field->table->insert_values)
  {
    ...
    if (!def_field)
      return TRUE;
    memcpy((void *)def_field, (void *)field_arg->field, field_arg->field->size_of());
    def_field->move_field_offset((my_ptrdiff_t)(def_field->table->insert_values - def_field->table->record[0]));
    set_field(def_field);
  }
  else
  {
    ...
    Field *tmp_field =
        new Field_string(0, 0, &null_bit, 1, Field::NONE, &field_arg->field->field_name, &my_charset_bin);
    if (tmp_field)
    {
...
void Item_trigger_field::setup_field(THD *thd, TABLE *table, GRANT_INFO *table_grant_info)
{
  /*
  ...
  */
  ...
  thd->column_usage = want_privilege == SELECT_ACL ? COLUMNS_READ : COLUMNS_WRITE;
  /*
    Try to find field by its name and if it will be found
    set field_idx properly.
  */
  (void)find_field_in_table(thd, table, field_name.str, field_name.length, 0, &field_idx);
...
bool Item_trigger_field::eq(const Item *item, bool binary_cmp) const
{
  return item->type() == TRIGGER_FIELD_ITEM && row_version == ((Item_trigger_field *)item)->row_version &&
         !lex_string_cmp(system_charset_info, &field_name, &((Item_trigger_field *)item)->field_name);
}
...
bool Item_trigger_field::fix_fields(THD *thd, Item **items)
{
  ...
  */
    ...
    */
    ...
    if (table_grants)
    {
      ...
      if (check_grant_column(thd, table_grants, triggers->trigger_table->s->db.str,
                             triggers->trigger_table->s->table_name.str, field_name.str, field_name.length,
                             thd->security_ctx))
        return TRUE;
    }
  ...
  my_error(ER_BAD_FIELD_ERROR, MYF(0), field_name.str, (row_version == NEW_ROW) ? "NEW" : "OLD");
  return TRUE;
}
...
void resolve_const_item(THD *thd, Item **ref, Item *comp_item)
{
  ...
  {
    Item *new_item = cmp.type_handler()->make_const_item_for_comparison(thd, item, comp_item);
    if (new_item)
      thd->change_item_tree(ref, new_item);
  }
}
...
Item *Item_cache_int::convert_to_basic_const_item(THD *thd)
{
  ...
  if (!value_cached)
    cache_value();
  new_item = null_value ? (Item *)new (thd->mem_root) Item_null(thd)
                        : (Item *)new (thd->mem_root) Item_int(thd, val_int(), max_length);
  return new_item;
}

Item_cache_temporal::Item_cache_temporal(THD *thd, const Type_handler *handler) : Item_cache_int(thd, handler)
{
...
bool Item_cache_timestamp::get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate)
{
  if (!has_value())
  {
...
bool Item_cache_timestamp::cache_value()
{
  if (!example)
    return false;
  value_cached = true;
  null_value = example->val_native_with_conversion_result(current_thd, &m_native, type_handler());
  return true;
}
...
Item *Item_cache_real::convert_to_basic_const_item(THD *thd)
{
  ...
  if (!value_cached)
    cache_value();
  new_item = null_value ? (Item *)new (thd->mem_root) Item_null(thd)
                        : (Item *)new (thd->mem_root) Item_float(thd, val_real(), decimals);
  return new_item;
}
...
bool Item_cache_decimal::cache_value()
{
  ...
  if (!(null_value_inside = null_value = example->null_value) && val != &decimal_value)
    my_decimal2decimal(val, &decimal_value);
  return TRUE;
}

double Item_cache_decimal::val_real() { return !has_value() ? 0.0 : decimal_value.to_double(); }

longlong Item_cache_decimal::val_int() { return !has_value() ? 0 : decimal_value.to_longlong(unsigned_flag); }

String *Item_cache_decimal::val_str(String *str)
{
  return !has_value() ? NULL : decimal_value.to_string_round(str, decimals, &decimal_value);
}
...
int Item_cache_str::save_in_field(Field *field, bool no_conversions)
{
  ...
  return (is_varbinary && field->type() == MYSQL_TYPE_STRING && value->length() < field->field_length) ? 1 : res;
}
...
Item *Item_cache_str::convert_to_basic_const_item(THD *thd)
{
  ...
  else
  {
    ...
    new_item = new (thd->mem_root) Item_string(thd, tmp_str, length, result->charset());
  }
  return new_item;
}
...
void Item_cache_row::set_null()
{
  ...
  if (!values)
    return;
  for (uint i = 0; i < item_count; i++) values[i]->set_null();
};
...
table_map Item_direct_view_ref::used_tables() const
{
  ...
  if (view->is_merged_derived() || view->merged || !view->table)
  {
    ...
    return (used ? used : ((null_ref_table != NO_NULL_TABLE) ? null_ref_table->map : (table_map)0));
  }
...
{
  return (get_depended_from() ? OUTER_REF_TABLE_BIT : (*ref)->used_tables() | RAND_TABLE_BIT);
}
