...
static bool subquery_types_allow_materialization(THD *thd, Item_in_subselect *in_subs);
static bool replace_where_subcondition(JOIN *, Item **, Item *, Item *, bool);
static int subq_sj_candidate_cmp(Item_in_subselect *el1, Item_in_subselect *el2, void *arg);
static void reset_equality_number_for_subq_conds(Item *cond);
static bool convert_subq_to_sj(JOIN *parent_join, Item_in_subselect *subq_pred);
static bool convert_subq_to_jtbm(JOIN *parent_join, Item_in_subselect *subq_pred, bool *remove);
static TABLE_LIST *alloc_join_nest(THD *thd);
static uint get_tmp_table_rec_length(Ref_ptr_array p_list, uint elements);
bool find_eq_ref_candidate(TABLE *table, table_map sj_inner_tables);
static SJ_MATERIALIZATION_INFO *at_sjmat_pos(const JOIN *join, table_map remaining_tables, const JOIN_TAB *tab,
                                             uint idx, bool *loose_scan);
static Item *create_subq_in_equalities(THD *thd, SJ_MATERIALIZATION_INFO *sjm, Item_in_subselect *subq_pred);
...
bool is_materialization_applicable(THD *thd, Item_in_subselect *in_subs, st_select_lex *child_select)
{
  ...
  */
  if (optimizer_flag(thd, OPTIMIZER_SWITCH_MATERIALIZATION) &&                           // 0
      ...
      child_select->outer_select() && child_select->outer_select()->table_list.first &&  // 2A
...
int check_and_do_in_subquery_rewrites(JOIN *join)
{
  ...
  */
      ...
      (subselect = parent_unit->item))          // (2)
  {
    ...
    */
    if (substype == Item_subselect::IN_SUBS || substype == Item_subselect::EXISTS_SUBS ||
        substype == Item_subselect::ANY_SUBS || substype == Item_subselect::ALL_SUBS)
    {
    ...
    */
    if (optimizer_flag(thd, OPTIMIZER_SWITCH_SEMIJOIN) && in_subs &&  // 1
        ...
        select_lex->first_cond_optimization)                          // 11
    {
      ...
      if (!in_subs->is_registered_semijoin)
      {
        ...
        select_lex->outer_select()->sj_subselects.push_back(in_subs, thd->mem_root);
        if (arena)
      ...
      */
      if (in_subs->is_registered_semijoin)
      {
        OPT_TRACE_TRANSFORM(thd, trace_wrapper, trace_transform, select_lex->select_number, "IN (SELECT)", "semijoin");
    ...
    else
    {
      ...
      DBUG_ASSERT(optimizer_flag(thd, OPTIMIZER_SWITCH_IN_TO_EXISTS | OPTIMIZER_SWITCH_MATERIALIZATION));
      /*
      ...
      */
      if (in_subs && !in_subs->has_strategy())
      {
        if (is_materialization_applicable(thd, in_subs, select_lex))
        {
          ...
          /*
            ...
            with jtbm strategy
          */
          if (in_subs->emb_on_expr_nest == NO_JOIN_NEST && optimizer_flag(thd, OPTIMIZER_SWITCH_SEMIJOIN))
          {
        ...
        /*
          ...
          possible.
        */
        if (optimizer_flag(thd, OPTIMIZER_SWITCH_IN_TO_EXISTS) || !in_subs->has_strategy())
      ...
      {
        uchar strategy =
            (allany_subs->is_maxmin_applicable(join) ? (SUBS_MAXMIN_INJECTED | SUBS_MAXMIN_ENGINE) : SUBS_IN_TO_EXISTS);
...
static bool subquery_types_allow_materialization(THD *thd, Item_in_subselect *in_subs)
{
  ...
  OPT_TRACE_TRANSFORM(thd, trace_wrapper, trace_transform, in_subs->get_select_lex()->select_number, "IN (SELECT)",
                      "materialization");
  ...
  */
  ...
  for (uint i = 0; i < elements; i++)
  {
    ...
    Item *inner = it++;
    all_are_fields &=
        (outer->real_item()->type() == Item::FIELD_ITEM && inner->real_item()->type() == Item::FIELD_ITEM);
    total_key_length += inner->max_length;
    if (!inner->type_handler()->subquery_type_allows_materialization(inner, outer, converted_from_in_predicate))
    {
  ...
  */
  ...
  else
  {
    ...
    trace_transform.add("sjm_scan_allowed", all_are_fields).add("possible", true);
...
bool JOIN::transform_max_min_subquery()
{
  ...
  if (!subselect ||
      (subselect->substype() != Item_subselect::ALL_SUBS && subselect->substype() != Item_subselect::ANY_SUBS))
    DBUG_RETURN(0);
  DBUG_RETURN(((Item_allany_subselect *)subselect)->transform_into_max_min(this));
}
...
bool make_in_exists_conversion(THD *thd, JOIN *join, Item_in_subselect *item)
{
  ...
  */
  ...
  Item **tree = (item->emb_on_expr_nest == NO_JOIN_NEST) ? &join->conds : &(item->emb_on_expr_nest->on_expr);
  if (replace_where_subcondition(join, tree, replace_me, substitute, do_fix_fields))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  {
    tree = (item->emb_on_expr_nest == (TABLE_LIST *)NO_JOIN_NEST) ? &join->select_lex->prep_where
                                                                  : &(item->emb_on_expr_nest->prep_on_expr);

    if (replace_where_subcondition(join, tree, replace_me, substitute, FALSE))
      DBUG_RETURN(TRUE);
  }
  DBUG_RETURN(FALSE);
}
...
void find_and_block_conversion_to_sj(Item *to_find, List_iterator_fast<Item_in_subselect> &li)
{
  if (to_find->type() == Item::FUNC_ITEM && ((Item_func *)to_find)->functype() == Item_func::IN_OPTIMIZER_FUNC)
    to_find = ((Item_in_optimizer *)to_find)->get_wrapped_in_subselect_item();

  if (to_find->type() != Item::SUBSELECT_ITEM || ((Item_subselect *)to_find)->substype() != Item_subselect::IN_SUBS)
    return;
...
bool convert_join_subqueries_to_semijoins(JOIN *join)
{
  ...
  while ((tbl = ti++))
  {
    ...
    do
    {
      ...
      if (block_conversion_to_sj)
      {
        ...
        else if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC)
        {
          Item *item;
  ...
  */
  ...
  while ((in_subq = li++))
  {
    if (in_subq->emb_on_expr_nest != NO_JOIN_NEST && in_subq->emb_on_expr_nest->effective_with_check)
    {
      in_subq->block_conversion_to_sj();
    }
  }
  ...
  while ((in_subq = li++))
  {
    ...
    */
    ...
    in_subq->sj_convert_priority = MY_TEST(in_subq->do_not_convert_to_sj) * MAX_TABLES * 2 +
  ...
  */
  bubble_sort<Item_in_subselect>(&join->select_lex->sj_subselects, subq_sj_candidate_cmp, NULL);
  ...
  while ((in_subq = li++))
  {
    ...
    {
      OPT_TRACE_TRANSFORM(thd, trace_wrapper, trace_transform, in_subq->get_select_lex()->select_number, "IN (SELECT)",
                          "semijoin");
      trace_transform.add("converted_to_semi_join", false).add("cause", "subquery attached to the ON clause");
      break;
    }
    ...
    if (in_subq->is_flattenable_semijoin)
    {
      OPT_TRACE_TRANSFORM(thd, trace_wrapper, trace_transform, in_subq->get_select_lex()->select_number, "IN (SELECT)",
                          "semijoin");
      if (join->table_count + in_subq->unit->first_select()->join->table_count >= MAX_TABLES)
      {
        trace_transform.add("converted_to_semi_join", false);
        trace_transform.add("cause", "table in parent join now exceeds MAX_TABLES");
        break;
      }
    ...
    if (remove_item)
    {
      Item **tree = (in_subq->emb_on_expr_nest == NO_JOIN_NEST) ? &join->conds : &(in_subq->emb_on_expr_nest->on_expr);
      Item *replace_me = in_subq->original_item();
      if (replace_where_subcondition(join, tree, replace_me, new (thd->mem_root) Item_int(thd, 1), FALSE))
        goto restore_arena_and_fail;
    }
  }
  // skip_conversion:
  ...
  */
  while (in_subq)
  {
    ...
    */
    if (!thd->stmt_arena->is_conventional())
    {
      tree = (in_subq->emb_on_expr_nest == NO_JOIN_NEST) ? &join->select_lex->prep_where
      ...
      */
      if (*tree && replace_where_subcondition(join, tree, replace_me, substitute, FALSE))
        DBUG_RETURN(TRUE);
    }
    /*
    ...
    */
    ...
    if (is_materialization_applicable(thd, in_subq, in_subq->unit->first_select()))
    {
...
void get_delayed_table_estimates(TABLE *table, ha_rows *out_rows, double *scan_time, double *startup_cost)
{
  Item_in_subselect *item = table->pos_in_table_list->jtbm_subselect;
  Table_function_json_table *table_function = table->pos_in_table_list->table_function;
  ...
  DBUG_ASSERT(item->engine->engine_type() == subselect_engine::HASH_SJ_ENGINE);

  subselect_hash_sj_engine *hash_sj_engine = ((subselect_hash_sj_engine *)item->engine);

  *out_rows = (ha_rows)item->jtbm_record_count;
  *startup_cost = item->jtbm_read_time;

  /* Calculate cost of scanning the temptable */
  double data_size = COST_MULT(item->jtbm_record_count, hash_sj_engine->tmp_table->s->reclength);
  /* Do like in handler::scan_time() */
  *scan_time = ((data_size / table->file->stats.block_size + 2) * table->file->avg_io_cost());
}
...
static bool replace_where_subcondition(JOIN *join, Item **expr, Item *old_cond, Item *new_cond, bool do_fix_fields)
{
  if (*expr == old_cond)
  {
    *expr = new_cond;
  ...
  if ((*expr)->type() == Item::COND_ITEM)
  {
    ...
    while ((item = li++))
    {
      ...
      {
        replace_where_subcondition(join, li.ref(), old_cond, new_cond, do_fix_fields);
      }
    }
  }
  /*
...
static bool convert_subq_to_sj(JOIN *parent_join, Item_in_subselect *subq_pred)
{
  ...
  */
  ...
  sj_nest->original_subq_pred_used_tables = subq_pred->used_tables() | subq_pred->left_exp()->used_tables();
  ...
  */
  ...
  for (orig_tl = (TABLE_LIST *)(parent_lex->table_list.first); orig_tl->next_local; orig_tl = orig_tl->next_local)
  {
  }
  ...
  */
  ...
  sj_nest->nested_join->sj_depends_on = subq_pred_used_tables | left_exp->used_tables();
  ...
  */
  ...
  if (ncols == 1)
  {
    ...
    nested_join->sj_outer_expr_list.push_back(left, thd->mem_root);
    /*
    ...
    */
    Item_func_eq *item_eq = new (thd->mem_root) Item_func_eq(thd, left_exp_orig, subq_lex->ref_pointer_array[0]);
    if (!item_eq)
      goto restore_tl_and_exit;
  ...
  else if (left_exp->type() == Item::ROW_ITEM)
  {
    ...
    */
    ...
    {
      nested_join->sj_outer_expr_list.push_back(left_exp->addr(i), thd->mem_root);
      Item_func_eq *item_eq =
          new (thd->mem_root) Item_func_eq(thd, left_exp_orig->element_index(i), subq_lex->ref_pointer_array[i]);
      if (!item_eq)
        goto restore_tl_and_exit;
      DBUG_ASSERT(left_exp->element_index(i)->fixed());
      if (left_exp_orig->element_index(i) != left_exp->element_index(i))
        thd->change_item_tree(item_eq->arguments(), left_exp->element_index(i));
  ...
  else
  {
    ...
    */
    ...
    Item_func_eq *item_eq = new (thd->mem_root) Item_func_eq(thd, left_exp_orig, row);
    if (!item_eq)
      goto restore_tl_and_exit;
  ...
  */
  sj_nest->sj_on_expr->fix_after_pullout(parent_lex, &sj_nest->sj_on_expr, TRUE);
  ...
  if (emb_tbl_nest)
  {
    emb_tbl_nest->on_expr = and_items(thd, emb_tbl_nest->on_expr, sj_nest->sj_on_expr);
    emb_tbl_nest->on_expr->top_level_item();
    if (emb_tbl_nest->on_expr->fix_fields_if_needed(thd, &emb_tbl_nest->on_expr))
      goto restore_tl_and_exit;
  }
  else
  {
  ...
  if (subq_lex->ftfunc_list->elements)
  {
    ...
    while ((ifm = li++)) parent_lex->ftfunc_list->push_front(ifm, thd->mem_root);
  }
...
static bool convert_subq_to_jtbm(JOIN *parent_join, Item_in_subselect *subq_pred, bool *remove_item)
{
  SELECT_LEX *parent_lex = parent_join->select_lex;
  List<TABLE_LIST> *emb_join_list = &parent_lex->top_join_list;
  ...
  */
  ...
  if (optimization_delayed)
  {
    ...
    create_subquery_temptable_name(&tbl_alias, subq_pred->unit->first_select()->select_number);
    jtbm->alias.str = tbl_alias.str;
    jtbm->alias.length = tbl_alias.length;
    parent_join->table_count++;
    DBUG_RETURN(thd->is_fatal_error);
  }
  subselect_hash_sj_engine *hash_sj_engine = ((subselect_hash_sj_engine *)subq_pred->engine);
  ...
  create_subquery_temptable_name(&tbl_alias, hash_sj_engine->materialize_join->select_lex->select_number);
  jtbm->alias.str = tbl_alias.str;
  jtbm->alias.length = tbl_alias.length;
...
{
  TABLE_LIST *tbl;
  if (!(tbl = (TABLE_LIST *)thd->calloc(ALIGN_SIZE(sizeof(TABLE_LIST)) + sizeof(NESTED_JOIN))))
    return NULL;
  tbl->nested_join = (NESTED_JOIN *)((uchar *)tbl + ALIGN_SIZE(sizeof(TABLE_LIST)));
  return tbl;
}
...
int pull_out_semijoin_tables(JOIN *join)
{
  ...
  while ((sj_nest = sj_list_it++))
  {
    ...
    */
    ...
    while ((tbl = child_li++))
    {
      ...
      if (table && (table->reginfo.join_tab->dependent & sj_nest->nested_join->used_tables))
    ...
    */
    ...
    do
    {
      ...
      while ((tbl = child_li++))
      {
        if (tbl->table && !(pulled_tables & tbl->table->map) && !(dep_tables & tbl->table->map))
        {
          if (find_eq_ref_candidate(tbl->table, sj_nest->nested_join->used_tables & ~pulled_tables))
          {
            pulled_a_table = TRUE;
            pulled_tables |= tbl->table->map;
            DBUG_PRINT("info", ("Table %s pulled out (reason: func dep)", tbl->table->alias.c_ptr_safe()));
  ...
  skip:
    ...
    */
    table_map inner_tables = sj_nest->nested_join->used_tables & ~pulled_tables;
    /* Record the bitmap of inner tables */
    sj_nest->sj_inner_tables = inner_tables;
    if (pulled_tables)
    {
      List<TABLE_LIST> *upper_join_list = (sj_nest->embedding != NULL) ? (&sj_nest->embedding->nested_join->join_list)
...
bool optimize_semijoin_nests(JOIN *join, table_map all_table_map)
{
  ...
  Json_writer_object trace_semijoin_nest(thd, "execution_plan_for_potential_materialization");
  ...
  while ((sj_nest = sj_list_it++))
  {
        ...
        optimizer_flag(join->thd, OPTIMIZER_SWITCH_MATERIALIZATION))
    {
      if ((sj_nest->sj_inner_tables & ~join->const_table_map) && /* not everything was pulled out */
          !sj_nest->sj_subq_pred->is_correlated && sj_nest->sj_subq_pred->types_allow_materialization)
      {
        ...
        */
        ...
        if (!(sjm = new SJ_MATERIALIZATION_INFO) ||
            !(sjm->positions = (POSITION *)join->thd->alloc(sizeof(POSITION) * n_tables)))
          DBUG_RETURN(TRUE); /* purecov: inspected */
        sjm->tables = n_tables;
        sjm->is_used = FALSE;
        ...
        join->get_partial_cost_and_fanout(n_tables + join->const_tables,
                                          ...
                                          &subjoin_out_rows);
        */
        join->get_prefix_cost_and_fanout(n_tables, &subjoin_read_time, &subjoin_out_rows);
        ...
        */
        SELECT_LEX *subq_select = sj_nest->sj_subq_pred->unit->first_select();
        {
          ...
          while ((tableno = tm_it.next_bit()) != Table_map_iterator::BITMAP_END)
            rows = COST_MULT(rows, join->map2table[tableno]->table->opt_range_condition_rows);
          sjm->rows = MY_MIN(sjm->rows, rows);
        }
        memcpy((uchar *)sjm->positions, (uchar *)(join->best_positions + join->const_tables),
               sizeof(POSITION) * n_tables);

        /*
          Calculate temporary table parameters and usage costs
        */
        uint rowlen = get_tmp_table_rec_length(subq_select->ref_pointer_array, subq_select->item_list.elements);
        double lookup_cost = get_tmp_table_lookup_cost(join->thd, subjoin_out_rows, rowlen);
        double write_cost = get_tmp_table_write_cost(join->thd, subjoin_out_rows, rowlen);
...
bool find_eq_ref_candidate(TABLE *table, table_map sj_inner_tables)
{
  ...
  if (keyuse)
  {
    do
    {
      ...
      if (!keyuse->is_for_hash_join() && (table->key_info[key].flags & HA_NOSAME))
      {
        ...
        do /* For all equalities on all key parts */
        {
          ...
          */
          if (!(keyuse->used_tables & sj_inner_tables) && !(keyuse->optimize & KEY_OPTIMIZE_REF_OR_NULL) &&
  ...
  for (int i = (int)idx; i >= 0; i--)
  {
    ...
    if ((emb_sj_nest = prefix[i].table->emb_sj_nest))
    {
      if (inner_tables & emb_sj_nest->sj_inner_tables)
        return !MY_TEST(inner_tables == (emb_sj_nest->sj_inner_tables & ~join->const_table_map));
    }
  }
  return FALSE;
}

void optimize_semi_joins(JOIN *join, table_map remaining_tables, uint idx, double *current_record_count,
                         double *current_read_time, POSITION *loose_scan_pos)
{
  ...
  Semi_join_strategy_picker *pickers[] = {
      &pos->firstmatch_picker, &pos->loosescan_picker, &pos->sjmat_picker, &pos->dups_weedout_picker, NULL,
  };
  ...
  */
  ...
  table_map dups_producing_tables, UNINIT_VAR(prev_dups_producing_tables), UNINIT_VAR(prev_sjm_lookup_tables);
  ...
  if (idx == join->const_tables)
  {
    ...
    for (strategy = pickers; *strategy != NULL; strategy++) (*strategy)->set_empty();
  ...
  else
  {
    ...
    pos->inner_tables_handled_with_other_sjs = pos[-1].inner_tables_handled_with_other_sjs;
  }
...

  {
    ...
    for (strategy = pickers; *strategy != NULL; strategy++)
    {
      ...
      if ((*strategy)->check_qep(join, idx, remaining_tables, new_join_tab, &rec_count, &read_time, &handled_fanout,
                                 &sj_strategy, loose_scan_pos))
      {
        /*
        ...
        */
        if ((dups_producing_tables & handled_fanout) ||
            (read_time < *current_read_time && !(handled_fanout & pos->inner_tables_handled_with_other_sjs)))
        {
          ...
          /*
            ...
            tables
          */
          if (pos->sj_strategy == SJ_OPT_NONE || handled_fanout == (prev_dups_producing_tables ^ dups_producing_tables))
          {
          ...
          else
          {
            ...
            pos->sj_strategy = SJ_OPT_NONE;
            // next skip to last;
            strategy = pickers + (sizeof(pickers) / sizeof(Semi_join_strategy_picker *) - 3);
            continue;
          }
        }
        else
...
void update_sj_state(JOIN *join, const JOIN_TAB *new_tab, uint idx, table_map remaining_tables)
{
  ...
  if (TABLE_LIST *emb_sj_nest = new_tab->emb_sj_nest)
  {
    ...
    if (!(remaining_tables & emb_sj_nest->sj_inner_tables & ~new_tab->table->map))
      join->cur_sj_inner_tables &= ~emb_sj_nest->sj_inner_tables;
  }
#ifndef DBUG_OFF
...
bool Sj_materialization_picker::check_qep(JOIN *join, uint idx, table_map remaining_tables,
                                          const JOIN_TAB *new_join_tab, double *record_count, double *read_time,
                                          table_map *handled_fanout, sj_strategy_enum *strategy,
                                          POSITION *loose_scan_pos)
{
  bool sjm_scan;
  SJ_MATERIALIZATION_INFO *mat_info;
  THD *thd = join->thd;
  if ((mat_info = at_sjmat_pos(join, remaining_tables, new_join_tab, idx, &sjm_scan)))
  {
    if (sjm_scan)
    {
      /*
      ...
      */
      sjm_scan_need_tables = new_join_tab->emb_sj_nest->sj_inner_tables |
    ...
    else
    {
      ...
      mat_read_time = COST_ADD(prefix_cost, COST_ADD(mat_info->materialization_cost.total_cost(),
                                                     COST_MULT(prefix_rec_count, mat_info->lookup_cost.total_cost())));
      ...
      !(sjm_scan_need_tables & remaining_tables))
  {
    ...
    TABLE_LIST *mat_nest = join->positions[sjm_scan_last_inner].table->emb_sj_nest;
    ...
    prefix_cost = COST_ADD(prefix_cost, COST_ADD(mat_info->materialization_cost.total_cost(),
                                                 COST_MULT(prefix_rec_count, mat_info->scan_cost.total_cost())));
    prefix_rec_count = COST_MULT(prefix_rec_count, mat_info->rows);

    uint i;
    table_map rem_tables = remaining_tables;
    for (i = idx; i != (first_tab + mat_info->tables - 1); i--) rem_tables |= join->positions[i].table->table->map;
    ...
    {
      best_access_path(join, join->positions[i].table, rem_tables, join->positions, i, disable_jbuf, prefix_rec_count,
                       &curpos, &dummy);
      prefix_rec_count = COST_MULT(prefix_rec_count, curpos.records_read);
      prefix_cost = COST_ADD(prefix_cost, curpos.read_time);
      prefix_cost = COST_ADD(prefix_cost, prefix_rec_count / TIME_FOR_COMPARE);
...
bool LooseScan_picker::check_qep(JOIN *join, uint idx, table_map remaining_tables, const JOIN_TAB *new_join_tab,
                                 double *record_count, double *read_time, table_map *handled_fanout,
                                 sj_strategy_enum *strategy, POSITION *loose_scan_pos)
{
  ...
  */
  if (loose_scan_pos->read_time != DBL_MAX && !join->outer_join)
  {
    ...
    loosescan_need_tables = new_join_tab->emb_sj_nest->sj_inner_tables |
                            new_join_tab->emb_sj_nest->nested_join->sj_depends_on |
                            new_join_tab->emb_sj_nest->nested_join->sj_corr_tables;
  }

  if ((first_loosescan_table != MAX_TABLES) && !(remaining_tables & loosescan_need_tables) &&
      (new_join_tab->table->map & loosescan_need_tables))
  {
    ...
    */
    bool disable_jbuf = (join->thd->variables.join_cache_level == 0);
    optimize_wo_join_buffering(join, first_loosescan_table, idx, remaining_tables,
                               TRUE,  // first_alt
                               disable_jbuf ? join->table_count : first_loosescan_table + n_tables, record_count,
                               read_time);
    /*
...
bool Firstmatch_picker::check_qep(JOIN *join, uint idx, table_map remaining_tables, const JOIN_TAB *new_join_tab,
                                  double *record_count, double *read_time, table_map *handled_fanout,
                                  sj_strategy_enum *strategy, POSITION *loose_scan_pos)
{
  if (new_join_tab->emb_sj_nest && optimizer_flag(join->thd, OPTIMIZER_SWITCH_FIRSTMATCH) && !join->outer_join)
  {
    const table_map outer_corr_tables =
        new_join_tab->emb_sj_nest->nested_join->sj_corr_tables | new_join_tab->emb_sj_nest->nested_join->sj_depends_on;
    const table_map sj_inner_tables = new_join_tab->emb_sj_nest->sj_inner_tables & ~join->const_table_map;
    ...
    */
    ...
    if (in_firstmatch_prefix())
    {
      ...
      if (in_firstmatch_prefix() && !(firstmatch_need_tables & remaining_tables))
      {
        Json_writer_object trace(join->thd);
        ...
        */
        ...
        if (idx == first_firstmatch_table && optimizer_flag(join->thd, OPTIMIZER_SWITCH_SEMIJOIN_WITH_CACHE))
        {
          /*
        ...
        {
          optimize_wo_join_buffering(join, first_firstmatch_table, idx, remaining_tables, FALSE, idx, record_count,
                                     read_time);
        }
        /*
...
bool Duplicate_weedout_picker::check_qep(JOIN *join, uint idx, table_map remaining_tables, const JOIN_TAB *new_join_tab,
                                         double *record_count, double *read_time, table_map *handled_fanout,
                                         sj_strategy_enum *strategy, POSITION *loose_scan_pos)
{
  TABLE_LIST *nest;
  if ((nest = new_join_tab->emb_sj_nest))
  {
    ...
    dupsweedout_tables |= nest->sj_inner_tables | nest->nested_join->sj_depends_on | nest->nested_join->sj_corr_tables;
  }
  ...
  if (dupsweedout_tables && !(remaining_tables & ~new_join_tab->table->map & dupsweedout_tables))
  {
    /*
    ...
    */
    ...
    for (uint j = first_dupsweedout_table; j <= idx; j++)
    {
      ...
      dups_cost = COST_ADD(dups_cost, COST_ADD(p->read_time, current_fanout / TIME_FOR_COMPARE));
    ...
    */
    double one_lookup_cost = get_tmp_table_lookup_cost(join->thd, sj_outer_fanout, temptable_rec_size);
    double one_write_cost = get_tmp_table_write_cost(join->thd, sj_outer_fanout, temptable_rec_size);

    double write_cost = COST_MULT(join->positions[first_tab].prefix_record_count, sj_outer_fanout * one_write_cost);
    double full_lookup_cost = COST_MULT(join->positions[first_tab].prefix_record_count,
                                        COST_MULT(sj_outer_fanout, sj_inner_fanout * one_lookup_cost));
...
void restore_prev_sj_state(const table_map remaining_tables, const JOIN_TAB *tab, uint idx)
{
  TABLE_LIST *emb_sj_nest;
  ...
  {
    table_map subq_tables = emb_sj_nest->sj_inner_tables & ~tab->join->const_table_map;
                                             ...
                                             uint idx, bool *loose_scan)
{
  ...
  */
  ...
  if (emb_sj_nest && emb_sj_nest->sj_mat_info && !(suffix & emb_sj_nest->sj_inner_tables))
  {
    /*
...
static void recalculate_prefix_record_count(JOIN *join, uint start, uint end)
{
  for (uint j = start; j < end; j++)
  {
    ...
    if (j == join->const_tables)
      ...
      prefix_count =
          COST_MULT(join->best_positions[j - 1].prefix_record_count, join->best_positions[j - 1].records_read);
...
void fix_semijoin_strategies_for_picked_join_order(JOIN *join)
{
  ...
  Json_writer_array trace_semijoin_strategies(thd, "fix_semijoin_strategies_for_picked_join_order");
  ...
  for (tablenr = table_count - 1; tablenr != join->const_tables - 1; tablenr--)
  {
    ...
    if (pos->sj_strategy == SJ_OPT_MATERIALIZE)
    {
      ...
      memcpy((uchar *)(pos - sjm->tables + 1), (uchar *)sjm->positions, sizeof(POSITION) * sjm->tables);
      recalculate_prefix_record_count(join, tablenr - sjm->tables + 1, tablenr);
    ...
    else if (pos->sj_strategy == SJ_OPT_MATERIALIZE_SCAN)
    {
      ...
      memcpy((uchar *)(join->best_positions + first), (uchar *)sjm->positions, sizeof(POSITION) * sjm->tables);
      ...
      */
      ...
      uint i;
      table_map rem_tables = remaining_tables;
      for (i = tablenr; i != (first + sjm->tables - 1); i--) rem_tables |= join->best_positions[i].table->table->map;

      for (i = first; i < first + sjm->tables; i++) join->sjm_scan_tables |= join->best_positions[i].table->table->map;
      ...
      for (i = first + sjm->tables; i <= tablenr; i++)
      {
        ...
        best_access_path(join, join->best_positions[i].table, rem_tables, join->best_positions, i, FALSE,
    ...
    if (pos->sj_strategy == SJ_OPT_FIRST_MATCH)
    {
      ...
      double record_count = (first == join->const_tables) ? 1.0 : join->best_positions[tablenr - 1].prefix_record_count;
      ...
      */
      ...
      for (idx = first; idx <= tablenr; idx++)
      {
        ...
        {
          best_access_path(join, join->best_positions[idx].table, rem_tables, join->best_positions, idx,
                           TRUE /* no jbuf */, record_count, join->best_positions + idx, &dummy);
        }
    ...
    if (pos->sj_strategy == SJ_OPT_LOOSE_SCAN)
    {
      ...
      table_map rem_tables = remaining_tables;
      uint idx;
      for (idx = first; idx <= tablenr; idx++) rem_tables |= join->best_positions[idx].table->table->map;
      /*
        Re-run best_access_path to produce best access methods that do not use
        join buffering
      */
      ...
      for (idx = first; idx <= tablenr; idx++)
      {
        ...
        {
          best_access_path(join, join->best_positions[idx].table, rem_tables, join->best_positions, idx,
                           TRUE /* no jbuf */, record_count, join->best_positions + idx, &loose_scan_pos);
          if (idx == first)
          {
...
bool setup_sj_materialization_part1(JOIN_TAB *sjm_tab)
{
  ...
  while (!emb_sj_nest->sj_mat_info) emb_sj_nest = emb_sj_nest->embedding;
  ...
  */
  ...
  if (!(sjm->table = create_tmp_table(
            thd, &sjm->sjm_table_param, sjm->sjm_table_cols, (ORDER *)0, TRUE /* distinct */, 1, /*save_sum_fields*/
            thd->variables.option_bits | TMP_TABLE_ALL_COLUMNS, HA_POS_ERROR /*rows_limit */, &sj_materialize_name)))
    DBUG_RETURN(TRUE); /* purecov: inspected */
...
bool setup_sj_materialization_part2(JOIN_TAB *sjm_tab)
{
  ...
  if (!sjm->is_sj_scan)
  {
    ...
    */
    ...
    if (!(tab_ref->key_buff = (uchar *)thd->calloc(ALIGN_SIZE(tmp_key->key_length) * 2)) ||
        !(tab_ref->key_copy = (store_key **)thd->alloc((sizeof(store_key *) * (tmp_key_parts + 1)))) ||
        !(tab_ref->items = (Item **)thd->alloc(sizeof(Item *) * tmp_key_parts)))
      DBUG_RETURN(TRUE); /* purecov: inspected */
    ...
    {
      tab_ref->items[i] = emb_sj_nest->sj_subq_pred->left_exp()->element_index(i);
    ...
    */
    ...
    {
      if (remove_sj_conds(thd, &tab[i].select_cond) || (tab[i].select && remove_sj_conds(thd, &tab[i].select->cond)))
        DBUG_RETURN(TRUE);
    }
  ...
  else
  {
    ...
    */
    ...
    sjm_tab->read_record.copy_field_end = sjm->copy_field + sjm->sjm_table_cols.elements;
...
static Item *create_subq_in_equalities(THD *thd, SJ_MATERIALIZATION_INFO *sjm, Item_in_subselect *subq_pred)
{
  ...
  if (ncols == 1)
  {
    if (!(res = new (thd->mem_root)
              Item_func_eq(thd, left_exp, new (thd->mem_root) Item_field(thd, sjm->table->field[0]))))
...
bool SJ_TMP_TABLE::create_sj_weedout_tmp_table(THD *thd)
{
  ...
  */
  ...
  if (temp_pool_slot != MY_BIT_NONE)  // we got a slot
    sprintf(path, "%s-subquery-%lx-%i", tmp_file_prefix, current_pid, temp_pool_slot);
  else
  {
    /* if we run out of slots or we are not using tempool */
    sprintf(path, "%s-subquery-%lx-%lx-%x", tmp_file_prefix, current_pid, (ulong)thd->thread_id, thd->tmp_table++);
  }
  ...
  init_sql_alloc(PSI_INSTRUMENT_ME, &own_root, TABLE_ALLOC_BLOCK_SIZE, 0, MYF(MY_THREAD_SPECIFIC));
  if (!multi_alloc_root(
          &own_root, &table, sizeof(*table), &share, sizeof(*share), &reg_field, sizeof(Field *) * (1 + 1), &blob_field,
          sizeof(uint) * 2, &keyinfo, sizeof(*keyinfo), &key_part_info, sizeof(*key_part_info) * 2, &start_recinfo,
          sizeof(*recinfo) * (1 * 2 + 4), &tmpname, (uint)strlen(path) + 1, &group_buff,
          (!using_unique_constraint ? uniq_tuple_length_arg : 0), &bitmaps, bitmap_buffer_size(1) * 6, NullS))
  {
  ...
  table->alias.set("weedout-tmp", sizeof("weedout-tmp") - 1, table_alias_charset);
  ...
  /* Create the field */
  {
    ...
    */
    field = new Field_varstring(uniq_tuple_length_arg, FALSE, &field_name, share, &my_charset_bin);
    if (!field)
      DBUG_RETURN(0);
    field->table = table;
  ...
  if (TRUE)
  {
    ...
    keyinfo->name = weedout_key;
    {
      ...
      {
        if (!(key_field = field->new_key_field(thd->mem_root, table, group_buff, key_part_info->length, field->null_ptr,
                                               field->null_bit)))
          goto err;
      }
  ...
  {
    if (unlikely(create_internal_tmp_table(table, keyinfo, start_recinfo, &recinfo, 0)))
      goto err;
  }
...
int SJ_TMP_TABLE::sj_weedout_check_row(THD *thd)
{
  ...
  if (unlikely(error))
  {
    ...
    bool is_duplicate;
    if (create_internal_tmp_table_from_heap(thd, tmp_table, start_recinfo, &recinfo, error, 1, &is_duplicate))
      DBUG_RETURN(-1);
    if (is_duplicate)
      DBUG_RETURN(1);
  }
...
int init_dups_weedout(JOIN *join, uint first_table, int first_fanout_table, uint n_tables)
{
  ...
  */
  for (JOIN_TAB *j = join->join_tab + first_table; j < join->join_tab + first_table + n_tables; j++)
  {
  ...
  for (i = join->const_tables; i < join->top_join_tab_count;)
  {
    ...
    switch (pos->sj_strategy)
    {
      ...
      case SJ_OPT_LOOSE_SCAN:
      {
        ...
        for (uint j = i; j < i + pos->n_sj_tables; j++) join->join_tab[j].inside_loosescan_range = TRUE;
        ...
        */
        ...
        for (uint j = i; j < i + pos->n_sj_tables; j++)
        {
          ...
          */
          ...
          if (j != join->const_tables && js_tab->use_quick != 2 && j <= no_jbuf_after &&
              ((js_tab->type == JT_ALL && join_cache_level != 0) ||
               (join_cache_level > 2 && (js_tab->type == JT_REF || js_tab->type == JT_EQ_REF))))
          {
...
int rewrite_to_index_subquery_engine(JOIN *join)
{
  ...
  */
  if (!join->group_list && !join->order && join->unit->item &&
      join->unit->item->substype() == Item_subselect::IN_SUBS && join->table_count == 1 && join->conds &&
      !join->unit->is_unit_op())
  {
    if (!join->having)
    {
      Item *where = join->conds;
      if (join_tab[0].type == JT_EQ_REF && join_tab[0].ref.items[0]->name.str == in_left_expr_name.str)
      {
        remove_subq_pushed_predicates(join, &where);
        save_index_subquery_explain_info(join_tab, where);
        join_tab[0].type = JT_UNIQUE_SUBQUERY;
        join->error = 0;
        DBUG_RETURN(unit->item->change_engine(
            new subselect_uniquesubquery_engine(thd, join_tab, unit->item->get_IN_subquery(), where)));
      }
      else if (join_tab[0].type == JT_REF && join_tab[0].ref.items[0]->name.str == in_left_expr_name.str)
      {
        remove_subq_pushed_predicates(join, &where);
        save_index_subquery_explain_info(join_tab, where);
        ...
        join->error = 0;
        DBUG_RETURN(unit->item->change_engine(
            new subselect_indexsubquery_engine(thd, join_tab, unit->item->get_IN_subquery(), where, NULL, 0)));
      }
    }
    else if (join_tab[0].type == JT_REF_OR_NULL && join_tab[0].ref.items[0]->name.str == in_left_expr_name.str &&
             join->having->name.str == in_having_cond.str)
    {
      ...
      DBUG_RETURN(unit->item->change_engine(new subselect_indexsubquery_engine(
          thd, join_tab, unit->item->get_IN_subquery(), join->conds, join->having, 1)));
    }
  }
...
static void remove_subq_pushed_predicates(JOIN *join, Item **where)
{
  if (join->conds->type() == Item::FUNC_ITEM && ((Item_func *)join->conds)->functype() == Item_func::EQ_FUNC &&
      ((Item_func *)join->conds)->arguments()[0]->type() == Item::REF_ITEM &&
      ((Item_func *)join->conds)->arguments()[1]->type() == Item::FIELD_ITEM &&
      test_if_ref(join->conds, (Item_field *)((Item_func *)join->conds)->arguments()[1],
...
bool JOIN::optimize_unflattened_subqueries() { return select_lex->optimize_unflattened_subqueries(false); }
...
enum_nested_loop_state join_tab_execution_startup(JOIN_TAB *tab)
{
  ...
  if (tab->table->pos_in_table_list && (in_subs = tab->table->pos_in_table_list->jtbm_subselect))
  {
    /* It's a non-merged SJM nest */
    DBUG_ASSERT(in_subs->engine->engine_type() == subselect_engine::HASH_SJ_ENGINE);
    subselect_hash_sj_engine *hash_sj_engine = ((subselect_hash_sj_engine *)in_subs->engine);
    if (!hash_sj_engine->is_materialized)
    {
      hash_sj_engine->materialize_join->exec();
      hash_sj_engine->is_materialized = TRUE;

      if (unlikely(hash_sj_engine->materialize_join->error) || unlikely(tab->join->thd->is_fatal_error))
...
TABLE *create_dummy_tmp_table(THD *thd)
{
  ...
  if (!(table = create_tmp_table(thd, &sjm_table_param, sjm_table_cols, (ORDER *)0, TRUE /* distinct */,
                                 1, /*save_sum_fields*/
                                 thd->variables.option_bits | TMP_TABLE_ALL_COLUMNS, HA_POS_ERROR /*rows_limit */,
...
{
 public:
  select_value_catcher(THD *thd_arg, Item_subselect *item_arg) : select_subselect(thd_arg, item_arg) {}
...
Item *and_new_conditions_to_optimized_cond(THD *thd, Item *cond, COND_EQUAL **cond_eq, List<Item> &new_conds,
  ...
  */
  while ((item = li++))
  {
    if (item->type() == Item::FUNC_ITEM && ((Item_func *)item)->functype() == Item_func::EQ_FUNC &&
        check_simple_equality(thd,
                              Item::Context(Item::ANY_SUBST, ((Item_func_eq *)item)->compare_type_handler(),
                                            ((Item_func_eq *)item)->compare_collation()),
                              ((Item_func *)item)->arguments()[0], ((Item_func *)item)->arguments()[1],
                              &new_cond_equal))
      li.remove();
  }

  it.rewind();
  if (cond && cond->type() == Item::COND_ITEM && ((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC)
  {
    /*
    ...
    */
    ...
    while ((item = li++))
    {
      ...
      */
      ...
      {
        item = item->build_equal_items(thd, &((Item_cond_and *)cond)->m_cond_equal, false, NULL);
      }
  ...
  else
  {
    ...
    */
    ...
    bool is_mult_eq =
        (cond && cond->type() == Item::FUNC_ITEM && ((Item_func *)cond)->functype() == Item_func::MULT_EQUAL_FUNC);

    /*
      If cond is non-empty and is not multiple equality save it as
      a part of a new condition.
    */
    if (cond && !is_mult_eq && new_conds_list.push_back(cond, thd->mem_root))
      return NULL;
    ...
    */
    if (is_mult_eq)
    {
      ...
      eq_cond->merge_into_list(thd, &new_cond_equal.current_level, false, false);
    }
    ...
    */
    ...
    if (new_conds_list.elements + new_conds.elements + new_cond_equal.current_level.elements > 1)
    {
    ...
    else
    {
      ...
      cond = iter++;
      if (cond->type() == Item::FUNC_ITEM && ((Item_func *)cond)->functype() == Item_func::MULT_EQUAL_FUNC)
      {
        if (!(*cond_eq))
  ...
  if (*cond_eq)
  {
    ...
    */
    propagate_new_equalities(thd, cond, &(*cond_eq)->current_level, 0, &is_simplified_cond);
    cond = cond->propagate_equal_fields(thd, Item::Context_boolean(), *cond_eq);
    cond->update_used_tables();
  }
  /* Check if conds has knowingly true or false parts. */
  if (cond && !is_simplified_cond && cond->walk(&Item::is_simplified_cond_processor, 0, 0))
    is_simplified_cond = true;
...
bool execute_degenerate_jtbm_semi_join(THD *thd, TABLE_LIST *tbl, Item_in_subselect *subq_pred, List<Item> &eq_list)
{
  ...
  subselect_single_select_engine *engine = (subselect_single_select_engine *)subq_pred->engine;
  if (!(new_sink = new (thd->mem_root) select_value_catcher(thd, subq_pred)))
    DBUG_RETURN(TRUE);
  if (new_sink->setup(&engine->select_lex->join->fields_list) ||
      engine->select_lex->join->change_result(new_sink, NULL) || engine->exec())
  {
    DBUG_RETURN(TRUE);
  }
  ...
  if (new_sink->assigned)
  {
    ...
    */
    ...
    for (uint i = 0; i < ncols; i++)
    {
      eq_cond = new (thd->mem_root) Item_func_eq(thd, left_exp->element_index(i), new_sink->row[i]);
      if (!eq_cond || eq_cond->fix_fields(thd, NULL) || eq_list.push_back(eq_cond, thd->mem_root))
        DBUG_RETURN(TRUE);
    }
  }
  else
...
bool setup_degenerate_jtbm_semi_joins(JOIN *join, List<TABLE_LIST> *join_list, List<Item> &eq_list)
{
  TABLE_LIST *table;
  NESTED_JOIN *nested_join;
  List_iterator<TABLE_LIST> li(*join_list);
  ...
  while ((table = li++))
  {
    ...
    if ((subq_pred = table->jtbm_subselect))
    {
      ...
      {
        if (execute_degenerate_jtbm_semi_join(thd, table, subq_pred, eq_list))
          DBUG_RETURN(TRUE);
...
bool setup_jtbm_semi_joins(JOIN *join, List<TABLE_LIST> *join_list, List<Item> &eq_list)
{
  TABLE_LIST *table;
  NESTED_JOIN *nested_join;
  List_iterator<TABLE_LIST> li(*join_list);
  ...
  while ((table = li++))
  {
    ...
    if ((subq_pred = table->jtbm_subselect))
    {
      ...
      */
      ...
      if (!subq_join->tables_list || !subq_join->table_count)
      {
        if (!join->is_orig_degenerated && execute_degenerate_jtbm_semi_join(thd, table, subq_pred, eq_list))
          DBUG_RETURN(TRUE);
      }
      else
      {
...
bool JOIN::choose_subquery_plan(table_map join_tables)
{
  ...
  */
  ...
  /*
    ...
    phase.
  */
  if (in_subs->test_strategy(SUBS_MATERIALIZATION) && in_subs->test_strategy(SUBS_IN_TO_EXISTS))
  {
    JOIN *outer_join;
    ...
    */
        ...
        !in_subs->const_item())
    {
      ...
      */
      outer_join->get_partial_cost_and_fanout(in_subs->get_join_tab_idx(), table_map(-1), &dummy, &outer_lookup_keys);
    }
    else
    {
      /*
    ...
    */
    ...
    uint rowlen = get_tmp_table_rec_length(ref_ptrs, select_lex->item_list.elements);
    /* The cost of writing one row into the temporary table. */
    double write_cost = get_tmp_table_write_cost(thd, inner_record_count_1, rowlen);
    /* The cost of a lookup into the unique index of the materialized table. */
    double lookup_cost = get_tmp_table_lookup_cost(thd, inner_record_count_1, rowlen);
    /*
      The cost of executing the subquery and storing its result in an indexed
      temporary table.
    */
    double materialization_cost = COST_ADD(inner_read_time_1, COST_MULT(write_cost, inner_record_count_1));

    materialize_strategy_cost = COST_ADD(materialization_cost, COST_MULT(outer_lookup_keys, lookup_cost));
    ...
    DBUG_PRINT("info", ("mat_strategy_cost: %.2f, mat_cost: %.2f, write_cost: %.2f, lookup_cost: %.2f",
                        materialize_strategy_cost, materialization_cost, write_cost, lookup_cost));
    DBUG_PRINT("info",
               ("inx_strategy_cost: %.2f, inner_read_time_2: %.2f", in_exists_strategy_cost, inner_read_time_2));
  ...
  /*
    ...
    otherwise
      use materialization.
  */
  if (in_subs->test_strategy(SUBS_MATERIALIZATION) && in_subs->setup_mat_engine())
  {
    /*
  ...
  {
    if (reopt_result == REOPT_NONE && in_to_exists_where && const_tables != table_count)
    {
      /*
    ...
    */
    if ((in_to_exists_where && in_to_exists_where->used_tables() & OUTER_REF_TABLE_BIT) ||
        (in_to_exists_having && in_to_exists_having->used_tables() & OUTER_REF_TABLE_BIT))
    {
      in_subs->unit->uncacheable |= UNCACHEABLE_DEPENDENT_INJECTED;
      select_lex->uncacheable |= UNCACHEABLE_DEPENDENT_INJECTED;
    }
...
bool JOIN::choose_tableless_subquery_plan()
{
  ...
  if (unit->item)
  {
    ...
    */
    ...
    if (in_subs && !(subs_predicate->substype() == Item_subselect::IN_SUBS && in_subs->is_jtbm_merged))
    {
      in_subs->set_strategy(SUBS_IN_TO_EXISTS);
      if (in_subs->create_in_to_exists_cond(this) || in_subs->inject_in_to_exists_cond(this))
        return TRUE;
      tmp_having = having;
    }
  }
...
{
  return get_corresponding_field_pair(this, ((Item *)arg)->get_IN_subquery()->corresponding_fields);
}
...
static Item *get_corresponding_item(THD *thd, Item *item, Item_in_subselect *subq_pred)
{
  DBUG_ASSERT(item->type() == Item::FIELD_ITEM ||
  ...
  if (item_equal)
  {
    ...
    while ((equal_item = it++))
    {
      field_pair = get_corresponding_field_pair(equal_item, subq_pred->corresponding_fields);
      if (field_pair)
  ...
  {
    field_pair = get_corresponding_field_pair(item, subq_pred->corresponding_fields);
    if (field_pair)
      return field_pair->corresponding_item;
  }
  return NULL;
...
Item *Item_direct_view_ref::in_subq_field_transformer_for_where(THD *thd, uchar *arg)
{
  if (item_equal)
  {
    Item_in_subselect *subq_pred = ((Item *)arg)->get_IN_subquery();
...
static Item *get_corresponding_item_for_in_subq_having(THD *thd, Item *in_item, Item_in_subselect *subq_pred)
{
  Item *new_item = get_corresponding_item(thd, in_item, subq_pred);

  if (new_item)
  {
    Item_ref *ref = new (thd->mem_root) Item_ref(thd, &subq_pred->unit->first_select()->context, new_item->name);
    if (!ref)
      DBUG_ASSERT(0);
    return ref;
  }
...
Item *Item_field::in_subq_field_transformer_for_having(THD *thd, uchar *arg)
{
  ...
  return get_corresponding_item_for_in_subq_having(thd, this, (Item_in_subselect *)arg);
}

Item *Item_direct_view_ref::in_subq_field_transformer_for_having(THD *thd, uchar *arg)
{
  if (!item_equal)
    return this;
  else
  {
    DBUG_ASSERT(((Item *)arg)->get_IN_subquery());
    Item *new_item = get_corresponding_item_for_in_subq_having(thd, this, (Item_in_subselect *)arg);
    if (!new_item)
      return this;
    return new_item;
  }
...
bool grouping_fields_in_the_in_subq_left_part(THD *thd, st_select_lex *sel, List<Field_pair> *fields,
                                              ORDER *grouping_list)
{
...
bool Item_in_subselect::pushdown_cond_for_in_subquery(THD *thd, Item *cond)
{
  ...
  */
  ...
  for (uint i = 0; i < left_expr->cols(); i++)
  {
    ...
    if (corresponding_fields.push_back(new Field_pair(((Item_field *)(elem->real_item()))->field, item)))
      DBUG_RETURN(TRUE);
  }

  /* 1. Check what pushable formula can be extracted from cond */
  Item *extracted_cond;
  cond->check_pushable_cond(&Item::pushable_cond_checker_for_subquery, (uchar *)this);
  /* 2. Build a clone PC of the formula that can be extracted */
  extracted_cond = cond->build_pushable_cond(thd, &Item::pushable_equality_checker_for_subquery, (uchar *)this);
  /* Nothing to push */
  if (!extracted_cond)
  {
    DBUG_RETURN(FALSE);
  ...
  if (sel->have_window_funcs())
  {
    if (sel->group_list.first || sel->join->implicit_grouping)
      goto exit;
    ORDER *common_partition_fields = sel->find_common_window_func_partition_fields(thd);
    if (!common_partition_fields)
      goto exit;

    if (grouping_fields_in_the_in_subq_left_part(thd, sel, &corresponding_fields, common_partition_fields))
      DBUG_RETURN(TRUE);
  }
  else if (grouping_fields_in_the_in_subq_left_part(thd, sel, &corresponding_fields, sel->group_list.first))
    DBUG_RETURN(TRUE);

  /* Do 4-6 */
  sel->pushdown_cond_into_where_clause(thd, extracted_cond, &remaining_cond, &Item::in_subq_field_transformer_for_where,
                                       (uchar *)this);
  if (!remaining_cond)
    goto exit;
  /*
    7. Prepare PC_having to be conjuncted with the HAVING clause of
       the IN subquery
  */
  remaining_cond = remaining_cond->transform(thd, &Item::in_subq_field_transformer_for_having, (uchar *)this);
  if (!remaining_cond || remaining_cond->walk(&Item::cleanup_excluding_const_fields_processor, 0, 0))
    goto exit;
...
/*
  ...
  @retval
    ...
    FALSE    Otherwise
*/
bool TABLE_LIST::is_sjm_scan_table() { return is_active_sjm() && sj_mat_info->is_sj_scan; }
