...
static char wsrep_defaults_file[FN_REFLEN * 2 + 10 + 30 + sizeof(WSREP_SST_OPT_CONF) +
                                sizeof(WSREP_SST_OPT_CONF_SUFFIX) + sizeof(WSREP_SST_OPT_CONF_EXTRA)] = {0};
...
struct sst_thread_arg
{
  ...
  sst_thread_arg(const char *c, char **e) : cmd(c), env(e), ret_str(0), err(-1)
  {
...
static void *wsrep_sst_donor_monitor_thread(void *arg __attribute__((unused)))
{
  ...
  while (!sst_donor_completed)
  {
    ...
    {
      WSREP_DEBUG(
          "Donor waited %lu sec, extending systemd startup timeout as SST"
          "is not completed",
          time_waited);
      service_manager_extend_timeout(WSREP_EXTEND_TIMEOUT_INTERVAL, "WSREP state transfer ongoing...");
    }
  }
...
static void *wsrep_sst_joiner_monitor_thread(void *arg __attribute__((unused)))
{
  ...
  while (!sst_joiner_completed)
  {
    ...
    {
      WSREP_DEBUG(
          "Joiner waited %lu sec, extending systemd startup timeout as SST"
          "is not completed",
          time_waited);
      service_manager_extend_timeout(WSREP_EXTEND_TIMEOUT_INTERVAL, "WSREP state transfer ongoing...");
    }
...
bool wsrep_sst_method_check(sys_var *self, THD *thd, set_var *var)
{
  if ((!var->save_result.string_value.str) || (var->save_result.string_value.length == 0))
  {
    my_error(ER_WRONG_VALUE_FOR_VAR, MYF(0), var->var->name.str,
             var->save_result.string_value.str ? var->save_result.string_value.str : "NULL");
    return 1;
  }
...
void wsrep_set_data_home_dir(const char *data_dir) { data_home_dir = (data_dir && *data_dir) ? data_dir : NULL; }
...
static void make_wsrep_defaults_file()
{
  if (!wsrep_defaults_file[0])
  {
    ...
    if (my_defaults_file)
      ptr = strxnmov(ptr, end - ptr, WSREP_SST_OPT_CONF, " '", my_defaults_file, "' ", NULL);

    if (my_defaults_extra_file)
      ptr = strxnmov(ptr, end - ptr, WSREP_SST_OPT_CONF_EXTRA, " '", my_defaults_extra_file, "' ", NULL);

    if (my_defaults_group_suffix)
      ptr = strxnmov(ptr, end - ptr, WSREP_SST_OPT_CONF_SUFFIX, " '", my_defaults_group_suffix, "' ", NULL);
  }
}

bool wsrep_sst_receive_address_check(sys_var *self, THD *thd, set_var *var)
{
  if ((!var->save_result.string_value.str) || (var->save_result.string_value.length > (FN_REFLEN - 1)))  // safety
  {
    goto err;
  }
...
bool wsrep_sst_receive_address_update(sys_var *self, THD *thd, enum_var_type type) { return 0; }

bool wsrep_sst_auth_check(sys_var *self, THD *thd, set_var *var) { return 0; }
...
bool wsrep_sst_auth_update(sys_var *self, THD *thd, enum_var_type type) { return sst_auth_real_set(wsrep_sst_auth); }

void wsrep_sst_auth_init() { sst_auth_real_set(wsrep_sst_auth); }

bool wsrep_sst_donor_check(sys_var *self, THD *thd, set_var *var) { return 0; }

bool wsrep_sst_donor_update(sys_var *self, THD *thd, enum_var_type type) { return 0; }

bool wsrep_before_SE()
{
  return (wsrep_provider != NULL && strcmp(wsrep_provider, WSREP_NONE) && strcmp(wsrep_sst_method, WSREP_SST_SKIP) &&
          strcmp(wsrep_sst_method, WSREP_SST_MYSQLDUMP));
}

// Signal end of SST
static bool wsrep_sst_complete(THD *thd, int const rcode, wsrep::gtid const sst_gtid)
{
  ...
  // in incorrect state.
  if ((state == Wsrep_server_state::s_joiner || state == Wsrep_server_state::s_initialized))
  {
  ...
  {
    WSREP_ERROR("SST failed for position %s initialized %d server_state %s", start_pos_buf,
                server_state.is_initialized(), wsrep::to_c_string(state));
    failed = true;
  }
...
*/
bool wsrep_sst_received(THD *thd, const wsrep_uuid_t &uuid, wsrep_seqno_t const seqno, const void *const state,
                        size_t const state_len)
{
  ...
  */
   ...
   */
  wsrep::gtid const sst_gtid(wsrep::id(uuid.data, sizeof(uuid.data)), wsrep::seqno(seqno));
...
static int sst_scan_uuid_seqno(const char *str, wsrep_uuid_t *uuid, wsrep_seqno_t *seqno)
{
...
static char *generate_name_value(const char *name, const char *value)
{
  ...
  char *buf = (char *)my_malloc(PSI_INSTRUMENT_ME, (name_len + value_len + 5), MYF(0));
  if (buf)
  {
    char *ref = buf;
    *ref++ = ' ';
...
static int generate_binlog_opt_val(char **ret)
{
  ...
  *ret = NULL;
  if (opt_bin_log)
  {
    ...
    *ret = strcmp(opt_bin_logname, "0") ? generate_name_value(WSREP_SST_OPT_BINLOG, opt_bin_logname)
...
static int generate_binlog_index_opt_val(char **ret)
{
  ...
  *ret = NULL;
  ...
  {
    *ret = strcmp(opt_binlog_index_name, "0") ? generate_name_value(WSREP_SST_OPT_BINLOG_INDEX, opt_binlog_index_name)
                                              : my_strdup(PSI_INSTRUMENT_ME, "", MYF(0));
  }
  else
  {
...
static void sst_report_progress(int const from, long long const total_prev, long long const total,
...
static void sst_handle_complete(const char *const input, long long const total_prev, long long *total,
                                long long *complete, int const from)
{
  long long x;
...
static void sst_handle_total(const char *const input, long long *total_prev, long long *total, long long *complete,
                             int const from)
{
  long long x;
  int n = sscanf(input, " %lld", &x);
...
static void *sst_joiner_thread(void *a)
{
...

  {
    ...
    if (proc.pipe() && !proc.error())
    {
      ...
      if (!tmp || strlen(tmp) < (magic_len + 2) || strncasecmp(tmp, magic, magic_len))
      {
        WSREP_ERROR("Failed to read '%s <addr>' from: %s\n\tRead: '%s'", magic, arg->cmd, tmp);
        proc.wait();
        if (proc.error())
          err = proc.error();
      }
  ...
  wait_signal:
    ...
    if (tmp)
    {
      ...
      {
        sst_handle_complete(tmp + complete_len, total_prev, &total, &complete, from);
        goto wait_signal;
      }
    ...
    {
      WSREP_ERROR(
          "Failed to read uuid:seqno and wsrep_gtid_domain_id from "
          "joiner script.");
      proc.wait();
      if (proc.error())
        err = proc.error();
    ...
    if (tmp)
    {
      ...
      if (!pos)
      {
        if (wsrep_gtid_mode)
        {
          ...
          WSREP_WARN(
              "Did not find domain ID from SST script output '%s'. "
  ...
  err:
    ...
    {
      ret_gtid = wsrep::gtid(wsrep::id(ret_uuid.data, sizeof(ret_uuid.data)), wsrep::seqno(ret_seqno));
    }
...
static int sst_append_env_var(wsp::env &env, const char *const var, const char *const val)
{
  ...
  {
    WSREP_ERROR("sst_append_env_var(): snprintf(%s=%s) failed: %d", var, val, ret);
...
*/
#define IS_SPECIAL(c) (isspace(c) || c == '\'' || c == '&' || c == '`' || c == '|' || c == '>' || c == '<' || c == ';')
/*
...
static ssize_t sst_prepare_other(const char *method, const char *sst_auth, const char *addr_in, const char **addr_out)
{
  bool extra_args;
  ...
  if (!cmd_str())
  {
    WSREP_ERROR("sst_prepare_other(): could not allocate cmd buffer of %zd bytes", cmd_len);
    return -ENOMEM;
  }
  ...
  {
    WSREP_ERROR("sst_prepare_other(): generate_binlog_opt_val() failed: %d", ret);
    return ret;
  }

  if ((ret = generate_binlog_index_opt_val(&binlog_index_opt_val)))
  {
    WSREP_ERROR("sst_prepare_other(): generate_binlog_index_opt_val() failed %d", ret);
    if (binlog_opt_val)
  ...
  ret = snprintf(cmd_str(), cmd_len,
                 "wsrep_sst_%s " WSREP_SST_OPT_ROLE " 'joiner' " WSREP_SST_OPT_ADDR " '%s' " WSREP_SST_OPT_DATA
                 " '%s' "
                 "%s" WSREP_SST_OPT_PARENT " %d " WSREP_SST_OPT_PROGRESS
                 " %d"
                 "%s"
                 "%s",
                 method, addr_in, mysql_real_data_home, wsrep_defaults_file, (int)getpid(), wsrep_debug ? 1 : 0,
  ...
  if (data_home_dir)
  {
    ...
    {
      WSREP_ERROR(
          "sst_prepare_other(): appending data "
          "directory failed: %d",
          ret);
      return ret;
    }
  ...
  if (ret)
  {
    WSREP_ERROR("sst_prepare_other(): mysql_thread_create() failed: %d (%s)", ret, strerror(ret));
    return -ret;
  }
...
static ssize_t sst_prepare_mysqldump(const char *addr_in, const char **addr_out)
{
...
std::string wsrep_sst_prepare()
{
  ...
  */
  ...
  if (wsrep_sst_receive_address && strcmp(wsrep_sst_receive_address, WSREP_SST_ADDRESS_AUTO))
  {
    addr_in_parser = new wsp::Address(wsrep_sst_receive_address);

    if (!addr_in_parser->is_valid())
    {
      WSREP_ERROR("Could not parse wsrep_sst_receive_address : %s", wsrep_sst_receive_address);
      unireg_abort(1);
    }
  }
  ...
  else if (wsrep_node_address && *wsrep_node_address)
  {
    ...
    {
      WSREP_ERROR("Could not parse wsrep_node_address : %s", wsrep_node_address);
  ...
  {
    WSREP_ERROR("Address to accept state transfer is too long: '%s'", address);
    unireg_abort(1);
  }
  ...
  if (port)
  {
    ...
    {
      WSREP_ERROR("Address to accept state transfer is too long: '%s:%d'", address, port);
      unireg_abort(1);
    }
  }
  ...
  else
  {
    ...
    {
      if (!strcmp(method, WSREP_SST_XTRABACKUP) || !strcmp(method, WSREP_SST_XTRABACKUPV2))
      {
        WSREP_WARN(
            "The %s SST method is deprecated, so it is automatically "
            "replaced by %s",
    ...
    addr_len = sst_prepare_other(method, sst_auth_real, addr_in, &addr_out);
    if (addr_len < 0)
    {
      WSREP_ERROR("Failed to prepare for '%s' SST. Unrecoverable.", method);
      throw wsrep::runtime_error("Failed to prepare for SST. Unrecoverable");
    }
  }
...
static int sst_run_shell(const char *cmd_str, char **env, int max_tries)
{
  ...
  for (int tries = 1; tries <= max_tries; tries++)
  {
    ...
    if ((ret = proc.error()))
    {
      WSREP_ERROR("Try %d/%d: '%s' failed: %d (%s)", tries, max_tries, proc.cmd(), ret, strerror(ret));
      sleep(1);
    }
    else
    {
...
static int sst_donate_mysqldump(const char *addr, const wsrep::gtid &gtid, bool bypass,
                                char **env)  // carries auth info
{
  ...
  {
    WSREP_ERROR(
        "sst_donate_mysqldump(): "
        "could not allocate cmd buffer of %zd bytes",
        cmd_len);
    return -ENOMEM;
  }
  ...
  /*
    ...
    data intact
  */
  if (!bypass && wsrep_sst_donor_rejects_queries)
    sst_reject_queries(TRUE);
  ...
  int ret = snprintf(cmd_str(), cmd_len,
                     "wsrep_sst_mysqldump " WSREP_SST_OPT_ADDR " '%s' " WSREP_SST_OPT_PORT " '%u' " WSREP_SST_OPT_LPORT
                     " '%u' " WSREP_SST_OPT_SOCKET
                     " '%s' "
                     "%s" WSREP_SST_OPT_GTID " '%s:%lld,%d-%d-%llu' " WSREP_SST_OPT_GTID_DOMAIN_ID
                     " '%d'"
                     "%s",
                     addr, port, mysqld_port, mysqld_unix_port, wsrep_defaults_file, uuid_oss.str().c_str(),
                     gtid.seqno().get(), wsrep_gtid_server.domain_id, wsrep_gtid_server.server_id,
                     wsrep_gtid_server.seqno(), wsrep_gtid_server.domain_id, bypass ? " " WSREP_SST_OPT_BYPASS : "");
  ...
  wsrep::gtid sst_sent_gtid(ret == 0 ? gtid : wsrep::gtid(gtid.id(), wsrep::seqno::undefined()));
...
static int sst_create_file(const char *name, const char *content)
{
  ...
  else
  {
    ...
    {
      err = errno;
      WSREP_ERROR("Failed to rename '%s' to '%s': %d (%s)", tmp_name, real_name, err, strerror(err));
    }
  }
...
static int run_sql_command(THD *thd, const char *query)
{
  ...
  if (thd->is_error())
  {
    ...
    WSREP_WARN("Error executing '%s': %d (%s)", query, err, thd->get_stmt_da()->message());
    thd->clear_error();
    return -1;
  }
  return 0;
...
static int sst_flush_tables(THD *thd)
{
  ...
  */
  ...
  if (!is_supported_parser_charset(current_charset))
  {
    ...
    WSREP_WARN("For SST temporally setting character set to : %s", my_charset_latin1.cs_name.str);
  }
  ...
  else
  {
    ...
    */
    if (reload_acl_and_cache(thd, REFRESH_ENGINE_LOG | REFRESH_BINARY_LOG, (TABLE_LIST *)0, &not_used))
    {
...
static void *sst_donor_thread(void *a)
{
  ...
  wait_signal:
    ...
    if (out)
    {
      ...
      {
        sst_handle_complete(out + complete_len, total_prev, &total, &complete, from);
        goto wait_signal;
      }
      ...
      {
        err = sst_scan_uuid_seqno(out + strlen(magic_done) + 1, &ret_uuid, &ret_seqno);
      }
      else
      {
  ...
  {
    WSREP_ERROR("Failed to execute: %s : %d (%s)", proc.cmd(), err, strerror(err));
  }
...
static int sst_donate_other(const char *method, const char *addr, const wsrep::gtid &gtid, bool bypass,
                            char **env)  // carries auth info
{
  bool extra_args;
  size_t const cmd_len = estimate_cmd_len(&extra_args);
  ...
  uuid_oss << gtid.id();
  ret = snprintf(cmd_str(), cmd_len,
                 "wsrep_sst_%s " WSREP_SST_OPT_ROLE " 'donor' " WSREP_SST_OPT_ADDR " '%s' " WSREP_SST_OPT_LPORT
                 " %u " WSREP_SST_OPT_SOCKET " '%s' " WSREP_SST_OPT_PROGRESS " %d " WSREP_SST_OPT_DATA
                 " '%s' "
                 "%s" WSREP_SST_OPT_GTID " '%s:%lld' " WSREP_SST_OPT_GTID_DOMAIN_ID
                 " %d"
                 "%s"
                 "%s"
                 "%s",
                 method, addr, mysqld_port, mysqld_unix_port, wsrep_debug ? 1 : 0, mysql_real_data_home,
                 wsrep_defaults_file, uuid_oss.str().c_str(), gtid.seqno().get(), wsrep_gtid_server.domain_id,
                 binlog_opt_val, binlog_index_opt_val, bypass ? " " WSREP_SST_OPT_BYPASS : "");
  ...
  if (ret)
  {
    WSREP_ERROR("sst_donate_other(): mysql_thread_create() failed: %d (%s)", ret, strerror(ret));
    return ret;
  }
...
static bool check_request_str(const char *const str, bool (*check)(int c))
{
  for (size_t i(0); str[i] != '\0'; ++i)
  {
    if (!check(str[i]))
    {
      WSREP_WARN("Illegal character in state transfer request: %i (%c).", str[i], str[i]);
      return true;
    }
  }
...
int wsrep_sst_donate(const std::string &msg, const wsrep::gtid &current_gtid, const bool bypass)
{
  ...
  if (remote_auth())
  {
    ...
    {
      WSREP_ERROR(
          "wsrep_sst_donate_cb(): appending remote auth env failed: "
          "%d",
          ret);
      return WSREP_CB_FAILURE;
    }
  ...
  if (data_home_dir)
  {
    ...
    {
      WSREP_ERROR(
          "wsrep_sst_donate_cb(): appending data "
          "directory failed: %d",
          ret);
      return WSREP_CB_FAILURE;
    }
  ...
  if (ret)
  {
    WSREP_ERROR("sst_donate: mysql_thread_create() failed: %d (%s)", ret, strerror(ret));
    return WSREP_CB_FAILURE;
  }
