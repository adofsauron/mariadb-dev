...
extern "C" my_bool wsrep_on(const THD *thd) { return my_bool(WSREP(thd)); }

extern "C" void wsrep_thd_LOCK(const THD *thd) { mysql_mutex_lock(&thd->LOCK_thd_data); }

extern "C" void wsrep_thd_UNLOCK(const THD *thd) { mysql_mutex_unlock(&thd->LOCK_thd_data); }

extern "C" void wsrep_thd_kill_LOCK(const THD *thd) { mysql_mutex_lock(&thd->LOCK_thd_kill); }

extern "C" void wsrep_thd_kill_UNLOCK(const THD *thd) { mysql_mutex_unlock(&thd->LOCK_thd_kill); }
...
extern "C" const char *wsrep_thd_client_mode_str(const THD *thd) { return wsrep::to_c_string(thd->wsrep_cs().mode()); }
...
extern "C" query_id_t wsrep_thd_transaction_id(const THD *thd) { return thd->wsrep_cs().transaction().id().get(); }
...
extern "C" void wsrep_thd_self_abort(THD *thd) { thd->wsrep_cs().bf_abort(wsrep::seqno(0)); }

extern "C" const char *wsrep_get_sr_table_name() { return wsrep_sr_table_name_full; }

extern "C" my_bool wsrep_get_debug() { return wsrep_debug; }
...
extern "C" my_bool wsrep_thd_is_local(const THD *thd)
{
  ...
  */
  return (thd->system_thread != SYSTEM_THREAD_SLAVE_BACKGROUND && thd->system_thread != SYSTEM_THREAD_SLAVE_IO &&
...
extern "C" my_bool wsrep_thd_is_toi(const THD *thd) { return thd->wsrep_cs().mode() == wsrep::client_state::m_toi; }
...
extern "C" my_bool wsrep_thd_is_in_rsu(const THD *thd) { return thd->wsrep_cs().mode() == wsrep::client_state::m_rsu; }
...
extern "C" my_bool wsrep_thd_is_SR(const THD *thd) { return thd && thd->wsrep_cs().transaction().is_streaming(); }
...
extern "C" void wsrep_handle_SR_rollback(THD *bf_thd, THD *victim_thd)
{
  ...
  if (!victim_thd || !wsrep_on(bf_thd))
    return;

  WSREP_DEBUG("handle rollback, for deadlock: thd %llu trx_id %" PRIu64 " frags %zu conf %s", victim_thd->thread_id,
              victim_thd->wsrep_trx_id(), victim_thd->wsrep_sr().fragments_certified(),
...
extern "C" my_bool wsrep_thd_bf_abort(THD *bf_thd, THD *victim_thd, my_bool signal)
{
  ...
  if ((ret || !wsrep_on(victim_thd)) && signal)
  {
    ...
    {
      WSREP_DEBUG("victim is killed already by %llu, skipping awake", victim_thd->wsrep_aborter);
...
extern "C" my_bool wsrep_thd_skip_locking(const THD *thd) { return thd && thd->wsrep_skip_locking; }
...
extern "C" my_bool wsrep_thd_is_aborting(const MYSQL_THD thd)
{
  ...
  switch (tx_state)
  {
    case wsrep::transaction::s_must_abort:
      return (cs.state() == wsrep::client_state::s_exec || cs.state() == wsrep::client_state::s_result);
...
static inline enum wsrep::key::type map_key_type(enum Wsrep_service_key_type type)
{
  switch (type)
  {
...
extern "C" int wsrep_thd_append_key(THD *thd, const struct wsrep_key *key, int n_keys,
...
{
  if (wsrep_is_active(thd) && (thd->wsrep_trx().state() == wsrep::transaction::s_committing ||
                               thd->wsrep_trx().state() == wsrep::transaction::s_ordered_commit))
  {
    ...
    if (thd->wsrep_trx().state() != wsrep::transaction::s_ordered_commit && !wsrep_commit_will_write_binlog(thd))
    {
...
extern "C" my_bool wsrep_thd_has_ignored_error(const THD *thd) { return thd->wsrep_has_ignored_error; }

extern "C" void wsrep_thd_set_ignored_error(THD *thd, my_bool val) { thd->wsrep_has_ignored_error = val; }
...
extern "C" void wsrep_report_bf_lock_wait(const THD *thd, unsigned long long trx_id)
{
  if (thd)
  {
    WSREP_ERROR(
        "Thread %s trx_id: %llu thread: %ld "
        "seqno: %lld client_state: %s client_mode: %s transaction_mode: %s "
        "applier: %d toi: %d local: %d "
        "query: %s",
        wsrep_thd_is_BF(thd, false) ? "BF" : "normal", trx_id, thd_get_thread_id(thd), wsrep_thd_trx_seqno(thd),
        wsrep_thd_client_state_str(thd), wsrep_thd_client_mode_str(thd), wsrep_thd_transaction_state_str(thd),
        wsrep_thd_is_applying(thd), wsrep_thd_is_toi(thd), wsrep_thd_is_local(thd), wsrep_thd_query(thd));
  }
}
