...
class MY_XPATH_FLT
{
 ...
 public:
  MY_XPATH_FLT(uint32 num_arg, uint32 pos_arg) : num(num_arg), pos(pos_arg), size(0) {}
  MY_XPATH_FLT(uint32 num_arg, uint32 pos_arg, uint32 size_arg) : num(num_arg), pos(pos_arg), size(size_arg) {}
  bool append_to(Native *to) const { return to->append((const char *)this, (uint32)sizeof(*this)); }
};
...
class NativeNodesetBuffer : public NativeBuffer<16 * sizeof(MY_XPATH_FLT)>
{
 public:
  ...
  uint32 elements() const { return length() / sizeof(MY_XPATH_FLT); }
};
...
class Item_xml_str_func : public Item_str_func
{
 ...
 public:
  Item_xml_str_func(THD *thd, Item *a, Item *b) : Item_str_func(thd, a, b) { set_maybe_null(); }
  Item_xml_str_func(THD *thd, Item *a, Item *b, Item *c) : Item_str_func(thd, a, b, c) { set_maybe_null(); }
  bool fix_fields(THD *thd, Item **ref) override;
  bool fix_length_and_dec(THD *thd) override;
  bool const_item() const override { return const_item_cache && (!nodeset_func || nodeset_func->const_item()); }
};

class Item_func_xml_extractvalue : public Item_xml_str_func
{
 public:
  Item_func_xml_extractvalue(THD *thd, Item *a, Item *b) : Item_xml_str_func(thd, a, b) {}
  ...
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_xml_extractvalue>(thd, this); }
};
...
class Item_func_xml_update : public Item_xml_str_func
{
  ...
  String tmp_value3;
  bool collect_result(String *str, const MY_XML_NODE *cut, const String *replace);

 public:
  Item_func_xml_update(THD *thd, Item *a, Item *b, Item *c) : Item_xml_str_func(thd, a, b, c) {}
  LEX_CSTRING func_name_cstring() const override
  {
  ...
  String *val_str(String *) override;
  Item *get_copy(THD *thd) override { return get_item_copy<Item_func_xml_update>(thd, this); }
};
