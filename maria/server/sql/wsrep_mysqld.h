...
int wsrep_show_status(THD *thd, SHOW_VAR *var, void *buff, system_status_var *status_var, enum_var_type scope);
...
extern void wsrep_close_client_connections(my_bool wait_to_end, THD *except_caller_thd = NULL);
...
extern bool wsrep_check_mode_after_open_table(THD *thd, const handlerton *hton, TABLE_LIST *tables);
...
extern enum wsrep::provider::status wsrep_sync_wait_upto(THD *thd, wsrep_gtid_t *upto, int timeout);
...
#define WSREP_SYNC_WAIT(thd_, before_)                        \
  {                                                           \
    if (WSREP_CLIENT(thd_) && wsrep_sync_wait(thd_, before_)) \
...
#define WSREP_TO_ISOLATION_BEGIN_CREATE(db_, table_, table_list_, create_info_)                \
  if (WSREP_ON && WSREP(thd) &&                                                                \
      wsrep_to_isolation_begin(thd, db_, table_, table_list_, nullptr, nullptr, create_info_)) \
    goto wsrep_error_label;

#define WSREP_TO_ISOLATION_BEGIN_ALTER(db_, table_, table_list_, alter_info_, fk_tables_, create_info_) \
  if (WSREP(thd) && wsrep_thd_is_local(thd) &&                                                          \
      wsrep_to_isolation_begin(thd, db_, table_, table_list_, alter_info_, fk_tables_, create_info_))
...
#define WSREP_TO_ISOLATION_BEGIN_WRTCHK(db_, table_, table_list_)                                             \
  if (WSREP(thd) && !thd->lex->no_write_to_binlog && wsrep_to_isolation_begin(thd, db_, table_, table_list_)) \
    goto wsrep_error_label;
...
int wsrep_to_isolation_begin(THD *thd, const char *db_, const char *table_, const TABLE_LIST *table_list,
                             const Alter_info *alter_info = nullptr, const wsrep::key_array *fk_tables = nullptr,
...
class Wsrep_gtid_server
{
 public:
  uint32 domain_id;
  uint32 server_id;
  Wsrep_gtid_server() : m_force_signal(false), m_seqno(0), m_committed_seqno(0) {}
  ...
  const wsrep_server_gtid_t &undefined() { return m_undefined; }
  int wait_gtid_upto(const uint64_t seqno, uint timeout)
  {
    ...
    if (seqno > m_seqno)
    {
      ...
      {
        wait_result = mysql_cond_timedwait(&wait_cond, &LOCK_wsrep_gtid_wait_upto, &wait_time);
...
extern void wsrep_handle_mdl_conflict(MDL_context *requestor_ctx, const MDL_ticket *ticket, const MDL_key *key);
...
{
 public:
  Wsrep_thd_args(wsrep_thd_processor_fun fun, wsrep_thread_type thread_type, pthread_t thread_id)
      : fun_(fun), thread_type_(thread_type), thread_id_(thread_id)
  {
  }
...
#define wsrep_thr_init() \
  do                     \
  {                      \
  } while (0)
#define wsrep_thr_deinit() \
  do                       \
  {                        \
  } while (0)
#define wsrep_init_globals() \
  do                         \
  {                          \
  } while (0)
#define wsrep_create_appliers(X) \
  do                             \
  {                              \
  } while (0)
