...
uchar ddl_log_file_magic[] = {(uchar)254, (uchar)254, (uchar)11, (uchar)2};

/* Action names for ddl_log_action_code */

const char *ddl_log_action_name[DDL_LOG_LAST_ACTION] = {"Unknown",
                                                        "partitioning delete",
                                                        "partitioning rename",
...
const uchar ddl_log_entry_phases[DDL_LOG_LAST_ACTION] = {0,
                                                         1,
                                                         1,
                                                         2,
                                                         (uchar)EXCH_PHASE_END,
                                                         (uchar)DDL_RENAME_PHASE_END,
                                                         1,
                                                         1,
                                                         (uchar)DDL_DROP_PHASE_END,
                                                         1,
                                                         1,
                                                         (uchar)DDL_DROP_DB_PHASE_END,
                                                         (uchar)DDL_CREATE_TABLE_PHASE_END,
                                                         (uchar)DDL_CREATE_VIEW_PHASE_END,
                                                         0,
                                                         (uchar)DDL_CREATE_TRIGGER_PHASE_END,
...
{
  fn_format(file_name, opt_ddl_recovery_file, mysql_data_home, backup ? "-backup.log" : ".log", MYF(MY_REPLACE_EXT));
}
...
static bool write_ddl_log_header()
{
  ...
  if (mysql_file_pwrite(global_ddl_log.file_id, header, sizeof(header), 0, MYF(MY_WME | MY_NABP)))
    DBUG_RETURN(TRUE);
  DBUG_RETURN(ddl_log_sync_file());
}
...
static void mark_ddl_log_header_backup_done()
{
  uchar marker[1];
  marker[0] = 1;
  (void)mysql_file_pwrite(global_ddl_log.file_id, marker, sizeof(marker), DDL_LOG_BACKUP_OFFSET_POS,
                          MYF(MY_WME | MY_NABP));
}
...
static bool read_ddl_log_file_entry(uint entry_pos)
{
  ...
  mysql_mutex_assert_owner(&LOCK_gdl);
  DBUG_RETURN(
      mysql_file_pread(global_ddl_log.file_id, file_entry_buf, io_size, io_size * entry_pos, MYF(MY_WME | MY_NABP)));
}
...
static bool write_ddl_log_file_entry(uint entry_pos)
{
  ...
  DBUG_RETURN(mysql_file_pwrite(file_id, file_entry_buf, global_ddl_log.io_size, global_ddl_log.io_size * entry_pos,
                                MYF(MY_WME | MY_NABP)));
  DBUG_RETURN(error);
}
...
static bool update_phase(uint entry_pos, uchar phase)
{
  ...
  DBUG_RETURN(mysql_file_pwrite(global_ddl_log.file_id, &phase, 1,
                                global_ddl_log.io_size * entry_pos + DDL_LOG_PHASE_POS, MYF(MY_WME | MY_NABP)) ||
              ddl_log_sync_file());
}
...
static bool update_flags(uint entry_pos, uint16 flags)
{
  ...
  DBUG_RETURN(mysql_file_pwrite(global_ddl_log.file_id, buff, sizeof(buff),
                                global_ddl_log.io_size * entry_pos + DDL_LOG_FLAG_POS, MYF(MY_WME | MY_NABP)));
}
...
static bool update_next_entry_pos(uint entry_pos, uint next_entry)
{
  ...
  DBUG_RETURN(mysql_file_pwrite(global_ddl_log.file_id, buff, sizeof(buff),
                                global_ddl_log.io_size * entry_pos + DDL_LOG_NEXT_ENTRY_POS, MYF(MY_WME | MY_NABP)));
}
...
static bool update_xid(uint entry_pos, ulonglong xid)
{
  ...
  int8store(buff, xid);
  DBUG_RETURN(mysql_file_pwrite(global_ddl_log.file_id, buff, sizeof(buff),
                                global_ddl_log.io_size * entry_pos + DDL_LOG_XID_POS, MYF(MY_WME | MY_NABP)) ||
              ddl_log_sync_file());
}
...
static bool update_unique_id(uint entry_pos, ulonglong id)
{
  ...
  int8store(buff, id);
  DBUG_RETURN(mysql_file_pwrite(global_ddl_log.file_id, buff, sizeof(buff),
                                global_ddl_log.io_size * entry_pos + DDL_LOG_ID_POS, MYF(MY_WME | MY_NABP)) ||
              ddl_log_sync_file());
}
...
static bool disable_execute_entry(uint entry_pos)
{
  ...
  DBUG_RETURN(mysql_file_pwrite(global_ddl_log.file_id, buff, sizeof(buff),
                                global_ddl_log.io_size * entry_pos + DDL_LOG_ENTRY_TYPE_POS, MYF(MY_WME | MY_NABP)));
}
...
static int read_ddl_log_header(const char *file_name)
{
  ...
  if ((file_id = mysql_file_open(key_file_global_ddl_log, file_name, O_RDWR | O_BINARY, MYF(0))) < 0)
    DBUG_RETURN(-1);

  if (mysql_file_read(file_id, header, sizeof(header), MYF(MY_WME | MY_NABP)))
  {
    /* Write message into error log */
    sql_print_error(
        "DDL_LOG: Failed to read ddl log file '%s' during "
        "recovery",
        file_name);
    goto err;
  }
  ...
  if (memcmp(header, ddl_log_file_magic, 4))
  {
    ...
    sql_print_warning(
        "DDL_LOG: Wrong header in %s.  Assuming it is an old "
        "recovery file from MariaDB 10.5 or earlier. "
        "Skipping DDL recovery",
        file_name);
    goto err;
  }
  ...
  max_entry = (uint)(mysql_file_seek(file_id, 0L, MY_SEEK_END, MYF(0)) / io_size);
  if (max_entry)
    max_entry--;  // Don't count first block

  if (!(global_ddl_log.file_entry_buf =
            (uchar *)my_malloc(key_memory_DDL_LOG_MEMORY_ENTRY, io_size, MYF(MY_WME | MY_ZEROFILL))))
    goto err;
...
static void set_global_from_ddl_log_entry(const DDL_LOG_ENTRY *ddl_log_entry)
{
  ...
  bzero(file_entry_buf + DDL_LOG_END_POS, global_ddl_log.name_pos - DDL_LOG_END_POS);
...
static void set_ddl_log_entry_from_global(DDL_LOG_ENTRY *ddl_log_entry, const uint read_entry)
{
...
static bool create_ddl_log()
{
  ...
  */
  ...
  {
    uchar *ptr = (uchar *)my_realloc(key_memory_DDL_LOG_MEMORY_ENTRY, global_ddl_log.file_entry_buf, DDL_LOG_IO_SIZE,
  ...
  if ((global_ddl_log.file_id = mysql_file_create(key_file_global_ddl_log, file_name, CREATE_MODE,
                                                  O_RDWR | O_TRUNC | O_BINARY, MYF(MY_WME | ME_ERROR_LOG))) < 0)
  {
...
static bool ddl_log_increment_phase_no_lock(uint entry_pos)
{
  ...
  {
    ddl_log_entry_code code = ((ddl_log_entry_code)file_entry_buf[DDL_LOG_ENTRY_TYPE_POS]);
    ddl_log_action_code action = ((ddl_log_action_code)file_entry_buf[DDL_LOG_ACTION_TYPE_POS]);
...
class ddl_log_error_handler : public Internal_error_handler
{
 public:
  ...
  ddl_log_error_handler() : handled_errors(0), unhandled_errors(0), first_error(0), only_ignore_non_existing_errors(0)
  {
  }
    ...
    *cond_hdl = NULL;
    if (non_existing_table_error(sql_errno) ||
        (!only_ignore_non_existing_errors &&
         (sql_errno == EE_LINK || sql_errno == EE_DELETE || sql_errno == ER_TRG_NO_DEFINER)))
    {
      handled_errors++;
      return TRUE;
    }
  ...
  bool safely_trapped_errors() { return (handled_errors > 0 && unhandled_errors == 0); }
};
...
static bool build_filename_and_delete_tmp_file(char *path, size_t path_length, const LEX_CSTRING *db,
                                               const LEX_CSTRING *name, const char *ext, PSI_file_key psi_key)
{
  bool deleted;
  uint length = build_table_filename(path, path_length - 1, db->str, name->str, ext, 0);
  path[length] = '~';
  path[length + 1] = 0;
...
static LEX_CSTRING end_comment = {STRING_WITH_LEN(" /* generated by ddl recovery */")};
...
static void ddl_log_to_binary_log(THD *thd, String *query)
{
  ...
  (void)thd->binlog_query(THD::STMT_QUERY_TYPE, query->ptr(), query->length(), TRUE, FALSE, FALSE, 0);
...
static bool ddl_log_drop_to_binary_log(THD *thd, DDL_LOG_ENTRY *ddl_log_entry, String *query)
{
  DBUG_ENTER("ddl_log_drop_to_binary_log");
  if (mysql_bin_log.is_open())
  {
    if (!ddl_log_entry->next_entry ||
        query->length() + end_comment.length + NAME_LEN + 100 > thd->variables.max_allowed_packet)
    {
      if (recovery_state.drop_table.length() > recovery_state.drop_table_init_length)
      {
      ...
      if (recovery_state.drop_view.length() > recovery_state.drop_view_init_length)
      {
...
static handler *create_handler(THD *thd, MEM_ROOT *mem_root, LEX_CSTRING *name)
{
  handlerton *hton;
  handler *file;
  plugin_ref plugin = my_plugin_lock_by_name(thd, name, MYSQL_STORAGE_ENGINE_PLUGIN);
  if (!plugin)
  {
...
static void execute_rename_table(DDL_LOG_ENTRY *ddl_log_entry, handler *file, const LEX_CSTRING *from_db,
                                 const LEX_CSTRING *from_table, const LEX_CSTRING *to_db, const LEX_CSTRING *to_table,
                                 uint flags, char *from_path, char *to_path)
{
  ...
  {
    build_lower_case_table_filename(from_path, FN_REFLEN, from_db, from_table, flags & FN_FROM_IS_TMP);
    build_lower_case_table_filename(to_path, FN_REFLEN, to_db, to_table, flags & FN_TO_IS_TMP);
  }
  else
  {
    fr_length = build_table_filename(from_path, FN_REFLEN, from_db->str, from_table->str, "", flags & FN_TO_IS_TMP);
    to_length = build_table_filename(to_path, FN_REFLEN, to_db->str, to_table->str, "", flags & FN_TO_IS_TMP);
  }
  ...
  if (file->needs_lower_case_filenames())
  {
    ...
    */
    fr_length =
        build_table_filename(from_path, FN_REFLEN, from_db->str, from_table->str, reg_ext, flags & FN_FROM_IS_TMP);
    to_length = build_table_filename(to_path, FN_REFLEN, to_db->str, to_table->str, reg_ext, flags & FN_TO_IS_TMP);
  }
  else
  {
...
static void rename_triggers(THD *thd, DDL_LOG_ENTRY *ddl_log_entry, bool swap_tables)
{
  ...
  build_filename_and_delete_tmp_file(from_path, sizeof(from_path), &from_db, &from_table, TRG_EXT, key_file_trg);
  ...
  else if (!access(from_path, F_OK))
  {
    ...
    */
    ...
    MDL_REQUEST_INIT(&mdl_request, MDL_key::TABLE, from_db.str, from_converted_name.str, MDL_EXCLUSIVE, MDL_EXPLICIT);
    error = thd->mdl_context.acquire_lock(&mdl_request, 1);
    /* acquire_locks() should never fail during recovery */
    DBUG_ASSERT(error == 0);

    (void)Table_triggers_list::prepare_for_rename(thd, &trigger_param, &from_db, &from_table, &from_converted_name,
                                                  &to_db, &to_table);
    (void)Table_triggers_list::change_table_name(thd, &trigger_param, &from_db, &from_table, &from_converted_name,
                                                 &to_db, &to_table);
...
static void rename_in_stat_tables(THD *thd, DDL_LOG_ENTRY *ddl_log_entry, bool swap_tables)
{
  ...
  (void)rename_table_in_stat_tables(thd, &from_db, &from_converted_name, &to_db, &to_table);
}
...
static int ddl_log_execute_action(THD *thd, MEM_ROOT *mem_root, DDL_LOG_ENTRY *ddl_log_entry)
{
  ...
  mysql_mutex_assert_owner(&LOCK_gdl);
  DBUG_PRINT("ddl_log", ("pos: %u=>%u->%u  type: %u  action: %u (%s) phase: %u  "
                         "handler: '%s'  name: '%s'  from_name: '%s'  tmp_name: '%s'",
                         recovery_state.execute_entry_pos, ddl_log_entry->entry_pos, ddl_log_entry->next_entry,
                         (uint)ddl_log_entry->entry_type, (uint)ddl_log_entry->action_type,
                         ddl_log_action_name[ddl_log_entry->action_type], (uint)ddl_log_entry->phase,
                         ddl_log_entry->handler_name.str, ddl_log_entry->name.str, ddl_log_entry->from_name.str,
                         ddl_log_entry->tmp_name.str));

  if (ddl_log_entry->entry_type == DDL_LOG_IGNORE_ENTRY_CODE || ddl_log_entry->phase == DDL_LOG_FINAL_PHASE)
    DBUG_RETURN(FALSE);
  ...
  switch (ddl_log_entry->action_type)
  {
    ...
    case DDL_LOG_DELETE_ACTION:
    {
      if (ddl_log_entry->phase == 0)
      {
        if (frm_action)
        {
          ...
          if (unlikely((error = mysql_file_delete(key_file_frm, to_path, MYF(MY_WME | MY_IGNORE_ENOENT)))))
            break;
    ...
    */
    ...
    case DDL_LOG_RENAME_ACTION:
    {
      ...
      if (frm_action)
      {
        ...
        (void)file->ha_rename_table(ddl_log_entry->from_name.str, ddl_log_entry->name.str);
      if (increment_phase(entry_pos))
        break;
      break;
    }
      ...
      */
      switch (ddl_log_entry->phase)
      {
        case EXCH_PHASE_TEMP_TO_FROM:
          ...
          (void)file->ha_rename_table(ddl_log_entry->from_name.str, ddl_log_entry->tmp_name.str);
        ...
        case EXCH_PHASE_FROM_TO_NAME:
          ...
          (void)file->ha_rename_table(ddl_log_entry->name.str, ddl_log_entry->from_name.str);
          /* decrease the phase and sync */
          file_entry_buf[DDL_LOG_PHASE_POS]--;
          if (write_ddl_log_file_entry(entry_pos))
            break;
        ...
        case EXCH_PHASE_NAME_TO_TEMP:
          ...
          (void)file->ha_rename_table(ddl_log_entry->tmp_name.str, ddl_log_entry->name.str);
      ...
      */
      switch (ddl_log_entry->phase)
      {
        ...
        case DDL_RENAME_PHASE_TABLE:
          ...
          execute_rename_table(ddl_log_entry, file, &ddl_log_entry->db, &ddl_log_entry->name, &ddl_log_entry->from_db,
                               &ddl_log_entry->from_name, 0, from_path, to_path);
    ...
    case DDL_LOG_RENAME_VIEW_ACTION:
    {
      ...
      build_filename_and_delete_tmp_file(to_path, sizeof(to_path) - 1, &ddl_log_entry->db, &ddl_log_entry->name,
                                         reg_ext, key_file_fileparser);
      build_filename_and_delete_tmp_file(from_path, sizeof(from_path) - 1, &ddl_log_entry->from_db,
                                         &ddl_log_entry->from_name, reg_ext, key_file_fileparser);

      /* Rename view back if the original rename did succeed */
      if (!access(to_path, F_OK))
        (void)mysql_rename_view(thd, &ddl_log_entry->from_db, &from_table, &ddl_log_entry->db, &to_table);
      (void)update_phase(entry_pos, DDL_LOG_FINAL_PHASE);
    }
    break;
    /*
    ...
    */
    case DDL_LOG_DROP_INIT_ACTION:
    {
      ...
      strmake(recovery_state.current_db, ddl_log_entry->from_db.str, sizeof(recovery_state.current_db) - 1);
    ...
    case DDL_LOG_DROP_TABLE_ACTION:
    {
      ...
      switch (ddl_log_entry->phase)
      {
        ...
        case DDL_DROP_PHASE_TRIGGER:
          Table_triggers_list::drop_all_triggers(thd, &db, &table, MYF(MY_WME | MY_IGNORE_ENOENT));
          if (increment_phase(entry_pos))
            break;
          /* Fall through */
        case DDL_DROP_PHASE_BINLOG:
          ...
          if (ddl_log_drop_to_binary_log(thd, ddl_log_entry, &recovery_state.drop_table))
          {
            if (increment_phase(entry_pos))
              break;
          }
    ...
    case DDL_LOG_DROP_VIEW_ACTION:
    {
      ...
      if (ddl_log_entry->phase == 0)
      {
        ...
        if (ddl_log_drop_to_binary_log(thd, ddl_log_entry, &recovery_state.drop_view))
        {
          if (increment_phase(entry_pos))
            break;
        }
    ...
    case DDL_LOG_DROP_TRIGGER_ACTION:
    {
      ...
      if (!build_filename_and_delete_tmp_file(to_path, sizeof(to_path) - 1, &ddl_log_entry->db, &ddl_log_entry->name,
                                              TRG_EXT, key_file_fileparser))
      {
      ...
      if (frm_length != (off_t)ddl_log_entry->unique_id && mysql_bin_log.is_open())
      {
        /*
          File size changed and it was not binlogged (as this entry was
          executed)
        */
        (void)rm_trigname_file(to_path, &ddl_log_entry->db, &ddl_log_entry->from_name, MYF(0));
        ...
        else
        {
          /* Generate new query */
          recovery_state.drop_table.append(
              STRING_WITH_LEN("DROP TRIGGER IF "
                              "EXISTS "));
          append_identifier(thd, &recovery_state.drop_table, &ddl_log_entry->from_name);
        ...
        if (mysql_bin_log.is_open())
        {
          ...
          (void)thd->binlog_query(THD::STMT_QUERY_TYPE, recovery_state.drop_table.ptr(),
                                  recovery_state.drop_table.length(), TRUE, FALSE, FALSE, 0);
          thd->db = thd_db;
          mysql_mutex_lock(&LOCK_gdl);
        }
      }
    ...
    case DDL_LOG_DROP_DB_ACTION:
    {
      ...
      switch (ddl_log_entry->phase)
      {
        case DDL_DROP_DB_PHASE_INIT:
          drop_database_objects(thd, &path, &db, !my_strcasecmp(system_charset_info, MYSQL_SCHEMA_NAME.str, db.str));
    ...
    case DDL_LOG_CREATE_VIEW_ACTION:
    {
      ...
      path[path_length] = '~';
      mysql_file_delete(key_file_fileparser, path, MYF(MY_WME | MY_IGNORE_ENOENT));
      path[path_length] = 0;
    ...
    case DDL_LOG_DELETE_TMP_FILE_ACTION:
    {
      ...
      if (!ddl_log_entry->unique_id || !is_execute_entry_active((uint)ddl_log_entry->unique_id))
        mysql_file_delete(key_file_fileparser, path.str, MYF(MY_WME | MY_IGNORE_ENOENT));
      (void)update_phase(entry_pos, DDL_LOG_FINAL_PHASE);
      break;
    }
    case DDL_LOG_CREATE_TRIGGER_ACTION:
    {
      ...
      (void)build_filename_and_delete_tmp_file(to_path, sizeof(to_path) - 1, &db, &table, TRG_EXT, key_file_fileparser);
      (void)build_filename_and_delete_tmp_file(to_path, sizeof(to_path) - 1, &db, &trigger, TRN_EXT,
                                               key_file_fileparser);
      switch (ddl_log_entry->phase)
      {
        case DDL_CREATE_TRIGGER_PHASE_DELETE_COPY:
        {
          ...
          length = build_table_filename(to_path, sizeof(to_path) - 1, db.str, table.str, TRG_EXT, 0);
          to_path[length] = '-';
          to_path[length + 1] = 0;
          mysql_file_delete(key_file_fileparser, to_path, MYF(MY_WME | MY_IGNORE_ENOENT));

          length = build_table_filename(to_path, sizeof(to_path) - 1, db.str, trigger.str, TRN_EXT, 0);
          to_path[length] = '-';
          to_path[length + 1] = 0;
          mysql_file_delete(key_file_fileparser, to_path, MYF(MY_WME | MY_IGNORE_ENOENT));
        }
        ...
        case DDL_CREATE_TRIGGER_PHASE_NO_OLD_TRIGGER:
        {
          ...
          build_table_filename(to_path, sizeof(to_path) - 1, db.str, table.str, TRG_EXT, 0);
          mysql_file_delete(key_file_fileparser, to_path, MYF(MY_WME | MY_IGNORE_ENOENT));
          build_table_filename(to_path, sizeof(to_path) - 1, db.str, trigger.str, TRN_EXT, 0);
          mysql_file_delete(key_file_fileparser, to_path, MYF(MY_WME | MY_IGNORE_ENOENT));
          (void)update_phase(entry_pos, DDL_LOG_FINAL_PHASE);
          break;
        }
    ...
    case DDL_LOG_ALTER_TABLE_ACTION:
    {
      ...
      if (!(org_file = create_handler(thd, mem_root, &ddl_log_entry->from_handler_name)))
        goto end;
      ...
      switch (ddl_log_entry->phase)
      {
        case DDL_ALTER_TABLE_PHASE_RENAME_FAILED:
          ...
          */
          ...
          if (!is_renamed)
          {
            execute_rename_table(ddl_log_entry, file, &ddl_log_entry->from_db,
                                 &ddl_log_entry->extra_name,  // #sql-backup
                                 &ddl_log_entry->from_db, &ddl_log_entry->from_name, FN_FROM_IS_TMP, from_path,
                                 to_path);
          }
          (void)update_phase(entry_pos, DDL_LOG_FINAL_PHASE);
          break;
        ...
        case DDL_ALTER_TABLE_PHASE_INPLACE:
        {
          ...
          */
          fr_length = build_table_filename(from_path, sizeof(from_path) - 1, ddl_log_entry->db.str,
                                           ddl_log_entry->name.str, reg_ext, 0);
          to_length = build_table_filename(to_path, sizeof(to_path) - 1, ddl_log_entry->from_db.str,
                                           ddl_log_entry->from_name.str, reg_ext, 0);
            ...
            */
            ...
            if (!new_version_unusable &&
                (!partition_hton->check_version || new_version_ready ||
                 !partition_hton->check_version(partition_hton, to_path, &version, ddl_log_entry->unique_id)))
            {
              /* Table is up to date */
              ...
              */
              if (ddl_log_entry->phase != DDL_ALTER_TABLE_PHASE_INPLACE_COPIED)
                (void)update_phase(entry_pos, DDL_ALTER_TABLE_PHASE_INPLACE_COPIED);
            ...
            else
            {
              ...
              */
              ...
              mysql_file_delete(key_file_frm, from_path, MYF(MY_WME | MY_IGNORE_ENOENT));
              (void)update_phase(entry_pos, DDL_LOG_FINAL_PHASE);
              break;
            }
          }
          if (is_renamed && new_version_ready)
          {
            ...
            execute_rename_table(ddl_log_entry, file, &ddl_log_entry->db, &ddl_log_entry->name, &ddl_log_entry->db,
                                 &ddl_log_entry->extra_name, 0, from_path, to_path);
          }
        ...
        case DDL_ALTER_TABLE_PHASE_COPIED:
        {
          ...
          /*
            ...
            it with the new one.
          */
          build_table_filename(from_path, sizeof(from_path) - 1, ddl_log_entry->from_db.str,
                               ddl_log_entry->from_name.str, "", 0);
          build_table_filename(to_path, sizeof(to_path) - 1, ddl_log_entry->db.str, ddl_log_entry->name.str, "", 0);
        ...
        case DDL_ALTER_TABLE_PHASE_OLD_RENAMED:
        {
          ...
          */
          if (!is_renamed)
          {
            ...
            execute_rename_table(ddl_log_entry, file, &ddl_log_entry->db, &ddl_log_entry->name, &ddl_log_entry->from_db,
                                 &ddl_log_entry->from_name, FN_FROM_IS_TMP, from_path, to_path);
            ...
            */
            length = build_table_filename(from_path, sizeof(from_path) - 1, ddl_log_entry->from_db.str,
                                          ddl_log_entry->extra_name.str, "", FN_IS_TMP);
            if (likely(org_hton))
            {
              if (ddl_log_entry->flags & DDL_LOG_FLAG_ALTER_ENGINE_CHANGED)
              {
                /* Only frm is renamed, storage engine files have original name */
                build_table_filename(to_path, sizeof(from_path) - 1, ddl_log_entry->from_db.str,
                                     ddl_log_entry->from_name.str, "", 0);
            ...
            execute_rename_table(ddl_log_entry, file, &ddl_log_entry->db, &ddl_log_entry->name, &ddl_log_entry->db,
                                 &ddl_log_entry->extra_name, FN_FROM_IS_TMP, from_path, to_path);
          (void)update_phase(entry_pos, DDL_ALTER_TABLE_PHASE_UPDATE_TRIGGERS);
        }
        /* fall through */
        ...
        case DDL_ALTER_TABLE_PHASE_UPDATE_BINARY_LOG:
        {
          ...
          if (recovery_state.query.length() && mysql_bin_log.is_open())
          {
            ...
            mysql_mutex_unlock(&LOCK_gdl);
            save_db = thd->db;
            lex_string_set3(&thd->db, recovery_state.db.ptr(), recovery_state.db.length());
        ...
        */
        ...
        case DDL_ALTER_TABLE_PHASE_INIT:
        {
          ...
          */
          ...
          mysql_file_delete(key_file_partition_ddl_log, to_path, MYF(MY_WME | MY_IGNORE_ENOENT));
          (void)update_phase(entry_pos, DDL_LOG_FINAL_PHASE);
          break;
        }
      }
      ...
      if (ddl_log_entry->flags)
      {
        ...
        */
        if (recovery_state.query.alloc((size_t)(ddl_log_entry->unique_id + 1)))
          goto end;
        recovery_state.query.length(0);
        recovery_state.db.copy(ddl_log_entry->db.str, ddl_log_entry->db.length, system_charset_info);
      }
      if (unlikely(recovery_state.query.length() + ddl_log_entry->extra_name.length >
                   recovery_state.query.alloced_length()))
      {
        /* Impossible length. Ignore query */
        recovery_state.query.length(0);
        error = 1;
        my_error(ER_INTERNAL_ERROR, MYF(0), "DDL log: QUERY event has impossible length");
        break;
      }
...
static bool ddl_log_get_free_entry(DDL_LOG_MEMORY_ENTRY **active_entry)
{
  ...
  {
    if (!(used_entry = ((DDL_LOG_MEMORY_ENTRY *)my_malloc(key_memory_DDL_LOG_MEMORY_ENTRY, sizeof(DDL_LOG_MEMORY_ENTRY),
                                                          MYF(MY_WME)))))
    {
      sql_print_error(
          "DDL_LOG: Failed to allocate memory for ddl log free "
          "list");
      *active_entry = 0;
      DBUG_RETURN(TRUE);
    }
...
static bool ddl_log_execute_entry_no_lock(THD *thd, uint first_entry)
{
  ...
  mysql_mutex_assert_owner(&LOCK_gdl);
  init_sql_alloc(key_memory_gdl, &mem_root, TABLE_ALLOC_BLOCK_SIZE, 0, MYF(MY_THREAD_SPECIFIC));
  do
  {
    ...
    if (ddl_log_execute_action(thd, &mem_root, &ddl_log_entry))
    {
      ...
      sql_print_error(
          "DDL_LOG: Got error %d when trying to execute action "
          "for entry %u of type '%s'",
          (int)my_errno, read_entry, ddl_log_action_name[action_type]);
      break;
    }
...
bool ddl_log_write_entry(DDL_LOG_ENTRY *ddl_log_entry, DDL_LOG_MEMORY_ENTRY **active_entry)
{
  bool error;
  ...
  *active_entry = 0;
  ...
  error = FALSE;
  DBUG_PRINT("ddl_log", ("pos: %u->%u  action: %u (%s) phase: %u  "
                         "handler: '%s'  name: '%s'  from_name: '%s'  tmp_name: '%s'",
                         (*active_entry)->entry_pos, (uint)ddl_log_entry->next_entry, (uint)ddl_log_entry->action_type,
                         ddl_log_action_name[ddl_log_entry->action_type], (uint)ddl_log_entry->phase,
                         ddl_log_entry->handler_name.str, ddl_log_entry->name.str, ddl_log_entry->from_name.str,
                         ddl_log_entry->tmp_name.str));
  ...
  {
    sql_print_error("DDL_LOG: Failed to write entry %u", (*active_entry)->entry_pos);
...
bool ddl_log_write_execute_entry(uint first_entry, uint cond_entry, DDL_LOG_MEMORY_ENTRY **active_entry)
{
  uchar *file_entry_buf = global_ddl_log.file_entry_buf;
  ...
  */
  ...
  DBUG_PRINT("ddl_log", ("pos: %u=>%u", (*active_entry)->entry_pos, first_entry));
  if (write_ddl_log_file_entry((*active_entry)->entry_pos))
  {
    sql_print_error("DDL_LOG: Error writing execute entry %u", (*active_entry)->entry_pos);
    if (got_free_entry)
    {
      ddl_log_release_memory_entry(*active_entry);
      *active_entry = 0;
...
bool ddl_log_close_binlogged_events(HASH *xids)
{
  ...
  for (i = 1; i <= global_ddl_log.num_entries; i++)
  {
    ...
    if (ddl_log_entry.entry_type == DDL_LOG_EXECUTE_CODE && ddl_log_entry.xid != 0 &&
        my_hash_search(xids, (uchar *)&ddl_log_entry.xid, sizeof(ddl_log_entry.xid)))
    {
...
int ddl_log_execute_recovery()
{
  ...
  */
  ...
  for (i = 1; i <= global_ddl_log.num_entries; i++)
  {
    ...
    if (ddl_log_entry.entry_type == DDL_LOG_EXECUTE_CODE)
    {
      ...
      */
      ...
      {
        sql_print_error(
            "DDL_LOG: Aborting executing entry %u after %llu "
            "retries",
            i, ddl_log_entry.unique_id);
        error = -1;
        continue;
      }
      /* purecov: end tested */
  ...
  if (create_ddl_log())
    error = 1;
  if (count > 0)
    sql_print_information("DDL_LOG: Crash recovery executed %u entries", count);
...
void ddl_log_release_entries(DDL_LOG_STATE *ddl_log_state)
{
  ...
  for (DDL_LOG_MEMORY_ENTRY *log_entry = ddl_log_state->list; log_entry; log_entry = next)
  {
...
static bool ddl_log_write(DDL_LOG_STATE *ddl_state, DDL_LOG_ENTRY *ddl_log_entry)
{
  int error;
  ...
  error = ((ddl_log_write_entry(ddl_log_entry, &log_entry)) ||
           ddl_log_write_execute_entry(log_entry->entry_pos, 0, &ddl_state->execute_entry));
  mysql_mutex_unlock(&LOCK_gdl);
  if (error)
  {
    if (log_entry)
...
bool ddl_log_rename_table(DDL_LOG_STATE *ddl_state, handlerton *hton, const LEX_CSTRING *org_db,
                          const LEX_CSTRING *org_alias, const LEX_CSTRING *new_db, const LEX_CSTRING *new_alias)
{
  DDL_LOG_ENTRY ddl_log_entry;
  ...
  lex_string_set(&ddl_log_entry.handler_name, ha_resolve_storage_engine_name(hton));
...
bool ddl_log_rename_view(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *org_db, const LEX_CSTRING *org_alias,
                         const LEX_CSTRING *new_db, const LEX_CSTRING *new_alias)
{
  DDL_LOG_ENTRY ddl_log_entry;
  DBUG_ENTER("ddl_log_rename_file");
...
static bool ddl_log_drop_init(DDL_LOG_STATE *ddl_state, ddl_log_action_code action_code, const LEX_CSTRING *db,
...
bool ddl_log_drop_table_init(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *db, const LEX_CSTRING *comment)
{
  return ddl_log_drop_init(ddl_state, DDL_LOG_DROP_INIT_ACTION, db, comment);
}

bool ddl_log_drop_view_init(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *db)
{
  return ddl_log_drop_init(ddl_state, DDL_LOG_DROP_INIT_ACTION, db, &empty_clex_str);
}
...
static bool ddl_log_drop(DDL_LOG_STATE *ddl_state, ddl_log_action_code action_code, uint phase, handlerton *hton,
                         const LEX_CSTRING *path, const LEX_CSTRING *db, const LEX_CSTRING *table)
{
  DDL_LOG_ENTRY ddl_log_entry;
  DDL_LOG_MEMORY_ENTRY *log_entry;
  ...
  (void)ddl_log_sync_no_lock();
  if (update_next_entry_pos(ddl_state->list->entry_pos, log_entry->entry_pos))
  {
    ddl_log_release_memory_entry(log_entry);
    goto error;
  }
...
bool ddl_log_drop_table(DDL_LOG_STATE *ddl_state, handlerton *hton, const LEX_CSTRING *path, const LEX_CSTRING *db,
                        const LEX_CSTRING *table)
{
  DBUG_ENTER("ddl_log_drop_table");
  DBUG_RETURN(ddl_log_drop(ddl_state, DDL_LOG_DROP_TABLE_ACTION, DDL_DROP_PHASE_TABLE, hton, path, db, table));
}

bool ddl_log_drop_view(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *path, const LEX_CSTRING *db,
                       const LEX_CSTRING *table)
{
  DBUG_ENTER("ddl_log_drop_view");
  DBUG_RETURN(ddl_log_drop(ddl_state, DDL_LOG_DROP_VIEW_ACTION, 0, (handlerton *)0, path, db, table));
}

bool ddl_log_drop_trigger(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *db, const LEX_CSTRING *table,
                          const LEX_CSTRING *trigger_name, const LEX_CSTRING *query)
{
  DDL_LOG_ENTRY ddl_log_entry;
  MY_STAT stat_info;
...
bool ddl_log_drop_db(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *db, const LEX_CSTRING *path)
{
  DDL_LOG_ENTRY ddl_log_entry;
...
bool ddl_log_create_table(DDL_LOG_STATE *ddl_state, handlerton *hton, const LEX_CSTRING *path, const LEX_CSTRING *db,
                          const LEX_CSTRING *table, bool only_frm)
{
  DDL_LOG_ENTRY ddl_log_entry;
...
bool ddl_log_create_view(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *path, enum_ddl_log_create_view_phase phase)
{
  DDL_LOG_ENTRY ddl_log_entry;
...
bool ddl_log_delete_tmp_file(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *path, DDL_LOG_STATE *depending_state)
{
  DDL_LOG_ENTRY ddl_log_entry;
...
bool ddl_log_create_trigger(DDL_LOG_STATE *ddl_state, const LEX_CSTRING *db, const LEX_CSTRING *table,
                            const LEX_CSTRING *trigger_name, enum_ddl_log_create_trigger_phase phase)
{
  DDL_LOG_ENTRY ddl_log_entry;
  DBUG_ENTER("ddl_log_create_view");
...
bool ddl_log_alter_table(DDL_LOG_STATE *ddl_state, handlerton *org_hton, const LEX_CSTRING *db,
                         const LEX_CSTRING *table, handlerton *new_hton, handlerton *partition_underlying_hton,
                         const LEX_CSTRING *new_db, const LEX_CSTRING *new_table, const LEX_CSTRING *frm_path,
                         const LEX_CSTRING *backup_name, const LEX_CUSTRING *version, ulonglong table_version,
                         bool is_renamed)
{
  DDL_LOG_ENTRY ddl_log_entry;
  ...
  if (new_hton)
    lex_string_set(&ddl_log_entry.handler_name, ha_resolve_storage_engine_name(new_hton));
  /* Store temporary table name */
  ddl_log_entry.db = *const_cast<LEX_CSTRING *>(new_db);
  ddl_log_entry.name = *const_cast<LEX_CSTRING *>(new_table);
  if (org_hton)
    lex_string_set(&ddl_log_entry.from_handler_name, ha_resolve_storage_engine_name(org_hton));
  ddl_log_entry.from_db = *const_cast<LEX_CSTRING *>(db);
  ...
  */
  ...
  {
    lex_string_set(&ddl_log_entry.handler_name, ha_resolve_storage_engine_name(partition_underlying_hton));
...
bool ddl_log_store_query(THD *thd, DDL_LOG_STATE *ddl_state, const char *query, size_t length)
{
  DDL_LOG_ENTRY ddl_log_entry;
  ...
  if (ddl_log_write_execute_entry(first_entry->entry_pos, &ddl_state->execute_entry))
    goto err;
