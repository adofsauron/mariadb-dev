...
static ST_FIELD_INFO groups_fields_info[] = {Column("GROUP_ID", SLong(6), NOT_NULL),
...
static int groups_fill_table(THD *thd, TABLE_LIST *tables, COND *)
{
  ...
  for (uint i = 0; i < threadpool_max_size && all_groups[i].pollfd != INVALID_HANDLE_VALUE; i++)
  {
    ...
    /* QUEUE LENGTH */
    uint queue_len = group->queues[TP_PRIORITY_LOW].elements() + group->queues[TP_PRIORITY_HIGH].elements();
...
static ST_FIELD_INFO queues_field_info[] = {Column("GROUP_ID", SLong(6), NOT_NULL),
...
static int queues_fill_table(THD *thd, TABLE_LIST *tables, COND *)
{
  ...
  TABLE *table = tables->table;
  for (uint group_id = 0; group_id < threadpool_max_size && all_groups[group_id].pollfd != INVALID_HANDLE_VALUE;
       group_id++)
  {
...
static ST_FIELD_INFO stats_fields_info[] = {Column("GROUP_ID", SLong(6), NOT_NULL),
...
static ST_FIELD_INFO waits_fields_info[] = {Column("REASON", Varchar(16), NOT_NULL),
                                            Column("COUNT", SLonglong(19), NOT_NULL), CEnd()};

}  // namespace Show

/* See thd_wait_type enum for explanation*/
static const LEX_CSTRING wait_reasons[THD_WAIT_LAST] = {
    {STRING_WITH_LEN("UNKNOWN")},      {STRING_WITH_LEN("SLEEP")},       {STRING_WITH_LEN("DISKIO")},
    {STRING_WITH_LEN("ROW_LOCK")},     {STRING_WITH_LEN("GLOBAL_LOCK")}, {STRING_WITH_LEN("META_DATA_LOCK")},
    {STRING_WITH_LEN("TABLE_LOCK")},   {STRING_WITH_LEN("USER_LOCK")},   {STRING_WITH_LEN("BINLOG")},
    {STRING_WITH_LEN("GROUP_COMMIT")}, {STRING_WITH_LEN("SYNC")},        {STRING_WITH_LEN("NET")}};
...
{
  for (auto i = 0; i < THD_WAIT_LAST; i++) tp_waits[i] = 0;
...
static struct st_mysql_information_schema plugin_descriptor = {MYSQL_INFORMATION_SCHEMA_INTERFACE_VERSION};

maria_declare_plugin(thread_pool_info){MYSQL_INFORMATION_SCHEMA_PLUGIN,
                                       &plugin_descriptor,
                                       "THREAD_POOL_GROUPS",
                                       "Vladislav Vaintroub",
