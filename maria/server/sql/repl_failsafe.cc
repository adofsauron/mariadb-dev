...
const char *rpl_status_type[] = {"AUTH_MASTER",   "IDLE_SLAVE",       "ACTIVE_SLAVE", "LOST_SOLDIER",
...
int THD::register_slave(uchar *packet, size_t packet_length)
{
  ...
  if (check_access(this, PRIV_COM_REGISTER_SLAVE, any_db.str, NULL, NULL, 0, 0))
    return 1;
  if (!(si = (Slave_info *)my_malloc(key_memory_SLAVE_INFO, sizeof(Slave_info), MYF(MY_WME))))
    return 1;
...
bool show_slave_hosts(THD *thd)
{
  ...
  field_list.push_back(new (mem_root) Item_return_int(thd, "Server_id", 10, MYSQL_TYPE_LONG), thd->mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Host", 20), thd->mem_root);
  if (opt_show_slave_auth_info)
  {
    field_list.push_back(new (mem_root) Item_empty_string(thd, "User", 20), thd->mem_root);
    field_list.push_back(new (mem_root) Item_empty_string(thd, "Password", 20), thd->mem_root);
  }
  field_list.push_back(new (mem_root) Item_return_int(thd, "Port", 7, MYSQL_TYPE_LONG), thd->mem_root);
  field_list.push_back(new (mem_root) Item_return_int(thd, "Master_id", 10, MYSQL_TYPE_LONG), thd->mem_root);
