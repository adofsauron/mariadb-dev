...
const char *Sql_condition::get_message_text() const { return m_message_text.ptr(); }

int Sql_condition::get_message_octet_length() const { return m_message_text.length(); }
...
Diagnostics_area::Diagnostics_area(bool initialize) : is_bulk_execution(0), m_main_wi(0, false, initialize)
{
...
Diagnostics_area::Diagnostics_area(ulonglong warning_info_id, bool allow_unlimited_warnings, bool initialize)
    : is_bulk_execution(0), m_main_wi(warning_info_id, allow_unlimited_warnings, initialize)
{
  push_warning_info(&m_main_wi);
...
void Diagnostics_area::set_ok_status(ulonglong affected_rows, ulonglong last_insert_id, const char *message)
{
...
{
  set_error_status(sql_errno, ER(sql_errno), mysql_errno_to_sqlstate(sql_errno), Sql_user_condition_identity(), NULL);
}
...
void Diagnostics_area::set_error_status(uint sql_errno, const char *message, const char *sqlstate,
...
Warning_info::Warning_info(ulonglong warn_id_arg, bool allow_unlimited_warnings, bool initialize)
...
void Warning_info::init()
{
  ...
  init_sql_alloc(PSI_INSTRUMENT_ME, &m_warn_root, WARN_ALLOC_BLOCK_SIZE, WARN_ALLOC_PREALLOC_SIZE,
...
Warning_info::~Warning_info() { free_memory(); }
...
*/
void Diagnostics_area::copy_non_errors_from_wi(THD *thd, const Warning_info *src_wi)
{
...
void Warning_info::mark_sql_conditions_for_removal()
{
  ...
  while ((cond = it++)) mark_condition_for_removal(cond);
}
...
{
  List_iterator_fast<Sql_condition> it(const_cast<List<Sql_condition> &>(m_marked_sql_conditions));
  Sql_condition *c;
...
{
  while (m_warn_list.elements() && (m_warn_list.elements() + count) > thd->variables.max_error_count)
    m_warn_list.remove(m_warn_list.front());
}

Sql_condition *Warning_info::push_warning(THD *thd, const Sql_condition_identity *value, const char *msg,
                                          ulong current_row_number)
{
  ...
  if (!m_read_only)
  {
    if (m_allow_unlimited_warnings || m_warn_list.elements() < thd->variables.max_error_count)
    {
      cond = new (&m_warn_root) Sql_condition(&m_warn_root, *value, msg, current_row_number);
      if (cond)
...
Sql_condition *Warning_info::push_warning(THD *thd, const Sql_condition *sql_condition)
{
  Sql_condition *new_condition =
      push_warning(thd, sql_condition, sql_condition->get_message_text(), sql_condition->m_row_number);
...
void push_warning_printf(THD *thd, Sql_condition::enum_warning_level level, uint code, const char *format, ...)
{
  ...
  my_vsnprintf_ex(&my_charset_utf8mb3_general_ci, warning, sizeof(warning), format, args);
  va_end(args);
...
const LEX_CSTRING warning_level_names[] = {
    {STRING_WITH_LEN("Note")}, {STRING_WITH_LEN("Warning")}, {STRING_WITH_LEN("Error")}, {STRING_WITH_LEN("?")}};
...
bool mysqld_show_warnings(THD *thd, ulong levels_to_show)
{
  ...
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Level", 7), mem_root);
  field_list.push_back(new (mem_root) Item_return_int(thd, "Code", 4, MYSQL_TYPE_LONG), mem_root);
  field_list.push_back(new (mem_root) Item_empty_string(thd, "Message", MYSQL_ERRMSG_SIZE), mem_root);
  ...
  for (idx = 0; (err = it++); idx++)
  {
    ...
    protocol->store_warning(err->get_message_text(), err->get_message_octet_length());
    if (protocol->write())
      DBUG_RETURN(TRUE);
  }
  my_eof(thd);
...
*/
extern "C" int my_wc_mb_utf8_null_terminated(CHARSET_INFO *cs, my_wc_t wc, uchar *r, uchar *e)
{
  return wc == '\0' ? cs->wc_to_printable(wc, r, e) : my_charset_utf8mb3_handler.wc_mb(cs, wc, r, e);
}
...
size_t err_conv(char *buff, uint to_length, const char *from, uint from_length, CHARSET_INFO *from_cs)
{
  char *to = buff;
  ...
  {
    uint errors;
    res = my_convert_using_func(to, to_length, system_charset_info, my_wc_mb_utf8_null_terminated, from, from_length,
                                from_cs, from_cs->cset->mb_wc, &errors);
    to[res] = 0;
  }
  return res;
}
                             ...
                             CHARSET_INFO *from_cs, uint *errors)
{
  ...
  uint32 cnv_length = my_convert_using_func(to, to_length, to_cs, to_cs->cset->wc_to_printable, from, from_length,
                                            from_cs, from_cs->cset->mb_wc, errors);
