...
class Rpl_filter
{
 public:
  ...
  void set_parallel_mode(enum_slave_parallel_mode mode) { parallel_mode = mode; }
  /* Return given parallel mode or if one is not given, the default mode */
  enum_slave_parallel_mode get_parallel_mode() { return parallel_mode; }
 ...
 private:
  ...
  void table_rule_ent_dynamic_array_to_str(String *s, DYNAMIC_ARRAY *a, bool inited);
