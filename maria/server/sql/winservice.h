...
#pragma warning(pop)
  ...
  extern int get_mysql_service_properties(const wchar_t *bin_path, mysqld_service_properties *props);
...
#define AWSTRDUP(dest, src)         \
                                           ...
                                           LPCSTR lpPassword)
  {
    ...
    sch = CreateServiceW(hSCManager, w_ServiceName, w_DisplayName, dwDesiredAccess, dwServiceType, dwStartType,
                         dwErrorControl, w_BinaryPathName, w_LoadOrderGroup, lpdwTagId, w_Dependencies,
                         w_ServiceStartName, w_Password);
    if (!sch)
    {
      ok = FALSE;
      last_error = GetLastError();
  ...
  static inline BOOL my_ChangeServiceConfig(SC_HANDLE hService, DWORD dwServiceType, DWORD dwStartType,
                                            DWORD dwErrorControl, LPCSTR lpBinaryPathName, LPCSTR lpLoadOrderGroup,
                                            LPDWORD lpdwTagId, LPCSTR lpDependencies, LPCSTR lpServiceStartName,
                                            LPCSTR lpPassword, LPCSTR lpDisplayName)
  {
    ...
    ok = ChangeServiceConfigW(hService, dwServiceType, dwStartType, dwErrorControl, w_BinaryPathName, w_LoadOrderGroup,
                              lpdwTagId, w_Dependencies, w_ServiceStartName, w_Password, w_DisplayName);
    if (!ok)
    {
      last_error = GetLastError();
    }
