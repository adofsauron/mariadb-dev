...
class Window_frame_bound : public Sql_alloc
{
  ...
  */
  ...
  Window_frame_bound(Bound_precedence_type prec_type, Item *offset_val) : precedence_type(prec_type), offset(offset_val)
  {
  }
...
class Window_frame : public Sql_alloc
{
 public:
  ...
  Window_frame(Frame_units win_frame_units, Window_frame_bound *win_frame_top_bound,
               Window_frame_bound *win_frame_bottom_bound, Frame_exclusion win_frame_exclusion)
      : units(win_frame_units),
        top_bound(win_frame_top_bound),
        bottom_bound(win_frame_bottom_bound),
...
class Window_spec : public Sql_alloc
{
  ...
  */
  ...
  Window_spec(LEX_CSTRING *win_ref, SQL_I_List<ORDER> *part_list, SQL_I_List<ORDER> *ord_list, Window_frame *win_frame)
      : window_names_are_checked(false),
        window_ref(win_ref),
        partition_list(part_list),
        save_partition_list(NULL),
        order_list(ord_list),
        save_order_list(NULL),
        window_frame(win_frame),
        referenced_win_spec(NULL)
  {
  }
  ...
  const char *window_reference() { return window_ref ? window_ref->str : NULL; }

  void join_partition_and_order_lists() { *(partition_list->next) = order_list->first; }

  void disjoin_partition_and_order_lists() { *(partition_list->next) = NULL; }
...
class Window_def : public Window_spec
{
 public:
  ...
  Window_def(LEX_CSTRING *win_name, LEX_CSTRING *win_ref, SQL_I_List<ORDER> *part_list, SQL_I_List<ORDER> *ord_list,
             Window_frame *win_frame)
      : Window_spec(win_ref, part_list, ord_list, win_frame), window_name(win_name)
  {
  }
...
{
 public:
  bool setup(THD *thd, SQL_SELECT *sel, List_iterator<Item_window_func> &it, st_join_table *join_tab);
