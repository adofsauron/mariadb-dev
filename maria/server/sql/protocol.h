...
class Protocol
{
 protected:
  ...
  {
    return field_handlers == 0 || field_handlers[field_pos]->protocol_send_type() == type;
  }
#endif
  uint field_count;
  virtual bool net_store_data(const uchar *from, size_t length);
  virtual bool net_store_data_cs(const uchar *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs);
  virtual bool net_send_ok(THD *, uint, uint, ulonglong, ulonglong, const char *, bool);
  ...
  MEM_ROOT *alloc;
#endif
  bool needs_conversion(CHARSET_INFO *fromcs, CHARSET_INFO *tocs) const
  {
    // 'tocs' is set 0 when client issues SET character_set_results=NULL
    return tocs && !my_charset_same(fromcs, tocs) && fromcs != &my_charset_bin && tocs != &my_charset_bin;
  }
  /*
  ...
  */
  bool store_string_aux(const char *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs);
 ...
 public:
  ...
  inline bool store(const LEX_CSTRING *from, CHARSET_INFO *cs) { return store(from->str, from->length, cs); }
  ...
  virtual bool store_str(const char *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs) = 0;
  ...
  bool store_lex_cstring(const LEX_CSTRING &s, CHARSET_INFO *fromcs, CHARSET_INFO *tocs)
  {
  ...
  bool store_ident(const LEX_CSTRING &s) { return store_lex_cstring(s, system_charset_info, character_set_results()); }
  // End of wrappers
...
class Protocol_text : public Protocol
{
 ...
 public:
  Protocol_text(THD *thd_arg, ulong prealloc = 0) : Protocol(thd_arg)
  {
    if (prealloc)
  ...
  bool store_str(const char *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs) override;
  ...
  bool store_numeric_zerofill_str(const char *from, size_t length, protocol_send_type_t send_type);

#ifdef EMBEDDED_LIBRARY
  void remove_last_row() override;
#endif
  virtual bool store_field_metadata(const THD *thd, const Send_field &field, CHARSET_INFO *charset_for_protocol,
                                    uint pos);
  bool store_item_metadata(THD *thd, Item *item, uint pos);
  bool store_field_metadata_for_list_fields(const THD *thd, Field *field, const TABLE_LIST *table_list, uint pos);
...
class Protocol_binary final : public Protocol
{
 ...
 public:
  ...
  bool net_store_data_cs(const uchar *from, size_t length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs) override;
#endif
  bool store_null() override;
  bool store_tiny(longlong from) override;
  bool store_short(longlong from) override;
...
class Protocol_discard final : public Protocol
{
  ...
  */
  ...
  bool store_str(const char *, size_t, CHARSET_INFO *, CHARSET_INFO *) override { return false; }
